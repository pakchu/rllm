...
### 목표
최근 w(기본 96)개의 캔들 차트 이미지와 스칼라 특징(포지션 규모 %, 마지막 진입 가격, 윈도우 변동성)을 입력으로 받아 BTCUSDT에 대해 buy, hold, sell 액션을 결정하는 강화학습 에이전트를 개발한다. 롱/숏 모두 허용한다. 정책(policy)은 초기에는 CNN/CLIP 인코더 + SB3 PPO로 학습하고, 필요 시 VLM(+LoRA/TRL)로 확장한다.
...
- `position_size_pct`(총자산 대비 %), `last_entry_price`
...
- 이미지는 픽셀 텐서, 스칼라는 표준화된 연속값으로 변환합니다: `position_size_pct`, `last_entry_price`, `range_volatility_pct`.
