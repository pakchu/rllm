...
현재 runner의 계산/주문 구조는 다음과 같습니다.

- DB completed-bar 대기와 공통 source/feature snapshot은 cycle당 한 번만 수행합니다.
- 각 sleeve는 전용 spawn process에서 특수 feature와 score를 계산합니다.
- worker는 거래소 client나 JSON state를 소유하지 않고 order intent만 parent coordinator에 반환합니다.
- parent는 모든 entry intent를 DB에서 한 transaction으로 예약한 뒤 sleeve별 주문을 동시에 제출합니다.
- due exit도 sleeve별 coroutine으로 동시에 실행하며 state/ledger 반영은 parent가 결정적 순서로 수행합니다.
- 동일 state file의 local parent는 `.lock`으로, 다른 host의 parent는 Postgres advisory lease로 차단합니다.
- 동일 signal 중복 주문은 exchange/symbol까지 포함한 `trade_execution_reservations` PK로 차단합니다.
- 기존 주문 scan/cancel 실패와 alpha worker timeout은 신규 주문 없이 fail-closed 하며, timeout process는 강제 종료 후 교체합니다.

운영 옵션:

```bash
# 기본: sleeve별 process 병렬 score
python -m execution.portfolio_live ... --parallel-alpha-scoring

# 장애 분석용 직렬 fallback
python -m execution.portfolio_live ... --serial-alpha-scoring

# 느린 REX/LLM worker timeout 조정
python -m execution.portfolio_live ... --alpha-worker-timeout-sec 240
```
