"""Sequential strict economics for frozen HVSRAR-8."""
...
PREREG = Path("results/high_volatility_sign_run_asymmetry_relay_preregistration_2026-08-09.json")
PREREG_SHA = "75dd3a7e286421d274335b139806bff0191b4bad499edf13f4f1e7b0963ed939"
SUPPORT = Path("results/high_volatility_sign_run_asymmetry_relay_support_2026-08-09.json")
SUPPORT_SHA = "dbcdc106dca41be442fa385df4768a6bad58c1af002a21c57d4a5733e65fde9f"
NOVELTY = Path("results/high_volatility_sign_run_asymmetry_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "c60d651f416aa12f14d32dbc649f43c196787c95727928a4957a0b3abc425486"
CLOCK = Path("data/high_volatility_sign_run_asymmetry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "b3e4263f581e23f0c53de4684fd1b8ae5b956d15fce11972499b93cee1450188"
CONTROL_DIR = Path("data/high_volatility_sign_run_asymmetry_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_sign_run_asymmetry_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_sign_run_asymmetry_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_run_share_gate", "no_variation_gate", "net_block_return_side", "one_block_stale_features", "direction_flip", "same_clock_forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSRAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSRAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvsrar_8_sequential_economics_v1", "policy_id": POLICY_ID,
