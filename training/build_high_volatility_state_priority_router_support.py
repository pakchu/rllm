"""Materialize deterministic, outcome-blind source support for frozen HVSPR-8."""
...
CLOCK_DIR = Path("data/high_volatility_state_priority_router_clocks_2023_2026")
RESULT = Path("results/high_volatility_state_priority_router_support_2026-08-14.json")
HOLD = hvsof.HOLD
...
def _read_registration() -> Mapping[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVSPR-8 preregistration artifact drift")
    value = json.loads(prereg.DEFAULT_OUTPUT.read_text())
    if not isinstance(value, dict):
        raise TypeError("HVSPR-8 expected preregistration JSON object")
    prereg.validate(value)
    return value


def verify_frozen_inputs() -> dict[str, Any]:
    """Verify HVSPR-8 and the exact HVSOF component contracts it reuses."""
    _read_registration()
    verified = hvsof.verify_frozen_inputs()
    return {
        component: verified[component]
        for component in prereg.ACTION_ORDER + prereg.ELIGIBILITY_ORDER
    }


def load_action_clock(action: str) -> pd.DataFrame:
    """Load an action through the verified HVSOF primary-clock contract."""
    if action not in prereg.ACTION_ORDER:
        raise ValueError(f"unknown HVSPR-8 action: {action}")
    return hvsof.load_action_clock(action)


def _validate_exact_eight_hour_grid(frame: pd.DataFrame, eligibility: str) -> pd.DataFrame:
    expected = list(FILTER_COLUMNS[eligibility])
    if frame.columns.tolist() != expected:
        raise RuntimeError(f"HVSPR-8 {eligibility} state schema drift")
    if frame.empty or frame["decision_time"].duplicated().any():
        raise RuntimeError(f"HVSPR-8 {eligibility} invalid state decision grid")
    ...
    if not decision.dt.minute.eq(0).all() or not decision.dt.second.eq(0).all():
        raise RuntimeError(f"HVSPR-8 {eligibility} decision is off exact 8h grid")
    if not decision.dt.hour.isin((0, 8, 16)).all():
        raise RuntimeError(f"HVSPR-8 {eligibility} decision is off exact 8h grid")
    if len(ordered) > 1 and not decision.diff().iloc[1:].eq(HOLD).all():
        raise RuntimeError(f"HVSPR-8 {eligibility} state grid is not contiguous exact 8h")
    return ordered
...
def load_eligibility_state(eligibility: str) -> pd.DataFrame:
    ...
    if eligibility not in prereg.ELIGIBILITY_ORDER:
        raise ValueError(f"unknown HVSPR-8 eligibility: {eligibility}")
...
def _validate_action_grid(action_clock: pd.DataFrame, action: str) -> pd.DataFrame:
    ...
    if not decision.dt.minute.eq(0).all() or not decision.dt.second.eq(0).all():
        raise RuntimeError(f"HVSPR-8 {action} action is off exact 8h grid")
    if not decision.dt.hour.isin((0, 8, 16)).all():
        raise RuntimeError(f"HVSPR-8 {action} action is off exact 8h grid")
    return actions
...
def route_priority_clock(
    ...
    if priority not in prereg.PRIORITY_ORDERS:
        raise ValueError("HVSPR-8 priority order must be one of the frozen orders")
    candidate = prereg.candidate_id(priority, eligibility)
    if candidate not in prereg.CANDIDATE_FAMILY:
        raise ValueError("HVSPR-8 candidate must use the exact frozen family")
    if set(action_clocks) != set(prereg.ACTION_ORDER):
        raise RuntimeError("HVSPR-8 requires all and only frozen action clocks")
    ...
    for decision, split in state.itertuples(index=False, name=None):
        ...
        if source["split"] != split:
            raise RuntimeError(f"HVSPR-8 {candidate} chosen action split drift")
    ...
    if not output.empty:
        ...
        if output["decision_time"].duplicated().any():
            raise RuntimeError(f"HVSPR-8 {candidate} selected more than one action")
        if not output["feature_available_time"].le(output["entry_time"]).all():
            raise RuntimeError(f"HVSPR-8 {candidate} selected unavailable action")
        if not output["exit_time"].eq(output["entry_time"] + HOLD).all():
            raise RuntimeError(f"HVSPR-8 {candidate} selected action hold drift")
        if len(output) > 1 and not output["entry_time"].iloc[1:].reset_index(drop=True).ge(
            output["exit_time"].iloc[:-1].reset_index(drop=True)
        ).all():
            raise RuntimeError(f"HVSPR-8 {candidate} exact 8h routing overlap")
    routing = {
    ...
    if routing["routed_action_decisions"] + routing["cash_decisions"] != routing[
        "eligible_state_decisions"
    ]:
        raise RuntimeError(f"HVSPR-8 {candidate} routing accounting drift")
...
def run(clock_dir: Path = CLOCK_DIR, result_path: Path = RESULT) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvspr_8_source_support_v1",
