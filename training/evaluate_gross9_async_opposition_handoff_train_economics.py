"""Train-only economics for Gross9 async opposition handoff candidates.

Only source-supported, same-side-disjoint, and Gross9-novel candidates from the
frozen 36-pair G9ASYNCHANDOFF-8 family are opened for train economic outcomes.
The raw rank-one candidate is selected by the preregistered train
CAGR/strict-MDD ranking; if that rank-one candidate fails any train economic
gate, the family terminates without substitution.
"""
...
from training import evaluate_gross9_async_opposition_handoff_train_novelty as novelty
...
PROTOCOL_VERSION = "gross9_async_opposition_handoff_train_economics_v1"
NOVELTY = novelty.OUTPUT
OUTPUT = Path("results/gross9_async_opposition_handoff_train_economics_2026-09-02.json")
...
NOVELTY_SHA256 = "a8ef6dbd84faacb846e586c819777ab736515bd8ff9d629229fa6fd31f7edd41"
NOVELTY_MANIFEST_HASH = "d6afe7a162a378386390558dac8b3006acd414ae746b782e227bf96e58c29917"
...
def load_novelty_authorization() -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError(f"{POLICY_ID} train novelty boundary already opened economics")
    return report


def load_pair_clock(clock_record: Mapping[str, Any], candidate: str) -> pd.DataFrame:
...
def evaluate_candidate(candidate: str, clock_record: Mapping[str, Any], market: pd.DataFrame, funding: pd.DataFrame) -> dict[str, Any]:
    clock = load_pair_clock(clock_record, candidate)
    ...
    checks = {
        ...
        "cluster_signflip_p_max_bonferroni_0_1_over_36": cluster["pvalue"] <= prereg.BONFERRONI_RAW_P_MAX,
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    pairs: dict[str, Any] = {}
    economics_evaluated_pairs: list[str] = []
    for candidate in prereg.CANDIDATE_FAMILY:
        novelty_row = report["pairs"][candidate]
        source_pass = novelty_row.get("source_pass") is True
        same_side_disjointness_pass = novelty_row.get("same_side_pre_reservation_intersection_pass") is True
        gross9_pass = novelty_row.get("gross9_pass") is True
        row: dict[str, Any] = {
            "candidate": candidate,
            "source_pass": source_pass,
            "same_side_disjointness_pass": same_side_disjointness_pass,
        ...
        if source_pass and same_side_disjointness_pass and gross9_pass:
            economics_evaluated_pairs.append(candidate)
        ...
        rows_for_selection.append(
            {
                ...
                "same_side_disjointness_pass": row["same_side_disjointness_pass"],
    ...
    try:
        ...
        eligible = [row for row in rows_for_selection if row["source_pass"] and row["same_side_disjointness_pass"] and row["gross9_pass"]]
        eligible.sort(
            key=lambda row: (
    ...
    core = {
        ...
        "economics_evaluated_pairs": economics_evaluated_pairs,
        "pairs": pairs,
        "selection": {
        ...
        "physical_rows_opened": {
            "market": len(market),
            "funding": len(funding),
            "candidate_pair_clock": sum(pairs[c].get("candidate_clock_rows_opened", 0) for c in pairs),
        },
        ...
        json.dumps(
            {
                ...
                "economics_evaluated_pairs": result["economics_evaluated_pairs"],
