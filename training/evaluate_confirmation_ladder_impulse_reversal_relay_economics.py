"""Sequential strict economics for frozen CLIRR-6."""
...
PREREG = Path("results/confirmation_ladder_impulse_reversal_relay_preregistration_2026-08-09.json")
PREREG_SHA = "fbc084b980cf292d69756f736c3b391ee8a6ff5d2eb8eb5f6b369d69cf8c1ab3"
SUPPORT = Path("results/confirmation_ladder_impulse_reversal_relay_support_2026-08-09.json")
SUPPORT_SHA = "32c888ded6d2aeb7b1ed561e68c013556dd6e9de8d9d7191fca3f2072dec9129"
NOVELTY = Path("results/confirmation_ladder_impulse_reversal_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "37fa86e0444b42e335a3b8d8dccb9b1e65c304f0901373c498a4ff273d08e764"
CLOCK = Path("data/confirmation_ladder_impulse_reversal_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "c57ece4a565ff2ab25d404de33d17638ccc3d80ce40744f2389f23a25e9983be"
CONTROL_DIR = Path("data/confirmation_ladder_impulse_reversal_relay_controls_2023_2026")
FREEZE = Path("results/confirmation_ladder_impulse_reversal_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/confirmation_ladder_impulse_reversal_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CLIRR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CLIRR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "clirr_6_sequential_economics_v1", "policy_id": POLICY_ID,
