"""Build outcome-blind source support for frozen HVMD-24."""
...
PREREG_SHA = "3b03bc28a623ce2ece674557c92f681466f727d8d4a44473bc2e86f2fb29fb85"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_mcginley_dynamic_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_mcginley_dynamic_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_mcginley_dynamic_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_mcginley_dynamic_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_mcginley_dynamic_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "dynamic", "difference", "simple_average",
    "simple_difference", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "dynamic", "difference", "simple_average",
    "simple_difference", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVMD source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVMD invalid source key")
...
def mcginley(close: pd.Series, valid: pd.Series) -> pd.Series:
    output=np.full(len(close),np.nan);state=np.nan;period=P["dynamic_periods"];constant=P["adjustment_constant"];power=P["ratio_power"]
    for index,(value,is_valid) in enumerate(zip(close.to_numpy(float),valid.to_numpy(bool))):
        if not is_valid or not math.isfinite(value) or value<=0:state=np.nan;continue
        if not math.isfinite(state):state=value
        else:
            denominator=constant*period*(value/state)**power
            if not math.isfinite(denominator) or denominator<=0:state=np.nan;continue
            state=state+(value-state)/denominator
            if not math.isfinite(state) or state<=0:state=np.nan;continue
        output[index]=state
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["dynamic"]=mcginley(bars.bar_close,bars.valid_bar);bars["difference"]=bars.bar_close-bars.dynamic;period=P["dynamic_periods"];complete=bars.valid_bar.rolling(period,min_periods=period).sum().eq(period);bars["simple_average"]=bars.bar_close.rolling(period,min_periods=period).mean().where(complete);bars["simple_difference"]=bars.bar_close-bars.simple_average
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["dynamic","difference","simple_average","simple_difference","variation"]]).all(axis=1)&bars.dynamic.gt(0)&bars.difference.ne(0)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="simple_average_crossover":signal=pd.to_numeric(used.simple_difference,errors="coerce");previous=signal.shift(1);state=valid&signal.ne(0)&previous.ne(0)&np.sign(signal).ne(np.sign(previous))&variation;side=np.sign(signal).fillna(0).astype(int)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVMD prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvmd_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvmd_24_source_support_v1", "policy_id": prereg.POLICY_ID,
