"""Sequential strict economics for frozen CTPTR-6."""
...
PREREG = Path("results/cboe_tail_pressure_crypto_volatility_transmission_relay_preregistration_2026-08-08.json")
PREREG_SHA = "437d234bfac94b54373f7fd5ffdbb1d491b90fea558734e5fa6da80c6ea07b90"
SUPPORT = Path("results/cboe_tail_pressure_crypto_volatility_transmission_relay_support_2026-08-08.json")
SUPPORT_SHA = "c2901d455962da89eca01449b91b0eff46957059a505afd0ab4ce978f98d0587"
NOVELTY = Path("results/cboe_tail_pressure_crypto_volatility_transmission_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "1df41ca2dd2261cc300c44ada94321f085c9006dd9c1b312dfabf3d199e76948"
CLOCK = Path("data/cboe_tail_pressure_crypto_volatility_transmission_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "0e650ee1d9073de66c67fad83a9359c30de8bcd69be843416b194ccef9784c02"
CONTROL_DIR = Path("data/cboe_tail_pressure_crypto_volatility_transmission_relay_controls_2023_2026")
FREEZE = Path("results/cboe_tail_pressure_crypto_volatility_transmission_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/cboe_tail_pressure_crypto_volatility_transmission_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
CONTROLS = ("no_crypto_confirmation", "bvol_only_confirmation", "dvol_only_confirmation", "one_session_stale_tail_pressure_change", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CTPTR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CTPTR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "ctptr_6_sequential_economics_v1", "policy_id": POLICY_ID,
