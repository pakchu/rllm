"""Evaluate frozen HVVRAT-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVVRAT-24"
PROTOCOL = "hvvrat_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_volume_ratio_centerline_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVVRAT predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVVRAT novelty limits drift")
    if (
        ...
        raise RuntimeError("HVVRAT predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVVRAT primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvvrat_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVVRAT-24 unchanged before economic outcomes",
    }
