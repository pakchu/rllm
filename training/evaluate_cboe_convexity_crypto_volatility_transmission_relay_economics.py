"""Sequential strict economics for frozen CCXTR-6."""
...
PREREG = Path("results/cboe_convexity_crypto_volatility_transmission_relay_preregistration_2026-08-08.json")
PREREG_SHA = "cfd1f2f963341f2517eaf943561382cc5afb89f4a9bbccee8dcd292d660e3a60"
SUPPORT = Path("results/cboe_convexity_crypto_volatility_transmission_relay_support_2026-08-08.json")
SUPPORT_SHA = "748fe09db027c50d9b06809dbd9a5c686cef81b1e4476e1a5c0ec37c6a500135"
NOVELTY = Path("results/cboe_convexity_crypto_volatility_transmission_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "a9a3aa268c074342b8447a6df4d0576e3ad1005ad529c14c56fd132c7901f0b4"
CLOCK = Path("data/cboe_convexity_crypto_volatility_transmission_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d9e53b1c7780caefe3a5143f7cafa7374e8a7371a828d23d1b4af837403214d7"
CONTROL_DIR = Path("data/cboe_convexity_crypto_volatility_transmission_relay_controls_2023_2026")
FREEZE = Path("results/cboe_convexity_crypto_volatility_transmission_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/cboe_convexity_crypto_volatility_transmission_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
CONTROLS = ("no_crypto_confirmation", "bvol_only_confirmation", "dvol_only_confirmation", "one_session_stale_convexity_change", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CCXTR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CCXTR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "ccxtr_6_sequential_economics_v1", "policy_id": POLICY_ID,
