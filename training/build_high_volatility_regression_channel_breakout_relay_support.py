"""Build outcome-blind source support for frozen HVRC-B20-2-24."""
...
from training import preregister_high_volatility_regression_channel_breakout_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "857b8d133e08bd27f07e39a90b29192603811e05ecb164756182750c50fe02b6"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_regression_channel_breakout_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_regression_channel_breakout_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_regression_channel_breakout_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_regression_channel_breakout_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_regression_channel_breakout_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "intercept", "slope", "regression", "price_std",
    "upper_channel", "lower_channel", "entry_side", "center_side", "variation",
    "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "intercept", "slope", "regression",
    "price_std", "upper_channel", "lower_channel", "entry_side", "center_side",
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVRC source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVRC invalid source key")
...
def regression_channel(close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    """Replicate LEAN RegressionChannel(20,2) OLS endpoint and population dispersion."""
    names=("intercept","slope","regression","price_std","upper_channel","lower_channel")
    values={name:np.full(len(close),np.nan) for name in names}
    entry_side=np.zeros(len(close),dtype=int);center_side=np.zeros(len(close),dtype=int)
    history:list[float]=[];prior_outer=prior_center=0
    period=P["period"];x=np.arange(1.,period+1);x_mean=float(x.mean());den=float(np.square(x-x_mean).sum())
    for index,is_valid in enumerate(valid.to_numpy(bool)):
        if not is_valid:
            history=[];prior_outer=prior_center=0;continue
        history.append(float(close.iloc[index]));history=history[-period:]
        if len(history)<period:continue
        y=np.asarray(history,float);slope=float(((x-x_mean)*(y-y.mean())).sum()/den);intercept=float(y.mean()-slope*x_mean)
        regression=intercept+slope*period;price_std=float(np.sqrt(np.square(y-y.mean()).mean()))
        upper=regression+P["channel_width"]*price_std;lower=regression-P["channel_width"]*price_std
        for name,value in (("intercept",intercept),("slope",slope),("regression",regression),("price_std",price_std),("upper_channel",upper),("lower_channel",lower)):values[name][index]=value
        current=y[-1];outer=1 if current>upper else -1 if current<lower else 0
        entry_side[index]=outer if outer and outer!=prior_outer else 0;prior_outer=outer
        center=1 if current>regression else -1 if current<regression else 0
        center_side[index]=center if center and center!=prior_center else 0;prior_center=center
    return pd.DataFrame({**values,"entry_side":entry_side,"center_side":center_side},index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=bars.join(regression_channel(bars.bar_close,bars.valid_bar));bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["intercept","slope","regression","price_std","upper_channel","lower_channel","variation"]]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="regression_center_crossover":side=pd.to_numeric(used.center_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVRC prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvrc_b20_2_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvrc_b20_2_24_source_support_v1", "policy_id": prereg.POLICY_ID,
