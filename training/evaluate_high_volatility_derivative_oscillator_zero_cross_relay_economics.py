"""Sequential strict economics for frozen HVDO-Z14-5-3-9-24."""
...
PREREG = Path("results/high_volatility_derivative_oscillator_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "4a8707f8c711c54bbe77506ee95890defae70cd91977c5126aebc124f09247cd"
SUPPORT = Path("results/high_volatility_derivative_oscillator_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "5a8cd75be82e435864da6ff425eca2bc5c6e1e89c117c6957cb54b1d6a915821"
NOVELTY = Path("results/high_volatility_derivative_oscillator_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "56c1fa3c753d89128cf60a2a9fc1ef53c3e7273bf4ac5f7cae7824bda2f0da51"
CLOCK = Path("data/high_volatility_derivative_oscillator_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "baecc1150b9a9acd4d2ae622698323095d3537f23121c63c094150e092666ed9"
CONTROL_DIR = Path("data/high_volatility_derivative_oscillator_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_derivative_oscillator_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_derivative_oscillator_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "raw_rsi_midline", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDO frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDO novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdo_z14_5_3_9_24_sequential_economics_v1", "policy_id": POLICY_ID,
