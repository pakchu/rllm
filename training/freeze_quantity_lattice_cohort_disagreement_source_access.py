"""Hash-seal the built QLCD source without parsing any feature row."""
...
def build_seal(cfg: SealConfig, *, expected_rows: int = EXPECTED_ROWS) -> dict[str, Any]:
    ...
    if manifest.get("combined_output") != cfg.source:
        raise ValueError("QLCD source manifest path mismatch")
    if manifest.get("combined_sha256") != source_hash:
        raise ValueError("QLCD source manifest hash mismatch")
    if int(manifest.get("rows", -1)) != expected_rows:
        raise ValueError("QLCD source manifest row count mismatch")
    if protocol.get("source_archive_manifest_sha256") != prereg.ARCHIVE_MANIFEST_SHA256:
        raise ValueError("QLCD archive source contract mismatch")
    if protocol.get("source_audit_sha256") != prereg.SOURCE_AUDIT_SHA256:
        raise ValueError("QLCD source audit contract mismatch")
    return {
        "protocol_version": "quantity_lattice_cohort_disagreement_source_access_seal_v1",
...
def write_once(path: str | Path, payload: dict[str, Any]) -> str:
    ...
    if output.exists():
        if output.read_bytes() != content:
            raise RuntimeError("refusing to overwrite frozen QLCD source access seal")
