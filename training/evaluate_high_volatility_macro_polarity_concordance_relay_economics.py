"""Sequential strict economics for frozen HVMPC-24."""
...
PREREG = Path("results/high_volatility_macro_polarity_concordance_relay_preregistration_2026-08-09.json")
PREREG_SHA = "65ec613d9a7ed21b6dba46d57351e36824fd00a96fd17f71e299ead1c192091d"
SUPPORT = Path("results/high_volatility_macro_polarity_concordance_relay_support_2026-08-09.json")
SUPPORT_SHA = "375d9edd771959f75af43d3213d476c044f5ee5ac163f8a9cadf77804600d6fb"
NOVELTY = Path("results/high_volatility_macro_polarity_concordance_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "7228e7b8b823a2b20c9c9122c106072c0c097d2cccdd79ecf59cabb2c5c2699c"
CLOCK = Path("data/high_volatility_macro_polarity_concordance_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "e09036dedafa36b826f7d63ba3e940cbdab5763b7d046ed443b9acc34a6cd717"
CONTROL_DIR = Path("data/high_volatility_macro_polarity_concordance_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_macro_polarity_concordance_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_macro_polarity_concordance_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("news_only", "epu_only", "no_variation_gate", "one_week_stale_pair", "direction_flip", "same_clock_forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVMPC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVMPC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvmpc_24_sequential_economics_v1", "policy_id": POLICY_ID,
