"""Evaluate frozen HVCAESR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCAESR-8"
PROTOCOL = "hvcaesr_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_cross_alt_execution_swarm_exhaustion_reversal_preregistration_2026-08-13.json")
PREREG_SHA = "ad2eb74800fd3d8b03b0fc391787dc7efcac5c9c9d60d794cc14deb1636e79f4"
SUPPORT = Path("results/high_volatility_cross_alt_execution_swarm_exhaustion_reversal_support_2026-08-13.json")
SUPPORT_SHA = "75fbb10626b22eb30dd756878c7470a64ab2bac7673b7559f873e483cd18796e"
CLOCK = Path("data/high_volatility_cross_alt_execution_swarm_exhaustion_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "4fc41fc4581f126927c3bc68d4f2b0c3923a8cba00a1b71a6de9d2c1dd0a24a1"
OUTPUT = Path("results/high_volatility_cross_alt_execution_swarm_exhaustion_reversal_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCAESR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCAESR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCAESR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCAESR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCAESR-8 unchanged before economic outcomes",
    }
