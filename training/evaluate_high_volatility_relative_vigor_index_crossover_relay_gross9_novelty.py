"""Evaluate frozen HVRVI-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVRVI-24"
PROTOCOL = "hvrvi_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_relative_vigor_index_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVRVI predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVRVI novelty limits drift")
    if (
        ...
        raise RuntimeError("HVRVI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVRVI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVRVI-24 unchanged before economic outcomes",
    }
