"""Evaluate frozen HVSTC-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVSTC-24"
PROTOCOL = "hvstc_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_schaff_trend_cycle_reentry_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVSTC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVSTC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVSTC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVSTC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvstc_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVSTC-24 unchanged before economic outcomes",
    }
