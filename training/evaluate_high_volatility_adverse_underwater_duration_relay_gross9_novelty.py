"""Evaluate frozen HVAUD-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVAUD-8"
PROTOCOL = "hvaud_8_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_adverse_underwater_duration_relay_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVAUD predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVAUD novelty limits drift")
    if (
        ...
        raise RuntimeError("HVAUD predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVAUD primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvaud_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVAUD-8 unchanged before economic outcomes"
        ),
    }
