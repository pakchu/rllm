"""Evaluate the frozen HVCVAIR-8 action-vote clock against Gross9."""
...
POLICY = "HVCVAIR-8"
PROTOCOL = "hvcvair_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_volume_acceleration_inventory_relay_clocks_2020_2026.csv.gz",
    "sha256": "8e099848add857b43b178f71ffbc90062303085125c1551f80019510a06a3c14",
    "rows": 187,
}
OUTPUT = Path("results/high_volatility_causal_volume_acceleration_inventory_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCVAIR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCVAIR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVAIR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCVAIR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCVAIR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCVAIR-8 unchanged before economics",
    }
