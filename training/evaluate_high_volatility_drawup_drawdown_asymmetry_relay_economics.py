"""Sequential strict economics for frozen HVDUDAR-8."""
...
PREREG = Path("results/high_volatility_drawup_drawdown_asymmetry_relay_preregistration_2026-08-10.json")
PREREG_SHA = "bdb0a9c000fb8091b7f9a3fc99b8f186b7781a7573dc613715dcf1316d9cc017"
SUPPORT = Path("results/high_volatility_drawup_drawdown_asymmetry_relay_support_2026-08-10.json")
SUPPORT_SHA = "cd0d1e71b23baa8623edbcd4021b75855a74043230d3c04055f730d46260866b"
NOVELTY = Path("results/high_volatility_drawup_drawdown_asymmetry_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "a219b944d69565a29d550778bb1c2149a3ac072a1c3119289ca20c2fb91bf8f0"
CLOCK = Path("data/high_volatility_drawup_drawdown_asymmetry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "a46c75e13ae3a198ba93ffb0eddd62d640572a0523f464f05f1422800a02e725"
CONTROL_DIR = Path("data/high_volatility_drawup_drawdown_asymmetry_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_drawup_drawdown_asymmetry_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_drawup_drawdown_asymmetry_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_asymmetry_tail", "no_variation_gate", "endpoint_displacement", "one_block_stale_excursions", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDUDAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDUDAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdudar_8_sequential_economics_v1", "policy_id": POLICY_ID,
