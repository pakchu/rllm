"""Sequential strict economics for frozen HVRVOL-24."""
...
PREREG = Path("results/high_volatility_relative_volatility_index_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "bebfb8fcd44d9818597e1f9faf25e50e3a274495264cf8e02c1a3e2062de7d87"
SUPPORT = Path("results/high_volatility_relative_volatility_index_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "df1fe911eff250352a3df02ea82c9f9b8e829281a682e1358eb968818605bd04"
NOVELTY = Path("results/high_volatility_relative_volatility_index_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "2268e631e8cf6605199cd7e90aaffbd50b2007bf412519a93a2d9375c91c0af8"
CLOCK = Path("data/high_volatility_relative_volatility_index_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "c7cd070eb6fb9f50744d7e95057fd21cf4c88136de7b28f19e90a6b1bfe089f3"
CONTROL_DIR = Path("data/high_volatility_relative_volatility_index_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_relative_volatility_index_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_relative_volatility_index_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVRVOL frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVRVOL novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrvol_24_sequential_economics_v1", "policy_id": POLICY_ID,
