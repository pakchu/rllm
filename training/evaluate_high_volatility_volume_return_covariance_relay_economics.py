"""Sequential strict economics for frozen HVVRCR-12."""
...
PREREG = Path("results/high_volatility_volume_return_covariance_relay_preregistration_2026-08-09.json")
PREREG_SHA = "f636bed76ff62a636d3a6ec6b14dabd1017847aeab300eab351c05b69397d8bb"
SUPPORT = Path("results/high_volatility_volume_return_covariance_relay_support_2026-08-09.json")
SUPPORT_SHA = "94deb8a2c0c2102ff42823bd91df23ee1857eedff79de283c1acd2603bf7cba4"
NOVELTY = Path("results/high_volatility_volume_return_covariance_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "56877d0a1f5d42f1a3e63d7fa658d2559ab184d8bcfac370348b6dbcb6b85591"
CLOCK = Path("data/high_volatility_volume_return_covariance_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "8b7e2dea20844e3487297f7bd67167e3bd1d5affb5b0b5ec452d80d19db1beb6"
CONTROL_DIR = Path("data/high_volatility_volume_return_covariance_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_volume_return_covariance_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_volume_return_covariance_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVVRCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVVRCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvvrcr_12_sequential_economics_v1", "policy_id": POLICY_ID,
