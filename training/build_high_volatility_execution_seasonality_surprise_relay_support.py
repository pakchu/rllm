"""Source-only support gate for frozen HVESSR-8."""
...
from training import preregister_high_volatility_execution_seasonality_surprise_relay as prereg
...
PREREG_SHA = "efeb549e1bc904b6f165e9a47795c60e15fe4ae458bee4fa6fe39f006fcd16dd"
SPLITS = {
...
ROOT = Path("data/high_volatility_execution_seasonality_surprise_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_execution_seasonality_surprise_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_execution_seasonality_surprise_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_execution_seasonality_surprise_relay_support_2026-08-13.json")
QUERY = """SELECT ts,open,high,low,close,number_of_trades
FROM bars_binance
...
def aggregate_execution_count(counts: np.ndarray) -> float:
...
def seasonal_surprise(counts: pd.Series) -> tuple[pd.Series, pd.Series]:
    references = pd.concat([counts.shift(21 * week) for week in range(1, 9)], axis=1)
    complete = references.notna().all(axis=1)
    seasonal_reference = references.median(axis=1).where(complete)
    surprise = np.log(counts / seasonal_reference).where(
        counts.gt(0) & seasonal_reference.gt(0)
    )
    return seasonal_reference, surprise
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for column in ("open", "high", "low", "close", "number_of_trades"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVESSR source timestamps")
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        if valid:
            ...
            execution_count = aggregate_execution_count(
                window["number_of_trades"].to_numpy(float)
            )
            valid = (
                variation > 0
                and math.isfinite(block_return)
                and block_return != 0
                and math.isfinite(execution_count)
            )
        if not valid:
            variation = execution_count = block_return = math.nan
        rows.append(
            {
    ...
    states["seasonal_reference"], states["execution_surprise"] = seasonal_surprise(
        states["execution_count"].where(states["source_valid"])
    )
    states["source_valid"] &= states["execution_surprise"].notna()
    states["variation_rank"] = prior_rank(states["realized_variation"].where(states["source_valid"]))
    states["surprise_rank"] = prior_rank(
        states["execution_surprise"].abs().where(states["source_valid"])
    )
    states["count_rank"] = prior_rank(
        states["execution_count"].where(states["source_valid"])
    )
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvessr_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    surprise_rank = states["surprise_rank"]
    surprise = states["execution_surprise"]
    count_rank = states["count_rank"]
    if control == "one_block_stale_geometry":
        block_return, variation_rank, surprise_rank, surprise, count_rank = [
            value.shift(1)
            for value in (
                block_return,
                variation_rank,
    ...
    if control == "no_surprise_tail":
        surprise_gate = pd.Series(True, index=states.index)
    elif control == "absolute_count_tail":
        surprise_gate = count_rank.ge(0.80)
    else:
        surprise_gate = surprise_rank.ge(0.80)
    eligible = states["source_valid"] & variation_gate & surprise_gate & block_return.ne(0)
        ...
        rows.append(
            {
                ...
                "execution_count": float(states.at[index, "execution_count"]),
                "seasonal_reference": float(states.at[index, "seasonal_reference"]),
                "variation_rank": float(states.at[index, "variation_rank"]),
                "execution_surprise": float(states.at[index, "execution_surprise"]),
                "surprise_rank": float(states.at[index, "surprise_rank"]),
            }
        )
    columns = [
        "candidate", "control", "split", "decision_time", "feature_available_time",
        "entry_time", "exit_time", "side", "realized_variation", "variation_rank",
        "execution_count", "seasonal_reference", "execution_surprise", "surprise_rank",
    ]
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvessr_8_source_support_v1",
        "policy_id": "HVESSR-8",
        "preregistration": {
