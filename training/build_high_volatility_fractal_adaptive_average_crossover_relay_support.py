"""Build outcome-blind source support for frozen HVFRAMA-24."""
...
PREREG_SHA = "97e490eab84e5a042ad90ec40b5f44f45c282526363167ed692052bd881d96aa"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_fractal_adaptive_average_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_fractal_adaptive_average_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_fractal_adaptive_average_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_fractal_adaptive_average_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_fractal_adaptive_average_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "dimension", "alpha", "frama", "difference",
    "fixed_ema", "fixed_difference", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "dimension", "alpha", "frama", "difference",
    "fixed_ema", "fixed_difference", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVFRAMA source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVFRAMA invalid source key")
...
def fractal_average(high: pd.Series, low: pd.Series, close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    n=len(close);period=P["frama_periods"];half=P["half_periods"];dimension=np.full(n,np.nan);alpha=np.full(n,np.nan);frama=np.full(n,np.nan);state=np.nan
    highs=high.to_numpy(float);lows=low.to_numpy(float);closes=close.to_numpy(float);validity=valid.to_numpy(bool)
    for index in range(n):
        if not validity[index]:state=np.nan;continue
        left=index-period+1
        if left<0 or not validity[left:index+1].all():state=np.nan;continue
        first=(np.max(highs[left:left+half])-np.min(lows[left:left+half]))/half;second=(np.max(highs[left+half:index+1])-np.min(lows[left+half:index+1]))/half;full=(np.max(highs[left:index+1])-np.min(lows[left:index+1]))/period
        if first<=0 or second<=0 or full<=0:state=np.nan;continue
        value=(math.log(first+second)-math.log(full))/math.log(2.0);speed=min(P["alpha_max"],max(P["alpha_min"],math.exp(-P["dimension_exponent"]*(value-1.0))))
        state=closes[index] if not math.isfinite(state) else speed*closes[index]+(1-speed)*state
        dimension[index]=value;alpha[index]=speed;frama[index]=state
    return pd.DataFrame({"dimension":dimension,"alpha":alpha,"frama":frama},index=close.index)


def fixed_ema(close: pd.Series, valid: pd.Series) -> pd.Series:
    period=P["frama_periods"];smoothing=2.0/(period+1.0);output=np.full(len(close),np.nan);state=np.nan;run=0
    for index,(value,is_valid) in enumerate(zip(close.to_numpy(float),valid.to_numpy(bool))):
        if not is_valid:state=np.nan;run=0;continue
        state=value if not math.isfinite(state) else smoothing*value+(1-smoothing)*state;run+=1
        if run>=period:output[index]=state
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=bars.join(fractal_average(bars.bar_high,bars.bar_low,bars.bar_close,bars.valid_bar));bars["difference"]=bars.bar_close-bars.frama;bars["fixed_ema"]=fixed_ema(bars.bar_close,bars.valid_bar);bars["fixed_difference"]=bars.bar_close-bars.fixed_ema
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["dimension","alpha","frama","difference","fixed_ema","fixed_difference","variation"]]).all(axis=1)&bars.frama.gt(0)&bars.difference.ne(0)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.difference.shift(1);panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&previous.ne(0)&np.sign(panel.difference).ne(np.sign(previous))&panel.variation_rank.ge(P["variation_rank_min"])
    return panel.loc[:,PANEL_COLUMNS]

def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);signal=pd.to_numeric(used.difference,errors="coerce");previous=signal.shift(1);crossing=signal.ne(0)&previous.ne(0)&np.sign(signal).ne(np.sign(previous));variation=used.variation_rank.ge(P["variation_rank_min"]);side=np.sign(signal).fillna(0).astype(int);state=valid&side.ne(0)&crossing&variation
    if control=="no_variation_gate":state=valid&side.ne(0)&crossing
    elif control=="fixed_ema_crossover":signal=pd.to_numeric(used.fixed_difference,errors="coerce");previous=signal.shift(1);state=valid&signal.ne(0)&previous.ne(0)&np.sign(signal).ne(np.sign(previous))&variation;side=np.sign(signal).fillna(0).astype(int)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVFRAMA prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvframa_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvframa_24_source_support_v1", "policy_id": prereg.POLICY_ID,
