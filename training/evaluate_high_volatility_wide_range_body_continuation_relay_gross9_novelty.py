"""Evaluate frozen HVWRBC-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVWRBC-24"
PROTOCOL = "hvwrbc_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_wide_range_body_continuation_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVWRBC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVWRBC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVWRBC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVWRBC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvwrbc_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVWRBC-24 unchanged before economic outcomes",
    }
