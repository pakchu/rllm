"""Evaluate frozen HVLDAR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVLDAR-8"
PROTOCOL = "hvldar_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_leverage_displacement_amplification_reversal_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVLDAR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVLDAR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVLDAR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVLDAR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVLDAR-8 unchanged before economic outcomes",
    }
