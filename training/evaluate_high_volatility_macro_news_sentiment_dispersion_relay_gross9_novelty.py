"""Evaluate frozen HVMNSD-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVMNSD-24"
PROTOCOL = "hvmnsd_24_gross9_novelty_v1"
PREREG = Path("results/high_volatility_macro_news_sentiment_dispersion_relay_preregistration_2026-08-13.json")
PREREG_SHA = "0f1d99f422b4b198d542773a6bdb198ae168df652b94b782f34f47c6419fbe5f"
SUPPORT = Path("results/high_volatility_macro_news_sentiment_dispersion_relay_support_2026-08-13.json")
SUPPORT_SHA = "4a117c0de3b75d9225e9f0f122bdaf253917cc9e1fd4fa4f68921b439b3ecf39"
CLOCK = Path("data/high_volatility_macro_news_sentiment_dispersion_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d5665424eff2bf1abb1474a3e1868ae46c49b364f7f2181095a27bcd49431048"
OUTPUT = Path("results/high_volatility_macro_news_sentiment_dispersion_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVMNSD predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVMNSD novelty limits drift")
    if (
        ...
        or support.get("source_artifacts", {}).get("clock", {}).get("sha256") != CLOCK_SHA
    ):
        raise RuntimeError("HVMNSD predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVMNSD primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvmnsd_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVMNSD-24 unchanged before economic outcomes",
    }
