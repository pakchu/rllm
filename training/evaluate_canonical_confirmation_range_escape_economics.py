"""Sequential strict economics for frozen CCRE-6."""
...
NOVELTY = Path("results/canonical_confirmation_range_escape_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "ddd202ca9aee568800784a9a0925824b8ac377f09099c3ec765d1dd1db629c9a"
...
FREEZE = Path("results/canonical_confirmation_range_escape_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/canonical_confirmation_range_escape_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CCRE frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CCRE novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "ccre_6_sequential_economics_v1", "policy_id": POLICY_ID,
