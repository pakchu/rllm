"""Strict staged economics for the singleton HVCATCLOICR-8 policy."""
...
PREREG = Path("results/high_volatility_causal_average_ticket_close_location_oi_contraction_reversal_preregistration_2026-08-16.json")
PREREG_SHA = "5787e64ce20777655abaf758a72a7de7d938a24bc6024c7991b0a28ea995a32c"
SUPPORT = Path("results/high_volatility_causal_average_ticket_close_location_oi_contraction_reversal_support_2026-08-16.json")
SUPPORT_SHA = "b9cfcd900bf3a9abb60bfe0d5595fdecf6d9d5fd08e2eb38e48c244439489442"
GROSS9 = Path("results/high_volatility_causal_average_ticket_close_location_oi_contraction_reversal_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "40a98110ae95d35936ad8931bd319627d2d0d1c82beba625674fdcd45fb9ce26"
CLOCK = {
    "path": Path("data/high_volatility_causal_average_ticket_close_location_oi_contraction_reversal_clocks_2020_2026.csv.gz"),
    "sha256": "fe5b0acaeb868917f458ca8633cfd7448fe2a4728ff73f3334e0252edc0923bd",
    "rows": 74,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_average_ticket_close_location_oi_contraction_reversal_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCATCLOICR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCATCLOICR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCATCLOICR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCATCLOICR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATCLOICR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATCLOICR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCATCLOICR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCATCLOICR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCATCLOICR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCATCLOICR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCATCLOICR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcatcloicr_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
