"""Evaluate frozen HVPCR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVPCR-8"
PROTOCOL = "hvpcr_8_gross9_novelty_v1"
PREREG = Path(
...
SUPPORT = Path("results/high_volatility_premium_catchup_relay_support_2026-08-09.json")
SUPPORT_SHA = "5ec47ffe05d4250524ebc0a6ad0650ad8783fbd0da9b0d2f36e93f4aa137cf61"
CLOCK = Path("data/high_volatility_premium_catchup_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "ee1f19a50d7b31f12bcbf0d6aefe33d4e54df178bef16ae0777141f2edc65766"
OUTPUT = Path(
    "results/high_volatility_premium_catchup_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVPCR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVPCR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVPCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVPCR primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject HVPCR-8 unchanged before economic outcomes"
        ),
    }
