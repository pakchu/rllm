"""Build source-only CCRE-6 confirmation-ladder clocks."""
...
from training import preregister_canonical_confirmation_range_escape as prereg
...
CONTROLS = ("one_anchor_stale_escape", "direction_flip", "same_clock_forced_long")
...
SOURCE_DIR = Path("data/canonical_confirmation_range_escape_sources_2023_2026")
FEATURES = SOURCE_DIR / "confirmation_ranges.csv.gz"
MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/canonical_confirmation_range_escape_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/canonical_confirmation_range_escape_controls_2023_2026")
RESULT = Path("results/canonical_confirmation_range_escape_support_2026-08-09.json")
FEATURE_COLUMNS = (
    "anchor_height", "confirmation_height", "feature_available_time", "source_valid",
    "signal_close", "reference_high", "reference_low", "eligible_state",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "anchor_height", "confirmation_height",
    "feature_available_time", "entry_time", "exit_time", "side", "signal_close",
...
def validate_blocks(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CCRE block schema drift")
    ...
    if not np.array_equal(np.diff(blocks.height), np.ones(len(blocks) - 1, dtype=np.int64)):
        raise RuntimeError("CCRE block heights are not contiguous")
    if not blocks.previousblockhash.iloc[1:].reset_index(drop=True).equals(blocks.id.iloc[:-1].reset_index(drop=True)):
        raise RuntimeError("CCRE canonical previous-hash continuity failed")
    return blocks
...
def prepare_minutes(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CCRE minute schema drift")
    ...
    if result.ts.duplicated().any():
        raise RuntimeError("CCRE duplicate timestamps")
    return result.sort_values("ts", kind="mergesort").set_index("ts")


def range_escape(market: pd.DataFrame, decision: pd.Timestamp):
    start = decision - pd.Timedelta(hours=24, minutes=5)
    expected = pd.date_range(start, decision, freq="1min", inclusive="left")
    if len(expected) != 289 * 5:
        raise AssertionError("CCRE window length drift")
    window = market.reindex(expected)
    if window.source_valid.isna().any() or not bool(window.source_valid.all()):
        return None
    reference = window.iloc[: 288 * 5]
    signal = window.iloc[288 * 5 :]
    signal_close = float(signal.close.iloc[-1])
    reference_high = float(reference.high.max())
    reference_low = float(reference.low.min())
    if not np.isfinite([signal_close, reference_high, reference_low]).all():
        return None
    side = 1 if signal_close > reference_high else -1 if signal_close < reference_low else 0
    return signal_close, reference_high, reference_low, side
...
def build_features(blocks: pd.DataFrame, market: pd.DataFrame) -> pd.DataFrame:
    ...
    for index in range(0, len(blocks) - 6):
        ...
        values = range_escape(market, decision)
        valid = values is not None
        signal_close, reference_high, reference_low, side = values if valid else (np.nan, np.nan, np.nan, 0)
        rows.append({
            "anchor_height": height,
            "confirmation_height": int(blocks.height.iloc[index + 6]),
...
def active_and_side(features: pd.DataFrame, control: str = "primary"):
    ...
    state = pd.to_numeric(features.eligible_state, errors="raise").astype(int)
    if control == "one_anchor_stale_escape":
        state = state.shift(1, fill_value=0)
    active = pd.Series(False, index=features.index); previous_valid_state = 0
    for index in features.index:
        if not bool(features.at[index, "source_valid"]):
            continue
        current = int(state.at[index]); active.at[index] = current != 0 and current != previous_valid_state
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active & sides.ne(0)]:
        ...
        rows.append({
            ...
            "side": int(sides.at[index]), "signal_close": float(features.at[index, "signal_close"]),
            "reference_high": float(features.at[index, "reference_high"]),
            "reference_low": float(features.at[index, "reference_low"]),
        })
...
def run() -> dict[str, Any]:
    ...
    source_core = {
        "protocol_version": "ccre_6_source_v1", "query_sha256": hashlib.sha256(QUERY.encode()).hexdigest(), "window": [QUERY_START.isoformat(), END.isoformat()],
    ...
    core = {
        "protocol_version": "ccre_6_source_support_v1", "policy_id": prereg.POLICY_ID,
