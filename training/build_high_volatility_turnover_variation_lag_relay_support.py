"""Build outcome-blind source support for frozen HVTVLR-8."""
...
from training import preregister_high_volatility_turnover_variation_lag_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-04-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="59886b851207b391297a9ca865496089742561ded0c222898c2e0a9b7acd67d2"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_turnover_variation_lag_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_turnover_variation_lag_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_turnover_variation_lag_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_turnover_variation_lag_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_turnover_variation_lag_relay_support_2026-08-10.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","timing_lag","lag_rank","fixed_half_backloading","backloading_rank","realized_variation","variation_rank","completed_return","final_two_hour_return","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","timing_lag","lag_rank","fixed_half_backloading","backloading_rank","realized_variation","variation_rank","completed_return","final_two_hour_return")
...
def timing_statistics(block:pd.DataFrame)->tuple[float,float,float,float,float]:
 quote=block.quote_asset_volume.to_numpy(float);total=float(quote.sum())
 returns=np.log(block.close.to_numpy(float)/block.open.to_numpy(float))
 variation=float(np.square(returns).sum())
 if total<=0 or variation<=0:return (math.nan,)*5
 time=np.arange(480,dtype=float)/479.;turnover_centroid=float(np.dot(time,quote)/total);variation_centroid=float(np.dot(time,np.square(returns))/variation);lag=turnover_centroid-variation_centroid
 backloading=float(quote[240:].sum()/total-quote[:240].sum()/total)
 ...
 return lag,backloading,variation,completed,final_two_hour
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close","quote_asset_volume"]:raise RuntimeError("HVTVLR source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVTVLR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("7h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   lag,backloading,var,ret,final_ret=timing_statistics(block);valid=all(math.isfinite(v) for v in (lag,backloading,var,ret,final_ret)) and var>0 and lag>0 and ret!=0 and final_ret!=0
  else:lag=backloading=var=ret=final_ret=math.nan;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"timing_lag":lag,"fixed_half_backloading":backloading,"realized_variation":var,"completed_return":ret,"final_two_hour_return":final_ret})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["lag_rank"]=prior_rank(panel.timing_lag.where(valid));panel["backloading_rank"]=prior_rank(panel.fixed_half_backloading.where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));agreement=np.sign(panel.completed_return).eq(np.sign(panel.final_two_hour_return));panel["eligible"]=valid&panel.lag_rank.ge(P["lag_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"])&agreement;panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_boundary_stale_geometry":
  cols=["source_valid","timing_lag","lag_rank","fixed_half_backloading","backloading_rank","variation_rank","completed_return","final_two_hour_return","feature_available_time"];used[cols]=panel[cols].shift(1)
 valid=used.source_valid.eq(True);strong=used.lag_rank.ge(P["lag_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"]);agreement=np.sign(used.completed_return).eq(np.sign(used.final_two_hour_return));state=valid&strong&variation&agreement
 if control=="no_lag_tail":state=valid&variation&agreement
 elif control=="no_variation_gate":state=valid&strong&agreement
 elif control=="fixed_half_backloading":state=valid&used.backloading_rank.ge(P["lag_rank_min"])&variation&agreement
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVTVLR prereg drift")
 ...
 source_core={"protocol_version":"hvtvlr_8_source_v1","query":QUERY,"query_sha256":hashlib.sha256(QUERY.encode()).hexdigest(),"window":[START.isoformat(),END.isoformat()],"physical_rows":len(raw),"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"panel":{"path":str(PANEL),"sha256":sha(PANEL),"rows":len(panel),"valid_rows":int(panel.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};manifest={**source_core,"manifest_hash":chash(source_core)};immutable(MANIFEST,json_bytes(manifest))
