"""Build outcome-blind source support for frozen HVRSVRP-12."""
...
from training import preregister_high_volatility_relative_signed_variation_risk_premium as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2023-04-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "1012e85acf505f50af4adbaa6d204909c7e7fe099a5b2df95244da6b7eca6205"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_relative_signed_variation_risk_premium_sources_2023_2026")
PANEL = ROOT / "daily_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_relative_signed_variation_risk_premium_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_relative_signed_variation_risk_premium_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_relative_signed_variation_risk_premium_controls_2023_2026")
RESULT = Path("results/high_volatility_relative_signed_variation_risk_premium_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "perp_rows", "positive_semivariance",
    "negative_semivariance", "realized_variation", "relative_signed_variation",
    "signed_variation_rank", "variation_rank", "signed_tail", "variation_tail",
    "day_return", "entry_side", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "positive_semivariance", "negative_semivariance",
    "realized_variation", "relative_signed_variation", "signed_variation_rank", "variation_rank",
    "signed_tail", "variation_tail", "day_return", "entry_side", "eligible",
)
...
def signed_variation(returns: np.ndarray) -> tuple[float, float, float, float]:
    values=np.asarray(returns,float)
    if values.shape!=(P["five_minute_bars"],) or not np.isfinite(values).all():return (math.nan,)*4
    positive=float(np.square(values[values>0]).sum());negative=float(np.square(values[values<0]).sum());total=positive+negative
    if not math.isfinite(total) or total<=0:return (math.nan,)*4
    return positive,negative,total,(positive-negative)/total
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVRSVRP source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVRSVRP invalid source key")
...
def build_panel(perp_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = []
    for source_start in pd.date_range(START, END, freq="1D", inclusive="left"):
        expected = pd.date_range(source_start, source_start + pd.Timedelta("1D"), freq="1min", inclusive="left")
        perp_block = perp.reindex(expected);perp_rows=int(perp_block.row_valid.eq(True).sum())
        valid = perp_rows == 1440 and bool(perp_block.row_valid.eq(True).all())
        if valid:
            values=perp_block[["open","close"]].to_numpy(float).reshape(288,5,2);returns=np.log(values[:,-1,1]/values[:,0,0])
            positive,negative,variation,relative=signed_variation(returns);day_return=float(np.log(values[-1,-1,1]/values[0,0,0]))
            valid=np.isfinite([positive,negative,variation,relative,day_return]).all() and variation>0 and day_return!=0
        else:
            positive=negative=variation=relative=day_return=math.nan
        rows.append({"source_start":source_start,"source_valid":valid,"perp_rows":perp_rows,"positive_semivariance":positive,"negative_semivariance":negative,"realized_variation":variation,"relative_signed_variation":relative,"day_return":day_return})
    panel = pd.DataFrame(rows)
    panel["feature_available_time"] = panel.source_start + pd.Timedelta("1D")
    panel["signed_variation_rank"] = prior_rank(panel.relative_signed_variation.where(panel.source_valid), panel.source_valid)
    panel["variation_rank"] = prior_rank(panel.realized_variation.where(panel.source_valid), panel.source_valid)
    panel["signed_tail"] = panel.signed_variation_rank.le(P["lower_signed_variation_rank_max"]) | panel.signed_variation_rank.ge(P["upper_signed_variation_rank_min"])
    panel["variation_tail"] = panel.variation_rank.ge(P["variation_rank_min"])
    panel["entry_side"] = np.where(panel.signed_variation_rank.ge(P["upper_signed_variation_rank_min"]),-1,np.where(panel.signed_variation_rank.le(P["lower_signed_variation_rank_max"]),1,0))
    panel["eligible"] = panel.source_valid & panel.signed_tail & panel.variation_tail & pd.Series(panel.entry_side,index=panel.index).ne(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    if control == "no_signed_variation_tail":
        state = valid & used.variation_tail;side=-np.sign(used.relative_signed_variation).fillna(0).astype(int)
    elif control == "no_variation_gate":
        state = valid & used.signed_tail & side.ne(0)
    elif control == "raw_day_return_reversal":
        state = valid & used.signed_tail & used.variation_tail;side=-np.sign(used.day_return).fillna(0).astype(int)
    elif control == "one_day_stale_state":
...
def build_clock(panel: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in panel.index[activity]:
        ...
        rows.append({
            ...
            **{column: bool(used.at[index, column]) if column in ("signed_tail", "variation_tail", "eligible") else float(used.at[index, column]) for column in CLOCK_COLUMNS[8:]},
        })
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVRSVRP prereg drift")
    ...
    source_core = {
        "protocol_version": "hvrsvrp_12_source_v1", "queries": {"perpetual": PERP_QUERY},
    ...
    core = {
        "protocol_version": "hvrsvrp_12_source_support_v1", "policy_id": prereg.POLICY_ID,
