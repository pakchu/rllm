"""Evaluate frozen HVAPS-F2-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVAPS-F2-24"
PROTOCOL = "hvaps_f2_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_augen_price_spike_fade_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVAPS predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVAPS novelty limits drift")
    if (
        ...
        raise RuntimeError("HVAPS predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVAPS primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvaps_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVAPS-F2-24 unchanged before economic outcomes",
    }
