"""Deterministic outcome-blind source support for HVAFGF-8."""
...
from training import preregister_high_volatility_adverse_funding_aggressive_flow_continuation as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2023-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "d0bc10b333491b8693705d2a04da8ede46f2e4b1a0c2d51ad8cfd4c7d81f908e"
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if funding.columns.tolist() != ["decision_time", "funding_rate"]:
        raise RuntimeError("HVAFGF-8 funding schema drift")
    expected = ["decision_time", "price_return", "flow_numerator", "quote_turnover", "minute_squared_return", "source_rows", "distinct_rows", "first_ts", "last_ts", "coherent"]
    if bars.columns.tolist() != expected:
        raise RuntimeError("HVAFGF-8 bar schema drift")
    funding = funding.copy(); funding["decision_time"] = pd.to_datetime(funding["decision_time"], utc=True, errors="raise"); funding["funding_rate"] = pd.to_numeric(funding["funding_rate"], errors="raise")
    if funding["decision_time"].duplicated().any() or not np.isfinite(funding["funding_rate"]).all():
        raise RuntimeError("HVAFGF-8 invalid funding rows")
    bars = bars.copy()
    for column in ("decision_time", "first_ts", "last_ts"): bars[column] = pd.to_datetime(bars[column], utc=True, errors="raise")
    numeric = ("price_return", "flow_numerator", "quote_turnover", "minute_squared_return", "source_rows", "distinct_rows")
    ...
    bars["flow_share"] = bars["flow_numerator"] / bars["quote_turnover"]
    return funding.sort_values("decision_time"), bars.set_index("decision_time").sort_index()


def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    funding, bars = prepare(raw); panel = funding.copy()
    panel = panel.join(bars[["bar_valid","price_return","flow_share","realized_variation"]],on="decision_time")
    panel["source_valid"] = (panel["bar_valid"].eq(True) & np.isfinite(panel[["funding_rate","price_return","flow_share","realized_variation"]]).all(axis=1) & panel["funding_rate"].ne(0) & panel["price_return"].ne(0) & panel["flow_share"].ne(0) & panel["realized_variation"].gt(0))
    valid=panel["source_valid"].eq(True); panel["variation_rank"]=causal_midrank(panel["realized_variation"].where(valid))
    side = np.sign(panel["flow_share"])
    panel["eligible"]=(valid & np.sign(panel["price_return"]).eq(side) & np.sign(panel["funding_rate"]).eq(-side) & panel["variation_rank"].ge(POLICY["variation_rank_min"]))
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if split is None:
            continue
        side = int(np.sign(row.flow_share))
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVAFGF-8 side or availability drift")
        ...
        rows.append({
            ...
            "flow_share": float(row.flow_share),
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVAFGF-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvafgf_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvafgf_8_source_support_v1", "policy_id": prereg.POLICY_ID,
