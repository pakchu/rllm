"""Sequential strict economics for frozen HVKVO-24."""
...
PREREG = Path("results/high_volatility_klinger_volume_oscillator_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "ef8d550bae4930516e88d487618eca2160d9fe5f747c89413daf3a1aa22bdaff"
SUPPORT = Path("results/high_volatility_klinger_volume_oscillator_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "174c981a705cb1547c96a31404fd0029d5223a9f34c5dd26619a7943d22ba946"
NOVELTY = Path("results/high_volatility_klinger_volume_oscillator_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "444711944bcaefc49d7244776a0df4da391fd1c7f1a149d9d1773f51e757e99e"
CLOCK = Path("data/high_volatility_klinger_volume_oscillator_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "1f61c258b2677c2400f673b4967759e668264011696873d76663e4f9c59e772f"
CONTROL_DIR = Path("data/high_volatility_klinger_volume_oscillator_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_klinger_volume_oscillator_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_klinger_volume_oscillator_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "raw_volume_force_zero_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVKVO frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVKVO novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvkvo_24_sequential_economics_v1", "policy_id": POLICY_ID,
