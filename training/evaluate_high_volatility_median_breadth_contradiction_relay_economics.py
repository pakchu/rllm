"""Sequential strict economics for frozen HVMBC-8."""
...
PREREG = Path("results/high_volatility_median_breadth_contradiction_relay_preregistration_2026-08-09.json")
PREREG_SHA = "b5c9670de622539af5de133381dc20c2cf0f3ace40281b66c87cf1984a21f1ae"
SUPPORT = Path("results/high_volatility_median_breadth_contradiction_relay_support_2026-08-09.json")
SUPPORT_SHA = "6a42c8375ab6abe3309a2d5335cb4fa0a383c1bd057a8959f2faa70c7c68089b"
NOVELTY = Path("results/high_volatility_median_breadth_contradiction_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "3adb416f330d7873a2ce05cf85b380b494c11cd46723ab5661269a88d508b569"
CLOCK = Path("data/high_volatility_median_breadth_contradiction_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "b3b6c52f86481112d6d155f07dd124ff6bb0629a5d38dc4ac25bd651e03139af"
CONTROL_DIR = Path("data/high_volatility_median_breadth_contradiction_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_median_breadth_contradiction_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_median_breadth_contradiction_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_contradiction_gate", "no_variation_gate", "block_return_side", "mean_bar_return_side", "one_block_stale_features", "direction_flip", "same_clock_forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVMBC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVMBC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvmbc_8_sequential_economics_v1", "policy_id": POLICY_ID,
