"""Sequential strict economics for frozen HVMRSR-8."""
...
PREREG = Path("results/high_volatility_median_return_shift_relay_preregistration_2026-08-10.json")
PREREG_SHA = "29c87e3b08c500c2ffa0371c94849a5d84dcbf99ae0eb3f9df25922c3393863b"
SUPPORT = Path("results/high_volatility_median_return_shift_relay_support_2026-08-10.json")
SUPPORT_SHA = "41f8754ad340d062cb550151d2017c3567b12661671573fc7f9307129e091b46"
NOVELTY = Path("results/high_volatility_median_return_shift_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "83fb5e444b8d19519c833a3f13021bb7ad89189bd5ab1a97d8403ba96130bdcf"
CLOCK = Path("data/high_volatility_median_return_shift_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "71adf3d170a68a26928d6d4600d8fb46046b7adddb64f464948355df41f98d5c"
CONTROL_DIR = Path("data/high_volatility_median_return_shift_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_median_return_shift_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_median_return_shift_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_shift_tail", "no_variation_gate", "mean_return_shift", "one_boundary_stale_geometry", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVMRSR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVMRSR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvmrsr_8_sequential_economics_v1", "policy_id": POLICY_ID,
