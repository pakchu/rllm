"""Strict staged economics for the singleton HVRM3-8 policy."""
...
PREREG = Path("results/high_volatility_robust_state_three_action_majority_preregistration_2026-08-16.json")
PREREG_SHA = "c98bd816384c92cac4395434d41455a98dc3fa5a52d9509f624d4a35bc6749a0"
SUPPORT = Path("results/high_volatility_robust_state_three_action_majority_support_2026-08-16.json")
SUPPORT_SHA = "0a2c3a0ff2849416be9553d58fa9ab6345b0dfe5ac44cd04765c0a68a5a1486c"
GROSS9 = Path("results/high_volatility_robust_state_three_action_majority_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "2a95311ead600c49e6a13072f66beb5b84f6e1a5e582d8c2a9366a9baa285dd3"
CLOCK = {
    "path": Path("data/high_volatility_robust_state_three_action_majority_clocks_2023_2026.csv.gz"),
    "sha256": "f51b8db96b3e7a6d709f0bce414fab61273618b4dab23f88658b15fe8d8bd576",
    "rows": 206,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_robust_state_three_action_majority_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVRM3-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVRM3-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVRM3-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVRM3-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVRM3-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVRM3-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVRM3-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVRM3-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVRM3-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVRM3-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVRM3-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrm3_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
