"""Outcome-blind preregistration for HVCARQ-6."""
...
POLICY_ID = "HVCARQ-6"
SLUG = "high_volatility_cross_alt_realized_quarticity_concentration_spillover_relay"
...
def build() -> dict[str, Any]:
    core = {
        ...
        "mechanism": {
            "claim": "At three fixed daily decisions, simultaneous upper-tail realized-quarticity concentration in at least four liquid alt perpetuals identifies a broad few-jump information wave rather than diffuse variance. During elevated BTC variation, BTC follows the quarticity-weighted common completed alt displacement for six hours.",
            "side": "strict sign of the selected-alt realized-quarticity-weighted eight-hour displacement consensus",
            "why_distinct": "Variance-shock propagation ranks one-hour quadratic variation, cojump propagation thresholds individual five-minute returns, return-tail asymmetry uses a cross-sectional cubic endpoint statistic, and BTC realized-quarticity concentration is single-asset. HVCARQ applies a scale-free fourth-moment concentration statistic separately to each alt's completed eight-hour path, selects own-history quarticity tails, and transfers only their broad signed consensus into BTC. It reuses no event set or control and uses no BTC return direction, volume, flow, funding, OI, premium, or fitted outcome.",
            "why_suited_to_volatile_regimes": "BTC completed twenty-four-hour realized variation must rank in its causal upper 35 percent against strictly prior scheduled decisions, while the alt filter distinguishes concentrated jump-like volatility from diffuse noise.",
            "why_low_gross9_overlap_is_plausible": "02:35, 10:35 and 18:35 UTC entries conditioned on cross-alt fourth-moment concentration are absent from Gross9 primitive clocks.",
        },
        "features": {
            ...
            "alt_path": "96 completed five-minute returns ending at decision for each alt",
            "realized_quarticity_concentration": "96*sum(return^4)/(sum(return^2)^2), finite with strictly positive denominator",
            "alt_displacement": "sum of the same 96 returns, finite strict nonzero for a selected alt",
            "quarticity_rank": "for each alt independently, strict-prior midrank of quarticity over at most 120 prior jointly valid scheduled decisions, minimum 60, current excluded; selected when rank>=0.75",
            "score": "sum(quarticity*sign(displacement) over selected alts)/sum(quarticity over selected alts)",
            "consensus_gate": "at least four selected alts, abs(score)>=0.60, and at least three selected displacement signs equal score sign",
        ...
        "policy": {
            "path_bars": 96,
            "quarticity_prior_decisions": 120,
            "quarticity_minimum_decisions": 60,
            "quarticity_rank_min": 0.75,
        ...
        "diagnostic_controls": {"names": ["no_variation_gate", "no_quarticity_tail", "three_alt_consensus", "variance_weighted_score", "one_decision_stale_consensus", "direction_flip", "forced_long"], "cannot_be_promoted": True},
        ...
        "research_boundary": {"prior_cross_alt_and_single_asset_quarticity_outcomes_known": True, "repository_exact_cross_alt_quarticity_consensus_event_found": False, "prior_event_sets_or_controls_reused": False, "candidate_incidence_opened": False, "postentry_return_or_pnl_opened": False, "gross9_rows_opened": False, "candidate_count": 1, "grid": False, "repair_of_prior_candidate": False, "promoted_prior_control": False, "selection_basis": "independent cross-alt fourth-moment concentration spillover mechanism selected by outcome-blind primitive-gap audit"},
...
def validate(value: dict[str, Any]) -> None:
    ...
    if value.get("manifest_hash") != canonical_hash(core) or value != build():
        raise RuntimeError("HVCARQ prereg drift")
