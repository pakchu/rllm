"""Evaluate frozen HVESSR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVESSR-8"
PROTOCOL = "hvessr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_execution_seasonality_surprise_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVESSR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVESSR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVESSR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVESSR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvessr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVESSR-8 unchanged before economic outcomes",
    }
