"""Build outcome-blind source support for frozen HVDEADR-8."""
...
from training import preregister_high_volatility_directional_excursion_area_dominance_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-04-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="5e6d73b848addeeaa6fcab1b4f9958e4660c452e67980ecdc58f06c264396cba"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_directional_excursion_area_dominance_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_directional_excursion_area_dominance_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_directional_excursion_area_dominance_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_directional_excursion_area_dominance_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_directional_excursion_area_dominance_relay_support_2026-08-10.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","signed_excursion_area","absolute_excursion_area","directional_area_dominance","dominance_rank","endpoint_displacement","endpoint_rank","realized_variation","variation_rank","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","signed_excursion_area","absolute_excursion_area","directional_area_dominance","dominance_rank","endpoint_displacement","endpoint_rank","realized_variation","variation_rank")
...
def area_statistics(block:pd.DataFrame)->tuple[float,float,float,float,float]:
 anchor=float(block.open.iloc[0]);displacement=np.log(block.close.to_numpy(float)/anchor);signed=float(displacement.sum());absolute=float(np.abs(displacement).sum());dominance=signed/absolute;endpoint=float(displacement[-1]);minute_returns=np.log(block.close.to_numpy(float)/block.open.to_numpy(float));variation=float(np.sqrt(np.square(minute_returns).sum()));return signed,absolute,dominance,endpoint,variation
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVDEADR source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVDEADR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("3h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   signed,absolute,dominance,endpoint,var=area_statistics(block);valid=all(math.isfinite(v) for v in (signed,absolute,dominance,endpoint,var)) and absolute>0 and var>0 and dominance!=0
  else:signed=absolute=dominance=endpoint=var=math.nan;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"signed_excursion_area":signed,"absolute_excursion_area":absolute,"directional_area_dominance":dominance,"endpoint_displacement":endpoint,"realized_variation":var})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["dominance_rank"]=prior_rank(panel.directional_area_dominance.abs().where(valid));panel["endpoint_rank"]=prior_rank(panel.endpoint_displacement.abs().where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));panel["eligible"]=valid&panel.dominance_rank.ge(P["dominance_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"]);panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_block_stale_area":
  current_valid=panel.source_valid.eq(True);cols=["signed_excursion_area","absolute_excursion_area","directional_area_dominance","dominance_rank","endpoint_displacement","endpoint_rank","feature_available_time"];used[cols]=panel[cols].shift(1);used["source_valid"]=current_valid&panel.source_valid.shift(1).fillna(False)
 valid=used.source_valid.eq(True);strong=used.dominance_rank.ge(P["dominance_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&strong&variation
 if control=="no_dominance_tail":state=valid&variation
 elif control=="no_variation_gate":state=valid&strong
 elif control=="endpoint_displacement":state=valid&used.endpoint_displacement.ne(0)&used.endpoint_rank.ge(P["dominance_rank_min"])&variation
 onset=previous_valid_onset(state,valid);side=np.sign(pd.to_numeric(used.directional_area_dominance,errors="coerce").fillna(0)).astype(int)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVDEADR prereg drift")
 ...
 source_core={"protocol_version":"hvdeadr_8_source_v1","query":QUERY,"query_sha256":hashlib.sha256(QUERY.encode()).hexdigest(),"window":[START.isoformat(),END.isoformat()],"physical_rows":len(raw),"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"panel":{"path":str(PANEL),"sha256":sha(PANEL),"rows":len(panel),"valid_rows":int(panel.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};manifest={**source_core,"manifest_hash":chash(source_core)};immutable(MANIFEST,json_bytes(manifest))
