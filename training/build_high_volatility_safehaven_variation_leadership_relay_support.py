"""Materialize source-only HVSVL-8 support clocks."""
...
ENV_FILE="/home/pakchu/rllm/.env"; PREREG_SHA="21ffaa568d9d79a14a3cbf2e36fa7acf2edcbb2d2a20e320b5b899eb3704129e"; START=pd.Timestamp("2023-01-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
SOURCE_DIR=Path("data/high_volatility_safehaven_variation_leadership_relay_sources_2023_2026"); SESSION=SOURCE_DIR/"safehaven_variation_sessions.csv.gz"; SOURCE_MANIFEST=SOURCE_DIR/"manifest.json"
...
CLOCK=Path("data/high_volatility_safehaven_variation_leadership_relay_clocks_2023_2026.csv.gz"); CONTROL_DIR=Path("data/high_volatility_safehaven_variation_leadership_relay_controls_2023_2026"); RESULT=Path("results/high_volatility_safehaven_variation_leadership_relay_support_2026-08-13.json")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00:00Z"),pd.Timestamp("2024-01-01T00:00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00:00Z"),pd.Timestamp("2025-01-01T00:00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00:00Z"),pd.Timestamp("2026-01-01T00:00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00:00Z"),pd.Timestamp("2026-08-01T00:00:00Z"))}; MINIMUM={"train":8,"test":12,"eval":12,"final":8}; CONTROLS=("no_variation_gate","no_leadership_tail","one_session_stale_leader","subordinate_pair_direction","direction_flip","forced_long")
SYMBOLS=("USDJPY","USDCHF"); RISK_MULTIPLIER={"USDJPY":-1.,"USDCHF":-1.}
COLUMNS=("candidate","control","split","source_day","decision_time","feature_available_time","entry_time","exit_time","side","dominant_pair","dominant_safehaven_return","leadership_score","leadership_rank","btc_realized_variation","btc_realized_variation_rank")
QUERY="""SELECT date_trunc('day',ts) AS source_day,(array_agg(open ORDER BY ts))[1] AS session_open,max(high) AS session_high,min(low) AS session_low,(array_agg(close ORDER BY ts DESC))[1] AS session_close,sqrt(sum(CASE WHEN open>0 AND close>0 THEN power(ln(close/open),2) END)) AS realized_variation,bool_and(open>0 AND high>0 AND low>0 AND close>0) AS all_positive,count(*) AS source_rows,count(DISTINCT ts) AS distinct_timestamps,min(ts) AS first_ts,max(ts) AS last_ts FROM bars_polygon WHERE symbol=:symbol AND interval='1m' AND ts>=:start AND ts<:end AND extract(isodow from ts) BETWEEN 1 AND 5 AND extract(hour from ts)>=7 AND extract(hour from ts)<13 GROUP BY 1 ORDER BY 1"""
...
def materialize_sessions()->dict:
 ...
 with db.connect() as connection:
  for symbol in SYMBOLS:
   ...
   for column in ("session_open","session_high","session_low","session_close","realized_variation"):raw[column]=pd.to_numeric(raw[column],errors="coerce")
   valid=raw.distinct_timestamps.ge(330)&raw.first_ts.le(raw.source_day+pd.Timedelta(hours=7,minutes=5))&raw.last_ts.ge(raw.source_day+pd.Timedelta(hours=12,minutes=55))&raw.all_positive.eq(True)&np.isfinite(raw[["session_open","session_high","session_low","session_close","realized_variation"]]).all(axis=1)&raw[["session_open","session_high","session_low","session_close","realized_variation"]].gt(0).all(axis=1)&raw.session_high.ge(raw[["session_open","session_close"]].max(axis=1))&raw.session_low.le(raw[["session_open","session_close"]].min(axis=1))
   canonical=(RISK_MULTIPLIER[symbol]*np.log(raw.session_close/raw.session_open)).where(valid);piece=pd.DataFrame({"source_day":raw.source_day,f"{symbol}_valid":valid,f"{symbol}_safehaven_return":canonical,f"{symbol}_realized_variation":raw.realized_variation.where(valid)});frames.append(piece)
 ...
 j=frame.USDJPY_realized_variation;c=frame.USDCHF_realized_variation;frame["leadership_score"]=np.log(j/c).abs().where(frame.source_valid);frame["leadership_rank"]=strict_prior_midrank(frame.leadership_score);frame["dominant_pair"]=np.where(j.gt(c),"USDJPY",np.where(c.gt(j),"USDCHF",""));frame["dominant_safehaven_return"]=np.where(j.gt(c),frame.USDJPY_safehaven_return,np.where(c.gt(j),frame.USDCHF_safehaven_return,np.nan));frame["subordinate_safehaven_return"]=np.where(j.gt(c),frame.USDCHF_safehaven_return,np.where(c.gt(j),frame.USDJPY_safehaven_return,np.nan));frame["decision_time"]=frame.source_day+pd.Timedelta(hours=13)
 SOURCE_DIR.mkdir(parents=True,exist_ok=True);_write_gzip_csv(frame,SESSION);core={"protocol_version":"hvsvl_8_safehaven_variation_source_v1","query":QUERY,"table":"bars_polygon","symbols":list(SYMBOLS),"canonical_risk_multipliers":RISK_MULTIPLIER,"interval":"1m","window":[START.isoformat(),END.isoformat()],"outcomes_opened":False,"candidate_incidence_opened":False,"no_imputation":True,"output":{"path":str(SESSION),"sha256":sha(SESSION),"rows":len(frame),"valid_rows":int(frame.source_valid.sum())}};payload={**core,"manifest_hash":chash(core)};SOURCE_MANIFEST.write_text(json.dumps(payload,indent=2,allow_nan=False)+"\n");return payload
def features()->pd.DataFrame:
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVSVL prereg drift")
 if sha(PRICE)!=PRICE_SHA or sha(PRICE_MANIFEST)!=PRICE_MANIFEST_SHA:raise RuntimeError("HVSVL BTC source drift")
 ...
 for column in ("dominant_safehaven_return","subordinate_safehaven_return","leadership_score","leadership_rank"):f[column]=pd.to_numeric(f[column],errors="coerce")
 ...
 d=p[["decision_time","btc_realized_variation","btc_valid"]];f=f.merge(d,on="decision_time",how="left",validate="one_to_one");f["btc_realized_variation_rank"]=strict_prior_midrank(f.btc_realized_variation.where(f.source_valid&f.btc_valid));f["signal_valid"]=f.source_valid&f.btc_valid&f.dominant_pair.isin(SYMBOLS)&np.isfinite(f[["dominant_safehaven_return","leadership_score","leadership_rank","btc_realized_variation","btc_realized_variation_rank"]]).all(axis=1);return f
def conditions(f:pd.DataFrame,control:str)->tuple[pd.Series,pd.Series]:
 signal=pd.to_numeric(f.dominant_safehaven_return,errors="coerce");tail=pd.to_numeric(f.leadership_rank,errors="coerce")
 if control=="subordinate_pair_direction":signal=pd.to_numeric(f.subordinate_safehaven_return,errors="coerce")
 if control=="one_session_stale_leader":signal=signal.shift(1);tail=tail.shift(1)
 vol=pd.Series(True,index=f.index) if control=="no_variation_gate" else f.btc_realized_variation_rank.ge(.65)
 tail_gate=pd.Series(True,index=f.index) if control=="no_leadership_tail" else tail.ge(.65)
...
def clock(f:pd.DataFrame,control:str="primary")->pd.DataFrame:
 ...
 for i in f.index[active]:
  ...
  next_allowed=exit_;rows.append({"candidate":"HVSVL-8","control":control,"split":split,"source_day":f.at[i,"source_day"],"decision_time":decision,"feature_available_time":decision,"entry_time":entry,"exit_time":exit_,"side":int(side.at[i]),"dominant_pair":f.at[i,"dominant_pair"],"dominant_safehaven_return":float(f.at[i,"dominant_safehaven_return"]),"leadership_score":float(f.at[i,"leadership_score"]),"leadership_rank":float(f.at[i,"leadership_rank"]),"btc_realized_variation":float(f.at[i,"btc_realized_variation"]),"btc_realized_variation_rank":float(f.at[i,"btc_realized_variation_rank"])})
