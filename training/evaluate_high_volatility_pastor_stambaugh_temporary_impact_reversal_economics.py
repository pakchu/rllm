"""Sequential strict economics for frozen HVPST-12."""
...
PREREG = Path("results/high_volatility_pastor_stambaugh_temporary_impact_reversal_preregistration_2026-08-11.json")
PREREG_SHA = "3b703782e9315b22ecb18be5789cfb485e9fcc21f843ebd4d853bc1082032d40"
SUPPORT = Path("results/high_volatility_pastor_stambaugh_temporary_impact_reversal_support_2026-08-11.json")
SUPPORT_SHA = "9b39f5285a15ca4a1043bd6dc27007a45ddf24a1400f6e24a97abb45c59f1fab"
NOVELTY = Path("results/high_volatility_pastor_stambaugh_temporary_impact_reversal_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "655252a0eb3c664abf3c269e9bcc9939b3c55d0c5d142c764f5f1c28a8045ed6"
CLOCK = Path("data/high_volatility_pastor_stambaugh_temporary_impact_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "b60302041542cbd89e8c24a0a68cf3eca0ac69f3b8e70a6c565005b4780085aa"
CONTROL_DIR = Path("data/high_volatility_pastor_stambaugh_temporary_impact_reversal_controls_2023_2026")
FREEZE = Path("results/high_volatility_pastor_stambaugh_temporary_impact_reversal_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_pastor_stambaugh_temporary_impact_reversal_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_illiquidity_tail_gate", "no_variation_gate", "omit_return_control", "one_bar_stale_onset", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVPST frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVPST novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvpst_12_sequential_economics_v1", "policy_id": POLICY_ID,
