"""Sequential strict economics for frozen HVCASI-6."""
...
PREREG = Path("results/high_volatility_cross_alt_semivariance_imbalance_cascade_relay_preregistration_2026-08-13.json")
PREREG_SHA = "bfa24ad3de932076ebe261bd54b482d21f6c3384bc69e30e4ce7ab9fb85bea3a"
SUPPORT = Path("results/high_volatility_cross_alt_semivariance_imbalance_cascade_relay_support_2026-08-13.json")
SUPPORT_SHA = "fecc1d2d3d7e8266fb1e448fb4ff1a0302a3da1484c24b283832784151f0af98"
NOVELTY = Path("results/high_volatility_cross_alt_semivariance_imbalance_cascade_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "690937f544fa7c5c98ae5fed5ffb2166651e2095b1a6becb82823c97b58f1529"
CLOCK = Path("data/high_volatility_cross_alt_semivariance_imbalance_cascade_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "b12ecd9739dc1f248c695aca5c82a9254f065f7f7dc98cb2d975eb2ecdd68241"
CONTROL_DIR = Path("data/high_volatility_cross_alt_semivariance_imbalance_cascade_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_cross_alt_semivariance_imbalance_cascade_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_alt_semivariance_imbalance_cascade_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "no_semivariance_imbalance_tail", "three_alt_consensus", "equal_weighted_score", "one_decision_stale_consensus", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCASI frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCASI novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcasi_6_sequential_economics_v1", "policy_id": POLICY_ID,
