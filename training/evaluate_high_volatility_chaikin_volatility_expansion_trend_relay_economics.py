"""Sequential strict economics for frozen HVCHV-12."""
...
NOVELTY = Path("results/high_volatility_chaikin_volatility_expansion_trend_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "c6bc4d3d45f0b694668de9e9ffe629e2fb0e9afe41ffc39824e4d4554fc873c6"
...
FREEZE = Path("results/high_volatility_chaikin_volatility_expansion_trend_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_chaikin_volatility_expansion_trend_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = (
    "no_chaikin_gate",
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCHV frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCHV novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvchv_12_sequential_economics_v1", "policy_id": POLICY_ID,
