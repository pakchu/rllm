"""Evaluate frozen HVCATCR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCATCR-8"
PROTOCOL = "hvcatcr_8_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_cross_alt_extreme_traversal_consensus_relay_preregistration_2026-08-12.json"
)
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCATCR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVCATCR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCATCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCATCR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcatcr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVCATCR-8 unchanged before economic outcomes"
        ),
    }
