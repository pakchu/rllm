"""Sequential strict economics for frozen HVDVTM-8."""
...
PREREG = Path("results/high_volatility_directional_variation_timing_migration_relay_preregistration_2026-08-10.json")
PREREG_SHA = "7f9e37d31cc07c76c29bd3630b162f948ed2bf246476d9439372b8e428858dfb"
SUPPORT = Path("results/high_volatility_directional_variation_timing_migration_relay_support_2026-08-10.json")
SUPPORT_SHA = "06a9737acdc0289bdec95ceb7ade3bdf9eb13c463a812ab0ceaf8e2878281215"
NOVELTY = Path("results/high_volatility_directional_variation_timing_migration_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "1dc5ef0b65a88353b39dda9a26ee6b6064fcea31441fc5463003e49828584bf7"
CLOCK = Path("data/high_volatility_directional_variation_timing_migration_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "a72957ff98e9ea8150157b90de65e4409cd67699badc1bf1d956f4728dd967f0"
CONTROL_DIR = Path("data/high_volatility_directional_variation_timing_migration_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_directional_variation_timing_migration_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_directional_variation_timing_migration_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_timing_tail", "no_variation_gate", "directional_variation_mass_asymmetry", "one_block_stale_timing", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDVTM frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDVTM novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdvtm_8_sequential_economics_v1", "policy_id": POLICY_ID,
