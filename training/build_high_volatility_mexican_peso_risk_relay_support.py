"""Materialize source-only HVMXRR-8 support clocks."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_mexican_peso_risk_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv

ENV_FILE="/home/pakchu/rllm/.env"; PREREG_SHA="c9cc539b886240748f858c8fe60cf44f179e20d834623a8900b06db0c1af63db"; START=pd.Timestamp("2023-01-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
SOURCE_DIR=Path("data/high_volatility_mexican_peso_risk_relay_sources_2023_2026"); SESSION=SOURCE_DIR/"dollar_breadth_sessions.csv.gz"; SOURCE_MANIFEST=SOURCE_DIR/"manifest.json"
...
CLOCK=Path("data/high_volatility_mexican_peso_risk_relay_clocks_2023_2026.csv.gz"); CONTROL_DIR=Path("data/high_volatility_mexican_peso_risk_relay_controls_2023_2026"); RESULT=Path("results/high_volatility_mexican_peso_risk_relay_support_2026-08-13.json")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00:00Z"),pd.Timestamp("2024-01-01T00:00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00:00Z"),pd.Timestamp("2025-01-01T00:00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00:00Z"),pd.Timestamp("2026-01-01T00:00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00:00Z"),pd.Timestamp("2026-08-01T00:00:00Z"))}; MINIMUM={"train":8,"test":12,"eval":12,"final":8}; CONTROLS=("no_variation_gate","no_shock_tail","one_session_stale_peso","direction_flip","forced_long")
SYMBOLS=("USDMXN",)
COLUMNS=("candidate","control","split","source_day","decision_time","feature_available_time","entry_time","exit_time","side","peso_risk_direction","session_return","shock_rank","btc_realized_variation","btc_realized_variation_rank")
...
def materialize_sessions()->dict:
 ...
 db=postgres_engine()
 with db.connect() as connection:raw=pd.read_sql_query(text(QUERY),connection,params={"symbol":"USDMXN","start":START.to_pydatetime(),"end":END.to_pydatetime()})
 ...
 frame=pd.DataFrame({"source_day":raw.source_day,"source_valid":valid,"session_return":np.log(raw.session_close/raw.session_open).where(valid)});frame["shock_rank"]=strict_prior_midrank(frame.session_return.abs().where(frame.source_valid));frame["peso_risk_direction"]=-np.sign(frame.session_return).fillna(0).astype(int);frame["decision_time"]=frame.source_day+pd.Timedelta(hours=21)
 SOURCE_DIR.mkdir(parents=True,exist_ok=True);_write_gzip_csv(frame,SESSION);core={"protocol_version":"hvmxrr_8_peso_source_v1","query":QUERY,"table":"bars_polygon","symbols":["USDMXN"],"interval":"1m","window":[START.isoformat(),END.isoformat()],"outcomes_opened":False,"candidate_incidence_opened":False,"no_imputation":True,"output":{"path":str(SESSION),"sha256":sha(SESSION),"rows":len(frame),"valid_rows":int(frame.source_valid.sum())}};payload={**core,"manifest_hash":chash(core)};SOURCE_MANIFEST.write_text(json.dumps(payload,indent=2,allow_nan=False)+"\n");return payload
def features()->pd.DataFrame:
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVMXRR prereg drift")
 if sha(PRICE)!=PRICE_SHA or sha(PRICE_MANIFEST)!=PRICE_MANIFEST_SHA:raise RuntimeError("HVMXRR BTC source drift")
 ...
 for column in ("peso_risk_direction","session_return","shock_rank"):f[column]=pd.to_numeric(f[column],errors="coerce")
 ...
 d=p[["decision_time","btc_realized_variation","btc_valid"]];f=f.merge(d,on="decision_time",how="left",validate="one_to_one");f["btc_realized_variation_rank"]=strict_prior_midrank(f.btc_realized_variation.where(f.btc_valid));f["signal_valid"]=f.source_valid&f.btc_valid&np.isfinite(f[["peso_risk_direction","session_return","shock_rank","btc_realized_variation","btc_realized_variation_rank"]]).all(axis=1)&f.peso_risk_direction.abs().eq(1);return f
def conditions(f:pd.DataFrame,control:str)->tuple[pd.Series,pd.Series]:
 direction=f.peso_risk_direction;shock=f.shock_rank
 if control=="one_session_stale_peso":direction=direction.shift(1);shock=shock.shift(1)
...
def clock(f:pd.DataFrame,control:str="primary")->pd.DataFrame:
 ...
 for i in f.index[active]:
  ...
  next_allowed=exit_;rows.append({"candidate":"HVMXRR-8","control":control,"split":split,"source_day":f.at[i,"source_day"],"decision_time":decision,"feature_available_time":decision,"entry_time":entry,"exit_time":exit_,"side":int(side.at[i]),"peso_risk_direction":int(f.at[i,"peso_risk_direction"]),"session_return":float(f.at[i,"session_return"]),"shock_rank":float(f.at[i,"shock_rank"]),"btc_realized_variation":float(f.at[i,"btc_realized_variation"]),"btc_realized_variation_rank":float(f.at[i,"btc_realized_variation_rank"])})
