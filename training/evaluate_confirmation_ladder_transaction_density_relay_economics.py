"""Sequential strict economics for frozen CLTDR-6."""
...
PREREG = Path("results/confirmation_ladder_transaction_density_relay_preregistration_2026-08-09.json")
PREREG_SHA = "b5066cd48f88dc4f5401379c0e0492c016d1fb92f841ce2f4dba7211a63f9ac9"
SUPPORT = Path("results/confirmation_ladder_transaction_density_relay_support_2026-08-09.json")
SUPPORT_SHA = "52abe6a7877d34f590579f8a1e63dc7c666410bbf2edf12644542ad3f84a271b"
NOVELTY = Path("results/confirmation_ladder_transaction_density_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "ce5e7ca478a3b3b6f542f117c2527ea2724f6f8903e88001a27e704d41ec2ca2"
CLOCK = Path("data/confirmation_ladder_transaction_density_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "0e578407e393b56120a4ee4901a7960484cef18c2c5f165f037a43d7851c5cfd"
CONTROL_DIR = Path("data/confirmation_ladder_transaction_density_relay_controls_2023_2026")
FREEZE = Path("results/confirmation_ladder_transaction_density_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/confirmation_ladder_transaction_density_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CLTDR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CLTDR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "cltdr_6_sequential_economics_v1", "policy_id": POLICY_ID,
