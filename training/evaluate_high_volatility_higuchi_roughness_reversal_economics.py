"""Sequential strict economics for frozen HVHRR-8."""
...
PREREG = Path("results/high_volatility_higuchi_roughness_reversal_preregistration_2026-08-10.json")
PREREG_SHA = "508e50e7c9331b39fe8486f98340ac0816651b3be6080dfc421b5e8a751847b3"
SUPPORT = Path("results/high_volatility_higuchi_roughness_reversal_support_2026-08-10.json")
SUPPORT_SHA = "a6ab6bbca4249367b9ebf025c0dfadac05b5f826704a89538600e2d53b6fa073"
NOVELTY = Path("results/high_volatility_higuchi_roughness_reversal_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "f2ac9772141e801792e217932e422a294b694be4b8dd35d3e5d5cbd5be4479f3"
CLOCK = Path("data/high_volatility_higuchi_roughness_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "49331ea03b994ed0a10f84c14df9e3cf604e94eb61f1d47c5e0055c3e08feadd"
CONTROL_DIR = Path("data/high_volatility_higuchi_roughness_reversal_controls_2023_2026")
FREEZE = Path("results/high_volatility_higuchi_roughness_reversal_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_higuchi_roughness_reversal_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_roughness_tail", "no_variation_gate", "raw_dimension_above_one_half", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVHRR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVHRR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvhrr_8_sequential_economics_v1", "policy_id": POLICY_ID,
