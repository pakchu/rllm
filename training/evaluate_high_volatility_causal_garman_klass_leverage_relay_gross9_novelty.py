"""Evaluate the frozen HVCGKR-24 action-vote clock against Gross9."""
...
POLICY = "HVCGKR-24"
PROTOCOL = "hvcgkr_24_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_garman_klass_leverage_relay_clocks_2020_2026.csv.gz",
    "sha256": "9f49ead8c652eae9ab1164c93296a2ddf585a934c55d01a3242db22130655802",
    "rows": 104,
}
OUTPUT = Path("results/high_volatility_causal_garman_klass_leverage_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCGKR-24 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCGKR-24 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCGKR-24 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCGKR-24 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCGKR-24 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCGKR-24 unchanged before economics",
    }
