"""Materialize outcome-blind source support for frozen HVEBPR-24."""
...
from training import preregister_high_volatility_equity_breadth_participation_relay as prereg
...
BUILDER = Path("training/build_high_volatility_equity_breadth_participation_relay_support.py")
PREREG_SHA256 = "d931804a571acf46ef6c58dc2f4f774ccacb364c9fc74f0fb91549eb37686b90"
SYMBOLS = ("RSP", "SPY")
SOURCE_DIR = Path("data/high_volatility_equity_breadth_participation_relay_sources_2022_2026")
...
FEATURE_PANEL = SOURCE_DIR / "hvebpr_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_equity_breadth_participation_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_equity_breadth_participation_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_equity_breadth_participation_relay_support_2026-08-12.json")
...
def normalize_yahoo_chart(payload: bytes, symbol: str) -> tuple[bytes, pd.DataFrame, dict[str, Any]]:
    ...
    if chart.get("error") is not None:
        raise RuntimeError(f"HVEBPR Yahoo chart error for {symbol}")
    results = chart.get("result") or []
    if len(results) != 1:
        raise RuntimeError(f"HVEBPR Yahoo result count drift for {symbol}")
    result = results[0]
    meta = result.get("meta") or {}
    if meta.get("symbol") != symbol or meta.get("exchangeTimezoneName") != "America/New_York":
        raise RuntimeError(f"HVEBPR {symbol} Yahoo metadata drift")
    timestamps = result.get("timestamp") or []
    indicators = result.get("indicators") or {}
    quotes = indicators.get("quote") or []
    adjusted = indicators.get("adjclose") or []
    if len(quotes) != 1 or len(adjusted) != 1 or not timestamps:
        raise RuntimeError(f"HVEBPR {symbol} adjusted chart incomplete")
    quote = quotes[0]
    ...
    if any(values is None or len(values) != len(timestamps) for values in vectors.values()):
        raise RuntimeError(f"HVEBPR {symbol} Yahoo vector length drift")
    ...
    if frame.empty or frame.session_date.duplicated().any():
        raise RuntimeError(f"HVEBPR {symbol} session dates invalid")
    ...
    if not valid.all():
        ...
        raise RuntimeError(f"HVEBPR {symbol} invalid adjusted rows: {dates[:5]}")
    metadata = {
        "symbol": symbol,
...
def build_equity_panel(equities: Mapping[str, pd.DataFrame]) -> pd.DataFrame:
    if set(equities) != set(SYMBOLS):
        raise RuntimeError("HVEBPR equity symbol set drift")
    ...
    for symbol in SYMBOLS:
        ...
        if not observed.equals(official):
            ...
            raise RuntimeError(
                f"HVEBPR {symbol}/NYSE calendar mismatch missing={missing[:5]} extra={extra[:5]}"
            )
        if expected is None:
            expected = observed
        elif not observed.equals(expected):
            raise RuntimeError(f"HVEBPR {symbol} does not share every official common session")
    ...
    if len(panel) != len(expected):
        raise RuntimeError("HVEBPR common-session intersection dropped an official session")
    ...
    if observed_early != expected_early:
        raise RuntimeError("HVEBPR frozen early-close schedule drift")
    return panel
...
def normalize_btc_bars(
    ...
    if raw.columns.tolist() != ["ts", "open", "close"]:
        raise RuntimeError("HVEBPR BTC schema drift")
    frame = raw.copy()
    ...
    if len(frame) != len(expected) or not frame.ts.equals(pd.Series(expected, name="ts")):
        raise RuntimeError("HVEBPR BTC source is not the exact requested 1m grid")
    for column in ("open", "close"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    values = frame[["open", "close"]]
    if not np.isfinite(values.to_numpy(dtype=float)).all() or not values.gt(0.0).all(axis=None):
        raise RuntimeError("HVEBPR BTC prices invalid")
    return frame
...
def build_features(sessions: pd.DataFrame, bars: pd.DataFrame) -> pd.DataFrame:
    ...
    if not isinstance(indexed.index, pd.DatetimeIndex) or indexed.index.tz is None:
        raise RuntimeError("HVEBPR BTC index must be timezone-aware")
    ...
    frame["breadth_return"] = frame.rsp_return - frame.spy_return
    frame["breadth_shock_rank"] = strict_prior_midrank(
        frame.breadth_return.abs(), lookback=252, minimum=126
    )
...
def _signal(frame: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    score = frame.breadth_return.copy()
    active = score.notna() & score.ne(0.0) & frame.breadth_shock_rank.ge(0.70)
    if control == "one_session_stale_breadth":
        score, active = score.shift(1), active.shift(1, fill_value=False)
    elif control == "rsp_return_only":
        score = frame.breadth_return
        score = frame.rsp_return
        active = score.notna() & score.ne(0.0) & frame.breadth_shock_rank.ge(0.70)
    elif control == "no_breadth_shock_gate":
        ...
        rows.append(
            {
                ...
                "rsp_return": float(frame.at[index, "rsp_return"]),
                "spy_return": float(frame.at[index, "spy_return"]),
                "breadth_return": float(frame.at[index, "breadth_return"]),
                "breadth_shock_rank": float(frame.at[index, "breadth_shock_rank"]),
...
def run(env_file: str = ENV_FILE) -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA256:
        raise RuntimeError("HVEBPR preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvebpr_24_sources_v1",
    ...
    core = {
        "protocol_version": "hvebpr_24_source_support_v1",
        "policy_id": "HVEBPR-24",
        "preregistration": {
