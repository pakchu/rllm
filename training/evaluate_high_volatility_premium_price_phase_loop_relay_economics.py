"""Sequential strict economics for frozen HVPPL-8."""
...
PREREG = Path("results/high_volatility_premium_price_phase_loop_relay_preregistration_2026-08-13.json")
PREREG_SHA = "9af958f3414f44cc540f6d520cb0b3de0ebae268fabb5d354b67f66d3b0619e8"
SUPPORT = Path("results/high_volatility_premium_price_phase_loop_relay_support_2026-08-13.json")
SUPPORT_SHA = "6b4c58796288407d833867b800bf45fd7934548a0044fc5d7b4020f8713131d0"
NOVELTY = Path("results/high_volatility_premium_price_phase_loop_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "1babbe86672d8a5ddd08c23cbab4c25110643d8097fae3668c472c60a2aae7fc"
CLOCK = Path("data/high_volatility_premium_price_phase_loop_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "da7a4b90cbecc756209f36fb219d64aaf73c5bc6e8318ed6fefde4ec8d80284e"
CONTROL_DIR = Path("data/high_volatility_premium_price_phase_loop_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_premium_price_phase_loop_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_premium_price_phase_loop_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_area_tail", "no_variation_gate", "either_loop_orientation", "one_decision_stale_loop", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVPPL frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVPPL novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvppl_8_sequential_economics_v1", "policy_id": POLICY_ID,
