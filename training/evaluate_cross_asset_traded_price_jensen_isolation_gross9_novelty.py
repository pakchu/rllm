"""Evaluate frozen CATPJI-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "CATPJI-6"
PROTOCOL = "catpji_6_gross9_novelty_v1"
PREREG = Path("results/cross_asset_traded_price_jensen_isolation_preregistration_2026-08-09.json")
PREREG_SHA = "7ee34adfca61a01a46151e5932ea65643a8b4c1336bdc73104e7bd044833f440"
SUPPORT = Path("results/cross_asset_traded_price_jensen_isolation_support_2026-08-09.json")
SUPPORT_SHA = "dcacd8669807d7c5ed85b8be3cf9e8a659bc09fa63f4c984727e78866eada901"
CLOCK = Path("data/cross_asset_traded_price_jensen_isolation_clocks_2023_2026.csv.gz")
CLOCK_SHA = "1ceb6a25e1683c2c7cfb4395597fd7c417aeddffd7bc31064deb917e83be13ef"
OUTPUT = Path("results/cross_asset_traded_price_jensen_isolation_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CATPJI predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("CATPJI novelty limits drift")
    if (
        ...
        raise RuntimeError("CATPJI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CATPJI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "catpji_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject CATPJI-6 unchanged before economic outcomes",
    }
