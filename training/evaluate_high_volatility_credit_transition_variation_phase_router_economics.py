"""Strict staged economics for the singleton HVCQVAR-24 policy."""
...
PREREG = Path("results/high_volatility_credit_transition_variation_phase_router_preregistration_2026-08-18.json")
PREREG_SHA = "f11c7a2a5b02f8e1b2cdddaccf98d66c7943d29e3e271a36119096d50dfc8559"
SUPPORT = Path("results/high_volatility_credit_transition_variation_phase_router_support_2026-08-18.json")
SUPPORT_SHA = "4a2eff24ff07354f1c3b4f67ee62dbbf78f453def7544808026ba6a78cb82038"
GROSS9 = Path("results/high_volatility_credit_transition_variation_phase_router_gross9_novelty_2026-08-18.json")
GROSS9_SHA = "540df8f1d1074f437b8c3fddc7269231b421f9431eef7e4655c9c55f8acba8e0"
CLOCK = {
    "path": Path("data/high_volatility_credit_transition_variation_phase_router_clocks_2023_2026.csv.gz"),
    "sha256": "7a2d8c67934c2c1b8f46f1bb6c0782b4c1bb80d3bb139a5966be50b9c178c75c",
    "rows": 199,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_credit_transition_variation_phase_router_{stage}_economics_2026-08-18.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCQVAR-24 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCQVAR-24 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCQVAR-24 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCQVAR-24 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCQVAR-24 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCQVAR-24 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCQVAR-24 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCQVAR-24 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCQVAR-24 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCQVAR-24 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCQVAR-24 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcqvar_24_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
