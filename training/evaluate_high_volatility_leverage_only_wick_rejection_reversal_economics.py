"""Sequential strict economics for frozen HVLOWR-8."""
...
PREREG = Path("results/high_volatility_leverage_only_wick_rejection_reversal_preregistration_2026-08-13.json")
PREREG_SHA = "fe53b114eef4515f29aa4f34a07bea3cfc48c8127640edda34daa4708aa88470"
SUPPORT = Path("results/high_volatility_leverage_only_wick_rejection_reversal_support_2026-08-13.json")
SUPPORT_SHA = "7fcc2c7d647044570a0d076969d709266308dbb0d75b9cd32b687fcb8059a0d1"
NOVELTY = Path("results/high_volatility_leverage_only_wick_rejection_reversal_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "fc38e13e1cb3b41186c4de48bb4c259e4b976cf8482ed4cbc8d1d0ce602320c1"
CLOCK = Path("data/high_volatility_leverage_only_wick_rejection_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "b01bb54b87e2927cd1ff4a5b60467896c577fcb90166511084225d4930fe2be1"
CONTROL_DIR = Path("data/high_volatility_leverage_only_wick_rejection_reversal_controls_2023_2026")
FREEZE = Path("results/high_volatility_leverage_only_wick_rejection_reversal_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_leverage_only_wick_rejection_reversal_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "rank_0_99", "no_close_nonconfirmation", "one_bar_stale_rejection", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVLOWR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVLOWR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvlowr_8_sequential_economics_v1", "policy_id": POLICY_ID,
