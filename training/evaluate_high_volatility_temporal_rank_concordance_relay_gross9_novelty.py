"""Evaluate frozen HVTRC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVTRC-8"
PROTOCOL = "hvtrc_8_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_temporal_rank_concordance_relay_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTRC predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVTRC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTRC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTRC primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvtrc_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVTRC-8 unchanged before economic outcomes"
        ),
    }
