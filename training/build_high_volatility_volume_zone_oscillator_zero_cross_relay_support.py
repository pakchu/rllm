"""Build outcome-blind source support for frozen HVVZO-24."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_volume_zone_oscillator_zero_cross_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2020-01-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="8225f5c608ad58448507b98b24db7497f29b3fbb3e809063a8725db6716ba734";REG=prereg.build();P=REG["policy"]
...
ROOT=Path("data/high_volatility_volume_zone_oscillator_zero_cross_relay_sources_2023_2026");PANEL=ROOT/"four_hour_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_volume_zone_oscillator_zero_cross_relay_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_volume_zone_oscillator_zero_cross_relay_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_volume_zone_oscillator_zero_cross_relay_controls_2023_2026");RESULT=Path("results/high_volatility_volume_zone_oscillator_zero_cross_relay_support_2026-08-11.json");BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("source_start","feature_available_time","source_valid","bar_open","bar_high","bar_low","bar_close","base_volume","quote_asset_volume","signed_volume","quote_signed_volume","volume_ema","quote_volume_ema","vzo","quote_vzo","variation","variation_rank","eligible")
CLOCK_COLUMNS=("candidate","control","split","source_start","feature_available_time","entry_time","exit_time","side","bar_close","base_volume","quote_asset_volume","signed_volume","quote_signed_volume","volume_ema","quote_volume_ema","vzo","quote_vzo","variation","variation_rank","eligible")
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 expected=["ts","open","high","low","close","volume","quote_asset_volume"]
 if frame.columns.tolist()!=expected:raise RuntimeError("HVVZO source schema drift")
 x=frame.copy();x["ts"]=pd.to_datetime(x.ts,utc=True,errors="coerce")
 for c in expected[1:]:x[c]=pd.to_numeric(x[c],errors="coerce")
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVVZO invalid source key")
 prices=x[["open","high","low","close"]];x["row_valid"]=np.isfinite(prices).all(axis=1)&prices.gt(0).all(axis=1)&x.high.ge(prices[["open","close"]].max(axis=1))&x.low.le(prices[["open","close"]].min(axis=1))&x.high.ge(x.low)&np.isfinite(x[["volume","quote_asset_volume"]]).all(axis=1)&x.volume.ge(0)&x.quote_asset_volume.ge(0);x["minute_sq_return"]=np.square(np.log(x.close/x.open)).where(x.row_valid);return x.set_index("ts").sort_index()

def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 source=prepare(raw).reindex(pd.date_range(START,END,freq="1min",inclusive="left"));g=source.groupby(source.index.floor("4h"),sort=True);bars=pd.DataFrame({"rows":g.row_valid.sum(),"bar_open":g.open.first(),"bar_high":g.high.max(),"bar_low":g.low.min(),"bar_close":g.close.last(),"base_volume":g.volume.sum(min_count=240),"quote_asset_volume":g.quote_asset_volume.sum(min_count=240),"variation_component":g.minute_sq_return.sum(min_count=240)});bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[["bar_open","bar_high","bar_low","bar_close","base_volume","quote_asset_volume"]]).all(axis=1)&bars[["bar_open","bar_high","bar_low","bar_close"]].gt(0).all(axis=1)&bars.base_volume.ge(0)&bars.quote_asset_volume.ge(0);transition=bars.valid_bar&bars.valid_bar.shift(1,fill_value=False);direction=np.sign(bars.bar_close-bars.bar_close.shift(1));bars["signed_volume"]=(direction*bars.base_volume).where(transition);bars["quote_signed_volume"]=(direction*bars.quote_asset_volume).where(transition);alpha=2/(P["ema_periods"]+1);bars["signed_volume_ema"]=segmented_ewm(bars.signed_volume,transition,alpha,P["ema_periods"]);bars["quote_signed_volume_ema"]=segmented_ewm(bars.quote_signed_volume,transition,alpha,P["ema_periods"]);bars["volume_ema"]=segmented_ewm(bars.base_volume,transition,alpha,P["ema_periods"]);bars["quote_volume_ema"]=segmented_ewm(bars.quote_asset_volume,transition,alpha,P["ema_periods"]);bars["vzo"]=100*bars.signed_volume_ema/bars.volume_ema.where(bars.volume_ema.gt(0));bars["quote_vzo"]=100*bars.quote_signed_volume_ema/bars.quote_volume_ema.where(bars.quote_volume_ema.gt(0));bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["signed_volume","quote_signed_volume","volume_ema","quote_volume_ema","vzo","quote_vzo","variation"]]).all(axis=1)&bars.volume_ema.gt(0)&bars.quote_volume_ema.gt(0)&bars.variation.gt(0);panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.vzo.shift(1);long=previous.le(P["zero_level"])&panel.vzo.gt(P["zero_level"]);short=previous.ge(P["zero_level"])&panel.vzo.lt(P["zero_level"]);panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&(long|short)&panel.variation_rank.ge(P["variation_rank_min"]);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);value=pd.to_numeric(used.quote_vzo if control=="quote_volume_vzo" else used.vzo,errors="coerce");previous=value.shift(1);long=previous.le(P["zero_level"])&value.gt(P["zero_level"]);short=previous.ge(P["zero_level"])&value.lt(P["zero_level"]);side=pd.Series(np.where(long,1,np.where(short,-1,0)),index=used.index,dtype=int);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&side.ne(0)&variation
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVVZO prereg drift")
