"""Evaluate the frozen HVCELV-8 action-vote clock against Gross9."""
...
POLICY = "HVCELV-8"
PROTOCOL = "hvcelv_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_caspc_ehlers_active_veto_clocks_2023_2026.csv.gz",
    "sha256": "1f8512a74906921169b115ec917c80c76b714c5a2f5fda25698feb6b4b026294",
    "rows": 130,
}
OUTPUT = Path("results/high_volatility_caspc_ehlers_active_veto_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCELV-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCELV-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCELV-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCELV-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCELV-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCELV-8 unchanged before economics",
    }
