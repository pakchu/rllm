"""Sequential strict economics for frozen HVCMF-24."""
...
PREREG = Path("results/high_volatility_chaikin_money_flow_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "fec6dfe1ba4de3953743933835a90580bc5643d038722ca1655ccb4973fcbf4b"
SUPPORT = Path("results/high_volatility_chaikin_money_flow_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "840b59d05328c328573cb3e9a49832f2692f4c521b324d765bc97599a7f7b6b1"
NOVELTY = Path("results/high_volatility_chaikin_money_flow_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "cee1bc5fc049fa71ec8bdac64dcb90bdc486028ef11b03f536179f7ac9ed8e8e"
CLOCK = Path("data/high_volatility_chaikin_money_flow_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "83c468d95cfc6b326ce51ab65094d2255b8402b8e27a04f6ef62808abe3e91c4"
CONTROL_DIR = Path("data/high_volatility_chaikin_money_flow_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_chaikin_money_flow_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_chaikin_money_flow_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "unweighted_close_location", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCMF frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCMF novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcmf_24_sequential_economics_v1", "policy_id": POLICY_ID,
