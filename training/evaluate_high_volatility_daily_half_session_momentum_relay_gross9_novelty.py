"""Evaluate frozen HVDHSM-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDHSM-12"
PROTOCOL = "hvdhsm_12_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_daily_half_session_momentum_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDHSM predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVDHSM novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDHSM predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDHSM primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdhsm_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVDHSM-12 unchanged before economic outcomes",
    }
