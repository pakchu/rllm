"""Sequential strict economics for frozen HVETCR-48."""
...
PREREG = Path("results/high_volatility_extra_trees_classification_relay_preregistration_2026-08-09.json")
PREREG_SHA = "2ebcc54789d06d97cd689fab600bb7cb547888c0e6f56ff77472db0a27a7e1ba"
SUPPORT = Path("results/high_volatility_extra_trees_classification_relay_support_2026-08-09.json")
SUPPORT_SHA = "03beb3e3c97e8c97eb82524007128f77b54012475a522f3a664b63c8587bb787"
NOVELTY = Path("results/high_volatility_extra_trees_classification_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "4726916c053ea6948ac3c693a90c74760c9cfbccca82265d3129f58efcf8131a"
CLOCK = Path("data/high_volatility_extra_trees_classification_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "36659ce3a1d680af31397ac8bf6e933458a360e5092b0357120594d278947544"
CONTROL_DIR = Path("data/high_volatility_extra_trees_classification_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_extra_trees_classification_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_extra_trees_classification_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVETCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVETCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvetcr_48_sequential_economics_v1", "policy_id": POLICY_ID,
