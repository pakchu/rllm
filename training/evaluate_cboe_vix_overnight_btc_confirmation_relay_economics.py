"""Sequential strict economics for frozen CVOBR-6."""
...
PREREG = Path("results/cboe_vix_overnight_btc_confirmation_relay_preregistration_2026-08-08.json")
PREREG_SHA = "76a770729e4cb4eaa8915a8751632cbb0020a8e8b1cba4d09f7fec53616e09bb"
SUPPORT = Path("results/cboe_vix_overnight_btc_confirmation_relay_support_2026-08-08.json")
SUPPORT_SHA = "f42e7d3447fdd65eba2f8fac986f66a9b3a1130013177b1d2d517733978d6540"
NOVELTY = Path("results/cboe_vix_overnight_btc_confirmation_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "ccf0a32b2b86ad8c6f93c459ca0cdf323a4801df5e8e1f13c211a57f0385730f"
CLOCK = Path("data/cboe_vix_overnight_btc_confirmation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "a23578f32046fc8247c1df3b42697eda1ea864c51d868efbb83bbc572414f98a"
CONTROL_DIR = Path("data/cboe_vix_overnight_btc_confirmation_relay_controls_2023_2026")
FREEZE = Path("results/cboe_vix_overnight_btc_confirmation_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/cboe_vix_overnight_btc_confirmation_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CVOBR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CVOBR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "cvobr_6_sequential_economics_v1", "policy_id": POLICY_ID,
