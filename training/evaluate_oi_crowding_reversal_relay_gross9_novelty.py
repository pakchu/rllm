"""Evaluate frozen OICRR-18 structural novelty against every Gross9 sleeve."""
...
POLICY = "OICRR-18"
PROTOCOL = "oicrr_18_gross9_novelty_v1"
PREREG = Path(
    "results/oi_crowding_reversal_relay_preregistration_2026-08-09.json"
)
...
OUTPUT = Path(
    "results/oi_crowding_reversal_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("OICRR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("OICRR novelty limits drift")
    if (
        ...
        raise RuntimeError("OICRR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="OICRR primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject OICRR-18 unchanged before economic outcomes"
        ),
    }
