"""Evaluate frozen HVBBC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVBBC-8"
PROTOCOL = "hvbbc_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_bilateral_barrier_completion_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVBBC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVBBC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVBBC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVBBC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvbbc_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVBBC-8 unchanged before economic outcomes",
    }
