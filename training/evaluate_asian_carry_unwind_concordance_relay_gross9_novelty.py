"""Evaluate frozen ACUCR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "ACUCR-12"
PROTOCOL = "acucr_6_gross9_novelty_v1"
...
OUTPUT = Path("results/asian_carry_unwind_concordance_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("ACUCR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("ACUCR novelty limits drift")
    if (
        ...
        raise RuntimeError("ACUCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="ACUCR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "acucr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject ACUCR-12 unchanged before economic outcomes",
    }
