"""Strict staged economics for the singleton HVCATCLIR-8 policy."""
...
PREREG = Path("results/high_volatility_causal_average_ticket_close_location_inventory_relay_preregistration_2026-08-16.json")
PREREG_SHA = "9058d54ff2cad323358b048a9a1baea6a0881b411f96a04774c02528a4e4faca"
SUPPORT = Path("results/high_volatility_causal_average_ticket_close_location_inventory_relay_support_2026-08-16.json")
SUPPORT_SHA = "f5eadb93992e7a6a5085f93072e75767ebf0bd6a7fb0898cb479445d5574d1fd"
GROSS9 = Path("results/high_volatility_causal_average_ticket_close_location_inventory_relay_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "daa961f1cd49c1e058b521161c76d896ce25a6e4b9acd5290b66836c6db3fd88"
CLOCK = {
    "path": Path("data/high_volatility_causal_average_ticket_close_location_inventory_relay_clocks_2020_2026.csv.gz"),
    "sha256": "754949b6b3f12b1e82a3d66d470d1a45287e86a8c983661948cb534d04c26785",
    "rows": 112,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_average_ticket_close_location_inventory_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCATCLIR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCATCLIR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCATCLIR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCATCLIR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATCLIR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATCLIR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCATCLIR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCATCLIR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCATCLIR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCATCLIR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCATCLIR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcatclir_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
