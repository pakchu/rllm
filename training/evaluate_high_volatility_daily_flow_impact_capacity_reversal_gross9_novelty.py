"""Evaluate frozen HVDFICR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDFICR-12"
PROTOCOL = "hvdfcr_12_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_daily_flow_impact_capacity_reversal_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDFICR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVDFICR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDFICR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDFICR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdfcr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVDFICR-12 unchanged before economic outcomes",
    }
