"""Build outcome-blind source support for frozen HVCAWCR-8."""
...
from training import preregister_high_volatility_cross_alt_wick_consensus_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-04-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="87ce1a7cec438782646a3814a2115ae4f0918275dd51a09860b3cbca3dcb2ab8"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_cross_alt_wick_consensus_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_cross_alt_wick_consensus_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_cross_alt_wick_consensus_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_cross_alt_wick_consensus_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_cross_alt_wick_consensus_relay_support_2026-08-10.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","consensus_pressure","strength_rank","equal_weight_pressure","consensus_breadth","realized_variation","variation_rank","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","consensus_pressure","strength_rank","equal_weight_pressure","consensus_breadth","realized_variation","variation_rank")
...
def wick_consensus_statistics(block:pd.DataFrame)->tuple[float,float,int,float]:
 pressures=[]
 for symbol in ALTS:
  x=block.xs(symbol,level="symbol");upper=np.log(x.high.to_numpy(float)/np.maximum(x.open.to_numpy(float),x.close.to_numpy(float))).sum();lower=np.log(np.minimum(x.open.to_numpy(float),x.close.to_numpy(float))/x.low.to_numpy(float)).sum();den=upper+lower
  if not math.isfinite(den) or den<=0:return math.nan,math.nan,0,math.nan
  p=float((lower-upper)/den)
  if not math.isfinite(p) or p==0:return math.nan,math.nan,0,math.nan
  pressures.append(p)
 median=float(np.median(pressures));side=int(np.sign(median));breadth=sum(int(np.sign(x))==side for x in pressures);equal=float(np.sum(pressures))
 ...
 return median,equal,breadth,variation
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","symbol","open","high","low","close"]:raise RuntimeError("HVCAWCR source schema drift")
 ...
 if x.ts.isna().any() or x.duplicated(["ts","symbol"]).any() or not x.symbol.isin(SYMBOLS).all():raise RuntimeError("HVCAWCR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("5h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   pressure,equal,breadth,var=wick_consensus_statistics(block);valid=all(math.isfinite(v) for v in (pressure,equal,var)) and pressure!=0 and var>0 and breadth>=P["minimum_consensus_breadth"]
  else:pressure=equal=var=math.nan;breadth=0;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"consensus_pressure":pressure,"equal_weight_pressure":equal,"consensus_breadth":breadth,"realized_variation":var})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["strength_rank"]=prior_rank(panel.consensus_pressure.abs().where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));panel["eligible"]=valid&panel.strength_rank.ge(P["strength_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"]);panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_boundary_stale_geometry":
  cols=["source_valid","consensus_pressure","strength_rank","equal_weight_pressure","consensus_breadth","variation_rank","feature_available_time"];used[cols]=panel[cols].shift(1)
 valid=used.source_valid.eq(True);strong=used.strength_rank.ge(P["strength_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&strong&variation
 if control=="no_strength_tail":state=valid&variation
 elif control=="no_variation_gate":state=valid&strong
 onset=previous_valid_onset(state,valid);side=np.sign(pd.to_numeric(used.consensus_pressure,errors="coerce").fillna(0)).astype(int)
 if control=="equal_weight_signed_wick_sum":side=np.sign(pd.to_numeric(used.equal_weight_pressure,errors="coerce").fillna(0)).astype(int)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCAWCR prereg drift")
 ...
 source_core={"protocol_version":"hvcawcr_8_source_v1","query":QUERY,"query_sha256":hashlib.sha256(QUERY.encode()).hexdigest(),"window":[START.isoformat(),END.isoformat()],"physical_rows":len(raw),"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"panel":{"path":str(PANEL),"sha256":sha(PANEL),"rows":len(panel),"valid_rows":int(panel.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};manifest={**source_core,"manifest_hash":chash(source_core)};immutable(MANIFEST,json_bytes(manifest))
