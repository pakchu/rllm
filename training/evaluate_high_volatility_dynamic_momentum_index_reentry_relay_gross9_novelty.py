"""Evaluate frozen HVDYMI-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDYMI-24"
PROTOCOL = "hvdymi_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_dynamic_momentum_index_reentry_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDYMI predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVDYMI novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDYMI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDYMI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdymi_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVDYMI-24 unchanged before economic outcomes",
    }
