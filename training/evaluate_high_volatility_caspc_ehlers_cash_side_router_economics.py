"""Sequential strict economics for frozen HVCELVCSR-8."""
...
POLICY_ID = "HVCELVCSR-8"
BAR = pd.Timedelta(minutes=5)
LEVERAGE = 0.5
BASE_COST = 0.0006
STRESS_COST = 0.0010
ENV_FILE = "/home/pakchu/rllm/.env"
PREREG = Path("results/high_volatility_caspc_ehlers_cash_side_router_preregistration_2026-08-16.json")
PREREG_SHA = "f5ba5f811001ee26bee38579d8528e142ab1ddfbdf8cc3e0f64efcdee615667f"
SUPPORT = Path("results/high_volatility_caspc_ehlers_cash_side_router_support_2026-08-16.json")
SUPPORT_SHA = "d4f90b3f3c2d8b5d1beb12d77f9e5c5cf3f58412cc0b5ce870ff1e0af8f9885b"
NOVELTY = Path("results/high_volatility_caspc_ehlers_cash_side_router_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "4cb8e8b60492ed3f8b01e1817b74316de6b7676c9d0d889e4b9795e09338af8d"
CLOCK = Path("data/high_volatility_caspc_ehlers_cash_side_router_clocks_2023_2026.csv.gz")
CLOCK_SHA = "a74cb23179ce75ebc92d8898bc597ff55875692bdd8df1b17c24701fa6cd9640"
CONTROL_DIR = Path("data/high_volatility_caspc_ehlers_cash_side_router_controls_2023_2026")
FREEZE = Path("results/high_volatility_caspc_ehlers_cash_side_router_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_caspc_ehlers_cash_side_router_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCELVCSR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCELVCSR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcelvcsr_8_sequential_economics_v1", "policy_id": POLICY_ID,
