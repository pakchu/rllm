"""Evaluate frozen HVMFCR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVMFCR-8"
PROTOCOL = "hvmfcr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_multifractal_coherence_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVMFCR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVMFCR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVMFCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVMFCR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvmfcr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVMFCR-8 unchanged before economic outcomes",
    }
