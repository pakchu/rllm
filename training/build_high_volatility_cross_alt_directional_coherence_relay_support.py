"""Build outcome-blind source support for frozen HVCADCR-8."""
...
from training import preregister_high_volatility_cross_alt_directional_coherence_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-01-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="c4cce2313221bbd039dcfa0c50885ce596746ea63a23c60e3146fc139d10befd"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_cross_alt_directional_coherence_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_cross_alt_directional_coherence_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_cross_alt_directional_coherence_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_cross_alt_directional_coherence_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_cross_alt_directional_coherence_relay_support_2026-08-12.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","aggregate_alt_return","gross_cross_sectional_displacement","directional_coherence","coherence_rank","sign_breadth","breadth_side","realized_variation","variation_rank","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","aggregate_alt_return","gross_cross_sectional_displacement","directional_coherence","coherence_rank","sign_breadth","realized_variation","variation_rank")
...
def coherence_statistics(alt_block:pd.DataFrame,btc_block:pd.DataFrame)->tuple[float,float,float,int,int,float]:
 values=[]
 for symbol in ALTS:
  ...
  values.append(value)
 returns=np.asarray(values,float);aggregate=float(returns.sum());gross=float(np.abs(returns).sum())
 coherence=abs(aggregate)/gross if gross>0 else math.nan
 positive=int(np.sum(returns>0));negative=int(np.sum(returns<0));breadth=max(positive,negative);breadth_side=1 if positive>negative else -1 if negative>positive else 0
 ...
 return aggregate,gross,coherence,breadth,breadth_side,variation
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","symbol","open","high","low","close"]:raise RuntimeError("HVCADCR source schema drift")
 ...
 if x.ts.isna().any() or x.duplicated(["ts","symbol"]).any() or not x.symbol.isin(SYMBOLS).all():raise RuntimeError("HVCADCR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("27h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   aggregate,gross,coherence,breadth,breadth_side,var=coherence_statistics(alt_block,btc_block);valid=all(math.isfinite(v) for v in (aggregate,gross,coherence,breadth,breadth_side,var)) and aggregate!=0 and gross>0 and coherence>0 and breadth_side!=0 and var>0
  else:aggregate=gross=coherence=breadth=breadth_side=var=math.nan;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"aggregate_alt_return":aggregate,"gross_cross_sectional_displacement":gross,"directional_coherence":coherence,"sign_breadth":breadth,"breadth_side":breadth_side,"realized_variation":var})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["coherence_rank"]=prior_rank(panel.directional_coherence.where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));panel["eligible"]=valid&panel.coherence_rank.ge(P["coherence_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"]);panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_block_stale_features":
  cols=["source_valid","aggregate_alt_return","directional_coherence","coherence_rank","sign_breadth","breadth_side","variation_rank","feature_available_time"];used[cols]=panel[cols].shift(1)
 valid=used.source_valid.eq(True);strong=used.coherence_rank.ge(P["coherence_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&strong&variation
 if control=="no_coherence_gate":state=valid&variation
 elif control=="no_variation_gate":state=valid&strong
 elif control=="sign_breadth_instead_of_l1_coherence":state=valid&used.sign_breadth.ge(4)&variation
 onset=previous_valid_onset(state,valid);side=np.sign(pd.to_numeric(used.aggregate_alt_return,errors="coerce").fillna(0)).astype(int)
 if control=="sign_breadth_instead_of_l1_coherence":side=pd.to_numeric(used.breadth_side,errors="coerce").fillna(0).astype(int)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCADCR prereg drift")
 ...
 source_core={"protocol_version":"hvcadcr_8_source_v1","query":QUERY,"query_sha256":hashlib.sha256(QUERY.encode()).hexdigest(),"window":[START.isoformat(),END.isoformat()],"physical_rows":len(raw),"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"panel":{"path":str(PANEL),"sha256":sha(PANEL),"rows":len(panel),"valid_rows":int(panel.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};manifest={**source_core,"manifest_hash":chash(source_core)};immutable(MANIFEST,json_bytes(manifest))
