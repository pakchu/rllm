"""Sequential strict economics for frozen HVGATOR-24."""
...
POLICY_ID = "HVGATOR-24"
BAR = pd.Timedelta(minutes=5)
LEVERAGE = 0.5
BASE_COST = 0.0006
STRESS_COST = 0.0010
ENV_FILE = "/home/pakchu/rllm/.env"
PREREG = Path("results/high_volatility_gator_awakening_relay_preregistration_2026-08-11.json")
PREREG_SHA = "ed9f9d6146b09519acbf167a67b0541d1ef9026603fe25d27bafd51c907bbfc3"
SUPPORT = Path("results/high_volatility_gator_awakening_relay_support_2026-08-11.json")
SUPPORT_SHA = "c0422a186127aae22fd98e352084732dc69a217ad3c91ed3f6cee0019eefafa9"
NOVELTY = Path("results/high_volatility_gator_awakening_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "7cb92e2a0744fee22a85210c6f58eac00f792a4901d031b74d16bfbbd02d869a"
CLOCK = Path("data/high_volatility_gator_awakening_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "90408930af4228f43f1be2551b1c7ae7a07dcce548186bb611bc5ddd7b8ffe0f"
CONTROL_DIR = Path("data/high_volatility_gator_awakening_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_gator_awakening_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_gator_awakening_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "single_gap_expansion", "one_bar_stale_awakening", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVGATOR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVGATOR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvgator_24_sequential_economics_v1", "policy_id": POLICY_ID,
