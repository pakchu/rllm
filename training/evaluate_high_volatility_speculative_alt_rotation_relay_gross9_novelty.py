"""Evaluate frozen HVSARR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVSARR-8"
PROTOCOL = "hvsarr_8_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_speculative_alt_rotation_relay_preregistration_2026-08-12.json"
)
...
OUTPUT = Path(
    "results/high_volatility_speculative_alt_rotation_relay_gross9_novelty_2026-08-12.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVSARR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVSARR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVSARR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVSARR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvsarr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVSARR-8 unchanged before economic outcomes"
        ),
    }
