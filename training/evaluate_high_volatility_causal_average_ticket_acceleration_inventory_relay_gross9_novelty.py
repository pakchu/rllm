"""Evaluate the frozen HVCATAIR-8 action-vote clock against Gross9."""
...
POLICY = "HVCATAIR-8"
PROTOCOL = "hvcatair_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_average_ticket_acceleration_inventory_relay_clocks_2020_2026.csv.gz",
    "sha256": "3f37f672e2805e0a5dda33eb5614cb53003664935e2b0eb3660564fa2b31cc4a",
    "rows": 146,
}
OUTPUT = Path("results/high_volatility_causal_average_ticket_acceleration_inventory_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCATAIR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCATAIR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATAIR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCATAIR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCATAIR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCATAIR-8 unchanged before economics",
    }
