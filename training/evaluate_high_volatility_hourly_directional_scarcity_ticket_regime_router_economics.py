"""Sequential strict economics for frozen HVHDSTR-8."""
...
NOVELTY = Path("results/high_volatility_hourly_directional_scarcity_ticket_regime_router_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "18f8ea0c615ad0fe8f1cf834a93b49d37d7dbeef22f56c0982e1eea6a3947de9"
CLOCK = Path("data/high_volatility_hourly_directional_scarcity_ticket_regime_router_clocks_2023_2026.csv.gz")
CLOCK_SHA = "7d44abe09d3f7df632391e9ee24b4306d348690b095bd07de9c91e9a0ad82b8d"
CONTROL_DIR = Path("data/high_volatility_hourly_directional_scarcity_ticket_regime_router_controls_2023_2026")
FREEZE = Path("results/high_volatility_hourly_directional_scarcity_ticket_regime_router_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_hourly_directional_scarcity_ticket_regime_router_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVHDSTR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVHDSTR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvhdstr_8_sequential_economics_v1", "policy_id": POLICY_ID,
