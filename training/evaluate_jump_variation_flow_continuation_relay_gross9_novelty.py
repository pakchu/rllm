"""Evaluate frozen JVFCR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "JVFCR-8"
PROTOCOL = "jvfcr_8_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/jump_variation_flow_continuation_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("JVFCR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("JVFCR novelty limits drift")
    if (
        ...
        raise RuntimeError("JVFCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="JVFCR primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject JVFCR-8 unchanged before economic outcomes"
        ),
    }
