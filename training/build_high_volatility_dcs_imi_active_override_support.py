"""Materialize deterministic, outcome-blind source support for HVDIMIO-8."""
...
from training import preregister_high_volatility_dcs_imi_active_override as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv

PREREG_SHA = "0f65c339a809584308740821f347786f1ee03d1a53ad5144d82c63d5b4ef650b"
CLOCK = Path("data/high_volatility_dcs_imi_active_override_clocks_2023_2026.csv.gz")
RESULT = Path("results/high_volatility_dcs_imi_active_override_support_2026-08-16.json")
...
def _registration() -> dict[str, Any]:
    if prereg.sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVDIMIO-8 preregistration hash drift")
    ...
    for component, artifacts in prereg.COMPONENT_ARTIFACTS.items():
        for kind, artifact in artifacts.items():
            if prereg.sha256_file(artifact["path"]) != artifact["sha256"]:
                raise RuntimeError(f"HVDIMIO-8 {component} {kind} drift")
...
def load_clock(component: str) -> pd.DataFrame:
    ...
    if frame["feature_available_time"].duplicated().any() or not frame["side"].isin((-1, 1)).all():
        raise RuntimeError(f"HVDIMIO-8 invalid {component} clock")
    if not frame["feature_available_time"].dt.hour.isin((0, 4, 8, 12, 16, 20)).all() or not frame["feature_available_time"].dt.minute.eq(0).all():
        raise RuntimeError(f"HVDIMIO-8 off-grid {component} clock")
    ...
    if not frame["entry_time"].eq(frame["feature_available_time"] + ENTRY_DELAY).all() or not frame["exit_time"].eq(frame["entry_time"] + expected_hold).all():
        raise RuntimeError(f"HVDIMIO-8 {component} timing drift")
    if not frame["feature_available_time"].le(frame["entry_time"]).all():
        raise RuntimeError(f"HVDIMIO-8 unavailable {component} feature")
...
def build_clock(clocks: Mapping[str, pd.DataFrame]) -> tuple[pd.DataFrame, dict[str, Any]]:
    if set(clocks) != set(prereg.COMPONENT_IDS):
        raise RuntimeError("HVDIMIO-8 component roster drift")
    ...
    counts: dict[str, Any] = {
        ...
        "override_absent": 0, "override_same_side": 0, "override_opposite_side": 0,
        "override_opposite_by_stage": {stage: 0 for stage in prereg.build()["stages"]},
        "longs": 0, "shorts": 0,
    ...
    for decision in decisions:
        ...
        if len(active_vetoes) > 1:
            raise RuntimeError("HVDIMIO-8 overlapping active veto positions")
        ...
        if primary.split != split or primary.feature_available_time > entry:
            raise RuntimeError("HVDIMIO-8 primary stage or availability drift")
        if veto is not None and (veto.split != split or veto.feature_available_time > entry):
            raise RuntimeError("HVDIMIO-8 veto stage or availability drift")
        active_state_override_side = 0 if veto is None else veto.side
        side = prereg.active_state_override_side(primary.side, active_state_override_side)
        if veto is None:
            counts["override_absent"] += 1
        elif veto.side == primary.side:
            counts["override_same_side"] += 1
        else:
            counts["override_opposite_side"] += 1
            counts["override_opposite_by_stage"][split] += 1
        selected = veto if veto is not None and veto.side == -primary.side else primary
        selected_action = veto_id if selected is veto else primary_id
        rows.append({"candidate": prereg.POLICY_ID, "control": "primary", "split": split, "selected_action": selected_action, "decision_time": decision, "feature_available_time": selected.feature_available_time, "entry_time": entry, "exit_time": exit_, "side": side, "active_action_count": active_count})
    ...
    if counts["emitted_decisions"] != counts["primary_decisions"]:
        raise RuntimeError("HVDIMIO-8 incidence accounting drift")
    if len(output) > 1 and not output["entry_time"].iloc[1:].reset_index(drop=True).ge(output["exit_time"].iloc[:-1].reset_index(drop=True)).all():
        raise RuntimeError("HVDIMIO-8 output overlap")
...
def run(clock_path: Path = CLOCK, result_path: Path = RESULT) -> dict[str, Any]:
    ...
    activation_checks = {
        "minimum_opposite_overrides_train": accounting["override_opposite_by_stage"]["train"] >= activation_gate["minimum_opposite_overrides_train"],
        "minimum_opposite_overrides_full": accounting["override_opposite_side"] >= activation_gate["minimum_opposite_overrides_full"],
    }
    passed = all(checks.values()) and all(activation_checks.values())
    registration = _registration()
    core = {"protocol_version": "hvdimio_8_source_support_v1", "policy_id": prereg.POLICY_ID, "preregistration": {"path": str(prereg.DEFAULT_OUTPUT), "sha256": PREREG_SHA, "manifest_hash": registration["manifest_hash"]}, "verified_component_artifacts": verified, "component_clock_fields_opened": list(FIELDS), "combined_incidence_opened": True, "combined_postentry_returns_or_pnl_opened": False, "entry_exit_prices_opened": False, "returns_opened": False, "funding_opened": False, "pnl_opened": False, "gross9_comparator_rows_opened": False, "clock": {"path": str(clock_path), "sha256": prereg.sha256_file(clock_path), "rows": len(clock)}, "active_override_accounting": accounting, "support": stats, "support_checks": checks, "operator_activation_checks": activation_checks, "support_passed": passed, "advance_to_gross9_novelty": passed, "advance_to_economic_outcomes": False, "decision": "pass_to_gross9_novelty" if passed else "terminal_source_support_reject"}
...
if __name__ == "__main__":
    ...
    print(json.dumps({"events": {k: v["events"] for k, v in report["support"].items()}, "accounting": report["active_override_accounting"], "support_passed": report["support_passed"]}, indent=2))
