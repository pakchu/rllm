"""Sequential strict economics for frozen HVERF-24."""
...
PREREG = Path("results/high_volatility_ehlers_roofing_filter_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "8ecb968c49b12e420b7cd600e8d5eb83253a882f53f0fb5cfad64bfed094601d"
SUPPORT = Path("results/high_volatility_ehlers_roofing_filter_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "9f7b264ce9ebcfb31eaf69995d714f7562a07bfe4dc5de366f43d24351b2ddc8"
NOVELTY = Path("results/high_volatility_ehlers_roofing_filter_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "90d5bc57946c124bf8090c2e1ef7a0d55df6ecd082e8a3bb3bfe8002e553ebdb"
CLOCK = Path("data/high_volatility_ehlers_roofing_filter_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "c75481fe24f6da2579d83369da00be24c6c1fc57c57c65c408ab05ea0c6c1c84"
CONTROL_DIR = Path("data/high_volatility_ehlers_roofing_filter_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_ehlers_roofing_filter_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ehlers_roofing_filter_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "raw_high_pass_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVERF frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVERF novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hverf_24_sequential_economics_v1", "policy_id": POLICY_ID,
