"""Sequential strict economics for frozen HVOER-8."""
...
PREREG = Path("results/high_volatility_online_expert_rotation_relay_preregistration_2026-08-13.json")
PREREG_SHA = "4506c0b453935b2035328eb3e93709b7c67b1652353f4ddad0c6610e57f69ed2"
SUPPORT = Path("results/high_volatility_online_expert_rotation_relay_support_2026-08-13.json")
SUPPORT_SHA = "b1d654ee70201a3023b1adf8387a608bae2359cd5e289db5ae8662a0db80638f"
NOVELTY = Path("results/high_volatility_online_expert_rotation_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "ee560bed5d29b0927f46a3e5957d91aa1cbd92cc7b7fb42644c4cda09290a4e9"
CLOCK = Path("data/high_volatility_online_expert_rotation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "f1a0e5765cab81427658f8235b1ead37a3a9065f360f11338a3af6bece4b8b52"
CONTROL_DIR = Path("data/high_volatility_online_expert_rotation_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_online_expert_rotation_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_online_expert_rotation_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVOER frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVOER novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvoer_8_sequential_economics_v1", "policy_id": POLICY_ID,
