"""Evaluate frozen HVSDD-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVSDD-8"
PROTOCOL = "hvsdd_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_spot_displacement_dominance_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVSDD predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVSDD novelty limits drift")
    if (
        ...
        raise RuntimeError("HVSDD predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVSDD primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvsdd_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVSDD-8 unchanged before economic outcomes",
    }
