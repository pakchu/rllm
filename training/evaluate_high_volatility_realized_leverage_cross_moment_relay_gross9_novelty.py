"""Evaluate frozen HVRLXC-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVRLXC-12"
PROTOCOL = "hvrlxc_12_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_realized_leverage_cross_moment_relay_preregistration_2026-08-12.json"
)
...
OUTPUT = Path(
    "results/high_volatility_realized_leverage_cross_moment_relay_gross9_novelty_2026-08-12.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVRLXC predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVRLXC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVRLXC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVRLXC primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject HVRLXC-12 unchanged before economic outcomes"
        ),
    }
