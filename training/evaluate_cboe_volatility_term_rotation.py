"""Sequential strict evaluator for frozen CVTR-1.
...
SUPPORT_COMMIT = "7b2fed6"
STATIC_INPUT_SHA256 = {
    "training/preregister_cboe_volatility_term_rotation.py": "73ed06bd2f3a027ad425a8edc1758dcc0b28e98b5f0fc586aaccb7f288966ecb",
    "training/cboe_volatility_term_rotation_clock.py": "4684946211adfd02616a24cce630d3223c1162d65a84b951dc0781518a9d149b",
    "docs/cboe-volatility-term-rotation-preregistration-2026-07-17.md": "c27f684a8b8ce409d9ece12511ec135cb06baa7c426efa5aac5047ac8196e3cf",
    "results/cboe_volatility_term_rotation_preregistration_2026-07-17.json": "af440f63cee9fac526fe9731c2e203ab37e9ffa19847403fbd51845a25c4b7f6",
    "training/build_cboe_volatility_term_rotation_support.py": "3edcdff582eedf6607412c0341887578379350d32925375f363d3a94ebe4eb63",
    "docs/cboe-volatility-term-rotation-support-2026-07-17.md": "16eac36d1d18fad67b800cebed54e612ec3e190679fc8e179976755aa98b9fab",
    "results/cboe_volatility_term_rotation_support_2026-07-17.json": "ce89b3fb831f0283d9e19537d50086329b28b0f7bfe474baa074f4bcd23531d8",
    "results/cboe_volatility_term_rotation_clocks_2026-07-17.csv.gz": "47f4ca447daa2b03a0827ad243ed1107eb34a37e5d7bab18ecd3c4331736959d",
...
SUPPORT_RESULT = Path("results/cboe_volatility_term_rotation_support_2026-07-17.json")
CLOCKS = Path("results/cboe_volatility_term_rotation_clocks_2026-07-17.csv.gz")
...
EVALUATOR_SOURCE = Path("training/evaluate_cboe_volatility_term_rotation.py")
EVALUATOR_FREEZE = Path(
    "results/cboe_volatility_term_rotation_evaluator_freeze_2026-07-17.json"
)
STAGE1_OUTPUT = Path(
    "results/cboe_volatility_term_rotation_stage1_2021_2022_2026-07-17.json"
)
STAGE1_DOC = Path(
    "docs/cboe-volatility-term-rotation-stage1-2021-2022-2026-07-17.md"
)
STAGE2_OUTPUT = Path(
    "results/cboe_volatility_term_rotation_stage2_2023_2026-07-17.json"
)
STAGE2_DOC = Path("docs/cboe-volatility-term-rotation-stage2-2023-2026-07-17.md")
...
def _timestamp(value: Any) -> pd.Timestamp:
    ...
    if result is pd.NaT:
        raise ValueError("CVTR-1 timestamp is NaT")
...
MECHANISM_CONTROLS = ("front_slope_tail", "broad_slope_tail", "vix_level_tail")
...
def _load_json(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"CVTR-1 expected a JSON object: {path}")
    return payload
...
def _verify_evaluation_contract(
    ...
    for key in (
        ...
        if asdict(cfg)[key] != policy[key]:
            raise ValueError(f"CVTR-1 strict engine differs on {key}")
    if cfg.cluster_draws != 20_000 or cfg.cluster_seed != 20_260_717:
        raise ValueError("CVTR-1 sign-flip configuration changed")
    ...
    expected = {
        ...
        "minimum_trades": 80 if stage2 else 250,
        "mean_gross_underlying_bp_min": 35.0,
        "stress_cost_absolute_return_positive": True,
        "each_subperiod_absolute_return_positive": True,
        "each_subperiod_minimum_trades": 40 if stage2 else 110,
        "minimum_each_side_trades": 20 if stage2 else 100,
        "mechanism_margin_ratio_min": 0.25,
    }
    if gates != expected:
        raise ValueError(f"CVTR-1 {key} contract changed")
    return cfg, gates
...
def _verify_static_inputs() -> tuple[dict[str, Any], dict[str, Any]]:
    for path, expected in STATIC_INPUT_SHA256.items():
        if _sha256(path) != expected:
            raise ValueError(f"CVTR-1 frozen input changed: {path}")
    ...
    if support.get("manifest_hash") != _canonical_hash(support_core):
        raise ValueError("CVTR-1 support manifest changed")
    if support.get("policy_id") != "CVTR-1":
        raise ValueError("CVTR-1 support identity changed")
    if support.get("outcomes_opened") is not False or support.get("outcome_sources_opened") != []:
        raise ValueError("CVTR-1 support opened outcomes")
    if support.get("support_passed") is not True or support.get("advance_to_stage1_outcomes") is not True:
        raise ValueError("CVTR-1 source support failed")
    if support.get("preregistration_manifest_hash") != registration["manifest_hash"]:
        raise ValueError("CVTR-1 support is not bound to preregistration")
    if support.get("clocks", {}).get("sha256") != STATIC_INPUT_SHA256[str(CLOCKS)]:
        raise ValueError("CVTR-1 support is not bound to its clock ledger")
    return registration, support


def _random_side(entry_time: pd.Timestamp) -> int:
    digest = hashlib.sha256(
        ("CVTR-1-random-side-20260717|" + entry_time.isoformat()).encode()
    ).digest()
...
def load_schedules() -> dict[str, pd.DataFrame]:
    ...
    if set(ledger["control"]) != base_names:
        raise ValueError("CVTR-1 support clock family changed")
    schedules = {
    ...
    for name in base_names:
        if len(schedules[name]) != int(expected_counts[name]):
            raise ValueError(f"CVTR-1 {name} source count changed")
    ...
    for name, schedule in schedules.items():
        ...
        if len(schedule) > 1:
            ...
            if not bool(entries.ge(exits).all()):
                raise ValueError(f"CVTR-1 {name} schedule overlaps")
        if not bool(schedule["exit_time"].lt(STAGE2[1]).all()):
            raise ValueError(f"CVTR-1 {name} clock crosses source horizon")
    return schedules
...
def freeze_evaluator(output_path: str | Path = EVALUATOR_FREEZE) -> dict[str, Any]:
    ...
    if not all(
        ...
        raise ValueError("CVTR-1 evaluator schedule invariant failed")
    core = {
        "protocol_version": "cboe_volatility_term_rotation_evaluator_v1",
        "policy_id": "CVTR-1",
...
def verify_evaluator_freeze(path: str | Path = EVALUATOR_FREEZE) -> dict[str, Any]:
    ...
    if report.get("manifest_hash") != _canonical_hash(core):
        raise ValueError("CVTR-1 evaluator freeze manifest changed")
    if report.get("evaluator_source_sha256") != _sha256(EVALUATOR_SOURCE):
        raise ValueError("CVTR-1 evaluator source changed after freeze")
    if report.get("support_commit") != SUPPORT_COMMIT:
        raise ValueError("CVTR-1 evaluator froze another support commit")
    if report.get("evaluation_config") != asdict(EvaluationConfig()):
        raise ValueError("CVTR-1 evaluation configuration changed")
    if report.get("opened_windows") != [] or report.get("mutable_parameters") != []:
        raise ValueError("CVTR-1 evaluator freeze is not sealed")
    if report.get("execution_ohlc_rows_parsed_during_freeze") != 0:
        raise ValueError("CVTR-1 evaluator freeze parsed OHLC")
    if report.get("funding_rows_parsed_during_freeze") != 0:
        raise ValueError("CVTR-1 evaluator freeze parsed funding")
    if report.get("simulation_run_during_freeze") is not False:
        raise ValueError("CVTR-1 evaluator freeze simulated an outcome")
    schedules = load_schedules()
    for name, schedule in schedules.items():
        if report["control_schedules"][name]["schedule_hash"] != _schedule_hash(schedule):
            raise ValueError(f"CVTR-1 {name} schedule changed after freeze")
    return report
...
def _verify_execution_manifests() -> None:
    ...
    if market_manifest.get("combined_sha256") != source["market_sha256"]:
        raise ValueError("CVTR-1 market manifest no longer binds the frozen source")
    funding_manifest = _load_json(FUNDING_MANIFEST)
    if funding_manifest.get("data", {}).get("sha256") != source["funding_sha256"]:
        raise ValueError("CVTR-1 funding manifest no longer binds the frozen source")
    if funding_manifest.get("outcomes_opened") is not False:
        raise ValueError("CVTR-1 funding source manifest opened an outcome")
...
def _evaluate_clock(
    ...
    *,
    ...
    each_side = min(int(selected["side"].eq(1).sum()), int(selected["side"].eq(-1).sum()))
    gates = {
        ...
        "minimum_each_side_trades": each_side >= gate_contract["minimum_each_side_trades"],
    }
    return {
        "metrics": base,
    ...
    *,
    ...
    gates = {
        **clocks["primary"]["gates"],
        ...
        "falsification_controls_do_not_fully_qualify": not any(falsification.values()),
    }
    passed = all(gates.values())
    core = {
        "protocol_version": f"cboe_volatility_term_rotation_{stage}_v1",
        "policy_id": "CVTR-1",
        "stage": stage,
...
def _verified_passing_stage1(expected_freeze_hash: str) -> dict[str, Any]:
    if not STAGE1_OUTPUT.exists():
        raise ValueError("CVTR-1 Stage1 has not been run")
    ...
    if stored.get("manifest_hash") != _canonical_hash(core):
        raise ValueError("CVTR-1 stored Stage1 manifest changed")
    if stored.get("evaluator_freeze_manifest_hash") != expected_freeze_hash:
        raise ValueError("CVTR-1 stored Stage1 freeze identity changed")
    if stored.get("evaluator_source_sha256") != _sha256(EVALUATOR_SOURCE):
        raise ValueError("CVTR-1 stored Stage1 evaluator changed")
    if stored.get("gate_passed") is not True:
        raise ValueError("CVTR-1 Stage1 failed; 2023 remains sealed")
    market, funding, diagnostics = load_execution_window(STAGE1)
    ...
    if replayed != stored:
        raise ValueError("CVTR-1 Stage1 does not exactly replay")
    return stored
...
def render_stage_doc(report: dict[str, Any]) -> str:
    ...
    return f"""# CVTR-1 {report['stage']} result — 2026-07-17
