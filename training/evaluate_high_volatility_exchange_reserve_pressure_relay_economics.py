"""Sequential strict economics for frozen HVEXRP-24."""
...
PREREG = Path("results/high_volatility_exchange_reserve_pressure_relay_preregistration_2026-08-13.json")
PREREG_SHA = "bb1ed431c3d1fe28235f16d7e53318382999ecb74dbf41ad5cac44561b2fb0d0"
SUPPORT = Path("results/high_volatility_exchange_reserve_pressure_relay_support_2026-08-13.json")
SUPPORT_SHA = "e04ae04940c83959f68367e2b0e7b5a7cff8fbee139c0767aa3c9780f6f60134"
NOVELTY = Path("results/high_volatility_exchange_reserve_pressure_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "3d28ac63803b36a277b28b520d2e636c5ba7addb47d23999f48d0d958f8b7cc1"
CLOCK = Path("data/high_volatility_exchange_reserve_pressure_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "88fafd32d95815bdb7b9dddf78c2f536b1ff854d118f5215807757b83878bb56"
CONTROL_DIR = Path("data/high_volatility_exchange_reserve_pressure_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_exchange_reserve_pressure_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_exchange_reserve_pressure_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = (
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVEXRP frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVEXRP novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvexrp_24_sequential_economics_v1", "policy_id": POLICY_ID,
