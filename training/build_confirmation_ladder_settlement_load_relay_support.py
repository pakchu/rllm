"""Build source-only CLSLR-6 confirmation-ladder clocks."""
...
PREREG_SHA = "8e3df3ece9c57a94e83ba685bfb0dca4449932dd495bc8c4cc607c3f06fadfbe"
...
CONTROLS = ("late_unanimity_only", "settlement_load_only", "one_anchor_stale_ladder", "direction_flip")
...
SOURCE_DIR = Path("data/confirmation_ladder_settlement_load_relay_sources_2023_2026")
FEATURES = SOURCE_DIR / "confirmation_ladders.csv.gz"
MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/confirmation_ladder_settlement_load_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/confirmation_ladder_settlement_load_relay_controls_2023_2026")
RESULT = Path("results/confirmation_ladder_settlement_load_relay_support_2026-08-09.json")
...
FEATURE_COLUMNS = (
    ...
    *RETURN_COLUMNS, *WEIGHT_COLUMNS, "late_return", "late_unanimous",
    "settlement_load_expansion", "eligible_state",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "anchor_height", "confirmation_height",
    "feature_available_time", "entry_time", "exit_time", "side", "late_return",
    "late_unanimous", "settlement_load_expansion",
)
...
def validate_blocks(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CLSLR block schema drift")
...
def prepare_minutes(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CLSLR minute schema drift")
    ...
    if result.ts.duplicated().any():
        raise RuntimeError("CLSLR duplicate timestamps")
...
def build_features(blocks: pd.DataFrame, market: pd.DataFrame) -> pd.DataFrame:
    ...
    for index in range(0, len(blocks) - 6):
        ...
        settlement_load_expansion = bool(valid and sum(weights[3:]) > sum(weights[:3]))
        rows.append({
            ...
            **{column: weights[position] for position, column in enumerate(WEIGHT_COLUMNS)},
            ...
            "settlement_load_expansion": settlement_load_expansion,
        })
    output = pd.DataFrame(rows)
    output["eligible_state"] = output.source_valid & output.late_unanimous & output.settlement_load_expansion
...
def active_and_side(features: pd.DataFrame, control: str = "primary"):
    ...
    elif control == "settlement_load_only":
        state = features.source_valid & features.settlement_load_expansion
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active & sides.ne(0)]:
        ...
        rows.append({
            ...
            "late_unanimous": bool(features.at[index, "late_unanimous"]), "settlement_load_expansion": bool(features.at[index, "settlement_load_expansion"]),
        })
...
def run() -> dict[str, Any]:
    ...
    source_core = {
        "protocol_version": "clslr_6_source_v1", "query_sha256": hashlib.sha256(QUERY.encode()).hexdigest(), "window": [QUERY_START.isoformat(), END.isoformat()],
    ...
    core = {
        "protocol_version": "clslr_6_source_support_v1", "policy_id": prereg.POLICY_ID,
