"""Build outcome-blind source support for frozen HVSTOCH-24."""
...
from training import preregister_high_volatility_stochastic_extreme_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "3d98b6f957c53def0991275a3e7ac8ed79755ec39a8a477db2d546305477fa42"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_stochastic_extreme_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_stochastic_extreme_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_stochastic_extreme_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_stochastic_extreme_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_stochastic_extreme_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "rolling_high", "rolling_low", "percent_k", "percent_d",
    "difference", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "rolling_high", "rolling_low", "percent_k",
    "percent_d", "difference", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVSTOCH source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVSTOCH invalid source key")
...
def stochastic(high: pd.Series, low: pd.Series, close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    period=P["range_periods"];rolling_high=high.where(valid).rolling(period,min_periods=period).max();rolling_low=low.where(valid).rolling(period,min_periods=period).min();span=rolling_high-rolling_low;percent_k=(100*(close-rolling_low)/span).where(valid&span.gt(0));percent_d=percent_k.rolling(P["signal_periods"],min_periods=P["signal_periods"]).mean()
    return pd.DataFrame({"rolling_high":rolling_high,"rolling_low":rolling_low,"percent_k":percent_k,"percent_d":percent_d},index=high.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[["bar_open","bar_high","bar_low","bar_close"]]).all(axis=1)&bars[["bar_open","bar_high","bar_low","bar_close"]].gt(0).all(axis=1);bars=pd.concat([bars,stochastic(bars.bar_high,bars.bar_low,bars.bar_close,bars.valid_bar)],axis=1);bars["difference"]=bars.percent_k-bars.percent_d;bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum())
    bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["rolling_high","rolling_low","percent_k","percent_d","difference","variation"]]).all(axis=1)&bars.rolling_high.gt(bars.rolling_low)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.difference.shift(1);bull=panel.difference.gt(0)&previous.lt(0)&panel.percent_k.le(P["oversold_max"]);bear=panel.difference.lt(0)&previous.gt(0)&panel.percent_k.ge(P["overbought_min"]);panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&(bull|bear)&panel.variation_rank.ge(P["variation_rank_min"]);return panel.loc[:,PANEL_COLUMNS]


def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);difference=pd.to_numeric(used.difference,errors="coerce");previous=difference.shift(1);bull=difference.gt(0)&previous.lt(0);bear=difference.lt(0)&previous.gt(0);extreme=(bull&used.percent_k.le(P["oversold_max"]))|(bear&used.percent_k.ge(P["overbought_min"]));variation=used.variation_rank.ge(P["variation_rank_min"]);side=pd.Series(np.where(bull,1,np.where(bear,-1,0)),index=used.index,dtype=int);state=valid&side.ne(0)&extreme&variation
    if control=="no_variation_gate":state=valid&side.ne(0)&extreme
    elif control=="no_extreme_gate":state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVSTOCH prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvstoch_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvstoch_24_source_support_v1", "policy_id": prereg.POLICY_ID,
