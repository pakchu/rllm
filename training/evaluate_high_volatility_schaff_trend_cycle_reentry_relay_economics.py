"""Sequential strict economics for frozen HVSTC-24."""
...
PREREG = Path("results/high_volatility_schaff_trend_cycle_reentry_relay_preregistration_2026-08-11.json")
PREREG_SHA = "ffaa990b42548a0eef1389be70376ef63497ab7479096db418df8f5b27d7f415"
SUPPORT = Path("results/high_volatility_schaff_trend_cycle_reentry_relay_support_2026-08-11.json")
SUPPORT_SHA = "0575571e29a9614e0f2f8ad632404c9a0cefbea7fb1a234b182d29eb09f6ebf6"
NOVELTY = Path("results/high_volatility_schaff_trend_cycle_reentry_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "a87270e3eed6cd47a4edce7dadbcf79b6e70e0849ee10990379dca3d37f4cfd4"
CLOCK = Path("data/high_volatility_schaff_trend_cycle_reentry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "f9f38b2f9ac4abb5e3412c86e240d3b407be9b0009f7555f0931b09a907d07f4"
CONTROL_DIR = Path("data/high_volatility_schaff_trend_cycle_reentry_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_schaff_trend_cycle_reentry_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_schaff_trend_cycle_reentry_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "raw_macd_zero_cross", "one_bar_stale_reentry", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSTC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSTC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvstc_24_sequential_economics_v1", "policy_id": POLICY_ID,
