"""Build outcome-blind source support for frozen HVPICR-8."""
...
from training import preregister_high_volatility_premium_inventory_conditioned_rotation_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv


ENV_FILE = "/home/pakchu/rllm/.env"
PREREG_SHA = "83468a88d4515e597f4654e4d5cb24be16aa8c5fcb235561d7af2ab4ac0a195e"
...
SOURCE_DIR = Path("data/high_volatility_premium_inventory_conditioned_rotation_relay_sources_2023_2026")
FEATURES = SOURCE_DIR / "eight_hour_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_premium_inventory_conditioned_rotation_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_premium_inventory_conditioned_rotation_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_premium_inventory_conditioned_rotation_relay_support_2026-08-13.json")
SPLITS = {
...
CONTROLS = ("no_dvol_gate", "no_premium_tail", "ignore_inventory_sign_follow_premium", "one_block_stale_features", "direction_flip", "forced_long")
PREMIUM_QUERY = """
...
GROUP BY 1 ORDER BY 1
"""
CLOCK_COLUMNS = ("candidate", "control", "split", "decision_time", "feature_available_time", "entry_time", "exit_time", "side", "prior_premium_displacement", "current_premium_displacement", "premium_displacement_rank", "current_oi_change", "dvol_close", "dvol_level_rank")
...
def build_features(premium: pd.DataFrame, oi: pd.DataFrame, dvol: pd.DataFrame) -> pd.DataFrame:
    ...
    frame["current_oi_change"] = np.log((oi_last / oi_first).where(oi_first.gt(0) & oi_last.gt(0)))
    oi_valid = (
        ...
        & np.isfinite(frame.current_oi_change) & oi_first.gt(0) & oi_last.gt(0)
    )
    premium_valid = (
    ...
    current_valid = (
        premium_valid & oi_valid
        & frame.dvol_close.gt(0) & np.isfinite(frame.dvol_close)
    )
    frame = frame.sort_values("decision_time").reset_index(drop=True)
    contiguous_prior = frame.block_start.diff().eq(pd.Timedelta(hours=8))
    frame["prior_premium_displacement"] = frame.premium_displacement.shift(1)
    frame["current_premium_displacement"] = frame.premium_displacement
    frame["source_valid"] = current_valid & premium_valid.shift(1, fill_value=False) & contiguous_prior
...
def conditions(frame: pd.DataFrame, control: str = "primary") -> tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    active &= np.sign(used.prior_premium_displacement) * np.sign(used.current_premium_displacement) < 0
    active &= used.current_oi_change.ne(0)
    side = (np.sign(used.current_premium_displacement) * np.sign(used.current_oi_change)).fillna(0).astype(int)
    if control == "ignore_inventory_sign_follow_premium": side = np.sign(used.current_premium_displacement).fillna(0).astype(int)
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active]:
        ...
        rows.append({"candidate": "HVPICR-8", "control": control, "split": split, "decision_time": decision, "feature_available_time": decision, "entry_time": entry, "exit_time": exit_time, "side": int(sides.at[index]), "prior_premium_displacement": float(source.prior_premium_displacement), "current_premium_displacement": float(source.current_premium_displacement), "premium_displacement_rank": float(source.premium_displacement_rank), "current_oi_change": float(source.current_oi_change), "dvol_close": float(source.dvol_close), "dvol_level_rank": float(source.dvol_level_rank)})
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA: raise RuntimeError("HVPICR preregistration drift")
