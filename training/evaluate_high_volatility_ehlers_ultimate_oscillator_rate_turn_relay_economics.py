"""Sequential strict economics for frozen HVEUO-24."""
...
PREREG = Path("results/high_volatility_ehlers_ultimate_oscillator_rate_turn_relay_preregistration_2026-08-11.json")
PREREG_SHA = "f74e43a546ffeb83000a788a75e809155e051f288ba9611553d6436cb2570453"
SUPPORT = Path("results/high_volatility_ehlers_ultimate_oscillator_rate_turn_relay_support_2026-08-11.json")
SUPPORT_SHA = "431db9471a12d9341c94079116600ee5446f827d0f54cda7e173383524c45feb"
NOVELTY = Path("results/high_volatility_ehlers_ultimate_oscillator_rate_turn_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "e8b591c896ac55c4bc62c47fa3e0bea566270c153c5cf994b20a6eb31eda938d"
CLOCK = Path("data/high_volatility_ehlers_ultimate_oscillator_rate_turn_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "dd339ec44508d0c077e1ff30784249fd59925096b0158597832cf0a408bbad12"
CONTROL_DIR = Path("data/high_volatility_ehlers_ultimate_oscillator_rate_turn_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_ehlers_ultimate_oscillator_rate_turn_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ehlers_ultimate_oscillator_rate_turn_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "chart_30_band_edge", "oscillator_zero_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVEUO frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVEUO novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hveuo_24_sequential_economics_v1", "policy_id": POLICY_ID,
