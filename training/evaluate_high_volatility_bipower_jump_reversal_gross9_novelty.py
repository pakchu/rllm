"""Evaluate frozen HVBJR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVBJR-12"
PROTOCOL = "hvbjr_12_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_bipower_jump_reversal_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVBJR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVBJR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVBJR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVBJR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvbjr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVBJR-12 unchanged before economic outcomes"
        ),
    }
