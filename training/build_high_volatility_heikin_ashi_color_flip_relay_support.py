"""Build outcome-blind source support for frozen HVHA-24."""
...
PREREG_SHA = "692c76a4b3a6f54b4b6ca251c20443eb3f14150507c1d317eb3292c5c1db2a41"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_heikin_ashi_color_flip_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_heikin_ashi_color_flip_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_heikin_ashi_color_flip_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_heikin_ashi_color_flip_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_heikin_ashi_color_flip_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "ha_open", "ha_high", "ha_low", "ha_close",
    "ha_body", "color", "raw_color", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "ha_open", "ha_high", "ha_low", "ha_close",
    "ha_body", "color", "raw_color", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVHA source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVHA invalid source key")
...
def heikin_ashi(open_: pd.Series, high: pd.Series, low: pd.Series, close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    n=len(close);ha_open=np.full(n,np.nan);ha_high=np.full(n,np.nan);ha_low=np.full(n,np.nan);ha_close=np.full(n,np.nan);previous_open=previous_close=np.nan
    arrays=[item.to_numpy(float) for item in (open_,high,low,close)];validity=valid.to_numpy(bool)
    for i in range(n):
        if not validity[i]:previous_open=previous_close=np.nan;continue
        current_close=sum(values[i] for values in arrays)/4.0
        current_open=(arrays[0][i]+arrays[3][i])/2.0 if not math.isfinite(previous_open) else (previous_open+previous_close)/2.0
        ha_open[i]=current_open;ha_close[i]=current_close;ha_high[i]=max(arrays[1][i],current_open,current_close);ha_low[i]=min(arrays[2][i],current_open,current_close)
        previous_open=current_open;previous_close=current_close
    return pd.DataFrame({"ha_open":ha_open,"ha_high":ha_high,"ha_low":ha_low,"ha_close":ha_close},index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=bars.join(heikin_ashi(bars.bar_open,bars.bar_high,bars.bar_low,bars.bar_close,bars.valid_bar));bars["ha_body"]=bars.ha_close-bars.ha_open;bars["color"]=np.sign(bars.ha_body);bars["raw_color"]=np.sign(bars.bar_close-bars.bar_open)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["ha_open","ha_high","ha_low","ha_close","ha_body","color","raw_color","variation"]]).all(axis=1)&bars.color.ne(0)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.color.shift(1)
    panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&previous.ne(0)&panel.color.ne(previous)&panel.variation_rank.ge(P["variation_rank_min"])
    return panel.loc[:,PANEL_COLUMNS]

def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);side=pd.to_numeric(used.color,errors="coerce").fillna(0).astype(int);previous=side.shift(1);flip=side.ne(0)&previous.ne(0)&side.ne(previous);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&flip&variation
    if control=="no_variation_gate":state=valid&flip
    elif control=="raw_candle_color_flip":side=pd.to_numeric(used.raw_color,errors="coerce").fillna(0).astype(int);previous=side.shift(1);state=valid&side.ne(0)&previous.ne(0)&side.ne(previous)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVHA prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvha_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvha_24_source_support_v1", "policy_id": prereg.POLICY_ID,
