"""Evaluate frozen HVAMF-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVAMF-8"
PROTOCOL = "hvamf_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_alt_modularity_fragmentation_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVAMF predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVAMF novelty limits drift")
    if (
        ...
        raise RuntimeError("HVAMF predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVAMF primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVAMF-8 unchanged before economic outcomes",
    }
