"""Build outcome-blind source support for frozen HVSTR-24."""
...
PREREG_SHA = "4f38b64a2d37e20b5e95d07eaa3e7f5d1b23ab4521c44a859e451d8378faa787"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_supertrend_regime_flip_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_supertrend_regime_flip_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_supertrend_regime_flip_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_supertrend_regime_flip_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_supertrend_regime_flip_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "true_range", "atr", "basic_upper", "basic_lower",
    "final_upper", "final_lower", "state", "state_change", "basic_signal",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "true_range", "atr", "basic_upper", "basic_lower",
    "final_upper", "final_lower", "state", "state_change", "basic_signal",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVSTR source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVSTR invalid source key")
...
def supertrend(high: pd.Series, low: pd.Series, close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    n=len(close);period=P["atr_periods"];multiplier=P["atr_multiplier"]
    tr=np.full(n,np.nan);atr=np.full(n,np.nan);bu=np.full(n,np.nan);bl=np.full(n,np.nan);fu=np.full(n,np.nan);fl=np.full(n,np.nan);state=np.full(n,np.nan);change=np.zeros(n,dtype=bool);basic=np.zeros(n,dtype=int)
    prev_atr=np.nan;tr_history=[];prev_fu=prev_fl=prev_close=np.nan;prev_state=np.nan
    highs=high.to_numpy(float);lows=low.to_numpy(float);closes=close.to_numpy(float);validity=valid.to_numpy(bool)
    for i in range(n):
        if i==0 or not validity[i] or not validity[i-1]:
            prev_atr=np.nan;tr_history=[];prev_fu=prev_fl=prev_close=prev_state=np.nan;continue
        value=max(highs[i]-lows[i],abs(highs[i]-closes[i-1]),abs(lows[i]-closes[i-1]));tr[i]=value
        if not math.isfinite(prev_atr):
            tr_history.append(value)
            if len(tr_history)<period:continue
            prev_atr=float(np.mean(tr_history[-period:]))
        else:prev_atr=((period-1)*prev_atr+value)/period
        atr[i]=prev_atr;midpoint=(highs[i]+lows[i])/2;bu[i]=midpoint+multiplier*prev_atr;bl[i]=midpoint-multiplier*prev_atr
        if not math.isfinite(prev_state):
            fu[i]=bu[i];fl[i]=bl[i];state[i]=P["initial_state"]
        else:
            fu[i]=bu[i] if bu[i]<prev_fu or prev_close>prev_fu else prev_fu
            fl[i]=bl[i] if bl[i]>prev_fl or prev_close<prev_fl else prev_fl
            current=int(prev_state)
            if current==-1 and closes[i]>fu[i]:current=1
            elif current==1 and closes[i]<fl[i]:current=-1
            state[i]=current;change[i]=current!=int(prev_state)
        basic[i]=1 if closes[i]>bu[i] else -1 if closes[i]<bl[i] else 0
        prev_fu=fu[i];prev_fl=fl[i];prev_close=closes[i];prev_state=state[i]
    return pd.DataFrame({"true_range":tr,"atr":atr,"basic_upper":bu,"basic_lower":bl,"final_upper":fu,"final_lower":fl,"state":state,"state_change":change,"basic_signal":basic},index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=bars.join(supertrend(bars.bar_high,bars.bar_low,bars.bar_close,bars.valid_bar));bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum())
    bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["true_range","atr","basic_upper","basic_lower","final_upper","final_lower","state","variation"]]).all(axis=1)&bars.state.ne(0)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="basic_band_break":side=pd.to_numeric(used.basic_signal,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVSTR prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvstr_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvstr_24_source_support_v1", "policy_id": prereg.POLICY_ID,
