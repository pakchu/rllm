"""Build outcome-blind source support for frozen HVCFO-24."""
...
from training import preregister_high_volatility_chande_forecast_oscillator_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "fd83ac6390a05a76ff1e57eb2fabfe09a62cffa409b49009704be67f6d64de53"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_chande_forecast_oscillator_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_chande_forecast_oscillator_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_chande_forecast_oscillator_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_chande_forecast_oscillator_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_chande_forecast_oscillator_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "regression_endpoint", "cfo", "entry_side", "raw_close_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "regression_endpoint", "cfo", "entry_side",
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVCFO source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVCFO invalid source key")
...
def causal_regression_endpoint(series: pd.Series, valid: pd.Series, period: int) -> pd.Series:
    ...
    for i,(value,is_valid) in enumerate(zip(series.to_numpy(float),valid.to_numpy(bool))):
        ...
        if len(window)==period:
            x=np.arange(period,dtype=float);y=np.asarray(window,float);xm=x.mean();ym=y.mean();slope=np.sum((x-xm)*(y-ym))/np.sum(np.square(x-xm));output[i]=ym+slope*((period-1)-xm)
    return pd.Series(output,index=series.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["regression_endpoint"]=causal_regression_endpoint(bars.bar_close,bars.valid_bar,P["regression_periods"]);bars["cfo"]=100.0*(bars.bar_close-bars.regression_endpoint)/bars.bar_close;bars["entry_side"]=cross_side(bars.cfo);bars["raw_close_side"]=np.sign(bars.bar_close-bars.bar_close.shift(1)).fillna(0).astype(int)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["regression_endpoint","cfo","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.regression_endpoint.gt(0)&bars.variation.gt(0)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCFO prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvcfo_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvcfo_24_source_support_v1", "policy_id": prereg.POLICY_ID,
