"""Sequential strict economics for frozen HVCELVDCV-8."""
...
PREREG = Path("results/high_volatility_caspc_ehlers_directional_change_opposite_veto_preregistration_2026-08-17.json")
PREREG_SHA = "70c4a3cc89584b3f2d3c6dd7228127556b07164235e9933140e7166a71fc3fd9"
SUPPORT = Path("results/high_volatility_caspc_ehlers_directional_change_opposite_veto_support_2026-08-17.json")
SUPPORT_SHA = "0fb63bca3a394e3363772d6bc486a1b4bae210eb0adec7fd821a14e031ae6d64"
NOVELTY = Path("results/high_volatility_caspc_ehlers_directional_change_opposite_veto_gross9_novelty_2026-08-17.json")
NOVELTY_SHA = "21998b2f015ec9d86c31c3e5c24a51196e2a9b9d9f193ee48b5c564fb3229842"
CLOCK = Path("data/high_volatility_caspc_ehlers_directional_change_opposite_veto_clocks_2023_2026.csv.gz")
CLOCK_SHA = "dd408a9d4a569519069f8a971d76b2702c6001dead58e046e8b8381486d8898d"
CONTROL_DIR = Path("data/high_volatility_caspc_ehlers_directional_change_opposite_veto_controls_2023_2026")
FREEZE = Path("results/high_volatility_caspc_ehlers_directional_change_opposite_veto_economic_evaluator_freeze_2026-08-17.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_caspc_ehlers_directional_change_opposite_veto_{stage}_economics_2026-08-17.json")
    for stage in STAGES
}
CONTROLS = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCELVDCV frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCELVDCV novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcelvdcv_8_sequential_economics_v1", "policy_id": POLICY_ID,
