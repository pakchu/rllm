"""Build outcome-blind source support for frozen HVITK-24."""
...
from training import preregister_high_volatility_ichimoku_tenkan_kijun_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "18455801a2f11901052381ac3d89fa096998145086496208f1cd22b26b353b05"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ichimoku_tenkan_kijun_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ichimoku_tenkan_kijun_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ichimoku_tenkan_kijun_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ichimoku_tenkan_kijun_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ichimoku_tenkan_kijun_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "tenkan", "kijun", "difference", "variation",
    "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "tenkan", "kijun", "difference",
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVITK source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVITK invalid source key")
...
def ichimoku(high: pd.Series, low: pd.Series, valid: pd.Series) -> pd.DataFrame:
    h=high.where(valid);l=low.where(valid);tenkan=midpoint(h,l,P["tenkan_periods"]);kijun=midpoint(h,l,P["kijun_periods"]);return pd.DataFrame({"tenkan":tenkan,"kijun":kijun,"difference":tenkan-kijun},index=high.index)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);difference=pd.to_numeric(used.difference,errors="coerce");previous=difference.shift(1);cross=difference.ne(0)&previous.ne(0)&np.sign(difference).ne(np.sign(previous));variation=used.variation_rank.ge(P["variation_rank_min"]);side=np.sign(difference).fillna(0).astype(int);state=valid&cross&side.ne(0)&variation
    if control=="no_variation_gate":state=valid&cross&side.ne(0)
    elif control=="persistent_equilibrium_state":state=used.source_valid.eq(True)&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVITK prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvitk_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvitk_24_source_support_v1", "policy_id": prereg.POLICY_ID,
