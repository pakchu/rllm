"""Evaluate frozen HVRSR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVRSR-12"
PROTOCOL = "hvrsr_12_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_realized_skew_reversal_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVRSR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVRSR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVRSR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVRSR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvrsr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVRSR-12 unchanged before economic outcomes",
    }
