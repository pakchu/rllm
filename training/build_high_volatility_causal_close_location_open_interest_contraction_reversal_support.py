"""Deterministic outcome-blind source support for HVCCLOICR-8."""
...
from training import preregister_high_volatility_causal_close_location_open_interest_contraction_reversal as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "c255fa0997c34f0bcf052db42db7a02c9087d9a5f07b7f38ec636d9886aa8045"
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVCCLOICR-8 OI schema drift")
    expected = ["decision_time", "final_close", "cycle_high", "cycle_low", "completed_return", "minute_squared_return", "source_rows", "distinct_rows", "first_ts", "last_ts", "coherent"]
    if bars.columns.tolist() != expected:
        raise RuntimeError("HVCCLOICR-8 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all() or not oi["sum_open_interest"].ge(0).all():
        raise RuntimeError("HVCCLOICR-8 invalid OI rows")
    bars = bars.copy()
    for column in ("decision_time", "first_ts", "last_ts"): bars[column] = pd.to_datetime(bars[column], utc=True, errors="raise")
    numeric = ("final_close", "cycle_high", "cycle_low", "completed_return", "minute_squared_return", "source_rows", "distinct_rows")
    for column in numeric: bars[column] = pd.to_numeric(bars[column], errors="raise")
    start = bars["decision_time"] - pd.Timedelta("8h")
    bars["bar_valid"] = (np.isfinite(bars[list(numeric)]).all(axis=1) & bars["cycle_high"].gt(bars["cycle_low"]) & bars["minute_squared_return"].gt(0) & bars["source_rows"].eq(480) & bars["distinct_rows"].eq(480) & bars["first_ts"].eq(start) & bars["last_ts"].eq(bars["decision_time"]-pd.Timedelta("1m")) & bars["coherent"].eq(True))
    bars["realized_variation"] = np.sqrt(bars["minute_squared_return"])
    bars["close_location"] = (bars["final_close"]-bars["cycle_low"])/(bars["cycle_high"]-bars["cycle_low"])
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel = panel.join(bars[["bar_valid","close_location","completed_return","realized_variation"]],on="decision_time")
    panel["source_valid"] = (panel["previous_time"].notna() & panel["decision_time"].sub(panel["previous_time"]).eq(pd.Timedelta("8h")) & panel["bar_valid"].eq(True) & np.isfinite(panel[["sum_open_interest","previous_open_interest","oi_change","close_location","completed_return","realized_variation"]]).all(axis=1) & panel["sum_open_interest"].gt(0) & panel["previous_open_interest"].gt(0) & panel["completed_return"].ne(0) & panel["close_location"].between(0,1) & panel["realized_variation"].gt(0))
    ...
    panel["eligible"]=(valid & panel["oi_change"].lt(0) & ((panel["completed_return"].gt(0)&panel["close_location"].gt(1-outer))|(panel["completed_return"].lt(0)&panel["close_location"].lt(outer))) & panel["variation_rank"].ge(POLICY["variation_rank_min"]))
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCCLOICR-8 side or availability drift")
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCCLOICR-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvccloicr_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvccloicr_8_source_support_v1", "policy_id": prereg.POLICY_ID,
