"""Build outcome-blind source support for frozen HVCASI-6."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_cross_alt_semivariance_imbalance_cascade_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2023-01-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="bfa24ad3de932076ebe261bd54b482d21f6c3384bc69e30e4ce7ab9fb85bea3a";REG=prereg.build();P=REG["policy"];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG["stages"].items()};GATES=REG["source_support_gates"];CONTROLS=tuple(REG["diagnostic_controls"]["names"]);SYMBOLS=("BTCUSDT",*prereg.ALTS);ALTS=prereg.ALTS
...
ROOT=Path("data/high_volatility_cross_alt_semivariance_imbalance_cascade_relay_sources_2023_2026");PANEL=ROOT/"scheduled_semivariance_imbalance_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_cross_alt_semivariance_imbalance_cascade_relay_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_cross_alt_semivariance_imbalance_cascade_relay_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_cross_alt_semivariance_imbalance_cascade_relay_controls_2023_2026");RESULT=Path("results/high_volatility_cross_alt_semivariance_imbalance_cascade_relay_support_2026-08-13.json");BUILDER=Path(__file__).relative_to(Path.cwd())
BASE_COLS=("decision_time","feature_available_time","source_valid","semivariance_imbalance_score","selected_alts","agreeing_alts","side","btc_realized_variation","variation_rank","variation_active","eligible")
CLOCK_COLS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","semivariance_imbalance_score","selected_alts","agreeing_alts","btc_realized_variation","variation_rank")
...
def prepare(raw):
 ...
 if raw.columns.tolist()!=expected:raise RuntimeError("HVCASI source schema drift")
...
def semivariance_imbalance_frame(ret,valid):
 scheduled=pd.date_range(START,END,freq="8h",inclusive="left")+pd.Timedelta("1h");scheduled=scheduled[(scheduled>=START)&(scheduled<END)];rows=[];window_valid=valid.rolling(P["variation_bars"],min_periods=P["variation_bars"]).sum().eq(P["variation_bars"]).all(axis=1);rv=np.sqrt(ret.BTCUSDT.pow(2).where(valid.BTCUSDT).rolling(P["variation_bars"],min_periods=P["variation_bars"]).sum());clean={a:ret[a].where(valid[a]) for a in ALTS};alt_up={a:clean[a].clip(lower=0).pow(2).rolling(P["path_bars"],min_periods=P["path_bars"]).sum() for a in ALTS};alt_down={a:clean[a].clip(upper=0).pow(2).rolling(P["path_bars"],min_periods=P["path_bars"]).sum() for a in ALTS}
 for decision in scheduled:
  ...
  for a in ALTS:
   up=float(alt_up[a].at[end]) if math.isfinite(alt_up[a].at[end]) else np.nan;down=float(alt_down[a].at[end]) if math.isfinite(alt_down[a].at[end]) else np.nan;den=up+down
   row[f"{a}_semivariance_imbalance"]=(up-down)/den if math.isfinite(den) and den>0 and up!=down else np.nan
  rows.append(row)
 return pd.DataFrame(rows)
def score_states(f,control="primary"):
 u=f.copy()
 for a in ALTS:
  u[f"{a}_semivariance_imbalance_rank"]=midrank(u[f"{a}_semivariance_imbalance"].abs().where(u.source_valid),P["semivariance_imbalance_prior_decisions"],P["semivariance_imbalance_minimum_decisions"])
 ...
 for i,row in u.iterrows():
  chosen=[a for a in ALTS if math.isfinite(row[f"{a}_semivariance_imbalance"]) and (control=="no_semivariance_imbalance_tail" or row[f"{a}_semivariance_imbalance_rank"]>=P["semivariance_imbalance_rank_min"])]
  signs={a:int(np.sign(row[f"{a}_semivariance_imbalance"])) for a in chosen};weights={a:1. if control=="equal_weighted_score" else abs(row[f"{a}_semivariance_imbalance"]) for a in chosen};den=sum(weights.values());score=sum(weights[a]*signs[a] for a in chosen)/den if den>0 else np.nan;sgn=int(np.sign(score)) if math.isfinite(score) else 0;scores.append(score);selected.append(len(chosen));agree.append(sum(v==sgn for v in signs.values()) if sgn else 0)
 u["semivariance_imbalance_score"]=scores;u["selected_alts"]=selected;u["agreeing_alts"]=agree;minimum=3 if control=="three_alt_consensus" else P["minimum_selected_alts"];u["side"]=np.sign(u.semivariance_imbalance_score).fillna(0).astype(int);u["variation_active"]=u.variation_rank.ge(P["variation_rank_min"]);u["eligible"]=u.source_valid&u.selected_alts.ge(minimum)&u.semivariance_imbalance_score.abs().ge(P["score_absolute_min"])&u.agreeing_alts.ge(P["minimum_agreeing_alts"])&u.variation_active
 if control=="no_variation_gate":u["eligible"]=u.source_valid&u.selected_alts.ge(minimum)&u.semivariance_imbalance_score.abs().ge(P["score_absolute_min"])&u.agreeing_alts.ge(P["minimum_agreeing_alts"])
 if control=="one_decision_stale_consensus":
  for c in ("semivariance_imbalance_score","selected_alts","agreeing_alts","side"):u[c]=u[c].shift(1)
  u["eligible"]=u.source_valid&u.selected_alts.ge(minimum)&u.semivariance_imbalance_score.abs().ge(P["score_absolute_min"])&u.agreeing_alts.ge(P["minimum_agreeing_alts"])&u.variation_active
 if control=="direction_flip":u.side=-u.side
 if control=="forced_long":u.side=1
 return u
def build_panel(raw,control="primary"):
 x=prepare(raw);grid=pd.date_range(START,END,freq="5min",inclusive="left");ret=x.pivot(index="date",columns="symbol",values="return").reindex(grid,columns=SYMBOLS);valid=x.pivot(index="date",columns="symbol",values="valid").reindex(grid,columns=SYMBOLS).eq(True);return score_states(semivariance_imbalance_frame(ret,valid),control)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCASI prereg drift")
