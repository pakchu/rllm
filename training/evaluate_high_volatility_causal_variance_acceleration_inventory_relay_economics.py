"""Strict staged economics for the singleton HVCVARIR-8 policy."""
...
PREREG = Path("results/high_volatility_causal_variance_acceleration_inventory_relay_preregistration_2026-08-16.json")
PREREG_SHA = "8cf94eac46378a61a737f3ef857e07b3e728cb150ce3a24b6ae10f0e9839ac64"
SUPPORT = Path("results/high_volatility_causal_variance_acceleration_inventory_relay_support_2026-08-16.json")
SUPPORT_SHA = "b45760985d3c9094333c6d4c58400c24488cbe0d8a48fb235a8118306442f649"
GROSS9 = Path("results/high_volatility_causal_variance_acceleration_inventory_relay_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "c60537536ecda6303e88253f0b8537782c229a55ae368c5b50d1df99d69f6f8a"
CLOCK = {
    "path": Path("data/high_volatility_causal_variance_acceleration_inventory_relay_clocks_2020_2026.csv.gz"),
    "sha256": "86f7e4f3b7624396b5a39be132b3c1630087648fbabf2d38a6604863e21b507c",
    "rows": 152,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_variance_acceleration_inventory_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCVARIR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCVARIR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCVARIR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCVARIR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVARIR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVARIR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCVARIR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCVARIR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCVARIR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCVARIR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCVARIR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcvarir_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
