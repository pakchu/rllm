"""Outcome-blind preregistration for HVCVARIR-8."""
...
DEFAULT_OUTPUT = Path("results/high_volatility_causal_variance_acceleration_inventory_relay_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_causal_variance_acceleration_inventory_relay_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            ...
            "why_distinct": "HVCVARIR uses within-cycle acceleration in realized variance, not turnover or transaction count alone, plus exact OI expansion and variation. It reuses neither prior event set nor control and uses no taker-flow sign, spot source, ETH, funding, premium-index, fitted outcome, or promoted control.",
            "why_suited_to_volatile_regimes": "the completed eight-hour BTC realized variation must be in its causal upper 40 percent",
            "why_low_gross9_overlap_is_plausible": "fixed eight-hour variance-acceleration plus OI-sponsored states are absent from Gross9 primitives",
        },
        "features": {
            ...
            "eligible": "exact coherent completed perpetual cycle, positive variance acceleration, nonzero completed return, two exact OI endpoints with strict expansion, and both variance-acceleration and variation ranks pass at D; no taker-flow sign, spot source, funding, range, premium-index, alt, or post-decision condition",
            "no_imputation": True,
        },
        "clock": {
        ...
        "stopping_rule": "terminal first failure; no OI endpoint or expansion rule, variance-acceleration formula, acceleration or variation history, side, clock, hold, subset, threshold, comparator, or control repair",
    }
