"""Evaluate frozen HVECE-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVECE-8"
PROTOCOL = "hvece_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_execution_count_elasticity_relay_gross9_novelty_2026-08-12.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVECE predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVECE novelty limits drift")
    if (
        ...
        raise RuntimeError("HVECE predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVECE primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvece_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVECE-8 unchanged before economic outcomes",
    }
