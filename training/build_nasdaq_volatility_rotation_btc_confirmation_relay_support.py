"""Build outcome-blind source support for preregistered NVXCR-6."""
...
from training import preregister_nasdaq_volatility_rotation_btc_confirmation_relay as prereg
...
CONTROL_DIR = Path("data/nasdaq_volatility_rotation_btc_confirmation_relay_controls_2023_2026")
RESULT = Path("results/nasdaq_volatility_rotation_btc_confirmation_relay_support_2026-08-08.json")
SPLITS = {
...
CONTROLS = ("vxn_change_only", "vix_change_only", "no_btc_confirmation", "one_session_stale_rotation", "direction_flip")
...
COLUMNS = (
    ...
    "side", "delta_log_relative", "absolute_rotation_rank", "delta_log_vxn", "delta_log_vix", "overnight_btc_return",
)
VXN = Path("data/cboe_vxn_2021_2026/source/VXN_History.csv")
...
def load_features() -> pd.DataFrame:
    for raw, expected in HASHES.items():
        if sha(raw) != expected:
            raise RuntimeError(f"NVXCR frozen source changed: {raw}")
    vix = pd.read_csv(VIX, compression="gzip", usecols=["observation_date", "VIX_close"])
    vxn = pd.read_csv(VXN, usecols=["DATE", "CLOSE"]).rename(columns={"DATE": "observation_date", "CLOSE": "VXN_close"})
    vix["observation_date"] = pd.to_datetime(vix.observation_date, format="%Y-%m-%d")
    vxn["observation_date"] = pd.to_datetime(vxn.observation_date, format="%m/%d/%Y")
    cboe = vix.merge(vxn, on="observation_date", validate="one_to_one").sort_values("observation_date").reset_index(drop=True)
    cboe["VIX_close"] = pd.to_numeric(cboe.VIX_close, errors="coerce")
    cboe["VXN_close"] = pd.to_numeric(cboe.VXN_close, errors="coerce")
    if not cboe.observation_date.is_monotonic_increasing or cboe.observation_date.duplicated().any():
        raise RuntimeError("NVXCR Cboe dates invalid")
    if not np.isfinite(cboe[["VIX_close", "VXN_close"]]).all(axis=None) or not cboe[["VIX_close", "VXN_close"]].gt(0).all(axis=None):
        raise RuntimeError("NVXCR volatility values invalid")
    cboe["delta_log_vix"] = np.log(cboe.VIX_close).diff()
    cboe["delta_log_vxn"] = np.log(cboe.VXN_close).diff()
    cboe["delta_log_relative"] = np.log(cboe.VXN_close / cboe.VIX_close).diff()
    cboe["absolute_rotation_rank"] = strict_prior_midrank(cboe.delta_log_relative.abs())
    cboe["absolute_vxn_rank"] = strict_prior_midrank(cboe.delta_log_vxn.abs())
    ...
    if price.columns.tolist() != ["hour_start", "hour_open", "first_half_close", "second_half_open", "hour_close", "source_rows", "distinct_timestamps", "source_valid", "decision_time"]:
        raise RuntimeError("NVXCR completed-price schema changed")
    ...
    if price.hour_start.duplicated().any() or not price.hour_start.is_monotonic_increasing:
        raise RuntimeError("NVXCR completed-price clock invalid")
    ...
    for index in range(1, len(cboe) - 1):
        ...
        rows.append({
            ...
            "absolute_vxn_rank": float(cboe.at[index, "absolute_vxn_rank"]),
            "absolute_vix_rank": float(cboe.at[index, "absolute_vix_rank"]),
            "delta_log_vxn": float(cboe.at[index, "delta_log_vxn"]),
...
def signal(frame: pd.DataFrame, control: str) -> pd.Series:
    ...
    if control == "vxn_change_only":
        eligible = frame.valid & frame.delta_log_vxn.ne(0) & frame.absolute_vxn_rank.ge(0.60)
        side = -np.sign(frame.delta_log_vxn).astype("Int64").fillna(0).astype(int)
...
def build_clock(frame: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in frame.index[sides.ne(0)]:
        ...
        rows.append({
            "candidate": "NVXCR-6", "control": control, "split": split,
            ...
            "delta_log_vxn": float(frame.at[index, "delta_log_vxn"]),
...
def run() -> dict[str, Any]:
    ...
    if registration.get("manifest_hash") != prereg.canonical_hash(registration_core):
        raise RuntimeError("NVXCR preregistration drift")
