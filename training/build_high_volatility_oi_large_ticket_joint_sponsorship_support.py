"""Deterministic outcome-blind source support for HVOILTJ-8."""
...
from training import preregister_high_volatility_oi_large_ticket_joint_sponsorship as prereg


START = pd.Timestamp("2023-04-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "3496b0b54c186361d37b40d479bba8dac88070bac638393274b1bf7f2d1aa1eb"
...
ROOT = Path("data/high_volatility_oi_large_ticket_joint_sponsorship_sources_2023_2026")
PANEL = ROOT / "joint_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_oi_large_ticket_joint_sponsorship_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_oi_large_ticket_joint_sponsorship_split_clocks_2023_2026")
RESULT = Path("results/high_volatility_oi_large_ticket_joint_sponsorship_support_2026-08-16.json")
...
PANEL_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "joint_eligible", "joint_onset",
    "completed_return_3h", "oi_variation", "gross_oi_activity", "coactivity",
    "gross_oi_activity_rank", "coactivity_rank", "oi_variation_rank",
    "large_ticket_clustering", "clustering_rank", "ticket_variation",
    "ticket_variation_rank", "block_return_6h", "final_hour_return",
)
CLOCK_COLUMNS = (
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    numeric = [column for column in keep_oi[2:] + keep_ticket[2:]]
    panel["source_valid"] = (
        panel["oi_source_valid"].eq(True)
        & panel["ticket_source_valid"].eq(True)
        & np.isfinite(panel[numeric]).all(axis=1)
        & panel[["completed_return_3h", "block_return_6h", "final_hour_return"]].ne(0).all(axis=1)
    )
    side = np.sign(panel["completed_return_3h"])
    direction = (
        side.ne(0)
        & np.sign(panel["block_return_6h"]).eq(side)
        & np.sign(panel["final_hour_return"]).eq(side)
    )
    panel["joint_eligible"] = (
        panel["source_valid"]
        ...
        & panel["clustering_rank"].ge(POLICY["clustering_rank_min"])
        & panel["ticket_variation_rank"].ge(POLICY["ticket_variation_rank_min"])
        & direction
    )
    ...
    panel["joint_onset"] = (
        panel["joint_eligible"]
        & ~panel["joint_eligible"].shift(1, fill_value=False)
        & prior_valid
        & prior_exact
    )
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["joint_onset"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVOILTJ-8 side or availability drift")
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVOILTJ-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvoiltj_8_sources_v1", "queries": queries,
    ...
    core = {
        "protocol_version": "hvoiltj_8_source_support_v1", "policy_id": prereg.POLICY_ID,
