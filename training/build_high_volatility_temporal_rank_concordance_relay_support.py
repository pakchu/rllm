"""Source-only support gate for frozen HVTRC-8."""
...
from training import preregister_high_volatility_temporal_rank_concordance_relay as prereg
...
PREREG_SHA = "87556ccab968c68b7ec70967680eaa6e57057e34960fb0b6e4db1aa29fbc0f39"
SPLITS = {
...
ROOT = Path("data/high_volatility_temporal_rank_concordance_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_temporal_rank_concordance_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_temporal_rank_concordance_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_temporal_rank_concordance_relay_support_2026-08-10.json")
...
def temporal_rank_concordance(close: np.ndarray) -> float:
    ...
    if len(close) != 480 or not np.isfinite(close).all() or np.any(close <= 0):
        return math.nan
    ranks = pd.Series(np.log(close)).rank(method="average").to_numpy(float)
    if float(np.std(ranks)) <= 0:
        return math.nan
    value = float(np.corrcoef(np.arange(len(ranks), dtype=float), ranks)[0, 1])
    if not math.isfinite(value) or value == 0 or value < -1 or value > 1:
        return math.nan
    return value
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVTRC source timestamps")
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        if valid:
            ...
            concordance = temporal_rank_concordance(window["close"].to_numpy(float))
            ...
            valid = variation > 0 and math.isfinite(concordance)
        if not valid:
            variation = concordance = block_return = late_return = math.nan
        rows.append(
            {
                "decision_time": decision,
                "source_valid": valid,
                "realized_variation": variation,
                "temporal_rank_concordance": concordance,
    ...
    states["concordance_rank"] = prior_rank(
        states["temporal_rank_concordance"].abs().where(states["source_valid"])
    )
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvtrc_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    concordance_rank = states["concordance_rank"]
    concordance = states["temporal_rank_concordance"]
    if control == "one_block_stale_geometry":
        block_return, late_return, variation_rank, concordance_rank, concordance = [
            value.shift(1)
            for value in (block_return, late_return, variation_rank, concordance_rank, concordance)
        ]
    variation_gate = (
    ...
    if control == "no_concordance_tail":
        concordance_gate = pd.Series(True, index=states.index)
    elif control == "raw_absolute_concordance_positive":
        concordance_gate = concordance.abs().gt(0)
    else:
        concordance_gate = concordance_rank.ge(0.75)
    agreement = (
        block_return.ne(0)
        & late_return.ne(0)
        & np.sign(block_return).eq(np.sign(late_return))
        & np.sign(block_return).eq(np.sign(concordance))
    )
    eligible = states["source_valid"] & variation_gate & concordance_gate & agreement
        ...
        rows.append(
            {
                ...
                "temporal_rank_concordance": float(
                    states.at[index, "temporal_rank_concordance"]
                ),
                "concordance_rank": float(states.at[index, "concordance_rank"]),
            }
        )
    columns = [
        "candidate", "control", "split", "decision_time", "feature_available_time",
        "entry_time", "exit_time", "side", "realized_variation", "variation_rank",
        "temporal_rank_concordance", "concordance_rank",
    ]
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVTRC preregistration drift")
    ...
    core = {
        "protocol_version": "hvtrc_8_source_support_v1",
        "policy_id": "HVTRC-8",
        "preregistration": {
