"""Build outcome-blind source support for frozen HVLDAR-8."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_leverage_displacement_amplification_reversal as prereg
ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2023-05-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="145d23f6a0ae48b1f7ab5af8ddc6f822292e2912b85ae43b24dc670cdbed830d";REG=prereg.build();P=REG["policy"];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG["stages"].items()};GATES=REG["source_support_gates"];CONTROLS=tuple(REG["diagnostic_controls"]["names"]);VENUES=("perpetual","spot")
...
ROOT=Path("data/high_volatility_leverage_displacement_amplification_reversal_sources_2023_2026");PANEL=ROOT/"five_minute_barrier_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_leverage_displacement_amplification_reversal_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_leverage_displacement_amplification_reversal_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_leverage_displacement_amplification_reversal_controls_2023_2026");RESULT=Path("results/high_volatility_leverage_displacement_amplification_reversal_support_2026-08-13.json");BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLS=("decision_time","feature_available_time","source_valid","amplification_side","leverage_excess","excess_rank","cash_excess","cash_excess_rank","perpetual_return","spot_return","btc_realized_variation","variation_rank","variation_active","eligible")
CLOCK_COLS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","leverage_excess","excess_rank","btc_realized_variation","variation_rank")
...
def prepare(raw:pd.DataFrame)->pd.DataFrame:
 ...
 if raw.columns.tolist()!=expected:raise RuntimeError("HVLDAR source schema drift")
 ...
 if x.date.isna().any() or x.duplicated(["date","venue"]).any() or not x.venue.isin(("perpetual","spot")).all():raise RuntimeError("HVLDAR invalid source key")
...
def amplification_side(perpetual_return:pd.Series,spot_return:pd.Series,tail:pd.Series,fade:bool=True)->pd.Series:
 relation=perpetual_return.mul(spot_return);eligible=tail&relation.gt(0)&spot_return.ne(0);side=pd.Series(0,index=spot_return.index,dtype=int);side.loc[eligible]=np.sign(spot_return.loc[eligible]).astype(int)*(-1 if fade else 1);return side
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 x=prepare(raw);grid=pd.date_range(START,END,freq="5min",inclusive="left");op=x.pivot(index="date",columns="venue",values="open").reindex(index=grid,columns=VENUES);close=x.pivot(index="date",columns="venue",values="close").reindex(index=grid,columns=VENUES);sq=x.pivot(index="date",columns="venue",values="minute_squared_return").reindex(index=grid,columns=VENUES);valid=x.pivot(index="date",columns="venue",values="row_valid").reindex(index=grid,columns=VENUES).eq(True);joint=valid.all(axis=1);pret=np.log(close.perpetual/op.perpetual);sret=np.log(close.spot/op.spot);excess=pret.abs()-sret.abs();cash=-excess;rank=strict_prior_midrank(excess.where(joint),P["history_decisions"],P["minimum_history_decisions"]);crank=strict_prior_midrank(cash.where(joint),P["history_decisions"],P["minimum_history_decisions"]);side=amplification_side(pret,sret,rank.ge(P["excess_rank_min"]));variation=np.sqrt(sq.perpetual.where(valid.perpetual).rolling(P["variation_bars"],min_periods=P["variation_bars"]).sum());vrank=strict_prior_midrank(variation,P["history_decisions"],P["minimum_history_decisions"]);source_valid=joint&rank.notna()&crank.notna()&variation.notna()&vrank.notna();vactive=vrank.ge(P["variation_rank_min"]);state=side.ne(0);prior_valid=source_valid.shift(1,fill_value=False);eligible=source_valid&prior_valid&state&~state.shift(1,fill_value=False)&vactive
 return pd.DataFrame({"decision_time":grid+pd.Timedelta("5m"),"feature_available_time":grid+pd.Timedelta("5m"),"source_valid":source_valid.to_numpy(bool),"amplification_side":side.to_numpy(int),"leverage_excess":excess.to_numpy(float),"excess_rank":rank.to_numpy(float),"cash_excess":cash.to_numpy(float),"cash_excess_rank":crank.to_numpy(float),"perpetual_return":pret.to_numpy(float),"spot_return":sret.to_numpy(float),"btc_realized_variation":variation.to_numpy(float),"variation_rank":vrank.to_numpy(float),"variation_active":vactive.fillna(False).to_numpy(bool),"eligible":eligible.to_numpy(bool)},columns=PANEL_COLS)
def active(panel:pd.DataFrame,control="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 u=panel.copy();side=pd.to_numeric(u.amplification_side,errors="coerce").fillna(0).astype(int);state=side.ne(0);prior_valid=u.source_valid.shift(1,fill_value=False);active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)&u.variation_active.eq(True)
 if control=="no_variation_gate":active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)
 elif control in ("rank_0_99","cash_displacement_dominance"):
  tail=u.excess_rank.ge(.99) if control=="rank_0_99" else u.cash_excess_rank.ge(P["excess_rank_min"]);side=amplification_side(u.perpetual_return,u.spot_return,tail,control!="cash_displacement_dominance");state=side.ne(0);active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)&u.variation_active.eq(True)
 elif control=="one_bar_stale_amplification":
  cols=["amplification_side","leverage_excess","excess_rank","feature_available_time"];u[cols]=panel[cols].shift(1);side=pd.to_numeric(u.amplification_side,errors="coerce").fillna(0).astype(int);state=side.ne(0);active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)&u.variation_active.eq(True)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVLDAR prereg drift")
