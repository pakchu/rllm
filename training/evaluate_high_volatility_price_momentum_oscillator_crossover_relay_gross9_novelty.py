"""Evaluate frozen HVPMO-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVPMO-24"
PROTOCOL = "hvpmo_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_price_momentum_oscillator_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVPMO predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVPMO novelty limits drift")
    if (
        ...
        raise RuntimeError("HVPMO predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVPMO primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvpmo_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVPMO-24 unchanged before economic outcomes",
    }
