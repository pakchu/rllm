"""Build outcome-blind source support for frozen HVSNDR-8."""
...
from training import preregister_high_volatility_self_normalized_displacement_reversal as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-01-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="5753b620f75a90d0de379b25880050acfd8bf9c945614f9615d04919bfcc9c7f"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_self_normalized_displacement_reversal_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_self_normalized_displacement_reversal_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_self_normalized_displacement_reversal_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_self_normalized_displacement_reversal_controls_2023_2026")
RESULT=Path("results/high_volatility_self_normalized_displacement_reversal_support_2026-08-10.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","displacement","realized_variation","self_normalized_displacement","magnitude","magnitude_rank","l1_efficiency","l1_efficiency_rank","variation_rank","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","displacement","realized_variation","self_normalized_displacement","magnitude","magnitude_rank","l1_efficiency","l1_efficiency_rank","variation_rank")
...
def displacement_statistics(block:pd.DataFrame)->tuple[float,float,float,float]:
 if len(block)!=480:return (math.nan,)*4
 closes=block.close.to_numpy(float).reshape(96,5)[:,-1];returns=np.diff(np.log(closes));variation=float(np.square(returns).sum());displacement=float(math.log(closes[-1]/block.open.iloc[0]));travel=float(np.abs(returns).sum())
 if variation<=0 or travel<=0:return (math.nan,)*4
 return displacement,variation,displacement/math.sqrt(variation),abs(displacement)/travel
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVSNDR source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVSNDR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("12h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   displacement,var,score,l1=displacement_statistics(block);valid=all(math.isfinite(v) for v in (displacement,var,score,l1)) and displacement!=0 and score!=0 and var>0
  else:displacement=var=score=l1=math.nan;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"displacement":displacement,"realized_variation":var,"self_normalized_displacement":score,"magnitude":abs(score) if math.isfinite(score) else math.nan,"l1_efficiency":l1})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["magnitude_rank"]=prior_rank(panel.magnitude.where(valid));panel["l1_efficiency_rank"]=prior_rank(panel.l1_efficiency.where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));panel["eligible"]=valid&panel.magnitude_rank.ge(P["magnitude_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"]);panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_block_stale_geometry":
  cols=["source_valid","magnitude_rank","l1_efficiency_rank","variation_rank","displacement","feature_available_time"];used[cols]=panel[cols].shift(1)
 valid=used.source_valid.eq(True);magnitude=used.magnitude_rank.ge(P["magnitude_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&magnitude&variation
 if control=="no_magnitude_tail_gate":state=valid&variation
 elif control=="no_variation_gate":state=valid&magnitude
 elif control=="l1_efficiency":state=valid&used.l1_efficiency_rank.ge(P["l1_efficiency_rank_min"])&variation
 onset=previous_valid_onset(state,valid);side=-np.sign(pd.to_numeric(used.displacement,errors="coerce").fillna(0)).astype(int)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVSNDR prereg drift")
 ...
 source_core={"protocol_version":"hvsndr_8_source_v1","query":QUERY,"query_sha256":hashlib.sha256(QUERY.encode()).hexdigest(),"window":[START.isoformat(),END.isoformat()],"physical_rows":len(raw),"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"panel":{"path":str(PANEL),"sha256":sha(PANEL),"rows":len(panel),"valid_rows":int(panel.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};manifest={**source_core,"manifest_hash":chash(source_core)};immutable(MANIFEST,json_bytes(manifest))
