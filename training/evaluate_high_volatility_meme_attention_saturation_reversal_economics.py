"""Sequential strict economics for frozen HVMASR-8."""
...
PREREG = Path("results/high_volatility_meme_attention_saturation_reversal_preregistration_2026-08-09.json")
PREREG_SHA = "8237a9843254807a1e11410d8cc516c79208f7e0ec161c904dcd4d32dfa82198"
SUPPORT = Path("results/high_volatility_meme_attention_saturation_reversal_support_2026-08-09.json")
SUPPORT_SHA = "99943d567fbe705259ea592589f0c8d01ba6e21250989a05fc85cfa985f06789"
NOVELTY = Path("results/high_volatility_meme_attention_saturation_reversal_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "e9b6170713444e3788328662a67a598ce6c4c5b4bcad1d7bf6703f868229535f"
CLOCK = Path("data/high_volatility_meme_attention_saturation_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "dc607a3032797472757e71763c4ff6c4bbddcf545c42ab24cd80febf36f3dcb5"
CONTROL_DIR = Path("data/high_volatility_meme_attention_saturation_reversal_controls_2023_2026")
FREEZE = Path("results/high_volatility_meme_attention_saturation_reversal_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_meme_attention_saturation_reversal_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_doge_return_tail", "no_doge_turnover_tail", "no_btc_variation_gate", "one_block_stale_doge", "direction_flip", "same_clock_forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVMASR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVMASR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvmasr_8_sequential_economics_v1", "policy_id": POLICY_ID,
