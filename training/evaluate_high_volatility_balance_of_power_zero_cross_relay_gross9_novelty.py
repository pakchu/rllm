"""Evaluate frozen HVBOP-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVBOP-24"
PROTOCOL = "hvbop_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_balance_of_power_zero_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVBOP predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVBOP novelty limits drift")
    if (
        ...
        raise RuntimeError("HVBOP predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVBOP primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvbop_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVBOP-24 unchanged before economic outcomes",
    }
