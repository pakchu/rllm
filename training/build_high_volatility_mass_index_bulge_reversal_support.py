"""Build outcome-blind source support for frozen HVMIB-24."""
...
PREREG_SHA = "fa7cbca428184002ec2668fb1c7aee7fa4b94fc1585fb0b8a02f3d1b5a67fef3"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_mass_index_bulge_reversal_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_mass_index_bulge_reversal_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_mass_index_bulge_reversal_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_mass_index_bulge_reversal_controls_2023_2026")
RESULT = Path("results/high_volatility_mass_index_bulge_reversal_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "bar_range", "range_ema1", "range_ema2",
    "ema_ratio", "mass_index", "bulge_release", "displacement", "direction",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "bar_range", "range_ema1", "range_ema2",
    "ema_ratio", "mass_index", "bulge_release", "displacement", "direction",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVMIB source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVMIB invalid source key")
...
def causal_ema_reset(series: pd.Series, valid: pd.Series, period: int) -> pd.Series:
    alpha = 2.0 / (period + 1.0)
    output = np.full(len(series), np.nan)
    state = np.nan
    count = 0
    for index, (value, is_valid) in enumerate(zip(series.to_numpy(float), valid.to_numpy(bool))):
        if not is_valid or not math.isfinite(value):
            state = np.nan; count = 0
            continue
...
def release_state(mass_index: pd.Series, valid: pd.Series) -> pd.Series:
    output = np.zeros(len(mass_index), dtype=bool)
    armed = False
    for index, (value, is_valid) in enumerate(zip(mass_index.to_numpy(float), valid.to_numpy(bool))):
        if not is_valid or not math.isfinite(value):
            armed = False
            continue
        if value > P["bulge_arm_level"]:
            armed = True
        elif armed and value < P["bulge_release_level"]:
            output[index] = True
            armed = False
    return pd.Series(output, index=mass_index.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["bar_range"] = (bars.bar_high - bars.bar_low).where(bars.valid_bar)
    bars["range_ema1"] = causal_ema_reset(bars.bar_range, bars.valid_bar & bars.bar_range.gt(0), P["ema_periods"])
    bars["range_ema2"] = causal_ema_reset(bars.range_ema1, np.isfinite(bars.range_ema1), P["ema_periods"])
    bars["ema_ratio"] = (bars.range_ema1 / bars.range_ema2).where(bars.range_ema2.gt(0))
    bars["mass_index"] = bars.ema_ratio.rolling(P["mass_sum_periods"], min_periods=P["mass_sum_periods"]).sum()
    bars["displacement"] = bars.bar_close - bars.bar_close.shift(P["direction_lag_bars"])
    bars["direction"] = -np.sign(bars.displacement)
    bars["variation"] = np.sqrt(bars.variation_component.rolling(P["variation_hours"] // 4, min_periods=P["variation_hours"] // 4).sum())
    mass_valid = (
        bars.valid_bar
        & np.isfinite(
            bars[["bar_range", "range_ema1", "range_ema2", "ema_ratio", "mass_index"]]
        ).all(axis=1)
        & bars.bar_range.gt(0)
        & bars.range_ema1.gt(0)
        & bars.range_ema2.gt(0)
        & bars.ema_ratio.gt(0)
    )
    bars["source_valid"] = (
        mass_valid
        & np.isfinite(bars[["displacement", "direction", "variation"]]).all(axis=1)
        & bars.direction.ne(0)
        & bars.variation.gt(0)
    )
    bars["bulge_release"] = release_state(bars.mass_index, mass_valid)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    release = used.bulge_release.eq(True)
    variation = used.variation_rank.ge(P["variation_rank_min"])
    side = pd.to_numeric(used.direction, errors="coerce").fillna(0).astype(int)
    state = valid & side.ne(0) & release & variation
    if control == "no_variation_gate": state = valid & side.ne(0) & release
    elif control == "one_bar_stale_bulge":
        state = state.shift(1, fill_value=False); side = side.shift(1, fill_value=0)
    elif control=="direction_flip":side=-side
    elif control == "same_direction": side = -side
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVMIB prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvmib_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvmib_24_source_support_v1", "policy_id": prereg.POLICY_ID,
