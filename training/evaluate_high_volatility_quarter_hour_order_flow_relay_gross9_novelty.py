"""Evaluate frozen HVQHOFR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVQHOFR-8"
PROTOCOL = "hvqhofr_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_quarter_hour_order_flow_relay_preregistration_2026-08-11.json")
PREREG_SHA = "6a81cc414f2966d2b2ae3524cc45e9c36848c0eed2541cd7b048758d3f5558be"
SUPPORT = Path("results/high_volatility_quarter_hour_order_flow_relay_support_2026-08-11.json")
SUPPORT_SHA = "7fed9e4547f1e7e207904ce76c99b3545de9a2c5f3aa36adc2b04e7341c054bd"
CLOCK = Path("data/high_volatility_quarter_hour_order_flow_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "46bdb23aad37c6c80fd371b0439644d1acc5a9cb6b2843fd319bb518f08d3a29"
OUTPUT = Path("results/high_volatility_quarter_hour_order_flow_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVQHOFR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVQHOFR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVQHOFR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVQHOFR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvqhofr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVQHOFR-8 unchanged before economic outcomes",
    }
