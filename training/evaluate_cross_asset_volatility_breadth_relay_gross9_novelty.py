"""Evaluate frozen CAVBR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "CAVBR-12"
PROTOCOL = "cavbr_12_gross9_novelty_v1"
...
OUTPUT = Path("results/cross_asset_volatility_breadth_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CAVBR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("CAVBR novelty limits drift")
    if (
        ...
        raise RuntimeError("CAVBR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CAVBR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "cavbr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject CAVBR-12 unchanged before economic outcomes",
    }
