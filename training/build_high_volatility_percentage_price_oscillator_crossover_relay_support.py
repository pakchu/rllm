"""Build outcome-blind source support for frozen HVPPO-24."""
...
from training import preregister_high_volatility_percentage_price_oscillator_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "be6d04502fadf35019b3282eed3196d71eca36d6e01aafaec9254c439061c915"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_percentage_price_oscillator_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_percentage_price_oscillator_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_percentage_price_oscillator_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_percentage_price_oscillator_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_percentage_price_oscillator_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "fast_ema", "slow_ema", "ppo", "signal", "difference",
    "entry_side", "zero_line_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "fast_ema", "slow_ema", "ppo", "signal", "difference",
    "entry_side", "zero_line_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVPPO source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVPPO invalid source key")
...
def ppo_values(series: pd.Series, valid: pd.Series) -> pd.DataFrame:
    fast=causal_ema(series,valid,P["fast_periods"])
    slow=causal_ema(series,valid,P["slow_periods"])
    ppo=(100.0*(fast-slow)/slow).where(slow.gt(0))
    signal=causal_ema(ppo,np.isfinite(ppo),P["signal_periods"])
    return pd.DataFrame({"fast_ema":fast,"slow_ema":slow,"ppo":ppo,"signal":signal,"difference":ppo-signal},index=series.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    values=ppo_values(bars.bar_close,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["entry_side"]=cross_side(bars.difference);bars["zero_line_side"]=cross_side(bars.ppo)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["fast_ema","slow_ema","ppo","signal","difference","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVPPO prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvppo_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvppo_24_source_support_v1", "policy_id": prereg.POLICY_ID,
