"""Evaluate frozen HVVAR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVVAR-8"
PROTOCOL = "hvvar_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_value_area_rejection_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVVAR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVVAR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVVAR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVVAR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvvar_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVVAR-8 unchanged before economic outcomes",
    }
