"""Strict staged evaluator for frozen VTMS-288.
...
SUPPORT_COMMIT = "206494b689c397f6e8dabf7ec3a6337f813a1f28"
SUPPORT_SOURCE_SHA256 = (
    "5dde6454df4e4d54048443ffe71fbd29f25a7a9b0e7fae02012b45e958f01eef"
)
SUPPORT_DOCUMENT = Path(
    "docs/venue-ticket-migration-shock-support-freeze-2026-07-17.md"
)
SUPPORT_DOCUMENT_SHA256 = (
    "4e6ef08da1bad99f7e1902b61a889928d290e76ab175d6f300ccf3050fdefb5b"
)
SUPPORT_RESULT = Path(support.DEFAULT_OUTPUT)
SUPPORT_RESULT_SHA256 = (
    "e52016fd2fb8bbeaedb8bb81e66b4e5315c6ddd37073539577d9e46eae02fc23"
)
EVENT_CLOCK = Path(support.DEFAULT_CLOCK)
EVENT_CLOCK_SHA256 = "7baf6f7de33e66417061dbea6f51efc6ea4993b2b5f2b9e0c09627a68adc57e2"
...
EVALUATION_SOURCE = Path("training/evaluate_venue_ticket_migration_shock.py")
EVALUATION_FREEZE = Path(
    "results/venue_ticket_migration_shock_evaluator_freeze_2026-07-17.json"
)
STAGE1_OUTPUT = Path(
    "results/venue_ticket_migration_shock_stage1_2020_2022_2026-07-17.json"
)
STAGE2_OUTPUT = Path("results/venue_ticket_migration_shock_stage2_2023_2026-07-17.json")
...
COMPONENT_CONTROLS = support.COMPONENT_CONTROLS
REJECTION_PLACEBOS = (
...
class EvaluationConfig:
    ...
    maximum_cluster_p_value: float = 0.10
    minimum_mean_gross_underlying_bp: float = 20.0
    minimum_train_trades: int = 250
    minimum_selection_trades: int = 75
    minimum_selection_half_trades: int = 35


def verify_evaluation_config_contract() -> None:
    policy = prereg.Policy()
    cfg = EvaluationConfig()
    gates = prereg.build_manifest()["selection_protocol"]["gates"]
    expected = {
        ...
        "minimum_cagr_to_strict_mdd": gates["train_and_2023_cagr_to_strict_mdd_min"],
        "maximum_strict_mdd_pct": gates["train_and_2023_strict_mdd_pct_max"],
        "maximum_cluster_p_value": gates[
            "train_and_2023_weekly_cluster_signflip_p_max"
        ],
        "minimum_mean_gross_underlying_bp": gates[
            "train_and_2023_mean_gross_underlying_bp_min"
        ],
        "minimum_train_trades": gates["train_trades_min"],
        "minimum_selection_trades": gates["2023_trades_min"],
        "minimum_selection_half_trades": gates["2023_h1_and_h2_trades_min"],
    }
    actual = {key: getattr(cfg, key) for key in expected}
    if actual != expected:
        raise ValueError("VTMS-288 evaluator differs from frozen execution policy")
...
def verify_support_and_replay() -> tuple[
    ...
    for path, expected in (
        ...
        if _sha256(path) != expected:
            raise ValueError(f"frozen VTMS-288 support dependency changed: {path}")
    frozen = json.loads(SUPPORT_RESULT.read_text())
    if frozen.get("protocol", {}).get("outcomes_opened") is not False:
        raise ValueError("VTMS-288 support artifact opened outcomes")
    if frozen.get("support_decision") != "pass":
        raise ValueError("VTMS-288 support did not pass")
    if frozen.get("policy") != asdict(prereg.Policy()):
        raise ValueError("VTMS-288 support policy changed")
    ...
    if _canonical_clock(schedules["primary"]) != _canonical_clock(frozen_clock):
        raise ValueError("VTMS-288 primary clock replay differs from freeze")
    if _clock_sha256(schedules["primary"]) != EVENT_CLOCK_SHA256:
        raise ValueError("VTMS-288 primary clock hash differs from freeze")
    if source != frozen.get("source"):
        raise ValueError("VTMS-288 support source replay differs from freeze")
    for name, schedule in schedules.items():
        if name == "primary":
            continue
        expected = frozen["controls"][name]
        if len(schedule) != expected["nonoverlap_count"]:
            raise ValueError(f"VTMS-288 control row count changed: {name}")
        if _clock_sha256(schedule) != expected["clock_sha256"]:
            raise ValueError(f"VTMS-288 control clock changed: {name}")
...
def build_freeze_manifest(evaluation_source_commit: str) -> dict[str, Any]:
    if len(evaluation_source_commit) != 40:
        raise ValueError("VTMS-288 evaluator commit must be a full Git hash")
    verify_evaluation_config_contract()
    _, schedules, _ = verify_support_and_replay()
    core = {
        "candidate_id": "VTMS-288",
        ...
        "causal_quarantine_contract": (
            "origin/signal inputs must be clean at decision time; a source gap that "
            "occurs after entry never censors an already scheduled trade"
        ),
    }
    return _seal(core)
...
def verify_evaluation_freeze() -> dict[str, Any]:
    ...
    if not EVALUATION_FREEZE.is_file():
        raise ValueError("VTMS-288 evaluator was not frozen before outcomes")
    payload = json.loads(EVALUATION_FREEZE.read_text())
    core = {key: value for key, value in payload.items() if key != "manifest_hash"}
    if _canonical_hash(core) != payload.get("manifest_hash"):
        raise ValueError("VTMS-288 evaluator freeze manifest hash mismatch")
    if payload.get("candidate_id") != "VTMS-288":
        raise ValueError("VTMS-288 evaluator freeze candidate differs")
    if payload.get("evaluation_source") != str(EVALUATION_SOURCE):
        raise ValueError("VTMS-288 evaluator freeze source path differs")
    if payload.get("evaluation_source_sha256") != _sha256(EVALUATION_SOURCE):
        raise ValueError("VTMS-288 evaluator source differs from freeze")
    if payload.get("support_commit") != SUPPORT_COMMIT:
        raise ValueError("VTMS-288 support commit differs from freeze")
    if payload.get("evaluation_config") != asdict(EvaluationConfig()):
        raise ValueError("VTMS-288 evaluation configuration drifted")
    expected_controls = set(support.POLICY_NAMES)
    if (
        set(payload.get("control_names", [])) != expected_controls
        or set(payload.get("control_schedules", {})) != expected_controls
    ):
        raise ValueError("VTMS-288 evaluator freeze control set differs")
    if payload.get("sealed_windows") != SEALED_WINDOWS:
        raise ValueError("VTMS-288 evaluator freeze window set differs")
    source = prereg.build_manifest()["source_contract"]
    if (
        payload.get("market_sha256") != source["market_sha256"]
        or payload.get("funding_data_sha256") != FUNDING_DATA_SHA256
        or payload.get("funding_manifest_sha256") != FUNDING_MANIFEST_SHA256
    ):
        raise ValueError("VTMS-288 evaluator freeze execution source differs")
    if payload.get("opened_windows") != [] or payload.get("mutable_parameters") != []:
        raise ValueError("VTMS-288 evaluator freeze is not sealed")
    if payload.get("execution_ohlc_rows_parsed_during_freeze") != 0:
        raise ValueError("VTMS-288 freeze parsed execution OHLC")
    if payload.get("funding_settlement_marks_loaded_during_freeze") != 0:
        raise ValueError("VTMS-288 freeze loaded funding marks")
    if payload.get("execution_simulation_run_during_freeze") is not False:
        raise ValueError("VTMS-288 freeze simulated outcomes")
    _, schedules, _ = verify_support_and_replay()
    for name, schedule in schedules.items():
        ...
        if expected["rows"] != len(schedule) or expected[
            ...
            raise ValueError(f"VTMS-288 frozen clock drifted: {name}")
    return payload
...
def _parse_market_before(path: Path, cutoff: pd.Timestamp) -> pd.DataFrame:
    ...
    with gzip.open(path, "rt", encoding="utf-8", newline="") as handle:
        ...
        if positions["date"] != 0:
            raise ValueError("VTMS-288 market date must be the first physical column")
    ...
    if cutoff < pd.Timestamp("2024-01-01") and not boundary_seen:
        raise ValueError("VTMS-288 market source did not reach the sealed boundary")
...
def _parse_funding_before(path: Path, cutoff: pd.Timestamp) -> pd.DataFrame:
    ...
    with gzip.open(path, "rt", encoding="utf-8", newline="") as handle:
        ...
        if positions["funding_time_ms"] != 0:
            raise ValueError("VTMS-288 funding milliseconds must be first")
        for line in handle:
    ...
    if cutoff < pd.Timestamp("2024-01-01") and not boundary_seen:
        raise ValueError("VTMS-288 funding source did not reach the sealed boundary")
    frame = pd.DataFrame(rows, columns=wanted)
...
def load_execution_inputs(
    ...
    if _sha256(prereg.MARKET_PATH) != source["market_sha256"]:
        raise ValueError("VTMS-288 execution market hash changed")
    if _sha256(FUNDING_DATA) != FUNDING_DATA_SHA256:
        raise ValueError("VTMS-288 funding marks hash changed")
    if _sha256(FUNDING_MANIFEST) != FUNDING_MANIFEST_SHA256:
        raise ValueError("VTMS-288 funding manifest hash changed")
    funding_manifest = json.loads(FUNDING_MANIFEST.read_text())
    if funding_manifest.get("outcomes_opened") is not False:
        raise ValueError("VTMS-288 funding mark source opened outcomes")
    ...
    if not market["date"].equals(expected_dates):
        raise ValueError("VTMS-288 execution prefix is not aligned to the signal clock")
    values = market[["open", "high", "low", "close"]].to_numpy(float)
    if not np.isfinite(values).all() or (values <= 0.0).any():
        raise ValueError("VTMS-288 execution market has invalid prices")
    ...
    if (
        ...
        raise ValueError("VTMS-288 market violates OHLC invariants")
    ...
    if (
        ...
        raise ValueError("VTMS-288 funding source has invalid values")
    if len(funding) and funding["funding_time"].max() >= cutoff:
        raise ValueError("VTMS-288 funding parser crossed cutoff")
    return (
        market,
        funding,
        {
...
def linear_pnl(*, side: int, quantity: float, entry: float, mark: float) -> float:
    if side not in {-1, 1} or min(quantity, entry, mark) <= 0.0:
        raise ValueError("invalid VTMS-288 linear-ledger input")
...
def weekly_cluster_signflip(
    ...
    *,
    ...
    if len(trade_returns) != len(entry_dates):
        raise ValueError("VTMS-288 trade returns and dates differ in length")
...
def simulate_schedule(
    ...
    *,
    ...
    if cfg.leverage <= 0.0 or not 0.0 <= cost_rate < 0.1 or period_end <= period_start:
        raise ValueError("invalid VTMS-288 simulation configuration")
    policy = prereg.Policy()
    equity = 1.0
    high_water = 1.0
    strict_mdd = 0.0
    ...
    for event in schedule.itertuples(index=False):
        ...
        side = int(event.side)
        if side not in {-1, 1}:
            raise ValueError("VTMS-288 side must be long or short")
        if entry_position != signal_position + policy.execution_delay_bars:
            raise ValueError("VTMS-288 entry differs from frozen t+2 delay")
        if exit_position != entry_position + policy.hold_bars:
            raise ValueError("VTMS-288 exit differs from frozen hold")
        if (
            int(event.delay_bars) != policy.execution_delay_bars
            or int(event.hold_bars) != policy.hold_bars
        ):
            raise ValueError("VTMS-288 schedule metadata differs from freeze")
        if not origin_position <= signal_position < entry_position < exit_position:
            raise ValueError("VTMS-288 scheduled positions are invalid")
        if entry_position < previous_exit:
            raise ValueError("VTMS-288 schedules overlap")
        if exit_position >= len(market):
            raise ValueError("VTMS-288 schedule exceeds parsed market prefix")
        dates = market["date"]
        if not (
            ...
            raise ValueError("VTMS-288 trade crosses simulation split")
        frozen_times = {
        ...
        for label, (position, frozen_time) in frozen_times.items():
            if pd.Timestamp(dates.iloc[position]) != pd.Timestamp(frozen_time):
                raise ValueError(
                    f"VTMS-288 {label} timestamp differs from frozen clock"
                )
...
def branch_diagnostics(
    ...
    *,
    ...
    expected = {"primary_spot_dominant", "primary_perp_dominant"}
    observed = set(schedule["branch"].astype(str).unique()) if len(schedule) else set()
    if not observed.issubset(expected):
        raise ValueError("VTMS-288 primary schedule has an unknown branch")
    return {
        branch: simulate_schedule(
            market,
            funding,
            schedule.loc[schedule["branch"].eq(branch)].reset_index(drop=True),
...
def _stage_gate(metrics: dict[str, Any], stress: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        ...
        "cagr_to_strict_mdd_at_least_3": metrics["cagr_to_strict_mdd"]
        >= cfg.minimum_cagr_to_strict_mdd,
        "strict_mdd_at_most_15pct": metrics["strict_mdd_pct"]
        <= cfg.maximum_strict_mdd_pct,
        "weekly_cluster_p_at_most_0p10": metrics["weekly_cluster_signflip"][
            "p_value_one_sided"
        ]
        <= cfg.maximum_cluster_p_value,
...
def _passes_stage2_gate(
    ...
    return (
        ...
        and _passes_stage_gate(
            metrics, stress, minimum_trades=cfg.minimum_selection_trades
        )
        and all(
            value["absolute_return_pct"] > 0.0
            and value["trade_count"] >= cfg.minimum_selection_half_trades
...
def verify_stage1_output() -> tuple[dict[str, Any], dict[str, Any]]:
    if not STAGE1_OUTPUT.is_file():
        raise ValueError("VTMS-288 stage 1 result is missing")
    ...
    if _canonical_hash(core) != stage1.get("manifest_hash"):
        raise ValueError("VTMS-288 stage 1 manifest hash mismatch")
    if stage1.get("candidate_id") != "VTMS-288":
        raise ValueError("VTMS-288 stage 1 candidate differs")
    if stage1.get("stage") != "stage1_2020_2022":
        raise ValueError("VTMS-288 stage 1 label differs")
    ...
    if (
        ...
        raise ValueError("VTMS-288 stage 1 did not authorize 2023")
    freeze = verify_evaluation_freeze()
    if stage1.get("evaluation_freeze_sha256") != _sha256(EVALUATION_FREEZE):
        raise ValueError("VTMS-288 stage 1 used a different evaluator freeze")
    if stage1.get("evaluation_source_commit") != freeze["evaluation_source_commit"]:
        raise ValueError("VTMS-288 stage 1 used a different evaluator source")
    source = stage1.get("source")
    if not isinstance(source, dict) or source.get("cutoff") != STAGE1_END.isoformat():
        raise ValueError("VTMS-288 stage 1 physical cutoff differs")
    for key in ("last_market_time", "last_funding_time"):
        value = source.get(key)
        if value is not None and pd.Timestamp(value) >= STAGE1_END:
            raise ValueError(f"VTMS-288 stage 1 {key} crossed 2023")
    expected = evaluate_stage1()
    if stage1 != expected:
        raise ValueError("VTMS-288 stage 1 differs from deterministic replay")
...
def evaluate_stage1() -> dict[str, Any]:
    ...
    gate["trade_count_at_least_250"] = base["trade_count"] >= cfg.minimum_train_trades
    ...
    passing_placebos = [
        ...
        if _passes_stage_gate(
            ...
            minimum_trades=cfg.minimum_train_trades,
        )
    ]
...
def evaluate_stage2() -> dict[str, Any]:
    ...
    gate = _stage_gate(base, stress)
    gate["trade_count_at_least_75"] = (
        base["trade_count"] >= cfg.minimum_selection_trades
    )
...
def _write_once(path: Path, payload: dict[str, Any]) -> str:
    ...
    if path.exists():
        if path.read_bytes() != content:
            raise RuntimeError(f"refusing to overwrite frozen VTMS-288 result: {path}")
