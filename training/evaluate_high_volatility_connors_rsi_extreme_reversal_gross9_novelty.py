"""Evaluate frozen HVCRSI-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCRSI-24"
PROTOCOL = "hvcrsi_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_connors_rsi_extreme_reversal_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCRSI predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCRSI novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCRSI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCRSI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcrsi_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCRSI-24 unchanged before economic outcomes",
    }
