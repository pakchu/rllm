"""Evaluate frozen HVFI-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVFI-24"
PROTOCOL = "hvfi_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_elder_force_index_zero_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVFI predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVFI novelty limits drift")
    if (
        ...
        raise RuntimeError("HVFI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVFI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVFI-24 unchanged before economic outcomes",
    }
