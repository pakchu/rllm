"""Evaluate frozen HVUI-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVUI-12"
PROTOCOL = "hvui_12_gross9_novelty_v1"
PREREG = Path("results/high_volatility_ulcer_index_stress_trend_relay_preregistration_2026-08-11.json")
PREREG_SHA = "3dd1b892d9810daebb282f0e29035ae47f49319030d45ecf4c535d707b9dcaf4"
SUPPORT = Path("results/high_volatility_ulcer_index_stress_trend_relay_support_2026-08-11.json")
SUPPORT_SHA = "9a09636643675b6aaed9159883e029b2647d605e5f774fab20a8b2fda170ab17"
CLOCK = Path("data/high_volatility_ulcer_index_stress_trend_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "2e986274580000f5da871dad3e6ef109eec5d68c405ed4f0fde5edf1bdff9889"
OUTPUT = Path("results/high_volatility_ulcer_index_stress_trend_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVUI predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVUI novelty limits drift")
    if (
        ...
        raise RuntimeError("HVUI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVUI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvui_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVUI-12 unchanged before economic outcomes",
    }
