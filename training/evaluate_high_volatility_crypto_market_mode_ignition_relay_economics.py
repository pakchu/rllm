"""Sequential strict economics for frozen HVCMMI-8."""
...
PREREG = Path("results/high_volatility_crypto_market_mode_ignition_relay_preregistration_2026-08-10.json")
PREREG_SHA = "78c1a90c8aef0d67cb00f1951afb8fb1b49c427632b490b9cd37aa4dfc10e9e2"
SUPPORT = Path("results/high_volatility_crypto_market_mode_ignition_relay_support_2026-08-10.json")
SUPPORT_SHA = "ca2e22d1dd96d90a015fdb3024b64571edeb94e38d0aed02ae91bb28359b382c"
NOVELTY = Path("results/high_volatility_crypto_market_mode_ignition_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "d00a81c1a4021bad945d9498e449819e04e43505ef44b83688f57d6a8ddbebfc"
CLOCK = Path("data/high_volatility_crypto_market_mode_ignition_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "faa4f8c09b107c6846f0d8143aa740d9f5e665c4c628696af7fc535f572c3aab"
CONTROL_DIR = Path("data/high_volatility_crypto_market_mode_ignition_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_crypto_market_mode_ignition_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_crypto_market_mode_ignition_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_btc_variation_gate", "no_mode_onset", "equal_weight_final_hour", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCMMI frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCMMI novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcmmi_8_sequential_economics_v1", "policy_id": POLICY_ID,
