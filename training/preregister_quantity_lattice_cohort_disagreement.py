"""Freeze QLCD-288 before source incidence or market outcomes are opened."""
...
DEFAULT_OUTPUT = "results/quantity_lattice_cohort_disagreement_preregistration_2026-07-20.json"
ARCHIVE_MANIFEST = (
...
class Policy:
    policy_id: str = "QLCD-288"
    baseline_bars: int = 8_640
    baseline_min_periods: int = 2_016
    score_quantile: float = 0.9975
    minimum_bar_agg_trade_count: int = 64
    minimum_coarse_event_count: int = 3
    minimum_fine_event_count: int = 16
...
COMPARATOR_REGISTRY: list[dict[str, Any]] = [
    {
        "family": "MFIC",
...
def build_manifest() -> dict[str, Any]:
    core: dict[str, Any] = {
        "protocol_version": "quantity_lattice_cohort_disagreement_v1",
        ...
        "source_contract": {
            ...
            "builder": "training.build_binance_quantity_lattice_cohort",
            "transform": "preprocessing.quantity_lattice_cohort",
            "range": ["2020-01-01", "2024-01-01"],
            "end_is_exclusive": True,
            "raw_archives_persisted": False,
            "quantity_contract": (
                "parse positive BTC quantity and require quantity*1000 to equal an "
                "integer within 1e-9 absolute tolerance"
            ),
            ...
            "underlying_overlap_quarantine": {"2020-01-15": 1},
            "gap_policy": (
                "quarantine required UTC days, verified missing buckets, and the "
                "following 24 five-minute bars; never impute"
            ),
        },
        "feature_contract": {
            "candidate_count": 1,
            "bucket": "completed UTC five-minute bar t",
            "coarse": "quantity_mbtc % 100 == 0 (multiples of 0.1 BTC)",
            "medium": (
                "quantity_mbtc % 10 == 0 and quantity_mbtc % 100 != 0; "
                "diagnostic/control only"
            ),
            "fine": "quantity_mbtc % 10 != 0 (remaining 0.001-BTC increments)",
            "aggressive_side": "+1 when is_buyer_maker=false, else -1",
            "coarse_share": "coarse_quantity_mbtc / total_quantity_mbtc",
            "coarse_coherence": (
                "abs(coarse_signed_quantity_mbtc) / coarse_quantity_mbtc"
            ),
            "fine_signed_share": (
                "fine_signed_quantity_mbtc / fine_quantity_mbtc"
            ),
            "coarse_side": "sign(coarse_signed_quantity_mbtc)",
            "opposition": "max(-coarse_side * fine_signed_share, 0)",
            "score": "coarse_share * coarse_coherence * opposition",
            "direction": "coarse_side",
            "price_fields_used": False,
            "notional_fields_used": False,
            "threshold_grid": False,
        },
        "support_schedule_contract": {
            "threshold": (
                "mask score to source_complete, shift one calendar row, then "
                "rolling(8640,min_periods=2016).quantile(0.9975)"
            ),
            "eligibility": (
                "source_complete; agg_trade_count>=64; coarse_event_count>=3; "
                "fine_event_count>=16; coarse_side!=0; opposition>0; score>0; "
                "score>=strictly-prior threshold"
            ),
            "decision_time": "after completed bar t closes",
            "entry_time": "bucket date at t+2",
            "exit_time": "bucket date at entry_position+288",
            "nonoverlap": (
                "accept eligible bars chronologically; skip while entry precedes the "
                "last scheduled exit; re-entry at the exit timestamp is allowed"
            ),
            "future_source_rule": (
            ...
            "severity_rationale": (
                "q99.75 defines an exceptional joint coarse-share/coherence/opposition "
                "state on a bounded [0,1] score; it is frozen without incidence and is "
                "not selected from an event-count grid"
            ),
            "hold_rationale": (
                "288 five-minute bars span one full UTC day and three ordinary eight-hour "
                "perpetual funding cycles, the fixed horizon for cross-session inventory "
                "resolution; no hold alternative is registered"
            ),
        },
        "support_gates": {
            "total_2020_2023_min": 200,
            "total_2020_2023_max": 800,
            "each_calendar_year_min": 35,
        ...
        "novelty_gates": {
            ...
            "dense_bafr": {
                ...
                "errors_gate_decision": False,
            },
            "missing_or_malformed_comparator": (
                "sparse registry fails closed; dense BAFR remains report-only"
            ),
            "explicit_exclusions": {
                "NETF": (
                    "no canonical NETF clock artifact exists; RIFT is the frozen topology "
                    "substitute already used by prior microstructure novelty batteries"
                ),
                "PRIOR_TRADE_SIZE_SCAN": (
                    "the candle-average trade-size search has no frozen outcome-blind "
                    "canonical clock; AFCS and MFIC are the closest executable size/fill "
                    "comparators, while QLCD uses exact quantity residue classes"
                ),
            },
        },
        "support_artifact_contract": {
            ...
            "clock": "data/quantity_lattice_cohort_disagreement_clock_2020_2023.csv.gz",
            "result": "results/quantity_lattice_cohort_disagreement_support_2026-07-20.json",
            "evaluator": (
        ...
        "falsification_controls": {
            ...
            "medium_vs_fine": (
                "replace coarse cohort with medium cohort, same q99.75 prior-only "
                "score and scheduler"
            ),
            "remove_opposition": (
                "score=coarse_share*coarse_coherence, no fine-flow opposition gate, "
                "own q99.75 prior-only clock"
            ),
            "all_quantity_imbalance": (
                "score=abs(total_signed_quantity_mbtc)/total_quantity_mbtc, direction "
                "its sign, own q99.75 prior-only clock"
            ),
        ...
        "claim_boundary": {
            "claim": (
                "when exact-denomination coarse aggregate-event quantity flow opposes "
                "fine-lot flow, the coarse cohort may lead slower inventory resolution"
            ),
            "not_claimed": [
                "participant identity",
                "institutional ownership",
                "one parent order",
                "profitability of round-number trading in general",
            ],
        },
        "rejection_contract": (
            "any source, support, novelty, train, or sequential-stage failure retires "
            "QLCD-288 without changing lattice tiers, direction, quantile, delay, hold, "
            "costs, or gates"
        ),
    }
    return {
...
def validate_manifest(payload: dict[str, Any]) -> None:
    ...
    if canonical_hash(core) != payload.get("manifest_hash"):
        raise RuntimeError("QLCD-288 preregistration hash mismatch")
    if payload.get("outcomes_opened") is not False:
        raise RuntimeError("QLCD-288 preregistration cannot open outcomes")
    if payload.get("source_incidence_opened") is not False:
        raise RuntimeError("QLCD-288 preregistration must precede source incidence")
    if payload.get("policy") != asdict(Policy()):
        raise RuntimeError("QLCD-288 frozen policy differs from code")
    if payload.get("feature_contract", {}).get("candidate_count") != 1:
        raise RuntimeError("QLCD-288 must remain a singleton policy")
...
def write_manifest_once(path: str | Path, payload: dict[str, Any]) -> str:
    ...
    if output.exists():
        ...
        if existing["manifest_hash"] != payload["manifest_hash"]:
            raise RuntimeError("refusing to overwrite frozen QLCD-288 preregistration")
