"""Sequential strict economics for frozen HVCMO-24."""
...
PREREG = Path("results/high_volatility_chande_momentum_oscillator_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "010b09bd881c753cd9c7e28a6763e6558df8eb4bd24d959768556869eba528cc"
SUPPORT = Path("results/high_volatility_chande_momentum_oscillator_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "64f61d951fefbbeae60ab48812049b2bdd7a72d05f38abb82ef2c80ef029de0e"
NOVELTY = Path("results/high_volatility_chande_momentum_oscillator_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "58dc181656ce3c99749b9a6631fddd6a62cc4aa3c6de6fc75a5469ee48f7a772"
CLOCK = Path("data/high_volatility_chande_momentum_oscillator_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "faa18a604b3448dc86b861eb7c79bfdb84a482c5e063b5c3080b16bac0c47661"
CONTROL_DIR = Path("data/high_volatility_chande_momentum_oscillator_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_chande_momentum_oscillator_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_chande_momentum_oscillator_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCMO frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCMO novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcmo_24_sequential_economics_v1", "policy_id": POLICY_ID,
