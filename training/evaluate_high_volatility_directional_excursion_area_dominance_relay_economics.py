"""Sequential strict economics for frozen HVDEADR-8."""
...
PREREG = Path("results/high_volatility_directional_excursion_area_dominance_relay_preregistration_2026-08-10.json")
PREREG_SHA = "5e6d73b848addeeaa6fcab1b4f9958e4660c452e67980ecdc58f06c264396cba"
SUPPORT = Path("results/high_volatility_directional_excursion_area_dominance_relay_support_2026-08-10.json")
SUPPORT_SHA = "6dd3d5567eea0057b9d31627ccff0e11813b9bb53ac153f98d1564f45c896851"
NOVELTY = Path("results/high_volatility_directional_excursion_area_dominance_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "724c7def568e3d21901d8d1e3a97c5ce915876d689d4530827e7abb0b22b51d9"
CLOCK = Path("data/high_volatility_directional_excursion_area_dominance_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "eefaced770338127c1274f5dfcc9c6bb8b4475e841ff97bb1dda1f98dec4e857"
CONTROL_DIR = Path("data/high_volatility_directional_excursion_area_dominance_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_directional_excursion_area_dominance_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_directional_excursion_area_dominance_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_dominance_tail", "no_variation_gate", "endpoint_displacement", "one_block_stale_area", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDEADR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDEADR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdeadr_8_sequential_economics_v1", "policy_id": POLICY_ID,
