"""Evaluate frozen CLIRR-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "CLIRR-6"
PROTOCOL = "clirr_6_gross9_novelty_v1"
...
OUTPUT = Path("results/confirmation_ladder_impulse_reversal_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CLIRR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("CLIRR novelty limits drift")
    if (
        ...
        raise RuntimeError("CLIRR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CLIRR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "clirr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject CLIRR-6 unchanged before economic outcomes",
    }
