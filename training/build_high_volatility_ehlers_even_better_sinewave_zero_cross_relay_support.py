"""Build outcome-blind source support for frozen HVEBS-24."""
...
from training import preregister_high_volatility_ehlers_even_better_sinewave_zero_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "c48b094eeb1588f861f24e3b374d8844985512f4314f869e648414f0bd5c9a46"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ehlers_even_better_sinewave_zero_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ehlers_even_better_sinewave_zero_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ehlers_even_better_sinewave_zero_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ehlers_even_better_sinewave_zero_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ehlers_even_better_sinewave_zero_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "run_length", "high_pass", "filtered", "wave", "power", "normalized_wave",
    "entry_side", "filtered_cross_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "run_length", "high_pass", "filtered", "wave", "power", "normalized_wave",
    "entry_side", "filtered_cross_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVEBS source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVEBS invalid source key")
...
def even_better_sinewave(series: pd.Series, valid: pd.Series) -> pd.DataFrame:
    duration=float(P["duration"]);ss_period=float(P["super_smoother_period"]);angle=2*math.pi/duration;alpha=(1-math.sin(angle))/math.cos(angle)
    a1=math.exp(-1.414*math.pi/ss_period);c2=2*a1*math.cos(1.414*math.pi/ss_period);c3=-a1*a1;c1=1-c2-c3
    values=series.to_numpy(float);ok=valid.to_numpy(bool);hp=np.full(len(series),np.nan);filt=np.full(len(series),np.nan);wave=np.full(len(series),np.nan);power=np.full(len(series),np.nan);normalized=np.full(len(series),np.nan);runs=np.zeros(len(series),dtype=int);run=0
    for i,(price,is_valid) in enumerate(zip(values,ok)):
        if not is_valid or not math.isfinite(price):run=0;continue
        run+=1;runs[i]=run
        if run==1:hp[i]=0.;filt[i]=0.;continue
        hp[i]=.5*(1+alpha)*(price-values[i-1])+alpha*hp[i-1]
        if run==2:filt[i]=0.;continue
        filt[i]=c1*(hp[i]+hp[i-1])/2+c2*filt[i-1]+c3*filt[i-2]
        wave[i]=(filt[i]+filt[i-1]+filt[i-2])/3;power[i]=(filt[i]**2+filt[i-1]**2+filt[i-2]**2)/3
        if power[i]>0:normalized[i]=wave[i]/math.sqrt(power[i])
    return pd.DataFrame({"run_length":runs,"high_pass":hp,"filtered":filt,"wave":wave,"power":power,"normalized_wave":normalized},index=series.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    fields=["bar_open","bar_high","bar_low","bar_close"];bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[fields]).all(axis=1)&bars[fields].gt(0).all(axis=1);values=even_better_sinewave(bars.bar_close,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["entry_side"]=cross_side(bars.normalized_wave);bars["filtered_cross_side"]=cross_side(bars.filtered);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["high_pass","filtered","wave","power","normalized_wave","variation"];bars["source_valid"]=bars.valid_bar&bars.run_length.ge(P["warmup_observations"])&np.isfinite(bars[numeric]).all(axis=1)&bars.power.gt(0)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="unnormalized_filtered_cross":side=pd.to_numeric(used.filtered_cross_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVEBS prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvebs_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvebs_24_source_support_v1", "policy_id": prereg.POLICY_ID,
