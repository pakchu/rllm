"""Build outcome-blind source support for frozen HV3INSIDE-R10-8."""
...
PREREG_SHA = "308c39e5745fdc2abc72c9c22411716272a3ebf89a1d4d43e6d5499794c46816"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_three_inside_reversal_relay_sources_2023_2026")
PANEL = ROOT / "hourly_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_three_inside_reversal_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_three_inside_reversal_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_three_inside_reversal_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_three_inside_reversal_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "three_inside_ready", "three_inside_output", "entry_side",
    "no_body_size_side", "containment_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "three_inside_ready", "three_inside_output",
    "entry_side", "no_body_size_side", "containment_side", "variation",
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HV3INSIDE source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HV3INSIDE invalid source key")
...
def three_inside_outputs(bars: pd.DataFrame, valid: pd.Series) -> pd.DataFrame:
    """Replicate LEAN ThreeInside defaults, readiness, and strict inequalities."""
    output = np.zeros(len(bars), dtype=int)
    ready = np.zeros(len(bars), dtype=bool)
    no_size = np.zeros(len(bars), dtype=int)
    containment_only = np.zeros(len(bars), dtype=int)
    ...
    for index, is_valid in enumerate(valid.to_numpy(bool)):
        ...
        history.append((float(current.bar_open), float(current.bar_close)))
        history = history[-P["indicator_period_hours"] :]
        ready[index] = len(history) >= P["indicator_period_hours"]
        if not ready[index]:
            continue
        first_open, first_close = history[-3]
        second_open, second_close = history[-2]
        current_open, current_close = history[-1]
        bodies = [abs(close - open_) for open_, close in history]
        long_average = float(np.mean(bodies[-13:-3])) * P["body_long_factor"]
        short_average = float(np.mean(bodies[-12:-2])) * P["body_short_factor"]
        ...
        current_color = 1 if current_close >= current_open else -1
        contained = (
            max(second_close, second_open) < max(first_close, first_open)
            and min(second_close, second_open) > min(first_close, first_open)
        )
        body_sizes = first_body > long_average and second_body <= short_average
        confirmed = (
            (first_color == 1 and current_color == -1 and current_close < first_open)
            or (first_color == -1 and current_color == 1 and current_close > first_open)
        )
        side = -first_color
        if contained and confirmed:
            no_size[index] = side
        if body_sizes and contained:
            containment_only[index] = side
        if body_sizes and contained and confirmed:
            output[index] = side
    entry = output.copy()
    return pd.DataFrame(
        {
            "three_inside_ready": ready,
            "three_inside_output": output,
            "entry_side": entry,
            "no_body_size_side": no_size,
            "containment_side": containment_only,
        },
        index=bars.index,
    )
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control == "no_body_size_requirements":
        side = pd.to_numeric(used.no_body_size_side, errors="coerce").fillna(0).astype(int)
        state = valid & side.ne(0) & variation
    elif control == "containment_without_third_confirmation":
        side = pd.to_numeric(used.containment_side, errors="coerce").fillna(0).astype(int)
        state = valid & side.ne(0) & variation
        ...
        rows.append(
            {
                ...
                **{
                    ...
                    if column in ("three_inside_ready", "eligible")
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HV3INSIDE prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hv3inside_r10_8_source_v1",
        "query": QUERY,
    ...
    core = {
        "protocol_version": "hv3inside_r10_8_source_support_v1",
