"""Evaluate frozen HVDRA-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDRA-8"
PROTOCOL = "hvdra_8_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_directional_return_acceleration_relay_preregistration_2026-08-09.json"
)
...
OUTPUT = Path(
    "results/high_volatility_directional_return_acceleration_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDRA predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVDRA novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDRA predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDRA primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject HVDRA-8 unchanged before economic outcomes"
        ),
    }
