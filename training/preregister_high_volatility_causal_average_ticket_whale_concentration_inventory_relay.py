"""Outcome-blind preregistration for HVCATWIR-8."""
...
POLICY_ID = "HVCATWIR-8"
DEFAULT_OUTPUT = Path("results/high_volatility_causal_average_ticket_whale_concentration_inventory_relay_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_causal_average_ticket_whale_concentration_inventory_relay_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": "When BTC perpetual average quote ticket size accelerates unusually from the first to the second four-hour half, transaction arrivals decelerate while average ticket size accelerates from the first to the second four-hour half, realized variation is elevated, and open interest expands, fewer but larger transactions and fresh leveraged inventory sponsor the completed direction for eight hours.",
            "side": "strict sign of the completed eight-hour BTC perpetual return",
            "why_distinct": "HVCATWIR requires within-cycle acceleration in quote turnover per transaction and requires simultaneous deceleration in transaction arrival count, separating concentrated large-order sponsorship from broad activity, plus exact OI expansion and variation. No prior composite event set or control is reused; it uses no spot, ETH, funding, premium-index, fitted outcome, or promoted control.",
        ...
        "features": {
            ...
            "trade_arrival_deceleration": "log(first-half transaction count/second-half transaction count), finite strict positive",
            ...
            "eligible": "exact coherent completed perpetual cycle, positive average-ticket acceleration, nonzero completed return, strict positive trade-arrival deceleration, two exact OI endpoints with strict expansion, and both average-ticket-acceleration and variation ranks pass at D; no close-location gate, taker-flow sign, spot source, funding, premium-index, alt, or post-decision condition",
            "no_imputation": True,
        },
        "clock": {
        ...
        "source_plan": {
            ...
            "bars": "Postgres bars_binance BTCUSDT exact coherent 1m OHLC, quote_asset_volume, and positive integer number_of_trades",
            "window": ["2020-01-01T00:00:00Z", "2026-08-01T00:00:00Z"],
            "read_after_preregistration": True,
            "execution_prices": "sealed until source support and Gross9 pass",
        },
        "research_boundary": {
            "prior_HVCATTAIR_source_novelty_and_train_failure_known": True,
            "same_mechanism_as_HVCATTAIR": False,
        ...
        "stopping_rule": "terminal first failure; no OI endpoint or expansion rule, average-ticket acceleration formula, trade-arrival deceleration formula, acceleration or variation history, side, clock, hold, subset, threshold, comparator, or control repair",
    }
