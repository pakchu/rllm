"""Build source-support clocks for CBSTR-6 without BTC outcomes."""
...
from training import preregister_cboe_broad_slope_crypto_volatility_transmission_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv

CLOCK=Path("data/cboe_broad_slope_crypto_volatility_transmission_relay_clocks_2023_2026.csv.gz");CONTROL_DIR=Path("data/cboe_broad_slope_crypto_volatility_transmission_relay_controls_2023_2026");RESULT=Path("results/cboe_broad_slope_crypto_volatility_transmission_relay_support_2026-08-08.json")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00:00Z"),pd.Timestamp("2024-01-01T00:00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00:00Z"),pd.Timestamp("2025-01-01T00:00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00:00Z"),pd.Timestamp("2026-01-01T00:00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00:00Z"),pd.Timestamp("2026-08-01T00:00:00Z"))};MIN={"train":8,"test":12,"eval":12,"final":8};CONTROLS=("no_crypto_confirmation","bvol_only_confirmation","dvol_only_confirmation","one_session_stale_broad_slope_change","direction_flip");NY=ZoneInfo("America/New_York");ECONOMIC_OUTCOMES_AUTHORIZED=False
COLUMNS=("candidate","control","split","cboe_observation_date","next_cboe_source_date","decision_time","feature_available_time","entry_time","exit_time","side","delta_broad_slope","previous_delta_broad_slope","bvol_body","dvol_body")
...
def features()->pd.DataFrame:
 for raw,want in prereg.HASHES.items():
  if sha(Path(raw))!=want:raise RuntimeError(f"CBSTR source drift: {raw}")
 c=pd.read_csv(prereg.CBOE,compression="gzip",usecols=["observation_date","VIX_close","VIX3M_close"]);c.observation_date=pd.to_datetime(c.observation_date,format="%Y-%m-%d");c.VIX_close=pd.to_numeric(c.VIX_close,errors="coerce");c.VIX3M_close=pd.to_numeric(c.VIX3M_close,errors="coerce")
 if c.observation_date.duplicated().any() or not c.observation_date.is_monotonic_increasing or not np.isfinite(c[["VIX_close","VIX3M_close"]]).all(axis=1).all() or not c[["VIX_close","VIX3M_close"]].gt(0).all(axis=1).all():raise RuntimeError("CBSTR Cboe source invalid")
 c["delta_broad_slope"]=np.log(c.VIX_close/c.VIX3M_close).diff();c["previous_delta_broad_slope"]=c.delta_broad_slope.shift(1)
 ...
 for i in range(1,len(c)-1):
  ...
  x=v.loc[decision];rows.append({"observation_date":c.at[i,"observation_date"].date(),"next_source_date":nd.date(),"decision_time":decision,"delta_broad_slope":float(c.at[i,"delta_broad_slope"]),"previous_delta_broad_slope":float(c.at[i,"previous_delta_broad_slope"]),"bvol_body":float(x.bvol_body),"dvol_body":float(x.dvol_body),"valid":bool(x.bvol_valid and x.dvol_valid)})
 return pd.DataFrame(rows)
def sides(f:pd.DataFrame,control:str)->pd.Series:
 delta=f.previous_delta_broad_slope if control=="one_session_stale_broad_slope_change" else f.delta_broad_slope;valid=f.valid&delta.notna()&delta.ne(0)&f.bvol_body.ne(0)&f.dvol_body.ne(0);same_b=np.sign(f.bvol_body).eq(np.sign(delta));same_d=np.sign(f.dvol_body).eq(np.sign(delta))
...
def clock(f:pd.DataFrame,control:str="primary")->pd.DataFrame:
 ...
 for i in f.index[s.ne(0)]:
  ...
  next_allowed=exit_;rows.append({"candidate":"CBSTR-6","control":control,"split":split,"cboe_observation_date":f.at[i,"observation_date"].isoformat(),"next_cboe_source_date":f.at[i,"next_source_date"].isoformat(),"decision_time":decision,"feature_available_time":decision,"entry_time":entry,"exit_time":exit_,"side":int(s.at[i]),"delta_broad_slope":float(f.at[i,"delta_broad_slope"]),"previous_delta_broad_slope":float(f.at[i,"previous_delta_broad_slope"]),"bvol_body":float(f.at[i,"bvol_body"]),"dvol_body":float(f.at[i,"dvol_body"])})
