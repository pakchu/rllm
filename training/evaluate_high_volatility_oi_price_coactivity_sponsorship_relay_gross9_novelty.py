"""Evaluate frozen HVOIPCSR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVOIPCSR-8"
PROTOCOL = "hvoipcsr_8_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_oi_price_coactivity_sponsorship_relay_preregistration_2026-08-10.json"
)
...
OUTPUT = Path(
    "results/high_volatility_oi_price_coactivity_sponsorship_relay_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVOIPCSR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVOIPCSR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVOIPCSR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVOIPCSR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvoipcsr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVOIPCSR-8 unchanged before economic outcomes"
        ),
    }
