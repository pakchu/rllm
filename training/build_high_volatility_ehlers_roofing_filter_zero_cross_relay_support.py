...
from training import preregister_high_volatility_ehlers_roofing_filter_zero_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "8ecb968c49b12e420b7cd600e8d5eb83253a882f53f0fb5cfad64bfed094601d"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ehlers_roofing_filter_zero_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ehlers_roofing_filter_zero_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ehlers_roofing_filter_zero_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ehlers_roofing_filter_zero_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ehlers_roofing_filter_zero_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "run_length", "high_pass", "roofing_filter",
    "entry_side", "raw_high_pass_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "run_length", "high_pass", "roofing_filter",
    "entry_side", "raw_high_pass_side", "variation", "variation_rank", "eligible",
)
...
def roofing_filter(series: pd.Series, valid: pd.Series) -> pd.DataFrame:
    hp_period=float(P["high_pass_period"]);ss_period=float(P["super_smoother_period"])
    angle=.707*2*math.pi/hp_period;alpha=(math.cos(angle)+math.sin(angle)-1)/math.cos(angle)
    a1=math.exp(-1.414*math.pi/ss_period);c2=2*a1*math.cos(1.414*math.pi/ss_period);c3=-a1*a1;c1=1-c2-c3
    values=series.to_numpy(float);ok=valid.to_numpy(bool);hp=np.full(len(series),np.nan);filt=np.full(len(series),np.nan);runs=np.zeros(len(series),dtype=int);run=0
    for i,(price,is_valid) in enumerate(zip(values,ok)):
        if not is_valid or not math.isfinite(price):run=0;continue
        run+=1;runs[i]=run
        if run<=2:hp[i]=0.;filt[i]=0.;continue
        hp[i]=(1-alpha/2)**2*(price-2*values[i-1]+values[i-2])+2*(1-alpha)*hp[i-1]-(1-alpha)**2*hp[i-2]
        filt[i]=c1*(hp[i]+hp[i-1])/2+c2*filt[i-1]+c3*filt[i-2]
    return pd.DataFrame({"run_length":runs,"high_pass":hp,"roofing_filter":filt},index=series.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    fields=["bar_open","bar_high","bar_low","bar_close"];bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[fields]).all(axis=1)&bars[fields].gt(0).all(axis=1);values=roofing_filter(bars.bar_close,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["entry_side"]=cross_side(bars.roofing_filter);bars["raw_high_pass_side"]=cross_side(bars.high_pass);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["high_pass","roofing_filter","variation"];bars["source_valid"]=bars.valid_bar&bars.run_length.ge(P["warmup_observations"])&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="raw_high_pass_cross":side=pd.to_numeric(used.raw_high_pass_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
