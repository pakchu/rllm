"""Evaluate the frozen HVCELVCVDR-8 action-vote clock against Gross9."""
...
POLICY = "HVCELVCVDR-8"
PROTOCOL = "hvcelvcvdr_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_caspc_ehlers_cross_venue_resolution_router_clocks_2023_2026.csv.gz",
    "sha256": "abf5091cc6f451d44d61684d73f8c1dce13f925f1d28efbd09d153f7f929b265",
    "rows": 130,
}
OUTPUT = Path("results/high_volatility_caspc_ehlers_cross_venue_resolution_router_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCELVCVDR-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCELVCVDR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCELVCVDR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCELVCVDR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCELVCVDR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCELVCVDR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCELVCVDR-8 unchanged before economics",
    }
