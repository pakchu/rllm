"""Strict staged economics for the singleton HVELDCV-24 policy."""
...
PREREG = Path("results/high_volatility_ehlers_dcs_active_veto_preregistration_2026-08-16.json")
PREREG_SHA = "eb451bc162d978d135850a1bfb332c34d637292451acf053652142b1150fe765"
SUPPORT = Path("results/high_volatility_ehlers_dcs_active_veto_support_2026-08-16.json")
SUPPORT_SHA = "051da05e6221ae17ee440c17c76d07da924e2e6dfed59c46d5cffdebb8fdb69b"
GROSS9 = Path("results/high_volatility_ehlers_dcs_active_veto_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "27507215286f7aac6b25d0634a6f820723f272351f2113405f3ed0878d98280e"
CLOCK = {
    "path": Path("data/high_volatility_ehlers_dcs_active_veto_clocks_2023_2026.csv.gz"),
    "sha256": "762fb551b28f92796ce53ae396e1616316479ed7bdee32eb651750fa70c5215b",
    "rows": 170,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ehlers_dcs_active_veto_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVELDCV-24 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVELDCV-24 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVELDCV-24 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVELDCV-24 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVELDCV-24 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVELDCV-24 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVELDCV-24 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVELDCV-24 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVELDCV-24 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVELDCV-24 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVELDCV-24 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hveldvc_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
