"""Evaluate frozen HVSRAR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVSRAR-8"
PROTOCOL = "hvsrar_8_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_sign_run_asymmetry_relay_preregistration_2026-08-09.json"
)
...
OUTPUT = Path(
    "results/high_volatility_sign_run_asymmetry_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVSRAR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVSRAR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVSRAR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVSRAR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvsrar_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVSRAR-8 unchanged before economic outcomes"
        ),
    }
