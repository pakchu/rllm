"""Open source-only OOS incidence for the frozen HVFPSC-8 model."""
...
from training import train_high_volatility_funding_premium_state_classifier as trained
from training.build_binance_aggtrade_microstructure import _write_gzip_csv


PREREG_SHA = "165cc96dfeebff14a65f103d4f5c8d2a84f802f92eeee8f5002bc315984a20e9"
SOURCE_MANIFEST_SHA = "3237d3e237c8a2b6aeff1e8f29af9be4b4004c1f3b8c836b1f0cfdbbcf85fe0e"
PANEL_SHA = "d44bf5b158ada7e3422b2b510d846e9855eb60230461c290efa866be6baf33ea"
MODEL_SHA = "0fd363f3f34f6e94d3ce49211596f6ccfe9e2a6b12a77292a9a68fc3c27f99c0"
CLOCK = Path("data/high_volatility_funding_premium_state_classifier_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_funding_premium_state_classifier_controls_2023_2026")
RESULT = Path("results/high_volatility_funding_premium_state_classifier_support_2026-08-10.json")
SPLITS = {
...
def verify_frozen_inputs() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if any(sha(path) != digest for path, digest in expected):
        raise RuntimeError("HVFPSC frozen predecessor hash drift")
    ...
    if source.get("oos_incidence_opened") is not False or source.get("oos_outcomes_opened") is not False:
        raise RuntimeError("HVFPSC source was not OOS sealed")
    if model.get("oos_incidence_opened") is not False or model.get("oos_outcomes_opened") is not False or model.get("refit_authorized") is not False:
        raise RuntimeError("HVFPSC model was not frozen OOS blind")
    if tuple(model.get("feature_order", ())) != trained.FEATURES:
        raise RuntimeError("HVFPSC feature order drift")
...
def clock(frame: pd.DataFrame, model: dict[str, Any], control: str = "primary") -> pd.DataFrame:
    ...
    for index in frame.index[active & frame.decision_time.ge(SPLITS["train"][0])]:
        ...
        if split is None:
            continue
        row = {
            "candidate": "HVFPSC-8", "control": control, "split": split, "decision_time": decision,
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvfpsc_8_oos_source_support_v1", "policy_id": "HVFPSC-8",
