"""Evaluate frozen HVCABBD-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCABBD-8"
PROTOCOL = "hvcabbd_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_cross_alt_broad_barrier_discovery_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCABBD predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCABBD novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCABBD predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCABBD primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCABBD-8 unchanged before economic outcomes",
    }
