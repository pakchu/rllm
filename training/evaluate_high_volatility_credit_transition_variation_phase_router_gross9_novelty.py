"""Evaluate the frozen HVCQVAR-24 action-vote clock against Gross9."""
...
POLICY = "HVCQVAR-24"
PROTOCOL = "hvcqvar_24_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_credit_transition_variation_phase_router_clocks_2023_2026.csv.gz",
    "sha256": "7a2d8c67934c2c1b8f46f1bb6c0782b4c1bb80d3bb139a5966be50b9c178c75c",
    "rows": 199,
}
OUTPUT = Path("results/high_volatility_credit_transition_variation_phase_router_gross9_novelty_2026-08-18.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCQVAR-24 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCQVAR-24 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCQVAR-24 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCQVAR-24 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCQVAR-24 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCQVAR-24 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCQVAR-24 unchanged before economics",
    }
