"""Evaluate frozen HVRTCR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVRTCR-8"
PROTOCOL = "hvrtcr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_range_turnover_coupling_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVRTCR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVRTCR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVRTCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVRTCR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvrtcr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVRTCR-8 unchanged before economic outcomes",
    }
