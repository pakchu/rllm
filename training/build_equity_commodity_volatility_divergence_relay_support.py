"""Materialize outcome-blind source support for frozen ECVDR-12."""
...
BUILDER = Path("training/build_equity_commodity_volatility_divergence_relay_support.py")
PREREG_SHA = "ad718d3fcd5d2885813a98dd2131a4ab7feca2c95ab89658a26100db50319637"
SOURCE_DIR = Path("data/equity_commodity_volatility_divergence_relay_sources_2021_2026")
FEATURE_PANEL = SOURCE_DIR / "equity_commodity_volatility_divergence_relay_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/equity_commodity_volatility_divergence_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/equity_commodity_volatility_divergence_relay_controls_2023_2026")
RESULT = Path("results/equity_commodity_volatility_divergence_relay_support_2026-08-09.json")
...
CONTROLS = (
    ...
    "equity_pair_only",
    "vix_gvz_only",
    "one_session_stale_divergence",
    "direction_flip",
)
ECONOMIC_OUTCOMES_AUTHORIZED = False
CHANGE_COLUMNS = ("vix_change", "vxn_change", "gvz_change", "ovx_change")
CLOCK_COLUMNS = (
    "candidate", "control", "split", "divergence_source_date", "next_common_source_date",
...
def _validate_daily(frame: pd.DataFrame, value_column: str, label: str) -> pd.DataFrame:
    ...
    if result.empty or result.observation_date.duplicated().any():
        raise RuntimeError(f"ECVDR {label} dates invalid")
    values = result[value_column]
    if not np.isfinite(values).all() or not values.gt(0).all():
        raise RuntimeError(f"ECVDR {label} values invalid")
    return result
...
def combine_cboe_sources(
    ...
    if len(common) < 3:
        raise RuntimeError("ECVDR common Cboe intersection is insufficient")
...
def load_cboe() -> pd.DataFrame:
    ...
    for label, specification in sources.items():
        ...
        if sha(path) != specification["sha256"]:
            raise RuntimeError(f"ECVDR {label} source hash drift")
    vix = pd.read_csv(
...
def load_bars(start: pd.Timestamp, end: pd.Timestamp) -> pd.DataFrame:
    ...
    if frame.columns.tolist() != ["ts", "open", "close"]:
        raise RuntimeError("ECVDR BTC schema must be exactly ts,open,close")
    ...
    if len(frame) != len(expected) or not frame.ts.equals(pd.Series(expected, name="ts")):
        raise RuntimeError("ECVDR BTC source is not the exact requested 1m grid")
    for column in ("open", "close"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    prices = frame[["open", "close"]]
    if not np.isfinite(prices).all(axis=None) or not prices.gt(0).all(axis=None):
        raise RuntimeError("ECVDR BTC source contains invalid prices")
...
def build_features(
    ...
    for current in range(1, len(cboe) - 1):
        ...
        records.append(
            {
                "divergence_source_date": pd.Timestamp(cboe.at[current, "observation_date"]),
...
def _divergence_side(frame: pd.DataFrame) -> pd.Series:
    equity_sign = np.sign(frame.vix_change)
    equity_pair = equity_sign.eq(np.sign(frame.vxn_change)) & equity_sign.ne(0)
    commodity_sign = np.sign(frame.gvz_change)
    commodity_pair = commodity_sign.eq(np.sign(frame.ovx_change)) & commodity_sign.ne(0)
    active = equity_pair & commodity_pair & equity_sign.eq(-commodity_sign)
    return pd.Series(np.where(active, -equity_sign, 0), index=frame.index).fillna(0).astype(int)
...
def conditions(
    ...
    used = frame.shift(1) if control == "one_session_stale_divergence" else frame
    if control == "equity_pair_only":
    ...
    elif control == "vix_gvz_only":
        vix_sign = np.sign(used.vix_change)
        gvz_sign = np.sign(used.gvz_change)
        side = pd.Series(np.where((vix_sign == -gvz_sign) & (vix_sign != 0) & (gvz_sign != 0), -vix_sign, 0), index=frame.index).fillna(0).astype(int)
    else:
        side = _divergence_side(used)
    volatility_gate = (
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active]:
        ...
        rows.append(
            {
                "candidate": "ECVDR-12", "control": control, "split": split,
                "divergence_source_date": pd.Timestamp(source.divergence_source_date).date().isoformat(),
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("ECVDR preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "ecvdr_12_sources_v1",
    ...
    core = {
        "protocol_version": "ecvdr_12_source_support_v1", "policy_id": "ECVDR-12",
