"""Sequential strict economics for frozen HVBJR-12."""
...
PREREG = Path("results/high_volatility_bipower_jump_reversal_preregistration_2026-08-09.json")
PREREG_SHA = "6c69fff90fa23d0589644a96dff1f698cb7646a1156e9723985ff32bdf4aee9a"
SUPPORT = Path("results/high_volatility_bipower_jump_reversal_support_2026-08-09.json")
SUPPORT_SHA = "275ecc4d4fe6157f351c615519289975e25ef2e1030190f04dd45ae40c390ef7"
NOVELTY = Path("results/high_volatility_bipower_jump_reversal_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "11bf1561a5eac7d6913d785f6545045bf38e11570bfd35034f551ad4eaa35256"
CLOCK = Path("data/high_volatility_bipower_jump_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "9952cd0941704be7fad51d81f2eff3ca8ba6168a63193fa11bc6cf1e8f864dcb"
CONTROL_DIR = Path("data/high_volatility_bipower_jump_reversal_controls_2023_2026")
FREEZE = Path("results/high_volatility_bipower_jump_reversal_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_bipower_jump_reversal_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVBJR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVBJR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvbjr_12_sequential_economics_v1", "policy_id": POLICY_ID,
