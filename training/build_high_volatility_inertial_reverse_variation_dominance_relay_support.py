"""Source-only support gate for frozen HVIRVD-8."""
...
from training import preregister_high_volatility_inertial_reverse_variation_dominance_relay as prereg
...
PREREG_SHA = "2cebe55116926081fb3858295cf4fc7f03cda9f1044ebb3d6f39d63033dd6a31"
SPLITS = {
...
ROOT = Path("data/high_volatility_inertial_reverse_variation_dominance_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_inertial_reverse_variation_dominance_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_inertial_reverse_variation_dominance_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_inertial_reverse_variation_dominance_relay_support_2026-08-10.json")
...
def transition_statistics(close: np.ndarray) -> tuple[float, float, float, float, float]:
    close = np.asarray(close, dtype=float)
    if len(close) != 96 or not np.isfinite(close).all() or np.any(close <= 0):
        return (math.nan,) * 5
    returns = np.diff(np.log(close))
    products = returns[1:] * returns[:-1]
    current_squared = np.square(returns[1:])
    inertial = float(current_squared[products > 0].sum())
    reverse = float(current_squared[products < 0].sum())
    dominance = inertial - reverse
    denominator = inertial + reverse
    share = abs(dominance) / denominator if denominator > 0 else math.nan
    completed_return = float(np.log(close[-1] / close[0]))
    return inertial, reverse, float(dominance), float(share), completed_return
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVIRVD source timestamps")
    frame = frame.set_index("ts").sort_index()
    rows: list[dict[str, Any]] = []
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        index = pd.date_range(decision - pd.Timedelta("8h"), decision, freq="5min", inclusive="left")
        ...
        if valid:
            ...
            inertial_variation, reverse_variation, dominance, dominance_share, completed_return = transition_statistics(
            ...
            valid = bool(
                ...
                and math.isfinite(inertial_variation)
                and math.isfinite(reverse_variation)
                and math.isfinite(dominance_share)
                and math.isfinite(completed_return)
            )
        if not valid:
            variation = inertial_variation = reverse_variation = dominance_share = completed_return = math.nan
            dominance = 0.0
        rows.append(
            {
                ...
                "inertial_variation": inertial_variation,
                "reverse_variation": reverse_variation,
                "dominance": dominance,
    ...
    states["dominance_share_rank"] = prior_rank(
        states["dominance_share"].where(states["source_valid"])
    )
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvirvd_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    dominance_rank = states["dominance_share_rank"]
    dominance = states["dominance"]
    if control == "one_block_stale_features":
        variation_rank, dominance_rank, dominance = [
            value.shift(1) for value in (variation_rank, dominance_rank, dominance)
        ]
    variation_gate = (
    ...
    if control == "no_dominance_share_gate":
        dominance_gate = pd.Series(True, index=states.index)
    else:
        dominance_gate = dominance_rank.ge(0.75)
    eligible = states["source_valid"] & variation_gate & dominance_gate & dominance.ne(0)
    onset = eligible & ~eligible.shift(1, fill_value=False) & states["source_valid"].shift(1, fill_value=False)
    side = np.sign(states["completed_return"]) * np.sign(dominance)
    if control == "dominance_interpretation_flip":
        side = -side
    return onset, side
        ...
        rows.append(
            {
                ...
                "inertial_variation": float(states.at[index, "inertial_variation"]),
                "reverse_variation": float(states.at[index, "reverse_variation"]),
                "dominance": float(states.at[index, "dominance"]),
                "dominance_share": float(states.at[index, "dominance_share"]),
                "completed_return": float(states.at[index, "completed_return"]),
                "dominance_share_rank": float(
                    states.at[index, "dominance_share_rank"]
                ),
            }
        )
    columns = [
        ...
        "inertial_variation", "reverse_variation", "dominance", "dominance_share",
        "completed_return", "dominance_share_rank",
    ]
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvirvd_8_source_support_v1",
        "policy_id": "HVIRVD-8",
        "preregistration": {
