"""Sequential strict economics for frozen HVEBS-24."""
...
PREREG = Path("results/high_volatility_ehlers_even_better_sinewave_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "c48b094eeb1588f861f24e3b374d8844985512f4314f869e648414f0bd5c9a46"
SUPPORT = Path("results/high_volatility_ehlers_even_better_sinewave_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "4a487127fd4f33c80d81dbecef1b6efe6a3f8c03e6e2a4406481e391a330d0aa"
NOVELTY = Path("results/high_volatility_ehlers_even_better_sinewave_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "75bc25fa2c6a4319723990b72cbf9f5d4c1c9a6c506826bcbe57264bebc766e9"
CLOCK = Path("data/high_volatility_ehlers_even_better_sinewave_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "ffc7d8e9ca3167530d9cae818781127c30d42efbe32c7cc3dd839d4b86539691"
CONTROL_DIR = Path("data/high_volatility_ehlers_even_better_sinewave_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_ehlers_even_better_sinewave_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ehlers_even_better_sinewave_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "unnormalized_filtered_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVEBS frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVEBS novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvebs_24_sequential_economics_v1", "policy_id": POLICY_ID,
