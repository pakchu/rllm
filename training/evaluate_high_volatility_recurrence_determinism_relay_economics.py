"""Sequential strict economics for frozen HVRQA-8."""
...
PREREG = Path("results/high_volatility_recurrence_determinism_relay_preregistration_2026-08-10.json")
PREREG_SHA = "fb1ebcf09a0b9fb12861e6ed03ae8706b131773cd0a69905ca647200e7787339"
SUPPORT = Path("results/high_volatility_recurrence_determinism_relay_support_2026-08-10.json")
SUPPORT_SHA = "43b7488f922c67a3256033408b5de2f51ed82c824e2848a23baa4799efd7a866"
NOVELTY = Path("results/high_volatility_recurrence_determinism_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "209c5a694342f4cbd63ed3257c2559eedbba1bbb60565eee98368e63bcd4aada"
CLOCK = Path("data/high_volatility_recurrence_determinism_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "c81323fe967557989d13e309e9919029bdc5cf9bdea749d5a8551ad4e43260dc"
CONTROL_DIR = Path("data/high_volatility_recurrence_determinism_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_recurrence_determinism_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_recurrence_determinism_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_determinism_tail", "no_variation_gate", "raw_determinism_at_least_half", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVRQA frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVRQA novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrqa_8_sequential_economics_v1", "policy_id": POLICY_ID,
