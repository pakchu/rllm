"""Build outcome-blind source support for frozen HVCASK-8."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_cross_alt_realized_skew_spillover_consensus_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2023-01-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="d2b32b2b020b27cee4e8106a7c3b790ddeb54cc0d099e42e8e6d286b81fead7d";REG=prereg.build();P=REG["policy"];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG["stages"].items()};GATES=REG["source_support_gates"];CONTROLS=tuple(REG["diagnostic_controls"]["names"]);SYMBOLS=("BTCUSDT",*prereg.ALTS);ALTS=prereg.ALTS
...
ROOT=Path("data/high_volatility_cross_alt_realized_skew_spillover_consensus_relay_sources_2023_2026");PANEL=ROOT/"scheduled_realized_skew_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_cross_alt_realized_skew_spillover_consensus_relay_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_cross_alt_realized_skew_spillover_consensus_relay_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_cross_alt_realized_skew_spillover_consensus_relay_controls_2023_2026");RESULT=Path("results/high_volatility_cross_alt_realized_skew_spillover_consensus_relay_support_2026-08-13.json");BUILDER=Path(__file__).relative_to(Path.cwd())
BASE_COLS=("decision_time","feature_available_time","source_valid","skew_score","selected_alts","agreeing_alts","side","btc_realized_variation","variation_rank","variation_active","eligible")
CLOCK_COLS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","skew_score","selected_alts","agreeing_alts","btc_realized_variation","variation_rank")
...
def prepare(raw):
 ...
 if raw.columns.tolist()!=expected:raise RuntimeError("HVCASK source schema drift")
...
def skew_frame(ret,valid):
 scheduled=pd.date_range(START,END,freq="12h",inclusive="left")+pd.Timedelta("2h30m");scheduled=scheduled[(scheduled>=START)&(scheduled<END)];rows=[];window_valid=valid.rolling(P["variation_bars"],min_periods=P["variation_bars"]).sum().eq(P["variation_bars"]).all(axis=1);rv=np.sqrt(ret.BTCUSDT.pow(2).where(valid.BTCUSDT).rolling(P["variation_bars"],min_periods=P["variation_bars"]).sum());sum2={a:ret[a].pow(2).where(valid[a]).rolling(P["skew_bars"],min_periods=P["skew_bars"]).sum() for a in ALTS};sum3={a:ret[a].pow(3).where(valid[a]).rolling(P["skew_bars"],min_periods=P["skew_bars"]).sum() for a in ALTS};skew={a:sum3[a]/sum2[a].pow(1.5).where(sum2[a].gt(0)) for a in ALTS}
 for decision in scheduled:
  ...
  for a in ALTS:row[f"{a}_realized_skew"]=float(skew[a].at[end]) if math.isfinite(skew[a].at[end]) else np.nan;row[f"{a}_third_moment"]=float(sum3[a].at[end]) if math.isfinite(sum3[a].at[end]) else np.nan
  rows.append(row)
 return pd.DataFrame(rows)
def score_states(f,control="primary"):
 u=f.copy();ranks={}
 for a in ALTS:ranks[a]=midrank(u[f"{a}_realized_skew"].abs().where(u.source_valid),P["skew_prior_decisions"],P["skew_minimum_decisions"]);u[f"{a}_skew_rank"]=ranks[a]
 ...
 for i,row in u.iterrows():
  chosen=[a for a in ALTS if math.isfinite(row[f"{a}_realized_skew"]) and (control=="no_skew_tail" or row[f"{a}_skew_rank"]>=P["skew_rank_min"])]
  value={a:(row[f"{a}_third_moment"] if control=="unnormalized_third_moment" else row[f"{a}_realized_skew"]) for a in chosen};signs={a:int(np.sign(value[a])) for a in chosen};weights={a:abs(value[a]) for a in chosen};den=sum(weights.values());score=sum(weights[a]*signs[a] for a in chosen)/den if den>0 else np.nan;sgn=int(np.sign(score)) if math.isfinite(score) else 0;scores.append(score);selected.append(len(chosen));agree.append(sum(v==sgn for v in signs.values()) if sgn else 0)
 u["skew_score"]=scores;u["selected_alts"]=selected;u["agreeing_alts"]=agree;minimum=3 if control=="three_alt_consensus" else P["minimum_selected_alts"];u["side"]=np.sign(u.skew_score).fillna(0).astype(int);u["variation_active"]=u.variation_rank.ge(P["variation_rank_min"]);u["eligible"]=u.source_valid&u.selected_alts.ge(minimum)&u.skew_score.abs().ge(P["score_absolute_min"])&u.agreeing_alts.ge(P["minimum_agreeing_alts"])&u.variation_active
 if control=="no_variation_gate":u["eligible"]=u.source_valid&u.selected_alts.ge(minimum)&u.skew_score.abs().ge(P["score_absolute_min"])&u.agreeing_alts.ge(P["minimum_agreeing_alts"])
 if control=="one_decision_stale_skew":
  for c in ("skew_score","selected_alts","agreeing_alts","side"):u[c]=u[c].shift(1)
  u["eligible"]=u.source_valid&u.selected_alts.ge(minimum)&u.skew_score.abs().ge(P["score_absolute_min"])&u.agreeing_alts.ge(P["minimum_agreeing_alts"])&u.variation_active
 if control=="direction_flip":u.side=-u.side
 if control=="forced_long":u.side=1
 return u
def build_panel(raw,control="primary"):
 x=prepare(raw);grid=pd.date_range(START,END,freq="5min",inclusive="left");ret=x.pivot(index="date",columns="symbol",values="return").reindex(grid,columns=SYMBOLS);valid=x.pivot(index="date",columns="symbol",values="valid").reindex(grid,columns=SYMBOLS).eq(True);return score_states(skew_frame(ret,valid),control)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCASK prereg drift")
