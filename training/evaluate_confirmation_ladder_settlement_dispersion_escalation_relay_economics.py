"""Sequential strict economics for frozen CLSDER-6."""
...
PREREG = Path("results/confirmation_ladder_settlement_dispersion_escalation_relay_preregistration_2026-08-12.json")
PREREG_SHA = "43e5081b606437f5c9d90ca22684752d45bcf1c169d4250ed2aab51a3089feb1"
SUPPORT = Path("results/confirmation_ladder_settlement_dispersion_escalation_relay_support_2026-08-12.json")
SUPPORT_SHA = "69c61be961bd8c27b30f3c3d4d410904a0b1b57657c8a0af3517383d092c32bf"
NOVELTY = Path("results/confirmation_ladder_settlement_dispersion_escalation_relay_gross9_novelty_2026-08-12.json")
NOVELTY_SHA = "4fce76614b308a839b15776b969c75a34b741daff60ab258e6c09b346a6fe326"
CLOCK = Path("data/confirmation_ladder_settlement_dispersion_escalation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "dd4cf1add6a8e53089207c89fb9272661aa328d0ae5b11ea7ab8435394bf65f0"
CONTROL_DIR = Path("data/confirmation_ladder_settlement_dispersion_escalation_relay_controls_2023_2026")
FREEZE = Path("results/confirmation_ladder_settlement_dispersion_escalation_relay_economic_evaluator_freeze_2026-08-12.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/confirmation_ladder_settlement_dispersion_escalation_relay_{stage}_economics_2026-08-12.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CLSDER frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CLSDER novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "clsder_6_sequential_economics_v1", "policy_id": POLICY_ID,
