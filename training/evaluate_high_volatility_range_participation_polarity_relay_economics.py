"""Sequential strict economics for frozen HVRPPR-8."""
...
PREREG = Path("results/high_volatility_range_participation_polarity_relay_preregistration_2026-08-10.json")
PREREG_SHA = "5a6191b6a660849b4a33ef23094db55fe690b843262a7ee638709fa49d47459f"
SUPPORT = Path("results/high_volatility_range_participation_polarity_relay_support_2026-08-10.json")
SUPPORT_SHA = "b506b3201380e7b3db5e62406371d566910e47d639b7b0168265457eba379ed4"
NOVELTY = Path("results/high_volatility_range_participation_polarity_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "490c9190479b47896172edceb1892135604e0cf6ff82e870106285b409320432"
CLOCK = Path("data/high_volatility_range_participation_polarity_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "7fa67b1ddaeb7b2e02840f9f96011333763f78ddffd7ce47a2bccf4f05c9e21d"
CONTROL_DIR = Path("data/high_volatility_range_participation_polarity_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_range_participation_polarity_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_range_participation_polarity_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_polarity_tail_gate", "no_variation_gate", "unweighted_range_polarity", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVRPPR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVRPPR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrppr_8_sequential_economics_v1", "policy_id": POLICY_ID,
