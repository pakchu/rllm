"""Evaluate frozen HVGATOR-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVGATOR-24"
PROTOCOL = "hvgator_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_gator_awakening_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVGATOR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVGATOR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVGATOR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVGATOR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvgator_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVGATOR-24 unchanged before economic outcomes",
    }
