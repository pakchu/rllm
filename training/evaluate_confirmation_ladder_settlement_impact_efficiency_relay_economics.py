"""Sequential strict economics for frozen CLSIER-6."""
...
PREREG = Path("results/confirmation_ladder_settlement_impact_efficiency_relay_preregistration_2026-08-12.json")
PREREG_SHA = "0e5baf2b0199a26fb680e1c5d21e4fa3d77abd40a6cf1d70e006115d2d6fc1ee"
SUPPORT = Path("results/confirmation_ladder_settlement_impact_efficiency_relay_support_2026-08-12.json")
SUPPORT_SHA = "4306375a7e5928731f853b1c827487833d04d3ec089134101fc9fc970f533d46"
NOVELTY = Path("results/confirmation_ladder_settlement_impact_efficiency_relay_gross9_novelty_2026-08-12.json")
NOVELTY_SHA = "e326e976729f1c4bc9fdbc3ec35a73105c74daaaa7615c408fae2ec1b347c9e6"
CLOCK = Path("data/confirmation_ladder_settlement_impact_efficiency_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "a25d968b96f3245aa1cd0f4887d00710146da3b227c1ab19d1161a70b53385f0"
CONTROL_DIR = Path("data/confirmation_ladder_settlement_impact_efficiency_relay_controls_2023_2026")
FREEZE = Path("results/confirmation_ladder_settlement_impact_efficiency_relay_economic_evaluator_freeze_2026-08-12.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/confirmation_ladder_settlement_impact_efficiency_relay_{stage}_economics_2026-08-12.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CLSIER frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CLSIER novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "clsier_6_sequential_economics_v1", "policy_id": POLICY_ID,
