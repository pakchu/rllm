"""Evaluate frozen TSPI-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "TSPI-12"
PROTOCOL = "tspi_12_gross9_novelty_v1"
...
OUTPUT = Path("results/temporal_sign_persistence_imbalance_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("TSPI predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("TSPI novelty limits drift")
    if (
        ...
        raise RuntimeError("TSPI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="TSPI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "tspi_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject TSPI-12 unchanged before economic outcomes",
    }
