"""Evaluate frozen HVCVCOIR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCVCOIR-12"
PROTOCOL = "hvcvcoir_12_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_variance_concentration_open_interest_release_preregistration_2026-08-16.json"
)
...
CLOCK_SHA = "0951df1d4f5af2be52122b1cba429bc1659c93e8a91c1d278b757b64dc9ec1c8"
OUTPUT = Path(
    "results/high_volatility_variance_concentration_open_interest_release_gross9_novelty_2026-08-16.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCVCOIR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVCVCOIR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCVCOIR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCVCOIR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcvcoir_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVCVCOIR-12 unchanged before economic outcomes"
        ),
    }
