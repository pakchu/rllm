"""Evaluate frozen HVIARR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVIARR-8"
PROTOCOL = "hviarr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_inside_auction_resolution_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVIARR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVIARR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVIARR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVIARR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hviarr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVIARR-8 unchanged before economic outcomes",
    }
