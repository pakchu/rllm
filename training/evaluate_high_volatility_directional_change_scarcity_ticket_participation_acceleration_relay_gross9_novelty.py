"""Evaluate frozen HVDCSATPA-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDCSATPA-8"
PROTOCOL = "hvdcsatpa_8_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_directional_change_scarcity_ticket_participation_acceleration_relay_gross9_novelty_2026-08-16.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDCSATPA predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVDCSATPA novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDCSATPA predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDCSATPA primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdcs_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVDCSATPA-8 unchanged before economic outcomes"
        ),
    }
