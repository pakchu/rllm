"""Build outcome-blind source support for frozen HVCMO-24."""
...
from training import preregister_high_volatility_chande_momentum_oscillator_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "010b09bd881c753cd9c7e28a6763e6558df8eb4bd24d959768556869eba528cc"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_chande_momentum_oscillator_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_chande_momentum_oscillator_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_chande_momentum_oscillator_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_chande_momentum_oscillator_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_chande_momentum_oscillator_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "change", "gain_sum", "loss_sum", "cmo", "entry_side", "raw_close_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "change", "gain_sum", "loss_sum", "cmo", "entry_side",
    "raw_close_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVCMO source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVCMO invalid source key")
...
def cmo_values(series: pd.Series, valid: pd.Series, period: int) -> pd.DataFrame:
    change=(series-series.shift(1)).where(valid&valid.shift(1,fill_value=False));gain=change.clip(lower=0);loss=(-change.clip(upper=0));complete=np.isfinite(change).rolling(period,min_periods=period).sum().eq(period);gain_sum=gain.rolling(period,min_periods=period).sum().where(complete);loss_sum=loss.rolling(period,min_periods=period).sum().where(complete);denominator=gain_sum+loss_sum;cmo=(100.0*(gain_sum-loss_sum)/denominator).where(denominator.gt(0));return pd.DataFrame({"change":change,"gain_sum":gain_sum,"loss_sum":loss_sum,"cmo":cmo},index=series.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    values=cmo_values(bars.bar_close,bars.valid_bar,P["momentum_periods"])
    for column in values:bars[column]=values[column]
    bars["entry_side"]=cross_side(bars.cmo);bars["raw_close_side"]=np.sign(bars.change).fillna(0).astype(int)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["change","gain_sum","loss_sum","cmo","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&(bars.gain_sum+bars.loss_sum).gt(0)&bars.variation.gt(0)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCMO prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvcmo_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvcmo_24_source_support_v1", "policy_id": prereg.POLICY_ID,
