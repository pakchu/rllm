"""Evaluate frozen HVPPO-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVPPO-24"
PROTOCOL = "hvppo_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_percentage_price_oscillator_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVPPO predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVPPO novelty limits drift")
    if (
        ...
        raise RuntimeError("HVPPO predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVPPO primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvppo_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVPPO-24 unchanged before economic outcomes",
    }
