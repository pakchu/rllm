"""Build causal HVSCER-24 state-conditioned winner rotations before novelty/economics."""
from __future__ import annotations
import argparse,hashlib,json
from pathlib import Path
from typing import Any
import numpy as np,pandas as pd
from training import preregister_high_volatility_state_conditioned_expert_rotation_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.build_scheduled_trend_concordance_relay_support import load_market
PREREG_SHA="ea3b7848fcfe2caa4e8539abbdbdb03a4a4f3384e2ad8e5b146ae7aa27948aef";LOADER=Path("training/build_scheduled_trend_concordance_relay_support.py");LOADER_SHA="8ca554d88506df277434f73e5eb8850426614a880110088eb91aaae3b23c154f";END=pd.Timestamp("2026-08-01T00:00:00Z");SOURCE_DIR=Path("data/high_volatility_state_conditioned_expert_rotation_relay_sources_2020_2026");STATES=SOURCE_DIR/"causal_state_conditioned_expert_states.csv.gz";MANIFEST=SOURCE_DIR/"manifest.json";CLOCK=Path("data/high_volatility_state_conditioned_expert_rotation_relay_clocks_2023_2026.csv.gz");CONTROL_DIR=Path("data/high_volatility_state_conditioned_expert_rotation_relay_controls_2023_2026");RESULT=Path("results/high_volatility_state_conditioned_expert_rotation_relay_support_2026-08-13.json")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00:00Z"),pd.Timestamp("2024-01-01T00:00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00:00Z"),pd.Timestamp("2025-01-01T00:00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00:00Z"),pd.Timestamp("2026-01-01T00:00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00:00Z"),END)};MINIMUM={"train":8,"test":12,"eval":12,"final":8};EXPERTS=("momentum_4h","reversal_4h","momentum_24h","reversal_24h");CONTROLS=("no_variation_gate","no_positive_score","winner_level","one_decision_stale_winner","direction_flip","same_clock_forced_long");COLS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","return_4h","return_24h","winner","prior_winner","winner_score","variation","variation_rank")
...
def online_states(market,memory=60,minimum=30):
 ...
 decisions=pd.date_range(f.index.min().ceil("D")+pd.Timedelta(hours=2),END,freq="24h",inclusive="left");hist=[[] for _ in EXPERTS];variation_history=[];pending=[];rows=[];prior_ranking_winner=""
 for d in decisions:
  ...
  closes=block[:,1];r4=float(np.log(closes[-1]/closes[-49]));r24=float(np.log(closes[-1]/closes[0]));intrabar=np.log(block[-96:,1]/block[-96:,0]);var=float(np.sqrt(np.square(intrabar).sum()));q=np.asarray(variation_history[-270:],float);vr=((q<var).sum()+.5*(q==var).sum())/len(q) if np.isfinite(var) and len(q)>=180 else np.nan;current=sides(r4,r24)
  ...
  rows.append({"decision_time":d,"source_valid":bool(r4!=0 and r24!=0 and var>0),"return_4h":r4,"return_24h":r24,"variation":var,"variation_rank":vr,**{f"{n}_side":int(current[j]) for j,n in enumerate(EXPERTS)},**{f"{n}_score":float(scores[j]) for j,n in enumerate(EXPERTS)},"winner":winner,"prior_winner":prior_ranking_winner,"winner_side":int(current[winner_i]) if winner_i>=0 else 0,"winner_score":float(scores[winner_i]) if winner_i>=0 else np.nan,"ranking_valid":ranking_valid,"rotation":rotation,"positive_score":positive,"included_mature_labels_before_decision":min((len(h) for h in hist),default=0)})
  if ranking_valid:prior_ranking_winner=winner
  origin_high=bool(np.isfinite(vr) and vr>=.65);pending.append({"entry":d+pd.Timedelta(minutes=5),"exit":d+pd.Timedelta(hours=24,minutes=5),"sides":current.copy(),"origin_high":origin_high})
...
def clock(x,control="primary"):
 ...
 for i in x.index[active]:
  ...
  next_allowed=exit_;rows.append({"candidate":prereg.POLICY_ID,"control":control,"split":split,"decision_time":d,"feature_available_time":d,"entry_time":entry,"exit_time":exit_,"side":int(side.at[i]),"return_4h":float(x.at[i,"return_4h"]),"return_24h":float(x.at[i,"return_24h"]),"winner":x.at[i,"winner"],"prior_winner":x.at[i,"prior_winner"],"winner_score":float(x.at[i,"winner_score"]),"variation":float(x.at[i,"variation"]),"variation_rank":float(x.at[i,"variation_rank"])})
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA or sha(prereg.MARKET)!=prereg.MARKET_SHA or sha(LOADER)!=LOADER_SHA:raise RuntimeError("HVSCER binding drift")
