"""Evaluate frozen HVFAER-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVFAER-8"
PROTOCOL = "hvfaer_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_failed_auction_escape_reversal_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVFAER predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVFAER novelty limits drift")
    if (
        ...
        raise RuntimeError("HVFAER predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVFAER primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvfaer_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVFAER-8 unchanged before economic outcomes",
    }
