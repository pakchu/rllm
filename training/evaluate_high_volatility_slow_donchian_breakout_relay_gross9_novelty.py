"""Evaluate frozen HVSDBR-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVSDBR-24"
PROTOCOL = "hvsdbr_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_slow_donchian_breakout_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVSDBR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVSDBR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVSDBR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVSDBR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvsdbr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVSDBR-24 unchanged before economic outcomes",
    }
