"""Build outcome-blind source support for frozen HVDMIC-24."""
...
PREREG_SHA = "93318788831a973a8dbe04d23d8e48a141a7f0a4ae717481efb7e258decb960f"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_directional_movement_index_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_directional_movement_index_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_directional_movement_index_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_directional_movement_index_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_directional_movement_index_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "true_range", "plus_dm", "minus_dm", "plus_di",
    "minus_di", "difference", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "plus_di", "minus_di", "difference", "variation",
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVDMIC source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVDMIC invalid source key")
...
def wilder_adx(
    ...
    ).max(axis=1).where(transition_valid)
    plus_dm = up.where((up > down) & (up > 0), 0.0).where(transition_valid)
    minus_dm = down.where((down > up) & (down > 0), 0.0).where(transition_valid)
    n = len(high)
    smooth_tr = np.full(n, np.nan)
    smooth_plus = np.full(n, np.nan)
    smooth_minus = np.full(n, np.nan)
    tr = true_range.to_numpy(float)
    pdm = plus_dm.to_numpy(float)
    mdm = minus_dm.to_numpy(float)
    for index in range(period, n):
        if index == period:
            if np.isfinite(tr[1 : period + 1]).all():
                smooth_tr[index] = tr[1 : period + 1].sum()
                smooth_plus[index] = pdm[1 : period + 1].sum()
                smooth_minus[index] = mdm[1 : period + 1].sum()
        elif all(math.isfinite(value) for value in (smooth_tr[index - 1], tr[index], pdm[index], mdm[index])):
            smooth_tr[index] = smooth_tr[index - 1] - smooth_tr[index - 1] / period + tr[index]
            smooth_plus[index] = smooth_plus[index - 1] - smooth_plus[index - 1] / period + pdm[index]
            smooth_minus[index] = smooth_minus[index - 1] - smooth_minus[index - 1] / period + mdm[index]
    plus_di = np.where(smooth_tr > 0, 100 * smooth_plus / smooth_tr, np.nan)
    minus_di = np.where(smooth_tr > 0, 100 * smooth_minus / smooth_tr, np.nan)
    denominator = plus_di + minus_di
    dx = np.where(denominator > 0, 100 * np.abs(plus_di - minus_di) / denominator, np.nan)
    adx = np.full(n, np.nan)
    first = period * 2 - 1
    if n > first and np.isfinite(dx[period : first + 1]).all():
        adx[first] = dx[period : first + 1].mean()
        for index in range(first + 1, n):
            if math.isfinite(adx[index - 1]) and math.isfinite(dx[index]):
                adx[index] = ((period - 1) * adx[index - 1] + dx[index]) / period
    return pd.DataFrame(
        {"true_range": true_range, "plus_dm": plus_dm, "minus_dm": minus_dm,
         "plus_di": plus_di, "minus_di": minus_di, "dx": dx, "adx": adx},
        index=high.index,
    )
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars = pd.DataFrame(
        {"rows": groups.row_valid.sum(), "bar_open": groups.open.first(),
         "bar_high": groups.high.max(), "bar_low": groups.low.min(),
         "bar_close": groups.close.last(), "variation_component": groups.minute_sq_return.sum(min_count=240)}
    )
    bars["valid_bar"] = (
        bars.rows.eq(240) & np.isfinite(bars[["bar_open", "bar_high", "bar_low", "bar_close"]]).all(axis=1)
        & bars[["bar_open", "bar_high", "bar_low", "bar_close"]].gt(0).all(axis=1)
    )
    indicators = wilder_adx(bars.bar_high, bars.bar_low, bars.bar_close, bars.valid_bar)
    bars = pd.concat([bars, indicators], axis=1)
    bars["difference"] = bars.plus_di - bars.minus_di
    bars["variation"] = np.sqrt(
        bars.variation_component.rolling(P["variation_hours"] // 4, min_periods=P["variation_hours"] // 4).sum()
    )
    bars["source_valid"] = (
        bars.valid_bar & np.isfinite(bars[["plus_di", "minus_di", "difference", "variation"]]).all(axis=1)
        & bars.variation.gt(0)
    )
    ...
    panel["eligible"] = (panel.source_valid & panel.source_valid.shift(1, fill_value=False)
        & panel.difference.ne(0) & previous.ne(0) & np.sign(panel.difference).ne(np.sign(previous))
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);difference=pd.to_numeric(used.difference,errors="coerce");previous=difference.shift(1);cross=difference.ne(0)&previous.ne(0)&np.sign(difference).ne(np.sign(previous));side=np.sign(difference).fillna(0).astype(int);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&cross&side.ne(0)&variation
    if control=="no_variation_gate":state=valid&cross&side.ne(0)
    elif control=="persistent_direction_state":state=used.source_valid.eq(True)&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVDMIC prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvdmic_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvdmic_24_source_support_v1", "policy_id": prereg.POLICY_ID,
