"""Evaluate frozen HVSVF-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVSVF-8"
PROTOCOL = "hvsvf_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_signed_variance_feedback_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVSVF predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVSVF novelty limits drift")
    if (
        ...
        raise RuntimeError("HVSVF predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVSVF primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvsvf_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVSVF-8 unchanged before economic outcomes",
    }
