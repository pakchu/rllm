"""Evaluate frozen TRTSR-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "TRTSR-24"
PROTOCOL = "trtsr_24_gross9_novelty_v1"
...
OUTPUT = Path("results/treasury_real_term_spread_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("TRTSR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("TRTSR novelty limits drift")
    if (
        ...
        raise RuntimeError("TRTSR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="TRTSR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "trtsr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject TRTSR-24 unchanged before economic outcomes",
    }
