"""Evaluate frozen HVDTIAR-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDTIAR-6"
PROTOCOL = "hvdtiar_6_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_directional_tail_index_asymmetry_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDTIAR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVDTIAR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDTIAR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDTIAR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVDTIAR-6 unchanged before economic outcomes",
    }
