"""Deterministic outcome-blind source support for HVCEDR-24."""
...
BAR_QUERY = """SELECT date_bin('4 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '4 hours' AS decision_time,(array_agg(close ORDER BY ts DESC))[1] AS last_close,max(high) AS auction_high,min(low) AS auction_low,sum(power(ln(close/open),2)) AS minute_squared_return,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND high>=greatest(open,close,low) AND low<=least(open,close,high)) AS coherent FROM bars_binance WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end GROUP BY 1 ORDER BY 1"""

ROOT = Path("data/high_volatility_causal_extreme_displacement_reversal_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_extreme_displacement_reversal_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_extreme_displacement_reversal_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_causal_extreme_displacement_reversal_support_2026-08-16.json")
...
PANEL_COLUMNS = (
    ...
    "displacement_rank", "realized_variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
...
def prepare(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCEDR-24 bar schema drift")
    ...
    if bars["decision_time"].duplicated().any():
        raise RuntimeError("HVCEDR-24 duplicate four-hour bar")
    start = bars["decision_time"] - pd.Timedelta("4h")
    bars["bar_valid"] = (
    ...
    bars["prior_close_valid"] = (
        ...
        & bars["decision_time"].shift(periods).eq(
            bars["decision_time"] - pd.Timedelta(hours=4 * periods)
        )
    )
    ...
    bars["variation_valid"] = (
        ...
        & bars["decision_time"].shift(variation_periods - 1).eq(
            bars["decision_time"] - pd.Timedelta(hours=4 * (variation_periods - 1))
        )
    )
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    panel["eligible"] = (
        valid
        & panel["displacement_rank"].ge(POLICY["displacement_rank_min"])
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCEDR-24 side or availability drift")
...
def run() -> dict[str, Any]:
    ...
    source_core = {
        "protocol_version": "hvcedr_24_sources_v1",
    ...
    core = {
        "protocol_version": "hvcedr_24_source_support_v1", "policy_id": prereg.POLICY_ID,
