"""Evaluate frozen HVLIR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVLIR-8"
PROTOCOL = "hvlir_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_liquidity_impact_relay_preregistration_2026-08-09.json")
PREREG_SHA = "8d4c718b1ea443b3789d9ae30e7efa6675547b4aac51c2429038c40d3d288488"
SUPPORT = Path("results/high_volatility_liquidity_impact_relay_support_2026-08-09.json")
SUPPORT_SHA = "52eb924bf59500ee60ae3b4eab0e97741890a234e1d235b793904a04b63ca3b5"
CLOCK = Path("data/high_volatility_liquidity_impact_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "62776c2e30b1228623b3cbe9f8190063842ee1285d08830649280ba3f88e7599"
OUTPUT = Path("results/high_volatility_liquidity_impact_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVLIR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVLIR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVLIR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVLIR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvlir_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVLIR-8 unchanged before economic outcomes",
    }
