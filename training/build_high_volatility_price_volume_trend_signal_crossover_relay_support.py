"""Build outcome-blind source support for frozen HVPVT-24."""
...
from training import preregister_high_volatility_price_volume_trend_signal_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "deaf14da7fc41f42c0a2e2907dee747b9b649864614a9e89390a3ba6a2f46d6b"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_price_volume_trend_signal_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_price_volume_trend_signal_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_price_volume_trend_signal_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_price_volume_trend_signal_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_price_volume_trend_signal_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "bar_volume", "volume_return_contribution", "pvt", "signal", "difference", "contribution_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "bar_volume", "volume_return_contribution", "pvt", "signal", "difference", "contribution_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close", "volume"]:
        raise RuntimeError("HVPVT source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVPVT invalid source key")
...
def causal_pvt(close: pd.Series, volume: pd.Series, valid: pd.Series) -> pd.DataFrame:
    contribution=np.full(len(close),np.nan);pvt=np.full(len(close),np.nan);state=np.nan;prior=np.nan
    for i,(price,amount,ok) in enumerate(zip(close.to_numpy(float),volume.to_numpy(float),valid.to_numpy(bool))):
        if not ok or not math.isfinite(price) or not math.isfinite(amount):state=np.nan;prior=np.nan;continue
        if not math.isfinite(prior):increment=0.0;state=0.0
        else:increment=amount*(price/prior-1.0);state+=increment
        contribution[i]=increment;pvt[i]=state;prior=price
    return pd.DataFrame({"volume_return_contribution":contribution,"pvt":pvt},index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=pd.DataFrame({"rows":groups.row_valid.sum(),"bar_open":groups.open.first(),"bar_high":groups.high.max(),"bar_low":groups.low.min(),"bar_close":groups.close.last(),"bar_volume":groups.volume.sum(min_count=240),"variation_component":groups.minute_sq_return.sum(min_count=240)})
    fields=["bar_open","bar_high","bar_low","bar_close","bar_volume"];bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[fields]).all(axis=1)&bars[["bar_open","bar_high","bar_low","bar_close"]].gt(0).all(axis=1)&bars.bar_volume.gt(0)
    values=causal_pvt(bars.bar_close,bars.bar_volume,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["signal"]=causal_ema(bars.pvt,np.isfinite(bars.pvt),P["signal_periods"]);bars["difference"]=bars.pvt-bars.signal
    previous_contribution=bars.volume_return_contribution.shift(1);bars["contribution_side"]=np.where((bars.volume_return_contribution>0)&(previous_contribution<=0),1,np.where((bars.volume_return_contribution<0)&(previous_contribution>=0),-1,0))
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["bar_volume","volume_return_contribution","pvt","signal","difference","variation"]]).all(axis=1)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.difference.shift(1);panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&panel.difference.ne(0)&previous.ne(0)&np.sign(panel.difference).ne(np.sign(previous))&panel.variation_rank.ge(P["variation_rank_min"]);return panel.loc[:,PANEL_COLUMNS]
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="contribution_zero_cross":
        side=pd.to_numeric(used.contribution_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVPVT prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvpvt_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvpvt_24_source_support_v1", "policy_id": prereg.POLICY_ID,
