"""Sequential strict evaluator for frozen ORFR-1.

Evaluator freeze opens no outcome. Stage1 physically parses only 2021-2022.
The 2023 window is unreachable unless the unchanged Stage1 report passes and
...
SUPPORT_COMMIT = "80ef698"
STATIC_INPUT_SHA256 = {
    "training/preregister_overnight_rrp_flow_release.py": "fb35397bca6d3a96cdbcd04fa047d80f2f925f2ffd950e47cc1c025a15f35726",
    "training/overnight_rrp_flow_release_clock.py": "aba33cc420fb133e4e4a952b595b7b5915df808d435dee56754d90724cb364d8",
    "docs/overnight-rrp-flow-release-preregistration-2026-07-17.md": "12c30a6a3e2c7567388765fc5d260237ab2bfc557b9567c3ee4f9331825d72ef",
    "results/overnight_rrp_flow_release_preregistration_2026-07-17.json": "5ff8ffbe448a76dfdf6dde6aefbd83194f90429e55c75f1c6fc4e9749d3f09dd",
    "training/build_overnight_rrp_flow_release_support.py": "14b5fc1fa74e4bcf6e2f309de9e565f00ead1a191a6ef6b2e1d8ec936c83bab8",
    "docs/overnight-rrp-flow-release-support-2026-07-17.md": "bf483b3ebd5e3fb18f201f6e2a0ce3f2b64dc3a5c5306215a53b4eaed0665e3a",
    "results/overnight_rrp_flow_release_support_2026-07-17.json": "7ac6b888bc993015951f09ee1f0ef3c19b47faa8d92e99864a87b139ec57dd6a",
    "results/overnight_rrp_flow_release_clocks_2026-07-17.csv.gz": "7242d9870627dfc0cf067ff87d9664a1576dd374cb8985e927b40f15d1e3d480",
...
SUPPORT_RESULT = Path("results/overnight_rrp_flow_release_support_2026-07-17.json")
CLOCKS = Path("results/overnight_rrp_flow_release_clocks_2026-07-17.csv.gz")
...
EVALUATOR_SOURCE = Path("training/evaluate_overnight_rrp_flow_release.py")
EVALUATOR_FREEZE = Path(
    "results/overnight_rrp_flow_release_evaluator_freeze_2026-07-17.json"
)
STAGE1_OUTPUT = Path(
    "results/overnight_rrp_flow_release_stage1_2021_2022_2026-07-17.json"
)
STAGE1_DOC = Path(
    "docs/overnight-rrp-flow-release-stage1-2021-2022-2026-07-17.md"
)
STAGE2_OUTPUT = Path(
    "results/overnight_rrp_flow_release_stage2_2023_2026-07-17.json"
)
STAGE2_DOC = Path("docs/overnight-rrp-flow-release-stage2-2023-2026-07-17.md")
...
def _timestamp(value: Any) -> pd.Timestamp:
    ...
    if result is pd.NaT:
        raise ValueError("ORFR-1 timestamp is NaT")
...
MECHANISM_CONTROLS = ("one_day_delta_tail",)
...
def _load_json(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"ORFR-1 expected a JSON object: {path}")
    return payload
...
def _verify_evaluation_contract(
    ...
    for key in (
        ...
        if asdict(cfg)[key] != policy[key]:
            raise ValueError(f"ORFR-1 strict engine differs on {key}")
    if cfg.cluster_draws != 20_000 or cfg.cluster_seed != 20_260_717:
        raise ValueError("ORFR-1 sign-flip configuration changed")
    ...
    expected = {
        ...
        "minimum_trades": 60 if stage2 else 100,
        "mean_gross_underlying_bp_min": 35.0,
        "stress_cost_absolute_return_positive": True,
        "each_subperiod_absolute_return_positive": True,
        "each_subperiod_minimum_trades": 20 if stage2 else 45,
        "minimum_each_side_trades": 15 if stage2 else 35,
        "mechanism_margin_ratio_min": 0.25,
    }
    if gates != expected:
        raise ValueError(f"ORFR-1 {key} contract changed")
    return cfg, gates
...
def _verify_static_inputs() -> tuple[dict[str, Any], dict[str, Any]]:
    for path, expected in STATIC_INPUT_SHA256.items():
        if _sha256(path) != expected:
            raise ValueError(f"ORFR-1 frozen input changed: {path}")
    ...
    if support.get("policy_id") != "ORFR-1":
        raise ValueError("ORFR-1 support identity changed")
    if support.get("outcomes_opened") is not False:
        raise ValueError("ORFR-1 support opened outcomes")
    if support.get("outcome_sources_opened") != []:
        raise ValueError("ORFR-1 support opened outcome sources")
    if support.get("support_passed") is not True:
        raise ValueError("ORFR-1 support failed")
    if support.get("advance_to_stage1_outcomes") is not True:
        raise ValueError("ORFR-1 support did not authorize Stage1")
    if support.get("preregistration_manifest_hash") != registration["manifest_hash"]:
        raise ValueError("ORFR-1 support is not bound to preregistration")
    if support.get("clocks", {}).get("sha256") != STATIC_INPUT_SHA256[str(CLOCKS)]:
        raise ValueError("ORFR-1 support is not bound to its clock ledger")
    return registration, support


def _random_side(entry_time: pd.Timestamp) -> int:
    digest = hashlib.sha256(
        ("ORFR-1-random-side-20260717|" + entry_time.isoformat()).encode()
    ).digest()
...
def load_schedules() -> dict[str, pd.DataFrame]:
    ...
    expected_controls = {"primary", "one_day_delta_tail", "one_release_delay"}
    if set(ledger["control"]) != expected_controls:
        raise ValueError("ORFR-1 support clock family changed")
    schedules = {
    ...
    for name, schedule in schedules.items():
        if not bool(schedule["side"].isin((-1, 1)).all()):
            raise ValueError(f"ORFR-1 {name} side changed")
        if not bool(
            schedule["entry_time"]
            .eq(schedule["signal_day"] + pd.Timedelta(minutes=5))
            .all()
        ):
            raise ValueError(f"ORFR-1 {name} source delay changed")
        if not bool(schedule["exit_time"].gt(schedule["entry_time"]).all()):
            raise ValueError(f"ORFR-1 {name} exit no longer follows entry")
        if len(schedule) > 1:
            ...
            if not bool(entries.ge(exits).all()):
                raise ValueError(f"ORFR-1 {name} schedule overlaps")
        if not bool(schedule["exit_time"].lt(STAGE2[1]).all()):
            raise ValueError(f"ORFR-1 {name} clock crosses source horizon")
    return schedules
...
def _schedule_record(frame: pd.DataFrame) -> dict[str, Any]:
    return {
        ...
        "entry_delay_exact": bool(
            frame["entry_time"]
            .eq(frame["signal_day"] + pd.Timedelta(minutes=5))
            .all()
        ),
        "positive_holding_intervals": bool(
            frame["exit_time"].gt(frame["entry_time"]).all()
        ),
...
def freeze_evaluator(output_path: str | Path = EVALUATOR_FREEZE) -> dict[str, Any]:
    ...
    if not all(
        ...
        for check in (
            "entry_delay_exact",
            "positive_holding_intervals",
            "globally_nonoverlapping",
        )
    ):
        raise ValueError("ORFR-1 evaluator schedule invariant failed")
    core = {
        "protocol_version": "overnight_rrp_flow_release_evaluator_v1",
        "policy_id": "ORFR-1",
...
def verify_evaluator_freeze(path: str | Path = EVALUATOR_FREEZE) -> dict[str, Any]:
    ...
    if report.get("manifest_hash") != _canonical_hash(core):
        raise ValueError("ORFR-1 evaluator freeze manifest mismatch")
    if report.get("evaluator_source_sha256") != _sha256(EVALUATOR_SOURCE):
        raise ValueError("ORFR-1 evaluator source changed after freeze")
    if report.get("opened_windows") != []:
        raise ValueError("ORFR-1 evaluator freeze opened a window")
    if report.get("execution_ohlc_rows_parsed_during_freeze") != 0:
        raise ValueError("ORFR-1 evaluator freeze parsed OHLC")
    if report.get("funding_rows_parsed_during_freeze") != 0:
        raise ValueError("ORFR-1 evaluator freeze parsed funding")
    if report.get("simulation_run_during_freeze") is not False:
        raise ValueError("ORFR-1 evaluator freeze simulated outcomes")
    schedules = load_schedules()
    for name, schedule in schedules.items():
        if report["control_schedules"][name] != _schedule_record(schedule):
            raise ValueError(f"ORFR-1 {name} schedule changed after freeze")
    return report
...
def _verify_execution_manifests() -> None:
    ...
    if market_manifest.get("combined_sha256") != source["market_sha256"]:
        raise ValueError("ORFR-1 market manifest no longer binds the source")
    funding_manifest = _load_json(FUNDING_MANIFEST)
    if funding_manifest.get("data", {}).get("sha256") != source["funding_sha256"]:
        raise ValueError("ORFR-1 funding manifest no longer binds the source")
    if funding_manifest.get("outcomes_opened") is not False:
        raise ValueError("ORFR-1 funding manifest opened an outcome")
    ...
    *,
    ...
    gates = {
        ...
        "each_subperiod_minimum_trades": all(
            value["trades"] >= gate_contract["each_subperiod_minimum_trades"]
            for value in split.values()
        ),
        "minimum_each_side_trades": each_side
        >= gate_contract["minimum_each_side_trades"],
    }
    return {
        "metrics": base,
...
def _build_stage_report(
    *,
    ...
    cfg, gate_contract = _verify_evaluation_contract(
        registration, stage2=is_stage2
    )
    schedules = load_schedules()
    clocks = {
    ...
    comparisons = {
        name: {
            ...
            "margin": primary_ratio
            - clocks[name]["headline"]["cagr_to_strict_mdd"],
            "passed": primary_ratio
            - clocks[name]["headline"]["cagr_to_strict_mdd"]
            >= gate_contract["mechanism_margin_ratio_min"],
        }
        for name in MECHANISM_CONTROLS
    }
    falsification = {
        name: clocks[name]["fully_qualified"] for name in FALSIFICATION_CONTROLS
    }
    gates = {
        **clocks["primary"]["gates"],
        ...
        "falsification_controls_do_not_fully_qualify": not any(
            falsification.values()
        ),
    }
    passed = all(gates.values())
    core = {
        "protocol_version": f"overnight_rrp_flow_release_{stage}_v1",
        "policy_id": "ORFR-1",
        "stage": stage,
...
def _verified_passing_stage1(expected_freeze_hash: str) -> dict[str, Any]:
    if not STAGE1_OUTPUT.exists():
        raise ValueError("ORFR-1 Stage1 has not been run")
    ...
    if stored.get("manifest_hash") != _canonical_hash(core):
        raise ValueError("ORFR-1 stored Stage1 manifest changed")
    if stored.get("evaluator_freeze_manifest_hash") != expected_freeze_hash:
        raise ValueError("ORFR-1 stored Stage1 freeze identity changed")
    if stored.get("evaluator_source_sha256") != _sha256(EVALUATOR_SOURCE):
        raise ValueError("ORFR-1 stored Stage1 evaluator changed")
    if stored.get("gate_passed") is not True:
        raise ValueError("ORFR-1 Stage1 failed; 2023 remains sealed")
    market, funding, diagnostics = load_execution_window(STAGE1)
    ...
    if replayed != stored:
        raise ValueError("ORFR-1 Stage1 does not exactly replay")
    return stored
...
def render_stage_doc(report: dict[str, Any]) -> str:
    ...
    return f"""# ORFR-1 {report['stage']} result — 2026-07-17
