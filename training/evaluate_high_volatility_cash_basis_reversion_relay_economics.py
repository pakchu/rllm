"""Sequential strict economics for frozen HVCBR-6."""
...
PREREG = Path("results/high_volatility_cash_basis_reversion_relay_preregistration_2026-08-09.json")
PREREG_SHA = "54153842b10ec87adc542d5edcb5afe1139ad086d4746bced38d1fb85873adbc"
SUPPORT = Path("results/high_volatility_cash_basis_reversion_relay_support_2026-08-08.json")
SUPPORT_SHA = "ab5f07fda7843ee9c6c5efff550e799db1c0934f045ebb6c48a5eec75af25287"
NOVELTY = Path("results/high_volatility_cash_basis_reversion_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "e380e433491a07ab5d034356058b32ae3946c8f3699d3d4450cc1772f3c8df6b"
CLOCK = Path("data/high_volatility_cash_basis_reversion_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "58b46adccc33e12eb3ed6636915ae0348821815aac76e35d47bfefa347dbbb83"
CONTROL_DIR = Path("data/high_volatility_cash_basis_reversion_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_cash_basis_reversion_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cash_basis_reversion_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "no_basis_shock_gate", "one_boundary_stale_geometry", "direction_follow_basis")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCBR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCBR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcbr_6_sequential_economics_v1", "policy_id": POLICY_ID,
