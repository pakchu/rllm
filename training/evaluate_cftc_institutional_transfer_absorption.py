"""Sequential strict evaluator for frozen CITA-1.
...
SUPPORT_COMMIT = "a6fe858"
STATIC_INPUT_SHA256 = {
    "training/preregister_cftc_institutional_transfer_absorption.py": "8c119147155c17c9f59fd51387fb549b547ca7bfa97c0ef89b9a74e666ffe2ea",
    "training/cftc_institutional_transfer_absorption_clock.py": "c5dcdd38451130b4e1efccf0251d65ff6c2b96a08a69495b1f7784998297dae1",
    "docs/cftc-institutional-transfer-absorption-preregistration-2026-07-17.md": "b911bed21cc3502aa08854509d383617f6d3d0b2f1348b31b4deb89863652cd6",
    "results/cftc_institutional_transfer_absorption_preregistration_2026-07-17.json": "3315cb85bc9235e686be1d848e3a319da94663472703ad34d52e4e7bd1b7a1aa",
    "results/cftc_institutional_transfer_absorption_preregistered_clock_2026-07-17.csv.gz": "65f64544ff8d45ab20882b770907cc958d96c5bd8982f731e09941de2e1a28d2",
    "training/build_cftc_institutional_transfer_absorption_support.py": "83c38289564199f4f1d5c041d5948336f163ee537f938c34031e0cc7a1bcdd78",
    "results/cftc_institutional_transfer_absorption_support_2026-07-17.json": "21067592f5774bbcd59ee0bf20b8dcd57a3f68a5adff73d7e2f5b4b8dff57e94",
    "results/cftc_institutional_transfer_absorption_clocks_2026-07-17.csv.gz": "8fb23176461d73c8be4e4069552e1920dfaec20a6e833bc37d6f108d8382da2d",
    ...
    "data/cftc_institutional_transfer_absorption_2018_2023/cftc_institutional_transfer_absorption_2018_2023.csv.gz": "064eed3fa340b1701f4686d1176de2a10f39128abc5ebf846e8b6319b8144ee6",
    "data/cftc_institutional_transfer_absorption_2018_2023/build_manifest.json": "e9d4ca15da671c086265557e5d302518fd4b9a9ad59fe9d0ff1181d772d60406",
    "data/cftc_institutional_transfer_absorption_2018_2023/source_manifest.json": "a594b02d1191c32f905c13be3faaa74ec2f3f0e04723d3b11b76ee8b454d6897",
}
...
SUPPORT_RESULT = Path("results/cftc_institutional_transfer_absorption_support_2026-07-17.json")
CLOCKS = Path("results/cftc_institutional_transfer_absorption_clocks_2026-07-17.csv.gz")
...
EVALUATOR_SOURCE = Path("training/evaluate_cftc_institutional_transfer_absorption.py")
EVALUATOR_FREEZE = Path(
    "results/cftc_institutional_transfer_absorption_evaluator_freeze_2026-07-17.json"
)
STAGE1_OUTPUT = Path(
    "results/cftc_institutional_transfer_absorption_stage1_2020_2022_2026-07-17.json"
)
STAGE1_DOC = Path(
    "docs/cftc-institutional-transfer-absorption-stage1-2020-2022-2026-07-17.md"
)
STAGE2_OUTPUT = Path(
    "results/cftc_institutional_transfer_absorption_stage2_2023_2026-07-17.json"
)
STAGE2_DOC = Path(
    "docs/cftc-institutional-transfer-absorption-stage2-2023-2026-07-17.md"
)
...
MECHANISM_CONTROLS = ("asset_manager_only", "leveraged_contrarian_only")
...
def _load_json(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"CITA-1 expected a JSON object: {path}")
    return payload
...
def _verify_registration(registration: dict[str, Any]) -> None:
    ...
    if registration.get("manifest_hash") != prereg.sha256_bytes(
        ...
        raise ValueError("CITA-1 preregistration manifest mismatch")
    if registration.get("policy_id") != "CITA-1":
        raise ValueError("CITA-1 preregistration identity changed")
    if registration.get("opened_outcome_windows") != []:
        raise ValueError("CITA-1 preregistration opened outcomes")
...
def _verify_evaluation_contract(registration: dict[str, Any]) -> EvaluationConfig:
    ...
    for key in (
        ...
        if asdict(cfg)[key] != policy[key]:
            raise ValueError(f"CITA-1 strict engine differs on {key}")
    if cfg.cluster_draws != 20_000 or cfg.cluster_seed != 20_260_717:
        raise ValueError("CITA-1 sign-flip configuration changed")
    if cfg.mdd_denominator_floor != 1e-9:
        raise ValueError("CITA-1 MDD denominator floor changed")
    ...
    if stage["stage1_gates"] != expected_stage1:
        raise ValueError("CITA-1 Stage1 gate contract changed")
    if stage["stage2_gates"] != expected_stage2:
        raise ValueError("CITA-1 Stage2 gate contract changed")
    return cfg
...
def _verify_static_inputs() -> tuple[dict[str, Any], dict[str, Any]]:
    for path, expected in STATIC_INPUT_SHA256.items():
        if _sha256(path) != expected:
            raise ValueError(f"CITA-1 frozen input changed: {path}")
    ...
    if support.get("manifest_hash") != _canonical_hash(support_core):
        raise ValueError("CITA-1 support manifest changed")
    if support.get("policy_id") != "CITA-1":
        raise ValueError("CITA-1 support identity changed")
    if support.get("outcomes_opened") is not False:
        raise ValueError("CITA-1 support opened outcomes")
    if support.get("outcome_sources_opened") != []:
        raise ValueError("CITA-1 support opened outcome sources")
    if support.get("market_or_funding_rows_opened") != 0:
        raise ValueError("CITA-1 support opened execution rows")
    if support.get("support_passed") is not True:
        raise ValueError("CITA-1 support failed")
    if support.get("advance_to_stage1_outcomes") is not True:
        raise ValueError("CITA-1 support did not authorize Stage1")
    if support.get("preregistration_manifest_hash") != registration["manifest_hash"]:
        raise ValueError("CITA-1 support is not bound to preregistration")
    if support.get("clocks", {}).get("sha256") != STATIC_INPUT_SHA256[str(CLOCKS)]:
        raise ValueError("CITA-1 support is not bound to the clock ledger")
...
def load_schedules() -> dict[str, pd.DataFrame]:
    ...
    if set(ledger["control"]) != set(ALL_CLOCK_NAMES):
        raise ValueError("CITA-1 support clock family changed")
    schedules = {
    ...
    for name, schedule in schedules.items():
        if not bool(schedule["side"].isin((-1, 1)).all()):
            raise ValueError(f"CITA-1 {name} side changed")
        if not bool(
            schedule["entry_time"]
            .eq(schedule["signal_day"] + pd.Timedelta(minutes=5))
            .all()
        ):
            raise ValueError(f"CITA-1 {name} source delay changed")
        if not bool(
            schedule["exit_time"]
            .eq(schedule["entry_time"] + pd.Timedelta(days=7))
            .all()
        ):
            raise ValueError(f"CITA-1 {name} hold changed")
        if len(schedule) > 1:
            ...
            if not bool(entries.ge(exits).all()):
                raise ValueError(f"CITA-1 {name} schedule overlaps")
        report_dates = pd.to_datetime(schedule["report_date"], utc=True)
        if not bool(report_dates.lt(STAGE2[1]).all()):
            raise ValueError(f"CITA-1 {name} source report crosses 2023")
    return schedules
...
def freeze_evaluator(output_path: str | Path = EVALUATOR_FREEZE) -> dict[str, Any]:
    ...
    if not all(
        ...
        raise ValueError("CITA-1 evaluator schedule invariant failed")
    core = {
        "protocol_version": "cftc_institutional_transfer_absorption_evaluator_v1",
        "policy_id": "CITA-1",
...
def verify_evaluator_freeze(path: str | Path = EVALUATOR_FREEZE) -> dict[str, Any]:
    ...
    if report.get("manifest_hash") != _canonical_hash(core):
        raise ValueError("CITA-1 evaluator freeze manifest mismatch")
    if report.get("evaluator_source_sha256") != _sha256(EVALUATOR_SOURCE):
        raise ValueError("CITA-1 evaluator source changed after freeze")
    if report.get("opened_windows") != []:
        raise ValueError("CITA-1 evaluator freeze opened a window")
    if report.get("execution_ohlc_rows_parsed_during_freeze") != 0:
        raise ValueError("CITA-1 evaluator freeze parsed OHLC")
    if report.get("funding_rows_parsed_during_freeze") != 0:
        raise ValueError("CITA-1 evaluator freeze parsed funding")
    if report.get("simulation_run_during_freeze") is not False:
        raise ValueError("CITA-1 evaluator freeze simulated outcomes")
    schedules = load_schedules()
    for name, schedule in schedules.items():
        if report["control_schedules"][name] != _schedule_record(schedule):
            raise ValueError(f"CITA-1 {name} schedule changed after freeze")
    return report
...
def _verify_execution_manifests() -> None:
    ...
    if market_manifest.get("combined_output") != str(MARKET):
        raise ValueError("CITA-1 market manifest path changed")
    if market_manifest.get("combined_sha256") != MARKET_DATA_SHA256:
        raise ValueError("CITA-1 market manifest data hash changed")
    if market_manifest.get("rows") != 420_768:
        raise ValueError("CITA-1 market manifest row count changed")
    if market_manifest.get("protocol", {}).get("outcomes_opened") is not False:
        raise ValueError("CITA-1 market manifest opened an outcome")
    funding_manifest = _load_json(FUNDING_MANIFEST)
    funding_data = funding_manifest.get("data", {})
    if funding_data.get("path") != str(FUNDING):
        raise ValueError("CITA-1 funding manifest path changed")
    if funding_data.get("sha256") != FUNDING_DATA_SHA256:
        raise ValueError("CITA-1 funding manifest data hash changed")
    if funding_data.get("rows") != 4_383:
        raise ValueError("CITA-1 funding manifest row count changed")
    if funding_manifest.get("outcomes_opened") is not False:
        raise ValueError("CITA-1 funding manifest opened an outcome")
    ...
    *,
    ...
    if stage == "stage1_2020_2022":
        gates = {
            ...
            <= 0.10,
            "minimum_75_trades": base["trades"] >= 75,
            ...
            "each_calendar_year_minimum_20_trades": all(
                value["trades"] >= 20 for value in split.values()
            ),
        }
    else:
        gates = {
            ...
            <= 0.20,
            "minimum_15_trades": base["trades"] >= 15,
            "h1_and_h2_absolute_return_nonnegative": all(
                value["absolute_return_pct"] >= 0.0 for value in split.values()
            ),
            "each_half_minimum_5_trades": all(
                value["trades"] >= 5 for value in split.values()
            ),
            "stress_cost_absolute_return_positive": stress["absolute_return_pct"]
            > 0.0,
        }
...
def _build_stage_report(
    *,
    ...
    core = {
        "protocol_version": f"cftc_institutional_transfer_absorption_{stage}_v1",
        "policy_id": "CITA-1",
        "stage": stage,
...
def _verified_passing_stage1(expected_freeze_hash: str) -> dict[str, Any]:
    if not STAGE1_OUTPUT.exists():
        raise ValueError("CITA-1 Stage1 has not been run")
    ...
    if stored.get("manifest_hash") != _canonical_hash(core):
        raise ValueError("CITA-1 stored Stage1 manifest changed")
    if stored.get("evaluator_freeze_manifest_hash") != expected_freeze_hash:
        raise ValueError("CITA-1 stored Stage1 freeze identity changed")
    if stored.get("evaluator_source_sha256") != _sha256(EVALUATOR_SOURCE):
        raise ValueError("CITA-1 stored Stage1 evaluator changed")
    if stored.get("gate_passed") is not True:
        raise ValueError("CITA-1 Stage1 failed; 2023 remains sealed")
    market, funding, diagnostics = load_execution_window(STAGE1)
    ...
    if replayed != stored:
        raise ValueError("CITA-1 Stage1 does not exactly replay")
    return stored
...
def render_stage_doc(report: dict[str, Any]) -> str:
    ...
    return f"""# CITA-1 {report['stage']} result — {prereg.AS_OF_DATE}
