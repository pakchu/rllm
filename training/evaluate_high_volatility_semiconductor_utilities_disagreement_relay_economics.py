"""Sequential strict economics for frozen HVSUDR-24."""
...
PREREG = Path("results/high_volatility_semiconductor_utilities_disagreement_relay_preregistration_2026-08-11.json")
PREREG_SHA = "0aa3096ecb9cb5fffbe12e55cbcd33084e33b95904f7da1cdb64cec87a2abd56"
SUPPORT = Path("results/high_volatility_semiconductor_utilities_disagreement_relay_support_2026-08-11.json")
SUPPORT_SHA = "c225a932c0af7083d8e197b132e37658eccc9baaa5d1d55902f034944888379d"
NOVELTY = Path("results/high_volatility_semiconductor_utilities_disagreement_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "5dc1b7c85b0679c7d6e9cfa6dcbee408777f3972b3928801cff5d552c7bf19fd"
CLOCK = Path("data/high_volatility_semiconductor_utilities_disagreement_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "2a820577fd820e56ad22950404ca892d20d30446c1b8b584223418a14be95ee9"
CONTROL_DIR = Path("data/high_volatility_semiconductor_utilities_disagreement_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_semiconductor_utilities_disagreement_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_semiconductor_utilities_disagreement_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSUDR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSUDR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvsudr_24_sequential_economics_v1", "policy_id": POLICY_ID,
