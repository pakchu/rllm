"""Evaluate frozen STCR-72 structural novelty against every Gross9 sleeve."""
...
POLICY = "STCR-72"
PROTOCOL = "stcr_72_gross9_novelty_v1"
...
OUTPUT = Path("results/scheduled_trend_concordance_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("STCR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("STCR novelty limits drift")
    if (
        ...
        raise RuntimeError("STCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="STCR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "stcr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject STCR-72 unchanged before economic outcomes",
    }
