"""Evaluate frozen HVAROON-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVAROON-24"
PROTOCOL = "hvaroon_24_gross9_novelty_v1"
PREREG = Path("results/high_volatility_aroon_recency_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "9e37b9667e813a344430362cced124bdecd37289b406a17df5ca17deffe80e5f"
SUPPORT = Path("results/high_volatility_aroon_recency_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "d2141c288a2b4fb7fce2033dda35ef6b96c19fc1430a51dc9dd7e992661f2ec6"
CLOCK = Path("data/high_volatility_aroon_recency_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "87e785b7ac37c17f705e86dfb687eea572835c69a9544e07eab4abd96c3aa115"
OUTPUT = Path("results/high_volatility_aroon_recency_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVAROON predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVAROON novelty limits drift")
    if (
        ...
        raise RuntimeError("HVAROON predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVAROON primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvaroon_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVAROON-24 unchanged before economic outcomes",
    }
