"""Evaluate frozen HVPIAR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVPIAR-8"
PROTOCOL = "hvpiar_8_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_price_impact_asymmetry_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVPIAR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVPIAR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVPIAR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVPIAR primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject HVPIAR-8 unchanged before economic outcomes"
        ),
    }
