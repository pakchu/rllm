"""Sequential strict economics for frozen HVPASR-24."""
...
PREREG = Path("results/high_volatility_palladium_asymmetric_spillover_relay_preregistration_2026-08-12.json")
PREREG_SHA = "2bee32685189f497337af215b14940e40884cd6f6ce9980bfd1cc10cbf21e9b6"
SUPPORT = Path("results/high_volatility_palladium_asymmetric_spillover_relay_support_2026-08-12.json")
SUPPORT_SHA = "ba7c673f6441fb3b8333f0a5ea89f7a102c49698c3354bc85f64dc916bd24d80"
NOVELTY = Path("results/high_volatility_palladium_asymmetric_spillover_relay_gross9_novelty_2026-08-12.json")
NOVELTY_SHA = "c2a32e3791e6be6a518f91f9c13d8d27f6b0ca5efffed91f7f09b311c48ac995"
CLOCK = Path("data/high_volatility_palladium_asymmetric_spillover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "5e9882601f3b2fd8d7fc19c30fb2982a8c615d6ab11a5147bf39781416f3bbf7"
CONTROL_DIR = Path("data/high_volatility_palladium_asymmetric_spillover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_palladium_asymmetric_spillover_relay_economic_evaluator_freeze_2026-08-12.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_palladium_asymmetric_spillover_relay_{stage}_economics_2026-08-12.json")
    for stage in STAGES
}
CONTROLS = (
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVPASR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVPASR novelty did not authorize economics")
...
def load_clock_allow_empty(path: Path, split: str, start: pd.Timestamp, end: pd.Timestamp) -> pd.DataFrame:
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    for name in CONTROLS:
        ...
        controls[name] = evaluate_control(load_clock_allow_empty(path, split, start, end), market, funding, start, end)
    ...
    core = {
        "protocol_version": "hvpasr_24_sequential_economics_v1",
        "policy_id": POLICY_ID,
        "stage": stage,
        ...
        "source": source,
        "physical_rows_opened": {"market": len(market), "funding": len(funding), "primary_clock": len(primary_clock)},
        "later_stage_outcomes_opened": False,
        "primary": primary,
        "controls_diagnostic_only": controls,
        "checks": checks,
        "passed": passed,
        "advance_to_next_stage": passed and stage != "final",
        "advance_to_post_stage_volatility_audit": passed and stage == "final",
        "decision": "terminal_reject_no_repair" if not passed else "pass_to_post_stage_volatility_audit" if stage == "final" else "pass",
    }
...
if __name__ == "__main__":
    ...
    if args.verify_only:
        ...
        print(json.dumps({"stage": args.stage, "passed": result["passed"], "output": str(args.output or OUTPUTS[args.stage])}))
