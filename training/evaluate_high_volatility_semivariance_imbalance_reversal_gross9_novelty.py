"""Evaluate frozen HVSIR-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVSIR-6"
PROTOCOL = "hvsir_6_gross9_novelty_v1"
PREREG = Path(
...
SUPPORT = Path("results/high_volatility_semivariance_imbalance_reversal_support_2026-08-09.json")
SUPPORT_SHA = "0cd9a46a73a4f7e9deec200cbdae76d1f6be4f5a3b6c240b034abafd8c5f0305"
CLOCK = Path("data/high_volatility_semivariance_imbalance_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "49bd3ea1ca8e0041da9a89b79f11679ee8f7cba7a3d3dee0d595ef4dcf1dd8d0"
OUTPUT = Path(
    "results/high_volatility_semivariance_imbalance_reversal_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVSIR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVSIR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVSIR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVSIR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvsir_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVSIR-6 unchanged before economic outcomes"
        ),
    }
