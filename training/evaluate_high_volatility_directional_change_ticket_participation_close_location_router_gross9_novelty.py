"""Evaluate the frozen HVDCSATPCLR-8 action-vote clock against Gross9."""
...
POLICY = "HVDCSATPCLR-8"
PROTOCOL = "hvdcsatpclr_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_directional_change_ticket_participation_close_location_router_preregistration_2026-08-16.json")
PREREG_SHA = "b204e4e2d27d96ddc617d0223654801ca45001245696369baf9f5d238fb57bf7"
SUPPORT = Path("results/high_volatility_directional_change_ticket_participation_close_location_router_support_2026-08-16.json")
SUPPORT_SHA = "2cfcdfc119d60dbbe4e0e1f6ade2b7846b0ab11e586ccc52231b938b77a82d43"
CLOCK = {
    "path": "data/high_volatility_directional_change_ticket_participation_close_location_router_clocks_2023_2026.csv.gz",
    "sha256": "1b1353d89b7be237a0bff09ab1b8d6f9c390b5760a9a98ca1db155bfc3df4acf",
    "rows": 147,
}
OUTPUT = Path("results/high_volatility_directional_change_ticket_participation_close_location_router_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVDCSATPCLR-8 preregistration hash drift")
    ...
    if (
        ...
        or registration.get("singleton") is not True
        or registration.get("research_boundary", {}).get("candidate_count") != 1
        or registration.get("novelty_gates") != expected_limits
    ):
        raise RuntimeError("HVDCSATPCLR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVDCSATPCLR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVDCSATPCLR-8 source eligibility drift")
    sealed = (
        "postentry_return_pnl_execution_price_opened",
        "held_interval_funding_values_opened",
        "gross9_rows_opened",
    )
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVDCSATPCLR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVDCSATPCLR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVDCSATPCLR-8 unchanged before economics",
    }
