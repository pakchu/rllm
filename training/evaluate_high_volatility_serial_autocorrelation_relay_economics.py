"""Sequential strict economics for frozen HVSAR-12."""
...
PREREG = Path("results/high_volatility_serial_autocorrelation_relay_preregistration_2026-08-09.json")
PREREG_SHA = "82407cce2a8b6c74a7ac9a849be070697431cc80ce3eaae4c054d5c7ad49321d"
SUPPORT = Path("results/high_volatility_serial_autocorrelation_relay_support_2026-08-09.json")
SUPPORT_SHA = "2d2a1d9c14a3698abbcb83b96dbed49bb86c630a326b3f0f2cd46f62db9d7c1f"
NOVELTY = Path("results/high_volatility_serial_autocorrelation_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "de9c8afc7e96e7aaf4c3187e543974c2b82ff44c3b88088acf57f9256f73ad7c"
CLOCK = Path("data/high_volatility_serial_autocorrelation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "a9b24ec077c5d82d716ada0240960f4ed41b2b0495d46e72b07ea9c59166d0be"
CONTROL_DIR = Path("data/high_volatility_serial_autocorrelation_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_serial_autocorrelation_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_serial_autocorrelation_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "no_autocorrelation_tail_gate", "one_boundary_stale_features", "fixed_momentum", "fixed_reversal", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvsar_12_sequential_economics_v1", "policy_id": POLICY_ID,
