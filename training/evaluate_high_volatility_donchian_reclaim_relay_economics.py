"""Sequential strict economics for frozen HVDCR-72."""
...
PREREG = Path("results/high_volatility_donchian_reclaim_relay_preregistration_2026-08-09.json")
PREREG_SHA = "c85052fccf42387a2d9756aa98bf791993bfa7edea3f661ae959904fcc441906"
SUPPORT = Path("results/high_volatility_donchian_reclaim_relay_support_2026-08-09.json")
SUPPORT_SHA = "b1322206c88b3437a758b15ca2bd62ee0a39d2e779396f8f941addd6cd5a331d"
NOVELTY = Path("results/high_volatility_donchian_reclaim_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "ce8e3eff9d6ef0a7529e26e83eedebb438e23bd5a39c6642dc8e37dd25298e1f"
CLOCK = Path("data/high_volatility_donchian_reclaim_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "6658e7e819be89353ff4d876c43405d455aeb2dd22fe8bd9febba55c460469e2"
CONTROL_DIR = Path("data/high_volatility_donchian_reclaim_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_donchian_reclaim_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_donchian_reclaim_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdcr_72_sequential_economics_v1", "policy_id": POLICY_ID,
