"""Deterministic outcome-blind source support for HVCIOAR-8."""
...
from training import preregister_high_volatility_causal_open_interest_acceleration_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "d1cb6bc18469bee3ad6995e260b034dac7dbbb300e0fbc182c965b6074cd1ca1"
...
OI_QUERY = """SELECT ts AS decision_time,sum_open_interest FROM open_interest_binance WHERE symbol='BTCUSDT' AND period='5m' AND ts>=:start AND ts<:end AND extract(minute FROM ts)=0 AND extract(second FROM ts)=0 AND extract(hour FROM ts) IN (0,4,8,12,16,20) ORDER BY ts"""
BAR_QUERY = """SELECT date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '8 hours' AS decision_time,sum(ln(close/open)) AS completed_return,sum(power(ln(close/open),2)) AS minute_squared_return,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND high>=greatest(open,close,low) AND low<=least(open,close,high)) AS coherent FROM bars_binance WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end GROUP BY 1 ORDER BY 1"""

ROOT = Path("data/high_volatility_causal_open_interest_acceleration_relay_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_open_interest_acceleration_relay_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_open_interest_acceleration_relay_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_causal_open_interest_acceleration_relay_support_2026-08-16.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "sum_open_interest",
    "midpoint_open_interest", "start_open_interest", "first_half_oi_change", "second_half_oi_change", "oi_change",
    "oi_acceleration", "oi_acceleration_rank", "completed_return", "realized_variation",
    "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "decision_time", "feature_available_time",
    "entry_time", "exit_time", "side", "start_open_interest", "midpoint_open_interest", "sum_open_interest",
    "first_half_oi_change", "second_half_oi_change", "oi_change", "oi_acceleration", "oi_acceleration_rank",
    "completed_return", "realized_variation", "variation_rank",
)
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVCIOAR-8 OI schema drift")
    expected_bars = [
        "decision_time", "completed_return", "minute_squared_return",
        "source_rows", "distinct_rows", "first_ts", "last_ts", "coherent",
    ]
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCIOAR-8 bar schema drift")
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    oi, bars = prepare(raw)
    panel = oi.copy()
    panel["midpoint_time"] = panel["decision_time"].shift(1)
    panel["start_time"] = panel["decision_time"].shift(2)
    panel["midpoint_open_interest"] = panel["sum_open_interest"].shift(1)
    panel["start_open_interest"] = panel["sum_open_interest"].shift(2)
    panel = panel.loc[panel["decision_time"].dt.hour.isin([0, 8, 16])].copy()
    positive = panel[["sum_open_interest", "midpoint_open_interest", "start_open_interest"]].gt(0).all(axis=1)
    panel["first_half_oi_change"] = np.nan
    panel["second_half_oi_change"] = np.nan
    panel["oi_change"] = np.nan
    panel.loc[positive, "first_half_oi_change"] = np.log(panel.loc[positive, "midpoint_open_interest"] / panel.loc[positive, "start_open_interest"])
    panel.loc[positive, "second_half_oi_change"] = np.log(panel.loc[positive, "sum_open_interest"] / panel.loc[positive, "midpoint_open_interest"])
    panel.loc[positive, "oi_change"] = np.log(panel.loc[positive, "sum_open_interest"] / panel.loc[positive, "start_open_interest"])
    panel["oi_acceleration"] = panel["second_half_oi_change"] - panel["first_half_oi_change"]
    panel = panel.join(
        bars[["bar_valid", "completed_return", "realized_variation"]],
        on="decision_time",
    )
    panel["source_valid"] = (
        panel["start_time"].notna()
        & panel["decision_time"].sub(panel["midpoint_time"]).eq(pd.Timedelta(hours=4))
        & panel["midpoint_time"].sub(panel["start_time"]).eq(pd.Timedelta(hours=4))
        & panel["bar_valid"].eq(True)
        & np.isfinite(
            panel[["sum_open_interest", "midpoint_open_interest", "start_open_interest", "first_half_oi_change", "second_half_oi_change", "oi_change", "oi_acceleration", "completed_return", "realized_variation"]]
        ).all(axis=1)
        & panel["sum_open_interest"].gt(0)
        & panel["midpoint_open_interest"].gt(0)
        & panel["start_open_interest"].gt(0)
        & panel["realized_variation"].gt(0)
        & panel["completed_return"].ne(0)
        & panel["oi_change"].gt(0)
        & panel["oi_acceleration"].gt(0)
    )
    valid = panel["source_valid"].eq(True)
    panel["oi_acceleration_rank"] = causal_midrank(panel["oi_acceleration"].where(valid))
    panel["variation_rank"] = causal_midrank(panel["realized_variation"].where(valid))
    panel["eligible"] = (
        valid
        & panel["oi_acceleration_rank"].ge(POLICY["oi_acceleration_rank_min"])
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCIOAR-8 side or availability drift")
        ...
        rows.append({
            ...
            "start_open_interest": float(row.start_open_interest),
            "midpoint_open_interest": float(row.midpoint_open_interest),
            "sum_open_interest": float(row.sum_open_interest),
            "first_half_oi_change": float(row.first_half_oi_change),
            "second_half_oi_change": float(row.second_half_oi_change),
            "oi_change": float(row.oi_change),
            "oi_acceleration": float(row.oi_acceleration),
            "oi_acceleration_rank": float(row.oi_acceleration_rank),
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCIOAR-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcioar_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvcioar_8_source_support_v1", "policy_id": prereg.POLICY_ID,
