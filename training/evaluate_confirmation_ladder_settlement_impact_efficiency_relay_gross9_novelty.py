"""Evaluate frozen CLSIER-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "CLSIER-6"
PROTOCOL = "clsier_6_gross9_novelty_v1"
...
OUTPUT = Path("results/confirmation_ladder_settlement_impact_efficiency_relay_gross9_novelty_2026-08-12.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CLSIER predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("CLSIER novelty limits drift")
    if (
        ...
        raise RuntimeError("CLSIER predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CLSIER primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "clsier_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject CLSIER-6 unchanged before economic outcomes",
    }
