"""Sequential strict evaluator for frozen CTHD-1.
...
SUPPORT_COMMIT = "95a3fcd"
STATIC_INPUT_SHA256 = {
    "training/preregister_cboe_tail_hedge_disagreement.py": "4bb5082dd95ff32efa1f2d3fa444089e44eb2d1190537a5039a9f1923a47b9f1",
    "training/cboe_tail_hedge_disagreement_clock.py": "4ec3fee79bc76772213efb6a5598eb08a4c6b74979ec156ef15dad1c94ba6c93",
    "docs/cboe-tail-hedge-disagreement-preregistration-2026-07-18.md": "639038db6e9aec6bb451cfd70d9d7a870f25840c5ab4b5f6f7e486c451b15e42",
    "results/cboe_tail_hedge_disagreement_preregistration_2026-07-18.json": "d9e0e767e293d17c4845d300dad22c113b863796ef309d4d06ec8ecbe7330d0b",
    "training/build_cboe_tail_hedge_disagreement_support.py": "70f117cfbbc9b81fb021376c4dc48ec2acbc12cafde52decb1c2365e5794b220",
    "docs/cboe-tail-hedge-disagreement-support-2026-07-18.md": "d3f8c841cf3de57e99cbae24b6656c5d0c71135c9e0e017ba7b5c5b4a6d69008",
    "results/cboe_tail_hedge_disagreement_support_2026-07-18.json": "2a2f8bca6adc04812949a0af9f005b83e72f97395297c3547d1bd9748466c937",
    "results/cboe_tail_hedge_disagreement_clocks_2026-07-18.csv.gz": "0e19455e2fb5ab2d36cc996c9adf514adc85c69dd1a325562344a8015464d546",
...
SUPPORT_RESULT = Path("results/cboe_tail_hedge_disagreement_support_2026-07-18.json")
CLOCKS = Path("results/cboe_tail_hedge_disagreement_clocks_2026-07-18.csv.gz")
...
EVALUATOR_SOURCE = Path("training/evaluate_cboe_tail_hedge_disagreement.py")
EVALUATOR_FREEZE = Path(
    "results/cboe_tail_hedge_disagreement_evaluator_freeze_2026-07-18.json"
)
STAGE1_OUTPUT = Path(
    "results/cboe_tail_hedge_disagreement_stage1_2021_2022_2026-07-18.json"
)
STAGE1_DOC = Path(
    "docs/cboe-tail-hedge-disagreement-stage1-2021-2022-2026-07-18.md"
)
STAGE2_OUTPUT = Path(
    "results/cboe_tail_hedge_disagreement_stage2_2023_2026-07-18.json"
)
STAGE2_DOC = Path("docs/cboe-tail-hedge-disagreement-stage2-2023-2026-07-18.md")
...
def _timestamp(value: Any) -> pd.Timestamp:
    ...
    if result is pd.NaT:
        raise ValueError("CTHD-1 timestamp is NaT")
...
def _load_json(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"CTHD-1 expected a JSON object: {path}")
    return payload
...
def _verify_evaluation_contract(
    ...
    for key in (
        ...
        if asdict(cfg)[key] != policy[key]:
            raise ValueError(f"CTHD-1 strict engine differs on {key}")
    if cfg.cluster_draws != 20_000 or cfg.cluster_seed != 20_260_717:
        raise ValueError("CTHD-1 sign-flip configuration changed")
    ...
    expected = {
        ...
        "minimum_trades": 100 if stage2 else 150,
        "minimum_short_trades": 100 if stage2 else 150,
        "mean_gross_underlying_bp_min": 35.0,
        "stress_cost_absolute_return_positive": True,
        "each_subperiod_absolute_return_positive": True,
        "each_subperiod_minimum_trades": 20 if stage2 else 30,
        "mechanism_margin_ratio_min": 0.25,
    }
    if gates != expected:
        raise ValueError(f"CTHD-1 {key} contract changed")
    return cfg, gates
...
def _verify_static_inputs() -> tuple[dict[str, Any], dict[str, Any]]:
    for path, expected in STATIC_INPUT_SHA256.items():
        if _sha256(path) != expected:
            raise ValueError(f"CTHD-1 frozen input changed: {path}")
    ...
    if support.get("manifest_hash") != _canonical_hash(support_core):
        raise ValueError("CTHD-1 support manifest changed")
    if support.get("policy_id") != "CTHD-1":
        raise ValueError("CTHD-1 support identity changed")
    if support.get("outcomes_opened") is not False or support.get("outcome_sources_opened") != []:
        raise ValueError("CTHD-1 support opened outcomes")
    if support.get("support_passed") is not True or support.get("advance_to_stage1_outcomes") is not True:
        raise ValueError("CTHD-1 source support failed")
    if support.get("preregistration_manifest_hash") != registration["manifest_hash"]:
        raise ValueError("CTHD-1 support is not bound to preregistration")
    if support.get("clocks", {}).get("sha256") != STATIC_INPUT_SHA256[str(CLOCKS)]:
        raise ValueError("CTHD-1 support is not bound to its clock ledger")
...
def load_schedules() -> dict[str, pd.DataFrame]:
    ...
    if set(ledger["control"]) != base_names:
        raise ValueError("CTHD-1 support clock family changed")
    schedules = {
    ...
    for name in base_names:
        if len(schedules[name]) != int(expected_counts[name]):
            raise ValueError(f"CTHD-1 {name} source count changed")
    ...
    for name, schedule in schedules.items():
        if not bool(schedule["side"].isin((-1, 1)).all()):
            raise ValueError(f"CTHD-1 {name} side changed")
        if not bool(schedule["entry_time"].eq(schedule["signal_day"]).all()):
            raise ValueError(f"CTHD-1 {name} decision/entry clock changed")
        if not bool(schedule["exit_time"].gt(schedule["entry_time"]).all()):
            raise ValueError(f"CTHD-1 {name} exit no longer follows entry")
        if len(schedule) > 1:
            ...
            if not bool(entries.ge(exits).all()):
                raise ValueError(f"CTHD-1 {name} schedule overlaps")
        if not bool(schedule["exit_time"].lt(STAGE2[1]).all()):
            raise ValueError(f"CTHD-1 {name} clock crosses source horizon")
    return schedules
...
def freeze_evaluator(output_path: str | Path = EVALUATOR_FREEZE) -> dict[str, Any]:
    ...
    if not all(
        ...
        raise ValueError("CTHD-1 evaluator schedule invariant failed")
    core = {
        "protocol_version": "cboe_tail_hedge_disagreement_evaluator_v1",
        "policy_id": "CTHD-1",
...
def verify_evaluator_freeze(path: str | Path = EVALUATOR_FREEZE) -> dict[str, Any]:
    ...
    if report.get("manifest_hash") != _canonical_hash(core):
        raise ValueError("CTHD-1 evaluator freeze manifest changed")
    if report.get("evaluator_source_sha256") != _sha256(EVALUATOR_SOURCE):
        raise ValueError("CTHD-1 evaluator source changed after freeze")
    if report.get("support_commit") != SUPPORT_COMMIT:
        raise ValueError("CTHD-1 evaluator froze another support commit")
    if report.get("evaluation_config") != asdict(EvaluationConfig()):
        raise ValueError("CTHD-1 evaluation configuration changed")
    if report.get("opened_windows") != [] or report.get("mutable_parameters") != []:
        raise ValueError("CTHD-1 evaluator freeze is not sealed")
    if report.get("execution_ohlc_rows_parsed_during_freeze") != 0:
        raise ValueError("CTHD-1 evaluator freeze parsed OHLC")
    if report.get("funding_rows_parsed_during_freeze") != 0:
        raise ValueError("CTHD-1 evaluator freeze parsed funding")
    if report.get("simulation_run_during_freeze") is not False:
        raise ValueError("CTHD-1 evaluator freeze simulated an outcome")
    schedules = load_schedules()
    for name, schedule in schedules.items():
        if report["control_schedules"][name]["schedule_hash"] != _schedule_hash(schedule):
            raise ValueError(f"CTHD-1 {name} schedule changed after freeze")
    return report
...
def _verify_execution_manifests() -> None:
    ...
    if market_manifest.get("combined_sha256") != source["market_sha256"]:
        raise ValueError("CTHD-1 market manifest no longer binds the frozen source")
    funding_manifest = _load_json(FUNDING_MANIFEST)
    if funding_manifest.get("data", {}).get("sha256") != source["funding_sha256"]:
        raise ValueError("CTHD-1 funding manifest no longer binds the frozen source")
    if funding_manifest.get("outcomes_opened") is not False:
        raise ValueError("CTHD-1 funding source manifest opened an outcome")
...
def _build_stage_report(
    *,
    ...
    core = {
        "protocol_version": f"cboe_tail_hedge_disagreement_{stage}_v1",
        "policy_id": "CTHD-1",
        "stage": stage,
...
def _verified_passing_stage1(expected_freeze_hash: str) -> dict[str, Any]:
    if not STAGE1_OUTPUT.exists():
        raise ValueError("CTHD-1 Stage1 has not been run")
    ...
    if stored.get("manifest_hash") != _canonical_hash(core):
        raise ValueError("CTHD-1 stored Stage1 manifest changed")
    if stored.get("evaluator_freeze_manifest_hash") != expected_freeze_hash:
        raise ValueError("CTHD-1 stored Stage1 freeze identity changed")
    if stored.get("evaluator_source_sha256") != _sha256(EVALUATOR_SOURCE):
        raise ValueError("CTHD-1 stored Stage1 evaluator changed")
    if stored.get("gate_passed") is not True:
        raise ValueError("CTHD-1 Stage1 failed; 2023 remains sealed")
    market, funding, diagnostics = load_execution_window(STAGE1)
    ...
    if replayed != stored:
        raise ValueError("CTHD-1 Stage1 does not exactly replay")
    return stored
...
def render_stage_doc(report: dict[str, Any]) -> str:
    ...
    return f"""# CTHD-1 {report['stage']} result — 2026-07-18
