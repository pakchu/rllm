"""Evaluate frozen HVDFR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDFR-8"
PROTOCOL = "hvdfr_8_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_directional_feedback_relay_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDFR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVDFR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDFR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDFR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdfr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVDFR-8 unchanged before economic outcomes"
        ),
    }
