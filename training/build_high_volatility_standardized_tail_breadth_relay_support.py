"""Source-only support gate for frozen HVSTBR-8."""
...
PREREG_SHA = "6d70486179ac1ac8b35c53fbff02a580e526207216f286c746ef7659c45b69ca"
SPLITS = {
...
ROOT = Path("data/high_volatility_standardized_tail_breadth_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_standardized_tail_breadth_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_standardized_tail_breadth_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_standardized_tail_breadth_relay_support_2026-08-10.json")
...
def tail_statistics(close: np.ndarray) -> tuple[float, int, int, int, float]:
    close = np.asarray(close, dtype=float)
    if len(close) != 480 or not np.isfinite(close).all() or np.any(close <= 0):
        return math.nan, 0, 0, 0, math.nan
    returns = np.diff(np.log(close))
    rms = float(np.sqrt(np.square(returns).mean()))
    if rms <= 0 or not math.isfinite(rms):
        return math.nan, 0, 0, 0, math.nan
    positive = int(np.sum(returns >= rms))
    negative = int(np.sum(returns <= -rms))
    breadth = positive - negative
    denominator = positive + negative
    share = abs(breadth) / denominator if denominator > 0 else math.nan
    return rms, positive, negative, breadth, float(share)
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVSTBR source timestamps")
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        if valid:
            ...
            rms_scale, positive_count, negative_count, tail_breadth, breadth_share = tail_statistics(
                window["close"].to_numpy(float)
            )
            valid = bool(
                variation > 0
                and tail_breadth != 0
                and math.isfinite(rms_scale)
                and math.isfinite(breadth_share)
            )
        if not valid:
            variation = rms_scale = breadth_share = math.nan
            positive_count = negative_count = tail_breadth = 0
        rows.append(
            {
                ...
                "tail_breadth_share": breadth_share,
            }
        )
    states = pd.DataFrame(rows)
    states["variation_rank"] = prior_rank(states["realized_variation"].where(states["source_valid"]))
    states["tail_breadth_share_rank"] = prior_rank(
        states["tail_breadth_share"].where(states["source_valid"])
    )
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvstbr_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    breadth_rank = states["tail_breadth_share_rank"]
    breadth = states["tail_breadth"]
    if control == "one_block_stale_features":
        variation_rank, breadth_rank, breadth = [
            value.shift(1) for value in (variation_rank, breadth_rank, breadth)
        ]
    variation_gate = (
    ...
    if control == "no_tail_breadth_share_gate":
        breadth_gate = pd.Series(True, index=states.index)
    elif control == "raw_absolute_tail_breadth_at_least_one":
        breadth_gate = breadth.abs().ge(1)
    else:
        breadth_gate = breadth_rank.ge(0.75)
    eligible = states["source_valid"] & variation_gate & breadth_gate & breadth.ne(0)
    onset = eligible & ~eligible.shift(1, fill_value=False) & states["source_valid"].shift(1, fill_value=False)
    return onset, np.sign(breadth)
        ...
        rows.append(
            {
                ...
                "rms_scale": float(states.at[index, "rms_scale"]),
                "positive_tail_count": int(states.at[index, "positive_tail_count"]),
                "negative_tail_count": int(states.at[index, "negative_tail_count"]),
                "tail_breadth": int(states.at[index, "tail_breadth"]),
                "tail_breadth_share": float(states.at[index, "tail_breadth_share"]),
                "tail_breadth_share_rank": float(
                    states.at[index, "tail_breadth_share_rank"]
                ),
            }
        )
    columns = [
        ...
        "rms_scale", "positive_tail_count", "negative_tail_count", "tail_breadth",
        "tail_breadth_share", "tail_breadth_share_rank",
    ]
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVSTBR preregistration drift")
    ...
    core = {
        "protocol_version": "hvstbr_8_source_support_v1",
        "policy_id": "HVSTBR-8",
        "preregistration": {
