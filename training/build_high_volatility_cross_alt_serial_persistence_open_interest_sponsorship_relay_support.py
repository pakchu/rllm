"""Outcome-blind source support for frozen HVCASPCOIS-8."""
...
from training import preregister_high_volatility_cross_alt_serial_persistence_open_interest_sponsorship_relay as prereg
ENV_FILE='/home/pakchu/rllm/.env';START=pd.Timestamp('2023-01-01T00:00:00Z');END=pd.Timestamp('2026-08-01T00:00:00Z');PREREG_SHA='ae55ad43aeee388bf80eb33963a9fac32a3f116ac906e8b1b73a51041b7f6e69';REG=prereg.build();P=REG['policy'];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG['stages'].items()};GATES=REG['source_support_gates'];CONTROLS=tuple(REG['diagnostic_controls']['names']);SYMBOLS=('BTCUSDT',*prereg.ALTS)
...
ROOT=Path('data/high_volatility_cross_alt_serial_persistence_open_interest_sponsorship_relay_sources_2023_2026');PANEL=ROOT/'block_states.csv.gz';MANIFEST=ROOT/'manifest.json';CLOCK=Path('data/high_volatility_cross_alt_serial_persistence_open_interest_sponsorship_relay_clocks_2023_2026.csv.gz');SPLIT_DIR=Path('data/high_volatility_cross_alt_serial_persistence_open_interest_sponsorship_relay_split_clocks_2023_2026');CONTROL_DIR=Path('data/high_volatility_cross_alt_serial_persistence_open_interest_sponsorship_relay_controls_2023_2026');RESULT=Path('results/high_volatility_cross_alt_serial_persistence_open_interest_sponsorship_relay_support_2026-08-16.json');BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=('decision_time','feature_available_time','source_valid','consensus_side','consensus_breadth','consensus_strength','broad_side','broad_breadth','negative_side','negative_breadth','negative_strength','btc_realized_variation','variation_rank','oi_start','oi_end','oi_change','eligible');CLOCK_COLUMNS=('candidate','control','split','decision_time','feature_available_time','entry_time','exit_time','side','consensus_breadth','consensus_strength','btc_realized_variation','variation_rank','oi_start','oi_end','oi_change')
...
def load_source():
 ...
 try:
  with db.connect() as c:
   ...
   oi=pd.read_sql_query(text(OI_QUERY),c,params={'start':START-pd.Timedelta('24h'),'end':END})
   return bars,oi
 finally:db.dispose()
def prepare(raw):
 ...
 if raw.columns.tolist()!=required:raise RuntimeError('HVCASPCOIS source schema drift')
 x=raw.copy()
 ...
 if x[['block_start','symbol']].isna().any().any() or x.duplicated(['block_start','symbol']).any():raise RuntimeError('HVCASPCOIS invalid key')
...
def build_panel(raw):
 bars_raw,oi_raw=raw;x=prepare(bars_raw);decisions=pd.date_range(START+pd.Timedelta('3h'),END,freq='8h',inclusive='left');full=x.reindex(pd.MultiIndex.from_product([decisions,SYMBOLS],names=['decision_time','symbol']));valid=full.row_valid.unstack('symbol').reindex(columns=SYMBOLS);disp=full.displacement.unstack('symbol').reindex(columns=SYMBOLS);pers=full.serial_persistence.unstack('symbol').reindex(columns=SYMBOLS);sq=full.minute_squared_return.unstack('symbol').reindex(columns=SYMBOLS);oi=oi_raw.copy();oi['ts']=pd.to_datetime(oi.ts,utc=True,errors='raise');oi['sum_open_interest']=pd.to_numeric(oi.sum_open_interest,errors='raise')
 if oi.columns.tolist()!=['ts','sum_open_interest'] or oi.ts.duplicated().any() or not np.isfinite(oi.sum_open_interest).all() or not oi.sum_open_interest.ge(0).all():raise RuntimeError('HVCASPCOIS invalid OI source')
 oi_series=oi.set_index('ts').sort_index().sum_open_interest;ranks=pd.DataFrame(index=decisions,columns=prereg.ALTS,dtype=float);negative_ranks=pd.DataFrame(index=decisions,columns=prereg.ALTS,dtype=float)
 ...
 for d in decisions:
  oi_start=oi_series.get(d-pd.Timedelta(hours=P['open_interest_lookback_hours']),math.nan);oi_end=oi_series.get(d,math.nan);oi_ok=math.isfinite(oi_start) and math.isfinite(oi_end) and oi_start>0 and oi_end>0;oi_change=math.log(oi_end/oi_start) if oi_ok else math.nan;ok=bool(valid.loc[d].eq(True).all() and np.isfinite(ranks.loc[d]).all() and math.isfinite(float(btcvar.loc[d])) and btcvar.loc[d]>0 and oi_ok);sign=np.sign(disp.loc[d,list(prereg.ALTS)].to_numpy(float));pv=pers.loc[d,list(prereg.ALTS)].to_numpy(float);rr=ranks.loc[d].to_numpy(float);nr=negative_ranks.loc[d].to_numpy(float);side,breadth,st=consensus(sign,rr,pv>0,P['persistence_rank_min']) if ok else (0,0,math.nan);bside,bbreadth,_=consensus(sign,np.ones(6),pv>0,0.) if ok else (0,0,math.nan);nside,nbreadth,nst=consensus(sign,nr,pv<0,P['persistence_rank_min']) if ok else (0,0,math.nan);rows.append({'decision_time':d,'feature_available_time':d,'source_valid':ok,'consensus_side':side,'consensus_breadth':breadth,'consensus_strength':st,'broad_side':bside,'broad_breadth':bbreadth,'negative_side':nside,'negative_breadth':nbreadth,'negative_strength':nst,'btc_realized_variation':float(btcvar.loc[d]) if ok else math.nan,'oi_start':oi_start,'oi_end':oi_end,'oi_change':oi_change})
 panel=pd.DataFrame(rows);v=panel.source_valid.eq(True);panel['variation_rank']=causal(panel.btc_realized_variation.where(v));panel['eligible']=v&panel.consensus_breadth.ge(P['minimum_consensus_breadth'])&panel.variation_rank.ge(P['variation_rank_min'])&panel.oi_change.gt(P['open_interest_log_change_min_strict']);return panel.loc[:,PANEL_COLUMNS]
...
def active(panel,control='primary'):
 ...
 valid=used.source_valid.eq(True)&np.isfinite(strength);oi_gate=pd.Series(True,index=used.index) if control=='no_open_interest_sponsorship_gate' else used.oi_change.gt(P['open_interest_log_change_min_strict']);state=valid&breadth.ge(P['minimum_consensus_breadth'])&used.variation_rank.ge(P['variation_rank_min'])&oi_gate
 if control=='no_variation_gate':state=valid&breadth.ge(P['minimum_consensus_breadth'])&oi_gate
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError('HVCASPCOIS prereg drift')
