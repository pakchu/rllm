"""Evaluate frozen HVPSAR-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVPSAR-24"
PROTOCOL = "hvpsar_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_parabolic_sar_reversal_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVPSAR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVPSAR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVPSAR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVPSAR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVPSAR-24 unchanged before economic outcomes",
    }
