"""Outcome-blind preregistration for HVCFZR-8."""
...
POLICY_ID = "HVCFZR-8"
DEFAULT_OUTPUT = Path("results/high_volatility_causal_funding_zero_cross_relay_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_causal_funding_zero_cross_relay_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": "When the actual funding rate crosses zero with an elevated absolute new level in a causally high-variation cycle, the new leverage-demand regime persists over the next settlement cycle.",
            "side": "strict sign of funding_rate[S]",
            "why_distinct": "Unlike HVFADR, HVCFDR, and HVCFIR, this policy uses an actual funding-level zero crossing rather than price/funding divergence or first-difference direction, and follows the new level regime without an onset rule.",
            ...
            "why_low_gross9_overlap_is_plausible": "actual funding-settlement zero crossings and causal funding-level ranks are absent from Gross9 primitives",
        },
        "features": {
            ...
            "funding_levels": "finite nonzero funding_rate[P] and funding_rate[S]",
            "zero_cross": "strict sign(funding_rate[S]) equals negative strict sign(funding_rate[P])",
            ...
            "eligible": "the exact eight-hour gap, both finite nonzero funding levels, exact opposite signs, and both ranks pass at S; no divergence, onset, first-difference-direction, OI, premium, alt, or post-settlement condition",
            "no_imputation": True,
        },
        "clock": {
        ...
        "stopping_rule": "terminal first failure; no settlement gap, zero-cross, history, level-rank, side, variation, clock, hold, subset, threshold, comparator, or control repair",
    }
    return {**core, "manifest_hash": canonical_hash(core)}

def validate(value: Mapping[str, Any]) -> None:
    if value != build():
        raise RuntimeError("HVCFZR-8 preregistration drift")
    contract = hvcav.build()
    for key in ("stages", "source_support_gates", "gross9_novelty_gates", "economic_gates"):
        if value[key] != contract[key]:
            raise RuntimeError(f"HVCFZR-8 {key} drift")
