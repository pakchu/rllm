"""Evaluate frozen HVALMA-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVALMA-24"
PROTOCOL = "hvalma_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_arnaud_legoux_moving_average_turn_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVALMA predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVALMA novelty limits drift")
    if (
        ...
        raise RuntimeError("HVALMA predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVALMA primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvalma_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVALMA-24 unchanged before economic outcomes",
    }
