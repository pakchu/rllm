"""Evaluate the frozen HVCORR-24 action-vote clock against Gross9."""
...
POLICY = "HVCORR-24"
PROTOCOL = "hvcorr_24_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_oi_range_regime_relay_clocks_2020_2026.csv.gz",
    "sha256": "20372bf662aa9ab99800b59562f04e8923e6dd7f1fa574e7a2122fcd85dfd38d",
    "rows": 70,
}
OUTPUT = Path("results/high_volatility_causal_oi_range_regime_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCORR-24 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCORR-24 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCORR-24 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCORR-24 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCORR-24 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCORR-24 unchanged before economics",
    }
