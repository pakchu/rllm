"""Evaluate frozen HVREI-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVREI-24"
PROTOCOL = "hvrei_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_range_expansion_index_reentry_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVREI predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVREI novelty limits drift")
    if (
        ...
        raise RuntimeError("HVREI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVREI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvrei_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVREI-24 unchanged before economic outcomes",
    }
