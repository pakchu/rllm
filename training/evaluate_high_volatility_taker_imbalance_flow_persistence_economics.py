"""Sequential strict economics for frozen HVTIFP-8."""
...
PREREG = Path("results/high_volatility_taker_imbalance_flow_persistence_preregistration_2026-08-13.json")
PREREG_SHA = "ce94bf435f3939edc867deea1bb7757756d716f57ff968e677b89b56d39dbc29"
SUPPORT = Path("results/high_volatility_taker_imbalance_flow_persistence_support_2026-08-13.json")
SUPPORT_SHA = "9942814f49d32d548c3933c2ea21946a33bca8ab327f7a0466fa31b7f54e50a2"
NOVELTY = Path("results/high_volatility_taker_imbalance_flow_persistence_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "0220610bdec1fe05b1eeb17727f060b2b619e5706857db13df54962ef9b812f4"
CLOCK = Path("data/high_volatility_taker_imbalance_flow_persistence_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d09e1bbd40238d3d75b3d487b08901fc8ea85a1990617ec607e75d8b338cab91"
CONTROL_DIR = Path("data/high_volatility_taker_imbalance_flow_persistence_controls_2023_2026")
FREEZE = Path("results/high_volatility_taker_imbalance_flow_persistence_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_taker_imbalance_flow_persistence_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_persistence_gate", "no_variation_gate", "imbalance_level_tail", "one_decision_stale_persistence", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTIFP frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTIFP novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvtifp_8_sequential_economics_v1", "policy_id": POLICY_ID,
