"""Build outcome-blind source support for frozen HVALPR-24."""
...
PREREG_SHA = "0564b539e243f3a26de28dd21af6a1665efea1edc3aefa250f47ff2021e38f6b"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_amihud_liquidity_premium_relay_sources_2023_2026")
PANEL = ROOT / "daily_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_amihud_liquidity_premium_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_amihud_liquidity_premium_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_amihud_liquidity_premium_relay_controls_2023_2026")
...
PANEL_COLUMNS = (
    ...
    "daily_amihud", "log_amihud", "reference_count", "reference_log_amihud_median",
    "innovation", "innovation_rank", "raw_illiquidity_rank", "realized_variation",
    "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_day", "feature_available_time",
    "decision_time", "entry_time", "exit_time", "side", "daily_amihud",
    "log_amihud", "reference_count", "reference_log_amihud_median", "innovation",
    "innovation_rank", "raw_illiquidity_rank", "realized_variation", "variation_rank",
    "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close", "quote_asset_volume"]:
        raise RuntimeError("HVALPR source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVALPR invalid source key")
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars = pd.DataFrame({
        ...
        "quote_turnover": groups.quote_asset_volume.sum(min_count=5),
    })
    ...
    daily = pd.DataFrame({
        ...
        "daily_amihud": daily_group.amihud_piece.mean(),
        "realized_variation": np.sqrt(daily_group["return"].apply(lambda values: float(np.square(values.to_numpy(float)).sum()))),
    })
    daily["source_valid"] = daily.valid_groups.eq(288) & np.isfinite(daily[["daily_amihud", "realized_variation"]]).all(axis=1) & daily.daily_amihud.gt(0) & daily.realized_variation.gt(0)
    daily["log_amihud"] = np.log(daily.daily_amihud).where(daily.source_valid)
    prior = daily.log_amihud.shift(1)
    daily["reference_log_amihud_median"] = prior.rolling(P["reference_days"], min_periods=P["reference_days"]).median()
    daily["reference_count"] = prior.notna().rolling(P["reference_days"], min_periods=P["reference_days"]).sum()
    daily["innovation"] = daily.log_amihud - daily.reference_log_amihud_median
    daily["source_valid"] &= daily.reference_count.eq(P["reference_days"]) & np.isfinite(daily[["reference_log_amihud_median", "innovation"]]).all(axis=1) & daily.innovation.ne(0)
    panel = daily.reset_index(names="source_day")
    panel["feature_available_time"] = panel.source_day + pd.Timedelta("1d")
    panel["innovation_rank"] = prior_rank(panel.innovation.where(panel.source_valid))
    panel["raw_illiquidity_rank"] = prior_rank(panel.log_amihud.where(panel.source_valid))
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    rank = pd.to_numeric(used.innovation_rank, errors="coerce")
    ...
    side = pd.Series(np.where(rank.ge(P["innovation_rank_long_min"]), 1, np.where(rank.le(P["innovation_rank_short_max"]), -1, 0)), index=used.index, dtype=int)
    state = valid & side.ne(0) & tail & variation
    if control == "no_liquidity_tail":
        side = np.sign(pd.to_numeric(used.innovation, errors="coerce")).fillna(0).astype(int)
        state = valid & side.ne(0) & variation
    elif control == "no_variation_gate":
        state = valid & side.ne(0) & tail
    elif control == "raw_illiquidity_rank":
        raw_rank = pd.to_numeric(used.raw_illiquidity_rank, errors="coerce")
        side = pd.Series(np.where(raw_rank.ge(P["innovation_rank_long_min"]), 1, np.where(raw_rank.le(P["innovation_rank_short_max"]), -1, 0)), index=used.index, dtype=int)
        state = valid & side.ne(0) & variation
    elif control == "one_day_stale_innovation":
        stale_valid = valid.shift(1, fill_value=False)
        side = side.shift(1, fill_value=0)
        state = stale_valid & side.ne(0) & tail.shift(1, fill_value=False) & variation.shift(1, fill_value=False)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVALPR prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvalpr_24_source_v1",
        "query": QUERY,
    ...
    core = {
        "protocol_version": "hvalpr_24_source_support_v1",
