"""Evaluate frozen HVRVTE-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVRVTE-8"
PROTOCOL = "hvrvte_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_return_volume_transfer_entropy_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVRVTE predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVRVTE novelty limits drift")
    if (
        ...
        raise RuntimeError("HVRVTE predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVRVTE primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVRVTE-8 unchanged before economic outcomes",
    }
