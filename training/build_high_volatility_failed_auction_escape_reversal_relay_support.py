"""Build outcome-blind source support for frozen HVFAER-8."""
...
from training import preregister_high_volatility_failed_auction_escape_reversal_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-04-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="476bc4f8b33e04e04cd8f8004c40f83e65d721c64bb63b31cabb66a54d880870"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_failed_auction_escape_reversal_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_failed_auction_escape_reversal_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_failed_auction_escape_reversal_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_failed_auction_escape_reversal_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_failed_auction_escape_reversal_relay_support_2026-08-11.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","prior_high","prior_low","prior_midpoint","escape_high","escape_low","escape_close","failed_escape_side","confirmation_open","confirmation_close","accepted_side","no_failed_escape_side","reclaim_close_side","stale_prior_range_side","realized_variation","variation_rank","eligible")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","prior_high","prior_low","prior_midpoint","escape_high","escape_low","escape_close","failed_escape_side","confirmation_open","confirmation_close","accepted_side","no_failed_escape_side","reclaim_close_side","stale_prior_range_side","realized_variation","variation_rank")
...
def failed_escape_pattern(escape:pd.DataFrame,confirmation:pd.DataFrame,prior_high:float,prior_low:float)->tuple[float,float,float,int,float,float,int,int,int]:
 escape_high=float(escape.high.max());escape_low=float(escape.low.min());escape_close=float(escape.close.iloc[-1]);midpoint=(prior_high+prior_low)/2
 upper=escape_high>prior_high and escape_low>prior_low and prior_low<escape_close<prior_high
 lower=escape_low<prior_low and escape_high<prior_high and prior_low<escape_close<prior_high
 failed_side=-1 if upper and not lower else 1 if lower and not upper else 0
 confirmation_open=float(confirmation.open.iloc[0]);confirmation_close=float(confirmation.close.iloc[-1])
 no_failed=1 if confirmation_close>midpoint and confirmation_close>confirmation_open else -1 if confirmation_close<midpoint and confirmation_close<confirmation_open else 0
 accepted=failed_side if failed_side!=0 and no_failed==failed_side else 0
 reclaim=failed_side
 return escape_high,escape_low,escape_close,failed_side,confirmation_open,confirmation_close,accepted,no_failed,reclaim
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVFAER source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVFAER invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("2h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   prior=block.iloc[:240];escape=block.iloc[240:360];confirmation=block.iloc[360:];prior_high=float(prior.high.max());prior_low=float(prior.low.min());prior_midpoint=(prior_high+prior_low)/2
   escape_high,escape_low,escape_close,failed_side,confirmation_open,confirmation_close,side,no_failed,reclaim=failed_escape_pattern(escape,confirmation,prior_high,prior_low)
   stale_side=0 if previous_prior is None else failed_escape_pattern(escape,confirmation,*previous_prior)[6]
  ...
  else:prior_high=prior_low=prior_midpoint=escape_high=escape_low=escape_close=confirmation_open=confirmation_close=var=math.nan;side=failed_side=no_failed=reclaim=stale_side=0;valid=False;previous_prior=None
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"prior_high":prior_high,"prior_low":prior_low,"prior_midpoint":prior_midpoint,"escape_high":escape_high,"escape_low":escape_low,"escape_close":escape_close,"failed_escape_side":failed_side,"confirmation_open":confirmation_open,"confirmation_close":confirmation_close,"accepted_side":side,"no_failed_escape_side":no_failed,"reclaim_close_side":reclaim,"stale_prior_range_side":stale_side,"realized_variation":var})
...
def active(panel:pd.DataFrame,control:str="primary"):
 ...
 if control=="no_failed_escape_requirement":side=pd.to_numeric(used.no_failed_escape_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
 elif control=="no_variation_gate":state=valid&side.ne(0)
 elif control=="reclaim_close_only":side=pd.to_numeric(used.reclaim_close_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVFAER prereg drift")
 ...
 source_core={"protocol_version":"hvfaer_8_source_v1","query":QUERY,"query_sha256":hashlib.sha256(QUERY.encode()).hexdigest(),"window":[START.isoformat(),END.isoformat()],"physical_rows":len(raw),"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"panel":{"path":str(PANEL),"sha256":sha(PANEL),"rows":len(panel),"valid_rows":int(panel.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};manifest={**source_core,"manifest_hash":chash(source_core)};immutable(MANIFEST,json_bytes(manifest))
