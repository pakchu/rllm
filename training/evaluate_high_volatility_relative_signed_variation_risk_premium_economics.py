"""Sequential strict economics for frozen HVRSVRP-12."""
...
PREREG = Path("results/high_volatility_relative_signed_variation_risk_premium_preregistration_2026-08-11.json")
PREREG_SHA = "1012e85acf505f50af4adbaa6d204909c7e7fe099a5b2df95244da6b7eca6205"
SUPPORT = Path("results/high_volatility_relative_signed_variation_risk_premium_support_2026-08-11.json")
SUPPORT_SHA = "0e1fb61e86c7f224cb86cf60898922679cd512cf39e70835df4c74963dd8ff56"
NOVELTY = Path("results/high_volatility_relative_signed_variation_risk_premium_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "6c3d67e8a026b8c103a0cbaa64591f6c79b61049e8ffec160780175b1da34f72"
CLOCK = Path("data/high_volatility_relative_signed_variation_risk_premium_clocks_2023_2026.csv.gz")
CLOCK_SHA = "4f5c25eb8ccaa473b2251f7655e55b9db2d5a06e57b16f82abe0c423521bd66d"
CONTROL_DIR = Path("data/high_volatility_relative_signed_variation_risk_premium_controls_2023_2026")
FREEZE = Path("results/high_volatility_relative_signed_variation_risk_premium_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_relative_signed_variation_risk_premium_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = (
    "no_signed_variation_tail",
    "no_variation_gate",
    "raw_day_return_reversal",
    "one_day_stale_state",
    "direction_flip",
    "forced_long",
)
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVRSVRP frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVRSVRP novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrsvrp_12_sequential_economics_v1", "policy_id": POLICY_ID,
