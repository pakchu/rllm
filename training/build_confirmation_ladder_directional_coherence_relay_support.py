"""Build source-only CLDCR-6 confirmation-ladder clocks."""
...
from training import preregister_confirmation_ladder_directional_coherence_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.download_fetls_block_summaries import OUTPUT as BLOCKS


ENV_FILE = "/home/pakchu/rllm/.env"
PREREG_SHA = "8f1399818a61fe5632966f1b137e3a0b2ebcae2e67b2a32d7b6f9890ec5e6dde"
...
CONTROLS = ("magnitude_only", "coherence_only", "one_anchor_stale_ladder", "direction_flip")
...
SOURCE_DIR = Path("data/confirmation_ladder_directional_coherence_relay_sources_2023_2026")
FEATURES = SOURCE_DIR / "confirmation_ladders.csv.gz"
MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/confirmation_ladder_directional_coherence_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/confirmation_ladder_directional_coherence_relay_controls_2023_2026")
RESULT = Path("results/confirmation_ladder_directional_coherence_relay_support_2026-08-09.json")
...
FEATURE_COLUMNS = (
    ...
    *RETURN_COLUMNS, "cumulative_return", "path_agree_count",
    "cumulative_abs_rank", "eligible_state",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "anchor_height", "confirmation_height",
    "feature_available_time", "entry_time", "exit_time", "side", "cumulative_return",
    "path_agree_count", "cumulative_abs_rank",
)
...
def validate_blocks(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CLDCR block schema drift")
    ...
    if not np.array_equal(np.diff(blocks.height), np.ones(len(blocks) - 1, dtype=np.int64)):
        raise RuntimeError("CLDCR block heights are not contiguous")
    if not blocks.previousblockhash.iloc[1:].reset_index(drop=True).equals(blocks.id.iloc[:-1].reset_index(drop=True)):
        raise RuntimeError("CLDCR canonical previous-hash continuity failed")
    return blocks
...
def prepare_minutes(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CLDCR minute schema drift")
    ...
    if result.ts.duplicated().any():
        raise RuntimeError("CLDCR duplicate timestamps")
...
def build_features(blocks: pd.DataFrame, market: pd.DataFrame) -> pd.DataFrame:
    ...
    for index in range(0, len(blocks) - 6):
        ...
        for offset in range(1, 7):
            values = interval_return(market, int(timestamp[index + offset - 1]), int(timestamp[index + offset]))
            if values is None:
                valid = False; break
            value, _ = values; returns.append(value)
        cumulative = float(np.sum(returns)) if valid else np.nan
        cumulative_sign = int(np.sign(cumulative)) if valid else 0
        path_count = int(sum(np.sign(value) == cumulative_sign for value in returns)) if valid and cumulative_sign else 0
        valid = bool(valid and np.isfinite(cumulative) and cumulative != 0)
        rows.append({
            ...
            **{column: returns[position] if valid else np.nan for position, column in enumerate(RETURN_COLUMNS)},
            "cumulative_return": cumulative if valid else np.nan, "path_agree_count": path_count if valid else 0,
        })
    output = pd.DataFrame(rows)
    output["cumulative_abs_rank"] = strict_prior_midrank(output.cumulative_return.abs().where(output.source_valid))
    output["eligible_state"] = output.source_valid & output.cumulative_abs_rank.ge(0.90) & output.path_agree_count.ge(5)
...
def active_and_side(features: pd.DataFrame, control: str = "primary"):
    ...
    primary = features.eligible_state.astype(bool); cumulative = features.cumulative_return.copy()
    if control == "magnitude_only":
        state = features.source_valid & features.cumulative_abs_rank.ge(0.90)
    elif control == "coherence_only":
        state = features.source_valid & features.path_agree_count.ge(5)
    elif control == "one_anchor_stale_ladder":
        state = primary.shift(1, fill_value=False); cumulative = cumulative.shift(1)
    else:
        state = primary
    ...
    if control == "direction_flip":
        side = -side
    return active & cumulative.ne(0), pd.Series(side, index=features.index).astype("Int64").fillna(0).astype(int)
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active & sides.ne(0)]:
        ...
        rows.append({
            ...
            "cumulative_return": float(features.at[index, "cumulative_return"]), "path_agree_count": int(features.at[index, "path_agree_count"]),
            "cumulative_abs_rank": float(features.at[index, "cumulative_abs_rank"]),
        })
...
def run() -> dict[str, Any]:
    ...
    source_core = {
        "protocol_version": "cldcr_6_source_v1", "query_sha256": hashlib.sha256(QUERY.encode()).hexdigest(), "window": [QUERY_START.isoformat(), END.isoformat()],
    ...
    core = {
        "protocol_version": "cldcr_6_source_support_v1", "policy_id": prereg.POLICY_ID,
