"""Build outcome-blind source support for frozen HVMOMER-R10-20-24."""
...
from training import preregister_high_volatility_momersion_regime_switch_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "fc222f0217a072dfcf0b978b025551c1e2ffeb2569656a9c728ec3be774f80a5"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_momersion_regime_switch_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_momersion_regime_switch_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_momersion_regime_switch_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_momersion_regime_switch_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_momersion_regime_switch_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "close_change", "change_product", "momersion",
    "momersion_ready", "entry_side", "raw_one_bar_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "close_change", "change_product", "momersion",
    "momersion_ready", "entry_side", "raw_one_bar_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVMOMER source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVMOMER invalid source key")
...
def momersion_regime_switch(close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    """Replicate LEAN Momersion(10,20) and freeze directional regime switches."""
    change=np.full(len(close),np.nan);product=np.full(len(close),np.nan);momersion=np.full(len(close),np.nan)
    ready=np.zeros(len(close),dtype=bool);entry=np.zeros(len(close),dtype=int);raw=np.zeros(len(close),dtype=int)
    closes:list[float]=[];products:list[float]=[];prior_momersion=50.0
    for index,is_valid in enumerate(valid.to_numpy(bool)):
        if not is_valid:
            closes=[];products=[];prior_momersion=50.0;continue
        current=float(close.iloc[index]);closes.append(current);closes=closes[-P["full_product_observations"]:]
        if len(closes)>=2:
            change[index]=closes[-1]-closes[-2];raw[index]=1 if change[index]>0 else -1 if change[index]<0 else 0
        if len(closes)>=3:
            product[index]=(closes[-1]-closes[-2])*(closes[-2]-closes[-3]);products.append(float(product[index]));products=products[-P["full_product_observations"]:]
        ready[index]=len(products)>=P["minimum_product_observations"]
        value=50.0
        if ready[index] and sum(x==0 for x in products)<0.5*len(products):
            positive=sum(x>0 for x in products);negative=sum(x<0 for x in products);value=100.0*positive/(positive+negative)
        momersion[index]=value
        if ready[index] and math.isfinite(change[index]) and change[index]!=0:
            change_sign=1 if change[index]>0 else -1
            if value>P["neutral_level"] and prior_momersion<=P["neutral_level"]:entry[index]=change_sign
            elif value<P["neutral_level"] and prior_momersion>=P["neutral_level"]:entry[index]=-change_sign
        prior_momersion=value
    return pd.DataFrame({"close_change":change,"change_product":product,"momersion":momersion,
                         "momersion_ready":ready,"entry_side":entry,"raw_one_bar_side":raw},index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=bars.join(momersion_regime_switch(bars.bar_close,bars.valid_bar));bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&bars.momersion_ready&np.isfinite(bars[["close_change","change_product","momersion","variation"]]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="one_bar_stale_switch":state=state.shift(1,fill_value=False);side=side.shift(1,fill_value=0)
    elif control=="direction_flip":side=-side
    elif control=="raw_one_bar_direction":side=pd.to_numeric(used.raw_one_bar_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVMOMER prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvmomer_r10_20_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvmomer_r10_20_24_source_support_v1", "policy_id": prereg.POLICY_ID,
