"""Sequential strict economics for frozen DRCMR-12."""
...
PREREG = Path("results/daily_range_close_momentum_relay_preregistration_2026-08-08.json")
PREREG_SHA = "f470307402b16e3a32aa8650ac3064e5da7c422f3768b056870485d4f4820f0d"
SUPPORT = Path("results/daily_range_close_momentum_relay_support_2026-08-08.json")
SUPPORT_SHA = "31b6a2c74d3b7ec37513dc2e783ba3e9db5cdfbc0d56986de97c4f9a39fb925d"
NOVELTY = Path("results/daily_range_close_momentum_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "1a2fae500b344c8d1bb896cf15af99dcc88d07e9c545e78cd297ffb8141baad2"
CLOCK = Path("data/daily_range_close_momentum_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "da215b7ed12695b5efa10d5b11b74dcd9f5162589b6984b7c82ed72030e19b50"
CONTROL_DIR = Path("data/daily_range_close_momentum_relay_controls_2023_2026")
FREEZE = Path("results/daily_range_close_momentum_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/daily_range_close_momentum_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
CONTROLS = ("no_range_gate", "no_close_location_gate", "one_day_stale_geometry", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("DRCMR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("DRCMR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "drcmr_12_sequential_economics_v1", "policy_id": POLICY_ID,
