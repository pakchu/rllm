"""Freeze first-print and first-revised COMPOUT values from decision-date ALFRED vintages."""
...
DEFAULT_OUTPUT = Path("data/commercial_paper_alfred_causal_vintages_2020_2026.csv")
DEFAULT_MANIFEST = Path("data/commercial_paper_alfred_causal_vintages_2020_2026_manifest.json")
FIRST_REFERENCE = date(2020, 1, 1)
LAST_REFERENCE = date(2026, 7, 22)
PREREG_SHA256 = "fd2b4cc7d48bda12489df9e8f6ed2350050129327bfc36e7734dc1335144fda9"
...
def wednesdays(start: date, end: date) -> list[date]:
    if start.weekday() != 2 or end.weekday() != 2:
        raise ValueError("reference endpoints must be Wednesdays")
    return [start + timedelta(days=7 * i) for i in range((end - start).days // 7 + 1)]


def bulk_url(references: list[date]) -> str:
    vintages = [v for reference in references for v in (reference + timedelta(days=1), reference + timedelta(days=8))]
    series = ",".join("COMPOUT" for _ in vintages)
...
def parse_bulk(payload: bytes, references: list[date]) -> list[dict[str, object]]:
    ...
    for reference in references:
        first_vintage = reference + timedelta(days=1)
        revised_vintage = reference + timedelta(days=8)
        first_column = f"COMPOUT_{first_vintage:%Y%m%d}"
        revised_column = f"COMPOUT_{revised_vintage:%Y%m%d}"
        ...
        rows.append(
            {
                ...
                "compout_first": float(first) if first not in {"", ".", None} else "",
                "compout_revised": float(revised) if revised not in {"", ".", None} else "",
            }
        )
    return rows
...
def write(rows: list[dict[str, object]], output: Path, manifest_path: Path, requests: list[dict[str, str]]) -> dict[str, object]:
    columns = ["reference_date", "first_vintage_date", "revised_vintage_date", "first_available", "revised_available", "compout_first", "compout_revised"]
    ...
    core = {
        "protocol_version": "commercial_paper_alfred_causal_vintages_v1",
        "series": "COMPOUT",
        "availability_rule": "Wednesday D first print from D+1 Thursday vintage and first revision from D+8 Thursday decision vintage",
...
def main() -> None:
    ...
    if file_sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA256:
        raise RuntimeError("HVCPFR preregistration hash drift")
    references = wednesdays(FIRST_REFERENCE, LAST_REFERENCE)
    rows: list[dict[str, object]] = []
