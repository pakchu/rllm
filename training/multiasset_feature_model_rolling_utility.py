...
TARGETS = ["utility1_excess", "utility2_excess", "utility3_excess"]
...
def _future_close_path_utility(prices: np.ndarray, positions: np.ndarray, horizon: int, penalty: float) -> np.ndarray:
    out = np.zeros((len(positions), prices.shape[1]), dtype=float)
    for j, pos in enumerate(positions):
        entry = prices[pos]
        exitp = prices[pos + horizon]
        raw = exitp / entry - 1.0
        path = prices[pos + 1 : pos + horizon + 1] / entry - 1.0
        adverse = np.minimum(0.0, np.nanmin(path, axis=0))
        out[j] = raw + penalty * adverse
    return out
...
def make_xy(close: pd.DataFrame, feats: dict[str, pd.DataFrame | pd.Series], feature_names: list[str], positions: np.ndarray, horizon: int, target_mode: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    if target_mode in {"raw", "excess"}:
        ymat = prices[positions + horizon] / prices[positions] - 1.0
        if target_mode == "excess":
            ymat = ymat - ymat.mean(axis=1, keepdims=True)
    elif target_mode.startswith("utility"):
        if target_mode.startswith("utility1"):
            penalty = 1.0
        elif target_mode.startswith("utility2"):
            penalty = 2.0
        elif target_mode.startswith("utility3"):
            penalty = 3.0
        else:
            raise ValueError(target_mode)
        ymat = _future_close_path_utility(prices, positions, horizon, penalty)
        if target_mode.endswith("_excess"):
            ymat = ymat - ymat.mean(axis=1, keepdims=True)
    else:
        raise ValueError(target_mode)
    y = ymat.reshape(-1)
...
def rolling_eval_period(close: pd.DataFrame, feats: dict[str, pd.DataFrame | pd.Series], feature_names: list[str], cfg: Cfg, params: dict[str, Any], train_days: int, start: str, end: str) -> dict[str, Any]:
    score_all = np.full(close.shape, np.nan, dtype=float)
    models: list[dict[str, Any]] = []
    idx = close.index
    period_end = pd.Timestamp(end)
    for mstart in month_starts(start, end):
        mend = min(mstart + pd.offsets.MonthBegin(1), period_end)
        ...
        score = predict_score(feats, feature_names, model, close) * float(params.get("score_sign", 1.0))
        mpos = np.flatnonzero((idx >= mstart) & (idx < mend) & (idx <= period_end))
    ...
    bt = backtest_scores(close, feats, score_all, params, cfg, start, end)
    return {"params": params, "train_days": train_days, "period_start": start, "period_end": end, "result": bt, "model_windows": models}

def run(cfg: Cfg) -> dict[str, Any]:
    close = load_close_matrix(cfg.data_dir)
    feats, feature_names = build_features(close, cfg.aux_dir)
    rows: list[dict[str, Any]] = []
    base = {"horizon": 288, "stride": 96, "l2": 100.0, "top_k": 2, "portfolio": "long_short", "regime_filter": "market_mom_pos"}
    for target in TARGETS:
        for score_sign in [1.0, -1.0]:
            for spread in [0.0, 0.001, 0.002, 0.004, 0.008]:
                params = dict(base)
                params["target"] = target
                params["score_sign"] = score_sign
                params["min_score_spread"] = spread
                val = rolling_eval_period(close, feats, feature_names, cfg, params, 365, cfg.val_start, cfg.val_end)
                sim = val["result"]["sim"]
                score = float(sim["cagr_to_strict_mdd"])
                if int(sim["rebalance_count"]) < cfg.min_val_rebalances:
                    score -= 1000.0
                if cfg.val_mdd_ceiling > 0 and float(sim["strict_mdd_pct"]) > cfg.val_mdd_ceiling:
                    score -= 1000.0 + (float(sim["strict_mdd_pct"]) - cfg.val_mdd_ceiling)
                if float(val["result"]["stats"]["rebalance_p_value_mean_ret_approx"]) > 0.10:
                    score -= 0.25
                rows.append({"score": score, "validation": val})
    rows.sort(key=lambda r: r["score"], reverse=True)
    selected = rows[0]
    params = selected["validation"]["params"]
    ev = rolling_eval_period(close, feats, feature_names, cfg, params, 365, cfg.eval_start, cfg.eval_end)
    report = {
        "config": cfg.__dict__,
        "target_family": "close-path utility = final return - penalty * adverse close excursion; cross-sectional excess target",
        "selection_protocol": "target and score-spread selected on 2024 rolling validation only; selected params then applied to 2025-2026 rolling eval",
        "feature_count": len(feature_names),
        "searched": len(rows),
        "top_validation": rows[: cfg.top_n],
        "selected": selected,
        "eval": ev,
        "leakage_guard": "monthly models fit only on data before each validation/eval month; target/spread selected on validation only; eval untouched",
        "accounting_note": "bar-level turnover-cost strict-MDD evaluator; adverse utility target uses future close path only for labels, never features",
    }
