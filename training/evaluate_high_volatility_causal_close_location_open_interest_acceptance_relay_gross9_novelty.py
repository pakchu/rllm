"""Evaluate the frozen HVCCLOIAR-8 action-vote clock against Gross9."""
...
POLICY = "HVCCLOIAR-8"
PROTOCOL = "hvccloiar_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_close_location_open_interest_acceptance_relay_clocks_2020_2026.csv.gz",
    "sha256": "c70fb821c89241be95a944fb5f3980486f158d753e425c148dea3002a6869d65",
    "rows": 378,
}
OUTPUT = Path("results/high_volatility_causal_close_location_open_interest_acceptance_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCCLOIAR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCCLOIAR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCCLOIAR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCCLOIAR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCCLOIAR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCCLOIAR-8 unchanged before economics",
    }
