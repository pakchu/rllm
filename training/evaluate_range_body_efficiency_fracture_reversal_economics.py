"""Sequential strict economics for frozen RBEFR-8."""
...
PREREG = Path("results/range_body_efficiency_fracture_reversal_preregistration_2026-08-09.json")
PREREG_SHA = "a3611f577e227a2aba9fb6affb13fabe906d42b0da10c2995c78a54c6e9f5e72"
SUPPORT = Path("results/range_body_efficiency_fracture_reversal_support_2026-08-09.json")
SUPPORT_SHA = "32276081f69150fbe39aebcd2f2a19dd045f219525d3859aa3c4850c1c34ff7f"
NOVELTY = Path("results/range_body_efficiency_fracture_reversal_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "5518e1c5f3900f207b112bb118dc4f90b7785df358d47eaffc4b39a0088bf522"
CLOCK = Path("data/range_body_efficiency_fracture_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d4cb7b639ac0f482e9acf311d95cdfd405ef28cf515004a32e828755f212126c"
CONTROL_DIR = Path("data/range_body_efficiency_fracture_reversal_controls_2023_2026")
FREEZE = Path("results/range_body_efficiency_fracture_reversal_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/range_body_efficiency_fracture_reversal_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("raw_range_variance_tail", "inverse_body_variance_tail", "one_decision_stale_fracture", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("RBEFR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("RBEFR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "rbefr_8_sequential_economics_v1", "policy_id": POLICY_ID,
