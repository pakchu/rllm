"""Sequential strict economics for frozen HVDEC-24."""
...
PREREG = Path("results/high_volatility_ehlers_decycler_oscillator_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "040146f1efc175daa31b63e38f9dd7c13b3dd7362b8febe6f86724ca8b485248"
SUPPORT = Path("results/high_volatility_ehlers_decycler_oscillator_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "66ea8cca379ccc240a79428fb7df2f083a0f0fdc9717a2264dcfef1c33fad921"
NOVELTY = Path("results/high_volatility_ehlers_decycler_oscillator_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "381e5c7039d8af68dffba795c10ae7cbc43b6a5e07c8e29b92b6e3c6c96fc9e6"
CLOCK = Path("data/high_volatility_ehlers_decycler_oscillator_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "6028f6d2eca46c2c430a8b884242e9bba7dc390e83dce1517c765b390dfc1766"
CONTROL_DIR = Path("data/high_volatility_ehlers_decycler_oscillator_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_ehlers_decycler_oscillator_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ehlers_decycler_oscillator_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "fast_100_decycler", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDEC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDEC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdec_24_sequential_economics_v1", "policy_id": POLICY_ID,
