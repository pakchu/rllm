"""Build source-only HVNSR-24 clocks before Gross9 or economics."""
...
PREREG_SHA="bae7fd67164c5cb876a20a0de290319443e6d211fc0b75d2f063412a28c0bd3e";SOURCE_SHA="f9b13dfa907d1cba3e411aac5564f773bae9c249e215c09e721137218e65d85d";DERIVED=Path("data/frbsf_daily_news_sentiment_1980_2026_aug.csv");DERIVED_SHA="4ed53d522d650028d6aaace39a5c800ab88295bcf5f170c70497c494ffcd809e";SOURCE_MANIFEST=Path("data/frbsf_daily_news_sentiment_1980_2026_aug_manifest.json");SOURCE_MANIFEST_SHA="255a7fa7df461616dc34f3b9ce660cb582b80f1479bf60642ced7e1011ea86d1";HELPER=Path("training/build_scheduled_trend_concordance_relay_support.py");HELPER_SHA="8ca554d88506df277434f73e5eb8850426614a880110088eb91aaae3b23c154f";END=pd.Timestamp("2026-08-01T00:00:00Z")
STATE=Path("data/high_volatility_news_sentiment_relay_sources_2023_2026/daily_states.csv.gz");CLOCK=Path("data/high_volatility_news_sentiment_relay_clocks_2023_2026.csv.gz");CONTROL_DIR=Path("data/high_volatility_news_sentiment_relay_controls_2023_2026");RESULT=Path("results/high_volatility_news_sentiment_relay_support_2026-08-09.json")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00:00Z"),pd.Timestamp("2024-01-01T00:00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00:00Z"),pd.Timestamp("2025-01-01T00:00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00:00Z"),pd.Timestamp("2026-01-01T00:00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00:00Z"),END)};MINIMUM={"train":8,"test":12,"eval":12,"final":8};CONTROLS=("no_sentiment_tail","no_btc_variation_gate","one_week_stale_sentiment","direction_flip","same_clock_forced_long")
COLUMNS=("candidate","control","split","source_day","decision_time","feature_available_time","entry_time","exit_time","side","sentiment_change","sentiment_shock_rank","btc_variation","btc_variation_rank")
...
def score_states(sentiment:pd.DataFrame,market:pd.DataFrame)->pd.DataFrame:
 e=sentiment.copy();e["source_day"]=pd.to_datetime(e.date,utc=True);e["news_sentiment"]=pd.to_numeric(e.news_sentiment,errors="coerce");e=e.sort_values("source_day").reset_index(drop=True)
 if e.source_day.duplicated().any():raise RuntimeError("HVNSR duplicate source day")
 lookup=e.set_index("source_day").news_sentiment;e["sentiment_lag7"]=lookup.reindex(e.source_day-pd.Timedelta(days=7)).to_numpy();e["sentiment_change_valid"]=np.isfinite(e[["news_sentiment","sentiment_lag7"]]).all(axis=1);e["sentiment_change"]=(e.news_sentiment-e.sentiment_lag7).where(e.sentiment_change_valid);e["sentiment_shock_rank"]=rank(e.sentiment_change.abs().where(e.sentiment_change_valid));e["decision_time"]=e.source_day+pd.Timedelta(days=8)
 m=market.copy();m["date"]=pd.to_datetime(m.date,utc=True);m=m.sort_values("date").set_index("date");close=pd.to_numeric(m.close,errors="coerce");valid=np.isfinite(close)&close.gt(0);step=m.index.to_series().diff().eq(pd.Timedelta(minutes=5));variation=np.sqrt(np.log(close/close.shift(1)).pow(2).rolling(288,min_periods=288).sum());ok=valid.rolling(289,min_periods=289).sum().eq(289)&step.rolling(288,min_periods=288).sum().eq(288);lookup=pd.DataFrame({"decision_time":m.index+pd.Timedelta(minutes=5),"btc_variation":variation.where(ok).to_numpy()});e=e.merge(lookup,on="decision_time",how="left",validate="one_to_one");e["state_valid"]=e.sentiment_change_valid&np.isfinite(e[["sentiment_change","sentiment_shock_rank","btc_variation"]]).all(axis=1)&e.sentiment_change.ne(0);e["btc_variation_rank"]=rank(e.btc_variation.where(e.state_valid));return e
