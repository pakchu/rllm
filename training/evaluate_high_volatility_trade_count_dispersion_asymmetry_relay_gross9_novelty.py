"""Evaluate frozen HVTCDAR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVTCDAR-8"
PROTOCOL = "hvtcdar_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_trade_count_dispersion_asymmetry_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTCDAR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVTCDAR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTCDAR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTCDAR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvtcdar_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVTCDAR-8 unchanged before economic outcomes",
    }
