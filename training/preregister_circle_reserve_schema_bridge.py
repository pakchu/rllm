"""Produce the frozen, source-unseen CRSB-336 preregistration artifact."""
...
POLICY_ID = "CRSB-336"
PROTOCOL_VERSION = "circle_reserve_schema_bridge_preregistration_v1"
EXPECTED_BRANCH = "codex/circle-reserve-schema-bridge-20260730"
...
SOURCE_DECISION_COMMIT = "48672fd58e1da01146a0f31e62bdd6f3736f3944"
SOURCE_DECISION_GIT_BLOB = "9286274301ae021cd97565e4164b8b8fd3d46563"
MECHANISM_DECISION_PATH = Path(
    "docs/circle-reserve-schema-bridge-mechanism-decision-2026-07-30.md"
)
MECHANISM_DECISION_SHA256 = (
    "1fd13415935bfaa67d22e3b2ae6fe2b17c1eb6e54ab3b054ceb8dcd79a86490d"
)
MECHANISM_DECISION_COMMIT = "f05c58df216f34c14ba7600e1e3699abb2581a89"
MECHANISM_DECISION_GIT_BLOB = "1f513e229946ea111c04cc11d5758e2df00ce993"
PRODUCER_PATH = Path(
    "training/preregister_circle_reserve_schema_bridge.py"
)
TEST_PATH = Path(
    "tests/test_preregister_circle_reserve_schema_bridge.py"
)
DEFAULT_OUTPUT = Path(
    "results/circle_reserve_schema_bridge_"
...
FUTURE_PROTOCOL_PATHS = (
    Path("training/build_circle_reserve_schema_bridge_source.py"),
    Path("tests/test_build_circle_reserve_schema_bridge_source.py"),
    Path(
        "training/evaluate_circle_reserve_schema_bridge_"
        "source_support.py"
    ),
    Path(
        "tests/test_evaluate_circle_reserve_schema_bridge_"
        "source_support.py"
    ),
    Path(
        "training/evaluate_circle_reserve_schema_bridge_novelty.py"
    ),
    Path(
        "tests/test_evaluate_circle_reserve_schema_bridge_novelty.py"
    ),
    Path(
        "training/evaluate_circle_reserve_schema_bridge_economics.py"
    ),
    Path(
        "tests/test_evaluate_circle_reserve_schema_bridge_economics.py"
    ),
)

SOURCE_IDENTITY = {
    "daily_index_company_name": "BlackRock Funds",
    "registrant_cik": "0000844779",
    "registrant_numeric_cik": 844779,
...
SCHEMA_PATH_KINDS = ("nmfp2_friday_slots", "nmfp3_dated_details")
...
def _safe_path(path: str | Path) -> Path:
    ...
    if (
        ...
        raise RuntimeError("CRSB-336 repository path is unsafe")
    return candidate
...
def _read_regular(path: str | Path) -> bytes:
    ...
    try:
        if not stat.S_ISREG(os.fstat(descriptor).st_mode):
            raise RuntimeError("CRSB-336 dependency is not a regular file")
        chunks: list[bytes] = []
...
def _git(*arguments: str) -> bytes:
    try:
        ...
        raise RuntimeError("CRSB-336 Git identity validation failed") from error


def _git_blob(raw: bytes, object_id: str) -> str:
    if not _HEX40_RE.fullmatch(object_id):
        raise RuntimeError("CRSB-336 requires SHA-1 Git object format")
    digest = hashlib.sha1()
...
def _canonical_json(raw: bytes, label: str) -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(f"CRSB-336 {label} is not canonical JSON") from error
    if not isinstance(value, dict):
        raise RuntimeError(f"CRSB-336 {label} root must be an object")
    canonical = (
        json.dumps(
            value,
            sort_keys=True,
    ...
    if raw != canonical:
        raise RuntimeError(f"CRSB-336 {label} bytes are not canonical")
    return value
        ...
        if set(("path", "sha256")).issubset(value):
            ...
            if isinstance(path, str) and isinstance(digest, str):
                ...
                if not _HEX64_RE.fullmatch(digest):
                    raise RuntimeError("CRSB-336 authority SHA-256 is invalid")
                previous = found.get(candidate)
                if previous is not None and previous != digest:
                    raise RuntimeError("CRSB-336 authority path hash conflicts")
...
def load_esdi_authority() -> dict[str, Any]:
    ...
    if not isinstance(comparators, Mapping) or len(comparators) != ESDI_COMPARATOR_COUNT:
        raise RuntimeError("CRSB-336 comparator registry count drift")
    if canonical_hash(comparators) != ESDI_COMPARATOR_SUBTREE_SHA256:
        raise RuntimeError("CRSB-336 comparator registry hash drift")
    if canonical_hash(gross9) != ESDI_GROSS9_SUBTREE_SHA256:
        raise RuntimeError("CRSB-336 Gross9 subtree hash drift")
    authority = gross9.get("authority")
    if not isinstance(authority, Mapping):
        raise RuntimeError("CRSB-336 Gross9 authority is missing")
    if canonical_hash(authority) != ESDI_GROSS9_AUTHORITY_SHA256:
        raise RuntimeError("CRSB-336 Gross9 authority hash drift")
    closure = authority.get("runtime_code_closure")
    if not isinstance(closure, Mapping):
        raise RuntimeError("CRSB-336 runtime closure is missing")
    if canonical_hash(closure) != ESDI_RUNTIME_CLOSURE_SHA256:
        raise RuntimeError("CRSB-336 runtime closure hash drift")
    return {
...
def esdi_bound_path_hashes() -> dict[Path, str]:
    ...
    if not isinstance(paths, list) or not all(isinstance(item, str) for item in paths):
        raise RuntimeError("CRSB-336 runtime closure paths are invalid")
    ...
    if isinstance(closure_sha, Mapping):
        for raw_path in paths:
            ...
            if not isinstance(digest, str) or not _HEX64_RE.fullmatch(digest):
                raise RuntimeError("CRSB-336 closure path hash is missing")
        ...
        if not isinstance(sha, Mapping):
            raise RuntimeError("CRSB-336 ESDI repository SHA map is missing")
        for raw_path in paths:
            digest = sha.get(raw_path)
            if not isinstance(digest, str) or not _HEX64_RE.fullmatch(digest):
                raise RuntimeError("CRSB-336 closure path SHA is missing")
            found[_safe_path(raw_path)] = digest
...
def validate_frozen_decision_commits() -> None:
    ...
    for commit, path, blob, label in decision_authorities:
        ...
        if observed != {str(path): blob}:
            raise RuntimeError(f"CRSB-336 frozen {label} commit blob drift")
...
def validate_frozen_dependencies() -> None:
    ...
    for path, digest in expected.items():
        ...
        if actual != digest:
            raise RuntimeError(
                f"CRSB-336 frozen dependency changed: {path}: {actual} != {digest}"
            )
    authority_paths = {
    ...
    if authority_blobs != authority_paths:
        raise RuntimeError("CRSB-336 ESDI authority commit blob drift")
...
def _parse_tree(raw: bytes, expected: set[str]) -> dict[str, str]:
    ...
    for record in raw.split(b"\0"):
        ...
        try:
            ...
            raise RuntimeError("CRSB-336 Git tree record is malformed") from error
        if mode != "100644" or object_type != "blob" or not _HEX40_RE.fullmatch(object_id):
            raise RuntimeError("CRSB-336 identity path is not a plain SHA-1 blob")
        blobs[path] = object_id
    if set(blobs) != expected:
        raise RuntimeError("CRSB-336 committed identity paths are incomplete")
    return blobs
...
def validate_repository_identity(identity: Mapping[str, Any]) -> None:
    ...
    if set(identity) != expected_keys:
        raise RuntimeError("CRSB-336 repository identity schema drift")
    ...
    if (
        ...
        raise RuntimeError("CRSB-336 repository identity values drift")
    if any(
    ...
    if any(blobs.get(path) != expected for path, expected in fixed_blobs.items()):
        raise RuntimeError("CRSB-336 fixed Git blob authority drift")
    fixed_sha256 = {
    ...
    if any(sha.get(path) != expected for path, expected in fixed_sha256.items()):
        raise RuntimeError("CRSB-336 fixed SHA-256 authority drift")
...
def frozen_repository_identity() -> dict[str, Any]:
    ...
    if len(revision) != 4:
        raise RuntimeError("CRSB-336 Git revision evidence is incomplete")
    root, head, tree, upstream_commit = revision
    if Path(root).resolve() != REPOSITORY_ROOT.resolve() or head != upstream_commit:
        raise RuntimeError("CRSB-336 requires pushed exact HEAD")
    status = _git(
        "status",
        "--porcelain=v2",
        "--branch",
    ...
    if not _branch_status_is_clean(status, head):
        raise RuntimeError("CRSB-336 creation requires a clean pushed branch")
    ...
    for path, blob in blobs.items():
        ...
        if _git_blob(raw, blob) != blob:
            raise RuntimeError("CRSB-336 worktree differs from committed blob")
...
def validate_recorded_repository(identity: Mapping[str, Any]) -> None:
    ...
    if records != identity["git_blobs"]:
        raise RuntimeError("CRSB-336 recorded Git blobs are unavailable")
    for path, expected in identity["sha256"].items():
        raw = _git("show", f"{head}:{path}")
        if hashlib.sha256(raw).hexdigest() != expected:
            raise RuntimeError("CRSB-336 recorded path SHA is unavailable")
        if _git_blob(raw, records[path]) != records[path]:
            raise RuntimeError("CRSB-336 recorded path blob is invalid")


def parse_exact_decimal(value: str) -> Fraction:
    if not isinstance(value, str) or not _DECIMAL_RE.fullmatch(value):
        raise ValueError("CRSB-336 decimal must be canonical plain nonnegative")
    try:
        decimal = Decimal(value)
    except InvalidOperation as error:
        raise ValueError("CRSB-336 decimal is invalid") from error
    if not decimal.is_finite() or decimal < 0:
        raise ValueError("CRSB-336 decimal is invalid")
    return Fraction(decimal)


def path_vote(values: Sequence[str | Fraction]) -> int:
    if not values:
        raise ValueError("CRSB-336 path cannot be empty")
    parsed = tuple(
    ...
    if any(not isinstance(value, Fraction) or value < 0 for value in parsed):
        raise ValueError("CRSB-336 path values must be exact nonnegative Fractions")
...
def maturity_vote(current: int, previous: int) -> int:
    if type(current) is not int or type(previous) is not int or min(current, previous) < 0:
        raise ValueError("CRSB-336 maturity values must be nonnegative integers")
...
def primary_vote(component_votes: Sequence[int]) -> int:
    ...
    if len(values) != 4 or any(type(value) is not int or value not in {-1, 0, 1} for value in values):
        raise ValueError("CRSB-336 primary requires four exact component votes")
    total = sum(values)
...
def canonical_utc_timestamp(value: str | datetime) -> str:
    if isinstance(value, str):
        if not _TIMESTAMP_RE.fullmatch(value):
            raise ValueError("CRSB-336 timestamp grammar is invalid")
        try:
            parsed = datetime.strptime(value, "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=timezone.utc)
        except ValueError as error:
            raise ValueError("CRSB-336 timestamp is invalid") from error
        if parsed.strftime("%Y-%m-%dT%H:%M:%SZ") != value:
            raise ValueError("CRSB-336 timestamp is noncanonical")
        return value
    if not isinstance(value, datetime) or value.tzinfo is None or value.microsecond:
        raise ValueError("CRSB-336 datetime must be aware whole-second")
    return value.astimezone(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")


def _canonical_signed(value: int, *, nonzero: bool = True) -> str:
    if type(value) is not int or (nonzero and value == 0):
        raise ValueError("CRSB-336 identity integer is invalid")
    return str(value)
...
def primary_signal_id(accession: str, source_available_at: str, vote_sum: int) -> str:
    if not isinstance(accession, str) or not _ACCESSION_RE.fullmatch(accession):
        raise ValueError("CRSB-336 accession grammar is invalid")
    if type(vote_sum) is not int or vote_sum not in {
        ...
        raise ValueError("CRSB-336 primary vote sum is outside the frozen domain")
...
def source_control_signal_id(
    ...
    if (
        ...
        raise ValueError("CRSB-336 source-control vote is invalid")
    if not isinstance(accession, str) or not _ACCESSION_RE.fullmatch(accession):
        raise ValueError("CRSB-336 accession grammar is invalid")
    timestamp = canonical_utc_timestamp(source_available_at)
    raw = (
...
def deterministic_random_side(primary_id: str) -> int:
    if not isinstance(primary_id, str) or not _HEX64_RE.fullmatch(primary_id):
        raise ValueError("CRSB-336 primary signal ID is invalid")
    raw = f"{POLICY_ID}|{primary_id}|RANDOM_SIDE".encode("utf-8")
    return 1 if hashlib.sha256(raw).digest()[0] < 128 else -1


def same_parent_control_fields(
    control: str,
    primary_id: str,
    primary_entry_time: str,
    primary_exit_time: str,
    primary_side: int,
) -> tuple[str, str, int]:
    if control not in SAME_PARENT_CONTROL_NAMES:
        raise ValueError("CRSB-336 same-parent control name is invalid")
    if not isinstance(primary_id, str) or not _HEX64_RE.fullmatch(primary_id):
        raise ValueError("CRSB-336 primary signal ID is invalid")
    if type(primary_side) is not int or primary_side not in {-1, 1}:
        raise ValueError("CRSB-336 primary side is invalid")
    primary_entry = canonical_utc_timestamp(primary_entry_time)
    primary_exit = canonical_utc_timestamp(primary_exit_time)
    entry_dt = datetime.strptime(
        primary_entry, "%Y-%m-%dT%H:%M:%SZ"
    ).replace(tzinfo=timezone.utc)
    exit_dt = datetime.strptime(primary_exit, "%Y-%m-%dT%H:%M:%SZ").replace(
        tzinfo=timezone.utc
    )
    if exit_dt - entry_dt != timedelta(hours=336):
        raise ValueError("CRSB-336 primary interval must span exactly 336 hours")

    control_entry = entry_dt
    control_exit = exit_dt
    if control == "exact_direction_flip":
        control_side = -primary_side
    elif control == "deterministic_random_side":
        control_side = deterministic_random_side(primary_id)
    elif control == "constant_long":
        control_side = 1
    elif control == "constant_short":
        control_side = -1
    else:
        control_entry += timedelta(minutes=5)
        control_exit += timedelta(minutes=5)
        control_side = primary_side
    return (
        canonical_utc_timestamp(control_entry),
        canonical_utc_timestamp(control_exit),
        control_side,
    )
...
def same_parent_control_signal_id(
    ...
    entry, exit_, side = same_parent_control_fields(
        control,
        primary_id,
        primary_entry_time,
        primary_exit_time,
...
def authenticated_gross9_weights() -> dict[str, float]:
    ...
    if not isinstance(weights, Mapping) or dict(weights) != GROSS9_WEIGHTS:
        raise RuntimeError("CRSB-336 Gross9 weights differ from ESDI authority")
    ...
    if (
        ...
        raise RuntimeError("CRSB-336 Gross9 configured gross authority drift")
...
def same_gross_weights(candidate_weight: float) -> dict[str, float]:
    if isinstance(candidate_weight, bool) or not isinstance(
        ...
        raise ValueError("CRSB-336 candidate weight must be a frozen numeric value")
    weight = float(candidate_weight)
    if weight not in CANDIDATE_WEIGHTS:
        raise ValueError("CRSB-336 candidate weight is outside the frozen grid")
    scale = (9.0 - weight) / 9.0
    treatment = {
        name: value * scale
        for name, value in authenticated_gross9_weights().items()
    }
    treatment["crsb"] = weight
    if not math.isclose(sum(treatment.values()), 9.0, rel_tol=0.0, abs_tol=1e-12):
        raise RuntimeError("CRSB-336 same-gross arithmetic drift")
    return treatment
...
def rank_same_gross_rows(rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for raw in rows:
        ...
        if isinstance(value, bool) or not isinstance(value, (int, float)):
            raise ValueError("CRSB-336 ranking weight type is invalid")
        observed.append(float(value))
    if sorted(observed) != list(CANDIDATE_WEIGHTS) or len(set(observed)) != 4:
        raise ValueError("CRSB-336 ranking requires the exact four-weight grid")
    normalized: list[dict[str, Any]] = []
    for raw, weight in zip(rows, observed):
        raw_score = raw.get("minimum_improvement")
        if isinstance(raw_score, bool) or not isinstance(raw_score, (int, float)):
            raise ValueError("CRSB-336 ranking score type is invalid")
        score = float(raw_score)
        if not math.isfinite(score) or type(raw.get("passes")) is not bool:
            raise ValueError("CRSB-336 ranking row is invalid")
        row = dict(raw)
    ...
    if not ranked[0]["passes"]:
        raise RuntimeError("CRSB-336 raw rank one failed; no substitution")
    return ranked
...
def downstream_artifact_contract() -> dict[str, Any]:
    return {
        "paths": {
            ...
            "source_access_claim": (
                "results/circle_reserve_schema_bridge_source_access_"
                "claim_2026-07-30.json"
            ),
            "source_csv": (
                "data/circle_reserve_schema_bridge_source_2026-07-30.csv.gz"
            ),
            "source_manifest": (
                "results/circle_reserve_schema_bridge_source_manifest_"
                "2026-07-30.json"
            ),
            "sealed_source_root": (
            ...
            "primary_clock": (
                "results/circle_reserve_schema_bridge_primary_"
                "clock_2026-07-30.csv.gz"
            ),
            "control_clocks": (
                "results/circle_reserve_schema_bridge_control_"
                "clocks_2026-07-30.csv.gz"
            ),
            "source_support": (
                "results/circle_reserve_schema_bridge_source_"
                "support_2026-07-30.json"
            ),
            "novelty_claim": (
                "results/circle_reserve_schema_bridge_novelty_"
                "claim_2026-07-30.json"
            ),
            "novelty": (
                "results/circle_reserve_schema_bridge_novelty_"
            ...
            "economics_directory": (
                "results/circle_reserve_schema_bridge_economics_"
                "2026-07-30"
            ),
        },
        "json": {
        ...
        "gzip_csv": {
            ...
            "writer": "manual RFC1952",
            "header_hex": "1f8b08000000000002ff",
            "raw_deflate": {
                "level": 9,
                "wbits": -15,
                "memLevel": 8,
            ...
            "trailer": "CRC32 then ISIZE little-endian",
            "required_zlib_runtime": "1.3",
            "empty_golden_hex": "1f8b08000000000002ff03000000000000000000",
        },
    ...
    return {
        ...
        "source": {
            ...
            "transport": {
                ...
                "attempts_per_url": 1,
                "user_agent_environment_variable": "CRSB_SEC_USER_AGENT",
                "redirects": 0,
            ...
            "first_dissemination": {
                ...
                "daily_index_exact_company_name": "BlackRock Funds",
            ...
            "xml": {
                "validation_boundary": (
                    "exact selected-field parser; pinned XSDs are QName and "
                    "cardinality metadata, not general validating-XSD execution"
                ),
                "exact_root_namespace_events": {
                    "default": "form-family main namespace",
                    "ns3": "form-family common namespace",
                    "xsi": "http://www.w3.org/2001/XMLSchema-instance",
                },
                "n_mfp2": {
                    "main_namespace": "http://www.sec.gov/edgar/nmfp2",
                    "common_namespace": "http://www.sec.gov/edgar/nmfp2common",
                    "identity_and_form": [
                        "submissionType",
                        "reportDate",
                        "cik",
                    ...
                    "paired_percentage_slots": "friday1..4 required; friday5 paired optional",
                    "path_length": [4, 5],
                    "dollar_container_text_access": False,
                    "schema_path_kind": SCHEMA_PATH_KINDS[0],
                },
                "n_mfp3": {
                    "main_namespace": "http://www.sec.gov/edgar/nmfp3",
                    "common_namespace": "http://www.sec.gov/edgar/nmfp3common",
                    "identity_and_form": [
                        "submissionType",
                        "reportDate",
                        "registrantFullName",
                    ...
                    "detail_container": "liquidAssetsDetails in main namespace",
                    "detail_children": [
                    ...
                    "detail_count": [15, 31],
                    "dollar_child_text_access": False,
                    "schema_path_kind": SCHEMA_PATH_KINDS[1],
                },
                "retained": list(SOURCE_CSV_COLUMNS),
                "liquidity_path_json": {
                    "object_key_order": [
                        "position",
                        "source_label",
                    ...
                    "n_mfp2_labels": "friday1 through friday4|5",
                    "n_mfp3_labels": "exact source YYYY-MM-DD",
                    "source_order_preserved": True,
                    "inferred_n_mfp2_dates": False,
                },
                "forbidden_values": [
                ...
                "selected_or_retained_text_ampersand": "terminal",
                "CDATA_comments_noninitial_PI": "terminal",
                "DTD_entity_XInclude_XSLT_network_resolution": "terminal",
            },
            "validation": {
                "n_mfp2_category": (
                    "nonempty duplicate-free source-ordered subset of Treasury, "
                    "Government/Agency, Exempt Government"
                ),
                "n_mfp3_category": "Government",
                "n_mfp3_government_flag": "Y",
                "wam_integer_range": [0, 60],
                "wal_integer_range": [0, 120],
                "wam_at_most_wal": True,
                "percentage_lexical_grammar": (
                    "^(?:0|[1-9][0-9]*)(?:[.][0-9]+)?$"
                ),
                "percentage_range": [0, 100],
                "daily_percentage_at_most_weekly": True,
                "n_mfp3_first_detail_day_at_most": 4,
                "n_mfp3_maximum_adjacent_day_gap": 4,
                "n_mfp3_last_detail_zero_to_three_days_before_report": True,
                "n_mfp3_source_dates_already_strictly_increasing_unique": True,
            ...
            "source_csv_columns": list(SOURCE_CSV_COLUMNS),
        },
        "feature_and_signal": {
            "warmup": (
                "report months 2022-11 and 2022-12 cannot emit; source state "
                "advances and 2023-01 maturity compares with 2022-12"
            ),
        ...
        "support_gates": {
            ...
            "schema_family_support": {
                "partition": "schema_path_kind over vote_diversity_population",
                "required_path_kinds": list(SCHEMA_PATH_KINDS),
        ...
        "controls": {
            ...
            "source_derived": {
                ...
                "identity": (
                    "CRSB-336|control|<name>|<accession>|"
                    "<YYYY-MM-DDTHH:MM:SSZ>|<raw_nonzero_control_vote_sum>"
                ),
            },
            "same_parent": {
                "names": list(SAME_PARENT_CONTROL_NAMES),
                "fields_are_derived_not_caller_supplied": True,
                "accepted_primary_interval_hours": 336,
                "preserve_primary_interval": [
                    "exact_direction_flip",
                    "deterministic_random_side",
                    "constant_long",
                    "constant_short",
                ],
                "exact_direction_flip_side": "opposite primary",
                "constant_sides": {"constant_long": 1, "constant_short": -1},
                "random_digest": (
                    "SHA256(raw UTF-8 CRSB-336|<primary_id>|RANDOM_SIDE)."
                ...
                "identity": (
                    "CRSB-336|control|<name>|<primary_id>|<entry>|<exit>|"
                    "<LONG_or_SHORT>"
                ),
            },
        ...
        "gross9": {
            ...
            "treatment": "scale every Gross9 sleeve by (9-c)/9 and add CRSB at c",
...
def _matches_type(value: Any, expected: str) -> bool:
    ...
    raise RuntimeError("CRSB-336 unknown schema type")
...
def validate_manifest(payload: Mapping[str, Any]) -> None:
    if set(payload) != set(TOP_LEVEL_TYPES) or any(
        ...
        raise RuntimeError("CRSB-336 preregistration top-level schema drift")
    identity = payload["frozen_preregistration"].get("repository_identity")
    if not isinstance(identity, Mapping):
        raise RuntimeError("CRSB-336 repository identity is missing")
    expected = build_manifest(identity)
    if dict(payload) != expected:
        raise RuntimeError("CRSB-336 preregistration differs from frozen code")
    core = {key: value for key, value in payload.items() if key != "manifest_hash"}
    if payload["manifest_hash"] != canonical_hash(core):
        raise RuntimeError("CRSB-336 preregistration manifest hash drift")
    if any(payload[name] is not False for name in EVIDENCE_BOUNDARIES):
        raise RuntimeError("CRSB-336 preregistration evidence boundary opened")
    if REPORT_MONTHS[0] != "2022-11" or REPORT_MONTHS[-1] != "2026-04" or len(REPORT_MONTHS) != 42:
        raise RuntimeError("CRSB-336 report-month generation drift")
    if DISCOVERY_MONTHS[0] != "2022-12" or DISCOVERY_MONTHS[-1] != "2026-05" or len(DISCOVERY_MONTHS) != 42:
        raise RuntimeError("CRSB-336 discovery-month generation drift")
    if sum(authenticated_gross9_weights().values()) != 9.0:
        raise RuntimeError("CRSB-336 Gross9 baseline drift")
    if payload["gross9"].get("baseline_weights") != GROSS9_WEIGHTS:
        raise RuntimeError("CRSB-336 manifest Gross9 weights drift")
...
def _output_path(path: str | Path) -> Path:
    ...
    if candidate != DEFAULT_OUTPUT:
        raise RuntimeError("CRSB-336 output must equal the frozen singleton path")
    return candidate
...
def _validate_creation_publish_state(identity: Mapping[str, Any], temporary: Path) -> None:
    ...
    if not _branch_status_is_clean(status, str(identity["head_commit"]), temporary):
        raise RuntimeError("CRSB-336 worktree changed before artifact publish")
...
def write_once(
    ...
    if full.exists() or full.is_symlink():
        ...
        if raw != canonical_manifest_bytes(stored):
            raise RuntimeError("CRSB-336 existing preregistration bytes drift")
        if payload is not None and dict(payload) != stored:
            raise RuntimeError("CRSB-336 supplied preregistration payload drift")
    ...
    try:
        ...
        try:
            ...
            if _read_regular(candidate) != canonical:
                raise RuntimeError("CRSB-336 preregistration race drift")
