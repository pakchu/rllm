...
SUPPORT_SHA = "c30b20c8267cc446457a38f4bfafc3681c2c60bed7c5c30a535d31b51ce6a172"
CLOCK = Path("data/high_volatility_cross_alt_return_turnover_concordance_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "678779d981467ca42332ffa527c7e640b6d75f941c5a7002ebaa1f4b39605185"
OUTPUT = Path("results/high_volatility_cross_alt_return_turnover_concordance_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCARTC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCARTC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCARTC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCARTC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCARTC-24 unchanged before economic outcomes",
    }
