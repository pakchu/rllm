"""Sequential strict economics for frozen HVREI-24."""
...
PREREG = Path("results/high_volatility_range_expansion_index_reentry_relay_preregistration_2026-08-11.json")
PREREG_SHA = "d922f1b7011d44f24afa509b4e39eb60e75e2810d43c5d5318e228ea6a910106"
SUPPORT = Path("results/high_volatility_range_expansion_index_reentry_relay_support_2026-08-11.json")
SUPPORT_SHA = "0b4042e8c0aeb87480e2b0f9c7abde1302d1e9d645dcc0380462783182cdde1c"
NOVELTY = Path("results/high_volatility_range_expansion_index_reentry_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "c8c65ea1989e9f8eafb0500830bf63a8807e5e6757dfd820fdc344fc46ebb04d"
CLOCK = Path("data/high_volatility_range_expansion_index_reentry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "85b18f562053a93ab94ee17c186f8901dad7573d70b2e29d6220e1c4603e32bc"
CONTROL_DIR = Path("data/high_volatility_range_expansion_index_reentry_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_range_expansion_index_reentry_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_range_expansion_index_reentry_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "unfiltered_rei", "one_bar_stale_reentry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVREI frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVREI novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrei_24_sequential_economics_v1", "policy_id": POLICY_ID,
