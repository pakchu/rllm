"""Evaluate frozen HVDTCAR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDTCAR-8"
PROTOCOL = "hvdtcar_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_directional_turnover_concentration_asymmetry_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDTCAR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVDTCAR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDTCAR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDTCAR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdtcar_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVDTCAR-8 unchanged before economic outcomes",
    }
