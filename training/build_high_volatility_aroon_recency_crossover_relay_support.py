"""Build outcome-blind source support for frozen HVAROON-24."""
...
from training import preregister_high_volatility_aroon_recency_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "9e37b9667e813a344430362cced124bdecd37289b406a17df5ca17deffe80e5f"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_aroon_recency_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_aroon_recency_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_aroon_recency_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_aroon_recency_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_aroon_recency_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "aroon_up", "aroon_down", "difference",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "aroon_up", "aroon_down", "difference",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVAROON source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVAROON invalid source key")
...
def aroon(high: pd.Series, low: pd.Series, valid: pd.Series) -> tuple[pd.Series, pd.Series]:
    period = P["aroon_periods"]
    up = np.full(len(high), np.nan); down = np.full(len(high), np.nan)
    highs = pd.to_numeric(high, errors="coerce").to_numpy(float)
    lows = pd.to_numeric(low, errors="coerce").to_numpy(float)
    validity = valid.to_numpy(bool)
    for index in range(period - 1, len(high)):
        left = index - period + 1
        if not validity[left : index + 1].all():
            continue
        high_window = highs[left : index + 1]; low_window = lows[left : index + 1]
        high_position = int(np.flatnonzero(high_window == np.max(high_window))[-1])
        low_position = int(np.flatnonzero(low_window == np.min(low_window))[-1])
        up[index] = 100.0 * (period - ((period - 1) - high_position)) / period
        down[index] = 100.0 * (period - ((period - 1) - low_position)) / period
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["aroon_up"], bars["aroon_down"] = aroon(bars.bar_high, bars.bar_low, bars.valid_bar)
    bars["difference"] = bars.aroon_up - bars.aroon_down
    bars["variation"] = np.sqrt(bars.variation_component.rolling(P["variation_hours"] // 4, min_periods=P["variation_hours"] // 4).sum())
    bars["source_valid"] = bars.valid_bar & np.isfinite(bars[["aroon_up","aroon_down","difference","variation"]]).all(axis=1) & bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    difference=pd.to_numeric(used.difference,errors="coerce");previous=difference.shift(1)
    crossover=difference.ne(0)&previous.ne(0)&np.sign(difference).ne(np.sign(previous))
    variation=used.variation_rank.ge(P["variation_rank_min"]);side=pd.Series(np.sign(difference),index=used.index).fillna(0).astype(int)
    state=valid&side.ne(0)&crossover&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVAROON prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvaroon_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvaroon_24_source_support_v1", "policy_id": prereg.POLICY_ID,
