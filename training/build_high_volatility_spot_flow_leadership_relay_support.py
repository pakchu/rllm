"""Materialize source-only HVSFLR-6 clocks."""
...
SOURCE_DIR = Path("data/high_volatility_spot_flow_leadership_relay_sources_2023_2026")
PANEL = SOURCE_DIR / "eight_hour_flow_confirmation_panel.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_spot_flow_leadership_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_spot_flow_leadership_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_spot_flow_leadership_relay_support_2026-08-08.json")
SPLITS = {
...
COLUMNS = (
    ...
    "exit_time",
    "side",
    "spot_late_return","perp_late_return","spot_late_taker_imbalance","perp_late_taker_imbalance",
    "full_variation",
    "variation_rank",
)
PERP_QUERY = """
...
def _late_metrics(window:pd.DataFrame)->tuple[float,float]:
    late=window.iloc[360:];quote=float(late.quote_asset_volume.sum());return float(np.log(float(late.close.iloc[-1])/float(late.open.iloc[0]))),(2*float(late.taker_buy_quote.sum())-quote)/quote


def _spot_flow_leadership_panel(perp: pd.DataFrame,spot:pd.DataFrame) -> pd.DataFrame:
    ...
    for decision in decisions:
        ...
        perp_window=perp.reindex(expected);spot_window=spot.reindex(expected);valid=_valid_window(perp_window) and _valid_window(spot_window) and float(perp_window.iloc[360:].quote_asset_volume.sum())>0 and float(spot_window.iloc[360:].quote_asset_volume.sum())>0
        if valid:
            perp_late_return,perp_late_taker_imbalance=_late_metrics(perp_window);spot_late_return,spot_late_taker_imbalance=_late_metrics(spot_window);full_variation=float(np.log(perp_window.close.astype(float)).diff().dropna().pow(2).sum());valid=full_variation>0
        else:
            perp_late_return=perp_late_taker_imbalance=spot_late_return=spot_late_taker_imbalance=full_variation=float("nan")
        rows.append(
            {
                "decision_time": decision,
                "perp_source_rows":int(perp_window.notna().all(axis=1).sum()),"spot_source_rows":int(spot_window.notna().all(axis=1).sum()),
                "source_valid": valid,
                "spot_late_return":spot_late_return,"perp_late_return":perp_late_return,"spot_late_taker_imbalance":spot_late_taker_imbalance,"perp_late_taker_imbalance":perp_late_taker_imbalance,"full_variation":full_variation,
            }
        )
...
def materialize() -> dict[str, Any]:
    ...
    database.dispose()
    panel = _spot_flow_leadership_panel(perp,spot)
    SOURCE_DIR.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(panel, PANEL)
    core = {
        "protocol_version": "hvsflr_6_btc_source_v1",
...
def conditions(frame: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    spot_return=frame.spot_late_return;perp_return=frame.perp_late_return;spot_imbalance=frame.spot_late_taker_imbalance;perp_imbalance=frame.perp_late_taker_imbalance
    rank = frame.variation_rank
    if control == "one_boundary_stale_geometry":
        spot_return=spot_return.shift(1);perp_return=perp_return.shift(1);spot_imbalance=spot_imbalance.shift(1);perp_imbalance=perp_imbalance.shift(1)
        rank = rank.shift(1)
    volatility_gate = (
    ...
    magnitude_gate=spot_imbalance.abs().ge(.1);leadership_gate=pd.Series(True,index=frame.index) if control=="no_leadership_ratio_gate" else spot_imbalance.abs().ge(1.5*perp_imbalance.abs());long=spot_return.gt(0)&perp_return.gt(0)&spot_imbalance.gt(0);short=spot_return.lt(0)&perp_return.lt(0)&spot_imbalance.lt(0)
    active = (
        frame.signal_valid
        & np.isfinite(spot_return)&np.isfinite(perp_return)&np.isfinite(spot_imbalance)&np.isfinite(perp_imbalance)
        & np.isfinite(rank)
        & volatility_gate
        & magnitude_gate
        & leadership_gate
        ...
        rows.append(
            {
                ...
                "spot_late_return":float(frame.at[index,"spot_late_return"]),"perp_late_return":float(frame.at[index,"perp_late_return"]),"spot_late_taker_imbalance":float(frame.at[index,"spot_late_taker_imbalance"]),"perp_late_taker_imbalance":float(frame.at[index,"perp_late_taker_imbalance"]),
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvsflr_6_source_support_v1",
        "policy_id": "HVSFLR-6",
        "preregistration": {
