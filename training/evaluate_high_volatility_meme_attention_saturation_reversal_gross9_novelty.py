"""Evaluate frozen HVMASR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVMASR-8"
PROTOCOL = "hvmasr_8_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_meme_attention_saturation_reversal_preregistration_2026-08-09.json"
)
...
OUTPUT = Path(
    "results/high_volatility_meme_attention_saturation_reversal_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVMASR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVMASR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVMASR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVMASR primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject HVMASR-8 unchanged before economic outcomes"
        ),
    }
