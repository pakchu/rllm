"""Build outcome-blind source support for frozen HVTLRR-8."""
...
from training import preregister_high_volatility_turnover_lead_response_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-04-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="b5b66bb4ddb8738132db5795b68dd0a68489190e5412ba5fa2aa67db42929d8e"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_turnover_lead_response_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_turnover_lead_response_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_turnover_lead_response_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_turnover_lead_response_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_turnover_lead_response_relay_support_2026-08-10.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","lead_response","response_strength_rank","contemporaneous_response","contemporaneous_strength_rank","realized_variation","variation_rank","completed_return","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","lead_response","response_strength_rank","contemporaneous_response","contemporaneous_strength_rank","realized_variation","variation_rank","completed_return")
...
def turnover_statistics(block:pd.DataFrame)->tuple[float,float,float,float]:
 groups=np.arange(len(block))//5
 five=block.groupby(groups,sort=True).agg(open=("open","first"),close=("close","last"),quote=("quote_asset_volume","sum"))
 if len(five)!=96 or not np.isfinite(five[["open","close","quote"]]).all().all() or not five[["open","close"]].gt(0).all().all() or not five.quote.ge(0).all():return (math.nan,)*4
 turnover=np.log1p(five.quote.to_numpy(float));returns=np.log(five.close.to_numpy(float)/five.open.to_numpy(float))
 if float(np.std(turnover[:-1],ddof=1))<=0 or float(np.std(returns[1:],ddof=1))<=0:return (math.nan,)*4
 lead=float(np.corrcoef(turnover[:-1],returns[1:])[0,1])
 contemporaneous=float(np.corrcoef(turnover,returns)[0,1]) if float(np.std(turnover,ddof=1))>0 and float(np.std(returns,ddof=1))>0 else math.nan
 variation=float(np.sqrt(np.square(returns).sum()));completed=float(math.log(block.close.iloc[-1]/block.open.iloc[0]))
 return lead,contemporaneous,variation,completed
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close","quote_asset_volume"]:raise RuntimeError("HVTLRR source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVTLRR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("12h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   lead,contemporaneous,var,ret=turnover_statistics(block);valid=all(math.isfinite(v) for v in (lead,contemporaneous,var,ret)) and var>0 and lead!=0 and ret!=0
  else:lead=contemporaneous=var=ret=math.nan;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"lead_response":lead,"contemporaneous_response":contemporaneous,"realized_variation":var,"completed_return":ret})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["response_strength_rank"]=prior_rank(panel.lead_response.abs().where(valid));panel["contemporaneous_strength_rank"]=prior_rank(panel.contemporaneous_response.abs().where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));agreement=np.sign(panel.lead_response)==np.sign(panel.completed_return);panel["eligible"]=valid&panel.response_strength_rank.ge(P["response_strength_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"])&agreement;panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_boundary_stale_response":
  cols=["source_valid","lead_response","response_strength_rank","variation_rank","completed_return","feature_available_time"];used[cols]=panel[cols].shift(1)
 valid=used.source_valid.eq(True);strong=used.response_strength_rank.ge(P["response_strength_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"]);agreement=np.sign(used.lead_response)==np.sign(used.completed_return);state=valid&strong&variation&agreement
 if control=="no_response_tail":state=valid&variation&agreement
 elif control=="no_variation_gate":state=valid&strong&agreement
 elif control=="contemporaneous_turnover_response":
  agreement=np.sign(used.contemporaneous_response)==np.sign(used.completed_return);state=valid&used.contemporaneous_strength_rank.ge(P["response_strength_rank_min"])&variation&agreement
 onset=previous_valid_onset(state,valid);side=np.sign(pd.to_numeric(used.lead_response,errors="coerce").fillna(0)).astype(int)
 if control=="contemporaneous_turnover_response":side=np.sign(pd.to_numeric(used.contemporaneous_response,errors="coerce").fillna(0)).astype(int)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVTLRR prereg drift")
 ...
 source_core={"protocol_version":"hvtlrr_8_source_v1","query":QUERY,"query_sha256":hashlib.sha256(QUERY.encode()).hexdigest(),"window":[START.isoformat(),END.isoformat()],"physical_rows":len(raw),"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"panel":{"path":str(PANEL),"sha256":sha(PANEL),"rows":len(panel),"valid_rows":int(panel.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};manifest={**source_core,"manifest_hash":chash(source_core)};immutable(MANIFEST,json_bytes(manifest))
