"""Evaluate frozen HVSAR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVSAR-12"
PROTOCOL = "hvsar_12_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_serial_autocorrelation_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVSAR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVSAR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVSAR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVSAR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvsar_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVSAR-12 unchanged before economic outcomes"
        ),
    }
