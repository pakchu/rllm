"""Materialize deterministic, outcome-blind source support for HVIMR-8."""
...
PREREG_SHA = "95ae4f54c1f59d4059687105d975faf964b67f2816c98af1fd2593e1cb329569"
CLOCK = Path("data/high_volatility_intraday_mechanism_router_clocks_2023_2026.csv.gz")
RESULT = Path("results/high_volatility_intraday_mechanism_router_support_2026-08-16.json")
...
def _registration() -> dict[str, Any]:
    if prereg.sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVIMR-8 preregistration hash drift")
    ...
    for component, artifacts in prereg.COMPONENT_ARTIFACTS.items():
        for kind, artifact in artifacts.items():
            if prereg.sha256_file(artifact["path"]) != artifact["sha256"]:
                raise RuntimeError(f"HVIMR-8 {component} {kind} drift")
...
def build_clock(clocks: Mapping[str, pd.DataFrame]) -> tuple[pd.DataFrame, dict[str, Any]]:
    if set(clocks) != set(prereg.COMPONENT_IDS):
        raise RuntimeError("HVIMR-8 component roster drift")
    assignment = dict(zip((0, 8, 16), prereg.ACTION_IDS))
    rows = []
    counts: dict[str, Any] = {
        "component_input_rows": {action: len(clocks[action]) for action in prereg.ACTION_IDS},
        "routed_rows_by_action": {action: 0 for action in prereg.ACTION_IDS},
        "discarded_off_slot_rows": {action: 0 for action in prereg.ACTION_IDS},
        "emitted_decisions": 0, "longs": 0, "shorts": 0,
    }
    for action in prereg.ACTION_IDS:
        for row in clocks[action].itertuples(index=False):
            decision = row.decision_time
            if assignment[decision.hour] != action:
                counts["discarded_off_slot_rows"][action] += 1
                continue
            split = _stage(decision)
            if split is None:
                continue
            sides = [0, 0, 0]
            sides[prereg.ACTION_IDS.index(action)] = row.side
            side = prereg.intraday_router_side(decision.hour, *sides)
            entry, exit_ = decision + ENTRY_DELAY, decision + ENTRY_DELAY + HOLD
            if row.split != split or row.feature_available_time > entry:
                raise RuntimeError("HVIMR-8 stage or availability drift")
            rows.append({"candidate": prereg.POLICY_ID, "control": "primary", "split": split, "routed_action": action, "decision_time": decision, "feature_available_time": row.feature_available_time, "entry_time": entry, "exit_time": exit_, "side": side})
            counts["routed_rows_by_action"][action] += 1
            counts["longs" if side == 1 else "shorts"] += 1
    columns = ("candidate", "control", "split", "routed_action", "decision_time", "feature_available_time", "entry_time", "exit_time", "side")
    ...
    if counts["emitted_decisions"] != sum(counts["routed_rows_by_action"].values()):
        raise RuntimeError("HVIMR-8 incidence accounting drift")
    if len(output) > 1 and not output["entry_time"].iloc[1:].reset_index(drop=True).ge(output["exit_time"].iloc[:-1].reset_index(drop=True)).all():
        raise RuntimeError("HVIMR-8 output overlap")
...
def run(clock_path: Path = CLOCK, result_path: Path = RESULT) -> dict[str, Any]:
    ...
    core = {"protocol_version": "hvimr_8_source_support_v1", "policy_id": prereg.POLICY_ID, "preregistration": {"path": str(prereg.DEFAULT_OUTPUT), "sha256": PREREG_SHA, "manifest_hash": registration["manifest_hash"]}, "verified_component_artifacts": verified, "component_clock_fields_opened": list(FIELDS), "combined_incidence_opened": True, "combined_postentry_returns_or_pnl_opened": False, "entry_exit_prices_opened": False, "returns_opened": False, "funding_opened": False, "pnl_opened": False, "gross9_comparator_rows_opened": False, "clock": {"path": str(clock_path), "sha256": prereg.sha256_file(clock_path), "rows": len(clock)}, "intraday_router_accounting": accounting, "support": stats, "support_checks": checks, "support_passed": passed, "advance_to_gross9_novelty": passed, "advance_to_economic_outcomes": False, "decision": "pass_to_gross9_novelty" if passed else "terminal_source_support_reject"}
...
if __name__ == "__main__":
    ...
    print(json.dumps({"events": {k: v["events"] for k, v in report["support"].items()}, "accounting": report["intraday_router_accounting"], "support_passed": report["support_passed"]}, indent=2))
