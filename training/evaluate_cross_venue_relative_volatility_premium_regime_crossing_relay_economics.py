"""Sequential strict economics for frozen CVRVPR-12."""
...
PREREG = Path("results/cross_venue_relative_volatility_premium_regime_crossing_relay_preregistration_2026-08-08.json")
PREREG_SHA = "1d28e79d946483d21286f48fedb360012aabe4bbef56e2d364b3419554392015"
SUPPORT = Path("results/cross_venue_relative_volatility_premium_regime_crossing_relay_support_2026-08-08.json")
SUPPORT_SHA = "a8fec64c957be3680ac783b6481fa7e727c8f90d48a0b50fe355bcaa3eb0d924"
NOVELTY = Path("results/cross_venue_relative_volatility_premium_regime_crossing_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "4432ee2922773d33866421edc12d76e724536060a81e4493b6c8f7e9e68d7466"
CLOCK = Path("data/cross_venue_relative_volatility_premium_regime_crossing_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "3e95024b93938157c5ec47d2326451ed76805643959ccaeeb296fc8b4ee0a598"
CONTROL_DIR = Path("data/cross_venue_relative_volatility_premium_regime_crossing_relay_controls_2023_2026")
FREEZE = Path("results/cross_venue_relative_volatility_premium_regime_crossing_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/cross_venue_relative_volatility_premium_regime_crossing_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
CONTROLS = ("absolute_dvol_level_crossing", "absolute_bvol_level_crossing", "one_hour_stale_ratio", "no_crossing", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CVRVPR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CVRVPR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "cvrvpr_12_sequential_economics_v1", "policy_id": POLICY_ID,
