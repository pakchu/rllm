"""Evaluate the frozen HVSMRSR-8 action-vote clock against Gross9."""
...
POLICY = "HVSMRSR-8"
PROTOCOL = "hvsmrsr_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_spot_median_return_shift_relay_clocks_2023_2026.csv.gz",
    "sha256": "0bf35d87f0bdd058283bc536d7c9012fd6edbf1fbc61f9871cdabf6b189b1cd3",
    "rows": 342,
}
OUTPUT = Path("results/high_volatility_spot_median_return_shift_relay_gross9_novelty_2026-08-18.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVSMRSR-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVSMRSR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVSMRSR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVSMRSR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVSMRSR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVSMRSR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVSMRSR-8 unchanged before economics",
    }
