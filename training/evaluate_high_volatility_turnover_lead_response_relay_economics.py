"""Sequential strict economics for frozen HVTLRR-8."""
...
PREREG = Path("results/high_volatility_turnover_lead_response_relay_preregistration_2026-08-10.json")
PREREG_SHA = "b5b66bb4ddb8738132db5795b68dd0a68489190e5412ba5fa2aa67db42929d8e"
SUPPORT = Path("results/high_volatility_turnover_lead_response_relay_support_2026-08-10.json")
SUPPORT_SHA = "c7cd0be2e2c20958181138ec92429d52968a80035c9f1d841384c5d5740bf493"
NOVELTY = Path("results/high_volatility_turnover_lead_response_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "a247a4ffb47437d8f1a67ce8bdc66f0c2487d7bc6ec8d15860f48ab33c98fdda"
CLOCK = Path("data/high_volatility_turnover_lead_response_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "392f972c3df7e0e9aaaa447729066235c0c5c50ae403e42fae6126a9dedb4e74"
CONTROL_DIR = Path("data/high_volatility_turnover_lead_response_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_turnover_lead_response_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_turnover_lead_response_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_response_tail", "no_variation_gate", "contemporaneous_turnover_response", "one_boundary_stale_response", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTLRR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTLRR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvtlrr_8_sequential_economics_v1", "policy_id": POLICY_ID,
