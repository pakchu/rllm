"""Sequential strict economics for frozen LFIC-12."""
...
PREREG = Path("results/lagged_flow_impact_coherence_relay_preregistration_2026-08-09.json")
PREREG_SHA = "6f407a92d68f8c771ee8cf90bfd5b9df4711f020b33f55f273ef716cf2886485"
SUPPORT = Path("results/lagged_flow_impact_coherence_relay_support_2026-08-09.json")
SUPPORT_SHA = "d026f8eee0cb9945e297ea7fd304ae868b6471dbeb07a60905643986b9d5e4b5"
NOVELTY = Path("results/lagged_flow_impact_coherence_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "159e6203191a1ce7cc95b6e1077c289727c8c3f63c67fc2cefb0353fdc00722b"
CLOCK = Path("data/lagged_flow_impact_coherence_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d437d875671db6dc2b1c52bd80e6c13c56c0f000a9fbeb4cc729949a6f501e21"
CONTROL_DIR = Path("data/lagged_flow_impact_coherence_relay_controls_2023_2026")
FREEZE = Path("results/lagged_flow_impact_coherence_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/lagged_flow_impact_coherence_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("contemporaneous_coherence", "no_flow_magnitude_tail", "stale_recent_flow", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("LFIC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("LFIC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "lfic_12_sequential_economics_v1", "policy_id": POLICY_ID,
