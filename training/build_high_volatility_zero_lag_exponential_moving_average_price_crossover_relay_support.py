"""Build outcome-blind source support for frozen HVZLEMA-C10-24."""
...
from training import preregister_high_volatility_zero_lag_exponential_moving_average_price_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "70f541b3d3ccc31bd95f5eb524c03083f2d879c1e95c1ed127754db691401816"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_zero_lag_exponential_moving_average_price_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_zero_lag_exponential_moving_average_price_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_zero_lag_exponential_moving_average_price_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_zero_lag_exponential_moving_average_price_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_zero_lag_exponential_moving_average_price_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "adjusted_close", "zlema", "ordinary_ema",
    "entry_side", "ordinary_ema_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "adjusted_close", "zlema", "ordinary_ema",
    "entry_side", "ordinary_ema_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVZLEMA source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVZLEMA invalid source key")
...
def zlema_price_cross(close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    """Replicate frozen LEAN ZLEMA(10) inputs with conservative WarmUpPeriod readiness."""
    closes = close.to_numpy(float)
    validity = valid.to_numpy(bool)
    adjusted = np.full(len(close), np.nan)
    zlema = np.full(len(close), np.nan)
    ordinary = np.full(len(close), np.nan)
    entry_side = np.zeros(len(close), dtype=int)
    ordinary_side = np.zeros(len(close), dtype=int)
    history: list[float] = []
    zema = LeanEma(P["period"])
    ema = LeanEma(P["period"])
    prior_relation = 0
    prior_ordinary_relation = 0
    ...
    for index, is_valid in enumerate(validity):
        if not is_valid:
            ...
            zema = LeanEma(P["period"])
            ema = LeanEma(P["period"])
            prior_relation = prior_ordinary_relation = 0
            continue
        history.append(closes[index])
        ordinary_value = ema.update(closes[index])
        if math.isfinite(ordinary_value):
            ordinary[index] = ordinary_value
        if len(history) > P["delay_bars"]:
            adjusted[index] = 2.0 * closes[index] - history[-P["delay_bars"] - 1]
            value = zema.update(adjusted[index])
            if len(history) >= P["period"] + P["period"] // 2 and math.isfinite(value):
                zlema[index] = value
        if math.isfinite(zlema[index]):
            relation = 1 if closes[index] > zlema[index] else -1 if closes[index] < zlema[index] else 0
            entry_side[index] = relation if relation and relation != prior_relation else 0
            prior_relation = relation
        if math.isfinite(ordinary[index]):
            relation = 1 if closes[index] > ordinary[index] else -1 if closes[index] < ordinary[index] else 0
            ordinary_side[index] = relation if relation and relation != prior_ordinary_relation else 0
            prior_ordinary_relation = relation
    return pd.DataFrame({"adjusted_close": adjusted, "zlema": zlema, "ordinary_ema": ordinary,
                         "entry_side": entry_side, "ordinary_ema_side": ordinary_side}, index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=bars.join(zlema_price_cross(bars.bar_close,bars.valid_bar));bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["adjusted_close","zlema","ordinary_ema","variation"]]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="ordinary_ema_crossover":side=pd.to_numeric(used.ordinary_ema_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVZLEMA prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvzlema_c10_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvzlema_c10_24_source_support_v1", "policy_id": prereg.POLICY_ID,
