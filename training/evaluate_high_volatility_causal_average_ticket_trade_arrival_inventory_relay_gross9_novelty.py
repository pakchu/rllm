"""Evaluate the frozen HVCATTAIR-8 action-vote clock against Gross9."""
...
POLICY = "HVCATTAIR-8"
PROTOCOL = "hvcattair_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_average_ticket_trade_arrival_inventory_relay_clocks_2020_2026.csv.gz",
    "sha256": "8d070f4e2545353aa87d931abb05836e9fe6fa7275d773e3f6b3ecfef20cdbc3",
    "rows": 99,
}
OUTPUT = Path("results/high_volatility_causal_average_ticket_trade_arrival_inventory_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCATTAIR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCATTAIR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATTAIR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCATTAIR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCATTAIR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCATTAIR-8 unchanged before economics",
    }
