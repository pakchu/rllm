"""Evaluate frozen HVPVO-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVPVO-12"
PROTOCOL = "hvpvo_12_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_percentage_volume_oscillator_ignition_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVPVO predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVPVO novelty limits drift")
    if (
        ...
        raise RuntimeError("HVPVO predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVPVO primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvpvo_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVPVO-12 unchanged before economic outcomes",
    }
