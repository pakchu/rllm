"""Build outcome-blind source support for frozen HVVTX-24."""
...
from training import preregister_high_volatility_vortex_path_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "92753d47fa9416286efbd0efe7bde6f27ce85ea9869a1658b1a32937ed8d765a"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_vortex_path_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_vortex_path_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_vortex_path_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_vortex_path_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_vortex_path_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "true_range", "positive_path", "negative_path",
    "vi_plus", "vi_minus", "difference", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "true_range", "positive_path", "negative_path",
    "vi_plus", "vi_minus", "difference", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVVTX source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVVTX invalid source key")
...
def vortex(high: pd.Series, low: pd.Series, close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    period=P["vortex_periods"];transition=valid&valid.shift(1,fill_value=False)
    prior_close=close.shift(1)
    true_range=pd.concat([(high-low),(high-prior_close).abs(),(low-prior_close).abs()],axis=1).max(axis=1).where(transition)
    positive=(high-low.shift(1)).abs().where(transition);negative=(low-high.shift(1)).abs().where(transition)
    tr_sum=true_range.rolling(period,min_periods=period).sum();plus=positive.rolling(period,min_periods=period).sum()/tr_sum;minus=negative.rolling(period,min_periods=period).sum()/tr_sum
    return pd.DataFrame({"true_range":true_range,"positive_path":positive,"negative_path":negative,"vi_plus":plus,"vi_minus":minus},index=high.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=pd.concat([bars,vortex(bars.bar_high,bars.bar_low,bars.bar_close,bars.valid_bar)],axis=1);bars["difference"]=bars.vi_plus-bars.vi_minus
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum())
    bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["true_range","positive_path","negative_path","vi_plus","vi_minus","difference","variation"]]).all(axis=1)&bars.true_range.gt(0)&bars.variation.gt(0)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVVTX prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvvtx_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvvtx_24_source_support_v1", "policy_id": prereg.POLICY_ID,
