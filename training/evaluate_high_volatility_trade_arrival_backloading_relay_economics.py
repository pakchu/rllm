"""Sequential strict economics for frozen HVTAB-6."""
...
PREREG = Path("results/high_volatility_trade_arrival_backloading_relay_preregistration_2026-08-10.json")
PREREG_SHA = "abbd1443802bf4ce596a9cbe1d0578ad2ae55e835403773baa57e25f07b8fcd7"
SUPPORT = Path("results/high_volatility_trade_arrival_backloading_relay_support_2026-08-10.json")
SUPPORT_SHA = "26baf5ce0be7ac12d38f57427144ea46c8bb7970b8a7038c8d60c29fbefd0cb8"
NOVELTY = Path("results/high_volatility_trade_arrival_backloading_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "ced490b8674c58dd432a210e6375d22629b009b1e5076651c9c8438109a25310"
CLOCK = Path("data/high_volatility_trade_arrival_backloading_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "885152436744b59e36df7a0578ea109dbee48414a00f2c3fb9404ab5cc5b0104"
CONTROL_DIR = Path("data/high_volatility_trade_arrival_backloading_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_trade_arrival_backloading_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_trade_arrival_backloading_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTAB frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTAB novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvtab_6_sequential_economics_v1", "policy_id": POLICY_ID,
