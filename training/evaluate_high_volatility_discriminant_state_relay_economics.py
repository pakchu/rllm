"""Sequential strict economics for frozen HVDLDA-24."""
...
PREREG = Path("results/high_volatility_discriminant_state_relay_preregistration_2026-08-09.json")
PREREG_SHA = "eaeb58a45878b45d2ab24fdcbf3ca4ba2c5055dc853e51b45811d2890f8c63db"
SUPPORT = Path("results/high_volatility_discriminant_state_relay_support_2026-08-09.json")
SUPPORT_SHA = "1a9cab691b5f8f817ad70bca64e2cb9e8ee62a4202ac3f7d60e17f684ec5f71b"
NOVELTY = Path("results/high_volatility_discriminant_state_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "908fd0660dc14d38b68071255f652750c86dd41731f130aa4f9388a5e3f7316d"
CLOCK = Path("data/high_volatility_discriminant_state_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "1953f9122aaf8f4fb4e0d3a13445ce1f855fed2122e7802795f6c67fdef8376f"
CONTROL_DIR = Path("data/high_volatility_discriminant_state_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_discriminant_state_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_discriminant_state_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDLDA frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDLDA novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvlda_24_sequential_economics_v1", "policy_id": POLICY_ID,
