"""Build source-only HVSIR-6 clocks before opening outcomes or Gross9."""
...
from training import preregister_high_volatility_semivariance_imbalance_reversal as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.long_regime_combo_scan import LongComboScanConfig, _load_market


PREREG_SHA = "503a69b496a724d7c27e68e90a678c667c11b13656b7b79bd9099782a2bd17fd"
...
CLOCK = Path("data/high_volatility_semivariance_imbalance_reversal_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_semivariance_imbalance_reversal_controls_2023_2026")
SNAPSHOT = Path("data/high_volatility_semivariance_imbalance_reversal_sources_2023_2026/semivariance.csv.gz")
RESULT = Path("results/high_volatility_semivariance_imbalance_reversal_support_2026-08-09.json")
SPLITS = {
...
CONTROLS = ("no_volatility_gate", "one_bar_stale_features", "direction_flip")
...
def load_combined_market() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if combined.date.duplicated().any() or not combined.date.diff().dropna().eq(pd.Timedelta(minutes=5)).all():
        raise RuntimeError("HVSIR combined market continuity drift")
    return combined, {
...
def score_snapshot(market: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, float]]:
    ...
    valid_count = returns.notna().astype(int).rolling(72, min_periods=72).sum()
    squared = returns.pow(2)
    positive = squared.where(returns > 0, 0.0).rolling(72, min_periods=72).sum()
    negative = squared.where(returns < 0, 0.0).rolling(72, min_periods=72).sum()
    total = positive + negative
    imbalance = ((positive - negative) / total.replace(0.0, np.nan)).where(valid_count == 72)
    realized_volatility = np.sqrt(total).where(valid_count == 72)
    positions = np.arange(72, len(market), dtype=np.int64)
    dates = pd.to_datetime(market.date, utc=True)
    frame = pd.DataFrame(
        {
            "position": positions,
            "decision_bar_time": dates.iloc[positions].to_numpy(),
            "semivariance_imbalance": imbalance.iloc[positions].to_numpy(float),
            "absolute_imbalance": imbalance.abs().iloc[positions].to_numpy(float),
            "realized_volatility_6h": realized_volatility.iloc[positions].to_numpy(float),
            "valid_return_count": valid_count.iloc[positions].to_numpy(float),
        }
    )
    calibration = frame[
    ...
    calibration = calibration[
        calibration.valid_return_count.eq(72)
        & np.isfinite(calibration.absolute_imbalance)
        & np.isfinite(calibration.realized_volatility_6h)
    ]
    if len(calibration) < 40_000:
        raise RuntimeError("HVSIR source-only calibration floor failed")
    thresholds = {
        "absolute_imbalance_q80": float(calibration.absolute_imbalance.quantile(0.80)),
        "realized_volatility_q70": float(calibration.realized_volatility_6h.quantile(0.70)),
    }
...
def build_clock(scores: pd.DataFrame, thresholds: dict[str, float], control: str = "primary") -> pd.DataFrame:
    ...
    if control == "one_bar_stale_features":
        frame[["semivariance_imbalance", "absolute_imbalance", "realized_volatility_6h", "valid_return_count"]] = frame[
            ["semivariance_imbalance", "absolute_imbalance", "realized_volatility_6h", "valid_return_count"]
        ].shift(1)
    valid = (
        frame.valid_return_count.eq(72)
        & np.isfinite(frame.semivariance_imbalance)
        & np.isfinite(frame.absolute_imbalance)
        & np.isfinite(frame.realized_volatility_6h)
        & frame.semivariance_imbalance.ne(0)
    )
    crossing = frame.absolute_imbalance.ge(thresholds["absolute_imbalance_q80"]) & frame.absolute_imbalance.shift(1).lt(
        thresholds["absolute_imbalance_q80"]
    )
    volatility_gate = frame.realized_volatility_6h.ge(thresholds["realized_volatility_q70"])
    if control == "no_volatility_gate":
        volatility_gate[:] = True
    active = valid & crossing & volatility_gate
    ...
    for index in np.flatnonzero(active.to_numpy(bool)):
        ...
        side = -1 if frame.semivariance_imbalance.iloc[index] > 0 else 1
        rows.append(
            {
                "candidate": "HVSIR-6",
                "control": control,
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVSIR preregistration drift")
    ...
    if registration["manifest_hash"] != prereg.canonical_hash(core_registration):
        raise RuntimeError("HVSIR preregistration manifest drift")
    ...
    core = {
        "protocol_version": "hvsir_6_source_support_v1",
        "policy_id": "HVSIR-6",
