"""Source-only support gate for frozen HVIDBR-8."""
...
from training import preregister_high_volatility_intraday_distribution_break_relay as prereg
...
ROOT = Path("data/high_volatility_intraday_distribution_break_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_intraday_distribution_break_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_intraday_distribution_break_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_intraday_distribution_break_relay_support_2026-08-10.json")
...
def ks_distance(first: np.ndarray, second: np.ndarray) -> float:
    """Exact two-sample empirical-CDF supremum distance."""
    if len(first) != 240 or len(second) != 240:
        return math.nan
    ...
    return distance if math.isfinite(distance) else math.nan
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVIDBR source timestamps")
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        if valid:
            ...
            distribution_break = ks_distance(returns[:240], returns[240:])
            block_return = float(np.log(window["close"].iloc[-1] / window["open"].iloc[0]))
            late_return = float(np.log(window["close"].iloc[-1] / window["open"].iloc[-240]))
            valid = variation > 0 and math.isfinite(distribution_break)
        if not valid:
            variation = distribution_break = block_return = late_return = math.nan
        rows.append(
            {
                "decision_time": decision,
                "source_valid": valid,
                "realized_variation": variation,
                "ks_distance": distribution_break,
    ...
    states["distribution_break_rank"] = prior_rank(states["ks_distance"].where(states["source_valid"]))
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvidbr_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    break_rank = states["distribution_break_rank"]
    distance = states["ks_distance"]
    if control == "one_block_stale_distribution":
        block_return, late_return, variation_rank, break_rank, distance = [
            value.shift(1)
            for value in (block_return, late_return, variation_rank, break_rank, distance)
        ]
    variation_gate = (
    ...
    if control == "no_distribution_break_gate":
        break_gate = pd.Series(True, index=states.index)
    elif control == "raw_ks_above_quarter":
        break_gate = distance.gt(0.25)
    else:
        break_gate = break_rank.ge(0.75)
    agreement = (
        block_return.ne(0)
        & late_return.ne(0)
        & np.sign(block_return).eq(np.sign(late_return))
    )
    eligible = states["source_valid"] & variation_gate & break_gate & agreement
        ...
        rows.append(
            {
                ...
                "ks_distance": float(states.at[index, "ks_distance"]),
                "distribution_break_rank": float(states.at[index, "distribution_break_rank"]),
            }
        )
    columns = [
        "candidate", "control", "split", "decision_time", "feature_available_time",
        "entry_time", "exit_time", "side", "realized_variation", "variation_rank",
        "ks_distance", "distribution_break_rank",
    ]
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVIDBR preregistration drift")
    ...
    core = {
        "protocol_version": "hvidbr_8_source_support_v1",
        "policy_id": "HVIDBR-8",
        "preregistration": {
