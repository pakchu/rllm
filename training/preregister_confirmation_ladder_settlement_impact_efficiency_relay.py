"""Outcome-blind preregistration for CLSIER-6."""
...
POLICY_ID = "CLSIER-6"
DEFAULT_OUTPUT = Path(
    "results/confirmation_ladder_settlement_impact_efficiency_relay_preregistration_2026-08-12.json"
)
...
def build() -> dict[str, Any]:
    core = {
        "protocol_version": "confirmation_ladder_settlement_impact_efficiency_relay_v1",
        "policy_id": POLICY_ID,
        ...
        "singleton": True,
        "mechanism": {
            "claim": "When the final three canonical confirmation intervals have higher mean absolute BTC return per finalized block-weight unit than the first three while their returns share one strict sign, settlement-adjusted price impact is escalating; follow that common late direction for six elapsed hours.",
            "side": "common strict sign of confirmation-interval returns 4, 5, and 6",
            "why_distinct": "CLSIER computes each interval's absolute price response per finalized block-weight unit before comparing ladder halves. CLSLR compared aggregate weight only; CLTDR compared transaction density only; CLBHAR compared unscaled early and late returns. CLSIER reuses none of their event sets or controls and uses no fitted threshold.",
            "volatile_market_target": "directional price discovery accompanied by escalating settlement-adjusted impact identifies unstable block-time repricing; causal RV20 q90 remains only a later audit",
            "why_low_gross9_overlap_is_plausible": "height-modulo anchors, six per-block settlement-impact ratios, late return unanimity, and six-hour reservation form a sparse asynchronous clock",
        },
        "features": {
            ...
            "settlement_impact": "I_k=abs(R_k)/W_k for k=1..6, finite and strict positive",
            "early_impact": "mean(I_1,I_2,I_3)",
            "late_impact": "mean(I_4,I_5,I_6)",
            "impact_efficiency_escalation": "late_impact>early_impact, strict and threshold-free",
            "eligible_state": "late unanimity and impact-efficiency escalation are both true",
        ...
        "diagnostic_controls": {
            "definitions": {
                "late_unanimity_only": "late-three return unanimity onset without impact-efficiency escalation",
                "impact_efficiency_only": "impact-efficiency escalation onset without late-three unanimity; side is sign(L)",
        ...
        "research_boundary": {
            ...
            "exact_clsier_incidence_known": False,
            "exact_clsier_outcomes_known": False,
        ...
        "stopping_rule": (
            "terminal first failure; no block predicate, impact formula, threshold, confirmation, embargo, "
