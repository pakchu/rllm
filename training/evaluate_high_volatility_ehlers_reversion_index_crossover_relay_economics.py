"""Sequential strict economics for frozen HVERI-24."""
...
PREREG = Path("results/high_volatility_ehlers_reversion_index_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "23a5e28dce104959eaa5d1833002ab1c7cf307fbaa3e785335881a734b5c9c80"
SUPPORT = Path("results/high_volatility_ehlers_reversion_index_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "8646458d86e435838855bd7888107a00dfbf11aa0794f1df0b3fe7034173f5f1"
NOVELTY = Path("results/high_volatility_ehlers_reversion_index_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "af0021d23d621fd9e404ce5ac775e34a22e4953939b39b5976bc27585c8ca0d1"
CLOCK = Path("data/high_volatility_ehlers_reversion_index_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "59fe9a1a4b4b46ed6c4a14a53a2d2fc61eca4917fe1341c2a081243c2c48be59"
CONTROL_DIR = Path("data/high_volatility_ehlers_reversion_index_crossover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_ehlers_reversion_index_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ehlers_reversion_index_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "raw_ratio_zero_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVERI frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVERI novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hveri_24_sequential_economics_v1", "policy_id": POLICY_ID,
