"""Materialize deterministic, outcome-blind source support for HVLAV-8."""
...
from training import preregister_high_volatility_lagged_agreement_veto as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv

PREREG_SHA = "391656b8a3d866b763d893d973691a0eb8cdce56ede6f7ef51191693fef1d26a"
CLOCK = Path("data/high_volatility_lagged_agreement_veto_clocks_2023_2026.csv.gz")
RESULT = Path("results/high_volatility_lagged_agreement_veto_support_2026-08-16.json")
...
def _registration() -> dict[str, Any]:
    if prereg.sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVLAV-8 preregistration hash drift")
    ...
    for component, artifacts in prereg.COMPONENT_ARTIFACTS.items():
        for kind, artifact in artifacts.items():
            if prereg.sha256_file(artifact["path"]) != artifact["sha256"]:
                raise RuntimeError(f"HVLAV-8 {component} {kind} drift")
...
def load_clock(component: str) -> pd.DataFrame:
    ...
    if frame["decision_time"].duplicated().any() or not frame["side"].isin((-1, 1)).all():
        raise RuntimeError(f"HVLAV-8 invalid {component} clock")
    expected_hours = (3, 11, 19) if component == prereg.ACTION_IDS[0] else (0, 8, 16)
    if not frame["decision_time"].dt.hour.isin(expected_hours).all() or not frame["decision_time"].dt.minute.eq(0).all():
        raise RuntimeError(f"HVLAV-8 off-grid {component} clock")
    if not frame["entry_time"].eq(frame["decision_time"] + ENTRY_DELAY).all() or not frame["exit_time"].eq(frame["entry_time"] + HOLD).all():
        raise RuntimeError(f"HVLAV-8 {component} timing drift")
    if not frame["feature_available_time"].le(frame["entry_time"]).all():
        raise RuntimeError(f"HVLAV-8 unavailable {component} feature")
...
def build_clock(clocks: Mapping[str, pd.DataFrame]) -> tuple[pd.DataFrame, dict[str, Any]]:
    if set(clocks) != set(prereg.COMPONENT_IDS):
        raise RuntimeError("HVLAV-8 component roster drift")
    ...
    for decision in decisions:
        ...
        veto = indexed[veto_id].get(decision - LAG)
        ...
        if primary.split != split or primary.feature_available_time > entry:
            raise RuntimeError("HVLAV-8 primary stage or availability drift")
        if veto is not None and (veto.split != split or veto.feature_available_time > entry):
            raise RuntimeError("HVLAV-8 veto stage or availability drift")
        lagged_veto_side = 0 if veto is None else veto.side
        side = prereg.lagged_veto_side(primary.side, lagged_veto_side)
        if veto is None:
        ...
        if side == 0:
            continue
        available = max(primary.feature_available_time, primary.feature_available_time if veto is None else veto.feature_available_time)
        rows.append({"candidate": prereg.POLICY_ID, "control": "primary", "split": split, "selected_action": primary_id, "decision_time": decision, "lagged_veto_decision_time": decision - LAG, "feature_available_time": available, "entry_time": entry, "exit_time": exit_, "side": side, "active_action_count": active_count})
        counts["longs" if side == 1 else "shorts"] += 1
    columns = ("candidate", "control", "split", "selected_action", "decision_time", "lagged_veto_decision_time", "feature_available_time", "entry_time", "exit_time", "side", "active_action_count")
    ...
    if counts["emitted_decisions"] + counts["veto_opposite_side"] != counts["primary_decisions"]:
        raise RuntimeError("HVLAV-8 incidence accounting drift")
    if len(output) > 1 and not output["entry_time"].iloc[1:].reset_index(drop=True).ge(output["exit_time"].iloc[:-1].reset_index(drop=True)).all():
        raise RuntimeError("HVLAV-8 output overlap")
...
def run(clock_path: Path = CLOCK, result_path: Path = RESULT) -> dict[str, Any]:
    ...
    core = {"protocol_version": "hvlav_8_source_support_v1", "policy_id": prereg.POLICY_ID, "preregistration": {"path": str(prereg.DEFAULT_OUTPUT), "sha256": PREREG_SHA, "manifest_hash": registration["manifest_hash"]}, "verified_component_artifacts": verified, "component_clock_fields_opened": list(FIELDS), "combined_incidence_opened": True, "combined_postentry_returns_or_pnl_opened": False, "entry_exit_prices_opened": False, "returns_opened": False, "funding_opened": False, "pnl_opened": False, "gross9_comparator_rows_opened": False, "clock": {"path": str(clock_path), "sha256": prereg.sha256_file(clock_path), "rows": len(clock)}, "lagged_veto_accounting": accounting, "support": stats, "support_checks": checks, "support_passed": passed, "advance_to_gross9_novelty": passed, "advance_to_economic_outcomes": False, "decision": "pass_to_gross9_novelty" if passed else "terminal_source_support_reject"}
...
if __name__ == "__main__":
    ...
    print(json.dumps({"events": {k: v["events"] for k, v in report["support"].items()}, "accounting": report["lagged_veto_accounting"], "support_passed": report["support_passed"]}, indent=2))
