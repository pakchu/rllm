"""Source-only support gate for frozen HVCSR-8."""
...
from training import preregister_high_volatility_return_curvature_scarcity_relay as prereg
...
PREREG_SHA = "306172f8379ee38236634cec6482d7b6d2be837c1451990a99f81c7cb1e331f7"
SPLITS = {
...
ROOT = Path("data/high_volatility_return_curvature_scarcity_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_return_curvature_scarcity_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_return_curvature_scarcity_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_return_curvature_scarcity_relay_support_2026-08-10.json")
...
def return_curvature_ratio(close: np.ndarray) -> float:
    close = np.asarray(close, dtype=float)
    if len(close) != 480 or not np.isfinite(close).all() or np.any(close <= 0):
        return math.nan
    returns = np.diff(np.log(close))
    path_mass = float(np.abs(returns).sum())
    if path_mass <= 0 or not math.isfinite(path_mass):
        return math.nan
    curvature_mass = float(np.abs(np.diff(returns)).sum())
    value = curvature_mass / (2.0 * path_mass)
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVCSR source timestamps")
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        if valid:
            ...
            curvature_ratio = return_curvature_ratio(window["close"].to_numpy(float))
            ...
            valid = variation > 0 and math.isfinite(curvature_ratio)
        if not valid:
            variation = curvature_ratio = block_return = late_return = math.nan
        rows.append(
            {
    ...
    states["curvature_rank"] = prior_rank(states["curvature_ratio"].where(states["source_valid"]))
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvcsr_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    curvature_rank = states["curvature_rank"]
    curvature_ratio = states["curvature_ratio"]
    if control == "one_block_stale_geometry":
        block_return, late_return, variation_rank, curvature_rank, curvature_ratio = [
            value.shift(1)
            for value in (block_return, late_return, variation_rank, curvature_rank, curvature_ratio)
        ]
    variation_gate = (
    ...
    if control == "no_curvature_scarcity_tail":
        curvature_gate = pd.Series(True, index=states.index)
    elif control == "raw_curvature_ratio_at_most_half":
        curvature_gate = curvature_ratio.le(0.5)
    else:
        curvature_gate = curvature_rank.le(0.25)
    agreement = (
        block_return.ne(0)
        & late_return.ne(0)
        & np.sign(block_return).eq(np.sign(late_return))
    )
    eligible = states["source_valid"] & variation_gate & curvature_gate & agreement
        ...
        rows.append(
            {
                ...
                "curvature_ratio": float(states.at[index, "curvature_ratio"]),
                "curvature_rank": float(states.at[index, "curvature_rank"]),
            }
        )
    columns = [
        "candidate", "control", "split", "decision_time", "feature_available_time",
        "entry_time", "exit_time", "side", "realized_variation", "variation_rank",
        "curvature_ratio", "curvature_rank",
    ]
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCSR preregistration drift")
    ...
    core = {
        "protocol_version": "hvcsr_8_source_support_v1",
        "policy_id": "HVCSR-8",
        "preregistration": {
