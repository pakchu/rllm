"""Evaluate the frozen HVRM3-8 action-vote clock against Gross9."""
...
POLICY = "HVRM3-8"
PROTOCOL = "hvrm3_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_robust_state_three_action_majority_clocks_2023_2026.csv.gz",
    "sha256": "f51b8db96b3e7a6d709f0bce414fab61273618b4dab23f88658b15fe8d8bd576",
    "rows": 206,
}
OUTPUT = Path("results/high_volatility_robust_state_three_action_majority_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVRM3-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVRM3-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVRM3-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVRM3-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVRM3-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVRM3-8 unchanged before economics",
    }
