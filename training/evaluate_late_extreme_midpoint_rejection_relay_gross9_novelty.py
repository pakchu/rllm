"""Evaluate frozen LEMRR-16 structural novelty against every Gross9 sleeve."""
...
POLICY = "LEMRR-16"
PROTOCOL = "lemrr_16_gross9_novelty_v1"
...
OUTPUT = Path("results/late_extreme_midpoint_rejection_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("LEMRR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("LEMRR novelty limits drift")
    if (
        ...
        raise RuntimeError("LEMRR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="LEMRR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "lemrr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject LEMRR-16 unchanged before economic outcomes",
    }
