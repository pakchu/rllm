"""Outcome-blind preregistration for the fixed HVSPR-8 priority-router battery."""
...
ACTION_ORDER = ("HVTFR-8", "HVSVF-8", "HVRSSR-8")
PRIORITY_ORDERS = (
...
# Reuse, without substitution, every constituent binding frozen by HVSOF-8.
ACTION_ARTIFACTS = hvsof.ACTION_ARTIFACTS
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_state_priority_router_v1",
        "policy_id": POLICY_ID,
        ...
        "artifact_binding_source": {
            "policy_id": hvsof.POLICY_ID,
            "rule": "reuse the exact action and eligibility artifact bindings frozen by HVSOF-8",
        },
...
def select_train_winner(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    if len(rows) != FAMILY_SIZE:
        raise ValueError("HVSPR-8 selection requires exactly four candidate rows")
    by_id: dict[str, Mapping[str, Any]] = {}
    for row in rows:
        candidate = row.get("candidate")
        if not isinstance(candidate, str) or candidate in by_id:
            raise ValueError("HVSPR-8 candidate IDs must be unique strings")
        by_id[candidate] = row
    if set(by_id) != set(CANDIDATE_FAMILY):
        raise ValueError("HVSPR-8 selection requires the exact frozen family")
    ...
    for order, candidate in enumerate(CANDIDATE_FAMILY):
        ...
        if (
            ...
            raise ValueError("HVSPR-8 pass flags must be booleans")
...
def validate(value: Mapping[str, Any]) -> None:
    ...
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError("HVSPR-8 preregistration drift")
    if tuple(value.get("candidate_family", ())) != CANDIDATE_FAMILY:
        raise RuntimeError("HVSPR-8 candidate family drift")
    if value.get("priority_orders") != [list(order) for order in PRIORITY_ORDERS]:
        raise RuntimeError("HVSPR-8 priority order drift")
    if value.get("action_artifacts") != hvsof.ACTION_ARTIFACTS:
        raise RuntimeError("HVSPR-8 action artifact bindings drift")
    if value.get("eligibility_artifacts") != hvsof.ELIGIBILITY_ARTIFACTS:
        raise RuntimeError("HVSPR-8 eligibility artifact bindings drift")
    if (
        value.get("familywise_multiplicity", {}).get("winner_raw_weekly_signflip_p_max")
        != BONFERRONI_RAW_P_MAX
    ):
        raise RuntimeError("HVSPR-8 multiplicity drift")
    for artifacts in (*ACTION_ARTIFACTS.values(), *ELIGIBILITY_ARTIFACTS.values()):
        for artifact_type, artifact in artifacts.items():
            if sha256_file(artifact["path"]) != artifact["sha256"]:
                raise RuntimeError(f"HVSPR-8 {artifact_type} artifact drift")
