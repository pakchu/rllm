"""Build outcome-blind source support for frozen HVABANDS-B10-W4-24."""
...
from training import preregister_high_volatility_acceleration_bands_breakout_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "364973b184aead235bf490f5bdb8ab777db5973264feeddc6ebc00bd5708416b"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_acceleration_bands_breakout_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_acceleration_bands_breakout_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_acceleration_bands_breakout_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_acceleration_bands_breakout_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_acceleration_bands_breakout_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "coefficient", "raw_upper", "raw_lower", "upper_band",
    "middle_band", "lower_band", "bands_ready", "entry_side", "middle_cross_side",
    "raw_boundary_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "coefficient", "raw_upper", "raw_lower",
    "upper_band", "middle_band", "lower_band", "bands_ready", "entry_side",
    "middle_cross_side", "raw_boundary_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVABANDS source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVABANDS invalid source key")
...
def acceleration_bands(bars: pd.DataFrame, valid: pd.Series) -> pd.DataFrame:
    """Replicate LEAN AccelerationBands(10,4,SMA) and frozen outward crossings."""
    coefficient=np.full(len(bars),np.nan);raw_upper=np.full(len(bars),np.nan);raw_lower=np.full(len(bars),np.nan)
    upper=np.full(len(bars),np.nan);middle=np.full(len(bars),np.nan);lower=np.full(len(bars),np.nan)
    ready=np.zeros(len(bars),dtype=bool);entry=np.zeros(len(bars),dtype=int);middle_side=np.zeros(len(bars),dtype=int);raw_side=np.zeros(len(bars),dtype=int)
    history:list[tuple[float,float,float,float]]=[];prior=None;prior_raw=None
    for index,is_valid in enumerate(valid.to_numpy(bool)):
        if not is_valid:
            history=[];prior=None;prior_raw=None;continue
        row=bars.iloc[index];high=float(row.bar_high);low=float(row.bar_low);close=float(row.bar_close)
        coefficient[index]=P["width"]*(high-low)/(high+low);raw_upper[index]=high*(1+coefficient[index]);raw_lower[index]=low*(1-coefficient[index])
        history.append((raw_upper[index],close,raw_lower[index],close));history=history[-P["period"]:]
        ready[index]=len(history)>=P["period"]
        current_raw=(close,raw_upper[index],raw_lower[index])
        if ready[index]:
            upper[index]=float(np.mean([x[0] for x in history]));middle[index]=float(np.mean([x[1] for x in history]));lower[index]=float(np.mean([x[2] for x in history]))
            if prior is not None:
                previous_close,previous_upper,previous_lower,previous_middle=prior
                if close>upper[index] and previous_close<=previous_upper:entry[index]=1
                elif close<lower[index] and previous_close>=previous_lower:entry[index]=-1
                if close>middle[index] and previous_close<=previous_middle:middle_side[index]=1
                elif close<middle[index] and previous_close>=previous_middle:middle_side[index]=-1
            if prior_raw is not None:
                previous_close,previous_upper,previous_lower=prior_raw
                if close>raw_upper[index] and previous_close<=previous_upper:raw_side[index]=1
                elif close<raw_lower[index] and previous_close>=previous_lower:raw_side[index]=-1
            prior=(close,upper[index],lower[index],middle[index])
        prior_raw=current_raw
    return pd.DataFrame({"coefficient":coefficient,"raw_upper":raw_upper,"raw_lower":raw_lower,"upper_band":upper,"middle_band":middle,"lower_band":lower,"bands_ready":ready,"entry_side":entry,"middle_cross_side":middle_side,"raw_boundary_side":raw_side},index=bars.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=bars.join(acceleration_bands(bars,bars.valid_bar));bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&bars.bands_ready&np.isfinite(bars[["coefficient","raw_upper","raw_lower","upper_band","middle_band","lower_band","variation"]]).all(axis=1)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid),panel.valid_bar);panel["eligible"]=panel.source_valid&panel.entry_side.ne(0)&panel.variation_rank.ge(P["variation_rank_min"])
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="middle_band_cross":side=pd.to_numeric(used.middle_cross_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
    elif control=="raw_transformed_boundary_break":side=pd.to_numeric(used.raw_boundary_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def build_clock(panel: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in panel.index[activity]:
        ...
        rows.append(
             ...
             **{column: bool(used.at[index, column]) if column in ("bands_ready", "eligible") else float(used.at[index, column])
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVABANDS prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvabands_b10_w4_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvabands_b10_w4_24_source_support_v1", "policy_id": prereg.POLICY_ID,
