"""Evaluate the frozen HVCOPR-24 action-vote clock against Gross9."""
...
POLICY = "HVCOPR-24"
PROTOCOL = "hvcopr_24_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_oi_path_persistence_relay_clocks_2020_2026.csv.gz",
    "sha256": "ccb0eeb36eaa0e15018c321e9e5f8d1ee1eee7c746b6eb82d4552864a83a2492",
    "rows": 174,
}
OUTPUT = Path("results/high_volatility_causal_oi_path_persistence_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCOPR-24 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCOPR-24 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCOPR-24 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCOPR-24 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCOPR-24 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCOPR-24 unchanged before economics",
    }
