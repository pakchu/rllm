"""Sequential strict economics for frozen HVAPS-F2-24."""
...
PREREG = Path("results/high_volatility_augen_price_spike_fade_relay_preregistration_2026-08-11.json")
PREREG_SHA = "690fcbc398317dcd9570baf307e65aeebe1b5a66980263c3d306afb9a92aeebf"
SUPPORT = Path("results/high_volatility_augen_price_spike_fade_relay_support_2026-08-11.json")
SUPPORT_SHA = "51f4201aabdc86d3c279c2ee4b1a5ea228c64ccfc0ed752103b530ccf21344b0"
NOVELTY = Path("results/high_volatility_augen_price_spike_fade_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "c4eba9bf3644b5a9eb7592ea19dfed9b782ee2e55e9197619d6d64713cbffdcd"
CLOCK = Path("data/high_volatility_augen_price_spike_fade_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "c4e11c8ed5ecd7053fe4cba937aabef131b913ecc2bff0d72b0a7cdf4543d786"
CONTROL_DIR = Path("data/high_volatility_augen_price_spike_fade_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_augen_price_spike_fade_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_augen_price_spike_fade_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "one_bar_stale_cross", "direction_follow", "zero_cross")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVAPS frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVAPS novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvaps_f2_24_sequential_economics_v1", "policy_id": POLICY_ID,
