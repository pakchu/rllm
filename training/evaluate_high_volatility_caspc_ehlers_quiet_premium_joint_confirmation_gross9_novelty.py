"""Evaluate the frozen HVCELVPQA-8 action-vote clock against Gross9."""
...
POLICY = "HVCELVPQA-8"
PROTOCOL = "hvcelvpqa_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_caspc_ehlers_quiet_premium_joint_confirmation_clocks_2023_2026.csv.gz",
    "sha256": "875ecdc531f8f31142be01ca9f8cc9f8df5e141bc11b6138680ee78f451c60fe",
    "rows": 115,
}
OUTPUT = Path("results/high_volatility_caspc_ehlers_quiet_premium_joint_confirmation_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCELVPQA-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCELVPQA-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCELVPQA-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCELVPQA-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCELVPQA-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCELVPQA-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCELVPQA-8 unchanged before economics",
    }
