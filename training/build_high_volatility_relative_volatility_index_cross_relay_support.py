"""Build outcome-blind source support for frozen HVRVOL-24."""
...
from training import preregister_high_volatility_relative_volatility_index_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "bebfb8fcd44d9818597e1f9faf25e50e3a274495264cf8e02c1a3e2062de7d87"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_relative_volatility_index_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_relative_volatility_index_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_relative_volatility_index_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_relative_volatility_index_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_relative_volatility_index_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "close_std", "up_input", "down_input", "up_smooth",
    "down_smooth", "rvi", "entry_side", "raw_close_side", "variation",
    "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "close_std", "up_input", "down_input",
    "up_smooth", "down_smooth", "rvi", "entry_side", "raw_close_side",
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVRVOL source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVRVOL invalid source key")
...
def causal_std(series: pd.Series, valid: pd.Series, period: int) -> pd.Series:
    ...
    for i,(value,is_valid) in enumerate(zip(series.to_numpy(float),valid.to_numpy(bool))):
        ...
        if len(window)==period:output[i]=float(np.std(window,ddof=0))
    return pd.Series(output,index=series.index)


def wilder_smooth(series: pd.Series, valid: pd.Series, period: int) -> pd.Series:
    output=np.full(len(series),np.nan);state=np.nan;seed=[]
    for i,(value,ok) in enumerate(zip(series.to_numpy(float),valid.to_numpy(bool))):
        if not ok or not math.isfinite(value):state=np.nan;seed=[];continue
        if not math.isfinite(state):
            seed.append(float(value))
            if len(seed)==period:state=sum(seed)/period;output[i]=state
        else:state=((period-1)*state+value)/period;output[i]=state
    return pd.Series(output,index=series.index)


def cross_side(signal: pd.Series) -> pd.Series:
    line=P["balance_line"];previous=signal.shift(1);side=np.zeros(len(signal),dtype=int)
    side[(signal>line)&(previous<=line)]=1;side[(signal<line)&(previous>=line)]=-1
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["close_std"]=causal_std(bars.bar_close,bars.valid_bar,P["standard_deviation_periods"]);prior_close=bars.bar_close.shift(1);input_valid=bars.valid_bar&bars.valid_bar.shift(1,fill_value=False)&np.isfinite(bars.close_std);bars["up_input"]=np.where(bars.bar_close>prior_close,bars.close_std,0.0);bars["down_input"]=np.where(bars.bar_close<prior_close,bars.close_std,0.0);bars["up_smooth"]=wilder_smooth(bars.up_input,input_valid,P["smoothing_periods"]);bars["down_smooth"]=wilder_smooth(bars.down_input,input_valid,P["smoothing_periods"]);denominator=bars.up_smooth+bars.down_smooth;bars["rvi"]=(100.0*bars.up_smooth/denominator).where(denominator.gt(0));bars["entry_side"]=cross_side(bars.rvi);bars["raw_close_side"]=np.sign(bars.bar_close-prior_close).fillna(0).astype(int)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["close_std","up_input","down_input","up_smooth","down_smooth","rvi","variation"];bars["source_valid"]=bars.valid_bar&input_valid&np.isfinite(bars[numeric]).all(axis=1)&denominator.gt(0)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="raw_close_direction":side=pd.to_numeric(used.raw_close_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVRVOL prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvrvol_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvrvol_24_source_support_v1", "policy_id": prereg.POLICY_ID,
