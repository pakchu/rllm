#!/usr/bin/env python3
"""Frozen RV20 participation-breakout candidate evaluator.
...
PREREGISTRATION = Path(
    "results/rv20_participation_breakout_preregistration_2026-08-08.json"
)
EXPECTED_PREREGISTRATION_SHA256 = (
    "b3ecb25538b23ffdea7b91fbc8084458e8c83a566d5b47b710f022403d0fe405"
)
OUTPUT = Path("results/rv20_participation_breakout_evaluation_2026-08-08.json")
REPORT = Path("docs/rv20-participation-breakout-evaluation-2026-08-08.md")
ATTEMPT = Path(
    "results/rv20_participation_breakout_evaluation_attempt_consumed_2026-08-08.json"
)
...
ENTRY_COST = 0.0006
STRESS_COST = 0.0010
CANDIDATE = "rv20_participation_breakout_continuation"
...
GRID_FIELDS = (
    ...
    "hour_range_quantile",
    "max_hold_hours",
    "oi_change_lower_quantile",
    "rv20_regime_quantile",
    "stop_loss_pct",
    "take_profit_pct",
    "taker_alignment_minimum",
)
...
class Cell:
    ...
    max_hold_hours: int
    oi_change_lower_quantile: float
    rv20_regime_quantile: float
    stop_loss_pct: float
    take_profit_pct: float
    taker_alignment_minimum: float
...
def load_preregistration(path: str | Path = PREREGISTRATION) -> dict[str, Any]:
    ...
    if payload.get("schema") != "rllm.rv20-participation-breakout.preregistration.v1":
    ...
    if len(frozen_grid(payload)) != 384:
...
def build_rv20_daily_clock(market: pd.DataFrame) -> pd.DataFrame:
    """Causal daily RV20 with selectable shifted Q80/Q90 reference clocks."""
    ...
    return pd.DataFrame({
        "rv20": rv20,
        "rv20_q80": shifted_rolling_quantile(
            rv20, 0.80, RV_REFERENCE_WINDOW
        ),
        "rv20_q90": shifted_rolling_quantile(
            rv20, 0.90, RV_REFERENCE_WINDOW
        ),
    })
...
def attach_causal_thresholds(
    ...
    specs = {
        ...
        ("oi", cell.oi_change_lower_quantile) for cell in cells
    }
    source = {
...
def cell_signal_sides(hourly: pd.DataFrame, cell: Cell) -> np.ndarray:
    ...
    oi_threshold = hourly[f"q_oi_{cell.oi_change_lower_quantile:g}"]
    ...
    long = common & (hourly["hour_return"] > 0.0) & (
        hourly["taker_fraction"] >= cell.taker_alignment_minimum
    )
    short = common & (hourly["hour_return"] < 0.0) & (
        hourly["taker_fraction"] <= 1.0 - cell.taker_alignment_minimum
    )
...
def train_cell_passes(normal: Mapping[str, Any], stress: Mapping[str, Any]) -> bool:
    ...
    return bool(
        int(normal["trades"]) >= 30
        and int(normal["active_weeks"]) >= 15
...
def render_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [(
        "# RV20 Participation Breakout Evaluation\n\n"
...
def _execution_config(
    ...
    return ExecutionConfig(
        ...
        output="/tmp/no_write_rv20_participation_breakout.json",
        manifest_output="/tmp/no_write_rv20_participation_breakout_manifest.json",
...
def _install_candidate_sleeve() -> None:
    ...
    portfolio.FAMILIES[CANDIDATE] = "rv20_participation_breakout"
...
def evaluate_frozen_protocol(
    ...
    if top is None:
        return {
            "schema": "rllm.rv20-participation-breakout.evaluation.v1",
    ...
    if selected_weight is None:
        ...
        control = persistent_long_vol_control(
            ...
            regime_quantile=top_cell.rv20_regime_quantile,
            leverage=leverage, cost_rate=ENTRY_COST,
        )
        residuals = {
