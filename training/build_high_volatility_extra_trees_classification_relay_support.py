"""Build OOS source-only HVETCR-48 clocks after the model freeze."""
...
PREREG_SHA = "2ebcc54789d06d97cd689fab600bb7cb547888c0e6f56ff77472db0a27a7e1ba"
MODEL_FREEZE = Path(
    "results/high_volatility_extra_trees_classification_relay_model_freeze_2026-08-09.json"
)
MODEL_FREEZE_SHA = "2aaf56992a15dd9ece2263e0337e8f8dda599a27c43af0689d73bbe2664853e8"
MODEL = Path("data/high_volatility_extra_trees_classification_relay_model_2026-08-09.joblib")
MODEL_SHA = "3b398127ed59e02d99622414b8bfc8bc10a38559502fa60ef16f91f4c88be8ad"
CLOCK = Path("data/high_volatility_extra_trees_classification_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_extra_trees_classification_relay_controls_2023_2026")
SNAPSHOT = Path(
    "data/high_volatility_extra_trees_classification_relay_sources_2023_2026/model_scores.csv.gz"
)
RESULT = Path(
    "results/high_volatility_extra_trees_classification_relay_support_2026-08-09.json"
)
...
def verify_predecessors() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVETCR preregistration drift")
    if sha256(MODEL_FREEZE) != MODEL_FREEZE_SHA or sha256(MODEL) != MODEL_SHA:
        raise RuntimeError("HVETCR model predecessor drift")
    ...
    if freeze.get("manifest_hash") != canonical_hash(freeze_core):
        raise RuntimeError("HVETCR model freeze manifest drift")
    if (
        ...
        raise RuntimeError("HVETCR model freeze state drift")
    artifact = joblib.load(MODEL)
    if artifact.get("policy_id") != "HVETCR-48":
        raise RuntimeError("HVETCR model policy drift")
...
def load_combined_market() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if combined.date.duplicated().any() or not combined.date.is_monotonic_increasing:
        raise RuntimeError("HVETCR combined market time drift")
    deltas = combined.date.diff().dropna()
    if not deltas.eq(pd.Timedelta(minutes=5)).all():
        raise RuntimeError("HVETCR combined market is not continuous 5m")
    return combined, {
...
def score_snapshot(market: pd.DataFrame, artifact: dict[str, Any]) -> pd.DataFrame:
    ...
    if missing:
        raise RuntimeError(f"HVETCR OOS feature drift: {missing}")
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvetcr_48_oos_source_support_v1",
        "policy_id": "HVETCR-48",
