"""Evaluate frozen HVTPDCR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVTPDCR-8"
PROTOCOL = "hvtpdcr_8_gross9_novelty_v1"
PREREG = Path(
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTPDCR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVTPDCR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTPDCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTPDCR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvtpdcr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVTPDCR-8 unchanged before economic outcomes"
        ),
    }
