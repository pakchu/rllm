"""Sequential strict economics for frozen CVSKDMR-6."""
...
PREREG = Path("results/cboe_skew_acceleration_dual_confirmation_relay_preregistration_2026-08-08.json")
PREREG_SHA = "a55b6bccc5d9264d51738751e372f73ae5d1f1432dc8c4452ef98a5620cf2746"
SUPPORT = Path("results/cboe_skew_acceleration_dual_confirmation_relay_support_2026-08-08.json")
SUPPORT_SHA = "36ab77ad9cf6dc27553a92ad04720f454b0f986f16052415f0934dfe265fc6a8"
NOVELTY = Path("results/cboe_skew_acceleration_dual_confirmation_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "580ed697d676c1f15ea0c9a24744372a93296868cb0aac179f5bf8761f990232"
CLOCK = Path("data/cboe_skew_acceleration_dual_confirmation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "f912495583b14d30583738c4d6ecb728648532480fe548d0881725b5a030d9ed"
CONTROL_DIR = Path("data/cboe_skew_acceleration_dual_confirmation_relay_controls_2023_2026")
FREEZE = Path("results/cboe_skew_acceleration_dual_confirmation_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/cboe_skew_acceleration_dual_confirmation_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CVSKDMR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CVSKDMR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "cvskdmr_6_sequential_economics_v1", "policy_id": POLICY_ID,
