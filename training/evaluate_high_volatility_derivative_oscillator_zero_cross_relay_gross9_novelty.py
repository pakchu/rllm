"""Evaluate frozen HVDO-Z14-5-3-9-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDO-Z14-5-3-9-24"
PROTOCOL = "hvdo_z14_5_3_9_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_derivative_oscillator_zero_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDO predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVDO novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDO predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDO primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdo_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVDO-Z14-5-3-9-24 unchanged before economic outcomes",
    }
