"""Evaluate frozen CMMTCR-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "CMMTCR-24"
PROTOCOL = "cmmtcr_24_gross9_novelty_v1"
...
OUTPUT = Path("results/cross_market_multi_horizon_trend_concordance_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CMMTCR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("CMMTCR novelty limits drift")
    if (
        ...
        raise RuntimeError("CMMTCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CMMTCR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "cmmtcr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject CMMTCR-24 unchanged before economic outcomes",
    }
