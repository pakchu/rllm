"""Build outcome-blind source support for frozen HVEIS-24."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_elder_impulse_color_transition_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2020-01-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="f9bd9dfb4bdbfa112dfeb2d435597133bbadfeca15256c192ea19aaae40b0e46";REG=prereg.build();P=REG["policy"]
...
ROOT=Path("data/high_volatility_elder_impulse_color_transition_relay_sources_2023_2026");PANEL=ROOT/"four_hour_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_elder_impulse_color_transition_relay_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_elder_impulse_color_transition_relay_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_elder_impulse_color_transition_relay_controls_2023_2026");RESULT=Path("results/high_volatility_elder_impulse_color_transition_relay_support_2026-08-11.json");BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("source_start","feature_available_time","source_valid","bar_open","bar_high","bar_low","bar_close","trend_ema","trend_slope","macd_fast","macd_slow","macd","macd_signal","histogram","histogram_slope","color","histogram_only_color","variation","variation_rank","eligible")
CLOCK_COLUMNS=("candidate","control","split","source_start","feature_available_time","entry_time","exit_time","side","bar_close","trend_ema","trend_slope","macd","macd_signal","histogram","histogram_slope","color","histogram_only_color","variation","variation_rank","eligible")
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVEIS source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVEIS invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 source=prepare(raw).reindex(pd.date_range(START,END,freq="1min",inclusive="left"));g=source.groupby(source.index.floor("4h"),sort=True);bars=pd.DataFrame({"rows":g.row_valid.sum(),"bar_open":g.open.first(),"bar_high":g.high.max(),"bar_low":g.low.min(),"bar_close":g.close.last(),"variation_component":g.minute_sq_return.sum(min_count=240)});bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[["bar_open","bar_high","bar_low","bar_close"]]).all(axis=1)&bars[["bar_open","bar_high","bar_low","bar_close"]].gt(0).all(axis=1);bars["trend_ema"]=segmented_ewm(bars.bar_close,bars.valid_bar,2/(P["trend_ema_periods"]+1),P["trend_ema_periods"]);bars["macd_fast"]=segmented_ewm(bars.bar_close,bars.valid_bar,2/(P["macd_fast_periods"]+1),P["macd_fast_periods"]);bars["macd_slow"]=segmented_ewm(bars.bar_close,bars.valid_bar,2/(P["macd_slow_periods"]+1),P["macd_slow_periods"]);bars["macd"]=bars.macd_fast-bars.macd_slow;macd_valid=np.isfinite(bars.macd);bars["macd_signal"]=segmented_ewm(bars.macd,macd_valid,2/(P["macd_signal_periods"]+1),P["macd_signal_periods"]);bars["histogram"]=bars.macd-bars.macd_signal;bars["trend_slope"]=bars.trend_ema-bars.trend_ema.shift(1);bars["histogram_slope"]=bars.histogram-bars.histogram.shift(1);bars["color"]=np.where((bars.trend_slope>0)&(bars.histogram_slope>0),1,np.where((bars.trend_slope<0)&(bars.histogram_slope<0),-1,0));bars["histogram_only_color"]=np.where(bars.histogram_slope>0,1,np.where(bars.histogram_slope<0,-1,0));bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["trend_ema","trend_slope","macd_fast","macd_slow","macd","macd_signal","histogram","histogram_slope","variation"]]).all(axis=1)&bars.variation.gt(0);panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.color.shift(1);long=panel.color.eq(1)&previous.ne(1);short=panel.color.eq(-1)&previous.ne(-1);panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&(long|short)&panel.variation_rank.ge(P["variation_rank_min"]);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);color=pd.to_numeric(used.histogram_only_color if control=="histogram_slope_only" else used.color,errors="coerce").fillna(0).astype(int);previous=color.shift(1);long=color.eq(1)&previous.ne(1);short=color.eq(-1)&previous.ne(-1);side=pd.Series(np.where(long,1,np.where(short,-1,0)),index=used.index,dtype=int);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&side.ne(0)&variation
 if control=="no_variation_gate":state=valid&side.ne(0)
 elif control=="one_bar_stale_transition":state=state.shift(1,fill_value=False);side=side.shift(1,fill_value=0)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVEIS prereg drift")
