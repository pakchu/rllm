"""Evaluate frozen HVMPC-8 structural novelty against every Gross9 sleeve."""
...
OUTPUT = Path(
    "results/high_volatility_macro_polarity_concordance_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVMPC predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVMPC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVMPC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVMPC primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvmpc_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVMPC-8 unchanged before economic outcomes"
        ),
    }
