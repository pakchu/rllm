"""Evaluate frozen MXRBR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "MXRBR-12"
PROTOCOL = "mxrbr_12_gross9_novelty_v1"
...
OUTPUT = Path("results/mexican_peso_risk_barometer_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("MXRBR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("MXRBR novelty limits drift")
    if (
        ...
        raise RuntimeError("MXRBR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="MXRBR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "mxrbr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject MXRBR-12 unchanged before economic outcomes",
    }
