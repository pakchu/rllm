"""Evaluate frozen HVHA-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVHA-24"
PROTOCOL = "hvha_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_heikin_ashi_color_flip_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVHA predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVHA novelty limits drift")
    if (
        ...
        raise RuntimeError("HVHA predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVHA primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvha_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVHA-24 unchanged before economic outcomes",
    }
