"""Evaluate frozen HVKVO-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVKVO-24"
PROTOCOL = "hvkvo_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_klinger_volume_oscillator_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVKVO predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVKVO novelty limits drift")
    if (
        ...
        raise RuntimeError("HVKVO predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVKVO primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvkvo_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVKVO-24 unchanged before economic outcomes",
    }
