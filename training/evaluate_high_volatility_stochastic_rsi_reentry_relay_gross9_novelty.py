"""Evaluate frozen HVSR-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVSR-24"
PROTOCOL = "hvsr_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_stochastic_rsi_reentry_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVSR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVSR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVSR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVSR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvsr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVSR-24 unchanged before economic outcomes",
    }
