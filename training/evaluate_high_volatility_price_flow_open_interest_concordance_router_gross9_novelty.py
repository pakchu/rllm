"""Evaluate the frozen HVPFOICR-8 action-vote clock against Gross9."""
...
POLICY = "HVPFOICR-8"
PROTOCOL = "hvpfoicr_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_price_flow_open_interest_concordance_router_preregistration_2026-08-16.json")
PREREG_SHA = "047536b94c59286fbfa6786e950f27d673b65e552f0e17b542078ad0d3133fb5"
SUPPORT = Path("results/high_volatility_price_flow_open_interest_concordance_router_support_2026-08-16.json")
SUPPORT_SHA = "6bbb9de9dc3ebb954a6d12f6b091ce67217f56eece729ce97c1deb7931bc497a"
CLOCK = {
    "path": "data/high_volatility_price_flow_open_interest_concordance_router_clocks_2020_2026.csv.gz",
    "sha256": "805d1b49e34800dd8e1c86613fc1a2bf1f78f893970c82f06eb954eefa7dbe9f",
    "rows": 593,
}
OUTPUT = Path("results/high_volatility_price_flow_open_interest_concordance_router_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVPFOICR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVPFOICR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVPFOICR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVPFOICR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVPFOICR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVPFOICR-8 unchanged before economics",
    }
