"""Sequential strict economics for frozen HVCAPO-8."""
...
PREREG = Path("results/high_volatility_cross_alt_path_ordered_excursion_dominance_relay_preregistration_2026-08-13.json")
PREREG_SHA = "c8359f6523d93e5ccb957223501dd909ad16fd0c42aabc915532b5cf20d3ba64"
SUPPORT = Path("results/high_volatility_cross_alt_path_ordered_excursion_dominance_relay_support_2026-08-13.json")
SUPPORT_SHA = "d1c377d55d0319481f2e9d27e814accd095536bc1f5faf6be65964032c321f20"
NOVELTY = Path("results/high_volatility_cross_alt_path_ordered_excursion_dominance_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "e55bcd0c48f4c3d111859034c070a6343f5a09fec3cbf9c27e59ef8b4c5a4830"
CLOCK = Path("data/high_volatility_cross_alt_path_ordered_excursion_dominance_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "653ae2e57c94396c128fb22796c52c5ed998595816e31f6bd2603c4d8ae992b2"
CONTROL_DIR = Path("data/high_volatility_cross_alt_path_ordered_excursion_dominance_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_cross_alt_path_ordered_excursion_dominance_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_alt_path_ordered_excursion_dominance_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "no_asymmetry_tail", "three_alt_consensus", "equal_weighted_score", "one_decision_stale_consensus", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCAPO frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCAPO novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcapo_8_sequential_economics_v1", "policy_id": POLICY_ID,
