"""Evaluate frozen SERBR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "SERBR-12"
PROTOCOL = "serbr_12_gross9_novelty_v1"
...
OUTPUT = Path("results/sterling_euro_risk_beta_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("SERBR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("SERBR novelty limits drift")
    if (
        ...
        raise RuntimeError("SERBR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="SERBR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "serbr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject SERBR-12 unchanged before economic outcomes",
    }
