"""Evaluate frozen HVBB-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVBB-24"
PROTOCOL = "hvbb_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_bollinger_band_reentry_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVBB predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVBB novelty limits drift")
    if (
        ...
        raise RuntimeError("HVBB predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVBB primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVBB-24 unchanged before economic outcomes",
    }
