"""Deterministic outcome-blind source support for HVCATFIR-8."""
...
from training import preregister_high_volatility_causal_average_ticket_flow_inventory_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "aee1c5f573850945dd3e2793ac7191004ce08b65bf84647cdd0d1988ead3b26a"
...
ROOT = Path("data/high_volatility_causal_average_ticket_flow_inventory_relay_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_average_ticket_flow_inventory_relay_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_average_ticket_flow_inventory_relay_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_causal_average_ticket_flow_inventory_relay_support_2026-08-16.json")
...
PANEL_COLUMNS = (
    ...
    "first_half_trade_count", "second_half_trade_count", "first_half_average_ticket", "second_half_average_ticket", "second_half_aggressive_flow", "completed_return",
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVCATFIR-8 OI schema drift")
    expected_bars = [
        "decision_time", "first_quote_turnover", "first_trade_count", "second_quote_turnover", "second_trade_count", "second_signed_flow", "completed_return", "minute_squared_return",
        "source_rows", "distinct_rows", "first_ts", "last_ts", "coherent",
    ]
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCATFIR-8 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all():
        raise RuntimeError("HVCATFIR-8 invalid OI rows")

    bars = bars.copy()
    for column in ("decision_time", "first_ts", "last_ts"):
        bars[column] = pd.to_datetime(bars[column], utc=True, errors="raise")
    numeric = ("first_quote_turnover", "first_trade_count", "second_quote_turnover", "second_trade_count", "second_signed_flow", "completed_return", "minute_squared_return", "source_rows", "distinct_rows")
    for column in numeric:
        bars[column] = pd.to_numeric(bars[column], errors="raise")
    if bars["decision_time"].duplicated().any():
        raise RuntimeError("HVCATFIR-8 duplicate eight-hour bar")
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel = panel.join(
        bars[["bar_valid", "first_half_quote_turnover", "second_half_quote_turnover", "first_half_trade_count", "second_half_trade_count", "first_half_average_ticket", "second_half_average_ticket", "second_half_aggressive_flow", "completed_return", "average_ticket_acceleration", "realized_variation"]],
        on="decision_time",
    )
    panel["source_valid"] = (
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCATFIR-8 side or availability drift")
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCATFIR-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcatfir_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvcatfir_8_source_support_v1", "policy_id": prereg.POLICY_ID,
