"""Sequential strict economics for frozen DEPAR-6."""
...
PREREG = Path("results/deribit_expansion_partial_absorption_relay_preregistration_2026-08-08.json")
PREREG_SHA = "d5ae12db6ae389de60612ab41cef3847196175ef5142d3c971eb60be64e64c46"
SUPPORT = Path("results/deribit_expansion_partial_absorption_relay_support_2026-08-08.json")
SUPPORT_SHA = "d3473bbbe2ef39a7304f63da252e5709d612e293009bb14a4ca598cbb1ac9fff"
NOVELTY = Path("results/deribit_expansion_partial_absorption_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "ba5f01607d87369f274df96743f7528b3f4c07e1ace4bde2f7fc18411e62b654"
CLOCK = Path("data/deribit_expansion_partial_absorption_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "9087ed48c339edb9193172e5609ede22dca2b56ef938aff0806148c9cd4d34dd"
CONTROL_DIR = Path("data/deribit_expansion_partial_absorption_relay_controls_2023_2026")
FREEZE = Path("results/deribit_expansion_partial_absorption_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/deribit_expansion_partial_absorption_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
CONTROLS = ("opposite_venue_polarity", "no_asymmetric_vol_state", "no_first_half_q60", "no_absorption_ratio", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("DEPAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("DEPAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "depar_6_sequential_economics_v1", "policy_id": POLICY_ID,
