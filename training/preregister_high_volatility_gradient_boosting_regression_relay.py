"""Outcome-sequenced preregistration for HVGBR-72."""
...
DEFAULT_OUTPUT = Path(
    "results/high_volatility_gradient_boosting_regression_relay_preregistration_2026-08-09.json"
)
SOURCE_BINDINGS = {
...
def build() -> dict[str, Any]:
    core = {
        "protocol_version": "high_volatility_gradient_boosting_regression_relay_v1",
        "policy_id": "HVGBR-72",
        ...
        "mechanism": {
            "claim": (
                "A nonlinear regressor fitted before 2023 can separate positive from "
                "negative 72-hour BTC paths using causal price, derivatives, macro, and "
                "range-state features. Trading only prediction tails during high range "
                "volatility targets persistent volatile directional regimes."
            ),
            "side": "long in calibrated upper predicted-return tail; short in lower tail",
            "why_distinct": (
                "HVGBR uses one frozen histogram gradient-boosting regressor and a 72-hour "
                "continuous-return target. HVETCR used ExtraTrees class probabilities at 48 hours; "
                "HVMRR used an eight-hour raw microstructure block and linear six-hour ridge. "
                "No terminal model output, fitted artifact, threshold, control, or OOS result is reused."
            ),
            "why_suited_to_volatile_regimes": (
                "range_vol must exceed a calibration-only 70th percentile before either side can trade"
            ),
        },
        "training_contract": {
            ...
            "label": "continuous log(open at anchor+1+864/open at anchor+1)",
            ...
            "prediction_thresholds": (
                "strict empirical 15th and 85th percentiles of predicted 72-hour log return "
                "on calibration anchors only"
            ),
            "volatility_threshold": "70th percentile of range_vol on calibration anchors only",
        ...
        "oos_clock": {
            ...
            "eligibility": (
                ...
                ">= frozen q70; predicted return <= frozen q15 or >= frozen q85"
            ),
            "entry": "anchor completed bar plus one 5m open",
            "side": "prediction>=q85 long; prediction<=q15 short; conflict impossible",
        ...
        "research_boundary": {
            "prior_hist_gradient_boosting_families_exist_and_outcomes_are_known": True,
            "prior_hvetcr_and_hvmrr_terminal_outcomes_known": True,
...
def validate(payload: dict[str, Any]) -> None:
    ...
    if payload.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError("HVGBR preregistration hash mismatch")
    for raw, expected in SOURCE_BINDINGS.items():
        if hashlib.sha256(Path(raw).read_bytes()).hexdigest() != expected:
            raise RuntimeError(f"HVGBR source drift: {raw}")
