"""Evaluate frozen HVTSF3-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVTSF3-24"
PROTOCOL = "hvtsf3_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_time_series_forecast_slope_flip_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTSF3 predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVTSF3 novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTSF3 predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTSF3 primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVTSF3-24 unchanged before economic outcomes",
    }
