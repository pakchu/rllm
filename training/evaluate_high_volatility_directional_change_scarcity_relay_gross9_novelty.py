"""Evaluate frozen HVDCS-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDCS-8"
PROTOCOL = "hvdcs_8_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_directional_change_scarcity_relay_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDCS predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVDCS novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDCS predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDCS primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject HVDCS-8 unchanged before economic outcomes"
        ),
    }
