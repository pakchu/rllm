"""Evaluate frozen FGPLR-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "FGPLR-24"
PROTOCOL = "fgplr_24_gross9_novelty_v1"
...
OUTPUT = Path("results/fear_greed_price_leadlag_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("FGPLR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("FGPLR novelty limits drift")
    if (
        ...
        raise RuntimeError("FGPLR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="FGPLR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "fgplr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject FGPLR-24 unchanged before economic outcomes",
    }
