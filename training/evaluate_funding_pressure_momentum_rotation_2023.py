"""Open only FPMR-1's frozen 2023 outcomes under a strict simulator.
...
from training.build_funding_pressure_momentum_rotation_support import (
    CLOCK_COLUMNS,
    SYMBOLS,
    assert_clock_contract,
)
from training.preregister_funding_pressure_momentum_rotation import canonical_hash
...
EVALUATION_SOURCE = Path("training/evaluate_funding_pressure_momentum_rotation_2023.py")
TEST_PATH = Path("tests/test_evaluate_funding_pressure_momentum_rotation_2023.py")
FREEZE_SOURCE = Path("training/freeze_funding_pressure_momentum_rotation_2023_evaluator.py")
FREEZE_TEST_PATH = Path(
    "tests/test_freeze_funding_pressure_momentum_rotation_2023_evaluator.py"
)
EVALUATION_FREEZE = Path(
    "results/funding_pressure_momentum_rotation_2023_evaluator_freeze_2026-07-18.json"
)
...
CLOCK_PATH = Path(
    "data/funding_pressure_momentum_rotation_clock_2023_2024.csv.gz"
)
...
DEFAULT_OUTPUT = Path(
    "results/funding_pressure_momentum_rotation_selection_2023_2026-07-18.json"
)
DEFAULT_DOCS = Path(
    "docs/funding-pressure-momentum-rotation-selection-2023-2026-07-18.md"
)

PREREGISTRATION_SHA256 = "7a7d4d174862d02389fb9b3d95f4e31bcc6d722c89ae7565d42b9d606f8f6f10"
SUPPORT_MANIFEST_SHA256 = "bade7f1ea409bd2656e0ba25c2c9f9c90cd04f5bcb9d5aa27b6c67e2ff90dfc5"
SUPPORT_MANIFEST_HASH = "b5be3bf0cc91d089a62403a57f5ab2cdf9ce0300a3c73671c97bfbb36219e254"
CLOCK_SHA256 = "4824f2643e521b20d940c07e286451201a1f3953dc7ad21ed8600a921d4cecbd"
SUPPORT_SOURCE_SHA256 = "773adbb851b1156c69105a59a8f5ce16535964beba4f66ecf28c700ee595ba7d"
PREREGISTRATION_SOURCE_SHA256 = "379e22eb851e787f937295707d178c4e46605ac9484149a18f124280ddcf4e6b"
...
class EvaluationConfig:
    ...
    cluster_signflip_seed: int = 20_260_718
...
def verify_support_and_clock() -> tuple[dict[str, Any], pd.DataFrame]:
    dependencies = (
        ...
        (
            Path("training/build_funding_pressure_momentum_rotation_support.py"),
            SUPPORT_SOURCE_SHA256,
        ),
        (
            Path("training/preregister_funding_pressure_momentum_rotation.py"),
    ...
    for dependency, expected in dependencies:
        if _sha256(dependency) != expected:
            raise RuntimeError(f"frozen FPMR-1 dependency changed: {dependency}")
    support = json.loads(SUPPORT_MANIFEST.read_text())
    if (
        support.get("manifest_hash") != SUPPORT_MANIFEST_HASH
        or canonical_hash(
            {key: value for key, value in support.items() if key != "manifest_hash"}
        )
        != SUPPORT_MANIFEST_HASH
    ):
        raise RuntimeError("FPMR-1 support manifest identity changed")
    if support.get("post_entry_returns_or_pnl_calculated") is not False:
        raise RuntimeError("FPMR-1 support artifact already opened outcomes")
    if support.get("support", {}).get("passes_support") is not True:
        raise RuntimeError("FPMR-1 support gate did not pass")
    if support.get("clock_sha256") != CLOCK_SHA256:
        raise RuntimeError("FPMR-1 support-to-clock binding changed")
    ...
    if len(clock) != 99:
        raise RuntimeError("FPMR-1 frozen clock row count changed")
...
def execution_clock(clock: pd.DataFrame) -> pd.DataFrame:
    ...
    if not (out["last_price_time"] < out["funding_cutoff_time"]).all():
        raise RuntimeError("FPMR-1 price cutoff crossed funding boundary")
    if not (out["funding_cutoff_time"] < out["decision_time"]).all():
        raise RuntimeError("FPMR-1 funding settlement crossed decision")
    if not (out["decision_time"] < out["entry_time"]).all():
        raise RuntimeError("FPMR-1 decision crossed entry")
    gross = out["long_weight"] + out["short_weight_abs"]
    if not np.allclose(gross, 1.0, atol=1e-12):
        raise RuntimeError("FPMR-1 execution gross changed")
    beta_exposure = out["long_weight"] * out["long_beta"] - out[
        "short_weight_abs"
    ] * out["short_beta"]
    if not np.allclose(beta_exposure, 0.0, atol=1e-12):
        raise RuntimeError("FPMR-1 execution clock lost beta neutrality")
    return out.sort_values("entry_time").reset_index(drop=True)


def transform_clock(clock: pd.DataFrame, kind: str) -> pd.DataFrame:
    """Apply one preregistered falsification control without fitting outcomes."""
    ...
    elif kind in {"price_only", "funding_only", "static_residual"}:
        prefix = kind
        out["long_symbol"] = out[f"{prefix}_long_symbol"]
        out["short_symbol"] = out[f"{prefix}_short_symbol"]
        out["long_weight"] = out[f"{prefix}_long_weight"]
        out["short_weight_abs"] = out[f"{prefix}_short_weight_abs"]
        # The frozen support clock stores each control's beta-neutral weights,
        # not its beta estimates. Do not leave the primary pair's beta labels
        # attached to a different control pair.
        out = out.drop(columns=["long_beta", "short_beta"])
    else:
        raise ValueError(kind)
    gross = out["long_weight"] + out["short_weight_abs"]
    if not np.allclose(gross, 1.0, atol=1e-12):
        raise RuntimeError(f"FPMR-1 {kind} control gross changed")
    return out


def verify_evaluation_freeze() -> dict[str, Any]:
    if not EVALUATION_FREEZE.exists():
        raise RuntimeError("FPMR-1 2023 evaluator freeze is missing")
    payload = json.loads(EVALUATION_FREEZE.read_text())
    core = {key: value for key, value in payload.items() if key != "manifest_hash"}
    if canonical_hash(core) != payload.get("manifest_hash"):
        raise RuntimeError("FPMR-1 evaluator freeze hash mismatch")
    checks = {
        "protocol": "FPMR-1 strict 2023 evaluator pre-outcome freeze v1",
        ...
        "decision_rule": (
            "open the singleton FPMR-1 2023 clock once under every preregistered "
            "gate; a failure retires FPMR-1 before loading any 2024 outcome"
        ),
    }
    for key, expected in checks.items():
        if payload.get(key) != expected:
            raise RuntimeError(f"FPMR-1 evaluator freeze changed: {key}")
    commit = str(payload.get("evaluation_source_commit", ""))
    if len(commit) != 40 or any(
        character not in "0123456789abcdef" for character in commit
    ):
        raise RuntimeError("FPMR-1 evaluator freeze commit is invalid")
    for dependency in (EVALUATION_SOURCE, TEST_PATH, FREEZE_SOURCE, FREEZE_TEST_PATH):
        committed = subprocess.check_output(["git", "show", f"{commit}:{dependency}"])
        if hashlib.sha256(committed).hexdigest() != _sha256(dependency):
            raise RuntimeError(f"FPMR-1 evaluator freeze commit does not bind {dependency}")
    return payload
...
def _source_records() -> dict[str, dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("FPMR-1 physical 2023 source manifest identity changed")
    if payload.get("2024_rows_parsed") != 0 or payload.get("2024_rows_written") != 0:
        raise RuntimeError("FPMR-1 physical source opened a future row")
...
def load_bundle_2023(source_dir: str | Path = EXECUTION_SOURCE_DIR) -> MarketBundle:
    ...
    for symbol in sorted(SYMBOLS):
        ...
        if dates is None:
            ...
            raise RuntimeError("FPMR-1 symbol grids differ")
        arrays = {
...
def _event_indices(
    ...
    if entry_index >= len(bundle.dates) or bundle.dates[entry_index] != entry:
        raise RuntimeError(f"missing exact FPMR-1 entry open: {entry}")
    if exit_index >= len(bundle.dates) or bundle.dates[exit_index] != exit_time:
        raise RuntimeError(f"missing exact FPMR-1 exit open: {exit_time}")
    if exit_index <= entry_index:
        raise RuntimeError("non-positive FPMR-1 hold")
...
def simulate(
    ...
    *,
    ...
    """Strict gross-one two-leg simulation on the physically sealed prefix."""

    start_ts, end_ts = pd.Timestamp(start), pd.Timestamp(end)
    if not bundle.dates[0] <= start_ts < end_ts <= bundle.dates[-1] + pd.Timedelta(minutes=5):
        raise ValueError("FPMR-1 simulation escaped loaded outcome prefix")
    if cost_bp < 0:
    ...
    for row in selected.itertuples(index=False):
        ...
        if previous_exit is not None and entry_time < previous_exit:
            raise RuntimeError("FPMR-1 execution clock overlaps")
        ...
        if not math.isclose(gross, 1.0, rel_tol=0.0, abs_tol=1e-12):
            raise RuntimeError("FPMR-1 gross escaped one")
        if min(long_weight, short_weight) <= 0.0:
            raise RuntimeError("FPMR-1 non-positive leg")
        start_equity = equity
        ...
        def settle_funding(bar_index: int) -> None:
            nonlocal cumulative_funding, total_funding
...
def selection_checks(
    ...
    return {
        ...
        "2023_cagr_to_strict_mdd_at_least_3": annual["cagr_to_strict_mdd"] >= 3.0,
        "2023_strict_mdd_at_most_15": annual["strict_mdd_pct"] <= 15.0,
        "2023_trades_at_least_40": annual["trades"] >= 40,
        "2023_h1_absolute_return_positive": primary["2023_h1"]["absolute_return_pct"] > 0.0,
        "2023_h1_trades_at_least_18": primary["2023_h1"]["trades"] >= 18,
        "2023_h2_absolute_return_positive": primary["2023_h2"]["absolute_return_pct"] > 0.0,
        "2023_h2_trades_at_least_18": primary["2023_h2"]["trades"] >= 18,
...
def evaluate(bundle: MarketBundle, clock: pd.DataFrame) -> dict[str, Any]:
    ...
    stress, delay, opposite = map(_slim, (stress_raw, delay_raw, opposite_raw))
    mechanisms = {name: _slim(stats) for name, stats in mechanism_raw.items()}
    ...
    return {
        ...
        "price_only": mechanisms["price_only"],
        "funding_only": mechanisms["funding_only"],
        "static_residual": mechanisms["static_residual"],
...
def _markdown(result: dict[str, Any]) -> str:
    ...
    for key, label in (
        ...
        ("price_only", "Price level + rotation only"),
        ("funding_only", "Funding-change only"),
        ("static_residual", "Static residual level only"),
    ):
...
def _clean_repository_head() -> str:
    ...
    if status:
        raise RuntimeError("repository must be clean before FPMR-1 2023 outcomes open")
...
def run(
    ...
    if Path(output).exists():
        raise RuntimeError("FPMR-1 2023 outcome result already exists")
    ...
    result: dict[str, Any] = {
        "protocol_version": "fpmr_v1_2023_selection_2026-07-18",
        ...
        "anti_repair": (
            "A rejection seals 2024+ for FPMR-1; no sign, score, coefficient, "
            "lookback, hold, pair, or weight repair."
        ),
    }
        ...
        json.dumps(
            {
                ...
                "controls": {
                    key: result["evaluation"][key] for key in control_keys
                },
