"""Evaluate frozen HVRNRR-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVRNRR-6"
PROTOCOL = "hvrnrr_6_gross9_novelty_v1"
PREREG = Path("results/high_volatility_round_number_rejection_reversal_preregistration_2026-08-10.json")
PREREG_SHA = "ccdde7ff713e7a41932339970c1f94422eb99c904e175b7519d9bd5a544919d2"
SUPPORT = Path("results/high_volatility_round_number_rejection_reversal_support_2026-08-10.json")
SUPPORT_SHA = "20fc62594cbbee730e662a43d652fb9c3eefa0b143d4fec245a15de40acfd097"
CLOCK = Path("data/high_volatility_round_number_rejection_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d63d8cd3d0194b2d65558d49d781867db25f76186aac831c90560f86ff8a52d0"
OUTPUT = Path("results/high_volatility_round_number_rejection_reversal_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVRNRR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVRNRR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVRNRR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVRNRR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvrnrr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVRNRR-6 unchanged before economic outcomes",
    }
