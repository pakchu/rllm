"""Sequential strict economics for frozen HVCELVOIS-8."""
...
PREREG = Path("results/high_volatility_caspc_ehlers_oi_joint_sponsorship_preregistration_2026-08-16.json")
PREREG_SHA = "0a17e358f1b0838e5cc4a90556c6e328e98f73279631fa7c0030af071ac523fa"
SUPPORT = Path("results/high_volatility_caspc_ehlers_oi_joint_sponsorship_support_2026-08-16.json")
SUPPORT_SHA = "e7cced91c411218d9d80a7981cf0dcf993d71cdba1c5c96fbc10b618b7cedbe5"
NOVELTY = Path("results/high_volatility_caspc_ehlers_oi_joint_sponsorship_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "79a34cc46ed4311fa2e00eb612a2a0e552013c0487387c19ea9c1d1c47eb778a"
CLOCK = Path("data/high_volatility_caspc_ehlers_oi_joint_sponsorship_clocks_2023_2026.csv.gz")
CLOCK_SHA = "13b3dd6af0c567a4d8b2fac8e4ea92991130d88ce6f7af9e1ceacd6b5d8a7053"
CONTROL_DIR = Path("data/high_volatility_caspc_ehlers_oi_joint_sponsorship_controls_2023_2026")
FREEZE = Path("results/high_volatility_caspc_ehlers_oi_joint_sponsorship_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_caspc_ehlers_oi_joint_sponsorship_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCELVOIS frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCELVOIS novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcelvois_8_sequential_economics_v1", "policy_id": POLICY_ID,
