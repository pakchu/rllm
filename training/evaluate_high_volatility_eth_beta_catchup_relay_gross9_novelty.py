"""Evaluate frozen HVEBCR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVEBCR-12"
PROTOCOL = "hvebcr_12_gross9_novelty_v1"
PREREG = Path(
...
SUPPORT = Path("results/high_volatility_eth_beta_catchup_relay_support_2026-08-09.json")
SUPPORT_SHA = "ca94a63b845e4c982fa532904d114cdbd124d799a727edac08b5520ee3c03f99"
CLOCK = Path("data/high_volatility_eth_beta_catchup_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "9bb26eed5a251f27b9890b539fda6c888fab1551b4efcd78c35109a2f4594205"
OUTPUT = Path(
    "results/high_volatility_eth_beta_catchup_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVEBCR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVEBCR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVEBCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVEBCR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvebcr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVEBCR-12 unchanged before economic outcomes"
        ),
    }
