"""Sequential strict economics for frozen CVVDMR-6."""
...
PREREG = Path("results/cboe_vix_acceleration_dual_confirmation_relay_preregistration_2026-08-08.json")
PREREG_SHA = "d58d3eebb95b69dd87c3b24e9d169cd117be1f63835711c9938447fef13c5c3e"
SUPPORT = Path("results/cboe_vix_acceleration_dual_confirmation_relay_support_2026-08-08.json")
SUPPORT_SHA = "30267913937b9f2032eabdaaf401c7b640ef133ad5e56d1a1e7163c44bd57e14"
NOVELTY = Path("results/cboe_vix_acceleration_dual_confirmation_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "bae3acc00b0b51ac8369b3cdafe2576e0249ae6ba56acb8fc27aa2206146f9b2"
CLOCK = Path("data/cboe_vix_acceleration_dual_confirmation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "624a693cc972a168339fb8251239ab1a2ef78ce88cf0e1040f4340e509bad27d"
CONTROL_DIR = Path("data/cboe_vix_acceleration_dual_confirmation_relay_controls_2023_2026")
FREEZE = Path("results/cboe_vix_acceleration_dual_confirmation_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/cboe_vix_acceleration_dual_confirmation_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CVVDMR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CVVDMR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "cvvdmr_6_sequential_economics_v1", "policy_id": POLICY_ID,
