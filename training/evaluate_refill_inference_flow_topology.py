"""Frozen pre-2024 return evaluator for RIFT-96.

The evaluator must be committed and hash-recorded before it can load execution
prices.  It opens only 2020-2023 outcomes and never tunes the frozen sequence,
clock, action, hold, cost, or qualification rules.
"""
...
PREREGISTRATION_COMMIT = "b9af831b6117f0eb5843fc3ee71d4ddff51c3a39"
SUPPORT_COMMIT = "ca4e6d64f37692d5436c85f6b127da4b293b1cc1"
PREREGISTRATION_SOURCE = Path(
    "training/preregister_refill_inference_flow_topology.py"
)
PREREGISTRATION_SOURCE_SHA256 = (
    "af72edfd14f2cf1ab8840bf2bbc9e94acb80dafd0487b692dd940a6c7920742e"
)
PREREGISTRATION_DOCUMENT = Path(
    "docs/refill-inference-flow-topology-preregistration-2026-07-14.md"
)
PREREGISTRATION_DOCUMENT_SHA256 = (
    "d071723eaeea61ffad910ffdead761d14868013fda3697aa0c9e3e5f1d64c287"
)
SUPPORT_DOCUMENT = Path(
    "docs/refill-inference-flow-topology-support-freeze-2026-07-14.md"
)
SUPPORT_DOCUMENT_SHA256 = (
    "b38f0db0ffede2df455be49d87721a0d8c0d7d68965f0898a013d94d014637d4"
)
EVALUATOR_DOCUMENT = Path(
    "docs/refill-inference-flow-topology-evaluator-freeze-2026-07-14.md"
)
EVALUATOR_DOCUMENT_SHA256 = (
    "873b6741fd893d638e223ae50826810023b7321d52b3312838dd09aecbe8b68c"
)
PREREGISTRATION_RESULT = Path(
    "results/refill_inference_flow_topology_support_2026-07-14.json"
)
PREREGISTRATION_RESULT_SHA256 = (
    "d026ceb2af9d586c5327659cb98d69a4d19ec3bfb8e2357891ac20c8e53a31e6"
)
EVENT_CLOCK = Path("results/refill_inference_flow_topology_clock_2026-07-14.csv")
EVENT_CLOCK_SHA256 = (
...
EVALUATION_SOURCE = Path("training/evaluate_refill_inference_flow_topology.py")
EVALUATION_FREEZE = Path(
    "results/refill_inference_flow_topology_evaluator_freeze_2026-07-14.json"
)

SELECTED_QUANTILE = 0.925
...
POLICY_NAMES = tuple(rift.CONTROL_ACTIONS)
...
class EvaluationConfig:
    output: str = "results/refill_inference_flow_topology_selection_2026-07-14.json"
    leverage: float = 0.5
...
def _validate_evaluation_config(cfg: EvaluationConfig) -> None:
    ...
    if changed:
        raise ValueError(f"RIFT evaluation config is frozen: {changed}")


def verify_evaluation_freeze() -> dict[str, Any]:
    if not EVALUATION_FREEZE.is_file():
        raise ValueError("RIFT evaluator freeze manifest is missing")
    freeze = json.loads(EVALUATION_FREEZE.read_text())
    if freeze.get("outcomes_opened_for_rift96") is not False:
        raise ValueError("RIFT evaluator was not frozen before outcomes")
    if freeze.get("evaluation_source") != str(EVALUATION_SOURCE):
        raise ValueError("RIFT evaluator freeze path changed")
    if freeze.get("evaluation_source_sha256") != _sha256(EVALUATION_SOURCE):
        raise ValueError("RIFT evaluator differs from pre-outcome freeze")
    if len(str(freeze.get("evaluation_source_commit", ""))) != 40:
        raise ValueError("RIFT evaluator source commit is not full length")
    if freeze.get("preregistration_commit") != PREREGISTRATION_COMMIT:
        raise ValueError("RIFT evaluator freeze preregistration changed")
    if freeze.get("support_commit") != SUPPORT_COMMIT:
        raise ValueError("RIFT evaluator freeze support commit changed")
    if freeze.get("support_result_sha256") != PREREGISTRATION_RESULT_SHA256:
        raise ValueError("RIFT evaluator freeze support hash changed")
...
def verify_preregistration() -> dict[str, Any]:
    for path, expected in (
        ...
        if _sha256(path) != expected:
            raise ValueError(f"frozen RIFT dependency changed: {path}")

    result = json.loads(PREREGISTRATION_RESULT.read_text())
    protocol = result.get("protocol", {})
    if protocol.get("outcomes_opened") is not False:
        raise ValueError("RIFT support artifact opened outcomes")
    if protocol.get("support_only") is not True:
        raise ValueError("RIFT support artifact is not support-only")
    if protocol.get("control_actions") != rift.CONTROL_ACTIONS:
        raise ValueError("RIFT control actions changed")
    if result.get("support_decision") != "pass":
        raise ValueError("RIFT support gate did not pass")
    if result.get("selected_quantile") != SELECTED_QUANTILE:
        raise ValueError("RIFT selected quantile changed")
    if result.get("config") != asdict(rift.Config()):
        raise ValueError("RIFT signal config differs from frozen support")
    selected = result.get("selected_support", {})
    if selected.get("passes_support") is not True:
        raise ValueError("RIFT selected support row is not passing")
    if selected.get("support", {}).get("nonoverlap_total") != 460:
        raise ValueError("RIFT selected event count changed")
    ...
    if frozen_protocol.get("outcomes_opened") is not False:
        raise ValueError("RIFT event clock opened outcomes")
    if frozen_protocol.get("preregistration_commit") != PREREGISTRATION_COMMIT:
        raise ValueError("RIFT event clock preregistration changed")
    if frozen_protocol.get("selected_quantile") != SELECTED_QUANTILE:
        raise ValueError("RIFT event clock quantile changed")
    if clock.get("sha256") != EVENT_CLOCK_SHA256 or clock.get("rows") != 460:
        raise ValueError("RIFT event clock record changed")
    if manifest.get("support", {}).get("sha256") != PREREGISTRATION_RESULT_SHA256:
        raise ValueError("RIFT event clock support hash changed")
    return result
...
def _canonical_clock_records(schedule: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    if missing:
        raise ValueError(f"RIFT clock is missing columns: {sorted(missing)}")
...
def verify_signal_replay(
    ...
    signal, controls, _ = rift.classify_sequences(
        frame, cfg, quantile=SELECTED_QUANTILE
    )
    replayed = rift.nonoverlapping_schedule(signal, frame)
    frozen = pd.read_csv(EVENT_CLOCK)
    if _canonical_clock_records(replayed) != _canonical_clock_records(frozen):
        raise ValueError("RIFT primary event-clock replay differs from freeze")
    if not replayed["entry_position"].eq(replayed["signal_position"] + 1).all():
        raise ValueError("RIFT replay is not next-open entry")
    if not replayed["hold_bars"].eq(cfg.hold_bars).all():
        raise ValueError("RIFT replay hold changed")
    if not replayed["exit_position"].eq(
        replayed["entry_position"] + cfg.hold_bars
    ).all():
        raise ValueError("RIFT replay exit changed")
    support = rift._support(replayed, cfg)
    if support != preregistration["selected_support"]["support"]:
        raise ValueError("RIFT selected support replay differs from freeze")
    raw_counts = {
        name: int(mask.sum())
        for name, mask in controls.items()
        if name != "primary"
    }
    if raw_counts != preregistration["selected_support"]["control_raw_counts"]:
        raise ValueError("RIFT control raw clocks differ from freeze")
    if int(controls["primary"].sum()) != preregistration["selected_support"][
        "raw_primary"
    ]:
        raise ValueError("RIFT raw primary clock differs from freeze")
    clock_manifest = json.loads(EVENT_CLOCK_MANIFEST.read_text())
    if source != clock_manifest.get("source"):
        raise ValueError("RIFT source replay differs from clock freeze")
...
def _schedule_from_control(
    ...
    *,
    ...
    if action not in (-1, 1):
        raise ValueError("RIFT control action must be long or short")
    signal = pd.DataFrame(
        {
            "side": np.where(mask.fillna(False), action, 0).astype(np.int8),
    ...
    return rift.nonoverlapping_schedule(signal, schedule_frame).reindex(
...
def build_control_schedules(
    ...
    joint_quarantine = frame["quarantined"].astype(bool)
    spot_quarantine = frame["spot_quarantined"].astype(bool)
    for name in POLICY_NAMES[2:]:
        quarantine = joint_quarantine
        if name == "spot_only":
            quarantine = spot_quarantine
        elif name == "signal_delay_1bar":
            quarantine = joint_quarantine.shift(1, fill_value=True)
        schedules[name] = _schedule_from_control(
            frame,
            cfg,
            mask=controls[name],
            action=rift.CONTROL_ACTIONS[name],
            quarantine=quarantine,
            branch=name,
        )
    if tuple(schedules) != POLICY_NAMES:
        raise ValueError("RIFT evaluator control set differs from preregistration")
    return schedules
...
def slice_schedule(
    ...
    *,
    ...
    if start_timestamp >= end_timestamp:
        raise ValueError("RIFT split start must precede end")
    if schedule.empty:
...
def mean_gross_underlying_move_bp(
    ...
    gross = ((1.0 + mean_net) / (1.0 - per_side_cost) ** 2 - 1.0) / (
        cfg.leverage
    )
...
def evaluate_policy(
    ...
    *,
    ...
    metrics = simulate_schedule(
        frame, schedule, start=start, end=end, cfg=cfg
    )
    ...
    metrics["mean_gross_underlying_move_bp"] = (
        mean_gross_underlying_move_bp(mean_net, cfg)
...
def _qualification(windows: dict[str, Any], cfg: EvaluationConfig) -> dict[str, Any]:
    ...
    primary_min_ratio = min(
        train["cagr_to_strict_mdd"], select["cagr_to_strict_mdd"]
    )
...
def run_evaluation(cfg: EvaluationConfig) -> dict[str, Any]:
    ...
    signal_cfg = rift.Config()
    frame, source = rift.load_causal_frame(signal_cfg)
    controls, primary_schedule = verify_signal_replay(
        frame, signal_cfg, preregistration, source
    )
    schedules = build_control_schedules(
        frame, controls, primary_schedule, signal_cfg
    )
    ...
    return {
        "protocol": {
            "name": "RIFT-96 frozen pre-2024 selection evaluation",
            ...
            "outcomes_opened_for_rift96": True,
            ...
            "control_actions": rift.CONTROL_ACTIONS,
            "entry": "next 5m open after setup plus completed confirmation",
            "exit": "scheduled open 96 completed 5m bars after entry",
        ...
        "selection": {
            "selected_alpha": "rift96" if qualification["qualifies"] else None,
            "rejected": not qualification["qualifies"],
            "reason": (
                "passed every frozen pre-2024 gate"
                if qualification["qualifies"]
                else "RIFT-96 failed at least one frozen pre-2024 gate"
            ),
        },
    }
