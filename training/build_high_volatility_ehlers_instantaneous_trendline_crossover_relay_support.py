"""Build outcome-blind source support for frozen HVEIT-24."""
...
from training import preregister_high_volatility_ehlers_instantaneous_trendline_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "fb2f567d129079ed56bd083ca877511b4a0d54269ce9d51ca3ac322716db8365"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ehlers_instantaneous_trendline_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ehlers_instantaneous_trendline_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ehlers_instantaneous_trendline_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ehlers_instantaneous_trendline_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ehlers_instantaneous_trendline_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "median_price", "itrend", "trigger", "difference",
    "entry_side", "price_cross_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "median_price", "itrend", "trigger", "difference",
    "entry_side", "price_cross_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVEIT source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVEIT invalid source key")
...
def instantaneous_trendline(series: pd.Series, valid: pd.Series) -> pd.DataFrame:
    alpha=float(P["alpha"]);warm=int(P["initialization_observations"]);values=series.to_numpy(float);ok=valid.to_numpy(bool);itrend=np.full(len(series),np.nan);run=0
    for i,(price,is_valid) in enumerate(zip(values,ok)):
        if not is_valid or not math.isfinite(price):run=0;continue
        run+=1
        if run<3:continue
        if run<=warm:itrend[i]=(values[i]+2*values[i-1]+values[i-2])/4.0
        elif all(math.isfinite(x) for x in (values[i-1],values[i-2],itrend[i-1],itrend[i-2])):
            itrend[i]=(alpha-alpha*alpha/4)*values[i]+0.5*alpha*alpha*values[i-1]-(alpha-0.75*alpha*alpha)*values[i-2]+2*(1-alpha)*itrend[i-1]-(1-alpha)*(1-alpha)*itrend[i-2]
    line=pd.Series(itrend,index=series.index);trigger=2*line-line.shift(2);return pd.DataFrame({"itrend":line,"trigger":trigger,"difference":trigger-line},index=series.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    fields=["bar_open","bar_high","bar_low","bar_close"];bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[fields]).all(axis=1)&bars[fields].gt(0).all(axis=1);bars["median_price"]=(bars.bar_high+bars.bar_low)/2;values=instantaneous_trendline(bars.median_price,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["entry_side"]=cross_side(bars.difference);bars["price_cross_side"]=cross_side(bars.median_price-bars.itrend);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["median_price","itrend","trigger","difference","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="price_itrend_cross":side=pd.to_numeric(used.price_cross_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVEIT prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hveit_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hveit_24_source_support_v1", "policy_id": prereg.POLICY_ID,
