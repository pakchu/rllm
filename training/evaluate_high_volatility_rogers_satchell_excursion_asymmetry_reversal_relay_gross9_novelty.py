"""Evaluate frozen HVRSAR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVRSAR-8"
PROTOCOL = "hvrsar_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_rogers_satchell_excursion_asymmetry_reversal_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVRSAR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVRSAR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVRSAR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVRSAR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvrsar_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVRSAR-8 unchanged before economic outcomes",
    }
