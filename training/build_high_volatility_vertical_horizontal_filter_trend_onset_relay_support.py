"""Build outcome-blind source support for frozen HVVHF-24."""
...
from training import preregister_high_volatility_vertical_horizontal_filter_trend_onset_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "4b7aaf52ebe56668442bdf3f5783ef91b91c82dfdff2e2ffcf7f77236367a5d5"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_vertical_horizontal_filter_trend_onset_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_vertical_horizontal_filter_trend_onset_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_vertical_horizontal_filter_trend_onset_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_vertical_horizontal_filter_trend_onset_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_vertical_horizontal_filter_trend_onset_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "close_range", "absolute_travel", "vhf", "net_displacement", "entry_side", "direction_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "close_range", "absolute_travel", "vhf", "net_displacement", "entry_side",
    "direction_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVVHF source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVVHF invalid source key")
...
def vhf_frame(close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    period=P["vhf_periods"];result=np.full((len(close),4),np.nan);window=[]
    for i,(value,is_valid) in enumerate(zip(close.to_numpy(float),valid.to_numpy(bool))):
        if not is_valid or not math.isfinite(value):window=[];continue
        window.append(float(value));window=window[-(period+1):]
        if len(window)<period+1:continue
        recent=window[1:];price_range=max(recent)-min(recent);travel=sum(abs(window[j]-window[j-1]) for j in range(1,len(window)));displacement=window[-1]-window[0]
        result[i]=(price_range,travel,price_range/travel if travel>0 else np.nan,displacement)
    return pd.DataFrame(result,index=close.index,columns=("close_range","absolute_travel","vhf","net_displacement"))
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=bars.join(vhf_frame(bars.bar_close,bars.valid_bar));bars["direction_side"]=np.sign(bars.net_displacement).fillna(0).astype(int);prior=bars.vhf.shift(1);onset=(prior<=P["trend_threshold"])&(bars.vhf>P["trend_threshold"]);bars["entry_side"]=bars.direction_side.where(onset,0)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["close_range","absolute_travel","vhf","net_displacement","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.absolute_travel.gt(0)&bars.net_displacement.ne(0)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="direction_only":side=pd.to_numeric(used.direction_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVVHF prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvvhf_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvvhf_24_source_support_v1", "policy_id": prereg.POLICY_ID,
