...
OUT='results/portfolio_gross6_cost6bp_mdd20_with_dynamic_2026-07-08.json'
DOC='docs/portfolio-gross6-cost6bp-mdd20-with-dynamic-2026-07-08.md'
COST=0.0006
...
def scale_to_mdd(by,years,w,target=19.7):
    ...
    while maxm(hi)<target and sum(w.values())*hi<6:
        hi*=1.25
    hi=min(hi, 6/max(1e-12,sum(w.values())))
    for _ in range(12):
        mid=(lo+hi)/2
...
def candidate_weights(by,years):
    ...
    def add(w):
        ...
        if sum(ww.values())<=0 or sum(ww.values())>6+1e-9: return
    ...
    for _ in range(900):
        ...
        gross=rng.choice([3.5,4.0,4.5,5.0,5.5,6.0])
...
def main():
    ...
    out={'protocol':{'gross_cap':6.0,'cost_each_side':COST,'condition':'test2024/eval2025/ytd2026 strict_mdd_pct<=20 and CAGR/MDD>=5','rank':'among qualified: OOS split return sum, then min ratio/ytd ratio','selection_scope':'seeded/local/random scan over pre-wave live sleeves + wave/volume sleeves + dynamic-exit sleeves'},'sleeves':SLEEVES,'event_counts':{sp:dict(Counter(e['sleeve'] for e in events if e['split']==sp)) for sp in masks},'evaluated_unique':len(rows),'qualified_count':len(qualified),'current_live_reference':{'weights':current_live['weights'],'metrics':current_live['metrics']},'top_qualified':[{k:v for k,v in r.items() if k!='score_tuple'} for r in qualified[:30]],'top_diagnostic':[{k:v for k,v in r.items() if k!='score_tuple'} for r in rows[:30]]}
    Path(OUT).write_text(json.dumps(out,indent=2,ensure_ascii=False))
    md=['# Gross<=6 cost 6bp MDD20 portfolio scan with dynamic sleeves (2026-07-08)','',json.dumps(out['protocol'],ensure_ascii=False),'',f"evaluated_unique={len(rows)}, qualified_count={len(qualified)}",'','## Top qualified','| rank | gross | weights | 2024 ret/CAGR/MDD/ratio | 2025 ret/CAGR/MDD/ratio | 2026 ret/CAGR/MDD/ratio | trades 24/25/26 |','|---:|---:|---|---:|---:|---:|---:|']
