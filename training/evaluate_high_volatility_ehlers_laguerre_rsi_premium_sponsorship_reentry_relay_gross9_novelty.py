"""Evaluate frozen HVELRSIPSC-24 structural novelty against every Gross9 sleeve."""
...
from training import preregister_high_volatility_ehlers_laguerre_rsi_premium_sponsorship_reentry_relay as registration

POLICY = "HVELRSIPSC-24"
PROTOCOL = "hvelrsi_psc_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_ehlers_laguerre_rsi_premium_sponsorship_reentry_relay_gross9_novelty_2026-08-16.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVELRSIPSC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVELRSIPSC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVELRSIPSC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVELRSIPSC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvelrsi_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVELRSIPSC-24 unchanged before economic outcomes",
    }
