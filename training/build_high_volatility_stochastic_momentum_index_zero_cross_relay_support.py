"""Build outcome-blind source support for frozen HVSMI-24."""
...
from training import preregister_high_volatility_stochastic_momentum_index_zero_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "d8fc102b5d888f88cbd2ffda30e3853e65f98caeb8676054ebd12c9e046cb718"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_stochastic_momentum_index_zero_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_stochastic_momentum_index_zero_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_stochastic_momentum_index_zero_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_stochastic_momentum_index_zero_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_stochastic_momentum_index_zero_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "rolling_high", "rolling_low", "relative_range", "range_width",
    "ema1_relative", "ema1_width", "ema2_relative", "ema2_width", "smi",
    "entry_side", "unsmoothed_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "rolling_high", "rolling_low", "relative_range", "range_width",
    "ema1_relative", "ema1_width", "ema2_relative", "ema2_width", "smi",
    "entry_side", "unsmoothed_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVSMI source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVSMI invalid source key")
...
def smi_values(high: pd.Series, low: pd.Series, close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    period=P["k_periods"];continuous=valid.rolling(period,min_periods=period).sum().eq(period)
    rolling_high=high.rolling(period,min_periods=period).max().where(continuous)
    rolling_low=low.rolling(period,min_periods=period).min().where(continuous)
    relative=(close-(rolling_high+rolling_low)/2).where(continuous)
    width=(rolling_high-rolling_low).where(continuous&rolling_high.gt(rolling_low))
    ok=np.isfinite(relative)&np.isfinite(width)
    ema1_relative=causal_ema(relative,ok,P["d_periods"]);ema1_width=causal_ema(width,ok,P["d_periods"])
    ok2=np.isfinite(ema1_relative)&np.isfinite(ema1_width)
    ema2_relative=causal_ema(ema1_relative,ok2,P["d_periods"]);ema2_width=causal_ema(ema1_width,ok2,P["d_periods"])
    smi=(200*ema2_relative/ema2_width).where(ema2_width.gt(0))
    return pd.DataFrame({"rolling_high":rolling_high,"rolling_low":rolling_low,"relative_range":relative,"range_width":width,"ema1_relative":ema1_relative,"ema1_width":ema1_width,"ema2_relative":ema2_relative,"ema2_width":ema2_width,"smi":smi},index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    values=smi_values(bars.bar_high,bars.bar_low,bars.bar_close,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["entry_side"]=cross_side(bars.smi);bars["unsmoothed_side"]=cross_side(bars.relative_range)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["rolling_high","rolling_low","relative_range","range_width","ema1_relative","ema1_width","ema2_relative","ema2_width","smi","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.ema2_width.gt(0)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="unsmoothed_midpoint_cross":side=pd.to_numeric(used.unsmoothed_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVSMI prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvsmie_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvsmie_24_source_support_v1", "policy_id": prereg.POLICY_ID,
