"""Build outcome-blind source support for frozen HVERB-24."""
...
from training import preregister_high_volatility_elder_ray_balance_zero_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "d35ab818b96134125d45d33e50c3d4b8d00d5c6a5b2a4a53713fcec0d57c63d7"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_elder_ray_balance_zero_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_elder_ray_balance_zero_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_elder_ray_balance_zero_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_elder_ray_balance_zero_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_elder_ray_balance_zero_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "ema", "bull_power", "bear_power", "balance",
    "close_ema_difference", "entry_side", "close_ema_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "ema", "bull_power", "bear_power",
    "balance", "close_ema_difference", "entry_side", "close_ema_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVERB source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVERB invalid source key")
...
def elder_values(bars: pd.DataFrame, valid: pd.Series) -> pd.DataFrame:
    ema=causal_ema(bars.bar_close,valid,P["ema_periods"]);bull=(bars.bar_high-ema).where(valid);bear=(bars.bar_low-ema).where(valid)
    return pd.DataFrame({"ema":ema,"bull_power":bull,"bear_power":bear,"balance":bull+bear,"close_ema_difference":bars.bar_close-ema},index=bars.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    values=elder_values(bars,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["entry_side"]=cross_side(bars.balance);bars["close_ema_side"]=cross_side(bars.close_ema_difference)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["ema","bull_power","bear_power","balance","close_ema_difference","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="close_ema_cross":side=pd.to_numeric(used.close_ema_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVERB prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hverb_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hverb_24_source_support_v1", "policy_id": prereg.POLICY_ID,
