"""Evaluate frozen HVBDRR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVBDRR-8"
PROTOCOL = "hvbdrr_8_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_btc_dominance_rotation_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVBDRR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVBDRR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVBDRR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVBDRR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvbdrr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVBDRR-8 unchanged before economic outcomes"
        ),
    }
