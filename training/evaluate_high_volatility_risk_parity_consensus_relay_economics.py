"""Sequential strict economics for frozen HVRPC-24."""
...
PREREG = Path("results/high_volatility_risk_parity_consensus_relay_preregistration_2026-08-10.json")
PREREG_SHA = "fbf24f786677c02ad63e91329c96487cebe545e61cb4d31500114203d54ca8da"
SUPPORT = Path("results/high_volatility_risk_parity_consensus_relay_support_2026-08-10.json")
SUPPORT_SHA = "2924dce337e8704e27c4a696b7dfa724c405276229bace588db469b7418ead68"
NOVELTY = Path("results/high_volatility_risk_parity_consensus_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "ee23a11a0964594115840e0f3f670ef84768f104a70c18b5f04a219ae597faf7"
CLOCK = Path("data/high_volatility_risk_parity_consensus_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "27c4136fd5b857b1e32fc773c95d67338a7701d5ae552c61f96fd543e908c651"
CONTROL_DIR = Path("data/high_volatility_risk_parity_consensus_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_risk_parity_consensus_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_risk_parity_consensus_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVRPC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVRPC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrpc_24_sequential_economics_v1", "policy_id": POLICY_ID,
