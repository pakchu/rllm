"""Sequential strict economics for frozen HVLWRB-24."""
...
PREREG = Path("results/high_volatility_larry_williams_range_breakout_relay_preregistration_2026-08-11.json")
PREREG_SHA = "209f543519ebf60adf54356bca8f308fea7e0a5c9d62ab72a44df47b6e1260ab"
SUPPORT = Path("results/high_volatility_larry_williams_range_breakout_relay_support_2026-08-11.json")
SUPPORT_SHA = "05d146ab4f0b2bef26911cefb1399bdabf4ee9e0375608f1d7c3f7f26d713abd"
NOVELTY = Path("results/high_volatility_larry_williams_range_breakout_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "9a590f13e1101ab35d3eebd3250a70533ff217e963071f2673ed41992287a5a1"
CLOCK = Path("data/high_volatility_larry_williams_range_breakout_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "5dfb290b42c8902f9195a2a966a0383654b3b4e7a0d5a1f6371b9ff1501d14fa"
CONTROL_DIR = Path("data/high_volatility_larry_williams_range_breakout_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_larry_williams_range_breakout_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_larry_williams_range_breakout_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "prior_day_body_projection", "one_hour_stale_crossing", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVLWRB frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVLWRB novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvlwrb_24_sequential_economics_v1", "policy_id": POLICY_ID,
