"""Evaluate frozen HVCE-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCE-24"
PROTOCOL = "hvce_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_chandelier_exit_state_flip_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCE predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCE novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCE predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCE primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvce_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCE-24 unchanged before economic outcomes",
    }
