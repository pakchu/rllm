"""Build outcome-blind source support for frozen HVREI-24."""
...
START = pd.Timestamp("2023-05-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "d922f1b7011d44f24afa509b4e39eb60e75e2810d43c5d5318e228ea6a910106"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_range_expansion_index_reentry_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_range_expansion_index_reentry_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_range_expansion_index_reentry_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_range_expansion_index_reentry_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_range_expansion_index_reentry_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "range_difference", "denominator_term", "high_filter_zero",
    "low_filter_zero", "admitted", "numerator_sum", "denominator_sum", "rei",
    "unfiltered_rei", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "range_difference", "denominator_term",
    "numerator_sum", "denominator_sum", "rei", "unfiltered_rei", "variation",
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVREI source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVREI invalid source key")
...
def td_rei(high: pd.Series, low: pd.Series, close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    required = valid.copy()
    for lag in (2, 5, 6, 7, 8):
        required &= valid.shift(lag, fill_value=False)
    range_difference = ((high - high.shift(2)) + (low - low.shift(2))).where(required)
    denominator_term = ((high - high.shift(2)).abs() + (low - low.shift(2)).abs()).where(required)
    high_filter_zero = (
        high.shift(2).lt(close.shift(7))
        & high.shift(2).lt(close.shift(8))
        & high.lt(high.shift(5))
        & high.lt(high.shift(6))
    ).where(required)
    low_filter_zero = (
        low.shift(2).gt(close.shift(7))
        & low.shift(2).gt(close.shift(8))
        & low.gt(low.shift(5))
        & low.gt(low.shift(6))
    ).where(required)
    admitted = (~(high_filter_zero.eq(True) | low_filter_zero.eq(True))).where(required)
    numerator_term = range_difference.where(admitted.eq(True), 0.0).where(required)
    period = P["rei_periods"]
    numerator_sum = numerator_term.rolling(period, min_periods=period).sum()
    unfiltered_sum = range_difference.rolling(period, min_periods=period).sum()
    denominator_sum = denominator_term.rolling(period, min_periods=period).sum()
    rei = (100.0 * numerator_sum / denominator_sum).where(denominator_sum.gt(0))
    unfiltered_rei = (100.0 * unfiltered_sum / denominator_sum).where(denominator_sum.gt(0))
    return pd.DataFrame(
        {
            "range_difference": range_difference,
            "denominator_term": denominator_term,
            "high_filter_zero": high_filter_zero,
            "low_filter_zero": low_filter_zero,
            "admitted": admitted,
            "numerator_sum": numerator_sum,
            "denominator_sum": denominator_sum,
            "rei": rei,
            "unfiltered_rei": unfiltered_rei,
        },
        index=high.index,
    )
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[["bar_open","bar_high","bar_low","bar_close"]]).all(axis=1)&bars[["bar_open","bar_high","bar_low","bar_close"]].gt(0).all(axis=1);bars=pd.concat([bars,td_rei(bars.bar_high,bars.bar_low,bars.bar_close,bars.valid_bar)],axis=1);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["range_difference","denominator_term","numerator_sum","denominator_sum","rei","unfiltered_rei","variation"]]).all(axis=1)&bars.denominator_sum.gt(0)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.rei.shift(1);long=previous.le(P["lower_level"])&panel.rei.gt(P["lower_level"]);short=previous.ge(P["upper_level"])&panel.rei.lt(P["upper_level"]);panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&(long|short)&panel.variation_rank.ge(P["variation_rank_min"]);return panel.loc[:,PANEL_COLUMNS]


def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);value=pd.to_numeric(used.rei,errors="coerce");
    if control=="unfiltered_rei":value=pd.to_numeric(used.unfiltered_rei,errors="coerce")
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVREI prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvrei_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvrei_24_source_support_v1", "policy_id": prereg.POLICY_ID,
