"""Sequential strict economics for frozen HVPVO-12."""
...
PREREG = Path("results/high_volatility_percentage_volume_oscillator_ignition_relay_preregistration_2026-08-11.json")
PREREG_SHA = "fbfd95ca5e8d67072ee61d3dfc15613d12f95bcf03aa6684f5dc143c0715ada2"
SUPPORT = Path("results/high_volatility_percentage_volume_oscillator_ignition_relay_support_2026-08-11.json")
SUPPORT_SHA = "e3ec1846c0e2e5b2768964326ae464e380887bc4ad86d856399813cb26760bfd"
NOVELTY = Path("results/high_volatility_percentage_volume_oscillator_ignition_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "9955a020d821f2d586d0caae8ecbcefdc0759079b2cc826095c5f65ac1d0c8e1"
CLOCK = Path("data/high_volatility_percentage_volume_oscillator_ignition_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "f44bff2ab0475265e1e338c9aa2b0096da307fd7327528184395363795877f96"
CONTROL_DIR = Path("data/high_volatility_percentage_volume_oscillator_ignition_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_percentage_volume_oscillator_ignition_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_percentage_volume_oscillator_ignition_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "zero_line_ignition", "one_bar_stale_cross", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVPVO frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVPVO novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvpvo_12_sequential_economics_v1", "policy_id": POLICY_ID,
