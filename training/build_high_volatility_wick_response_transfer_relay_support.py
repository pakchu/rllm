"""Build outcome-blind source support for frozen HVWRTR-8."""
...
from training import preregister_high_volatility_wick_response_transfer_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-04-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="b94e3cc19df075ef00da6084f6bf92a63d0acca6da9d9596e9579773150b1a98"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_wick_response_transfer_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_wick_response_transfer_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_wick_response_transfer_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_wick_response_transfer_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_wick_response_transfer_relay_support_2026-08-10.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","wick_response","response_rank","final_hour_pressure","realized_variation","variation_rank","transfer_signal","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","wick_response","response_rank","final_hour_pressure","realized_variation","variation_rank","transfer_signal")
...
def prior_rank(series:pd.Series)->pd.Series:
 ...
 for i,value in enumerate(values):
  prior=np.asarray(history[-P["history_hours"]:],float)
  if math.isfinite(value) and len(prior)>=P["minimum_history_hours"]: out[i]=(np.sum(prior<value)+.5*np.sum(prior==value))/len(prior)
  if math.isfinite(value): history.append(float(value))
 return pd.Series(out,index=series.index)

def wick_statistics(block:pd.DataFrame)->tuple[float,float,float,float]:
 groups=np.arange(len(block))//5
 five=block.groupby(groups,sort=True).agg(open=("open","first"),high=("high","max"),low=("low","min"),close=("close","last"))
 if len(five)!=96 or not np.isfinite(five).all().all() or not five.gt(0).all().all():return (math.nan,)*4
 upper=np.log(five.high.to_numpy(float)/np.maximum(five.open.to_numpy(float),five.close.to_numpy(float)))
 lower=np.log(np.minimum(five.open.to_numpy(float),five.close.to_numpy(float))/five.low.to_numpy(float))
 wick=lower-upper;returns=np.log(five.close.to_numpy(float)/five.open.to_numpy(float))
 if float(np.std(wick[:83],ddof=1))<=0 or float(np.std(returns[1:84],ddof=1))<=0:return (math.nan,)*4
 response=float(np.corrcoef(wick[:83],returns[1:84])[0,1]);pressure=float(wick[84:].sum())
 minute_returns=np.log(block.close.to_numpy(float)/block.open.to_numpy(float));variation=float(np.sqrt(np.square(minute_returns).sum()));signal=float(response*pressure)
 return response,pressure,variation,signal
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVWRTR source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVWRTR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("8h"),END,freq="1h",inclusive="left"):
  ...
  if path_valid:
   response,pressure,var,signal=wick_statistics(block);valid=all(math.isfinite(v) for v in (response,pressure,var,signal)) and var>0 and response!=0 and pressure!=0 and signal!=0
  else:response=pressure=var=signal=math.nan;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"wick_response":response,"final_hour_pressure":pressure,"realized_variation":var,"transfer_signal":signal})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["response_rank"]=prior_rank(panel.wick_response.abs().where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));panel["eligible"]=valid&panel.response_rank.ge(P["response_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"]);panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_hour_stale_transfer":
  used[["wick_response","response_rank"]]=panel[["wick_response","response_rank"]].shift(1)
  used["transfer_signal"]=used.wick_response*used.final_hour_pressure
 valid=used.source_valid.eq(True)&pd.to_numeric(used.transfer_signal,errors="coerce").notna();strong=used.response_rank.ge(P["response_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&strong&variation
 if control=="no_response_tail":state=valid&variation
 elif control=="no_variation_gate":state=valid&strong
 onset=previous_valid_onset(state,valid);side=np.sign(pd.to_numeric(used.transfer_signal,errors="coerce").fillna(0)).astype(int)
 if control=="late_wick_pressure_only":side=np.sign(pd.to_numeric(used.final_hour_pressure,errors="coerce").fillna(0)).astype(int)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVWRTR prereg drift")
