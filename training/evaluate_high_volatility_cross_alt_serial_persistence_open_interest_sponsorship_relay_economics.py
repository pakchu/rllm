"""Sequential strict economics for frozen HVCASPCOIS-8."""
...
POLICY_ID = "HVCASPCOIS-8"
BAR = pd.Timedelta(minutes=5)
LEVERAGE = 0.5
BASE_COST = 0.0006
STRESS_COST = 0.0010
ENV_FILE = "/home/pakchu/rllm/.env"
PREREG = Path("results/high_volatility_cross_alt_serial_persistence_open_interest_sponsorship_relay_preregistration_2026-08-16.json")
PREREG_SHA = "ae55ad43aeee388bf80eb33963a9fac32a3f116ac906e8b1b73a51041b7f6e69"
SUPPORT = Path("results/high_volatility_cross_alt_serial_persistence_open_interest_sponsorship_relay_support_2026-08-16.json")
SUPPORT_SHA = "1afcd0a3551c1484aef35c2b3206daf099d5f200a808b1bd9dbc7b4be8d789f0"
NOVELTY = Path("results/high_volatility_cross_alt_serial_persistence_open_interest_sponsorship_relay_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "37d4ace0085b3548118c1dbe584ba38b087606bb4973f2dbd6f27df64ec97cf0"
CLOCK = Path("data/high_volatility_cross_alt_serial_persistence_open_interest_sponsorship_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "e12b5a576cbf7c29e6c2ca510036c6152b95ed73bc5c8defa487d8aef39d9fbe"
CONTROL_DIR = Path("data/high_volatility_cross_alt_serial_persistence_open_interest_sponsorship_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_cross_alt_serial_persistence_open_interest_sponsorship_relay_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_alt_serial_persistence_open_interest_sponsorship_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ("no_open_interest_sponsorship_gate", "no_persistence_tail", "no_variation_gate", "negative_autocorrelation_consensus", "one_block_stale_persistence", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCASPCOIS frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCASPCOIS novelty did not authorize economics")
...
def load_clock_allow_empty(
    path: Path,
    split: str,
    start: pd.Timestamp,
    end: pd.Timestamp,
) -> pd.DataFrame:
    """Load a frozen clock while preserving valid zero-row diagnostic controls."""
    ...
    return legacy.load_clock(path, split, start, end)
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    primary_clock = load_clock_allow_empty(CLOCK, split, start, end)
    ...
    for name in CONTROLS:
        ...
        controls[name] = evaluate_control(
            load_clock_allow_empty(path, split, start, end),
            market,
            funding,
            start,
            end,
    ...
    core = {
        "protocol_version": "hvcaspc_ois_8_sequential_economics_v1", "policy_id": POLICY_ID,
        ...
        "advance_to_post_stage_volatility_audit": passed and stage == "final",
        "decision": (
            "terminal_reject_no_repair"
            if not passed
            else "pass_to_post_stage_volatility_audit"
            if stage == "final"
            else "pass"
        ),
    }
