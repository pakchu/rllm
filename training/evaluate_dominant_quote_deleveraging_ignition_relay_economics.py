"""Sequential strict economics for frozen DQDIR-6."""
...
PREREG = Path("results/dominant_quote_deleveraging_ignition_relay_preregistration_2026-08-08.json")
PREREG_SHA = "00b0342257dc29604cd2106099bac7541efafd8e7778cb241c30353603970b28"
SUPPORT = Path("results/dominant_quote_deleveraging_ignition_relay_support_2026-08-08.json")
SUPPORT_SHA = "918c66e59f620f7191158a66e9129e87a8a0726506d4956c27a16028bae4090e"
NOVELTY = Path("results/dominant_quote_deleveraging_ignition_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "2d80e478a054d88ad99309fe29fc17195af58676976dc977aea5417103d04fa2"
CLOCK = Path("data/dominant_quote_deleveraging_ignition_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "304f4c27ccafb60e396b39a8cf054409ccc5ca7fdc382d4fbd11f7b429dd87e7"
CONTROL_DIR = Path("data/dominant_quote_deleveraging_ignition_relay_controls_2023_2026")
FREEZE = Path("results/dominant_quote_deleveraging_ignition_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/dominant_quote_deleveraging_ignition_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
CONTROLS = ("no_joint_expansion", "no_oi_deleveraging", "no_alternative_quiet", "one_hour_stale_flow", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("DQDIR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("DQDIR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "dqdir_6_sequential_economics_v1", "policy_id": POLICY_ID,
