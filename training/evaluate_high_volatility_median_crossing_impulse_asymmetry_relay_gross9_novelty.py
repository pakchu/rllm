"""Evaluate frozen HVMCIA-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVMCIA-8"
PROTOCOL = "hvmcia_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_median_crossing_impulse_asymmetry_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVMCIA predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVMCIA novelty limits drift")
    if (
        ...
        raise RuntimeError("HVMCIA predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVMCIA primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVMCIA-8 unchanged before economic outcomes",
    }
