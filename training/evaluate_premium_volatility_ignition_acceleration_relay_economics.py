"""Sequential strict economics for frozen PVIAR-6."""
...
NOVELTY = Path("results/premium_volatility_ignition_acceleration_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "71c6ec6cf33729ef8e513452441eec707fbf7b46e040b5163e598249d3b0461b"
...
FREEZE = Path("results/premium_volatility_ignition_acceleration_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/premium_volatility_ignition_acceleration_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
CONTROLS = ("no_joint_expansion", "no_first_half_tail", "no_same_direction", "no_acceleration", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("PVIAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("PVIAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "pviar_6_sequential_economics_v1", "policy_id": POLICY_ID,
