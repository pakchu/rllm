"""Strict staged economics for the singleton HVRC-8 policy."""
...
PREREG = Path("results/high_volatility_robust_pair_concordance_preregistration_2026-08-16.json")
PREREG_SHA = "796b4de24bb28d1a1a465578435f5799ed395a6fd0de94d63ca4fe046e01bc1d"
SUPPORT = Path("results/high_volatility_robust_pair_concordance_support_2026-08-16.json")
SUPPORT_SHA = "414226114510035151d5d86ca9533e163d4817079687c3ae6ea499de446c720b"
GROSS9 = Path("results/high_volatility_robust_pair_concordance_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "f9b9a666e003c3758f4e5dee00a8eabebc6083b69b733490ced2e5987d853eb2"
CLOCK = {
    "path": Path("data/high_volatility_robust_pair_concordance_clocks_2023_2026.csv.gz"),
    "sha256": "3d616adee222de68e2bdf0d792773ef96ed599f661730224f7a1a5a754add0a5",
    "rows": 71,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_robust_pair_concordance_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVRC-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVRC-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVRC-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVRC-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVRC-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVRC-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVRC-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVRC-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVRC-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVRC-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVRC-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrc_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
