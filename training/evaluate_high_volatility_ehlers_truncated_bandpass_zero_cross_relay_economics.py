"""Sequential strict economics for frozen HVTBP-24."""
...
PREREG = Path("results/high_volatility_ehlers_truncated_bandpass_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "635b8a5ea8ff4743cc46a4b4121067b8203f4ddad2c45d39bc7ee4af8fef675c"
SUPPORT = Path("results/high_volatility_ehlers_truncated_bandpass_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "24103a73092f46f34161e31a09098746f6d9d0e29c236b95918bd27633a7f067"
NOVELTY = Path("results/high_volatility_ehlers_truncated_bandpass_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "aab4182215715e9bcfde98d904c1adcfaad8372c30219f8947a7db384c76c1b7"
CLOCK = Path("data/high_volatility_ehlers_truncated_bandpass_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "dfa37f255ef726d27414a185d32dd7b1f2c69b3a30c60763deb0e55d99515651"
CONTROL_DIR = Path("data/high_volatility_ehlers_truncated_bandpass_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_ehlers_truncated_bandpass_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ehlers_truncated_bandpass_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "standard_bandpass_zero_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTBP frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTBP novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvtbp_24_sequential_economics_v1", "policy_id": POLICY_ID,
