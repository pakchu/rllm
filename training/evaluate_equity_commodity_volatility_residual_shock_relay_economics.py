"""Sequential strict economics for frozen ECVRS-12."""
...
PREREG = Path("results/equity_commodity_volatility_residual_shock_relay_preregistration_2026-08-09.json")
PREREG_SHA = "c4ec96adc9bfe4bb762ddda13b005611c774720171de45126f973b43d24a7290"
SUPPORT = Path("results/equity_commodity_volatility_residual_shock_relay_support_2026-08-09.json")
SUPPORT_SHA = "ca5080d27fd0f98f079f31c2c86f5e1797c310aec5071916881d8e3510bbf9bb"
NOVELTY = Path("results/equity_commodity_volatility_residual_shock_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "e63e1279828a6418b655918d54756d2434809466b58310fc7b38402ec8396a23"
CLOCK = Path("data/equity_commodity_volatility_residual_shock_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "0d701b2b63484528089bf2d6ab3b10fe68c3379364a2621777210c0f92683c44"
CONTROL_DIR = Path("data/equity_commodity_volatility_residual_shock_relay_controls_2023_2026")
FREEZE = Path("results/equity_commodity_volatility_residual_shock_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/equity_commodity_volatility_residual_shock_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "no_residual_tail", "vix_minus_gvz", "one_session_stale_residual", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("ECVRS frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("ECVRS novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "ecvrs_12_sequential_economics_v1", "policy_id": POLICY_ID,
