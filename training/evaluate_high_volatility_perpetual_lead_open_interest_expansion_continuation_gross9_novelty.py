"""Evaluate the frozen HVPLOIEC-8 action-vote clock against Gross9."""
...
from training import preregister_high_volatility_perpetual_lead_open_interest_expansion_continuation as prereg

POLICY = "HVPLOIEC-8"
PROTOCOL = "hvploiec_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_perpetual_lead_open_interest_expansion_continuation_clocks_2020_2026.csv.gz",
    "sha256": "2c905d399320bc3be7cc1fc4649e8be7d077d60668bb8bdc17dd9923455aad0f",
    "rows": 286,
}
OUTPUT = Path("results/high_volatility_perpetual_lead_open_interest_expansion_continuation_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVPLOIEC-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVPLOIEC-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVPLOIEC-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVPLOIEC-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVPLOIEC-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVPLOIEC-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVPLOIEC-8 unchanged before economics",
    }
