"""Strict staged economics for the singleton HVDEMWMV-24 policy."""
...
PREREG = Path("results/high_volatility_demarker_weekly_momentum_active_veto_preregistration_2026-08-17.json")
PREREG_SHA = "6209b6cee86acfe3d2e68d26368426fd9153031a1fd33a37253e9d55170ec5cf"
SUPPORT = Path("results/high_volatility_demarker_weekly_momentum_active_veto_support_2026-08-17.json")
SUPPORT_SHA = "a699c3d8d0b21f0cb3d8784e4bbc2556cef1b0780b24bd8983a2a70f101816d5"
GROSS9 = Path("results/high_volatility_demarker_weekly_momentum_active_veto_gross9_novelty_2026-08-17.json")
GROSS9_SHA = "eb831ff0204a3497863377d14b76e155e329d3b80b92fed95cf29267f8ee6cc3"
CLOCK = {
    "path": Path("data/high_volatility_demarker_weekly_momentum_active_veto_clocks_2023_2026.csv.gz"),
    "sha256": "fd0ae6e974f5d0f3026d62fed76eed07fdb19af5e3e242458fdbcac59729a12a",
    "rows": 110,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_demarker_weekly_momentum_active_veto_{stage}_economics_2026-08-17.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVDEMWMV-24 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVDEMWMV-24 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVDEMWMV-24 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVDEMWMV-24 clock hash drift")
    ...
    if (
        ...
        or preregistration.get("single_candidate_only") is not True
        or preregistration.get("economic_gates", {}).get("weekly_signflip_one_sided_p_max")
        != NORMAL_WEEKLY_P_MAX
    ):
        raise RuntimeError("HVDEMWMV-24 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVDEMWMV-24 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVDEMWMV-24 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVDEMWMV-24 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVDEMWMV-24 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVDEMWMV-24 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVDEMWMV-24 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdemwmv_24_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
