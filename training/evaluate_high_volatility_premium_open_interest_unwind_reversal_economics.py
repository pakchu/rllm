"""Sequential strict economics for frozen HVPOIUR-8."""
...
PREREG = Path("results/high_volatility_premium_open_interest_unwind_reversal_preregistration_2026-08-11.json")
PREREG_SHA = "b69c04df556938efd5bb3856f65407f90c64dc10b5ad205a6d104e440475b75c"
SUPPORT = Path("results/high_volatility_premium_open_interest_unwind_reversal_support_2026-08-11.json")
SUPPORT_SHA = "27c907be3694df3bed243291c83eb09b7ba1b5663e4d7c932c101843a5f14766"
NOVELTY = Path("results/high_volatility_premium_open_interest_unwind_reversal_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "f81aefbecfef725e596b59d9f87fd130acc378827226077ae053855cf40fa1ef"
CLOCK = Path("data/high_volatility_premium_open_interest_unwind_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "537904e52cbf3de0048c7e283bb5caf09a646875eca4e2558b975375be9cdb0b"
CONTROL_DIR = Path("data/high_volatility_premium_open_interest_unwind_reversal_controls_2023_2026")
FREEZE = Path("results/high_volatility_premium_open_interest_unwind_reversal_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_premium_open_interest_unwind_reversal_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVPOIUR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVPOIUR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvpoiur_8_sequential_economics_v1", "policy_id": POLICY_ID,
