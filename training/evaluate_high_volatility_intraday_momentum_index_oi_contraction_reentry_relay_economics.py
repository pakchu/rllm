"""Sequential strict economics for frozen HVIMIOICR-24."""
...
POLICY_ID = "HVIMIOICR-24"
BAR = pd.Timedelta(minutes=5)
LEVERAGE = 0.5
BASE_COST = 0.0006
STRESS_COST = 0.0010
ENV_FILE = "/home/pakchu/rllm/.env"
PREREG = Path("results/high_volatility_intraday_momentum_index_oi_contraction_reentry_relay_preregistration_2026-08-16.json")
PREREG_SHA = "8e63251d732cc3277fb02d0e30fed0b482f0f88c8679ba4287e8c05b2b464464"
SUPPORT = Path("results/high_volatility_intraday_momentum_index_oi_contraction_reentry_relay_support_2026-08-16.json")
SUPPORT_SHA = "15b5a61178491dae1fdbe7a027a0e7236c4ce62d66843a487b8730118078bcde"
NOVELTY = Path("results/high_volatility_intraday_momentum_index_oi_contraction_reentry_relay_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "c63c4fc303cb3f5a032f5e0a8e2779c71428baa8fb99842bc8bde8681efaecb6"
CLOCK = Path("data/high_volatility_intraday_momentum_index_oi_contraction_reentry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "9f78a04d87c31344040ce2eaea1b49fc47c5787eace031a4a8dfd0dc1fa8c5a5"
CONTROL_DIR = Path("data/high_volatility_intraday_momentum_index_oi_contraction_reentry_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_intraday_momentum_index_oi_contraction_reentry_relay_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_intraday_momentum_index_oi_contraction_reentry_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ("no_open_interest_contraction_gate", "no_variation_gate", "outward_crossing", "one_bar_stale_reentry", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVIMIOICR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVIMIOICR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvimi_oicr_24_sequential_economics_v1", "policy_id": POLICY_ID,
