"""Frozen pre-2024 return evaluator for UMFR-36.
...
CLOCK_COMMIT = "46eda96"
PREREGISTRATION_SOURCE = Path("training/preregister_um_forced_flow_reversion.py")
...
SUPPORT_RESULT = Path("results/um_forced_flow_reversion_support_2026-07-14.json")
SUPPORT_RESULT_SHA256 = (
    "22a6cabe015020fa427a660d611917590a95d1212426be1d391476ddce77d3ba"
)
EVENT_CLOCK = Path("results/um_forced_flow_reversion_clock_2026-07-14.csv")
EVENT_CLOCK_SHA256 = "ce86e517fe42d2f1b5a2f45594e280e9484cca312a270e19f0122ebe454ab53c"
EVENT_CLOCK_MANIFEST = Path(
    "results/um_forced_flow_reversion_clock_manifest_2026-07-14.json"
)
EVENT_CLOCK_MANIFEST_SHA256 = (
    "a77609a80ecbc124a8fa10542a20c12dd958eb28ec83c659604d8510e84ffbd2"
)
CLOCK_FREEZE_SOURCE = Path("training/freeze_um_forced_flow_reversion_clock.py")
CLOCK_FREEZE_SOURCE_SHA256 = (
    "5b90c31462b5d7fd276e1ca81a140c6c4691341f85d30c1166a2a6d353786f07"
)
...
EVALUATION_SOURCE = Path("training/evaluate_um_forced_flow_reversion.py")
EVALUATION_FREEZE = Path(
    "results/um_forced_flow_reversion_evaluator_freeze_2026-07-14.json"
)
...
POLICY_NAMES = tuple(umfr.CONTROL_SIDE_RULES)
SCORE_BEARING_CONTROLS = umfr.SCORE_BEARING_CONTROLS
FALSIFICATION_CONTROLS = ("direction_flip", "signal_delay_1bar")
CLOCK_COLUMNS = umfr.SCHEDULE_COLUMNS


@dataclass(frozen=True)
class EvaluationConfig:
    output: str = "results/um_forced_flow_reversion_selection_2026-07-14.json"
    leverage: float = 0.5
    ...
    minimum_2023_half_trades: int = 100
...
def _validate_evaluation_config(cfg: EvaluationConfig) -> None:
    ...
    if changed:
        raise ValueError(f"UMFR evaluation config is frozen: {changed}")


def verify_evaluation_freeze() -> dict[str, Any]:
    if not EVALUATION_FREEZE.is_file():
        raise ValueError("UMFR evaluator freeze manifest is missing")
    freeze = _read_json(EVALUATION_FREEZE)
    if freeze.get("outcomes_opened_for_umfr36") is not False:
        raise ValueError("UMFR evaluator was not frozen before outcomes")
    if freeze.get("evaluation_source") != str(EVALUATION_SOURCE):
        raise ValueError("UMFR evaluator freeze path changed")
    if freeze.get("evaluation_source_sha256") != _sha256(EVALUATION_SOURCE):
        raise ValueError("UMFR evaluator differs from pre-outcome freeze")
    if len(str(freeze.get("evaluation_source_commit", ""))) != 40:
        raise ValueError("UMFR evaluator source commit is not full length")
    expected_values = {
    ...
    for key, expected in expected_values.items():
        if freeze.get(key) != expected:
            raise ValueError(f"UMFR evaluator freeze {key} changed")
    if freeze.get("opened_windows") != []:
        raise ValueError("UMFR evaluator freeze already opened a window")
    if freeze.get("returns_prices_or_funding_loaded_during_freeze") is not False:
        raise ValueError("UMFR evaluator freeze loaded outcomes")
    if freeze.get("mutable_parameters") != []:
        raise ValueError("UMFR evaluator freeze permits mutable parameters")
    expected_sealed = [*WINDOWS, "test2024", "eval2025", "ytd2026"]
    if freeze.get("sealed_windows") != expected_sealed:
        raise ValueError("UMFR evaluator freeze sealed windows changed")
    return freeze
...
def verify_preregistration() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    for path, expected in frozen_files:
        if _sha256(path) != expected:
            raise ValueError(f"frozen UMFR dependency changed: {path}")

    result = _read_json(SUPPORT_RESULT)
    protocol = result.get("protocol", {})
    if protocol.get("umfr_outcomes_opened") is not False:
        raise ValueError("UMFR support artifact opened outcomes")
    if protocol.get("support_only") is not True:
        raise ValueError("UMFR support artifact is not support-only")
    if protocol.get("control_side_rules") != umfr.CONTROL_SIDE_RULES:
        raise ValueError("UMFR control side rules changed")
    if result.get("support_decision") != "pass":
        raise ValueError("UMFR support gate did not pass")
    if result.get("selected_quantile") != SELECTED_QUANTILE:
        raise ValueError("UMFR selected quantile changed")
    if result.get("config") != asdict(umfr.Config()):
        raise ValueError("UMFR signal config differs from frozen support")
    selected = result.get("selected_support", {})
    if selected.get("passes_support") is not True:
        raise ValueError("UMFR selected support row is not passing")
    if selected.get("support", {}).get("nonoverlap_total") != 915:
        raise ValueError("UMFR selected event count changed")
    ...
    if frozen_protocol.get("outcomes_opened") is not False:
        raise ValueError("UMFR event clock opened outcomes")
    if frozen_protocol.get("preregistration_commit") != PREREGISTRATION_COMMIT:
        raise ValueError("UMFR event clock preregistration changed")
    if frozen_protocol.get("support_commit") != SUPPORT_COMMIT:
        raise ValueError("UMFR event clock support commit changed")
    if frozen_protocol.get("selected_quantile") != SELECTED_QUANTILE:
        raise ValueError("UMFR event clock quantile changed")
    if clock.get("sha256") != EVENT_CLOCK_SHA256 or clock.get("rows") != 915:
        raise ValueError("UMFR event clock record changed")
    if clock_manifest.get("support", {}).get("sha256") != SUPPORT_RESULT_SHA256:
        raise ValueError("UMFR event clock support hash changed")
    ...
    if market_protocol.get("archive_checksums_verified") is not True:
        raise ValueError("UMFR execution archives were not checksum-verified")
    if market_protocol.get("end_is_exclusive") is not True:
        raise ValueError("UMFR execution manifest end contract changed")
    if market_manifest.get("combined_output") != str(MARKET_DATA):
        raise ValueError("UMFR execution market path changed")
    if market_manifest.get("combined_sha256") != MARKET_DATA_SHA256:
        raise ValueError("UMFR execution market hash changed")
    if market_manifest.get("rows") != 420_768:
        raise ValueError("UMFR execution market row count changed")
    ...
    if funding_protocol.get("luri_outcomes_opened") is not False:
        raise ValueError("UMFR funding source opened outcomes")
    if funding_data.get("path") != str(FUNDING_DATA):
        raise ValueError("UMFR funding data path changed")
    if funding_data.get("sha256") != FUNDING_DATA_SHA256:
        raise ValueError("UMFR funding data hash changed")
    if funding_data.get("rows") != 4_383:
        raise ValueError("UMFR funding row count changed")
...
def _canonical_clock_records(schedule: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    if missing:
        raise ValueError(f"UMFR clock is missing columns: {sorted(missing)}")
...
def verify_signal_replay(
    ...
    signal, controls, control_sides, _ = umfr.classify_events(
        frame, cfg, quantile=SELECTED_QUANTILE
    )
    replayed = umfr.nonoverlapping_schedule(signal, frame)
    frozen = pd.read_csv(EVENT_CLOCK)
    if _canonical_clock_records(replayed) != _canonical_clock_records(frozen):
        raise ValueError("UMFR primary event-clock replay differs from freeze")
    if not replayed["entry_position"].eq(replayed["signal_position"] + 1).all():
        raise ValueError("UMFR replay is not next-open entry")
    if not replayed["hold_bars"].eq(cfg.hold_bars).all():
        raise ValueError("UMFR replay hold changed")
    if (
        not replayed["exit_position"]
        .eq(replayed["entry_position"] + cfg.hold_bars)
        .all()
    ):
        raise ValueError("UMFR replay exit changed")

    selected = support_result["selected_support"]
    if umfr._support(replayed, cfg) != selected["support"]:
        raise ValueError("UMFR selected support replay differs from freeze")
    raw_counts = {
        name: int(mask.sum()) for name, mask in controls.items() if name != "primary"
    }
    if raw_counts != selected["control_raw_counts"]:
        raise ValueError("UMFR control raw clocks differ from freeze")
    if int(controls["primary"].sum()) != selected["raw_primary"]:
        raise ValueError("UMFR raw primary clock differs from freeze")
    clock_manifest = _read_json(EVENT_CLOCK_MANIFEST)
    if source != clock_manifest.get("source"):
        raise ValueError("UMFR source replay differs from clock freeze")
...
def _schedule_from_control(
    ...
    *,
    ...
    if not numeric_side.loc[active].isin((-1.0, 1.0)).all():
        raise ValueError(f"UMFR control {branch} has an invalid active side")
    signal = pd.DataFrame(
        {
    ...
    return (
        umfr.nonoverlapping_schedule(signal, frame)
...
def build_control_schedules(
    ...
    if tuple(schedules) != POLICY_NAMES:
        raise ValueError("UMFR evaluator control set differs from preregistration")
    ...
    if support_result is not None:
        ...
        if observed != expected:
            raise ValueError("UMFR control scheduled clocks differ from freeze")
    return schedules
...
def slice_schedule(schedule: pd.DataFrame, *, start: str, end: str) -> pd.DataFrame:
    ...
    if start_timestamp >= end_timestamp:
        raise ValueError("UMFR split start must precede end")
    if schedule.empty:
...
def load_execution_market(
    ...
    if _sha256(MARKET_DATA) != MARKET_DATA_SHA256:
        raise ValueError("UMFR execution market differs from frozen SHA-256")
    market = pd.read_csv(
        MARKET_DATA,
        compression="gzip",
    ...
    if len(market) != market_manifest["rows"]:
        raise ValueError("UMFR execution market row count differs from manifest")
    if market["date"].duplicated().any() or not market["date"].is_monotonic_increasing:
        raise ValueError("UMFR execution timestamps are duplicate or unordered")
    if not market["date"].equals(signal_frame["date"]):
        raise ValueError("UMFR execution market does not align with signal clock")
    prices = market[["open", "high", "low", "close"]].to_numpy(float)
    if not np.isfinite(prices).all() or (prices <= 0.0).any():
        raise ValueError("UMFR execution market contains invalid prices")
    ...
    if (
        ...
        raise ValueError("UMFR execution market violates OHLC invariants")
...
def load_realized_funding(
    ...
    if _sha256(FUNDING_DATA) != FUNDING_DATA_SHA256:
        raise ValueError("UMFR realized funding differs from frozen SHA-256")
    funding = pd.read_csv(
        FUNDING_DATA,
    ...
    if list(funding.columns) != expected_columns:
        raise ValueError("UMFR funding columns differ from freeze")
    if len(funding) != funding_manifest["data"]["rows"]:
        raise ValueError("UMFR funding row count differs from manifest")
    ...
    if not funding_time.equals(funding_from_ms):
        raise ValueError("UMFR funding UTC strings differ from epoch milliseconds")
    if (
        funding["funding_time_ms"].duplicated().any()
        or not funding["funding_time_ms"].is_monotonic_increasing
    ):
        raise ValueError("UMFR funding timestamps are duplicate or unordered")
    if not funding["symbol"].eq("BTCUSDT").all():
        raise ValueError("UMFR funding source contains a non-BTCUSDT symbol")
    rates = pd.to_numeric(funding["funding_rate"], errors="raise").to_numpy(float)
    if not np.isfinite(rates).all():
        raise ValueError("UMFR funding source contains an invalid rate")
    if funding_time.max() >= umfr.SELECTION_END:
        raise ValueError("UMFR funding source opens the sealed interval")
...
def simulate_funding_schedule(
    ...
    *,
    ...
    for row in schedule.itertuples(index=False):
        ...
        if exit_position != entry_position + 36:
            raise ValueError("scheduled exit does not match the frozen 36-bar hold")
        if int(row.hold_bars) != 36:
...
def _qualification(windows: dict[str, Any], cfg: EvaluationConfig) -> dict[str, Any]:
    ...
    for name, metrics in (("select2023_h1", h1), ("select2023_h2", h2)):
        ...
        if metrics["trade_count"] < cfg.minimum_2023_half_trades:
            failures.append(f"{name}: fewer than 100 trades")
...
def run_evaluation(cfg: EvaluationConfig) -> dict[str, Any]:
    ...
    signal_cfg = umfr.Config()
    signal_frame, signal_source = umfr.load_causal_frame()
    ...
    return {
        "protocol": {
            "name": "UMFR-36 frozen pre-2024 selection evaluation",
            "preregistration_commit": PREREGISTRATION_COMMIT,
            "support_commit": SUPPORT_COMMIT,
            "clock_commit": CLOCK_COMMIT,
            ...
            "outcomes_opened_for_umfr36": True,
            ...
            "control_side_rules": umfr.CONTROL_SIDE_RULES,
            ...
            "exit": "scheduled USD-M open 36 completed 5m bars after entry",
        ...
        "selection": {
            "selected_alpha": "umfr36" if qualification["qualifies"] else None,
            "rejected": not qualification["qualifies"],
            "reason": (
                "passed every frozen pre-2024 gate"
                if qualification["qualifies"]
                else "UMFR-36 failed at least one frozen pre-2024 gate"
            ),
        },
    }
