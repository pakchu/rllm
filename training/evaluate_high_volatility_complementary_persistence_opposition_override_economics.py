"""Strict staged economics for the singleton HVCPO-8 policy."""
...
PREREG = Path("results/high_volatility_complementary_persistence_opposition_override_preregistration_2026-08-16.json")
PREREG_SHA = "501b8539fac1c36d52168d1a9e8808344305c7cb8999b73763a8497fe4b3dfa6"
SUPPORT = Path("results/high_volatility_complementary_persistence_opposition_override_support_2026-08-16.json")
SUPPORT_SHA = "7f8ced0c8d03867bae6b4540884662d787a02dfa245bf3c258eb9cf4438ae910"
GROSS9 = Path("results/high_volatility_complementary_persistence_opposition_override_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "f5c6a4a2fb2ec5af5222c3c6b3076820bdeac200c87861c0636309d139f3523d"
CLOCK = {
    "path": Path("data/high_volatility_complementary_persistence_opposition_override_clocks_2023_2026.csv.gz"),
    "sha256": "4d9a822a556781709ed0cde81425cf12e4bb60cfc2edde3247a6d6ee12c01e71",
    "rows": 373,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_complementary_persistence_opposition_override_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCPO-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCPO-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCPO-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCPO-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCPO-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCPO-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCPO-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCPO-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCPO-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCPO-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCPO-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcpo_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
