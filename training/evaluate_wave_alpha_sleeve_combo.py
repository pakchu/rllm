...
OUT='results/all_alpha_with_wave_alpha_sleeves_combo_2026-07-08.json'
DOC='docs/all-alpha-with-wave-alpha-sleeves-combo-2026-07-08.md'
SLEEVES=['pb30_base','pb30_addon','nonpb30_taker','oi_raw','rex_rule','short_kimchi3d','short_premium_panic','oi_wave_lowpos144','oi_wave_slope288_low','rex_wave_pricez288_low','rex_wave_vol144_high','short_premium_wave_lowvol']
COST=0.0005
...
def build_events():
    market,feat,masks,years=ep._prep(); feat=add_struct_wave(market,feat); events=[]
    ...
    return market,feat,masks,years,events,wave_alpha_thresholds
...
def weight_grid():
    # Bounded diagnostic grid around prior bests, now allowing wave-derived sleeves.
    seeds=[
      {'pb30_base':0,'pb30_addon':0,'nonpb30_taker':2.0,'oi_raw':1.0,'rex_rule':2.5,'short_kimchi3d':0,'short_premium_panic':0,'oi_wave_lowpos144':0,'oi_wave_slope288_low':0,'rex_wave_pricez288_low':0,'rex_wave_vol144_high':0,'short_premium_wave_lowvol':0},
      {'pb30_base':0,'pb30_addon':0,'nonpb30_taker':1.5,'oi_raw':0.5,'rex_rule':2.0,'short_kimchi3d':0,'short_premium_panic':0,'oi_wave_lowpos144':0.5,'oi_wave_slope288_low':0,'rex_wave_pricez288_low':0.5,'rex_wave_vol144_high':0,'short_premium_wave_lowvol':0},
      {'pb30_base':0,'pb30_addon':0,'nonpb30_taker':1.5,'oi_raw':0,'rex_rule':2.0,'short_kimchi3d':0,'short_premium_panic':0,'oi_wave_lowpos144':0.75,'oi_wave_slope288_low':0.5,'rex_wave_pricez288_low':0.5,'rex_wave_vol144_high':0,'short_premium_wave_lowvol':0},
      {'pb30_base':0,'pb30_addon':0,'nonpb30_taker':1.75,'oi_raw':0.75,'rex_rule':2.0,'short_kimchi3d':0,'short_premium_panic':0,'oi_wave_lowpos144':0,'oi_wave_slope288_low':0,'rex_wave_pricez288_low':0,'rex_wave_vol144_high':0,'short_premium_wave_lowvol':0.25},
    ]
    vary=['nonpb30_taker','oi_raw','rex_rule','oi_wave_lowpos144','oi_wave_slope288_low','rex_wave_pricez288_low','rex_wave_vol144_high','short_premium_wave_lowvol']
    out=[]; seen=set()
    for c in seeds:
        # one-at-a-time and pair perturbations to avoid huge grid/overfit.
        candidates=[dict(c)]
        for k in vary:
        ...
        for w in candidates:
            for s in SLEEVES: w.setdefault(s,0)
...
def token_builder(feat,masks):
    cols=['w_pos_144','w_pos_288','w_retr_144','w_retr_288','w_eff_144','w_eff_288','w_price_z_144','w_price_z_288','w_slope_atr_144','w_slope_atr_288','w_vol_z_144','wr_vwap_dev_z','wr_flow_mom','wr_vol_spike']
    qmap={}
    tr=masks['train']
    for c in cols:
...
def main():
    market,feat,masks,years,events,wave_alpha_thresholds=build_events()
    ...
    wave_cols=['w_pos_144','w_pos_288','w_retr_144','w_retr_288','w_eff_144','w_eff_288','w_price_z_144','w_price_z_288','w_slope_atr_144','w_slope_atr_288','w_vol_z_144','wr_vwap_dev_z','wr_flow_mom','wr_vol_spike']
    ...
    md=['# All alpha + wave-derived alpha sleeves combo scan (2026-07-08)','',report['protocol'],'','## Event counts','```json',json.dumps(report['event_counts'],indent=2,ensure_ascii=False),'```','']
    def fmt(name, row):
        md.append(f'## {name}')
