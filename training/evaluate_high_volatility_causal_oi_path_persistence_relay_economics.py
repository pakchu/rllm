"""Strict staged economics for the singleton HVCOPR-24 policy."""
...
PREREG = Path("results/high_volatility_causal_oi_path_persistence_relay_preregistration_2026-08-16.json")
PREREG_SHA = "4170f3657b23ced4a20ed330efcaa0b1803c006299ce6a8b8eb40dd36a373c55"
SUPPORT = Path("results/high_volatility_causal_oi_path_persistence_relay_support_2026-08-16.json")
SUPPORT_SHA = "64a71384c3927c9f60ea6dcc805250757916d0b6271a70bc283e54037c22f7e0"
GROSS9 = Path("results/high_volatility_causal_oi_path_persistence_relay_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "a6f0b7d7f96069876db47fe8693fb41ce84df1cd96025e927cfd1c8005931960"
CLOCK = {
    "path": Path("data/high_volatility_causal_oi_path_persistence_relay_clocks_2020_2026.csv.gz"),
    "sha256": "ccb0eeb36eaa0e15018c321e9e5f8d1ee1eee7c746b6eb82d4552864a83a2492",
    "rows": 174,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_oi_path_persistence_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCOPR-24 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCOPR-24 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCOPR-24 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCOPR-24 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCOPR-24 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCOPR-24 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCOPR-24 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCOPR-24 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCOPR-24 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCOPR-24 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCOPR-24 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcopr_24_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
