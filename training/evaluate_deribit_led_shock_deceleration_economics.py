"""Sequential strict economics for frozen DLSDR-6."""
...
PREREG = Path("results/deribit_led_shock_deceleration_reversal_preregistration_2026-08-08.json")
PREREG_SHA = "b2ee3b6083177cad6b619b2d43dbdcae10c4b85c846e8f65b9b328a97bc58491"
SUPPORT = Path("results/deribit_led_shock_deceleration_reversal_support_2026-08-08.json")
SUPPORT_SHA = "62656838515078b69c8e6d4d95c0e2d7a55b64f8ea189dccb73b33a76ca64635"
NOVELTY = Path("results/deribit_led_shock_deceleration_reversal_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "b66d95bceaefd22de512c4b6c19ed4f890a7e839d1a6fd26dc7085bf3bd290d3"
CLOCK = Path("data/deribit_led_shock_deceleration_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "bb41a4ac7dc9ccc321df656a0a718602be551f9a0c118a25a71cc6d3d662edb6"
CONTROL_DIR = Path("data/deribit_led_shock_deceleration_reversal_controls_2023_2026")
FREEZE = Path("results/deribit_led_shock_deceleration_reversal_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/deribit_led_shock_deceleration_reversal_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
CONTROLS = ("no_deribit_lead", "no_vol_disagreement", "no_first_half_tail", "no_deceleration_cap", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("DLSDR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("DLSDR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "dlsdr_6_sequential_economics_v1", "policy_id": POLICY_ID,
