"""Outcome-blind preregistration for HVCVARCLIR-8."""
...
POLICY_ID = "HVCVARCLIR-8"
DEFAULT_OUTPUT = Path("results/high_volatility_causal_variance_acceleration_close_location_inventory_relay_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_causal_variance_acceleration_close_location_inventory_relay_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": "When BTC perpetual realized variance accelerates unusually from the first to the second four-hour half, the cycle closes in the directional outer 35 percent of its exact range, and realized variation is elevated, open interest expands, and the completed eight-hour return is nonzero, late volatility expansion, directional price acceptance, and fresh leveraged inventory sponsor the completed price direction for eight hours.",
            "side": "strict sign of the completed eight-hour BTC perpetual return",
            "why_distinct": "HVCVARCLIR uses within-cycle acceleration in realized variance, not turnover or transaction count alone, plus exact OI expansion and variation. It reuses neither prior event set nor control and uses no taker-flow sign, spot source, ETH, funding, premium-index, fitted outcome, or promoted control.",
            "why_suited_to_volatile_regimes": "the completed eight-hour BTC realized variation must be in its causal upper 40 percent",
            "why_low_gross9_overlap_is_plausible": "fixed eight-hour variance-acceleration plus close-location and OI-sponsored states are absent from Gross9 primitives",
        },
        "features": {
            ...
            "eligible": "exact coherent completed perpetual cycle, positive variance acceleration, nonzero completed return, directional outer-35-percent completed-range close location, two exact OI endpoints with strict expansion, and both variance-acceleration and variation ranks pass at D; no taker-flow sign, spot source, funding, range, premium-index, alt, or post-decision condition",
            "no_imputation": True,
        },
        "clock": {
        ...
        "stopping_rule": "terminal first failure; no OI endpoint or expansion rule, variance-acceleration formula, close-location confirmation, acceleration or variation history, side, clock, hold, subset, threshold, comparator, or control repair",
    }
