"""Sequential strict economics for frozen CVACR-24."""
...
PREREG = Path("results/cboe_volatility_acceleration_curve_relay_preregistration_2026-08-08.json")
PREREG_SHA = "1831eb34a7371a47a27f4d236c7cd19897a9a11b85bb3321fc0f41f6e4a11d82"
SUPPORT = Path("results/cboe_volatility_acceleration_curve_relay_support_2026-08-08.json")
SUPPORT_SHA = "f9ead0b9c5b470fa4c1a241f835cd3bdbe17030994fc9324c69eece39bfc3cf6"
NOVELTY = Path("results/cboe_volatility_acceleration_curve_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "1953e2e48e754d14d78865172a476db6326d339cdde95703c1fd88dfac37ca28"
CLOCK = Path("data/cboe_volatility_acceleration_curve_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "95842fca3ab294ab2b2b9f891ec299aa6ee402ad1b1cd31a15c87fc2d5992d0d"
CONTROL_DIR = Path("data/cboe_volatility_acceleration_curve_relay_controls_2023_2026")
FREEZE = Path("results/cboe_volatility_acceleration_curve_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/cboe_volatility_acceleration_curve_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
CONTROLS = ("no_vix_confirmation", "vix_only", "front_vix_only", "broad_vix_only", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CVACR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CVACR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "cvacr_24_sequential_economics_v1", "policy_id": POLICY_ID,
