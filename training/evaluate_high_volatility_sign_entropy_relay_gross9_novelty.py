"""Evaluate frozen HVSER-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVSER-12"
PROTOCOL = "hvser_12_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_sign_entropy_relay_preregistration_2026-08-09.json"
)
PREREG_SHA = "9ab221716d525997b4da303ee01afebe0336436d92d01e0b18a51db4e97f5854"
SUPPORT = Path("results/high_volatility_sign_entropy_relay_support_2026-08-09.json")
SUPPORT_SHA = "09271729f9d810ad0fb9c42935b88bf161cc9a902e17eba2eeee81d090c8d91f"
CLOCK = Path("data/high_volatility_sign_entropy_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "5936c687616c763af90aa69b5875fb413450c0b38c1b0f5fab5465c203422787"
OUTPUT = Path(
    "results/high_volatility_sign_entropy_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVSER predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVSER novelty limits drift")
    if (
        ...
        raise RuntimeError("HVSER predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVSER primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvser_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVSER-12 unchanged before economic outcomes"
        ),
    }
