"""Sequential strict economics for frozen HVCASPCPQA-8."""
...
POLICY_ID = "HVCASPCPQA-8"
BAR = pd.Timedelta(minutes=5)
LEVERAGE = 0.5
BASE_COST = 0.0006
STRESS_COST = 0.0010
ENV_FILE = "/home/pakchu/rllm/.env"
PREREG = Path("results/high_volatility_caspc_quiet_premium_activity_preregistration_2026-08-16.json")
PREREG_SHA = "d3fc1e0d152abd2c87a568efa6c2236389bf787de2271c847713ab99c2783e01"
SUPPORT = Path("results/high_volatility_caspc_quiet_premium_activity_support_2026-08-16.json")
SUPPORT_SHA = "f0498c805b3bf93a9fe63fc374f03b7afe0d803382104bc2aa7c6b0fc92c67a2"
NOVELTY = Path("results/high_volatility_caspc_quiet_premium_activity_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "b8cf184ed63dc8b9b8934413ec3641a7992d2d161802711f784548b7e1fa0fe1"
CLOCK = Path("data/high_volatility_caspc_quiet_premium_activity_clocks_2023_2026.csv.gz")
CLOCK_SHA = "684ddfc240679d99ed5d5f30f095a359940c9ed098e5c6b0cf7f15d6fdf6bcce"
CONTROL_DIR = Path("data/high_volatility_caspc_quiet_premium_activity_controls_2023_2026")
FREEZE = Path("results/high_volatility_caspc_quiet_premium_activity_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_caspc_quiet_premium_activity_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCASPCPQA frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCASPCPQA novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcaspcpqa_8_sequential_economics_v1", "policy_id": POLICY_ID,
