"""Evaluate frozen HVAO-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVAO-24"
PROTOCOL = "hvao_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_awesome_oscillator_zero_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVAO predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVAO novelty limits drift")
    if (
        ...
        raise RuntimeError("HVAO predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVAO primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvao_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVAO-24 unchanged before economic outcomes",
    }
