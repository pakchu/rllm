"""Build outcome-blind source support for frozen HVECC-24."""
...
from training import preregister_high_volatility_ehlers_cyber_cycle_trigger_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "af16d2e8e0749b2cea11d21cf2ffcd63a5166ff8e83911260cd8bebf38edbca8"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ehlers_cyber_cycle_trigger_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ehlers_cyber_cycle_trigger_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ehlers_cyber_cycle_trigger_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ehlers_cyber_cycle_trigger_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ehlers_cyber_cycle_trigger_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "median_price", "smooth", "cycle", "trigger",
    "difference", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "median_price", "smooth", "cycle", "trigger",
    "difference", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVECC source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVECC invalid source key")
...
def cyber_cycle(price: pd.Series, valid: pd.Series) -> pd.DataFrame:
    values=price.to_numpy(float);validity=valid.to_numpy(bool);alpha=P["alpha"]
    smooth=np.full(len(price),np.nan);raw_cycle=np.full(len(price),np.nan);cycle=np.full(len(price),np.nan);run=0
    for i,value in enumerate(values):
        if not validity[i] or not math.isfinite(value):run=0;continue
        run+=1
        if run>=4:smooth[i]=(values[i]+2*values[i-1]+2*values[i-2]+values[i-3])/6
        if run>=3 and run<7:raw_cycle[i]=(values[i]-2*values[i-1]+values[i-2])/4
        elif run>=7:
            raw_cycle[i]=(1-alpha/2)**2*(smooth[i]-2*smooth[i-1]+smooth[i-2])+2*(1-alpha)*raw_cycle[i-1]-(1-alpha)**2*raw_cycle[i-2]
            cycle[i]=raw_cycle[i]
    trigger=pd.Series(cycle,index=price.index).shift(1)
    return pd.DataFrame({"median_price":values,"smooth":smooth,"cycle":cycle,"trigger":trigger},index=price.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=bars.join(cyber_cycle((bars.bar_high+bars.bar_low)/2,bars.valid_bar));bars["difference"]=bars.cycle-bars.trigger
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum())
    bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["median_price","smooth","cycle","trigger","difference","variation"]]).all(axis=1)&bars.difference.ne(0)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="zero_cross":
        signal=pd.to_numeric(used.cycle,errors="coerce");previous=signal.shift(1);state=valid&signal.ne(0)&previous.ne(0)&np.sign(signal).ne(np.sign(previous))&variation;side=np.sign(signal).fillna(0).astype(int)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVECC prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvecc_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvecc_24_source_support_v1", "policy_id": prereg.POLICY_ID,
