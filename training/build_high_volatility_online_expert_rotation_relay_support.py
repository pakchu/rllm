"""Build causal HVOER-8 winner-rotation clocks before novelty/economics."""
from __future__ import annotations
import argparse,hashlib,json
from pathlib import Path
from typing import Any
import numpy as np,pandas as pd
from training import preregister_high_volatility_online_expert_rotation_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.build_scheduled_trend_concordance_relay_support import load_market
PREREG_SHA="4506c0b453935b2035328eb3e93709b7c67b1652353f4ddad0c6610e57f69ed2";LOADER=Path("training/build_scheduled_trend_concordance_relay_support.py");LOADER_SHA="8ca554d88506df277434f73e5eb8850426614a880110088eb91aaae3b23c154f";END=pd.Timestamp("2026-08-01T00:00:00Z");SOURCE_DIR=Path("data/high_volatility_online_expert_rotation_relay_sources_2020_2026");STATES=SOURCE_DIR/"causal_online_expert_rotation_states.csv.gz";MANIFEST=SOURCE_DIR/"manifest.json";CLOCK=Path("data/high_volatility_online_expert_rotation_relay_clocks_2023_2026.csv.gz");CONTROL_DIR=Path("data/high_volatility_online_expert_rotation_relay_controls_2023_2026");RESULT=Path("results/high_volatility_online_expert_rotation_relay_support_2026-08-13.json")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00:00Z"),pd.Timestamp("2024-01-01T00:00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00:00Z"),pd.Timestamp("2025-01-01T00:00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00:00Z"),pd.Timestamp("2026-01-01T00:00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00:00Z"),END)};MINIMUM={"train":8,"test":12,"eval":12,"final":8};EXPERTS=("momentum_4h","reversal_4h","momentum_12h","reversal_12h");CONTROLS=("no_variation_gate","winner_level","one_decision_stale_winner","direction_flip","same_clock_forced_long");COLS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","return_4h","return_12h","winner","prior_winner","winner_score","variation","variation_rank")
...
def online_states(market,memory=60,minimum=30):
 ...
 first=f.index.min().ceil("D")+pd.Timedelta(hours=2);decisions=pd.date_range(first,END,freq="4h",inclusive="left");hist=[[] for _ in EXPERTS];pending=[];rows=[]
 for d in decisions:
  matured=[p for p in pending if p["exit"]<=d]
  pending=[p for p in pending if p["exit"]>d]
  for p in matured:
   if p["entry"] not in f.index or p["exit"] not in f.index:continue
   opens=f.loc[[p["entry"],p["exit"]],"open"].to_numpy(float)
   if not np.isfinite(opens).all() or not (opens>0).all():continue
   forward=float(np.log(opens[1]/opens[0]))
   for j,h in enumerate(hist):h.append(float(p["sides"][j]*forward))
  ...
  closes=f.loc[times,"close"].to_numpy(float)
  if not np.isfinite(closes).all() or not (closes>0).all():continue
  r4=float(np.log(closes[-1]/closes[-49]));r12=float(np.log(closes[-1]/closes[0]));var=float(np.sqrt(np.square(np.diff(np.log(closes[-97:]))).sum()));ss=sides(r4,r12);scores=np.asarray([np.mean(h[-memory:]) if len(h)>=minimum else np.nan for h in hist]);wi=int(np.nanargmax(scores)) if np.isfinite(scores).any() else -1;winner=EXPERTS[wi] if wi>=0 else "";prior=rows[-1]["winner"] if rows else "";valid=bool(r4!=0 and r12!=0 and var>0)
  rows.append({"decision_time":d,"source_valid":valid,"return_4h":r4,"return_12h":r12,"variation":var,**{f"{n}_side":int(ss[j]) for j,n in enumerate(EXPERTS)},**{f"{n}_score":float(scores[j]) for j,n in enumerate(EXPERTS)},"winner":winner,"prior_winner":prior,"winner_side":int(ss[wi]) if wi>=0 else 0,"winner_score":float(scores[wi]) if wi>=0 else np.nan,"mature_labels_before_decision":min((len(h) for h in hist),default=0)})
  pending.append({"entry":d+pd.Timedelta(minutes=5),"exit":d+pd.Timedelta(hours=8,minutes=5),"sides":ss.copy()})
 x=pd.DataFrame(rows);x["variation_rank"]=rank(x.variation.where(x.source_valid));x["signal_valid"]=x.source_valid&x.winner.ne("")&x.winner_side.ne(0)&np.isfinite(x[["winner_score","variation","variation_rank"]]).all(axis=1);x["rotation"]=x.signal_valid&x.prior_winner.ne("")&x.winner.ne(x.prior_winner);return x
def conditions(x,control):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 active=x.signal_valid.copy() if control=="winner_level" else x.rotation.copy();side=x.winner_side.astype(int).copy()
 if control=="one_decision_stale_winner":side=x.winner_side.shift(1).fillna(0).astype(int);active&=side.ne(0)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA or sha(prereg.MARKET)!=prereg.MARKET_SHA or sha(LOADER)!=LOADER_SHA:raise RuntimeError("HVOER binding drift")
