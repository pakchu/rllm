"""Deterministic outcome-blind source support for HVCSPVR-8."""
...
from training import preregister_high_volatility_causal_spot_participation_inventory_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "2db38a14423f2a05248a5f9bc88f2a989f6327727f4a2ce71dc8c9b54810afe8"
...
BAR_QUERY = """WITH paths AS (SELECT ts,open,high,low,close,quote_asset_volume,'perpetual' AS market FROM bars_binance WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end UNION ALL SELECT ts,open,high,low,close,quote_asset_volume,'spot' AS market FROM bars_binance_spot WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end) SELECT date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '8 hours' AS decision_time,market,sum(ln(close/open)) AS completed_return,sum(quote_asset_volume) AS quote_turnover,sum(power(ln(close/open),2)) AS minute_squared_return,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND high>=greatest(open,close,low) AND low<=least(open,close,high) AND quote_asset_volume>=0) AS coherent FROM paths GROUP BY 1,2 ORDER BY 1,2"""

ROOT = Path("data/high_volatility_causal_spot_participation_inventory_relay_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_spot_participation_inventory_relay_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_spot_participation_inventory_relay_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_causal_spot_participation_inventory_relay_support_2026-08-16.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "sum_open_interest",
    "previous_open_interest", "oi_change", "perpetual_return", "spot_return",
    "spot_participation", "spot_participation_rank", "realized_variation",
    "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "decision_time", "feature_available_time",
    "entry_time", "exit_time", "side", "oi_change", "perpetual_return", "spot_return",
    "spot_participation", "spot_participation_rank", "realized_variation", "variation_rank",
)
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVCSPVR-8 OI schema drift")
    expected_bars = [
        "decision_time", "market", "completed_return", "quote_turnover", "minute_squared_return",
        "source_rows", "distinct_rows", "first_ts", "last_ts", "coherent",
    ]
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCSPVR-8 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all():
        raise RuntimeError("HVCSPVR-8 invalid OI rows")

    bars = bars.copy()
    for column in ("decision_time", "first_ts", "last_ts"):
        bars[column] = pd.to_datetime(bars[column], utc=True, errors="raise")
    numeric = ("completed_return", "quote_turnover", "minute_squared_return", "source_rows", "distinct_rows")
    for column in numeric:
        bars[column] = pd.to_numeric(bars[column], errors="raise")
    if bars[["decision_time", "market"]].duplicated().any() or set(bars["market"]) != {"perpetual", "spot"}:
        raise RuntimeError("HVCSPVR-8 duplicate or missing symbol bar")
    start = bars["decision_time"] - pd.Timedelta("8h")
    bars["bar_valid"] = (
        np.isfinite(bars[list(numeric)]).all(axis=1)
        & bars["quote_turnover"].ge(0)
    ...
    fields = ["bar_valid", "completed_return", "quote_turnover", "minute_squared_return"]
    wide = bars.set_index(["decision_time", "market"])[fields].unstack("market")
    wide.columns = [f"{field}_{market}" for field, market in wide.columns]
    wide["perpetual_return"] = wide["completed_return_perpetual"]
    wide["spot_return"] = wide["completed_return_spot"]
    wide["realized_variation"] = np.sqrt(wide["minute_squared_return_perpetual"])
    wide["joint_bar_valid"] = wide["bar_valid_perpetual"].eq(True) & wide["bar_valid_spot"].eq(True)
    total_turnover = wide["quote_turnover_spot"] + wide["quote_turnover_perpetual"]
    wide["spot_participation"] = wide["quote_turnover_spot"] / total_turnover.where(total_turnover.gt(0))
    return oi.sort_values("decision_time"), wide.sort_index()
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel = panel.join(
        bars[["joint_bar_valid", "perpetual_return", "spot_return", "spot_participation", "realized_variation"]],
        on="decision_time",
    )
    panel["source_valid"] = (
        panel["previous_time"].notna()
        & panel["decision_time"].sub(panel["previous_time"]).eq(pd.Timedelta(hours=8))
        & panel["joint_bar_valid"].eq(True)
        & np.isfinite(
            panel[["sum_open_interest", "previous_open_interest", "oi_change", "perpetual_return", "spot_return", "spot_participation", "realized_variation"]]
        ).all(axis=1)
        & panel["sum_open_interest"].gt(0)
        & panel["previous_open_interest"].gt(0)
        & panel["realized_variation"].gt(0)
        & panel["perpetual_return"].ne(0)
        & panel["spot_return"].ne(0)
        & panel["spot_participation"].gt(0)
        & panel["spot_participation"].le(1)
    )
    valid = panel["source_valid"].eq(True)
    panel["spot_participation_rank"] = causal_midrank(panel["spot_participation"].where(valid))
    panel["variation_rank"] = causal_midrank(panel["realized_variation"].where(valid))
    panel["eligible"] = (
        valid
        & np.sign(panel["perpetual_return"]).eq(np.sign(panel["spot_return"]))
        & panel["oi_change"].gt(0)
        & panel["spot_participation_rank"].ge(POLICY["spot_participation_rank_min"])
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if split is None:
            continue
        side = int(np.sign(row.perpetual_return))
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCSPVR-8 side or availability drift")
        ...
        rows.append({
            ...
            "spot_participation_rank": float(row.spot_participation_rank),
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCSPVR-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcspvr_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvcspvr_8_source_support_v1", "policy_id": prereg.POLICY_ID,
