"""Evaluate frozen HVCACM-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCACM-8"
PROTOCOL = "hvcacm_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_cross_alt_channel_position_migration_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCACM predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCACM novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCACM predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCACM primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCACM-8 unchanged before economic outcomes",
    }
