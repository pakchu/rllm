"""Evaluate frozen HVDMIC-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDMIC-24"
PROTOCOL = "hvdmic_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_directional_movement_index_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDMIC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVDMIC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDMIC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDMIC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdmic_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVDMIC-24 unchanged before economic outcomes",
    }
