"""Evaluate frozen DLSDR-6 structural novelty against every Gross9 sleeve."""
...
POLICY='DLSDR-6';PROTOCOL='dlsdr_6_gross9_novelty_v1';PREREG=Path('results/deribit_led_shock_deceleration_reversal_preregistration_2026-08-08.json');PREREG_SHA='b2ee3b6083177cad6b619b2d43dbdcae10c4b85c846e8f65b9b328a97bc58491';SUPPORT=Path('results/deribit_led_shock_deceleration_reversal_support_2026-08-08.json');SUPPORT_SHA='62656838515078b69c8e6d4d95c0e2d7a55b64f8ea189dccb73b33a76ca64635';CLOCK=Path('data/deribit_led_shock_deceleration_reversal_clocks_2023_2026.csv.gz');CLOCK_SHA='bb41a4ac7dc9ccc321df656a0a718602be551f9a0c118a25a71cc6d3d662edb6';OUTPUT=Path('results/deribit_led_shock_deceleration_reversal_gross9_novelty_2026-08-08.json');LIMITS=metric.LIMITS
...
def run(output:Path=OUTPUT)->dict:
 if sha(PREREG)!=PREREG_SHA or sha(SUPPORT)!=SUPPORT_SHA or sha(CLOCK)!=CLOCK_SHA:raise RuntimeError('DLSDR predecessor hash drift')
 ...
 if reg.get('novelty_gates')!=expected:raise RuntimeError('DLSDR novelty limits drift')
 if support.get('policy_id')!=POLICY or support.get('support_passed') is not True or support.get('advance_to_gross9_novelty') is not True or support.get('advance_to_economic_outcomes') is not False or support.get('clock',{}).get('sha256')!=CLOCK_SHA:raise RuntimeError('DLSDR predecessor state drift')
 ...
 candidate=metric.load_clock(CLOCK,label='DLSDR primary');results={}
 ...
 passed=all(x['passed'] for x in results.values());advance=support['support_passed'] and passed;core={'protocol_version':PROTOCOL,'policy_id':POLICY,'preregistration':{'path':str(PREREG),'sha256':PREREG_SHA,'manifest_hash':reg['manifest_hash']},'source_support':{'path':str(SUPPORT),'sha256':SUPPORT_SHA,'manifest_hash':support['manifest_hash'],'predecessor_mutated':False},'gross9_structural_clocks':{'path':str(gross9.DEFAULT_MANIFEST),'sha256':sha(gross9.DEFAULT_MANIFEST),'manifest_hash':manifest['manifest_hash'],'authority_sha256':gross9.ANCHOR_SHA256,'complete_roster':list(gross9.EXPECTED_WEIGHTS)},'evidence_boundary':{'dlsdr_clock_rows_opened':len(candidate),'gross9_structural_clock_rows_opened':sum(x['rows'] for x in manifest['clocks'].values()),'btc_execution_rows_opened':0,'btc_price_or_return_rows_opened':0,'funding_rows_opened':0,'economic_outcome_rows_opened':0,'portfolio_return_or_pnl_metrics_computed':False,'outcomes_opened':False},'limits':LIMITS,'gross9_sleeves':results,'source_support_passed':support['support_passed'],'every_gross9_sleeve_passed':passed,'gross9_novelty_status':'passed' if passed else 'failed','advance_to_economic_outcomes':advance,'failure_action':None if advance else 'reject DLSDR-6 unchanged before economic outcomes'};r={**core,'manifest_hash':chash(core)};output.write_text(json.dumps(r,indent=2,ensure_ascii=False,allow_nan=False)+'\n');return r
