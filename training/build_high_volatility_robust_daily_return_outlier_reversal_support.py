"""Build outcome-blind source support for frozen HVRDOR-8."""
...
from training import preregister_high_volatility_robust_daily_return_outlier_reversal as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-02-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="f2ffdd250e195bdd9b0a9f94dedad2dfae4f79fd5f789efa4eeb5f14c98272fc"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_robust_daily_return_outlier_reversal_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_robust_daily_return_outlier_reversal_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_robust_daily_return_outlier_reversal_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_robust_daily_return_outlier_reversal_controls_2023_2026")
RESULT=Path("results/high_volatility_robust_daily_return_outlier_reversal_support_2026-08-11.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","reference_return_count","current_return","reference_median","reference_mad","robust_score","outlier_strength","outlier_strength_rank","raw_return_strength_rank","variation","variation_rank","eligible")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","current_return","reference_median","reference_mad","robust_score","outlier_strength","outlier_strength_rank","raw_return_strength_rank","variation","variation_rank","eligible")
...
def robust_score(current:float,prior:np.ndarray)->tuple[float,float,float]:
 x=np.asarray(prior,float)
 if len(x)!=P["reference_days"] or not np.isfinite(x).all() or not math.isfinite(current):return math.nan,math.nan,math.nan
 median=float(np.median(x));mad=float(np.median(np.abs(x-median)));score=(current-median)/(P["mad_consistency_scale"]*mad) if mad>0 else math.nan
 return median,mad,float(score)
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVRDOR source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVRDOR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 returns=daily["return"];prior=returns.shift(1);median=prior.rolling(P["reference_days"],min_periods=P["reference_days"]).median();mad=prior.rolling(P["reference_days"],min_periods=P["reference_days"]).apply(lambda x:float(np.median(np.abs(x-np.median(x)))),raw=True);score=(returns-median)/(P["mad_consistency_scale"]*mad);variation=np.sqrt(np.square(returns).rolling(P["variation_days"],min_periods=P["variation_days"]).sum());panel=pd.DataFrame({"decision_time":daily.index,"feature_available_time":daily.index,"reference_return_count":prior.notna().rolling(P["reference_days"],min_periods=P["reference_days"]).sum(),"current_return":returns,"reference_median":median,"reference_mad":mad,"robust_score":score,"variation":variation});panel=panel[(panel.decision_time>=START+pd.Timedelta("32d"))&(panel.decision_time<END)].reset_index(drop=True);panel["source_valid"]=panel.reference_return_count.eq(P["reference_days"])&np.isfinite(panel[["current_return","reference_median","reference_mad","robust_score","variation"]]).all(axis=1)&panel.reference_mad.gt(0)&panel.robust_score.ne(0)&panel.current_return.ne(0)&panel.variation.gt(0);panel["outlier_strength"]=panel.robust_score.abs();panel["outlier_strength_rank"]=prior_rank(panel.outlier_strength.where(panel.source_valid));panel["raw_return_strength_rank"]=prior_rank(panel.current_return.abs().where(panel.source_valid));panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));panel["eligible"]=panel.source_valid&panel.outlier_strength_rank.ge(P["outlier_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"]);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy();valid=used.source_valid.eq(True);strength=used.outlier_strength_rank.ge(P["outlier_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"]);side=-np.sign(pd.to_numeric(used.robust_score,errors="coerce")).fillna(0).astype(int);state=valid&side.ne(0)&strength&variation
 if control=="no_outlier_strength":state=valid&side.ne(0)&variation
 elif control=="no_variation_gate":state=valid&side.ne(0)&strength
 elif control=="raw_return_fade":side=-np.sign(pd.to_numeric(used.current_return,errors="coerce")).fillna(0).astype(int);state=valid&side.ne(0)&used.raw_return_strength_rank.ge(P["outlier_rank_min"])&variation
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVRDOR prereg drift")
