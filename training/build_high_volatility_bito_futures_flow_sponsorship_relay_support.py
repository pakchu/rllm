"""Materialize source-only HVBITOFL-24 clocks before Gross9 or economics."""
...
PREREG_SHA = "72f40737c05a08fccf5fd52100936c19f91627ce55b7a2a3f52b87ba016334df"
SOURCE_SHA = "ea49e137ba31a68eaab585b4f441cb8c6a33fa1bc32d1b085a942b45dab2dc5a"
SOURCE = Path("data/high_volatility_bito_futures_flow_sponsorship_relay_sources_2022_2026/bito_sessions.csv.gz")
SOURCE_MANIFEST = SOURCE.parent / "build_manifest.json"
SOURCE_MANIFEST_SHA = "84fd559569f6fb27c213a1343464d608ea9da4730408e44eb2e164746dd81de0"
...
STATE = SOURCE.parent / "daily_bito_states.csv.gz"
CLOCK = Path("data/high_volatility_bito_futures_flow_sponsorship_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_bito_futures_flow_sponsorship_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_bito_futures_flow_sponsorship_relay_support_2026-08-11.json")
...
CONTROLS = ("no_variation_gate", "no_relative_volume_gate", "one_session_stale_flow", "direction_flip", "forced_long")
COLUMNS = (
    "candidate", "control", "split", "session_date", "decision_time",
    "feature_available_time", "entry_time", "exit_time", "side", "bito_cash_return",
    "bito_volume", "bito_relative_volume_rank", "btc_variation", "btc_variation_rank",
)
...
def score_sessions(source: pd.DataFrame, market: pd.DataFrame) -> pd.DataFrame:
    ...
    if frame.session_date.duplicated().any() or not frame.session_date.is_monotonic_increasing:
        raise RuntimeError("HVBITOFL source session order drift")
    columns = ("bito_open", "bito_close", "bito_volume")
    for column in columns:
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    finite = np.isfinite(frame[list(columns)]).all(axis=1)
    frame["bito_cash_return"] = np.log(frame.bito_close / frame.bito_open)
    frame["bito_relative_volume_rank"] = strict_prior_midrank(frame.bito_volume)
    ...
    frame["source_valid"] = finite & frame[list(columns)].gt(0).all(axis=1) & np.isfinite(frame.bito_cash_return)
    return frame
...
def build_clock(states: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    used = states.shift(1) if control == "one_session_stale_flow" else states
    cash_return = used.bito_cash_return
    volume_rank = used.bito_relative_volume_rank
    valid = used.source_valid.eq(True) & np.isfinite(cash_return) & np.isfinite(volume_rank)
    side = np.sign(cash_return).fillna(0).astype(int)
    volume_gate = pd.Series(True, index=states.index) if control == "no_relative_volume_gate" else volume_rank.ge(0.65)
    variation_gate = pd.Series(True, index=states.index) if control == "no_variation_gate" else states.btc_variation_rank.ge(0.65)
    active = valid & volume_gate & variation_gate & np.isfinite(states.btc_variation_rank) & side.ne(0)
    ...
    for index in states.index[active]:
        ...
        rows.append({
            ...
            "side": int(side.at[index]), "bito_cash_return": float(cash_return.at[index]),
            "bito_volume": float(used.at[index, "bito_volume"]),
            "bito_relative_volume_rank": float(volume_rank.at[index]),
...
def run() -> dict[str, Any]:
    ...
    for path, expected in bindings.items():
        if sha(path) != expected:
            raise RuntimeError(f"HVBITOFL binding drift: {path}")
    ...
    core = {
        "protocol_version": "hvbitofl_24_source_support_v1", "policy_id": prereg.POLICY_ID,
