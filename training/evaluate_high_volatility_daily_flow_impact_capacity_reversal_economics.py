"""Sequential strict economics for frozen HVDFICR-12."""
...
PREREG = Path("results/high_volatility_daily_flow_impact_capacity_reversal_preregistration_2026-08-13.json")
PREREG_SHA = "e3b1b63ca127cacea6bdb81f5b134ef1ceaadfbb3a127314c3d2b62fce8d2ed9"
SUPPORT = Path("results/high_volatility_daily_flow_impact_capacity_reversal_support_2026-08-13.json")
SUPPORT_SHA = "89d3b0f17046578c4148d0207921f2f728fa769f234121eea2b981d8ec5d19bb"
NOVELTY = Path("results/high_volatility_daily_flow_impact_capacity_reversal_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "0981bccee9b0f45a97a4f8b5933dea091b3b40f5bd04ee275f5f2abce7e943d5"
CLOCK = Path("data/high_volatility_daily_flow_impact_capacity_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "34754638699c7ffc4e69dc3c19907e849c4c4603bc2801027f5e82b6ab944d65"
CONTROL_DIR = Path("data/high_volatility_daily_flow_impact_capacity_reversal_controls_2023_2026")
FREEZE = Path("results/high_volatility_daily_flow_impact_capacity_reversal_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_daily_flow_impact_capacity_reversal_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_impact_tail", "no_variation_gate", "negative_beta_state", "one_day_stale_capacity", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDFICR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDFICR novelty did not authorize economics")
