"""Sequential strict economics for frozen HVSRP-8."""
...
PREREG = Path("results/high_volatility_shock_response_polarity_relay_preregistration_2026-08-10.json")
PREREG_SHA = "2164103502c9bebabce4ece727e961533076e8b1e05bf1b145f7f0765c06e09e"
SUPPORT = Path("results/high_volatility_shock_response_polarity_relay_support_2026-08-10.json")
SUPPORT_SHA = "ccb536e6103e47b6f5b60cb2c516d5aa511255ebf3b37b7ad4570d2879eed6d4"
NOVELTY = Path("results/high_volatility_shock_response_polarity_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "48b0cbfdbd5f319436b44fd62ef871d1b42f7b5fe294cd26b0ef2bdde5167a77"
CLOCK = Path("data/high_volatility_shock_response_polarity_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "3c11e844c638205cb950e5856944a8c5404f4f0c0301d713ef75b7b18240e534"
CONTROL_DIR = Path("data/high_volatility_shock_response_polarity_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_shock_response_polarity_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_shock_response_polarity_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_response_tail", "no_variation_gate", "raw_absolute_response_above_tenth", "one_block_stale_response", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSRP frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSRP novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvsrp_8_sequential_economics_v1", "policy_id": POLICY_ID,
