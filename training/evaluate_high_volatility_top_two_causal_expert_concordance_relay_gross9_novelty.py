"""Evaluate frozen HVTCEC-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVTCEC-24"
PROTOCOL = "hvtcec_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_top_two_causal_expert_concordance_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTCEC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVTCEC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTCEC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTCEC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvtcec_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVTCEC-24 unchanged before economic outcomes",
    }
