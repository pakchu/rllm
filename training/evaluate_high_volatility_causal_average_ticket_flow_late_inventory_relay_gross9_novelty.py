"""Evaluate the frozen HVCATFLIR-8 action-vote clock against Gross9."""
...
POLICY = "HVCATFLIR-8"
PROTOCOL = "hvcatflir_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_average_ticket_flow_late_inventory_relay_clocks_2020_2026.csv.gz",
    "sha256": "42d7c3f3b21741bb3364fdf4804cdd950217280f307fa3d042ac997d952f5003",
    "rows": 99,
}
OUTPUT = Path("results/high_volatility_causal_average_ticket_flow_late_inventory_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCATFLIR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCATFLIR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATFLIR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCATFLIR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCATFLIR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCATFLIR-8 unchanged before economics",
    }
