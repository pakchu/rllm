"""Build outcome-blind source support for frozen HVABLR-6."""
...
BUILDER = Path("training/build_high_volatility_alt_basket_leadership_relay_support.py")
PREREG_SHA = "285d84214e03743e6120380016fbaf35cee4978fedb3d209359b4e91efd7cc76"
...
SOURCE_DIR = Path("data/high_volatility_alt_basket_leadership_relay_sources_2023_2026")
PAIR_PANEL = SOURCE_DIR / "hourly_leadership_pairs.csv.gz"
FEATURE_PANEL = SOURCE_DIR / "hourly_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_alt_basket_leadership_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_alt_basket_leadership_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_alt_basket_leadership_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_alt_basket_leadership_relay_support_2026-08-10.json")

PAIR_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "minute_count",
    "alt_leads_btc", "btc_leads_alt", "leadership_advantage",
    "contemporaneous_correlation", "btc_variation", "btc_final_hour_return",
    "alt_final_hour_return", "direction_side",
)
FEATURE_COLUMNS = (
    *PAIR_COLUMNS, "leadership_rank", "contemporaneous_correlation_rank",
    "btc_variation_rank",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "decision_time", "feature_available_time",
    "entry_time", "exit_time", "side", "alt_leads_btc", "btc_leads_alt",
    "leadership_advantage", "leadership_rank", "contemporaneous_correlation",
    "contemporaneous_correlation_rank", "btc_variation", "btc_variation_rank",
    "btc_final_hour_return", "alt_final_hour_return", "direction_side",
)
...
def prepare_source(bars: pd.DataFrame) -> pd.DataFrame:
    ...
    if bars.columns.tolist() != required:
        raise RuntimeError("HVABLR source schema drift")
    frame = bars.copy()
    ...
    if frame.ts.isna().any() or frame.duplicated(["ts", "symbol"]).any():
        raise RuntimeError("HVABLR invalid or duplicate source key")
    if not frame.symbol.isin(SYMBOLS).all():
        raise RuntimeError("HVABLR unexpected source symbol")
...
def _invalid_pair(minute_count: int) -> dict[str, Any]:
    return {
        ...
        "alt_leads_btc": np.nan, "btc_leads_alt": np.nan,
        "leadership_advantage": np.nan, "contemporaneous_correlation": np.nan,
        "btc_variation": np.nan, "btc_final_hour_return": np.nan,
        "alt_final_hour_return": np.nan, "direction_side": 0,
    }
...
def boundary_pair(source: pd.DataFrame, decision: pd.Timestamp) -> dict[str, Any]:
    ...
    alt_now, btc_next = alt[:-1], btc[1:]
    btc_now, alt_next = btc[:-1], alt[1:]
    if not (
        _finite_positive_variance(alt_now, btc_next)
        and _finite_positive_variance(btc_now, alt_next)
        and _finite_positive_variance(alt, btc)
    ):
        return _invalid_pair(minute_count)

    alt_leads_btc = float(np.corrcoef(alt_now, btc_next)[0, 1])
    btc_leads_alt = float(np.corrcoef(btc_now, alt_next)[0, 1])
    contemporaneous = float(np.corrcoef(alt, btc)[0, 1])
    btc_variation = float(np.square(btc).sum())
    btc_final_hour = float(btc[-60:].sum())
    alt_final_hour = float(alt[-60:].sum())
    metrics = np.asarray([
        alt_leads_btc, btc_leads_alt, contemporaneous, btc_variation,
        btc_final_hour, alt_final_hour,
    ])
    if not np.isfinite(metrics).all() or btc_variation <= 0:
        return _invalid_pair(minute_count)
    same_sign = (
        int(np.sign(btc_final_hour))
        if btc_final_hour != 0 and np.sign(btc_final_hour) == np.sign(alt_final_hour)
        else 0
    )
    return {
        "source_valid": True, "minute_count": minute_count,
        "alt_leads_btc": alt_leads_btc, "btc_leads_alt": btc_leads_alt,
        "leadership_advantage": alt_leads_btc - btc_leads_alt,
        "contemporaneous_correlation": contemporaneous,
        "btc_variation": btc_variation,
        "btc_final_hour_return": btc_final_hour,
        "alt_final_hour_return": alt_final_hour, "direction_side": same_sign,
    }
...
def build_features(pair: pd.DataFrame) -> pd.DataFrame:
    if pair.columns.tolist() != list(PAIR_COLUMNS):
        raise RuntimeError("HVABLR pair-panel schema drift")
    ...
    if decisions.isna().any() or decisions.duplicated().any() or not decisions.is_monotonic_increasing:
        raise RuntimeError("HVABLR pair-panel decision order invalid")
    ...
    features["contemporaneous_correlation_rank"] = strict_prior_midrank(
        features.contemporaneous_correlation.where(valid), *rank_args
    )
...
def active_and_side(
    ...
    if control not in ("primary", *CONTROLS):
        raise ValueError(f"unknown HVABLR control: {control}")
    ...
    elif control == "contemporaneous_correlation":
        leadership_gate = pd.to_numeric(
            used.contemporaneous_correlation_rank, errors="coerce"
    ...
    eligible = source_valid & leadership_gate & btc_variation_gate & direction_gate
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in ordered.index[onset]:
        ...
        if decision.minute != 0 or decision.second != 0 or decision.microsecond != 0:
            raise RuntimeError("HVABLR decision grid drift")
...
def write_immutable(path: Path, content: bytes) -> None:
    ...
    if path.exists():
        if path.read_bytes() != content:
            raise RuntimeError(f"refusing to overwrite immutable HVABLR artifact: {path}")
        return
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVABLR preregistration hash drift")
    registration = json.loads(prereg.DEFAULT_OUTPUT.read_text())
    prereg.validate(registration)
    if registration != REGISTRATION:
        raise RuntimeError("HVABLR committed preregistration payload drift")
    if tuple(registration["diagnostic_controls"]["names"]) != CONTROLS:
        raise RuntimeError("HVABLR diagnostic-control drift")
    ...
    source_core = {
        "protocol_version": "hvablr_6_alt_basket_leadership_source_v1",
    ...
    core = {
        "protocol_version": "hvablr_6_source_support_v1", "policy_id": prereg.POLICY_ID,
        "preregistration": {
