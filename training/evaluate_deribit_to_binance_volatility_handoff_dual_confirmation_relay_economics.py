"""Sequential strict economics for frozen DBVHDR-6."""
...
PREREG = Path("results/deribit_to_binance_volatility_handoff_dual_confirmation_relay_preregistration_2026-08-08.json")
PREREG_SHA = "285ea1acab47a34b5d5bb7eacec4867e2a2db59baf6c0ba5ca50597c50e0b718"
SUPPORT = Path("results/deribit_to_binance_volatility_handoff_dual_confirmation_relay_support_2026-08-08.json")
SUPPORT_SHA = "af5ea6006193a37808750f0f3a4ef56a972735589e7c438633fdc54a85c3ea58"
NOVELTY = Path("results/deribit_to_binance_volatility_handoff_dual_confirmation_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "8cbe053a89b19b8bcc97cca9acdad6b3474d2a0277f8a0c9c0b77050c186ca0b"
CLOCK = Path("data/deribit_to_binance_volatility_handoff_dual_confirmation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "8bb8c0d4438459202ab912d01d74acc74f55d48ad7e6d8d4e9a5300b075dc40a"
CONTROL_DIR = Path("data/deribit_to_binance_volatility_handoff_dual_confirmation_relay_controls_2023_2026")
FREEZE = Path("results/deribit_to_binance_volatility_handoff_dual_confirmation_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/deribit_to_binance_volatility_handoff_dual_confirmation_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("DBVHDR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("DBVHDR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "dbvhdr_6_sequential_economics_v1", "policy_id": POLICY_ID,
