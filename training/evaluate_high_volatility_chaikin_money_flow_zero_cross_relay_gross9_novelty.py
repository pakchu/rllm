"""Evaluate frozen HVCMF-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCMF-24"
PROTOCOL = "hvcmf_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_chaikin_money_flow_zero_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCMF predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCMF novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCMF predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCMF primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCMF-24 unchanged before economic outcomes",
    }
