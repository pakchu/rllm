"""Evaluate the frozen HVECTR-24 action-vote clock against Gross9."""
...
POLICY = "HVECTR-24"
PROTOCOL = "hvectr_24_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_energy_commodity_transition_relay_clocks_2023_2026.csv.gz",
    "sha256": "d78ae8c0bac506fd95da3ef11d814de936b7ea3dc8cbffebfdb24d056bfefa83",
    "rows": 125,
}
OUTPUT = Path("results/high_volatility_energy_commodity_transition_relay_gross9_novelty_2026-08-18.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVECTR-24 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVECTR-24 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVECTR-24 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVECTR-24 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVECTR-24 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVECTR-24 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVECTR-24 unchanged before economics",
    }
