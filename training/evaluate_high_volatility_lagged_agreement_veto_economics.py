"""Strict staged economics for the singleton HVLAV-8 policy."""
...
PREREG = Path("results/high_volatility_lagged_agreement_veto_preregistration_2026-08-16.json")
PREREG_SHA = "391656b8a3d866b763d893d973691a0eb8cdce56ede6f7ef51191693fef1d26a"
SUPPORT = Path("results/high_volatility_lagged_agreement_veto_support_2026-08-16.json")
SUPPORT_SHA = "6322ffd5c6001f736210cdfa7bf4d7f36e1454f846bb8254411e5896e160d3e6"
GROSS9 = Path("results/high_volatility_lagged_agreement_veto_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "d2fbcd4ad1d1200bb600220423b4deb0bd4c38a41553c808f92a98217c5d2f64"
CLOCK = {
    "path": Path("data/high_volatility_lagged_agreement_veto_clocks_2023_2026.csv.gz"),
    "sha256": "7eedc539d66cd42c3e8fb9038d8788a21704f63fec25fcdfc73d8d36429c0dd6",
    "rows": 146,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_lagged_agreement_veto_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVLAV-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVLAV-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVLAV-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVLAV-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVLAV-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVLAV-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVLAV-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVLAV-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVLAV-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVLAV-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVLAV-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvlav_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
