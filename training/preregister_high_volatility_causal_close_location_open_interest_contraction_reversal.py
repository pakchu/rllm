"""Outcome-blind preregistration for HVCCLOICR-8."""
...
DEFAULT_OUTPUT = Path("results/high_volatility_causal_close_location_open_interest_contraction_reversal_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_causal_close_location_open_interest_contraction_reversal_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": "When a completed BTC perpetual eight-hour cycle closes in the directional outer 35 percent of its exact high-low range, realized variation is elevated, and open interest contracts, the directional close is liquidation-driven without fresh leveraged sponsorship and should reverse over eight hours.",
            "side": "opposite strict sign of the completed eight-hour BTC perpetual return",
            "why_distinct": "HVCCLOICR requires an exact completed-range close location with exact OI contraction and variation. No prior composite event set or control is reused; it uses no spot, ETH, funding, premium-index, fitted outcome, or promoted control.",
            "why_suited_to_volatile_regimes": "the completed eight-hour BTC realized variation must be in its causal upper 40 percent",
            "why_low_gross9_overlap_is_plausible": "fixed eight-hour close-location plus OI-contraction exhaustion states are absent from Gross9 primitives",
        },
        "features": {
            ...
            "eligible": "exact coherent 480-row OHLC cycle, nonzero completed return, directional outer-35-percent completed-range close location, two exact OI endpoints with strict contraction, and the variation rank passes at D; no taker-flow sign, spot source, funding, premium-index, alt, or post-decision condition",
            "no_imputation": True,
        },
        "clock": {
        ...
        "research_boundary": {
            "prior_HVCCLOIAR_source_novelty_and_train_failure_known": True,
            "same_mechanism_as_HVCCLOIAR": False,
        ...
        "stopping_rule": "terminal first failure; no OI endpoint or contraction rule, close-location confirmation, variation history, side, clock, hold, subset, threshold, comparator, or control repair",
    }
