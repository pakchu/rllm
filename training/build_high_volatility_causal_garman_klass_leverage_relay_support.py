"""Deterministic outcome-blind source support for HVCGKR-24."""
...
from training import preregister_high_volatility_causal_garman_klass_leverage_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "ba7c9422df04694c2c1004cba21cd5b0786d2cd79e56ab955db87b57eb5a3053"
...
BAR_QUERY = """SELECT date_bin('1 day',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '1 day' AS decision_time,(array_agg(open ORDER BY ts))[1] AS daily_open,max(high) AS daily_high,min(low) AS daily_low,(array_agg(close ORDER BY ts DESC))[1] AS daily_close,sum(power(ln(close/open),2)) AS minute_squared_return,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND high>=greatest(open,close,low) AND low<=least(open,close,high)) AS coherent FROM bars_binance WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end GROUP BY 1 ORDER BY 1"""

ROOT = Path("data/high_volatility_causal_garman_klass_leverage_relay_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_garman_klass_leverage_relay_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_garman_klass_leverage_relay_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_causal_garman_klass_leverage_relay_support_2026-08-16.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "sum_open_interest",
    "previous_open_interest", "oi_change", "daily_open", "daily_high", "daily_low",
    "daily_close", "body", "garman_klass_variance", "directional_efficiency", "efficiency_rank", "realized_variation",
    "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "decision_time", "feature_available_time",
    "entry_time", "exit_time", "side", "oi_change", "body",
    "garman_klass_variance", "directional_efficiency", "efficiency_rank", "realized_variation", "variation_rank",
)
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVCGKR-24 OI schema drift")
    expected_bars = [
        "decision_time", "daily_open", "daily_high", "daily_low", "daily_close",
        "minute_squared_return",
        "source_rows", "distinct_rows", "first_ts", "last_ts", "coherent",
    ]
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCGKR-24 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all():
        raise RuntimeError("HVCGKR-24 invalid OI rows")

    bars = bars.copy()
    for column in ("decision_time", "first_ts", "last_ts"):
        bars[column] = pd.to_datetime(bars[column], utc=True, errors="raise")
    numeric = ("daily_open", "daily_high", "daily_low", "daily_close", "minute_squared_return", "source_rows", "distinct_rows")
    for column in numeric:
        bars[column] = pd.to_numeric(bars[column], errors="raise")
    if bars["decision_time"].duplicated().any():
        raise RuntimeError("HVCGKR-24 duplicate daily bar")
    start = bars["decision_time"] - pd.Timedelta("24h")
    bars["bar_valid"] = (
        np.isfinite(bars[list(numeric)]).all(axis=1)
        & bars[["daily_open", "daily_high", "daily_low", "daily_close"]].gt(0).all(axis=1)
        & bars["daily_high"].ge(bars[["daily_open", "daily_close", "daily_low"]].max(axis=1))
        & bars["daily_low"].le(bars[["daily_open", "daily_close", "daily_high"]].min(axis=1))
    ...
    bars["body"] = np.log(bars["daily_close"] / bars["daily_open"])
    log_range = np.log(bars["daily_high"] / bars["daily_low"])
    bars["garman_klass_variance"] = 0.5 * np.square(log_range) - (2 * math.log(2) - 1) * np.square(bars["body"])
    bars["directional_efficiency"] = np.square(bars["body"]) / bars["garman_klass_variance"]
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel = panel.join(
        bars[["bar_valid", "daily_open", "daily_high", "daily_low", "daily_close", "body", "garman_klass_variance", "directional_efficiency", "realized_variation"]],
        on="decision_time",
    )
    panel["source_valid"] = (
        ...
        & np.isfinite(
            panel[["sum_open_interest", "previous_open_interest", "oi_change", "daily_open", "daily_high", "daily_low", "daily_close", "body", "garman_klass_variance", "directional_efficiency", "realized_variation"]]
        ).all(axis=1)
        & panel["sum_open_interest"].gt(0)
        & panel["previous_open_interest"].gt(0)
        & panel["realized_variation"].gt(0)
        & panel["body"].ne(0)
        & panel["garman_klass_variance"].gt(0)
        & panel["directional_efficiency"].gt(0)
    )
    valid = panel["source_valid"].eq(True)
    panel["efficiency_rank"] = causal_midrank(panel["directional_efficiency"].where(valid))
    panel["variation_rank"] = causal_midrank(panel["realized_variation"].where(valid))
    panel["eligible"] = (
        valid
        & panel["oi_change"].gt(0)
        & panel["efficiency_rank"].ge(POLICY["efficiency_rank_min"])
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if split is None:
            continue
        side = int(np.sign(row.body))
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCGKR-24 side or availability drift")
        ...
        rows.append({
            ...
            "body": float(row.body),
            "garman_klass_variance": float(row.garman_klass_variance),
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCGKR-24 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcgkr_24_sources_v1",
    ...
    core = {
        "protocol_version": "hvcgkr_24_source_support_v1", "policy_id": prereg.POLICY_ID,
