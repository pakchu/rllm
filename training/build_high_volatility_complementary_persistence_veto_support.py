"""Materialize deterministic, outcome-blind source support for HVCPV-8."""
...
from training import preregister_high_volatility_complementary_persistence_veto as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv

PREREG_SHA = "3695cba4142d9c116a1a5ae859177dbd42f7ae9b65e58461c3bce09ab07ee19e"
CLOCK = Path("data/high_volatility_complementary_persistence_veto_clocks_2023_2026.csv.gz")
RESULT = Path("results/high_volatility_complementary_persistence_veto_support_2026-08-16.json")
...
def _registration() -> dict[str, Any]:
    if prereg.sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCPV-8 preregistration hash drift")
    ...
    for component, artifacts in prereg.COMPONENT_ARTIFACTS.items():
        for kind, artifact in artifacts.items():
            if prereg.sha256_file(artifact["path"]) != artifact["sha256"]:
                raise RuntimeError(f"HVCPV-8 {component} {kind} drift")
...
def build_clock(clocks: Mapping[str, pd.DataFrame]) -> tuple[pd.DataFrame, dict[str, Any]]:
    if set(clocks) != set(prereg.COMPONENT_IDS):
        raise RuntimeError("HVCPV-8 component roster drift")
    ...
    for decision in decisions:
        ...
        if primary.split != split or primary.feature_available_time > entry:
            raise RuntimeError("HVCPV-8 primary stage or availability drift")
        if veto is not None and (veto.split != split or veto.feature_available_time > entry):
            raise RuntimeError("HVCPV-8 veto stage or availability drift")
        veto_side = 0 if veto is None else veto.side
        side = prereg.veto_side(primary.side, veto_side)
        if veto is None:
    ...
    if counts["emitted_decisions"] + counts["veto_opposite_side"] != counts["primary_decisions"]:
        raise RuntimeError("HVCPV-8 incidence accounting drift")
    if len(output) > 1 and not output["entry_time"].iloc[1:].reset_index(drop=True).ge(output["exit_time"].iloc[:-1].reset_index(drop=True)).all():
        raise RuntimeError("HVCPV-8 output overlap")
...
def run(clock_path: Path = CLOCK, result_path: Path = RESULT) -> dict[str, Any]:
    ...
    core = {"protocol_version": "hvcpv_8_source_support_v1", "policy_id": prereg.POLICY_ID, "preregistration": {"path": str(prereg.DEFAULT_OUTPUT), "sha256": PREREG_SHA, "manifest_hash": registration["manifest_hash"]}, "verified_component_artifacts": verified, "component_clock_fields_opened": list(FIELDS), "combined_incidence_opened": True, "combined_postentry_returns_or_pnl_opened": False, "entry_exit_prices_opened": False, "returns_opened": False, "funding_opened": False, "pnl_opened": False, "gross9_comparator_rows_opened": False, "clock": {"path": str(clock_path), "sha256": prereg.sha256_file(clock_path), "rows": len(clock)}, "opposite_veto_accounting": accounting, "support": stats, "support_checks": checks, "support_passed": passed, "advance_to_gross9_novelty": passed, "advance_to_economic_outcomes": False, "decision": "pass_to_gross9_novelty" if passed else "terminal_source_support_reject"}
...
if __name__ == "__main__":
    ...
    print(json.dumps({"events": {k: v["events"] for k, v in report["support"].items()}, "accounting": report["opposite_veto_accounting"], "support_passed": report["support_passed"]}, indent=2))
