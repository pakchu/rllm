"""Fit and freeze HVDLDA-24 using only pre-OOS labels and calibration sources."""
...
RESULT = Path("results/high_volatility_discriminant_state_relay_model_freeze_2026-08-09.json")
...
def load_training_frame() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if missing:
        raise RuntimeError(f"HVDLDA pretraining feature drift: {missing}")
...
def run() -> dict[str, Any]:
    ...
    if int(fit_mask.sum()) < 1000 or int(calibration_mask.sum()) < 500:
        raise RuntimeError("HVDLDA pretraining/calibration row floor failed")
    ...
    low, high = np.quantile(probabilities, (0.15, 0.85))
    ...
    if len(calibration_volatility) < 500:
        raise RuntimeError("HVDLDA calibration volatility floor failed")
    ...
    if not (0.0 <= low < high <= 1.0) or not np.isfinite(volatility_threshold):
        raise RuntimeError("HVDLDA calibration threshold drift")
    artifact = {
        "policy_id": "HVDLDA-24",
        "pipeline": pipeline,
    ...
    core = {
        "protocol_version": "hvlda_24_model_freeze_v1",
        "policy_id": "HVDLDA-24",
        "preregistration": {
        ...
        "calibration": {
            ...
            "probability_low_q15": float(low),
            "probability_high_q85": float(high),
