"""Build source-only CLRCR-6 confirmation-ladder clocks."""
...
from training import preregister_confirmation_ladder_reversal_completion_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.download_fetls_block_summaries import OUTPUT as BLOCKS


ENV_FILE = "/home/pakchu/rllm/.env"
PREREG_SHA = "cd816d1b20cd0f87bb69bdf9181dbd61ba5bb9e577abfff95b79db7726d646aa"
...
CONTROLS = ("late_unanimity_only", "completion_dominance_only", "one_anchor_stale_ladder", "direction_flip")
...
SOURCE_DIR = Path("data/confirmation_ladder_reversal_completion_relay_sources_2023_2026")
FEATURES = SOURCE_DIR / "confirmation_ladders.csv.gz"
MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/confirmation_ladder_reversal_completion_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/confirmation_ladder_reversal_completion_relay_controls_2023_2026")
RESULT = Path("results/confirmation_ladder_reversal_completion_relay_support_2026-08-09.json")
...
FEATURE_COLUMNS = (
    ...
    *RETURN_COLUMNS, "early_return", "late_return", "completion_ratio",
    "late_unanimous", "eligible_state",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "anchor_height", "confirmation_height",
    "feature_available_time", "entry_time", "exit_time", "side", "early_return",
    "late_return", "completion_ratio", "late_unanimous",
)
...
def validate_blocks(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CLRCR block schema drift")
    ...
    if not np.array_equal(np.diff(blocks.height), np.ones(len(blocks) - 1, dtype=np.int64)):
        raise RuntimeError("CLRCR block heights are not contiguous")
    if not blocks.previousblockhash.iloc[1:].reset_index(drop=True).equals(blocks.id.iloc[:-1].reset_index(drop=True)):
        raise RuntimeError("CLRCR canonical previous-hash continuity failed")
    return blocks
...
def prepare_minutes(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CLRCR minute schema drift")
    ...
    if result.ts.duplicated().any():
        raise RuntimeError("CLRCR duplicate timestamps")
...
def build_features(blocks: pd.DataFrame, market: pd.DataFrame) -> pd.DataFrame:
    ...
    for index in range(0, len(blocks) - 6):
        ...
        completion = abs(late) / abs(early) if valid and early != 0 else np.nan
        valid = bool(valid and np.isfinite([early, late, completion]).all() and early != 0 and late != 0 and completion > 0)
        late_signs = np.sign(returns[3:]) if valid else np.array([])
        late_unanimous = bool(
            valid and len(late_signs) == 3
            and np.all(late_signs == late_signs[0])
            and late_signs[0] != 0 and late_signs[0] == -np.sign(early)
        )
        rows.append({
            ...
            **{column: returns[position] if valid else np.nan for position, column in enumerate(RETURN_COLUMNS)},
            ...
            "completion_ratio": completion if valid else np.nan,
            "late_unanimous": late_unanimous,
        })
    output = pd.DataFrame(rows)
    output["eligible_state"] = (
        output.source_valid & output.late_unanimous & output.completion_ratio.gt(1.0)
    )
...
def active_and_side(features: pd.DataFrame, control: str = "primary"):
    ...
    elif control == "completion_dominance_only":
        state = features.source_valid & reversal & features.completion_ratio.gt(1.0)
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active & sides.ne(0)]:
        ...
        rows.append({
            ...
            "completion_ratio": float(features.at[index, "completion_ratio"]), "late_unanimous": bool(features.at[index, "late_unanimous"]),
        })
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("CLRCR preregistration drift")
    ...
    source_core = {
        "protocol_version": "clrcr_6_source_v1", "query_sha256": hashlib.sha256(QUERY.encode()).hexdigest(), "window": [QUERY_START.isoformat(), END.isoformat()],
    ...
    core = {
        "protocol_version": "clrcr_6_source_support_v1", "policy_id": prereg.POLICY_ID,
