"""Evaluate frozen ASCR-16 structural novelty against every Gross9 sleeve."""
...
POLICY = "ASCR-16"
PROTOCOL = "ascr_16_gross9_novelty_v1"
...
OUTPUT = Path("results/asian_session_concordance_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("ASCR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("ASCR novelty limits drift")
    if (
        ...
        raise RuntimeError("ASCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="ASCR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "ascr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject ASCR-16 unchanged before economic outcomes",
    }
