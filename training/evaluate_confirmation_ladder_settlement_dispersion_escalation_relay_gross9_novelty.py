"""Evaluate frozen CLSDER-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "CLSDER-6"
PROTOCOL = "clsder_6_gross9_novelty_v1"
...
OUTPUT = Path("results/confirmation_ladder_settlement_dispersion_escalation_relay_gross9_novelty_2026-08-12.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CLSDER predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("CLSDER novelty limits drift")
    if (
        ...
        raise RuntimeError("CLSDER predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CLSDER primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "clsder_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject CLSDER-6 unchanged before economic outcomes",
    }
