"""Strict staged economics for the singleton HVSMRSR-8 policy."""
...
PREREG = Path("results/high_volatility_spot_median_return_shift_relay_preregistration_2026-08-18.json")
PREREG_SHA = "a920ebc4bd9be6126f1889b263314d55d8c3ef7535fc337e404a3bc1aae12b6a"
SUPPORT = Path("results/high_volatility_spot_median_return_shift_relay_support_2026-08-18.json")
SUPPORT_SHA = "fa749dcc91f90e4b49275016e377de309cafce6b31778244bea31c2122f88f34"
GROSS9 = Path("results/high_volatility_spot_median_return_shift_relay_gross9_novelty_2026-08-18.json")
GROSS9_SHA = "11879670b6a4e6f1341f19053977cec578001b2ce576f46ea1f17362d434d6d3"
CLOCK = {
    "path": Path("data/high_volatility_spot_median_return_shift_relay_clocks_2023_2026.csv.gz"),
    "sha256": "0bf35d87f0bdd058283bc536d7c9012fd6edbf1fbc61f9871cdabf6b189b1cd3",
    "rows": 342,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_spot_median_return_shift_relay_{stage}_economics_2026-08-18.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVSMRSR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVSMRSR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVSMRSR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVSMRSR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVSMRSR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVSMRSR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVSMRSR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVSMRSR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVSMRSR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVSMRSR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVSMRSR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvsmrsr_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
