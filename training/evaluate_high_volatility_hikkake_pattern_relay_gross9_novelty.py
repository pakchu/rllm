"""Evaluate frozen HVHIKKAKE-C3-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVHIKKAKE-C3-8"
PROTOCOL = "hvhikkake_c3_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_hikkake_pattern_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVHIKKAKE predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVHIKKAKE novelty limits drift")
    if (
        ...
        raise RuntimeError("HVHIKKAKE predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVHIKKAKE primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvhikkake_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVHIKKAKE-C3-8 unchanged before economic outcomes",
    }
