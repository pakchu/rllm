"""Evaluate frozen HVQGR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVQGR-12"
PROTOCOL = "hvqgr_12_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_qqq_gold_rotation_relay_preregistration_2026-08-09.json"
)
...
OUTPUT = Path(
    "results/high_volatility_qqq_gold_rotation_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVQGR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVQGR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVQGR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVQGR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvqgr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVQGR-12 unchanged before economic outcomes"
        ),
    }
