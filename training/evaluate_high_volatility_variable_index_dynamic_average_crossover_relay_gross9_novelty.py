"""Evaluate frozen HVVIDYA-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVVIDYA-24"
PROTOCOL = "hvvidya_24_gross9_novelty_v1"
PREREG = Path("results/high_volatility_variable_index_dynamic_average_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "e95910c9e2a65a5d25d5c35b1b3e2c0988ebf282cbc299b1502c97c531d84459"
SUPPORT = Path("results/high_volatility_variable_index_dynamic_average_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "038bd460c6c387aec8b5e1fc8fdb929badc40ea27a9691b93d0d46561cb827ff"
CLOCK = Path("data/high_volatility_variable_index_dynamic_average_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "8ec9d8218eb958850727f24ce82e8fbd2005d53192aa09f62d7e7fefd172c67a"
OUTPUT = Path("results/high_volatility_variable_index_dynamic_average_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVVIDYA predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVVIDYA novelty limits drift")
    if (
        ...
        raise RuntimeError("HVVIDYA predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVVIDYA primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvvidya_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVVIDYA-24 unchanged before economic outcomes",
    }
