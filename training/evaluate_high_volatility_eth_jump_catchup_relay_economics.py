"""Sequential strict economics for frozen HVEJCR-6."""
...
PREREG = Path("results/high_volatility_eth_jump_catchup_relay_preregistration_2026-08-09.json")
PREREG_SHA = "df6f4adc36eaf2f64f854a39b989ed3182f84d8442eed133d2fa3239013e74c2"
SUPPORT = Path("results/high_volatility_eth_jump_catchup_relay_support_2026-08-09.json")
SUPPORT_SHA = "c9806adedacc65919c53a412d44688e0643360c9183dce770dcf3a88a8ff6eeb"
NOVELTY = Path("results/high_volatility_eth_jump_catchup_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "ecbd80928a6dccbc747a060e6cb41c536cc9f6fc86a7ea1f8c063bc36e1a56b8"
CLOCK = Path("data/high_volatility_eth_jump_catchup_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "6349b8c7562c985fbb898e24e3d8d4602d8aa5ced79fd8db2b421ed597e1cec5"
CONTROL_DIR = Path("data/high_volatility_eth_jump_catchup_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_eth_jump_catchup_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_eth_jump_catchup_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVEJCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVEJCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvejcr_6_sequential_economics_v1", "policy_id": POLICY_ID,
