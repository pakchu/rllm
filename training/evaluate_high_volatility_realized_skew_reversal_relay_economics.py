"""Sequential strict economics for frozen HVRSR-12."""
...
PREREG = Path("results/high_volatility_realized_skew_reversal_relay_preregistration_2026-08-09.json")
PREREG_SHA = "3e3c9323760d0aecabb789d1a0c689f2455905f0d69236b86d7eb97d4112a3d9"
SUPPORT = Path("results/high_volatility_realized_skew_reversal_relay_support_2026-08-09.json")
SUPPORT_SHA = "e6a22ec228de7e90b06dca4ffc6a903ac1bdef48b4948f7e3398eefb422f3dcd"
NOVELTY = Path("results/high_volatility_realized_skew_reversal_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "c960e028819405e1900d0b332022d7e09f0f27c446c3421fb522dd1f16591c91"
CLOCK = Path("data/high_volatility_realized_skew_reversal_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "bd0e15b0ad6ca25f5e396ef8633b42fa17d69576ea37c9956c1594e4c7507167"
CONTROL_DIR = Path("data/high_volatility_realized_skew_reversal_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_realized_skew_reversal_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_realized_skew_reversal_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVRSR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVRSR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrsr_12_sequential_economics_v1", "policy_id": POLICY_ID,
