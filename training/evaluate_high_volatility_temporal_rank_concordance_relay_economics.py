"""Sequential strict economics for frozen HVTRC-8."""
...
PREREG = Path("results/high_volatility_temporal_rank_concordance_relay_preregistration_2026-08-10.json")
PREREG_SHA = "87556ccab968c68b7ec70967680eaa6e57057e34960fb0b6e4db1aa29fbc0f39"
SUPPORT = Path("results/high_volatility_temporal_rank_concordance_relay_support_2026-08-10.json")
SUPPORT_SHA = "497c93b5f82869cb8996593c795e655bf8660e92b03aa3d206dfa940cf64456a"
NOVELTY = Path("results/high_volatility_temporal_rank_concordance_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "d9a827e7e9d07b2ba121f2b44a9e401fcf2b4465d748355a99401a9a6fd19593"
CLOCK = Path("data/high_volatility_temporal_rank_concordance_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "2bcb73ab48ee081bfe2f059ea479189d76eecbf4298d1aae498df0e9f9834c4d"
CONTROL_DIR = Path("data/high_volatility_temporal_rank_concordance_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_temporal_rank_concordance_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_temporal_rank_concordance_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_concordance_tail", "no_variation_gate", "raw_absolute_concordance_positive", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTRC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTRC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvtrc_8_sequential_economics_v1", "policy_id": POLICY_ID,
