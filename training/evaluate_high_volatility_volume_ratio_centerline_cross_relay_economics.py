"""Sequential strict economics for frozen HVVRAT-24."""
...
PREREG = Path("results/high_volatility_volume_ratio_centerline_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "7134e6a79f941c4f787861052c78a8f10b1b5174ef535c9e33d8d67b3d5cc945"
SUPPORT = Path("results/high_volatility_volume_ratio_centerline_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "a40afc86d03b49857aa12114d83a98d5695cb6c2a8b46ec6bcd82eefd8156edc"
NOVELTY = Path("results/high_volatility_volume_ratio_centerline_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "5728076d2ddbf692cad9e7780925eed7d8c209a450ca8ab35dfbd94f3e18a06b"
CLOCK = Path("data/high_volatility_volume_ratio_centerline_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "03dbed6f0aac250d4c72a0d920f112a2fcb657b81fd90e31c0da0ba22def7410"
CONTROL_DIR = Path("data/high_volatility_volume_ratio_centerline_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_volume_ratio_centerline_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_volume_ratio_centerline_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "net_volume_zero_cross", "one_bar_stale_cross", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVVRAT frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVVRAT novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvvrat_24_sequential_economics_v1", "policy_id": POLICY_ID,
