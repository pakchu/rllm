"""Build source-only CLCVER-6 confirmation-ladder clocks."""
...
PREREG_SHA = "7426104234c6c03ed09689889fd45e609597a579bc547928218dd8e22d0a99b0"
...
CONTROLS = ("late_unanimity_only", "velocity_only", "one_anchor_stale_ladder", "direction_flip")
...
CACHED_PANEL = Path("data/confirmation_ladder_tempo_compression_relay_sources_2023_2026/confirmation_ladders.csv.gz")
CACHED_PANEL_SHA = "3dbfd8d4418559814c2a711c2f0bd67429d57268a8b031a9d64c131b31956dd8"
SOURCE_DIR = Path("data/confirmation_ladder_confirmation_velocity_escalation_relay_sources_2023_2026")
FEATURES = SOURCE_DIR / "confirmation_ladders.csv.gz"
MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/confirmation_ladder_confirmation_velocity_escalation_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/confirmation_ladder_confirmation_velocity_escalation_relay_controls_2023_2026")
RESULT = Path("results/confirmation_ladder_confirmation_velocity_escalation_relay_support_2026-08-12.json")
...
FEATURE_COLUMNS = (
    ...
    *RETURN_COLUMNS, *DURATION_COLUMNS, "late_return", "late_unanimous", "early_velocity", "late_velocity",
    "velocity_escalation", "eligible_state",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "anchor_height", "confirmation_height",
    "feature_available_time", "entry_time", "exit_time", "side", "late_return",
    "late_unanimous", "early_velocity", "late_velocity", "velocity_escalation",
)
...
def validate_blocks(frame: pd.DataFrame) -> pd.DataFrame:
    required = {"height", "id", "previousblockhash", "timestamp", "mediantime", "tx_count", "size", "duration"}
    if set(frame) != required:
        raise ValueError("CLCVER block schema drift")
    blocks = frame.sort_values("height", kind="mergesort").reset_index(drop=True)
    for column in ("height", "timestamp", "mediantime", "duration"):
        blocks[column] = pd.to_numeric(blocks[column], errors="raise").astype("int64")
    if not blocks.duration.gt(0).all():
        raise RuntimeError("CLCVER block duration must be positive")
    if not np.array_equal(np.diff(blocks.height), np.ones(len(blocks) - 1, dtype=np.int64)):
        raise RuntimeError("CLCVER block heights are not contiguous")
    if not blocks.previousblockhash.iloc[1:].reset_index(drop=True).equals(blocks.id.iloc[:-1].reset_index(drop=True)):
        raise RuntimeError("CLCVER canonical previous-hash continuity failed")
    return blocks
...
def prepare_minutes(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CLCVER minute schema drift")
    ...
    if result.ts.duplicated().any():
        raise RuntimeError("CLCVER duplicate timestamps")
...
def build_features(blocks: pd.DataFrame, market: pd.DataFrame) -> pd.DataFrame:
    ...
    for index in range(0, len(blocks) - 6):
        ...
        durations = [int(blocks.duration.iloc[index + offset]) for offset in range(1, 7)]
        velocities = [abs(value) / duration for value, duration in zip(returns, durations)] if valid else [np.nan] * 6
        early_velocity = float(np.mean(velocities[:3])) if valid else np.nan
        late_velocity = float(np.mean(velocities[3:])) if valid else np.nan
        velocity_escalation = bool(valid and late_velocity > early_velocity)
        rows.append({
            ...
            **{column: durations[position] for position, column in enumerate(DURATION_COLUMNS)},
            "late_return": late if valid else np.nan, "late_unanimous": late_unanimous,
            "early_velocity": early_velocity, "late_velocity": late_velocity,
            "velocity_escalation": velocity_escalation,
        })
    output = pd.DataFrame(rows)
    output["eligible_state"] = output.source_valid & output.late_unanimous & output.velocity_escalation
...
def build_features_from_cache(cached: pd.DataFrame) -> pd.DataFrame:
    input_columns = (
        "anchor_height", "confirmation_height", "feature_available_time", "source_valid",
        *RETURN_COLUMNS, *DURATION_COLUMNS,
    )
    if not set(input_columns).issubset(cached.columns):
        raise RuntimeError("CLCVER cached pre-entry panel schema drift")
    ...
    for column in (*RETURN_COLUMNS, *DURATION_COLUMNS):
        output[column] = pd.to_numeric(output[column], errors="raise")
    valid = output.source_valid.astype(bool)
    returns = output.loc[:, RETURN_COLUMNS].to_numpy(float)
    durations = output.loc[:, DURATION_COLUMNS].to_numpy(float)
    valid &= np.isfinite(returns).all(axis=1) & np.isfinite(durations).all(axis=1) & (durations > 0).all(axis=1) & (returns != 0).all(axis=1)
    velocities = np.abs(returns) / durations
    ...
    output["early_velocity"] = velocities[:, :3].mean(axis=1)
    output["late_velocity"] = velocities[:, 3:].mean(axis=1)
    output["velocity_escalation"] = valid & output.late_velocity.gt(output.early_velocity)
    output["source_valid"] = valid
    output["eligible_state"] = valid & output.late_unanimous & output.velocity_escalation
...
def active_and_side(features: pd.DataFrame, control: str = "primary"):
    ...
    elif control == "velocity_only":
        state = features.source_valid & features.velocity_escalation
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active & sides.ne(0)]:
        ...
        rows.append({
            ...
            "late_unanimous": bool(features.at[index, "late_unanimous"]),
            "early_velocity": float(features.at[index, "early_velocity"]), "late_velocity": float(features.at[index, "late_velocity"]),
            "velocity_escalation": bool(features.at[index, "velocity_escalation"]),
        })
...
def run() -> dict[str, Any]:
    ...
    source_core = {
        "protocol_version": "clcver_6_source_v1", "query_sha256": hashlib.sha256(QUERY.encode()).hexdigest(), "window": [QUERY_START.isoformat(), END.isoformat()],
    ...
    core = {
        "protocol_version": "clcver_6_source_support_v1", "policy_id": prereg.POLICY_ID,
