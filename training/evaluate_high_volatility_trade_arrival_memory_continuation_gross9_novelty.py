"""Evaluate frozen HVTAMC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVTAMC-8"
PROTOCOL = "hvtamc_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_trade_arrival_memory_continuation_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTAMC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVTAMC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTAMC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTAMC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvtamc_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVTAMC-8 unchanged before economic outcomes",
    }
