"""Evaluate frozen HVGBR-72 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVGBR-72"
PROTOCOL = "hvgbr_72_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_gradient_boosting_regression_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVGBR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVGBR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVGBR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVGBR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvgbr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVGBR-72 unchanged before economic outcomes"
        ),
    }
