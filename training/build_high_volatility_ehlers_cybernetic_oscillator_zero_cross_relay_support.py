"""Build outcome-blind source support for frozen HVECO-24."""
...
from training import preregister_high_volatility_ehlers_cybernetic_oscillator_zero_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "ab893e77036f113b16553fbb15b3f594f59af13990f362c2da2d5eb0b9fad6b4"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ehlers_cybernetic_oscillator_zero_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ehlers_cybernetic_oscillator_zero_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ehlers_cybernetic_oscillator_zero_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ehlers_cybernetic_oscillator_zero_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ehlers_cybernetic_oscillator_zero_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "run_length", "high_pass", "smoothed_high_pass", "rms",
    "cybernetic_oscillator", "rate", "long_cybernetic_oscillator", "entry_side",
    "long_entry_side", "rate_entry_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "run_length", "high_pass", "smoothed_high_pass",
    "rms", "cybernetic_oscillator", "rate", "long_cybernetic_oscillator", "entry_side",
    "long_entry_side", "rate_entry_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVECO source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVECO invalid source key")
...
def cybernetic_filter(series: pd.Series, valid: pd.Series, hp_period: int) -> pd.DataFrame:
    hp_period=float(hp_period);ss_period=float(P["low_pass_periods"])
    hp_a1=math.exp(-1.414*math.pi/hp_period);hp_c2=2*hp_a1*math.cos(1.414*math.pi/hp_period);hp_c3=-hp_a1*hp_a1;hp_c1=(1+hp_c2-hp_c3)/4
    a1=math.exp(-1.414*math.pi/ss_period);c2=2*a1*math.cos(1.414*math.pi/ss_period);c3=-a1*a1;c1=1-c2-c3
    values=series.to_numpy(float);ok=valid.to_numpy(bool);hp=np.full(len(series),np.nan);filt=np.full(len(series),np.nan);rms=np.full(len(series),np.nan);osc=np.full(len(series),np.nan);runs=np.zeros(len(series),dtype=int);run=0;window=[]
    for i,(price,is_valid) in enumerate(zip(values,ok)):
        if not is_valid or not math.isfinite(price):run=0;window=[];continue
        run+=1;runs[i]=run
        if run<=3:hp[i]=0.;filt[i]=hp[i]
        else:
            hp[i]=hp_c1*(price-2*values[i-1]+values[i-2])+hp_c2*hp[i-1]+hp_c3*hp[i-2]
            filt[i]=c1*(hp[i]+hp[i-1])/2+c2*filt[i-1]+c3*filt[i-2]
        window.append(float(filt[i]));window=window[-P["rms_periods"]:]
        if len(window)==P["rms_periods"]:
            scale=math.sqrt(float(np.mean(np.square(window))))
            if scale>0:rms[i]=scale;osc[i]=filt[i]/scale
    oscillator=pd.Series(osc,index=series.index)
    return pd.DataFrame({"run_length":runs,"high_pass":hp,"smoothed_high_pass":filt,"rms":rms,"cybernetic_oscillator":oscillator,"rate":oscillator-oscillator.shift(1)},index=series.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    fields=["bar_open","bar_high","bar_low","bar_close"];bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[fields]).all(axis=1)&bars[fields].gt(0).all(axis=1);values=cybernetic_filter(bars.bar_close,bars.valid_bar,P["high_pass_periods"]);long=cybernetic_filter(bars.bar_close,bars.valid_bar,P["long_control_high_pass_periods"])
    for column in values:bars[column]=values[column]
    bars["long_cybernetic_oscillator"]=long.cybernetic_oscillator;bars["entry_side"]=cross_side(bars.cybernetic_oscillator);bars["long_entry_side"]=cross_side(bars.long_cybernetic_oscillator);bars["rate_entry_side"]=cross_side(bars.rate);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["high_pass","smoothed_high_pass","rms","cybernetic_oscillator","variation"];bars["source_valid"]=bars.valid_bar&bars.run_length.ge(P["rms_periods"])&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="long_250_20_cybernetic":side=pd.to_numeric(used.long_entry_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVECO prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hveco_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hveco_24_source_support_v1", "policy_id": prereg.POLICY_ID,
