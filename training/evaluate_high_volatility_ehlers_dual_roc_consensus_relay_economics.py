"""Sequential strict economics for frozen HVEDR-24."""
...
PREREG = Path("results/high_volatility_ehlers_dual_roc_consensus_relay_preregistration_2026-08-11.json")
PREREG_SHA = "629288a392c64bd708e6f3ff167bceca1d8c77a6dafb1ff07319be2135ea08b3"
SUPPORT = Path("results/high_volatility_ehlers_dual_roc_consensus_relay_support_2026-08-11.json")
SUPPORT_SHA = "3bad3cfbb3a7f436049872c280c4df06e14b315a3beb715f43df14b5a66ae95b"
NOVELTY = Path("results/high_volatility_ehlers_dual_roc_consensus_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "48754b507bb6d63b87a3768d35bea91f76d6dea2e0be6de994761f1225a8c687"
CLOCK = Path("data/high_volatility_ehlers_dual_roc_consensus_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "718a8799753754a4a4824b0670bdce9c7c75f7316f09eb949cc1e9e93c590444"
CONTROL_DIR = Path("data/high_volatility_ehlers_dual_roc_consensus_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_ehlers_dual_roc_consensus_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ehlers_dual_roc_consensus_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "fast_roc_only", "slow_roc_only", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVEDR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVEDR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvedr_24_sequential_economics_v1", "policy_id": POLICY_ID,
