"""Sequential strict economics for frozen HVCIIC-8."""
...
PREREG = Path("results/high_volatility_cross_alt_illiquidity_impulse_consensus_relay_preregistration_2026-08-13.json")
PREREG_SHA = "294f136efe593478a53f5abfd5a8ef9e6048b0b9f27d55adb5c6f301c4efb157"
SUPPORT = Path("results/high_volatility_cross_alt_illiquidity_impulse_consensus_relay_support_2026-08-13.json")
SUPPORT_SHA = "7a0474f5df4af997ab1dec6c891420bc6697acffc2200403845a4aed0cb3c1f7"
NOVELTY = Path("results/high_volatility_cross_alt_illiquidity_impulse_consensus_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "b3050b9ba8d3085722cebc3ec6abda39a38feb905a4326acf88a324d5386418b"
CLOCK = Path("data/high_volatility_cross_alt_illiquidity_impulse_consensus_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "9eb0cadd6b0cfaf6685a67e3e993a3c83c9f51136d77cc64617a24eeedaf52ee"
CONTROL_DIR = Path("data/high_volatility_cross_alt_illiquidity_impulse_consensus_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_cross_alt_illiquidity_impulse_consensus_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_alt_illiquidity_impulse_consensus_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_impulse_tail", "no_variation_gate", "raw_return_tail_consensus", "one_block_stale_impulse", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCIIC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCIIC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdfcr_12_sequential_economics_v1", "policy_id": POLICY_ID,
