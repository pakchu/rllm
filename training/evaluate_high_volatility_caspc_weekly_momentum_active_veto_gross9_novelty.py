"""Evaluate the frozen HVCASPCWMV-8 action-vote clock against Gross9."""
...
POLICY = "HVCASPCWMV-8"
PROTOCOL = "hvcaspcwmv_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_caspc_weekly_momentum_active_veto_clocks_2023_2026.csv.gz",
    "sha256": "b535d44ade990216e961f59e1d3a9e830980e369cb536ca54b174f349b945437",
    "rows": 125,
}
OUTPUT = Path("results/high_volatility_caspc_weekly_momentum_active_veto_gross9_novelty_2026-08-17.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCASPCWMV-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCASPCWMV-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCASPCWMV-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCASPCWMV-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCASPCWMV-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCASPCWMV-8 unchanged before economics",
    }
