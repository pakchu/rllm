"""Build outcome-blind source support for frozen HVCIR-24."""
...
PREREG_SHA = "50d593fe85ed6797c3870464c80bdad6655e737cb8d0dfee7397f6e2a32450d6"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_choppiness_index_release_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_choppiness_index_release_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_choppiness_index_release_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_choppiness_index_release_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_choppiness_index_release_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "true_range", "range_span", "true_range_sum",
    "choppiness", "displacement", "direction",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "true_range", "range_span", "true_range_sum",
    "choppiness", "displacement", "direction",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVCIR source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVCIR invalid source key")
...
def choppiness(
    ...
    period = P["choppiness_periods"]
    previous_close = close.shift(1)
    true_range = pd.concat(
        [high - low, (high - previous_close).abs(), (low - previous_close).abs()], axis=1
    ).max(axis=1).where(valid & valid.shift(1, fill_value=False))
    true_range_sum = true_range.rolling(period, min_periods=period).sum()
    range_span = high.rolling(period, min_periods=period).max() - low.rolling(
        period, min_periods=period
    ).min()
    complete = valid.rolling(period, min_periods=period).sum().eq(period)
    ratio = (true_range_sum / range_span).where(complete & range_span.gt(0))
    index = 100.0 * np.log10(ratio) / math.log10(period)
    return pd.DataFrame(
        {
            "true_range": true_range,
            "range_span": range_span,
            "true_range_sum": true_range_sum,
            "choppiness": index.where(np.isfinite(index)),
        }
    )
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    chop = choppiness(bars.bar_high, bars.bar_low, bars.bar_close, bars.valid_bar)
    bars = bars.join(chop)
    ...
    bars["source_valid"] = (
        bars.valid_bar
        & np.isfinite(
            bars[["true_range", "range_span", "true_range_sum", "choppiness", "displacement", "direction", "variation"]]
        ).all(axis=1)
        & bars.range_span.gt(0)
        & bars.true_range_sum.gt(0)
        & bars.direction.ne(0)
        & bars.variation.gt(0)
    )
    ...
    previous = panel.choppiness.shift(1)
    panel["eligible"] = (
        panel.source_valid
        & panel.source_valid.shift(1, fill_value=False)
        & panel.choppiness.lt(P["release_threshold"])
        & previous.ge(P["release_threshold"])
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    used = panel.copy()
    valid = used.source_valid.eq(True) & used.source_valid.shift(1, fill_value=False)
    previous = used.choppiness.shift(1)
    release = used.choppiness.lt(P["release_threshold"]) & previous.ge(P["release_threshold"])
    ...
    elif control == "one_bar_stale_release":
        state = state.shift(1, fill_value=False); side = side.shift(1, fill_value=0)
    elif control=="direction_flip":side=-side
    elif control == "range_midpoint_direction":
        midpoint = (used.bar_high.rolling(P["choppiness_periods"]).max() + used.bar_low.rolling(P["choppiness_periods"]).min()) / 2
        side = np.sign(used.bar_close - midpoint).fillna(0).astype(int)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCIR prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvcir_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvcir_24_source_support_v1", "policy_id": prereg.POLICY_ID,
