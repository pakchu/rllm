"""Sequential strict economics for frozen HVHA-24."""
...
PREREG = Path("results/high_volatility_heikin_ashi_color_flip_relay_preregistration_2026-08-11.json")
PREREG_SHA = "692c76a4b3a6f54b4b6ca251c20443eb3f14150507c1d317eb3292c5c1db2a41"
SUPPORT = Path("results/high_volatility_heikin_ashi_color_flip_relay_support_2026-08-11.json")
SUPPORT_SHA = "80ad4b0f6526b60f2aeaec37db0b6a4d1885ff68bd2ddb78a459af821d4a3458"
NOVELTY = Path("results/high_volatility_heikin_ashi_color_flip_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "7a4efaaff31bffb8cd257767b142b9c8a8fcf084a5ee696d187e2a49d2306cee"
CLOCK = Path("data/high_volatility_heikin_ashi_color_flip_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "1715f1a7a77d4ce8888b7ffc506410857e8282e57f745562701e97dcf7a1011f"
CONTROL_DIR = Path("data/high_volatility_heikin_ashi_color_flip_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_heikin_ashi_color_flip_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_heikin_ashi_color_flip_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "raw_candle_color_flip", "one_bar_stale_flip", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVHA frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVHA novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvha_24_sequential_economics_v1", "policy_id": POLICY_ID,
