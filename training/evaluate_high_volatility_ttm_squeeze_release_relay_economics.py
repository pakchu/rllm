"""Sequential strict economics for frozen HVTTS-24."""
...
PREREG = Path("results/high_volatility_ttm_squeeze_release_relay_preregistration_2026-08-11.json")
PREREG_SHA = "d8b99307e1490e639f3606e1f737c4ea989604be325a1ba21502f404b76b7b42"
SUPPORT = Path("results/high_volatility_ttm_squeeze_release_relay_support_2026-08-11.json")
SUPPORT_SHA = "9d914168c68665cbd06450dd749862e79aa7bed1e859da3824638b36de2f57b3"
NOVELTY = Path("results/high_volatility_ttm_squeeze_release_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "80c4fb08a6330078f6648ed3bfe47a18b7a88fb7420240850a951ac7e59e8883"
CLOCK = Path("data/high_volatility_ttm_squeeze_release_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "1b5f12a071afb3343eea2c7a43659113f3f59e6276fd521f8dc0e0bc37a3e362"
CONTROL_DIR = Path("data/high_volatility_ttm_squeeze_release_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_ttm_squeeze_release_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ttm_squeeze_release_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "close_sma_direction", "one_bar_stale_release", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTTS frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTTS novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvtts_24_sequential_economics_v1", "policy_id": POLICY_ID,
