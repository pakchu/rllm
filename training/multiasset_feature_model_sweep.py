...
def run(cfg: Cfg) -> dict[str, Any]:
    ...
    cache_model: dict[tuple[int, int, str, float], tuple[dict[str, np.ndarray], np.ndarray]] = {}
    for horizon in HORIZONS:
        for stride in STRIDES:
            train_pos = sample_indices(close, cfg.train_start, cfg.train_end, stride, horizon)
            for target in TARGETS:
                x, y = make_xy(close, feats, feature_names, train_pos, horizon, target)
                if len(y) < 100:
                    continue
                for l2 in L2S:
                    key = (horizon, stride, target, l2)
                    model = fit_ridge(x, y, l2)
                    score = predict_score(feats, feature_names, model, close)
                    cache_model[key] = (model, score)
                    for top_k in TOP_KS:
                        for portfolio in PORTFOLIOS:
                            for regime_filter in REGIME_FILTERS:
                                params = {"horizon": horizon, "stride": stride, "target": target, "l2": l2, "top_k": top_k, "portfolio": portfolio, "regime_filter": regime_filter}
                                res = backtest_scores(close, feats, score, params, cfg, cfg.val_start, cfg.val_end)
                                ...
                                if cfg.val_mdd_ceiling > 0 and float(res["sim"]["strict_mdd_pct"]) > cfg.val_mdd_ceiling:
                                    sc -= 1000.0 + (float(res["sim"]["strict_mdd_pct"]) - cfg.val_mdd_ceiling)
                                if float(res["stats"]["rebalance_p_value_mean_ret_approx"]) > 0.10:
                                    sc -= 0.25
                                rows.append({"score": sc, "params": params, **res})
    rows.sort(key=lambda r: r["score"], reverse=True)
    selected = rows[0]
    key = (selected["params"]["horizon"], selected["params"]["stride"], selected["params"]["target"], selected["params"]["l2"])
    model, score = cache_model[key]
    ev = backtest_scores(close, feats, score, selected["params"], cfg, cfg.eval_start, cfg.eval_end)
    coef_rows = sorted(zip(feature_names, model["coef"].tolist()), key=lambda x: abs(x[1]), reverse=True)[:30]
    report = {"config": cfg.__dict__, "assets": list(close.columns), "date_range": {"start": str(close.index.min()), "end": str(close.index.max()), "bars": int(len(close))}, "feature_count": len(feature_names), "searched": len(rows), "top_val": rows[: cfg.top_n], "selected": selected, "eval": ev, "top_abs_coefficients": coef_rows, "leakage_guard": "model fit on train only; hyperparameters selected on validation only; eval final holdout; features are rolling/ffilled from already observed timestamps; positions apply next close-to-close interval", "accounting_note": "bar-level portfolio equity with turnover fee+slippage and streaming strict MDD"}
