"""Build outcome-blind source support for frozen HVKVO-24."""
...
from training import preregister_high_volatility_klinger_volume_oscillator_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "ef8d550bae4930516e88d487618eca2160d9fe5f747c89413daf3a1aa22bdaff"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_klinger_volume_oscillator_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_klinger_volume_oscillator_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_klinger_volume_oscillator_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_klinger_volume_oscillator_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_klinger_volume_oscillator_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "bar_volume", "trend", "dm", "cm", "volume_force",
    "fast_ema", "slow_ema", "kvo", "signal", "difference", "raw_vf_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "bar_volume", "trend", "dm", "cm",
    "volume_force", "fast_ema", "slow_ema", "kvo", "signal", "difference",
    "raw_vf_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close", "volume"]:
        raise RuntimeError("HVKVO source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVKVO invalid source key")
...
def klinger_force(high: pd.Series, low: pd.Series, close: pd.Series, volume: pd.Series, valid: pd.Series) -> pd.DataFrame:
    trend=np.full(len(high),np.nan);cm=np.full(len(high),np.nan);force=np.full(len(high),np.nan);dm=(high-low).to_numpy(float)
    previous_sum=previous_dm=previous_cm=previous_trend=np.nan
    for i,(h,l,c,v,ok) in enumerate(zip(high.to_numpy(float),low.to_numpy(float),close.to_numpy(float),volume.to_numpy(float),valid.to_numpy(bool))):
        if not ok:previous_sum=previous_dm=previous_cm=previous_trend=np.nan;continue
        total=h+l+c
        if not math.isfinite(previous_sum):previous_sum=total;previous_dm=dm[i];continue
        current_trend=1.0 if total>previous_sum else -1.0
        current_cm=previous_cm+dm[i] if math.isfinite(previous_trend) and current_trend==previous_trend else previous_dm+dm[i]
        if current_cm>0:
            trend[i]=current_trend;cm[i]=current_cm;force[i]=v*abs(2.0*(dm[i]/current_cm)-1.0)*current_trend*100.0
        previous_sum=total;previous_dm=dm[i];previous_cm=current_cm;previous_trend=current_trend
    return pd.DataFrame({"trend":trend,"dm":dm,"cm":cm,"volume_force":force},index=high.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars = pd.DataFrame({"rows": groups.row_valid.sum(), "bar_open": groups.open.first(),
        "bar_high": groups.high.max(), "bar_low": groups.low.min(), "bar_close": groups.close.last(),
        "bar_volume": groups.volume.sum(min_count=240),
        "variation_component": groups.minute_sq_return.sum(min_count=240)})
    fields=["bar_open","bar_high","bar_low","bar_close","bar_volume"]
    bars["valid_bar"] = bars.rows.eq(240) & np.isfinite(bars[fields]).all(axis=1) & bars[fields].gt(0).all(axis=1)
    force=klinger_force(bars.bar_high,bars.bar_low,bars.bar_close,bars.bar_volume,bars.valid_bar)
    for column in force:bars[column]=force[column]
    force_valid=np.isfinite(bars.volume_force);bars["fast_ema"]=causal_ema(bars.volume_force,force_valid,P["fast_periods"]);bars["slow_ema"]=causal_ema(bars.volume_force,force_valid,P["slow_periods"]);bars["kvo"]=bars.fast_ema-bars.slow_ema;bars["signal"]=causal_ema(bars.kvo,np.isfinite(bars.kvo),P["signal_periods"]);bars["difference"]=bars.kvo-bars.signal
    previous_force=bars.volume_force.shift(1);bars["raw_vf_side"]=np.where((bars.volume_force>0)&(previous_force<=0),1,np.where((bars.volume_force<0)&(previous_force>=0),-1,0))
    bars["variation"] = np.sqrt(bars.variation_component.rolling(P["variation_hours"] // 4, min_periods=P["variation_hours"] // 4).sum())
    bars["source_valid"] = bars.valid_bar & np.isfinite(bars[["bar_volume","trend","dm","cm","volume_force","fast_ema","slow_ema","kvo","signal","difference","variation"]]).all(axis=1) & bars.bar_volume.gt(0) & bars.cm.gt(0) & bars.difference.ne(0) & bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h")
    panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid))
    previous=panel.difference.shift(1)
    panel["eligible"]=panel.source_valid & panel.source_valid.shift(1,fill_value=False) & previous.ne(0) & np.sign(panel.difference).ne(np.sign(previous)) & panel.variation_rank.ge(P["variation_rank_min"])
    return panel.loc[:,PANEL_COLUMNS]

def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary", *CONTROLS): raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False)
    signal=pd.to_numeric(used.difference,errors="coerce");previous=signal.shift(1)
    crossing=signal.ne(0)&previous.ne(0)&np.sign(signal).ne(np.sign(previous))
    variation=used.variation_rank.ge(P["variation_rank_min"]);side=np.sign(signal).fillna(0).astype(int)
    state=valid&side.ne(0)&crossing&variation
    if control=="no_variation_gate":state=valid&side.ne(0)&crossing
    ...
    elif control=="raw_volume_force_zero_cross":
        side=pd.to_numeric(used.raw_vf_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVKVO prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvkvo_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvkvo_24_source_support_v1", "policy_id": prereg.POLICY_ID,
