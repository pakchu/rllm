"""Sequential strict economics for frozen HVDHSM-12."""
...
PREREG = Path("results/high_volatility_daily_half_session_momentum_relay_preregistration_2026-08-11.json")
PREREG_SHA = "0d197d9dbc601220b399a1ac3942cde0568ab4fa14bc9969d8fd910aa7e6d07a"
SUPPORT = Path("results/high_volatility_daily_half_session_momentum_relay_support_2026-08-11.json")
SUPPORT_SHA = "b3d8e4ca16739f2c74d39dac8047be335bca4f7dcb6a4aa3ae0f075999ea98d2"
NOVELTY = Path("results/high_volatility_daily_half_session_momentum_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "161a6395daff1d6efee3bec35ec56b5d66b9e79bdbb1b457d7971df3bdefcd77"
CLOCK = Path("data/high_volatility_daily_half_session_momentum_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "f000f83e481d3d087921a7d45c17b4df0ff474e30b7bff2e6eeab71fce3908f1"
CONTROL_DIR = Path("data/high_volatility_daily_half_session_momentum_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_daily_half_session_momentum_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_daily_half_session_momentum_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "late_six_hour_direction", "one_day_stale_first_half", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDHSM frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDHSM novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdhsm_12_sequential_economics_v1", "policy_id": POLICY_ID,
