"""Source-only support gate for frozen HVVSSR-8."""
...
from training import preregister_high_volatility_variation_seasonality_surprise_relay as prereg
...
PREREG_SHA = "5627622554546ceccaed79bf0ec77795a2cc0e0094486f08627b521dcf5eff33"
SPLITS = {
...
CONTROLS = (
    "no_variation_surprise_tail",
    "no_variation_gate",
    "absolute_variation_tail",
    "one_block_stale_geometry",
    "direction_flip",
    "forced_long",
)
ROOT = Path("data/high_volatility_variation_seasonality_surprise_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_variation_seasonality_surprise_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_variation_seasonality_surprise_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_variation_seasonality_surprise_relay_support_2026-08-13.json")
...
def seasonal_surprise(variation: pd.Series) -> tuple[pd.Series, pd.Series]:
    references = pd.concat(
        [variation.shift(21 * week) for week in range(1, 9)], axis=1
    )
    complete = references.notna().all(axis=1)
    seasonal_reference = references.median(axis=1).where(complete)
    surprise = np.log(variation / seasonal_reference).where(
        variation.gt(0) & seasonal_reference.gt(0)
    )
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVVSSR source timestamps")
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        if not valid:
            variation = block_return = math.nan
        rows.append(
            {
    ...
    states["seasonal_reference"], states["variation_surprise"] = seasonal_surprise(
        states["realized_variation"].where(states["source_valid"])
    )
    states["source_valid"] &= states["variation_surprise"].notna()
    states["variation_rank"] = prior_rank(states["realized_variation"].where(states["source_valid"]))
    states["surprise_rank"] = prior_rank(
        states["variation_surprise"].abs().where(states["source_valid"])
    )
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvvssr_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    surprise = states["variation_surprise"]
    absolute_variation_rank = states["variation_rank"]
    if control == "one_block_stale_geometry":
        block_return, variation_rank, surprise_rank, surprise, absolute_variation_rank = [
            value.shift(1)
            for value in (
                block_return,
                variation_rank,
    ...
    if control == "no_variation_surprise_tail":
        surprise_gate = pd.Series(True, index=states.index)
    elif control == "absolute_variation_tail":
        surprise_gate = absolute_variation_rank.ge(0.80)
    else:
...
def clock(states: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
        ...
        rows.append(
            {
                ...
                "variation_surprise": float(states.at[index, "variation_surprise"]),
    ...
    columns = [
        ...
        "seasonal_reference", "variation_surprise", "surprise_rank",
    ]
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVVSSR preregistration drift")
    ...
    core = {
        "protocol_version": "hvvssr_8_source_support_v1",
        "policy_id": "HVVSSR-8",
        "preregistration": {
