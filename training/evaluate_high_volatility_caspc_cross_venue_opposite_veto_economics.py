"""Sequential strict economics for frozen HVCASPCCVDRV-8."""
...
POLICY_ID = "HVCASPCCVDRV-8"
BAR = pd.Timedelta(minutes=5)
LEVERAGE = 0.5
BASE_COST = 0.0006
STRESS_COST = 0.0010
ENV_FILE = "/home/pakchu/rllm/.env"
PREREG = Path("results/high_volatility_caspc_cross_venue_opposite_veto_preregistration_2026-08-16.json")
PREREG_SHA = "c02528f477e2c1264a9a6f926c04d8eac032c9f5d6b0b5948b908755bed3a21b"
SUPPORT = Path("results/high_volatility_caspc_cross_venue_opposite_veto_support_2026-08-16.json")
SUPPORT_SHA = "1bb413ad6eacad4245519d6f26f8101a8fb354469d6682ba304aa57c2e8944a0"
NOVELTY = Path("results/high_volatility_caspc_cross_venue_opposite_veto_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "4056c985a84976d29a4e01298337bf543f33e33127142b6351a16dc36c0346f9"
CLOCK = Path("data/high_volatility_caspc_cross_venue_opposite_veto_clocks_2023_2026.csv.gz")
CLOCK_SHA = "8dac068f55281664fb2d2f2d9c653fd96b90a10988b57661f5883c7ffc25772d"
CONTROL_DIR = Path("data/high_volatility_caspc_cross_venue_opposite_veto_controls_2023_2026")
FREEZE = Path("results/high_volatility_caspc_cross_venue_opposite_veto_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_caspc_cross_venue_opposite_veto_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCASPCCVDRV frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCASPCCVDRV novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcaspccvdrv_8_sequential_economics_v1", "policy_id": POLICY_ID,
