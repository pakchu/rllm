"""Evaluate frozen HVCAFC-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCAFC-6"
PROTOCOL = "hvcafc_6_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_cross_alt_three_bar_flip_cascade_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCAFC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCAFC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCAFC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCAFC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCAFC-6 unchanged before economic outcomes",
    }
