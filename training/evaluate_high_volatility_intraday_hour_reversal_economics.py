"""Sequential strict economics for frozen HVIHR-1."""
...
PREREG = Path("results/high_volatility_intraday_hour_reversal_preregistration_2026-08-11.json")
PREREG_SHA = "f47272927574f87db59b774f1aeef5c56ea3e42a52b5e6a59ab5911d064f484c"
SUPPORT = Path("results/high_volatility_intraday_hour_reversal_support_2026-08-11.json")
SUPPORT_SHA = "7c35df808edb23f5861d49d55b08f336e77f270d901bc9cdb419b48a5336f243"
NOVELTY = Path("results/high_volatility_intraday_hour_reversal_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "036e1a2b2548ee1a156d3b029752cd4e660a68ce56fba6ceff46f479d4494261"
CLOCK = Path("data/high_volatility_intraday_hour_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "20ca8a21ede858e1607dcc6b164bb5f9b194b7a1e02aa606603492d6e35355a0"
CONTROL_DIR = Path("data/high_volatility_intraday_hour_reversal_controls_2023_2026")
FREEZE = Path("results/high_volatility_intraday_hour_reversal_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_intraday_hour_reversal_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_predictor_magnitude_gate", "no_variation_gate", "one_day_stale_predictor", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVIHR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVIHR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvihr_1_sequential_economics_v1", "policy_id": POLICY_ID,
