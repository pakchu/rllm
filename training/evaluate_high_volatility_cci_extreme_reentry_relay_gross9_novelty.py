"""Evaluate frozen HVCCI-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCCI-24"
PROTOCOL = "hvcci_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_cci_extreme_reentry_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCCI predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCCI novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCCI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCCI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcci_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCCI-24 unchanged before economic outcomes",
    }
