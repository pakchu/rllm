"""Strict staged economics for the singleton HVECTR-24 policy."""
...
PREREG = Path("results/high_volatility_energy_commodity_transition_relay_preregistration_2026-08-18.json")
PREREG_SHA = "e822bc5895b39fcdef97997178bbf3409ea94fd412b25e00555ad4e66802f4f5"
SUPPORT = Path("results/high_volatility_energy_commodity_transition_relay_support_2026-08-18.json")
SUPPORT_SHA = "2de281eb7054c2b433c062e91b3dc162fb0d204f629cee0d02498c80ce3e2d79"
GROSS9 = Path("results/high_volatility_energy_commodity_transition_relay_gross9_novelty_2026-08-18.json")
GROSS9_SHA = "22ec7ad84cae9c4868bd67860d44b58c2575f8482255edfc6183004de3db8659"
CLOCK = {
    "path": Path("data/high_volatility_energy_commodity_transition_relay_clocks_2023_2026.csv.gz"),
    "sha256": "d78ae8c0bac506fd95da3ef11d814de936b7ea3dc8cbffebfdb24d056bfefa83",
    "rows": 125,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_energy_commodity_transition_relay_{stage}_economics_2026-08-18.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVECTR-24 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVECTR-24 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVECTR-24 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVECTR-24 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVECTR-24 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVECTR-24 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVECTR-24 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVECTR-24 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVECTR-24 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVECTR-24 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVECTR-24 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvectr_24_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
