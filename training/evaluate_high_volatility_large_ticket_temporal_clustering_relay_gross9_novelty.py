"""Evaluate frozen HVLTTC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVLTTC-8"
PROTOCOL = "hvlttc_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_large_ticket_temporal_clustering_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVLTTC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVLTTC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVLTTC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVLTTC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVLTTC-8 unchanged before economic outcomes",
    }
