"""Sequential strict economics for frozen HVECE-8."""
...
PREREG = Path("results/high_volatility_execution_count_elasticity_relay_preregistration_2026-08-12.json")
PREREG_SHA = "62094724e9d4bcb58ec3f8582cb3ce0e296debbbbfc04bb04dae2332b5ad7d47"
SUPPORT = Path("results/high_volatility_execution_count_elasticity_relay_support_2026-08-12.json")
SUPPORT_SHA = "e493f0c25654b00b0353f629bcc7495b849175557bfefd452f91674955d24e91"
NOVELTY = Path("results/high_volatility_execution_count_elasticity_relay_gross9_novelty_2026-08-12.json")
NOVELTY_SHA = "b5435f22ef269e5a8323ea42e1697ac986aa43f9aa4c1172eb324a57afd74006"
CLOCK = Path("data/high_volatility_execution_count_elasticity_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "5f280288026c9df24a1821a28209b8e8b0f4fa36c1ca209429c0f652a6e5ee48"
CONTROL_DIR = Path("data/high_volatility_execution_count_elasticity_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_execution_count_elasticity_relay_economic_evaluator_freeze_2026-08-12.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_execution_count_elasticity_relay_{stage}_economics_2026-08-12.json")
    for stage in STAGES
}
CONTROLS = ("no_elasticity_tail", "no_variation_gate", "raw_return_tail", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVECE frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVECE novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvece_8_sequential_economics_v1", "policy_id": POLICY_ID,
