"""Evaluate frozen HVTTS-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVTTS-24"
PROTOCOL = "hvtts_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_ttm_squeeze_release_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTTS predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVTTS novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTTS predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTTS primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvtts_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVTTS-24 unchanged before economic outcomes",
    }
