"""Strict staged economics for the singleton HVDIMIO-8 policy."""
...
PREREG = Path("results/high_volatility_dcs_imi_active_override_preregistration_2026-08-16.json")
PREREG_SHA = "0f65c339a809584308740821f347786f1ee03d1a53ad5144d82c63d5b4ef650b"
SUPPORT = Path("results/high_volatility_dcs_imi_active_override_support_2026-08-16.json")
SUPPORT_SHA = "512cad6f872ec16f1a1d393848dfaf7e726ae4dd9a48c53653d6f5ace096ad8a"
GROSS9 = Path("results/high_volatility_dcs_imi_active_override_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "e78124cadbf0a11fb7e345b0cb2b4dcee35891db3d363f97d6a7a103880730a4"
CLOCK = {
    "path": Path("data/high_volatility_dcs_imi_active_override_clocks_2023_2026.csv.gz"),
    "sha256": "1d00f42635f911020a79147a592f60d7b54773e0426778d3d2a027817216be3d",
    "rows": 235,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_dcs_imi_active_override_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVDIMIO-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVDIMIO-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVDIMIO-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVDIMIO-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVDIMIO-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVDIMIO-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVDIMIO-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVDIMIO-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVDIMIO-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVDIMIO-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVDIMIO-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdimio_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
