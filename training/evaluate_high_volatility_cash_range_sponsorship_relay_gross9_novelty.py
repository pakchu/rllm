"""Evaluate frozen HVCRS-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCRS-8"
PROTOCOL = "hvcrs_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_cash_range_sponsorship_relay_preregistration_2026-08-13.json")
PREREG_SHA = "40f49daffc1346d51c86f8f51aa2f5391d9e3f5c035883920c49594903414eaa"
SUPPORT = Path("results/high_volatility_cash_range_sponsorship_relay_support_2026-08-13.json")
SUPPORT_SHA = "f812c1abb209a60060f890cd3907944595c9f41e804d75234af39c63c401ee1d"
CLOCK = Path("data/high_volatility_cash_range_sponsorship_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "17e64b3dffdad35c1e0b9c9ca81548cf66ce448bf10816d77558e3550adbe0f0"
OUTPUT = Path("results/high_volatility_cash_range_sponsorship_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCRS predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCRS novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCRS predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCRS primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCRS-8 unchanged before economic outcomes",
    }
