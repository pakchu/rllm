"""Evaluate frozen CATLR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "CATLR-12"
PROTOCOL = "catlr_12_gross9_novelty_v1"
...
OUTPUT = Path("results/cross_asset_ticket_leadership_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CATLR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("CATLR novelty limits drift")
    if (
        ...
        raise RuntimeError("CATLR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CATLR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "catlr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject CATLR-12 unchanged before economic outcomes",
    }
