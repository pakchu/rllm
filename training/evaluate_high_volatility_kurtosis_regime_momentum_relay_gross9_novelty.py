"""Evaluate frozen HVKRMR-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVKRMR-24"
PROTOCOL = "hvkrmr_24_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_kurtosis_regime_momentum_relay_preregistration_2026-08-09.json"
)
...
OUTPUT = Path(
    "results/high_volatility_kurtosis_regime_momentum_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVKRMR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVKRMR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVKRMR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVKRMR primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject HVKRMR-24 unchanged before economic outcomes"
        ),
    }
