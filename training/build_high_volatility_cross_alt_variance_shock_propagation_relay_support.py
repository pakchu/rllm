"""Build outcome-blind source support for frozen HVCAVS-8."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_cross_alt_variance_shock_propagation_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2023-01-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="dbc0eadc608cd8008a5ab029cface8474d868631a3ce1f37c3f3f5ce3856f560";REG=prereg.build();P=REG["policy"];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG["stages"].items()};GATES=REG["source_support_gates"];CONTROLS=tuple(REG["diagnostic_controls"]["names"]);SYMBOLS=("BTCUSDT",*prereg.ALTS);ALTS=prereg.ALTS
...
ROOT=Path("data/high_volatility_cross_alt_variance_shock_propagation_relay_sources_2023_2026");PANEL=ROOT/"scheduled_variance_shock_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_cross_alt_variance_shock_propagation_relay_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_cross_alt_variance_shock_propagation_relay_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_cross_alt_variance_shock_propagation_relay_controls_2023_2026");RESULT=Path("results/high_volatility_cross_alt_variance_shock_propagation_relay_support_2026-08-13.json");BUILDER=Path(__file__).relative_to(Path.cwd())
BASE_COLS=("decision_time","feature_available_time","source_valid","shock_score","shocked_alts","agreeing_alts","side","btc_realized_variation","variation_rank","variation_active","eligible")
CLOCK_COLS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","shock_score","shocked_alts","agreeing_alts","btc_realized_variation","variation_rank")
...
def prepare(raw):
 ...
 if raw.columns.tolist()!=expected:raise RuntimeError("HVCAVS source schema drift")
...
def shock_frame(ret,valid):
 ...
 for decision in scheduled:
  ...
  for a in ALTS:row[f"{a}_realized_variance"]=float(alt_rv[a].at[end]) if math.isfinite(alt_rv[a].at[end]) else np.nan;row[f"{a}_displacement"]=float(alt_move[a].at[end]) if math.isfinite(alt_move[a].at[end]) else np.nan
  rows.append(row)
 return pd.DataFrame(rows)
def score_states(f,control="primary"):
 u=f.copy();ranks={}
 for a in ALTS:
  ranks[a]=midrank(u[f"{a}_realized_variance"].where(u.source_valid),P["shock_prior_decisions"],P["shock_minimum_decisions"]);u[f"{a}_shock_rank"]=ranks[a];u[f"{a}_absolute_return_rank"]=midrank(u[f"{a}_displacement"].abs().where(u.source_valid),P["shock_prior_decisions"],P["shock_minimum_decisions"])
 ...
 for i,row in u.iterrows():
  rank_name="absolute_return_rank" if control=="absolute_return_shock" else "shock_rank";chosen=[a for a in ALTS if math.isfinite(row[f"{a}_realized_variance"]) and (control=="no_shock_tail" or row[f"{a}_{rank_name}"]>=P["shock_rank_min"])]
  signs={a:int(np.sign(row[f"{a}_displacement"])) for a in chosen};weights={a:row[f"{a}_realized_variance"] for a in chosen};den=sum(weights.values());score=sum(weights[a]*signs[a] for a in chosen)/den if den>0 else np.nan;sgn=int(np.sign(score)) if math.isfinite(score) else 0;scores.append(score);selected.append(len(chosen));agree.append(sum(v==sgn for v in signs.values()) if sgn else 0)
 u["shock_score"]=scores;u["shocked_alts"]=selected;u["agreeing_alts"]=agree;minimum=3 if control=="three_alt_consensus" else P["minimum_shocked_alts"];u["side"]=np.sign(u.shock_score).fillna(0).astype(int);u["variation_active"]=u.variation_rank.ge(P["variation_rank_min"]);u["eligible"]=u.source_valid&u.shocked_alts.ge(minimum)&u.shock_score.abs().ge(P["score_absolute_min"])&u.agreeing_alts.ge(P["minimum_agreeing_alts"])&u.variation_active
 if control=="no_variation_gate":u["eligible"]=u.source_valid&u.shocked_alts.ge(minimum)&u.shock_score.abs().ge(P["score_absolute_min"])&u.agreeing_alts.ge(P["minimum_agreeing_alts"])
 if control=="one_decision_stale_shock":
  for c in ("shock_score","shocked_alts","agreeing_alts","side"):u[c]=u[c].shift(1)
  u["eligible"]=u.source_valid&u.shocked_alts.ge(minimum)&u.shock_score.abs().ge(P["score_absolute_min"])&u.agreeing_alts.ge(P["minimum_agreeing_alts"])&u.variation_active
 if control=="direction_flip":u.side=-u.side
 if control=="forced_long":u.side=1
 return u
def build_panel(raw,control="primary"):
 x=prepare(raw);grid=pd.date_range(START,END,freq="5min",inclusive="left");ret=x.pivot(index="date",columns="symbol",values="return").reindex(grid,columns=SYMBOLS);valid=x.pivot(index="date",columns="symbol",values="valid").reindex(grid,columns=SYMBOLS).eq(True);return score_states(shock_frame(ret,valid),control)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCAVS prereg drift")
