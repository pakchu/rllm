"""Sequential strict economics for frozen HVSMIE-24."""
...
PREREG = Path("results/high_volatility_smi_ergodic_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "ea11f758d83e7ee568e2a9343f9652f3f3c6c2c3a7804a613360ff5d5149fad0"
SUPPORT = Path("results/high_volatility_smi_ergodic_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "e8aefec2155661c696201a08f14aa1ffcc372f1716fa0eaf19c6a11e5bc6df6a"
NOVELTY = Path("results/high_volatility_smi_ergodic_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "3a35bda48e384c04f77c853f43cace19e3ba281cd85c3dbc8d70f5d935e90ff6"
CLOCK = Path("data/high_volatility_smi_ergodic_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "23fe208ddc9ae7c19a8bdd1c013f8ecd64a66781db1e735ff48857f79fff9e1f"
CONTROL_DIR = Path("data/high_volatility_smi_ergodic_crossover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_smi_ergodic_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_smi_ergodic_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSMIE frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSMIE novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvsmie_24_sequential_economics_v1", "policy_id": POLICY_ID,
