"""Evaluate frozen HVPOIUR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVPOIUR-8"
PROTOCOL = "hvpoiur_8_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_premium_open_interest_unwind_reversal_preregistration_2026-08-11.json"
)
...
OUTPUT = Path(
    "results/high_volatility_premium_open_interest_unwind_reversal_gross9_novelty_2026-08-11.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVPOIUR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVPOIUR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVPOIUR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVPOIUR primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject HVPOIUR-8 unchanged before economic outcomes"
        ),
    }
