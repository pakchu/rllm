"""Build outcome-blind source support for frozen HVEXDP-24.
...
from training import preregister_high_volatility_exchange_deposit_pressure_relay as prereg


POLICY_ID = "HVEXDP-24"
PREREG_SHA256 = "6ab06891bda0126c101277d8e0bf0eecf5b7cc30e5dd8aa09952bfc8010baf92"
PREREG_MANIFEST_HASH = "8fde40055dcab198734ea28e780eaea08508464cf71379a5aacb94077e4c50ee"
ENDPOINT = "https://community-api.coinmetrics.io/v4/timeseries/asset-metrics"
METRICS = ("FlowInExNtv", "AssetEODCompletionTime")
STATUS_KEYS = frozenset({"FlowInExNtv-status", "FlowInExNtv-status-time"})
...
SOURCE_DIR = Path("data/high_volatility_exchange_deposit_pressure_relay_sources_2022_2026")
...
SPLIT_DIR = Path("data/high_volatility_exchange_deposit_pressure_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_exchange_deposit_pressure_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_exchange_deposit_pressure_relay_support_2026-08-13.json")

SOURCE_COLUMNS = (
    "observation_time", "feature_available_time", "FlowInExNtv",
)
PANEL_COLUMNS = (
    "observation_time", "feature_available_time", "decision_time", "source_valid",
    "minute_count", "FlowInExNtv", "deposit_log_change", "deposit_level_rank",
    "btc_variation", "btc_variation_rank",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "observation_time", "feature_available_time",
    "decision_time", "entry_time", "exit_time", "side", "FlowInExNtv",
    "deposit_log_change", "deposit_level_rank", "btc_variation", "btc_variation_rank",
)
...
def parse_source_row(raw: Any) -> dict[str, Any]:
    ...
    if not isinstance(raw["FlowInExNtv-status"], str) or not raw["FlowInExNtv-status"].strip():
        raise ValueError("FlowInExNtv-status must be a non-empty string")
    _parse_utc(raw["FlowInExNtv-status-time"], "FlowInExNtv-status-time")
    ...
    deposit = _positive_decimal(raw["FlowInExNtv"], "FlowInExNtv")
    return {
        "observation_time": observation,
        "feature_available_time": available,
        "FlowInExNtv": str(deposit),
    }
...
def build_panel(source: pd.DataFrame, btc_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    daily["FlowInExNtv"] = pd.to_numeric(daily.FlowInExNtv, errors="coerce")
    expected = pd.date_range(SOURCE_START, SOURCE_END, freq="1d", inclusive="left")
    if (
    ...
    value_valid = np.isfinite(daily.FlowInExNtv) & daily.FlowInExNtv.gt(0)
    ...
    deposit_change = np.log(daily.FlowInExNtv / daily.FlowInExNtv.shift(1)).where(pair_valid)
    ...
    panel = pd.DataFrame({
        ...
        "FlowInExNtv": daily.FlowInExNtv.astype(float),
        "deposit_log_change": deposit_change,
        "btc_variation": variation,
    })
    panel["deposit_level_rank"] = strict_prior_midrank(
        panel.FlowInExNtv.where(panel.source_valid)
    )
...
def active(panel: pd.DataFrame, control: str = "primary") -> tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    used = panel.copy()
    signal = used.deposit_log_change
    available = used.feature_available_time
    valid = used.source_valid.eq(True)
    if control == "one_day_stale_deposit_change":
        signal = used.deposit_log_change.shift(1)
        available = used.feature_available_time.shift(1)
        valid = valid & signal.notna()
    elif control == "exchange_deposit_level_rank":
        signal = used.deposit_level_rank - 0.5
        valid = valid & signal.notna()
    volatility = (
...
def build_clock(panel: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in panel.index[eligible]:
        ...
        rows.append({
            ...
            "FlowInExNtv": float(panel.at[index, "FlowInExNtv"]),
            "deposit_log_change": float(used.at[index, "deposit_log_change"]),
            "deposit_level_rank": float(panel.at[index, "deposit_level_rank"]),
...
def run(*, fetch: Fetch | None = None) -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA256:
        raise RuntimeError("HVEXDP frozen preregistration artifact drift")
    ...
    if (
        ...
        raise RuntimeError("HVEXDP frozen preregistration payload drift")
    ...
    source_core = {
        "protocol_version": "hvexdp_24_source_materialization_v1",
    ...
    core = {
        "protocol_version": "hvexdp_24_source_support_v1",
        "policy_id": POLICY_ID,
        "preregistration": {"path": str(prereg.DEFAULT_OUTPUT), "sha256": PREREG_SHA256, "manifest_hash": registration["manifest_hash"]},
