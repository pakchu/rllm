"""Evaluate the frozen HVCEDOIR-24 action-vote clock against Gross9."""
...
POLICY = "HVCEDOIR-24"
PROTOCOL = "hvcedoir_24_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_extreme_displacement_open_interest_reversal_clocks_2020_2026.csv.gz",
    "sha256": "215ec68ce56b87193e95b25e267d93b31cfafd1e8a0a0ef472bcaf0b101a88c7",
    "rows": 253,
}
OUTPUT = Path("results/high_volatility_causal_extreme_displacement_open_interest_reversal_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCEDOIR-24 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCEDOIR-24 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCEDOIR-24 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCEDOIR-24 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCEDOIR-24 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCEDOIR-24 unchanged before economics",
    }
