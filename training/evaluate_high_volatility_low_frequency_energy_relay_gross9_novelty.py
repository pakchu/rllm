"""Evaluate frozen HVLFE-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVLFE-8"
PROTOCOL = "hvlfe_8_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_low_frequency_energy_relay_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVLFE predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVLFE novelty limits drift")
    if (
        ...
        raise RuntimeError("HVLFE predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVLFE primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvlfe_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVLFE-8 unchanged before economic outcomes"
        ),
    }
