"""Evaluate the frozen HVCSPIR-8 action-vote clock against Gross9."""
...
POLICY = "HVCSPIR-8"
PROTOCOL = "hvcspir_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_spot_perpetual_inventory_relay_clocks_2020_2026.csv.gz",
    "sha256": "718a84d4635e7b72d8ca5983c66494dfa43405e00e081b1a78b3c2e190cb0989",
    "rows": 130,
}
OUTPUT = Path("results/high_volatility_causal_spot_perpetual_inventory_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCSPIR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCSPIR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCSPIR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCSPIR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCSPIR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCSPIR-8 unchanged before economics",
    }
