"""Build outcome-blind source support for frozen HVCDBR-8."""
...
from training import preregister_high_volatility_closing_drive_breakout_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-04-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="fedf22c6f863fccd110dfa4fbe448092af22c04f785920021910a1f311d6412a"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_closing_drive_breakout_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_closing_drive_breakout_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_closing_drive_breakout_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_closing_drive_breakout_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_closing_drive_breakout_relay_support_2026-08-11.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","balance_high","balance_low","drive_open","drive_close","inside_open","accepted_side","no_inside_side","drive_body_side","stale_balance_range_side","realized_variation","variation_rank","eligible")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","balance_high","balance_low","drive_open","drive_close","inside_open","accepted_side","no_inside_side","drive_body_side","stale_balance_range_side","realized_variation","variation_rank")
...
def closing_drive_pattern(balance:pd.DataFrame,drive:pd.DataFrame,balance_high:float|None=None,balance_low:float|None=None)->tuple[float,float,float,float,bool,int,int,int]:
 high=float(balance.high.max()) if balance_high is None else float(balance_high);low=float(balance.low.min()) if balance_low is None else float(balance_low)
 drive_open=float(drive.open.iloc[0]);drive_close=float(drive.close.iloc[-1]);inside_open=low<drive_open<high;body=1 if drive_close>drive_open else -1 if drive_close<drive_open else 0
 outside=1 if drive_close>high and body==1 else -1 if drive_close<low and body==-1 else 0;accepted=outside if inside_open else 0
 return high,low,drive_open,drive_close,inside_open,accepted,outside,body
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVCDBR source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVCDBR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 source=prepare(raw);rows=[];previous_balance=None
 for decision in pd.date_range(START+pd.Timedelta("2h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   balance=block.iloc[:420];drive=block.iloc[420:];balance_high,balance_low,drive_open,drive_close,inside_open,side,no_inside,body=closing_drive_pattern(balance,drive)
   stale_side=0 if previous_balance is None else closing_drive_pattern(balance,drive,*previous_balance)[5]
   minute_returns=np.log(block.close.to_numpy(float)/block.open.to_numpy(float));var=float(np.sqrt(np.square(minute_returns).sum()));valid=all(math.isfinite(v) for v in (balance_high,balance_low,var)) and balance_high>balance_low and var>0;previous_balance=(balance_high,balance_low)
  else:balance_high=balance_low=drive_open=drive_close=var=math.nan;inside_open=False;side=no_inside=body=stale_side=0;valid=False;previous_balance=None
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"balance_high":balance_high,"balance_low":balance_low,"drive_open":drive_open,"drive_close":drive_close,"inside_open":inside_open,"accepted_side":side,"no_inside_side":no_inside,"drive_body_side":body,"stale_balance_range_side":stale_side,"realized_variation":var})
...
def active(panel:pd.DataFrame,control:str="primary"):
 ...
 if control=="no_inside_open":side=pd.to_numeric(used.no_inside_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
 elif control=="no_variation_gate":state=valid&side.ne(0)
 elif control=="drive_body_only":side=pd.to_numeric(used.drive_body_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
 elif control=="one_block_stale_balance_range":side=pd.to_numeric(used.stale_balance_range_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCDBR prereg drift")
