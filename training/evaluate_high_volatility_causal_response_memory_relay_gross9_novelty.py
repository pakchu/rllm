"""Evaluate frozen HVCRMR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCRMR-12"
PROTOCOL = "hvcrmr_12_gross9_novelty_v1"
PREREG = Path("results/high_volatility_causal_response_memory_relay_preregistration_2026-08-09.json")
PREREG_SHA = "0313017b1c3aebd690a13fb2c490159782537ede1f1971b01a9b6e611912351b"
SUPPORT = Path("results/high_volatility_causal_response_memory_relay_support_2026-08-09.json")
SUPPORT_SHA = "b5f115f5f6902546332d0efc1ab0893bb02954ba10455add61376208f526c1f8"
CLOCK = Path("data/high_volatility_causal_response_memory_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "ed2f83a284c9b98292b3731c3a50ced547a82a497c34bd11004501de1193c5f4"
OUTPUT = Path("results/high_volatility_causal_response_memory_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCRMR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCRMR novelty limits drift")
    if (
        ...
        or support.get("advance_to_economic_metrics") is not False
        or support.get("clock", {}).get("sha256") != CLOCK_SHA
    ):
        raise RuntimeError("HVCRMR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCRMR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcrmr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCRMR-12 unchanged before economic outcomes",
    }
