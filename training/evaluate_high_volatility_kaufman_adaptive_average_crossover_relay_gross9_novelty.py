"""Evaluate frozen HVKAMA-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVKAMA-24"
PROTOCOL = "hvkama_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_kaufman_adaptive_average_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVKAMA predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVKAMA novelty limits drift")
    if (
        ...
        raise RuntimeError("HVKAMA predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVKAMA primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvkama_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVKAMA-24 unchanged before economic outcomes",
    }
