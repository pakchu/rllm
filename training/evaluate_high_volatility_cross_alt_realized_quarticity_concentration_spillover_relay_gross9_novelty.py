"""Evaluate frozen HVCARQ-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCARQ-6"
PROTOCOL = "hvcarq_6_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_cross_alt_realized_quarticity_concentration_spillover_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCARQ predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCARQ novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCARQ predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCARQ primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCARQ-6 unchanged before economic outcomes",
    }
