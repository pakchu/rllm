"""Evaluate frozen HVDHSVPR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDHSVPR-8"
PROTOCOL = "hvdhsvpr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_dual_half_signed_volume_persistence_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDHSVPR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVDHSVPR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDHSVPR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDHSVPR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdtiar_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVDHSVPR-8 unchanged before economic outcomes",
    }
