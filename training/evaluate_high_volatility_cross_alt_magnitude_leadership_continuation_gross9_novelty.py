"""Evaluate the frozen HVCAMLC-8 action-vote clock against Gross9."""
...
POLICY = "HVCAMLC-8"
PROTOCOL = "hvcamlc_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_cross_alt_magnitude_leadership_continuation_clocks_2023_2026.csv.gz",
    "sha256": "36052d8b75c71a0e50dcb167372977011fcd394e0dbda94a6ba3a1115f908c82",
    "rows": 373,
}
OUTPUT = Path("results/high_volatility_cross_alt_magnitude_leadership_continuation_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCAMLC-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCAMLC-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCAMLC-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCAMLC-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCAMLC-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCAMLC-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCAMLC-8 unchanged before economics",
    }
