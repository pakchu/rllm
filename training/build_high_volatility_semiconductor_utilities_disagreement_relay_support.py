"""Materialize source-only HVSUDR-24 clocks before Gross9 or economics."""
...
PREREG_SHA = "0aa3096ecb9cb5fffbe12e55cbcd33084e33b95904f7da1cdb64cec87a2abd56"
SOURCE_SHA = "16964714cbff8850f7ee042df9bbcd52975fcc25abbc06a71c96d7079b3d28cf"
SOURCE = Path("data/high_volatility_semiconductor_utilities_disagreement_relay_sources_2022_2026/smh_xlu_sessions.csv.gz")
SOURCE_MANIFEST = SOURCE.parent / "build_manifest.json"
SOURCE_MANIFEST_SHA = "723160ad7b0185c7b0257f5f55e13c5cb180a695063fac025a53ada93919480d"
...
STATE = SOURCE.parent / "daily_sector_states.csv.gz"
CLOCK = Path("data/high_volatility_semiconductor_utilities_disagreement_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_semiconductor_utilities_disagreement_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_semiconductor_utilities_disagreement_relay_support_2026-08-11.json")
...
CONTROLS = ("no_variation_gate", "smh_only", "inverse_xlu_only", "one_session_stale_rotation", "direction_flip", "forced_long")
COLUMNS = (
    "candidate", "control", "split", "session_date", "decision_time",
    "feature_available_time", "entry_time", "exit_time", "side", "smh_return",
    "xlu_return", "sector_spread_return", "btc_variation", "btc_variation_rank",
)
...
def score_sessions(source: pd.DataFrame, market: pd.DataFrame) -> pd.DataFrame:
    ...
    if frame.session_date.duplicated().any() or not frame.session_date.is_monotonic_increasing:
        raise RuntimeError("HVSUDR source session order drift")
    for column in ("smh_open", "smh_close", "xlu_open", "xlu_close"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    frame["smh_return"] = np.log(frame.smh_close / frame.smh_open)
    frame["xlu_return"] = np.log(frame.xlu_close / frame.xlu_open)
    frame["sector_spread_return"] = frame.smh_return - frame.xlu_return
    ...
    frame["source_valid"] = np.isfinite(frame[["smh_return", "xlu_return", "sector_spread_return"]]).all(axis=1) & frame.smh_return.ne(0) & frame.xlu_return.ne(0)
    frame["disagreement"] = np.sign(frame.smh_return).ne(np.sign(frame.xlu_return))
...
def build_clock(states: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    smh, xlu, spread = used.smh_return, used.xlu_return, used.sector_spread_return
    valid = used.source_valid.eq(True) & np.isfinite(smh) & np.isfinite(xlu) & np.isfinite(spread)
    relation = np.sign(smh).ne(np.sign(xlu))
    if control in ("smh_only", "inverse_xlu_only"):
        relation = pd.Series(True, index=states.index)
    ...
    side = np.sign(smh).fillna(0).astype(int)
    if control == "inverse_xlu_only":
        side = -np.sign(xlu).fillna(0).astype(int)
    ...
    for index in states.index[active]:
        ...
        rows.append({
            ...
            "side": int(side.at[index]), "smh_return": float(smh.at[index]), "xlu_return": float(xlu.at[index]),
            "sector_spread_return": float(spread.at[index]),
...
def run() -> dict[str, Any]:
    ...
    for path, expected in bindings.items():
        if sha(path) != expected:
            raise RuntimeError(f"HVSUDR binding drift: {path}")
    ...
    core = {
        "protocol_version": "hvsudr_24_source_support_v1", "policy_id": prereg.POLICY_ID,
