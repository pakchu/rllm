"""Sequential strict economics for frozen HVEBLR-8."""
...
PREREG = Path("results/high_volatility_eth_btc_lead_response_relay_preregistration_2026-08-10.json")
PREREG_SHA = "66c88198d44c462ed8774c9a2f1c0f4698431c9103e9eac721efb0da18e5df2d"
SUPPORT = Path("results/high_volatility_eth_btc_lead_response_relay_support_2026-08-10.json")
SUPPORT_SHA = "c2df9df969983f54621284c280ca5e6499674f39db4c3cb0c29a7bbea73d10ee"
NOVELTY = Path("results/high_volatility_eth_btc_lead_response_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "efeb2896df9570cc8de2623a4c1b8d5aef3a7b1aa83d2a666b7cb0a444ed365c"
CLOCK = Path("data/high_volatility_eth_btc_lead_response_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "824b2f21070adf509ba40c3163d687777e908a98b36a2607041dbc9e4aa4228f"
CONTROL_DIR = Path("data/high_volatility_eth_btc_lead_response_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_eth_btc_lead_response_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_eth_btc_lead_response_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_response_tail", "no_variation_gate", "contemporaneous_eth_btc_response", "one_boundary_stale_response", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVEBLR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVEBLR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hveblr_8_sequential_economics_v1", "policy_id": POLICY_ID,
