"""Materialize source-only HVAFC-6 clocks."""
...
SOURCE_DIR = Path("data/high_volatility_aggressive_flow_confirmation_relay_sources_2023_2026")
PANEL = SOURCE_DIR / "eight_hour_flow_confirmation_panel.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_aggressive_flow_confirmation_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_aggressive_flow_confirmation_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_aggressive_flow_confirmation_relay_support_2026-08-08.json")
SPLITS = {
...
def materialize() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvafc_6_btc_source_v1",
        "query": QUERY,
        "table": "bars_binance",
        "symbol": "BTCUSDT",
        "interval": "1m",
...
def conditions(frame: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    magnitude_gate=pd.Series(True,index=frame.index) if control=="no_flow_magnitude_gate" else imbalance.abs().ge(.1);long=late.gt(0)&imbalance.gt(0);short=late.lt(0)&imbalance.lt(0)
    active = (
        frame.signal_valid
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvafc_6_source_support_v1",
        "policy_id": "HVAFC-6",
        "preregistration": {
