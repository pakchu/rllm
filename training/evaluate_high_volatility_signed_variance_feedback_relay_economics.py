"""Sequential strict economics for frozen HVSVF-8."""
...
PREREG = Path("results/high_volatility_signed_variance_feedback_relay_preregistration_2026-08-10.json")
PREREG_SHA = "c7193999573aa0dffe77afbd8637d7fcf482598a8124907b73bfd12d967508e8"
SUPPORT = Path("results/high_volatility_signed_variance_feedback_relay_support_2026-08-10.json")
SUPPORT_SHA = "e0566e354f2ade805201db19d6c81a7c717447937ab993a962ad9954cc1f471b"
NOVELTY = Path("results/high_volatility_signed_variance_feedback_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "d6cffcf528c6d095123cba430706898473ac150ccff11f05ddac9882efee2b69"
CLOCK = Path("data/high_volatility_signed_variance_feedback_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "4b584277f27fc8791f7ca4bc3902099b477d97f69cf9d7d36ab2e414ae6b287d"
CONTROL_DIR = Path("data/high_volatility_signed_variance_feedback_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_signed_variance_feedback_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_signed_variance_feedback_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_feedback_tail_gate", "no_variation_gate", "contemporaneous_signed_quarticity", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSVF frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSVF novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvsvf_8_sequential_economics_v1", "policy_id": POLICY_ID,
