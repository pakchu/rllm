"""Evaluate frozen HVEVC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVEVC-8"
PROTOCOL = "hvevc_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_equal_variance_clock_concordance_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVEVC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVEVC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVEVC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVEVC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvevc_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVEVC-8 unchanged before economic outcomes",
    }
