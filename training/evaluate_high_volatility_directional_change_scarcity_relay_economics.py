"""Sequential strict economics for frozen HVDCS-8."""
...
PREREG = Path("results/high_volatility_directional_change_scarcity_relay_preregistration_2026-08-10.json")
PREREG_SHA = "746594b628f0ee377de6847479d8a84bf58ae4f1a06ab2cb23dc30986b89e325"
SUPPORT = Path("results/high_volatility_directional_change_scarcity_relay_support_2026-08-10.json")
SUPPORT_SHA = "f0dc6e4a24792caba7a1967280158d8aed55de27a0c297146b2ed20b0c94d8dc"
NOVELTY = Path("results/high_volatility_directional_change_scarcity_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "0aa717adc52752a6c10cea752f9482a12b4eaf27ff12699999dfd48ac9e90232"
CLOCK = Path("data/high_volatility_directional_change_scarcity_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "055d86ba9bcb9f619a3362a242febceb28d2f1b07ebef74d5d70b7393dc542fc"
CONTROL_DIR = Path("data/high_volatility_directional_change_scarcity_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_directional_change_scarcity_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_directional_change_scarcity_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_directional_change_scarcity_tail", "no_variation_gate", "raw_directional_changes_at_most_478", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDCS frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDCS novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdcs_8_sequential_economics_v1", "policy_id": POLICY_ID,
