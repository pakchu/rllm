"""Sequential strict economics for frozen HVTCDAR-8."""
...
PREREG = Path("results/high_volatility_trade_count_dispersion_asymmetry_relay_preregistration_2026-08-10.json")
PREREG_SHA = "4a4c8842bed0faf8ff9abe6dce643c556077f337e7382eefd28507514a14dcf4"
SUPPORT = Path("results/high_volatility_trade_count_dispersion_asymmetry_relay_support_2026-08-10.json")
SUPPORT_SHA = "68a363ca7efb8b223a354ed26c9af867aab8ec83cfa53ea2457b4a868d116ef5"
NOVELTY = Path("results/high_volatility_trade_count_dispersion_asymmetry_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "097ff65f200fc034ee7b1f330bf8a28418e5a8972e942bc7c6517a46f945865d"
CLOCK = Path("data/high_volatility_trade_count_dispersion_asymmetry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "1bf7606a3b381e100b34154827c749be8a58649e9f2df26333d14f408ef66924"
CONTROL_DIR = Path("data/high_volatility_trade_count_dispersion_asymmetry_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_trade_count_dispersion_asymmetry_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_trade_count_dispersion_asymmetry_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_dispersion_tail_gate", "no_variation_gate", "mean_count_asymmetry", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTCDAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTCDAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvtcdar_8_sequential_economics_v1", "policy_id": POLICY_ID,
