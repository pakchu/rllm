"""Evaluate frozen HVCMMI-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCMMI-8"
PROTOCOL = "hvcmmi_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_crypto_market_mode_ignition_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCMMI predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCMMI novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCMMI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCMMI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcmmi_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCMMI-8 unchanged before economic outcomes",
    }
