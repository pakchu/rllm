"""Materialize source-only EMDFR-12 support clocks."""
...
PREREG_SHA="a6fb55d2186e4f6398303ee690a0742394684be0f1a5695c0adc199182919f78"
SOURCE_DIR=Path("data/emerging_market_dollar_factor_relay_sources_2023_2026"); SESSION=SOURCE_DIR/"emerging_market_dollar_factor_sessions.csv.gz"; SOURCE_MANIFEST=SOURCE_DIR/"manifest.json"
...
CLOCK=Path("data/emerging_market_dollar_factor_relay_clocks_2023_2026.csv.gz"); CONTROL_DIR=Path("data/emerging_market_dollar_factor_relay_controls_2023_2026"); RESULT=Path("results/emerging_market_dollar_factor_relay_support_2026-08-09.json")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00:00Z"),pd.Timestamp("2024-01-01T00:00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00:00Z"),pd.Timestamp("2025-01-01T00:00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00:00Z"),pd.Timestamp("2026-01-01T00:00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00:00Z"),pd.Timestamp("2026-08-01T00:00:00Z"))}; MINIMUM={"train":8,"test":12,"eval":12,"final":8}; CONTROLS=("no_volatility_gate","no_factor_tail","raw_return_factor","one_session_stale_factor","direction_flip","forced_long")
SYMBOLS=("USDMXN","USDINR","USDCNY"); DOLLAR_MULTIPLIER={"USDMXN":1.,"USDINR":1.,"USDCNY":1.}
COLUMNS=("candidate","control","split","source_day","decision_time","feature_available_time","entry_time","exit_time","side","dollar_factor","absolute_factor_rank","raw_return_factor","btc_realized_variation","btc_realized_variation_rank")
...
def features()->pd.DataFrame:
 ...
 for column in ("dollar_factor","absolute_factor_rank","raw_return_factor"):f[column]=pd.to_numeric(f[column],errors="coerce")
 ...
 d=p[["decision_time","btc_realized_variation","btc_valid"]];f=f.merge(d,on="decision_time",how="left",validate="one_to_one");f["btc_realized_variation_rank"]=strict_prior_midrank(f.btc_realized_variation.where(f.btc_valid));f["signal_valid"]=f.source_valid&f.btc_valid&np.isfinite(f[["dollar_factor","absolute_factor_rank","raw_return_factor","btc_realized_variation","btc_realized_variation_rank"]]).all(axis=1)&f.dollar_factor.ne(0);return f
def conditions(f:pd.DataFrame,control:str)->tuple[pd.Series,pd.Series]:
 factor=f.raw_return_factor if control=="raw_return_factor" else f.dollar_factor;rank=f.absolute_factor_rank
 if control=="one_session_stale_factor":factor=factor.shift(1);rank=rank.shift(1)
 tail=pd.Series(True,index=f.index) if control=="no_factor_tail" else rank.ge(.70);vol=pd.Series(True,index=f.index) if control=="no_volatility_gate" else f.btc_realized_variation_rank.ge(.65);active=f.signal_valid&np.isfinite(factor)&factor.ne(0)&tail&vol;side=-np.sign(factor);side=-side if control=="direction_flip" else side;side=pd.Series(1.,index=f.index) if control=="forced_long" else side;return active,side
def clock(f:pd.DataFrame,control:str="primary")->pd.DataFrame:
 ...
 for i in f.index[active]:
  ...
  next_allowed=exit_;rows.append({"candidate":"EMDFR-12","control":control,"split":split,"source_day":f.at[i,"source_day"],"decision_time":decision,"feature_available_time":decision,"entry_time":entry,"exit_time":exit_,"side":int(side.at[i]),"dollar_factor":float(f.at[i,"dollar_factor"]),"absolute_factor_rank":float(f.at[i,"absolute_factor_rank"]),"raw_return_factor":float(f.at[i,"raw_return_factor"]),"btc_realized_variation":float(f.at[i,"btc_realized_variation"]),"btc_realized_variation_rank":float(f.at[i,"btc_realized_variation_rank"])})
...
def run()->dict:
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("EMDFR preregistration hash drift")
