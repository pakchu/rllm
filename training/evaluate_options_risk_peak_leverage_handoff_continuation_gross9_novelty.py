"""Evaluate frozen ORPLHC-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "ORPLHC-6"
PROTOCOL = "orplhc_6_gross9_novelty_v1"
PREREG = Path(
    "results/options_risk_peak_leverage_handoff_continuation_preregistration_2026-08-11.json"
)
...
OUTPUT = Path(
    "results/options_risk_peak_leverage_handoff_continuation_gross9_novelty_2026-08-11.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("ORPLHC predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("ORPLHC novelty limits drift")
    if (
        ...
        raise RuntimeError("ORPLHC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="ORPLHC primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject ORPLHC-6 unchanged before economic outcomes"
        ),
    }
