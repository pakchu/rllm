"""Build outcome-blind source support for frozen HVTII-24."""
...
from training import preregister_high_volatility_trend_intensity_index_midline_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "f035c2e1bf089175b0bd7d19091ee9ce698d539dcd20c685b1c00112e029a3c7"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_trend_intensity_index_midline_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_trend_intensity_index_midline_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_trend_intensity_index_midline_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_trend_intensity_index_midline_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_trend_intensity_index_midline_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "major_average", "deviation", "positive_sum",
    "absolute_sum", "tii", "close_sma_difference", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "major_average", "deviation", "positive_sum",
    "absolute_sum", "tii", "close_sma_difference", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVTII source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVTII invalid source key")
...
def trend_intensity(close:pd.Series,valid:pd.Series)->pd.DataFrame:
    major=P["major_periods"];minor=P["minor_periods"];groups=(~valid).cumsum();average=close.where(valid).groupby(groups).transform(lambda x:x.rolling(major,min_periods=major).mean());deviation=(close-average).where(valid);ready=np.isfinite(deviation);minor_groups=(~ready).cumsum();positive=deviation.clip(lower=0).groupby(minor_groups).transform(lambda x:x.rolling(minor,min_periods=minor).sum());absolute=deviation.abs().groupby(minor_groups).transform(lambda x:x.rolling(minor,min_periods=minor).sum());tii=100*positive/absolute.where(absolute.gt(0));return pd.DataFrame({"major_average":average,"deviation":deviation,"positive_sum":positive,"absolute_sum":absolute,"tii":tii,"close_sma_difference":deviation},index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=pd.concat([bars,trend_intensity(bars.bar_close,bars.valid_bar)],axis=1)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum())
    bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["major_average","deviation","positive_sum","absolute_sum","tii","variation"]]).all(axis=1)&bars.absolute_sum.gt(0)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.tii.shift(1);long=previous.le(P["midline"])&panel.tii.gt(P["midline"]);short=previous.ge(P["midline"])&panel.tii.lt(P["midline"])
    panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&(long|short)&panel.variation_rank.ge(P["variation_rank_min"])
    return panel.loc[:,PANEL_COLUMNS]


def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);difference=pd.to_numeric(used.close_sma_difference if control=="close_sma_cross" else used.tii-P["midline"],errors="coerce");previous=difference.shift(1);crossover=difference.ne(0)&previous.ne(0)&np.sign(difference).ne(np.sign(previous));variation=used.variation_rank.ge(P["variation_rank_min"]);side=pd.Series(np.sign(difference),index=used.index).fillna(0).astype(int);state=valid&side.ne(0)&crossover&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVTII prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvtii_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvtii_24_source_support_v1", "policy_id": prereg.POLICY_ID,
