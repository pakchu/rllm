"""Strict staged economics for the singleton HVCAV-8 policy."""
...
GROSS9 = Path("results/high_volatility_cross_structure_action_vote_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "9e9334cdefe333ff8bb02d44a404040299a66ca0da78d0237ee4b322abdcde10"
CLOCK = {
    "path": Path("data/high_volatility_cross_structure_action_vote_clocks_2023_2026.csv.gz"),
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_structure_action_vote_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCAV-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCAV-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCAV-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCAV-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCAV-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCAV-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCAV-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCAV-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCAV-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCAV-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCAV-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcav_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
