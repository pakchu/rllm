"""Evaluate frozen HVTOCM-30M structural novelty against every Gross9 sleeve."""
...
POLICY = "HVTOCM-30M"
PROTOCOL = "hvtocm_30m_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_turn_of_candle_momentum_preregistration_2026-08-09.json"
)
PREREG_SHA = "1ef020ed2d3220400e8a4488e8670dd932dc75f8dd20867ce18abff4fcddd265"
SUPPORT = Path("results/high_volatility_turn_of_candle_momentum_support_2026-08-09.json")
SUPPORT_SHA = "40bae2633c5a48d6c2cd3528b35cc9450b20ef90ab1fa82a06e2a1f8ec3cbf6f"
CLOCK = Path("data/high_volatility_turn_of_candle_momentum_clocks_2023_2026.csv.gz")
CLOCK_SHA = "f07fb19ab50f1bc070bea91b0fc33e1402939e372b8bbf3cba263a83d2e9c5a6"
OUTPUT = Path(
    "results/high_volatility_turn_of_candle_momentum_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTOCM predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVTOCM novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTOCM predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTOCM primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdoer_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVTOCM-30M unchanged before economic outcomes"
        ),
    }
