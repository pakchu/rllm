"""Sequential strict economics for frozen CCVDAR-6."""
...
PREREG = Path("results/cboe_crypto_volatility_disagreement_absorption_relay_preregistration_2026-08-08.json")
PREREG_SHA = "12bea7a56c0540fc3c2a2c9edcbe2ca687779a7ceacee2fe13740384789379aa"
SUPPORT = Path("results/cboe_crypto_volatility_disagreement_absorption_relay_support_2026-08-08.json")
SUPPORT_SHA = "142203c4cb408860564ac3aa70a939b654348512a9938654b2f0bd88c23a71e0"
NOVELTY = Path("results/cboe_crypto_volatility_disagreement_absorption_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "fe3bfc5fb31273b78e810dfa8353457eba5ae7522fa389b12502033b90afae07"
CLOCK = Path("data/cboe_crypto_volatility_disagreement_absorption_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "14ff7b1406be9a147eebda2746cf2c2a71f60c23cdb90928e6189cfb2bb19ee3"
CONTROL_DIR = Path("data/cboe_crypto_volatility_disagreement_absorption_relay_controls_2023_2026")
FREEZE = Path("results/cboe_crypto_volatility_disagreement_absorption_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/cboe_crypto_volatility_disagreement_absorption_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
CONTROLS = ("no_crypto_disagreement", "bvol_only_disagreement", "dvol_only_disagreement", "one_session_stale_cboe_change", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CCVDAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CCVDAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "ccvdar_6_sequential_economics_v1", "policy_id": POLICY_ID,
