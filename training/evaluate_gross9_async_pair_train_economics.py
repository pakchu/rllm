"""Train-only economics for Gross9 async same-side pair candidates.

Only source-supported and Gross9-novel candidates from the frozen 36-pair family
are opened for train economic outcomes.  The raw rank-one candidate is selected
by the preregistered train CAGR/strict-MDD ranking; if that rank-one candidate
fails any train economic gate, the family terminates without substitution.
"""
...
from training import evaluate_gross9_async_pair_train_novelty as novelty
...
PROTOCOL_VERSION = "gross9_async_pair_train_economics_v1"
NOVELTY = novelty.OUTPUT
OUTPUT = Path("results/gross9_async_pair_train_economics_2026-09-02.json")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    for candidate in prereg.CANDIDATE_FAMILY:
        ...
        if source_pass and gross9_pass:
    ...
    try:
        ...
        eligible = [row for row in rows_for_selection if row["source_pass"] and row["gross9_pass"]]
        eligible.sort(
            key=lambda row: (
