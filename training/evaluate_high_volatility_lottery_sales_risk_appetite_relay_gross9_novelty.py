"""Evaluate frozen HVLSRA-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVLSRA-24"
PROTOCOL = "hvlsra_24_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_lottery_sales_risk_appetite_relay_preregistration_2026-08-12.json"
)
...
OUTPUT = Path(
    "results/high_volatility_lottery_sales_risk_appetite_relay_gross9_novelty_2026-08-12.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVLSRA predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVLSRA novelty limits drift")
    if (
        ...
        raise RuntimeError("HVLSRA predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVLSRA primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject HVLSRA-24 unchanged before economic outcomes"
        ),
    }
