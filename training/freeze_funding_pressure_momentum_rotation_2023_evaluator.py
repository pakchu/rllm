"""Freeze FPMR-1's strict evaluator before opening 2023 outcomes."""
...
from training import evaluate_funding_pressure_momentum_rotation_2023 as evaluate
...
def build_manifest(commit: str, *, clock_rows: int) -> dict[str, Any]:
    ...
    core: dict[str, Any] = {
        ...
        "decision_rule": (
            "open the singleton FPMR-1 2023 clock once under every preregistered gate; "
            "a failure retires FPMR-1 before loading any 2024 outcome"
        ),
    }
...
def validate_manifest(payload: dict[str, Any]) -> None:
    ...
    if payload.get("clock_rows") != 99:
        raise RuntimeError("FPMR-1 frozen clock row count changed")
    if payload.get("opened_windows") != []:
        raise RuntimeError("FPMR-1 freeze opened an outcome window")
    if payload.get("sealed_windows") != ["2023", "2024", "2025", "2026"]:
        raise RuntimeError("FPMR-1 sealed-window declaration changed")
    ...
    if prefix != {
        ...
        raise RuntimeError("FPMR-1 source-prefix contract changed")
...
def write_once(path: str | Path, payload: dict[str, Any]) -> str:
    ...
    if output.exists():
        ...
        if existing != payload:
            raise RuntimeError("refusing to overwrite frozen FPMR-1 evaluator")
...
def current_clean_commit() -> str:
    ...
    if status:
        raise RuntimeError("FPMR-1 evaluator source or tests are not clean at HEAD")
    commit = subprocess.check_output(["git", "rev-parse", "HEAD"], text=True).strip()
    for path in paths:
        committed = subprocess.check_output(["git", "show", f"HEAD:{path}"])
        if hashlib.sha256(committed).hexdigest() != _sha256(path):
            raise RuntimeError(f"FPMR-1 evaluator dependency is not reproducible from HEAD: {path}")
    return commit
...
def main() -> None:
    ...
    if args.output != DEFAULT_OUTPUT:
        raise ValueError("FPMR-1 evaluator freeze path is immutable")
