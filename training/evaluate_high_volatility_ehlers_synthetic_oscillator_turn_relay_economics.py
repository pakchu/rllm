"""Sequential strict economics for frozen HVESO-24."""
...
PREREG = Path("results/high_volatility_ehlers_synthetic_oscillator_turn_relay_preregistration_2026-08-11.json")
PREREG_SHA = "6bafab6faf0e6a229c6cda48cd976bf27f1ccbb3f34866f68f9ab074fde7eb0c"
SUPPORT = Path("results/high_volatility_ehlers_synthetic_oscillator_turn_relay_support_2026-08-11.json")
SUPPORT_SHA = "9ffcd735089bb9bff8cb89c4223a935c963a26e13ec2b18eb0d3c2e590e30b75"
NOVELTY = Path("results/high_volatility_ehlers_synthetic_oscillator_turn_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "e56b536cddd2681647130fed4705edc25383e2ee1863d2f2c782af544b659541"
CLOCK = Path("data/high_volatility_ehlers_synthetic_oscillator_turn_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "335d421164c192a32f42b44b4d64e2e197fc443ccb3069208fa28c6475069f35"
CONTROL_DIR = Path("data/high_volatility_ehlers_synthetic_oscillator_turn_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_ehlers_synthetic_oscillator_turn_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ehlers_synthetic_oscillator_turn_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "direct_synth_rate", "wealthlab_12_25_12", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVESO frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVESO novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hveso_24_sequential_economics_v1", "policy_id": POLICY_ID,
