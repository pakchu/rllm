"""Sequential strict economics for frozen HVABDS-8."""
...
PREREG = Path("results/high_volatility_alt_breadth_diffusion_slope_relay_preregistration_2026-08-13.json")
PREREG_SHA = "4ad369076ceb6288d9da9a58d357d12638554df038f3eff8be506c145e63154b"
SUPPORT = Path("results/high_volatility_alt_breadth_diffusion_slope_relay_support_2026-08-13.json")
SUPPORT_SHA = "2942b4ffc497c1fe688044bac4cb533e2bc2a32c2b42335b59d6c681b0f60cff"
NOVELTY = Path("results/high_volatility_alt_breadth_diffusion_slope_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "59e0ea96d227376e6e89ac2e7783d92086d65ffcece7f00e4c9d614d5781eec4"
CLOCK = Path("data/high_volatility_alt_breadth_diffusion_slope_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "743ac3f33aba4673e554d68c89e6485aaa9581e10b2cf6cd0cd8024852ce7519"
CONTROL_DIR = Path("data/high_volatility_alt_breadth_diffusion_slope_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_alt_breadth_diffusion_slope_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_alt_breadth_diffusion_slope_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_slope_tail_gate", "no_variation_gate", "endpoint_static_breadth", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVABDS frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVABDS novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvabds_8_sequential_economics_v1", "policy_id": POLICY_ID,
