"""Sequential strict economics for frozen HVPLO-8."""
...
PREREG = Path("results/high_volatility_price_level_occupation_relay_preregistration_2026-08-10.json")
PREREG_SHA = "30b6cf17893c11be3dcdba1490c7916d173da4bccc3cff1ab83f870f13aa9041"
SUPPORT = Path("results/high_volatility_price_level_occupation_relay_support_2026-08-10.json")
SUPPORT_SHA = "2f59e76f121b3d4532f6250d47e98685b1bb5ee397a0b36aff92960f2ab631ab"
NOVELTY = Path("results/high_volatility_price_level_occupation_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "76032f9d2be4cc7c044faf4cbb78550d98079ff28c8f17fa532f5afd31956946"
CLOCK = Path("data/high_volatility_price_level_occupation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "8bde2384041f94b8e64aac4317c086cb363ae9903ef0d60cfaa13503bb3a4d26"
CONTROL_DIR = Path("data/high_volatility_price_level_occupation_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_price_level_occupation_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_price_level_occupation_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_occupation_tail", "no_variation_gate", "median_level_displacement", "one_boundary_stale_occupation", "direction_flip", "same_clock_forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVPLO frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVPLO novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvplo_8_sequential_economics_v1", "policy_id": POLICY_ID,
