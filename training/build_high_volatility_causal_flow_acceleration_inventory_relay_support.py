"""Deterministic outcome-blind source support for HVCFAR-8."""
...
from training import preregister_high_volatility_causal_flow_acceleration_inventory_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "778cc0ddd400560765dd750b554528e8e9d18e2f01b58acac83d9d724060a513"
...
BAR_QUERY = """SELECT date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '8 hours' AS decision_time,sum(2*taker_buy_quote-quote_asset_volume) FILTER (WHERE ts<date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '4 hours') AS first_signed_flow,sum(quote_asset_volume) FILTER (WHERE ts<date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '4 hours') AS first_turnover,sum(2*taker_buy_quote-quote_asset_volume) FILTER (WHERE ts>=date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '4 hours') AS second_signed_flow,sum(quote_asset_volume) FILTER (WHERE ts>=date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '4 hours') AS second_turnover,sum(power(ln(close/open),2)) AS minute_squared_return,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND high>=greatest(open,close,low) AND low<=least(open,close,high) AND quote_asset_volume>=0 AND taker_buy_quote>=0 AND taker_buy_quote<=quote_asset_volume) AS coherent FROM bars_binance WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end GROUP BY 1 ORDER BY 1"""

ROOT = Path("data/high_volatility_causal_flow_acceleration_inventory_relay_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_flow_acceleration_inventory_relay_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_flow_acceleration_inventory_relay_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_causal_flow_acceleration_inventory_relay_support_2026-08-16.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "sum_open_interest",
    "previous_open_interest", "oi_change", "first_half_flow_share", "second_half_flow_share",
    "flow_acceleration", "flow_acceleration_rank", "realized_variation",
    "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "decision_time", "feature_available_time",
    "entry_time", "exit_time", "side", "oi_change", "first_half_flow_share", "second_half_flow_share",
    "flow_acceleration", "flow_acceleration_rank", "realized_variation", "variation_rank",
)
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVCFAR-8 OI schema drift")
    expected_bars = [
        "decision_time", "first_signed_flow", "first_turnover", "second_signed_flow", "second_turnover", "minute_squared_return",
        "source_rows", "distinct_rows", "first_ts", "last_ts", "coherent",
    ]
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCFAR-8 bar schema drift")
    ...
    bars["first_half_flow_share"] = bars["first_signed_flow"] / bars["first_turnover"]
    bars["second_half_flow_share"] = bars["second_signed_flow"] / bars["second_turnover"]
    bars["flow_acceleration"] = bars["second_half_flow_share"].abs() - bars["first_half_flow_share"].abs()
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel = panel.join(
        bars[["bar_valid", "first_half_flow_share", "second_half_flow_share", "flow_acceleration", "realized_variation"]],
        on="decision_time",
    )
    panel["source_valid"] = (
        ...
        & np.isfinite(
            panel[["sum_open_interest", "previous_open_interest", "oi_change", "first_half_flow_share", "second_half_flow_share", "flow_acceleration", "realized_variation"]]
        ).all(axis=1)
        & panel["sum_open_interest"].gt(0)
        & panel["previous_open_interest"].gt(0)
        & panel["realized_variation"].gt(0)
        & panel["first_half_flow_share"].ne(0)
        & panel["second_half_flow_share"].ne(0)
        & np.sign(panel["first_half_flow_share"]).eq(np.sign(panel["second_half_flow_share"]))
        & panel["flow_acceleration"].gt(0)
    )
    valid = panel["source_valid"].eq(True)
    panel["flow_acceleration_rank"] = causal_midrank(panel["flow_acceleration"].where(valid))
    panel["variation_rank"] = causal_midrank(panel["realized_variation"].where(valid))
    panel["eligible"] = (
        valid
        & panel["oi_change"].gt(0)
        & panel["flow_acceleration_rank"].ge(POLICY["flow_acceleration_rank_min"])
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if split is None:
            continue
        side = int(np.sign(row.second_half_flow_share))
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCFAR-8 side or availability drift")
        ...
        rows.append({
            ...
            "first_half_flow_share": float(row.first_half_flow_share),
            "second_half_flow_share": float(row.second_half_flow_share),
            "flow_acceleration": float(row.flow_acceleration),
            "flow_acceleration_rank": float(row.flow_acceleration_rank),
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCFAR-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcfar_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvcfar_8_source_support_v1", "policy_id": prereg.POLICY_ID,
