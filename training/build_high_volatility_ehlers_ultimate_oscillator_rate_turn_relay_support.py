"""Build outcome-blind source support for frozen HVEUO-24."""
...
from training import preregister_high_volatility_ehlers_ultimate_oscillator_rate_turn_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "f74e43a546ffeb83000a788a75e809155e051f288ba9611553d6436cb2570453"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ehlers_ultimate_oscillator_rate_turn_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ehlers_ultimate_oscillator_rate_turn_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ehlers_ultimate_oscillator_rate_turn_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ehlers_ultimate_oscillator_rate_turn_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ehlers_ultimate_oscillator_rate_turn_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "run_length", "hp_long", "hp_short", "signal", "rms",
    "ultimate_oscillator", "rate", "chart_oscillator", "chart_rate", "entry_side",
    "chart_entry_side", "oscillator_entry_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "run_length", "hp_long", "hp_short", "signal",
    "rms", "ultimate_oscillator", "rate", "chart_oscillator", "chart_rate", "entry_side",
    "chart_entry_side", "oscillator_entry_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVEUO source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVEUO invalid source key")
...
def high_pass(series: pd.Series, valid: pd.Series, period: int) -> pd.DataFrame:
    ...
    return pd.DataFrame({"run_length":runs,"high_pass":output},index=series.index)


def ultimate_oscillator(series: pd.Series, valid: pd.Series, band_edge: int) -> pd.DataFrame:
    long=high_pass(series,valid,P["bandwidth"]*band_edge);short=high_pass(series,valid,band_edge)
    signal=long.high_pass-short.high_pass;window=[];rms=np.full(len(series),np.nan);osc=np.full(len(series),np.nan)
    for i,(value,run) in enumerate(zip(signal.to_numpy(float),long.run_length.to_numpy(int))):
        if run==0 or not math.isfinite(value):window=[];continue
        window.append(float(value));window=window[-P["rms_periods"]:]
        if len(window)==P["rms_periods"]:
            scale=math.sqrt(float(np.mean(np.square(window))))
            if scale>0:rms[i]=scale;osc[i]=value/scale
    oscillator=pd.Series(osc,index=series.index)
    return pd.DataFrame({"run_length":long.run_length,"hp_long":long.high_pass,"hp_short":short.high_pass,"signal":signal,"rms":rms,"ultimate_oscillator":oscillator,"rate":oscillator-oscillator.shift(1)},index=series.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    fields=["bar_open","bar_high","bar_low","bar_close"];bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[fields]).all(axis=1)&bars[fields].gt(0).all(axis=1);values=ultimate_oscillator(bars.bar_close,bars.valid_bar,P["band_edge"]);chart=ultimate_oscillator(bars.bar_close,bars.valid_bar,P["chart_control_band_edge"])
    for column in values:bars[column]=values[column]
    bars["chart_oscillator"]=chart.ultimate_oscillator;bars["chart_rate"]=chart.rate;bars["entry_side"]=cross_side(bars.rate);bars["chart_entry_side"]=cross_side(bars.chart_rate);bars["oscillator_entry_side"]=cross_side(bars.ultimate_oscillator);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["hp_long","hp_short","signal","rms","ultimate_oscillator","rate","variation"];bars["source_valid"]=bars.valid_bar&bars.run_length.ge(P["rms_periods"]+1)&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="chart_30_band_edge":side=pd.to_numeric(used.chart_entry_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
    elif control=="oscillator_zero_cross":side=pd.to_numeric(used.oscillator_entry_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVEUO prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hveuo_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hveuo_24_source_support_v1", "policy_id": prereg.POLICY_ID,
