"""Build outcome-blind source support for frozen HVDO-Z14-5-3-9-24."""
...
from training import preregister_high_volatility_derivative_oscillator_zero_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "4a8707f8c711c54bbe77506ee95890defae70cd91977c5126aebc124f09247cd"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_derivative_oscillator_zero_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_derivative_oscillator_zero_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_derivative_oscillator_zero_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_derivative_oscillator_zero_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_derivative_oscillator_zero_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "average_gain", "average_loss", "rsi", "smoothed_rsi",
    "double_smoothed_rsi", "signal", "derivative_oscillator", "entry_side",
    "raw_rsi_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "average_gain", "average_loss", "rsi",
    "smoothed_rsi", "double_smoothed_rsi", "signal", "derivative_oscillator",
    "entry_side", "raw_rsi_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVDO source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVDO invalid source key")
...
class LeanAverage:
    """Minimal LEAN initial-SMA moving average with a fixed recursive alpha."""
    ...
    def update(self, value: float) -> float:
        if len(self.seed) < self.period:
            self.seed.append(float(value))
            if len(self.seed) == self.period:
                self.value = float(np.mean(self.seed))
            return self.value
        self.value = value * self.alpha + self.value * (1 - self.alpha)
        return self.value


def derivative_oscillator(close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    """Replicate LEAN DerivativeOscillator(14,5,3,9) without bridging invalid bars."""
    closes = close.to_numpy(float)
    validity = valid.to_numpy(bool)
    count = len(close)
    names = (
        "average_gain", "average_loss", "rsi", "smoothed_rsi",
        "double_smoothed_rsi", "signal", "derivative_oscillator",
    )
    values = {name: np.full(count, np.nan) for name in names}
    entry_side = np.zeros(count, dtype=int)
    raw_rsi_side = np.zeros(count, dtype=int)
    gains = LeanAverage(P["rsi_periods"], 1.0 / P["rsi_periods"])
    losses = LeanAverage(P["rsi_periods"], 1.0 / P["rsi_periods"])
    first = LeanAverage(P["first_ema_periods"], 2.0 / (P["first_ema_periods"] + 1))
    second = LeanAverage(P["second_ema_periods"], 2.0 / (P["second_ema_periods"] + 1))
    signal_values: list[float] = []
    previous_close = math.nan

    for index in range(count):
        if not validity[index]:
            gains.reset(); losses.reset(); first.reset(); second.reset(); signal_values.clear()
            previous_close = math.nan
            continue
        ...
        previous_close = current_close
        average_gain = gains.update(max(change, 0.0))
        average_loss = losses.update(max(-change, 0.0))
        if not math.isfinite(average_gain) or not math.isfinite(average_loss):
            continue
        values["average_gain"][index] = average_gain
        values["average_loss"][index] = average_loss
        rsi = 100.0 if round(average_loss, 10) == 0 else 100.0 - 100.0 / (1 + average_gain / average_loss)
        values["rsi"][index] = rsi
        prior_rsi_ready = index > 0 and math.isfinite(values["rsi"][index - 1])
        prior_above_mid = prior_rsi_ready and values["rsi"][index - 1] > 50
        prior_below_mid = prior_rsi_ready and values["rsi"][index - 1] < 50
        raw_rsi_side[index] = 1 if rsi > 50 and not prior_above_mid else -1 if rsi < 50 and not prior_below_mid else 0
        smoothed = first.update(rsi)
        if not math.isfinite(smoothed):
            continue
        values["smoothed_rsi"][index] = smoothed
        double_smoothed = second.update(smoothed)
        if not math.isfinite(double_smoothed):
            continue
        values["double_smoothed_rsi"][index] = double_smoothed
        signal_values.append(double_smoothed)
        signal_values = signal_values[-P["signal_periods"] :]
        if len(signal_values) < P["signal_periods"]:
            continue
        signal = float(np.mean(signal_values))
        oscillator = double_smoothed - signal
        values["signal"][index] = signal
        values["derivative_oscillator"][index] = oscillator
        prior_ready = index > 0 and math.isfinite(values["derivative_oscillator"][index - 1])
        prior_positive = prior_ready and values["derivative_oscillator"][index - 1] > 0
        prior_negative = prior_ready and values["derivative_oscillator"][index - 1] < 0
        entry_side[index] = 1 if oscillator > 0 and not prior_positive else -1 if oscillator < 0 and not prior_negative else 0
    return pd.DataFrame({**values, "entry_side": entry_side, "raw_rsi_side": raw_rsi_side}, index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=bars.join(derivative_oscillator(bars.bar_close,bars.valid_bar));bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["average_gain","average_loss","rsi","smoothed_rsi","double_smoothed_rsi","signal","derivative_oscillator","variation"]]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="raw_rsi_midline":side=pd.to_numeric(used.raw_rsi_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVDO prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvdo_z14_5_3_9_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvdo_z14_5_3_9_24_source_support_v1", "policy_id": prereg.POLICY_ID,
