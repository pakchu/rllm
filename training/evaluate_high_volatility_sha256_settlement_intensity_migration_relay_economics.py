"""Sequential strict economics for frozen HVSSIM-24."""
...
PREREG = Path("results/high_volatility_sha256_settlement_intensity_migration_relay_preregistration_2026-08-10.json")
PREREG_SHA = "c5d0c02e1748cf7f9e5ff3cc824bf0165ef73f474c1fe59b5c5c13b193a6cdd0"
SUPPORT = Path("results/high_volatility_sha256_settlement_intensity_migration_relay_support_2026-08-10.json")
SUPPORT_SHA = "0693dab3d1033b378ce1763c0ce3564e5ae3fccdc687d21a7e7b376322b06a34"
NOVELTY = Path("results/high_volatility_sha256_settlement_intensity_migration_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "f2ffa773e6e74a6ef38c9e1b97f260cc9bde7cb46c6aca1c36d6f428f22c71f8"
CLOCK = Path("data/high_volatility_sha256_settlement_intensity_migration_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "1cf075557bc89e9050334c1cb1a34c82b701ed2b61829e7377dc02da25ae8938"
CONTROL_DIR = Path("data/high_volatility_sha256_settlement_intensity_migration_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_sha256_settlement_intensity_migration_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_sha256_settlement_intensity_migration_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "no_migration_gate", "btc_settlement_intensity_change_only", "one_observation_stale_features", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSSIM frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSSIM novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvssim_24_sequential_economics_v1", "policy_id": POLICY_ID,
