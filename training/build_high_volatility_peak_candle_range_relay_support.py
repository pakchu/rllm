"""Build source-only HVPHR-24 clocks before opening outcomes or Gross9."""
...
from training import preregister_high_volatility_peak_candle_range_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.long_regime_combo_scan import LongComboScanConfig, _load_market


PREREG_SHA = "eac1e9956b1fd25168a058c7591ff4a3ae33fb2e6dc1df29b885bf564a65d6e0"
MARKET = "data/cache_market_ext_5m_wavefull_2020-01-01_2026-06-01.csv.gz"
CLOCK = Path("data/high_volatility_peak_candle_range_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_peak_candle_range_relay_controls_2023_2026")
SNAPSHOT = Path(
    "data/high_volatility_peak_candle_range_relay_sources_2023_2026/peak_candle_scores.csv.gz"
)
RESULT = Path("results/high_volatility_peak_candle_range_relay_support_2026-08-09.json")
SPLITS = {
...
def load_combined_market() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if combined.date.duplicated().any() or not combined.date.diff().dropna().eq(
        ...
        raise RuntimeError("HVPHR combined market continuity drift")
    return combined, {
...
def score_snapshot(market: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, float]]:
    ...
    for position in positions:
        peak_highs = highs[position - 71 : position + 1]
        maximum = np.nanmax(peak_highs)
        ties = np.flatnonzero(peak_highs == maximum)
        first_peak = position - 71 + int(ties[0])
        last_peak = position - 71 + int(ties[-1])
        ...
        rows.append(
            {
                ...
                "peak_candle_range_last": (highs[last_peak] - lows[last_peak]) / current_close,
                "peak_candle_range_first": (highs[first_peak] - lows[first_peak]) / current_close,
    ...
    if len(calibration) < 4_000:
        raise RuntimeError("HVPHR source-only calibration floor failed")
    thresholds = {
        "peak_candle_range_q15": float(calibration.peak_candle_range_last.quantile(0.15)),
        "peak_candle_range_q85": float(calibration.peak_candle_range_last.quantile(0.85)),
...
def build_clock(
    ...
    feature = "peak_candle_range_last"
    if control == "first_peak_tie_break":
        feature = "peak_candle_range_first"
    ...
    high_tail = values >= thresholds["peak_candle_range_q85"]
    low_tail = values <= thresholds["peak_candle_range_q15"]
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVPHR preregistration drift")
    ...
    core = {
        "protocol_version": "hvphr_24_source_support_v1",
        "policy_id": "HVPHR-24",
        "preregistration": {
