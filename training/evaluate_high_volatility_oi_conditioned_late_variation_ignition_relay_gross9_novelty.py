"""Evaluate frozen HVOILVI-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVOILVI-12"
PROTOCOL = "hvoilvi_12_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_oi_conditioned_late_variation_ignition_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVOILVI predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVOILVI novelty limits drift")
    if (
        ...
        raise RuntimeError("HVOILVI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVOILVI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvoilvi_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVOILVI-12 unchanged before economic outcomes",
    }
