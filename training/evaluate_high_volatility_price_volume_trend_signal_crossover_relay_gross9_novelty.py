"""Evaluate frozen HVPVT-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVPVT-24"
PROTOCOL = "hvpvt_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_price_volume_trend_signal_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVPVT predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVPVT novelty limits drift")
    if (
        ...
        raise RuntimeError("HVPVT predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVPVT primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvpvt_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVPVT-24 unchanged before economic outcomes",
    }
