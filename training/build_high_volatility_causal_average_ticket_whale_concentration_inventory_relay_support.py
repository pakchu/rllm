"""Deterministic outcome-blind source support for HVCATWIR-8."""
...
from training import preregister_high_volatility_causal_average_ticket_whale_concentration_inventory_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "50e110c8a62cda0765d97d26e35691b5fa5dee74ebf259317e9eae9a4bf42085"
...
ROOT = Path("data/high_volatility_causal_average_ticket_whale_concentration_inventory_relay_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_average_ticket_whale_concentration_inventory_relay_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_average_ticket_whale_concentration_inventory_relay_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_causal_average_ticket_whale_concentration_inventory_relay_support_2026-08-16.json")
...
PANEL_COLUMNS = (
    ...
    "first_half_trade_count", "second_half_trade_count", "first_half_average_ticket", "second_half_average_ticket", "completed_return",
    "average_ticket_acceleration", "trade_arrival_deceleration", "average_ticket_acceleration_rank", "realized_variation",
...
CLOCK_COLUMNS = (
    ...
    "first_half_trade_count", "second_half_trade_count", "first_half_average_ticket", "second_half_average_ticket", "completed_return",
    "average_ticket_acceleration", "trade_arrival_deceleration", "average_ticket_acceleration_rank", "realized_variation", "variation_rank",
)
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVCATWIR-8 OI schema drift")
    expected_bars = [
        "decision_time", "first_quote_turnover", "first_trade_count", "second_quote_turnover", "second_trade_count", "completed_return", "minute_squared_return",
        "source_rows", "distinct_rows", "first_ts", "last_ts", "coherent",
    ]
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCATWIR-8 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all():
        raise RuntimeError("HVCATWIR-8 invalid OI rows")

    bars = bars.copy()
    for column in ("decision_time", "first_ts", "last_ts"):
        bars[column] = pd.to_datetime(bars[column], utc=True, errors="raise")
    numeric = ("first_quote_turnover", "first_trade_count", "second_quote_turnover", "second_trade_count", "completed_return", "minute_squared_return", "source_rows", "distinct_rows")
    for column in numeric:
        bars[column] = pd.to_numeric(bars[column], errors="raise")
    if bars["decision_time"].duplicated().any():
        raise RuntimeError("HVCATWIR-8 duplicate eight-hour bar")
    ...
    bars["trade_arrival_deceleration"] = np.log(bars["first_half_trade_count"] / bars["second_half_trade_count"])
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel = panel.join(
        bars[["bar_valid", "first_half_quote_turnover", "second_half_quote_turnover", "first_half_trade_count", "second_half_trade_count", "first_half_average_ticket", "second_half_average_ticket", "completed_return", "average_ticket_acceleration", "trade_arrival_deceleration", "realized_variation"]],
        on="decision_time",
    )
    panel["source_valid"] = (
        ...
        & np.isfinite(
            panel[["sum_open_interest", "previous_open_interest", "oi_change", "first_half_quote_turnover", "second_half_quote_turnover", "first_half_trade_count", "second_half_trade_count", "first_half_average_ticket", "second_half_average_ticket", "completed_return", "average_ticket_acceleration", "trade_arrival_deceleration", "realized_variation"]]
        ).all(axis=1)
        ...
        & panel["trade_arrival_deceleration"].gt(0)
    )
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCATWIR-8 side or availability drift")
        ...
        rows.append({
            ...
            "trade_arrival_deceleration": float(row.trade_arrival_deceleration),
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCATWIR-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcatwir_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvcatwir_8_source_support_v1", "policy_id": prereg.POLICY_ID,
