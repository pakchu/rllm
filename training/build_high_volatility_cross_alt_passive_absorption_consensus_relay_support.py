"""Outcome-blind source support for frozen HVCAPAC-8."""
...
from training import preregister_high_volatility_cross_alt_passive_absorption_consensus_relay as prereg
ENV_FILE='/home/pakchu/rllm/.env';START=pd.Timestamp('2023-01-01T00:00:00Z');END=pd.Timestamp('2026-08-01T00:00:00Z');PREREG_SHA='920a36b32048d9e5ec682ff0284008fb866e735bd6bed27feda9beba85fa4998';REG=prereg.build();P=REG['policy'];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG['stages'].items()};GATES=REG['source_support_gates'];CONTROLS=tuple(REG['diagnostic_controls']['names']);SYMBOLS=('BTCUSDT',*prereg.ALTS)
QUERY="""SELECT date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 04:00:00+00') AS block_start,symbol,(array_agg(open ORDER BY ts))[1] AS first_open,(array_agg(close ORDER BY ts DESC))[1] AS last_close,sum(quote_asset_volume) AS quote_turnover,sum(2*taker_buy_quote-quote_asset_volume) AS signed_taker_quote,sum(power(ln(close/open),2)) AS minute_squared_return,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND high>=greatest(open,close,low) AND low<=least(open,close,high) AND quote_asset_volume>=0 AND taker_buy_quote>=0 AND taker_buy_quote<=quote_asset_volume) AS coherent FROM bars_binance WHERE symbol=ANY(:symbols) AND interval='1m' AND ts>=:start AND ts<:end GROUP BY 1,2 ORDER BY 1,2"""
ROOT=Path('data/high_volatility_cross_alt_passive_absorption_consensus_relay_sources_2023_2026');PANEL=ROOT/'block_states.csv.gz';MANIFEST=ROOT/'manifest.json';CLOCK=Path('data/high_volatility_cross_alt_passive_absorption_consensus_relay_clocks_2023_2026.csv.gz');SPLIT_DIR=Path('data/high_volatility_cross_alt_passive_absorption_consensus_relay_split_clocks_2023_2026');CONTROL_DIR=Path('data/high_volatility_cross_alt_passive_absorption_consensus_relay_controls_2023_2026');RESULT=Path('results/high_volatility_cross_alt_passive_absorption_consensus_relay_support_2026-08-13.json');BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=('decision_time','feature_available_time','source_valid','consensus_side','consensus_breadth','consensus_strength','broad_side','broad_breadth','confirm_side','confirm_breadth','confirm_strength','btc_realized_variation','variation_rank','eligible');CLOCK_COLUMNS=('candidate','control','split','decision_time','feature_available_time','entry_time','exit_time','side','consensus_breadth','consensus_strength','btc_realized_variation','variation_rank')
...
def prepare(raw):
 required=['block_start','symbol','first_open','last_close','quote_turnover','signed_taker_quote','minute_squared_return','source_rows','distinct_rows','first_ts','last_ts','coherent']
 if raw.columns.tolist()!=required:raise RuntimeError('HVCAPAC source schema drift')
 x=raw.copy()
 for c in ('block_start','first_ts','last_ts'):x[c]=pd.to_datetime(x[c],utc=True,errors='coerce')
 for c in ('first_open','last_close','quote_turnover','signed_taker_quote','minute_squared_return','source_rows','distinct_rows'):x[c]=pd.to_numeric(x[c],errors='coerce')
 if x[['block_start','symbol']].isna().any().any() or x.duplicated(['block_start','symbol']).any():raise RuntimeError('HVCAPAC invalid key')
 x['row_valid']=np.isfinite(x[['first_open','last_close','quote_turnover','signed_taker_quote','minute_squared_return','source_rows','distinct_rows']]).all(axis=1)&x.first_open.gt(0)&x.last_close.gt(0)&x.quote_turnover.gt(0)&x.minute_squared_return.ge(0)&x.source_rows.eq(480)&x.distinct_rows.eq(480)&x.first_ts.eq(x.block_start)&x.last_ts.eq(x.block_start+pd.Timedelta('479m'))&x.coherent.eq(True);x['return']=np.log(x.last_close/x.first_open);x['flow']=x.signed_taker_quote/x.quote_turnover;x['strength']=x['return'].abs()*x.flow.abs();x['decision_time']=x.block_start+pd.Timedelta('8h');return x.set_index(['decision_time','symbol']).sort_index()
def build_panel(raw):
 x=prepare(raw);decisions=pd.date_range(START+pd.Timedelta('4h'),END,freq='8h',inclusive='left');full=x.reindex(pd.MultiIndex.from_product([decisions,SYMBOLS],names=['decision_time','symbol']));valid=full.row_valid.unstack('symbol').reindex(columns=SYMBOLS);ret=full['return'].unstack('symbol').reindex(columns=SYMBOLS);flow=full.flow.unstack('symbol').reindex(columns=SYMBOLS);strength=full.strength.unstack('symbol').reindex(columns=SYMBOLS);sq=full.minute_squared_return.unstack('symbol').reindex(columns=SYMBOLS);ranks=pd.DataFrame(index=decisions,columns=prereg.ALTS,dtype=float)
 for alt in prereg.ALTS:ranks[alt]=causal(strength[alt].where(valid[alt]))
 btcvar=np.sqrt(sq.BTCUSDT.where(valid.BTCUSDT).rolling(3,min_periods=3).sum());rows=[]
 for d in decisions:
  ok=bool(valid.loc[d].eq(True).all() and np.isfinite(ranks.loc[d]).all() and math.isfinite(float(btcvar.loc[d])) and btcvar.loc[d]>0);rs=np.sign(ret.loc[d,list(prereg.ALTS)].to_numpy(float));fs=np.sign(flow.loc[d,list(prereg.ALTS)].to_numpy(float));rr=ranks.loc[d].to_numpy(float);absorb=rs.eq(-fs) if isinstance(rs,pd.Series) else rs==-fs;confirm=rs==fs;side,breadth,st=consensus(rs,rr,absorb,P['absorption_strength_rank_min']) if ok else (0,0,math.nan);bside,bbreadth,_=consensus(rs,np.ones(6),absorb,0.) if ok else (0,0,math.nan);cside,cbreadth,cst=consensus(rs,rr,confirm,P['absorption_strength_rank_min']) if ok else (0,0,math.nan);rows.append({'decision_time':d,'feature_available_time':d,'source_valid':ok,'consensus_side':side,'consensus_breadth':breadth,'consensus_strength':st,'broad_side':bside,'broad_breadth':bbreadth,'confirm_side':cside,'confirm_breadth':cbreadth,'confirm_strength':cst,'btc_realized_variation':float(btcvar.loc[d]) if ok else math.nan})
...
def active(panel,control='primary'):
 ...
 if control=='no_absorption_tail':side=used.broad_side;breadth=used.broad_breadth;strength=pd.Series(1.,index=used.index)
 elif control=='flow_confirmation_consensus':side=used.confirm_side;breadth=used.confirm_breadth;strength=used.confirm_strength
 if control=='one_block_stale_absorption':side=side.shift(1);breadth=breadth.shift(1);strength=strength.shift(1)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError('HVCAPAC prereg drift')
