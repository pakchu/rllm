"""Build outcome-blind source support for frozen HVCAFC-6."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_cross_alt_three_bar_flip_cascade_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2023-01-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="8da080cbd9f607564d1e213c9cdf67b728ef46d786ce32280034c16f815c15c4";REG=prereg.build();P=REG["policy"];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG["stages"].items()};GATES=REG["source_support_gates"];CONTROLS=tuple(REG["diagnostic_controls"]["names"]);SYMBOLS=("BTCUSDT",*prereg.ALTS);ALTS=prereg.ALTS
...
ROOT=Path("data/high_volatility_cross_alt_three_bar_flip_cascade_relay_sources_2023_2026");PANEL=ROOT/"scheduled_flip_cascade_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_cross_alt_three_bar_flip_cascade_relay_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_cross_alt_three_bar_flip_cascade_relay_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_cross_alt_three_bar_flip_cascade_relay_controls_2023_2026");RESULT=Path("results/high_volatility_cross_alt_three_bar_flip_cascade_relay_support_2026-08-13.json");BUILDER=Path(__file__).relative_to(Path.cwd())
BASE_COLS=("decision_time","feature_available_time","source_valid","positive_flips","negative_flips","side","btc_realized_variation","variation_rank","variation_active","eligible")
CLOCK_COLS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","positive_flips","negative_flips","btc_realized_variation","variation_rank")
...
def prepare(raw):
 ...
 if raw.columns.tolist()!=expected:raise RuntimeError("HVCAFC source schema drift")
...
def cascade_frame(ret,valid,control="primary"):
 ...
 for decision in scheduled:
  ...
  for a in ALTS:
   signs=[int(np.sign(ret[a].at[end-pd.Timedelta(minutes=5*k)])) if end-pd.Timedelta(minutes=5*k) in ret.index and math.isfinite(ret[a].at[end-pd.Timedelta(minutes=5*k)]) else 0 for k in (2,1,0)];flip=(signs[1]!=0 and signs[2]==-signs[1] and (control=="two_bar_flip" or signs[0]==signs[1]));row[f"{a}_flip_side"]=signs[2] if flip else 0
  rows.append(row)
 return pd.DataFrame(rows)
def score_states(f,control="primary"):
 u=f.copy();u["variation_rank"]=midrank(u.btc_realized_variation.where(u.source_valid),P["variation_prior_decisions"],P["variation_minimum_decisions"]);u["positive_flips"]=u[[f"{a}_flip_side" for a in ALTS]].eq(1).sum(axis=1);u["negative_flips"]=u[[f"{a}_flip_side" for a in ALTS]].eq(-1).sum(axis=1);minimum=3 if control=="three_alt_cascade" else P["minimum_flip_alts"];u["side"]=np.where(u.positive_flips.ge(minimum)&u.negative_flips.lt(minimum),1,np.where(u.negative_flips.ge(minimum)&u.positive_flips.lt(minimum),-1,0));u["variation_active"]=u.variation_rank.ge(P["variation_rank_min"]);u["eligible"]=u.source_valid&u.side.ne(0)&u.variation_active
 if control=="no_variation_gate":u["eligible"]=u.source_valid&u.side.ne(0)
 if control=="one_decision_stale_cascade":
  for c in ("positive_flips","negative_flips","side"):u[c]=u[c].shift(1)
  u["eligible"]=u.source_valid&u.side.ne(0)&u.variation_active
 if control=="direction_flip":u.side=-u.side
 if control=="forced_long":u.side=1
 return u
def build_panel(raw,control="primary"):
 x=prepare(raw);grid=pd.date_range(START,END,freq="5min",inclusive="left");ret=x.pivot(index="date",columns="symbol",values="return").reindex(grid,columns=SYMBOLS);valid=x.pivot(index="date",columns="symbol",values="valid").reindex(grid,columns=SYMBOLS).eq(True);return score_states(cascade_frame(ret,valid,control),control)
def clock(panel,control="primary"):
 rows=[];reserved=None
 for i in panel.index[panel.eligible&panel.side.ne(0)]:
  d=pd.Timestamp(panel.at[i,"decision_time"]);entry=d+pd.Timedelta("5m");exit_=entry+pd.Timedelta("6h")
  if reserved is not None and entry<reserved:continue
  split=next((n for n,(a,b) in SPLITS.items() if entry>=a and exit_<=b),None)
  if split:reserved=exit_;rows.append({"candidate":prereg.POLICY_ID,"control":control,"split":split,"decision_time":d,"feature_available_time":pd.Timestamp(panel.at[i,"feature_available_time"]),"entry_time":entry,"exit_time":exit_,"side":int(panel.at[i,"side"]),**{c:float(panel.at[i,c]) for c in CLOCK_COLS[8:]}})
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCAFC prereg drift")
