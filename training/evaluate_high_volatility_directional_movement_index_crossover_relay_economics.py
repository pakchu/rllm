"""Sequential strict economics for frozen HVDMIC-24."""
...
PREREG = Path("results/high_volatility_directional_movement_index_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "93318788831a973a8dbe04d23d8e48a141a7f0a4ae717481efb7e258decb960f"
SUPPORT = Path("results/high_volatility_directional_movement_index_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "a793d40578a9a1b9265b4233a914803ab703d0b78cd2d9a4b5eb718289ccb360"
NOVELTY = Path("results/high_volatility_directional_movement_index_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "e38fa20bae5ba84a50f7f45ab166126197f50e939080d70ccab9edced6b98b1c"
CLOCK = Path("data/high_volatility_directional_movement_index_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "0a2a13a2f0a72feefaad82339c91add52dc89f77133220bc70d31c2a375160ca"
CONTROL_DIR = Path("data/high_volatility_directional_movement_index_crossover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_directional_movement_index_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_directional_movement_index_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "persistent_direction_state", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDMIC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDMIC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdmic_24_sequential_economics_v1", "policy_id": POLICY_ID,
