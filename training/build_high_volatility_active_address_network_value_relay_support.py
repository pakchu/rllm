"""Build outcome-blind source support for frozen HVAANV-24.
...
POLICY_ID = "HVAANV-24"
PREREG_SHA256 = "38458f49a5568135d8d23be2e3cf1d62e3dee9e50cd9f9aec42283961a337d07"
...
SOURCE_DIR = Path("data/high_volatility_active_address_network_value_relay_sources_2022_2026")
DAILY_SOURCE = SOURCE_DIR / "coinmetrics_btc_daily.csv.gz"
FEATURE_PANEL = SOURCE_DIR / "daily_feature_panel.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_active_address_network_value_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_active_address_network_value_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_active_address_network_value_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_active_address_network_value_relay_support_2026-08-12.json")

SOURCE_COLUMNS = (
    "observation_time", "feature_available_time", "AdrActCnt", "CapMrktCurUSD",
)
PANEL_COLUMNS = (
    "observation_time", "feature_available_time", "decision_time", "source_valid",
    "minute_count", "AdrActCnt", "CapMrktCurUSD", "active_addresses_30d", "aanv30",
    "aanv30_rank", "single_day_aanv", "single_day_aanv_rank", "btc_variation",
    "btc_variation_rank",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "observation_time", "feature_available_time",
    "decision_time", "entry_time", "exit_time", "side", "AdrActCnt",
    "CapMrktCurUSD", "active_addresses_30d", "aanv30", "aanv30_rank",
...
def parse_source_row(raw: Any) -> dict[str, Any]:
    ...
    active = _positive_decimal(raw["AdrActCnt"], "AdrActCnt", integer=True)
    market_cap = _positive_decimal(raw["CapMrktCurUSD"], "CapMrktCurUSD")
    return {
        "observation_time": observation,
        "feature_available_time": available,
        "AdrActCnt": int(active),
        "CapMrktCurUSD": str(market_cap),
    }
...
def strict_prior_midrank(
    values: pd.Series, maximum: int = 270, minimum: int = 180
) -> pd.Series:
...
def build_panel(source: pd.DataFrame, btc_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    daily["AdrActCnt"] = pd.to_numeric(daily.AdrActCnt, errors="coerce")
    daily["CapMrktCurUSD"] = pd.to_numeric(daily.CapMrktCurUSD, errors="coerce")
    expected = pd.date_range(SOURCE_START, SOURCE_END, freq="1d", inclusive="left")
    if (
    ...
    value_valid = (
        np.isfinite(daily[["AdrActCnt", "CapMrktCurUSD"]]).all(axis=1)
        & daily.AdrActCnt.gt(0) & daily.CapMrktCurUSD.gt(0)
        & daily.AdrActCnt.eq(np.floor(daily.AdrActCnt))
    )
    active_30 = daily.AdrActCnt.rolling(30, min_periods=30).mean()
    rolling_valid = value_valid.rolling(30, min_periods=30).sum().eq(30)
    aanv30 = (active_30 / daily.CapMrktCurUSD).where(rolling_valid & value_valid)
    single_day = (daily.AdrActCnt / daily.CapMrktCurUSD).where(value_valid)
    ...
    base_valid = (
        ...
        & np.isfinite(aanv30) & aanv30.gt(0)
    )
    panel = pd.DataFrame({
        ...
        "AdrActCnt": daily.AdrActCnt.astype(float),
        "CapMrktCurUSD": daily.CapMrktCurUSD.astype(float),
        "active_addresses_30d": active_30,
        "aanv30": aanv30,
        "single_day_aanv": single_day,
        "btc_variation": variation,
    })
    panel["aanv30_rank"] = strict_prior_midrank(panel.aanv30.where(panel.source_valid))
    panel["single_day_aanv_rank"] = strict_prior_midrank(panel.single_day_aanv.where(panel.source_valid))
...
def active(panel: pd.DataFrame, control: str = "primary") -> tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    used = panel.copy()
    ratio = used.aanv30
    ratio_rank = used.aanv30_rank
    available = used.feature_available_time
    valid = used.source_valid.eq(True)
    if control == "one_day_stale_aanv":
        ratio = used.aanv30.shift(1)
        ratio_rank = used.aanv30_rank.shift(1)
        available = used.feature_available_time.shift(1)
        valid = valid & ratio.notna() & ratio_rank.notna()
    elif control == "single_day_active_address_to_value":
        ratio = used.single_day_aanv
        ratio_rank = used.single_day_aanv_rank
    tail = ratio_rank.ge(0.80) | ratio_rank.le(0.20)
    volatility = pd.Series(True, index=panel.index) if control == "no_btc_volatility_gate" else used.btc_variation_rank.ge(0.65)
    eligible = valid & ratio.notna() & tail & volatility
    side = pd.Series(np.where(ratio_rank.ge(0.80), 1, -1), index=panel.index, dtype=int)
    if control == "aanv_direction_flip":
        side = -side
    elif control == "same_clock_forced_long":
        side = pd.Series(1, index=panel.index, dtype=int)
    adjusted = used.copy()
    adjusted["aanv30"] = ratio
    adjusted["aanv30_rank"] = ratio_rank
...
def build_clock(panel: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in panel.index[eligible]:
        ...
        rows.append({
            ...
            "AdrActCnt": float(panel.at[index, "AdrActCnt"]),
            "CapMrktCurUSD": float(panel.at[index, "CapMrktCurUSD"]),
            "active_addresses_30d": float(panel.at[index, "active_addresses_30d"]),
            "aanv30": float(used.at[index, "aanv30"]),
            "aanv30_rank": float(used.at[index, "aanv30_rank"]),
...
def run(*, fetch: Fetch | None = None) -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA256:
        raise RuntimeError("HVAANV frozen preregistration artifact drift")
    registration = json.loads(prereg.DEFAULT_OUTPUT.read_text(encoding="utf-8"))
    if registration != prereg.build():
        raise RuntimeError("HVAANV frozen preregistration payload drift")
    ...
    source_core = {
        "protocol_version": "hvaanv_24_source_materialization_v1",
    ...
    core = {
        "protocol_version": "hvaanv_24_source_support_v1",
        "policy_id": POLICY_ID,
        "preregistration": {"path": str(prereg.DEFAULT_OUTPUT), "sha256": PREREG_SHA256, "manifest_hash": registration["manifest_hash"]},
