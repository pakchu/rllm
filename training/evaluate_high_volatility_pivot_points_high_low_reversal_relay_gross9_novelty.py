"""Evaluate frozen HVPPHL-R10-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVPPHL-R10-24"
PROTOCOL = "hvpphl_r10_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_pivot_points_high_low_reversal_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVPPHL predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVPPHL novelty limits drift")
    if (
        ...
        raise RuntimeError("HVPPHL predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVPPHL primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvpphl_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVPPHL-R10-24 unchanged before economic outcomes",
    }
