"""Strict staged economics for the singleton HVCATFLIR-8 policy."""
...
PREREG = Path("results/high_volatility_causal_average_ticket_flow_late_inventory_relay_preregistration_2026-08-16.json")
PREREG_SHA = "13b61c893da9e6feb033f5f928619a8639c89dff7aaa2ee984f015d91ae6c145"
SUPPORT = Path("results/high_volatility_causal_average_ticket_flow_late_inventory_relay_support_2026-08-16.json")
SUPPORT_SHA = "4f054dc94dfb278166923e759964b5d564fc4f2e8d82c959d34be4595925464c"
GROSS9 = Path("results/high_volatility_causal_average_ticket_flow_late_inventory_relay_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "a556c0c6bda10423afa2d7ab3228f9c3af0a9aaf3c86ef6a117586add29d2b4d"
CLOCK = {
    "path": Path("data/high_volatility_causal_average_ticket_flow_late_inventory_relay_clocks_2020_2026.csv.gz"),
    "sha256": "42d7c3f3b21741bb3364fdf4804cdd950217280f307fa3d042ac997d952f5003",
    "rows": 99,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_average_ticket_flow_late_inventory_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCATFLIR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCATFLIR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCATFLIR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCATFLIR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATFLIR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATFLIR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCATFLIR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCATFLIR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCATFLIR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCATFLIR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCATFLIR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcatflir_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
