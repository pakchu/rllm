"""Build outcome-blind source support for frozen FCBIRR-8."""
...
from training import preregister_funding_cash_transfer_basis_inventory_reanchoring_reversal as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv


ENV_FILE = "/home/pakchu/rllm/.env"
PREREG_SHA = "564e0e1215cd681ad8701e1017aa3aa2929b966d80334e4807585db65e529e15"
...
SOURCE_DIR = Path("data/funding_cash_transfer_basis_inventory_reanchoring_reversal_sources_2023_2026")
FEATURES = SOURCE_DIR / "eight_hour_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/funding_cash_transfer_basis_inventory_reanchoring_reversal_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/funding_cash_transfer_basis_inventory_reanchoring_reversal_controls_2023_2026")
RESULT = Path("results/funding_cash_transfer_basis_inventory_reanchoring_reversal_support_2026-08-11.json")
SPLITS = {
...
CONTROLS = ("no_dvol_level", "no_premium_tail", "no_oi_increase", "no_funding_premium_sign_agreement", "one_settlement_stale_features", "direction_flip", "forced_long")
PREMIUM_QUERY = """
...
GROUP BY 1 ORDER BY 1
"""
CLOCK_COLUMNS = ("candidate", "control", "split", "funding_time", "source_end", "decision_time", "feature_available_time", "entry_time", "exit_time", "side", "funding_rate", "premium_mean", "premium_mean_rank", "oi_change", "dvol_close", "dvol_level_rank")
...
def load_database_sources() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
...
def build_features(premium: pd.DataFrame, oi: pd.DataFrame, funding: pd.DataFrame, dvol: pd.DataFrame) -> pd.DataFrame:
    blocks = premium.merge(oi, on="block_start", how="inner", validate="one_to_one")
    frame = funding.copy()
    frame["source_end"] = frame.funding_time.dt.floor("h")
    frame["block_start"] = frame.source_end - pd.Timedelta(hours=8)
    frame = frame.merge(blocks, on="block_start", how="left", validate="many_to_one")
    frame = frame.merge(dvol.rename(columns={"decision_time": "source_end"}), on="source_end", how="left", validate="many_to_one")
    frame["premium_mean"] = pd.to_numeric(frame.premium_mean, errors="coerce")
    ...
    frame["source_valid"] = (
        ...
        & frame.funding_rate.ne(0) & np.isfinite(frame.funding_rate)
        & frame.premium_mean.ne(0) & np.isfinite(frame.premium_mean)
    ...
    frame["premium_mean_rank"] = strict_prior_midrank(frame.premium_mean.abs().where(frame.source_valid))
...
def conditions(frame: pd.DataFrame, control: str = "primary") -> tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    used = frame.shift(1) if control == "one_settlement_stale_features" else frame
    active = used.source_valid.eq(True)
    if control != "no_dvol_level": active &= used.dvol_level_rank.ge(0.60)
    if control != "no_premium_tail": active &= used.premium_mean_rank.ge(0.60)
    if control != "no_oi_increase": active &= used.oi_change.gt(0)
    if control != "no_funding_premium_sign_agreement":
        active &= np.sign(used.funding_rate).eq(np.sign(used.premium_mean))
    side = -np.sign(used.funding_rate).fillna(0).astype(int)
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    active, sides, used = conditions(features, control); rows: list[dict[str, Any]] = []; next_allowed: pd.Timestamp | None = None
    for index in features.index[active]:
        funding_time = pd.Timestamp(features.at[index, "funding_time"])
        decision = funding_time.floor("5min") + pd.Timedelta(minutes=5)
        ...
        rows.append({"candidate": "FCBIRR-8", "control": control, "split": split, "funding_time": funding_time, "source_end": source.source_end, "decision_time": decision, "feature_available_time": funding_time, "entry_time": entry, "exit_time": exit_time, "side": int(sides.at[index]), "funding_rate": float(source.funding_rate), "premium_mean": float(source.premium_mean), "premium_mean_rank": float(source.premium_mean_rank), "oi_change": float(source.oi_change), "dvol_close": float(source.dvol_close), "dvol_level_rank": float(source.dvol_level_rank)})
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA: raise RuntimeError("FCBIRR preregistration drift")
    registration = json.loads(prereg.DEFAULT_OUTPUT.read_text()); prereg.validate(registration)
    premium, oi, funding = load_database_sources(); features = build_features(premium, oi, funding, load_dvol())
    ...
    source_core = {"protocol_version": "fcbirr_8_sources_v1", "queries": {"premium": PREMIUM_QUERY, "oi": OI_QUERY, "funding": FUNDING_QUERY}, "tables": ["bars_binance_premium", "open_interest_binance", "funding_rates_binance"], "dvol": {"path": str(DVOL), "sha256": sha(DVOL)}, "window": [START.isoformat(), END.isoformat()], "features": {"path": str(FEATURES), "sha256": sha(FEATURES), "rows": len(features), "valid_rows": int(features.source_valid.sum())}, "outcomes_opened": False, "gross9_rows_opened": False, "no_imputation": True}
