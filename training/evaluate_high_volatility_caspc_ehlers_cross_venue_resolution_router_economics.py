"""Sequential strict economics for frozen HVCELVCVDR-8."""
...
POLICY_ID = "HVCELVCVDR-8"
BAR = pd.Timedelta(minutes=5)
LEVERAGE = 0.5
BASE_COST = 0.0006
STRESS_COST = 0.0010
ENV_FILE = "/home/pakchu/rllm/.env"
PREREG = Path("results/high_volatility_caspc_ehlers_cross_venue_resolution_router_preregistration_2026-08-16.json")
PREREG_SHA = "a6b24af32d36aad76ebc0ae0b453790234e70a7862a257f9fb2c10886f58f3e1"
SUPPORT = Path("results/high_volatility_caspc_ehlers_cross_venue_resolution_router_support_2026-08-16.json")
SUPPORT_SHA = "20e397eafaf9ac81ca8bf6a30f02214efa2647b1e8aeb313860953c5cf8786df"
NOVELTY = Path("results/high_volatility_caspc_ehlers_cross_venue_resolution_router_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "c43941441e8e4f386161e69602ddbe85ffa1896734033eff870ea7700a17d4ec"
CLOCK = Path("data/high_volatility_caspc_ehlers_cross_venue_resolution_router_clocks_2023_2026.csv.gz")
CLOCK_SHA = "abf5091cc6f451d44d61684d73f8c1dce13f925f1d28efbd09d153f7f929b265"
CONTROL_DIR = Path("data/high_volatility_caspc_ehlers_cross_venue_resolution_router_controls_2023_2026")
FREEZE = Path("results/high_volatility_caspc_ehlers_cross_venue_resolution_router_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_caspc_ehlers_cross_venue_resolution_router_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCELVCVDR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCELVCVDR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcelvcvdr_8_sequential_economics_v1", "policy_id": POLICY_ID,
