"""Strict staged economics and train-only selection for HVCSPR-8."""
...
TRAIN_BONFERRONI_RAW_P_MAX = NORMAL_WEEKLY_P_MAX / FAMILY_SIZE
ENV_FILE = "/home/pakchu/rllm/.env"

PREREG = Path("results/high_volatility_cross_structure_priority_router_preregistration_2026-08-14.json")
PREREG_SHA = "d31652fd7bf80b9a2da73a15f4c4235d08b2f40ae583a0f90fd3b32dfd775bc0"
SUPPORT = Path("results/high_volatility_cross_structure_priority_router_support_2026-08-14.json")
SUPPORT_SHA = "c05351658479630e423a9a80df3e2cc61fb49cf4a3a40293138c6ecb62d1f42d"
GROSS9 = Path("results/high_volatility_cross_structure_priority_router_gross9_novelty_2026-08-14.json")
GROSS9_SHA = "052c68a5a6847a272e32758bb1eaef8b12745b4af347a9038f6ee40d501cd2ac"

FAMILY_ORDER = (
    "CARSC-8__THEN__HVCMMI-8__THEN__HVIABR-8__ELIGIBLE_BY__HVTCCR-8",
    "CARSC-8__THEN__HVCMMI-8__THEN__HVIABR-8__ELIGIBLE_BY__HVLZC-8",
    "HVIABR-8__THEN__HVCMMI-8__THEN__CARSC-8__ELIGIBLE_BY__HVTCCR-8",
    "HVIABR-8__THEN__HVCMMI-8__THEN__CARSC-8__ELIGIBLE_BY__HVLZC-8",
)
ELIGIBLE_CANDIDATES = (FAMILY_ORDER[0], FAMILY_ORDER[2])
CLOCKS = {
    FAMILY_ORDER[0]: {
        "path": Path("data/high_volatility_cross_structure_priority_router_clocks_2023_2026/CARSC-8__THEN__HVCMMI-8__THEN__HVIABR-8__ELIGIBLE_BY__HVTCCR-8.csv.gz"),
        "sha256": "66047ce21015e29478d3b704b8163788c31bec888c5207dd90cbce37d5d084bd",
        "rows": 181,
    },
    FAMILY_ORDER[1]: {
        "path": Path("data/high_volatility_cross_structure_priority_router_clocks_2023_2026/CARSC-8__THEN__HVCMMI-8__THEN__HVIABR-8__ELIGIBLE_BY__HVLZC-8.csv.gz"),
        "sha256": "3ad16185986f34be65a21d4ac96cdecd6b0c9e781197c39e5bf08d5610d545cf",
        "rows": 256,
    },
    FAMILY_ORDER[2]: {
        "path": Path("data/high_volatility_cross_structure_priority_router_clocks_2023_2026/HVIABR-8__THEN__HVCMMI-8__THEN__CARSC-8__ELIGIBLE_BY__HVTCCR-8.csv.gz"),
        "sha256": "0c0d20fb03a3cab700a0237dcd8e539355472d2a63fdb15a5cd1dd1a883d8e1f",
        "rows": 181,
    },
    FAMILY_ORDER[3]: {
        "path": Path("data/high_volatility_cross_structure_priority_router_clocks_2023_2026/HVIABR-8__THEN__HVCMMI-8__THEN__CARSC-8__ELIGIBLE_BY__HVLZC-8.csv.gz"),
        "sha256": "aabf884d28b8f031f04239ef5e4a94117a8f065dc411f7e3e870c03d08c30b20",
        "rows": 256,
    },
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_structure_priority_router_{stage}_economics_2026-08-14.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCSPR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCSPR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCSPR-8 {label} hash drift")
    for candidate, binding in CLOCKS.items():
        if sha256(binding["path"]) != binding["sha256"]:
            raise RuntimeError(f"HVCSPR-8 family clock hash drift: {candidate}")
    ...
    if (
        ...
        raise RuntimeError("HVCSPR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCSPR-8 source-supported candidate drift")
    for candidate, binding in CLOCKS.items():
        eligible = candidate in ELIGIBLE_CANDIDATES
        expected_clock = {
        ...
        if (
            candidates.get(candidate, {}).get("support_passed") is not eligible
            or candidates.get(candidate, {}).get("advance_to_combination_gross9") is not eligible
            ...
            raise RuntimeError(f"HVCSPR-8 source candidate drift: {candidate}")
    ...
    if (
        ...
        raise RuntimeError("HVCSPR-8 Gross9 did not authorize economics")
    for candidate in ELIGIBLE_CANDIDATES:
        ...
        if (
            ...
            raise RuntimeError(f"HVCSPR-8 Gross9 candidate drift: {candidate}")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCSPR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCSPR-8 predecessor did not pass with the frozen winner")
    ...
    if earlier_stage is None:
        if report.get("predecessor") is not None or report.get("selection", {}).get("raw_train_rank_one") != winner:
            raise RuntimeError("HVCSPR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link or earlier.get("frozen_train_winner") != winner:
            raise RuntimeError("HVCSPR-8 predecessor chain drift")
    return report
...
def economic_checks(stage: str, primary: Mapping[str, Any]) -> dict[str, bool]:
    ...
    if stage == "train":
        checks["train_bonferroni_raw_weekly_p_max_0_025"] = weekly_p <= TRAIN_BONFERRONI_RAW_P_MAX
    return checks
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcspr_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": winner,
        "stage": stage,
