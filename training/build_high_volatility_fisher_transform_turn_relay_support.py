"""Build outcome-blind source support for frozen HVFT-24."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_fisher_transform_turn_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2020-01-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="88cb712498240adb136adb369ebbaee2c4ea9ca1d155f88b62851c7fd2610e32";REG=prereg.build();P=REG["policy"]
...
ROOT=Path("data/high_volatility_fisher_transform_turn_relay_sources_2023_2026");PANEL=ROOT/"four_hour_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_fisher_transform_turn_relay_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_fisher_transform_turn_relay_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_fisher_transform_turn_relay_controls_2023_2026");RESULT=Path("results/high_volatility_fisher_transform_turn_relay_support_2026-08-11.json");BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("source_start","feature_available_time","source_valid","bar_open","bar_high","bar_low","bar_close","median_price","range_min","range_max","range_location","recursive_value","raw_fisher","fisher","fisher_slope","raw_fisher_slope","variation","variation_rank","eligible")
CLOCK_COLUMNS=("candidate","control","split","source_start","feature_available_time","entry_time","exit_time","side","bar_close","median_price","range_location","recursive_value","raw_fisher","fisher","fisher_slope","raw_fisher_slope","variation","variation_rank","eligible")
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVFT source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVFT invalid source key")
...
def fisher_transform(high:pd.Series,low:pd.Series,valid:pd.Series)->pd.DataFrame:
 median=(high+low)/2;period=P["range_periods"];consecutive=valid.rolling(period,min_periods=period).sum().eq(period);minimum=median.rolling(period,min_periods=period).min().where(consecutive);maximum=median.rolling(period,min_periods=period).max().where(consecutive);spread=maximum-minimum;location=((median-minimum)/spread).where(spread.gt(0));value=np.full(len(median),np.nan);raw=np.full(len(median),np.nan);fish=np.full(len(median),np.nan);active=False
 for i,loc in enumerate(location.to_numpy(float)):
  if not np.isfinite(loc):active=False;continue
  prior_value=value[i-1] if active else 0.;prior_fisher=fish[i-1] if active else 0.;current=P["value_new_weight"]*2*(loc-.5)+P["value_prior_weight"]*prior_value;current=float(np.clip(current,-P["clip_absolute"],P["clip_absolute"]));raw[i]=.5*np.log((1+current)/(1-current));fish[i]=raw[i]+P["fisher_prior_weight"]*prior_fisher;value[i]=current;active=True
 return pd.DataFrame({"median_price":median,"range_min":minimum,"range_max":maximum,"range_location":location,"recursive_value":value,"raw_fisher":raw,"fisher":fish},index=high.index)

def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 source=prepare(raw).reindex(pd.date_range(START,END,freq="1min",inclusive="left"));g=source.groupby(source.index.floor("4h"),sort=True);bars=pd.DataFrame({"rows":g.row_valid.sum(),"bar_open":g.open.first(),"bar_high":g.high.max(),"bar_low":g.low.min(),"bar_close":g.close.last(),"variation_component":g.minute_sq_return.sum(min_count=240)});bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[["bar_open","bar_high","bar_low","bar_close"]]).all(axis=1)&bars[["bar_open","bar_high","bar_low","bar_close"]].gt(0).all(axis=1);bars=pd.concat([bars,fisher_transform(bars.bar_high,bars.bar_low,bars.valid_bar)],axis=1);bars["fisher_slope"]=bars.fisher-bars.fisher.shift(1);bars["raw_fisher_slope"]=bars.raw_fisher-bars.raw_fisher.shift(1);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["range_min","range_max","range_location","recursive_value","raw_fisher","fisher","fisher_slope","raw_fisher_slope","variation"]]).all(axis=1)&bars.range_max.gt(bars.range_min)&bars.variation.gt(0);panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.fisher_slope.shift(1);long=previous.le(0)&panel.fisher_slope.gt(0);short=previous.ge(0)&panel.fisher_slope.lt(0);panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&(long|short)&panel.variation_rank.ge(P["variation_rank_min"]);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);slope=pd.to_numeric(used.raw_fisher_slope if control=="unsmoothed_fisher" else used.fisher_slope,errors="coerce");previous=slope.shift(1);long=previous.le(0)&slope.gt(0);short=previous.ge(0)&slope.lt(0);side=pd.Series(np.where(long,1,np.where(short,-1,0)),index=used.index,dtype=int);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&side.ne(0)&variation
 if control=="no_variation_gate":state=valid&side.ne(0)
 elif control=="one_bar_stale_turn":state=state.shift(1,fill_value=False);side=side.shift(1,fill_value=0)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVFT prereg drift")
