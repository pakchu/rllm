"""Source-only support gate for frozen HVDCS-8."""
...
from training import preregister_high_volatility_directional_change_scarcity_relay as prereg
...
PREREG_SHA = "746594b628f0ee377de6847479d8a84bf58ae4f1a06ab2cb23dc30986b89e325"
SPLITS = {
...
ROOT = Path("data/high_volatility_directional_change_scarcity_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_directional_change_scarcity_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_directional_change_scarcity_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_directional_change_scarcity_relay_support_2026-08-10.json")
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVDCS source timestamps")
    ...
    core = {
        "protocol_version": "hvdcs_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    eligible = states["source_valid"] & variation_gate & change_gate & agreement
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVDCS preregistration drift")
    ...
    core = {
        "protocol_version": "hvdcs_8_source_support_v1",
        "policy_id": "HVDCS-8",
        "preregistration": {
