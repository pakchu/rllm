"""Sequential strict economics for frozen FVSMR-12."""
...
PREREG = Path("results/fx_volatility_sponsored_momentum_relay_preregistration_2026-08-09.json")
PREREG_SHA = "51e3ad7d7255bae63f7b84832655aecea7983d03b2c5c10208c146ba26da00f4"
SUPPORT = Path("results/fx_volatility_sponsored_momentum_relay_support_2026-08-09.json")
SUPPORT_SHA = "7d78260f9a9dddc504679a540c3b253a0cad42f578892827c9dc997d799eb6dd"
NOVELTY = Path("results/fx_volatility_sponsored_momentum_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "5cbd2e7634d2739c29cb92d516199e5b2120244dbbd1a12889688f5ef6d1aefc"
CLOCK = Path("data/fx_volatility_sponsored_momentum_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "2151fc43d9582dc21a5af408baac0ed438efa8cf1fe1bd8c36f7bfd9e066ec34"
CONTROL_DIR = Path("data/fx_volatility_sponsored_momentum_relay_controls_2023_2026")
FREEZE = Path("results/fx_volatility_sponsored_momentum_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/fx_volatility_sponsored_momentum_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "no_fx_shock_tail", "raw_fx_absolute_return_shock", "one_session_stale_fx_shock", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("FVSMR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("FVSMR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "fvsmr_12_sequential_economics_v1", "policy_id": POLICY_ID,
