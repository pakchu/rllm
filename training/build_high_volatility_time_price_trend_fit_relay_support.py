"""Source-only support gate for frozen HVTFR-8."""
...
from training import preregister_high_volatility_time_price_trend_fit_relay as prereg
...
ROOT = Path("data/high_volatility_time_price_trend_fit_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_time_price_trend_fit_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_time_price_trend_fit_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_time_price_trend_fit_relay_support_2026-08-10.json")
...
def trend_fit(close: np.ndarray) -> tuple[float, float]:
    ...
    if sst <= 0 or not math.isfinite(slope) or slope == 0:
        return math.nan, math.nan
    value = 1.0 - sse / sst
    if not math.isfinite(value) or value < 0 or value > 1:
        return math.nan, math.nan
    return float(slope), float(value)
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVTFR source timestamps")
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        if valid:
            ...
            trend_slope, r_squared = trend_fit(window["close"].to_numpy(float))
            ...
            valid = variation > 0 and math.isfinite(r_squared) and math.isfinite(trend_slope)
        if not valid:
            variation = trend_slope = r_squared = block_return = late_return = math.nan
        rows.append(
            {
    ...
    states["trend_fit_rank"] = prior_rank(states["r_squared"].where(states["source_valid"]))
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvtfr_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    trend_fit_rank = states["trend_fit_rank"]
    r_squared = states["r_squared"]
    trend_slope = states["trend_slope"]
    if control == "one_block_stale_geometry":
        block_return, late_return, variation_rank, trend_fit_rank, r_squared, trend_slope = [
            value.shift(1)
            for value in (block_return, late_return, variation_rank, trend_fit_rank, r_squared, trend_slope)
        ]
    variation_gate = (
    ...
    if control == "no_trend_fit_tail":
        trend_fit_gate = pd.Series(True, index=states.index)
    elif control == "raw_r_squared_at_least_half":
        trend_fit_gate = r_squared.ge(0.5)
    else:
        trend_fit_gate = trend_fit_rank.ge(0.75)
    agreement = (
        block_return.ne(0)
        & late_return.ne(0)
        & np.sign(block_return).eq(np.sign(late_return))
        & np.sign(block_return).eq(np.sign(trend_slope))
    )
    eligible = states["source_valid"] & variation_gate & trend_fit_gate & agreement
        ...
        rows.append(
            {
                ...
                "trend_slope": float(states.at[index, "trend_slope"]),
                "r_squared": float(states.at[index, "r_squared"]),
                "trend_fit_rank": float(states.at[index, "trend_fit_rank"]),
            }
        )
    columns = [
        "candidate", "control", "split", "decision_time", "feature_available_time",
        "entry_time", "exit_time", "side", "realized_variation", "variation_rank",
        "trend_slope", "r_squared", "trend_fit_rank",
    ]
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVTFR preregistration drift")
    ...
    core = {
        "protocol_version": "hvtfr_8_source_support_v1",
        "policy_id": "HVTFR-8",
        "preregistration": {
