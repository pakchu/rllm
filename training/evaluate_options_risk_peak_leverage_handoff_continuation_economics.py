"""Sequential strict economics for frozen ORPLHC-6."""
...
PREREG = Path("results/options_risk_peak_leverage_handoff_continuation_preregistration_2026-08-11.json")
PREREG_SHA = "6258c92b9a30df222651883623a18d9c6bb7d60306f63e6eccdd5cc74eecbba3"
SUPPORT = Path("results/options_risk_peak_leverage_handoff_continuation_support_2026-08-11.json")
SUPPORT_SHA = "14226b64b4f1edaf0581c3fe486decc08384d5fe93532055dd18b14008d72469"
NOVELTY = Path("results/options_risk_peak_leverage_handoff_continuation_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "422c7eed49486ec4573958abe8877769c12477a52e254bcbca618766b927b9d2"
CLOCK = Path("data/options_risk_peak_leverage_handoff_continuation_clocks_2023_2026.csv.gz")
CLOCK_SHA = "c1f458740aecf82189376ba80c38483131ee2ad4ae769e83ff45f719fc6eb4ac"
CONTROL_DIR = Path("data/options_risk_peak_leverage_handoff_continuation_controls_2023_2026")
FREEZE = Path("results/options_risk_peak_leverage_handoff_continuation_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/options_risk_peak_leverage_handoff_continuation_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("ORPLHC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("ORPLHC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "orplhc_6_sequential_economics_v1", "policy_id": POLICY_ID,
