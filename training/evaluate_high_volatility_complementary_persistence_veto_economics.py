"""Strict staged economics for the singleton HVCPV-8 policy."""
...
PREREG = Path("results/high_volatility_complementary_persistence_veto_preregistration_2026-08-16.json")
PREREG_SHA = "3695cba4142d9c116a1a5ae859177dbd42f7ae9b65e58461c3bce09ab07ee19e"
SUPPORT = Path("results/high_volatility_complementary_persistence_veto_support_2026-08-16.json")
SUPPORT_SHA = "0af842a8b2b8bbca548426b8ac62061d4ec53a44ee68347bd0375deb94463bbb"
GROSS9 = Path("results/high_volatility_complementary_persistence_veto_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "c3752ec692252927d4780a2c3907036f1deae0378d415027c905cc5508b0168c"
CLOCK = {
    "path": Path("data/high_volatility_complementary_persistence_veto_clocks_2023_2026.csv.gz"),
    "sha256": "2141f5dd21e9f5437117939e646d572793cdcbf922882bd865120d1da3fbc5ba",
    "rows": 334,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_complementary_persistence_veto_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCPV-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCPV-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCPV-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCPV-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCPV-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCPV-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCPV-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCPV-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCPV-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCPV-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCPV-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcpv_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
