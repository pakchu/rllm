"""Evaluate frozen HVRQA-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVRQA-8"
PROTOCOL = "hvrqa_8_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_recurrence_determinism_relay_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVRQA predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVRQA novelty limits drift")
    if (
        ...
        raise RuntimeError("HVRQA predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVRQA primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvrqa_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVRQA-8 unchanged before economic outcomes"
        ),
    }
