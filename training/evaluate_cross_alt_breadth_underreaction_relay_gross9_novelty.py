"""Evaluate frozen CABUR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "CABUR-8"
PROTOCOL = "cabur_8_gross9_novelty_v1"
PREREG = Path("results/cross_alt_breadth_underreaction_relay_preregistration_2026-08-09.json")
PREREG_SHA = "8d51df306ceab0c90069c09dbb8c123f1705f80856748d48df7ff41fecaed621"
SUPPORT = Path("results/cross_alt_breadth_underreaction_relay_support_2026-08-09.json")
SUPPORT_SHA = "9297550b7e7e9adf4c7576f71987fa2e49f4d00063394b4733fa67d40850fb8d"
CLOCK = Path("data/cross_alt_breadth_underreaction_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "30c5e9fe282a495cc5b9b1638c6c7d7c22939e7bc4af29d1485ad91580b1421d"
OUTPUT = Path("results/cross_alt_breadth_underreaction_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CABUR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("CABUR novelty limits drift")
    if (
        ...
        raise RuntimeError("CABUR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CABUR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "cabur_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject CABUR-8 unchanged before economic outcomes",
    }
