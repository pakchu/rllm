"""Build outcome-blind source support for frozen HVLEBR-8."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_leverage_only_execution_burst_reversal as prereg
ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2023-05-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="212ed0a7279bf4e7fa1293bacaeb7a006dac57b612f1e6ee63cbb542b98bcaca";REG=prereg.build();P=REG["policy"];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG["stages"].items()};GATES=REG["source_support_gates"];CONTROLS=tuple(REG["diagnostic_controls"]["names"]);VENUES=("perpetual","spot")
QUERY="""SELECT date_bin('5 minutes',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00') AS date,'perpetual' AS venue,(array_agg(open ORDER BY ts))[1] AS open,max(high) AS high,min(low) AS low,(array_agg(close ORDER BY ts DESC))[1] AS close,sum(power(ln(close/open),2)) AS minute_squared_return,sum(number_of_trades) AS execution_count,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND high>=greatest(open,close,low) AND low<=least(open,close,high)) AS coherent FROM bars_binance WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end GROUP BY 1 UNION ALL SELECT date_bin('5 minutes',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00') AS date,'spot' AS venue,(array_agg(open ORDER BY ts))[1] AS open,max(high) AS high,min(low) AS low,(array_agg(close ORDER BY ts DESC))[1] AS close,sum(power(ln(close/open),2)) AS minute_squared_return,sum(number_of_trades) AS execution_count,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND high>=greatest(open,close,low) AND low<=least(open,close,high)) AS coherent FROM bars_binance_spot WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end GROUP BY 1 ORDER BY 1,2"""
ROOT=Path("data/high_volatility_leverage_only_execution_burst_reversal_sources_2023_2026");PANEL=ROOT/"five_minute_barrier_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_leverage_only_execution_burst_reversal_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_leverage_only_execution_burst_reversal_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_leverage_only_execution_burst_reversal_controls_2023_2026");RESULT=Path("results/high_volatility_leverage_only_execution_burst_reversal_support_2026-08-13.json");BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLS=("decision_time","feature_available_time","source_valid","burst_side","execution_dominance","dominance_rank","perpetual_return","spot_return","btc_realized_variation","variation_rank","variation_active","eligible")
CLOCK_COLS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","execution_dominance","dominance_rank","btc_realized_variation","variation_rank")
...
def prepare(raw:pd.DataFrame)->pd.DataFrame:
 expected=["date","venue","open","high","low","close","minute_squared_return","execution_count","source_rows","distinct_rows","first_ts","last_ts","coherent"]
 if raw.columns.tolist()!=expected:raise RuntimeError("HVLEBR source schema drift")
 x=raw.copy();x["date"]=pd.to_datetime(x.date,utc=True,errors="coerce");x["venue"]=x.venue.astype(str)
 for c in ("open","high","low","close","minute_squared_return","execution_count","source_rows","distinct_rows"):x[c]=pd.to_numeric(x[c],errors="coerce")
 for c in ("first_ts","last_ts"):x[c]=pd.to_datetime(x[c],utc=True,errors="coerce")
 if x.date.isna().any() or x.duplicated(["date","venue"]).any() or not x.venue.isin(("perpetual","spot")).all():raise RuntimeError("HVLEBR invalid source key")
 z=x[["open","high","low","close"]];x["row_valid"]=x.source_rows.eq(5)&x.distinct_rows.eq(5)&x.first_ts.eq(x.date)&x.last_ts.eq(x.date+pd.Timedelta("4m"))&x.coherent.eq(True)&np.isfinite(x[["open","high","low","close","minute_squared_return","execution_count"]]).all(axis=1)&z.gt(0).all(axis=1)&x.minute_squared_return.ge(0)&x.execution_count.ge(0);return x
def burst_side(perpetual_return:pd.Series,spot_return:pd.Series,tail:pd.Series,require_nonconfirmation:bool=True)->pd.Series:
 eligible=tail&perpetual_return.ne(0)
 if require_nonconfirmation:eligible&=perpetual_return.mul(spot_return).le(0)
 side=pd.Series(0,index=perpetual_return.index,dtype=int);side.loc[eligible]=-np.sign(perpetual_return.loc[eligible]).astype(int);return side
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 x=prepare(raw);grid=pd.date_range(START,END,freq="5min",inclusive="left");op=x.pivot(index="date",columns="venue",values="open").reindex(index=grid,columns=VENUES);close=x.pivot(index="date",columns="venue",values="close").reindex(index=grid,columns=VENUES);count=x.pivot(index="date",columns="venue",values="execution_count").reindex(index=grid,columns=VENUES);sq=x.pivot(index="date",columns="venue",values="minute_squared_return").reindex(index=grid,columns=VENUES);valid=x.pivot(index="date",columns="venue",values="row_valid").reindex(index=grid,columns=VENUES).eq(True);joint=valid.all(axis=1);pret=np.log(close.perpetual/op.perpetual);sret=np.log(close.spot/op.spot);dominance=np.log1p(count.perpetual)-np.log1p(count.spot);rank=strict_prior_midrank(dominance.where(joint),P["history_decisions"],P["minimum_history_decisions"]);side=burst_side(pret,sret,rank.ge(P["dominance_rank_min"]));variation=np.sqrt(sq.perpetual.where(valid.perpetual).rolling(P["variation_bars"],min_periods=P["variation_bars"]).sum());vrank=strict_prior_midrank(variation,P["history_decisions"],P["minimum_history_decisions"]);source_valid=joint&rank.notna()&variation.notna()&vrank.notna();vactive=vrank.ge(P["variation_rank_min"]);state=side.ne(0);prior_valid=source_valid.shift(1,fill_value=False);eligible=source_valid&prior_valid&state&~state.shift(1,fill_value=False)&vactive
 return pd.DataFrame({"decision_time":grid+pd.Timedelta("5m"),"feature_available_time":grid+pd.Timedelta("5m"),"source_valid":source_valid.to_numpy(bool),"burst_side":side.to_numpy(int),"execution_dominance":dominance.to_numpy(float),"dominance_rank":rank.to_numpy(float),"perpetual_return":pret.to_numpy(float),"spot_return":sret.to_numpy(float),"btc_realized_variation":variation.to_numpy(float),"variation_rank":vrank.to_numpy(float),"variation_active":vactive.fillna(False).to_numpy(bool),"eligible":eligible.to_numpy(bool)},columns=PANEL_COLS)
def active(panel:pd.DataFrame,control="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 u=panel.copy();side=pd.to_numeric(u.burst_side,errors="coerce").fillna(0).astype(int);state=side.ne(0);prior_valid=u.source_valid.shift(1,fill_value=False);active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)&u.variation_active.eq(True)
 if control=="no_variation_gate":active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)
 elif control in ("rank_0_99","no_cash_nonconfirmation"):
  side=burst_side(u.perpetual_return,u.spot_return,u.dominance_rank.ge(.99),control!="no_cash_nonconfirmation");state=side.ne(0);active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)&u.variation_active.eq(True)
 elif control=="one_bar_stale_burst":
  cols=["burst_side","execution_dominance","dominance_rank","feature_available_time"];u[cols]=panel[cols].shift(1);side=pd.to_numeric(u.burst_side,errors="coerce").fillna(0).astype(int);state=side.ne(0);active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)&u.variation_active.eq(True)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVLEBR prereg drift")
