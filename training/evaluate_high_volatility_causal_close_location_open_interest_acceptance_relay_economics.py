"""Strict staged economics for the singleton HVCCLOIAR-8 policy."""
...
PREREG = Path("results/high_volatility_causal_close_location_open_interest_acceptance_relay_preregistration_2026-08-16.json")
PREREG_SHA = "c9620956a58c2e88a78221900939ef3cba87a31b305b97414025fe86d9cf762c"
SUPPORT = Path("results/high_volatility_causal_close_location_open_interest_acceptance_relay_support_2026-08-16.json")
SUPPORT_SHA = "e4d2aa220411664eb5249fcde4e10712d34da9beaeb79eb673731d2a12c9f46b"
GROSS9 = Path("results/high_volatility_causal_close_location_open_interest_acceptance_relay_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "27ccfb3af6e08df5243b9188b51f0d33170135df1e49e572c568d6199da0f5fa"
CLOCK = {
    "path": Path("data/high_volatility_causal_close_location_open_interest_acceptance_relay_clocks_2020_2026.csv.gz"),
    "sha256": "c70fb821c89241be95a944fb5f3980486f158d753e425c148dea3002a6869d65",
    "rows": 378,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_close_location_open_interest_acceptance_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCCLOIAR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCCLOIAR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCCLOIAR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCCLOIAR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCCLOIAR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCCLOIAR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCCLOIAR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCCLOIAR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCCLOIAR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCCLOIAR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCCLOIAR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvccloiar_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
