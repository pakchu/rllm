"""Outcome-blind support preregistration for CLASP-24.

CLASP identifies completed five-minute bars in which a relatively large,
efficient Spot impulse arrives late, USD-M has only partially propagated that
impulse, and the remaining cross-venue price gap is still visible at the bar
close.  This module may inspect causal feature incidence and frozen-clock
overlap, but it contains no post-entry return, future OHLC, CAGR, or MDD logic.
"""
...
CONTROL_SIDE_RULES = {
    ...
    "no_timing": "sign(spot_flow_fraction)",
    "no_ticket_surprise": "sign(spot_flow_fraction)",
    "no_efficiency_advantage": "sign(spot_flow_fraction)",
    "no_basis_lag": "sign(spot_flow_fraction)",
    "no_response_advantage": "sign(spot_flow_fraction)",
    "early_cash": "sign(spot_flow_fraction)",
...
class Config:
    ...
    hold_bars: int = 24
    minimum_nonoverlap_total: int = 600
    minimum_nonoverlap_per_year: int = 130
    minimum_nonoverlap_per_2023_half: int = 65
    minimum_nonoverlap_per_2023_quarter: int = 28
    minimum_side_share: float = 0.40
    minimum_side_events_per_year: int = 50
    minimum_active_months: int = 42
    minimum_events_per_active_month: int = 5
    maximum_temporal_jaccard: float = 0.01
    maximum_temporal_containment: float = 0.02
    maximum_stale_jaccard: float = 0.01
    maximum_stale_containment: float = 0.02
    maximum_prior_clock_jaccard: float = 0.02
    maximum_prior_clock_containment: float = 0.05
...
def load_causal_frame() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["date"].max() >= SELECTION_END:
        raise ValueError("CLASP support frame contains 2024+ rows")
    forbidden = {"open", "high", "low", "close", "future_return", "profit"}
    if forbidden.intersection(frame.columns):
        raise ValueError("CLASP support frame unexpectedly contains outcome columns")
    return frame, source
...
def _geometric_mean(*components: pd.Series) -> pd.Series:
    if not components:
        raise ValueError("CLASP geometric mean requires at least one component")
    values = np.vstack(
        [
...
def strictly_prior_ticket_median(
    ticket_ratio: pd.Series,
    clean: pd.Series,
    cfg: Config,
) -> pd.Series:
    if not 1 <= cfg.ticket_min_periods <= cfg.ticket_baseline_bars:
        raise ValueError("CLASP ticket baseline periods are invalid")
    return (
...
def prior_event_quantile(
    ...
    *,
    ...
    """Return a causal percentile of prior eligible events, never current rows."""
    if not 0.0 <= quantile <= 1.0:
        raise ValueError("CLASP support quantile must be in [0, 1]")
    if not 1 <= cfg.event_min_periods <= cfg.event_baseline_bars:
        raise ValueError("CLASP event baseline periods are invalid")
    return (
...
def _minimum(*values: pd.Series) -> pd.Series:
    if not values:
        raise ValueError("CLASP minimum requires at least one series")
...
def _components(frame: pd.DataFrame, cfg: Config) -> dict[str, pd.Series]:
    ...
    values["spot_directed_return_bp"] = (
        values["spot_side"] * values["spot_log_return_5m"] * 10_000.0
    )
    values["um_in_spot_direction_bp"] = (
        values["spot_side"] * values["um_log_return_5m"] * 10_000.0
    )
    values["um_directed_return_bp"] = (
        values["um_side"] * values["um_log_return_5m"] * 10_000.0
    )
    values["spot_in_um_direction_bp"] = (
        values["um_side"] * values["spot_log_return_5m"] * 10_000.0
    )
    values["spot_basis_lag_bp"] = -values["spot_side"] * values["basis_change_bp"]
    values["um_basis_lag_bp"] = values["um_side"] * values["basis_change_bp"]

    spot_efficiency = (
        values["spot_log_return_5m"].abs()
        * 10_000.0
        / values["spot_abs_path_return_bp"].where(
            values["spot_abs_path_return_bp"].gt(0.0)
        )
    )
    um_efficiency = (
        values["um_log_return_5m"].abs()
        * 10_000.0
        / values["um_abs_path_return_bp"].where(values["um_abs_path_return_bp"].gt(0.0))
    )
    values["spot_efficiency_advantage"] = spot_efficiency - um_efficiency
    values["um_efficiency_advantage"] = um_efficiency - spot_efficiency

    spot_late = (
        values["spot_activity_time_centroid"] - values["um_activity_time_centroid"]
    )
    spot_flow_late = values["spot_flow_time_centroid"] - values["um_flow_time_centroid"]
    spot_return_late = (
        values["spot_return_time_centroid"] - values["um_return_time_centroid"]
    )
    values["spot_closing_latency"] = _minimum(
        spot_late, spot_flow_late, spot_return_late
    )
    values["um_closing_latency"] = _minimum(
        -spot_late, -spot_flow_late, -spot_return_late
    )

    spot_average_ticket = values["spot_quote_notional"] / values[
        "spot_trade_count"
    ].where(values["spot_trade_count"].gt(0.0))
    um_average_ticket = values["um_quote_notional"] / values["um_trade_count"].where(
        values["um_trade_count"].gt(0.0)
    )
    ticket_ratio = np.log(spot_average_ticket / um_average_ticket).replace(
        [np.inf, -np.inf], np.nan
    )
    ticket_median = strictly_prior_ticket_median(ticket_ratio, clean, cfg)
    values["spot_ticket_surprise"] = ticket_ratio - ticket_median
    values["um_ticket_surprise"] = ticket_median - ticket_ratio

    spot_forward = _minimum(
        values["spot_to_um_lagged_flow_response_bp"],
        values["lagged_flow_response_diff_bp"],
    )
    spot_reverse = _minimum(
        values["reverse_spot_to_um_lagged_flow_response_bp"],
        values["reverse_lagged_flow_response_diff_bp"],
    )
    values["spot_response_advantage_bp"] = spot_forward - spot_reverse.clip(lower=0.0)
    um_forward = _minimum(
        values["um_to_spot_lagged_flow_response_bp"],
        -values["lagged_flow_response_diff_bp"],
    )
    um_reverse = _minimum(
        values["reverse_um_to_spot_lagged_flow_response_bp"],
        -values["reverse_lagged_flow_response_diff_bp"],
    )
    values["um_response_advantage_bp"] = um_forward - um_reverse.clip(lower=0.0)
    return values


def _score_components(
    *,
    latency: pd.Series | None,
    efficiency: pd.Series | None,
    ...
    if response is not None:
        components.append(np.log1p(response.clip(lower=0.0)))
...
def classify_events(
    ...
    *,
    ...
    spot_common = (
        clean
        & v["spot_side"].ne(0.0)
        & v["spot_directed_return_bp"].gt(0.0)
        & v["um_in_spot_direction_bp"].ge(0.0)
    )
    um_common = (
        clean
        & v["um_side"].ne(0.0)
        & v["um_directed_return_bp"].gt(0.0)
        & v["spot_in_um_direction_bp"].ge(0.0)
    )
    required = {
        "latency": v["spot_closing_latency"].gt(0.0),
        "ticket": v["spot_ticket_surprise"].gt(0.0),
        "efficiency": v["spot_efficiency_advantage"].gt(0.0),
        "basis": v["spot_basis_lag_bp"].gt(0.0),
        "response": v["spot_response_advantage_bp"].gt(0.0),
    }
    base_masks = {
        "primary": spot_common & np.logical_and.reduce(list(required.values())),
        "no_timing": spot_common
        & required["ticket"]
        & required["efficiency"]
        & required["basis"]
        & required["response"],
        "no_ticket_surprise": spot_common
        & required["latency"]
        & required["efficiency"]
        & required["basis"]
        & required["response"],
        "no_efficiency_advantage": spot_common
        & required["latency"]
        & required["ticket"]
        & required["basis"]
        & required["response"],
        "no_basis_lag": spot_common
        & required["latency"]
        & required["ticket"]
        & required["efficiency"]
        & required["response"],
        "no_response_advantage": spot_common
        & required["latency"]
        & required["ticket"]
        & required["efficiency"]
        & required["basis"],
        "early_cash": spot_common
        & v["um_closing_latency"].gt(0.0)
        & required["ticket"]
        & required["efficiency"]
        & required["basis"]
        & required["response"],
        "venue_swap": um_common
        & v["um_closing_latency"].gt(0.0)
        & v["um_ticket_surprise"].gt(0.0)
        & v["um_efficiency_advantage"].gt(0.0)
        & v["um_basis_lag_bp"].gt(0.0)
        & v["um_response_advantage_bp"].gt(0.0),
        "aggregate_only": spot_common & required["basis"],
    }
    scores = {
        "primary": _score_components(
            latency=v["spot_closing_latency"],
            efficiency=v["spot_efficiency_advantage"],
            basis=v["spot_basis_lag_bp"],
            ticket=v["spot_ticket_surprise"],
            response=v["spot_response_advantage_bp"],
        ),
        "no_timing": _score_components(
            latency=None,
            efficiency=v["spot_efficiency_advantage"],
            basis=v["spot_basis_lag_bp"],
            ticket=v["spot_ticket_surprise"],
        ...
        "no_efficiency_advantage": _score_components(
            latency=v["spot_closing_latency"],
            efficiency=None,
            basis=v["spot_basis_lag_bp"],
            ticket=v["spot_ticket_surprise"],
        ...
        "no_basis_lag": _score_components(
            latency=v["spot_closing_latency"],
            efficiency=v["spot_efficiency_advantage"],
            basis=None,
            ticket=v["spot_ticket_surprise"],
        ...
        "no_response_advantage": _score_components(
            latency=v["spot_closing_latency"],
            efficiency=v["spot_efficiency_advantage"],
            basis=v["spot_basis_lag_bp"],
            ticket=v["spot_ticket_surprise"],
        ...
        "early_cash": _score_components(
            latency=v["um_closing_latency"],
            efficiency=v["spot_efficiency_advantage"],
            basis=v["spot_basis_lag_bp"],
            ticket=v["spot_ticket_surprise"],
            response=v["spot_response_advantage_bp"],
        ...
        "venue_swap": _score_components(
            latency=v["um_closing_latency"],
            efficiency=v["um_efficiency_advantage"],
            basis=v["um_basis_lag_bp"],
            ticket=v["um_ticket_surprise"],
            response=v["um_response_advantage_bp"],
        ),
        "aggregate_only": _geometric_mean(
            v["spot_flow_fraction"].abs(),
            v["spot_flow_coherence"],
            np.log1p(v["spot_basis_lag_bp"].clip(lower=0.0)),
        ),
    }
    thresholds = {
        name: prior_event_quantile(
            score,
            base_masks[name],
            quantile=quantile,
            cfg=cfg,
    ...
    control_sides = {
        "primary": v["spot_side"],
        "direction_flip": -v["spot_side"],
        **{
            name: (v["um_side"] if name == "venue_swap" else v["spot_side"])
            for name in selected
            if name != "primary"
        },
        "stale_1h": v["spot_side"].shift(12).fillna(0.0),
        "stale_24h": v["spot_side"].shift(288).fillna(0.0),
        "signal_delay_1bar": v["spot_side"].shift(1).fillna(0.0),
    }
    signal = pd.DataFrame(
        {
            "date": frame["date"],
            "side": np.where(primary, v["spot_side"], 0).astype(np.int8),
            "branch": np.where(primary, "clasp24", "none"),
            "hold_bars": np.where(primary, cfg.hold_bars, 0).astype(np.int16),
            "score": scores["primary"],
            "threshold": thresholds["primary"],
            "closing_latency": v["spot_closing_latency"],
            "ticket_surprise": v["spot_ticket_surprise"],
            "efficiency_advantage": v["spot_efficiency_advantage"],
            "basis_lag_bp": v["spot_basis_lag_bp"],
            "response_advantage_bp": v["spot_response_advantage_bp"],
...
def _control_signal(
    ...
    *,
    ...
    if not numeric_side.loc[active].isin((-1.0, 1.0)).all():
        raise ValueError(f"CLASP control {branch} has invalid active side")
    return pd.DataFrame(
        {
...
def run_support(cfg: Config) -> dict[str, Any]:
    ...
    for quantile in SUPPORT_QUANTILES:
        ...
        novelty = (
            all(
                ...
                for name in ("early_cash", "venue_swap")
            )
            and all(
                _passes_overlap(
                    overlap[name],
        ...
        ablation_names = (
            "no_timing",
            "no_ticket_surprise",
            "no_efficiency_advantage",
            "no_basis_lag",
            "no_response_advantage",
            "aggregate_only",
        )
        rows.append(
            {
    ...
    return {
        "protocol": {
            ...
            "economic_claim": (
                "a late, efficient, unusually large-ticket Spot impulse leaves "
                "a partially propagated USD-M response debt at the completed bar"
            ),
            "support_selection": (
                "highest frozen causal event-score quantile passing every count, "
                "era, side-balance, temporal-placebo, stale-clock, and prior-clock floor"
            ),
...
def main() -> None:
    ...
    print(
        json.dumps(
            {
                "outcomes_opened": result["protocol"]["clasp_outcomes_opened"],
