"""Materialize deterministic, outcome-blind source support for HVCELV-8."""
...
from training import preregister_high_volatility_caspc_ehlers_active_veto as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv

PREREG_SHA = "7dc8f6228fa32eba7d5aded586d389855a47fbf12917800c43537e5e18c6244f"
CLOCK = Path("data/high_volatility_caspc_ehlers_active_veto_clocks_2023_2026.csv.gz")
RESULT = Path("results/high_volatility_caspc_ehlers_active_veto_support_2026-08-16.json")
...
def _registration() -> dict[str, Any]:
    if prereg.sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCELV-8 preregistration hash drift")
    ...
    for component, artifacts in prereg.COMPONENT_ARTIFACTS.items():
        for kind, artifact in artifacts.items():
            if prereg.sha256_file(artifact["path"]) != artifact["sha256"]:
                raise RuntimeError(f"HVCELV-8 {component} {kind} drift")
...
def load_clock(component: str) -> pd.DataFrame:
    ...
    if frame["feature_available_time"].duplicated().any() or not frame["side"].isin((-1, 1)).all():
        raise RuntimeError(f"HVCELV-8 invalid {component} clock")
    expected_hours = (3, 11, 19) if component == prereg.ACTION_IDS[0] else (0, 4, 8, 12, 16, 20)
    if not frame["feature_available_time"].dt.hour.isin(expected_hours).all() or not frame["feature_available_time"].dt.minute.eq(0).all():
        raise RuntimeError(f"HVCELV-8 off-grid {component} clock")
    ...
    if not frame["entry_time"].eq(frame["feature_available_time"] + ENTRY_DELAY).all() or not frame["exit_time"].eq(frame["entry_time"] + expected_hold).all():
        raise RuntimeError(f"HVCELV-8 {component} timing drift")
    if not frame["feature_available_time"].le(frame["entry_time"]).all():
        raise RuntimeError(f"HVCELV-8 unavailable {component} feature")
...
def build_clock(clocks: Mapping[str, pd.DataFrame]) -> tuple[pd.DataFrame, dict[str, Any]]:
    if set(clocks) != set(prereg.COMPONENT_IDS):
        raise RuntimeError("HVCELV-8 component roster drift")
    ...
    for decision in decisions:
        ...
        if len(active_vetoes) > 1:
            raise RuntimeError("HVCELV-8 overlapping active veto positions")
        ...
        if primary.split != split or primary.feature_available_time > entry:
            raise RuntimeError("HVCELV-8 primary stage or availability drift")
        if veto is not None and (veto.split != split or veto.feature_available_time > entry):
            raise RuntimeError("HVCELV-8 veto stage or availability drift")
    ...
    if counts["emitted_decisions"] + counts["veto_opposite_side"] != counts["primary_decisions"]:
        raise RuntimeError("HVCELV-8 incidence accounting drift")
    if len(output) > 1 and not output["entry_time"].iloc[1:].reset_index(drop=True).ge(output["exit_time"].iloc[:-1].reset_index(drop=True)).all():
        raise RuntimeError("HVCELV-8 output overlap")
