"""Evaluate frozen HVSUDR-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVSUDR-24"
PROTOCOL = "hvsudr_24_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_semiconductor_utilities_disagreement_relay_preregistration_2026-08-11.json"
)
...
OUTPUT = Path(
    "results/high_volatility_semiconductor_utilities_disagreement_relay_gross9_novelty_2026-08-11.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVSUDR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVSUDR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVSUDR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVSUDR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvsudr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVSUDR-24 unchanged before economic outcomes"
        ),
    }
