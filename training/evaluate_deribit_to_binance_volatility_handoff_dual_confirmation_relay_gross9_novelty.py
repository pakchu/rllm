"""Evaluate frozen DBVHDR-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "DBVHDR-6"
PROTOCOL = "dbvhdr_6_gross9_novelty_v1"
...
OUTPUT = Path("results/deribit_to_binance_volatility_handoff_dual_confirmation_relay_gross9_novelty_2026-08-08.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha256(PREREG) != PREREG_SHA or sha256(SUPPORT) != SUPPORT_SHA or sha256(CLOCK) != CLOCK_SHA:
        raise RuntimeError("DBVHDR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("DBVHDR novelty limits drift")
    if (
        ...
        raise RuntimeError("DBVHDR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="DBVHDR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "dbvhdr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject DBVHDR-6 unchanged before economic outcomes",
    }
