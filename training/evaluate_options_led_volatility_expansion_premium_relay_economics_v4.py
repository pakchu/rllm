"""Fresh V4 OVEPR economics with verified package and gzip entrypoints."""
...
FAILURE=Path('results/options_led_volatility_expansion_premium_relay_train_economic_attempt_terminal_failure_v3_2026-08-08.json');FAILURE_SHA='a721a0566e703c25b7221037125af6c0149df204ff55de64742d1e84ad498372'
...
OUTPUTS={s:Path(f'results/options_led_volatility_expansion_premium_relay_{s}_economics_v4_2026-08-08.json') for s in STAGES}
...
def verify(stage:str)->dict[str,Any]:
 ...
 if sha(FAILURE)!=FAILURE_SHA:raise RuntimeError('V3 terminal failure receipt drift')
 failure=load_json(FAILURE)
 if failure.get('economic_metrics_computed') is not False or failure.get('btc_execution_numeric_rows_opened') is not False or failure.get('attempt_disposition')!='TERMINAL_FAILURE_NO_RETRY_UNDER_V3':raise RuntimeError('V3 failure is not eligible for infrastructure successor')
 if stage in PREDECESSOR:
  p=OUTPUTS[PREDECESSOR[stage]]
  if not p.is_file():raise RuntimeError(f'missing V4 predecessor: {p}')
  d=load_json(p);core={k:v for k,v in d.items() if k!='manifest_hash'}
  if d.get('manifest_hash')!=chash(core) or d.get('passed') is not True:raise RuntimeError('V4 predecessor did not pass')
 return n
