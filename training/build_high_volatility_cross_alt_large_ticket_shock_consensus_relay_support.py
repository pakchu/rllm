"""Build outcome-blind source support for frozen HVCALTS-8."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_cross_alt_large_ticket_shock_consensus_relay as prereg
ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2023-05-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="b24f71d7f180cd4f085fa3e5934cffe094665e9e3c6f489c9ee25a2f765eb719";REG=prereg.build();P=REG["policy"];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG["stages"].items()};GATES=REG["source_support_gates"];CONTROLS=tuple(REG["diagnostic_controls"]["names"]);SYMBOLS=("BTCUSDT",*prereg.ALTS);ALTS=prereg.ALTS
QUERY="""SELECT date_bin('5 minutes',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00') AS date,symbol,(array_agg(open ORDER BY ts))[1] AS open,max(high) AS high,min(low) AS low,(array_agg(close ORDER BY ts DESC))[1] AS close,sum(quote_asset_volume) AS quote_turnover,sum(number_of_trades) AS trade_count,sum(power(ln(close/open),2)) AS minute_squared_return,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND quote_asset_volume>=0 AND number_of_trades>=0 AND high>=greatest(open,close,low) AND low<=least(open,close,high)) AS coherent FROM bars_binance WHERE symbol IN ('BTCUSDT','ADAUSDT','BNBUSDT','DOGEUSDT','ETHUSDT','SOLUSDT','XRPUSDT') AND interval='1m' AND ts>=:start AND ts<:end GROUP BY 1,symbol ORDER BY 1,symbol"""
ROOT=Path("data/high_volatility_cross_alt_large_ticket_shock_consensus_relay_sources_2023_2026");PANEL=ROOT/"five_minute_large_ticket_shock_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_cross_alt_large_ticket_shock_consensus_relay_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_cross_alt_large_ticket_shock_consensus_relay_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_cross_alt_large_ticket_shock_consensus_relay_controls_2023_2026");RESULT=Path("results/high_volatility_cross_alt_large_ticket_shock_consensus_relay_support_2026-08-13.json");BUILDER=Path(__file__).relative_to(Path.cwd())
...
def prepare(raw:pd.DataFrame)->pd.DataFrame:
 expected=["date","symbol","open","high","low","close","quote_turnover","trade_count","minute_squared_return","source_rows","distinct_rows","first_ts","last_ts","coherent"]
 if raw.columns.tolist()!=expected:raise RuntimeError("HVCALTS source schema drift")
 x=raw.copy();x["date"]=pd.to_datetime(x.date,utc=True,errors="coerce");x["symbol"]=x.symbol.astype(str)
 for c in ("open","high","low","close","quote_turnover","trade_count","minute_squared_return","source_rows","distinct_rows"):x[c]=pd.to_numeric(x[c],errors="coerce")
 for c in ("first_ts","last_ts"):x[c]=pd.to_datetime(x[c],utc=True,errors="coerce")
 if x.date.isna().any() or x.duplicated(["date","symbol"]).any() or not x.symbol.isin(SYMBOLS).all():raise RuntimeError("HVCALTS invalid source key")
 z=x[["open","high","low","close"]];x["row_valid"]=x.source_rows.eq(5)&x.distinct_rows.eq(5)&x.first_ts.eq(x.date)&x.last_ts.eq(x.date+pd.Timedelta("4m"))&x.coherent.eq(True)&np.isfinite(x[["open","high","low","close","quote_turnover","trade_count","minute_squared_return"]]).all(axis=1)&z.gt(0).all(axis=1)&x.quote_turnover.gt(0)&x.trade_count.gt(0)&x.minute_squared_return.ge(0);x["return"]=np.log(x.close/x.open);x["average_ticket"]=x.quote_turnover/x.trade_count;return x
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 x=prepare(raw);grid=pd.date_range(START,END,freq="5min",inclusive="left");ret=x.pivot(index="date",columns="symbol",values="return").reindex(index=grid,columns=SYMBOLS);ticket=x.pivot(index="date",columns="symbol",values="average_ticket").reindex(index=grid,columns=SYMBOLS);sq=x.pivot(index="date",columns="symbol",values="minute_squared_return").reindex(index=grid,columns=SYMBOLS);valid=x.pivot(index="date",columns="symbol",values="row_valid").reindex(index=grid,columns=SYMBOLS).eq(True);alt_valid=valid.loc[:,list(ALTS)];alt_ticket=ticket.loc[:,list(ALTS)].where(alt_valid);threshold=alt_ticket.shift(1).rolling(P["ticket_history_bars"],min_periods=P["minimum_ticket_history_bars"]).quantile(P["ticket_quantile"],interpolation="linear");history_valid=alt_valid.shift(1).rolling(P["ticket_history_bars"],min_periods=P["minimum_ticket_history_bars"]).sum().ge(P["minimum_ticket_history_bars"]);shocks=alt_ticket.ge(threshold)&threshold.notna()&history_valid&alt_valid;side,pos,neg=shock_side(ret.loc[:,list(ALTS)],shocks,P["minimum_consensus_breadth"]);market_ticket=alt_ticket.mean(axis=1);market_threshold=market_ticket.shift(1).rolling(P["ticket_history_bars"],min_periods=P["minimum_ticket_history_bars"]).quantile(P["ticket_quantile"],interpolation="linear");all_shocked=pd.DataFrame(True,index=grid,columns=ALTS);market_side,_,_=shock_side(ret.loc[:,list(ALTS)],all_shocked,P["minimum_consensus_breadth"]);market_side=market_side.where(market_ticket.ge(market_threshold),0);variation=np.sqrt(sq.BTCUSDT.where(valid.BTCUSDT).rolling(P["variation_bars"],min_periods=P["variation_bars"]).sum());vrank=strict_prior_midrank(variation,P["variation_history_decisions"],P["minimum_variation_history_decisions"]);source_valid=valid.all(axis=1)&threshold.notna().all(axis=1)&variation.notna()&vrank.notna();vactive=vrank.ge(P["variation_rank_min"]);broad=side.ne(0);prior_valid=source_valid.shift(1,fill_value=False);eligible=source_valid&prior_valid&broad&~broad.shift(1,fill_value=False)&vactive
...
def active(panel:pd.DataFrame,control="primary"):
 ...
 elif control=="three_of_six_ticket_shock":
 ...
 elif control=="equal_weight_market_ticket_shock":
 ...
 elif control=="one_bar_stale_ticket_shock":
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCALTS prereg drift")
