"""Evaluate frozen CARSC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "CARSC-8"
PROTOCOL = "carsc_8_gross9_novelty_v1"
...
OUTPUT = Path("results/cross_alt_return_synchrony_continuation_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CARSC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("CARSC novelty limits drift")
    if (
        ...
        raise RuntimeError("CARSC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CARSC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "carsc_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject CARSC-8 unchanged before economic outcomes",
    }
