"""Sequential strict economics for frozen HVMFI-24."""
...
PREREG = Path("results/high_volatility_money_flow_index_reentry_relay_preregistration_2026-08-11.json")
PREREG_SHA = "c53a80828d472a3823613eb7da5a6885cf5552eeb9afacc4f38d0e6843484a03"
SUPPORT = Path("results/high_volatility_money_flow_index_reentry_relay_support_2026-08-11.json")
SUPPORT_SHA = "e2a4a64d23f6ca5dc2257c58b897fb38474b2154027e189f1493d485c62e7f7e"
NOVELTY = Path("results/high_volatility_money_flow_index_reentry_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "5a9c5e5549fda4370cbc361bbb6b6b194bd154ac2567e346f4e36abf9f18e999"
CLOCK = Path("data/high_volatility_money_flow_index_reentry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "bc4d36a491dcb99c9b8214ffab522fec158cc261c6b3acb2d6fa78dd2913c132"
CONTROL_DIR = Path("data/high_volatility_money_flow_index_reentry_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_money_flow_index_reentry_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_money_flow_index_reentry_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "unweighted_typical_direction", "one_bar_stale_reentry", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVMFI frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVMFI novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvmfi_24_sequential_economics_v1", "policy_id": POLICY_ID,
