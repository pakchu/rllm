"""Build source-only CLIRR-6 confirmation-ladder clocks."""
...
PREREG_SHA = "fbc084b980cf292d69756f736c3b391ee8a6ff5d2eb8eb5f6b369d69cf8c1ab3"
...
CONTROLS = ("direction_reversal_only", "acceleration_only", "one_anchor_stale_ladder", "direction_flip")
...
SOURCE_DIR = Path("data/confirmation_ladder_impulse_reversal_relay_sources_2023_2026")
FEATURES = SOURCE_DIR / "confirmation_ladders.csv.gz"
MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/confirmation_ladder_impulse_reversal_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/confirmation_ladder_impulse_reversal_relay_controls_2023_2026")
RESULT = Path("results/confirmation_ladder_impulse_reversal_relay_support_2026-08-09.json")
...
def validate_blocks(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CLIRR block schema drift")
    ...
    if not np.array_equal(np.diff(blocks.height), np.ones(len(blocks) - 1, dtype=np.int64)):
        raise RuntimeError("CLIRR block heights are not contiguous")
    if not blocks.previousblockhash.iloc[1:].reset_index(drop=True).equals(blocks.id.iloc[:-1].reset_index(drop=True)):
        raise RuntimeError("CLIRR canonical previous-hash continuity failed")
    return blocks
...
def prepare_minutes(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CLIRR minute schema drift")
    ...
    if result.ts.duplicated().any():
        raise RuntimeError("CLIRR duplicate timestamps")
...
def active_and_side(features: pd.DataFrame, control: str = "primary"):
    ...
    reversal = np.sign(features.early_return).eq(-np.sign(features.late_return))
    primary = features.eligible_state.astype(bool); late = features.late_return.copy()
    if control == "direction_reversal_only":
        state = features.source_valid & reversal
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("CLIRR preregistration drift")
    ...
    source_core = {
        "protocol_version": "clirr_6_source_v1", "query_sha256": hashlib.sha256(QUERY.encode()).hexdigest(), "window": [QUERY_START.isoformat(), END.isoformat()],
    ...
    core = {
        "protocol_version": "clirr_6_source_support_v1", "policy_id": prereg.POLICY_ID,
