"""Build outcome-blind source support for frozen HVCHO-24."""
...
from training import preregister_high_volatility_chaikin_ad_oscillator_zero_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "af48092e85ebab11b93a9da5aa386380161a06c4797de75fbe3f0c49275a2217"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_chaikin_ad_oscillator_zero_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_chaikin_ad_oscillator_zero_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_chaikin_ad_oscillator_zero_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_chaikin_ad_oscillator_zero_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_chaikin_ad_oscillator_zero_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "bar_volume", "money_flow_multiplier", "money_flow_volume",
    "adl", "fast_ema", "slow_ema", "oscillator", "mfv_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "bar_volume", "money_flow_multiplier",
    "money_flow_volume", "adl", "fast_ema", "slow_ema", "oscillator", "mfv_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close", "volume"]:
        raise RuntimeError("HVCHO source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVCHO invalid source key")
...
def causal_adl(money_flow_volume: pd.Series, valid: pd.Series) -> pd.Series:
    output=np.full(len(money_flow_volume),np.nan);state=np.nan
    for i,(value,ok) in enumerate(zip(money_flow_volume.to_numpy(float),valid.to_numpy(bool))):
        if not ok or not math.isfinite(value):state=np.nan;continue
        state=value if not math.isfinite(state) else state+value;output[i]=state
    return pd.Series(output,index=money_flow_volume.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    spread=bars.bar_high-bars.bar_low;bars["money_flow_multiplier"]=((bars.bar_close-bars.bar_low)-(bars.bar_high-bars.bar_close))/spread;flow_valid=bars.valid_bar&spread.gt(0)&bars.bar_volume.gt(0);bars["money_flow_volume"]=(bars.money_flow_multiplier*bars.bar_volume).where(flow_valid);bars["adl"]=causal_adl(bars.money_flow_volume,flow_valid)
    adl_valid=np.isfinite(bars.adl);bars["fast_ema"]=causal_ema(bars.adl,adl_valid,P["fast_periods"]);bars["slow_ema"]=causal_ema(bars.adl,adl_valid,P["slow_periods"]);bars["oscillator"]=bars.fast_ema-bars.slow_ema
    previous_flow=bars.money_flow_volume.shift(1);bars["mfv_side"]=np.where((bars.money_flow_volume>0)&(previous_flow<=0),1,np.where((bars.money_flow_volume<0)&(previous_flow>=0),-1,0))
    bars["variation"] = np.sqrt(bars.variation_component.rolling(P["variation_hours"] // 4, min_periods=P["variation_hours"] // 4).sum())
    bars["source_valid"] = bars.valid_bar & np.isfinite(bars[["bar_volume","money_flow_multiplier","money_flow_volume","adl","fast_ema","slow_ema","oscillator","variation"]]).all(axis=1) & bars.bar_volume.gt(0) & spread.gt(0) & bars.oscillator.ne(0) & bars.variation.gt(0)
    ...
    previous=panel.oscillator.shift(1)
    panel["eligible"]=panel.source_valid & panel.source_valid.shift(1,fill_value=False) & previous.ne(0) & np.sign(panel.oscillator).ne(np.sign(previous)) & panel.variation_rank.ge(P["variation_rank_min"])
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    signal=pd.to_numeric(used.oscillator,errors="coerce");previous=signal.shift(1)
    ...
    elif control=="money_flow_volume_zero_cross":
        side=pd.to_numeric(used.mfv_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCHO prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvcho_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvcho_24_source_support_v1", "policy_id": prereg.POLICY_ID,
