"""Evaluate frozen HVWRTR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVWRTR-8"
PROTOCOL = "hvwrtr_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_wick_response_transfer_relay_preregistration_2026-08-10.json")
PREREG_SHA = "b94e3cc19df075ef00da6084f6bf92a63d0acca6da9d9596e9579773150b1a98"
SUPPORT = Path("results/high_volatility_wick_response_transfer_relay_support_2026-08-10.json")
SUPPORT_SHA = "bcf684242b610b78a33baad65516a6893ba34eae182e2ff6a8b0f7240d3909fd"
CLOCK = Path("data/high_volatility_wick_response_transfer_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "b9a5195f5b3704a75e9e79face964d2d972f18dceee269b9c0a57ddcc5c09b6f"
OUTPUT = Path("results/high_volatility_wick_response_transfer_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVWRTR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVWRTR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVWRTR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVWRTR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvwrtr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVWRTR-8 unchanged before economic outcomes",
    }
