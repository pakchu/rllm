"""Outcome-blind preregistration for HVCFAR-8."""
...
POLICY_ID = "HVCFAR-8"
DEFAULT_OUTPUT = Path("results/high_volatility_causal_flow_acceleration_inventory_relay_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_causal_flow_acceleration_inventory_relay_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": "When the first and second four-hour BTC perpetual aggressive-quote-flow shares have the same strict sign, the second half is more extreme by an unusually large amount, realized variation is elevated, and open interest expands over the completed eight-hour cycle, directional leveraged demand is accelerating with fresh inventory and should propagate for eight hours.",
            "side": "strict common sign of first-half and second-half BTC perpetual aggressive-quote-flow shares",
            "why_distinct": "HVCFAR uses within-cycle acceleration of perpetual aggressive-flow magnitude plus exact OI expansion and variation. HVCPOFR used full-cycle flow level and failed train; existing flow-acceleration candidates omit exact OI sponsorship. HVCFAR reuses neither event set nor control and uses no spot source, return direction, ETH, funding, premium-index, fitted outcome, or promoted control.",
            ...
            "why_low_gross9_overlap_is_plausible": "fixed eight-hour accelerating-flow plus OI-sponsored states are absent from Gross9 primitives",
        },
        "features": {
            ...
            "half_flow_shares": "the signed aggressive quote-flow share computed separately over first and second 240-minute halves; both denominators positive, shares strict nonzero and same strict sign",
            "flow_acceleration": "abs(second_half_flow_share)-abs(first_half_flow_share), finite strict positive",
            "flow_acceleration_rank": "strict-prior midrank over at most 270 source-valid cycles; current excluded; minimum 180; rank>=0.60",
            ...
            "eligible": "exact coherent completed perpetual cycle, same-sign nonzero half-flow shares with positive acceleration, two exact OI endpoints with strict expansion, and both acceleration and variation ranks pass at D; no spot source, return direction, funding, range, premium-index, alt, or post-decision condition",
            "no_imputation": True,
        },
        "clock": {
        ...
        "policy": {
            ...
            "path_minutes": 480,
            "half_minutes": 240,
            "flow_acceleration_rank_min": 0.60,
        ...
        "research_boundary": {
            ...
            "prior_flow_acceleration_candidate_outcomes_known": True,
            "same_mechanism_as_prior_flow_acceleration_candidates": False,
            "prior_HVCPOFR_source_novelty_and_train_failure_known": True,
            "same_mechanism_as_HVCPOFR": False,
        ...
        "stopping_rule": "terminal first failure; no OI endpoint or expansion rule, half-flow or acceleration formula, acceleration or variation history, side, clock, hold, subset, threshold, comparator, or control repair",
    }
    return {**core, "manifest_hash": canonical_hash(core)}

def validate(value: Mapping[str, Any]) -> None:
    if value != build():
        raise RuntimeError("HVCFAR-8 preregistration drift")
    contract = hvcav.build()
    for key in ("stages", "source_support_gates", "gross9_novelty_gates", "economic_gates"):
        if value[key] != contract[key]:
            raise RuntimeError(f"HVCFAR-8 {key} drift")
