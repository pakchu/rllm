"""Evaluate frozen HVACR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVACR-8"
PROTOCOL = "hvacr_8_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_absolute_return_clustering_relay_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVACR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVACR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVACR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVACR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvacr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVACR-8 unchanged before economic outcomes"
        ),
    }
