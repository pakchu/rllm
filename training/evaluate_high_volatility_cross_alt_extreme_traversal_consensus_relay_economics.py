"""Sequential strict economics for frozen HVCATCR-8."""
...
PREREG = Path("results/high_volatility_cross_alt_extreme_traversal_consensus_relay_preregistration_2026-08-12.json")
PREREG_SHA = "0966572a02908af2d206c66f91cc11f0955b93d95ad3eda56c57e33eb9a05f89"
SUPPORT = Path("results/high_volatility_cross_alt_extreme_traversal_consensus_relay_support_2026-08-12.json")
SUPPORT_SHA = "1124857eb65a1551b150e6b0b80114eafd111419bf07f36fc29a6a05d0fa5b6a"
NOVELTY = Path("results/high_volatility_cross_alt_extreme_traversal_consensus_relay_gross9_novelty_2026-08-12.json")
NOVELTY_SHA = "f361996ec06fd029dfe9dce24ff5e48d8e41e2ad11d7489264012dae35cd4747"
CLOCK = Path("data/high_volatility_cross_alt_extreme_traversal_consensus_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "648c3c2f3baffd0458aa9114b3411ccd1cd6dba7b6c489681b88a3da571cec83"
CONTROL_DIR = Path("data/high_volatility_cross_alt_extreme_traversal_consensus_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_cross_alt_extreme_traversal_consensus_relay_economic_evaluator_freeze_2026-08-12.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_alt_extreme_traversal_consensus_relay_{stage}_economics_2026-08-12.json")
    for stage in STAGES
}
CONTROLS = ("no_traversal_consensus_gate", "no_variation_gate", "close_location_consensus_instead_of_extreme_order", "one_block_stale_features", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCATCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCATCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcatcr_24_sequential_economics_v1", "policy_id": POLICY_ID,
