"""Evaluate frozen HVPPL-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVPPL-8"
PROTOCOL = "hvppl_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_premium_price_phase_loop_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVPPL predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVPPL novelty limits drift")
    if (
        ...
        raise RuntimeError("HVPPL predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVPPL primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVPPL-8 unchanged before economic outcomes",
    }
