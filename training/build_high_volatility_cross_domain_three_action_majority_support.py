"""Materialize deterministic, outcome-blind source support for HVCM3-8."""
...
from training import preregister_high_volatility_cross_domain_three_action_majority as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv

PREREG_SHA = "99e3ce77b847dbad313a2d62cc3ff8c0dcf7ab9631072bab38f5cf867de1dd0b"
CLOCK = Path("data/high_volatility_cross_domain_three_action_majority_clocks_2023_2026.csv.gz")
RESULT = Path("results/high_volatility_cross_domain_three_action_majority_support_2026-08-16.json")
...
def _registration() -> dict[str, Any]:
    if prereg.sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCM3-8 preregistration hash drift")
    ...
    for component, artifacts in prereg.COMPONENT_ARTIFACTS.items():
        for kind, artifact in artifacts.items():
            if prereg.sha256_file(artifact["path"]) != artifact["sha256"]:
                raise RuntimeError(f"HVCM3-8 {component} {kind} drift")
...
def build_clock(clocks: Mapping[str, pd.DataFrame]) -> tuple[pd.DataFrame, dict[str, Any]]:
    if set(clocks) != set(prereg.COMPONENT_IDS):
        raise RuntimeError("HVCM3-8 component roster drift")
    ...
    rows = []
    counts: dict[str, Any] = {
        "union_decisions": 0, "no_quorum_decisions": 0, "tie_decisions": 0,
        "emitted_decisions": 0, "long_majorities": 0, "short_majorities": 0,
        "active_count_distribution": {"1": 0, "2": 0, "3": 0},
    ...
    for decision in decisions:
        ...
        padded = sides + [0] * (3 - len(sides))
        side = prereg.three_action_majority_side(*padded)
        if side == 0:
            counts["tie_decisions"] += 1
            continue
        entry, exit_ = decision + ENTRY_DELAY, decision + ENTRY_DELAY + HOLD
        if any(row.split != split or row.feature_available_time > entry for row in active):
            raise RuntimeError("HVCM3-8 stage or availability drift")
    ...
    if counts["no_quorum_decisions"] + counts["tie_decisions"] + counts["emitted_decisions"] != counts["union_decisions"]:
        raise RuntimeError("HVCM3-8 incidence accounting drift")
    if len(output) > 1 and not output["entry_time"].iloc[1:].reset_index(drop=True).ge(output["exit_time"].iloc[:-1].reset_index(drop=True)).all():
        raise RuntimeError("HVCM3-8 output overlap")
...
def run(clock_path: Path = CLOCK, result_path: Path = RESULT) -> dict[str, Any]:
    ...
    core = {"protocol_version": "hvcm3_8_source_support_v1", "policy_id": prereg.POLICY_ID, "preregistration": {"path": str(prereg.DEFAULT_OUTPUT), "sha256": PREREG_SHA, "manifest_hash": registration["manifest_hash"]}, "verified_component_artifacts": verified, "component_clock_fields_opened": list(FIELDS), "combined_incidence_opened": True, "combined_postentry_returns_or_pnl_opened": False, "entry_exit_prices_opened": False, "returns_opened": False, "funding_opened": False, "pnl_opened": False, "gross9_comparator_rows_opened": False, "clock": {"path": str(clock_path), "sha256": prereg.sha256_file(clock_path), "rows": len(clock)}, "three_action_majority_accounting": accounting, "support": stats, "support_checks": checks, "support_passed": passed, "advance_to_gross9_novelty": passed, "advance_to_economic_outcomes": False, "decision": "pass_to_gross9_novelty" if passed else "terminal_source_support_reject"}
...
if __name__ == "__main__":
    ...
    print(json.dumps({"events": {k: v["events"] for k, v in report["support"].items()}, "accounting": report["three_action_majority_accounting"], "support_passed": report["support_passed"]}, indent=2))
