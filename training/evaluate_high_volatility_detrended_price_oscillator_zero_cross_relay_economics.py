"""Sequential strict economics for frozen HVDPO-24."""
...
PREREG = Path("results/high_volatility_detrended_price_oscillator_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "ae93b4c8ae58d18e229b81792ec35ed287453b77b64969c6801205fb0a83520a"
SUPPORT = Path("results/high_volatility_detrended_price_oscillator_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "a8a95f6b74a0d16b5af33f50e15ef79bb5fe55eeed85252b609a6c38ccc11451"
NOVELTY = Path("results/high_volatility_detrended_price_oscillator_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "7420455dfb8db3c200ec8c6e185dca89e902d5c1b1134ea2366cbd17026379aa"
CLOCK = Path("data/high_volatility_detrended_price_oscillator_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "cbbeb3a716cf6e2e6b58bc058919cc241873dc5b8e68bc3dff00aea8237a9627"
CONTROL_DIR = Path("data/high_volatility_detrended_price_oscillator_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_detrended_price_oscillator_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_detrended_price_oscillator_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "current_close_sma_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDPO frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDPO novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdpo_24_sequential_economics_v1", "policy_id": POLICY_ID,
