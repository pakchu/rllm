"""Strict staged economics for the singleton HVSAUDCA-8 policy."""
...
GROSS9 = Path("results/high_volatility_spot_underwater_causal_sign_adapter_gross9_novelty_2026-08-18.json")
GROSS9_SHA = "b69f55a1a636fd1c0bd81c0411d4598fbbab0fcfe2a4decf5523d8b354c79195"
CLOCK = {
    "path": Path("data/high_volatility_spot_underwater_causal_sign_adapter_clocks_2023_2026.csv.gz"),
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_spot_underwater_causal_sign_adapter_{stage}_economics_2026-08-18.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVSAUDCA-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVSAUDCA-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVSAUDCA-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVSAUDCA-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVSAUDCA-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVSAUDCA-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVSAUDCA-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVSAUDCA-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVSAUDCA-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVSAUDCA-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVSAUDCA-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvsaudca_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
