"""Sequential strict economics for frozen GVRCR-6."""
...
PREREG = Path("results/gold_volatility_rotation_btc_confirmation_relay_preregistration_2026-08-08.json")
PREREG_SHA = "f03f362718290799fbb56256255d77fc967b49139d0ee8947edd7de10234ab5a"
SUPPORT = Path("results/gold_volatility_rotation_btc_confirmation_relay_support_2026-08-08.json")
SUPPORT_SHA = "4811e41caff144bb7738d36c6d579d04d03d22913ae869615ac198251ccf7fe1"
NOVELTY = Path("results/gold_volatility_rotation_btc_confirmation_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "dc62cd0c646ae12a14cec82f3b7899750b46e157518c5893c077e473c13230cb"
CLOCK = Path("data/gold_volatility_rotation_btc_confirmation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "3f43e59b96445bdb1ef0d497b467585fa281ccf362fedea4eca08f6650453ed8"
CONTROL_DIR = Path("data/gold_volatility_rotation_btc_confirmation_relay_controls_2023_2026")
FREEZE = Path("results/gold_volatility_rotation_btc_confirmation_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/gold_volatility_rotation_btc_confirmation_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("GVRCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("GVRCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "gvrcr_6_sequential_economics_v1", "policy_id": POLICY_ID,
