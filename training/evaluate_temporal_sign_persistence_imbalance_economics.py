"""Sequential strict economics for frozen TSPI-12."""
...
PREREG = Path("results/temporal_sign_persistence_imbalance_preregistration_2026-08-09.json")
PREREG_SHA = "2ed7738d0ed934d7e01a8d0bfd9b1f0c81b0bf75bcf43d3a8b49a7c294aaf633"
SUPPORT = Path("results/temporal_sign_persistence_imbalance_support_2026-08-09.json")
SUPPORT_SHA = "8a8777bd439df1476edf7f7bb85c8d58f22174ff7e7502d2dc8db503fd78e7c7"
NOVELTY = Path("results/temporal_sign_persistence_imbalance_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "85a7c1e3b1b6451fb0e448c4fff86067805ef81849c042dc603b1d32b9857ef7"
CLOCK = Path("data/temporal_sign_persistence_imbalance_clocks_2023_2026.csv.gz")
CLOCK_SHA = "9b126a3d22a5e68942f782e32049fac3e60544d8f50fa3044782c7e123a6f7e9"
CONTROL_DIR = Path("data/temporal_sign_persistence_imbalance_controls_2023_2026")
FREEZE = Path("results/temporal_sign_persistence_imbalance_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/temporal_sign_persistence_imbalance_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("TSPI frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("TSPI novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "tspi_12_sequential_economics_v1", "policy_id": POLICY_ID,
