"""Evaluate frozen HVCARP-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCARP-8"
PROTOCOL = "hvcarp_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_cross_alt_rank_persistence_relay_preregistration_2026-08-10.json")
PREREG_SHA = "40f8ce8063e3ff52caa0185a45afc0b272150827dd81181f08ff8579a8cb588a"
SUPPORT = Path("results/high_volatility_cross_alt_rank_persistence_relay_support_2026-08-10.json")
SUPPORT_SHA = "7bfeb147fb1b4706e68193857bb47cbcef43935b1060f0c612cfd1492a6ab6da"
CLOCK = Path("data/high_volatility_cross_alt_rank_persistence_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "463848bef69bbe5993128c2b575fa1704b7749ed775c0dee7cc136e50846611c"
CLOCK_ROWS = 819
OUTPUT = Path("results/high_volatility_cross_alt_rank_persistence_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def write_immutable(path: Path, content: bytes) -> None:
    ...
    if path.exists():
        if path.read_bytes() != content:
            raise RuntimeError(f"refusing to overwrite immutable HVCARP novelty result: {path}")
        return
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCARP predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCARP novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCARP predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCARP primary")
    if len(candidate) != CLOCK_ROWS:
        raise RuntimeError("HVCARP clock count drift")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcarp_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCARP-8 unchanged before economic outcomes",
    }
