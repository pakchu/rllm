"""Build outcome-blind source support for frozen HVPSO-Z14-3-24."""
...
from training import preregister_high_volatility_premier_stochastic_oscillator_zero_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "3230f687b66df53075ef44de3bd93d2d09c156e42a77c035584b759ecff5cb66"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_premier_stochastic_oscillator_zero_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_premier_stochastic_oscillator_zero_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_premier_stochastic_oscillator_zero_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_premier_stochastic_oscillator_zero_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_premier_stochastic_oscillator_zero_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "range_width", "fast_k", "normalized_stochastic",
    "first_smoothing", "double_smoothing", "pso", "entry_side", "raw_fast_k_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "range_width", "fast_k",
    "normalized_stochastic", "first_smoothing", "double_smoothing", "pso",
    "entry_side", "raw_fast_k_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVPSO source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVPSO invalid source key")
...
class LeanEma:
    """Minimal LEAN EMA: initial period SMA, then alpha=2/(period+1)."""
...
def premier_stochastic(high: pd.Series, low: pd.Series, close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    """Replicate LEAN PremierStochasticOscillator(14,3) across coherent bars."""
    highs, lows, closes = (series.to_numpy(float) for series in (high, low, close))
    validity = valid.to_numpy(bool)
    count = len(close)
    names = (
        "range_width", "fast_k", "normalized_stochastic", "first_smoothing",
        "double_smoothing", "pso",
    )
    values = {name: np.full(count, np.nan) for name in names}
    entry_side = np.zeros(count, dtype=int)
    raw_fast_k_side = np.zeros(count, dtype=int)
    first = LeanEma(P["ema_periods"])
    second = LeanEma(P["ema_periods"])
    high_window: list[float] = []
    low_window: list[float] = []

    for index in range(count):
        if not validity[index]:
            first.reset(); second.reset(); high_window.clear(); low_window.clear()
            continue
        high_window.append(highs[index]); low_window.append(lows[index])
        high_window = high_window[-P["stochastic_periods"] :]
        low_window = low_window[-P["stochastic_periods"] :]
        if len(high_window) < P["stochastic_periods"]:
            continue
        range_width = max(high_window) - min(low_window)
        fast_k = 0.0 if range_width == 0 else 100.0 * (closes[index] - min(low_window)) / range_width
        normalized = 0.1 * (fast_k - 50.0)
        values["range_width"][index] = range_width
        values["fast_k"][index] = fast_k
        values["normalized_stochastic"][index] = normalized
        prior_fast_ready = index > 0 and math.isfinite(values["fast_k"][index - 1])
        prior_above_mid = prior_fast_ready and values["fast_k"][index - 1] > 50
        prior_below_mid = prior_fast_ready and values["fast_k"][index - 1] < 50
        raw_fast_k_side[index] = 1 if fast_k > 50 and not prior_above_mid else -1 if fast_k < 50 and not prior_below_mid else 0
        first_value = first.update(normalized)
        if not math.isfinite(first_value):
            continue
        values["first_smoothing"][index] = first_value
        second_value = second.update(first_value)
        if not math.isfinite(second_value):
            continue
        values["double_smoothing"][index] = second_value
        exponential = math.exp(second_value)
        pso = (exponential - 1) / (exponential + 1)
        values["pso"][index] = pso
        prior_ready = index > 0 and math.isfinite(values["pso"][index - 1])
        prior_positive = prior_ready and values["pso"][index - 1] > 0
        prior_negative = prior_ready and values["pso"][index - 1] < 0
        entry_side[index] = 1 if pso > 0 and not prior_positive else -1 if pso < 0 and not prior_negative else 0
    return pd.DataFrame({**values, "entry_side": entry_side, "raw_fast_k_side": raw_fast_k_side}, index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=bars.join(premier_stochastic(bars.bar_high,bars.bar_low,bars.bar_close,bars.valid_bar));bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["range_width","fast_k","normalized_stochastic","first_smoothing","double_smoothing","pso","variation"]]).all(axis=1)&bars.range_width.gt(0)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="raw_fast_k_midline":side=pd.to_numeric(used.raw_fast_k_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVPSO prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvpso_z14_3_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvpso_z14_3_24_source_support_v1", "policy_id": prereg.POLICY_ID,
