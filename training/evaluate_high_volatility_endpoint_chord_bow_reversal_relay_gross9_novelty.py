"""Evaluate frozen HVECBR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVECBR-8"
PROTOCOL = "hvecbr_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_endpoint_chord_bow_reversal_relay_preregistration_2026-08-10.json")
PREREG_SHA = "db66c8b194c84aff82d595b494aed49711ab0178677e23cc6acbb0c1eac764b4"
SUPPORT = Path("results/high_volatility_endpoint_chord_bow_reversal_relay_support_2026-08-10.json")
SUPPORT_SHA = "2eebaf592a9b945542030db71b31f2612e65f21c42bea158548735c7cf3fd202"
CLOCK = Path("data/high_volatility_endpoint_chord_bow_reversal_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d2d7521d17612c0da73dcde9625f87ef14ffdbe4280822c1998e0db057ec6a75"
OUTPUT = Path("results/high_volatility_endpoint_chord_bow_reversal_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVECBR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVECBR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVECBR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVECBR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVECBR-8 unchanged before economic outcomes",
    }
