"""Sequential strict economics for frozen HVCFO-24."""
...
PREREG = Path("results/high_volatility_chande_forecast_oscillator_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "fd83ac6390a05a76ff1e57eb2fabfe09a62cffa409b49009704be67f6d64de53"
SUPPORT = Path("results/high_volatility_chande_forecast_oscillator_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "a0dd4ec3d98de620f4601f3ea2f98793a990c59f7cc4513c69506ff8d23cf822"
NOVELTY = Path("results/high_volatility_chande_forecast_oscillator_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "97cdaf0bf3827ac4ff533f1bcbc7a2a3ac5af35ef39a3daa344242e0f5017b00"
CLOCK = Path("data/high_volatility_chande_forecast_oscillator_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "6421c607cfe25384d8dc0e9a0676d7e5abaf7fc20eb65f70566143757cd271a9"
CONTROL_DIR = Path("data/high_volatility_chande_forecast_oscillator_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_chande_forecast_oscillator_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_chande_forecast_oscillator_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "raw_close_direction", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCFO frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCFO novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcfo_24_sequential_economics_v1", "policy_id": POLICY_ID,
