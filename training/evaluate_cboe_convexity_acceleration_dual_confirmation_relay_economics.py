"""Sequential strict economics for frozen CVDCMR-6."""
...
PREREG = Path("results/cboe_convexity_acceleration_dual_confirmation_relay_preregistration_2026-08-08.json")
PREREG_SHA = "c7ef66c651f1d627eaf7c2c66a3c6f4b1c45a85ad93bdc630ae6ffb9c6df3c24"
SUPPORT = Path("results/cboe_convexity_acceleration_dual_confirmation_relay_support_2026-08-08.json")
SUPPORT_SHA = "5b8a9c3f33d8c6419b8681ac4f87638efae91c31f2e321a5691441ec25df333f"
NOVELTY = Path("results/cboe_convexity_acceleration_dual_confirmation_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "da08d26ddfa52568ba52ed0b286e8724b2636330dbdc88f362bedb4e83d9402b"
CLOCK = Path("data/cboe_convexity_acceleration_dual_confirmation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "8fe7fcc0475b6fbaa35b8e398ba1a5ec6920bb41e3e2fbf3da0236f1f7242e60"
CONTROL_DIR = Path("data/cboe_convexity_acceleration_dual_confirmation_relay_controls_2023_2026")
FREEZE = Path("results/cboe_convexity_acceleration_dual_confirmation_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/cboe_convexity_acceleration_dual_confirmation_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CVDCMR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CVDCMR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "cvdcmr_6_sequential_economics_v1", "policy_id": POLICY_ID,
