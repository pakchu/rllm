"""Sequential strict economics for frozen HVALMA-24."""
...
PREREG = Path("results/high_volatility_arnaud_legoux_moving_average_turn_relay_preregistration_2026-08-11.json")
PREREG_SHA = "b2a16f4d6e21388d0b77d5605870eb2ceed11c32ffccc2b5d83c288d87280188"
SUPPORT = Path("results/high_volatility_arnaud_legoux_moving_average_turn_relay_support_2026-08-11.json")
SUPPORT_SHA = "026e7fc8945378ccf6871bf366231385ad6627d0a239977aa0ec7bf2c7a73674"
NOVELTY = Path("results/high_volatility_arnaud_legoux_moving_average_turn_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "f8536d5b56ff3d7f3f6b371d924cdc1dd62edf4738cb064bc793ffec4b78e9f9"
CLOCK = Path("data/high_volatility_arnaud_legoux_moving_average_turn_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "7000a9b26aed36b74da042041244a4a0b6bf5879381425c1ba8193436ccabc39"
CONTROL_DIR = Path("data/high_volatility_arnaud_legoux_moving_average_turn_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_arnaud_legoux_moving_average_turn_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_arnaud_legoux_moving_average_turn_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "sma_9_turn", "one_bar_stale_turn", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVALMA frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVALMA novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvalma_24_sequential_economics_v1", "policy_id": POLICY_ID,
