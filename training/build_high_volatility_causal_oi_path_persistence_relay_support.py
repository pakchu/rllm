"""Deterministic outcome-blind source support for HVCOPR-24."""
...
from training import preregister_high_volatility_causal_oi_path_persistence_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "4170f3657b23ced4a20ed330efcaa0b1803c006299ce6a8b8eb40dd36a373c55"
...
BAR_QUERY = """SELECT date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '8 hours' AS decision_time,sum(ln(close/open)) FILTER (WHERE ts<date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '4 hours') AS first_half_return,sum(ln(close/open)) FILTER (WHERE ts>=date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '4 hours') AS second_half_return,sum(ln(close/open)) AS completed_return,sum(abs(ln(close/open))) AS absolute_path_return,sum(power(ln(close/open),2)) AS minute_squared_return,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND high>=greatest(open,close,low) AND low<=least(open,close,high)) AS coherent FROM bars_binance WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end GROUP BY 1 ORDER BY 1"""

ROOT = Path("data/high_volatility_causal_oi_path_persistence_relay_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_oi_path_persistence_relay_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_oi_path_persistence_relay_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_causal_oi_path_persistence_relay_support_2026-08-16.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "sum_open_interest",
    "previous_open_interest", "oi_change", "first_half_return", "second_half_return",
    "completed_return", "path_efficiency", "efficiency_rank", "realized_variation",
    "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "decision_time", "feature_available_time",
    "entry_time", "exit_time", "side", "oi_change", "completed_return",
    "path_efficiency", "efficiency_rank", "realized_variation", "variation_rank",
)
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVCOPR-24 OI schema drift")
    expected_bars = [
        "decision_time", "first_half_return", "second_half_return", "completed_return",
        "absolute_path_return", "minute_squared_return",
        "source_rows", "distinct_rows", "first_ts", "last_ts", "coherent",
    ]
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCOPR-24 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all():
        raise RuntimeError("HVCOPR-24 invalid OI rows")

    bars = bars.copy()
    for column in ("decision_time", "first_ts", "last_ts"):
        bars[column] = pd.to_datetime(bars[column], utc=True, errors="raise")
    numeric = ("first_half_return", "second_half_return", "completed_return", "absolute_path_return", "minute_squared_return", "source_rows", "distinct_rows")
    for column in numeric:
        bars[column] = pd.to_numeric(bars[column], errors="raise")
    if bars["decision_time"].duplicated().any():
        raise RuntimeError("HVCOPR-24 duplicate eight-hour bar")
    start = bars["decision_time"] - pd.Timedelta("8h")
    bars["bar_valid"] = (
        np.isfinite(bars[list(numeric)]).all(axis=1)
        & bars["absolute_path_return"].gt(0)
        & bars["completed_return"].abs().le(bars["absolute_path_return"] + 1e-12)
        & np.isclose(
            bars["completed_return"],
            bars["first_half_return"] + bars["second_half_return"],
            rtol=0,
            atol=1e-12,
        )
    ...
    bars["path_efficiency"] = bars["completed_return"].abs() / bars["absolute_path_return"]
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel = panel.join(
        bars[["bar_valid", "first_half_return", "second_half_return", "completed_return", "path_efficiency", "realized_variation"]],
        on="decision_time",
    )
    panel["source_valid"] = (
        ...
        & np.isfinite(
            panel[["sum_open_interest", "previous_open_interest", "oi_change", "first_half_return", "second_half_return", "completed_return", "path_efficiency", "realized_variation"]]
        ).all(axis=1)
        & panel["sum_open_interest"].gt(0)
        & panel["previous_open_interest"].gt(0)
        & panel["realized_variation"].gt(0)
        & panel["path_efficiency"].gt(0)
    )
    valid = panel["source_valid"].eq(True)
    panel["efficiency_rank"] = causal_midrank(panel["path_efficiency"].where(valid))
    panel["variation_rank"] = causal_midrank(panel["realized_variation"].where(valid))
    panel["eligible"] = (
        valid
        & panel["first_half_return"].ne(0)
        & panel["second_half_return"].ne(0)
        & np.sign(panel["first_half_return"]).eq(np.sign(panel["second_half_return"]))
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCOPR-24 side or availability drift")
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCOPR-24 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcopr_24_sources_v1",
    ...
    core = {
        "protocol_version": "hvcopr_24_source_support_v1", "policy_id": prereg.POLICY_ID,
