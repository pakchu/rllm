"""Evaluate frozen CVRVPR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "CVRVPR-12"
PROTOCOL = "cvrvpr_12_gross9_novelty_v1"
...
OUTPUT = Path("results/cross_venue_relative_volatility_premium_regime_crossing_relay_gross9_novelty_2026-08-08.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha256(PREREG) != PREREG_SHA or sha256(SUPPORT) != SUPPORT_SHA or sha256(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CVRVPR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("CVRVPR novelty limits drift")
    if (
        ...
        raise RuntimeError("CVRVPR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CVRVPR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "cvrvpr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject CVRVPR-12 unchanged before economic outcomes",
    }
