"""Evaluate frozen HVWTO-C10-21-4-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVWTO-C10-21-4-24"
PROTOCOL = "hvwto_c10_21_4_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_wave_trend_oscillator_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVWTO predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVWTO novelty limits drift")
    if (
        ...
        raise RuntimeError("HVWTO predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVWTO primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvwto_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVWTO-C10-21-4-24 unchanged before economic outcomes",
    }
