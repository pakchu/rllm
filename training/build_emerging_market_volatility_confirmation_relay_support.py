"""Materialize outcome-blind source support for frozen EMVCR-8."""
...
BUILDER = Path("training/build_emerging_market_volatility_confirmation_relay_support.py")
PREREG_SHA = "79de01d0042faddf83c5786a981860e8b91d4d2b364699dec2412bdde5b60086"
SOURCE_DIR = Path("data/emerging_market_volatility_confirmation_relay_sources_2022_2026")
CBOE_PANEL = SOURCE_DIR / "cboe_vxeem_vix_panel.csv.gz"
FEATURE_PANEL = SOURCE_DIR / "emvcr_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/emerging_market_volatility_confirmation_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/emerging_market_volatility_confirmation_relay_controls_2023_2026")
RESULT = Path("results/emerging_market_volatility_confirmation_relay_support_2026-08-09.json")
...
CONTROLS = ("no_btc_confirmation", "no_btc_volatility_gate", "vxeem_change_only", "one_session_stale_spread", "direction_flip")
CLOCK_COLUMNS = (
    ...
    "relative_volatility_shock", "shock_z", "btc_overnight_return", "btc_realized_variation", "btc_variation_rank",
)
QUERY = """
...
def _download_cboe(symbol: str) -> pd.DataFrame:
    ...
    if frame.empty or frame.observation_date.duplicated().any() or not frame.observation_date.is_monotonic_increasing:
        raise RuntimeError(f"EMVCR {symbol} dates invalid")
    if not np.isfinite(frame[f"{symbol}_close"]).all() or not frame[f"{symbol}_close"].gt(0).all():
        raise RuntimeError(f"EMVCR {symbol} values invalid")
    return frame


def load_cboe() -> pd.DataFrame:
    vxeem = _download_cboe("VXEEM")
    vix = _download_cboe("VIX")
    frame = vxeem.merge(vix, on="observation_date", validate="one_to_one").sort_values("observation_date").reset_index(drop=True)
    frame["relative_volatility"] = np.log(frame.VXEEM_close / frame.VIX_close)
    frame["relative_volatility_shock"] = frame.relative_volatility.diff()
    frame["vxeem_change"] = np.log(frame.VXEEM_close).diff()
    frame["shock_z"] = causal_z(frame.relative_volatility_shock)
    frame["vxeem_change_z"] = causal_z(frame.vxeem_change)
    return frame
...
def load_bars() -> pd.DataFrame:
    ...
    if frame.columns.tolist() != required:
        raise RuntimeError(f"EMVCR BTC schema must be exactly {required}")
    ...
    if len(frame) != len(expected) or not frame.ts.equals(pd.Series(expected, name="ts")):
        raise RuntimeError("EMVCR BTC source is not the exact requested 1m grid")
    for column in ("open", "close"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    if not np.isfinite(frame[["open", "close"]]).all(axis=None) or not frame[["open", "close"]].gt(0).all(axis=None):
        raise RuntimeError("EMVCR BTC source contains invalid prices")
...
def build_features(cboe: pd.DataFrame, bars: pd.DataFrame) -> pd.DataFrame:
    ...
    for index in range(1, len(cboe)):
        ...
        overnight_start = pd.Timestamp(cboe.at[index - 1, "observation_date"]).tz_localize(NY) + pd.Timedelta(hours=16)
        overnight_start = overnight_start.tz_convert("UTC")
        ...
        if overnight_start not in bars.index or decision - pd.Timedelta(minutes=1) not in bars.index:
            overnight = np.nan
        else:
            overnight = float(np.log(bars.at[decision - pd.Timedelta(minutes=1), "close"] / bars.at[overnight_start, "open"]))
        ...
        records.append({
            ...
            "vxeem_change": cboe.at[index - 1, "vxeem_change"],
            "vxeem_change_z": cboe.at[index - 1, "vxeem_change_z"],
            "btc_overnight_return": overnight,
...
def signal(features: pd.DataFrame, control: str) -> pd.Series:
    ...
    confirmation = np.sign(features.btc_overnight_return).eq(-np.sign(shock)) & features.btc_overnight_return.ne(0)
    eligible = shock.ne(0) & features.shock_z.abs().ge(0.75) & features.btc_variation_rank.ge(0.65) & confirmation
    side = -np.sign(shock).astype("Int64").fillna(0).astype(int)
    if control == "no_btc_confirmation":
        eligible = shock.ne(0) & features.shock_z.abs().ge(0.75) & features.btc_variation_rank.ge(0.65)
    elif control == "no_btc_volatility_gate":
        eligible = shock.ne(0) & features.shock_z.abs().ge(0.75) & confirmation
    elif control == "vxeem_change_only":
        vxeem_confirmation = np.sign(features.btc_overnight_return).eq(-np.sign(features.vxeem_change)) & features.btc_overnight_return.ne(0)
        eligible = features.vxeem_change.ne(0) & features.vxeem_change_z.abs().ge(0.75) & features.btc_variation_rank.ge(0.65) & vxeem_confirmation
        side = -np.sign(features.vxeem_change).astype("Int64").fillna(0).astype(int)
    elif control == "one_session_stale_spread":
        stale_shock, stale_z = shock.shift(1), features.shock_z.shift(1)
        stale_confirmation = np.sign(features.btc_overnight_return).eq(-np.sign(stale_shock)) & features.btc_overnight_return.ne(0)
        eligible = stale_shock.ne(0) & stale_z.abs().ge(0.75) & features.btc_variation_rank.ge(0.65) & stale_confirmation
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[sides.ne(0)]:
        ...
        rows.append({
            "candidate": "EMVCR-8", "control": control, "split": split,
            ...
            "shock_z": float(features.at[index, "shock_z"]), "btc_overnight_return": float(features.at[index, "btc_overnight_return"]), "btc_realized_variation": float(features.at[index, "btc_realized_variation"]),
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("EMVCR preregistration hash drift")
    ...
    source_core = {"protocol_version": "emvcr_8_sources_v1", "cboe_urls": prereg.SOURCES, "cboe_window": [str(CBOE_START.date()), str(CBOE_END.date())], "btc_query": QUERY, "btc_window": [SOURCE_START.isoformat(), SOURCE_END.isoformat()], "btc_rows": len(bars), "builder": {"path": str(BUILDER), "sha256": sha(BUILDER)}, "outputs": {"cboe": {"path": str(CBOE_PANEL), "sha256": sha(CBOE_PANEL), "rows": len(cboe)}, "features": {"path": str(FEATURE_PANEL), "sha256": sha(FEATURE_PANEL), "rows": len(features)}}, "candidate_outcomes_opened": False, "no_imputation": True}
