"""Sequential strict economics for frozen HVCARER-12."""
...
PREREG = Path("results/high_volatility_cross_alt_range_expansion_confirmation_relay_preregistration_2026-08-11.json")
PREREG_SHA = "b2ac6ac2092a9f23c69add331d6a49fec4ca2641f9adc9d220f51c3f4dcf9c7f"
SUPPORT = Path("results/high_volatility_cross_alt_range_expansion_confirmation_relay_support_2026-08-11.json")
SUPPORT_SHA = "81311ea40401c5625e09428ad57894953cd6dcae527d07cdac766324c41df8af"
NOVELTY = Path("results/high_volatility_cross_alt_range_expansion_confirmation_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "ef36829d249400c52570123e5fe7e078276ecee3e89983b051640341e63ef8a3"
CLOCK = Path("data/high_volatility_cross_alt_range_expansion_confirmation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "23021fe241d5735c1d56db466f5c692ad5098d8738887d3d89934fd092e1eb38"
CONTROL_DIR = Path("data/high_volatility_cross_alt_range_expansion_confirmation_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_cross_alt_range_expansion_confirmation_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_alt_range_expansion_confirmation_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_btc_variation_gate", "three_of_six_breadth", "median_alt_range_level", "one_day_stale_breadth", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCARER frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCARER novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcarer_12_sequential_economics_v1", "policy_id": POLICY_ID,
