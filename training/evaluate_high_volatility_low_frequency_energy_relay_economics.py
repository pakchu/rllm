"""Sequential strict economics for frozen HVLFE-8."""
...
PREREG = Path("results/high_volatility_low_frequency_energy_relay_preregistration_2026-08-10.json")
PREREG_SHA = "a4a6c0d2e0e6196aeda54536008372f322459a1aa147b947b3850973fac7e5cf"
SUPPORT = Path("results/high_volatility_low_frequency_energy_relay_support_2026-08-10.json")
SUPPORT_SHA = "894ea2be790b8bfb0f81785b78b29e7a9bc460c930ccb8b40c8ff11dbf1f0853"
NOVELTY = Path("results/high_volatility_low_frequency_energy_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "ee1333bb5a2f7a19a908d15ec1fd61f2ddd6b375c1b6ed3bc59fab6fa010e625"
CLOCK = Path("data/high_volatility_low_frequency_energy_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "c881f8e33f88ec551e429a0c4d085270df34a1a85f0843fd3ef2b14651dae4fa"
CONTROL_DIR = Path("data/high_volatility_low_frequency_energy_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_low_frequency_energy_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_low_frequency_energy_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_low_frequency_tail", "no_variation_gate", "raw_low_frequency_share_nonnegative", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVLFE frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVLFE novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvlfe_8_sequential_economics_v1", "policy_id": POLICY_ID,
