"""Evaluate frozen HVBDS-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVBDS-8"
PROTOCOL = "hvbds_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_bds_nonlinear_dependence_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVBDS predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVBDS novelty limits drift")
    if (
        ...
        raise RuntimeError("HVBDS predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVBDS primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVBDS-8 unchanged before economic outcomes",
    }
