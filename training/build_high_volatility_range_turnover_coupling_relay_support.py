"""Build outcome-blind source support for frozen HVRTCR-8."""
...
from training import preregister_high_volatility_range_turnover_coupling_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-04-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="34641678970a01dd162439ab780e0ed1c32f951d671eef0212bbbcd4f2f57894"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_range_turnover_coupling_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_range_turnover_coupling_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_range_turnover_coupling_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_range_turnover_coupling_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_range_turnover_coupling_relay_support_2026-08-10.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","range_turnover_coupling","coupling_rank","range_only_direction","realized_variation","variation_rank","completed_return","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","range_turnover_coupling","coupling_rank","range_only_direction","realized_variation","variation_rank","completed_return")
...
def coupling_statistics(block:pd.DataFrame)->tuple[float,float,float,float]:
 groups=np.arange(len(block))//5
 five=block.groupby(groups,sort=True).agg(open=("open","first"),high=("high","max"),low=("low","min"),close=("close","last"),quote=("quote_asset_volume","sum"))
 if len(five)!=96 or not np.isfinite(five[["open","high","low","close","quote"]]).all().all() or not five[["open","high","low","close"]].gt(0).all().all() or not five.quote.ge(0).all():return (math.nan,)*4
 turnover=np.log1p(five.quote.to_numpy(float));returns=np.log(five.close.to_numpy(float)/five.open.to_numpy(float));ranges=np.square(np.log(five.high.to_numpy(float)/five.low.to_numpy(float)))/(4*math.log(2))
 if float(np.std(turnover,ddof=1))<=0 or float(np.std(ranges,ddof=1))<=0:return (math.nan,)*4
 coupling=float(np.corrcoef(ranges,turnover)[0,1]);range_direction=float(np.sum(ranges*np.sign(returns)));variation=float(np.sqrt(np.square(returns).sum()));completed=float(math.log(block.close.iloc[-1]/block.open.iloc[0]))
 return coupling,range_direction,variation,completed
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close","quote_asset_volume"]:raise RuntimeError("HVRTCR source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVRTCR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("12h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   coupling,range_direction,var,ret=coupling_statistics(block);valid=all(math.isfinite(v) for v in (coupling,range_direction,var,ret)) and var>0 and coupling>0 and ret!=0
  else:coupling=range_direction=var=ret=math.nan;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"range_turnover_coupling":coupling,"range_only_direction":range_direction,"realized_variation":var,"completed_return":ret})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["coupling_rank"]=prior_rank(panel.range_turnover_coupling.where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));panel["eligible"]=valid&panel.coupling_rank.ge(P["coupling_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"]);panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_boundary_stale_coupling":
  cols=["source_valid","range_turnover_coupling","coupling_rank","variation_rank","completed_return","range_only_direction","feature_available_time"];used[cols]=panel[cols].shift(1)
 valid=used.source_valid.eq(True);strong=used.coupling_rank.ge(P["coupling_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&strong&variation
 if control=="no_coupling_tail":state=valid&variation
 ...
 if control=="range_only_direction":side=np.sign(pd.to_numeric(used.range_only_direction,errors="coerce").fillna(0)).astype(int)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVRTCR prereg drift")
 ...
 source_core={"protocol_version":"hvrtcr_8_source_v1","query":QUERY,"query_sha256":hashlib.sha256(QUERY.encode()).hexdigest(),"window":[START.isoformat(),END.isoformat()],"physical_rows":len(raw),"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"panel":{"path":str(PANEL),"sha256":sha(PANEL),"rows":len(panel),"valid_rows":int(panel.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};manifest={**source_core,"manifest_hash":chash(source_core)};immutable(MANIFEST,json_bytes(manifest))
