"""Build outcome-blind source support for frozen HVCE-24."""
...
from training import preregister_high_volatility_chandelier_exit_state_flip_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "0e429bb9a97bd213d519b28be9faf8fdac73a7b5f41dd97860a21a5323e3abd4"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_chandelier_exit_state_flip_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_chandelier_exit_state_flip_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_chandelier_exit_state_flip_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_chandelier_exit_state_flip_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_chandelier_exit_state_flip_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "true_range", "atr", "raw_long_stop", "raw_short_stop",
    "active_stop", "state", "state_change", "raw_signal", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "true_range", "atr", "raw_long_stop", "raw_short_stop",
    "active_stop", "state", "state_change", "raw_signal", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVCE source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVCE invalid source key")
...
def chandelier(high: pd.Series, low: pd.Series, close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    n=len(close);period=P["atr_periods"];lookback=P["lookback_periods"];multiplier=P["atr_multiplier"]
    tr=np.full(n,np.nan);atr=np.full(n,np.nan);raw_long=np.full(n,np.nan);raw_short=np.full(n,np.nan);active=np.full(n,np.nan);state=np.full(n,np.nan);change=np.zeros(n,dtype=bool);raw_signal=np.zeros(n,dtype=int)
    highs=high.to_numpy(float);lows=low.to_numpy(float);closes=close.to_numpy(float);validity=valid.to_numpy(bool);previous_atr=np.nan;history=[];previous_state=np.nan;previous_stop=np.nan
    for i in range(n):
        if i==0 or not validity[i] or not validity[i-1]:previous_atr=np.nan;history=[];previous_state=previous_stop=np.nan;continue
        ...
        atr[i]=previous_atr;left=i-lookback+1
        if left<0 or not validity[left:i+1].all():previous_state=previous_stop=np.nan;continue
        raw_long[i]=np.max(highs[left:i+1])-multiplier*previous_atr;raw_short[i]=np.min(lows[left:i+1])+multiplier*previous_atr
        raw_signal[i]=1 if closes[i]>raw_short[i] else -1 if closes[i]<raw_long[i] else 0
        if not math.isfinite(previous_state):state[i]=P["initial_state"];active[i]=raw_short[i] if state[i]==-1 else raw_long[i]
        elif int(previous_state)==-1:
            stop=min(previous_stop,raw_short[i])
            if closes[i]>stop:state[i]=1;active[i]=raw_long[i];change[i]=True
            else:state[i]=-1;active[i]=stop
        else:
            stop=max(previous_stop,raw_long[i])
            if closes[i]<stop:state[i]=-1;active[i]=raw_short[i];change[i]=True
            else:state[i]=1;active[i]=stop
        previous_state=state[i];previous_stop=active[i]
    return pd.DataFrame({"true_range":tr,"atr":atr,"raw_long_stop":raw_long,"raw_short_stop":raw_short,"active_stop":active,"state":state,"state_change":change,"raw_signal":raw_signal},index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=bars.join(chandelier(bars.bar_high,bars.bar_low,bars.bar_close,bars.valid_bar));bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["true_range","atr","raw_long_stop","raw_short_stop","active_stop","state","variation"]]).all(axis=1)&bars.state.ne(0)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));panel["eligible"]=panel.source_valid&panel.state_change&panel.variation_rank.ge(P["variation_rank_min"])
    return panel.loc[:,PANEL_COLUMNS]

def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True);variation=used.variation_rank.ge(P["variation_rank_min"]);side=pd.to_numeric(used.state,errors="coerce").fillna(0).astype(int);state=valid&used.state_change.eq(True)&variation
    if control=="no_variation_gate":state=valid&used.state_change.eq(True)
    elif control=="untrailed_raw_stop_flip":side=pd.to_numeric(used.raw_signal,errors="coerce").fillna(0).astype(int);previous=side.shift(1);state=valid&side.ne(0)&previous.ne(0)&side.ne(previous)&variation
    elif control=="one_bar_stale_flip":state=state.shift(1,fill_value=False);side=side.shift(1,fill_value=0)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCE prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvce_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvce_24_source_support_v1", "policy_id": prereg.POLICY_ID,
