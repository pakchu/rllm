"""Evaluate frozen RIVSCR-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "RIVSCR-6"
PROTOCOL = "rivscr_6_gross9_novelty_v1"
...
OUTPUT = Path("results/realized_over_implied_volatility_shock_continuation_relay_gross9_novelty_2026-08-08.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha256(PREREG) != PREREG_SHA or sha256(SUPPORT) != SUPPORT_SHA or sha256(CLOCK) != CLOCK_SHA:
        raise RuntimeError("RIVSCR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("RIVSCR novelty limits drift")
    if (
        ...
        raise RuntimeError("RIVSCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="RIVSCR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "rivscr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject RIVSCR-6 unchanged before economic outcomes",
    }
