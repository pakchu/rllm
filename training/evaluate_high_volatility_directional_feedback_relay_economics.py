"""Sequential strict economics for frozen HVDFR-8."""
...
PREREG = Path("results/high_volatility_directional_feedback_relay_preregistration_2026-08-10.json")
PREREG_SHA = "111da91ab5e052ebb9eb10288ea94fde13e926ad2158eeaaf07d4a4a197bb361"
SUPPORT = Path("results/high_volatility_directional_feedback_relay_support_2026-08-10.json")
SUPPORT_SHA = "3a4f33fcd195cb022489e67f9151ecd0e81ffe71b420c9c5cf19764619ee02d6"
NOVELTY = Path("results/high_volatility_directional_feedback_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "f6663d52219e549fcb488bed7411e8eee5d5e2c2bded3d8c0e62af3dcc8c3bee"
CLOCK = Path("data/high_volatility_directional_feedback_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d164e83ec1bfed39aa3665e897f99009c720a5c17ca093da55baaaa8235959a2"
CONTROL_DIR = Path("data/high_volatility_directional_feedback_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_directional_feedback_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_directional_feedback_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_feedback_tail", "no_variation_gate", "raw_absolute_feedback_above_tenth", "one_block_stale_feedback", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDFR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDFR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdfr_8_sequential_economics_v1", "policy_id": POLICY_ID,
