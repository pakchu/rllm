"""Build source-support clocks for CCVDAR-6 without BTC outcomes."""
...
CLOCK=Path("data/cboe_crypto_volatility_disagreement_absorption_relay_clocks_2023_2026.csv.gz");CONTROL_DIR=Path("data/cboe_crypto_volatility_disagreement_absorption_relay_controls_2023_2026");RESULT=Path("results/cboe_crypto_volatility_disagreement_absorption_relay_support_2026-08-08.json")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00:00Z"),pd.Timestamp("2024-01-01T00:00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00:00Z"),pd.Timestamp("2025-01-01T00:00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00:00Z"),pd.Timestamp("2026-01-01T00:00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00:00Z"),pd.Timestamp("2026-08-01T00:00:00Z"))};MIN={"train":8,"test":12,"eval":12,"final":8};CONTROLS=("no_crypto_disagreement","bvol_only_disagreement","dvol_only_disagreement","one_session_stale_cboe_change","direction_flip");NY=ZoneInfo("America/New_York");ECONOMIC_OUTCOMES_AUTHORIZED=False
COLUMNS=("candidate","control","split","cboe_observation_date","next_cboe_source_date","decision_time","feature_available_time","entry_time","exit_time","side","delta_front_slope","previous_delta_front_slope","bvol_body","dvol_body")
...
def features()->pd.DataFrame:
 for raw,want in prereg.HASHES.items():
  if sha(Path(raw))!=want:raise RuntimeError(f"CCVDAR source drift: {raw}")
 c=pd.read_csv(prereg.CBOE,compression="gzip",usecols=["observation_date","VIX9D_close","VIX_close"]);c.observation_date=pd.to_datetime(c.observation_date,format="%Y-%m-%d");c.VIX9D_close=pd.to_numeric(c.VIX9D_close,errors="coerce");c.VIX_close=pd.to_numeric(c.VIX_close,errors="coerce")
 if c.observation_date.duplicated().any() or not c.observation_date.is_monotonic_increasing or not np.isfinite(c[["VIX9D_close","VIX_close"]]).all().all() or not c[["VIX9D_close","VIX_close"]].gt(0).all().all():raise RuntimeError("CCVDAR Cboe source invalid")
 c["delta_front_slope"]=np.log(c.VIX9D_close/c.VIX_close).diff();c["previous_delta_front_slope"]=c.delta_front_slope.shift(1)
 ...
 for i in range(1,len(c)-1):
  ...
  x=v.loc[decision];rows.append({"observation_date":c.at[i,"observation_date"].date(),"next_source_date":nd.date(),"decision_time":decision,"delta_front_slope":float(c.at[i,"delta_front_slope"]),"previous_delta_front_slope":float(c.at[i,"previous_delta_front_slope"]),"bvol_body":float(x.bvol_body),"dvol_body":float(x.dvol_body),"valid":bool(x.bvol_valid and x.dvol_valid)})
 return pd.DataFrame(rows)
def sides(f:pd.DataFrame,control:str)->pd.Series:
 delta=f.previous_delta_front_slope if control=="one_session_stale_cboe_change" else f.delta_front_slope;valid=f.valid&delta.notna()&delta.ne(0)&f.bvol_body.ne(0)&f.dvol_body.ne(0);opp_b=np.sign(f.bvol_body).eq(-np.sign(delta));opp_d=np.sign(f.dvol_body).eq(-np.sign(delta))
 if control=="no_crypto_disagreement":eligible=valid
 elif control=="bvol_only_disagreement":eligible=valid&opp_b
 elif control=="dvol_only_disagreement":eligible=valid&opp_d
 else:eligible=valid&opp_b&opp_d
...
def clock(f:pd.DataFrame,control:str="primary")->pd.DataFrame:
 ...
 for i in f.index[s.ne(0)]:
  ...
  next_allowed=exit_;rows.append({"candidate":"CCVDAR-6","control":control,"split":split,"cboe_observation_date":f.at[i,"observation_date"].isoformat(),"next_cboe_source_date":f.at[i,"next_source_date"].isoformat(),"decision_time":decision,"feature_available_time":decision,"entry_time":entry,"exit_time":exit_,"side":int(s.at[i]),"delta_front_slope":float(f.at[i,"delta_front_slope"]),"previous_delta_front_slope":float(f.at[i,"previous_delta_front_slope"]),"bvol_body":float(f.at[i,"bvol_body"]),"dvol_body":float(f.at[i,"dvol_body"])})
