"""Evaluate the frozen HVSPOICR-8 action-vote clock against Gross9."""
...
from training import preregister_high_volatility_spot_perpetual_open_interest_contraction_reversal as prereg

POLICY = "HVSPOICR-8"
PROTOCOL = "hvspoicr_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_spot_perpetual_open_interest_contraction_reversal_clocks_2020_2026.csv.gz",
    "sha256": "0c67b3d919b7abbe3e1b34003aeab784b98c65682ca08a3e912a93d6bcf3ca07",
    "rows": 328,
}
OUTPUT = Path("results/high_volatility_spot_perpetual_open_interest_contraction_reversal_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVSPOICR-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVSPOICR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVSPOICR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVSPOICR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVSPOICR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVSPOICR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVSPOICR-8 unchanged before economics",
    }
