"""Evaluate the frozen HVCATLIR-8 action-vote clock against Gross9."""
...
POLICY = "HVCATLIR-8"
PROTOCOL = "hvcatlir_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_average_ticket_late_confirmation_inventory_relay_clocks_2020_2026.csv.gz",
    "sha256": "dbe8e5441049cd3d5b3e03cb852c34273176ce21530f5be99d8235fcc39f3d0d",
    "rows": 115,
}
OUTPUT = Path("results/high_volatility_causal_average_ticket_late_confirmation_inventory_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCATLIR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCATLIR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATLIR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCATLIR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCATLIR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCATLIR-8 unchanged before economics",
    }
