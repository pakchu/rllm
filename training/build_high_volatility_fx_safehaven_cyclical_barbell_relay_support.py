"""Materialize source-only HVFXBR-8 support clocks."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_fx_safehaven_cyclical_barbell_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv

ENV_FILE="/home/pakchu/rllm/.env"; PREREG_SHA="574cb9aae93689145fb03ae00a73ee47f89e473dae0877191a4a2c79dba866ab"; START=pd.Timestamp("2023-01-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
SOURCE_DIR=Path("data/high_volatility_fx_safehaven_cyclical_barbell_relay_sources_2023_2026"); SESSION=SOURCE_DIR/"fx_barbell_sessions.csv.gz"; SOURCE_MANIFEST=SOURCE_DIR/"manifest.json"
...
CLOCK=Path("data/high_volatility_fx_safehaven_cyclical_barbell_relay_clocks_2023_2026.csv.gz"); CONTROL_DIR=Path("data/high_volatility_fx_safehaven_cyclical_barbell_relay_controls_2023_2026"); RESULT=Path("results/high_volatility_fx_safehaven_cyclical_barbell_relay_support_2026-08-13.json")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00:00Z"),pd.Timestamp("2024-01-01T00:00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00:00Z"),pd.Timestamp("2025-01-01T00:00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00:00Z"),pd.Timestamp("2026-01-01T00:00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00:00Z"),pd.Timestamp("2026-08-01T00:00:00Z"))}; MINIMUM={"train":8,"test":12,"eval":12,"final":8}; CONTROLS=("no_variation_gate","two_of_four_barbell","common_dollar_direction","one_session_stale_barbell","direction_flip","forced_long")
SYMBOLS=("USDJPY","USDCHF","USDMXN","USDAUD"); RISK_MULTIPLIER={"USDJPY":1.,"USDCHF":1.,"USDMXN":-1.,"USDAUD":-1.}
COLUMNS=("candidate","control","split","source_day","decision_time","feature_available_time","entry_time","exit_time","side","common_risk_direction","agreeing_pairs","median_absolute_pair_z","btc_realized_variation","btc_realized_variation_rank")
...
def materialize_sessions()->dict:
 ...
 with db.connect() as connection:
  for symbol in SYMBOLS:
   ...
   valid=raw.distinct_timestamps.ge(330)&raw.first_ts.le(raw.source_day+pd.Timedelta(hours=7,minutes=5))&raw.last_ts.ge(raw.source_day+pd.Timedelta(hours=12,minutes=55))&np.isfinite(raw[["session_open","session_high","session_low","session_close"]]).all(axis=1)&raw[["session_open","session_high","session_low","session_close"]].gt(0).all(axis=1)&raw.session_high.ge(raw[["session_open","session_close"]].max(axis=1))&raw.session_low.le(raw[["session_open","session_close"]].min(axis=1))
   canonical=(RISK_MULTIPLIER[symbol]*np.log(raw.session_close/raw.session_open)).where(valid);piece=pd.DataFrame({"source_day":raw.source_day,f"{symbol}_valid":valid,f"{symbol}_risk_return":canonical,f"{symbol}_z":causal_z(canonical)});frames.append(piece)
 db.dispose();frame=frames[0]
 for piece in frames[1:]:frame=frame.merge(piece,on="source_day",how="outer",validate="one_to_one")
 frame=frame.sort_values("source_day").reset_index(drop=True);valid_columns=[f"{symbol}_valid" for symbol in SYMBOLS];z_columns=[f"{symbol}_z" for symbol in SYMBOLS];frame["source_valid"]=frame[valid_columns].fillna(False).all(axis=1);z=frame[z_columns].astype(float);qual=z.abs().ge(.5);positive=(z.gt(0)&qual).sum(axis=1);negative=(z.lt(0)&qual).sum(axis=1);frame["positive_qualifying_pairs"]=positive;frame["negative_qualifying_pairs"]=negative;frame["common_risk_direction"]=np.where((positive.ge(3)&negative.eq(0)),1,np.where((negative.ge(3)&positive.eq(0)),-1,0));frame["agreeing_pairs"]=np.maximum(positive,negative);frame["median_absolute_pair_z"]=z.abs().median(axis=1);raw_returns=pd.DataFrame({symbol:RISK_MULTIPLIER[symbol]*z[f"{symbol}_z"] for symbol in SYMBOLS});dpos=raw_returns.gt(0).sum(axis=1);dneg=raw_returns.lt(0).sum(axis=1);frame["common_dollar_side"]=np.where(dpos.ge(3),1,np.where(dneg.ge(3),-1,0));frame["decision_time"]=frame.source_day+pd.Timedelta(hours=13)
 SOURCE_DIR.mkdir(parents=True,exist_ok=True);_write_gzip_csv(frame,SESSION);core={"protocol_version":"hvfxbr_8_fx_barbell_source_v1","query":QUERY,"table":"bars_polygon","symbols":list(SYMBOLS),"canonical_risk_multipliers":RISK_MULTIPLIER,"interval":"1m","window":[START.isoformat(),END.isoformat()],"outcomes_opened":False,"candidate_incidence_opened":False,"no_imputation":True,"output":{"path":str(SESSION),"sha256":sha(SESSION),"rows":len(frame),"valid_rows":int(frame.source_valid.sum())}};payload={**core,"manifest_hash":chash(core)};SOURCE_MANIFEST.write_text(json.dumps(payload,indent=2,allow_nan=False)+"\n");return payload
def features()->pd.DataFrame:
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVFXBR prereg drift")
 if sha(PRICE)!=PRICE_SHA or sha(PRICE_MANIFEST)!=PRICE_MANIFEST_SHA:raise RuntimeError("HVFXBR BTC source drift")
 ...
 for column in ("common_risk_direction","agreeing_pairs","median_absolute_pair_z","positive_qualifying_pairs","negative_qualifying_pairs","common_dollar_side"):f[column]=pd.to_numeric(f[column],errors="coerce")
 ...
 d=p[["decision_time","btc_realized_variation","btc_valid"]];f=f.merge(d,on="decision_time",how="left",validate="one_to_one");f["btc_realized_variation_rank"]=strict_prior_midrank(f.btc_realized_variation.where(f.btc_valid));f["signal_valid"]=f.source_valid&f.btc_valid&np.isfinite(f[["common_risk_direction","agreeing_pairs","median_absolute_pair_z","btc_realized_variation","btc_realized_variation_rank"]]).all(axis=1);return f
def conditions(f:pd.DataFrame,control:str)->tuple[pd.Series,pd.Series]:
 direction=pd.to_numeric(f.common_risk_direction,errors="coerce");positive=f.positive_qualifying_pairs;negative=f.negative_qualifying_pairs
 if control=="one_session_stale_barbell":direction=direction.shift(1);positive=positive.shift(1);negative=negative.shift(1)
 if control=="two_of_four_barbell":direction=pd.Series(np.where((positive.ge(2)&negative.eq(0)),1,np.where((negative.ge(2)&positive.eq(0)),-1,0)),index=f.index)
 if control=="common_dollar_direction":direction=pd.to_numeric(f.common_dollar_side,errors="coerce")
 vol=pd.Series(True,index=f.index) if control=="no_variation_gate" else f.btc_realized_variation_rank.ge(.65)
 active=f.signal_valid&np.isfinite(direction)&direction.abs().eq(1)&vol;side=direction
...
def clock(f:pd.DataFrame,control:str="primary")->pd.DataFrame:
 ...
 for i in f.index[active]:
  ...
  next_allowed=exit_;rows.append({"candidate":"HVFXBR-8","control":control,"split":split,"source_day":f.at[i,"source_day"],"decision_time":decision,"feature_available_time":decision,"entry_time":entry,"exit_time":exit_,"side":int(side.at[i]),"common_risk_direction":int(f.at[i,"common_risk_direction"]),"agreeing_pairs":int(f.at[i,"agreeing_pairs"]),"median_absolute_pair_z":float(f.at[i,"median_absolute_pair_z"]),"btc_realized_variation":float(f.at[i,"btc_realized_variation"]),"btc_realized_variation_rank":float(f.at[i,"btc_realized_variation_rank"])})
