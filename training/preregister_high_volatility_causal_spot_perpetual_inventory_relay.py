"""Outcome-blind preregistration for HVCSPIR-8."""
...
POLICY_ID = "HVCSPIR-8"
DEFAULT_OUTPUT = Path("results/high_volatility_causal_spot_perpetual_inventory_relay_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_causal_spot_perpetual_inventory_relay_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": "When completed eight-hour BTC spot and perpetual returns have the same strict sign, spot has moved farther than the perpetual by an unusually large amount, perpetual realized variation is elevated, and perpetual open interest expands, cash-led price discovery with fresh leveraged sponsorship should pull the perpetual in the common direction for eight hours.",
            "side": "strict common sign of completed eight-hour BTC spot and perpetual returns",
            "why_distinct": "HVCSPIR compares actual BTC spot and perpetual completed returns, requires positive spot leadership and exact perpetual OI expansion, and follows the common direction. HVCLES used five-minute intrabar excursions without OI and failed Gross9; HVCBER used BTC-ETH consensus and failed train. HVCSPIR reuses neither event set nor control and uses no ETH, intrabar excursion, funding, premium-index, fitted outcome, or promoted control.",
            ...
            "why_low_gross9_overlap_is_plausible": "fixed eight-hour cash-led OI-sponsored spot-perpetual states are absent from Gross9 primitives",
        },
        "features": {
            ...
            "completed_cycles": "480 exact coherent one-minute open-to-close log returns from bars_binance_spot and bars_binance BTCUSDT over [D-8h,D)",
            "completed_returns": "sum for spot and perpetual; both finite strict nonzero and same strict sign",
            "spot_lead_displacement": "abs(spot_return)-abs(perpetual_return), finite strict positive",
            "spot_lead_rank": "strict-prior midrank over at most 270 source-valid cycles; current excluded; minimum 180; rank>=0.60",
            ...
            "eligible": "exact coherent completed spot and perpetual cycles with same-sign returns and positive spot lead, two exact perpetual OI endpoints with strict expansion, and both spot-lead and variation ranks pass at D; no funding, range, midpoint, premium-index, alt, or post-decision condition",
            "no_imputation": True,
        },
        "clock": {
        ...
        "source_plan": {
            ...
            "bars": "Postgres bars_binance_spot and bars_binance BTCUSDT exact coherent 1m OHLC",
        ...
        "research_boundary": {
            ...
            "prior_HVCLES_source_and_gross9_failure_known": True,
            "same_mechanism_as_HVCLES": False,
            "prior_HVCBER_source_novelty_and_train_failure_known": True,
            "same_mechanism_as_HVCBER": False,
        ...
        "stopping_rule": "terminal first failure; no OI endpoint or expansion rule, spot/perpetual source, completed-return agreement, spot-lead displacement, variation history, side, clock, hold, subset, threshold, comparator, or control repair",
    }
    return {**core, "manifest_hash": canonical_hash(core)}

def validate(value: Mapping[str, Any]) -> None:
    if value != build():
        raise RuntimeError("HVCSPIR-8 preregistration drift")
    contract = hvcav.build()
    for key in ("stages", "source_support_gates", "gross9_novelty_gates", "economic_gates"):
        if value[key] != contract[key]:
            raise RuntimeError(f"HVCSPIR-8 {key} drift")
