"""Source-only support gate for frozen HVIVDR-8."""
...
from training import preregister_high_volatility_intraday_variance_dispersion_relay as prereg
...
PREREG_SHA = "7193b15406e0d88266e61fe81131993350e17a2b7a1ee448b20b56f1abd23328"
SPLITS = {
...
ROOT = Path("data/high_volatility_intraday_variance_dispersion_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_intraday_variance_dispersion_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_intraday_variance_dispersion_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_intraday_variance_dispersion_relay_support_2026-08-12.json")
...
def variance_dispersion(minute_returns: np.ndarray) -> tuple[float, float]:
    values = np.asarray(minute_returns, dtype=float)
    if len(values) != 480 or not np.isfinite(values).all():
        return math.nan, math.nan
    subwindow_variances = np.square(values).reshape(16, 30).sum(axis=1)
    mean = float(subwindow_variances.mean())
    total = float(subwindow_variances.sum())
    if mean <= 0 or total <= 0:
        return math.nan, math.nan
    dispersion = float(np.std(subwindow_variances, ddof=0) / mean)
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVIVDR source timestamps")
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        if valid:
            ...
            variation, dispersion = variance_dispersion(minute_returns)
            block_return = float(np.log(window["close"].iloc[-1] / window["open"].iloc[0]))
            valid = variation > 0 and math.isfinite(dispersion) and block_return != 0
        if not valid:
            variation = dispersion = block_return = math.nan
        rows.append(
            {
    ...
    states["dispersion_rank"] = prior_rank(states["variance_dispersion"].where(states["source_valid"]))
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvivdr_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    dispersion_rank = states["dispersion_rank"]
    dispersion = states["variance_dispersion"]
    if control == "one_block_stale_geometry":
        block_return, variation_rank, dispersion_rank, dispersion = [
            value.shift(1) for value in (block_return, variation_rank, dispersion_rank, dispersion)
        ]
    variation_gate = (
    ...
    if control == "no_dispersion_tail":
        dispersion_gate = pd.Series(True, index=states.index)
    else:
        dispersion_gate = dispersion_rank.ge(0.75)
    eligible = states["source_valid"] & variation_gate & dispersion_gate & block_return.ne(0)
        ...
        rows.append(
            {
                ...
                "variance_dispersion": float(states.at[index, "variance_dispersion"]),
                "dispersion_rank": float(states.at[index, "dispersion_rank"]),
            }
        )
    columns = [
        "candidate", "control", "split", "decision_time", "feature_available_time",
        "entry_time", "exit_time", "side", "realized_variation", "variation_rank",
        "variance_dispersion", "dispersion_rank",
    ]
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVIVDR preregistration drift")
    ...
    core = {
        "protocol_version": "hvivdr_8_source_support_v1",
        "policy_id": "HVIVDR-8",
        "preregistration": {
