"""Open source-only OOS incidence for preregistered HVSMAM-24."""
...
PREREG_SHA256 = "c5036695ec4d2e9b61c11cd6d813545efa5b704a4d2431918f05f85b3ca89461"
API = "https://community-api.coinmetrics.io/v4/timeseries/asset-metrics"
SOURCE_DIR = Path("data/high_volatility_sha256_miner_allocation_migration_relay_sources_2023_2026")
PAIR_PANEL = SOURCE_DIR / "btc_bch_hashrate_daily.csv.gz"
FEATURE_PANEL = SOURCE_DIR / "daily_feature_panel.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_sha256_miner_allocation_migration_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_sha256_miner_allocation_migration_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_sha256_miner_allocation_migration_relay_support_2026-08-10.json")
QUERY = """
...
CLOCK_COLUMNS = (
    ...
    "decision_time", "entry_time", "exit_time", "side", "btc_hashrate", "bch_hashrate",
    "relative_hash_allocation", "allocation_migration", "absolute_migration_rank",
    "btc_hashrate_change", "realized_variation", "realized_variation_rank",
)
...
def source_url() -> str:
    return API + "?" + urllib.parse.urlencode({
        "assets": "btc,bch",
        "metrics": "HashRate,BlkCnt,AssetEODCompletionTime",
        "frequency": "1d",
...
def parse_source_row(raw: Any) -> dict[str, Any]:
    if not isinstance(raw, dict) or set(raw) != {
        "asset", "time", "HashRate", "BlkCnt", "AssetEODCompletionTime"
    }:
    ...
    hashrate = float(raw["HashRate"])
    blocks = float(raw["BlkCnt"])
    if (
        observation.tzinfo is None or observation != observation.floor("1d")
        or not math.isfinite(hashrate) or hashrate <= 0
        or not math.isfinite(blocks) or blocks <= 0
...
def download_pair_panel() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    pair = pd.DataFrame({
        ...
        "btc_hashrate": by_asset["btc"].hashrate.to_numpy(float),
        "bch_hashrate": by_asset["bch"].hashrate.to_numpy(float),
        "btc_block_count": by_asset["btc"].block_count.to_numpy(float),
        "bch_block_count": by_asset["bch"].block_count.to_numpy(float),
    })
...
def feature_panel(pair: pd.DataFrame, bars: pd.DataFrame) -> pd.DataFrame:
    ...
    frame = pair.copy()
    frame["relative_hash_allocation"] = np.log(frame.btc_hashrate / frame.bch_hashrate)
    frame["allocation_migration"] = frame.relative_hash_allocation.diff(3)
    frame["btc_hashrate_change"] = np.log(frame.btc_hashrate).diff(3)
    ...
    frame["absolute_migration_rank"] = strict_prior_midrank(
        frame.allocation_migration.abs().where(frame.source_valid)
    )
...
def candidate_clock(panel: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    frame = panel.copy()
    migration = frame.allocation_migration
    ...
    btc_change = frame.btc_hashrate_change
    ...
    side_source = btc_change if control == "btc_hashrate_change_only" else migration
    ...
    for index in frame.index[onset]:
        ...
        rows.append({
            ...
            "btc_hashrate": float(frame.at[index, "btc_hashrate"]),
            "bch_hashrate": float(frame.at[index, "bch_hashrate"]),
            "relative_hash_allocation": float(frame.at[index, "relative_hash_allocation"]),
            "allocation_migration": float(migration.at[index]),
            "absolute_migration_rank": float(migration_rank.at[index]),
            "btc_hashrate_change": float(btc_change.at[index]),
...
def run() -> dict[str, Any]:
    ...
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA256:
        raise RuntimeError("HVSMAM preregistration artifact drift")
    registration = json.loads(prereg.DEFAULT_OUTPUT.read_text())
    if registration != prereg.build():
        raise RuntimeError("HVSMAM preregistration payload drift")
    ...
    source_core = {
        "protocol_version": "hvsmam_24_source_materialization_v1",
    ...
    core = {
        "protocol_version": "hvsmam_24_oos_source_support_v1",
        "policy_id": "HVSMAM-24",
        "preregistration": {"path": str(prereg.DEFAULT_OUTPUT), "sha256": PREREG_SHA256, "manifest_hash": registration["manifest_hash"]},
