"""Evaluate frozen HVRPPR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVRPPR-8"
PROTOCOL = "hvrppr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_range_participation_polarity_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVRPPR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVRPPR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVRPPR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVRPPR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvrppr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVRPPR-8 unchanged before economic outcomes",
    }
