"""Evaluate frozen HVBITOFL-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVBITOFL-24"
PROTOCOL = "hvbitofl_24_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_bito_futures_flow_sponsorship_relay_preregistration_2026-08-11.json"
)
...
OUTPUT = Path(
    "results/high_volatility_bito_futures_flow_sponsorship_relay_gross9_novelty_2026-08-11.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVBITOFL predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVBITOFL novelty limits drift")
    if (
        ...
        raise RuntimeError("HVBITOFL predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVBITOFL primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvbitofl_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVBITOFL-24 unchanged before economic outcomes"
        ),
    }
