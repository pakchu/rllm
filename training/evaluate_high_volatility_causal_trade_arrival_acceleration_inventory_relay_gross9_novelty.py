"""Evaluate the frozen HVCNAIR-8 action-vote clock against Gross9."""
...
POLICY = "HVCNAIR-8"
PROTOCOL = "hvcnair_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_trade_arrival_acceleration_inventory_relay_clocks_2020_2026.csv.gz",
    "sha256": "8302d5b37819466b53f43e0d6b4e01cf4006419728e021b01637244e5821d1b4",
    "rows": 188,
}
OUTPUT = Path("results/high_volatility_causal_trade_arrival_acceleration_inventory_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCNAIR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCNAIR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCNAIR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCNAIR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCNAIR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCNAIR-8 unchanged before economics",
    }
