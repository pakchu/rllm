"""Evaluate frozen HVDCOS-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDCOS-8"
PROTOCOL = "hvdcos_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_directional_change_fresh_overshoot_relay_preregistration_2026-08-11.json")
PREREG_SHA = "1f4d6b55d2e83401898b1d787967a48d646c6f7f9c1f5ccbe69111d981df4e8a"
SUPPORT = Path("results/high_volatility_directional_change_fresh_overshoot_relay_support_2026-08-11.json")
SUPPORT_SHA = "af074a5ad1f4fba54a18e565166ff8700f25589b326dbbc07754c72c8f4d1060"
CLOCK = Path("data/high_volatility_directional_change_fresh_overshoot_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "95c5740516e645573f73668ea9369c807ff0824385e55df84b0a94dfb367f7fa"
OUTPUT = Path("results/high_volatility_directional_change_fresh_overshoot_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDCOS predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVDCOS novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDCOS predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDCOS primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdcos_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVDCOS-8 unchanged before economic outcomes",
    }
