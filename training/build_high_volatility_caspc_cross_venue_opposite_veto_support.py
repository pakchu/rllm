"""Source-only asynchronous CVDR opposite-side veto for HVCASPCCVDRV-8."""
from __future__ import annotations
import hashlib,json
from pathlib import Path
from typing import Any
import pandas as pd
from training import preregister_high_volatility_caspc_cross_venue_opposite_veto as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
PREREG_SHA='c02528f477e2c1264a9a6f926c04d8eac032c9f5d6b0b5948b908755bed3a21b';BASE=Path('data/high_volatility_cross_alt_serial_persistence_consensus_relay_clocks_2023_2026.csv.gz');CVDR=Path('data/cross_venue_disagreement_resolution_relay_clocks_2023_2026.csv.gz');CLOCK=Path('data/high_volatility_caspc_cross_venue_opposite_veto_clocks_2023_2026.csv.gz');CONTROL_DIR=Path('data/high_volatility_caspc_cross_venue_opposite_veto_controls_2023_2026');RESULT=Path('results/high_volatility_caspc_cross_venue_opposite_veto_support_2026-08-16.json');MINIMUM_EVENTS={'train':8,'test':12,'eval':12,'final':8}
...
def route(base,cvdr):
 ...
 for b in base.itertuples(index=False):
  ...
  cvdr_side=0;cvdr_entry=pd.NaT
  if len(active)==1:
   r=active.iloc[0];cvdr_side=int(r.side);cvdr_entry=r.entry_time;active_total+=1
   if cvdr_side==-int(b.side):
    vetoes[b.split]+=1
    continue
   same_side_total+=1
  rows.append({'candidate':prereg.POLICY_ID,'control':'primary','split':b.split,'decision_time':b.decision_time,'feature_available_time':b.feature_available_time,'entry_time':b.entry_time,'exit_time':b.exit_time,'side':int(b.side),'base_side':int(b.side),'cvdr_active':len(active)==1,'cvdr_side':cvdr_side,'cvdr_entry_time':cvdr_entry})
...
def run():
 ...
 checks['activation_train_opposite_veto_min_1']=activation['opposite_vetoes_by_split']['train']>=1;checks['activation_full_opposite_veto_min_3']=activation['opposite_vetoes_total']>=3;passed=all(checks.values());core={'protocol_version':'hvcaspccvdrv_8_source_support_v1','policy_id':prereg.POLICY_ID,'preregistration':{'path':str(prereg.DEFAULT_OUTPUT),'sha256':PREREG_SHA,'manifest_hash':reg['manifest_hash']},'completed_preentry_sources_opened':True,'postentry_return_pnl_execution_price_opened':False,'held_interval_funding_values_opened':False,'gross9_rows_opened':False,'component_clock_rows_opened':{'HVCASPC-8':len(base),'CVDR-6':len(cvdr)},'activation':activation,'clock':{'path':str(CLOCK),'sha256':sha(CLOCK),'rows':len(clock)},'controls':{'immutable_base_side':{'path':str(CONTROL_DIR/'immutable_base_side.csv.gz'),'sha256':sha(CONTROL_DIR/'immutable_base_side.csv.gz'),'rows':len(base),'promotion_authorized':False}},'support':support,'support_checks':checks,'support_passed':passed,'advance_to_gross9_novelty':passed,'advance_to_economic_outcomes':False,'decision':'pass_to_novelty' if passed else 'terminal_source_support_reject'}
