"""Evaluate frozen HVPHR-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVPHR-24"
PROTOCOL = "hvphr_24_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_peak_candle_range_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVPHR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVPHR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVPHR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVPHR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvphr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVPHR-24 unchanged before economic outcomes"
        ),
    }
