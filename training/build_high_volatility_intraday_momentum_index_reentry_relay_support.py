"""Build outcome-blind source support for frozen HVIMI-24."""
...
from training import preregister_high_volatility_intraday_momentum_index_reentry_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "f6446d3e6585fe56eaaab1ff6897e6e2659c3ba2c6bff255dd423cfee58fb1f7"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_intraday_momentum_index_reentry_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_intraday_momentum_index_reentry_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_intraday_momentum_index_reentry_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_intraday_momentum_index_reentry_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_intraday_momentum_index_reentry_relay_support_2026-08-11.json")
...
CLOCK_COLUMNS = (
    ...
    "imi", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVIMI source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVIMI invalid source key")
...
def intraday_momentum(open_: pd.Series, close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    body=(close-open_).where(valid);up=body.clip(lower=0);down=(-body).clip(lower=0);period=P["imi_periods"];up_sum=up.rolling(period,min_periods=period).sum();down_sum=down.rolling(period,min_periods=period).sum();den=up_sum+down_sum;value=(100.0*up_sum/den).where(den.gt(0));return pd.DataFrame({"up_body":up,"down_body":down,"up_sum":up_sum,"down_sum":down_sum,"imi":value},index=open_.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.imi.shift(1);long=previous.le(P["lower_level"])&panel.imi.gt(P["lower_level"]);short=previous.ge(P["upper_level"])&panel.imi.lt(P["upper_level"]);panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&(long|short)&panel.variation_rank.ge(P["variation_rank_min"]);return panel.loc[:,PANEL_COLUMNS]
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVIMI prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvimi_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvimi_24_source_support_v1", "policy_id": prereg.POLICY_ID,
