"""Evaluate frozen HVTCR-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVTCR-24"
PROTOCOL = "hvtcr_24_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_trough_candle_range_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTCR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVTCR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTCR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvtcr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVTCR-24 unchanged before economic outcomes"
        ),
    }
