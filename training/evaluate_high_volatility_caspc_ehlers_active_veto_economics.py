"""Strict staged economics for the singleton HVCELV-8 policy."""
...
PREREG = Path("results/high_volatility_caspc_ehlers_active_veto_preregistration_2026-08-16.json")
PREREG_SHA = "7dc8f6228fa32eba7d5aded586d389855a47fbf12917800c43537e5e18c6244f"
SUPPORT = Path("results/high_volatility_caspc_ehlers_active_veto_support_2026-08-16.json")
SUPPORT_SHA = "9b7d2230588465a73dd9646387d3d7000b3f26d98daeef4e25cf51f4118c6375"
GROSS9 = Path("results/high_volatility_caspc_ehlers_active_veto_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "b7d94871688fce546a6a187594cf9665856193d7129cba4c2ca6e69fa9aa3b17"
CLOCK = {
    "path": Path("data/high_volatility_caspc_ehlers_active_veto_clocks_2023_2026.csv.gz"),
    "sha256": "1f8512a74906921169b115ec917c80c76b714c5a2f5fda25698feb6b4b026294",
    "rows": 130,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_caspc_ehlers_active_veto_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCELV-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCELV-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCELV-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCELV-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCELV-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCELV-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCELV-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCELV-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCELV-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCELV-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCELV-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcelv_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
