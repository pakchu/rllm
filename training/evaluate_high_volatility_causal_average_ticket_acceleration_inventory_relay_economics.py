"""Strict staged economics for the singleton HVCATAIR-8 policy."""
...
PREREG = Path("results/high_volatility_causal_average_ticket_acceleration_inventory_relay_preregistration_2026-08-16.json")
PREREG_SHA = "49e3fa536b7c0ebd83143d2f024fb079af2b9fea7d3980eaf1b670691bc5803d"
SUPPORT = Path("results/high_volatility_causal_average_ticket_acceleration_inventory_relay_support_2026-08-16.json")
SUPPORT_SHA = "c014e646253f4ecebc269c08edc4c38a9f036859569c3c6bb717b852171dda8e"
GROSS9 = Path("results/high_volatility_causal_average_ticket_acceleration_inventory_relay_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "1c14408602eea75f519056f236e452826ae8d994ff38fc2591566bd3aea5f4d7"
CLOCK = {
    "path": Path("data/high_volatility_causal_average_ticket_acceleration_inventory_relay_clocks_2020_2026.csv.gz"),
    "sha256": "3f37f672e2805e0a5dda33eb5614cb53003664935e2b0eb3660564fa2b31cc4a",
    "rows": 146,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_average_ticket_acceleration_inventory_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCATAIR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCATAIR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCATAIR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCATAIR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATAIR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATAIR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCATAIR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCATAIR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCATAIR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCATAIR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCATAIR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcatair_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
