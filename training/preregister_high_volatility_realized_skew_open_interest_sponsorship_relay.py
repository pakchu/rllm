"""Outcome-blind preregistration for HVRSOIR-12."""
...
POLICY_ID = "HVRSOIR-12"
DEFAULT_OUTPUT = Path("results/high_volatility_realized_skew_open_interest_sponsorship_relay_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    core = {
        "protocol_version": "high_volatility_realized_skew_open_interest_sponsorship_relay_v1",
        "policy_id": POLICY_ID,
        ...
        "singleton": True,
        "mechanism": {
            "claim": "An extreme signed third moment in completed intraday BTC returns measures one-sided jump concentration rather than total variance. In an already high-variation state, exact twenty-four-hour open-interest expansion identifies newly sponsored one-sided risk, so follow the skew direction for twelve hours.",
            "side": "the strict sign of completed 24-hour realized skew",
            "why_distinct": "HVSIR used positive-versus-negative second-moment mass and an intraday threshold crossing; HVSER used return-sign entropy; BPJR used bipower jump variation. HVRSOIR combines a daily normalized third moment, strict-prior ranks, and exact OI expansion, reuses no event set, and promotes no control.",
            "why_suited_to_volatile_regimes": "completed 24-hour realized variation must rank in its causal upper 35%",
            "why_low_gross9_overlap_is_plausible": "one sparse 02:05 UTC daily third-moment plus OI clock is absent from Gross9 primitives",
        },
        "features": {
            ...
            "open_interest": "finite positive BTCUSDT period=5m sum_open_interest observations at decision-24h and decision, exact timestamps no later than decision",
            "oi_expansion": "strict positive log(OI[decision]/OI[decision-24h])",
        ...
        "clock": {
            ...
            "side": "strict realized-skew sign",
            "hold": "12 elapsed hours",
            "reservation": "global half-open; exit first on equal open",
            "funding_premium": "not signal inputs; exact funding only after novelty passes",
        },
        "policy": {
            "return_bars": 288,
            "prior_days": 90,
        ...
        "source_plan": {"open_interest": {"table": "open_interest_binance", "symbol": "BTCUSDT", "period": "5m", "columns": ["ts", "sum_open_interest"]}, "bars": {"table": "bars_binance", "symbol": "BTCUSDT", "interval": "5m", "columns": ["ts", "open", "high", "low", "close"], "read_after_preregistration": True}, "execution_prices": "sealed until source support and Gross9 novelty pass"},
        "research_boundary": {
            "prior_hvsir_hvser_bpjr_outcomes_known": True,
            "prior_HVRSR_source_novelty_and_train_failure_known": True,
            ...
            "selection_basis": "normalized third-moment sponsorship by expanding OI in volatile BTC regimes",
        },
        "stopping_rule": "Terminal first-failure sequence: source support, Gross9 novelty, strict economics; no OI endpoint or expansion rule, moment definition, rank, side, hold, clock, subset, or control repair.",
    }
