"""Build outcome-blind source support for frozen HVMVRV-24.
...
from training import preregister_high_volatility_mvrv_valuation_dislocation_relay as prereg


POLICY_ID = "HVMVRV-24"
PREREG_SHA256 = "945b068ac555f0a2ef1ab794c178044b53699e308df70c2c6372d054aad9353e"
...
ORDER BY ts"""
BTC_QUERY_START = SOURCE_START + pd.Timedelta(hours=12)
...
SOURCE_DIR = Path("data/high_volatility_mvrv_valuation_dislocation_relay_sources_2022_2026")
DAILY_SOURCE = SOURCE_DIR / "coinmetrics_btc_daily.csv.gz"
FEATURE_PANEL = SOURCE_DIR / "daily_feature_panel.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_mvrv_valuation_dislocation_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_mvrv_valuation_dislocation_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_mvrv_valuation_dislocation_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_mvrv_valuation_dislocation_relay_support_2026-08-13.json")

SOURCE_COLUMNS = (
    "observation_time", "feature_available_time", "CapMVRVCur",
)
PANEL_COLUMNS = (
    "observation_time", "feature_available_time", "decision_time", "source_valid",
    "minute_count", "CapMVRVCur", "mvrv_log_prior_mean_30d",
    "mvrv_log_prior_std_30d", "mvrv_local_z", "btc_decision_close",
    "market_proxy_log_prior_mean_30d", "market_proxy_log_prior_std_30d",
    "market_proxy_local_z", "btc_variation", "btc_variation_rank",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "observation_time", "feature_available_time",
    "decision_time", "entry_time", "exit_time", "side", "CapMVRVCur",
    "mvrv_log_prior_mean_30d", "mvrv_log_prior_std_30d", "mvrv_local_z",
...
def build_panel(source: pd.DataFrame, btc_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    daily["CapMVRVCur"] = pd.to_numeric(daily.CapMVRVCur, errors="coerce")
    expected = pd.date_range(SOURCE_START, SOURCE_END, freq="1d", inclusive="left")
    if (
    ...
    decision = daily.observation_time + pd.Timedelta(days=1, hours=12)
    completion_valid = (
        daily.feature_available_time.gt(daily.observation_time + pd.Timedelta(days=1))
        & daily.feature_available_time.le(decision)
    )
    value_valid = np.isfinite(daily.CapMVRVCur) & daily.CapMVRVCur.gt(0)
    log_mvrv = np.log(daily.CapMVRVCur.where(value_valid))
    prior_log_mvrv = log_mvrv.shift(1)
    mvrv_prior_mean = prior_log_mvrv.rolling(30, min_periods=30).mean()
    mvrv_prior_std = prior_log_mvrv.rolling(30, min_periods=30).std(ddof=1)
    rolling_valid = value_valid.shift(1).rolling(30, min_periods=30).sum().eq(30)
    mvrv_z = ((log_mvrv - mvrv_prior_mean) / mvrv_prior_std).where(
        rolling_valid & value_valid & np.isfinite(mvrv_prior_std) & mvrv_prior_std.gt(0)
    )
    ...
    for at in decision:
        ...
        decision_close.append(float(window.close.iloc[-1]) if good else math.nan)

    log_market_proxy = np.log(pd.Series(decision_close, index=daily.index))
    prior_market_proxy = log_market_proxy.shift(1)
    market_prior_mean = prior_market_proxy.rolling(30, min_periods=30).mean()
    market_prior_std = prior_market_proxy.rolling(30, min_periods=30).std(ddof=1)
    market_proxy_z = ((log_market_proxy - market_prior_mean) / market_prior_std).where(
        np.isfinite(market_prior_std) & market_prior_std.gt(0)
    )

    base_valid = (
        completion_valid & rolling_valid & value_valid
        & pd.Series(variation_valid, index=daily.index)
        & np.isfinite(mvrv_z)
    )
    panel = pd.DataFrame({
        ...
        "CapMVRVCur": daily.CapMVRVCur.astype(float),
        "mvrv_log_prior_mean_30d": mvrv_prior_mean,
        "mvrv_log_prior_std_30d": mvrv_prior_std,
        "mvrv_local_z": mvrv_z,
        "btc_decision_close": decision_close,
        "market_proxy_log_prior_mean_30d": market_prior_mean,
        "market_proxy_log_prior_std_30d": market_prior_std,
        "market_proxy_local_z": market_proxy_z,
...
def active(panel: pd.DataFrame, control: str = "primary") -> tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    used = panel.copy()
    signal = used.mvrv_local_z
    available = used.feature_available_time
    valid = used.source_valid.eq(True)
    if control == "one_day_stale_mvrv":
        signal = used.mvrv_local_z.shift(1)
        available = used.feature_available_time.shift(1)
        valid = valid & signal.notna()
    elif control == "market_cap_only_local_z":
        signal = used.market_proxy_local_z
        valid = valid & signal.notna()
    tail = signal.abs().ge(0.5)
    volatility = pd.Series(True, index=panel.index) if control == "no_btc_volatility_gate" else used.btc_variation_rank.ge(0.65)
    eligible = valid & signal.notna() & tail & volatility
    side = pd.Series(np.where(signal.lt(0), 1, -1), index=panel.index, dtype=int)
    if control == "mvrv_direction_flip":
        side = -side
    elif control == "same_clock_forced_long":
        side = pd.Series(1, index=panel.index, dtype=int)
    adjusted = used.copy()
    adjusted["mvrv_local_z"] = signal
...
def build_clock(panel: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in panel.index[eligible]:
        ...
        rows.append({
            ...
            "CapMVRVCur": float(panel.at[index, "CapMVRVCur"]),
            "mvrv_log_prior_mean_30d": float(panel.at[index, "mvrv_log_prior_mean_30d"]),
            "mvrv_log_prior_std_30d": float(panel.at[index, "mvrv_log_prior_std_30d"]),
            "mvrv_local_z": float(used.at[index, "mvrv_local_z"]),
...
def run(*, fetch: Fetch | None = None) -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA256:
        raise RuntimeError("HVMVRV frozen preregistration artifact drift")
    registration = json.loads(prereg.DEFAULT_OUTPUT.read_text(encoding="utf-8"))
    if registration != prereg.build():
        raise RuntimeError("HVMVRV frozen preregistration payload drift")
    ...
    source_core = {
        "protocol_version": "hvmvrv_24_source_materialization_v1",
    ...
    core = {
        "protocol_version": "hvmvrv_24_source_support_v1",
        "policy_id": POLICY_ID,
        "preregistration": {"path": str(prereg.DEFAULT_OUTPUT), "sha256": PREREG_SHA256, "manifest_hash": registration["manifest_hash"]},
