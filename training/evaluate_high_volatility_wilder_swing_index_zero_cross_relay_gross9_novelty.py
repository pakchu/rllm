"""Evaluate frozen HVWSI-Z20-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVWSI-Z20-24"
PROTOCOL = "hvwsi_z20_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_wilder_swing_index_zero_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVWSI predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVWSI novelty limits drift")
    if (
        ...
        raise RuntimeError("HVWSI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVWSI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvwsi_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVWSI-Z20-24 unchanged before economic outcomes",
    }
