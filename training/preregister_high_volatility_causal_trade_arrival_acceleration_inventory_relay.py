"""Outcome-blind preregistration for HVCNAIR-8."""
...
POLICY_ID = "HVCNAIR-8"
DEFAULT_OUTPUT = Path("results/high_volatility_causal_trade_arrival_acceleration_inventory_relay_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_causal_trade_arrival_acceleration_inventory_relay_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": "When BTC perpetual trade arrivals accelerate unusually from the first to the second four-hour half, realized variation is elevated, open interest expands, and the completed eight-hour price return is nonzero, broad transaction participation and fresh leveraged inventory sponsor the completed price direction for eight hours.",
            "side": "strict sign of the completed eight-hour BTC perpetual return",
            "why_distinct": "HVCNAIR uses within-cycle transaction-count acceleration plus exact OI expansion and variation. Unlike HVCVAIR it measures participation breadth rather than notional turnover; existing trade-arrival candidates omit exact OI sponsorship. HVCNAIR reuses neither event set nor control and uses no quote-volume magnitude, taker-flow sign, spot source, ETH, funding, premium-index, fitted outcome, or promoted control.",
            ...
            "why_low_gross9_overlap_is_plausible": "fixed eight-hour accelerating-trade-arrival plus OI-sponsored states are absent from Gross9 primitives",
        },
        "features": {
            ...
            "completed_cycle": "480 exact coherent bars_binance BTCUSDT one-minute rows with nonnegative integer number_of_trades over [D-8h,D)",
            "half_trade_counts": "sum number_of_trades separately over first and second 240-minute halves; both strict positive",
            "trade_arrival_acceleration": "log(second_half_trade_count/first_half_trade_count), finite strict positive",
            "trade_arrival_acceleration_rank": "strict-prior midrank over at most 270 source-valid cycles; current excluded; minimum 180; rank>=0.60",
            ...
            "eligible": "exact coherent completed perpetual cycle, positive trade-arrival acceleration, nonzero completed return, two exact OI endpoints with strict expansion, and both trade-arrival-acceleration and variation ranks pass at D; no quote-volume magnitude or taker-flow sign, spot source, funding, range, premium-index, alt, or post-decision condition",
            "no_imputation": True,
        },
        "clock": {
        ...
        "policy": {
            ...
            "path_minutes": 480,
            "half_minutes": 240,
            "trade_arrival_acceleration_rank_min": 0.60,
        ...
        "source_plan": {
            ...
            "bars": "Postgres bars_binance BTCUSDT exact coherent 1m OHLC plus number_of_trades and taker_buy_quote",
        ...
        "research_boundary": {
            ...
            "prior_trade_arrival_candidate_outcomes_known": True,
            "same_mechanism_as_prior_trade_arrival_candidates": False,
        ...
        "stopping_rule": "terminal first failure; no OI endpoint or expansion rule, trade-arrival acceleration formula, acceleration or variation history, side, clock, hold, subset, threshold, comparator, or control repair",
    }
    return {**core, "manifest_hash": canonical_hash(core)}

def validate(value: Mapping[str, Any]) -> None:
    if value != build():
        raise RuntimeError("HVCNAIR-8 preregistration drift")
    contract = hvcav.build()
    for key in ("stages", "source_support_gates", "gross9_novelty_gates", "economic_gates"):
        if value[key] != contract[key]:
            raise RuntimeError(f"HVCNAIR-8 {key} drift")
