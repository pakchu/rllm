"""Outcome-blind preregistration for HVBFRRFC-8."""
...
POLICY_ID = "HVBFRRFC-8"
DEFAULT_OUTPUT = Path("results/high_volatility_btc_factor_residual_flip_continuation_preregistration_2026-08-18.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_btc_factor_residual_flip_continuation_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": "A robust BTC-specific displacement that crosses through the broad crypto factor and finishes with the opposite residual sign at the next sampled auction marks a completed relative-leadership rotation; follow the new BTC-relative direction during elevated BTC variation.",
            "side": "strict sign of the current BTC-minus-six-alt-median residual",
            "why_distinct": "HVBFRR fits daily OLS and follows a current residual; HVBDRR follows a current residual tail plus contemporaneous dispersion. HVBFRRFC uses two consecutive frozen sampled auctions, requires a prior robust two-MAD BTC outlier and an already-completed residual sign flip, and follows the new relative-leadership direction without fitted beta, outcome fit, OI, funding, premium, options, or promoted control.",
            "why_suited_to_volatile_regimes": "current causal BTC realized-variation rank must be at least 0.65",
            "why_low_gross9_overlap_is_plausible": "irregular two-auction cross-sectional residual sign-flip clocks are absent from Gross9 primitives",
        },
        "features": {
            ...
            "residual_flip": "previous and current residuals are strict nonzero with opposite signs",
        ...
        "diagnostic_controls": {"names": ["no_prior_outlier", "no_residual_flip", "no_variation_gate", "previous_side", "forced_long"], "cannot_be_promoted": True},
        "research_boundary": {
            "prior_HVBFRR_source_failure_known": True,
            "prior_HVBDRR_train_failure_known": True,
            "repository_two_auction_btc_factor_residual_flip_found": False,
            "prior_HVBFRRCR_source_failure_known": True,
            ...
            "selection_basis": "robust cross-sectional overshoot followed by completed residual polarity transition",
        },
        "stopping_rule": "source, Gross9, train/test/eval/final; terminal first failure; no universe, factor, MAD, outlier threshold, residual flip, variation, side, clock, hold, subset, comparator, or control repair",
    }
    return {**core, "manifest_hash": canonical_hash(core)}

def validate(value: dict[str, Any]) -> None:
    if value != build():
        raise RuntimeError("HVBFRRFC-8 preregistration drift")
    if sha256(SOURCE) != SOURCE_SHA:
        raise RuntimeError("HVBFRRFC-8 source drift")
