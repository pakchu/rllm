"""Sequential strict economics for frozen HVDTIAR-6."""
...
PREREG = Path("results/high_volatility_directional_tail_index_asymmetry_relay_preregistration_2026-08-10.json")
PREREG_SHA = "7f27a47f94088507384ee7e03ec202c081b5395e89e4fbe9b6ed07e25cdaf31f"
SUPPORT = Path("results/high_volatility_directional_tail_index_asymmetry_relay_support_2026-08-10.json")
SUPPORT_SHA = "34e7447ca53cf951cbff9d23a6fdd38fd42b74895bdb9e10b921eb4cb7954ad0"
NOVELTY = Path("results/high_volatility_directional_tail_index_asymmetry_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "b7ce8732dcf24f37998f5843cfa48fc742179007dbcf63ef4db65ddb829fbad5"
CLOCK = Path("data/high_volatility_directional_tail_index_asymmetry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "5f79910b48fcce4e1e6be8cfa6f1e153af849cbf1cd603762e3565ad572f9e17"
CONTROL_DIR = Path("data/high_volatility_directional_tail_index_asymmetry_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_directional_tail_index_asymmetry_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_directional_tail_index_asymmetry_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_tail_asymmetry_rank", "no_variation_gate", "fixed_k_tail_mass_asymmetry", "one_boundary_stale_tail", "direction_flip", "same_clock_forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDTIAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDTIAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdtiar_6_sequential_economics_v1", "policy_id": POLICY_ID,
