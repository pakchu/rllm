"""Sequential strict economics for frozen CFSTR-6."""
...
PREREG = Path("results/cboe_front_slope_crypto_volatility_transmission_relay_preregistration_2026-08-08.json")
PREREG_SHA = "b7791f8544abc5c841ed7ba8e4361b9f29d991aad120d28ced6434595ca95780"
SUPPORT = Path("results/cboe_front_slope_crypto_volatility_transmission_relay_support_2026-08-08.json")
SUPPORT_SHA = "cb1b8d27c9f2bc8021541fbf36742f2a00319e19dea2500a4896bf23bda84575"
NOVELTY = Path("results/cboe_front_slope_crypto_volatility_transmission_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "fc5d55f372d9fda5e267ff9d8ed3a89559a0f290ae62d94b69b3ac1297db7b9b"
CLOCK = Path("data/cboe_front_slope_crypto_volatility_transmission_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "e44f6f7571eca60aceeecfa1bb4f6f2d574d626dfaff04c79d39aa2cbb7d28f9"
CONTROL_DIR = Path("data/cboe_front_slope_crypto_volatility_transmission_relay_controls_2023_2026")
FREEZE = Path("results/cboe_front_slope_crypto_volatility_transmission_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/cboe_front_slope_crypto_volatility_transmission_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
CONTROLS = ("no_crypto_confirmation", "bvol_only_confirmation", "dvol_only_confirmation", "one_session_stale_front_slope_change", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CFSTR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CFSTR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "cfstr_6_sequential_economics_v1", "policy_id": POLICY_ID,
