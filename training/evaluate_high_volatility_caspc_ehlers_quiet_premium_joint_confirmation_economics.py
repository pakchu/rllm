"""Sequential strict economics for frozen HVCELVPQA-8."""
...
PREREG = Path("results/high_volatility_caspc_ehlers_quiet_premium_joint_confirmation_preregistration_2026-08-16.json")
PREREG_SHA = "5aa77cc9fafca7aa7aba098641af010d0d697b8565ce48e2f7c5596cf52a40f1"
SUPPORT = Path("results/high_volatility_caspc_ehlers_quiet_premium_joint_confirmation_support_2026-08-16.json")
SUPPORT_SHA = "a9350b733e10d9b55f1fd8c5cf97b486a802696ac6ea07f579c50b6db3791be8"
NOVELTY = Path("results/high_volatility_caspc_ehlers_quiet_premium_joint_confirmation_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "bee83dab44fdf2b7965c679725dfea49f532bc42690dead01eb511d9d196ba99"
CLOCK = Path("data/high_volatility_caspc_ehlers_quiet_premium_joint_confirmation_clocks_2023_2026.csv.gz")
CLOCK_SHA = "875ecdc531f8f31142be01ca9f8cc9f8df5e141bc11b6138680ee78f451c60fe"
CONTROL_DIR = Path("data/high_volatility_caspc_ehlers_quiet_premium_joint_confirmation_controls_2023_2026")
FREEZE = Path("results/high_volatility_caspc_ehlers_quiet_premium_joint_confirmation_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_caspc_ehlers_quiet_premium_joint_confirmation_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCELVPQA frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCELVPQA novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcelvpqa_8_sequential_economics_v1", "policy_id": POLICY_ID,
