"""Sequential strict economics for frozen HVESSR-8."""
...
PREREG = Path("results/high_volatility_execution_seasonality_surprise_relay_preregistration_2026-08-13.json")
PREREG_SHA = "efeb549e1bc904b6f165e9a47795c60e15fe4ae458bee4fa6fe39f006fcd16dd"
SUPPORT = Path("results/high_volatility_execution_seasonality_surprise_relay_support_2026-08-13.json")
SUPPORT_SHA = "b97005d8dfd1df073adf71d7223aa1b68c55fc0b1c6f59cc4bb32d26c2b643e6"
NOVELTY = Path("results/high_volatility_execution_seasonality_surprise_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "e570b9ebaf3322a317b55b08e6889b85698c6a8e3d768b3779f0ce94ea720378"
CLOCK = Path("data/high_volatility_execution_seasonality_surprise_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "31181f51c08a8af0dafb00647bb06e1dd3c03c050e4f61a13406af04e21f7f0c"
CONTROL_DIR = Path("data/high_volatility_execution_seasonality_surprise_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_execution_seasonality_surprise_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_execution_seasonality_surprise_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_surprise_tail", "no_variation_gate", "absolute_count_tail", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVESSR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVESSR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvessr_8_sequential_economics_v1", "policy_id": POLICY_ID,
