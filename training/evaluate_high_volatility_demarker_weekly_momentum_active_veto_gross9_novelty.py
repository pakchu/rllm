"""Evaluate the frozen HVDEMWMV-24 action-vote clock against Gross9."""
...
POLICY = "HVDEMWMV-24"
PROTOCOL = "hvdemwmv_24_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_demarker_weekly_momentum_active_veto_clocks_2023_2026.csv.gz",
    "sha256": "fd0ae6e974f5d0f3026d62fed76eed07fdb19af5e3e242458fdbcac59729a12a",
    "rows": 110,
}
OUTPUT = Path("results/high_volatility_demarker_weekly_momentum_active_veto_gross9_novelty_2026-08-17.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVDEMWMV-24 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVDEMWMV-24 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVDEMWMV-24 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVDEMWMV-24 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVDEMWMV-24 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVDEMWMV-24 unchanged before economics",
    }
