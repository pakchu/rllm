"""Download availability-aware BTC blockspace/security metrics from Coin Metrics."""
...
METRICS = ("FeeTotNtv", "IssTotNtv", "BlkCnt", "TxCnt", "AssetEODCompletionTime")
OUTPUT_COLUMNS = (
    "observation_date",
    "available_at",
    "FeeTotNtv",
...
def _normalise_row(raw: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        ...
        "BlkCnt": _format_number(_parse_positive_number(raw.get("BlkCnt"), "BlkCnt")),
...
def run(cfg: Config) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "source_semantics": {
            "FeeTotNtv": "sum of transactor-paid fees; excludes new issuance",
            "IssTotNtv": "sum of newly issued native units",
            "BlkCnt": "main-chain blocks created in the interval",
            "TxCnt": "ledger transactions in the interval",
        },
        "excluded_on_purpose": {
            "exchange_flow_metrics": (
                "address-tagging history can change and is not admitted without a "
                "point-in-time tag-vintage archive"
            ),
            "price_or_market_cap_metrics": "preserve a price-independent signal source",
        },
    }
