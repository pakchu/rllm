"""Strict staged economics for the singleton HVCVAROIR-8 policy."""
...
PREREG = Path("results/high_volatility_variance_acceleration_oi_regime_router_preregistration_2026-08-18.json")
PREREG_SHA = "fd6c0149039435ba1c15a780a5d8015c516089986cf6dfa6a5e9dbf81a15ccbc"
SUPPORT = Path("results/high_volatility_variance_acceleration_oi_regime_router_support_2026-08-18.json")
SUPPORT_SHA = "3f469f3b4a9ce1f9014090a1c85ff1daf2a8ff151b6418c0065df86549988e8b"
GROSS9 = Path("results/high_volatility_variance_acceleration_oi_regime_router_gross9_novelty_2026-08-18.json")
GROSS9_SHA = "7726efce31a6b450e50a63378764e7fd823658129c8838c6eab361873440254f"
CLOCK = {
    "path": Path("data/high_volatility_variance_acceleration_oi_regime_router_clocks_2020_2026.csv.gz"),
    "sha256": "722b24201a5dc645b578e20198b258874edd25006abe342be1e7ff1cbdcbea24",
    "rows": 284,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_variance_acceleration_oi_regime_router_{stage}_economics_2026-08-18.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCVAROIR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCVAROIR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCVAROIR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCVAROIR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVAROIR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVAROIR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCVAROIR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCVAROIR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCVAROIR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCVAROIR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCVAROIR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcvaroir_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
