"""Sequential strict economics for frozen HVECC-24."""
...
PREREG = Path("results/high_volatility_ehlers_cyber_cycle_trigger_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "af16d2e8e0749b2cea11d21cf2ffcd63a5166ff8e83911260cd8bebf38edbca8"
SUPPORT = Path("results/high_volatility_ehlers_cyber_cycle_trigger_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "a4884dd2789ccf262774bccdd31d354d6edd8df0d19866cf5236d8b611c683b4"
NOVELTY = Path("results/high_volatility_ehlers_cyber_cycle_trigger_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "527b0e1a0ade7c2e1cf3c74c6dc33e6030c303ee40930ae56c09f375c415018f"
CLOCK = Path("data/high_volatility_ehlers_cyber_cycle_trigger_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "6b33ca984a8b5e1381f47d0ae62c56bcb858bf9d582536b751746ef13b9a2d91"
CONTROL_DIR = Path("data/high_volatility_ehlers_cyber_cycle_trigger_crossover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_ehlers_cyber_cycle_trigger_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ehlers_cyber_cycle_trigger_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "zero_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVECC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVECC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvecc_24_sequential_economics_v1", "policy_id": POLICY_ID,
