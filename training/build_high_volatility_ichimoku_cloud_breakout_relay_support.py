"""Build outcome-blind source support for frozen HVICHI-24."""
...
from training import preregister_high_volatility_ichimoku_cloud_breakout_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "a507172dff88f8a2c5801da2b2e1aa47e5ed899f3a7c7618475127db489593f3"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ichimoku_cloud_breakout_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ichimoku_cloud_breakout_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ichimoku_cloud_breakout_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ichimoku_cloud_breakout_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ichimoku_cloud_breakout_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "tenkan", "kijun", "source_span_a", "source_span_b",
    "visible_span_a", "visible_span_b", "cloud_top", "cloud_bottom", "variation",
    "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "tenkan", "kijun", "source_span_a",
    "source_span_b", "visible_span_a", "visible_span_b", "cloud_top", "cloud_bottom",
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVICHI source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVICHI invalid source key")
...
def ichimoku(high: pd.Series, low: pd.Series, valid: pd.Series) -> pd.DataFrame:
    h=high.where(valid);l=low.where(valid);tenkan=midpoint(h,l,P["tenkan_periods"]);kijun=midpoint(h,l,P["kijun_periods"]);source_a=(tenkan+kijun)/2;source_b=midpoint(h,l,P["span_b_periods"]);shift=P["cloud_displacement_periods"];visible_a=source_a.shift(shift);visible_b=source_b.shift(shift);return pd.DataFrame({"tenkan":tenkan,"kijun":kijun,"source_span_a":source_a,"source_span_b":source_b,"visible_span_a":visible_a,"visible_span_b":visible_b,"cloud_top":pd.concat([visible_a,visible_b],axis=1).max(axis=1),"cloud_bottom":pd.concat([visible_a,visible_b],axis=1).min(axis=1)},index=high.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[["bar_open","bar_high","bar_low","bar_close"]]).all(axis=1)&bars[["bar_open","bar_high","bar_low","bar_close"]].gt(0).all(axis=1);bars=pd.concat([bars,ichimoku(bars.bar_high,bars.bar_low,bars.valid_bar)],axis=1);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["tenkan","kijun","source_span_a","source_span_b","visible_span_a","visible_span_b","cloud_top","cloud_bottom","variation"]]).all(axis=1)&bars.cloud_top.ge(bars.cloud_bottom)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous_close=panel.bar_close.shift(1);long=previous_close.le(panel.cloud_top.shift(1))&panel.bar_close.gt(panel.cloud_top);short=previous_close.ge(panel.cloud_bottom.shift(1))&panel.bar_close.lt(panel.cloud_bottom);panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&(long|short)&panel.variation_rank.ge(P["variation_rank_min"]);return panel.loc[:,PANEL_COLUMNS]


def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);close=pd.to_numeric(used.bar_close,errors="coerce");previous_close=close.shift(1);top=used.cloud_top;bottom=used.cloud_bottom
    if control=="undisplaced_cloud":top=pd.concat([used.source_span_a,used.source_span_b],axis=1).max(axis=1);bottom=pd.concat([used.source_span_a,used.source_span_b],axis=1).min(axis=1)
    long=previous_close.le(top.shift(1))&close.gt(top);short=previous_close.ge(bottom.shift(1))&close.lt(bottom);variation=used.variation_rank.ge(P["variation_rank_min"]);side=pd.Series(np.where(long,1,np.where(short,-1,0)),index=used.index,dtype=int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVICHI prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvichi_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvichi_24_source_support_v1", "policy_id": prereg.POLICY_ID,
