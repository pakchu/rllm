"""Sequential strict economics for frozen HVCRKC-8."""
...
PREREG = Path("results/high_volatility_count_return_kendall_concordance_relay_preregistration_2026-08-13.json")
PREREG_SHA = "44e43bbb1bfbbb26f1983ce9557867fb7cac1819ebf9833a5e4d7ab299041a4f"
SUPPORT = Path("results/high_volatility_count_return_kendall_concordance_relay_support_2026-08-13.json")
SUPPORT_SHA = "620681c40770fee0ce4407e156b42190d7b80653879592f9b0e69a2ab0ad9b0b"
NOVELTY = Path("results/high_volatility_count_return_kendall_concordance_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "695eed16ae1f0f7bd32ecf5cea1ae8ebb1b16279e6b99c9312407a4957079ed7"
CLOCK = Path("data/high_volatility_count_return_kendall_concordance_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "ec9323e26e18f890edd9ec2f3ed6dbe6435cd47da919caef44209a655b5c4a71"
CONTROL_DIR = Path("data/high_volatility_count_return_kendall_concordance_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_count_return_kendall_concordance_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_count_return_kendall_concordance_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_strength_tail", "no_variation_gate", "pearson_instead_of_kendall", "one_decision_stale_tau", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCRKC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCRKC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcrkc_8_sequential_economics_v1", "policy_id": POLICY_ID,
