"""Sequential strict economics for frozen HVSENC-8."""
...
PREREG = Path("results/high_volatility_sample_entropy_collapse_continuation_preregistration_2026-08-13.json")
PREREG_SHA = "3c8e6b15e084895e16ee7c66952ee7ae1cf683d45064f5756889b90bd58789d2"
SUPPORT = Path("results/high_volatility_sample_entropy_collapse_continuation_support_2026-08-13.json")
SUPPORT_SHA = "3e84b5b54d336622bd7728356882c47daca902d8cb8dc37213d81fe412ac3298"
NOVELTY = Path("results/high_volatility_sample_entropy_collapse_continuation_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "58f9737e6b00f8eb778a2274258cb9219e66b844d849f57331a3e71150fa53bb"
CLOCK = Path("data/high_volatility_sample_entropy_collapse_continuation_clocks_2023_2026.csv.gz")
CLOCK_SHA = "3a7a71d53ffc460ea975819194ec102fae155f0b5a2c8cb2866ac07cd510d10f"
CONTROL_DIR = Path("data/high_volatility_sample_entropy_collapse_continuation_controls_2023_2026")
FREEZE = Path("results/high_volatility_sample_entropy_collapse_continuation_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_sample_entropy_collapse_continuation_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_entropy_gate", "no_variation_gate", "sign_entropy_low_tail", "one_decision_stale_entropy", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSENC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSENC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvsenc_8_sequential_economics_v1", "policy_id": POLICY_ID,
