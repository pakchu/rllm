"""Sequential strict evaluator for frozen IBRD-7.
...
SUPPORT_COMMIT = "015559e"
STATIC_INPUT_SHA256 = {
    "training/preregister_inflation_breadth_release_drift.py": "26c28d7eb0087a6ab5d846b2d470616b387505c835c9e0373dfb735addfc953b",
    "training/inflation_breadth_release_drift_clock.py": "1cbbe8dd5cfa107ab563a45ba0893f73b6c14e12df5589f153fe3d0cac107d28",
    "docs/inflation-breadth-release-drift-preregistration-2026-07-17.md": "20ef180668e48d9f783bad8ac287a05fbcbd2e9c5e46eef575cac5ab8ff5e1d5",
    "results/inflation_breadth_release_drift_preregistration_2026-07-17.json": "53f678ef2cbc6103f394b0976ad6a524433c1a0124d338e9c405a35fedce11b7",
    "results/inflation_breadth_release_drift_preregistered_clock_2026-07-17.csv.gz": "5938d851a17bf4dbd29e8cfecc3053f06bf1c764d11b595081e12ea7ae77a99e",
    "training/build_inflation_breadth_release_drift_support.py": "4f4af6d9e9a2343035bcaba30eca4e580ff7c26a7ecff499d4793fbb4d3ed2f8",
    "results/inflation_breadth_release_drift_support_2026-07-17.json": "9448209420a6e3297f92df6feb7ab129e25eb17b66ffffaa0cba8f1fb2be99b8",
    "results/inflation_breadth_release_drift_clocks_2026-07-17.csv.gz": "0ef88e2e029ade81bb8b64123133e753d0e9ac00fbf9e9b6b91b4647ba703402",
...
SUPPORT_RESULT = Path("results/inflation_breadth_release_drift_support_2026-07-17.json")
CLOCKS = Path("results/inflation_breadth_release_drift_clocks_2026-07-17.csv.gz")
...
EVALUATOR_SOURCE = Path("training/evaluate_inflation_breadth_release_drift.py")
EVALUATOR_FREEZE = Path(
    "results/inflation_breadth_release_drift_evaluator_freeze_2026-07-17.json"
)
STAGE1_OUTPUT = Path(
    "results/inflation_breadth_release_drift_stage1_2020_2022_2026-07-17.json"
)
STAGE1_DOC = Path(
    "docs/inflation-breadth-release-drift-stage1-2020-2022-2026-07-17.md"
)
STAGE2_OUTPUT = Path(
    "results/inflation_breadth_release_drift_stage2_2023_2026-07-17.json"
)
STAGE2_DOC = Path(
    "docs/inflation-breadth-release-drift-stage2-2023-2026-07-17.md"
)
...
MECHANISM_CONTROLS = ("headline_only", "core_only")
...
def _load_json(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"IBRD-7 expected a JSON object: {path}")
    return payload
...
def _verify_registration(registration: dict[str, Any]) -> None:
    ...
    if registration.get("policy_id") != "IBRD-7":
        raise ValueError("IBRD-7 preregistration identity changed")
...
def _verify_evaluation_contract(registration: dict[str, Any]) -> EvaluationConfig:
    ...
    for key in (
        ...
        if asdict(cfg)[key] != policy[key]:
            raise ValueError(f"IBRD-7 strict engine differs on {key}")
    if cfg.cluster_draws != 20_000 or cfg.cluster_seed != 20_260_717:
        raise ValueError("IBRD-7 sign-flip configuration changed")
    if cfg.mdd_denominator_floor != 1e-9:
        raise ValueError("IBRD-7 MDD denominator floor changed")
    ...
    if stage["stage1_gates"] != expected_stage1:
        raise ValueError("IBRD-7 Stage1 gate contract changed")
    if stage["stage2_gates"] != expected_stage2:
        raise ValueError("IBRD-7 Stage2 gate contract changed")
    return cfg
...
def _verify_static_inputs() -> tuple[dict[str, Any], dict[str, Any]]:
    for path, expected in STATIC_INPUT_SHA256.items():
        if _sha256(path) != expected:
            raise ValueError(f"IBRD-7 frozen input changed: {path}")
    ...
    if support.get("manifest_hash") != _canonical_hash(support_core):
        raise ValueError("IBRD-7 support manifest changed")
    if support.get("policy_id") != "IBRD-7":
        raise ValueError("IBRD-7 support identity changed")
    if support.get("outcomes_opened") is not False:
        raise ValueError("IBRD-7 support opened outcomes")
    if support.get("outcome_sources_opened") != []:
        raise ValueError("IBRD-7 support opened outcome sources")
    if support.get("market_or_funding_rows_opened") != 0:
        raise ValueError("IBRD-7 support opened execution rows")
    if support.get("support_passed") is not True:
        raise ValueError("IBRD-7 support failed")
    if support.get("advance_to_stage1_outcomes") is not True:
        raise ValueError("IBRD-7 support did not authorize Stage1")
    if support.get("preregistration_manifest_hash") != registration["manifest_hash"]:
        raise ValueError("IBRD-7 support is not bound to preregistration")
    if support.get("clocks", {}).get("sha256") != STATIC_INPUT_SHA256[str(CLOCKS)]:
        raise ValueError("IBRD-7 support is not bound to the clock ledger")
...
def load_schedules() -> dict[str, pd.DataFrame]:
    ...
    if set(ledger["control"]) != set(ALL_CLOCK_NAMES):
        raise ValueError("IBRD-7 support clock family changed")
    schedules = {
    ...
    for name, schedule in schedules.items():
        if not bool(schedule["side"].isin((-1, 1)).all()):
            raise ValueError(f"IBRD-7 {name} side changed")
        if not bool(
            schedule["entry_time"]
            .eq(schedule["signal_day"] + pd.Timedelta(minutes=5))
            .all()
        ):
            raise ValueError(f"IBRD-7 {name} source delay changed")
        if not bool(
            schedule["exit_time"]
            .eq(schedule["entry_time"] + pd.Timedelta(days=7))
            .all()
        ):
            raise ValueError(f"IBRD-7 {name} hold changed")
        if len(schedule) > 1:
            ...
            if not bool(entries.ge(exits).all()):
                raise ValueError(f"IBRD-7 {name} schedule overlaps")
        if not bool(schedule["exit_time"].lt(STAGE2[1]).all()):
            raise ValueError(f"IBRD-7 {name} clock crosses source horizon")
    return schedules
...
def freeze_evaluator(output_path: str | Path = EVALUATOR_FREEZE) -> dict[str, Any]:
    ...
    if not all(
        ...
        raise ValueError("IBRD-7 evaluator schedule invariant failed")
    core = {
        "protocol_version": "inflation_breadth_release_drift_evaluator_v1",
        "policy_id": "IBRD-7",
...
def verify_evaluator_freeze(path: str | Path = EVALUATOR_FREEZE) -> dict[str, Any]:
    ...
    if report.get("manifest_hash") != _canonical_hash(core):
        raise ValueError("IBRD-7 evaluator freeze manifest mismatch")
    if report.get("evaluator_source_sha256") != _sha256(EVALUATOR_SOURCE):
        raise ValueError("IBRD-7 evaluator source changed after freeze")
    if report.get("opened_windows") != []:
        raise ValueError("IBRD-7 evaluator freeze opened a window")
    if report.get("execution_ohlc_rows_parsed_during_freeze") != 0:
        raise ValueError("IBRD-7 evaluator freeze parsed OHLC")
    if report.get("funding_rows_parsed_during_freeze") != 0:
        raise ValueError("IBRD-7 evaluator freeze parsed funding")
    if report.get("simulation_run_during_freeze") is not False:
        raise ValueError("IBRD-7 evaluator freeze simulated outcomes")
    schedules = load_schedules()
    for name, schedule in schedules.items():
        if report["control_schedules"][name] != _schedule_record(schedule):
            raise ValueError(f"IBRD-7 {name} schedule changed after freeze")
    return report
...
def _verify_execution_manifests() -> None:
    ...
    if market_manifest.get("combined_output") != str(MARKET):
        raise ValueError("IBRD-7 market manifest path changed")
    if market_manifest.get("combined_sha256") != MARKET_DATA_SHA256:
        raise ValueError("IBRD-7 market manifest data hash changed")
    if market_manifest.get("rows") != 420_768:
        raise ValueError("IBRD-7 market manifest row count changed")
    if market_manifest.get("protocol", {}).get("outcomes_opened") is not False:
        raise ValueError("IBRD-7 market manifest opened an outcome")
    funding_manifest = _load_json(FUNDING_MANIFEST)
    funding_data = funding_manifest.get("data", {})
    if funding_data.get("path") != str(FUNDING):
        raise ValueError("IBRD-7 funding manifest path changed")
    if funding_data.get("sha256") != FUNDING_DATA_SHA256:
        raise ValueError("IBRD-7 funding manifest data hash changed")
    if funding_data.get("rows") != 4_383:
        raise ValueError("IBRD-7 funding manifest row count changed")
    if funding_manifest.get("outcomes_opened") is not False:
        raise ValueError("IBRD-7 funding manifest opened an outcome")
    ...
    *,
    ...
    if stage == "stage1_2020_2022":
        gates = {
            ...
            <= 0.10,
            "minimum_20_trades": base["trades"] >= 20,
            ...
            "each_calendar_year_minimum_4_trades": all(
        ...
        gates = {
            ...
            <= 0.20,
            "minimum_6_trades": base["trades"] >= 6,
...
def _build_stage_report(
    *,
    ...
    core = {
        "protocol_version": f"inflation_breadth_release_drift_{stage}_v1",
        "policy_id": "IBRD-7",
        "stage": stage,
...
def _verified_passing_stage1(expected_freeze_hash: str) -> dict[str, Any]:
    if not STAGE1_OUTPUT.exists():
        raise ValueError("IBRD-7 Stage1 has not been run")
    ...
    if stored.get("manifest_hash") != _canonical_hash(core):
        raise ValueError("IBRD-7 stored Stage1 manifest changed")
    if stored.get("evaluator_freeze_manifest_hash") != expected_freeze_hash:
        raise ValueError("IBRD-7 stored Stage1 freeze identity changed")
    if stored.get("evaluator_source_sha256") != _sha256(EVALUATOR_SOURCE):
        raise ValueError("IBRD-7 stored Stage1 evaluator changed")
    if stored.get("gate_passed") is not True:
        raise ValueError("IBRD-7 Stage1 failed; 2023 remains sealed")
    market, funding, diagnostics = load_execution_window(STAGE1)
    ...
    if replayed != stored:
        raise ValueError("IBRD-7 Stage1 does not exactly replay")
    return stored
...
def render_stage_doc(report: dict[str, Any]) -> str:
    ...
    return f"""# IBRD-7 {report['stage']} result — {prereg.AS_OF_DATE}
