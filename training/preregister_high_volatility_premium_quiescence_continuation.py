"""Outcome-blind preregistration for HVPQC-12."""
...
POLICY_ID = "HVPQC-12"
DEFAULT_OUTPUT = Path("results/high_volatility_premium_quiescence_continuation_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_premium_quiescence_continuation_v1",
        ...
        "mechanism": {
            "claim": (
                "A high-variation completed BTC day accompanied by unusually quiet premium-index "
                "path activity indicates spot-led repricing without broad derivatives churn. Follow "
                "the completed BTC direction for twelve hours while derivative participation remains underdeveloped."
            ),
            "side": "strict sign of the completed prior UTC-day BTC return",
            "why_distinct": (
                "HVPQC requires the lower tail of unsigned premium path activity jointly with the "
            ...
            "why_low_gross9_overlap_is_plausible": "one daily low-premium-activity/high-price-variation decision is absent from Gross9 primitives",
        },
        "features": {
            ...
            "premium_quiescence": "premium activity rank<=0.25",
            "high_variation": "BTC realized-variation rank>=0.65",
            "eligible": "source valid, premium quiescence, high BTC variation, and strict nonzero BTC return",
        ...
        "policy": {
            ...
            "premium_activity_rank_max": 0.25, "btc_variation_rank_min": 0.65,
        ...
        "research_boundary": {
            ...
            "low_premium_activity_control_existed": False,
            "exact_quiescence_incidence_or_outcomes_known": False,
        ...
        "stopping_rule": "terminal first failure; no source, activity rank, variation rank, side, clock, hold, subset, threshold, comparator, or control repair",
    }
    return {**core, "manifest_hash": canonical_hash(core)}


def validate(value: Mapping[str, Any]) -> None:
    if value != build(): raise RuntimeError("HVPQC-12 preregistration drift")
    contract = hvcav.build()
    for key in ("stages", "source_support_gates", "gross9_novelty_gates", "economic_gates"):
        if value[key] != contract[key]: raise RuntimeError(f"HVPQC-12 {key} drift")
