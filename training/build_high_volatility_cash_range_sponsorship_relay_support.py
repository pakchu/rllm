"""Build outcome-blind source support for frozen HVCRS-8."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_cash_range_sponsorship_relay as prereg
ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2023-05-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="40f49daffc1346d51c86f8f51aa2f5391d9e3f5c035883920c49594903414eaa";REG=prereg.build();P=REG["policy"];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG["stages"].items()};GATES=REG["source_support_gates"];CONTROLS=tuple(REG["diagnostic_controls"]["names"]);VENUES=("perpetual","spot")
...
ROOT=Path("data/high_volatility_cash_range_sponsorship_relay_sources_2023_2026");PANEL=ROOT/"five_minute_barrier_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_cash_range_sponsorship_relay_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_cash_range_sponsorship_relay_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_cash_range_sponsorship_relay_controls_2023_2026");RESULT=Path("results/high_volatility_cash_range_sponsorship_relay_support_2026-08-13.json");BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLS=("decision_time","feature_available_time","source_valid","range_side","cash_range_excess","range_rank","perpetual_range_excess","perpetual_range_rank","perpetual_return","spot_return","btc_realized_variation","variation_rank","variation_active","eligible")
CLOCK_COLS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","cash_range_excess","range_rank","btc_realized_variation","variation_rank")
...
def prepare(raw:pd.DataFrame)->pd.DataFrame:
 ...
 if raw.columns.tolist()!=expected:raise RuntimeError("HVCRS source schema drift")
 ...
 if x.date.isna().any() or x.duplicated(["date","venue"]).any() or not x.venue.isin(("perpetual","spot")).all():raise RuntimeError("HVCRS invalid source key")
...
def confirmed_side(perpetual_return:pd.Series,spot_return:pd.Series,tail:pd.Series)->pd.Series:
 relation=perpetual_return.mul(spot_return);eligible=tail&relation.gt(0)&spot_return.ne(0);side=pd.Series(0,index=spot_return.index,dtype=int);side.loc[eligible]=np.sign(spot_return.loc[eligible]).astype(int);return side
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 x=prepare(raw);grid=pd.date_range(START,END,freq="5min",inclusive="left");op=x.pivot(index="date",columns="venue",values="open").reindex(index=grid,columns=VENUES);high=x.pivot(index="date",columns="venue",values="high").reindex(index=grid,columns=VENUES);low=x.pivot(index="date",columns="venue",values="low").reindex(index=grid,columns=VENUES);close=x.pivot(index="date",columns="venue",values="close").reindex(index=grid,columns=VENUES);sq=x.pivot(index="date",columns="venue",values="minute_squared_return").reindex(index=grid,columns=VENUES);valid=x.pivot(index="date",columns="venue",values="row_valid").reindex(index=grid,columns=VENUES).eq(True);joint=valid.all(axis=1);pret=np.log(close.perpetual/op.perpetual);sret=np.log(close.spot/op.spot);cash=np.log(high.spot/low.spot)-np.log(high.perpetual/low.perpetual);perp=-cash;rank=strict_prior_midrank(cash.where(joint),P["history_decisions"],P["minimum_history_decisions"]);prank=strict_prior_midrank(perp.where(joint),P["history_decisions"],P["minimum_history_decisions"]);side=confirmed_side(pret,sret,rank.ge(P["range_rank_min"]));variation=np.sqrt(sq.perpetual.where(valid.perpetual).rolling(P["variation_bars"],min_periods=P["variation_bars"]).sum());vrank=strict_prior_midrank(variation,P["history_decisions"],P["minimum_history_decisions"]);source_valid=joint&rank.notna()&prank.notna()&variation.notna()&vrank.notna();vactive=vrank.ge(P["variation_rank_min"]);state=side.ne(0);prior_valid=source_valid.shift(1,fill_value=False);eligible=source_valid&prior_valid&state&~state.shift(1,fill_value=False)&vactive
 return pd.DataFrame({"decision_time":grid+pd.Timedelta("5m"),"feature_available_time":grid+pd.Timedelta("5m"),"source_valid":source_valid.to_numpy(bool),"range_side":side.to_numpy(int),"cash_range_excess":cash.to_numpy(float),"range_rank":rank.to_numpy(float),"perpetual_range_excess":perp.to_numpy(float),"perpetual_range_rank":prank.to_numpy(float),"perpetual_return":pret.to_numpy(float),"spot_return":sret.to_numpy(float),"btc_realized_variation":variation.to_numpy(float),"variation_rank":vrank.to_numpy(float),"variation_active":vactive.fillna(False).to_numpy(bool),"eligible":eligible.to_numpy(bool)},columns=PANEL_COLS)
def active(panel:pd.DataFrame,control="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 u=panel.copy();side=pd.to_numeric(u.range_side,errors="coerce").fillna(0).astype(int);state=side.ne(0);prior_valid=u.source_valid.shift(1,fill_value=False);active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)&u.variation_active.eq(True)
 if control=="no_variation_gate":active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)
 elif control in ("rank_0_99","perpetual_range_sponsorship"):
  tail=u.range_rank.ge(.99) if control=="rank_0_99" else u.perpetual_range_rank.ge(P["range_rank_min"]);side=confirmed_side(u.perpetual_return,u.spot_return,tail);state=side.ne(0);active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)&u.variation_active.eq(True)
 elif control=="one_bar_stale_range":
  cols=["range_side","cash_range_excess","range_rank","feature_available_time"];u[cols]=panel[cols].shift(1);side=pd.to_numeric(u.range_side,errors="coerce").fillna(0).astype(int);state=side.ne(0);active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)&u.variation_active.eq(True)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCRS prereg drift")
