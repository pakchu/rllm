"""Sequential strict economics for frozen HVKRMR-24."""
...
PREREG = Path("results/high_volatility_kurtosis_regime_momentum_relay_preregistration_2026-08-09.json")
PREREG_SHA = "4f1cdafce69f24cf785eaf8e1ce2e6e0ff2ada97654a3a4e426dea7c963e4d56"
SUPPORT = Path("results/high_volatility_kurtosis_regime_momentum_relay_support_2026-08-09.json")
SUPPORT_SHA = "8d41f3b75e4d0f3f1a8b7923f737db8f53082db15d141359f9dec5734fb3588a"
NOVELTY = Path("results/high_volatility_kurtosis_regime_momentum_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "e64e45079681b30bc32f0483cac6115fe10bcb079ad00e757ab6acf92991443a"
CLOCK = Path("data/high_volatility_kurtosis_regime_momentum_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "cc4a22f488f5bea029cb2ccdf8699dac5c68831f2d2134b594916e7421ca66a3"
CONTROL_DIR = Path("data/high_volatility_kurtosis_regime_momentum_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_kurtosis_regime_momentum_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_kurtosis_regime_momentum_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_kurtosis_tail", "no_variation_gate", "one_day_stale_moments", "direction_flip", "same_clock_forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVKRMR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVKRMR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvkrmr_24_sequential_economics_v1", "policy_id": POLICY_ID,
