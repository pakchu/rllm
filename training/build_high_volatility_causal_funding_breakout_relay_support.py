"""Deterministic outcome-blind source support for HVCFBR-24."""
...
from training import preregister_high_volatility_causal_funding_breakout_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "8a9e61fca5e1ac75c10b83c80d9bc2235c108fe705704c51feca1f2c88afc86f"
...
ROOT = Path("data/high_volatility_causal_funding_breakout_relay_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_funding_breakout_relay_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_funding_breakout_relay_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_causal_funding_breakout_relay_support_2026-08-16.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "funding_rate",
    "absolute_level_rank", "last_close", "prior_range_high", "prior_range_low",
    "breakout_side",
    "realized_variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "decision_time", "feature_available_time",
    "entry_time", "exit_time", "side", "funding_rate", "last_close",
    "prior_range_high", "prior_range_low",
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if funding.columns.tolist() != ["funding_time", "funding_rate"]:
        raise RuntimeError("HVCFBR-24 funding schema drift")
    expected_bars = [
    ...
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCFBR-24 bar schema drift")
    ...
    if funding["funding_time"].duplicated().any() or not np.isfinite(funding["funding_rate"]).all():
        raise RuntimeError("HVCFBR-24 invalid funding rows")
    ...
    if bars["decision_time"].duplicated().any():
        raise RuntimeError("HVCFBR-24 duplicate eight-hour bar")
    ...
    bars["breakout_side"] = np.select(
        [bars["last_close"].gt(bars["prior_range_high"]), bars["last_close"].lt(bars["prior_range_low"])],
        [1, -1],
        default=0,
    )
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel = panel.join(
        bars[["bar_valid", "prior_range_valid", "last_close", "prior_range_high", "prior_range_low", "breakout_side", "realized_variation"]],
        on="funding_time",
    )
    panel["source_valid"] = (
        ...
        & np.isfinite(
            panel[["funding_rate", "realized_variation", "last_close", "prior_range_high", "prior_range_low"]]
        ).all(axis=1)
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCFBR-24 side or availability drift")
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCFBR-24 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcfbr_24_sources_v1",
    ...
    core = {
        "protocol_version": "hvcfbr_24_source_support_v1", "policy_id": prereg.POLICY_ID,
