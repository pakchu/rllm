"""Sequential strict economics for frozen HVAC-24."""
...
PREREG = Path("results/high_volatility_accelerator_oscillator_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "e76d2c06c148cc8bef7af2a2e01046595c9f641bea0969036fc413b9a282224b"
SUPPORT = Path("results/high_volatility_accelerator_oscillator_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "2bdb747d115e4583b6eedee05c6a19436ac9218dbc81422bab043bc60531eb33"
NOVELTY = Path("results/high_volatility_accelerator_oscillator_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "ba6269924c31669e0eef7567f521797ff4d820e6c87a270b9fcc5c8e0bd5ca74"
CLOCK = Path("data/high_volatility_accelerator_oscillator_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "b6ce1bd1b736488321fd1d6c6db7b62104a26a446786810ae7af085422d90fb6"
CONTROL_DIR = Path("data/high_volatility_accelerator_oscillator_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_accelerator_oscillator_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_accelerator_oscillator_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "awesome_oscillator_zero_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVAC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVAC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvac_24_sequential_economics_v1", "policy_id": POLICY_ID,
