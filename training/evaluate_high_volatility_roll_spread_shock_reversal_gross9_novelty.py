"""Evaluate frozen HVRSSR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVRSSR-8"
PROTOCOL = "hvrssr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_roll_spread_shock_reversal_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVRSSR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVRSSR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVRSSR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVRSSR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvrssr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVRSSR-8 unchanged before economic outcomes",
    }
