"""Sequential strict economics for frozen HVVAR-8."""
...
PREREG = Path("results/high_volatility_value_area_rejection_relay_preregistration_2026-08-13.json")
PREREG_SHA = "ada048a779ca82c0b887c143b1f232bc5d68b008b7bf7fd9688683659fe169b3"
SUPPORT = Path("results/high_volatility_value_area_rejection_relay_support_2026-08-13.json")
SUPPORT_SHA = "84fdc25d3284495ae44efebd92798486f4e54f81f74c510f6dda1e2351af9734"
NOVELTY = Path("results/high_volatility_value_area_rejection_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "69ce9e93bd853abfd52e36621e27b60cc530a2c5ac6e9e6c558e226ad3d72a3f"
CLOCK = Path("data/high_volatility_value_area_rejection_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "4080e24e99f0871bd2cfded81bd37518f3354af16a490558d5551fbf9a2e708d"
CONTROL_DIR = Path("data/high_volatility_value_area_rejection_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_value_area_rejection_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_value_area_rejection_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVVAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVVAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvvar_8_sequential_economics_v1", "policy_id": POLICY_ID,
