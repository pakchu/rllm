"""Evaluate frozen CFTCMSPS-168 structural novelty against every Gross9 sleeve."""
...
POLICY = "CFTCMSPS-168"
PROTOCOL = "cftcmsps_168_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/cftc_micro_standard_positioning_segmentation_relay_gross9_novelty_2026-08-11.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CFTCMSPS predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("CFTCMSPS novelty limits drift")
    if (
        ...
        raise RuntimeError("CFTCMSPS predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CFTCMSPS primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject CFTCMSPS-168 unchanged before economic outcomes"
        ),
    }
