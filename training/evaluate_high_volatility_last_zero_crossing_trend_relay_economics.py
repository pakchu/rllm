"""Sequential strict economics for frozen HVLZCTR-6."""
...
PREREG = Path("results/high_volatility_last_zero_crossing_trend_relay_preregistration_2026-08-11.json")
PREREG_SHA = "93d66de2501271297ad9f27a5dfdb2324b7e6fc45ac5d95abc596d2554ee101a"
SUPPORT = Path("results/high_volatility_last_zero_crossing_trend_relay_support_2026-08-11.json")
SUPPORT_SHA = "cae682a693df16ccaa2c950e5743d349cb271708f692b60e2be77cbef48da2a2"
NOVELTY = Path("results/high_volatility_last_zero_crossing_trend_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "532c9d0492e82252535d13f04b6172b5818e1d143f789dbdf1301a23cb94d60d"
CLOCK = Path("data/high_volatility_last_zero_crossing_trend_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "5276a8cb8d5c658f118ac999d02cfef611f7789f1f5c5f40f83b1a9c8725b66d"
CONTROL_DIR = Path("data/high_volatility_last_zero_crossing_trend_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_last_zero_crossing_trend_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_last_zero_crossing_trend_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = (
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVLZCTR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVLZCTR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvlzctr_6_sequential_economics_v1", "policy_id": POLICY_ID,
