"""Build outcome-blind source support for frozen HVASVC-24.
...
and never opens an entry/exit price, post-entry return, PnL, funding, or
comparison set.
"""
...
POLICY_ID = "HVASVC-24"
PREREG_SHA256 = "6aaa76c421704f424739e2222b197f87088d88e091161a3a7a15a4aa5efbc0bc"
PREREG_MANIFEST_HASH = "160fc68391e77a13c46c704fb8b7d2d7c9dba1e1a01c4ac0a038ff58021f2f19"
...
BTC_QUERY_END = SOURCE_END + pd.Timedelta(hours=12, minutes=1)
...
SOURCE_DIR = Path("data/high_volatility_aggregate_spot_volume_confirmation_relay_sources_2022_2026")
DAILY_SOURCE = SOURCE_DIR / "coinmetrics_btc_daily.csv.gz"
FEATURE_PANEL = SOURCE_DIR / "daily_feature_panel.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_aggregate_spot_volume_confirmation_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_aggregate_spot_volume_confirmation_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_aggregate_spot_volume_confirmation_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_aggregate_spot_volume_confirmation_relay_support_2026-08-13.json")

SOURCE_COLUMNS = (
    "observation_time", "feature_available_time", "volume_reported_spot_usd_1d",
)
PANEL_COLUMNS = (
    "observation_time", "feature_available_time", "decision_time", "source_valid",
    "minute_count", "volume_reported_spot_usd_1d", "btc_return", "volume_rank",
    "btc_variation", "btc_variation_rank",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "observation_time", "feature_available_time",
    "decision_time", "entry_time", "exit_time", "side", "volume_reported_spot_usd_1d",
    "btc_return", "volume_rank", "btc_variation", "btc_variation_rank",
)
...
def parse_source_row(raw: Any) -> dict[str, Any]:
    ...
    volume = _positive_decimal(
        raw["volume_reported_spot_usd_1d"], "volume_reported_spot_usd_1d"
    )
    return {
        "observation_time": observation,
        "feature_available_time": available,
        "volume_reported_spot_usd_1d": str(volume),
    }
...
def build_panel(source: pd.DataFrame, btc_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    daily["volume_reported_spot_usd_1d"] = pd.to_numeric(daily.volume_reported_spot_usd_1d, errors="coerce")
    expected = pd.date_range(SOURCE_START, SOURCE_END, freq="1d", inclusive="left")
    if (
    ...
    value_valid = np.isfinite(daily.volume_reported_spot_usd_1d) & daily.volume_reported_spot_usd_1d.gt(0)
    daily_row_valid = completion_valid & value_valid

    btc = prepare_btc_bars(btc_raw)
    btc_return: list[float] = []
    ...
    for at in decision:
        if pd.isna(at):
            ...
            btc_return.append(math.nan)
            variation.append(math.nan)
            btc_valid.append(False)
            continue
        ...
        decision_open = btc.open.get(at, math.nan)
        good = len(window) == 1440 and count == 1440 and math.isfinite(decision_open) and decision_open > 0
        completed_return = math.nan
        completed_variation = math.nan
        if good:
            completed_return = float(math.log(decision_open / float(window.open.iloc[0])))
            log_returns = np.log(window.close.to_numpy(float) / window.open.to_numpy(float))
            completed_variation = float(np.sqrt(np.square(log_returns).sum()))
            good = (
                math.isfinite(completed_return)
                and completed_return != 0
                and math.isfinite(completed_variation)
                and completed_variation > 0
            )
        minute_count.append(count)
        btc_return.append(completed_return)
        variation.append(completed_variation)
        btc_valid.append(good)

    base_valid = daily_row_valid & pd.Series(btc_valid, index=daily.index)
    panel = pd.DataFrame({
        ...
        "volume_reported_spot_usd_1d": daily.volume_reported_spot_usd_1d.astype(float),
        "btc_return": btc_return,
        "btc_variation": variation,
    })
    panel["volume_rank"] = strict_prior_midrank(
        panel.volume_reported_spot_usd_1d.where(panel.source_valid)
    )
...
def active(panel: pd.DataFrame, control: str = "primary") -> tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    used = panel.copy()
    signal = used.btc_return
    volume_value = used.volume_reported_spot_usd_1d
    volume_rank = used.volume_rank
    available = used.feature_available_time
    valid = used.source_valid.eq(True)
    if control == "one_day_stale_volume":
        volume_value = used.volume_reported_spot_usd_1d.shift(1)
        volume_rank = used.volume_rank.shift(1)
        available = used.feature_available_time.shift(1)
        valid = valid & volume_rank.notna()
    volume = (
        pd.Series(True, index=panel.index)
        if control == "no_volume_tail"
        else volume_rank.ge(0.75)
    )
    volatility = (
        pd.Series(True, index=panel.index)
    ...
    eligible = valid & signal.notna() & signal.ne(0) & volume & volatility
    side = pd.Series(np.where(signal.gt(0), 1, -1), index=panel.index, dtype=int)
    if control == "direction_flip":
        side = -side
    elif control == "same_clock_forced_long":
        side = pd.Series(1, index=panel.index, dtype=int)
    adjusted = used.copy()
    adjusted["volume_reported_spot_usd_1d"] = volume_value
    adjusted["volume_rank"] = volume_rank
...
def build_clock(panel: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in panel.index[eligible]:
        ...
        rows.append({
            ...
            "volume_reported_spot_usd_1d": float(
                used.at[index, "volume_reported_spot_usd_1d"]
            ),
            "btc_return": float(used.at[index, "btc_return"]),
            "volume_rank": float(used.at[index, "volume_rank"]),
...
def run(*, fetch: Fetch | None = None) -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA256:
        raise RuntimeError("HVASVC frozen preregistration artifact drift")
    registration = json.loads(prereg.DEFAULT_OUTPUT.read_text(encoding="utf-8"))
    if (
        registration != prereg.build()
        or registration.get("manifest_hash") != PREREG_MANIFEST_HASH
    ):
        raise RuntimeError("HVASVC frozen preregistration payload drift")
    ...
    source_core = {
        "protocol_version": "hvasvc_24_source_materialization_v1",
    ...
    core = {
        "protocol_version": "hvasvc_24_source_support_v1",
        "policy_id": POLICY_ID,
        "preregistration": {"path": str(prereg.DEFAULT_OUTPUT), "sha256": PREREG_SHA256, "manifest_hash": registration["manifest_hash"]},
