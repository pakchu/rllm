"""Sequential strict economics for frozen HVAVC-8."""
...
PREREG = Path("results/high_volatility_cumulative_vwap_recross_relay_preregistration_2026-08-10.json")
PREREG_SHA = "9211f953469484a3788660bb5e7746707ee610747f9664a930243574e5a7dedb"
SUPPORT = Path("results/high_volatility_cumulative_vwap_recross_relay_support_2026-08-10.json")
SUPPORT_SHA = "3f70a5a3f3d67a14217b1cb8e1ed5276aa42f72b005554ad0e23077e8258f611"
NOVELTY = Path("results/high_volatility_cumulative_vwap_recross_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "3c1a3606c81725fe1c045e26b770e2345c30b9d96e56aca3f95ab4711e256542"
CLOCK = Path("data/high_volatility_cumulative_vwap_recross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "8d7ef04f5a7dd2a5efb56da7a12bac8346c85fc98e86607113d69252d1e29017"
CONTROL_DIR = Path("data/high_volatility_cumulative_vwap_recross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_cumulative_vwap_recross_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cumulative_vwap_recross_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_recross_tail", "no_variation_gate", "raw_recross_at_most_eight", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVAVC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVAVC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvavc_8_sequential_economics_v1", "policy_id": POLICY_ID,
