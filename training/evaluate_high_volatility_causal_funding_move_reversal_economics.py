"""Strict staged economics for the singleton HVCFMR-8 policy."""
...
PREREG = Path("results/high_volatility_causal_funding_move_reversal_preregistration_2026-08-16.json")
PREREG_SHA = "72aab4d713302a46423a38cb6c2fc9a3d7ea1a6ba1a08a4f3272a7b502414e18"
SUPPORT = Path("results/high_volatility_causal_funding_move_reversal_support_2026-08-16.json")
SUPPORT_SHA = "6e872f1efec0c813983dfc5c19bd552328a9690d3df54ef0c359c9263ac8e74b"
GROSS9 = Path("results/high_volatility_causal_funding_move_reversal_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "e9adf6fe694aba8454eaa6d8ce9a777fe979ffe2b7cef7e5f7f8c260d8a55503"
CLOCK = {
    "path": Path("data/high_volatility_causal_funding_move_reversal_clocks_2023_2026.csv.gz"),
    "sha256": "c5f3b6dcc5ad170d37bf7a41a5fb1c8d76d48322da8ec1f5688b6e25790e6bcd",
    "rows": 250,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_funding_move_reversal_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCFMR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCFMR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCFMR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCFMR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCFMR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCFMR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCFMR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCFMR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCFMR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCFMR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCFMR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcfmr_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
