...
def features()->pd.DataFrame:
 if sha(FLOW_PANEL)!=FLOW_SHA or sha(FLOW_MANIFEST)!=FLOW_MANIFEST_SHA:raise RuntimeError("DAFCAR flow source drift")
...
def conditions(f:pd.DataFrame,control:str)->tuple[pd.Series,pd.Series]:
 ...
 saturation=np.sign(flow).ne(np.sign(f.price_return))&flow.ne(0)&f.price_return.ne(0)
 if control=="flow_direction":side=np.sign(flow)
 else:side=-np.sign(f.price_return) if control=="price_reversal" else np.sign(f.price_return)
