"""Evaluate frozen HVCAVAC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCAVAC-8"
PROTOCOL = "hvcavac_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_cross_alt_volume_weighted_auction_consensus_relay_preregistration_2026-08-13.json")
PREREG_SHA = "4624e40689177633a7bd9f188636bf903a4c3cf3669b23643e2e33cb1fd5f321"
SUPPORT = Path("results/high_volatility_cross_alt_volume_weighted_auction_consensus_relay_support_2026-08-13.json")
SUPPORT_SHA = "81a1631a9c99c8f5c969bc1030349d1535de22a223b5fcf30cc82b2544cfa542"
CLOCK = Path("data/high_volatility_cross_alt_volume_weighted_auction_consensus_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "31fd0e159ca7c6c0a57ec23d9a55c8cf05dde3398cba2087f810198e57239339"
OUTPUT = Path("results/high_volatility_cross_alt_volume_weighted_auction_consensus_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCAVAC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCAVAC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCAVAC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCAVAC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCAVAC-8 unchanged before economic outcomes",
    }
