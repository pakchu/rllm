"""Sequential strict economics for frozen CLCVER-6."""
...
PREREG = Path("results/confirmation_ladder_confirmation_velocity_escalation_relay_preregistration_2026-08-12.json")
PREREG_SHA = "7426104234c6c03ed09689889fd45e609597a579bc547928218dd8e22d0a99b0"
SUPPORT = Path("results/confirmation_ladder_confirmation_velocity_escalation_relay_support_2026-08-12.json")
SUPPORT_SHA = "b54b548b1dc0981936d79b5b362d1ab2d3618acde63ac1ccab2305e29450d0bf"
NOVELTY = Path("results/confirmation_ladder_confirmation_velocity_escalation_relay_gross9_novelty_2026-08-12.json")
NOVELTY_SHA = "317625250fd2bb50c4bd309b817800dbd94d4752a201b26cf7af963a0bf9e63d"
CLOCK = Path("data/confirmation_ladder_confirmation_velocity_escalation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "1bb0921e8e79c0b38ace45bb36f73cfcbf5e046739f527161952fa0b218d9fcd"
CONTROL_DIR = Path("data/confirmation_ladder_confirmation_velocity_escalation_relay_controls_2023_2026")
FREEZE = Path("results/confirmation_ladder_confirmation_velocity_escalation_relay_economic_evaluator_freeze_2026-08-12.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/confirmation_ladder_confirmation_velocity_escalation_relay_{stage}_economics_2026-08-12.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CLCVER frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CLCVER novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "clcver_6_sequential_economics_v1", "policy_id": POLICY_ID,
