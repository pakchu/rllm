"""Build outcome-blind source support for preregistered CVHMR-6."""
...
from training import preregister_cboe_volatility_hour_momentum_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.build_cboe_volatility_surface_regime_crossing_relay_support import strict_prior_midrank

CLOCK = Path("data/cboe_volatility_hour_momentum_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/cboe_volatility_hour_momentum_relay_controls_2023_2026")
RESULT = Path("results/cboe_volatility_hour_momentum_relay_support_2026-08-08.json")
SPLITS = {
...
CONTROLS = ("no_relative_convexity_gate", "no_hour_tail", "vix_level_gate", "one_session_stale_volatility", "direction_flip")
NY = ZoneInfo("America/New_York")
ECONOMIC_OUTCOMES_AUTHORIZED = False
COLUMNS = (
    "candidate", "control", "split", "cboe_observation_date", "next_cboe_source_date",
    "decision_hour_start", "decision_time", "feature_available_time", "entry_time", "exit_time",
    "side", "relative_convexity_rank", "previous_relative_convexity_rank", "vix_level_rank", "hour_return", "prior_abs_hour_q75",
)
...
def load_features() -> pd.DataFrame:
    for raw, expected in prereg.HASHES.items():
        if sha(Path(raw)) != expected:
            raise RuntimeError(f"CVHMR frozen source changed: {raw}")
    ...
    for column in ("VVIX_close", "VIX_close"):
    ...
    if not cboe.observation_date.is_monotonic_increasing or cboe.observation_date.duplicated().any():
        raise RuntimeError("CVHMR Cboe dates invalid")
    values = cboe[["VVIX_close", "VIX_close"]]
    if not np.isfinite(values).all().all() or not values.gt(0).all().all():
        raise RuntimeError("CVHMR term values invalid")
    cboe["relative_convexity_rank"] = strict_prior_midrank(np.log(cboe.VVIX_close / cboe.VIX_close))
    cboe["previous_relative_convexity_rank"] = cboe.relative_convexity_rank.shift(1)
    cboe["vix_level_rank"] = strict_prior_midrank(np.log(cboe.VIX_close))
    ...
    if price.columns.tolist() != ["hour_start", "hour_open", "first_half_close", "second_half_open", "hour_close", "source_rows", "distinct_timestamps", "source_valid", "decision_time"]:
        raise RuntimeError("CVHMR completed-price schema changed")
    ...
    price["prior_abs_hour_q75"] = price.hour_return.abs().where(price.valid).shift(1).rolling(720, min_periods=672).quantile(0.75)
    if price.hour_start.duplicated().any() or not price.hour_start.is_monotonic_increasing:
        raise RuntimeError("CVHMR completed-price clock invalid")
    ...
    for index in range(1, len(cboe) - 1):
        ...
        final_hour = final_hour.tz_convert("UTC")
        if final_hour not in by_start.index:
            continue
        last = by_start.loc[final_hour]
        valid = bool(last.valid and np.isfinite([last.hour_return, last.prior_abs_hour_q75]).all() and last.prior_abs_hour_q75 >= 0)
        rows.append({
            "source_index": index, "observation_date": observation, "next_source_date": next_date,
            "decision_hour_start": final_hour, "decision_time": final_hour + pd.Timedelta(hours=1),
            "relative_convexity_rank": float(cboe.at[index, "relative_convexity_rank"]),
            "previous_relative_convexity_rank": float(cboe.at[index, "previous_relative_convexity_rank"]),
            "vix_level_rank": float(cboe.at[index, "vix_level_rank"]),
            "hour_return": float(last.hour_return) if valid else np.nan,
            "prior_abs_hour_q75": float(last.prior_abs_hour_q75) if valid else np.nan, "valid": valid,
        })
    return pd.DataFrame(rows)


def signal(frame: pd.DataFrame, control: str) -> pd.Series:
    btc, relative, threshold = frame.hour_return, frame.relative_convexity_rank, frame.prior_abs_hour_q75
    valid = frame.valid & btc.ne(0) & relative.notna() & threshold.notna()
    tail = btc.abs().ge(threshold)
    side = np.sign(btc).astype("Int64").fillna(0).astype(int)
    if control == "no_relative_convexity_gate":
        eligible = valid & tail
    elif control == "no_hour_tail":
        eligible = valid & relative.ge(0.60)
    elif control == "vix_level_gate":
        eligible = valid & tail & frame.vix_level_rank.ge(0.60)
    elif control == "one_session_stale_volatility":
        eligible = valid & tail & frame.previous_relative_convexity_rank.ge(0.60)
    else:
        eligible = valid & relative.ge(0.60) & tail
...
def build_clock(frame: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in frame.index[sides.ne(0)]:
        ...
        rows.append({
            "candidate": "CVHMR-6", "control": control, "split": split,
            ...
            "decision_hour_start": frame.at[index, "decision_hour_start"], "decision_time": decision,
            "feature_available_time": decision, "entry_time": entry, "exit_time": exit_time, "side": int(sides.at[index]),
            "relative_convexity_rank": float(frame.at[index, "relative_convexity_rank"]),
            "previous_relative_convexity_rank": float(frame.at[index, "previous_relative_convexity_rank"]),
            "vix_level_rank": float(frame.at[index, "vix_level_rank"]), "hour_return": float(frame.at[index, "hour_return"]),
            "prior_abs_hour_q75": float(frame.at[index, "prior_abs_hour_q75"]),
        })
    result = pd.DataFrame(rows, columns=COLUMNS)
    for column in ("decision_hour_start", "decision_time", "feature_available_time", "entry_time", "exit_time"):
        if not result.empty:
