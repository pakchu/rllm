"""Sequential strict economics for frozen HVWADX-24."""
...
PREREG = Path("results/high_volatility_wilder_adx_trend_relay_preregistration_2026-08-11.json")
PREREG_SHA = "8d6ba61b8b70a1989692082ae278ec6a0e06be57da0bccf71cbfcbb50bcae7d5"
SUPPORT = Path("results/high_volatility_wilder_adx_trend_relay_support_2026-08-11.json")
SUPPORT_SHA = "a3cab4b0045eb5aa7f3c8a71b4134324499c2a581bf716c5ac2a5bb2eda0e8b0"
NOVELTY = Path("results/high_volatility_wilder_adx_trend_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "fa583228e9db01d57583f94ca9ff67b9e4d909168e1f55b5d9ad00572906010d"
CLOCK = Path("data/high_volatility_wilder_adx_trend_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "2c687e0f3ebc929dd85092ad372c4d457cdb6a18875f736b2e4c66379c431a54"
CONTROL_DIR = Path("data/high_volatility_wilder_adx_trend_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_wilder_adx_trend_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_wilder_adx_trend_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_adx_gate", "no_variation_gate", "one_bar_stale_state", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVWADX frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVWADX novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvadx_24_sequential_economics_v1", "policy_id": POLICY_ID,
