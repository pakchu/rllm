"""Materialize source-only SHRR-12 support clocks."""
...
PREREG_SHA="edd262b953e0e579ad38cfe4e2a623b176924c9f25afc8c779ce68cb8dae6d10"
SOURCE_DIR=Path("data/scandinavian_haven_risk_rotation_relay_sources_2023_2026"); SESSION=SOURCE_DIR/"risk_rotation_sessions.csv.gz"; SOURCE_MANIFEST=SOURCE_DIR/"manifest.json"
...
CLOCK=Path("data/scandinavian_haven_risk_rotation_relay_clocks_2023_2026.csv.gz"); CONTROL_DIR=Path("data/scandinavian_haven_risk_rotation_relay_controls_2023_2026"); RESULT=Path("results/scandinavian_haven_risk_rotation_relay_support_2026-08-09.json")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00:00Z"),pd.Timestamp("2024-01-01T00:00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00:00Z"),pd.Timestamp("2025-01-01T00:00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00:00Z"),pd.Timestamp("2026-01-01T00:00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00:00Z"),pd.Timestamp("2026-08-01T00:00:00Z"))}; MINIMUM={"train":8,"test":12,"eval":12,"final":8}; CONTROLS=("no_volatility_gate","no_risk_pressure_tail","no_haven_orientation","one_session_stale_risk_pressure","direction_flip")
SYMBOLS=("USDSEK","USDCHF","USDJPY"); RISK_ORIENTATION={"USDSEK":1.,"USDCHF":-1.,"USDJPY":-1.}
COLUMNS=("candidate","control","split","source_day","decision_time","feature_available_time","entry_time","exit_time","side","risk_pressure","risk_pressure_rank","unoriented_pressure","unoriented_pressure_rank","btc_realized_variation","btc_realized_variation_rank")
...
def materialize_sessions()->dict:
 ...
 frame=frame.sort_values("source_day").reset_index(drop=True);valid_columns=[f"{symbol}_valid" for symbol in SYMBOLS];z_columns=[f"{symbol}_z" for symbol in SYMBOLS];frame["source_valid"]=frame[valid_columns].eq(True).all(axis=1);z=frame[z_columns].astype(float);oriented=z.rename(columns={f"{s}_z":s for s in SYMBOLS}).mul(pd.Series(RISK_ORIENTATION));frame["risk_pressure"]=oriented.median(axis=1).where(frame.source_valid);frame["risk_pressure_rank"]=strict_prior_midrank(frame.risk_pressure.abs().where(frame.source_valid));frame["unoriented_pressure"]=z.median(axis=1).where(frame.source_valid);frame["unoriented_pressure_rank"]=strict_prior_midrank(frame.unoriented_pressure.abs().where(frame.source_valid));frame["decision_time"]=frame.source_day+pd.Timedelta(hours=16)
 SOURCE_DIR.mkdir(parents=True,exist_ok=True);_write_gzip_csv(frame,SESSION);core={"protocol_version":"shrr_12_risk_rotation_source_v1","query":QUERY,"table":"bars_polygon","symbols":list(SYMBOLS),"risk_orientation":RISK_ORIENTATION,"interval":"1m","window":[START.isoformat(),END.isoformat()],"outcomes_opened":False,"candidate_incidence_opened":False,"no_imputation":True,"output":{"path":str(SESSION),"sha256":sha(SESSION),"rows":len(frame),"valid_rows":int(frame.source_valid.sum())}};payload={**core,"manifest_hash":chash(core)};SOURCE_MANIFEST.write_text(json.dumps(payload,indent=2,allow_nan=False)+"\n");return payload
def features()->pd.DataFrame:
 if sha(PRICE)!=PRICE_SHA or sha(PRICE_MANIFEST)!=PRICE_MANIFEST_SHA:raise RuntimeError("SHRR BTC source drift")
 ...
 for column in ("risk_pressure","risk_pressure_rank","unoriented_pressure","unoriented_pressure_rank"):f[column]=pd.to_numeric(f[column],errors="coerce")
 ...
 d=p[["decision_time","btc_realized_variation","btc_valid"]];f=f.merge(d,on="decision_time",how="left",validate="one_to_one");f["btc_realized_variation_rank"]=strict_prior_midrank(f.btc_realized_variation.where(f.btc_valid));f["signal_valid"]=f.source_valid&f.btc_valid&np.isfinite(f[["risk_pressure","risk_pressure_rank","unoriented_pressure","unoriented_pressure_rank","btc_realized_variation","btc_realized_variation_rank"]]).all(axis=1)&f.risk_pressure.ne(0);return f
def conditions(f:pd.DataFrame,control:str)->tuple[pd.Series,pd.Series]:
 pressure=f.unoriented_pressure if control=="no_haven_orientation" else f.risk_pressure;rank=f.unoriented_pressure_rank if control=="no_haven_orientation" else f.risk_pressure_rank
 if control=="one_session_stale_risk_pressure":pressure=pressure.shift(1);rank=rank.shift(1)
 tail=pd.Series(True,index=f.index) if control=="no_risk_pressure_tail" else rank.ge(.70);vol=pd.Series(True,index=f.index) if control=="no_volatility_gate" else f.btc_realized_variation_rank.ge(.65);active=f.signal_valid&np.isfinite(pressure)&pressure.ne(0)&tail&vol;side=-np.sign(pressure);side=-side if control=="direction_flip" else side;return active,side
def clock(f:pd.DataFrame,control:str="primary")->pd.DataFrame:
 ...
 for i in f.index[active]:
  ...
  next_allowed=exit_;rows.append({"candidate":"SHRR-12","control":control,"split":split,"source_day":f.at[i,"source_day"],"decision_time":decision,"feature_available_time":decision,"entry_time":entry,"exit_time":exit_,"side":int(side.at[i]),"risk_pressure":float(f.at[i,"risk_pressure"]),"risk_pressure_rank":float(f.at[i,"risk_pressure_rank"]),"unoriented_pressure":float(f.at[i,"unoriented_pressure"]),"unoriented_pressure_rank":float(f.at[i,"unoriented_pressure_rank"]),"btc_realized_variation":float(f.at[i,"btc_realized_variation"]),"btc_realized_variation_rank":float(f.at[i,"btc_realized_variation_rank"])})
...
def run()->dict:
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("SHRR preregistration hash drift")
