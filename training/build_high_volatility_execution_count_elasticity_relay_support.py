"""Source-only support gate for frozen HVECE-8."""
...
from training import preregister_high_volatility_execution_count_elasticity_relay as prereg
...
PREREG_SHA = "62094724e9d4bcb58ec3f8582cb3ce0e296debbbbfc04bb04dae2332b5ad7d47"
SPLITS = {
...
ROOT = Path("data/high_volatility_execution_count_elasticity_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_execution_count_elasticity_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_execution_count_elasticity_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_execution_count_elasticity_relay_support_2026-08-12.json")
...
def execution_elasticity(block_return: float, counts: np.ndarray) -> tuple[float, float]:
    values = np.asarray(counts, dtype=float)
    total = float(values.sum())
    if len(values) != 480 or not np.isfinite(values).all() or (values < 0).any() or not np.equal(values, np.floor(values)).all() or total <= 0 or not math.isfinite(block_return) or block_return == 0:
        return math.nan, math.nan
    return total, float(abs(block_return) / math.sqrt(total))
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVECE source timestamps")
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        if valid:
            ...
            execution_count, elasticity = execution_elasticity(block_return, window["number_of_trades"].to_numpy(float))
            valid = variation > 0 and math.isfinite(elasticity) and execution_count > 0
        if not valid:
            variation = execution_count = elasticity = block_return = math.nan
        rows.append(
            {
                ...
                "execution_elasticity": elasticity,
                "block_return": block_return,
            }
        )
    states = pd.DataFrame(rows)
    states["variation_rank"] = prior_rank(states["realized_variation"].where(states["source_valid"]))
    states["elasticity_rank"] = prior_rank(states["execution_elasticity"].where(states["source_valid"]))
    states["return_rank"] = prior_rank(states["block_return"].abs().where(states["source_valid"]))
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvece_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    elasticity_rank = states["elasticity_rank"]
    elasticity = states["execution_elasticity"]
    return_rank = states["return_rank"]
    if control == "one_block_stale_geometry":
        block_return, variation_rank, elasticity_rank, elasticity, return_rank = [
            value.shift(1) for value in (block_return, variation_rank, elasticity_rank, elasticity, return_rank)
        ]
    variation_gate = (
    ...
    if control == "no_elasticity_tail":
        elasticity_gate = pd.Series(True, index=states.index)
    elif control == "raw_return_tail":
        elasticity_gate = return_rank.ge(0.80)
    else:
        elasticity_gate = elasticity_rank.ge(0.80)
    eligible = states["source_valid"] & variation_gate & elasticity_gate & block_return.ne(0)
        ...
        rows.append(
            {
                ...
                "execution_elasticity": float(states.at[index, "execution_elasticity"]),
                "elasticity_rank": float(states.at[index, "elasticity_rank"]),
            }
        )
    columns = [
        "candidate", "control", "split", "decision_time", "feature_available_time",
        "entry_time", "exit_time", "side", "realized_variation", "variation_rank",
        "execution_count", "execution_elasticity", "elasticity_rank",
    ]
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVECE preregistration drift")
    ...
    core = {
        "protocol_version": "hvece_8_source_support_v1",
        "policy_id": "HVECE-8",
        "preregistration": {
