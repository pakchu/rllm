"""Sequential strict economics for frozen HVSPOICR-8."""
...
PREREG = Path("results/high_volatility_spot_perpetual_open_interest_contraction_reversal_preregistration_2026-08-16.json")
PREREG_SHA = "0377144b80042d120754bb9c4add4edf0665914a1086673cba566f00d90d633e"
SUPPORT = Path("results/high_volatility_spot_perpetual_open_interest_contraction_reversal_support_2026-08-16.json")
SUPPORT_SHA = "7ee40fb6e169edf96ccc5bfc7018eb53a1c2376d3935d6af19b4478318de7bed"
NOVELTY = Path("results/high_volatility_spot_perpetual_open_interest_contraction_reversal_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "d99a7863ccda1a666ca8d7175aa56eb215d2ef76eb75d39ff618662fab6ebdbb"
CLOCK = Path("data/high_volatility_spot_perpetual_open_interest_contraction_reversal_clocks_2020_2026.csv.gz")
CLOCK_SHA = "0c67b3d919b7abbe3e1b34003aeab784b98c65682ca08a3e912a93d6bcf3ca07"
CONTROL_DIR = Path("data/high_volatility_spot_perpetual_open_interest_contraction_reversal_controls_2023_2026")
FREEZE = Path("results/high_volatility_spot_perpetual_open_interest_contraction_reversal_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_spot_perpetual_open_interest_contraction_reversal_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSPOICR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSPOICR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvspoicr_8_sequential_economics_v1", "policy_id": POLICY_ID,
