"""Evaluate frozen HVMAMA-C05-005-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVMAMA-C05-005-24"
PROTOCOL = "hvmama_c05_005_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_mesa_adaptive_moving_average_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVMAMA predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVMAMA novelty limits drift")
    if (
        ...
        raise RuntimeError("HVMAMA predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVMAMA primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvmama_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVMAMA-C05-005-24 unchanged before economic outcomes",
    }
