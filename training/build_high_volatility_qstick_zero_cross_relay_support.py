"""Build outcome-blind source support for frozen HVQST-24."""
...
from training import preregister_high_volatility_qstick_zero_cross_relay as prereg
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVQST source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVQST invalid source key")
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["body"]=bars.bar_close-bars.bar_open;bars["qstick"]=causal_sma(bars.body,bars.valid_bar,P["qstick_periods"]);bars["entry_side"]=cross_side(bars.qstick);bars["raw_body_side"]=np.sign(bars.body).fillna(0).astype(int)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["body","qstick","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="raw_body_direction":side=pd.to_numeric(used.raw_body_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVQST prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvqst_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvqst_24_source_support_v1", "policy_id": prereg.POLICY_ID,
