"""Build outcome-blind source support for frozen HVHILBERT-I7-24."""
...
from training import preregister_high_volatility_hilbert_in_phase_zero_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "9df44a09c83785d5b1af2c8aa80d5599b7cd34c9c0054abbeb83dd5797b5c84f"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_hilbert_in_phase_zero_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_hilbert_in_phase_zero_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_hilbert_in_phase_zero_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_hilbert_in_phase_zero_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_hilbert_in_phase_zero_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "detrended", "in_phase", "quadrature", "in_phase_ready",
    "entry_side", "quadrature_cross_side", "component_cross_side", "variation",
    "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "detrended", "in_phase", "quadrature",
    "in_phase_ready", "entry_side", "quadrature_cross_side", "component_cross_side",
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVHILBERT source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVHILBERT invalid source key")
...
def hilbert_components(close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    """Replicate LEAN Hilbert(7,.635,.338) update order and frozen crossings."""
    detrended=np.full(len(close),np.nan);in_phase=np.zeros(len(close),dtype=float);quadrature=np.zeros(len(close),dtype=float)
    ready=np.zeros(len(close),dtype=bool);entry=np.zeros(len(close),dtype=int);quadrature_side=np.zeros(len(close),dtype=int);component_side=np.zeros(len(close),dtype=int)
    closes:list[float]=[];detrends:list[float]=[];in_phases:list[float]=[];quadratures:list[float]=[];prior_i=None;prior_q=None;prior_diff=None
    for index,is_valid in enumerate(valid.to_numpy(bool)):
        if not is_valid:
            closes=[];detrends=[];in_phases=[];quadratures=[];prior_i=prior_q=prior_diff=None;continue
        current=float(close.iloc[index]);closes.append(current)
        if len(closes)>P["length"]:
            value=current-closes[-P["length"]-1];detrended[index]=value;detrends.append(value)
            previous_detrend=detrends[-2] if len(detrends)>=2 else 0.0
            quadrature_delay2=quadratures[-2] if len(quadratures)>=2 else 0.0
            q=previous_detrend-P["quadrature_multiplication_factor"]*value+P["quadrature_multiplication_factor"]*quadrature_delay2
            quadrature[index]=q;quadratures.append(q)
            samples=len(closes);ready[index]=samples>P["length"]+2
            if ready[index]:
                delay4=detrends[-4] if len(detrends)>=4 else 0.0
                delay2=detrends[-2] if len(detrends)>=2 else 0.0
                in_phase_delay3=in_phases[-3] if len(in_phases)>=3 else 0.0
                i=1.25*(delay4-P["in_phase_multiplication_factor"]*delay2)+P["in_phase_multiplication_factor"]*in_phase_delay3
                in_phase[index]=i
            in_phases.append(in_phase[index])
            if ready[index]:
                if prior_i is not None:
                    if i>0 and prior_i<=0:entry[index]=1
                    elif i<0 and prior_i>=0:entry[index]=-1
                if prior_q is not None:
                    if q>0 and prior_q<=0:quadrature_side[index]=1
                    elif q<0 and prior_q>=0:quadrature_side[index]=-1
                difference=i-q
                if prior_diff is not None:
                    if difference>0 and prior_diff<=0:component_side[index]=1
                    elif difference<0 and prior_diff>=0:component_side[index]=-1
                prior_i=i;prior_q=q;prior_diff=difference
    return pd.DataFrame({"detrended":detrended,"in_phase":in_phase,"quadrature":quadrature,"in_phase_ready":ready,"entry_side":entry,"quadrature_cross_side":quadrature_side,"component_cross_side":component_side},index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=bars.join(hilbert_components(bars.bar_close,bars.valid_bar));bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&bars.in_phase_ready&np.isfinite(bars[["detrended","in_phase","quadrature","variation"]]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="quadrature_zero_cross":side=pd.to_numeric(used.quadrature_cross_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
    elif control=="in_phase_quadrature_cross":side=pd.to_numeric(used.component_cross_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def build_clock(panel: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in panel.index[activity]:
        ...
        rows.append(
             ...
             **{column: bool(used.at[index, column]) if column in ("in_phase_ready", "eligible") else float(used.at[index, column])
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVHILBERT prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvhilbert_i7_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvhilbert_i7_24_source_support_v1", "policy_id": prereg.POLICY_ID,
