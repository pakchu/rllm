"""Outcome-blind source-support gate for frozen HVOID-8."""
...
from training import (
    preregister_high_volatility_oi_unwind_directional_dominance_relay as prereg,
)
from training.build_binance_aggtrade_microstructure import _write_gzip_csv


ENV_FILE = "/home/pakchu/rllm/.env"
PREREG_SHA = "eb52de5d1f1f01a9222242e79e4f3d675c71c22ee07a25c144be587f0cf965ee"
...
"""
SOURCE_DIR = Path(
    "data/high_volatility_oi_unwind_directional_dominance_relay_sources_2023_2026"
)
FEATURES = SOURCE_DIR / "eight_hour_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path(
    "data/high_volatility_oi_unwind_directional_dominance_relay_clocks_2023_2026.csv.gz"
)
CONTROL_DIR = Path(
    "data/high_volatility_oi_unwind_directional_dominance_relay_controls_2023_2026"
)
RESULT = Path(
    "results/high_volatility_oi_unwind_directional_dominance_relay_support_2026-08-13.json"
)
FEATURE_COLUMNS = (
    "decision_time",
...
def build_features(bars: pd.DataFrame, oi: pd.DataFrame) -> pd.DataFrame:
    ...
    if not required_bars.issubset(bars) or not required_oi.issubset(oi):
        raise ValueError("HVOID source schema drift")
    prices = bars.copy()
    inventory = oi.copy()
    ...
    for decision in decisions:
        ...
        completed_return = variation = gross_activity = unwind_return = build_return = dominance = math.nan
        unwind_intervals = build_intervals = math.nan
        if valid:
            bar_returns = np.log(
            ...
            unwind_return = float(bar_returns[oi_changes < 0].sum())
            build_return = float(bar_returns[oi_changes > 0].sum())
            unwind_intervals = int(np.count_nonzero(oi_changes < 0))
            build_intervals = int(np.count_nonzero(oi_changes > 0))
            denominator = abs(unwind_return) + abs(build_return)
            dominance = abs(unwind_return) / denominator if denominator > 0 else math.nan
            ...
            valid = bool(
                completed_return != 0 and unwind_return != 0
                and variation > 0
                and gross_activity > 0
                and np.isfinite(
                    [completed_return, variation, gross_activity, unwind_return, build_return, dominance, unwind_intervals, build_intervals]
                ).all()
            )
        if not valid:
            completed_return = variation = gross_activity = unwind_return = build_return = dominance = math.nan
        ...
        rows.append(
            {
                ...
                "unwind_directional_dominance": dominance,
                "unwind_intervals": unwind_intervals,
                "build_intervals": build_intervals,
            }
        )
...
def conditions(
    ...
    used = frame
    valid = used["source_valid"].eq(True) & used["completed_return"].ne(0) & used["unwind_return"].ne(0)
    gross_gate = (
        pd.Series(True, index=frame.index)
        if control == "no_oi_activity_gate"
        else used["gross_oi_activity_rank"].ge(0.60)
    )
    dominance_gate = (
        pd.Series(True, index=frame.index)
        if control == "no_dominance_gate"
        else used["unwind_directional_dominance"].ge(2 / 3)
    )
    variation_gate = (
        pd.Series(True, index=frame.index)
    ...
    direction = np.sign(used["unwind_return"]).fillna(0).astype(int)
    confirmation = direction.eq(np.sign(used["completed_return"]).fillna(0).astype(int))
    eligible = valid & gross_gate & dominance_gate & variation_gate & confirmation
    side = direction
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVOID preregistration drift")
    ...
    source_core = {
        "protocol_version": "hvoid_8_sources_v1",
        "query_sha256": {
    ...
    core = {
        "protocol_version": "hvoid_8_source_support_v1",
