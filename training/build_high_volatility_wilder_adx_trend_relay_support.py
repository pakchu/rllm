"""Build outcome-blind source support for frozen HVWADX-24."""
...
PREREG_SHA = "8d6ba61b8b70a1989692082ae278ec6a0e06be57da0bccf71cbfcbb50bcae7d5"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_wilder_adx_trend_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_wilder_adx_trend_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_wilder_adx_trend_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_wilder_adx_trend_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_wilder_adx_trend_relay_support_2026-08-11.json")
...
PANEL_COLUMNS = (
    ...
    "minus_di", "dx", "adx", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "plus_di", "minus_di", "adx", "variation",
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVWADX source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVWADX invalid source key")
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["source_valid"] = (
        bars.valid_bar & np.isfinite(bars[["plus_di", "minus_di", "adx", "variation"]]).all(axis=1)
        & bars.variation.gt(0)
    )
    ...
    panel["eligible"] = (
        panel.source_valid & panel.adx.ge(P["adx_min"]) & panel.plus_di.ne(panel.minus_di)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    side = pd.Series(np.sign(used.plus_di - used.minus_di), index=used.index).fillna(0).astype(int)
    adx_gate = used.adx.ge(P["adx_min"])
    variation_gate = used.variation_rank.ge(P["variation_rank_min"])
    state = valid & side.ne(0) & adx_gate & variation_gate
    if control == "no_adx_gate":
        state = valid & side.ne(0) & variation_gate
    elif control == "no_variation_gate":
        state = valid & side.ne(0) & adx_gate
    elif control == "one_bar_stale_state":
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVWADX prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvadx_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvadx_24_source_support_v1", "policy_id": prereg.POLICY_ID,
