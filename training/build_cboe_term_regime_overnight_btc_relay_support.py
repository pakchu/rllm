"""Build outcome-blind source support for preregistered CVTBR-6."""
...
from training import preregister_cboe_term_regime_overnight_btc_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.build_cboe_volatility_surface_regime_crossing_relay_support import strict_prior_midrank

CLOCK = Path("data/cboe_term_regime_overnight_btc_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/cboe_term_regime_overnight_btc_relay_controls_2023_2026")
RESULT = Path("results/cboe_term_regime_overnight_btc_relay_support_2026-08-08.json")
SPLITS = {
...
CONTROLS = ("no_term_gate", "front_rank_only", "broad_rank_only", "confirmation_break", "direction_flip")
...
COLUMNS = (
    ...
    "side", "front_rank", "broad_rank", "term_pressure", "overnight_btc_return",
)
...
def load_features() -> pd.DataFrame:
    for raw, expected in prereg.HASHES.items():
        if sha(Path(raw)) != expected:
            raise RuntimeError(f"CVTBR frozen source changed: {raw}")
    cboe = pd.read_csv(prereg.CBOE, compression="gzip", usecols=["observation_date", "VIX9D_close", "VIX_close", "VIX3M_close"])
    cboe["observation_date"] = pd.to_datetime(cboe.observation_date, format="%Y-%m-%d")
    for column in ("VIX9D_close", "VIX_close", "VIX3M_close"):
    ...
    if not cboe.observation_date.is_monotonic_increasing or cboe.observation_date.duplicated().any():
        raise RuntimeError("CVTBR Cboe dates invalid")
    values = cboe[["VIX9D_close", "VIX_close", "VIX3M_close"]]
    if not np.isfinite(values).all().all() or not values.gt(0).all().all():
        raise RuntimeError("CVTBR term values invalid")
    ...
    if price.columns.tolist() != ["hour_start", "hour_open", "first_half_close", "second_half_open", "hour_close", "source_rows", "distinct_timestamps", "source_valid", "decision_time"]:
        raise RuntimeError("CVTBR completed-price schema changed")
    ...
    if price.hour_start.duplicated().any() or not price.hour_start.is_monotonic_increasing:
        raise RuntimeError("CVTBR completed-price clock invalid")
...
def signal(frame: pd.DataFrame, control: str) -> pd.Series:
    btc, term = frame.overnight_btc_return, frame.term_pressure
    ...
    if control == "no_term_gate":
        eligible = frame.valid & btc.ne(0)
    elif control == "front_rank_only":
        eligible = valid & ((frame.front_rank.le(0.40) & btc.gt(0)) | (frame.front_rank.ge(0.60) & btc.lt(0)))
    elif control == "broad_rank_only":
        eligible = valid & ((frame.broad_rank.le(0.40) & btc.gt(0)) | (frame.broad_rank.ge(0.60) & btc.lt(0)))
...
def build_clock(frame: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in frame.index[sides.ne(0)]:
        ...
        rows.append({
            "candidate": "CVTBR-6", "control": control, "split": split,
