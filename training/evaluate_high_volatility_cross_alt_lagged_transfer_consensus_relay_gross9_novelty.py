"""Evaluate frozen HVCALT-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCALT-6"
PROTOCOL = "hvcalt_6_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_cross_alt_lagged_transfer_consensus_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCALT predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCALT novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCALT predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCALT primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCALT-6 unchanged before economic outcomes",
    }
