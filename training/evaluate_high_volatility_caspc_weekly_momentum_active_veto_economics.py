"""Strict staged economics for the singleton HVCASPCWMV-8 policy."""
...
PREREG = Path("results/high_volatility_caspc_weekly_momentum_active_veto_preregistration_2026-08-17.json")
PREREG_SHA = "3bddb2b206bc2a64bc7d8247021e0e530ae7d375883f4355f0e2f46a4f18b96b"
SUPPORT = Path("results/high_volatility_caspc_weekly_momentum_active_veto_support_2026-08-17.json")
SUPPORT_SHA = "315c4b4772d4369176db95a7e80c15708a739c66c0e09e808ee83733269d69ef"
GROSS9 = Path("results/high_volatility_caspc_weekly_momentum_active_veto_gross9_novelty_2026-08-17.json")
GROSS9_SHA = "387ad714ae73795b7c8466b5f25bfc7111fdbcc902b75284b96c893b16b600fa"
CLOCK = {
    "path": Path("data/high_volatility_caspc_weekly_momentum_active_veto_clocks_2023_2026.csv.gz"),
    "sha256": "b535d44ade990216e961f59e1d3a9e830980e369cb536ca54b174f349b945437",
    "rows": 125,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_caspc_weekly_momentum_active_veto_{stage}_economics_2026-08-17.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCASPCWMV-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCASPCWMV-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCASPCWMV-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCASPCWMV-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCASPCWMV-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCASPCWMV-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCASPCWMV-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCASPCWMV-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCASPCWMV-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCASPCWMV-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCASPCWMV-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcaspcwmv_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
