"""Evaluate frozen HVEVP-24 structural novelty against every Gross9 sleeve."""
...
from training import preregister_high_volatility_ehlers_voss_predictor_crossover_relay as registration

POLICY = "HVEVP-24"
PROTOCOL = "hvevp_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_ehlers_voss_predictor_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVEVP predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVEVP novelty limits drift")
    if (
        ...
        raise RuntimeError("HVEVP predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVEVP primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVEVP-24 unchanged before economic outcomes",
    }
