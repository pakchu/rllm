"""Evaluate frozen GVRCR-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "GVRCR-6"
PROTOCOL = "gvrcr_6_gross9_novelty_v1"
...
OUTPUT = Path("results/gold_volatility_rotation_btc_confirmation_relay_gross9_novelty_2026-08-08.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha256(PREREG) != PREREG_SHA or sha256(SUPPORT) != SUPPORT_SHA or sha256(CLOCK) != CLOCK_SHA:
        raise RuntimeError("GVRCR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("GVRCR novelty limits drift")
    if (
        ...
        raise RuntimeError("GVRCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="GVRCR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "gvrcr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject GVRCR-6 unchanged before economic outcomes",
    }
