"""Evaluate frozen HVCIIC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCIIC-8"
PROTOCOL = "hvciic_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_cross_alt_illiquidity_impulse_consensus_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCIIC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCIIC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCIIC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCIIC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCIIC-8 unchanged before economic outcomes",
    }
