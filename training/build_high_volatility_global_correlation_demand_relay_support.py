"""Materialize outcome-blind source support for frozen HVGCDR-24."""
...
from training import preregister_high_volatility_global_correlation_demand_relay as prereg
...
BUILDER = Path("training/build_high_volatility_global_correlation_demand_relay_support.py")
PREREG_SHA256 = "6fa9a27c8465aa4c7dff5928a9141df142af458cae58d283936c3f55fce9927a"
SYMBOLS = ("SPY", "EFA", "EEM")
SOURCE_DIR = Path("data/high_volatility_global_correlation_demand_relay_sources_2022_2026")
...
FEATURE_PANEL = SOURCE_DIR / "hvgcdr_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_global_correlation_demand_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_global_correlation_demand_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_global_correlation_demand_relay_support_2026-08-12.json")
...
CONTROLS = (
    ...
    "global_correlation_direction_flip",
    "one_session_stale_change",
    "spy_only_correlation_change",
    "correlation_level",
...
def normalize_yahoo_chart(payload: bytes, symbol: str) -> tuple[bytes, pd.DataFrame, dict[str, Any]]:
    ...
    if chart.get("error") is not None:
        raise RuntimeError(f"HVGCDR Yahoo chart error for {symbol}")
    results = chart.get("result") or []
    if len(results) != 1:
        raise RuntimeError(f"HVGCDR Yahoo result count drift for {symbol}")
    result = results[0]
    meta = result.get("meta") or {}
    if meta.get("symbol") != symbol or meta.get("exchangeTimezoneName") != "America/New_York":
        raise RuntimeError(f"HVGCDR {symbol} Yahoo metadata drift")
    timestamps = result.get("timestamp") or []
    indicators = result.get("indicators") or {}
    quotes = indicators.get("quote") or []
    adjusted = indicators.get("adjclose") or []
    if len(quotes) != 1 or len(adjusted) != 1 or not timestamps:
        raise RuntimeError(f"HVGCDR {symbol} adjusted chart incomplete")
    quote = quotes[0]
    ...
    if any(values is None or len(values) != len(timestamps) for values in vectors.values()):
        raise RuntimeError(f"HVGCDR {symbol} Yahoo vector length drift")
    ...
    if frame.empty or frame.session_date.duplicated().any():
        raise RuntimeError(f"HVGCDR {symbol} session dates invalid")
    ...
    if not valid.all():
        ...
        raise RuntimeError(f"HVGCDR {symbol} invalid adjusted rows: {dates[:5]}")
    metadata = {
        "symbol": symbol,
...
def build_equity_panel(equities: Mapping[str, pd.DataFrame]) -> pd.DataFrame:
    if set(equities) != set(SYMBOLS):
        raise RuntimeError("HVGCDR equity symbol set drift")
    ...
    for symbol in SYMBOLS:
        ...
        if not observed.equals(official):
            ...
            raise RuntimeError(
                f"HVGCDR {symbol}/NYSE calendar mismatch missing={missing[:5]} extra={extra[:5]}"
            )
        if expected is None:
            expected = observed
        elif not observed.equals(expected):
            raise RuntimeError(f"HVGCDR {symbol} does not share every official common session")
    ...
    if len(panel) != len(expected):
        raise RuntimeError("HVGCDR common-session intersection dropped an official session")
    ...
    if observed_early != expected_early:
        raise RuntimeError("HVGCDR frozen early-close schedule drift")
    return panel
...
def normalize_btc_bars(
    ...
    if raw.columns.tolist() != ["ts", "open", "close"]:
        raise RuntimeError("HVGCDR BTC schema drift")
    frame = raw.copy()
    ...
    if len(frame) != len(expected) or not frame.ts.equals(pd.Series(expected, name="ts")):
        raise RuntimeError("HVGCDR BTC source is not the exact requested 1m grid")
    for column in ("open", "close"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    values = frame[["open", "close"]]
    if not np.isfinite(values.to_numpy(dtype=float)).all() or not values.gt(0.0).all(axis=None):
        raise RuntimeError("HVGCDR BTC prices invalid")
    return frame
...
def rolling_pearson(first: pd.Series, second: pd.Series, window: int = 20) -> pd.Series:
    output = pd.Series(np.nan, index=first.index, dtype=float)
    for index in range(window - 1, len(output)):
        x = first.iloc[index - window + 1 : index + 1].to_numpy(dtype=float)
        y = second.iloc[index - window + 1 : index + 1].to_numpy(dtype=float)
        if not np.isfinite(x).all() or not np.isfinite(y).all():
            continue
        if float(x.std(ddof=0)) > 0.0 and float(y.std(ddof=0)) > 0.0:
            output.iloc[index] = float(np.corrcoef(x, y)[0, 1])
    return output
...
def build_features(sessions: pd.DataFrame, bars: pd.DataFrame) -> pd.DataFrame:
    ...
    if not isinstance(indexed.index, pd.DatetimeIndex) or indexed.index.tz is None:
        raise RuntimeError("HVGCDR BTC index must be timezone-aware")
    ...
    for symbol in SYMBOLS:
        ...
        frame[f"{prefix}_correlation"] = rolling_pearson(
            frame.btc_return, frame[f"{prefix}_return"], window=20
        )
    correlation_columns = [f"{symbol.lower()}_correlation" for symbol in SYMBOLS]
    frame["global_correlation"] = frame[correlation_columns].mean(axis=1, skipna=False)
    frame["global_correlation_change"] = frame.global_correlation.diff()
    frame["magnitude_rank"] = strict_prior_midrank(
        frame.global_correlation_change.abs(), lookback=270, minimum=180
    )
    frame["btc_variation_rank"] = strict_prior_midrank(
        frame.btc_realized_variation, lookback=270, minimum=180
    )
    frame["spy_correlation_change"] = frame.spy_correlation.diff()
    frame["spy_magnitude_rank"] = strict_prior_midrank(
        frame.spy_correlation_change.abs(), lookback=270, minimum=180
    )
    frame["correlation_level_rank"] = strict_prior_midrank(
        frame.global_correlation.abs(), lookback=270, minimum=180
    )
    return frame
...
def _signal(frame: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    score = frame.global_correlation_change.copy()
    rank = frame.magnitude_rank.copy()
    if control == "one_session_stale_change":
        score, rank = score.shift(1), rank.shift(1)
    elif control == "spy_only_correlation_change":
        score, rank = frame.spy_correlation_change, frame.spy_magnitude_rank
    elif control == "correlation_level":
        score, rank = frame.global_correlation, frame.correlation_level_rank
    ...
    if control == "global_correlation_direction_flip":
        side = -side
        ...
        rows.append(
            {
                ...
                "spy_correlation": float(frame.at[index, "spy_correlation"]),
                "efa_correlation": float(frame.at[index, "efa_correlation"]),
                "eem_correlation": float(frame.at[index, "eem_correlation"]),
                "global_correlation": float(frame.at[index, "global_correlation"]),
                "global_correlation_change": float(frame.at[index, "global_correlation_change"]),
...
def run(env_file: str = ENV_FILE) -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA256:
        raise RuntimeError("HVGCDR preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvgcdr_24_sources_v1",
    ...
    core = {
        "protocol_version": "hvgcdr_24_source_support_v1",
        "policy_id": "HVGCDR-24",
        "preregistration": {
