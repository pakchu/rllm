"""Evaluate frozen HVHCEM-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVHCEM-12"
PROTOCOL = "hvhcem_12_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_haar_coarse_energy_migration_continuation_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVHCEM predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVHCEM novelty limits drift")
    if (
        ...
        raise RuntimeError("HVHCEM predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVHCEM primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvvrcr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVHCEM-12 unchanged before economic outcomes",
    }
