"""Build outcome-blind source support for frozen HVBOP-24."""
...
PANEL_COLUMNS = (
    ...
    "bar_low", "bar_close", "body", "bar_range", "bop", "entry_side",
    "close_change", "close_change_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "body", "bar_range", "bop", "entry_side",
    "close_change", "close_change_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVBOP source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVBOP invalid source key")
...
def bop_values(bars: pd.DataFrame, valid: pd.Series) -> pd.DataFrame:
    body=(bars.bar_close-bars.bar_open).where(valid);bar_range=(bars.bar_high-bars.bar_low).where(valid)
    bop=(body/bar_range).where(bar_range.gt(0));close_change=(bars.bar_close-bars.bar_close.shift(1)).where(valid&valid.shift(1,fill_value=False))
    return pd.DataFrame({"body":body,"bar_range":bar_range,"bop":bop,"close_change":close_change},index=bars.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    values=bop_values(bars,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["entry_side"]=cross_side(bars.bop);bars["close_change_side"]=cross_side(bars.close_change)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["body","bar_range","bop","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.bar_range.gt(0)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="close_change_sign":side=pd.to_numeric(used.close_change_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVBOP prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvbop_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvbop_24_source_support_v1", "policy_id": prereg.POLICY_ID,
