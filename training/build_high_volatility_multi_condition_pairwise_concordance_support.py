"""Materialize outcome-blind source support for frozen HVMCPAC-8 pairs."""
...
CLOCK_DIR = Path("data/high_volatility_multi_condition_pairwise_concordance_clocks_2023_2026")
RESULT = Path(
...
def _read_json(path: Path) -> Mapping[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVMCPAC-8 expected JSON object: {path}")
    return value
...
def verify_frozen_inputs() -> dict[str, Any]:
    ...
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVMCPAC-8 preregistration artifact drift")
    ...
    for component in prereg.COMPONENT_ORDER:
        ...
        for artifact_type in ("preregistration", "support", "gross9", "clock"):
            ...
            if actual != artifact["sha256"]:
                raise RuntimeError(
                    f"HVMCPAC-8 {component} {artifact_type} artifact drift"
                )
        ...
        # The frozen component result may be consulted only for its Gross9 pass state.
        ...
        if not gross9_passed:
            raise RuntimeError(f"HVMCPAC-8 {component} frozen Gross9 artifact did not pass")
...
def load_component_clock(component: str) -> pd.DataFrame:
    ...
    if component not in prereg.COMPONENT_ARTIFACTS:
        raise ValueError(f"unknown HVMCPAC-8 component: {component}")
    ...
    if frame.columns.tolist() != list(INPUT_COLUMNS):
        raise RuntimeError(f"HVMCPAC-8 {component} clock schema drift")
    ...
    if frame["entry_time"].duplicated().any():
        raise RuntimeError(f"HVMCPAC-8 {component} duplicate entry_time")
    if not frame["candidate"].eq(component).all() or not frame["control"].eq("primary").all():
        raise RuntimeError(f"HVMCPAC-8 {component} clock identity drift")
    if not frame["side"].isin((-1, 1)).all():
        raise RuntimeError(f"HVMCPAC-8 {component} clock has non-strict side")
    if not frame["exit_time"].eq(frame["entry_time"] + HOLD).all():
        raise RuntimeError(f"HVMCPAC-8 {component} clock hold drift")
    if not frame["decision_time"].le(frame["entry_time"]).all():
        raise RuntimeError(f"HVMCPAC-8 {component} decision after entry")
    if not frame["feature_available_time"].le(frame["entry_time"]).all():
        raise RuntimeError(f"HVMCPAC-8 {component} feature availability after entry")
    expected_splits = [
    ...
    if any(split is None for split in expected_splits) or frame["split"].tolist() != expected_splits:
        raise RuntimeError(f"HVMCPAC-8 {component} split drift")
...
def intersect_pair(
    ...
    if f"{left}__AND__{right}" not in prereg.CANDIDATE_FAMILY:
        raise ValueError("HVMCPAC-8 pair must use exact frozen unordered order")
    ...
    if not joined["side"].isin((-1, 1)).all():
        raise RuntimeError("HVMCPAC-8 intersection has non-strict side")
    if not joined["exit_time_left"].eq(joined["entry_time"] + HOLD).all() or not joined[
        "exit_time_right"
    ].eq(joined["entry_time"] + HOLD).all():
        raise RuntimeError("HVMCPAC-8 component exit mismatch")
    if not joined["split_left"].eq(joined["split_right"]).all():
        raise RuntimeError("HVMCPAC-8 component split mismatch")
    ...
    if not output["decision_time"].le(output["entry_time"]).all() or not output[
        ...
        raise RuntimeError("HVMCPAC-8 combined availability after entry")
...
def run() -> dict[str, Any]:
    ...
    registration = _read_json(prereg.DEFAULT_OUTPUT)
    core = {
        "protocol_version": "hvmcpac_8_source_support_v1",
