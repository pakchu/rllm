"""Evaluate frozen HVBLIC-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVBLIC-6"
PROTOCOL = "hvblic_6_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_basis_leverage_ignition_continuation_preregistration_2026-08-11.json"
)
PREREG_SHA = "48907aa183d5bcc24e1b7beef054e1561782aee6776bd8363557757b00c323e6"
SUPPORT = Path("results/high_volatility_basis_leverage_ignition_continuation_support_2026-08-11.json")
SUPPORT_SHA = "47a8f7d6b62ef4f1494a42a02d5359311492261a694a8700ad76f554842ee593"
CLOCK = Path("data/high_volatility_basis_leverage_ignition_continuation_clocks_2023_2026.csv.gz")
CLOCK_SHA = "3e9bea596f8c476c4ca9d1cbc495496b4ccae3f7a7190d0b46fff4466a6e3be7"
OUTPUT = Path(
    "results/high_volatility_basis_leverage_ignition_continuation_gross9_novelty_2026-08-11.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVBLIC predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVBLIC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVBLIC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVBLIC primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject HVBLIC-6 unchanged before economic outcomes"
        ),
    }
