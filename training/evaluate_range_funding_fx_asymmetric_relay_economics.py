"""Sequential strict economics for frozen RFFXAR-ASYM."""
...
PREREG = Path("results/range_funding_fx_asymmetric_relay_preregistration_2026-08-09.json")
PREREG_SHA = "c717f8759e90b02aa4391c0a64c64863b305ad06b4015ae73f892662c4707b2a"
SUPPORT = Path("results/range_funding_fx_asymmetric_relay_support_2026-08-09.json")
SUPPORT_SHA = "b4d13352760323d445035312577b5c653887cf3ebea57deea4e325c9367ca165"
NOVELTY = Path("results/range_funding_fx_asymmetric_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "8ac4e42975a63f4ea4c650a7e77321fd68f39dcaec37dc80806e12870bec316a"
CLOCK = Path("data/range_funding_fx_asymmetric_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "694adbc9bec7e3533241e0718a33c94632a2ce23ff5613a18510d03afe61dd5f"
CONTROL_DIR = Path("data/range_funding_fx_asymmetric_relay_controls_2023_2026")
FREEZE = Path("results/range_funding_fx_asymmetric_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/range_funding_fx_asymmetric_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("long_state_only", "short_state_only", "no_range_component", "no_funding_component", "no_premium_component", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("RFFXAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("RFFXAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "rffxar_asym_sequential_economics_v1", "policy_id": POLICY_ID,
