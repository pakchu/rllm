"""Evaluate frozen CAFAER-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "CAFAER-12"
PROTOCOL = "cafaer_12_gross9_novelty_v1"
PREREG = Path("results/cross_alt_flow_acceleration_exhaustion_reversal_preregistration_2026-08-09.json")
PREREG_SHA = "7918b8b251475b035aed2d5113669e9c045a4ed4068179b0232cc72a788ae1b7"
SUPPORT = Path("results/cross_alt_flow_acceleration_exhaustion_reversal_support_2026-08-09.json")
SUPPORT_SHA = "c5567d724dc07aae7d737ce2f2ff44c9e9730857136a2853829ae9ebb454d8a4"
CLOCK = Path("data/cross_alt_flow_acceleration_exhaustion_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "2f9dc54d54bd449d7875c95359161442e5ea768655e6a047261f24d552774957"
OUTPUT = Path("results/cross_alt_flow_acceleration_exhaustion_reversal_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CAFAER predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("CAFAER novelty limits drift")
    if (
        ...
        raise RuntimeError("CAFAER predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CAFAER primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "cafaer_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject CAFAER-12 unchanged before economic outcomes",
    }
