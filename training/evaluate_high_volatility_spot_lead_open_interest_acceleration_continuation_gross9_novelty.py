"""Evaluate the frozen HVSPOIAC-8 action-vote clock against Gross9."""
...
from training import preregister_high_volatility_spot_lead_open_interest_acceleration_continuation as prereg

POLICY = "HVSPOIAC-8"
PROTOCOL = "hvspoiac_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_spot_lead_open_interest_acceleration_continuation_clocks_2020_2026.csv.gz",
    "sha256": "a42d83455ed22ebc681a301b83b03b31cf9bd89b317331b7a8e4dc7f0508110f",
    "rows": 264,
}
OUTPUT = Path("results/high_volatility_spot_lead_open_interest_acceleration_continuation_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVSPOIAC-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVSPOIAC-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVSPOIAC-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVSPOIAC-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVSPOIAC-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVSPOIAC-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVSPOIAC-8 unchanged before economics",
    }
