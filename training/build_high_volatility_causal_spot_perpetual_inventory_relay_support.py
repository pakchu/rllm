"""Deterministic outcome-blind source support for HVCSPIR-8."""
...
from training import preregister_high_volatility_causal_spot_perpetual_inventory_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "7866b1badb3678562feed32d4c1ccc8d9484cffbe2380da8460637caddfee62a"
...
BAR_QUERY = """WITH paths AS (SELECT ts,open,high,low,close,'perpetual' AS market FROM bars_binance WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end UNION ALL SELECT ts,open,high,low,close,'spot' AS market FROM bars_binance_spot WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end) SELECT date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '8 hours' AS decision_time,market,sum(ln(close/open)) AS completed_return,sum(power(ln(close/open),2)) AS minute_squared_return,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND high>=greatest(open,close,low) AND low<=least(open,close,high)) AS coherent FROM paths GROUP BY 1,2 ORDER BY 1,2"""

ROOT = Path("data/high_volatility_causal_spot_perpetual_inventory_relay_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_spot_perpetual_inventory_relay_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_spot_perpetual_inventory_relay_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_causal_spot_perpetual_inventory_relay_support_2026-08-16.json")
...
PANEL_COLUMNS = (
    ...
    "spot_lead_displacement", "spot_lead_rank", "realized_variation",
...
CLOCK_COLUMNS = (
    ...
    "spot_lead_displacement", "spot_lead_rank", "realized_variation", "variation_rank",
)
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVCSPIR-8 OI schema drift")
    expected_bars = [
        "decision_time", "market", "completed_return", "minute_squared_return",
        "source_rows", "distinct_rows", "first_ts", "last_ts", "coherent",
    ]
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCSPIR-8 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all():
        raise RuntimeError("HVCSPIR-8 invalid OI rows")
    ...
    if bars[["decision_time", "market"]].duplicated().any() or set(bars["market"]) != {"perpetual", "spot"}:
        raise RuntimeError("HVCSPIR-8 duplicate or missing symbol bar")
    ...
    fields = ["bar_valid", "completed_return", "minute_squared_return"]
    ...
    wide["spot_lead_displacement"] = wide["spot_return"].abs() - wide["perpetual_return"].abs()
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel = panel.join(
        bars[["joint_bar_valid", "perpetual_return", "spot_return", "spot_lead_displacement", "realized_variation"]],
        on="decision_time",
    )
    panel["source_valid"] = (
        ...
        & np.isfinite(
            panel[["sum_open_interest", "previous_open_interest", "oi_change", "perpetual_return", "spot_return", "spot_lead_displacement", "realized_variation"]]
        ).all(axis=1)
        ...
        & panel["spot_lead_displacement"].gt(0)
    )
    valid = panel["source_valid"].eq(True)
    panel["spot_lead_rank"] = causal_midrank(panel["spot_lead_displacement"].where(valid))
    ...
    panel["eligible"] = (
        ...
        & panel["spot_lead_rank"].ge(POLICY["spot_lead_rank_min"])
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCSPIR-8 side or availability drift")
        ...
        rows.append({
            ...
            "spot_lead_displacement": float(row.spot_lead_displacement),
            "spot_lead_rank": float(row.spot_lead_rank),
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCSPIR-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcspir_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvcspir_8_source_support_v1", "policy_id": prereg.POLICY_ID,
