"""Evaluate the frozen HVCVAROIR-8 action-vote clock against Gross9."""
...
POLICY = "HVCVAROIR-8"
PROTOCOL = "hvcvaroir_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_variance_acceleration_oi_regime_router_clocks_2020_2026.csv.gz",
    "sha256": "722b24201a5dc645b578e20198b258874edd25006abe342be1e7ff1cbdcbea24",
    "rows": 284,
}
OUTPUT = Path("results/high_volatility_variance_acceleration_oi_regime_router_gross9_novelty_2026-08-18.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCVAROIR-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVAROIR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCVAROIR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVAROIR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCVAROIR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCVAROIR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCVAROIR-8 unchanged before economics",
    }
