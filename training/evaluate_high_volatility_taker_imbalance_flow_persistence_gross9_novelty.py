"""Evaluate frozen HVTIFP-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVTIFP-8"
PROTOCOL = "hvtifp_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_taker_imbalance_flow_persistence_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTIFP predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVTIFP novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTIFP predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTIFP primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVTIFP-8 unchanged before economic outcomes",
    }
