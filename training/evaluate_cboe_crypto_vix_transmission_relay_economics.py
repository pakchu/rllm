"""Sequential strict economics for frozen CCVTR-6."""
...
PREREG = Path("results/cboe_crypto_vix_transmission_relay_preregistration_2026-08-08.json")
PREREG_SHA = "3fa4aaac81eebec3482de516d4f8823762fe2faf0d5e5ba8c859e2e43b01816f"
SUPPORT = Path("results/cboe_crypto_vix_transmission_relay_support_2026-08-08.json")
SUPPORT_SHA = "8e871969430deef3d027749df03836c0dc34b8e4b921e70755453cda4e184bfd"
NOVELTY = Path("results/cboe_crypto_vix_transmission_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "5eff04b87752230b1da27f55d835eac01487f4075717c014dc1f842afd0c7d88"
CLOCK = Path("data/cboe_crypto_vix_transmission_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "0f4dbeb084e10e91c684a4ec8f98b59980dd64d5115d7a2ae8e09f882c96e6ca"
CONTROL_DIR = Path("data/cboe_crypto_vix_transmission_relay_controls_2023_2026")
FREEZE = Path("results/cboe_crypto_vix_transmission_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/cboe_crypto_vix_transmission_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
CONTROLS = ("no_crypto_confirmation", "bvol_only_confirmation", "dvol_only_confirmation", "one_session_stale_vix_change", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CCVTR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CCVTR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "ccvtr_6_sequential_economics_v1", "policy_id": POLICY_ID,
