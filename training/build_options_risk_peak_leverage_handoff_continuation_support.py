"""Build outcome-blind source support for frozen ORPLHC-6."""
...
from training import preregister_options_risk_peak_leverage_handoff_continuation as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv


ENV_FILE = "/home/pakchu/rllm/.env"
PREREG_SHA = "6258c92b9a30df222651883623a18d9c6bb7d60306f63e6eccdd5cc74eecbba3"
...
SOURCE_DIR = Path("data/options_risk_peak_leverage_handoff_continuation_sources_2023_2026")
FEATURES = SOURCE_DIR / "hourly_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/options_risk_peak_leverage_handoff_continuation_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/options_risk_peak_leverage_handoff_continuation_controls_2023_2026")
RESULT = Path("results/options_risk_peak_leverage_handoff_continuation_support_2026-08-11.json")
SPLITS = {
...
CONTROLS = ("no_prior_dvol_shock", "no_current_dvol_cooling", "no_dvol_level", "no_premium_tail", "no_oi_increase", "one_hour_stale_features", "direction_flip", "forced_long")
PREMIUM_QUERY = """
SELECT date_bin('1 hour',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00') AS block_start,
...
GROUP BY 1 ORDER BY 1
"""
CLOCK_COLUMNS = ("candidate", "control", "split", "decision_time", "feature_available_time", "entry_time", "exit_time", "side", "premium_displacement", "premium_displacement_rank", "oi_change", "prior_dvol_body", "prior_dvol_body_abs_rank", "dvol_body", "dvol_close", "dvol_level_rank")
...
def load_dvol() -> pd.DataFrame:
    ...
    frame["dvol_open"] = pd.to_numeric(frame.open, errors="coerce")
    frame["dvol_close"] = pd.to_numeric(frame.close, errors="coerce")
    return frame[["decision_time", "dvol_open", "dvol_close"]]
...
def build_features(premium: pd.DataFrame, oi: pd.DataFrame, dvol: pd.DataFrame) -> pd.DataFrame:
    ...
    frame["decision_time"] = frame.block_start + pd.Timedelta(hours=1)
    ...
    frame["source_valid"] = (
        frame.premium_rows.eq(60) & frame.premium_distinct.eq(60)
        & frame.oi_rows.eq(12) & frame.oi_distinct.eq(12)
        & frame.premium_first_ts.eq(frame.block_start)
        & frame.premium_last_ts.eq(frame.block_start + pd.Timedelta(minutes=59))
        & frame.oi_first_ts.eq(frame.block_start)
        & frame.oi_last_ts.eq(frame.block_start + pd.Timedelta(minutes=55))
        ...
        & frame.dvol_open.gt(0) & np.isfinite(frame.dvol_open)
        & frame.dvol_close.gt(0) & np.isfinite(frame.dvol_close)
    )
    frame = frame.sort_values("decision_time").reset_index(drop=True)
    frame["dvol_body"] = (frame.dvol_close - frame.dvol_open) / frame.dvol_open
    frame["dvol_body_abs_rank"] = strict_prior_midrank(frame.dvol_body.abs().where(frame.source_valid))
    frame["prior_dvol_body"] = frame.dvol_body.shift(1)
    frame["prior_dvol_body_abs_rank"] = frame.dvol_body_abs_rank.shift(1)
...
def conditions(frame: pd.DataFrame, control: str = "primary") -> tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    used = frame.shift(1) if control == "one_hour_stale_features" else frame
    active = used.source_valid.eq(True) & used.source_valid.shift(1).eq(True)
    if control != "no_prior_dvol_shock":
        active &= used.prior_dvol_body.gt(0) & used.prior_dvol_body_abs_rank.ge(0.75)
    if control != "no_current_dvol_cooling": active &= used.dvol_body.lt(0)
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active]:
        decision = pd.Timestamp(features.at[index, "decision_time"]); entry = decision + pd.Timedelta(minutes=5); exit_time = entry + pd.Timedelta(hours=6)
        if next_allowed is not None and entry < next_allowed: continue
        split = next((name for name, (start, end) in SPLITS.items() if entry >= start and exit_time <= end), None)
        if split is None: continue
        source = used.loc[index]
        rows.append({"candidate": "ORPLHC-6", "control": control, "split": split, "decision_time": decision, "feature_available_time": decision, "entry_time": entry, "exit_time": exit_time, "side": int(sides.at[index]), "premium_displacement": float(source.premium_displacement), "premium_displacement_rank": float(source.premium_displacement_rank), "oi_change": float(source.oi_change), "prior_dvol_body": float(source.prior_dvol_body), "prior_dvol_body_abs_rank": float(source.prior_dvol_body_abs_rank), "dvol_body": float(source.dvol_body), "dvol_close": float(source.dvol_close), "dvol_level_rank": float(source.dvol_level_rank)})
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA: raise RuntimeError("ORPLHC preregistration drift")
