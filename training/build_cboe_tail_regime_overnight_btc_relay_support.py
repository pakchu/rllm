"""Build outcome-blind source support for preregistered CVTLR-6."""
...
from training import preregister_cboe_tail_regime_overnight_btc_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.build_cboe_volatility_surface_regime_crossing_relay_support import strict_prior_midrank

CLOCK = Path("data/cboe_tail_regime_overnight_btc_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/cboe_tail_regime_overnight_btc_relay_controls_2023_2026")
RESULT = Path("results/cboe_tail_regime_overnight_btc_relay_support_2026-08-08.json")
SPLITS = {
...
CONTROLS = ("no_tail_gate", "skew_rank_only", "relative_convexity_rank_only", "confirmation_break", "direction_flip")
...
COLUMNS = (
    ...
    "side", "skew_rank", "relative_convexity_rank", "tail_pressure", "overnight_btc_return",
)
...
def load_features() -> pd.DataFrame:
    for raw, expected in prereg.HASHES.items():
        if sha(Path(raw)) != expected:
            raise RuntimeError(f"CVTLR frozen source changed: {raw}")
    cboe = pd.read_csv(prereg.CBOE, compression="gzip", usecols=["observation_date", "SKEW_close", "VVIX_close", "VIX_close"])
    cboe["observation_date"] = pd.to_datetime(cboe.observation_date, format="%Y-%m-%d")
    for column in ("SKEW_close", "VVIX_close", "VIX_close"):
    ...
    if not cboe.observation_date.is_monotonic_increasing or cboe.observation_date.duplicated().any():
        raise RuntimeError("CVTLR Cboe dates invalid")
    values = cboe[["SKEW_close", "VVIX_close", "VIX_close"]]
    if not np.isfinite(values).all().all() or not values.gt(0).all().all():
        raise RuntimeError("CVTLR tail values invalid")
    ...
    if price.columns.tolist() != ["hour_start", "hour_open", "first_half_close", "second_half_open", "hour_close", "source_rows", "distinct_timestamps", "source_valid", "decision_time"]:
        raise RuntimeError("CVTLR completed-price schema changed")
    ...
    if price.hour_start.duplicated().any() or not price.hour_start.is_monotonic_increasing:
        raise RuntimeError("CVTLR completed-price clock invalid")
    ...
    for index in range(1, len(cboe) - 1):
        ...
        rows.append({
            ...
            "tail_pressure": float(cboe.at[index, "tail_pressure"]),
...
def signal(frame: pd.DataFrame, control: str) -> pd.Series:
    btc, term = frame.overnight_btc_return, frame.tail_pressure
    valid = frame.valid & btc.ne(0) & term.notna()
    relief, stress = term.le(0.40), term.ge(0.60)
    confirmed = (relief & btc.gt(0)) | (stress & btc.lt(0))
    side = np.sign(btc).astype("Int64").fillna(0).astype(int)
    if control == "no_tail_gate":
        eligible = frame.valid & btc.ne(0)
    elif control == "skew_rank_only":
        eligible = valid & ((frame.skew_rank.le(0.40) & btc.gt(0)) | (frame.skew_rank.ge(0.60) & btc.lt(0)))
    elif control == "relative_convexity_rank_only":
        eligible = valid & ((frame.relative_convexity_rank.le(0.40) & btc.gt(0)) | (frame.relative_convexity_rank.ge(0.60) & btc.lt(0)))
...
def build_clock(frame: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in frame.index[sides.ne(0)]:
        ...
        rows.append({
            "candidate": "CVTLR-6", "control": control, "split": split,
            ...
            "tail_pressure": float(frame.at[index, "tail_pressure"]),
