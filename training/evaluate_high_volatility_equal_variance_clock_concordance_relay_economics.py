"""Sequential strict economics for frozen HVEVC-8."""
...
PREREG = Path("results/high_volatility_equal_variance_clock_concordance_relay_preregistration_2026-08-13.json")
PREREG_SHA = "a3f64ebe61ac8eef1d9b11370615a900dd5f6d1b6e93824eedb6cbee1a08363e"
SUPPORT = Path("results/high_volatility_equal_variance_clock_concordance_relay_support_2026-08-13.json")
SUPPORT_SHA = "731daa6aed39111c1b95971eaa0f56a10db5692801534cbe8426766b972678d2"
NOVELTY = Path("results/high_volatility_equal_variance_clock_concordance_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "6535986c353e310c9d80cc00787201a43eb60eb74fe91278bca0c5b50a21f9a3"
CLOCK = Path("data/high_volatility_equal_variance_clock_concordance_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "432b46593a8bab87e5e0dd0d62d41db8ded61b0d007576861ab770ad9e2fd82f"
CONTROL_DIR = Path("data/high_volatility_equal_variance_clock_concordance_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_equal_variance_clock_concordance_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_equal_variance_clock_concordance_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "three_of_four_concordance", "equal_physical_time", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVEVC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVEVC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvevc_8_sequential_economics_v1", "policy_id": POLICY_ID,
