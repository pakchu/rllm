"""Evaluate the frozen HVPRAPR-12 action-vote clock against Gross9."""
...
POLICY = "HVPRAPR-12"
PROTOCOL = "hvprapr_12_gross9_novelty_v1"
PREREG = Path("results/high_volatility_relative_premium_activity_polarity_router_preregistration_2026-08-16.json")
PREREG_SHA = "c12f2cc6ef2720f078b11248b82b2dc677d9c1af616fd034ec8aa2ccd321d169"
SUPPORT = Path("results/high_volatility_relative_premium_activity_polarity_router_support_2026-08-16.json")
SUPPORT_SHA = "5eb89e5ad0134e59235f9aaaa5001865c156010f4c8e5ebf5c0678d79781ca18"
CLOCK = {
    "path": "data/high_volatility_relative_premium_activity_polarity_router_clocks_2023_2026.csv.gz",
    "sha256": "a0f667168b450ac640e445c79142be37802d118f09bfb1556fedd9203eb663e0",
    "rows": 260,
}
OUTPUT = Path("results/high_volatility_relative_premium_activity_polarity_router_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVPRAPR-12 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVPRAPR-12 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVPRAPR-12 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVPRAPR-12 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVPRAPR-12 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVPRAPR-12 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVPRAPR-12 unchanged before economics",
    }
