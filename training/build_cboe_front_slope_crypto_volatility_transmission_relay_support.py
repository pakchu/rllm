"""Build source-support clocks for CFSTR-6 without BTC outcomes."""
...
from training import preregister_cboe_front_slope_crypto_volatility_transmission_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv

CLOCK=Path("data/cboe_front_slope_crypto_volatility_transmission_relay_clocks_2023_2026.csv.gz");CONTROL_DIR=Path("data/cboe_front_slope_crypto_volatility_transmission_relay_controls_2023_2026");RESULT=Path("results/cboe_front_slope_crypto_volatility_transmission_relay_support_2026-08-08.json")
...
def features()->pd.DataFrame:
 for raw,want in prereg.HASHES.items():
  if sha(Path(raw))!=want:raise RuntimeError(f"CFSTR source drift: {raw}")
 ...
 if c.observation_date.duplicated().any() or not c.observation_date.is_monotonic_increasing or not np.isfinite(c[["VIX9D_close","VIX_close"]]).all(axis=1).all() or not c[["VIX9D_close","VIX_close"]].gt(0).all(axis=1).all():raise RuntimeError("CFSTR Cboe source invalid")
...
def sides(f:pd.DataFrame,control:str)->pd.Series:
 delta=f.previous_delta_front_slope if control=="one_session_stale_front_slope_change" else f.delta_front_slope;valid=f.valid&delta.notna()&delta.ne(0)&f.bvol_body.ne(0)&f.dvol_body.ne(0);same_b=np.sign(f.bvol_body).eq(np.sign(delta));same_d=np.sign(f.dvol_body).eq(np.sign(delta))
