"""Evaluate frozen RVSBR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "RVSBR-12"
PROTOCOL = "rvsbr_12_gross9_novelty_v1"
...
OUTPUT = Path("results/russell_volatility_spread_break_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("RVSBR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("RVSBR novelty limits drift")
    if (
        ...
        raise RuntimeError("RVSBR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="RVSBR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "rvsbr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject RVSBR-12 unchanged before economic outcomes",
    }
