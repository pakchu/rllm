"""Evaluate frozen HVABDS-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVABDS-8"
PROTOCOL = "hvabds_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_alt_breadth_diffusion_slope_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVABDS predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVABDS novelty limits drift")
    if (
        ...
        raise RuntimeError("HVABDS predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVABDS primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvabds_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVABDS-8 unchanged before economic outcomes",
    }
