"""Sequential strict economics for frozen HVLFX-12."""
...
PREREG = Path("results/high_volatility_london_fix_dollar_impulse_relay_preregistration_2026-08-11.json")
PREREG_SHA = "2908e8cd471c185625e2b21e6a45289ae89b89a3eaa3bfb841eaa732ad6c6a0a"
SUPPORT = Path("results/high_volatility_london_fix_dollar_impulse_relay_support_2026-08-11.json")
SUPPORT_SHA = "dc522cd93e92d6c6acf2e5dff47e29e34b4a62ca0fbb67e9aedf0172e5258e17"
NOVELTY = Path("results/high_volatility_london_fix_dollar_impulse_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "d30b5173a7fc441a7cfc98285c5f9d7d643d8f57ea6bd8b6c8c2ee511d942db5"
CLOCK = Path("data/high_volatility_london_fix_dollar_impulse_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "0925bc42f602d72ca3fb4b5dc783fb91cd45bbe7d96d058c4133c10c931228d8"
CONTROL_DIR = Path("data/high_volatility_london_fix_dollar_impulse_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_london_fix_dollar_impulse_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_london_fix_dollar_impulse_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "eurusd_only", "one_session_stale_fix_impulse", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVLFX frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVLFX novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvlfx_12_sequential_economics_v1", "policy_id": POLICY_ID,
