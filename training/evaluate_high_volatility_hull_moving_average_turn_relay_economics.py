"""Sequential strict economics for frozen HVHMA-24."""
...
PREREG = Path("results/high_volatility_hull_moving_average_turn_relay_preregistration_2026-08-11.json")
PREREG_SHA = "41d4ed869f5bb25484eedcd3127c09bd6711ac47bda4255cb971a5bab578d635"
SUPPORT = Path("results/high_volatility_hull_moving_average_turn_relay_support_2026-08-11.json")
SUPPORT_SHA = "fee1083ea589a260288857bd48896a6b31ad4f8cb0c2c92ac230dafbad41108c"
NOVELTY = Path("results/high_volatility_hull_moving_average_turn_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "1e3d9f54776d874fc93e40139d5e556424c170d18939117955402e998903cfaf"
CLOCK = Path("data/high_volatility_hull_moving_average_turn_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "b48db36d10ccc6a5dc0dbfd5de6d09a417d2ac13722353702141ee3e1c9ff738"
CONTROL_DIR = Path("data/high_volatility_hull_moving_average_turn_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_hull_moving_average_turn_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_hull_moving_average_turn_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "full_length_wma_turn", "one_bar_stale_turn", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVHMA frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVHMA novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvhma_24_sequential_economics_v1", "policy_id": POLICY_ID,
