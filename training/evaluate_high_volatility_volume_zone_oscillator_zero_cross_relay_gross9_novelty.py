"""Evaluate frozen HVVZO-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVVZO-24"
PROTOCOL = "hvvzo_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_volume_zone_oscillator_zero_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVVZO predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVVZO novelty limits drift")
    if (
        ...
        raise RuntimeError("HVVZO predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVVZO primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVVZO-24 unchanged before economic outcomes",
    }
