"""Source-only support gate for frozen HVAVC-8."""
...
from training import preregister_high_volatility_cumulative_vwap_recross_relay as prereg
...
PREREG_SHA = "9211f953469484a3788660bb5e7746707ee610747f9664a930243574e5a7dedb"
SPLITS = {
...
ROOT = Path("data/high_volatility_cumulative_vwap_recross_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_cumulative_vwap_recross_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_cumulative_vwap_recross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_cumulative_vwap_recross_relay_support_2026-08-10.json")
...
def recross_count(close: np.ndarray, volume: np.ndarray, quote: np.ndarray) -> float:
    close = np.asarray(close, dtype=float)
    volume = np.asarray(volume, dtype=float)
    quote = np.asarray(quote, dtype=float)
    if len(close) != 480 or not np.isfinite(np.c_[close, volume, quote]).all() or np.any(np.c_[close, volume, quote] <= 0):
        return math.nan
    cumulative_vwap = np.cumsum(quote) / np.cumsum(volume)
    distance = np.log(close / cumulative_vwap)
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVAVC source timestamps")
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        if valid:
            ...
            recrosses = recross_count(window["close"].to_numpy(float), window["volume"].to_numpy(float), window["quote_asset_volume"].to_numpy(float))
            ...
            valid = variation > 0 and math.isfinite(recrosses)
        if not valid:
            variation = recrosses = block_return = late_return = math.nan
        rows.append(
            {
                "decision_time": decision,
                "source_valid": valid,
                "realized_variation": variation,
                "recross_count": recrosses,
    ...
    states["recross_rank"] = prior_rank(states["recross_count"].where(states["source_valid"]))
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvavc_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    recross_rank = states["recross_rank"]
    recrosses = states["recross_count"]
    if control == "one_block_stale_geometry":
        block_return, late_return, variation_rank, recross_rank, recrosses = [
            value.shift(1)
            for value in (block_return, late_return, variation_rank, recross_rank, recrosses)
        ]
    variation_gate = (
    ...
    if control == "no_recross_tail":
        recross_gate = pd.Series(True, index=states.index)
    elif control == "raw_recross_at_most_eight":
        recross_gate = recrosses.le(8)
    else:
        recross_gate = recross_rank.le(0.25)
    agreement = (
        block_return.ne(0)
        & late_return.ne(0)
        & np.sign(block_return).eq(np.sign(late_return))
    )
    eligible = states["source_valid"] & variation_gate & recross_gate & agreement
        ...
        rows.append(
            {
                ...
                "recross_count": float(states.at[index, "recross_count"]),
                "recross_rank": float(states.at[index, "recross_rank"]),
            }
        )
    columns = [
        "candidate", "control", "split", "decision_time", "feature_available_time",
        "entry_time", "exit_time", "side", "realized_variation", "variation_rank",
        "recross_count", "recross_rank",
    ]
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVAVC preregistration drift")
    ...
    core = {
        "protocol_version": "hvavc_8_source_support_v1",
        "policy_id": "HVAVC-8",
        "preregistration": {
