"""Outcome-blind preregistration for HVCORR-24."""
...
POLICY_ID = "HVCORR-24"
DEFAULT_OUTPUT = Path("results/high_volatility_causal_oi_range_regime_relay_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_causal_oi_range_regime_relay_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": "When BTC crosses the midpoint of its completed seven-day auction range while open interest expands over the same completed eight-hour cycle and realized variation is causally elevated, newly built leverage confirms a value-regime migration that persists for twenty-four hours.",
            "side": "long on a strict upward prior-range-midpoint crossing; short on a strict downward crossing",
            "why_distinct": "HVCORR requires exact endpoint OI expansion to confirm a transition through the fixed midpoint of a trailing external high-low range. It uses no funding value or rank and is not HVCFBR's range escape, HVCFRR's funding-tail regime, an OI-return attribution statistic, fitted channel, or prior control.",
            ...
            "why_low_gross9_overlap_is_plausible": "fixed eight-hour range-midpoint transitions confirmed by exact endpoint OI expansion are absent from Gross9 primitives",
        },
        "features": {
            ...
            "range_midpoint": "arithmetic midpoint of the prior range high and low",
            "regime_cross": "last close at D-8h at or below midpoint and last close before D strictly above maps long; last close at D-8h at or above midpoint and current close strictly below maps short; exactly one direction required",
            ...
            "eligible": "22 uninterrupted coherent cycles including current, two exact OI endpoints with strict expansion, one strict range-midpoint crossing, and variation rank pass at D; no funding, divergence, premium, alt, or post-decision condition",
            "no_imputation": True,
        },
        "clock": {
        ...
        "stopping_rule": "terminal first failure; no OI endpoint or expansion rule, prior-range length, midpoint-cross definition, variation history, side, clock, hold, subset, threshold, comparator, or control repair",
    }
    return {**core, "manifest_hash": canonical_hash(core)}

def validate(value: Mapping[str, Any]) -> None:
    if value != build():
        raise RuntimeError("HVCORR-24 preregistration drift")
    contract = hvcav.build()
    for key in ("stages", "source_support_gates", "gross9_novelty_gates", "economic_gates"):
        if value[key] != contract[key]:
            raise RuntimeError(f"HVCORR-24 {key} drift")
