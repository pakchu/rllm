"""Materialize outcome-blind source support for frozen HVCELSR-24."""
...
BUILDER = Path("training/build_high_volatility_crypto_equity_leadership_spread_relay_support.py")
PREREG_SHA256 = "0ef5cd7df670cf60b36f618e8bce9b85540cf7705f6460aa64360d6f6d801e28"
SYMBOLS = ("MSTR", "COIN")
SOURCE_DIR = Path("data/high_volatility_crypto_equity_leadership_spread_relay_sources_2022_2026")
...
FEATURE_PANEL = SOURCE_DIR / "hvcelsr_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_crypto_equity_leadership_spread_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_crypto_equity_leadership_spread_relay_controls_2023_2026")
...
CLOCK_COLUMNS = (
    ...
    "mstr_prior_scale",
    "coin_prior_scale",
    "mstr_standardized_return",
    "coin_standardized_return",
    "leadership_spread",
    "magnitude_rank",
    "raw_return_spread",
...
def normalize_yahoo_chart(payload: bytes, symbol: str) -> tuple[bytes, pd.DataFrame, dict[str, Any]]:
    ...
    if chart.get("error") is not None:
        raise RuntimeError(f"HVCELSR Yahoo chart error for {symbol}")
    results = chart.get("result") or []
    if len(results) != 1:
        raise RuntimeError(f"HVCELSR Yahoo result count drift for {symbol}")
    result = results[0]
    meta = result.get("meta") or {}
    if meta.get("symbol") != symbol or meta.get("exchangeTimezoneName") != "America/New_York":
        raise RuntimeError(f"HVCELSR {symbol} Yahoo metadata drift")
    timestamps = result.get("timestamp") or []
    indicators = result.get("indicators") or {}
    quotes = indicators.get("quote") or []
    adjusted = indicators.get("adjclose") or []
    if len(quotes) != 1 or len(adjusted) != 1 or not timestamps:
        raise RuntimeError(f"HVCELSR {symbol} adjusted chart incomplete")
    quote = quotes[0]
    ...
    if any(values is None or len(values) != len(timestamps) for values in vectors.values()):
        raise RuntimeError(f"HVCELSR {symbol} Yahoo vector length drift")
    ...
    if frame.empty or frame.session_date.duplicated().any():
        raise RuntimeError(f"HVCELSR {symbol} session dates invalid")
    ...
    if not valid.all():
        ...
        raise RuntimeError(f"HVCELSR {symbol} invalid adjusted rows: {dates[:5]}")
    metadata = {
        "symbol": symbol,
...
def build_equity_panel(equities: Mapping[str, pd.DataFrame]) -> pd.DataFrame:
    if set(equities) != set(SYMBOLS):
        raise RuntimeError("HVCELSR equity symbol set drift")
    ...
    for symbol in SYMBOLS:
        ...
        if not observed.equals(official):
            ...
            raise RuntimeError(
                f"HVCELSR {symbol}/NYSE calendar mismatch missing={missing[:5]} extra={extra[:5]}"
            )
        if expected is None:
            expected = observed
        elif not observed.equals(expected):
            raise RuntimeError(f"HVCELSR {symbol} does not share every official common session")
    ...
    if len(panel) != len(expected):
        raise RuntimeError("HVCELSR common-session intersection dropped an official session")
    ...
    if observed_early != expected_early:
        raise RuntimeError("HVCELSR frozen early-close schedule drift")
    return panel
...
def normalize_btc_bars(
    ...
    if raw.columns.tolist() != ["ts", "open", "close"]:
        raise RuntimeError("HVCELSR BTC schema drift")
    frame = raw.copy()
    ...
    if len(frame) != len(expected) or not frame.ts.equals(pd.Series(expected, name="ts")):
        raise RuntimeError("HVCELSR BTC source is not the exact requested 1m grid")
    for column in ("open", "close"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    values = frame[["open", "close"]]
    if not np.isfinite(values.to_numpy(dtype=float)).all() or not values.gt(0.0).all(axis=None):
        raise RuntimeError("HVCELSR BTC prices invalid")
    return frame
...
def strict_prior_scale(
    returns: pd.Series,
    lookback: int = 63,
    minimum: int = 42,
) -> pd.Series:
    """Population standard deviation using only returns before the current session."""
    output = pd.Series(np.nan, index=returns.index, dtype=float)
    for index in range(len(output)):
        start = max(0, index - lookback)
        prior = pd.to_numeric(returns.iloc[start:index], errors="coerce").dropna().to_numpy(float)
        if len(prior) < minimum:
            continue
        scale = float(np.std(prior, ddof=0))
        if np.isfinite(prior).all() and np.isfinite(scale) and scale > 0.0:
            output.iloc[index] = scale
    return output
...
def build_features(sessions: pd.DataFrame, bars: pd.DataFrame) -> pd.DataFrame:
    ...
    if not isinstance(indexed.index, pd.DatetimeIndex) or indexed.index.tz is None:
        raise RuntimeError("HVCELSR BTC index must be timezone-aware")
    ...
    for symbol in SYMBOLS:
        prefix = symbol.lower()
        frame[f"{prefix}_return"] = np.log(
            frame[f"{prefix}_adjusted_close"] / frame[f"{prefix}_adjusted_close"].shift(1)
        )
        frame[f"{prefix}_prior_scale"] = strict_prior_scale(
            frame[f"{prefix}_return"], lookback=63, minimum=42
        )
        frame[f"{prefix}_standardized_return"] = (
            frame[f"{prefix}_return"] / frame[f"{prefix}_prior_scale"]
        )
    frame["leadership_spread"] = frame.mstr_standardized_return - frame.coin_standardized_return
    frame["raw_return_spread"] = frame.mstr_return - frame.coin_return
    required = frame[[
        "mstr_return", "coin_return", "mstr_prior_scale", "coin_prior_scale",
        "leadership_spread", "raw_return_spread", "btc_realized_variation",
    ]]
    frame["source_valid"] = (
        np.isfinite(required).all(axis=1)
        & frame.mstr_prior_scale.gt(0.0)
        & frame.coin_prior_scale.gt(0.0)
        & frame.leadership_spread.ne(0.0)
        & frame.btc_realized_variation.gt(0.0)
    )
    frame["magnitude_rank"] = strict_prior_midrank(
        frame.leadership_spread.abs().where(frame.source_valid), lookback=270, minimum=180
    )
    frame["btc_variation_rank"] = strict_prior_midrank(
        frame.btc_realized_variation.where(frame.source_valid), lookback=270, minimum=180
    )
    frame["raw_magnitude_rank"] = strict_prior_midrank(
        frame.raw_return_spread.abs().where(frame.source_valid), lookback=270, minimum=180
    )
    return frame
...
def _signal(frame: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    score = frame.leadership_spread.copy()
    rank = frame.magnitude_rank.copy()
    if control == "one_session_stale_leadership":
        score, rank = score.shift(1), rank.shift(1)
    elif control == "raw_return_spread":
        score, rank = frame.raw_return_spread, frame.raw_magnitude_rank
    magnitude_gate = pd.Series(True, index=frame.index) if control == "no_leadership_tail" else rank.ge(0.75)
    eligible = frame.source_valid & score.notna() & score.ne(0.0) & magnitude_gate
    if control != "no_btc_variation_gate":
        eligible &= frame.btc_variation_rank.ge(0.65)
    side = np.sign(score).astype("Int64")
    if control == "direction_flip":
        side = -side
    elif control == "same_clock_forced_long":
        side = pd.Series(1, index=frame.index, dtype="Int64")
    eligible &= side.ne(0) & side.notna()
    onset = eligible & ~eligible.shift(1, fill_value=False) & frame.source_valid.shift(1, fill_value=False)
    return onset.fillna(False), side
        ...
        rows.append(
            {
                ...
                "mstr_return": float(frame.at[index, "mstr_return"]),
                "coin_return": float(frame.at[index, "coin_return"]),
                "mstr_prior_scale": float(frame.at[index, "mstr_prior_scale"]),
                "coin_prior_scale": float(frame.at[index, "coin_prior_scale"]),
                "mstr_standardized_return": float(frame.at[index, "mstr_standardized_return"]),
                "coin_standardized_return": float(frame.at[index, "coin_standardized_return"]),
                "leadership_spread": float(frame.at[index, "leadership_spread"]),
                "magnitude_rank": float(frame.at[index, "magnitude_rank"]),
                "raw_return_spread": float(frame.at[index, "raw_return_spread"]),
...
def run(env_file: str = ENV_FILE) -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA256:
        raise RuntimeError("HVCELSR preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcelsr_24_sources_v1",
    ...
    core = {
        "protocol_version": "hvcelsr_24_source_support_v1",
        "policy_id": "HVCELSR-24",
        "preregistration": {
