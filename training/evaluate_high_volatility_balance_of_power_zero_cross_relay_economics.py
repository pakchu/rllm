"""Sequential strict economics for frozen HVBOP-24."""
...
PREREG = Path("results/high_volatility_balance_of_power_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "9ea7f69856687951db624d388f5684b03acf1c19fb8f70d2d4526e1a257c90c2"
SUPPORT = Path("results/high_volatility_balance_of_power_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "7f69542e6e74c672d16ca78c9b17716b048fef1321611029717214023f886b46"
NOVELTY = Path("results/high_volatility_balance_of_power_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "9ff7099e2adfdacfed4f263bce67c840d2f269ffcb8d39d8fa35264240dc563c"
CLOCK = Path("data/high_volatility_balance_of_power_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "e4ee3d52f14d3156279709728b0f66eb37a0468da184f51958ac607d3277c4e1"
CONTROL_DIR = Path("data/high_volatility_balance_of_power_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_balance_of_power_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_balance_of_power_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "close_change_sign", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVBOP frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVBOP novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvbop_24_sequential_economics_v1", "policy_id": POLICY_ID,
