"""Build outcome-blind source support for frozen HVPLO-8."""
...
from training import preregister_high_volatility_price_level_occupation_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-04-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="30b6cf17893c11be3dcdba1490c7916d173da4bccc3cff1ab83f870f13aa9041"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_price_level_occupation_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_price_level_occupation_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_price_level_occupation_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_price_level_occupation_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_price_level_occupation_relay_support_2026-08-10.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","reference_level","above_count","below_count","level_occupation","occupation_rank","median_level_displacement","median_displacement_rank","realized_variation","variation_rank","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","reference_level","above_count","below_count","level_occupation","occupation_rank","median_level_displacement","median_displacement_rank","realized_variation","variation_rank")
...
def occupation_statistics(block:pd.DataFrame)->tuple[float,int,int,float,float,float]:
 reference=float(block.open.iloc[0]);relative=np.log(block.close.to_numpy(float)/reference);above=int(np.sum(relative>0));below=int(np.sum(relative<0));denominator=above+below
 occupation=(above-below)/denominator if denominator else math.nan;median_displacement=float(np.median(relative));returns=np.log(block.close.to_numpy(float)/block.open.to_numpy(float));variation=float(np.sqrt(np.square(returns).sum()))
 return reference,above,below,occupation,median_displacement,variation
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVPLO source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVPLO invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("3h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   reference,above,below,occupation,median_displacement,var=occupation_statistics(block);valid=all(math.isfinite(v) for v in (reference,occupation,median_displacement,var)) and var>0
  else:reference=occupation=median_displacement=var=math.nan;above=below=0;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"reference_level":reference,"above_count":above,"below_count":below,"level_occupation":occupation,"median_level_displacement":median_displacement,"realized_variation":var})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["occupation_rank"]=prior_rank(panel.level_occupation.abs().where(valid));panel["median_displacement_rank"]=prior_rank(panel.median_level_displacement.abs().where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));panel["eligible"]=valid&panel.level_occupation.ne(0)&panel.occupation_rank.ge(P["occupation_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"]);panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_boundary_stale_occupation":
  current_valid=panel.source_valid.eq(True);cols=["reference_level","above_count","below_count","level_occupation","occupation_rank","median_level_displacement","median_displacement_rank","feature_available_time"];used[cols]=panel[cols].shift(1);used["source_valid"]=current_valid&panel.source_valid.shift(1).fillna(False)
 valid=used.source_valid.eq(True);nonzero=used.level_occupation.ne(0);strong=used.occupation_rank.ge(P["occupation_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&nonzero&strong&variation
 if control=="no_occupation_tail":state=valid&nonzero&variation
 elif control=="no_variation_gate":state=valid&nonzero&strong
 elif control=="median_level_displacement":state=valid&used.median_level_displacement.ne(0)&used.median_displacement_rank.ge(P["occupation_rank_min"])&variation
 onset=previous_valid_onset(state,valid);side=np.sign(pd.to_numeric(used.level_occupation,errors="coerce").fillna(0)).astype(int)
 if control=="median_level_displacement":side=np.sign(pd.to_numeric(used.median_level_displacement,errors="coerce").fillna(0)).astype(int)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVPLO prereg drift")
