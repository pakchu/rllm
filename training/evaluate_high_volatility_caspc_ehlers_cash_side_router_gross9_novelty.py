"""Evaluate the frozen HVCELVCSR-8 action-vote clock against Gross9."""
...
POLICY = "HVCELVCSR-8"
PROTOCOL = "hvcelvcsr_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_caspc_ehlers_cash_side_router_clocks_2023_2026.csv.gz",
    "sha256": "a74cb23179ce75ebc92d8898bc597ff55875692bdd8df1b17c24701fa6cd9640",
    "rows": 130,
}
OUTPUT = Path("results/high_volatility_caspc_ehlers_cash_side_router_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCELVCSR-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCELVCSR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCELVCSR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCELVCSR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCELVCSR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCELVCSR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCELVCSR-8 unchanged before economics",
    }
