"""Evaluate the frozen HVIPRAPR-8 action-vote clock against Gross9."""
...
POLICY = "HVIPRAPR-8"
PROTOCOL = "hviprapr_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_intraday_premium_activity_polarity_router_preregistration_2026-08-16.json")
PREREG_SHA = "1314bc12d8515c0aa8fd6d91973043d7b2d98f556f7c92b05abe6784ed3cb867"
SUPPORT = Path("results/high_volatility_intraday_premium_activity_polarity_router_support_2026-08-16.json")
SUPPORT_SHA = "6abde15a448c08cc6fc08350368d7bfae1a9edcf704595da8fa379d1cf8a6607"
CLOCK = {
    "path": "data/high_volatility_intraday_premium_activity_polarity_router_clocks_2023_2026.csv.gz",
    "sha256": "bbe1d83ecc450f4e6a0dadce7be7ed9b823e0e9ae521931c23696b55f01d13d7",
    "rows": 727,
}
OUTPUT = Path("results/high_volatility_intraday_premium_activity_polarity_router_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVIPRAPR-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVIPRAPR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVIPRAPR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVIPRAPR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVIPRAPR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVIPRAPR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVIPRAPR-8 unchanged before economics",
    }
