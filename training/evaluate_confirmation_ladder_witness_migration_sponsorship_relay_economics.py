"""Sequential strict economics for frozen CLWMSR-6."""
...
PREREG = Path("results/confirmation_ladder_witness_migration_sponsorship_relay_preregistration_2026-08-13.json")
PREREG_SHA = "76cf7b78547b63a5943b672ea8e308189424df0ba2f8a24a17edb05eca60d964"
SUPPORT = Path("results/confirmation_ladder_witness_migration_sponsorship_relay_support_2026-08-13.json")
SUPPORT_SHA = "488b99076ced61f4f420593df8b98524462c3bb381cee62c8a57b487ad2a7ffa"
NOVELTY = Path("results/confirmation_ladder_witness_migration_sponsorship_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "8e5536be84dec6e19390937a7e1611b6654bdd389cd5efffa2c7b2f19b6828a0"
CLOCK = Path("data/confirmation_ladder_witness_migration_sponsorship_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "f7298a9bbda437fdb37b6dfd8ee35492f522f32a5f62899c06e99eac586a44d5"
CONTROL_DIR = Path("data/confirmation_ladder_witness_migration_sponsorship_relay_controls_2023_2026")
FREEZE = Path("results/confirmation_ladder_witness_migration_sponsorship_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/confirmation_ladder_witness_migration_sponsorship_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CLWMSR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CLWMSR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "clwmsr_6_sequential_economics_v1", "policy_id": POLICY_ID,
