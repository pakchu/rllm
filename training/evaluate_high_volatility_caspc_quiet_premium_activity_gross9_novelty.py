"""Evaluate the frozen HVCASPCPQA-8 action-vote clock against Gross9."""
...
POLICY = "HVCASPCPQA-8"
PROTOCOL = "hvcaspcpqa_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_caspc_quiet_premium_activity_clocks_2023_2026.csv.gz",
    "sha256": "684ddfc240679d99ed5d5f30f095a359940c9ed098e5c6b0cf7f15d6fdf6bcce",
    "rows": 134,
}
OUTPUT = Path("results/high_volatility_caspc_quiet_premium_activity_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCASPCPQA-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCASPCPQA-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCASPCPQA-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCASPCPQA-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCASPCPQA-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCASPCPQA-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCASPCPQA-8 unchanged before economics",
    }
