"""Evaluate frozen HVLWRB-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVLWRB-24"
PROTOCOL = "hvlwrb_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_larry_williams_range_breakout_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVLWRB predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVLWRB novelty limits drift")
    if (
        ...
        raise RuntimeError("HVLWRB predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVLWRB primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdem_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVLWRB-24 unchanged before economic outcomes",
    }
