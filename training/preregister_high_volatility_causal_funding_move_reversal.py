"""Outcome-blind preregistration for HVCFMR-8."""
...
POLICY_ID = "HVCFMR-8"
DEFAULT_OUTPUT = Path("results/high_volatility_causal_funding_move_reversal_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_causal_funding_move_reversal_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": "An extreme absolute funding level marks leveraged crowding; when the completed settlement cycle also has high realized variation, its directional price displacement mean-reverts during the next cycle regardless of the funding sign.",
            "side": "negative strict sign of log(last_close[S]/first_open[P]) over [P,S)",
            "why_distinct": "Unlike HVFADR this policy does not use funding first differences or require price/funding divergence, and unlike HVCFLR it uses funding magnitude only as a crowding regime while direction comes exclusively from fading the completed price move.",
        ...
        "features": {
            ...
            "completed_return": "finite nonzero log(last coherent BTCUSDT 1m close before S / first coherent BTCUSDT 1m open at P) over exactly [P,S)",
            ...
            "eligible": "the exact eight-hour gap, finite nonzero current funding level and completed return, and both ranks pass at S; no funding-sign relation, zero-cross, divergence, onset, first difference, OI, premium, alt, or post-settlement condition",
            "no_imputation": True,
        },
        "clock": {
        ...
        "stopping_rule": "terminal first failure; no settlement gap, completed-return, history, level-rank, side, variation, clock, hold, subset, threshold, comparator, or control repair",
    }
    return {**core, "manifest_hash": canonical_hash(core)}

def validate(value: Mapping[str, Any]) -> None:
    if value != build():
        raise RuntimeError("HVCFMR-8 preregistration drift")
    contract = hvcav.build()
    for key in ("stages", "source_support_gates", "gross9_novelty_gates", "economic_gates"):
        if value[key] != contract[key]:
            raise RuntimeError(f"HVCFMR-8 {key} drift")
