"""Sequential strict economics for frozen HVIMI-24."""
...
PREREG = Path("results/high_volatility_intraday_momentum_index_reentry_relay_preregistration_2026-08-11.json")
PREREG_SHA = "f6446d3e6585fe56eaaab1ff6897e6e2659c3ba2c6bff255dd423cfee58fb1f7"
SUPPORT = Path("results/high_volatility_intraday_momentum_index_reentry_relay_support_2026-08-11.json")
SUPPORT_SHA = "a96128d56f7c125dca5c0c8c865b7cd8b728423a9aa64e88d1434a5b2d85c47c"
NOVELTY = Path("results/high_volatility_intraday_momentum_index_reentry_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "791de02675ba9fc98c234cb65f6ffc5ee53e70d70adeaaa64665dcc419105883"
CLOCK = Path("data/high_volatility_intraday_momentum_index_reentry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "133f92a81a6f665a5a895eb1adbcd700b5a7a7736470edc8244b7ce750519cc2"
CONTROL_DIR = Path("data/high_volatility_intraday_momentum_index_reentry_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_intraday_momentum_index_reentry_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_intraday_momentum_index_reentry_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVIMI frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVIMI novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvimi_24_sequential_economics_v1", "policy_id": POLICY_ID,
