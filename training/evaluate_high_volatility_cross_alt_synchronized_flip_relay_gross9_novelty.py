"""Evaluate frozen HVCASFR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCASFR-8"
PROTOCOL = "hvcasfr_8_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_cross_alt_synchronized_flip_relay_preregistration_2026-08-12.json"
)
PREREG_SHA = "dde630cabdc943a225bada19fd5e2f6000698ae50514d919450014b9c351a5bb"
SUPPORT = Path("results/high_volatility_cross_alt_synchronized_flip_relay_support_2026-08-12.json")
SUPPORT_SHA = "e93ac2ad1859120b4c21b8cc5fe767a745b886194810ba3f5bfb8dff253bed32"
CLOCK = Path("data/high_volatility_cross_alt_synchronized_flip_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "2aee9674b5d42d8018fbbc596dab21d233f3e865f0f4fc10fefab7e1ca60af6a"
OUTPUT = Path(
    "results/high_volatility_cross_alt_synchronized_flip_relay_gross9_novelty_2026-08-12.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCASFR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVCASFR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCASFR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCASFR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcasfr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVCASFR-8 unchanged before economic outcomes"
        ),
    }
