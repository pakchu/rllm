"""Build source-only support for frozen HVSRAR-8."""
...
ENV_FILE="/home/pakchu/rllm/.env";PREREG_SHA="75dd3a7e286421d274335b139806bff0191b4bad499edf13f4f1e7b0963ed939";START=pd.Timestamp("2023-01-01T00:00Z");END=pd.Timestamp("2026-08-01T00:00Z")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00Z"),pd.Timestamp("2024-01-01T00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00Z"),pd.Timestamp("2025-01-01T00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00Z"),pd.Timestamp("2026-01-01T00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00Z"),END)};MINIMUM={"train":8,"test":12,"eval":12,"final":8};CONTROLS=("no_run_share_gate","no_variation_gate","net_block_return_side","one_block_stale_features","direction_flip","same_clock_forced_long")
...
SOURCE_DIR=Path("data/high_volatility_sign_run_asymmetry_relay_sources_2023_2026");FEATURES=SOURCE_DIR/"eight_hour_sign_runs.csv.gz";MANIFEST=SOURCE_DIR/"manifest.json";CLOCK=Path("data/high_volatility_sign_run_asymmetry_relay_clocks_2023_2026.csv.gz");CONTROL_DIR=Path("data/high_volatility_sign_run_asymmetry_relay_controls_2023_2026");RESULT=Path("results/high_volatility_sign_run_asymmetry_relay_support_2026-08-09.json")
FEATURE_COLUMNS=("decision_time","feature_available_time","source_valid","positive_run","negative_run","run_asymmetry","run_share","realized_variation","block_return","run_share_rank","variation_rank")
...
def prior_midrank(v:pd.Series,lookback:int=270,minimum:int=252)->pd.Series:
 ...
 return out
def run_lengths(returns:np.ndarray)->tuple[int,int]:
 positive=negative=longest_positive=longest_negative=0
 for value in returns:
  if value>0:positive+=1;negative=0
  elif value<0:negative+=1;positive=0
  else:positive=negative=0
  longest_positive=max(longest_positive,positive);longest_negative=max(longest_negative,negative)
...
def build_features(bars:pd.DataFrame)->pd.DataFrame:
 ...
 if not req.issubset(bars):raise ValueError("HVSRAR source schema drift")
 b=bars.copy()
 ...
 for d in pd.date_range(START+pd.Timedelta(hours=8),END,freq="8h",inclusive="left"):
  ...
  hi=lo=0;run_asymmetry=share=variation=block_return=np.nan
  if valid:
   returns=np.log(w.bar_close.to_numpy(float)/w.bar_open.to_numpy(float));hi,lo=run_lengths(returns);run_asymmetry=hi-lo;total=hi+lo;share=abs(run_asymmetry)/total if total>0 else np.nan;variation=float(np.sqrt(np.square(returns).sum()));block_return=float(np.log(w.bar_close.iloc[-1]/w.bar_open.iloc[0]));valid=bool(run_asymmetry!=0 and total>0 and variation>0 and np.isfinite([share,variation,block_return]).all())
  if not valid:run_asymmetry=share=variation=block_return=np.nan
  rows.append({"decision_time":d,"feature_available_time":d,"source_valid":valid,"positive_run":hi if valid else np.nan,"negative_run":lo if valid else np.nan,"run_asymmetry":run_asymmetry,"run_share":share,"realized_variation":variation,"block_return":block_return})
 out=pd.DataFrame(rows);out["run_share_rank"]=prior_midrank(out.run_share.where(out.source_valid));out["variation_rank"]=prior_midrank(out.realized_variation.where(out.source_valid));return out[list(FEATURE_COLUMNS)]
def conditions(f:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 u=f.shift(1) if control=="one_block_stale_features" else f;valid=u.source_valid.eq(True)&u.run_asymmetry.ne(0);dg=pd.Series(True,index=f.index) if control=="no_run_share_gate" else u.run_share_rank.ge(.80);vg=pd.Series(True,index=f.index) if control=="no_variation_gate" else u.variation_rank.ge(.65);active=valid&dg&vg;basis=u.block_return if control=="net_block_return_side" else u.run_asymmetry;side=np.sign(basis).fillna(0).astype(int)
...
def run()->dict[str,Any]:
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVSRAR prereg drift")
