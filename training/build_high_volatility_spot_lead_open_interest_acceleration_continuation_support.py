"""Deterministic outcome-blind source support for HVSPOIAC-8."""
...
ROOT = Path("data/high_volatility_spot_lead_open_interest_acceleration_continuation_sources_2020_2026")
...
SPLIT_DIR = Path("data/high_volatility_spot_lead_open_interest_acceleration_continuation_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_spot_lead_open_interest_acceleration_continuation_support_2026-08-16.json")
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVSPOIAC-8 OI schema drift")
    ...
    if bars.columns.tolist() != expected:
        raise RuntimeError("HVSPOIAC-8 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all() or not oi["sum_open_interest"].ge(0).all():
        raise RuntimeError("HVSPOIAC-8 invalid OI rows")
    bars = bars.copy()
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVSPOIAC-8 side or availability drift")
...
def run() -> dict[str, Any]:
    ...
    source_core = {
        "protocol_version": "hvspoiac_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvspoiac_8_source_support_v1", "policy_id": prereg.POLICY_ID,
