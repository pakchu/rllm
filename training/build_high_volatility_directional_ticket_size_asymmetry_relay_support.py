"""Build outcome-blind source support for frozen HVDTSA-8."""
...
from training import preregister_high_volatility_directional_ticket_size_asymmetry_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-01-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="0a05070b5b1fa07907cd6310d81c9b6c1631e051067744f63d4c5edb66896a05"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_directional_ticket_size_asymmetry_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_directional_ticket_size_asymmetry_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_directional_ticket_size_asymmetry_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_directional_ticket_size_asymmetry_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_directional_ticket_size_asymmetry_relay_support_2026-08-10.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","up_minutes","down_minutes","median_ticket_up","median_ticket_down","ticket_asymmetry","asymmetry_magnitude","asymmetry_magnitude_rank","mean_ticket_asymmetry","realized_variation","variation_rank","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","up_minutes","down_minutes","median_ticket_up","median_ticket_down","ticket_asymmetry","asymmetry_magnitude","asymmetry_magnitude_rank","mean_ticket_asymmetry","realized_variation","variation_rank")
...
def ticket_statistics(returns:np.ndarray,turnover:np.ndarray,counts:np.ndarray)->tuple[float,...]:
 returns=np.asarray(returns,float);turnover=np.asarray(turnover,float);counts=np.asarray(counts,float)
 if any(x.shape!=(480,) for x in (returns,turnover,counts)) or not all(np.isfinite(x).all() for x in (returns,turnover,counts)) or np.any(counts<=0):return (math.nan,)*7
 tickets=turnover/counts;up=tickets[returns>0];down=tickets[returns<0]
 if len(up)<P["minimum_direction_minutes"] or len(down)<P["minimum_direction_minutes"]:return (float(len(up)),float(len(down)),math.nan,math.nan,math.nan,math.nan,math.nan)
 mu=float(np.median(up));md=float(np.median(down));den=mu+md;asym=(mu-md)/den if den>0 else math.nan;mean_asym=float(up.mean()-down.mean());variation=float(np.square(returns).sum())
 return float(len(up)),float(len(down)),mu,md,asym,mean_asym,variation
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close","quote_asset_volume","number_of_trades"]:raise RuntimeError("HVDTSA source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVDTSA invalid source key")
 prices=x[["open","high","low","close"]];counts=x.number_of_trades
 x["row_valid"]=np.isfinite(prices).all(axis=1)&prices.gt(0).all(axis=1)&x.high.ge(prices[["open","close"]].max(axis=1))&x.low.le(prices[["open","close"]].min(axis=1))&x.high.ge(x.low)&np.isfinite(x.quote_asset_volume)&x.quote_asset_volume.ge(0)&np.isfinite(counts)&counts.gt(0)&counts.eq(np.floor(counts))
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("12h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   upn,dnn,mu,md,asym,mean_asym,var=ticket_statistics(block.minute_return.to_numpy(float),block.quote_asset_volume.to_numpy(float),block.number_of_trades.to_numpy(float));valid=all(math.isfinite(v) for v in (mu,md,asym,mean_asym,var)) and asym!=0 and mean_asym!=0 and var>0
  else:upn=dnn=mu=md=asym=mean_asym=var=math.nan;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"up_minutes":upn,"down_minutes":dnn,"median_ticket_up":mu,"median_ticket_down":md,"ticket_asymmetry":asym,"asymmetry_magnitude":abs(asym) if math.isfinite(asym) else math.nan,"mean_ticket_asymmetry":mean_asym,"realized_variation":var})
...
def active(panel:pd.DataFrame,control:str="primary"):
 ...
 if control=="one_block_stale_geometry":
  cols=["source_valid","asymmetry_magnitude_rank","variation_rank","ticket_asymmetry","mean_ticket_asymmetry","feature_available_time"];used[cols]=panel[cols].shift(1)
 ...
 if control=="no_ticket_tail_gate":state=valid&variation
 elif control=="no_variation_gate":state=valid&dispersion
 onset=previous_valid_onset(state,valid);side=np.sign(pd.to_numeric(used.ticket_asymmetry,errors="coerce").fillna(0)).astype(int)
 if control=="mean_ticket_asymmetry":side=np.sign(pd.to_numeric(used.mean_ticket_asymmetry,errors="coerce").fillna(0)).astype(int)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVDTSA prereg drift")
 ...
 source_core={"protocol_version":"hvdtsa_8_source_v1","query":QUERY,"query_sha256":hashlib.sha256(QUERY.encode()).hexdigest(),"window":[START.isoformat(),END.isoformat()],"physical_rows":len(raw),"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"panel":{"path":str(PANEL),"sha256":sha(PANEL),"rows":len(panel),"valid_rows":int(panel.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};manifest={**source_core,"manifest_hash":chash(source_core)};immutable(MANIFEST,json_bytes(manifest))
