"""Evaluate frozen HVPASR-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVPASR-24"
PROTOCOL = "hvpasr_24_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_palladium_asymmetric_spillover_relay_gross9_novelty_2026-08-12.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVPASR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVPASR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVPASR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVPASR primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject HVPASR-24 unchanged before economic outcomes"
        ),
    }
