"""Evaluate frozen HVCRKC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCRKC-8"
PROTOCOL = "hvcrkc_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_count_return_kendall_concordance_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCRKC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCRKC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCRKC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCRKC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCRKC-8 unchanged before economic outcomes",
    }
