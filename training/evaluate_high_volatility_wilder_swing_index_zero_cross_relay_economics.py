"""Sequential strict economics for frozen HVWSI-Z20-24."""
...
PREREG = Path("results/high_volatility_wilder_swing_index_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "29f66b788fa07e6cc52afb6040b801abe67d418aa371545534272ddcedafebd8"
SUPPORT = Path("results/high_volatility_wilder_swing_index_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "a8492cbe89f593d402325ecccb02e8ab49caa753dd1017c3c968bd151235784d"
NOVELTY = Path("results/high_volatility_wilder_swing_index_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "40c23db02d4b425528ac2951c3d42551afd413bc8c79afee934e3f9572093a95"
CLOCK = Path("data/high_volatility_wilder_swing_index_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "bda458581bd59482d21e5a00298fba1a02f830c3c963fee261189ce7775b302d"
CONTROL_DIR = Path("data/high_volatility_wilder_swing_index_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_wilder_swing_index_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_wilder_swing_index_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "raw_close_change", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVWSI frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVWSI novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvwsi_z20_24_sequential_economics_v1", "policy_id": POLICY_ID,
