"""Deterministic outcome-blind source support for HVCPOFR-8."""
...
from training import preregister_high_volatility_causal_perpetual_flow_inventory_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "7912fcdea95d7fd8568c1cfaa9f9ecc015c8267bb7b232e51f742d833647b87c"
...
ROOT = Path("data/high_volatility_causal_perpetual_flow_inventory_relay_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_perpetual_flow_inventory_relay_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_perpetual_flow_inventory_relay_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_causal_perpetual_flow_inventory_relay_support_2026-08-16.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "sum_open_interest",
    "previous_open_interest", "oi_change", "signed_flow_share", "absolute_flow",
...
CLOCK_COLUMNS = (
    ...
    "absolute_flow", "absolute_flow_rank", "realized_variation", "variation_rank",
)
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVCPOFR-8 OI schema drift")
    expected_bars = [
        "decision_time", "signed_quote_flow", "quote_turnover", "minute_squared_return",
        "source_rows", "distinct_rows", "first_ts", "last_ts", "coherent",
    ]
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCPOFR-8 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all():
        raise RuntimeError("HVCPOFR-8 invalid OI rows")
    ...
    if bars["decision_time"].duplicated().any():
        raise RuntimeError("HVCPOFR-8 duplicate eight-hour bar")
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel = panel.join(
        bars[["bar_valid", "signed_flow_share", "absolute_flow", "realized_variation"]],
        on="decision_time",
    )
    panel["source_valid"] = (
        ...
        & np.isfinite(
            panel[["sum_open_interest", "previous_open_interest", "oi_change", "signed_flow_share", "absolute_flow", "realized_variation"]]
        ).all(axis=1)
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if split is None:
            continue
        side = int(np.sign(row.signed_flow_share))
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCPOFR-8 side or availability drift")
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCPOFR-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcpofr_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvcpofr_8_source_support_v1", "policy_id": prereg.POLICY_ID,
