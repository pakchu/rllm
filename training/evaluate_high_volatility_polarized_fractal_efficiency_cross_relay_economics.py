"""Sequential strict economics for frozen HVPFE-24."""
...
PREREG = Path("results/high_volatility_polarized_fractal_efficiency_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "474eabb0d8dca856e821bbcce4a400925aee1f5944bfa9f89244a959510d7cda"
SUPPORT = Path("results/high_volatility_polarized_fractal_efficiency_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "0231a459dfbc626c76602436f11cfe96255018cf9edb3c4e0d74298699a6e5b9"
NOVELTY = Path("results/high_volatility_polarized_fractal_efficiency_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "63952e23c532cd64947903f989859dca8ab757960e0fa4d508a0e300379fbfbe"
CLOCK = Path("data/high_volatility_polarized_fractal_efficiency_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "490c1944939b83be793a4022a9a3505c69b4d029d6a493740cdfd07ea0043ea5"
CONTROL_DIR = Path("data/high_volatility_polarized_fractal_efficiency_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_polarized_fractal_efficiency_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_polarized_fractal_efficiency_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "raw_net_displacement", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVPFE frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVPFE novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvpfe_24_sequential_economics_v1", "policy_id": POLICY_ID,
