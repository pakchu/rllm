"""Evaluate frozen SHRR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "SHRR-12"
PROTOCOL = "shrr_12_gross9_novelty_v1"
...
OUTPUT = Path("results/scandinavian_haven_risk_rotation_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("SHRR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("SHRR novelty limits drift")
    if (
        ...
        raise RuntimeError("SHRR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="SHRR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "shrr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject SHRR-12 unchanged before economic outcomes",
    }
