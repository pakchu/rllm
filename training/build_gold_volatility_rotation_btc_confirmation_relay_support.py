"""Build outcome-blind source support for preregistered GVRCR-6."""
...
from training import preregister_gold_volatility_rotation_btc_confirmation_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.build_cboe_volatility_surface_regime_crossing_relay_support import strict_prior_midrank

CLOCK = Path("data/gold_volatility_rotation_btc_confirmation_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/gold_volatility_rotation_btc_confirmation_relay_controls_2023_2026")
RESULT = Path("results/gold_volatility_rotation_btc_confirmation_relay_support_2026-08-08.json")
SPLITS = {
...
CONTROLS = ("gvz_change_only", "vix_change_only", "no_btc_confirmation", "one_session_stale_rotation", "direction_flip")
...
COLUMNS = (
    ...
    "side", "delta_log_relative", "absolute_rotation_rank", "delta_log_gvz", "delta_log_vix", "overnight_btc_return",
)
GVZ = Path("data/cboe_gvz_2021_2026/source/GVZ_History.csv")
...
def load_features() -> pd.DataFrame:
    for raw, expected in HASHES.items():
        if sha(raw) != expected:
            raise RuntimeError(f"GVRCR frozen source changed: {raw}")
    vix = pd.read_csv(VIX, compression="gzip", usecols=["observation_date", "VIX_close"])
    gvz = pd.read_csv(GVZ, usecols=["DATE", "GVZ"]).rename(columns={"DATE": "observation_date", "GVZ": "GVZ_close"})
    vix["observation_date"] = pd.to_datetime(vix.observation_date, format="%Y-%m-%d")
    gvz["observation_date"] = pd.to_datetime(gvz.observation_date, format="%m/%d/%Y")
    cboe = vix.merge(gvz, on="observation_date", validate="one_to_one").sort_values("observation_date").reset_index(drop=True)
    cboe["VIX_close"] = pd.to_numeric(cboe.VIX_close, errors="coerce")
    cboe["GVZ_close"] = pd.to_numeric(cboe.GVZ_close, errors="coerce")
    if not cboe.observation_date.is_monotonic_increasing or cboe.observation_date.duplicated().any():
        raise RuntimeError("GVRCR Cboe dates invalid")
    if not np.isfinite(cboe[["VIX_close", "GVZ_close"]]).all(axis=None) or not cboe[["VIX_close", "GVZ_close"]].gt(0).all(axis=None):
        raise RuntimeError("GVRCR volatility values invalid")
    cboe["delta_log_vix"] = np.log(cboe.VIX_close).diff()
    cboe["delta_log_gvz"] = np.log(cboe.GVZ_close).diff()
    cboe["delta_log_relative"] = np.log(cboe.GVZ_close / cboe.VIX_close).diff()
    cboe["absolute_rotation_rank"] = strict_prior_midrank(cboe.delta_log_relative.abs())
    cboe["absolute_gvz_rank"] = strict_prior_midrank(cboe.delta_log_gvz.abs())
    ...
    if price.columns.tolist() != ["hour_start", "hour_open", "first_half_close", "second_half_open", "hour_close", "source_rows", "distinct_timestamps", "source_valid", "decision_time"]:
        raise RuntimeError("GVRCR completed-price schema changed")
    ...
    if price.hour_start.duplicated().any() or not price.hour_start.is_monotonic_increasing:
        raise RuntimeError("GVRCR completed-price clock invalid")
    ...
    for index in range(1, len(cboe) - 1):
        ...
        rows.append({
            ...
            "absolute_gvz_rank": float(cboe.at[index, "absolute_gvz_rank"]),
            "absolute_vix_rank": float(cboe.at[index, "absolute_vix_rank"]),
            "delta_log_gvz": float(cboe.at[index, "delta_log_gvz"]),
...
def signal(frame: pd.DataFrame, control: str) -> pd.Series:
    ...
    if control == "gvz_change_only":
        eligible = frame.valid & frame.delta_log_gvz.ne(0) & frame.absolute_gvz_rank.ge(0.60)
        side = -np.sign(frame.delta_log_gvz).astype("Int64").fillna(0).astype(int)
...
def build_clock(frame: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in frame.index[sides.ne(0)]:
        ...
        rows.append({
            "candidate": "GVRCR-6", "control": control, "split": split,
            ...
            "delta_log_gvz": float(frame.at[index, "delta_log_gvz"]),
...
def run() -> dict[str, Any]:
    ...
    if registration.get("manifest_hash") != prereg.canonical_hash(registration_core):
        raise RuntimeError("GVRCR preregistration drift")
