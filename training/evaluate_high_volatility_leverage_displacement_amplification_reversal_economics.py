"""Sequential strict economics for frozen HVLDAR-8."""
...
PREREG = Path("results/high_volatility_leverage_displacement_amplification_reversal_preregistration_2026-08-13.json")
PREREG_SHA = "145d23f6a0ae48b1f7ab5af8ddc6f822292e2912b85ae43b24dc670cdbed830d"
SUPPORT = Path("results/high_volatility_leverage_displacement_amplification_reversal_support_2026-08-13.json")
SUPPORT_SHA = "5eb93047bd41e1115800f3870fe6e3b88d0fc436f3c631847cd4c1a4acbcfbd1"
NOVELTY = Path("results/high_volatility_leverage_displacement_amplification_reversal_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "00371cd034cfe7bfa968ce328779f240b2d4954383646cc0e5ca1da0190d3378"
CLOCK = Path("data/high_volatility_leverage_displacement_amplification_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "fc26fd2de7ecf6f22e77cecb716d0d496eebffa3b5492b6a3ac3a7ab84f99bb0"
CONTROL_DIR = Path("data/high_volatility_leverage_displacement_amplification_reversal_controls_2023_2026")
FREEZE = Path("results/high_volatility_leverage_displacement_amplification_reversal_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_leverage_displacement_amplification_reversal_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "rank_0_99", "cash_displacement_dominance", "one_bar_stale_amplification", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVLDAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVLDAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvldar_8_sequential_economics_v1", "policy_id": POLICY_ID,
