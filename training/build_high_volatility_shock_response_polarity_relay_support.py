"""Source-only support gate for frozen HVSRP-8."""
...
from training import preregister_high_volatility_shock_response_polarity_relay as prereg
...
PREREG_SHA = "2164103502c9bebabce4ece727e961533076e8b1e05bf1b145f7f0765c06e09e"
SPLITS = {
...
ROOT = Path("data/high_volatility_shock_response_polarity_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_shock_response_polarity_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_shock_response_polarity_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_shock_response_polarity_relay_support_2026-08-10.json")
...
def response_correlation(returns: np.ndarray) -> float:
    if len(returns) != 480 or not np.isfinite(returns).all():
        return math.nan
    shock_magnitude = np.abs(returns[:-1])
    next_signed_return = returns[1:]
    if np.std(shock_magnitude) <= 0 or np.std(next_signed_return) <= 0:
        return math.nan
    correlation = float(np.corrcoef(shock_magnitude, next_signed_return)[0, 1])
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVSRP source timestamps")
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        if valid:
            ...
            response = response_correlation(returns)
            block_return = float(np.log(window["close"].iloc[-1] / window["open"].iloc[0]))
            valid = variation > 0 and math.isfinite(response)
        if not valid:
            variation = response = block_return = math.nan
        rows.append(
            {
    ...
    states["response_strength_rank"] = prior_rank(states["response_correlation"].abs().where(states["source_valid"]))
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvsrp_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    response_rank = states["response_strength_rank"]
    response = states["response_correlation"]
    if control == "one_block_stale_response":
        block_return, variation_rank, response_rank, response = [
            value.shift(1)
            for value in (block_return, variation_rank, response_rank, response)
        ]
    variation_gate = (
    ...
    if control == "no_response_tail":
        response_gate = pd.Series(True, index=states.index)
    elif control == "raw_absolute_response_above_tenth":
        response_gate = response.abs().gt(0.10)
    else:
        response_gate = response_rank.ge(0.75)
    agreement = (
        block_return.ne(0)
        & response.ne(0)
        & np.sign(block_return).eq(np.sign(response))
    )
    eligible = states["source_valid"] & variation_gate & response_gate & agreement
        ...
        rows.append(
            {
                ...
                "response_correlation": float(states.at[index, "response_correlation"]),
                "response_strength_rank": float(states.at[index, "response_strength_rank"]),
            }
        )
    columns = [
        "candidate", "control", "split", "decision_time", "feature_available_time",
        "entry_time", "exit_time", "side", "realized_variation", "variation_rank",
        "response_correlation", "response_strength_rank",
    ]
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVSRP preregistration drift")
    ...
    core = {
        "protocol_version": "hvsrp_8_source_support_v1",
        "policy_id": "HVSRP-8",
        "preregistration": {
