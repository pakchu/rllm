"""Evaluate frozen HVHHM-C10-N5-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVHHM-C10-N5-8"
PROTOCOL = "hvhhm_c10_n5_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_hammer_hanging_man_context_reversal_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVHHM predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVHHM novelty limits drift")
    if (
        ...
        raise RuntimeError("HVHHM predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVHHM primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvhhm_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVHHM-C10-N5-8 unchanged before economic outcomes",
    }
