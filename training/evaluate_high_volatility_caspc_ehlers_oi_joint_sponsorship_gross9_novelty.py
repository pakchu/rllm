"""Evaluate the frozen HVCELVOIS-8 action-vote clock against Gross9."""
...
from training import preregister_high_volatility_caspc_ehlers_oi_joint_sponsorship as prereg

POLICY = "HVCELVOIS-8"
PROTOCOL = "hvcelvois_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_caspc_ehlers_oi_joint_sponsorship_clocks_2023_2026.csv.gz",
    "sha256": "13b3dd6af0c567a4d8b2fac8e4ea92991130d88ce6f7af9e1ceacd6b5d8a7053",
    "rows": 64,
}
OUTPUT = Path("results/high_volatility_caspc_ehlers_oi_joint_sponsorship_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCELVOIS-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCELVOIS-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCELVOIS-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCELVOIS-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCELVOIS-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCELVOIS-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCELVOIS-8 unchanged before economics",
    }
