"""Sequential strict economics for frozen CARSC-8."""
...
PREREG = Path("results/cross_alt_return_synchrony_continuation_relay_preregistration_2026-08-10.json")
PREREG_SHA = "fa9ed9a30e8cbd0532b690acd585d371c91c91a725e7c1b8f4f3187544d0c8e4"
SUPPORT = Path("results/cross_alt_return_synchrony_continuation_relay_support_2026-08-10.json")
SUPPORT_SHA = "4302f99eae2772618c94af86af1f951efae1e7b0148c8c6432f2368e802b78a1"
NOVELTY = Path("results/cross_alt_return_synchrony_continuation_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "847be3fa882f345b487aa0fac363f7da2681b436df44294bbdc3b8ea54448d62"
CLOCK = Path("data/cross_alt_return_synchrony_continuation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "871d7201a04ae938294948d321abb23e99e7b745ddccc1149491940562b69e52"
CONTROL_DIR = Path("data/cross_alt_return_synchrony_continuation_relay_controls_2023_2026")
FREEZE = Path("results/cross_alt_return_synchrony_continuation_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/cross_alt_return_synchrony_continuation_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_synchrony_gate", "raw_median_correlation_above_half", "one_block_stale_synchrony", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CARSC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CARSC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "carsc_8_sequential_economics_v1", "policy_id": POLICY_ID,
