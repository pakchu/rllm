"""Materialize deterministic, outcome-blind source support for HVCASPCWMV-8."""
...
CLOCK = Path("data/high_volatility_caspc_weekly_momentum_active_veto_clocks_2023_2026.csv.gz")
RESULT = Path("results/high_volatility_caspc_weekly_momentum_active_veto_support_2026-08-17.json")
...
def _registration() -> dict[str, Any]:
    if prereg.sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCASPCWMV-8 preregistration hash drift")
    ...
    for component, artifacts in prereg.COMPONENT_ARTIFACTS.items():
        for kind, artifact in artifacts.items():
            if prereg.sha256_file(artifact["path"]) != artifact["sha256"]:
                raise RuntimeError(f"HVCASPCWMV-8 {component} {kind} drift")
        ...
        if support != {"policy_id": component, "support_passed": True}:
            raise RuntimeError(f"HVCASPCWMV-8 {component} source gate drift")
        if gross9 != {"policy_id": component, "source_support_passed": True, "every_gross9_sleeve_passed": True, "gross9_novelty_status": "passed"}:
            raise RuntimeError(f"HVCASPCWMV-8 {component} Gross9 gate drift")
...
def load_clock(component: str) -> pd.DataFrame:
    ...
    if frame["feature_available_time"].duplicated().any() or not frame["side"].isin((-1, 1)).all():
        raise RuntimeError(f"HVCASPCWMV-8 invalid {component} clock")
    expected_hours = (3, 11, 19) if component == prereg.ACTION_IDS[0] else (0,)
    if not frame["feature_available_time"].dt.hour.isin(expected_hours).all() or not frame["feature_available_time"].dt.minute.eq(0).all():
        raise RuntimeError(f"HVCASPCWMV-8 off-grid {component} clock")
    if component == prereg.ACTION_IDS[1] and not frame["feature_available_time"].dt.dayofweek.eq(2).all():
        raise RuntimeError(f"HVCASPCWMV-8 off-weekday {component} clock")
    ...
    if not frame["entry_time"].eq(frame["feature_available_time"] + ENTRY_DELAY).all() or not frame["exit_time"].eq(frame["entry_time"] + expected_hold).all():
        raise RuntimeError(f"HVCASPCWMV-8 {component} timing drift")
    if not frame["feature_available_time"].le(frame["entry_time"]).all():
        raise RuntimeError(f"HVCASPCWMV-8 unavailable {component} feature")
...
def build_clock(clocks: Mapping[str, pd.DataFrame]) -> tuple[pd.DataFrame, dict[str, Any]]:
    if set(clocks) != set(prereg.COMPONENT_IDS):
        raise RuntimeError("HVCASPCWMV-8 component roster drift")
    ...
    for decision in decisions:
        ...
        if len(active_vetoes) > 1:
            raise RuntimeError("HVCASPCWMV-8 overlapping active veto positions")
        ...
        if primary.split != split or primary.feature_available_time > entry:
            raise RuntimeError("HVCASPCWMV-8 primary stage or availability drift")
        if veto is not None and (veto.split != split or veto.feature_available_time > entry):
            raise RuntimeError("HVCASPCWMV-8 veto stage or availability drift")
    ...
    if counts["emitted_decisions"] + counts["veto_opposite_side"] != counts["primary_decisions"]:
        raise RuntimeError("HVCASPCWMV-8 incidence accounting drift")
    if len(output) > 1 and not output["entry_time"].iloc[1:].reset_index(drop=True).ge(output["exit_time"].iloc[:-1].reset_index(drop=True)).all():
        raise RuntimeError("HVCASPCWMV-8 output overlap")
