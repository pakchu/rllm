"""Sequential strict economics for frozen HVDVBR-12."""
...
PREREG = Path("results/high_volatility_dominant_volume_bar_relay_preregistration_2026-08-09.json")
PREREG_SHA = "8f9d6445fad819a351ff1cac9c7e1ddc7dec0f713f7425970c30bc4497731176"
SUPPORT = Path("results/high_volatility_dominant_volume_bar_relay_support_2026-08-09.json")
SUPPORT_SHA = "35c49e90eca64c936a96b695a2a4e842433e09663a57e06cbadbb66fed8b99a4"
NOVELTY = Path("results/high_volatility_dominant_volume_bar_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "03a0cdce8e6a99a0df24d1eed3816ef37157c6ed28cf1d78ffc1b2fe78209d50"
CLOCK = Path("data/high_volatility_dominant_volume_bar_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "db8dccd0be1872d0a3f21e0a2ee12180ded2714902279dd04005959143dcc449"
CONTROL_DIR = Path("data/high_volatility_dominant_volume_bar_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_dominant_volume_bar_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_dominant_volume_bar_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "first_dominant_tie_break", "block_direction_only", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDVBR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDVBR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdvbr_12_sequential_economics_v1", "policy_id": POLICY_ID,
