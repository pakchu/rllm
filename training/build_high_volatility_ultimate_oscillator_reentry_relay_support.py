"""Build outcome-blind source support for frozen HVUO-24."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_ultimate_oscillator_reentry_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2020-01-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="9828cf36ed7d2a5a69b1514bfd12ac269b21e2d54266813c1f18a14fad649db7";REG=prereg.build();P=REG["policy"]
...
ROOT=Path("data/high_volatility_ultimate_oscillator_reentry_relay_sources_2023_2026");PANEL=ROOT/"four_hour_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_ultimate_oscillator_reentry_relay_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_ultimate_oscillator_reentry_relay_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_ultimate_oscillator_reentry_relay_controls_2023_2026");RESULT=Path("results/high_volatility_ultimate_oscillator_reentry_relay_support_2026-08-11.json");BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("source_start","feature_available_time","source_valid","bar_open","bar_high","bar_low","bar_close","buying_pressure","true_range","bp_sum_7","tr_sum_7","bp_sum_14","tr_sum_14","bp_sum_28","tr_sum_28","ultimate_oscillator","equal_weight_oscillator","variation","variation_rank","eligible")
CLOCK_COLUMNS=("candidate","control","split","source_start","feature_available_time","entry_time","exit_time","side","bar_close","buying_pressure","true_range","bp_sum_7","tr_sum_7","bp_sum_14","tr_sum_14","bp_sum_28","tr_sum_28","ultimate_oscillator","equal_weight_oscillator","variation","variation_rank","eligible")
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVUO source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVUO invalid source key")
...
def ultimate_oscillator(high:pd.Series,low:pd.Series,close:pd.Series,valid:pd.Series)->pd.DataFrame:
 transition=valid&valid.shift(1,fill_value=False);previous=close.shift(1);floor=pd.concat([low,previous],axis=1).min(axis=1);ceiling=pd.concat([high,previous],axis=1).max(axis=1);bp=(close-floor).where(transition);tr=(ceiling-floor).where(transition);out={"buying_pressure":bp,"true_range":tr};averages=[]
 for name,period in (("7",P["short_periods"]),("14",P["medium_periods"]),("28",P["long_periods"])):
  bsum=bp.rolling(period,min_periods=period).sum();tsum=tr.rolling(period,min_periods=period).sum();out[f"bp_sum_{name}"]=bsum;out[f"tr_sum_{name}"]=tsum;averages.append((bsum/tsum).where(tsum.gt(0)))
 weighted=100*(P["short_weight"]*averages[0]+P["medium_weight"]*averages[1]+P["long_weight"]*averages[2])/(P["short_weight"]+P["medium_weight"]+P["long_weight"]);equal=100*sum(averages)/3;out["ultimate_oscillator"]=weighted;out["equal_weight_oscillator"]=equal;return pd.DataFrame(out,index=high.index)

def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 source=prepare(raw).reindex(pd.date_range(START,END,freq="1min",inclusive="left"));g=source.groupby(source.index.floor("4h"),sort=True);bars=pd.DataFrame({"rows":g.row_valid.sum(),"bar_open":g.open.first(),"bar_high":g.high.max(),"bar_low":g.low.min(),"bar_close":g.close.last(),"variation_component":g.minute_sq_return.sum(min_count=240)});bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[["bar_open","bar_high","bar_low","bar_close"]]).all(axis=1)&bars[["bar_open","bar_high","bar_low","bar_close"]].gt(0).all(axis=1);bars=pd.concat([bars,ultimate_oscillator(bars.bar_high,bars.bar_low,bars.bar_close,bars.valid_bar)],axis=1);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());required=["buying_pressure","true_range","bp_sum_7","tr_sum_7","bp_sum_14","tr_sum_14","bp_sum_28","tr_sum_28","ultimate_oscillator","equal_weight_oscillator","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[required]).all(axis=1)&bars.tr_sum_7.gt(0)&bars.tr_sum_14.gt(0)&bars.tr_sum_28.gt(0)&bars.variation.gt(0);panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.ultimate_oscillator.shift(1);long=previous.le(P["lower_level"])&panel.ultimate_oscillator.gt(P["lower_level"]);short=previous.ge(P["upper_level"])&panel.ultimate_oscillator.lt(P["upper_level"]);panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&(long|short)&panel.variation_rank.ge(P["variation_rank_min"]);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);value=pd.to_numeric(used.equal_weight_oscillator if control=="equal_horizon_weights" else used.ultimate_oscillator,errors="coerce");previous=value.shift(1);long=previous.le(P["lower_level"])&value.gt(P["lower_level"]);short=previous.ge(P["upper_level"])&value.lt(P["upper_level"]);side=pd.Series(np.where(long,1,np.where(short,-1,0)),index=used.index,dtype=int);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&side.ne(0)&variation
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVUO prereg drift")
