"""Strict staged economics for the singleton HVCVARCLIR-8 policy."""
...
PREREG = Path("results/high_volatility_causal_variance_acceleration_close_location_inventory_relay_preregistration_2026-08-16.json")
PREREG_SHA = "74e43c0942827bfee4bbabc37eb845aaeb3734137843db3d87a0f9a3afb2f40a"
SUPPORT = Path("results/high_volatility_causal_variance_acceleration_close_location_inventory_relay_support_2026-08-16.json")
SUPPORT_SHA = "4cb3e878eaba828fe8c3c050eefac1a8d8c9ca8dafb6d40f1419084bbfda9a41"
GROSS9 = Path("results/high_volatility_causal_variance_acceleration_close_location_inventory_relay_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "abc97c3a5e03917e89b93a66da6eb9eab3eebe42cf405c2cfb420b3cf6b84eb3"
CLOCK = {
    "path": Path("data/high_volatility_causal_variance_acceleration_close_location_inventory_relay_clocks_2020_2026.csv.gz"),
    "sha256": "51a194abe3d415422ffc19c596613175244f278352aeff76c20c794fc00f13dc",
    "rows": 113,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_variance_acceleration_close_location_inventory_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCVARCLIR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCVARCLIR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCVARCLIR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCVARCLIR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVARCLIR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVARCLIR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCVARCLIR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCVARCLIR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCVARCLIR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCVARCLIR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCVARCLIR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcvarclir_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
