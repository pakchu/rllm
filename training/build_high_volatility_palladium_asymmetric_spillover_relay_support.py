"""Materialize outcome-blind source support for frozen HVPASR-24."""
...
from training import preregister_high_volatility_palladium_asymmetric_spillover_relay as prereg
...
BUILDER = Path("training/build_high_volatility_palladium_asymmetric_spillover_relay_support.py")
PREREG_SHA256 = "2bee32685189f497337af215b14940e40884cd6f6ce9980bfd1cc10cbf21e9b6"
SYMBOLS = ("PALL", "GLD")
SOURCE_DIR = Path("data/high_volatility_palladium_asymmetric_spillover_relay_sources_2022_2026")
...
FEATURE_PANEL = SOURCE_DIR / "hvpasr_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_palladium_asymmetric_spillover_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_palladium_asymmetric_spillover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_palladium_asymmetric_spillover_relay_support_2026-08-12.json")
...
def normalize_yahoo_chart(payload: bytes, symbol: str) -> tuple[bytes, pd.DataFrame, dict[str, Any]]:
    ...
    if chart.get("error") is not None:
        raise RuntimeError(f"HVPASR Yahoo chart error for {symbol}")
    results = chart.get("result") or []
    if len(results) != 1:
        raise RuntimeError(f"HVPASR Yahoo result count drift for {symbol}")
    result = results[0]
    meta = result.get("meta") or {}
    if meta.get("symbol") != symbol or meta.get("exchangeTimezoneName") != "America/New_York":
        raise RuntimeError(f"HVPASR {symbol} Yahoo metadata drift")
    timestamps = result.get("timestamp") or []
    indicators = result.get("indicators") or {}
    quotes = indicators.get("quote") or []
    adjusted = indicators.get("adjclose") or []
    if len(quotes) != 1 or len(adjusted) != 1 or not timestamps:
        raise RuntimeError(f"HVPASR {symbol} adjusted chart incomplete")
    quote = quotes[0]
    ...
    if any(values is None or len(values) != len(timestamps) for values in vectors.values()):
        raise RuntimeError(f"HVPASR {symbol} Yahoo vector length drift")
    ...
    if frame.empty or frame.session_date.duplicated().any():
        raise RuntimeError(f"HVPASR {symbol} session dates invalid")
    ...
    if not valid.all():
        ...
        raise RuntimeError(f"HVPASR {symbol} invalid adjusted rows: {dates[:5]}")
    metadata = {
        "symbol": symbol,
...
def build_equity_panel(equities: Mapping[str, pd.DataFrame]) -> pd.DataFrame:
    if set(equities) != set(SYMBOLS):
        raise RuntimeError("HVPASR equity symbol set drift")
    ...
    for symbol in SYMBOLS:
        ...
        if not observed.equals(official):
            ...
            raise RuntimeError(
                f"HVPASR {symbol}/NYSE calendar mismatch missing={missing[:5]} extra={extra[:5]}"
            )
        if expected is None:
            expected = observed
        elif not observed.equals(expected):
            raise RuntimeError(f"HVPASR {symbol} does not share every official common session")
    ...
    if len(panel) != len(expected):
        raise RuntimeError("HVPASR common-session intersection dropped an official session")
    ...
    if observed_early != expected_early:
        raise RuntimeError("HVPASR frozen early-close schedule drift")
    return panel
...
def normalize_btc_bars(
    ...
    if raw.columns.tolist() != ["ts", "open", "close"]:
        raise RuntimeError("HVPASR BTC schema drift")
    frame = raw.copy()
    ...
    if len(frame) != len(expected) or not frame.ts.equals(pd.Series(expected, name="ts")):
        raise RuntimeError("HVPASR BTC source is not the exact requested 1m grid")
    for column in ("open", "close"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    values = frame[["open", "close"]]
    if not np.isfinite(values.to_numpy(dtype=float)).all() or not values.gt(0.0).all(axis=None):
        raise RuntimeError("HVPASR BTC prices invalid")
    return frame
...
def build_features(sessions: pd.DataFrame, bars: pd.DataFrame) -> pd.DataFrame:
    ...
    if not isinstance(indexed.index, pd.DatetimeIndex) or indexed.index.tz is None:
        raise RuntimeError("HVPASR BTC index must be timezone-aware")
    ...
    frame["pall_beta"] = strict_prior_beta(
        frame.gld_return, frame.pall_return, lookback=60, minimum=40
    )
    frame["pall_residual"] = frame.pall_return - frame.pall_beta * frame.gld_return
    frame["prior_pall_residual"] = frame.pall_residual.shift(1)
...
def _signal(frame: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    score = frame.pall_residual.copy()
    prior = frame.prior_pall_residual.copy()
    active = score.notna() & prior.notna() & score.ne(0.0) & prior.ne(0.0) & score.mul(prior).lt(0.0)
    if control == "one_session_stale_transition":
        score, active = score.shift(1), active.shift(1, fill_value=False)
    elif control == "raw_pall_return_transition":
        score = frame.pall_return
        raw_prior = score.shift(1)
        active = score.notna() & raw_prior.notna() & score.ne(0.0) & raw_prior.ne(0.0) & score.mul(raw_prior).lt(0.0)
    elif control == "residual_level_without_transition":
        ...
        rows.append(
            {
                ...
                "pall_beta": float(frame.at[index, "pall_beta"]),
                "pall_residual": float(frame.at[index, "pall_residual"]),
                "prior_pall_residual": float(frame.at[index, "prior_pall_residual"]),
...
def run(env_file: str = ENV_FILE) -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA256:
        raise RuntimeError("HVPASR preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvpasr_24_sources_v1",
    ...
    core = {
        "protocol_version": "hvpasr_24_source_support_v1",
        "policy_id": "HVPASR-24",
        "preregistration": {
