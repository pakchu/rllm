"""Strict sequential economics for the sole eligible HVMCPAC-8 pair."""
...
GROSS9_SHA = "de26f154dfa803b570f5b5d865b3e74e345aa3462a7f2236eaa6b79c8ffc9bda"
PAIR_CLOCK = Path(
...
PAIR_CLOCK_ROWS = 85
# Conventional economics-evaluator aliases retained for artifact tooling.
NOVELTY = GROSS9
NOVELTY_SHA = GROSS9_SHA
CLOCK = PAIR_CLOCK
CLOCK_SHA = PAIR_CLOCK_SHA
...
OUTPUTS = {
    stage: Path(
        "results/high_volatility_multi_condition_pairwise_concordance_"
        f"{stage}_economics_2026-08-14.json"
    )
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVMCPAC-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVMCPAC-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        or pair_support.get("support_passed") is not True
        or clock.get("path") != PAIR_CLOCK.as_posix()
        or clock.get("sha256") != PAIR_CLOCK_SHA
        or clock.get("rows") != PAIR_CLOCK_ROWS
    ):
        raise RuntimeError("HVMCPAC-8 sole source-supported pair drift")
    if (
        gross9.get("policy_id") != POLICY_ID
        or gross9.get("candidate") != PAIR
        ...
        raise RuntimeError("HVMCPAC-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVMCPAC-8 predecessor: {path}")
    report = load_manifest(path)
    if (
        report.get("policy_id") != POLICY_ID
        or report.get("candidate") != PAIR
        or report.get("stage") != stage
        or report.get("passed") is not True
        or report.get("frozen_train_winner") != PAIR
        or report.get("substitution_authorized") is not False
    ):
        raise RuntimeError("HVMCPAC-8 predecessor did not pass with the frozen winner")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVMCPAC-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVMCPAC-8 predecessor chain drift")
    return report


def _verify_predecessor(stage: str) -> dict[str, Any] | None:
    if stage == "train":
        return None
    return _load_passing_stage_report(PREDECESSOR[stage])


def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any] | None]:
    """Verify authorization and the immediate stage predecessor before any prices open."""
...
def load_postgres_sources(
        ...
        with database.connect() as connection:
            market = pd.read_sql_query(
                ...
                params={
                    "symbol": "BTCUSDT",
                    "start": start.to_pydatetime(),
                    "query_end": (end + BAR).to_pydatetime(),
                },
            )
    ...
    source = {
        "mode": "postgres_exact_1m_to_5m",
        "tables": ["bars_binance", "funding_rates_binance"],
        "symbol": "BTCUSDT",
    }
    return (
        market[["date", "open", "high", "low", "close"]],
        funding[["date", "funding_rate", "mark_price"]],
        source,
    )


def load_sources(
    stage: str, start: pd.Timestamp, end: pd.Timestamp
) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    if stage == "train":
        ...
        source = {
            ...
            "market_sha256": legacy.MARKET_SHA,
            "funding_marks_sha256": engine.TRAIN_FUNDING_SHA,
        }
    else:
...
def evaluate_primary(
    ...
    for name, left, right in (
        ...
        subset = clock[
            (clock["entry_time"] >= left) & (clock["exit_time"] <= right)
        ]
        halves[name] = public_metrics(
            engine.simulate(subset, market, funding, left, right, BASE_COST)
        )
    return {
...
def economic_checks(stage: str, primary: Mapping[str, Any]) -> dict[str, bool]:
    ...
    checks = {
        ...
        "each_calendar_half_positive": all(
            ...
            for half in primary["calendar_halves"].values()
        ),
    }
    if stage == "train":
        checks["train_bonferroni_raw_weekly_p_max_0_1_over_6"] = (
            weekly_p <= TRAIN_BONFERRONI_RAW_P_MAX
        )
    return checks
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    # No price or funding loader is reached until authorization and predecessor pass.
    ...
    clock = legacy.load_clock(PAIR_CLOCK, split, start, end)
    primary = evaluate_primary(clock, market, funding, start, end)
    checks = economic_checks(stage, primary)
    passed = all(checks.values())

    if stage == "train":
        frozen_train_winner = PAIR if passed else None
    else:
        frozen_train_winner = predecessor_report["frozen_train_winner"]
    predecessor = None
    if stage != "train":
    ...
    if not passed:
        decision = "terminal_reject_no_substitution"
    elif stage == "final":
        decision = "pass_final"
    else:
        decision = "pass"
    core = {
        "protocol_version": "hvmcpac_8_strict_sequential_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": PAIR,
        "stage": stage,
        ...
        "frozen_bindings": {
            ...
            "gross9_novelty": {
                "path": GROSS9.as_posix(),
                "sha256": GROSS9_SHA,
                "manifest_hash": gross9["manifest_hash"],
            },
            "pair_clock": {
                "path": PAIR_CLOCK.as_posix(),
                "sha256": PAIR_CLOCK_SHA,
                "rows": PAIR_CLOCK_ROWS,
            },
        },
        "selection": {
            "source_and_gross9_eligible_pairs": [PAIR],
            "raw_train_rank_one": PAIR,
...
if __name__ == "__main__":
    ...
    if args.verify_only:
        ...
        print(
            json.dumps(
                {"stage": args.stage, "verified": True, "outcomes_opened": False}
            )
        )
    else:
            ...
            json.dumps(
                {
                    ...
                    "frozen_train_winner": result["frozen_train_winner"],
