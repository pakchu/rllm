"""Sequential strict economics for frozen HVCACJP-6."""
...
PREREG = Path("results/high_volatility_cross_alt_cojump_propagation_relay_preregistration_2026-08-11.json")
PREREG_SHA = "95c3a7ddcfec5e89865a489f80a262a85cdb790d698a11d81281970e143f4e42"
SUPPORT = Path("results/high_volatility_cross_alt_cojump_propagation_relay_support_2026-08-11.json")
SUPPORT_SHA = "0f31fb6537f46269c222b2cd553e8111805b70f86ba51b450a9c8d6073ca21c2"
NOVELTY = Path("results/high_volatility_cross_alt_cojump_propagation_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "83afa31b0bb45f61063e8014426b3c0f26b8552aebb2c5aff7b8ee7908a8cfa5"
CLOCK = Path("data/high_volatility_cross_alt_cojump_propagation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "ba0aa4396d001fa0a91fd1dca8dbc79ff2c5134343f9d1af5775da8cc0b39bf4"
CONTROL_DIR = Path("data/high_volatility_cross_alt_cojump_propagation_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_cross_alt_cojump_propagation_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_alt_cojump_propagation_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "three_of_six_cojump", "btc_confirmation", "one_bar_stale_cojump", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCACJP frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCACJP novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcacjp_6_sequential_economics_v1", "policy_id": POLICY_ID,
