"""Source-only support gate for frozen HVDCSOIS-8."""
...
from training import preregister_high_volatility_directional_change_scarcity_open_interest_sponsorship_relay as prereg
...
PREREG_SHA = "098c6e713e8db00eef987911170783b81eaac7ffa34b2e4bbdcacd254e16152a"
SPLITS = {
...
ROOT = Path("data/high_volatility_directional_change_scarcity_open_interest_sponsorship_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_directional_change_scarcity_open_interest_sponsorship_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_directional_change_scarcity_open_interest_sponsorship_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_directional_change_scarcity_open_interest_sponsorship_relay_support_2026-08-16.json")
QUERY = """SELECT ts,open,high,low,close
FROM bars_binance
WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end
ORDER BY ts"""
OI_QUERY = """SELECT ts,sum_open_interest
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    with database.connect() as connection:
        ...
        oi = pd.read_sql_query(
            ...
            connection,
            params={
                "start": (START - pd.Timedelta("24h")).to_pydatetime(),
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVDCSOIS source timestamps")
    frame = frame.set_index("ts").sort_index()
    if oi.columns.tolist() != ["ts", "sum_open_interest"]:
        raise RuntimeError("HVDCSOIS OI source schema drift")
    oi["ts"] = pd.to_datetime(oi["ts"], utc=True, errors="raise")
    oi["sum_open_interest"] = pd.to_numeric(oi["sum_open_interest"], errors="raise")
    if (
        oi["ts"].duplicated().any()
        or not np.isfinite(oi["sum_open_interest"]).all()
        or not oi["sum_open_interest"].ge(0).all()
    ):
        raise RuntimeError("invalid HVDCSOIS OI source")
    oi_series = oi.set_index("ts").sort_index()["sum_open_interest"]
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        oi_start = oi_series.get(decision - pd.Timedelta("24h"), math.nan)
        oi_end = oi_series.get(decision, math.nan)
        oi_valid = (
            math.isfinite(oi_start)
            and math.isfinite(oi_end)
            and oi_start > 0
            and oi_end > 0
        )
        oi_change = math.log(oi_end / oi_start) if oi_valid else math.nan
        valid = bool(valid and oi_valid and math.isfinite(oi_change))
        rows.append(
            {
    ...
    core = {
        "protocol_version": "hvdcs_ois_source_v1",
        "query": QUERY,
        ...
        "physical_rows": {"bars": len(frame), "open_interest": len(oi)},
    }
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    oi_gate = (
        pd.Series(True, index=states.index)
        if control == "no_open_interest_sponsorship_gate"
        else oi_change.gt(0)
    )
    eligible = states["source_valid"] & variation_gate & change_gate & agreement & oi_gate
        ...
        rows.append(
            {
                ...
                "oi_start": float(states.at[index, "oi_start"]),
                "oi_end": float(states.at[index, "oi_end"]),
                "oi_change": float(states.at[index, "oi_change"]),
            }
        )
    columns = [
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVDCSOIS preregistration drift")
    ...
    core = {
        "protocol_version": "hvdcs_ois_8_source_support_v1",
        "policy_id": "HVDCSOIS-8",
        "preregistration": {
