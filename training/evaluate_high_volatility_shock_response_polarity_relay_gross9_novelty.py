"""Evaluate frozen HVSRP-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVSRP-8"
PROTOCOL = "hvsrp_8_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_shock_response_polarity_relay_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVSRP predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVSRP novelty limits drift")
    if (
        ...
        raise RuntimeError("HVSRP predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVSRP primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvsrp_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVSRP-8 unchanged before economic outcomes"
        ),
    }
