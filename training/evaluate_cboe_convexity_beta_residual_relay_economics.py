"""Sequential strict economics for frozen CCBRR-12."""
...
PREREG = Path("results/cboe_convexity_beta_residual_relay_preregistration_2026-08-08.json")
PREREG_SHA = "05e102f95e56ac5d5dedda42180809f2a28c20d6ace50143b31cacd5ead7c1f1"
SUPPORT = Path("results/cboe_convexity_beta_residual_relay_support_2026-08-08.json")
SUPPORT_SHA = "bb8d920d59a69eb2c2f15ea463f28bbbf62d79542d6a82bf717189ee38cfcb3e"
NOVELTY = Path("results/cboe_convexity_beta_residual_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "5abf7590698fb9cf77100a36a2dabf33e70156875476ed59a1a9a44f5e23f7c5"
CLOCK = Path("data/cboe_convexity_beta_residual_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "9ce41c6a863b156e8dfb449441c2dca19e11cab6d70c8994a61322a5e2ded240"
CONTROL_DIR = Path("data/cboe_convexity_beta_residual_relay_controls_2023_2026")
FREEZE = Path("results/cboe_convexity_beta_residual_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/cboe_convexity_beta_residual_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
CONTROLS = ("raw_vvix_change", "fixed_beta_one", "no_magnitude_gate", "one_session_stale_residual", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CCBRR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CCBRR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "ccbrr_12_sequential_economics_v1", "policy_id": POLICY_ID,
