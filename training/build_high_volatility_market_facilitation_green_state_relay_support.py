...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_market_facilitation_green_state_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2020-01-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="2de5ec2a1e596ab81cfdc2175f9d67372b47e0da614d69c5a3426aae7821ccaa";REG=prereg.build();P=REG["policy"]
...
ROOT=Path("data/high_volatility_market_facilitation_green_state_relay_sources_2023_2026");PANEL=ROOT/"four_hour_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_market_facilitation_green_state_relay_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_market_facilitation_green_state_relay_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_market_facilitation_green_state_relay_controls_2023_2026");RESULT=Path("results/high_volatility_market_facilitation_green_state_relay_support_2026-08-11.json");BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("source_start","feature_available_time","source_valid","bar_open","bar_high","bar_low","bar_close","base_volume","bar_range","bw_mfi","green_state","range_volume_up","bar_side","variation","variation_rank","eligible")
CLOCK_COLUMNS=("candidate","control","split","source_start","feature_available_time","entry_time","exit_time","side","bar_close","base_volume","bar_range","bw_mfi","green_state","range_volume_up","bar_side","variation","variation_rank","eligible")
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVMFI invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 source=prepare(raw).reindex(pd.date_range(START,END,freq="1min",inclusive="left"));g=source.groupby(source.index.floor("4h"),sort=True);bars=pd.DataFrame({"rows":g.row_valid.sum(),"bar_open":g.open.first(),"bar_high":g.high.max(),"bar_low":g.low.min(),"bar_close":g.close.last(),"base_volume":g.volume.sum(min_count=240),"variation_component":g.minute_sq_return.sum(min_count=240)});bars["bar_range"]=bars.bar_high-bars.bar_low;bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[["bar_open","bar_high","bar_low","bar_close","base_volume","bar_range"]]).all(axis=1)&bars[["bar_open","bar_high","bar_low","bar_close","base_volume","bar_range"]].gt(0).all(axis=1);bars["bw_mfi"]=bars.bar_range/bars.base_volume;transition=bars.valid_bar&bars.valid_bar.shift(1,fill_value=False);bars["green_state"]=transition&bars.bw_mfi.gt(bars.bw_mfi.shift(1))&bars.base_volume.gt(bars.base_volume.shift(1));bars["range_volume_up"]=transition&bars.bar_range.gt(bars.bar_range.shift(1))&bars.base_volume.gt(bars.base_volume.shift(1));bars["bar_side"]=np.sign(bars.bar_close-bars.bar_open).fillna(0).astype(int);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=transition&np.isfinite(bars[["bw_mfi","variation"]]).all(axis=1)&bars.bw_mfi.gt(0)&bars.bar_side.ne(0)&bars.variation.gt(0);panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));panel["eligible"]=panel.source_valid&panel.green_state&panel.variation_rank.ge(P["variation_rank_min"]);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy();valid=used.source_valid.eq(True);side=pd.to_numeric(used.bar_side,errors="coerce").fillna(0).astype(int);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&used.green_state.eq(True)&side.ne(0)&variation
 if control=="no_variation_gate":state=valid&used.green_state.eq(True)&side.ne(0)
 elif control=="range_and_volume_up":state=valid&used.range_volume_up.eq(True)&side.ne(0)&variation
 elif control=="one_bar_stale_state":state=state.shift(1,fill_value=False);side=side.shift(1,fill_value=0)
