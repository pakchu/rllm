"""Build source-only support for frozen HVDRA-8."""
...
ENV_FILE="/home/pakchu/rllm/.env";PREREG_SHA="af78e34ab3de62615fdd77b9404bb5423ce3b6f5c741499391c4a7ff54d1efca";START=pd.Timestamp("2023-01-01T00:00Z");END=pd.Timestamp("2026-08-01T00:00Z")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00Z"),pd.Timestamp("2024-01-01T00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00Z"),pd.Timestamp("2025-01-01T00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00Z"),pd.Timestamp("2026-01-01T00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00Z"),END)};MINIMUM={"train":8,"test":12,"eval":12,"final":8};CONTROLS=("no_acceleration_gate","no_variation_gate","net_block_return_side","one_block_stale_features","direction_flip","same_clock_forced_long")
...
SOURCE_DIR=Path("data/high_volatility_directional_return_acceleration_relay_sources_2023_2026");FEATURES=SOURCE_DIR/"eight_hour_directional_acceleration.csv.gz";MANIFEST=SOURCE_DIR/"manifest.json";CLOCK=Path("data/high_volatility_directional_return_acceleration_relay_clocks_2023_2026.csv.gz");CONTROL_DIR=Path("data/high_volatility_directional_return_acceleration_relay_controls_2023_2026");RESULT=Path("results/high_volatility_directional_return_acceleration_relay_support_2026-08-09.json")
FEATURE_COLUMNS=("decision_time","feature_available_time","source_valid","first_half_return","second_half_return","half_direction","acceleration_ratio","realized_variation","block_return","variation_rank")
...
def prior_midrank(v:pd.Series,lookback:int=270,minimum:int=252)->pd.Series:
 ...
 return out
def half_returns(opens:np.ndarray,closes:np.ndarray)->tuple[float,float,float]:
 first=float(np.log(closes[47]/opens[0]));second=float(np.log(closes[95]/opens[48]));ratio=abs(second)/abs(first) if first!=0 else np.nan;return first,second,ratio
def engine():
...
def build_features(bars:pd.DataFrame)->pd.DataFrame:
 ...
 if not req.issubset(bars):raise ValueError("HVDRA source schema drift")
 b=bars.copy()
 ...
 for d in pd.date_range(START+pd.Timedelta(hours=8),END,freq="8h",inclusive="left"):
  ...
  first_return=second_return=half_direction=ratio=variation=block_return=np.nan
  if valid:
   opens=w.bar_open.to_numpy(float);closes=w.bar_close.to_numpy(float);returns=np.log(closes/opens);first_return,second_return,ratio=half_returns(opens,closes);half_direction=float(np.sign(first_return));variation=float(np.sqrt(np.square(returns).sum()));block_return=float(np.log(closes[-1]/opens[0]));valid=bool(first_return!=0 and second_return!=0 and np.sign(first_return)==np.sign(second_return) and variation>0 and np.isfinite([ratio,variation,block_return]).all())
  if not valid:first_return=second_return=half_direction=ratio=variation=block_return=np.nan
  rows.append({"decision_time":d,"feature_available_time":d,"source_valid":valid,"first_half_return":first_return,"second_half_return":second_return,"half_direction":half_direction,"acceleration_ratio":ratio,"realized_variation":variation,"block_return":block_return})
...
def run()->dict[str,Any]:
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVDRA prereg drift")
 ...
 sc={"protocol_version":"hvdra_8_source_v1","query_sha256":hashlib.sha256(BAR_QUERY.encode()).hexdigest(),"table":"bars_binance","window":[START.isoformat(),END.isoformat()],"physical_rows_1m":int(pd.to_numeric(bars.source_rows).sum()),"features":{"path":str(FEATURES),"sha256":sha(FEATURES),"rows":len(features),"valid_rows":int(features.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};sm={**sc,"manifest_hash":chash(sc)};MANIFEST.write_text(json.dumps(sm,indent=2,allow_nan=False)+"\n");support={n:stats(primary,n) for n in SPLITS};checks={}
