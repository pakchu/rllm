"""Deterministic outcome-blind source support for HVCATTAIR-8."""
...
from training import preregister_high_volatility_causal_average_ticket_trade_arrival_inventory_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "fe9e9acf1dfd983c97a211d152a2e6cbf2a08a2729577ef48e92d1c2aa2c8e96"
...
ROOT = Path("data/high_volatility_causal_average_ticket_trade_arrival_inventory_relay_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_average_ticket_trade_arrival_inventory_relay_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_average_ticket_trade_arrival_inventory_relay_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_causal_average_ticket_trade_arrival_inventory_relay_support_2026-08-16.json")
...
PANEL_COLUMNS = (
    ...
    "average_ticket_acceleration", "trade_arrival_acceleration", "average_ticket_acceleration_rank", "realized_variation",
...
CLOCK_COLUMNS = (
    ...
    "first_half_trade_count", "second_half_trade_count", "first_half_average_ticket", "second_half_average_ticket", "completed_return",
    "average_ticket_acceleration", "trade_arrival_acceleration", "average_ticket_acceleration_rank", "realized_variation", "variation_rank",
)
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVCATTAIR-8 OI schema drift")
    expected_bars = [
    ...
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCATTAIR-8 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all():
        raise RuntimeError("HVCATTAIR-8 invalid OI rows")
    ...
    if bars["decision_time"].duplicated().any():
        raise RuntimeError("HVCATTAIR-8 duplicate eight-hour bar")
    ...
    bars["trade_arrival_acceleration"] = np.log(bars["second_half_trade_count"] / bars["first_half_trade_count"])
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel = panel.join(
        bars[["bar_valid", "first_half_quote_turnover", "second_half_quote_turnover", "first_half_trade_count", "second_half_trade_count", "first_half_average_ticket", "second_half_average_ticket", "completed_return", "average_ticket_acceleration", "trade_arrival_acceleration", "realized_variation"]],
        on="decision_time",
    )
    panel["source_valid"] = (
        ...
        & np.isfinite(
            panel[["sum_open_interest", "previous_open_interest", "oi_change", "first_half_quote_turnover", "second_half_quote_turnover", "first_half_trade_count", "second_half_trade_count", "first_half_average_ticket", "second_half_average_ticket", "completed_return", "average_ticket_acceleration", "trade_arrival_acceleration", "realized_variation"]]
        ).all(axis=1)
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCATTAIR-8 side or availability drift")
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCATTAIR-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcattair_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvcattair_8_source_support_v1", "policy_id": prereg.POLICY_ID,
