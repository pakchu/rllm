"""Evaluate frozen CATACF-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "CATACF-8"
PROTOCOL = "catacf_8_gross9_novelty_v1"
PREREG = Path("results/cross_asset_trade_arrival_concentration_fracture_preregistration_2026-08-09.json")
PREREG_SHA = "6b057ca960e2638cd1e0b4283ed9d9ac60ac598c9c192bd066dbc214864f1aa4"
SUPPORT = Path("results/cross_asset_trade_arrival_concentration_fracture_support_2026-08-09.json")
SUPPORT_SHA = "45bf64a6a352bf35f681957c86b867f3be29492a7b221c0bcc6304cbfb2835c5"
CLOCK = Path("data/cross_asset_trade_arrival_concentration_fracture_clocks_2023_2026.csv.gz")
CLOCK_SHA = "022fdbe4ea4860769b388be409fdf2407031d95e663ff800c9214c8d831eaec9"
OUTPUT = Path("results/cross_asset_trade_arrival_concentration_fracture_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CATACF predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("CATACF novelty limits drift")
    if (
        ...
        raise RuntimeError("CATACF predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CATACF primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "catacf_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject CATACF-8 unchanged before economic outcomes",
    }
