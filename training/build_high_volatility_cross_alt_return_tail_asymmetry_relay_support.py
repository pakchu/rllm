"""Build outcome-blind source support for frozen HVCARTAR-8."""
...
from training import preregister_high_volatility_cross_alt_return_tail_asymmetry_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-04-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="6497a661d784e1caede0f1f81bb70f7531bcab1ea6df0e4c5b9af3117d774268"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_cross_alt_return_tail_asymmetry_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_cross_alt_return_tail_asymmetry_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_cross_alt_return_tail_asymmetry_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_cross_alt_return_tail_asymmetry_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_cross_alt_return_tail_asymmetry_relay_support_2026-08-11.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","tail_asymmetry","tail_asymmetry_rank","mean_centered_tail_asymmetry","absolute_tail_mass","realized_variation","variation_rank","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","tail_asymmetry","tail_asymmetry_rank","mean_centered_tail_asymmetry","absolute_tail_mass","realized_variation","variation_rank")
...
def return_tail_asymmetry_statistics(block:pd.DataFrame)->tuple[float,float,float,float]:
 alt_returns=[]
 for symbol in ALTS:
  x=block.xs(symbol,level="symbol");value=float(np.log(float(x.close.iloc[-1])/float(x.open.iloc[0])))
  if not math.isfinite(value):return math.nan,math.nan,math.nan,math.nan
  alt_returns.append(value)
 values=np.asarray(alt_returns,float);median_centered=values-np.median(values);mean_centered=values-np.mean(values)
 ...
 return asymmetry,mean_asymmetry,absolute_mass,variation
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","symbol","open","high","low","close"]:raise RuntimeError("HVCARTAR source schema drift")
 ...
 if x.ts.isna().any() or x.duplicated(["ts","symbol"]).any() or not x.symbol.isin(SYMBOLS).all():raise RuntimeError("HVCARTAR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("12h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   pressure,equal,mass,var=return_tail_asymmetry_statistics(block);valid=all(math.isfinite(v) for v in (pressure,equal,mass,var)) and pressure!=0 and mass>0 and var>0
  else:pressure=equal=mass=var=math.nan;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"tail_asymmetry":pressure,"mean_centered_tail_asymmetry":equal,"absolute_tail_mass":mass,"realized_variation":var})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["tail_asymmetry_rank"]=prior_rank(panel.tail_asymmetry.abs().where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));panel["eligible"]=valid&panel.tail_asymmetry_rank.ge(P["tail_asymmetry_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"]);panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_boundary_stale_returns":
  cols=["source_valid","tail_asymmetry","tail_asymmetry_rank","mean_centered_tail_asymmetry","absolute_tail_mass","variation_rank","feature_available_time"];used[cols]=panel[cols].shift(1)
 valid=used.source_valid.eq(True);strong=used.tail_asymmetry_rank.ge(P["tail_asymmetry_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&strong&variation
 if control=="no_tail_asymmetry_gate":state=valid&variation
 elif control=="no_variation_gate":state=valid&strong
 onset=previous_valid_onset(state,valid);side=np.sign(pd.to_numeric(used.tail_asymmetry,errors="coerce").fillna(0)).astype(int)
 if control=="mean_centered_cubic_mass":side=np.sign(pd.to_numeric(used.mean_centered_tail_asymmetry,errors="coerce").fillna(0)).astype(int)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCARTAR prereg drift")
 ...
 source_core={"protocol_version":"hvcartar_8_source_v1","query":QUERY,"query_sha256":hashlib.sha256(QUERY.encode()).hexdigest(),"window":[START.isoformat(),END.isoformat()],"physical_rows":len(raw),"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"panel":{"path":str(PANEL),"sha256":sha(PANEL),"rows":len(panel),"valid_rows":int(panel.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};manifest={**source_core,"manifest_hash":chash(source_core)};immutable(MANIFEST,json_bytes(manifest))
