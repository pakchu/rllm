"""Evaluate the frozen HVBFRRFC-8 action-vote clock against Gross9."""
...
POLICY = "HVBFRRFC-8"
PROTOCOL = "hvbfrrfc_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_btc_factor_residual_flip_continuation_clocks_2023_2026.csv.gz",
    "sha256": "29ce66d8f9d8c0606262087fb71ab9dc6a3c7d5e215e50f03f72ee3d34a4613b",
    "rows": 202,
}
OUTPUT = Path("results/high_volatility_btc_factor_residual_flip_continuation_gross9_novelty_2026-08-18.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVBFRRFC-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVBFRRFC-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVBFRRFC-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVBFRRFC-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVBFRRFC-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVBFRRFC-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVBFRRFC-8 unchanged before economics",
    }
