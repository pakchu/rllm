"""Materialize source-only HVCREDIT-24 clocks before Gross9 or economics."""
...
from training import preregister_high_volatility_credit_risk_appetite_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.build_scheduled_trend_concordance_relay_support import load_market


PREREG_SHA = "edef0d226484713b2e9cb2497d5da388d7fb4fde90aeed6a34f0f3a0e405ae50"
SOURCE_SHA = "2b13f01d7b6fa7e79afd11d1307b799f6b9213b6c53c33190df677daef30ae00"
SOURCE = Path("data/high_volatility_credit_risk_appetite_relay_sources_2022_2026/hyg_lqd_sessions.csv.gz")
SOURCE_MANIFEST = SOURCE.parent / "build_manifest.json"
SOURCE_MANIFEST_SHA = "1cd912eff4eafee41abf36e2aa9486aa2230a7d87bcb615ec5a0bf1341167b0b"
...
STATE = SOURCE.parent / "daily_credit_states.csv.gz"
CLOCK = Path("data/high_volatility_credit_risk_appetite_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_credit_risk_appetite_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_credit_risk_appetite_relay_support_2026-08-10.json")
...
CONTROLS = ("no_variation_gate", "hyg_only", "inverse_lqd_only", "one_session_stale_credit", "direction_flip", "forced_long")
COLUMNS = (
    "candidate", "control", "split", "session_date", "decision_time",
    "feature_available_time", "entry_time", "exit_time", "side", "hyg_return",
    "lqd_return", "relative_credit_return", "btc_variation", "btc_variation_rank",
)
...
def score_sessions(source: pd.DataFrame, market: pd.DataFrame) -> pd.DataFrame:
    ...
    if frame.session_date.duplicated().any() or not frame.session_date.is_monotonic_increasing:
        raise RuntimeError("HVCREDIT source session order drift")
    for column in ("hyg_open", "hyg_close", "lqd_open", "lqd_close"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    frame["hyg_return"] = np.log(frame.hyg_close / frame.hyg_open)
    frame["lqd_return"] = np.log(frame.lqd_close / frame.lqd_open)
    frame["relative_credit_return"] = frame.hyg_return - frame.lqd_return
    ...
    frame["source_valid"] = np.isfinite(frame[["hyg_return", "lqd_return", "relative_credit_return"]]).all(axis=1) & frame.relative_credit_return.ne(0)
...
def build_clock(states: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    used = states.shift(1) if control == "one_session_stale_credit" else states
    hyg, lqd, relative = used.hyg_return, used.lqd_return, used.relative_credit_return
    valid = used.source_valid.eq(True) & np.isfinite(hyg) & np.isfinite(lqd) & np.isfinite(relative)
    relation = relative.ne(0)
    ...
    side = np.sign(relative).fillna(0).astype(int)
    if control == "hyg_only":
        side = np.sign(hyg).fillna(0).astype(int)
    elif control == "inverse_lqd_only":
        side = -np.sign(lqd).fillna(0).astype(int)
    ...
    for index in states.index[active]:
        ...
        rows.append({
            ...
            "side": int(side.at[index]), "hyg_return": float(hyg.at[index]), "lqd_return": float(lqd.at[index]),
            "relative_credit_return": float(relative.at[index]),
...
def run() -> dict[str, Any]:
    ...
    for path, expected in bindings.items():
        if sha(path) != expected:
            raise RuntimeError(f"HVCREDIT binding drift: {path}")
    ...
    core = {
        "protocol_version": "hvcredit_24_source_support_v1", "policy_id": prereg.POLICY_ID,
