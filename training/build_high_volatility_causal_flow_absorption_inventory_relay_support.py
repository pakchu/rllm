"""Deterministic outcome-blind source support for HVCFAIR-8."""
...
from training import preregister_high_volatility_causal_flow_absorption_inventory_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "d3e73a0ac1c6aeb4c5abd9b01afac2e629f1ef17f0c1d3011db51bc8b1778d91"
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVCFAIR-8 OI schema drift")
    expected_bars = [
        "decision_time", "signed_quote_flow", "quote_turnover", "completed_return", "minute_squared_return",
        "source_rows", "distinct_rows", "first_ts", "last_ts", "coherent",
    ]
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCFAIR-8 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all():
        raise RuntimeError("HVCFAIR-8 invalid OI rows")

    bars = bars.copy()
    for column in ("decision_time", "first_ts", "last_ts"):
        bars[column] = pd.to_datetime(bars[column], utc=True, errors="raise")
    numeric = ("signed_quote_flow", "quote_turnover", "completed_return", "minute_squared_return", "source_rows", "distinct_rows")
    for column in numeric:
        bars[column] = pd.to_numeric(bars[column], errors="raise")
    if bars["decision_time"].duplicated().any():
        raise RuntimeError("HVCFAIR-8 duplicate eight-hour bar")
    ...
    bars["signed_flow_share"] = bars["signed_quote_flow"] / bars["quote_turnover"]
    bars["absolute_flow"] = bars["signed_flow_share"].abs()
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel = panel.join(
        bars[["bar_valid", "signed_flow_share", "absolute_flow", "completed_return", "realized_variation"]],
        on="decision_time",
    )
    panel["source_valid"] = (
        ...
        & np.isfinite(
            panel[["sum_open_interest", "previous_open_interest", "oi_change", "signed_flow_share", "absolute_flow", "completed_return", "realized_variation"]]
        ).all(axis=1)
        & panel["sum_open_interest"].gt(0)
        & panel["previous_open_interest"].gt(0)
        & panel["realized_variation"].gt(0)
        & panel["signed_flow_share"].ne(0)
        & panel["absolute_flow"].gt(0)
        & panel["completed_return"].ne(0)
        & np.sign(panel["signed_flow_share"]).eq(-np.sign(panel["completed_return"]))
    )
    valid = panel["source_valid"].eq(True)
    panel["absolute_flow_rank"] = causal_midrank(panel["absolute_flow"].where(valid))
    panel["variation_rank"] = causal_midrank(panel["realized_variation"].where(valid))
    panel["eligible"] = (
        valid
        & panel["oi_change"].gt(0)
        & panel["absolute_flow_rank"].ge(POLICY["absolute_flow_rank_min"])
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCFAIR-8 side or availability drift")
        ...
        rows.append({
            ...
            "signed_flow_share": float(row.signed_flow_share),
            "absolute_flow": float(row.absolute_flow),
            "completed_return": float(row.completed_return),
            "absolute_flow_rank": float(row.absolute_flow_rank),
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCFAIR-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcfair_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvcfair_8_source_support_v1", "policy_id": prereg.POLICY_ID,
