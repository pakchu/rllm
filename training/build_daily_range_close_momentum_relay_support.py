"""Materialize source-only DRCMR-12 support clocks."""
...
ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2023-01-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");SOURCE_DIR=Path("data/daily_range_close_momentum_relay_sources_2023_2026");DAILY=SOURCE_DIR/"btc_daily_auction.csv.gz";SOURCE_MANIFEST=SOURCE_DIR/"manifest.json";CLOCK=Path("data/daily_range_close_momentum_relay_clocks_2023_2026.csv.gz");CONTROL_DIR=Path("data/daily_range_close_momentum_relay_controls_2023_2026");RESULT=Path("results/daily_range_close_momentum_relay_support_2026-08-08.json")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00:00Z"),pd.Timestamp("2024-01-01T00:00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00:00Z"),pd.Timestamp("2025-01-01T00:00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00:00Z"),pd.Timestamp("2026-01-01T00:00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00:00Z"),pd.Timestamp("2026-08-01T00:00:00Z"))};MINIMUM={"train":8,"test":12,"eval":12,"final":8};CONTROLS=("no_range_gate","no_close_location_gate","one_day_stale_geometry","direction_flip");COLUMNS=("candidate","control","split","source_day","decision_time","feature_available_time","entry_time","exit_time","side","daily_open","daily_high","daily_low","daily_close","log_range","range_rank","close_location")
QUERY="""SELECT date_trunc('day',ts) AS source_day,(array_agg(open ORDER BY ts))[1] AS daily_open,max(high) AS daily_high,min(low) AS daily_low,(array_agg(close ORDER BY ts DESC))[1] AS daily_close,count(*) AS source_rows,count(DISTINCT ts) AS distinct_timestamps,min(ts) AS first_ts,max(ts) AS last_ts FROM bars_binance WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end GROUP BY 1 ORDER BY 1"""
...
def materialize()->dict:
 ...
 f["source_valid"]=f.source_rows.eq(1440)&f.distinct_timestamps.eq(1440)&f.first_ts.eq(f.source_day)&f.last_ts.eq(f.source_day+pd.Timedelta(hours=23,minutes=59))&np.isfinite(f[["daily_open","daily_high","daily_low","daily_close"]]).all(axis=1)&f[["daily_open","daily_high","daily_low","daily_close"]].gt(0).all(axis=1)&f.daily_high.ge(f[["daily_open","daily_close"]].max(axis=1))&f.daily_low.le(f[["daily_open","daily_close"]].min(axis=1))&f.daily_high.gt(f.daily_low);f["log_range"]=np.log(f.daily_high/f.daily_low).where(f.source_valid);f["range_rank"]=strict_prior_midrank(f.log_range);f["close_location"]=(f.daily_close-f.daily_low)/(f.daily_high-f.daily_low);f["decision_time"]=f.source_day+pd.Timedelta(days=1)
 SOURCE_DIR.mkdir(parents=True,exist_ok=True);_write_gzip_csv(f,DAILY);core={"protocol_version":"drcmr_12_daily_btc_source_v1","query":QUERY,"table":"bars_binance","symbol":"BTCUSDT","interval":"1m","window":[START.isoformat(),END.isoformat()],"outcomes_opened":False,"candidate_incidence_opened":False,"no_imputation":True,"output":{"path":str(DAILY),"sha256":sha(DAILY),"rows":len(f),"valid_rows":int(f.source_valid.sum())}};p={**core,"manifest_hash":chash(core)};SOURCE_MANIFEST.write_text(json.dumps(p,indent=2,allow_nan=False)+"\n");return p
...
def conditions(f:pd.DataFrame,control:str)->tuple[pd.Series,pd.Series]:
 op,cl,loc,rank=f.daily_open,f.daily_close,f.close_location,f.range_rank
 if control=="one_day_stale_geometry":op,cl,loc,rank=op.shift(1),cl.shift(1),loc.shift(1),rank.shift(1)
 range_gate=pd.Series(True,index=f.index) if control=="no_range_gate" else rank.ge(.65);upper=loc.ge(.8)&cl.gt(op);lower=loc.le(.2)&cl.lt(op)
 if control=="no_close_location_gate":upper=cl.gt(op);lower=cl.lt(op)
 active=f.signal_valid&np.isfinite(op)&np.isfinite(cl)&np.isfinite(loc)&np.isfinite(rank)&range_gate&(upper|lower);side=pd.Series(np.where(upper,1,-1),index=f.index);side=-side if control=="direction_flip" else side;return active,side
def clock(f:pd.DataFrame,control:str="primary")->pd.DataFrame:
 ...
 for i in f.index[active]:
  ...
  rows.append({"candidate":"DRCMR-12","control":control,"split":split,"source_day":f.at[i,"source_day"],"decision_time":decision,"feature_available_time":decision,"entry_time":entry,"exit_time":exit_,"side":int(side.at[i]),"daily_open":float(f.at[i,"daily_open"]),"daily_high":float(f.at[i,"daily_high"]),"daily_low":float(f.at[i,"daily_low"]),"daily_close":float(f.at[i,"daily_close"]),"log_range":float(f.at[i,"log_range"]),"range_rank":float(f.at[i,"range_rank"]),"close_location":float(f.at[i,"close_location"])})
