"""Evaluate frozen OIER-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "OIER-8"
PROTOCOL = "oier_6_gross9_novelty_v1"
PREREG = Path("results/oi_expansion_recoil_relay_preregistration_2026-08-09.json")
PREREG_SHA = "f1261431cded1858134707b05549a349cf6601c0ffdd86978ea75f4d7cd2861d"
SUPPORT = Path("results/oi_expansion_recoil_relay_support_2026-08-09.json")
SUPPORT_SHA = "5d8f61727e9399a664c23b7dc1e04ebdd67f02b321a5b8ddb0fe5b03767a7cf3"
CLOCK = Path("data/oi_expansion_recoil_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d0b53433cea121b6f2f8489c951c08da46d1526dff8e7e668d4da1810284e388"
OUTPUT = Path("results/oi_expansion_recoil_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("OIER predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("OIER novelty limits drift")
    if (
        ...
        raise RuntimeError("OIER predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="OIER primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "oier_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject OIER-8 unchanged before economic outcomes",
    }
