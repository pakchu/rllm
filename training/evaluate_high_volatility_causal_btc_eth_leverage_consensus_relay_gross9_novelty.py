"""Evaluate the frozen HVCBER-8 action-vote clock against Gross9."""
...
POLICY = "HVCBER-8"
PROTOCOL = "hvcber_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_btc_eth_leverage_consensus_relay_clocks_2020_2026.csv.gz",
    "sha256": "00aa9c7d7eaf2389b26d55c9c42caf5c99c6d747603b293d2c9b61b82ab1ff54",
    "rows": 341,
}
OUTPUT = Path("results/high_volatility_causal_btc_eth_leverage_consensus_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCBER-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCBER-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCBER-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCBER-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCBER-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCBER-8 unchanged before economics",
    }
