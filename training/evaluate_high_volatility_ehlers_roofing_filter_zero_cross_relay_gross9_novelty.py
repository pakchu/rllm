"""Evaluate frozen HVERF-24 structural novelty against every Gross9 sleeve."""
...
from training import preregister_high_volatility_ehlers_roofing_filter_zero_cross_relay as registration

POLICY = "HVERF-24"
PROTOCOL = "hverf_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_ehlers_roofing_filter_zero_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVERF predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVERF novelty limits drift")
    if (
        ...
        raise RuntimeError("HVERF predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVERF primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hverf_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVERF-24 unchanged before economic outcomes",
    }
