"""Outcome-blind preregistration for HVCFIR-8."""
...
POLICY_ID = "HVCFIR-8"
DEFAULT_OUTPUT = Path("results/high_volatility_causal_funding_impulse_relay_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_causal_funding_impulse_relay_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": "When the funding-rate first difference reverses sign after the prior settlement in a causally high-variation cycle, an elevated new impulse identifies a change in leverage-demand direction that persists over the next settlement cycle.",
            "side": "strict sign of funding_rate[S]-funding_rate[P]",
            "why_distinct": "Unlike HVFADR and HVCFDR, this policy requires two consecutive funding first differences with exactly opposite signs and follows the new impulse; it has no price/funding divergence or onset rule and does not fade the current shock.",
        ...
        "features": {
            ...
            "prior_settlement": "settlement Q immediately before P with P-Q exactly 8 elapsed hours",
            "funding_change": "finite nonzero funding_rate[S]-funding_rate[P]",
            "previous_funding_change": "finite nonzero funding_rate[P]-funding_rate[Q]",
            "impulse_reversal": "strict sign(funding_rate[S]-funding_rate[P]) equals negative strict sign(funding_rate[P]-funding_rate[Q])",
            "change_rank": "strict-prior midrank of absolute funding_change over at most 270 source-valid cycles; current excluded; minimum 180; rank>=0.60",
            ...
            "eligible": "both exact eight-hour gaps, both finite nonzero changes, exact opposite signs, and both ranks pass at S; no divergence, onset, funding-level, OI, premium, alt, or post-settlement condition",
            "no_imputation": True,
        },
        "clock": {
        ...
        "stopping_rule": "terminal first failure; no settlement gap, previous-change, sign-reversal, history, rank, side, variation, clock, hold, subset, threshold, comparator, or control repair",
    }
    return {**core, "manifest_hash": canonical_hash(core)}

def validate(value: Mapping[str, Any]) -> None:
    if value != build():
        raise RuntimeError("HVCFIR-8 preregistration drift")
    contract = hvcav.build()
    for key in ("stages", "source_support_gates", "gross9_novelty_gates", "economic_gates"):
        if value[key] != contract[key]:
            raise RuntimeError(f"HVCFIR-8 {key} drift")
