"""Evaluate frozen HVDVBR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDVBR-12"
PROTOCOL = "hvdvbr_12_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_dominant_volume_bar_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDVBR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVDVBR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDVBR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDVBR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdvbr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVDVBR-12 unchanged before economic outcomes"
        ),
    }
