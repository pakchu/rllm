"""Sequential strict economics for frozen HVAO-24."""
...
PREREG = Path("results/high_volatility_awesome_oscillator_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "844a8984ab057c68db77ec6645c68cd5e1854a00057e0ab1cfc8c6b75110d8b8"
SUPPORT = Path("results/high_volatility_awesome_oscillator_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "e7b63d6448b8e6e4e6bc8eb1f8b8657c34a299fdf780efc105880fca5a88017d"
NOVELTY = Path("results/high_volatility_awesome_oscillator_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "8702c23f78ec282319ac145c967688337d161ec11e12fe3d41cf7c7b31c61b3d"
CLOCK = Path("data/high_volatility_awesome_oscillator_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "4fd4ec016476cc3970c1a57e9c3090ab83b1470954b03e1777e75e77c5680563"
CONTROL_DIR = Path("data/high_volatility_awesome_oscillator_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_awesome_oscillator_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_awesome_oscillator_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "close_price_sma_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVAO frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVAO novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvao_24_sequential_economics_v1", "policy_id": POLICY_ID,
