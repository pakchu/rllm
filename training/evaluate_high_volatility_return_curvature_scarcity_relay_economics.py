"""Sequential strict economics for frozen HVCSR-8."""
...
PREREG = Path("results/high_volatility_return_curvature_scarcity_relay_preregistration_2026-08-10.json")
PREREG_SHA = "306172f8379ee38236634cec6482d7b6d2be837c1451990a99f81c7cb1e331f7"
SUPPORT = Path("results/high_volatility_return_curvature_scarcity_relay_support_2026-08-10.json")
SUPPORT_SHA = "79117ebb37960354384ff2b7f9d68b42f19c0b61a45d5258848dc45972851da2"
NOVELTY = Path("results/high_volatility_return_curvature_scarcity_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "10ebe86bb1f59758c8afcc69e811e3574fa02d670f14c514c80d18558987b7cf"
CLOCK = Path("data/high_volatility_return_curvature_scarcity_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "80ade7e34a3fac0012dd0e28c1362c14509f1240a5efb631e076f08516207d94"
CONTROL_DIR = Path("data/high_volatility_return_curvature_scarcity_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_return_curvature_scarcity_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_return_curvature_scarcity_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_curvature_scarcity_tail", "no_variation_gate", "raw_curvature_ratio_at_most_half", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCSR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCSR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcsr_8_sequential_economics_v1", "policy_id": POLICY_ID,
