"""Evaluate frozen HVFRAMA-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVFRAMA-24"
PROTOCOL = "hvframa_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_fractal_adaptive_average_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVFRAMA predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVFRAMA novelty limits drift")
    if (
        ...
        raise RuntimeError("HVFRAMA predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVFRAMA primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvframa_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVFRAMA-24 unchanged before economic outcomes",
    }
