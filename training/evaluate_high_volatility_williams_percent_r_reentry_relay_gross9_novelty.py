"""Evaluate frozen HVWPR-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVWPR-24"
PROTOCOL = "hvwpr_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_williams_percent_r_reentry_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVWPR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVWPR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVWPR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVWPR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvwpr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVWPR-24 unchanged before economic outcomes",
    }
