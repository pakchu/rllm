"""Evaluate frozen HVSTBR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVSTBR-8"
PROTOCOL = "hvstbr_8_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_standardized_tail_breadth_relay_preregistration_2026-08-10.json"
)
...
OUTPUT = Path(
    "results/high_volatility_standardized_tail_breadth_relay_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVSTBR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVSTBR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVSTBR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVSTBR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvstbr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVSTBR-8 unchanged before economic outcomes"
        ),
    }
