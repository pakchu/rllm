"""Source-only support gate for frozen HVSAUD-8."""
...
from training import preregister_high_volatility_spot_adverse_underwater_duration_relay as prereg
...
ROOT = Path("data/high_volatility_spot_adverse_underwater_duration_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_spot_adverse_underwater_duration_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_spot_adverse_underwater_duration_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_spot_adverse_underwater_duration_relay_support_2026-08-18.json")
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVSAUD source timestamps")
    ...
    variation = pd.read_csv(
        prereg.VARIATION,
        usecols=["decision_time", "source_valid", "realized_variation", "variation_rank"],
    ).rename(
        columns={
            "source_valid": "perpetual_source_valid",
            "realized_variation": "perpetual_realized_variation",
            "variation_rank": "perpetual_variation_rank",
        }
    )
    variation["decision_time"] = pd.to_datetime(variation["decision_time"], utc=True, errors="raise")
    states = states.merge(variation, on="decision_time", how="left", validate="one_to_one")
    states["source_valid"] = states["source_valid"] & states["perpetual_source_valid"].eq(True)
    states["realized_variation"] = states["perpetual_realized_variation"]
    states["variation_rank"] = states["perpetual_variation_rank"]
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvsaud_source_v1",
        "query": QUERY,
        "window": [START.isoformat(), END.isoformat()],
        "outcomes_opened": False,
        "candidate_incidence_opened_before_materialization": False,
        "perpetual_variation_source": {
            "path": str(prereg.VARIATION),
            "sha256": prereg.VARIATION_SHA,
        },
        "output": {
            "path": str(PANEL),
            "sha256": sha256(PANEL),
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVSAUD preregistration drift")
    ...
    core = {
        "protocol_version": "hvsaud_8_source_support_v1",
        "policy_id": "HVSAUD-8",
        "preregistration": {
