"""Sequential strict economics for frozen VSDAR-24."""
...
PREREG = Path("results/volatility_shock_disagreement_absorption_relay_preregistration_2026-08-08.json")
PREREG_SHA = "6f7dba223c322c08d36f5400869dacfac74702e761e4b725b676bbdf8edd4132"
SUPPORT = Path("results/volatility_shock_disagreement_absorption_relay_support_2026-08-08.json")
SUPPORT_SHA = "e8d3efe78556be7762c010b495a363db0d45a62d90a5cc24a2e9c83796730751"
NOVELTY = Path("results/volatility_shock_disagreement_absorption_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "13e22d5ed44492577f7d0eb5634b09f6a57573814025308372a166dbcbe10e99"
CLOCK = Path("data/volatility_shock_disagreement_absorption_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "fbdbd8a5eba77e715880f0b2db1572df524074c7dca3ad3b3f30ef4cb4773027"
CONTROL_DIR = Path("data/volatility_shock_disagreement_absorption_relay_controls_2023_2026")
FREEZE = Path("results/volatility_shock_disagreement_absorption_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/volatility_shock_disagreement_absorption_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
CONTROLS = ("no_magnitude_gate", "vix_shock_only", "relative_convexity_only", "same_sign_confirmation", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("VSDAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("VSDAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "vsdar_24_sequential_economics_v1", "policy_id": POLICY_ID,
