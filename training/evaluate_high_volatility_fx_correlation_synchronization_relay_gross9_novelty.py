"""Evaluate frozen HVFXCSR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVFXCSR-12"
PROTOCOL = "hvfxcsr_12_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_fx_correlation_synchronization_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVFXCSR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVFXCSR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVFXCSR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVFXCSR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvfxcsr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVFXCSR-12 unchanged before economic outcomes",
    }
