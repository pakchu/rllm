"""Sequential strict economics for frozen HVRVTE-8."""
...
PREREG = Path("results/high_volatility_return_volume_transfer_entropy_relay_preregistration_2026-08-13.json")
PREREG_SHA = "6921cf0a08a7f816bf95ccf0dfdc4aea6152a161404e34f916e5ab2cbbba95c6"
SUPPORT = Path("results/high_volatility_return_volume_transfer_entropy_relay_support_2026-08-13.json")
SUPPORT_SHA = "42548442c88874040f73108a4938a87415e4d81d080846b88ca369e7a57df08a"
NOVELTY = Path("results/high_volatility_return_volume_transfer_entropy_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "8c1e2c0151c40b9290a6062b4891d9e5314448e016d7b737c9e090b44f1ad3c7"
CLOCK = Path("data/high_volatility_return_volume_transfer_entropy_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "e27f50cfe2a9d3eb90a41e23affa75c33c4a06360ce2627fcebe8647dc0b87c6"
CONTROL_DIR = Path("data/high_volatility_return_volume_transfer_entropy_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_return_volume_transfer_entropy_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_return_volume_transfer_entropy_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_information_tail", "no_variation_gate", "unconditional_transition_lift", "contemporaneous_conditioned_information", "one_decision_stale_information", "direction_flip", "same_clock_forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVRVTE frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVRVTE novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrvte_8_sequential_economics_v1", "policy_id": POLICY_ID,
