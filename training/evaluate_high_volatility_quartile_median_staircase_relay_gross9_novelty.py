"""Evaluate frozen HVQMSR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVQMSR-8"
PROTOCOL = "hvqmsr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_quartile_median_staircase_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVQMSR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVQMSR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVQMSR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVQMSR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvqmsr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVQMSR-8 unchanged before economic outcomes",
    }
