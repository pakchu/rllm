"""Evaluate frozen HVVSR-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVVSR-24"
PROTOCOL = "hvvsr_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_variance_signature_reversal_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVVSR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVVSR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVVSR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVVSR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvvsr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVVSR-24 unchanged before economic outcomes",
    }
