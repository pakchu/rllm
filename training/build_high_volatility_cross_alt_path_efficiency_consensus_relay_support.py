"""Outcome-blind source support for frozen HVCAPEC-8."""
...
from training import preregister_high_volatility_cross_alt_path_efficiency_consensus_relay as prereg
ENV_FILE='/home/pakchu/rllm/.env';START=pd.Timestamp('2023-01-01T00:00:00Z');END=pd.Timestamp('2026-08-01T00:00:00Z');PREREG_SHA='9bb7f6a4df8276586ac40b4a6fdc9fe1de7a9a09293490fcaf64d7972d759e23';REG=prereg.build();P=REG['policy'];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG['stages'].items()};GATES=REG['source_support_gates'];CONTROLS=tuple(REG['diagnostic_controls']['names']);SYMBOLS=('BTCUSDT',*prereg.ALTS)
...
ROOT=Path('data/high_volatility_cross_alt_path_efficiency_consensus_relay_sources_2023_2026');PANEL=ROOT/'block_states.csv.gz';MANIFEST=ROOT/'manifest.json';CLOCK=Path('data/high_volatility_cross_alt_path_efficiency_consensus_relay_clocks_2023_2026.csv.gz');SPLIT_DIR=Path('data/high_volatility_cross_alt_path_efficiency_consensus_relay_split_clocks_2023_2026');CONTROL_DIR=Path('data/high_volatility_cross_alt_path_efficiency_consensus_relay_controls_2023_2026');RESULT=Path('results/high_volatility_cross_alt_path_efficiency_consensus_relay_support_2026-08-13.json');BUILDER=Path(__file__).relative_to(Path.cwd())
...
def consensus(signs,ranks=None,threshold=0.):
 ...
 if ranks is not None:r=np.asarray(ranks,float);tail&=np.isfinite(r)&(r>=threshold)
...
def prepare(raw):
 ...
 if raw.columns.tolist()!=required:raise RuntimeError('HVCAPEC source schema drift')
 x=raw.copy()
 ...
 if x[['block_start','symbol']].isna().any().any() or x.duplicated(['block_start','symbol']).any():raise RuntimeError('HVCAPEC invalid key')
 x['row_valid']=np.isfinite(x[['displacement','path_variation','minute_squared_return','source_rows','five_minute_bars']]).all(axis=1)&x.displacement.ne(0)&x.path_variation.gt(0)&x.minute_squared_return.ge(0)&x.source_rows.eq(480)&x.five_minute_bars.eq(96)&x.first_bar.eq(x.block_start)&x.last_bar.eq(x.block_start+pd.Timedelta('475m'))&x.coherent.eq(True);x['path_efficiency']=x.displacement.abs()/x.path_variation;x['decision_time']=x.block_start+pd.Timedelta('8h');return x.set_index(['decision_time','symbol']).sort_index()
def build_panel(raw):
 x=prepare(raw);decisions=pd.date_range(START+pd.Timedelta('6h'),END,freq='8h',inclusive='left');full=x.reindex(pd.MultiIndex.from_product([decisions,SYMBOLS],names=['decision_time','symbol']));valid=full.row_valid.unstack('symbol').reindex(columns=SYMBOLS);disp=full.displacement.unstack('symbol').reindex(columns=SYMBOLS);eff=full.path_efficiency.unstack('symbol').reindex(columns=SYMBOLS);sq=full.minute_squared_return.unstack('symbol').reindex(columns=SYMBOLS);ranks=pd.DataFrame(index=decisions,columns=prereg.ALTS,dtype=float);rawranks=pd.DataFrame(index=decisions,columns=prereg.ALTS,dtype=float)
 for alt in prereg.ALTS:ranks[alt]=causal(eff[alt].where(valid[alt]));rawranks[alt]=causal(disp[alt].abs().where(valid[alt]))
 btcvar=np.sqrt(sq.BTCUSDT.where(valid.BTCUSDT).rolling(3,min_periods=3).sum());rows=[]
 for d in decisions:
  ok=bool(valid.loc[d].eq(True).all() and np.isfinite(ranks.loc[d]).all() and math.isfinite(float(btcvar.loc[d])) and btcvar.loc[d]>0);sign=np.sign(disp.loc[d,list(prereg.ALTS)].to_numpy(float));side,breadth,strength=consensus(sign,ranks.loc[d].to_numpy(float),P['efficiency_rank_min']) if ok else (0,0,math.nan);bside,bbreadth,_=consensus(sign) if ok else (0,0,math.nan);rawside,rawbreadth,rawstrength=consensus(sign,rawranks.loc[d].to_numpy(float),P['efficiency_rank_min']) if ok else (0,0,math.nan);rows.append({'decision_time':d,'feature_available_time':d,'source_valid':ok,'consensus_side':side,'consensus_breadth':breadth,'consensus_strength':strength,'broad_side':bside,'broad_breadth':bbreadth,'raw_side':rawside,'raw_breadth':rawbreadth,'raw_strength':rawstrength,'btc_realized_variation':float(btcvar.loc[d]) if ok else math.nan})
...
def active(panel,control='primary'):
 ...
 if control=='no_efficiency_tail':side=used.broad_side;breadth=used.broad_breadth;strength=pd.Series(1.,index=used.index)
 elif control=='endpoint_return_magnitude_tail':side=used.raw_side;breadth=used.raw_breadth;strength=used.raw_strength
 if control=='one_block_stale_efficiency':side=side.shift(1);breadth=breadth.shift(1);strength=strength.shift(1)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError('HVCAPEC prereg drift')
