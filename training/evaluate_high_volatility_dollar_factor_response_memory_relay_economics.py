"""Sequential strict economics for frozen HVDFRM-12."""
...
PREREG = Path("results/high_volatility_dollar_factor_response_memory_relay_preregistration_2026-08-09.json")
PREREG_SHA = "1f50436d79a2fe604793ad5592dbeba348c6960b95d24f4b8b46dcd742bba64a"
SUPPORT = Path("results/high_volatility_dollar_factor_response_memory_relay_support_2026-08-09.json")
SUPPORT_SHA = "a99c70dc1e2355e5db82958c503d6cdb49ab0c7ebf8e52827c1d7715be5809ba"
NOVELTY = Path("results/high_volatility_dollar_factor_response_memory_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "e5fd5055b32932c80c407a4d54232d1415b57e8dfce90068653638aa549ba4ce"
CLOCK = Path("data/high_volatility_dollar_factor_response_memory_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "ae6efab65877e817e6030bd1b2ae92075d0847cda29578fe7013495a19d4933c"
CONTROL_DIR = Path("data/high_volatility_dollar_factor_response_memory_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_dollar_factor_response_memory_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_dollar_factor_response_memory_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "fixed_inverse_factor", "fixed_direct_factor", "one_observation_stale_memory", "direction_flip", "same_clock_forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDFRM frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDFRM novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdfrm_12_sequential_economics_v1", "policy_id": POLICY_ID,
