"""Materialize deterministic, outcome-blind source support for HVCPSR-8."""
...
from training import preregister_high_volatility_complementary_persistence_sequential_router as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv

PREREG_SHA = "943a075a7209e43b204fe56c947c0d16fdc3495f4e6fbf9cb2c8754cc277b1a8"
CLOCK = Path("data/high_volatility_complementary_persistence_sequential_router_clocks_2023_2026.csv.gz")
RESULT = Path("results/high_volatility_complementary_persistence_sequential_router_support_2026-08-16.json")
...
def _registration() -> dict[str, Any]:
    if prereg.sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCPSR-8 preregistration hash drift")
    ...
    for component, artifacts in prereg.COMPONENT_ARTIFACTS.items():
        for kind, artifact in artifacts.items():
            if prereg.sha256_file(artifact["path"]) != artifact["sha256"]:
                raise RuntimeError(f"HVCPSR-8 {component} {kind} drift")
...
def build_clock(clocks: Mapping[str, pd.DataFrame]) -> tuple[pd.DataFrame, dict[str, Any]]:
    if set(clocks) != set(prereg.COMPONENT_IDS):
        raise RuntimeError("HVCPSR-8 component roster drift")
    ...
    rows = []
    counts: dict[str, Any] = {
        "union_decisions": 0, "emitted_decisions": 0,
        "selected_by_action": {action: 0 for action in prereg.ACTION_IDS},
        "coactive_same_side": 0, "coactive_opposite_side": 0,
        "longs": 0, "shorts": 0,
    ...
    for decision in decisions:
        ...
        primary = indexed[prereg.ACTION_IDS[0]].get(decision)
        fallback = indexed[prereg.ACTION_IDS[1]].get(decision)
        primary_side = 0 if primary is None else primary.side
        fallback_side = 0 if fallback is None else fallback.side
        side = prereg.priority_router_side(primary_side, fallback_side)
        selected_action = prereg.ACTION_IDS[0] if primary is not None else prereg.ACTION_IDS[1]
        selected = primary if primary is not None else fallback
        if primary is not None and fallback is not None:
            counts["coactive_same_side" if primary.side == fallback.side else "coactive_opposite_side"] += 1
        entry, exit_ = decision + ENTRY_DELAY, decision + ENTRY_DELAY + HOLD
        if selected is None or selected.split != split or selected.feature_available_time > entry:
            raise RuntimeError("HVCPSR-8 stage or availability drift")
        rows.append({"candidate": prereg.POLICY_ID, "control": "primary", "split": split, "selected_action": selected_action, "decision_time": decision, "feature_available_time": selected.feature_available_time, "entry_time": entry, "exit_time": exit_, "side": side, "active_action_count": len(active)})
        counts["selected_by_action"][selected_action] += 1
    ...
    if counts["emitted_decisions"] != counts["union_decisions"]:
        raise RuntimeError("HVCPSR-8 incidence accounting drift")
    if len(output) > 1 and not output["entry_time"].iloc[1:].reset_index(drop=True).ge(output["exit_time"].iloc[:-1].reset_index(drop=True)).all():
        raise RuntimeError("HVCPSR-8 output overlap")
...
def run(clock_path: Path = CLOCK, result_path: Path = RESULT) -> dict[str, Any]:
    ...
    core = {"protocol_version": "hvcpsr_8_source_support_v1", "policy_id": prereg.POLICY_ID, "preregistration": {"path": str(prereg.DEFAULT_OUTPUT), "sha256": PREREG_SHA, "manifest_hash": registration["manifest_hash"]}, "verified_component_artifacts": verified, "component_clock_fields_opened": list(FIELDS), "combined_incidence_opened": True, "combined_postentry_returns_or_pnl_opened": False, "entry_exit_prices_opened": False, "returns_opened": False, "funding_opened": False, "pnl_opened": False, "gross9_comparator_rows_opened": False, "clock": {"path": str(clock_path), "sha256": prereg.sha256_file(clock_path), "rows": len(clock)}, "priority_router_accounting": accounting, "support": stats, "support_checks": checks, "support_passed": passed, "advance_to_gross9_novelty": passed, "advance_to_economic_outcomes": False, "decision": "pass_to_gross9_novelty" if passed else "terminal_source_support_reject"}
...
if __name__ == "__main__":
    ...
    print(json.dumps({"events": {k: v["events"] for k, v in report["support"].items()}, "accounting": report["priority_router_accounting"], "support_passed": report["support_passed"]}, indent=2))
