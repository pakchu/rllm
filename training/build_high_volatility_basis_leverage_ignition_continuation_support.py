"""Build outcome-blind source support for frozen HVBLIC-6."""
...
from training import preregister_high_volatility_basis_leverage_ignition_continuation as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv


ENV_FILE = "/home/pakchu/rllm/.env"
PREREG_SHA = "48907aa183d5bcc24e1b7beef054e1561782aee6776bd8363557757b00c323e6"
...
SOURCE_DIR = Path("data/high_volatility_basis_leverage_ignition_continuation_sources_2023_2026")
FEATURES = SOURCE_DIR / "hourly_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_basis_leverage_ignition_continuation_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_basis_leverage_ignition_continuation_controls_2023_2026")
RESULT = Path("results/high_volatility_basis_leverage_ignition_continuation_support_2026-08-11.json")
SPLITS = {
...
CONTROLS = ("no_dvol_level", "no_dvol_rise", "no_premium_tail", "no_oi_increase", "one_hour_stale_features", "direction_flip", "forced_long")
PREMIUM_QUERY = """
...
GROUP BY 1 ORDER BY 1
"""
CLOCK_COLUMNS = ("candidate", "control", "split", "decision_time", "feature_available_time", "entry_time", "exit_time", "side", "premium_displacement", "premium_displacement_rank", "oi_change", "dvol_open", "dvol_close", "dvol_level_rank")
...
def conditions(frame: pd.DataFrame, control: str = "primary") -> tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    raw = used.source_valid.eq(True)
    if control != "no_dvol_level": raw &= used.dvol_level_rank.ge(0.60)
    if control != "no_dvol_rise": raw &= used.dvol_close.gt(used.dvol_open)
    if control != "no_premium_tail": raw &= used.premium_displacement_rank.ge(0.60)
    if control != "no_oi_increase": raw &= used.oi_change.gt(0)
    active = raw & used.source_valid.shift(1).eq(True) & ~raw.shift(1, fill_value=False)
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active]:
        ...
        rows.append({"candidate": "HVBLIC-6", "control": control, "split": split, "decision_time": decision, "feature_available_time": decision, "entry_time": entry, "exit_time": exit_time, "side": int(sides.at[index]), "premium_displacement": float(source.premium_displacement), "premium_displacement_rank": float(source.premium_displacement_rank), "oi_change": float(source.oi_change), "dvol_open": float(source.dvol_open), "dvol_close": float(source.dvol_close), "dvol_level_rank": float(source.dvol_level_rank)})
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA: raise RuntimeError("HVBLIC preregistration drift")
