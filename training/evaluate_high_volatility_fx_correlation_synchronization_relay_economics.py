"""Sequential strict economics for frozen HVFXCSR-8."""
...
PREREG = Path("results/high_volatility_fx_correlation_synchronization_relay_preregistration_2026-08-10.json")
PREREG_SHA = "802764a22fac7331570843626679dde3ba01b37b27c3002d407af9e976fa1311"
SUPPORT = Path("results/high_volatility_fx_correlation_synchronization_relay_support_2026-08-10.json")
SUPPORT_SHA = "3dfcb267d7e822a1c0cc6e65f5a3423ce31e21fa10a4492c454e7a724ac2aa56"
NOVELTY = Path("results/high_volatility_fx_correlation_synchronization_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "8e74a785313b57c4a69bff03ba48bfeb22c46d611a30726c93e9878695fb4fd8"
CLOCK = Path("data/high_volatility_fx_correlation_synchronization_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "83c93ec56272d86907a6c8622241be743b016be6908a9196702091388ff69536"
CONTROL_DIR = Path("data/high_volatility_fx_correlation_synchronization_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_fx_correlation_synchronization_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_fx_correlation_synchronization_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "no_synchronization_gate", "one_session_stale_features", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVFXCSR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVFXCSR novelty did not authorize economics")
