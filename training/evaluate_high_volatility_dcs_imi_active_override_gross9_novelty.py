"""Evaluate the frozen HVDIMIO-8 action-vote clock against Gross9."""
...
POLICY = "HVDIMIO-8"
PROTOCOL = "hvdimio_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_dcs_imi_active_override_clocks_2023_2026.csv.gz",
    "sha256": "1d00f42635f911020a79147a592f60d7b54773e0426778d3d2a027817216be3d",
    "rows": 235,
}
OUTPUT = Path("results/high_volatility_dcs_imi_active_override_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVDIMIO-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVDIMIO-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVDIMIO-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVDIMIO-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVDIMIO-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVDIMIO-8 unchanged before economics",
    }
