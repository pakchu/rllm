"""Sequential strict economics for frozen HVQGR-12."""
...
PREREG = Path("results/high_volatility_qqq_gold_rotation_relay_preregistration_2026-08-09.json")
PREREG_SHA = "f0dd9c8660aed4391157dd9eea0e30a84ffbe0bacdd4b4bdf835016d02374e08"
SUPPORT = Path("results/high_volatility_qqq_gold_rotation_relay_support_2026-08-09.json")
SUPPORT_SHA = "dcc44013443065436f9e34a0c4626f1e231c4a18efeffec5d170a91767f96e41"
NOVELTY = Path("results/high_volatility_qqq_gold_rotation_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "1cda38c7003d88d186e480d2f04e3ff402afd634675ba0cebbb4061cac1723a8"
CLOCK = Path("data/high_volatility_qqq_gold_rotation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "8432fb907026245b8a20df45f09321af77a0d72d2cab76cb5db27ca9045bee7c"
CONTROL_DIR = Path("data/high_volatility_qqq_gold_rotation_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_qqq_gold_rotation_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_qqq_gold_rotation_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVQGR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVQGR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvqgr_12_sequential_economics_v1", "policy_id": POLICY_ID,
