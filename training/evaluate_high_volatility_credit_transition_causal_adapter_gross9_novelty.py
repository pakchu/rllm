"""Evaluate the frozen HVCQCA-24 action-vote clock against Gross9."""
...
POLICY = "HVCQCA-24"
PROTOCOL = "hvcqca_24_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_credit_transition_causal_adapter_clocks_2023_2026.csv.gz",
    "sha256": "a4f1d0e154daed1eeeb156c42619f6238fca2e3c9f9a8f6b3214c3bb7631e9b6",
    "rows": 199,
}
OUTPUT = Path("results/high_volatility_credit_transition_causal_adapter_gross9_novelty_2026-08-18.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCQCA-24 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCQCA-24 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCQCA-24 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCQCA-24 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCQCA-24 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCQCA-24 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCQCA-24 unchanged before economics",
    }
