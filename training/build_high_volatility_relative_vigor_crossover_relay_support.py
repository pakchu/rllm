"""Build outcome-blind source support for frozen HVRVI-24."""
...
from training import preregister_high_volatility_relative_vigor_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "30e0243792b91ecbf47c40acb6b198c647bf9b4b98d5672f134c150a26dee71e"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_relative_vigor_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_relative_vigor_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_relative_vigor_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_relative_vigor_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_relative_vigor_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "drive", "bar_range", "smoothed_drive", "smoothed_range",
    "rvi", "signal", "difference", "raw_rvi", "raw_signal", "variation",
    "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "drive", "bar_range", "smoothed_drive",
    "smoothed_range", "rvi", "signal", "difference", "raw_rvi", "raw_signal",
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVRVI source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVRVI invalid source key")
...
def weighted_four(values: pd.Series) -> pd.Series:
    return (values+2*values.shift(1)+2*values.shift(2)+values.shift(3))/6


def relative_vigor(open_: pd.Series, high: pd.Series, low: pd.Series, close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    drive=(close-open_).where(valid);bar_range=(high-low).where(valid);smoothed_drive=weighted_four(drive);smoothed_range=weighted_four(bar_range);period=P["rvi_periods"];denominator=smoothed_range.rolling(period,min_periods=period).sum();rvi=(smoothed_drive.rolling(period,min_periods=period).sum()/denominator).where(denominator.gt(0));signal=weighted_four(rvi);raw_denominator=bar_range.rolling(period,min_periods=period).sum();raw_rvi=(drive.rolling(period,min_periods=period).sum()/raw_denominator).where(raw_denominator.gt(0));raw_signal=weighted_four(raw_rvi);return pd.DataFrame({"drive":drive,"bar_range":bar_range,"smoothed_drive":smoothed_drive,"smoothed_range":smoothed_range,"rvi":rvi,"signal":signal,"difference":rvi-signal,"raw_rvi":raw_rvi,"raw_signal":raw_signal},index=open_.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[["bar_open","bar_high","bar_low","bar_close"]]).all(axis=1)&bars[["bar_open","bar_high","bar_low","bar_close"]].gt(0).all(axis=1);bars=pd.concat([bars,relative_vigor(bars.bar_open,bars.bar_high,bars.bar_low,bars.bar_close,bars.valid_bar)],axis=1);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["drive","bar_range","smoothed_drive","smoothed_range","rvi","signal","difference","raw_rvi","raw_signal","variation"]]).all(axis=1)&bars.bar_range.gt(0)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.difference.shift(1);cross=panel.difference.ne(0)&previous.ne(0)&np.sign(panel.difference).ne(np.sign(previous));panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&cross&panel.variation_rank.ge(P["variation_rank_min"]);return panel.loc[:,PANEL_COLUMNS]


def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);difference=pd.to_numeric(used.difference,errors="coerce")
    if control=="unsmoothed_drive_ratio":difference=pd.to_numeric(used.raw_rvi-used.raw_signal,errors="coerce")
    previous=difference.shift(1);cross=difference.ne(0)&previous.ne(0)&np.sign(difference).ne(np.sign(previous));variation=used.variation_rank.ge(P["variation_rank_min"]);side=pd.Series(np.sign(difference),index=used.index).fillna(0).astype(int);state=valid&side.ne(0)&cross&variation
    if control=="no_variation_gate":state=valid&side.ne(0)&cross
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVRVI prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvrvi_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvrvi_24_source_support_v1", "policy_id": prereg.POLICY_ID,
