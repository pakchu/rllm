"""Build outcome-blind source support for frozen HVVIDYA-24."""
...
from training import preregister_high_volatility_variable_index_dynamic_average_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "e95910c9e2a65a5d25d5c35b1b3e2c0988ebf282cbc299b1502c97c531d84459"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_variable_index_dynamic_average_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_variable_index_dynamic_average_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_variable_index_dynamic_average_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_variable_index_dynamic_average_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_variable_index_dynamic_average_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "cmo", "smoothing_factor", "vidya",
    "difference", "fixed_ema", "fixed_difference", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "cmo", "smoothing_factor", "vidya",
    "difference", "fixed_ema", "fixed_difference", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVVIDYA source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVVIDYA invalid source key")
...
def variable_index_average(close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    period=P["cmo_periods"]
    base=2.0/(P["ema_periods"]+1.0)
    values=close.to_numpy(float);validity=valid.to_numpy(bool)
    cmo=np.full(len(close),np.nan);factor=np.full(len(close),np.nan);vidya=np.full(len(close),np.nan)
    state=np.nan;run=0
    for index,value in enumerate(values):
        if not validity[index] or not math.isfinite(value):
            state=np.nan;run=0;continue
        run+=1
        if run<period+1:continue
        changes=np.diff(values[index-period:index+1])
        up=float(np.clip(changes,0,None).sum());down=float(np.clip(-changes,0,None).sum())
        denominator=up+down
        if not np.isfinite(changes).all() or denominator<=0:
            state=np.nan;continue
        oscillator=(up-down)/denominator
        smoothing=base*abs(oscillator)
        cmo[index]=oscillator;factor[index]=smoothing
        if not math.isfinite(state):state=value
        else:state=state+smoothing*(value-state)
        vidya[index]=state
    return pd.DataFrame({"cmo":cmo,"smoothing_factor":factor,"vidya":vidya},index=close.index)


def fixed_ema(close: pd.Series, valid: pd.Series) -> pd.Series:
    alpha=2.0/(P["ema_periods"]+1.0);output=np.full(len(close),np.nan);state=np.nan;run=0
    for index,(value,is_valid) in enumerate(zip(close.to_numpy(float),valid.to_numpy(bool))):
        ...
        if run>=P["ema_periods"]:output[index]=state
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=bars.join(variable_index_average(bars.bar_close,bars.valid_bar));bars["difference"]=bars.bar_close-bars.vidya
    bars["fixed_ema"]=fixed_ema(bars.bar_close,bars.valid_bar);bars["fixed_difference"]=bars.bar_close-bars.fixed_ema
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum())
    bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["cmo","smoothing_factor","vidya","difference","fixed_ema","fixed_difference","variation"]]).all(axis=1)&bars.difference.ne(0)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h")
    panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.difference.shift(1)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    signal=pd.to_numeric(used.difference,errors="coerce");previous=signal.shift(1);crossing=signal.ne(0)&previous.ne(0)&np.sign(signal).ne(np.sign(previous))
    variation=used.variation_rank.ge(P["variation_rank_min"]);side=np.sign(signal).fillna(0).astype(int);state=valid&side.ne(0)&crossing&variation
    if control=="no_variation_gate":state=valid&side.ne(0)&crossing
    elif control=="fixed_ema_crossover":
        signal=pd.to_numeric(used.fixed_difference,errors="coerce");previous=signal.shift(1);state=valid&signal.ne(0)&previous.ne(0)&np.sign(signal).ne(np.sign(previous))&variation;side=np.sign(signal).fillna(0).astype(int)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVVIDYA prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvvidya_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvvidya_24_source_support_v1", "policy_id": prereg.POLICY_ID,
