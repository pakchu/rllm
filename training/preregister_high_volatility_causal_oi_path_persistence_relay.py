"""Outcome-blind preregistration for HVCOPR-24."""
...
POLICY_ID = "HVCOPR-24"
DEFAULT_OUTPUT = Path("results/high_volatility_causal_oi_path_persistence_relay_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_causal_oi_path_persistence_relay_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": "When both halves of a completed eight-hour BTC path move in the same direction, path efficiency and realized variation are causally elevated, and open interest expands over that exact cycle, newly built leverage sponsors price discovery that persists for twenty-four hours.",
            "side": "strict sign of the completed eight-hour BTC return",
            "why_distinct": "HVCOPR combines exact endpoint OI expansion with continuous one-minute path efficiency and same-sign four-hour halves. Unlike HVDPR it uses no BVOL or DVOL, and unlike HVCORR it uses no trailing range, midpoint, crossing, or price-level state.",
            ...
            "why_low_gross9_overlap_is_plausible": "fixed eight-hour OI-sponsored efficient-path states are absent from Gross9 primitives",
        },
        "features": {
            ...
            "completed_cycle": "480 exact coherent BTCUSDT one-minute open-to-close log returns over [D-8h,D)",
            "half_returns": "sum of the first 240 and last 240 returns; both finite strict nonzero and same strict sign",
            "completed_return": "sum of all 480 returns, finite strict nonzero",
            "path_efficiency": "abs(completed_return)/sum(abs(one-minute returns)), denominator strict positive",
            "efficiency_rank": "strict-prior midrank over at most 270 source-valid cycles; current excluded; minimum 180; rank>=0.60",
            ...
            "eligible": "exact coherent completed cycle, same-sign halves, two exact OI endpoints with strict expansion, and both efficiency and variation ranks pass at D; no funding, range, midpoint, premium, alt, or post-decision condition",
            "no_imputation": True,
        },
        "clock": {
        ...
        "stopping_rule": "terminal first failure; no OI endpoint or expansion rule, return path, half agreement, efficiency, variation history, side, clock, hold, subset, threshold, comparator, or control repair",
    }
    return {**core, "manifest_hash": canonical_hash(core)}

def validate(value: Mapping[str, Any]) -> None:
    if value != build():
        raise RuntimeError("HVCOPR-24 preregistration drift")
    contract = hvcav.build()
    for key in ("stages", "source_support_gates", "gross9_novelty_gates", "economic_gates"):
        if value[key] != contract[key]:
            raise RuntimeError(f"HVCOPR-24 {key} drift")
