"""Sequential hardened evaluator for frozen FCIR-12 clocks.
...
from training import preregister_flow_centrality_incubation_relay as prereg  # noqa: E402
...
def _utc(value: Any) -> pd.Timestamp:
    ...
    if timestamp is pd.NaT:
        raise ValueError("FCIR-12 timestamp is NaT")
...
SUPPORT_COMMIT = "a57fc55cf1507bc5d5cab33f3e5c936e7ac39e05"
SUPPORT_RESULT = Path(
    "results/flow_centrality_incubation_relay_support_2026-07-19.json"
)
PRIMARY_CLOCKS = Path("data/flow_centrality_incubation_relay_clocks_2023_2026.csv.gz")
CONTROL_CLOCKS = Path(
    "data/flow_centrality_incubation_relay_control_clocks_2023_2026.csv.gz"
)
EVALUATOR_SOURCE = Path("training/evaluate_flow_centrality_incubation_relay.py")
...
STATIC_INPUT_SHA256 = {
    str(prereg.PREREGISTRATION_SOURCE): (
        "2c0af80ed23c9de9f875d6d53bb93c7cc6bd31fae710b514204c80e9df68b1d3"
    ),
    str(SUPPORT_RESULT): (
        "2ecd30c6a4f8678207053522aabe7ef6bfbc24e3f5d29b4e52743c218e4d2e89"
    ),
    "docs/flow-centrality-incubation-relay-preregistration-2026-07-19.md": (
        "fa657716d0bc5c3be4dfaf975d97e80459cce8fbf5882d742e317ccc7d5067ed"
    ),
    str(PRIMARY_CLOCKS): (
        "d4bb6245f0bac34885e780e35ff1edb9b5cf2114dc3c13088ec19613ad8056ea"
    ),
    str(prereg.SOURCE_PANEL): prereg.SOURCE_PANEL_SHA256,
    str(prereg.SOURCE_MANIFEST): prereg.SOURCE_MANIFEST_SHA256,
...
STAGE_OUTPUTS = {
    "train": Path(
        "results/flow_centrality_incubation_relay_train_2023_2026-07-19.json"
    ),
    "test": Path("results/flow_centrality_incubation_relay_test_2024_2026-07-19.json"),
    "eval": Path("results/flow_centrality_incubation_relay_eval_2025_2026-07-19.json"),
    "final": Path(
        "results/flow_centrality_incubation_relay_final_2026h1_2026-07-19.json"
    ),
}
STAGE_DOCS = {
    stage: Path(f"docs/flow-centrality-incubation-relay-{stage}-result-2026-07-19.md")
    for stage in STAGE_ORDER
}
FUTURE_SOURCE_MANIFESTS = {
    stage: Path(
        f"results/flow_centrality_incubation_relay_{stage}_execution_source_2026-07-19.json"
    )
...
SAME_CLOCK_CONTROLS = (
    ...
    "equal_weight_side",
    "stale_network_24h",
    "symbol_permuted_network",
    "deterministic_random_side",
)
LATENCY_CONTROLS = ("extra_latency_1h",)
MECHANISM_CONTROLS = (*SAME_CLOCK_CONTROLS, *LATENCY_CONTROLS)
...
def _load_json(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"FCIR-12 expected a JSON object: {path}")
    return payload
...
def _verify_static_inputs() -> dict[str, Any]:
    ...
    if asdict(cfg) != expected:
        raise ValueError("FCIR-12 evaluation configuration drifted")
    return support
...
def _score_side(score: float, primary_side: int) -> int:
    if not np.isfinite(score):
        raise ValueError("FCIR-12 control score is non-finite")
    if score > 0.0:
        return 1
    if score < 0.0:
        return -1
...
def _primary_clock() -> pd.DataFrame:
    ...
    if tuple(frame.columns) != prereg.EVENT_COLUMNS:
        raise ValueError("FCIR-12 primary clock schema changed")
    for column in (
    ...
    if len(frame) != int(support["clock_rows"]):
        raise ValueError("FCIR-12 primary clock count changed")
    if set(frame["candidate"]) != {POLICY_ID}:
        raise ValueError("FCIR-12 primary clock identity changed")
    return frame
...
def derive_control_clocks() -> pd.DataFrame:
    ...
    flow = prereg.flow_matrix(prereg.load_source_prefix(end_exclusive=None))
    features = prereg.feature_panel(flow, prereg.Config())
    ...
    for raw in primary.to_dict(orient="records"):
        ...
        current_weights = np.array(
            [float(raw[f"weight_{symbol.lower()}"]) for symbol in symbols]
        )
        stale_time = naive - pd.Timedelta(hours=24)
        stale_weights = np.array(
            [
                float(features.loc[stale_time, f"weight_{symbol.lower()}"])
                for symbol in symbols
            ]
        )
        values = current_flow.to_numpy(float)
        sides = {
            "primary": primary_side,
            "direction_flip": -primary_side,
            "equal_weight_side": _score_side(float(values.mean()), primary_side),
            "stale_network_24h": _score_side(
                float(stale_weights @ values), primary_side
            ),
            "symbol_permuted_network": _score_side(
                float(np.roll(current_weights, 1) @ values), primary_side
            ),
    ...
    try:
        ...
        try:
            ...
            if CONTROL_CLOCKS.read_bytes() != rebuilt:
                raise RuntimeError("refusing to overwrite frozen FCIR control clock")
    finally:
...
def load_schedules(
    *, expected_clock_sha256: str | None = None
    ...
    if not CONTROL_CLOCKS.exists():
        raise FileNotFoundError("FCIR-12 control clock is not frozen")
    if (
        expected_clock_sha256 is not None
        and _sha256(CONTROL_CLOCKS) != expected_clock_sha256
    ):
        raise ValueError("FCIR-12 frozen control clock hash changed")
    frame = pd.read_csv(CONTROL_CLOCKS)
    if tuple(frame.columns) != CONTROL_CLOCK_COLUMNS:
        raise ValueError("FCIR-12 control clock schema changed")
    for column in (
        "source_hour_open_utc",
        "decision_time",
        "feature_available_time",
    ...
    if set(frame["control"]) != set(ALL_CONTROLS):
        raise ValueError("FCIR-12 control family changed")
    ...
    for control in ALL_CONTROLS:
        ...
        if len(schedule) != int(support["clock_rows"]):
            raise ValueError(f"FCIR-12 {control} count changed")
        ...
        if not bool(entries.iloc[1:].ge(exits.iloc[:-1].to_numpy()).all()):
            raise ValueError(f"FCIR-12 {control} schedule overlaps")
        ...
        if not bool(
            ...
            raise ValueError(f"FCIR-12 {control} latency changed")
        for split, (start, end) in STAGE_WINDOWS.items():
            ...
            if not (
                ...
                raise ValueError(f"FCIR-12 {control}/{split} containment changed")
    ...
    for control in SAME_CLOCK_CONTROLS:
        if not schedules[control][["decision_time", "entry_time", "exit_time"]].equals(
            ...
            raise ValueError(f"FCIR-12 {control} changed the primary clock")
    ...
    if not actual_semantics.equals(expected_semantics):
        raise ValueError("FCIR-12 frozen control semantics changed")
    return schedules
...
def _window_schedule(frame: pd.DataFrame, stage: str) -> pd.DataFrame:
    ...
    if not (
        ...
        raise ValueError(f"FCIR-12 {stage} schedule crosses its window")
...
def _train_source_contract(schedules: dict[str, pd.DataFrame]) -> dict[str, Any]:
    ...
    if market_manifest.get("protocol", {}).get("outcomes_opened") is not False:
        raise ValueError("FCIR-12 train market manifest provenance changed")
    if market_manifest.get("combined_sha256") != TRAIN_MARKET_SHA256:
        raise ValueError("FCIR-12 train market manifest output changed")
    ...
    if funding_manifest.get("manifest_hash") != _canonical_hash(funding_core):
        raise ValueError("FCIR-12 train funding manifest hash changed")
    if funding_manifest.get("outcomes_opened") is not False:
        raise ValueError("FCIR-12 train funding manifest opened outcomes")
    if funding_manifest.get("data", {}).get("sha256") != TRAIN_FUNDING_SHA256:
        raise ValueError("FCIR-12 train funding manifest output changed")
...
def _future_source_spec(
    ...
    return {
        ...
        "required_protocol_version": "flow_centrality_incubation_relay_execution_source_v1",
        "physical_window": [start.isoformat(), end.isoformat()],
...
def _load_future_source_contract(
    ...
    if payload.get("protocol_version") != spec["required_protocol_version"]:
        raise ValueError(f"FCIR-12 {stage} source protocol changed")
    if payload.get("candidate") != POLICY_ID or payload.get("stage") != stage:
        raise ValueError(f"FCIR-12 {stage} source identity changed")
    for key in (
        "physical_window",
        ...
        if payload.get(key) != spec[key]:
            raise ValueError(f"FCIR-12 {stage} source {key} changed")
    if payload.get("official_checksums_verified") is not True:
        raise ValueError(f"FCIR-12 {stage} source lacks official checksums")
    for name in ("market", "funding"):
        item = payload.get(name)
        if not isinstance(item, dict) or set(item) < {"path", "sha256"}:
            raise ValueError(f"FCIR-12 {stage} source lacks {name} identity")
    return payload
...
def freeze_evaluator(output_path: str | Path = EVALUATOR_FREEZE) -> dict[str, Any]:
    ...
    if output.exists():
        raise FileExistsError("FCIR-12 evaluator freeze is write-once")
    if any(path.exists() for path in STAGE_OUTPUTS.values()):
        raise RuntimeError("FCIR-12 cannot freeze after an outcome stage exists")
    support = _verify_static_inputs()
    ...
    core = {
        "protocol_version": "flow_centrality_incubation_relay_evaluator_v1",
        "candidate": POLICY_ID,
...
def verify_evaluator_freeze(path: str | Path = EVALUATOR_FREEZE) -> dict[str, Any]:
    ...
    if payload.get("evaluator_source_sha256") != _sha256(EVALUATOR_SOURCE):
        raise ValueError("FCIR-12 evaluator source changed after freeze")
    if payload.get("support_commit") != SUPPORT_COMMIT:
        raise ValueError("FCIR-12 evaluator froze another support commit")
    if payload.get("evaluation_config") != asdict(EvaluationConfig()):
        raise ValueError("FCIR-12 evaluator config changed after freeze")
    if payload.get("opened_windows") != [] or payload.get("mutable_parameters") != []:
        raise ValueError("FCIR-12 evaluator freeze is not sealed")
    if payload.get("sealed_windows") != list(STAGE_ORDER):
        raise ValueError("FCIR-12 evaluator freeze stage seal changed")
    if payload.get("execution_ohlc_rows_parsed_during_freeze") != 0:
        raise ValueError("FCIR-12 evaluator freeze parsed OHLC")
    if payload.get("funding_rows_parsed_during_freeze") != 0:
        raise ValueError("FCIR-12 evaluator freeze parsed funding")
    if payload.get("execution_outcome_data_bytes_hashed_during_freeze") is not False:
        raise ValueError("FCIR-12 evaluator freeze hashed execution outcomes")
    if payload.get("simulation_run_during_freeze") is not False:
        raise ValueError("FCIR-12 evaluator freeze simulated outcomes")
    support = _verify_static_inputs()
    if payload.get("outcome_gate") != support["protocol"]["outcome_gate"]:
        raise ValueError("FCIR-12 evaluator outcome gate changed")
    ...
    for name, schedule in schedules.items():
        if payload["schedule_records"][name]["schedule_hash"] != _schedule_hash(
            ...
            raise ValueError(f"FCIR-12 {name} schedule changed after freeze")
    return payload
...
def _verified_prior_reports(stage: str, *, freeze_hash: str) -> list[dict[str, Any]]:
    ...
    for prior in STAGE_ORDER[: STAGE_ORDER.index(stage)]:
        ...
        if payload.get("stage") != prior or payload.get("stage_passed") is not True:
            raise ValueError(f"FCIR-12 {prior} did not pass; {stage} remains sealed")
        index = STAGE_ORDER.index(prior)
        if payload.get("opened_windows") != list(STAGE_ORDER[: index + 1]):
            raise ValueError(f"FCIR-12 {prior} opened an unexpected window")
        if payload.get("sealed_windows") != list(STAGE_ORDER[index + 1 :]):
            raise ValueError(f"FCIR-12 {prior} stage seal changed")
        if payload.get("evaluator_freeze_manifest_hash") != freeze_hash:
            raise ValueError(f"FCIR-12 {prior} froze another evaluator")
        if payload.get("evaluator_source_sha256") != _sha256(EVALUATOR_SOURCE):
            raise ValueError(f"FCIR-12 {prior} evaluator source changed")
...
def load_execution_window(
    ...
    if stage not in STAGE_ORDER:
        raise ValueError(f"FCIR-12 unknown stage: {stage}")
    ...
    if _sha256(market_item["path"]) != market_item["sha256"]:
        raise ValueError(f"FCIR-12 {stage} market bytes changed")
    if _sha256(funding_item["path"]) != funding_item["sha256"]:
        raise ValueError(f"FCIR-12 {stage} funding bytes changed")
    return (
        market,
        funding,
        {
...
def simulate_strict(
    ...
    *,
    ...
    if cfg != EvaluationConfig():
        raise ValueError("FCIR-12 evaluation configuration is frozen")
    if not 0.0 <= cost_rate_per_side < 0.1 or end <= start:
        raise ValueError("FCIR-12 simulation window or cost is invalid")
    positions = {
        _utc(value): index
    ...
    def update_path(value: float) -> None:
        ...
        if not np.isfinite(value):
            raise ValueError("FCIR-12 strict equity path is non-finite")
    ...
    for clock in clocks.to_dict(orient="records"):
        ...
        if entry_time < start or exit_time > end:
            raise ValueError("FCIR-12 clock crosses the simulation window")
        if previous_exit is not None and entry_time < previous_exit:
            raise ValueError("FCIR-12 simulation schedule overlaps")
        ...
        if entry_position is None or exit_position is None:
            raise ValueError("FCIR-12 clock is absent from the market grid")
        if exit_position - entry_position != cfg.hold_bars:
            raise ValueError("FCIR-12 hold is not exactly 144 bars / 12 hours")
        side = int(clock["side"])
        if side not in (-1, 1):
            raise ValueError("FCIR-12 side must be -1 or 1")
        ...
        if next_funding != len(included_funding):
            raise ValueError("FCIR-12 funding event was not visited before exit")
...
def _build_stage_report(stage: str) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "flow_centrality_incubation_relay_stage_v1",
        "candidate": POLICY_ID,
        "stage": stage,
...
def render_stage_doc(report: dict[str, Any]) -> str:
    lines = [
        f"# FCIR-12 {report['stage']} strict result — 2026-07-19",
        "",
...
def evaluate_stage(stage: str) -> dict[str, Any]:
    ...
    if output.exists() or document.exists():
        raise FileExistsError(f"FCIR-12 {stage} result is write-once")
