"""Sequential strict economics for frozen HVPIAR-8."""
...
PREREG = Path("results/high_volatility_price_impact_asymmetry_relay_preregistration_2026-08-09.json")
PREREG_SHA = "a3cb689a6938708b76740e819a65db8d7e2c412bfaaaec37eda03f06bff3a55f"
SUPPORT = Path("results/high_volatility_price_impact_asymmetry_relay_support_2026-08-09.json")
SUPPORT_SHA = "8ef7ff49d77e6280724f35151e041dfcc659cd65914642cb209894ca96e0a721"
NOVELTY = Path("results/high_volatility_price_impact_asymmetry_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "270ec0ab0d1d18ddde3b438c93b82dce202bac32dde472648f0e8256e9ea3711"
CLOCK = Path("data/high_volatility_price_impact_asymmetry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "ca207639a686ddd4e95cf07388cf22192ccfeaa977797220164c1b2ad10d49ff"
CONTROL_DIR = Path("data/high_volatility_price_impact_asymmetry_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_price_impact_asymmetry_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_price_impact_asymmetry_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_asymmetry_tail", "no_variation_gate", "one_block_stale_asymmetry", "direction_flip", "same_clock_forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVPIAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVPIAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvpiar_8_sequential_economics_v1", "policy_id": POLICY_ID,
