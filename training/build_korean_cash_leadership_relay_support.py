"""Materialize outcome-blind source support for frozen KCLR-12."""
...
BUILDER = Path("training/build_korean_cash_leadership_relay_support.py")
PREREG_SHA = "058761e05f7884f36e4ba59e7b93f7d0cb42ead4f73471c4afbdc1dbd4bb9784"
SOURCE_DIR = Path("data/korean_cash_leadership_relay_sources_2023_2026")
FEATURE_PANEL = SOURCE_DIR / "korean_cash_leadership_relay_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/korean_cash_leadership_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/korean_cash_leadership_relay_controls_2023_2026")
RESULT = Path("results/korean_cash_leadership_relay_support_2026-08-09.json")
...
CONTROLS = (
    "no_volatility_gate", "no_leadership_gate", "binance_leadership",
...
CLOCK_COLUMNS = (
    ...
    "upbit_return", "perp_return", "perp_realized_variation", "variation_rank",
)
...
def load_bars(table: str, symbol: str) -> pd.DataFrame:
    ...
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError(f"KCLR {table} schema drift")
    frame["ts"] = pd.to_datetime(frame.ts, utc=True, errors="raise")
    frame = frame.sort_values("ts").reset_index(drop=True)
    if frame.empty or frame.ts.duplicated().any():
        raise RuntimeError(f"KCLR {table} empty or duplicated timestamps")
    ...
    if not np.isfinite(prices).all(axis=None) or not prices.gt(0).all(axis=None) or not coherent.all():
        raise RuntimeError(f"KCLR {table} contains invalid or incoherent OHLC")
...
def build_features(upbit: pd.DataFrame, perp: pd.DataFrame, lookback: int = 270, minimum: int = 180) -> pd.DataFrame:
    ...
    for start in pd.date_range(START, END, freq="1D", inclusive="left"):
        ...
        if u is None or p is None:
            continue
        upbit_return = float(np.log(u.close.iloc[-1] / u.open.iloc[0]))
...
def conditions(frame: pd.DataFrame, control: str = "primary") -> tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    same_direction = np.sign(used.upbit_return).eq(np.sign(used.perp_return)) & used.upbit_return.ne(0) & used.perp_return.ne(0)
    if control == "no_leadership_gate":
        leadership = pd.Series(True, index=frame.index)
    elif control == "binance_leadership":
        leadership = used.perp_return.abs().gt(used.upbit_return.abs())
    else:
        leadership = used.upbit_return.abs().gt(used.perp_return.abs())
    volatility = pd.Series(True, index=frame.index) if control == "no_volatility_gate" else frame.variation_rank.ge(0.65)
    active = same_direction & leadership & volatility
    side = np.sign(used.upbit_return).fillna(0).astype(int)
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active]:
        ...
        rows.append({
            "candidate": "KCLR-12", "control": control, "split": split,
            ...
            "side": int(sides.at[index]), "upbit_return": float(source.upbit_return),
            "perp_return": float(source.perp_return),
            "perp_realized_variation": float(features.at[index, "perp_realized_variation"]),
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("KCLR preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "kclr_12_sources_v1",
    ...
    core = {
        "protocol_version": "kclr_12_source_support_v1", "policy_id": "KCLR-12",
