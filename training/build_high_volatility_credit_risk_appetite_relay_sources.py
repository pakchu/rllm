"""Freeze outcome-blind Yahoo daily HYG/LQD source rows for HVCREDIT-24."""
...
SOURCE_DIR = Path("data/high_volatility_credit_risk_appetite_relay_sources_2022_2026")
OUTPUT = SOURCE_DIR / "hyg_lqd_sessions.csv.gz"
MANIFEST = SOURCE_DIR / "build_manifest.json"
BUILDER = Path("training/build_high_volatility_credit_risk_appetite_relay_sources.py")
PREREG_SHA256 = "edef0d226484713b2e9cb2497d5da388d7fb4fde90aeed6a34f0f3a0e405ae50"
...
def parse_payload(raw: bytes, symbol: str) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if chart.get("error") is not None:
        raise RuntimeError(f"HVCREDIT Yahoo error for {symbol}: {chart['error']}")
    results = chart.get("result") or []
    if len(results) != 1:
        raise RuntimeError(f"HVCREDIT Yahoo result count for {symbol}: {len(results)}")
    result = results[0]
    timestamps = result.get("timestamp") or []
    quotes = ((result.get("indicators") or {}).get("quote") or [])
    if not timestamps or len(quotes) != 1:
        raise RuntimeError(f"HVCREDIT incomplete Yahoo payload for {symbol}")
    quote = quotes[0]
    ...
    if opens is None or closes is None or len(opens) != len(timestamps) or len(closes) != len(timestamps):
        raise RuntimeError(f"HVCREDIT Yahoo vector mismatch for {symbol}")
    timezone = str((result.get("meta") or {}).get("exchangeTimezoneName") or "")
    if timezone != "America/New_York":
        raise RuntimeError(f"HVCREDIT timezone drift for {symbol}: {timezone}")
    ...
    if not valid.all() or frame["session_date"].duplicated().any() or not frame["session_date"].is_monotonic_increasing:
        raise RuntimeError(f"HVCREDIT invalid daily rows for {symbol}")
    metadata = {
        "symbol": symbol,
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA256:
        raise RuntimeError("HVCREDIT preregistration hash drift")
    ...
    panel = frames["HYG"].merge(frames["LQD"], on="session_date", how="inner", validate="one_to_one")
    if panel.empty or panel["session_date"].iloc[-1] != pd.Timestamp("2026-07-31"):
        raise RuntimeError("HVCREDIT shared-session coverage drift")
    deterministic_gzip_csv(panel, OUTPUT)
    core = {
        "protocol_version": "hvcredit_24_sources_v1",
