"""Sequential strict economics for frozen FGPLR-24."""
...
PREREG = Path("results/fear_greed_price_leadlag_relay_preregistration_2026-08-09.json")
PREREG_SHA = "eb729ad2f5e8d1e5153bb7db1fb528ac582211e4e724a25db6d99e96f0025073"
SUPPORT = Path("results/fear_greed_price_leadlag_relay_support_2026-08-09.json")
SUPPORT_SHA = "cb16a81851344c1067626cea39ae1a56ecb03b95e49ed5e42b1ab520c32b376b"
NOVELTY = Path("results/fear_greed_price_leadlag_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "0473a49c8116913d2c9dd036f65c290829706d341185decdf9be11c8d54e623f"
CLOCK = Path("data/fear_greed_price_leadlag_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "eead5fee437583f81985114c621bc4608c238d0c1cde2e9a23692ed1e02dca47"
CONTROL_DIR = Path("data/fear_greed_price_leadlag_relay_controls_2023_2026")
FREEZE = Path("results/fear_greed_price_leadlag_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/fear_greed_price_leadlag_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "no_sentiment_change_tail", "no_direction_disagreement", "sentiment_direction", "one_day_stale_sentiment_change", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("FGPLR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("FGPLR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "fgplr_24_sequential_economics_v1", "policy_id": POLICY_ID,
