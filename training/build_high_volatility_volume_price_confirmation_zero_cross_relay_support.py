"""Build outcome-blind source support for frozen HVVPCI-24."""
...
from training import preregister_high_volatility_volume_price_confirmation_zero_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "c2532e61901461ddbe5da93c2224216cbf400f8f6cd26cbc3234c3b86f5c057b"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_volume_price_confirmation_zero_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_volume_price_confirmation_zero_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_volume_price_confirmation_zero_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_volume_price_confirmation_zero_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_volume_price_confirmation_zero_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "bar_volume", "vwma_short", "vwma_long", "sma_short", "sma_long",
    "volume_short", "volume_long", "vpc", "vpr", "volume_multiplier", "vpci", "vpci_side", "vpc_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "bar_volume", "vwma_short", "vwma_long", "sma_short", "sma_long",
    "volume_short", "volume_long", "vpc", "vpr", "volume_multiplier", "vpci", "vpci_side", "vpc_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close", "volume"]:
        raise RuntimeError("HVVPCI source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVVPCI invalid source key")
...
def vpci_values(close: pd.Series, volume: pd.Series, valid: pd.Series) -> pd.DataFrame:
    short=P["short_periods"];long=P["long_periods"];price=close.where(valid);qty=volume.where(valid)
    def sma(x,n):return x.rolling(n,min_periods=n).mean()
    def vwma(n):
        denominator=qty.rolling(n,min_periods=n).sum()
        return ((price*qty).rolling(n,min_periods=n).sum()/denominator).where(denominator.gt(0))
    vwma_short=vwma(short);vwma_long=vwma(long);sma_short=sma(price,short);sma_long=sma(price,long);volume_short=sma(qty,short);volume_long=sma(qty,long)
    vpc=vwma_long-sma_long;vpr=(vwma_short/sma_short).where(sma_short.gt(0));multiplier=(volume_short/volume_long).where(volume_long.gt(0));vpci=vpc*vpr*multiplier
    return pd.DataFrame({"vwma_short":vwma_short,"vwma_long":vwma_long,"sma_short":sma_short,"sma_long":sma_long,"volume_short":volume_short,"volume_long":volume_long,"vpc":vpc,"vpr":vpr,"volume_multiplier":multiplier,"vpci":vpci},index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    values=vpci_values(bars.bar_close,bars.bar_volume,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["vpci_side"]=zero_cross_side(bars.vpci);bars["vpc_side"]=zero_cross_side(bars.vpc)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["bar_volume","vwma_short","vwma_long","sma_short","sma_long","volume_short","volume_long","vpc","vpr","volume_multiplier","vpci","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.volume_long.gt(0)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&panel.vpci_side.ne(0)&panel.variation_rank.ge(P["variation_rank_min"])
    return panel.loc[:,PANEL_COLUMNS]

def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);variation=used.variation_rank.ge(P["variation_rank_min"]);side=pd.to_numeric(used.vpci_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
    if control=="no_variation_gate":state=valid&side.ne(0)
    elif control=="vpc_only_zero_cross":side=pd.to_numeric(used.vpc_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVVPCI prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvvpci_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvvpci_24_source_support_v1", "policy_id": prereg.POLICY_ID,
