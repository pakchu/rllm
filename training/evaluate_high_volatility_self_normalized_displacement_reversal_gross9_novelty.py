"""Evaluate frozen HVSNDR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVSNDR-8"
PROTOCOL = "hvsndr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_self_normalized_displacement_reversal_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVSNDR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVSNDR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVSNDR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVSNDR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvsndr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVSNDR-8 unchanged before economic outcomes",
    }
