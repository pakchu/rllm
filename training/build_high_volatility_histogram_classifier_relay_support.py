"""Build OOS source-only HVHGC-12 clocks after the model freeze."""
...
PREREG_SHA = "338278324213e044bb5c647f5248d4727b4d87b16b28e230ebab3d033d39ae2f"
MODEL_FREEZE = Path(
    "results/high_volatility_histogram_classifier_relay_model_freeze_2026-08-09.json"
)
MODEL_FREEZE_SHA = "ce316402067e65f26ab255b095f6627ea6c122b8fa18c0bb8864157569b10c0d"
MODEL = Path("data/high_volatility_histogram_classifier_relay_model_2026-08-09.joblib")
MODEL_SHA = "3e5fb6c998232a3fdb30e8ab0d680ee9685c140faa079221629173a532618f0a"
CLOCK = Path("data/high_volatility_histogram_classifier_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_histogram_classifier_relay_controls_2023_2026")
SNAPSHOT = Path(
    "data/high_volatility_histogram_classifier_relay_sources_2023_2026/model_scores.csv.gz"
)
RESULT = Path(
    "results/high_volatility_histogram_classifier_relay_support_2026-08-09.json"
)
...
CONTROLS = ("no_volatility_gate", "no_probability_onset", "one_hour_stale_features", "direction_flip")
...
def verify_predecessors() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVHGC preregistration drift")
    if sha256(MODEL_FREEZE) != MODEL_FREEZE_SHA or sha256(MODEL) != MODEL_SHA:
        raise RuntimeError("HVHGC model predecessor drift")
    ...
    if freeze.get("manifest_hash") != canonical_hash(freeze_core):
        raise RuntimeError("HVHGC model freeze manifest drift")
    if (
        ...
        raise RuntimeError("HVHGC model freeze state drift")
    artifact = joblib.load(MODEL)
    if artifact.get("policy_id") != "HVHGC-12":
        raise RuntimeError("HVHGC model policy drift")
...
def load_combined_market() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if combined.date.duplicated().any() or not combined.date.is_monotonic_increasing:
        raise RuntimeError("HVHGC combined market time drift")
    deltas = combined.date.diff().dropna()
    if not deltas.eq(pd.Timedelta(minutes=5)).all():
        raise RuntimeError("HVHGC combined market is not continuous 5m")
    return combined, {
...
def score_snapshot(market: pd.DataFrame, artifact: dict[str, Any]) -> pd.DataFrame:
    ...
    if missing:
        raise RuntimeError(f"HVHGC OOS feature drift: {missing}")
    ...
    positions = positions[
        np.asarray(dates[positions + 1] == dates[positions] + pd.Timedelta(minutes=5))
    ]
    oos = np.asarray(dates[positions] >= pd.Timestamp("2023-07-01T00:00:00Z"))
    positions = positions[oos]
    probabilities = artifact["pipeline"].predict_proba(matrix[positions])[:, 1]
...
def build_clock(scores: pd.DataFrame, artifact: dict[str, Any], control: str = "primary") -> pd.DataFrame:
    ...
    if control == "one_hour_stale_features":
        frame[["positive_probability", "range_vol"]] = frame[["positive_probability", "range_vol"]].shift(1)
    probability = pd.to_numeric(frame.positive_probability, errors="coerce").to_numpy(float)
    volatility = pd.to_numeric(frame.range_vol, errors="coerce").to_numpy(float)
    long_signal = probability >= float(artifact["probability_high"])
    short_signal = probability <= float(artifact["probability_low"])
    if control != "no_probability_onset":
        previous_long = np.r_[False, long_signal[:-1]]
        previous_short = np.r_[False, short_signal[:-1]]
        long_signal &= ~previous_long
        short_signal &= ~previous_short
    ...
    finite = np.isfinite(probability) & np.isfinite(volatility)
    active = (long_signal | short_signal) & finite
    decisions = pd.to_datetime(frame.decision_bar_time, utc=True)
    rows: list[dict[str, Any]] = []
    next_allowed: pd.Timestamp | None = None
    for index in np.flatnonzero(active):
        if long_signal[index] and short_signal[index]:
            continue
        decision_time = decisions.iloc[index]
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvhgc_12_oos_source_support_v1",
        "policy_id": "HVHGC-12",
