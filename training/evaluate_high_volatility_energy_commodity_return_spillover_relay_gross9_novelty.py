"""Evaluate frozen HVECSP-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVECSP-24"
PROTOCOL = "hvecsp_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_energy_commodity_return_spillover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVECSP predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVECSP novelty limits drift")
    if (
        ...
        raise RuntimeError("HVECSP predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVECSP primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvecsp_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVECSP-24 unchanged before economic outcomes",
    }
