"""Evaluate frozen HVDLDA-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDLDA-24"
PROTOCOL = "hvlda_24_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_discriminant_state_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDLDA predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVDLDA novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDLDA predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDLDA primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvlda_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVDLDA-24 unchanged before economic outcomes"
        ),
    }
