"""Source-only support gate for frozen HVDFR-8."""
...
from training import preregister_high_volatility_directional_feedback_relay as prereg
...
PREREG_SHA = "111da91ab5e052ebb9eb10288ea94fde13e926ad2158eeaaf07d4a4a197bb361"
SPLITS = {
...
ROOT = Path("data/high_volatility_directional_feedback_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_directional_feedback_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_directional_feedback_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_directional_feedback_relay_support_2026-08-10.json")
...
def feedback_correlation(returns: np.ndarray) -> float:
    if len(returns) != 480 or not np.isfinite(returns).all():
        return math.nan
    signed = returns[:-1]
    next_magnitude = np.abs(returns[1:])
    if np.std(signed) <= 0 or np.std(next_magnitude) <= 0:
        return math.nan
    correlation = float(np.corrcoef(signed, next_magnitude)[0, 1])
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVDFR source timestamps")
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        if valid:
            ...
            feedback = feedback_correlation(returns)
            block_return = float(np.log(window["close"].iloc[-1] / window["open"].iloc[0]))
            valid = variation > 0 and math.isfinite(feedback)
        if not valid:
            variation = feedback = block_return = math.nan
        rows.append(
            {
                "decision_time": decision,
                "source_valid": valid,
                "realized_variation": variation,
                "feedback_correlation": feedback,
                "block_return": block_return,
            }
        )
    states = pd.DataFrame(rows)
    states["variation_rank"] = prior_rank(states["realized_variation"].where(states["source_valid"]))
    states["feedback_strength_rank"] = prior_rank(states["feedback_correlation"].abs().where(states["source_valid"]))
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvdfr_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    feedback_rank = states["feedback_strength_rank"]
    feedback = states["feedback_correlation"]
    if control == "one_block_stale_feedback":
        block_return, variation_rank, feedback_rank, feedback = [
            value.shift(1)
            for value in (block_return, variation_rank, feedback_rank, feedback)
        ]
    variation_gate = (
    ...
    if control == "no_feedback_tail":
        feedback_gate = pd.Series(True, index=states.index)
    elif control == "raw_absolute_feedback_above_tenth":
        feedback_gate = feedback.abs().gt(0.10)
    else:
        feedback_gate = feedback_rank.ge(0.75)
    agreement = (
        block_return.ne(0)
        & feedback.ne(0)
        & np.sign(block_return).eq(np.sign(feedback))
    )
    eligible = states["source_valid"] & variation_gate & feedback_gate & agreement
        ...
        rows.append(
            {
                ...
                "feedback_correlation": float(states.at[index, "feedback_correlation"]),
                "feedback_strength_rank": float(states.at[index, "feedback_strength_rank"]),
            }
        )
    columns = [
        "candidate", "control", "split", "decision_time", "feature_available_time",
        "entry_time", "exit_time", "side", "realized_variation", "variation_rank",
        "feedback_correlation", "feedback_strength_rank",
    ]
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVDFR preregistration drift")
    ...
    core = {
        "protocol_version": "hvdfr_8_source_support_v1",
        "policy_id": "HVDFR-8",
        "preregistration": {
