"""Sequential strict economics for frozen HVECOG-24."""
...
PREREG = Path("results/high_volatility_ehlers_center_of_gravity_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "1af12ac42688bc10f62fbd459191300781d00e709c8ea09c696d66861df0e3b5"
SUPPORT = Path("results/high_volatility_ehlers_center_of_gravity_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "00855e157e28dfba245b689b05efb74538a2618e38c90139305a3c3c101105af"
NOVELTY = Path("results/high_volatility_ehlers_center_of_gravity_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "417e1575dd6a00f83dd3c671f5cde5c215904dee91a62b0595429df416e09abb"
CLOCK = Path("data/high_volatility_ehlers_center_of_gravity_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "5544d02d3e990fdb6b51d31435229fbadcc1c4b0e00a6006545c710102a45666"
CONTROL_DIR = Path("data/high_volatility_ehlers_center_of_gravity_crossover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_ehlers_center_of_gravity_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ehlers_center_of_gravity_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "cog_zero_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVECOG frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVECOG novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvecog_24_sequential_economics_v1", "policy_id": POLICY_ID,
