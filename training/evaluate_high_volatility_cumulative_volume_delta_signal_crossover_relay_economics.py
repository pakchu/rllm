"""Sequential strict economics for frozen HVCVD-24."""
...
NOVELTY = Path("results/high_volatility_cumulative_volume_delta_signal_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "65d963e6dd852b55bb8965522af50430916fccaa30c567fd74a115a042ddf8ce"
...
FREEZE = Path("results/high_volatility_cumulative_volume_delta_signal_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cumulative_volume_delta_signal_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "delta_zero_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCVD frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCVD novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcvd_24_sequential_economics_v1", "policy_id": POLICY_ID,
