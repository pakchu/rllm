"""Sequential strict economics for frozen HVCASK-8."""
...
PREREG = Path("results/high_volatility_cross_alt_realized_skew_spillover_consensus_relay_preregistration_2026-08-13.json")
PREREG_SHA = "d2b32b2b020b27cee4e8106a7c3b790ddeb54cc0d099e42e8e6d286b81fead7d"
SUPPORT = Path("results/high_volatility_cross_alt_realized_skew_spillover_consensus_relay_support_2026-08-13.json")
SUPPORT_SHA = "404a22b99086df75c0c645c50d1ca0cfb88bce1b28737218955138f53f51542b"
NOVELTY = Path("results/high_volatility_cross_alt_realized_skew_spillover_consensus_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "255bbd26b6e43c062395a1c06904d8e31b5a7b8cd7e23f3c3dae7f5ddaf6004f"
CLOCK = Path("data/high_volatility_cross_alt_realized_skew_spillover_consensus_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "938105b7874ee67390b52bcc2d433e5f2c105cd8b626da271dc2976ec1dd83ef"
CONTROL_DIR = Path("data/high_volatility_cross_alt_realized_skew_spillover_consensus_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_cross_alt_realized_skew_spillover_consensus_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_alt_realized_skew_spillover_consensus_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "no_skew_tail", "three_alt_consensus", "unnormalized_third_moment", "one_decision_stale_skew", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCASK frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCASK novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcask_8_sequential_economics_v1", "policy_id": POLICY_ID,
