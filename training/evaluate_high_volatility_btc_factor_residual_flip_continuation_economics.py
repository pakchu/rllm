"""Strict staged economics for the singleton HVBFRRFC-8 policy."""
...
PREREG = Path("results/high_volatility_btc_factor_residual_flip_continuation_preregistration_2026-08-18.json")
PREREG_SHA = "21077ee8eee579633bc6aaad3df66fda6a5468cece4bdbb2e0da8ff2171b337f"
SUPPORT = Path("results/high_volatility_btc_factor_residual_flip_continuation_support_2026-08-18.json")
SUPPORT_SHA = "51016080e3ff68ffead9f368f5d0a0faf1627e6ccbb9c5a2f84852a0892a7a0b"
GROSS9 = Path("results/high_volatility_btc_factor_residual_flip_continuation_gross9_novelty_2026-08-18.json")
GROSS9_SHA = "2636941aa29c7a9665e9feb4e1df732f2085b6fb64dd73c7f43a7ededfcc6115"
CLOCK = {
    "path": Path("data/high_volatility_btc_factor_residual_flip_continuation_clocks_2023_2026.csv.gz"),
    "sha256": "29ce66d8f9d8c0606262087fb71ab9dc6a3c7d5e215e50f03f72ee3d34a4613b",
    "rows": 202,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_btc_factor_residual_flip_continuation_{stage}_economics_2026-08-18.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVBFRRFC-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVBFRRFC-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVBFRRFC-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVBFRRFC-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVBFRRFC-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVBFRRFC-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVBFRRFC-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVBFRRFC-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVBFRRFC-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVBFRRFC-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVBFRRFC-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvbfrrfc_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
