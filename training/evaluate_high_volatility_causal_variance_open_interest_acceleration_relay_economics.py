"""Strict staged economics for the singleton HVCVAROIAR-8 policy."""
...
PREREG = Path("results/high_volatility_causal_variance_open_interest_acceleration_relay_preregistration_2026-08-16.json")
PREREG_SHA = "6ede19674add22a2facd9f5f186baedf3a6ae90d5e80104ca1c8143bf6ddaea3"
SUPPORT = Path("results/high_volatility_causal_variance_open_interest_acceleration_relay_support_2026-08-16.json")
SUPPORT_SHA = "81043cd015b4825f256e1775a95b6eccd61037621c88fdff72c5701e2dac4f00"
GROSS9 = Path("results/high_volatility_causal_variance_open_interest_acceleration_relay_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "6cba39e59ff004cea4797ad3caa446be76b52c758cadf48d317e9d990cf9ad1c"
CLOCK = {
    "path": Path("data/high_volatility_causal_variance_open_interest_acceleration_relay_clocks_2020_2026.csv.gz"),
    "sha256": "c068d2dafeae2eba849acb355cba1b049e6b3603c6104dc507681fd36413e947",
    "rows": 109,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_variance_open_interest_acceleration_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCVAROIAR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCVAROIAR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCVAROIAR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCVAROIAR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVAROIAR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVAROIAR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCVAROIAR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCVAROIAR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCVAROIAR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCVAROIAR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCVAROIAR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcvaroair_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
