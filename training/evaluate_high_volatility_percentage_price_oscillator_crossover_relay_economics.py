"""Sequential strict economics for frozen HVPPO-24."""
...
PREREG = Path("results/high_volatility_percentage_price_oscillator_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "be6d04502fadf35019b3282eed3196d71eca36d6e01aafaec9254c439061c915"
SUPPORT = Path("results/high_volatility_percentage_price_oscillator_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "e8754485409ddbae410020b6de174fd49d33ebd73f2dc5cb312c71a8461a82c3"
NOVELTY = Path("results/high_volatility_percentage_price_oscillator_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "1f5de5f5ae5ad48729b83b6b7cbf49ccef28923b495186c8181e627ece777c1c"
CLOCK = Path("data/high_volatility_percentage_price_oscillator_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "08862d80846d7f77fd36ef67e8409f9a7097d606b87dd42655a14f0b4c9123a4"
CONTROL_DIR = Path("data/high_volatility_percentage_price_oscillator_crossover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_percentage_price_oscillator_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_percentage_price_oscillator_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "zero_line_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVPPO frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVPPO novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvppo_24_sequential_economics_v1", "policy_id": POLICY_ID,
