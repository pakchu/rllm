"""Build outcome-blind source support for frozen HVDYMI-24."""
...
from training import preregister_high_volatility_dynamic_momentum_index_reentry_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "443e0ff0f08c1b8240bd895f68ecb3ea8c9a7e399a1618b4f2a244ef91e2bfe8"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_dynamic_momentum_index_reentry_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_dynamic_momentum_index_reentry_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_dynamic_momentum_index_reentry_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_dynamic_momentum_index_reentry_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_dynamic_momentum_index_reentry_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high", "bar_low", "bar_close", "std_close_5", "std_average_10", "volatility_ratio", "dynamic_period", "dymi", "fixed_fourteen_rsi", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time", "entry_time", "exit_time", "side", "std_close_5", "std_average_10", "volatility_ratio", "dynamic_period", "dymi", "fixed_fourteen_rsi", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVDYMI source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVDYMI invalid source key")
...
def dynamic_momentum(close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    masked=close.where(valid);std5=masked.rolling(P["std_periods"],min_periods=P["std_periods"]).std(ddof=1);std_average=std5.rolling(P["std_average_periods"],min_periods=P["std_average_periods"]).mean();ratio=(std5/std_average).where(std_average.gt(0)&std5.gt(0));period=np.floor(P["base_rsi_periods"]/ratio).clip(P["minimum_rsi_periods"],P["maximum_rsi_periods"]);change=masked.diff();gain=change.clip(lower=0);loss=(-change).clip(lower=0);dymi=np.full(len(close),np.nan);fixed=np.full(len(close),np.nan)
    def value_at(i,n):
        if i-n<0 or not bool(valid.iloc[i-n:i+1].all()):return np.nan
        up=float(gain.iloc[i-n+1:i+1].sum());down=float(loss.iloc[i-n+1:i+1].sum());den=up+down
        return 100*up/den if den>0 else np.nan
    for i in range(len(close)):
        if np.isfinite(period.iloc[i]):dymi[i]=value_at(i,int(period.iloc[i]))
        fixed[i]=value_at(i,P["base_rsi_periods"])
    return pd.DataFrame({"std_close_5":std5,"std_average_10":std_average,"volatility_ratio":ratio,"dynamic_period":period,"dymi":dymi,"fixed_fourteen_rsi":fixed},index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[["bar_open","bar_high","bar_low","bar_close"]]).all(axis=1)&bars[["bar_open","bar_high","bar_low","bar_close"]].gt(0).all(axis=1);bars=pd.concat([bars,dynamic_momentum(bars.bar_close,bars.valid_bar)],axis=1);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["std_close_5","std_average_10","volatility_ratio","dynamic_period","dymi","fixed_fourteen_rsi","variation"]]).all(axis=1)&bars.dynamic_period.between(P["minimum_rsi_periods"],P["maximum_rsi_periods"])&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.dymi.shift(1);long=previous.le(P["lower_level"])&panel.dymi.gt(P["lower_level"]);short=previous.ge(P["upper_level"])&panel.dymi.lt(P["upper_level"]);panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&(long|short)&panel.variation_rank.ge(P["variation_rank_min"]);return panel.loc[:,PANEL_COLUMNS]


def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);value=pd.to_numeric(used.fixed_fourteen_rsi if control=="fixed_fourteen_rsi" else used.dymi,errors="coerce");previous=value.shift(1);long=previous.le(P["lower_level"])&value.gt(P["lower_level"]);short=previous.ge(P["upper_level"])&value.lt(P["upper_level"]);variation=used.variation_rank.ge(P["variation_rank_min"]);side=pd.Series(np.where(long,1,np.where(short,-1,0)),index=used.index,dtype=int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVDYMI prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvdymi_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvdymi_24_source_support_v1", "policy_id": prereg.POLICY_ID,
