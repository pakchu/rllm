"""Evaluate frozen CAEVC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "CAEVC-8"
PROTOCOL = "caevc_8_gross9_novelty_v1"
PREREG = Path("results/cross_asset_elasticity_vacuum_continuation_preregistration_2026-08-09.json")
PREREG_SHA = "234d09998bf8f3c56fdb4687a6c563874ebd1e56374ad3acf50a7112f218bc48"
SUPPORT = Path("results/cross_asset_elasticity_vacuum_continuation_support_2026-08-09.json")
SUPPORT_SHA = "4e761562113b1429a57bef923fa20f0c9e9dbb171f3aecd4ced4aea502829350"
CLOCK = Path("data/cross_asset_elasticity_vacuum_continuation_clocks_2023_2026.csv.gz")
CLOCK_SHA = "8ccd3d7799464492c02a73b930acb1730db61d8519293c2fbc7c8fab3486523d"
OUTPUT = Path("results/cross_asset_elasticity_vacuum_continuation_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CAEVC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("CAEVC novelty limits drift")
    if (
        ...
        raise RuntimeError("CAEVC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CAEVC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "caevc_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject CAEVC-8 unchanged before economic outcomes",
    }
