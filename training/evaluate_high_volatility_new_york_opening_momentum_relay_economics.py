"""Sequential strict economics for frozen HVCOM-355M."""
...
PREREG = Path("results/high_volatility_new_york_opening_momentum_relay_preregistration_2026-08-11.json")
PREREG_SHA = "45f5aba85e0b8eff2dfa119cff1b5f39e26be3b89a5b5f899b3c7e24b01d3ea4"
SUPPORT = Path("results/high_volatility_new_york_opening_momentum_relay_support_2026-08-11.json")
SUPPORT_SHA = "0ee6b5e60c4ded0c066c0a3f05d2bc437e41538cb7e3729f49e11f23ca4fc9a6"
NOVELTY = Path("results/high_volatility_new_york_opening_momentum_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "fb580072009f6c25e66323c3612d433f4c3cd4c732b6b29c5fe9f885a632c16a"
CLOCK = Path("data/high_volatility_new_york_opening_momentum_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "0de3d4d4bcb04b74d5aa3638dd6a38cc038bb2dfdec8f31664d61f04f2a699a0"
CONTROL_DIR = Path("data/high_volatility_new_york_opening_momentum_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_new_york_opening_momentum_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_new_york_opening_momentum_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "first_fifteen_minute_direction", "one_weekday_stale_reaction", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCOM frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCOM novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcom_355m_sequential_economics_v1", "policy_id": POLICY_ID,
