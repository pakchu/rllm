"""Evaluate frozen HVTGAL-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVTGAL-24"
PROTOCOL = "hvtgal_24_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_tga_liquidity_relay_preregistration_2026-08-10.json"
)
...
OUTPUT = Path(
    "results/high_volatility_tga_liquidity_relay_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTGAL predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVTGAL novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTGAL predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTGAL primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvtgal_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVTGAL-24 unchanged before economic outcomes"
        ),
    }
