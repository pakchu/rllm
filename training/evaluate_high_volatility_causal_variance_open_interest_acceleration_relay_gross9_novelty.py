"""Evaluate the frozen HVCVAROIAR-8 action-vote clock against Gross9."""
...
POLICY = "HVCVAROIAR-8"
PROTOCOL = "hvcvaroair_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_variance_open_interest_acceleration_relay_clocks_2020_2026.csv.gz",
    "sha256": "c068d2dafeae2eba849acb355cba1b049e6b3603c6104dc507681fd36413e947",
    "rows": 109,
}
OUTPUT = Path("results/high_volatility_causal_variance_open_interest_acceleration_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCVAROIAR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCVAROIAR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVAROIAR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCVAROIAR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCVAROIAR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCVAROIAR-8 unchanged before economics",
    }
