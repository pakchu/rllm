"""Evaluate frozen HVCABRR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCABRR-8"
PROTOCOL = "hvcabrr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_cross_alt_barrier_rejection_reversal_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCABRR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCABRR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCABRR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCABRR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCABRR-8 unchanged before economic outcomes",
    }
