"""Evaluate frozen HVVRCR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVVRCR-12"
PROTOCOL = "hvvrcr_12_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_volume_return_covariance_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVVRCR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVVRCR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVVRCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVVRCR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVVRCR-12 unchanged before economic outcomes",
    }
