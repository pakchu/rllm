"""Evaluate frozen HVDVTM-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDVTM-8"
PROTOCOL = "hvdvtm_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_directional_variation_timing_migration_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDVTM predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVDVTM novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDVTM predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDVTM primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVDVTM-8 unchanged before economic outcomes",
    }
