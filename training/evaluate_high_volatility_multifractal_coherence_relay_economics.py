"""Sequential strict economics for frozen HVMFCR-8."""
...
PREREG = Path("results/high_volatility_multifractal_coherence_relay_preregistration_2026-08-10.json")
PREREG_SHA = "807de7fc4dc2cb5762794d562e4178c6277c9903e5610797aa03b32a3a1a5476"
SUPPORT = Path("results/high_volatility_multifractal_coherence_relay_support_2026-08-10.json")
SUPPORT_SHA = "e0a6938cee24f306f364dea27d0ce066f0b54cb029458683cc8e1eb48358fe23"
NOVELTY = Path("results/high_volatility_multifractal_coherence_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "390a39f496ae075e81b2ccecb1fee2d6d40912e6afa1042f59809e1437a86343"
CLOCK = Path("data/high_volatility_multifractal_coherence_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "2c46dc9ec3974c2126f80ebdcff46cbe2f0d8f63db510acefd6ba8466fb106f0"
CONTROL_DIR = Path("data/high_volatility_multifractal_coherence_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_multifractal_coherence_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_multifractal_coherence_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_coherence_tail_gate", "no_variation_gate", "q1_persistence", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVMFCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVMFCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvmfcr_8_sequential_economics_v1", "policy_id": POLICY_ID,
