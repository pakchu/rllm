"""Deterministic outcome-blind source support for HVCFLR-8."""
...
from training import preregister_high_volatility_causal_funding_level_reversal as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2023-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "ed1a64f72bfbd88917344a15341dcc7780196e4005617d6157b8765132b8c129"
...
ROOT = Path("data/high_volatility_causal_funding_level_reversal_sources_2023_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_funding_level_reversal_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_funding_level_reversal_split_clocks_2023_2026")
RESULT = Path("results/high_volatility_causal_funding_level_reversal_support_2026-08-16.json")
...
CLOCK_COLUMNS = (
    ...
    "entry_time", "exit_time", "side", "funding_rate",
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if funding.columns.tolist() != ["funding_time", "funding_rate"]:
        raise RuntimeError("HVCFLR-8 funding schema drift")
    expected_bars = [
    ...
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCFLR-8 bar schema drift")
    ...
    if funding["funding_time"].duplicated().any() or not np.isfinite(funding["funding_rate"]).all():
        raise RuntimeError("HVCFLR-8 invalid funding rows")
    ...
    if bars["decision_time"].duplicated().any():
        raise RuntimeError("HVCFLR-8 duplicate eight-hour bar")
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel["source_valid"] = (
        ...
        & np.isfinite(panel[["funding_rate", "realized_variation"]]).all(axis=1)
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCFLR-8 side or availability drift")
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCFLR-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcflr_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvcflr_8_source_support_v1", "policy_id": prereg.POLICY_ID,
