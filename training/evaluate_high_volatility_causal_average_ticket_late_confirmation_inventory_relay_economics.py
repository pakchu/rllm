"""Strict staged economics for the singleton HVCATLIR-8 policy."""
...
PREREG = Path("results/high_volatility_causal_average_ticket_late_confirmation_inventory_relay_preregistration_2026-08-16.json")
PREREG_SHA = "ece1e90b37852f34f45dd52a2ce698f493cebf3aa174686429e2bfbf631edc51"
SUPPORT = Path("results/high_volatility_causal_average_ticket_late_confirmation_inventory_relay_support_2026-08-16.json")
SUPPORT_SHA = "27be8cb55b5756ff487820986d4edade8d2d53790db6c91080291a27ecffa00f"
GROSS9 = Path("results/high_volatility_causal_average_ticket_late_confirmation_inventory_relay_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "de2f790a91934d70660d7d53a595cdc0d094cc4820d8fa30d7ae0fd1e1a61d7e"
CLOCK = {
    "path": Path("data/high_volatility_causal_average_ticket_late_confirmation_inventory_relay_clocks_2020_2026.csv.gz"),
    "sha256": "dbe8e5441049cd3d5b3e03cb852c34273176ce21530f5be99d8235fcc39f3d0d",
    "rows": 115,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_average_ticket_late_confirmation_inventory_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCATLIR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCATLIR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCATLIR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCATLIR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATLIR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATLIR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCATLIR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCATLIR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCATLIR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCATLIR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCATLIR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcatlir_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
