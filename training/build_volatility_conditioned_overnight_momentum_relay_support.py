"""Build outcome-blind source support for preregistered VOMR-6."""
...
from training import preregister_volatility_conditioned_overnight_momentum_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.build_cboe_volatility_surface_regime_crossing_relay_support import strict_prior_midrank

CLOCK = Path("data/volatility_conditioned_overnight_momentum_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/volatility_conditioned_overnight_momentum_relay_controls_2023_2026")
RESULT = Path("results/volatility_conditioned_overnight_momentum_relay_support_2026-08-08.json")
SPLITS = {
...
CONTROLS = ("no_relative_convexity_gate", "no_overnight_tail", "vix_level_gate", "one_session_stale_volatility", "direction_flip")
...
COLUMNS = (
    ...
    "side", "relative_convexity_rank", "previous_relative_convexity_rank", "vix_level_rank", "overnight_abs_rank", "overnight_btc_return",
)
...
def load_features() -> pd.DataFrame:
    for raw, expected in prereg.HASHES.items():
        if sha(Path(raw)) != expected:
            raise RuntimeError(f"VOMR frozen source changed: {raw}")
    ...
    if price.columns.tolist() != ["hour_start", "hour_open", "first_half_close", "second_half_open", "hour_close", "source_rows", "distinct_timestamps", "source_valid", "decision_time"]:
        raise RuntimeError("VOMR completed-price schema changed")
    ...
    if price.hour_start.duplicated().any() or not price.hour_start.is_monotonic_increasing:
        raise RuntimeError("VOMR completed-price clock invalid")
    ...
    result["overnight_abs_rank"] = strict_prior_midrank(result.overnight_btc_return.abs())
    return result


def signal(frame: pd.DataFrame, control: str) -> pd.Series:
    btc, relative, tail = frame.overnight_btc_return, frame.relative_convexity_rank, frame.overnight_abs_rank
    valid = frame.valid & btc.ne(0) & relative.notna() & tail.notna()
    side = np.sign(btc).astype("Int64").fillna(0).astype(int)
    if control == "no_relative_convexity_gate":
        eligible = frame.valid & btc.ne(0) & tail.ge(0.75)
    elif control == "no_overnight_tail":
        eligible = frame.valid & btc.ne(0) & relative.ge(0.60)
    elif control == "vix_level_gate":
        eligible = frame.valid & btc.ne(0) & tail.ge(0.75) & frame.vix_level_rank.ge(0.60)
    elif control == "one_session_stale_volatility":
        eligible = frame.valid & btc.ne(0) & tail.ge(0.75) & frame.previous_relative_convexity_rank.ge(0.60)
    else:
        eligible = valid & relative.ge(0.60) & tail.ge(0.75)
...
def build_clock(frame: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in frame.index[sides.ne(0)]:
        ...
        rows.append({
            "candidate": "VOMR-6", "control": control, "split": split,
            ...
            "vix_level_rank": float(frame.at[index, "vix_level_rank"]), "overnight_abs_rank": float(frame.at[index, "overnight_abs_rank"]),
