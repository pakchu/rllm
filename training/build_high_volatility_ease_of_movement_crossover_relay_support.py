"""Build outcome-blind source support for frozen HVEOM-24."""
...
PREREG_SHA = "fc28b0927f339a2c8b2709aa856e6172697dd1a57f38f40b67da5b6cd76612a6"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ease_of_movement_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ease_of_movement_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ease_of_movement_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ease_of_movement_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ease_of_movement_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "bar_volume", "midpoint", "midpoint_move",
    "raw_eom", "smoothed_eom", "midpoint_sma", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "bar_volume", "midpoint", "midpoint_move",
    "raw_eom", "smoothed_eom", "midpoint_sma", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close", "volume"]:
        raise RuntimeError("HVEOM source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVEOM invalid source key")
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["midpoint"] = (bars.bar_high + bars.bar_low) / 2.0
    bars["midpoint_move"] = bars.midpoint - bars.midpoint.shift(1)
    bars["raw_eom"] = (bars.midpoint_move * (bars.bar_high - bars.bar_low) / bars.bar_volume).where(bars.valid_bar & bars.valid_bar.shift(1, fill_value=False))
    period=P["eom_periods"]; complete=bars.valid_bar.rolling(period,min_periods=period).sum().eq(period)
    bars["smoothed_eom"] = bars.raw_eom.rolling(period,min_periods=period).mean().where(complete)
    bars["midpoint_sma"] = bars.midpoint_move.rolling(period,min_periods=period).mean().where(complete)
    bars["variation"] = np.sqrt(bars.variation_component.rolling(P["variation_hours"] // 4, min_periods=P["variation_hours"] // 4).sum())
    bars["source_valid"] = bars.valid_bar & np.isfinite(bars[["bar_volume","midpoint","midpoint_move","raw_eom","smoothed_eom","midpoint_sma","variation"]]).all(axis=1) & bars.bar_volume.gt(0) & bars.smoothed_eom.ne(0) & bars.variation.gt(0)
    ...
    previous=panel.smoothed_eom.shift(1)
    panel["eligible"]=panel.source_valid & panel.source_valid.shift(1,fill_value=False) & previous.ne(0) & np.sign(panel.smoothed_eom).ne(np.sign(previous)) & panel.variation_rank.ge(P["variation_rank_min"])
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    signal=pd.to_numeric(used.smoothed_eom,errors="coerce");previous=signal.shift(1)
    ...
    elif control=="midpoint_only_crossover":
        midpoint=pd.to_numeric(used.midpoint_sma,errors="coerce");prior=midpoint.shift(1)
        state=valid&midpoint.ne(0)&prior.ne(0)&np.sign(midpoint).ne(np.sign(prior))&variation
        side=np.sign(midpoint).fillna(0).astype(int)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVEOM prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hveom_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hveom_24_source_support_v1", "policy_id": prereg.POLICY_ID,
