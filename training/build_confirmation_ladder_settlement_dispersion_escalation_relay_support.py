"""Build source-only CLSDER-6 confirmation-ladder clocks."""
...
from training import preregister_confirmation_ladder_settlement_dispersion_escalation_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv


ENV_FILE = "/home/pakchu/rllm/.env"
PREREG_SHA = "43e5081b606437f5c9d90ca22684752d45bcf1c169d4250ed2aab51a3089feb1"
...
CONTROLS = ("late_unanimity_only", "dispersion_only", "one_anchor_stale_ladder", "direction_flip")
...
SOURCE_DIR = Path("data/confirmation_ladder_settlement_dispersion_escalation_relay_sources_2023_2026")
FEATURES = SOURCE_DIR / "confirmation_ladders.csv.gz"
MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/confirmation_ladder_settlement_dispersion_escalation_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/confirmation_ladder_settlement_dispersion_escalation_relay_controls_2023_2026")
RESULT = Path("results/confirmation_ladder_settlement_dispersion_escalation_relay_support_2026-08-12.json")
...
FEATURE_COLUMNS = (
    ...
    *RETURN_COLUMNS, *WEIGHT_COLUMNS, "late_return", "late_unanimous", "early_dispersion", "late_dispersion",
    "dispersion_escalation", "eligible_state",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "anchor_height", "confirmation_height",
    "feature_available_time", "entry_time", "exit_time", "side", "late_return",
    "late_unanimous", "early_dispersion", "late_dispersion", "dispersion_escalation",
)
...
def prepare_minutes(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CLSDER minute schema drift")
    ...
    if result.ts.duplicated().any():
        raise RuntimeError("CLSDER duplicate timestamps")
...
def build_features(blocks: pd.DataFrame, market: pd.DataFrame) -> pd.DataFrame:
    ...
    for index in range(0, len(blocks) - 6):
        ...
        def normalized_dispersion(values):
            array = np.asarray(values, dtype=float)
            mean = float(array.mean())
            return float(np.mean(np.abs(array - mean)) / mean)
        early_dispersion = normalized_dispersion(weights[:3]) if valid else np.nan
        late_dispersion = normalized_dispersion(weights[3:]) if valid else np.nan
        dispersion_escalation = bool(valid and late_dispersion > early_dispersion)
        rows.append({
            ...
            **{column: weights[position] for position, column in enumerate(WEIGHT_COLUMNS)},
            ...
            "early_dispersion": early_dispersion, "late_dispersion": late_dispersion,
            "dispersion_escalation": dispersion_escalation,
        })
    output = pd.DataFrame(rows)
    output["eligible_state"] = output.source_valid & output.late_unanimous & output.dispersion_escalation
...
def build_features_from_cache(cached: pd.DataFrame) -> pd.DataFrame:
    ...
    if not set(input_columns).issubset(cached.columns):
        raise RuntimeError("CLSDER cached pre-entry panel schema drift")
    ...
    def row_dispersion(values):
        means = values.mean(axis=1)
        return np.mean(np.abs(values - means[:, None]), axis=1) / means
    ...
    output["early_dispersion"] = row_dispersion(weights[:, :3])
    output["late_dispersion"] = row_dispersion(weights[:, 3:])
    output["dispersion_escalation"] = valid & output.late_dispersion.gt(output.early_dispersion)
    output["source_valid"] = valid
    output["eligible_state"] = valid & output.late_unanimous & output.dispersion_escalation
...
def active_and_side(features: pd.DataFrame, control: str = "primary"):
    ...
    elif control == "dispersion_only":
        state = features.source_valid & features.dispersion_escalation
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active & sides.ne(0)]:
        ...
        rows.append({
            ...
            "early_dispersion": float(features.at[index, "early_dispersion"]), "late_dispersion": float(features.at[index, "late_dispersion"]),
            "dispersion_escalation": bool(features.at[index, "dispersion_escalation"]),
        })
...
def run() -> dict[str, Any]:
    ...
    if sha(CACHED_PANEL) != CACHED_PANEL_SHA:
        raise RuntimeError("CLSDER cached pre-entry panel hash drift")
    ...
    source_core = {
        "protocol_version": "clsder_6_source_v1", "query_sha256": hashlib.sha256(QUERY.encode()).hexdigest(), "window": [QUERY_START.isoformat(), END.isoformat()],
    ...
    core = {
        "protocol_version": "clsder_6_source_support_v1", "policy_id": prereg.POLICY_ID,
