"""Strict staged economics for the singleton HVCATTAIR-8 policy."""
...
PREREG = Path("results/high_volatility_causal_average_ticket_trade_arrival_inventory_relay_preregistration_2026-08-16.json")
PREREG_SHA = "fe9e9acf1dfd983c97a211d152a2e6cbf2a08a2729577ef48e92d1c2aa2c8e96"
SUPPORT = Path("results/high_volatility_causal_average_ticket_trade_arrival_inventory_relay_support_2026-08-16.json")
SUPPORT_SHA = "8783dab8b5a2c9310ad89eccdff62625df219d8d293d6274142624a4a2bfaa66"
GROSS9 = Path("results/high_volatility_causal_average_ticket_trade_arrival_inventory_relay_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "b7648fd38bb78e27b60965e4d0dd42183c7958ceb6785b561f51b46920646c00"
CLOCK = {
    "path": Path("data/high_volatility_causal_average_ticket_trade_arrival_inventory_relay_clocks_2020_2026.csv.gz"),
    "sha256": "8d070f4e2545353aa87d931abb05836e9fe6fa7275d773e3f6b3ecfef20cdbc3",
    "rows": 99,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_average_ticket_trade_arrival_inventory_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCATTAIR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCATTAIR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCATTAIR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCATTAIR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATTAIR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATTAIR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCATTAIR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCATTAIR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCATTAIR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCATTAIR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCATTAIR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcattair_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
