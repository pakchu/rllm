"""Outcome-blind preregistration for HVCEDR-24."""
...
POLICY_ID = "HVCEDR-24"
DEFAULT_OUTPUT = Path("results/high_volatility_causal_extreme_displacement_reversal_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_causal_extreme_displacement_reversal_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": "During causally elevated twenty-four-hour variation, an extreme current four-hour high or low displacement beyond the trailing close envelope identifies an intrabar excursion unsupported by prior accepted closes and should reverse over twenty-four hours.",
            "side": "long when normalized downside displacement dominates; short when normalized upside displacement dominates",
            "why_distinct": "HVCEDR compares the current completed auction extrema with the prior 22 accepted closes. It is not path-ordered drawup/drawdown, a high-low range breakout, wick/body geometry, OI or funding confirmation, a fitted channel, reused event set, or promoted control.",
        ...
        "features": {
            ...
            "eligible": "23 uninterrupted coherent four-hour auctions including current, one strict dominant displacement, and both ranks pass at D; no OI, funding, premium, alt, or post-decision condition",
            "no_imputation": True,
        },
        "clock": {
        ...
        "stopping_rule": "terminal first failure; no auction, prior-close envelope, displacement formula, rank history, variation, side, clock, hold, subset, threshold, comparator, or control repair",
    }
    return {**core, "manifest_hash": canonical_hash(core)}

def validate(value: Mapping[str, Any]) -> None:
    if value != build():
        raise RuntimeError("HVCEDR-24 preregistration drift")
    contract = hvcav.build()
    for key in ("stages", "source_support_gates", "gross9_novelty_gates", "economic_gates"):
        if value[key] != contract[key]:
            raise RuntimeError(f"HVCEDR-24 {key} drift")
