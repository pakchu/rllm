"""Build outcome-blind BTC quantity-lattice cohort features."""
...
from preprocessing.quantity_lattice_cohort import BAR_COLUMNS, aggregate_quantity_lattice_five_minute
...
QLCD_UNDERLYING_OVERLAP_COUNTS = {"2020-01-15": 1}
...
class BuildConfig:
    ...
    output_dir: str = "data/binance_um_quantity_lattice_btc_2020_2023"
    workers: int = 4
    retries: int = 5
...
def load_source_contract() -> SourceContract:
    ...
    if not set(QLCD_UNDERLYING_OVERLAP_COUNTS).issubset(archive_hashes):
        raise ValueError("QLCD underlying-overlap quarantine is outside the source interval")
    return SourceContract(
        archive_hashes,
        archive_facts,
        gap_days,
        dict(QLCD_UNDERLYING_OVERLAP_COUNTS),
        zero_bins,
    )
...
def _monthly_paths(cfg: BuildConfig, month: date) -> tuple[Path, Path]:
    ...
    stem = f"{cfg.symbol}_quantity_lattice_5m_{month:%Y-%m}"
...
def _materialize_daily_grid(
    ...
    if indexed.index.duplicated().any():
        raise ValueError(f"duplicate quantity-lattice bars on {day}")
...
def _process_month(
    ...
    *,
    ...
    for day in expected_days:
        ...
        observed_bars = aggregate_quantity_lattice_five_minute(raw)
        if not (
...
def build(
    ...
    *,
    ...
    if combined["date"].duplicated().any() or not combined["date"].is_monotonic_increasing:
        raise ValueError("combined quantity-lattice output has duplicate or unordered timestamps")
    if tuple(combined.columns) != BAR_COLUMNS:
        raise ValueError("combined quantity-lattice output schema changed")
    ...
    if len(combined) != len(expected_grid) or not combined["date"].reset_index(drop=True).equals(
        ...
        raise ValueError("combined quantity-lattice output is not a complete five-minute grid")
    ...
    combined_path = output_dir / f"{cfg.symbol}_quantity_lattice_5m_{cfg.start}_{last_day}.csv.gz"
    ...
    manifest = {
        ...
        "protocol": {
            ...
            "quantity_lattice": "exact integer milli-BTC derived from quantity*1000",
            "coarse_cohort": "quantity_mbtc divisible by 100",
            "medium_cohort": "quantity_mbtc divisible by 10 but not 100",
            "fine_cohort": "quantity_mbtc not divisible by 10",
            "price_or_notional_features_stored": False,
