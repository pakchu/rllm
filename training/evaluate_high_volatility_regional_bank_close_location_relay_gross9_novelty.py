"""Evaluate frozen HVKRECLV-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVKRECLV-24"
PROTOCOL = "hvkreclv_24_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_regional_bank_close_location_relay_preregistration_2026-08-11.json"
)
...
OUTPUT = Path(
    "results/high_volatility_regional_bank_close_location_relay_gross9_novelty_2026-08-11.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVKRECLV predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVKRECLV novelty limits drift")
    if (
        ...
        raise RuntimeError("HVKRECLV predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVKRECLV primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvkreclv_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVKRECLV-24 unchanged before economic outcomes"
        ),
    }
