"""Evaluate frozen HVAVC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVAVC-8"
PROTOCOL = "hvavc_8_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_cumulative_vwap_recross_relay_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVAVC predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVAVC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVAVC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVAVC primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvavc_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVAVC-8 unchanged before economic outcomes"
        ),
    }
