"""Sequential strict economics for frozen HVCACM-8."""
...
PREREG = Path("results/high_volatility_cross_alt_channel_position_migration_relay_preregistration_2026-08-13.json")
PREREG_SHA = "8f2a1379e27605e0e2a496f55051edc1571dc567f5db5038f067d9a265be8a0d"
SUPPORT = Path("results/high_volatility_cross_alt_channel_position_migration_relay_support_2026-08-13.json")
SUPPORT_SHA = "89c238f67146409e180ce25770e87f1e661075cb3bf44f77e8d6c751ff6f18b0"
NOVELTY = Path("results/high_volatility_cross_alt_channel_position_migration_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "75c14f838c82d87b562c0f264c5c28bf6528543d91cc81f8624d2a451856ce98"
CLOCK = Path("data/high_volatility_cross_alt_channel_position_migration_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "b4070bf80c5548fcd64cca2be5410c5d14ff973c4dd49d12c267a12278ee4bfb"
CONTROL_DIR = Path("data/high_volatility_cross_alt_channel_position_migration_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_cross_alt_channel_position_migration_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_alt_channel_position_migration_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "no_migration_tail", "three_alt_consensus", "fixed_prior_channel", "one_decision_stale_migration", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCACM frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCACM novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcacm_8_sequential_economics_v1", "policy_id": POLICY_ID,
