"""Evaluate frozen HVCOM-355M structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCOM-355M"
PROTOCOL = "hvcom_355m_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_new_york_opening_momentum_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCOM predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCOM novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCOM predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCOM primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcom_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCOM-355M unchanged before economic outcomes",
    }
