"""Evaluate the frozen HVSPOICC-8 action-vote clock against Gross9."""
...
POLICY = "HVSPOICC-8"
PROTOCOL = "hvspoicc_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_spot_lead_open_interest_contraction_continuation_clocks_2020_2026.csv.gz",
    "sha256": "4212bc52538591e9a1baf51b2115c2396d4d47912774b1047d399786d89a049b",
    "rows": 305,
}
OUTPUT = Path("results/high_volatility_spot_lead_open_interest_contraction_continuation_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVSPOICC-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVSPOICC-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVSPOICC-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVSPOICC-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVSPOICC-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVSPOICC-8 unchanged before economics",
    }
