"""Sequential strict economics for frozen STCR-72."""
...
PREREG = Path("results/scheduled_trend_concordance_relay_preregistration_2026-08-09.json")
PREREG_SHA = "096d6f80f18a2b33475016c8f545f3514b6d1400af7ba0ae1f69039f9d3f8f7e"
SUPPORT = Path("results/scheduled_trend_concordance_relay_support_2026-08-09.json")
SUPPORT_SHA = "d81e7432dc4db84180034a5c8f7128970ddc5d573c68b4a1fd6036cddbce57d7"
NOVELTY = Path("results/scheduled_trend_concordance_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "0628928321a03356661e6812e7822c2079068982b0df3a38e3b939b1c6809979"
CLOCK = Path("data/scheduled_trend_concordance_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d0d2ac554428282fcd7ee564f77b9691726613fcfb7dcf8760b3f89a8b836915"
CONTROL_DIR = Path("data/scheduled_trend_concordance_relay_controls_2023_2026")
FREEZE = Path("results/scheduled_trend_concordance_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/scheduled_trend_concordance_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("STCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("STCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "stcr_72_sequential_economics_v1", "policy_id": POLICY_ID,
