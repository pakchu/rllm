"""Sequential strict economics for frozen DTFAR-12."""
...
PREREG = Path("results/daily_taker_flow_acceleration_relay_preregistration_2026-08-08.json")
PREREG_SHA = "f8d591ec688be9864ca7969c46d522963eabefc22db2c2062d775ddb163c1996"
SUPPORT = Path("results/daily_taker_flow_acceleration_relay_support_2026-08-08.json")
SUPPORT_SHA = "dd5bab5b2061d36d0535a0463fb7a593308b3a14c7a79df2fc2eed955e83b795"
NOVELTY = Path("results/daily_taker_flow_acceleration_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "4bafb1b1eb2402624444c7d9f199786d1088df9bab405ae51b6342586ee9903f"
CLOCK = Path("data/daily_taker_flow_acceleration_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "0d323a439d2acf9974b4c2d3a152b5fc252a2f367db1872b017eecb1d4511411"
CONTROL_DIR = Path("data/daily_taker_flow_acceleration_relay_controls_2023_2026")
FREEZE = Path("results/daily_taker_flow_acceleration_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/daily_taker_flow_acceleration_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "no_flow_tail", "flow_level", "one_day_stale_acceleration", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("DTFAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("DTFAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "dtfar_12_sequential_economics_v1", "policy_id": POLICY_ID,
