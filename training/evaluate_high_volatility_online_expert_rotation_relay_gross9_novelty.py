"""Evaluate frozen HVOER-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVOER-8"
PROTOCOL = "hvoer_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_online_expert_rotation_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVOER predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVOER novelty limits drift")
    if (
        ...
        raise RuntimeError("HVOER predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVOER primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvoer_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVOER-8 unchanged before economic outcomes",
    }
