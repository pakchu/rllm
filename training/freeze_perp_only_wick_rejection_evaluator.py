"""Freeze POWR-12 execution logic and every control clock before outcomes."""
...
def build_manifest(
    ...
    *,
    ...
    core: dict[str, Any] = {
        "protocol": "POWR-12 evaluator pre-outcome freeze v1",
        ...
        "mutable_parameters": [],
        "signal_feature_ohlc_loaded_during_freeze": True,
        "post_signal_returns_computed_during_freeze": False,
        "funding_loaded_during_freeze": False,
...
def validate_manifest(payload: dict[str, Any]) -> None:
    ...
    if canonical_hash(core) != payload.get("manifest_hash"):
        raise RuntimeError("POWR-12 evaluator freeze hash mismatch")
    if payload.get("outcomes_opened") is not False:
        raise RuntimeError("POWR-12 evaluator freeze cannot open outcomes")
    if payload.get("signal_feature_ohlc_loaded_during_freeze") is not True:
        raise RuntimeError("POWR-12 evaluator freeze source disclosure changed")
    if payload.get("post_signal_returns_computed_during_freeze") is not False:
        raise RuntimeError("POWR-12 evaluator freeze computed post-signal returns")
    if payload.get("funding_loaded_during_freeze") is not False:
        raise RuntimeError("POWR-12 evaluator freeze loaded funding")
    if payload.get("execution_simulation_run_during_freeze") is not False:
        raise RuntimeError("POWR-12 evaluator freeze ran execution simulation")
    if payload.get("mutable_parameters") != []:
        raise RuntimeError("POWR-12 evaluator freeze permits mutable parameters")
    expected = set(evaluate.POLICY_NAMES)
    if set(payload.get("policy_clock_sha256", {})) != expected:
        raise RuntimeError("POWR-12 evaluator freeze clock hash set changed")
    if set(payload.get("policy_clock_rows", {})) != expected:
        raise RuntimeError("POWR-12 evaluator freeze clock row set changed")
    if payload["policy_clock_sha256"]["primary"] != evaluate.EVENT_CLOCK_SHA256:
        raise RuntimeError("POWR-12 evaluator freeze primary clock hash changed")
...
def write_once(path: str | Path, payload: dict[str, Any]) -> str:
    ...
    if output.exists():
        ...
        if existing != payload:
            raise RuntimeError("refusing to overwrite POWR-12 evaluator freeze")
...
def current_clean_commit() -> str:
    ...
    if status:
        raise RuntimeError("POWR-12 evaluator source is not clean at HEAD")
    ...
    if committed_sha256 != _sha256(evaluate.EVALUATION_SOURCE):
        raise RuntimeError("POWR-12 evaluator source is not reproducible from HEAD")
    return commit
...
def main() -> None:
    ...
    if args.output != DEFAULT_OUTPUT:
        raise ValueError("POWR-12 evaluator freeze path is immutable")
