"""Materialize outcome-blind source support for frozen HVMELR-24."""
...
from training import preregister_high_volatility_mining_equity_leadership_relay as prereg
...
BUILDER = Path("training/build_high_volatility_mining_equity_leadership_relay_support.py")
PREREG_SHA256 = "964da37bf373d754b3603bf2db8f4b53015d545068b8372ea74e40602367effc"
SYMBOLS = ("RIOT", "MARA", "HUT")
SOURCE_DIR = Path("data/high_volatility_mining_equity_leadership_relay_sources_2022_2026")
...
FEATURE_PANEL = SOURCE_DIR / "hvmelr_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_mining_equity_leadership_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_mining_equity_leadership_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_mining_equity_leadership_relay_support_2026-08-12.json")
...
def normalize_yahoo_chart(payload: bytes, symbol: str) -> tuple[bytes, pd.DataFrame, dict[str, Any]]:
    ...
    if chart.get("error") is not None:
        raise RuntimeError(f"HVMELR Yahoo chart error for {symbol}")
    results = chart.get("result") or []
    if len(results) != 1:
        raise RuntimeError(f"HVMELR Yahoo result count drift for {symbol}")
    result = results[0]
    meta = result.get("meta") or {}
    if meta.get("symbol") != symbol or meta.get("exchangeTimezoneName") != "America/New_York":
        raise RuntimeError(f"HVMELR {symbol} Yahoo metadata drift")
    timestamps = result.get("timestamp") or []
    indicators = result.get("indicators") or {}
    quotes = indicators.get("quote") or []
    adjusted = indicators.get("adjclose") or []
    if len(quotes) != 1 or len(adjusted) != 1 or not timestamps:
        raise RuntimeError(f"HVMELR {symbol} adjusted chart incomplete")
    quote = quotes[0]
    ...
    if any(values is None or len(values) != len(timestamps) for values in vectors.values()):
        raise RuntimeError(f"HVMELR {symbol} Yahoo vector length drift")
    ...
    if frame.empty or frame.session_date.duplicated().any():
        raise RuntimeError(f"HVMELR {symbol} session dates invalid")
    ...
    if not valid.all():
        ...
        raise RuntimeError(f"HVMELR {symbol} invalid adjusted rows: {dates[:5]}")
    metadata = {
        "symbol": symbol,
...
def build_equity_panel(equities: Mapping[str, pd.DataFrame]) -> pd.DataFrame:
    if set(equities) != set(SYMBOLS):
        raise RuntimeError("HVMELR equity symbol set drift")
    ...
    for symbol in SYMBOLS:
        ...
        if not observed.equals(official):
            ...
            raise RuntimeError(
                f"HVMELR {symbol}/NYSE calendar mismatch missing={missing[:5]} extra={extra[:5]}"
            )
        if expected is None:
            expected = observed
        elif not observed.equals(expected):
            raise RuntimeError(f"HVMELR {symbol} does not share every official common session")
    ...
    if len(panel) != len(expected):
        raise RuntimeError("HVMELR common-session intersection dropped an official session")
    ...
    if observed_early != expected_early:
        raise RuntimeError("HVMELR frozen early-close schedule drift")
    return panel
...
def normalize_btc_bars(
    ...
    if raw.columns.tolist() != ["ts", "open", "close"]:
        raise RuntimeError("HVMELR BTC schema drift")
    frame = raw.copy()
    ...
    if len(frame) != len(expected) or not frame.ts.equals(pd.Series(expected, name="ts")):
        raise RuntimeError("HVMELR BTC source is not the exact requested 1m grid")
    for column in ("open", "close"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    values = frame[["open", "close"]]
    if not np.isfinite(values.to_numpy(dtype=float)).all() or not values.gt(0.0).all(axis=None):
        raise RuntimeError("HVMELR BTC prices invalid")
    return frame
...
def strict_prior_beta(
    btc_returns: pd.Series,
    miner_returns: pd.Series,
    lookback: int = 60,
    minimum: int = 40,
) -> pd.Series:
    """OLS slope using only valid pairs strictly before the current session."""
    output = pd.Series(np.nan, index=btc_returns.index, dtype=float)
    for index in range(len(output)):
        start = max(0, index - lookback)
        prior = pd.DataFrame(
            {"btc": btc_returns.iloc[start:index], "miner": miner_returns.iloc[start:index]}
        ).dropna()
        if len(prior) < minimum:
            continue
        x = prior.btc.to_numpy(dtype=float)
        y = prior.miner.to_numpy(dtype=float)
        variance = float(np.var(x, ddof=0))
        if np.isfinite(x).all() and np.isfinite(y).all() and variance > 0.0:
            output.iloc[index] = float(np.mean((x - x.mean()) * (y - y.mean())) / variance)
    return output
...
def build_features(sessions: pd.DataFrame, bars: pd.DataFrame) -> pd.DataFrame:
    ...
    if not isinstance(indexed.index, pd.DatetimeIndex) or indexed.index.tz is None:
        raise RuntimeError("HVMELR BTC index must be timezone-aware")
    ...
    for symbol in SYMBOLS:
        ...
        frame[f"{prefix}_beta"] = strict_prior_beta(
            frame.btc_return, frame[f"{prefix}_return"], lookback=60, minimum=40
        )
        frame[f"{prefix}_residual"] = (
            frame[f"{prefix}_return"] - frame[f"{prefix}_beta"] * frame.btc_return
        )
    residual_columns = [f"{symbol.lower()}_residual" for symbol in SYMBOLS]
    return_columns = [f"{symbol.lower()}_return" for symbol in SYMBOLS]
    frame["leadership_residual"] = frame[residual_columns].mean(axis=1, skipna=False)
    frame["raw_equal_mean_miner_return"] = frame[return_columns].mean(axis=1, skipna=False)
    frame["magnitude_rank"] = strict_prior_midrank(
        frame.leadership_residual.abs(), lookback=270, minimum=180
    )
    ...
    frame["raw_magnitude_rank"] = strict_prior_midrank(
        frame.raw_equal_mean_miner_return.abs(), lookback=270, minimum=180
    )
    frame["mara_magnitude_rank"] = strict_prior_midrank(
        frame.mara_residual.abs(), lookback=270, minimum=180
    )
    return frame
...
def _signal(frame: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    score = frame.leadership_residual.copy()
    rank = frame.magnitude_rank.copy()
    if control == "one_session_stale_residual":
        score, rank = score.shift(1), rank.shift(1)
    elif control == "raw_equal_mean_miner_return":
        score, rank = frame.raw_equal_mean_miner_return, frame.raw_magnitude_rank
    elif control == "mara_only_residual":
        score, rank = frame.mara_residual, frame.mara_magnitude_rank
    active = score.notna() & score.ne(0.0) & rank.ge(0.80)
        ...
        rows.append(
            {
                ...
                "riot_beta": float(frame.at[index, "riot_beta"]),
                "mara_beta": float(frame.at[index, "mara_beta"]),
                "hut_beta": float(frame.at[index, "hut_beta"]),
                "riot_residual": float(frame.at[index, "riot_residual"]),
                "mara_residual": float(frame.at[index, "mara_residual"]),
                "hut_residual": float(frame.at[index, "hut_residual"]),
                "leadership_residual": float(frame.at[index, "leadership_residual"]),
...
def run(env_file: str = ENV_FILE) -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA256:
        raise RuntimeError("HVMELR preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvmelr_24_sources_v1",
    ...
    core = {
        "protocol_version": "hvmelr_24_source_support_v1",
        "policy_id": "HVMELR-24",
        "preregistration": {
