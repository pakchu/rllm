"""Sequential strict economics for frozen HVRTCR-8."""
...
PREREG = Path("results/high_volatility_range_turnover_coupling_relay_preregistration_2026-08-10.json")
PREREG_SHA = "34641678970a01dd162439ab780e0ed1c32f951d671eef0212bbbcd4f2f57894"
SUPPORT = Path("results/high_volatility_range_turnover_coupling_relay_support_2026-08-10.json")
SUPPORT_SHA = "50d804e9be2095d341a4104dc5ef1dda57a0d72907cb320def111b949444ed07"
NOVELTY = Path("results/high_volatility_range_turnover_coupling_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "a03d4ee39f80af5769badc31321701547163aaabe743ed5e7b29574da822c4bd"
CLOCK = Path("data/high_volatility_range_turnover_coupling_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "29de599c631e03a84155b84ea50915704533b42549e7031989ef3bc35c65b69c"
CONTROL_DIR = Path("data/high_volatility_range_turnover_coupling_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_range_turnover_coupling_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_range_turnover_coupling_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_coupling_tail", "no_variation_gate", "range_only_direction", "one_boundary_stale_coupling", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVRTCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVRTCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrtcr_8_sequential_economics_v1", "policy_id": POLICY_ID,
