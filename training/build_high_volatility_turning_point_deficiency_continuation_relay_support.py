"""Source-only support gate for frozen HVTPDCR-8."""
...
from training import preregister_high_volatility_turning_point_deficiency_continuation_relay as prereg
...
PREREG_SHA = "57ffeebf061e7adf8e7517e1c9287f94ba8a3360a5e91fc13bb35f12c12d3ae1"
SPLITS = {
...
ROOT = Path("data/high_volatility_turning_point_deficiency_continuation_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_turning_point_deficiency_continuation_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_turning_point_deficiency_continuation_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_turning_point_deficiency_continuation_relay_support_2026-08-12.json")
...
def turning_point_count(close: np.ndarray) -> int | None:
    close = np.asarray(close, dtype=float)
    if len(close) != 96 or not np.isfinite(close).all() or np.any(close <= 0):
        return None
    middle = close[1:-1]
    highs = (middle > close[:-2]) & (middle > close[2:])
    lows = (middle < close[:-2]) & (middle < close[2:])
    return int(np.sum(highs | lows))


def aggregate_five_minutes(window: pd.DataFrame) -> pd.DataFrame:
    groups = np.arange(len(window)) // 5
    return window.groupby(groups, sort=True).agg(
        open=("open", "first"),
        high=("high", "max"),
        low=("low", "min"),
        close=("close", "last"),
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVTPDCR source timestamps")
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        if valid:
            bars = aggregate_five_minutes(window)
            returns = np.log(bars["close"].to_numpy(float) / bars["open"].to_numpy(float))
            variation = float(np.sqrt(np.square(returns).sum()))
            count = turning_point_count(bars["close"].to_numpy(float))
            share = math.nan if count is None else count / 94.0
            block_return = float(np.log(bars["close"].iloc[-1] / bars["open"].iloc[0]))
            late_return = float(np.log(bars["close"].iloc[-1] / bars["open"].iloc[-24]))
            valid = variation > 0 and count is not None and math.isfinite(share)
        if not valid:
            variation = count = share = block_return = late_return = math.nan
        rows.append(
            {
                "decision_time": decision,
                "source_valid": valid,
                "realized_variation": variation,
                "turning_point_count": count,
                "turning_point_share": share,
    ...
    states["turning_point_rank"] = prior_rank(
        states["turning_point_share"].where(states["source_valid"])
    )
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvtpdcr_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    turning_rank = states["turning_point_rank"]
    turning_share = states["turning_point_share"]
    if control == "one_block_stale_geometry":
        block_return, late_return, variation_rank, turning_rank, turning_share = [
            value.shift(1)
            for value in (block_return, late_return, variation_rank, turning_rank, turning_share)
        ]
    variation_gate = (
    ...
    if control == "no_turning_point_deficiency_tail":
        turning_gate = pd.Series(True, index=states.index)
    elif control == "turning_point_share_below_two_thirds":
        turning_gate = turning_share.lt(2 / 3)
    else:
        turning_gate = turning_rank.le(0.20)
    agreement = (
        block_return.ne(0)
        & late_return.ne(0)
        & np.sign(block_return).eq(np.sign(late_return))
    )
    eligible = states["source_valid"] & variation_gate & turning_gate & agreement
        ...
        rows.append(
            {
                ...
                "turning_point_count": int(states.at[index, "turning_point_count"]),
                "turning_point_share": float(states.at[index, "turning_point_share"]),
                "turning_point_rank": float(states.at[index, "turning_point_rank"]),
            }
        )
    columns = [
        "candidate", "control", "split", "decision_time", "feature_available_time",
        "entry_time", "exit_time", "side", "realized_variation", "variation_rank",
        "turning_point_count", "turning_point_share", "turning_point_rank",
    ]
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVTPDCR preregistration drift")
    ...
    core = {
        "protocol_version": "hvtpdcr_8_source_support_v1",
        "policy_id": "HVTPDCR-8",
        "preregistration": {
