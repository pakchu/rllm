"""Build outcome-blind source support for frozen HVTRIX-24."""
...
from training import preregister_high_volatility_trix_zero_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "4d49a6c4342494350fef39eaf8e160b629a257d636295ba37588e891afda3e60"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_trix_zero_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_trix_zero_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_trix_zero_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_trix_zero_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_trix_zero_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "ema1", "ema2", "ema3", "trix", "entry_side",
    "single_ema_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "ema1", "ema2", "ema3", "trix",
    "entry_side", "single_ema_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVTRIX source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVTRIX invalid source key")
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["trix"]=100.0*(bars.ema3/bars.ema3.shift(1)-1.0);bars["entry_side"]=cross_side(bars.trix)
    single_rate=100.0*(bars.ema1/bars.ema1.shift(1)-1.0);bars["single_ema_side"]=cross_side(single_rate)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["ema1","ema2","ema3","trix","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="single_ema_rate_cross":side=pd.to_numeric(used.single_ema_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVTRIX prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvtrix_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvtrix_24_source_support_v1", "policy_id": prereg.POLICY_ID,
