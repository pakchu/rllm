"""Evaluate the frozen HVSAUD-8 action-vote clock against Gross9."""
...
POLICY = "HVSAUD-8"
PROTOCOL = "hvsaud_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_spot_adverse_underwater_duration_relay_gross9_novelty_2026-08-18.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVSAUD-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVSAUD-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVSAUD-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVSAUD-8 source eligibility drift")
    sealed = (
        "postentry_return_pnl_execution_price_opened",
        "gross9_rows_opened",
    )
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVSAUD-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVSAUD-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVSAUD-8 unchanged before economics",
    }
