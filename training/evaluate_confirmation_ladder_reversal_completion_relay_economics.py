"""Sequential strict economics for frozen CLRCR-6."""
...
PREREG = Path("results/confirmation_ladder_reversal_completion_relay_preregistration_2026-08-09.json")
PREREG_SHA = "cd816d1b20cd0f87bb69bdf9181dbd61ba5bb9e577abfff95b79db7726d646aa"
SUPPORT = Path("results/confirmation_ladder_reversal_completion_relay_support_2026-08-09.json")
SUPPORT_SHA = "2804c20b89f72141d12267106c8ef4285122e898a696c4c0f0cd7e6996135874"
NOVELTY = Path("results/confirmation_ladder_reversal_completion_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "2496ec32c47da871e2ce4c6fb2829ec9f0eb087c1a8ed23553c0b7d6a808c0b0"
CLOCK = Path("data/confirmation_ladder_reversal_completion_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "c25ca83aa0cd3d8ce4392824856f3c7064d6477dcb08e1886e0b25a93ff89138"
CONTROL_DIR = Path("data/confirmation_ladder_reversal_completion_relay_controls_2023_2026")
FREEZE = Path("results/confirmation_ladder_reversal_completion_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/confirmation_ladder_reversal_completion_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CLRCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CLRCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "clrcr_6_sequential_economics_v1", "policy_id": POLICY_ID,
