"""Evaluate the frozen HVCVARCLIR-8 action-vote clock against Gross9."""
...
POLICY = "HVCVARCLIR-8"
PROTOCOL = "hvcvarclir_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_variance_acceleration_close_location_inventory_relay_clocks_2020_2026.csv.gz",
    "sha256": "51a194abe3d415422ffc19c596613175244f278352aeff76c20c794fc00f13dc",
    "rows": 113,
}
OUTPUT = Path("results/high_volatility_causal_variance_acceleration_close_location_inventory_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCVARCLIR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCVARCLIR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVARCLIR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCVARCLIR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCVARCLIR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCVARCLIR-8 unchanged before economics",
    }
