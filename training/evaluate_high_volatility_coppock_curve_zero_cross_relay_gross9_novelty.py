"""Evaluate frozen HVCC-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCC-24"
PROTOCOL = "hvcc_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_coppock_curve_zero_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcc_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCC-24 unchanged before economic outcomes",
    }
