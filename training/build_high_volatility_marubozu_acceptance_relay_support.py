"""Build outcome-blind source support for frozen HVMARU-12."""
...
from training import preregister_high_volatility_marubozu_acceptance_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "f8d93a37ed3a5f56808ba35a7a100f9f884235b3da935c241b83d4db36581299"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_marubozu_acceptance_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_marubozu_acceptance_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_marubozu_acceptance_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_marubozu_acceptance_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_marubozu_acceptance_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "bar_range", "body", "upper_wick", "lower_wick",
    "body_share", "upper_wick_share", "lower_wick_share", "pattern", "body_side", "wick_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "bar_range", "body", "upper_wick", "lower_wick",
    "body_share", "upper_wick_share", "lower_wick_share", "pattern", "body_side", "wick_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVMARU source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVMARU invalid source key")
...
def marubozu(open_: pd.Series, high: pd.Series, low: pd.Series, close: pd.Series) -> pd.DataFrame:
    span=high-low;body=(close-open_).abs();upper=high-pd.concat([open_,close],axis=1).max(axis=1);lower=pd.concat([open_,close],axis=1).min(axis=1)-low
    body_share=(body/span).where(span.gt(0));upper_share=(upper/span).where(span.gt(0));lower_share=(lower/span).where(span.gt(0))
    pattern=body_share.ge(P["body_share_min"])&upper_share.le(P["upper_wick_share_max"])&lower_share.le(P["lower_wick_share_max"])&body.gt(0)
    body_side=pd.Series(np.sign(close-open_),index=close.index).fillna(0).astype(int);wick_side=pd.Series(np.sign(lower-upper),index=close.index).fillna(0).astype(int)
    return pd.DataFrame({"bar_range":span,"body":body,"upper_wick":upper,"lower_wick":lower,"body_share":body_share,"upper_wick_share":upper_share,"lower_wick_share":lower_share,"pattern":pattern,"body_side":body_side,"wick_side":wick_side},index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=pd.concat([bars,marubozu(bars.bar_open,bars.bar_high,bars.bar_low,bars.bar_close)],axis=1)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum())
    numeric=["bar_range","body","upper_wick","lower_wick","body_share","upper_wick_share","lower_wick_share","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.bar_range.gt(0)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));panel["eligible"]=panel.source_valid&panel.pattern&panel.body_side.ne(0)&panel.variation_rank.ge(P["variation_rank_min"])
    return panel.loc[:,PANEL_COLUMNS]


def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True);variation=used.variation_rank.ge(P["variation_rank_min"]);side=pd.to_numeric(used.body_side,errors="coerce").fillna(0).astype(int);state=valid&used.pattern&side.ne(0)&variation
    if control=="no_variation_gate":state=valid&used.pattern&side.ne(0)
    elif control=="wick_pressure_side":side=pd.to_numeric(used.wick_side,errors="coerce").fillna(0).astype(int)
    elif control=="one_bar_stale_pattern":state=state.shift(1,fill_value=False);side=side.shift(1,fill_value=0)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVMARU prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvmaru_12_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvmaru_12_source_support_v1", "policy_id": prereg.POLICY_ID,
