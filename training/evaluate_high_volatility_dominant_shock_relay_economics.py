"""Sequential strict economics for frozen HVDSR-12."""
...
PREREG = Path("results/high_volatility_dominant_shock_relay_preregistration_2026-08-09.json")
PREREG_SHA = "3f821d75a17044be86c1f4430e324bc209a19e9204e6553dd27e036ef0bd8466"
SUPPORT = Path("results/high_volatility_dominant_shock_relay_support_2026-08-09.json")
SUPPORT_SHA = "874bcab0ed6eccc3bdd5200dae72dfe50a4c997b16d73c8de691111ab70fa88c"
NOVELTY = Path("results/high_volatility_dominant_shock_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "574035e5d40bc52b4c6a059fb970607b11a519b2b4426ef8526f026a3c7dcfdc"
CLOCK = Path("data/high_volatility_dominant_shock_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "aa14926d2cefc5b89ebbcb4cccc61c379d1dd13f177ce44ccaaf5c6d56486b51"
CONTROL_DIR = Path("data/high_volatility_dominant_shock_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_dominant_shock_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_dominant_shock_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_dominant_share_gate", "no_variation_gate", "completed_day_return_side", "latest_maximum_tie_break", "one_day_stale_features", "direction_flip", "same_clock_forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDSR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDSR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdsr_12_sequential_economics_v1", "policy_id": POLICY_ID,
