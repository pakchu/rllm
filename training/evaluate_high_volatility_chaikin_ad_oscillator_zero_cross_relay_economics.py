"""Sequential strict economics for frozen HVCHO-24."""
...
PREREG = Path("results/high_volatility_chaikin_ad_oscillator_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "af48092e85ebab11b93a9da5aa386380161a06c4797de75fbe3f0c49275a2217"
SUPPORT = Path("results/high_volatility_chaikin_ad_oscillator_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "fa17c8bf880e2cdd681e7d2b90f490994bb405148f1c2e43485214b20123bb40"
NOVELTY = Path("results/high_volatility_chaikin_ad_oscillator_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "532d2cbdb011702873d03b4fcf43b91ed7ececba64f964d065693ec1ee070e3b"
CLOCK = Path("data/high_volatility_chaikin_ad_oscillator_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "ba23e182cbca183a12e09c74ccbe0094636d8fc177eec3aa8179e711990eaa17"
CONTROL_DIR = Path("data/high_volatility_chaikin_ad_oscillator_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_chaikin_ad_oscillator_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_chaikin_ad_oscillator_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "money_flow_volume_zero_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCHO frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCHO novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcho_24_sequential_economics_v1", "policy_id": POLICY_ID,
