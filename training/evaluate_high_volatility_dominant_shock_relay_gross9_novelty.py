"""Evaluate frozen HVDSR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDSR-12"
PROTOCOL = "hvdsr_12_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_dominant_shock_relay_preregistration_2026-08-09.json"
)
...
OUTPUT = Path(
    "results/high_volatility_dominant_shock_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDSR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVDSR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDSR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDSR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdsr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVDSR-12 unchanged before economic outcomes"
        ),
    }
