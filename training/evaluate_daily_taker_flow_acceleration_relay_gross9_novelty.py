"""Evaluate frozen DTFAR-12 structural novelty against every Gross9 sleeve."""
...
POLICY="DTFAR-12";PROTOCOL="dtfar_12_gross9_novelty_v1";PREREG=Path("results/daily_taker_flow_acceleration_relay_preregistration_2026-08-08.json");PREREG_SHA="f8d591ec688be9864ca7969c46d522963eabefc22db2c2062d775ddb163c1996";SUPPORT=Path("results/daily_taker_flow_acceleration_relay_support_2026-08-08.json");SUPPORT_SHA="dd5bab5b2061d36d0535a0463fb7a593308b3a14c7a79df2fc2eed955e83b795";CLOCK=Path("data/daily_taker_flow_acceleration_relay_clocks_2023_2026.csv.gz");CLOCK_SHA="0d323a439d2acf9974b4c2d3a152b5fc252a2f367db1872b017eecb1d4511411";OUTPUT=Path("results/daily_taker_flow_acceleration_relay_gross9_novelty_2026-08-08.json");LIMITS={"exact_entry_jaccard":.10,"one_to_one_6h_max_matched_share":.35,"occupied_5m_bar_jaccard":.25,"absolute_signed_exposure_pearson":.35}
...
def run(output:Path=OUTPUT)->dict:
 ...
 candidate=metric.load_clock(CLOCK,label="DTFAR primary");results={}
 ...
 passed=all(x["passed"] for x in results.values());advance=support["support_passed"] and passed;core={"protocol_version":PROTOCOL,"policy_id":POLICY,"preregistration":{"path":str(PREREG),"sha256":PREREG_SHA,"manifest_hash":reg["manifest_hash"]},"source_support":{"path":str(SUPPORT),"sha256":SUPPORT_SHA,"manifest_hash":support["manifest_hash"],"predecessor_mutated":False},"gross9_structural_clocks":{"path":str(gross9.DEFAULT_MANIFEST),"sha256":sha(gross9.DEFAULT_MANIFEST),"manifest_hash":manifest["manifest_hash"],"authority_sha256":gross9.ANCHOR_SHA256,"complete_roster":list(gross9.EXPECTED_WEIGHTS)},"evidence_boundary":{"dtfar_clock_rows_opened":len(candidate),"gross9_structural_clock_rows_opened":sum(x["rows"] for x in manifest["clocks"].values()),"btc_execution_rows_opened":0,"btc_price_or_return_rows_opened":0,"funding_rows_opened":0,"economic_outcome_rows_opened":0,"portfolio_return_or_pnl_metrics_computed":False,"outcomes_opened":False},"limits":LIMITS,"gross9_sleeves":results,"source_support_passed":support["support_passed"],"every_gross9_sleeve_passed":passed,"gross9_novelty_status":"passed" if passed else "failed","advance_to_economic_outcomes":advance,"failure_action":None if advance else "reject DTFAR-12 unchanged before economic outcomes"};r={**core,"manifest_hash":chash(core)};output.write_text(json.dumps(r,indent=2,ensure_ascii=False,allow_nan=False)+"\n");return r
