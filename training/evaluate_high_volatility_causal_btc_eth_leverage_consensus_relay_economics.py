"""Strict staged economics for the singleton HVCBER-8 policy."""
...
PREREG = Path("results/high_volatility_causal_btc_eth_leverage_consensus_relay_preregistration_2026-08-16.json")
PREREG_SHA = "33f569507743a8ccb3f67a07ccd2d119df3ca4f23360604f11f2278cb3b46a5d"
SUPPORT = Path("results/high_volatility_causal_btc_eth_leverage_consensus_relay_support_2026-08-16.json")
SUPPORT_SHA = "87a3a0169e447987dcd8a6815ae67aaaf161d2d844fcc5b827748d76543ac8c7"
GROSS9 = Path("results/high_volatility_causal_btc_eth_leverage_consensus_relay_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "e32a7971263d4d2465deacb800fb27ef93d8bc834dcd429188017a32f841a085"
CLOCK = {
    "path": Path("data/high_volatility_causal_btc_eth_leverage_consensus_relay_clocks_2020_2026.csv.gz"),
    "sha256": "00aa9c7d7eaf2389b26d55c9c42caf5c99c6d747603b293d2c9b61b82ab1ff54",
    "rows": 341,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_btc_eth_leverage_consensus_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCBER-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCBER-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCBER-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCBER-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCBER-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCBER-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCBER-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCBER-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCBER-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCBER-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCBER-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcber_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
