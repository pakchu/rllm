"""Evaluate frozen HVPST-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVPST-12"
PROTOCOL = "hvpst_12_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_pastor_stambaugh_temporary_impact_reversal_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVPST predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVPST novelty limits drift")
    if (
        ...
        raise RuntimeError("HVPST predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVPST primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvpst_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVPST-12 unchanged before economic outcomes",
    }
