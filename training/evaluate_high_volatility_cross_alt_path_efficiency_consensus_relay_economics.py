"""Sequential strict economics for frozen HVCAPEC-8."""
...
PREREG = Path("results/high_volatility_cross_alt_path_efficiency_consensus_relay_preregistration_2026-08-13.json")
PREREG_SHA = "9bb7f6a4df8276586ac40b4a6fdc9fe1de7a9a09293490fcaf64d7972d759e23"
SUPPORT = Path("results/high_volatility_cross_alt_path_efficiency_consensus_relay_support_2026-08-13.json")
SUPPORT_SHA = "2d3f9cfb7e93a10757deb27e1c4683bffcc2d7e46ac10c33ef1d43b006bdda38"
NOVELTY = Path("results/high_volatility_cross_alt_path_efficiency_consensus_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "a6b7520af009af370a31eacb1f10e96b38306421f658f47a53dfd5c21f8061de"
CLOCK = Path("data/high_volatility_cross_alt_path_efficiency_consensus_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "02823bbf78a9a8c77954429b720f100cbfc7214158d36345e15667885346f013"
CONTROL_DIR = Path("data/high_volatility_cross_alt_path_efficiency_consensus_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_cross_alt_path_efficiency_consensus_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_alt_path_efficiency_consensus_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_efficiency_tail", "no_variation_gate", "endpoint_return_magnitude_tail", "one_block_stale_efficiency", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCAPEC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCAPEC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcapec_8_sequential_economics_v1", "policy_id": POLICY_ID,
