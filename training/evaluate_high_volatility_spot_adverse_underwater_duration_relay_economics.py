"""Strict staged economics for the singleton HVSAUD-8 policy."""
...
PREREG = Path("results/high_volatility_spot_adverse_underwater_duration_relay_preregistration_2026-08-18.json")
PREREG_SHA = "6f65fe4632da0c714cf971a95d1be98fb28bb8ef2a4926188656ff58af676446"
SUPPORT = Path("results/high_volatility_spot_adverse_underwater_duration_relay_support_2026-08-18.json")
SUPPORT_SHA = "1fc578d0c95d12c4f9cea7da4ee0f712fc0546573c2e334011eb19e2ee55b27a"
GROSS9 = Path("results/high_volatility_spot_adverse_underwater_duration_relay_gross9_novelty_2026-08-18.json")
GROSS9_SHA = "77a222777e6baf434260657baf9b1768745b42f0ac02bcc6b9ae321b028edc6c"
CLOCK = {
    "path": Path("data/high_volatility_spot_adverse_underwater_duration_relay_clocks_2023_2026.csv.gz"),
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_spot_adverse_underwater_duration_relay_{stage}_economics_2026-08-18.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVSAUD-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVSAUD-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVSAUD-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVSAUD-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVSAUD-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVSAUD-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVSAUD-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVSAUD-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVSAUD-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVSAUD-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVSAUD-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvsaud_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
