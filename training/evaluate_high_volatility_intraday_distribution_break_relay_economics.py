"""Sequential strict economics for frozen HVIDBR-8."""
...
PREREG = Path("results/high_volatility_intraday_distribution_break_relay_preregistration_2026-08-10.json")
...
NOVELTY = Path("results/high_volatility_intraday_distribution_break_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "1093958280a6c8dd19ecce457aa7d4027cc9cb881d263a227a024c205072c741"
...
FREEZE = Path("results/high_volatility_intraday_distribution_break_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_intraday_distribution_break_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_distribution_break_gate", "no_variation_gate", "raw_ks_above_quarter", "one_block_stale_distribution", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVIDBR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVIDBR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvidbr_8_sequential_economics_v1", "policy_id": POLICY_ID,
