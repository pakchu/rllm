"""Evaluate frozen HVCFO-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCFO-24"
PROTOCOL = "hvcfo_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_chande_forecast_oscillator_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCFO predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCFO novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCFO predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCFO primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcfo_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCFO-24 unchanged before economic outcomes",
    }
