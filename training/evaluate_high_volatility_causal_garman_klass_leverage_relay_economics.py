"""Strict staged economics for the singleton HVCGKR-24 policy."""
...
PREREG = Path("results/high_volatility_causal_garman_klass_leverage_relay_preregistration_2026-08-16.json")
PREREG_SHA = "ba7c9422df04694c2c1004cba21cd5b0786d2cd79e56ab955db87b57eb5a3053"
SUPPORT = Path("results/high_volatility_causal_garman_klass_leverage_relay_support_2026-08-16.json")
SUPPORT_SHA = "89e087149c83596ed5588c2f200e5f46bf6208bc7b500552eb87729a624ac940"
GROSS9 = Path("results/high_volatility_causal_garman_klass_leverage_relay_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "29fe1b684b543bd8d087aee7c0d9f5edbc35aaf7d367b49cca04f387077a69f4"
CLOCK = {
    "path": Path("data/high_volatility_causal_garman_klass_leverage_relay_clocks_2020_2026.csv.gz"),
    "sha256": "9f49ead8c652eae9ab1164c93296a2ddf585a934c55d01a3242db22130655802",
    "rows": 104,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_garman_klass_leverage_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCGKR-24 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCGKR-24 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCGKR-24 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCGKR-24 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCGKR-24 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCGKR-24 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCGKR-24 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCGKR-24 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCGKR-24 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCGKR-24 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCGKR-24 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcgkr_24_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
