"""Sequential strict economics for frozen HVCRSI-24."""
...
PREREG = Path("results/high_volatility_connors_rsi_extreme_reversal_preregistration_2026-08-11.json")
PREREG_SHA = "c5d15adea99167743c988d54902dec4307069d4d04846f077759258b03fd8fc6"
SUPPORT = Path("results/high_volatility_connors_rsi_extreme_reversal_support_2026-08-11.json")
SUPPORT_SHA = "51f8d834902ca421f9e206b7981ed94f43a3ef43c2fa4db14d199ea5ddf82ca0"
NOVELTY = Path("results/high_volatility_connors_rsi_extreme_reversal_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "414157046d6c4f74ada293d64eefb35f72f9cfd84c3bb224d87c38b3f8068d58"
CLOCK = Path("data/high_volatility_connors_rsi_extreme_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "2f5bf0020a6463322304612cf71c38725d779215e7a47675e8bd3da9e2cebbc9"
CONTROL_DIR = Path("data/high_volatility_connors_rsi_extreme_reversal_controls_2023_2026")
FREEZE = Path("results/high_volatility_connors_rsi_extreme_reversal_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_connors_rsi_extreme_reversal_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "price_rsi_only_extreme", "one_bar_stale_entry", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCRSI frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCRSI novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcrsi_24_sequential_economics_v1", "policy_id": POLICY_ID,
