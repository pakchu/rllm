"""Evaluate frozen HVCASPCPSC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCASPCPSC-8"
PROTOCOL = "hvcaspc_psc_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_cross_alt_serial_persistence_premium_sponsorship_relay_gross9_novelty_2026-08-16.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCASPCPSC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCASPCPSC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCASPCPSC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCASPCPSC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCASPCPSC-8 unchanged before economic outcomes",
    }
