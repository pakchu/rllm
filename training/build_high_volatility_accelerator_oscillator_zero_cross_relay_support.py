"""Build outcome-blind source support for frozen HVAC-24."""
...
from training import preregister_high_volatility_accelerator_oscillator_zero_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "e76d2c06c148cc8bef7af2a2e01046595c9f641bea0969036fc413b9a282224b"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_accelerator_oscillator_zero_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_accelerator_oscillator_zero_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_accelerator_oscillator_zero_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_accelerator_oscillator_zero_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_accelerator_oscillator_zero_cross_relay_support_2026-08-11.json")
...
PANEL_COLUMNS = (
    ...
    "ao_signal", "ac", "entry_side", "ao_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "median_price", "fast_sma", "slow_sma",
    "ao", "ao_signal", "ac", "entry_side", "ao_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVAC source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVAC invalid source key")
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["median_price"]=(bars.bar_high+bars.bar_low)/2;bars["fast_sma"]=causal_sma(bars.median_price,bars.valid_bar,P["fast_periods"]);bars["slow_sma"]=causal_sma(bars.median_price,bars.valid_bar,P["slow_periods"]);bars["ao"]=bars.fast_sma-bars.slow_sma;bars["ao_signal"]=causal_sma(bars.ao,np.isfinite(bars.ao),P["acceleration_periods"]);bars["ac"]=bars.ao-bars.ao_signal;bars["entry_side"]=cross_side(bars.ac);bars["ao_side"]=cross_side(bars.ao)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["median_price","fast_sma","slow_sma","ao","ao_signal","ac","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="awesome_oscillator_zero_cross":side=pd.to_numeric(used.ao_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVAC prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvac_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvac_24_source_support_v1", "policy_id": prereg.POLICY_ID,
