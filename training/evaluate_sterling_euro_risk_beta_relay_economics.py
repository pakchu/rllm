"""Sequential strict economics for frozen SERBR-12."""
...
PREREG = Path("results/sterling_euro_risk_beta_relay_preregistration_2026-08-09.json")
PREREG_SHA = "2ecf95b99acc2e62b96ea717e373eabaa48781e1017803f19e3ba2a2da849550"
SUPPORT = Path("results/sterling_euro_risk_beta_relay_support_2026-08-09.json")
SUPPORT_SHA = "23816135a263e6cb4568bc357eb67d6725e9c63963bbea62d85033f71690daef"
NOVELTY = Path("results/sterling_euro_risk_beta_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "9afdb488e01eb4501db7691a7a86791cb2f2223000e0b7fda0af827f0541ac3e"
CLOCK = Path("data/sterling_euro_risk_beta_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "028b8cb784df5bddc11c9bbc09800656d2357f8471e8d705973460fab68b8e5d"
CONTROL_DIR = Path("data/sterling_euro_risk_beta_relay_controls_2023_2026")
FREEZE = Path("results/sterling_euro_risk_beta_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/sterling_euro_risk_beta_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "common_dollar_basket", "one_session_stale_relative_return", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("SERBR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("SERBR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "serbr_12_sequential_economics_v1", "policy_id": POLICY_ID,
