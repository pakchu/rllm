"""Evaluate frozen HVCASCE-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCASCE-8"
PROTOCOL = "hvcasce_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_cross_alt_sign_configuration_entropy_relay_preregistration_2026-08-13.json")
PREREG_SHA = "421932e169aebd1990eb313abc754c92225e92f1fc274058c51fdeac0b14d611"
SUPPORT = Path("results/high_volatility_cross_alt_sign_configuration_entropy_relay_support_2026-08-13.json")
SUPPORT_SHA = "646ea98e801291a2900a3f632309e31fdeda91b54403a48635c56a62d5125be9"
CLOCK = Path("data/high_volatility_cross_alt_sign_configuration_entropy_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "476d98250f233f2293e7a21aa2381313843a5583fec355c75649324e00728794"
OUTPUT = Path("results/high_volatility_cross_alt_sign_configuration_entropy_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCASCE predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCASCE novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCASCE predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCASCE primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCASCE-8 unchanged before economic outcomes",
    }
