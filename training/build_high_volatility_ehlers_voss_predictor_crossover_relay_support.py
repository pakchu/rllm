"""Build outcome-blind source support for frozen HVEVP-24."""
...
from training import preregister_high_volatility_ehlers_voss_predictor_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "36492921e1d7a6c4b60749237c178a8272e0b5fa523ed8d6c0298f62d271d4f7"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ehlers_voss_predictor_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ehlers_voss_predictor_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ehlers_voss_predictor_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ehlers_voss_predictor_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ehlers_voss_predictor_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "run_length", "bandpass", "voss", "spread",
    "entry_side", "voss_zero_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "run_length", "bandpass", "voss", "spread",
    "entry_side", "voss_zero_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVEVP source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVEVP invalid source key")
...
def voss_predictor(series: pd.Series, valid: pd.Series) -> pd.DataFrame:
    period=float(P["period"]);bandwidth=float(P["bandwidth"]);order=int(P["order"])
    l1=math.cos(2*math.pi/period);g1=math.cos(bandwidth*2*math.pi/period);s1=1/g1-math.sqrt(1/(g1*g1)-1)
    values=series.to_numpy(float);ok=valid.to_numpy(bool);bandpass=np.full(len(series),np.nan);voss=np.full(len(series),np.nan);runs=np.zeros(len(series),dtype=int);run=0
    for i,(price,is_valid) in enumerate(zip(values,ok)):
        if not is_valid or not math.isfinite(price):run=0;continue
        run+=1;runs[i]=run
        bandpass[i]=0. if run<=5 else .5*(1-s1)*(price-values[i-2])+l1*(1+s1)*bandpass[i-1]-s1*bandpass[i-2]
        if run<=order:voss[i]=0.;continue
        weighted=sum(((count+1)/order)*voss[i-(order-count)] for count in range(order))
        voss[i]=((3+order)/2)*bandpass[i]-weighted
    return pd.DataFrame({"run_length":runs,"bandpass":bandpass,"voss":voss,"spread":voss-bandpass},index=series.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    fields=["bar_open","bar_high","bar_low","bar_close"];bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[fields]).all(axis=1)&bars[fields].gt(0).all(axis=1);values=voss_predictor(bars.bar_close,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["entry_side"]=cross_side(bars.spread);bars["voss_zero_side"]=cross_side(bars.voss);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["bandpass","voss","spread","variation"];bars["source_valid"]=bars.valid_bar&bars.run_length.ge(P["warmup_observations"])&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="voss_zero_cross":side=pd.to_numeric(used.voss_zero_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVEVP prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvevp_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvevp_24_source_support_v1", "policy_id": prereg.POLICY_ID,
