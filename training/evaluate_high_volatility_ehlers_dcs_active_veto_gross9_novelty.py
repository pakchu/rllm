"""Evaluate the frozen HVELDCV-24 action-vote clock against Gross9."""
...
POLICY = "HVELDCV-24"
PROTOCOL = "hveldvc_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_ehlers_dcs_active_veto_clocks_2023_2026.csv.gz",
    "sha256": "762fb551b28f92796ce53ae396e1616316479ed7bdee32eb651750fa70c5215b",
    "rows": 170,
}
OUTPUT = Path("results/high_volatility_ehlers_dcs_active_veto_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVELDCV-24 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVELDCV-24 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVELDCV-24 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVELDCV-24 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVELDCV-24 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVELDCV-24 unchanged before economics",
    }
