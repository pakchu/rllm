"""Build source-only CLWMSR-6 confirmation-ladder clocks."""
...
PREREG_SHA = "76cf7b78547b63a5943b672ea8e308189424df0ba2f8a24a17edb05eca60d964"
...
CONTROLS = ("late_unanimity_only", "witness_migration_only", "serialized_size_migration", "one_anchor_stale_ladder", "direction_flip")
...
SOURCE_DIR = Path("data/confirmation_ladder_witness_migration_sponsorship_relay_sources_2023_2026")
FEATURES = SOURCE_DIR / "confirmation_ladders.csv.gz"
MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/confirmation_ladder_witness_migration_sponsorship_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/confirmation_ladder_witness_migration_sponsorship_relay_controls_2023_2026")
RESULT = Path("results/confirmation_ladder_witness_migration_sponsorship_relay_support_2026-08-13.json")
RETURN_COLUMNS = tuple(f"interval_return_{number}" for number in range(1, 7))
WEIGHT_COLUMNS = tuple(f"block_weight_{number}" for number in range(1, 7))
SIZE_COLUMNS = tuple(f"block_size_{number}" for number in range(1, 7))
FEATURE_COLUMNS = (
    "anchor_height", "confirmation_height", "feature_available_time", "source_valid",
    *RETURN_COLUMNS, *SIZE_COLUMNS, *WEIGHT_COLUMNS, "late_return", "late_unanimous",
    "early_witness_share", "late_witness_share", "witness_migration",
    "serialized_size_migration", "eligible_state",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "anchor_height", "confirmation_height",
    "feature_available_time", "entry_time", "exit_time", "side", "late_return",
    "late_unanimous", "early_witness_share", "late_witness_share",
    "witness_migration", "serialized_size_migration",
)
...
def validate_blocks(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CLWMSR block schema drift")
    blocks = frame.sort_values("height", kind="mergesort").reset_index(drop=True)
    for column in ("height", "timestamp", "mediantime", "size", "weight"):
        blocks[column] = pd.to_numeric(blocks[column], errors="raise").astype("int64")
    if not blocks[["size", "weight"]].gt(0).all().all():
        raise RuntimeError("CLWMSR block size and weight must be positive")
    if not (blocks.weight.ge(blocks["size"]) & blocks.weight.le(4 * blocks["size"]) & blocks.weight.le(4_000_000)).all():
        raise RuntimeError("CLWMSR BIP141 size/weight invariant failed")
    if not np.array_equal(np.diff(blocks.height), np.ones(len(blocks) - 1, dtype=np.int64)):
        raise RuntimeError("CLWMSR block heights are not contiguous")
    if not blocks.previousblockhash.iloc[1:].reset_index(drop=True).equals(blocks.id.iloc[:-1].reset_index(drop=True)):
        raise RuntimeError("CLWMSR canonical previous-hash continuity failed")
    return blocks
...
def prepare_minutes(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CLWMSR minute schema drift")
    ...
    if result.ts.duplicated().any():
        raise RuntimeError("CLWMSR duplicate timestamps")
...
def build_features(blocks: pd.DataFrame, market: pd.DataFrame) -> pd.DataFrame:
    ...
    for index in range(0, len(blocks) - 6):
        ...
        sizes = [int(blocks["size"].iloc[index + offset]) for offset in range(1, 7)]
        weights = [int(blocks.weight.iloc[index + offset]) for offset in range(1, 7)]
        witness = [(4.0 * size - weight) / (3.0 * size) for size, weight in zip(sizes, weights)]
        early_witness = float(np.mean(witness[:3])); late_witness = float(np.mean(witness[3:]))
        witness_migration = bool(valid and late_witness > early_witness)
        early_log_size = float(np.mean(np.log(sizes[:3]))); late_log_size = float(np.mean(np.log(sizes[3:])))
        size_migration = bool(valid and late_log_size > early_log_size)
        rows.append({
            ...
            **{column: sizes[position] for position, column in enumerate(SIZE_COLUMNS)},
            **{column: weights[position] for position, column in enumerate(WEIGHT_COLUMNS)},
            "late_return": late if valid else np.nan, "late_unanimous": late_unanimous,
            "early_witness_share": early_witness, "late_witness_share": late_witness,
            "witness_migration": witness_migration, "serialized_size_migration": size_migration,
        })
    output = pd.DataFrame(rows)
    output["eligible_state"] = output.source_valid & output.late_unanimous & output.witness_migration
...
def active_and_side(features: pd.DataFrame, control: str = "primary"):
    ...
    elif control == "witness_migration_only":
        state = features.source_valid & features.witness_migration
    elif control == "serialized_size_migration":
        state = features.source_valid & features.late_unanimous & features.serialized_size_migration
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active & sides.ne(0)]:
        ...
        rows.append({
            ...
            "early_witness_share": float(features.at[index, "early_witness_share"]),
            "late_witness_share": float(features.at[index, "late_witness_share"]),
            "witness_migration": bool(features.at[index, "witness_migration"]),
            "serialized_size_migration": bool(features.at[index, "serialized_size_migration"]),
        })
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("CLWMSR preregistration drift")
    ...
    source_core = {
        "protocol_version": "clwmsr_6_source_v1", "query_sha256": hashlib.sha256(QUERY.encode()).hexdigest(), "window": [QUERY_START.isoformat(), END.isoformat()],
        "physical_rows": {"blocks": len(blocks), "market": len(market)},
        "block_source": {"path": str(BLOCKS), "sha256": sha(BLOCKS)},
    ...
    core = {
        "protocol_version": "clwmsr_6_source_support_v1", "policy_id": prereg.POLICY_ID,
