"""Build source-only DUTF-6 clocks without opening execution outcomes."""
...
PREREG_SHA = "bb985e4003b03180d56cd2f05aa09978ad2418a1fe68726296912c347c031ac2"
...
CLOCK = Path("data/drought_underfill_template_fracture_clocks_2023_2026.csv.gz")
RESULT = Path("results/drought_underfill_template_fracture_support_2026-08-09.json")
COLUMNS = (
    "candidate", "split", "anchor_height", "confirmation_height", "decision_time",
    "entry_time", "exit_time", "side", "interblock_seconds", "block_weight",
    "side_return",
)
...
def validate_blocks(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("DUTF block schema drift")
...
def build_clock(blocks: pd.DataFrame, bars: pd.DataFrame) -> pd.DataFrame:
    ...
    for index in range(1, len(blocks) - 6):
        ...
        if not (int(current.tx_count) >= 2 and delta >= 1_800 and int(current.weight) <= 1_000_000):
            continue
        ...
        rows.append({
            ...
            "block_weight": int(current.weight), "side_return": side_return,
        })
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "dutf_6_source_support_v1", "policy_id": prereg.POLICY_ID,
