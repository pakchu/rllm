"""Outcome-sequenced preregistration for HVPHR-24."""
...
DEFAULT_OUTPUT = Path(
    "results/high_volatility_peak_candle_range_relay_preregistration_2026-08-09.json"
)
SOURCE_BINDINGS = {
...
def build() -> dict[str, Any]:
    core = {
        "protocol_version": "high_volatility_peak_candle_range_relay_v1",
        "policy_id": "HVPHR-24",
        ...
        "mechanism": {
            "claim": (
                ...
                "that established the trailing six-hour high measures whether price discovery at the "
                "peak was forceful or fragile. An upper-tail peak-candle range relays long, while a "
                "lower-tail range relays short for twenty-four hours."
            ),
            "side": "upper calibrated peak-candle-range tail long; lower tail short",
            "why_distinct": (
                "HVPHR attributes the rolling high to one causal candle and uses that candle's full "
                "range relative to current close. It is not a Donchian reclaim, current wick, range "
            ...
            "why_low_gross9_overlap_is_plausible": (
                "tail onsets depend on the identity and geometry of an endogenous peak-setting candle"
            ),
        },
        "causal_features": {
            ...
            "peak_row": (
                "last row attaining the maximum high in the lookback; deterministic last occurrence "
                "tie break"
            ),
            "peak_candle_range": "(high[peak_row]-low[peak_row])/close[decision]",
            "range_vol": (
                "(maximum high-minimum low)/midpoint over 144 completed five-minute bars including "
                "the decision bar"
            ),
            "calibration": (
                "2023H1 source-only q15/q85 of peak_candle_range and q60 of range_vol across complete "
                "hourly anchors; no calibration labels or post-entry prices"
            ),
            "eligibility": (
                "range_vol>=q60 and peak_candle_range<=q15 or >=q85; first row of a consecutive same-tail "
                "run only"
            ),
        ...
        "research_boundary": {
            "prior_univariate_peak_candle_forward_spread_diagnostics_known": True,
            "prior_diagnostics_are_not_a_strict_strategy_replay": True,
            "exact_hvphr_policy_clock_and_strict_economics_known": False,
...
def validate(payload: dict[str, Any]) -> None:
    ...
    if payload.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError("HVPHR preregistration hash mismatch")
    for raw, expected in SOURCE_BINDINGS.items():
        if hashlib.sha256(Path(raw).read_bytes()).hexdigest() != expected:
            raise RuntimeError(f"HVPHR source drift: {raw}")
