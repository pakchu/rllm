"""Deterministic outcome-blind source support for HVCELVPAR-8."""
...
from training import preregister_high_volatility_caspc_ehlers_premium_activity_router as prereg

START=pd.Timestamp("2023-01-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="b8eb72e1b5d5458f534266bed7504950920f2f256715102a80e3b4a4263d17e6";REG=prereg.build()
...
ROOT=Path("data/high_volatility_caspc_ehlers_premium_activity_router_sources_2023_2026");PANEL=ROOT/"cycle_states.csv.gz";MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_caspc_ehlers_premium_activity_router_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_caspc_ehlers_premium_activity_router_split_clocks_2023_2026")
RESULT=Path("results/high_volatility_caspc_ehlers_premium_activity_router_support_2026-08-16.json");BUILDER=Path(__file__).relative_to(Path.cwd())
BASE_CLOCK=Path("data/high_volatility_caspc_ehlers_active_veto_clocks_2023_2026.csv.gz")
...
def build_panel(raw):
 ...
 if p.columns.tolist()!=ep or b.columns.tolist()!=eb:raise RuntimeError("HVCELVPAR-8 schema drift")
 for f in (p,b):
...
def build_clock(p):
 ...
 for b in base.itertuples(index=False):
  ...
  if side not in (-1,1) or r.feature_available_time>b.entry_time:raise RuntimeError("HVCELVPAR-8 side/availability drift")
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCELVPAR-8 preregistration drift")
