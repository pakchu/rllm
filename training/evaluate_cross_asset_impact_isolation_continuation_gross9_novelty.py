"""Evaluate frozen CAIIC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "CAIIC-8"
PROTOCOL = "caiic_8_gross9_novelty_v1"
PREREG = Path(
    "results/cross_asset_impact_isolation_continuation_preregistration_2026-08-10.json"
)
PREREG_SHA = "9a01373831179b84757bb22adceb992fb609947f2323a8d97b23d77daaa9d6c5"
SUPPORT = Path("results/cross_asset_impact_isolation_continuation_support_2026-08-10.json")
SUPPORT_SHA = "ef4a0974da78be7832f3c0d037581f6f816d49ce621702d1ff9bcccbd152b074"
CLOCK = Path("data/cross_asset_impact_isolation_continuation_clocks_2023_2026.csv.gz")
CLOCK_SHA = "68a1fda885eaecdfbdf61e8410a9baaf8d748e6c1dd4b7f04f70bcd30d2602a2"
OUTPUT = Path(
    "results/cross_asset_impact_isolation_continuation_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CAIIC predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("CAIIC novelty limits drift")
    if (
        ...
        raise RuntimeError("CAIIC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CAIIC primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "caiic_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject CAIIC-8 unchanged before economic outcomes"
        ),
    }
