"""Evaluate frozen HVT3-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVT3-24"
PROTOCOL = "hvt3_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_tillson_t3_turn_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVT3 predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVT3 novelty limits drift")
    if (
        ...
        raise RuntimeError("HVT3 predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVT3 primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvt3_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVT3-24 unchanged before economic outcomes",
    }
