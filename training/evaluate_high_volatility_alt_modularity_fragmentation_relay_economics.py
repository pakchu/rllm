"""Sequential strict economics for frozen HVAMF-8."""
...
PREREG = Path("results/high_volatility_alt_modularity_fragmentation_relay_preregistration_2026-08-13.json")
PREREG_SHA = "578613ce3da6717f0a2ba5fd799bfa829fa5e1f17e575a9b47fda9ed2f4a66d4"
SUPPORT = Path("results/high_volatility_alt_modularity_fragmentation_relay_support_2026-08-13.json")
SUPPORT_SHA = "e90e54b25d86446e09cb9a0d4d476f4735233b72910063b73ea4728712f5f312"
NOVELTY = Path("results/high_volatility_alt_modularity_fragmentation_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "a289a13b60b2003c65267c9a338f5424611f07a15c3ca8cb848c425ef3cc2e80"
CLOCK = Path("data/high_volatility_alt_modularity_fragmentation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "274c25b86bfca29e67929f501bbc0eb284e191240b1616e477f0b5d0021e0fa0"
CONTROL_DIR = Path("data/high_volatility_alt_modularity_fragmentation_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_alt_modularity_fragmentation_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_alt_modularity_fragmentation_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_modularity_tail", "no_variation_gate", "all_alt_final_hour_median", "one_decision_stale_partition", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVAMF frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVAMF novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvamf_8_sequential_economics_v1", "policy_id": POLICY_ID,
