"""Evaluate frozen HVERI-24 structural novelty against every Gross9 sleeve."""
...
from training import preregister_high_volatility_ehlers_reversion_index_crossover_relay as registration

POLICY = "HVERI-24"
PROTOCOL = "hveri_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_ehlers_reversion_index_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVERI predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVERI novelty limits drift")
    if (
        ...
        raise RuntimeError("HVERI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVERI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVERI-24 unchanged before economic outcomes",
    }
