"""Build outcome-blind source support for frozen HVDEC-24."""
...
from training import preregister_high_volatility_ehlers_decycler_oscillator_zero_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "040146f1efc175daa31b63e38f9dd7c13b3dd7362b8febe6f86724ca8b485248"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ehlers_decycler_oscillator_zero_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ehlers_decycler_oscillator_zero_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ehlers_decycler_oscillator_zero_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ehlers_decycler_oscillator_zero_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ehlers_decycler_oscillator_zero_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "run_length", "high_pass", "decycler",
    "decycler_oscillator", "fast_decycler_oscillator", "entry_side", "fast_entry_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "run_length", "high_pass", "decycler",
    "decycler_oscillator", "fast_decycler_oscillator", "entry_side", "fast_entry_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVDEC source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVDEC invalid source key")
...
def decycler_filter(series: pd.Series, valid: pd.Series, hp_period: int, k: float) -> pd.DataFrame:
    angle1=.707*2*math.pi/hp_period;alpha1=(math.cos(angle1)+math.sin(angle1)-1)/math.cos(angle1)
    angle2=.707*2*math.pi/(.5*hp_period);alpha2=(math.cos(angle2)+math.sin(angle2)-1)/math.cos(angle2)
    values=series.to_numpy(float);ok=valid.to_numpy(bool);hp=np.full(len(series),np.nan);decycle=np.full(len(series),np.nan);osc=np.full(len(series),np.nan);runs=np.zeros(len(series),dtype=int);run=0
    for i,(price,is_valid) in enumerate(zip(values,ok)):
        if not is_valid or not math.isfinite(price):run=0;continue
        run+=1;runs[i]=run
        if run<=2:hp[i]=0.;decycle[i]=price;osc[i]=0.;continue
        hp[i]=(1-alpha1/2)**2*(price-2*values[i-1]+values[i-2])+2*(1-alpha1)*hp[i-1]-(1-alpha1)**2*hp[i-2]
        decycle[i]=price-hp[i]
        osc[i]=(1-alpha2/2)**2*(decycle[i]-2*decycle[i-1]+decycle[i-2])+2*(1-alpha2)*osc[i-1]-(1-alpha2)**2*osc[i-2]
    normalized=100*k*osc/values
    return pd.DataFrame({"run_length":runs,"high_pass":hp,"decycler":decycle,"decycler_oscillator":normalized},index=series.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    fields=["bar_open","bar_high","bar_low","bar_close"];bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[fields]).all(axis=1)&bars[fields].gt(0).all(axis=1);values=decycler_filter(bars.bar_close,bars.valid_bar,P["hp_periods"],P["k"]);fast=decycler_filter(bars.bar_close,bars.valid_bar,P["fast_control_hp_periods"],P["fast_control_k"])
    for column in values:bars[column]=values[column]
    bars["fast_decycler_oscillator"]=fast.decycler_oscillator;bars["entry_side"]=cross_side(bars.decycler_oscillator);bars["fast_entry_side"]=cross_side(bars.fast_decycler_oscillator);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["high_pass","decycler","decycler_oscillator","variation"];bars["source_valid"]=bars.valid_bar&bars.run_length.ge(3)&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="fast_100_decycler":side=pd.to_numeric(used.fast_entry_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVDEC prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvdec_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvdec_24_source_support_v1", "policy_id": prereg.POLICY_ID,
