"""Sequential strict economics for frozen HVTMF-24."""
...
PREREG = Path("results/high_volatility_twiggs_money_flow_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "a2c9785890cbad4b9c193adaa83422621144dd163db3b93b0c22b24953102e47"
SUPPORT = Path("results/high_volatility_twiggs_money_flow_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "e8820750affcbe0864b6d28996db672d37938d5c1c93fe74f70444bf13b43f03"
NOVELTY = Path("results/high_volatility_twiggs_money_flow_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "36dc1e2fa1891187b8a6b7bffc6069440653475157e6dcae7b967b0fe2f61978"
CLOCK = Path("data/high_volatility_twiggs_money_flow_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "bab4ebb0e668f1626f3ad9e184997d57b539f10037e1b4ae2cf4df0f7f5731eb"
CONTROL_DIR = Path("data/high_volatility_twiggs_money_flow_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_twiggs_money_flow_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_twiggs_money_flow_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "ordinary_high_low_bounds", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTMF frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTMF novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvtmf_24_sequential_economics_v1", "policy_id": POLICY_ID,
