"""Deterministic source support for HVCVMSSD-8."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_cross_venue_median_shift_spread_discovery as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
PREREG_SHA="c189a434058295aeb46696bdc15f6685b7eb4303976a7f5931da85ec3018a8af";FEATURES=Path("data/high_volatility_cross_venue_median_shift_spread_discovery_features_2023_2026.csv.gz");CLOCK=Path("data/high_volatility_cross_venue_median_shift_spread_discovery_clocks_2023_2026.csv.gz");RESULT=Path("results/high_volatility_cross_venue_median_shift_spread_discovery_support_2026-08-18.json");POLICY=prereg.build()["policy"];STAGES={k:tuple(pd.Timestamp(v) for v in values) for k,values in prereg.build()["stages"].items()};GATES=prereg.build()["source_support_gates"]
...
def prepare(perp:pd.DataFrame,spot:pd.DataFrame)->pd.DataFrame:
 ...
 x["joint_valid"]=(x["source_valid"].eq(True)&x["spot_source_valid"].eq(True)&np.isfinite(x[["median_shift","spot_shift","variation_rank"]]).all(axis=1));x["shift_spread"]=x["spot_shift"]-x["median_shift"];rank_valid=x["joint_valid"]&x["shift_spread"].ne(0);x["spread_rank"]=causal_rank(x["shift_spread"].abs(),rank_valid)
 level=(rank_valid&x["spread_rank"].ge(POLICY["spread_rank_min"])&x["variation_rank"].ge(POLICY["variation_rank_min"]));consecutive=x["decision_time"].sub(x["decision_time"].shift(1)).eq(pd.Timedelta("8h"));x["eligible"]=level&consecutive&~level.shift(1,fill_value=False)&x["joint_valid"].shift(1,fill_value=False);return x
...
def build_clock(features:pd.DataFrame)->pd.DataFrame:
 ...
 for r in features[features["eligible"]].itertuples(index=False):
  ...
  side=int(np.sign(r.shift_spread));rows.append({"candidate":prereg.POLICY_ID,"control":"primary","split":split,"decision_time":d,"feature_available_time":r.feature_available_time,"entry_time":e,"exit_time":z,"side":side,"shift_spread":float(r.shift_spread),"spread_rank":float(r.spread_rank),"spot_shift":float(r.spot_shift),"perpetual_shift":float(r.median_shift),"variation_rank":float(r.variation_rank)})
 return pd.DataFrame(rows,columns=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","shift_spread","spread_rank","spot_shift","perpetual_shift","variation_rank"))
...
def run(features_path:Path=FEATURES,clock_path:Path=CLOCK,result_path:Path=RESULT)->dict[str,Any]:
 if prereg.sha256(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCVMSSD prereg drift")
