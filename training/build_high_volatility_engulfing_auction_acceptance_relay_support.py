"""Build outcome-blind source support for frozen HVEAAR-8."""
...
from training import preregister_high_volatility_engulfing_auction_acceptance_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-04-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="ddf4ce76ea559da27eadcff9336e5e8521637a6681893efdf7cf7648851bf2ca"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_engulfing_auction_acceptance_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_engulfing_auction_acceptance_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_engulfing_auction_acceptance_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_engulfing_auction_acceptance_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_engulfing_auction_acceptance_relay_support_2026-08-11.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","prior_high","prior_low","engulfing_open","engulfing_high","engulfing_low","engulfing_close","close_location","strict_engulfment","accepted_side","no_outer_quartile_side","body_only_side","stale_prior_range_side","realized_variation","variation_rank","eligible")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","prior_high","prior_low","engulfing_open","engulfing_high","engulfing_low","engulfing_close","close_location","strict_engulfment","accepted_side","no_outer_quartile_side","body_only_side","stale_prior_range_side","realized_variation","variation_rank")
...
def engulfing_pattern(auction:pd.DataFrame,prior_high:float,prior_low:float)->tuple[float,float,float,float,float,bool,int,int,int]:
 open_=float(auction.open.iloc[0]);high=float(auction.high.max());low=float(auction.low.min());close=float(auction.close.iloc[-1]);location=(close-low)/(high-low);engulf=high>prior_high and low<prior_low;body=1 if close>open_ else -1 if close<open_ else 0
 accepted=1 if engulf and location>=.75 and body==1 else -1 if engulf and location<=.25 and body==-1 else 0
 no_outer=body if engulf else 0
 return open_,high,low,close,location,engulf,accepted,no_outer,body
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVEAAR source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVEAAR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("4h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   prior=block.iloc[:240];auction=block.iloc[240:];prior_high=float(prior.high.max());prior_low=float(prior.low.min());eo,eh,el,ec,location,engulf,side,no_outer,body=engulfing_pattern(auction,prior_high,prior_low)
   stale_side=0 if previous_prior is None else engulfing_pattern(auction,*previous_prior)[6]
  ...
  else:prior_high=prior_low=eo=eh=el=ec=location=var=math.nan;engulf=False;side=no_outer=body=stale_side=0;valid=False;previous_prior=None
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"prior_high":prior_high,"prior_low":prior_low,"engulfing_open":eo,"engulfing_high":eh,"engulfing_low":el,"engulfing_close":ec,"close_location":location,"strict_engulfment":engulf,"accepted_side":side,"no_outer_quartile_side":no_outer,"body_only_side":body,"stale_prior_range_side":stale_side,"realized_variation":var})
...
def active(panel:pd.DataFrame,control:str="primary"):
 ...
 if control=="no_outer_quartile":side=pd.to_numeric(used.no_outer_quartile_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
 elif control=="no_variation_gate":state=valid&side.ne(0)
 elif control=="engulfing_body_only":side=pd.to_numeric(used.body_only_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVEAAR prereg drift")
 ...
 source_core={"protocol_version":"hveaar_8_source_v1","query":QUERY,"query_sha256":hashlib.sha256(QUERY.encode()).hexdigest(),"window":[START.isoformat(),END.isoformat()],"physical_rows":len(raw),"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"panel":{"path":str(PANEL),"sha256":sha(PANEL),"rows":len(panel),"valid_rows":int(panel.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};manifest={**source_core,"manifest_hash":chash(source_core)};immutable(MANIFEST,json_bytes(manifest))
