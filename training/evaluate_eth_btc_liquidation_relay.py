"""Write-once staged evaluator for frozen EBLR-60/30 liquidation relay."""
...
from training.preregister_eth_btc_liquidation_relay import (
    ETH_ABS_IMBALANCE_MIN,
    ETH_MIN_EVENT_COUNT,
    ETH_SEVERITY_MIN,
    WAVE_BARS,
...
SUPPORT_COMMIT = "60e8279728a0eb3d28a667ec5634a8044b085e67"
...
EVALUATOR_SOURCE = Path(
    "training/evaluate_eth_btc_liquidation_relay.py"
)
EVALUATOR_FREEZE = Path(
    "results/eth_btc_liquidation_relay_evaluator_freeze_2026-07-19.json"
)
SUPPORT_RESULT = Path(
    "results/eth_btc_liquidation_relay_support_2026-07-19.json"
)
EXECUTION_MANIFEST = Path("results/clbr_execution_sources_2023_2024_manifest.json")
COMBINED_CLOCKS = Path(
    "data/eth_btc_liquidation_relay_clocks_2023_2024.csv.gz"
)
SPLIT_CLOCK_DIR = Path(
    "data/eth_btc_liquidation_relay_clocks_split_2023_2024"
)
CONTROL_CLOCK_DIR = Path(
    "data/eth_btc_liquidation_relay_controls_split_2023_2024"
)
RESULT_PATHS = {
    stage: Path(
        f"results/eth_btc_liquidation_relay_{stage}_2026-07-19.json"
    )
    for stage in SPLITS
}
STATIC_INPUT_SHA256 = {
    "training/preregister_eth_btc_liquidation_relay.py": (
        "fe40b29ab1cabc115304adce574033ab61a76d441ea730c379e63b1c7aa2d8cf"
    ),
    "docs/eth-btc-liquidation-relay-preregistration-2026-07-19.md": (
        "7a19c9f574546f54de3c624cf702260ea990339a4b45f88cf85c299a5f309182"
    ),
    "tests/test_preregister_eth_btc_liquidation_relay.py": (
        "87bb28e896f491c4f0647ef5557d1778e46119d3407e63152af88225fa768595"
    ),
    "tests/test_eth_btc_liquidation_relay_support_artifact.py": (
        "4d05bd81494a001b9adfa4f3637fbf53da066908bd9c342aa79b15d185d9e5e5"
    ),
    str(COMBINED_CLOCKS): (
    ...
    "docs/eblr-strict-evaluator-contract-2026-07-19.md": (
        "b1631c66162676eacf7230daf0812deb1d15de0590ed09d204f799cd2512340d"
    ),
    "tests/test_evaluate_eth_btc_liquidation_relay.py": (
        "7f3062f6e2cc30162fe43633a8af3ce5b0107d5dd45b79299df4ba9d1b1490d1"
    ),
}
...
EXPECTED_CLOCK_ROWS = {"train": 21, "test": 132, "eval": 113}
STAGES = tuple(SPLITS)
...
def _source_only_preconditions(support: dict[str, Any]) -> dict[str, Any]:
    ...
    clbr = overlap.get("CLBR", {})
    icla = overlap.get("ICLA", {})
    clbr_jaccard = float(clbr.get("entry_jaccard", float("inf")))
    icla_jaccard = float(icla.get("entry_jaccard", float("inf")))
    preconditions = {
        "support_passes": support.get("support_passes") is True,
        "clbr_overlap_rejected": overlap.get("passes") is True
        and maximum == MAX_ENTRY_JACCARD
        and clbr_jaccard <= MAX_ENTRY_JACCARD,
        "icla_overlap_rejected": overlap.get("passes") is True
        and maximum == MAX_ENTRY_JACCARD
        and icla_jaccard <= MAX_ENTRY_JACCARD,
        "clbr_entry_jaccard": clbr_jaccard,
        "icla_entry_jaccard": icla_jaccard,
        "maximum_entry_jaccard": maximum,
        "clbr_clock_sha256": clbr.get("sha256"),
        "icla_clock_sha256": icla.get("sha256"),
    }
    if not preconditions["support_passes"]:
        raise ValueError("source-only support gate failed")
    if not preconditions["clbr_overlap_rejected"]:
        raise ValueError("source-only CLBR overlap gate failed")
    if not preconditions["icla_overlap_rejected"]:
        raise ValueError("source-only ICLA overlap gate failed")
...
def _load_combined_clocks(
    ...
    if not cast(pd.Series, clocks["candidate"]).eq("EBLR-60/30").all():
...
def _minimal_control_frame(frame: pd.DataFrame, candidate: str) -> pd.DataFrame:
    output = frame[
        ["split", "entry_time", "planned_exit_time", "direction"]
    ].copy()
    output.insert(0, "candidate", candidate)
    return cast(
        pd.DataFrame,
        cast(Any, output).sort_values("entry_time", ignore_index=True),
    )


def _source_control_clocks(kind: str) -> pd.DataFrame:
    btc, eth = load_sources(PreregistrationConfig())
    state = derive_relay_state(btc, eth)
    if kind == "btc_only_direct_shock":
        btc_imbalance = cast(pd.Series, state["btc_wave_imbalance"])
        direction = pd.Series(
            np.where(btc_imbalance.ge(ETH_ABS_IMBALANCE_MIN), 1, -1),
            index=state.index,
            dtype="int64",
        ).where(btc_imbalance.abs().ge(ETH_ABS_IMBALANCE_MIN), 0)
        btc_valid = cast(
            pd.Series,
            cast(pd.Series, btc["source_valid"])
            .astype(int)
        ...
        state["is_candidate"] = (
            btc_valid
            & cast(pd.Series, state["btc_event_count_60m"]).ge(
                ETH_MIN_EVENT_COUNT
            )
            & cast(pd.Series, state["btc_quiet_severity"]).ge(
                ETH_SEVERITY_MIN
            )
            & btc_imbalance.abs().ge(ETH_ABS_IMBALANCE_MIN)
            & direction.ne(0)
        )
    elif kind == "no_btc_quiet_gate":
        eth_imbalance = cast(pd.Series, state["eth_wave_imbalance"])
        direction = cast(pd.Series, state["direction"])
        state["is_candidate"] = (
            cast(pd.Series, state["wave_source_valid"]).astype(bool)
            & cast(pd.Series, state["eth_event_count_60m"]).ge(
                ETH_MIN_EVENT_COUNT
            )
            & cast(pd.Series, state["eth_severity"]).ge(ETH_SEVERITY_MIN)
            & eth_imbalance.abs().ge(ETH_ABS_IMBALANCE_MIN)
            & direction.ne(0)
        )
    else:
        raise ValueError(f"unknown source control: {kind}")
    clocks = build_clocks(state)
    return _minimal_control_frame(clocks, f"control_{kind}")
...
def _shifted_control(
    ...
    *,
    ...
    output = _minimal_control_frame(primary, candidate)
    output["entry_time"] = output["entry_time"] + delta
    output["planned_exit_time"] = output["planned_exit_time"] + delta
    start = _timestamp(SPLITS[stage][0])
    end = _timestamp(SPLITS[stage][1])
    return cast(
        pd.DataFrame,
        output.loc[
            output["entry_time"].ge(start)
            & output["planned_exit_time"].lt(end)
        ].reset_index(drop=True),
    )
...
def _random_control_clocks(
    ...
    if "entry_time" in primary.columns:
        month_keys = cast(pd.Series, primary["entry_time"]).dt.to_period("M")
    else:
        month_keys = pd.Series([start.to_period("M")] * len(primary))
    rows: list[dict[str, Any]] = []
    for month in month_keys.drop_duplicates().sort_values():
        mask = month_keys.eq(month)
        subset = cast(pd.DataFrame, primary.loc[mask])
        month_start = max(start, _timestamp(month.start_time))
        month_end = min(end, _timestamp(month.end_time + pd.Timedelta(1, unit="ns")))
        slots = pd.date_range(month_start, month_end, freq="30min", inclusive="left")
        slots = slots[slots + BAR * HOLD_BARS < month_end]
        if len(slots) < len(subset):
            raise ValueError(f"{stage} {month} has too few random slots")
        chosen = np.sort(rng.choice(len(slots), size=len(subset), replace=False))
        directions = cast(pd.Series, subset["direction"]).to_numpy(dtype=int).copy()
        rng.shuffle(directions)
        for entry, direction in zip(slots[chosen], directions, strict=True):
            rows.append(
                {
                    "candidate": "control_random_clocks",
                    "split": stage,
                    "entry_time": entry,
                    "planned_exit_time": entry + BAR * HOLD_BARS,
                    "direction": int(direction),
                }
            )
    return pd.DataFrame(rows).sort_values("entry_time", ignore_index=True)
...
def _expected_control_clock_artifacts(
    ...
    btc_only = _source_control_clocks("btc_only_direct_shock")
    no_quiet = _source_control_clocks("no_btc_quiet_gate")
    metadata: dict[str, dict[str, Any]] = {}
    frames: dict[str, dict[str, pd.DataFrame]] = {}
    for stage_index, stage in enumerate(STAGES):
        primary = primary_frames[stage]
        direction_flip = _minimal_control_frame(
            primary, "control_direction_flip"
        )
        direction_flip["direction"] = -cast(
            pd.Series, direction_flip["direction"]
        ).to_numpy(dtype=int)
        delayed = _shifted_control(
            primary,
            stage,
            delta=BAR,
            candidate="control_delayed_5m",
        )
        future_placebo = _shifted_control(
            primary,
            stage,
            delta=-(BAR * HOLD_BARS),
            candidate="control_future_eth_placebo",
        )
        btc_stage = cast(
            pd.DataFrame,
            btc_only.loc[btc_only["split"].eq(stage)].copy(),
        )
        no_quiet_stage = cast(
            pd.DataFrame,
            no_quiet.loc[no_quiet["split"].eq(stage)].copy(),
        )

        stage_frames = {
            "direction_flip": direction_flip,
            "btc_only_direct_shock": btc_stage,
            "no_btc_quiet_gate": no_quiet_stage,
            "delayed_5m": delayed,
            "future_eth_placebo": future_placebo,
    ...
    report: dict[str, Any] = {
        ...
        "control_contract": {
            ...
            "random_clock_support": (
                "same stage month, trade-count, and long/short distributions as primary"
            ),
            "future_eth_placebo": "noncausal veto-only control; never promotable",
...
def _validate_clock_frame(
    ...
    *,
    ...
    for forbidden in ("stop_price", "take_profit_price"):
        if forbidden in clocks.columns:
            raise ValueError(f"{stage} clocks contain forbidden {forbidden}")
    ...
    if not exits.sub(entries).eq(BAR * HOLD_BARS).all():
        raise ValueError(f"{stage} hold differs from frozen 6 bars / 30 minutes")
    if (
        len(clocks) > 1
        and not entries.iloc[1:]
        .reset_index(drop=True)
...
def simulate_strict(
    ...
    *,
    ...
    """Simulate fixed 30-minute trades with hardened path accounting."""
    ...
    for clock in clocks.to_dict(orient="records"):
        ...
        if exit_position - entry_position != HOLD_BARS:
            raise ValueError("frozen hold is not exactly 6 bars / 30 minutes")
...
def promotion_gate_contract() -> dict[str, Any]:
    ...
    for stage in STAGES:
        contract[stage] = {
            ...
            "minimum_executable_trades": 20 if stage == "train" else 50,
            "minimum_trades_per_side": 6 if stage == "train" else 12,
...
def _apply_source_only_preconditions(
    ...
    promotion["checks"]["clbr_overlap_rejected"] = bool(
        preconditions["clbr_overlap_rejected"]
    )
    promotion["checks"]["icla_overlap_rejected"] = bool(
        preconditions["icla_overlap_rejected"]
    )
    promotion["passes"] = bool(
        promotion["passes"]
        and promotion["checks"]["source_support_passes"]
        and promotion["checks"]["clbr_overlap_rejected"]
        and promotion["checks"]["icla_overlap_rejected"]
    )
    return promotion
