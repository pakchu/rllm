"""Build outcome-blind source support for frozen HVVRAT-24."""
...
from training import preregister_high_volatility_volume_ratio_centerline_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "7134e6a79f941c4f787861052c78a8f10b1b5174ef535c9e33d8d67b3d5cc945"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_volume_ratio_centerline_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_volume_ratio_centerline_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_volume_ratio_centerline_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_volume_ratio_centerline_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_volume_ratio_centerline_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "bar_volume", "direction", "up_volume", "down_volume", "unchanged_volume",
    "up_sum", "down_sum", "unchanged_sum", "volume_ratio", "net_volume", "ratio_side", "net_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "bar_volume", "direction", "up_volume", "down_volume", "unchanged_volume",
    "up_sum", "down_sum", "unchanged_sum", "volume_ratio", "net_volume", "ratio_side", "net_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close", "volume"]:
        raise RuntimeError("HVVRAT source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVVRAT invalid source key")
...
def volume_ratio_values(close: pd.Series, volume: pd.Series, valid: pd.Series) -> pd.DataFrame:
    period=P["volume_ratio_periods"];transition=valid&valid.shift(1,fill_value=False);direction=np.sign(close-close.shift(1)).where(transition);qty=volume.where(transition)
    up=qty.where(direction.gt(0),0.0).where(transition);down=qty.where(direction.lt(0),0.0).where(transition);unchanged=qty.where(direction.eq(0),0.0).where(transition)
    up_sum=up.rolling(period,min_periods=period).sum();down_sum=down.rolling(period,min_periods=period).sum();unchanged_sum=unchanged.rolling(period,min_periods=period).sum();denominator=down_sum+0.5*unchanged_sum
    ratio=(100.0*(up_sum+0.5*unchanged_sum)/denominator).where(denominator.gt(0));net=up_sum-down_sum
    return pd.DataFrame({"direction":direction,"up_volume":up,"down_volume":down,"unchanged_volume":unchanged,"up_sum":up_sum,"down_sum":down_sum,"unchanged_sum":unchanged_sum,"volume_ratio":ratio,"net_volume":net},index=close.index)


def zero_cross_side(value: pd.Series) -> pd.Series:
    previous=value.shift(1);long=value.gt(0)&previous.le(0);short=value.lt(0)&previous.ge(0)
    return pd.Series(np.where(long,1,np.where(short,-1,0)),index=value.index,dtype=int)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    values=volume_ratio_values(bars.bar_close,bars.bar_volume,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["ratio_side"]=zero_cross_side(bars.volume_ratio-P["centerline"]);bars["net_side"]=zero_cross_side(bars.net_volume)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["bar_volume","direction","up_volume","down_volume","unchanged_volume","up_sum","down_sum","unchanged_sum","volume_ratio","net_volume","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&(bars.down_sum+0.5*bars.unchanged_sum).gt(0)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&panel.ratio_side.ne(0)&panel.variation_rank.ge(P["variation_rank_min"])
    return panel.loc[:,PANEL_COLUMNS]

def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);variation=used.variation_rank.ge(P["variation_rank_min"]);side=pd.to_numeric(used.ratio_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
    if control=="no_variation_gate":state=valid&side.ne(0)
    elif control=="net_volume_zero_cross":side=pd.to_numeric(used.net_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVVRAT prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvvrat_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvvrat_24_source_support_v1", "policy_id": prereg.POLICY_ID,
