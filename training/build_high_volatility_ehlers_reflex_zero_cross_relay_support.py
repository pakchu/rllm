"""Build outcome-blind source support for frozen HVERF-24."""
...
from training import preregister_high_volatility_ehlers_reflex_zero_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "60dc33c79028cd44f78e001c60a9747c181359efe885e4d7cf747e92e3e65380"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ehlers_reflex_zero_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ehlers_reflex_zero_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ehlers_reflex_zero_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ehlers_reflex_zero_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ehlers_reflex_zero_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "run_length", "filtered", "slope", "residual_sum", "mean_square",
    "reflex", "entry_side", "slope_cross_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "run_length", "filtered", "slope", "residual_sum",
    "mean_square", "reflex", "entry_side", "slope_cross_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVERF source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVERF invalid source key")
...
def reflex(series: pd.Series, valid: pd.Series) -> pd.DataFrame:
    length=int(P["length"]);critical=.5*length;a1=math.exp(-1.414*math.pi/critical);c2=2*a1*math.cos(1.414*math.pi/critical);c3=-a1*a1;c1=1-c2-c3
    values=series.to_numpy(float);ok=valid.to_numpy(bool);filt=np.full(len(series),np.nan);slope=np.full(len(series),np.nan);sums=np.full(len(series),np.nan);ms=np.full(len(series),np.nan);output=np.full(len(series),np.nan);runs=np.zeros(len(series),dtype=int);run=0;previous_ms=0.
    for i,(price,is_valid) in enumerate(zip(values,ok)):
        if not is_valid or not math.isfinite(price):run=0;previous_ms=0.;continue
        run+=1;runs[i]=run;filt[i]=price if run<=3 else c1*(price+values[i-1])/2+c2*filt[i-1]+c3*filt[i-2]
        if run<=length:continue
        slope[i]=(filt[i-length]-filt[i])/length
        counts=np.arange(1,length+1,dtype=float);sums[i]=float(np.mean((filt[i]+counts*slope[i])-filt[i-length:i][::-1]))
        ms[i]=P["mean_square_current_weight"]*sums[i]**2+P["mean_square_prior_weight"]*previous_ms;previous_ms=ms[i]
        if ms[i]>0:output[i]=sums[i]/math.sqrt(ms[i])
    return pd.DataFrame({"run_length":runs,"filtered":filt,"slope":slope,"residual_sum":sums,"mean_square":ms,"reflex":output},index=series.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    fields=["bar_open","bar_high","bar_low","bar_close"];bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[fields]).all(axis=1)&bars[fields].gt(0).all(axis=1);values=reflex(bars.bar_close,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["entry_side"]=cross_side(bars.reflex);bars["slope_cross_side"]=cross_side(-bars.slope);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["filtered","slope","residual_sum","mean_square","reflex","variation"];bars["source_valid"]=bars.valid_bar&bars.run_length.ge(P["warmup_observations"])&np.isfinite(bars[numeric]).all(axis=1)&bars.mean_square.gt(0)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="filtered_slope_zero_cross":side=pd.to_numeric(used.slope_cross_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVERF prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hverf_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hverf_24_source_support_v1", "policy_id": prereg.POLICY_ID,
