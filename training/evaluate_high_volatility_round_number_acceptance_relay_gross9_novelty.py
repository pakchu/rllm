"""Evaluate frozen HVRNAR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVRNAR-8"
PROTOCOL = "hvrnar_8_gross9_novelty_v1"
PREREG = Path(
...
SUPPORT = Path("results/high_volatility_round_number_acceptance_relay_support_2026-08-12.json")
SUPPORT_SHA = "72961c7e189f1532890fc5dbb11f3a7be0e76eda8e1ca7b1313dbac294d2c8be"
CLOCK = Path("data/high_volatility_round_number_acceptance_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "3bce212552e9e7ccf0ee4846f34cdcd74771e923fd654aed92f8a1c9b8c7519f"
OUTPUT = Path(
    "results/high_volatility_round_number_acceptance_relay_gross9_novelty_2026-08-12.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVRNAR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVRNAR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVRNAR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVRNAR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvrnar_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVRNAR-8 unchanged before economic outcomes"
        ),
    }
