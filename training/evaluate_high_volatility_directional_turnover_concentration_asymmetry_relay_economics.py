"""Sequential strict economics for frozen HVDTCAR-8."""
...
PREREG = Path("results/high_volatility_directional_turnover_concentration_asymmetry_relay_preregistration_2026-08-10.json")
PREREG_SHA = "9e312f1c5e332ecc85011daeb028a862cc3be29ada37f7f0cb5d685934a00cbe"
SUPPORT = Path("results/high_volatility_directional_turnover_concentration_asymmetry_relay_support_2026-08-10.json")
SUPPORT_SHA = "0649e9812f4382bb56bdcf13e8355f88f9e022cef18ca2602ef1448bbcb7e0ac"
NOVELTY = Path("results/high_volatility_directional_turnover_concentration_asymmetry_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "1208907e9f75380b1dc903bd2bbf5869fe55e4c205aacd60160422df4e0a861f"
CLOCK = Path("data/high_volatility_directional_turnover_concentration_asymmetry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "daeb113cd6a0c316545c5e888eb355fdf03bfd88f03c079737df047479031d1a"
CONTROL_DIR = Path("data/high_volatility_directional_turnover_concentration_asymmetry_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_directional_turnover_concentration_asymmetry_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_directional_turnover_concentration_asymmetry_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_concentration_tail", "no_variation_gate", "directional_turnover_mass_asymmetry", "one_block_stale_concentration", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDTCAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDTCAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdtcar_8_sequential_economics_v1", "policy_id": POLICY_ID,
