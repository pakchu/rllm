"""Evaluate frozen HVTCCR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVTCCR-8"
PROTOCOL = "hvtccr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_quote_turnover_concentration_continuation_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTCCR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVTCCR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTCCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTCCR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvtccr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVTCCR-8 unchanged before economic outcomes",
    }
