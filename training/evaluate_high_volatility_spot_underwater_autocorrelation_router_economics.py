"""Strict staged economics for the singleton HVSAUDAR-8 policy."""
...
PREREG = Path("results/high_volatility_spot_underwater_autocorrelation_router_preregistration_2026-08-18.json")
PREREG_SHA = "a8a9a64cc9eda46c02fc4f0f511d641ccd754c740c9cb41d18194e4780842feb"
SUPPORT = Path("results/high_volatility_spot_underwater_autocorrelation_router_support_2026-08-18.json")
SUPPORT_SHA = "911ecd4b4ff71149afc1cf2871305d1cd335b099aeec7bd0dd620243f6747ed8"
GROSS9 = Path("results/high_volatility_spot_underwater_autocorrelation_router_gross9_novelty_2026-08-18.json")
GROSS9_SHA = "432586c381e3c51a66f27bf5c4ea35d257c8805158975362da5113e259494170"
CLOCK = {
    "path": Path("data/high_volatility_spot_underwater_autocorrelation_router_clocks_2023_2026.csv.gz"),
    "sha256": "8277820894926479283dfba02abf953e639a9b322089c9a9fc9542509eb779d5",
    "rows": 183,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_spot_underwater_autocorrelation_router_{stage}_economics_2026-08-18.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVSAUDAR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVSAUDAR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVSAUDAR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVSAUDAR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVSAUDAR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVSAUDAR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVSAUDAR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVSAUDAR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVSAUDAR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVSAUDAR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVSAUDAR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvsaudar_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
