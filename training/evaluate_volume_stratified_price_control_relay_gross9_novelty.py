"""Evaluate frozen VSPCR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "VSPCR-8"
PROTOCOL = "vspcr_8_gross9_novelty_v1"
PREREG = Path("results/volume_stratified_price_control_relay_preregistration_2026-08-09.json")
PREREG_SHA = "e52a9aa8cc33aecb42c6e8496d95b0bd7d62e527fcbb0e38aaee02f036769a17"
SUPPORT = Path("results/volume_stratified_price_control_relay_support_2026-08-09.json")
SUPPORT_SHA = "35f852de9927f41683ad8b3e257f18c59afbb862b5ec06e7bb0290740e91e453"
CLOCK = Path("data/volume_stratified_price_control_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "9e30c9b2b604aed1acce14e276db7a3ec5046f1cbde3f7ffdd92097324fd5fde"
OUTPUT = Path("results/volume_stratified_price_control_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("VSPCR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("VSPCR novelty limits drift")
    if (
        ...
        raise RuntimeError("VSPCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="VSPCR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "vspcr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject VSPCR-8 unchanged before economic outcomes",
    }
