"""Evaluate the frozen HVSAUDAR-8 action-vote clock against Gross9."""
...
POLICY = "HVSAUDAR-8"
PROTOCOL = "hvsaudar_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_spot_underwater_autocorrelation_router_clocks_2023_2026.csv.gz",
    "sha256": "8277820894926479283dfba02abf953e639a9b322089c9a9fc9542509eb779d5",
    "rows": 183,
}
OUTPUT = Path("results/high_volatility_spot_underwater_autocorrelation_router_gross9_novelty_2026-08-18.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVSAUDAR-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVSAUDAR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVSAUDAR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVSAUDAR-8 source eligibility drift")
    sealed = (
        "postentry_return_pnl_execution_price_opened",
        "gross9_rows_opened",
    )
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVSAUDAR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVSAUDAR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVSAUDAR-8 unchanged before economics",
    }
