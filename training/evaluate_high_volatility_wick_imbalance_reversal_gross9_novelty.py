"""Evaluate frozen HVWIR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVWIR-8"
PROTOCOL = "hvwir_8_gross9_novelty_v1"
PREREG = Path(
...
SUPPORT = Path("results/high_volatility_wick_imbalance_reversal_support_2026-08-09.json")
SUPPORT_SHA = "4a6df22ced8036bd279383254d449ecdc5332458efd0907aa050b06f22bed7ea"
CLOCK = Path("data/high_volatility_wick_imbalance_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "526df5ab1f4991b3f3e10fbc7eddc15f5bbd21ac995cc0afcc57d11aa5589078"
OUTPUT = Path(
    "results/high_volatility_wick_imbalance_reversal_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVWIR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVWIR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVWIR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVWIR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvwir_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVWIR-8 unchanged before economic outcomes"
        ),
    }
