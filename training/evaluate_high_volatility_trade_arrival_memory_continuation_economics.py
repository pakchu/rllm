"""Sequential strict economics for frozen HVTAMC-8."""
...
PREREG = Path("results/high_volatility_trade_arrival_memory_continuation_preregistration_2026-08-10.json")
PREREG_SHA = "1b4e121fed923d600aa27721fce289c960f76049aa839c0b908b5d6c4f4eaae7"
SUPPORT = Path("results/high_volatility_trade_arrival_memory_continuation_support_2026-08-10.json")
SUPPORT_SHA = "2f588ed548f4083ac834193163acb60f9ee71f9ce74acadbb744f5d87ccf3c05"
NOVELTY = Path("results/high_volatility_trade_arrival_memory_continuation_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "01c222d91f2f303b5f4b750a7ff3a63d115a11dc50c7af76c0c1aead84d1bbb5"
CLOCK = Path("data/high_volatility_trade_arrival_memory_continuation_clocks_2023_2026.csv.gz")
CLOCK_SHA = "c09ee3a0a38a5044c915e3a8fa304de724e9c8dcd8aa6961c32de21e7d519440"
CONTROL_DIR = Path("data/high_volatility_trade_arrival_memory_continuation_controls_2023_2026")
FREEZE = Path("results/high_volatility_trade_arrival_memory_continuation_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_trade_arrival_memory_continuation_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_memory_tail_gate", "no_variation_gate", "quote_volume_memory", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTAMC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTAMC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvtamc_8_sequential_economics_v1", "policy_id": POLICY_ID,
