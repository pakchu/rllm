"""Sequential strict economics for frozen HVPHR-24."""
...
PREREG = Path("results/high_volatility_peak_candle_range_relay_preregistration_2026-08-09.json")
PREREG_SHA = "eac1e9956b1fd25168a058c7591ff4a3ae33fb2e6dc1df29b885bf564a65d6e0"
SUPPORT = Path("results/high_volatility_peak_candle_range_relay_support_2026-08-09.json")
SUPPORT_SHA = "068df91776d03cfa1b562236324ca72a3fffe0bdf23eee58c18d12aa50d98421"
NOVELTY = Path("results/high_volatility_peak_candle_range_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "cddedc39b4d20c323506c02ce4e6646c416b1bc3915ae31dbe5b7f61d2cf80ad"
CLOCK = Path("data/high_volatility_peak_candle_range_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "3747543efde2ca34888f567c27a48a8b3764168591b63d2824f161190b9ef0d5"
CONTROL_DIR = Path("data/high_volatility_peak_candle_range_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_peak_candle_range_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_peak_candle_range_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "first_peak_tie_break", "current_candle_range", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVPHR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVPHR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvphr_24_sequential_economics_v1", "policy_id": POLICY_ID,
