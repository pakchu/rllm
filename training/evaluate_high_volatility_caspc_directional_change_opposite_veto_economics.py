"""Sequential strict economics for frozen HVCASPCDCV-8."""
...
POLICY_ID = "HVCASPCDCV-8"
BAR = pd.Timedelta(minutes=5)
LEVERAGE = 0.5
BASE_COST = 0.0006
STRESS_COST = 0.0010
ENV_FILE = "/home/pakchu/rllm/.env"
PREREG = Path("results/high_volatility_caspc_directional_change_opposite_veto_preregistration_2026-08-17.json")
PREREG_SHA = "91fdec4432ce9572d088f2173e2d97a9049bf750ab22f539a6af62514ce64826"
SUPPORT = Path("results/high_volatility_caspc_directional_change_opposite_veto_support_2026-08-17.json")
SUPPORT_SHA = "26ec272011660d9d90da55184848822c9bb7089e85bc3bd2bb149b0da4216da5"
NOVELTY = Path("results/high_volatility_caspc_directional_change_opposite_veto_gross9_novelty_2026-08-17.json")
NOVELTY_SHA = "b2c97983f6b70c8fe3fd93b8623d3a085a7c244e04483b5400e1577ff9d6f8d8"
CLOCK = Path("data/high_volatility_caspc_directional_change_opposite_veto_clocks_2023_2026.csv.gz")
CLOCK_SHA = "7051dcddc2c5b2473fe86f3e7aec62f87aa5f87fe7f5d64b786653f4c7a26073"
CONTROL_DIR = Path("data/high_volatility_caspc_directional_change_opposite_veto_controls_2023_2026")
FREEZE = Path("results/high_volatility_caspc_directional_change_opposite_veto_economic_evaluator_freeze_2026-08-17.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_caspc_directional_change_opposite_veto_{stage}_economics_2026-08-17.json")
    for stage in STAGES
}
CONTROLS = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCASPCDCV frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCASPCDCV novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcaspcdcv_8_sequential_economics_v1", "policy_id": POLICY_ID,
