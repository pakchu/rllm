"""Sequential strict economics for frozen HVQST-24."""
...
PREREG = Path("results/high_volatility_qstick_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "6cbcbe6bc9872ad47a3a93f1aadeb63002bcea9139dc9cb8405ffd8329965a9f"
SUPPORT = Path("results/high_volatility_qstick_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "e378d09fa87cbfb3742a788b5ca86935619d0757e516f0e189e62726ef3bfd0b"
NOVELTY = Path("results/high_volatility_qstick_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "3c8ca46bc94c1ef4202fe668f2f368956a59a883ac45b77853f0ff48cdb7524d"
CLOCK = Path("data/high_volatility_qstick_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "163dfa0ab15dffff091c93e47a0e9c06edb2b280e948db5056a4a71442746693"
CONTROL_DIR = Path("data/high_volatility_qstick_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_qstick_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_qstick_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "raw_body_direction", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVQST frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVQST novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvqst_24_sequential_economics_v1", "policy_id": POLICY_ID,
