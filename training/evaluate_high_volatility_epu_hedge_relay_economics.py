"""Sequential strict economics for frozen HVEPUH-24."""
...
PREREG = Path("results/high_volatility_epu_hedge_relay_preregistration_2026-08-09.json")
PREREG_SHA = "12292e59ef6643d2835da872ff733526bce2e14661c936dbc38f82226e3b48e6"
SUPPORT = Path("results/high_volatility_epu_hedge_relay_support_2026-08-09.json")
SUPPORT_SHA = "15bce53d6b4259f8acdf163f22263f1f108cc69b7a35d3ebfabb17ab7f9236ae"
NOVELTY = Path("results/high_volatility_epu_hedge_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "ac2f358c36d9f3623209562cdc86cf51b4e5d0ca4b17e79beace6116ba7ecaf1"
CLOCK = Path("data/high_volatility_epu_hedge_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "123f330cbc65a3bda55b5eeff24c01cd30f6fa67970bd450e1be64c844de358e"
CONTROL_DIR = Path("data/high_volatility_epu_hedge_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_epu_hedge_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_epu_hedge_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_epu_tail", "no_btc_variation_gate", "one_day_stale_epu", "direction_flip", "same_clock_forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVEPUH frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVEPUH novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvepuh_24_sequential_economics_v1", "policy_id": POLICY_ID,
