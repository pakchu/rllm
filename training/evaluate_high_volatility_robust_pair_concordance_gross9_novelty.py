"""Evaluate the frozen HVRC-8 action-vote clock against Gross9."""
...
from training import preregister_high_volatility_robust_pair_concordance as prereg

POLICY = "HVRC-8"
PROTOCOL = "hvrc_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_robust_pair_concordance_clocks_2023_2026.csv.gz",
    "sha256": "3d616adee222de68e2bdf0d792773ef96ed599f661730224f7a1a5a754add0a5",
    "rows": 71,
}
OUTPUT = Path("results/high_volatility_robust_pair_concordance_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVRC-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVRC-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVRC-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVRC-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVRC-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVRC-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVRC-8 unchanged before economics",
    }
