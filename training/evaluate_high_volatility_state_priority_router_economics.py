"""Strict staged economics and train-only selection for HVSPR-8."""
...
PREREG = Path("results/high_volatility_state_priority_router_preregistration_2026-08-14.json")
...
GROSS9 = Path("results/high_volatility_state_priority_router_gross9_novelty_2026-08-14.json")
GROSS9_SHA = "761d738b06eb5a7cc02253ce8a536cac421d1bac27aa0fc0da04c320c1c364ec"
...
CLOCKS = {
    "HVTFR-8__THEN__HVSVF-8__THEN__HVRSSR-8__ELIGIBLE_BY__HVTCCR-8": {
        "path": Path("data/high_volatility_state_priority_router_clocks_2023_2026/HVTFR-8__THEN__HVSVF-8__THEN__HVRSSR-8__ELIGIBLE_BY__HVTCCR-8.csv.gz"),
        "sha256": "d15453de52f76eaf4e0610f9b84cac70e83c580ab732749820258ab902d83d81",
        "rows": 181,
    },
    "HVTFR-8__THEN__HVSVF-8__THEN__HVRSSR-8__ELIGIBLE_BY__HVLZC-8": {
        "path": Path("data/high_volatility_state_priority_router_clocks_2023_2026/HVTFR-8__THEN__HVSVF-8__THEN__HVRSSR-8__ELIGIBLE_BY__HVLZC-8.csv.gz"),
        "sha256": "d73262d5e6da57549a13fd585f0f8eefacbc860327dfd5f7cc93c0a5251833e4",
        "rows": 268,
    },
    "HVRSSR-8__THEN__HVSVF-8__THEN__HVTFR-8__ELIGIBLE_BY__HVTCCR-8": {
        "path": Path("data/high_volatility_state_priority_router_clocks_2023_2026/HVRSSR-8__THEN__HVSVF-8__THEN__HVTFR-8__ELIGIBLE_BY__HVTCCR-8.csv.gz"),
        "sha256": "10c91f140758a1f9b15ea7d55509ebac7664ec0f79a7636b9c5dbf403a862852",
        "rows": 181,
    },
    "HVRSSR-8__THEN__HVSVF-8__THEN__HVTFR-8__ELIGIBLE_BY__HVLZC-8": {
        "path": Path("data/high_volatility_state_priority_router_clocks_2023_2026/HVRSSR-8__THEN__HVSVF-8__THEN__HVTFR-8__ELIGIBLE_BY__HVLZC-8.csv.gz"),
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_state_priority_router_{stage}_economics_2026-08-14.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVSPR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVSPR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVSPR-8 {label} hash drift")
    for candidate, binding in CLOCKS.items():
        if sha256(binding["path"]) != binding["sha256"]:
            raise RuntimeError(f"HVSPR-8 eligible clock hash drift: {candidate}")
    ...
    if (
        ...
        raise RuntimeError("HVSPR-8 preregistered economics drift")
    ...
    for candidate, binding in CLOCKS.items():
        ...
        if (
            ...
            raise RuntimeError(f"HVSPR-8 source-supported candidate drift: {candidate}")
    ...
    if (
        ...
        raise RuntimeError("HVSPR-8 Gross9 did not authorize economics")
    for candidate in ELIGIBLE_CANDIDATES:
        ...
        if (
            ...
            raise RuntimeError(f"HVSPR-8 Gross9 candidate drift: {candidate}")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVSPR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVSPR-8 predecessor did not pass with the frozen winner")
    ...
    if earlier_stage is None:
        if report.get("predecessor") is not None or report.get("selection", {}).get("raw_train_rank_one") != winner:
            raise RuntimeError("HVSPR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link or earlier.get("frozen_train_winner") != winner:
            raise RuntimeError("HVSPR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvspr_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": winner,
        "stage": stage,
