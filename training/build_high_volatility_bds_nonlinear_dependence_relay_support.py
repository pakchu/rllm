"""Source-only support gate for frozen HVBDS-8."""
...
from training import preregister_high_volatility_bds_nonlinear_dependence_relay as prereg
...
PREREG_SHA = "9e10c67e2f1c589e3467abafa53b77f85842e9134dfddcdca6fec831aa2977cc"
SPLITS = {
...
ROOT = Path("data/high_volatility_bds_nonlinear_dependence_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_bds_nonlinear_dependence_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_bds_nonlinear_dependence_relay_controls_2023_2026")
...
def bds_departure(returns: np.ndarray) -> tuple[float, float, float]:
    ...
    first = standardized[:-1]
    histories = np.column_stack((standardized[:-1], standardized[1:]))
    left, right = np.triu_indices(len(histories), 1)
    epsilon = float(prereg.build()["policy"]["epsilon_standard_deviations"])
    c1 = float(np.mean(np.abs(first[left] - first[right]) < epsilon))
    c2 = float(np.mean(np.max(np.abs(histories[left] - histories[right]), axis=1) < epsilon))
    departure = float(c2 - c1 * c1)
    lag1 = float(np.corrcoef(values[:-1], values[1:])[0, 1])
    if not np.isfinite([departure, c1, c2, lag1]).all():
        return math.nan, math.nan, math.nan
    return departure, c1, c2
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVBDS source timestamps")
    frame = frame.set_index("ts").sort_index()
    rows: list[dict[str, Any]] = []
    first_decision = START.normalize() + pd.Timedelta("3h")
    for decision in pd.date_range(first_decision, END, freq="8h", inclusive="left"):
        ...
        if valid:
            opens = window["open"].to_numpy(float).reshape(96, 5)[:, 0]
            closes = window["close"].to_numpy(float).reshape(96, 5)[:, -1]
            five_minute_returns = np.log(closes / opens)
            departure, c1, c2 = bds_departure(five_minute_returns)
            variation = float(np.sqrt(np.square(five_minute_returns).sum()))
            lag1 = float(np.corrcoef(five_minute_returns[:-1], five_minute_returns[1:])[0, 1])
            ...
            direction_confirmed = block_return != 0 and late_return != 0 and np.sign(block_return) == np.sign(late_return)
            valid = (
                variation > 0 and departure > 0 and math.isfinite(lag1)
                and math.isfinite(c1) and math.isfinite(c2)
                and math.isfinite(block_return) and math.isfinite(late_return)
            )
        if not valid:
            variation = departure = c1 = c2 = lag1 = block_return = late_return = math.nan
        ...
        rows.append(
            {
                ...
                "bds_departure": departure,
                "correlation_integral_1": c1,
                "correlation_integral_2": c2,
    ...
    states["departure_rank"] = prior_rank(states["bds_departure"].where(states["source_valid"]))
    states["lag1_rank"] = prior_rank(states["lag1_autocorrelation"].where(states["source_valid"]))
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvbds_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    departure_rank = states["departure_rank"]
    departure = states["bds_departure"]
    lag1_rank = states["lag1_rank"]
    if control == "one_decision_stale_dependence":
        departure_rank, departure, lag1_rank = [value.shift(1) for value in (departure_rank, departure, lag1_rank)]
    variation_gate = (
        pd.Series(True, index=states.index)
        if control == "no_variation_gate"
        else variation_rank.ge(0.65)
    )
    if control == "no_departure_tail":
        dependence_gate = pd.Series(True, index=states.index)
    elif control == "linear_lag1_autocorrelation_tail":
        dependence_gate = lag1_rank.ge(0.75)
    else:
        dependence_gate = departure_rank.ge(0.75)
    eligible = (
        states["source_valid"] & states["direction_confirmed"]
        & variation_gate & dependence_gate & block_return.ne(0)
    )
        ...
        rows.append(
            {
                ...
                "bds_departure": float(states.at[index, "bds_departure"]),
                "departure_rank": float(states.at[index, "departure_rank"]),
            }
        )
    columns = [
        "candidate", "control", "split", "decision_time", "feature_available_time",
        "entry_time", "exit_time", "side", "realized_variation", "variation_rank",
        "bds_departure", "departure_rank",
    ]
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvbds_8_source_support_v1",
        "policy_id": "HVBDS-8",
        "preregistration": {
