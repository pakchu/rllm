"""Source-only support gate for frozen HVLFE-8."""
...
from training import preregister_high_volatility_low_frequency_energy_relay as prereg
...
PREREG_SHA = "a4a6c0d2e0e6196aeda54536008372f322459a1aa147b947b3850973fac7e5cf"
SPLITS = {
...
ROOT = Path("data/high_volatility_low_frequency_energy_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_low_frequency_energy_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_low_frequency_energy_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_low_frequency_energy_relay_support_2026-08-10.json")
...
def low_frequency_share(close: np.ndarray) -> float:
    ...
    non_dc_energy = float(energy[1:].sum())
    if non_dc_energy <= 0 or not math.isfinite(non_dc_energy):
        return math.nan
    value = float(energy[1:9].sum()) / non_dc_energy
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVLFE source timestamps")
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        if valid:
            ...
            frequency_share = low_frequency_share(window["close"].to_numpy(float))
            ...
            valid = variation > 0 and math.isfinite(frequency_share)
        if not valid:
            variation = frequency_share = block_return = late_return = math.nan
        rows.append(
            {
                "decision_time": decision,
                "source_valid": valid,
                "realized_variation": variation,
                "low_frequency_share": frequency_share,
    ...
    states["low_frequency_rank"] = prior_rank(
        states["low_frequency_share"].where(states["source_valid"])
    )
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvlfe_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    frequency_rank = states["low_frequency_rank"]
    frequency_share = states["low_frequency_share"]
    if control == "one_block_stale_geometry":
        block_return, late_return, variation_rank, frequency_rank, frequency_share = [
            value.shift(1)
            for value in (block_return, late_return, variation_rank, frequency_rank, frequency_share)
        ]
    variation_gate = (
    ...
    if control == "no_low_frequency_tail":
        frequency_gate = pd.Series(True, index=states.index)
    elif control == "raw_low_frequency_share_nonnegative":
        frequency_gate = frequency_share.ge(0)
    else:
        frequency_gate = frequency_rank.ge(0.75)
    agreement = (
        block_return.ne(0)
        & late_return.ne(0)
        & np.sign(block_return).eq(np.sign(late_return))
    )
    eligible = states["source_valid"] & variation_gate & frequency_gate & agreement
        ...
        rows.append(
            {
                ...
                "low_frequency_share": float(states.at[index, "low_frequency_share"]),
                "low_frequency_rank": float(states.at[index, "low_frequency_rank"]),
            }
        )
    columns = [
        "candidate", "control", "split", "decision_time", "feature_available_time",
        "entry_time", "exit_time", "side", "realized_variation", "variation_rank",
        "low_frequency_share", "low_frequency_rank",
    ]
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVLFE preregistration drift")
    ...
    core = {
        "protocol_version": "hvlfe_8_source_support_v1",
        "policy_id": "HVLFE-8",
        "preregistration": {
