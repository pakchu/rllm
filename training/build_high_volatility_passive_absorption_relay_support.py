"""Materialize source-only HVPAR-6 clocks."""
...
SOURCE_DIR = Path("data/high_volatility_passive_absorption_relay_sources_2023_2026")
PANEL = SOURCE_DIR / "eight_hour_passive_absorption_panel.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_passive_absorption_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_passive_absorption_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_passive_absorption_relay_support_2026-08-08.json")
SPLITS = {
...
COLUMNS = (
    ...
    "exit_time",
    "side",
    "late_open","late_close","late_return","late_quote_volume","late_taker_buy_quote","late_taker_imbalance",
    "full_variation",
    "variation_rank",
)
QUERY = """
...
def _passive_absorption_panel(bars: pd.DataFrame) -> pd.DataFrame:
    bars = bars.copy()
    ...
    for decision in decisions:
        ...
        if valid:
            late=window.iloc[360:];late_open=float(late.open.iloc[0]);late_close=float(late.close.iloc[-1]);late_return=float(np.log(late_close/late_open));late_quote_volume=float(late.quote_asset_volume.sum());late_taker_buy_quote=float(late.taker_buy_quote.sum());late_taker_imbalance=(2*late_taker_buy_quote-late_quote_volume)/late_quote_volume;full_variation=float(np.log(window.close.astype(float)).diff().dropna().pow(2).sum());valid=full_variation>0 and late_quote_volume>0
        else:
            late_open=late_close=late_return=late_quote_volume=late_taker_buy_quote=late_taker_imbalance=full_variation=float("nan")
        rows.append(
            {
                "decision_time": decision,
                "source_rows": int(window.notna().all(axis=1).sum()),
                "source_valid": valid,
                "late_open":late_open,"late_close":late_close,"late_return":late_return,"late_quote_volume":late_quote_volume,"late_taker_buy_quote":late_taker_buy_quote,"late_taker_imbalance":late_taker_imbalance,"full_variation":full_variation,
            }
        )
...
def materialize() -> dict[str, Any]:
    ...
    database.dispose()
    panel = _passive_absorption_panel(bars)
    SOURCE_DIR.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(panel, PANEL)
    core = {
        "protocol_version": "hvpar_6_btc_source_v1",
        "query": QUERY,
        "table": "bars_binance",
        "symbol": "BTCUSDT",
        "interval": "1m",
...
def conditions(frame: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    late=frame.late_return;imbalance=frame.late_taker_imbalance
    rank = frame.variation_rank
    if control == "one_boundary_stale_geometry":
        late=late.shift(1);imbalance=imbalance.shift(1)
        rank = rank.shift(1)
    volatility_gate = (
    ...
    magnitude_gate=pd.Series(True,index=frame.index) if control=="no_flow_magnitude_gate" else imbalance.abs().ge(.1);long=late.gt(0)&imbalance.lt(0);short=late.lt(0)&imbalance.gt(0)
    active = (
        frame.signal_valid
        & np.isfinite(late)&np.isfinite(imbalance)
        & np.isfinite(rank)
        & volatility_gate
        & magnitude_gate
        & ((late.ne(0) & imbalance.ne(0)) if control == "no_contradiction_gate" else (long | short))
    )
        ...
        rows.append(
            {
                ...
                "late_open":float(frame.at[index,"late_open"]),"late_close":float(frame.at[index,"late_close"]),"late_return":float(frame.at[index,"late_return"]),"late_quote_volume":float(frame.at[index,"late_quote_volume"]),"late_taker_buy_quote":float(frame.at[index,"late_taker_buy_quote"]),"late_taker_imbalance":float(frame.at[index,"late_taker_imbalance"]),
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvpar_6_source_support_v1",
        "policy_id": "HVPAR-6",
        "preregistration": {
