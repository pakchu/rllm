"""Sequential strict economics for frozen HVOBV-24."""
...
PREREG = Path("results/high_volatility_on_balance_volume_signal_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "5ac86c269566756ded9bc40d7fc3464e0c324f145ba52f7e118d95291b35bf26"
SUPPORT = Path("results/high_volatility_on_balance_volume_signal_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "f1b9c71459e3df7c1c37a906cda3599cd9f6dbdf77d1c5479d686414a1018a44"
NOVELTY = Path("results/high_volatility_on_balance_volume_signal_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "88795f67d447df25854cad3dc491ed454ae284bfa6266223a43f125b503257ef"
CLOCK = Path("data/high_volatility_on_balance_volume_signal_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "825c4e5a69e8660b48263411bc1f22707fd9e83ca1256bc6ffb88a8e2ecf357c"
CONTROL_DIR = Path("data/high_volatility_on_balance_volume_signal_crossover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_on_balance_volume_signal_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_on_balance_volume_signal_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "signed_volume_zero_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVOBV frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVOBV novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvobv_24_sequential_economics_v1", "policy_id": POLICY_ID,
