"""Build outcome-blind source support for frozen HVODHR-8."""
...
from training import preregister_high_volatility_opening_drive_hold_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-04-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="0f35623f7a7131600c564db427afe4635320eb1fac2e53cb66a6647cc4ebce0b"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_opening_drive_hold_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_opening_drive_hold_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_opening_drive_hold_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_opening_drive_hold_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_opening_drive_hold_relay_support_2026-08-11.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","opening_high","opening_low","opening_midpoint","opening_open","opening_close","opening_direction","acceptance_min_close","acceptance_max_close","terminal_close","accepted_side","no_midpoint_side","terminal_break_side","stale_opening_range_side","realized_variation","variation_rank","eligible")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","opening_high","opening_low","opening_midpoint","opening_open","opening_close","opening_direction","acceptance_min_close","acceptance_max_close","terminal_close","accepted_side","no_midpoint_side","terminal_break_side","stale_opening_range_side","realized_variation","variation_rank")
...
def opening_drive_pattern(opening:pd.DataFrame,acceptance:pd.DataFrame,opening_high:float|None=None,opening_low:float|None=None)->tuple[float,float,float,float,float,int,float,float,float,int,int,int]:
 high=float(opening.high.max()) if opening_high is None else float(opening_high);low=float(opening.low.min()) if opening_low is None else float(opening_low);midpoint=(high+low)/2
 opening_open=float(opening.open.iloc[0]);opening_close=float(opening.close.iloc[-1]);direction=1 if opening_close>opening_open else -1 if opening_close<opening_open else 0
 minimum=float(acceptance.close.min());maximum=float(acceptance.close.max());terminal=float(acceptance.close.iloc[-1])
 terminal_break=1 if terminal>high else -1 if terminal<low else 0
 no_midpoint=direction if direction!=0 and terminal_break==direction else 0
 held=(direction==1 and minimum>midpoint) or (direction==-1 and maximum<midpoint)
 accepted=no_midpoint if held else 0
 return high,low,midpoint,opening_open,opening_close,direction,minimum,maximum,terminal,accepted,no_midpoint,terminal_break
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVODHR source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVODHR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 source=prepare(raw);rows=[];previous_opening=None
 for decision in pd.date_range(START+pd.Timedelta("2h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   opening=block.iloc[:60];acceptance=block.iloc[60:];values=opening_drive_pattern(opening,acceptance);opening_high,opening_low,opening_midpoint,opening_open,opening_close,opening_direction,acceptance_min_close,acceptance_max_close,terminal_close,side,no_midpoint,terminal_break=values
   stale_side=0 if previous_opening is None else opening_drive_pattern(opening,acceptance,*previous_opening)[9]
   minute_returns=np.log(block.close.to_numpy(float)/block.open.to_numpy(float));var=float(np.sqrt(np.square(minute_returns).sum()));valid=all(math.isfinite(v) for v in (opening_high,opening_low,var)) and opening_high>opening_low and var>0;previous_opening=(opening_high,opening_low)
  else:opening_high=opening_low=opening_midpoint=opening_open=opening_close=acceptance_min_close=acceptance_max_close=terminal_close=var=math.nan;opening_direction=side=no_midpoint=terminal_break=stale_side=0;valid=False;previous_opening=None
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"opening_high":opening_high,"opening_low":opening_low,"opening_midpoint":opening_midpoint,"opening_open":opening_open,"opening_close":opening_close,"opening_direction":opening_direction,"acceptance_min_close":acceptance_min_close,"acceptance_max_close":acceptance_max_close,"terminal_close":terminal_close,"accepted_side":side,"no_midpoint_side":no_midpoint,"terminal_break_side":terminal_break,"stale_opening_range_side":stale_side,"realized_variation":var})
...
def active(panel:pd.DataFrame,control:str="primary"):
 ...
 if control=="no_midpoint_hold":side=pd.to_numeric(used.no_midpoint_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
 elif control=="no_variation_gate":state=valid&side.ne(0)
 elif control=="terminal_break_only":side=pd.to_numeric(used.terminal_break_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
 elif control=="one_block_stale_opening_range":side=pd.to_numeric(used.stale_opening_range_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVODHR prereg drift")
 ...
 source_core={"protocol_version":"hvodhr_8_source_v1","query":QUERY,"query_sha256":hashlib.sha256(QUERY.encode()).hexdigest(),"window":[START.isoformat(),END.isoformat()],"physical_rows":len(raw),"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"panel":{"path":str(PANEL),"sha256":sha(PANEL),"rows":len(panel),"valid_rows":int(panel.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};manifest={**source_core,"manifest_hash":chash(source_core)};immutable(MANIFEST,json_bytes(manifest))
