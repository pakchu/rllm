"""Evaluate frozen ECVRS-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "ECVRS-12"
PROTOCOL = "ecvrs_12_gross9_novelty_v1"
...
OUTPUT = Path("results/equity_commodity_volatility_residual_shock_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("ECVRS predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("ECVRS novelty limits drift")
    if (
        ...
        raise RuntimeError("ECVRS predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="ECVRS primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "ecvrs_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject ECVRS-12 unchanged before economic outcomes",
    }
