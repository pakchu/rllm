"""Build outcome-blind source support for frozen HVPRAPR-12."""
...
from training import preregister_high_volatility_relative_premium_activity_polarity_router as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv


ENV_FILE = "/home/pakchu/rllm/.env"
PREREG_SHA = "c12f2cc6ef2720f078b11248b82b2dc677d9c1af616fd034ec8aa2ccd321d169"
START = pd.Timestamp("2023-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
SOURCE_DIR = Path("data/high_volatility_relative_premium_activity_polarity_router_sources_2023_2026")
FEATURES = SOURCE_DIR / "daily_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_relative_premium_activity_polarity_router_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_relative_premium_activity_polarity_router_controls_2023_2026")
RESULT = Path("results/high_volatility_relative_premium_activity_polarity_router_support_2026-08-16.json")
SPLITS = {
...
CLOCK_COLUMNS = (
    ...
    "premium_total_variation", "relative_premium_activity", "relative_premium_activity_rank", "btc_return",
...
def build_features(premium: pd.DataFrame, btc: pd.DataFrame) -> pd.DataFrame:
    ...
    frame["relative_premium_activity"] = (
        pd.to_numeric(frame.premium_total_variation, errors="coerce")
        / pd.to_numeric(frame.btc_realized_variation, errors="coerce")
    ).where(frame.source_valid)
    frame["relative_premium_activity_rank"] = strict_prior_midrank(
        frame["relative_premium_activity"].where(frame.source_valid)
    )
...
def conditions(frame: pd.DataFrame, control: str = "primary") -> tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    premium_gate = (
        ...
        else (used.relative_premium_activity_rank.le(0.25) | used.relative_premium_activity_rank.ge(0.75))
    )
    variation_gate = (
        pd.Series(True, index=frame.index)
    ...
    if control == "primary":
        upper = used.relative_premium_activity_rank.ge(0.75)
        side = side.where(~upper, -side)
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active]:
        ...
        rows.append({
            "candidate": "HVPRAPR-12", "control": control, "split": split,
            ...
            "relative_premium_activity": float(source.relative_premium_activity),
            "relative_premium_activity_rank": float(source.relative_premium_activity_rank),
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVPRAPR preregistration drift")
    ...
    source_core = {
        "protocol_version": "hvprapr_12_sources_v1",
    ...
    core = {
        "protocol_version": "hvprapr_12_source_support_v1", "policy_id": "HVPRAPR-12",
        ...
        "completed_preentry_sources_opened": True, "candidate_incidence_opened": True, "postentry_return_pnl_execution_price_opened": False, "held_interval_funding_values_opened": False, "gross9_rows_opened": False,
