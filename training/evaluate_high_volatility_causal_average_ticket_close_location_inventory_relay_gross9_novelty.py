"""Evaluate the frozen HVCATCLIR-8 action-vote clock against Gross9."""
...
POLICY = "HVCATCLIR-8"
PROTOCOL = "hvcatclir_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_average_ticket_close_location_inventory_relay_clocks_2020_2026.csv.gz",
    "sha256": "754949b6b3f12b1e82a3d66d470d1a45287e86a8c983661948cb534d04c26785",
    "rows": 112,
}
OUTPUT = Path("results/high_volatility_causal_average_ticket_close_location_inventory_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCATCLIR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCATCLIR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATCLIR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCATCLIR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCATCLIR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCATCLIR-8 unchanged before economics",
    }
