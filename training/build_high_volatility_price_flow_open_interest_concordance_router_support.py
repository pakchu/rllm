"""Deterministic outcome-blind source support for HVPFOICR-8."""
...
from training import preregister_high_volatility_price_flow_open_interest_concordance_router as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "047536b94c59286fbfa6786e950f27d673b65e552f0e17b542078ad0d3133fb5"
...
ROOT = Path("data/high_volatility_price_flow_open_interest_concordance_router_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_price_flow_open_interest_concordance_router_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_price_flow_open_interest_concordance_router_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_price_flow_open_interest_concordance_router_support_2026-08-16.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "sum_open_interest",
    "previous_open_interest", "oi_change", "price_return", "flow_share",
    "realized_variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "decision_time", "feature_available_time",
    "entry_time", "exit_time", "side", "oi_change", "price_return",
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVPFOICR-8 OI schema drift")
    ...
    if bars.columns.tolist() != expected:
        raise RuntimeError("HVPFOICR-8 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all() or not oi["sum_open_interest"].ge(0).all():
        raise RuntimeError("HVPFOICR-8 invalid OI rows")
    bars = bars.copy()
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel["source_valid"] = (panel["previous_time"].notna() & panel["decision_time"].sub(panel["previous_time"]).eq(pd.Timedelta("8h")) & panel["bar_valid"].eq(True) & np.isfinite(panel[["sum_open_interest","previous_open_interest","oi_change","price_return","flow_share","realized_variation"]]).all(axis=1) & panel["sum_open_interest"].gt(0) & panel["previous_open_interest"].gt(0) & panel["price_return"].ne(0) & panel["flow_share"].ne(0) & panel["realized_variation"].gt(0))
    valid=panel["source_valid"].eq(True); panel["variation_rank"]=causal_midrank(panel["realized_variation"].where(valid))
    panel["eligible"]=(valid & panel["oi_change"].ne(0) & np.sign(panel["price_return"]).mul(np.sign(panel["flow_share"])).eq(np.sign(panel["oi_change"])) & panel["variation_rank"].ge(POLICY["variation_rank_min"]))
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVPFOICR-8 side or availability drift")
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVPFOICR-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvpfoicr_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvpfoicr_8_source_support_v1", "policy_id": prereg.POLICY_ID,
