"""Sequential strict economics for frozen LEMRR-16."""
...
PREREG = Path("results/late_extreme_midpoint_rejection_relay_preregistration_2026-08-09.json")
PREREG_SHA = "7153527a55247b42eae6214f1505acc629eff3569df70949b3a94df475c0ba7f"
SUPPORT = Path("results/late_extreme_midpoint_rejection_relay_support_2026-08-09.json")
SUPPORT_SHA = "aab0cb26c40edbb15ed6296c2a349f60327ee5a396d7c4fa6d36d7126bae5ef6"
NOVELTY = Path("results/late_extreme_midpoint_rejection_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "13823e398bf69767f0155cc543ae11ff8e778a4bc7a4a4119d8648b0adadb49c"
CLOCK = Path("data/late_extreme_midpoint_rejection_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "e7630dcbb6888f3530625258dc088910b346282c6094d836ccd89720bc3d274a"
CONTROL_DIR = Path("data/late_extreme_midpoint_rejection_relay_controls_2023_2026")
FREEZE = Path("results/late_extreme_midpoint_rejection_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/late_extreme_midpoint_rejection_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_midpoint_rejection", "close_location_only", "close_open_fade", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("LEMRR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("LEMRR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "lemrr_16_sequential_economics_v1", "policy_id": POLICY_ID,
