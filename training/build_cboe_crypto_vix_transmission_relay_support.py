"""Build source-support clocks for CCVTR-6 without BTC outcomes."""
...
CLOCK=Path("data/cboe_crypto_vix_transmission_relay_clocks_2023_2026.csv.gz");CONTROL_DIR=Path("data/cboe_crypto_vix_transmission_relay_controls_2023_2026");RESULT=Path("results/cboe_crypto_vix_transmission_relay_support_2026-08-08.json")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00:00Z"),pd.Timestamp("2024-01-01T00:00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00:00Z"),pd.Timestamp("2025-01-01T00:00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00:00Z"),pd.Timestamp("2026-01-01T00:00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00:00Z"),pd.Timestamp("2026-08-01T00:00:00Z"))};MIN={"train":8,"test":12,"eval":12,"final":8};CONTROLS=("no_crypto_confirmation","bvol_only_confirmation","dvol_only_confirmation","one_session_stale_vix_change","direction_flip");NY=ZoneInfo("America/New_York");ECONOMIC_OUTCOMES_AUTHORIZED=False
COLUMNS=("candidate","control","split","cboe_observation_date","next_cboe_source_date","decision_time","feature_available_time","entry_time","exit_time","side","delta_vix","previous_delta_vix","bvol_body","dvol_body")
...
def features()->pd.DataFrame:
 for raw,want in prereg.HASHES.items():
  if sha(Path(raw))!=want:raise RuntimeError(f"CCVTR source drift: {raw}")
 c=pd.read_csv(prereg.CBOE,compression="gzip",usecols=["observation_date","VIX_close"]);c.observation_date=pd.to_datetime(c.observation_date,format="%Y-%m-%d");c.VIX_close=pd.to_numeric(c.VIX_close,errors="coerce")
 if c.observation_date.duplicated().any() or not c.observation_date.is_monotonic_increasing or not np.isfinite(c.VIX_close).all() or not c.VIX_close.gt(0).all():raise RuntimeError("CCVTR Cboe source invalid")
 c["delta_vix"]=np.log(c.VIX_close).diff();c["previous_delta_vix"]=c.delta_vix.shift(1)
 ...
 for i in range(1,len(c)-1):
  ...
  x=v.loc[decision];rows.append({"observation_date":c.at[i,"observation_date"].date(),"next_source_date":nd.date(),"decision_time":decision,"delta_vix":float(c.at[i,"delta_vix"]),"previous_delta_vix":float(c.at[i,"previous_delta_vix"]),"bvol_body":float(x.bvol_body),"dvol_body":float(x.dvol_body),"valid":bool(x.bvol_valid and x.dvol_valid)})
 return pd.DataFrame(rows)
def sides(f:pd.DataFrame,control:str)->pd.Series:
 delta=f.previous_delta_vix if control=="one_session_stale_vix_change" else f.delta_vix;valid=f.valid&delta.notna()&delta.ne(0)&f.bvol_body.ne(0)&f.dvol_body.ne(0);same_b=np.sign(f.bvol_body).eq(np.sign(delta));same_d=np.sign(f.dvol_body).eq(np.sign(delta))
...
def clock(f:pd.DataFrame,control:str="primary")->pd.DataFrame:
 ...
 for i in f.index[s.ne(0)]:
  ...
  next_allowed=exit_;rows.append({"candidate":"CCVTR-6","control":control,"split":split,"cboe_observation_date":f.at[i,"observation_date"].isoformat(),"next_cboe_source_date":f.at[i,"next_source_date"].isoformat(),"decision_time":decision,"feature_available_time":decision,"entry_time":entry,"exit_time":exit_,"side":int(s.at[i]),"delta_vix":float(f.at[i,"delta_vix"]),"previous_delta_vix":float(f.at[i,"previous_delta_vix"]),"bvol_body":float(f.at[i,"bvol_body"]),"dvol_body":float(f.at[i,"dvol_body"])})
