"""Deterministic source-only support for HVTFDAC-8."""
...
from training import preregister_high_volatility_taker_flow_decay_acceptance_continuation as prereg
...
PREREG_SHA = "8b4561990ea6e8e600949c9d833babed3eaf30f651e4a22dd1ef474ee18df27e"
...
ROOT = Path("data/high_volatility_taker_flow_decay_acceptance_continuation_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_taker_flow_decay_acceptance_continuation_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_taker_flow_decay_acceptance_continuation_split_clocks_2023_2026")
RESULT = Path("results/high_volatility_taker_flow_decay_acceptance_continuation_support_2026-08-16.json")
QUERY = """SELECT ts,open,high,low,close,quote_asset_volume,taker_buy_quote FROM bars_binance
...
def build_panel(frame: pd.DataFrame) -> pd.DataFrame:
    expected = ["ts", "open", "high", "low", "close", "quote_asset_volume", "taker_buy_quote"]
    if frame.columns.tolist() != expected:
        raise RuntimeError("HVTFDAC-8 source schema drift")
    frame = frame.copy()
    frame["ts"] = pd.to_datetime(frame["ts"], utc=True, errors="raise")
    for column in expected[1:]:
        frame[column] = pd.to_numeric(frame[column], errors="raise")
    if frame["ts"].duplicated().any():
        raise RuntimeError("HVTFDAC-8 duplicate timestamps")
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        if valid:
            ...
            qv = float(np.square(returns).sum())
            quote = window["quote_asset_volume"].to_numpy(float)
            signed_taker = 2.0 * window["taker_buy_quote"].to_numpy(float) - quote
            first_flow_imbalance = float(signed_taker[:240].sum() / quote[:240].sum())
            second_flow_imbalance = float(signed_taker[240:].sum() / quote[240:].sum())
            variation = math.sqrt(qv) if qv > 0 else math.nan
            valid = all(math.isfinite(value) for value in (first_return, second_return, first_flow_imbalance, second_flow_imbalance, variation)) and qv > 0
        if not valid:
            first_return = second_return = first_flow_imbalance = second_flow_imbalance = variation = math.nan
        rows.append({
            "decision_time": decision,
            "source_valid": bool(valid),
            "first_half_return": first_return,
            "second_half_return": second_return,
            "first_half_flow_imbalance": first_flow_imbalance,
            "second_half_flow_imbalance": second_flow_imbalance,
    ...
    side = np.sign(panel["first_half_return"])
    first_directional_flow = side * panel["first_half_flow_imbalance"]
    second_directional_flow = side * panel["second_half_flow_imbalance"]
    panel["eligible"] = panel["source_valid"] & agreement & second_directional_flow.gt(0) & second_directional_flow.lt(first_directional_flow) & panel["variation_rank"].ge(POLICY["variation_rank_min"])
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["onset"]].itertuples(index=False):
        ...
        if side not in (-1, 1):
            raise RuntimeError("HVTFDAC-8 side drift")
        ...
        rows.append({
            ...
            "first_half_flow_imbalance": row.first_half_flow_imbalance,
            "second_half_flow_imbalance": row.second_half_flow_imbalance,
            "realized_variation": row.realized_variation, "variation_rank": row.variation_rank,
        })
    columns = ["candidate", "control", "split", "decision_time", "feature_available_time", "entry_time", "exit_time", "side", "first_half_return", "second_half_return", "first_half_flow_imbalance", "second_half_flow_imbalance", "realized_variation", "variation_rank"]
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVTFDAC-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvtfdac_8_sources_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvtfdac_8_source_support_v1", "policy_id": prereg.POLICY_ID,
