"""Sequential strict economics for frozen DFSR-12."""
...
PREREG = Path("results/dollar_factor_shock_relay_preregistration_2026-08-09.json")
PREREG_SHA = "e7a2309c478d0454d61a97c2fd49660b93757923ae2af9c625538dfa5a080b54"
SUPPORT = Path("results/dollar_factor_shock_relay_support_2026-08-09.json")
SUPPORT_SHA = "57018248d49fa0fa2a142adbffe2b42ae1d12fb55b762f84e4334cb8f21d46b1"
NOVELTY = Path("results/dollar_factor_shock_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "19c0270e3c3d2002a011708d4e43afac4d5dff56a1945f8c553c87642bdbae8f"
CLOCK = Path("data/dollar_factor_shock_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "a6b66b030039647bc6e0c7a48893a3c60be29aee5158027dbf9b8c8910bb93dc"
CONTROL_DIR = Path("data/dollar_factor_shock_relay_controls_2023_2026")
FREEZE = Path("results/dollar_factor_shock_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/dollar_factor_shock_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "no_factor_tail", "raw_return_factor", "one_session_stale_factor", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("DFSR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("DFSR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "dfsr_12_sequential_economics_v1", "policy_id": POLICY_ID,
