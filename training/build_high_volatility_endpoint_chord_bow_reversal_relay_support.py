"""Build outcome-blind source support for frozen HVECBR-8."""
...
from training import preregister_high_volatility_endpoint_chord_bow_reversal_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-04-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="db66c8b194c84aff82d595b494aed49711ab0178677e23cc6acbb0c1eac764b4"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_endpoint_chord_bow_reversal_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_endpoint_chord_bow_reversal_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_endpoint_chord_bow_reversal_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_endpoint_chord_bow_reversal_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_endpoint_chord_bow_reversal_relay_support_2026-08-10.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","normalized_integrated_bow","bow_rank","realized_variation","variation_rank","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","normalized_integrated_bow","bow_rank","realized_variation","variation_rank")
...
def bow_statistics(block:pd.DataFrame)->tuple[float,float]:
 points=np.r_[math.log(float(block.open.iloc[0])),np.log(block.close.to_numpy(float))];variation=float(np.sqrt(np.square(np.diff(points)).sum()));indices=np.arange(1,480,dtype=float);chord=points[0]+indices*(points[-1]-points[0])/480.;bow=float(np.mean(points[1:480]-chord)/variation)
 return bow,variation
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVECBR source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVECBR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("5h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   bow,var=bow_statistics(block);valid=all(math.isfinite(v) for v in (bow,var)) and var>0 and bow!=0
  else:bow=var=math.nan;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"normalized_integrated_bow":bow,"realized_variation":var})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["bow_rank"]=prior_rank(panel.normalized_integrated_bow.abs().where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));panel["eligible"]=valid&panel.bow_rank.ge(P["bow_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"]);panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_block_stale_geometry":
  cols=["source_valid","normalized_integrated_bow","bow_rank","variation_rank","feature_available_time"];used[cols]=panel[cols].shift(1)
 valid=used.source_valid.eq(True);strong=used.bow_rank.ge(P["bow_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&strong&variation
 if control=="no_bow_tail":state=valid&variation
 elif control=="no_variation_gate":state=valid&strong
 onset=previous_valid_onset(state,valid);side=-np.sign(pd.to_numeric(used.normalized_integrated_bow,errors="coerce").fillna(0)).astype(int)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVECBR prereg drift")
