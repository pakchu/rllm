"""Outcome-blind preregistration for HVCVMRS-8."""
...
POLICY_ID="HVCVMRS-8";DEFAULT_OUTPUT=Path("results/high_volatility_cross_venue_median_return_shift_consensus_preregistration_2026-08-18.json")
...
def build()->dict[str,Any]:
 g=gates.build();core={"protocol_version":"high_volatility_cross_venue_median_return_shift_consensus_v1","policy_id":POLICY_ID,"as_of_date":"2026-08-18","singleton":True,"candidate_family":[POLICY_ID],"candidate_family_size":1,"source_incidence_opened":False,"outcomes_opened":False,"gross9_rows_opened":False,"primary_artifacts":PRIMARY,
 "mechanism":{"claim":"A robust second-half versus first-half median return shift is more likely broad price discovery when the actual BTC spot auction exhibits the same signed location shift as the perpetual; follow the common shift for eight hours during frozen high variation.","side":"immutable HVMRSR-8 side, emitted only when spot median shift has the same strict sign","why_distinct":"HVMRSR is perpetual-only. Cash/perpetual candidates use endpoint returns, ranges, basis, flow, or lead-lag. HVCVMRS compares the signed median of 240 one-minute open-to-close returns in each half independently on both actual venues and uses no endpoint direction, volume, flow, OI, funding, premium, options, fitted outcome, or promoted control.","why_low_gross9_overlap_is_plausible":"cross-venue robust return-location consensus at irregular HVMRSR events is absent from Gross9"},
 "construction":{"primary":"immutable HVMRSR-8 event, side, entry, and hold","spot_window":"480 exact coherent bars_binance_spot BTCUSDT one-minute rows [D-8h,D)","spot_shift":"median rows 240..479 log(close/open) minus median rows 0..239","confirmation":"spot_shift strict nonzero and sign equals immutable perpetual median-shift side","entry":"immutable D+5m","hold":"8 elapsed hours","additional_or_tuned_thresholds":"none","weights":"none"},
 ...
 "research_boundary":{"HVMRSR_train_single_half_failure_known":True,"HVMRSR_test_eval_final_outcomes_known":False,"exact_cross_venue_consensus_incidence_or_outcomes_known":False,"candidate_incidence_opened":False,"postentry_return_or_pnl_opened":False,"repair_of_prior_candidate":False,"promoted_prior_control":False,"classification":"exploratory discovery; not fresh confirmatory evidence","selection_basis":"independent spot confirmation of robust within-auction location shift"},
 "stopping_rule":"source, Gross9, train/test/eval/final; terminal first failure; no primary, spot source, half partition, median, sign agreement, side, clock, hold, subset, threshold, or control repair"};return {**core,"manifest_hash":canonical_hash(core)}
def validate(x:dict[str,Any])->None:
 if x!=build():raise RuntimeError("HVCVMRS-8 preregistration drift")
 for artifact in PRIMARY.values():
  if sha256(artifact["path"])!=artifact["sha256"]:raise RuntimeError("HVCVMRS-8 primary drift")
