"""Sequential strict economics for frozen HVRLXC-12."""
...
PREREG = Path("results/high_volatility_realized_leverage_cross_moment_relay_preregistration_2026-08-12.json")
PREREG_SHA = "2d6d0e27aa90f054f0c3955fb7c1862d48128fcae045d9b8ce6886b54044f9e4"
SUPPORT = Path("results/high_volatility_realized_leverage_cross_moment_relay_support_2026-08-12.json")
SUPPORT_SHA = "69d30266be033abdc38f67154824d7778c6abf1f2c5f7ed24c81b00c5c0eadc0"
NOVELTY = Path("results/high_volatility_realized_leverage_cross_moment_relay_gross9_novelty_2026-08-12.json")
NOVELTY_SHA = "f1050217c75151297bb87cd732c2ec79cc8cf7bb4f9869db4d31847c0e0d2819"
CLOCK = Path("data/high_volatility_realized_leverage_cross_moment_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "0fc22b5e6632a78fc079383e1fa42fa43e04bd3f6ab35d081a96155a12bb6591"
CONTROL_DIR = Path("data/high_volatility_realized_leverage_cross_moment_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_realized_leverage_cross_moment_relay_economic_evaluator_freeze_2026-08-12.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_realized_leverage_cross_moment_relay_{stage}_economics_2026-08-12.json")
    for stage in STAGES
}
CONTROLS = (
    "no_variation_gate",
    "leverage_direction_flip",
    "one_decision_stale_cross_moment",
    "contemporaneous_return_variance_moment",
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVRLXC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVRLXC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrlxc_12_sequential_economics_v1",
        "policy_id": POLICY_ID,
        "stage": stage,
