"""Evaluate frozen HVMD-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVMD-24"
PROTOCOL = "hvmd_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_mcginley_dynamic_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVMD predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVMD novelty limits drift")
    if (
        ...
        raise RuntimeError("HVMD predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVMD primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvmd_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVMD-24 unchanged before economic outcomes",
    }
