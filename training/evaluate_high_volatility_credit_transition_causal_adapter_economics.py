"""Strict staged economics for the singleton HVCQCA-24 policy."""
...
PREREG = Path("results/high_volatility_credit_transition_causal_adapter_preregistration_2026-08-18.json")
PREREG_SHA = "a949d2c3f447ad676ff33d28829e32de159b0226a8898b5f6de157025cd11252"
SUPPORT = Path("results/high_volatility_credit_transition_causal_adapter_support_2026-08-18.json")
SUPPORT_SHA = "009ce172a48fc20319ec380964ef5a45d4e2697d8e53aebec688fe55a9161c37"
GROSS9 = Path("results/high_volatility_credit_transition_causal_adapter_gross9_novelty_2026-08-18.json")
GROSS9_SHA = "4f933b0979da3b0ff5386647f66163749724a13d1fb0c52b27cd5f4f7cfd33fe"
CLOCK = {
    "path": Path("data/high_volatility_credit_transition_causal_adapter_clocks_2023_2026.csv.gz"),
    "sha256": "a4f1d0e154daed1eeeb156c42619f6238fca2e3c9f9a8f6b3214c3bb7631e9b6",
    "rows": 199,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_credit_transition_causal_adapter_{stage}_economics_2026-08-18.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCQCA-24 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCQCA-24 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCQCA-24 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCQCA-24 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCQCA-24 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCQCA-24 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCQCA-24 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCQCA-24 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCQCA-24 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCQCA-24 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCQCA-24 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcqca_24_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
