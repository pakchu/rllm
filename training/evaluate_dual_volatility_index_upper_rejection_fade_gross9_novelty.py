"""Evaluate frozen DVURF-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "DVURF-6"
PROTOCOL = "dvurf_6_gross9_novelty_v1"
PREREG = Path("results/dual_volatility_index_upper_rejection_fade_preregistration_2026-08-09.json")
PREREG_SHA = "79da6f3d3b37826b84a8b3fdd2b51fe00d0ae45f022e0f762a6c2d7bf89f3b77"
SUPPORT = Path("results/dual_volatility_index_upper_rejection_fade_support_2026-08-09.json")
SUPPORT_SHA = "d00260620b2ed807f2a189794e39f5f89b070c9bd75681787ae59de92235feb0"
CLOCK = Path("data/dual_volatility_index_upper_rejection_fade_clocks_2023_2026.csv.gz")
CLOCK_SHA = "bfd90aadc2644360eb4d1acddde97954c7e9061064fe9502ecd7866fbf568fb1"
OUTPUT = Path("results/dual_volatility_index_upper_rejection_fade_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("DVURF predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("DVURF novelty limits drift")
    if (
        ...
        raise RuntimeError("DVURF predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="DVURF primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "dvurf_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject DVURF-6 unchanged before economic outcomes",
    }
