"""Sequential strict economics for frozen HVSTOCH-24."""
...
POLICY_ID = "HVSTOCH-24"
BAR = pd.Timedelta(minutes=5)
LEVERAGE = 0.5
BASE_COST = 0.0006
STRESS_COST = 0.0010
ENV_FILE = "/home/pakchu/rllm/.env"
PREREG = Path("results/high_volatility_stochastic_extreme_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "3d98b6f957c53def0991275a3e7ac8ed79755ec39a8a477db2d546305477fa42"
SUPPORT = Path("results/high_volatility_stochastic_extreme_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "7996c9c63198c1f76499a1e2fe8370ae0d9f394f1d160930bc93406955062f44"
NOVELTY = Path("results/high_volatility_stochastic_extreme_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "7aedf5d0e8bbcdea49ee18dc31f4cc6c126648064d73b9b3ba686ac45e0d260d"
CLOCK = Path("data/high_volatility_stochastic_extreme_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "a7b56d802c1bf15fac33004d39efd5fc2485f81be5f259c1a8d44b9746f71d9d"
CONTROL_DIR = Path("data/high_volatility_stochastic_extreme_crossover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_stochastic_extreme_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_stochastic_extreme_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "no_extreme_gate", "one_bar_stale_crossover", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSTOCH frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSTOCH novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvstoch_24_sequential_economics_v1", "policy_id": POLICY_ID,
