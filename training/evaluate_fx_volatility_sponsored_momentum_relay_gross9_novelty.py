"""Evaluate frozen FVSMR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "FVSMR-12"
PROTOCOL = "fvsmr_12_gross9_novelty_v1"
...
OUTPUT = Path("results/fx_volatility_sponsored_momentum_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("FVSMR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("FVSMR novelty limits drift")
    if (
        ...
        raise RuntimeError("FVSMR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="FVSMR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "fvsmr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject FVSMR-12 unchanged before economic outcomes",
    }
