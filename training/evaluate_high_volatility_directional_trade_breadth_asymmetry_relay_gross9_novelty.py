"""Evaluate frozen HVDTBA-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDTBA-6"
PROTOCOL = "hvdtba_6_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_directional_trade_breadth_asymmetry_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDTBA predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVDTBA novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDTBA predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDTBA primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdtba_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVDTBA-6 unchanged before economic outcomes",
    }
