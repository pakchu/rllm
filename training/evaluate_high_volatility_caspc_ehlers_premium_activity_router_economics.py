"""Sequential strict economics for frozen HVCELVPAR-8."""
...
POLICY_ID = "HVCELVPAR-8"
BAR = pd.Timedelta(minutes=5)
LEVERAGE = 0.5
BASE_COST = 0.0006
STRESS_COST = 0.0010
ENV_FILE = "/home/pakchu/rllm/.env"
PREREG = Path("results/high_volatility_caspc_ehlers_premium_activity_router_preregistration_2026-08-16.json")
PREREG_SHA = "b8eb72e1b5d5458f534266bed7504950920f2f256715102a80e3b4a4263d17e6"
SUPPORT = Path("results/high_volatility_caspc_ehlers_premium_activity_router_support_2026-08-16.json")
SUPPORT_SHA = "cb7995efededfe4671263b2aded25504c388aae04e0c82d385dc8f86c8524249"
NOVELTY = Path("results/high_volatility_caspc_ehlers_premium_activity_router_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "3d771d118aa0da29d3b8bb309133c7b47b6b0bd21379c71ac2999bf68c1a8e85"
CLOCK = Path("data/high_volatility_caspc_ehlers_premium_activity_router_clocks_2023_2026.csv.gz")
CLOCK_SHA = "2d91dfd720bbda5e562b11d3800b287aa4bbdc467e3e44ec8268cacdd33c5749"
CONTROL_DIR = Path("data/high_volatility_caspc_ehlers_premium_activity_router_controls_2023_2026")
FREEZE = Path("results/high_volatility_caspc_ehlers_premium_activity_router_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_caspc_ehlers_premium_activity_router_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCELVPAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCELVPAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcelvpar_8_sequential_economics_v1", "policy_id": POLICY_ID,
