"""Build outcome-blind source support for frozen HVCCI-24."""
...
from training import preregister_high_volatility_cci_extreme_reentry_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "c2f8aa8f42ff1ccb2ec093d84e53e0c48be57a1bdddbecba7c9f25e57cf6e543"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_cci_extreme_reentry_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_cci_extreme_reentry_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_cci_extreme_reentry_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_cci_extreme_reentry_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_cci_extreme_reentry_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "typical_price", "center", "mean_deviation", "cci",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "typical_price", "center", "mean_deviation",
    "cci", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVCCI source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVCCI invalid source key")
...
def cci(high: pd.Series, low: pd.Series, close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    period=P["cci_periods"];typical=((high+low+close)/3).where(valid);center=typical.rolling(period,min_periods=period).mean();mean_deviation=typical.rolling(period,min_periods=period).apply(lambda values: float(np.mean(np.abs(values-np.mean(values)))),raw=True);value=((typical-center)/(P["cci_constant"]*mean_deviation)).where(mean_deviation.gt(0));return pd.DataFrame({"typical_price":typical,"center":center,"mean_deviation":mean_deviation,"cci":value},index=high.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[["bar_open","bar_high","bar_low","bar_close"]]).all(axis=1)&bars[["bar_open","bar_high","bar_low","bar_close"]].gt(0).all(axis=1);bars=pd.concat([bars,cci(bars.bar_high,bars.bar_low,bars.bar_close,bars.valid_bar)],axis=1);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum())
    bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["typical_price","center","mean_deviation","cci","variation"]]).all(axis=1)&bars.mean_deviation.gt(0)&bars.variation.gt(0);panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.cci.shift(1);long=previous.le(P["lower_level"])&panel.cci.gt(P["lower_level"]);short=previous.ge(P["upper_level"])&panel.cci.lt(P["upper_level"]);panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&(long|short)&panel.variation_rank.ge(P["variation_rank_min"]);return panel.loc[:,PANEL_COLUMNS]


def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);value=pd.to_numeric(used.cci,errors="coerce");previous=value.shift(1);long=previous.le(P["lower_level"])&value.gt(P["lower_level"]);short=previous.ge(P["upper_level"])&value.lt(P["upper_level"]);variation=used.variation_rank.ge(P["variation_rank_min"]);side=pd.Series(np.where(long,1,np.where(short,-1,0)),index=used.index,dtype=int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCCI prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvcci_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvcci_24_source_support_v1", "policy_id": prereg.POLICY_ID,
