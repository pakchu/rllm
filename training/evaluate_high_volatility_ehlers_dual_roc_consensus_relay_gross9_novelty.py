"""Evaluate frozen HVEDR-24 structural novelty against every Gross9 sleeve."""
...
from training import preregister_high_volatility_ehlers_dual_roc_consensus_relay as registration

POLICY = "HVEDR-24"
PROTOCOL = "hvedr_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_ehlers_dual_roc_consensus_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVEDR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVEDR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVEDR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVEDR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVEDR-24 unchanged before economic outcomes",
    }
