"""Build outcome-blind source support for frozen HVRSAR-8."""
...
from training import preregister_high_volatility_rogers_satchell_excursion_asymmetry_reversal_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2023-04-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "4146c2d7e83ea4fd05a82ef1ab61ccd03a87e39c8c55ce989d90334e2b6dd3a4"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_rogers_satchell_excursion_asymmetry_reversal_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_rogers_satchell_excursion_asymmetry_reversal_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_rogers_satchell_excursion_asymmetry_reversal_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_rogers_satchell_excursion_asymmetry_reversal_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_rogers_satchell_excursion_asymmetry_reversal_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "upper_component", "lower_component", "upper_energy",
    "lower_energy", "total_energy", "rs_variation", "excursion_asymmetry",
    "latest_bar_asymmetry", "variation_rank", "asymmetry_rank", "entry_side", "onset", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "upper_energy", "lower_energy", "total_energy",
    "rs_variation", "excursion_asymmetry", "latest_bar_asymmetry", "variation_rank",
    "asymmetry_rank", "entry_side", "onset", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVRSAR source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVRSAR invalid source key")
...
def rogers_satchell_states(bars: pd.DataFrame) -> pd.DataFrame:
    upper = np.log(bars.bar_high / bars.bar_close) * np.log(bars.bar_high / bars.bar_open)
    lower = np.log(bars.bar_low / bars.bar_close) * np.log(bars.bar_low / bars.bar_open)
    upper = upper.where(bars.valid_bar)
    lower = lower.where(bars.valid_bar)
    upper_energy = upper.rolling(P["bars"], min_periods=P["bars"]).sum()
    lower_energy = lower.rolling(P["bars"], min_periods=P["bars"]).sum()
    total = upper_energy + lower_energy
    asymmetry = ((upper_energy - lower_energy) / total).where(total.gt(0))
    latest = ((upper - lower) / (upper + lower)).where((upper + lower).gt(0))
    return pd.DataFrame({"upper_component":upper,"lower_component":lower,"upper_energy":upper_energy,"lower_energy":lower_energy,"total_energy":total,"rs_variation":np.sqrt(total/P["bars"]),"excursion_asymmetry":asymmetry,"latest_bar_asymmetry":latest},index=bars.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=bars.join(rogers_satchell_states(bars));bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["upper_energy","lower_energy","total_energy","rs_variation","excursion_asymmetry","latest_bar_asymmetry"]]).all(axis=1)&bars.total_energy.gt(0)&bars.excursion_asymmetry.ne(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.rs_variation.where(panel.source_valid),panel.valid_bar);panel["asymmetry_rank"]=prior_rank(panel.excursion_asymmetry.abs().where(panel.source_valid),panel.valid_bar);panel["entry_side"]=-np.sign(panel.excursion_asymmetry).fillna(0).astype(int);state=panel.source_valid&panel.variation_rank.ge(P["variation_rank_min"])&panel.asymmetry_rank.ge(P["asymmetry_rank_min"]);panel["onset"]=state&panel.source_valid.shift(1,fill_value=False)&~state.shift(1,fill_value=False);panel["eligible"]=panel.onset
    return panel.loc[:,PANEL_COLUMNS]

def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True);variation=used.variation_rank.ge(P["variation_rank_min"]);asymmetry=used.asymmetry_rank.ge(P["asymmetry_rank_min"]);side=pd.to_numeric(used.entry_side,errors="coerce").fillna(0).astype(int);state=used.onset.eq(True)&side.ne(0)
    if control=="no_asymmetry_tail":state=valid&side.ne(0)&variation
    elif control=="no_variation_gate":state=valid&side.ne(0)&asymmetry
    ...
    elif control=="latest_bar_asymmetry":side=-np.sign(pd.to_numeric(used.latest_bar_asymmetry,errors="coerce")).fillna(0).astype(int)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVRSAR prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvrsar_8_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvrsar_8_source_support_v1", "policy_id": prereg.POLICY_ID,
