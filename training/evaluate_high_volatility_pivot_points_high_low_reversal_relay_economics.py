"""Sequential strict economics for frozen HVPPHL-R10-24."""
...
PREREG = Path("results/high_volatility_pivot_points_high_low_reversal_relay_preregistration_2026-08-11.json")
PREREG_SHA = "c8ca87b0237b332c408e6de1c3983a9ae6e40a35e93da41f471d1b09c2da78f1"
SUPPORT = Path("results/high_volatility_pivot_points_high_low_reversal_relay_support_2026-08-11.json")
SUPPORT_SHA = "8e8d90fb6b6c6c16500068c1bff9ffc70f1909a107c3f916a02f45456ff2a518"
NOVELTY = Path("results/high_volatility_pivot_points_high_low_reversal_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "6fbc54cf84c8795c3e65745b28ed9ab7ae8dd7fab0d2f90449143908438b7613"
CLOCK = Path("data/high_volatility_pivot_points_high_low_reversal_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "97a4ad51482b7054b80532ccdc3cd881b1edb0ba327170a9a0dd9dad56c988bc"
CONTROL_DIR = Path("data/high_volatility_pivot_points_high_low_reversal_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_pivot_points_high_low_reversal_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_pivot_points_high_low_reversal_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "raw_bar_direction", "one_bar_stale_confirmation", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVPPHL frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVPPHL novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvpphl_r10_24_sequential_economics_v1", "policy_id": POLICY_ID,
