"""Build outcome-blind source support for frozen HVOIRR-8."""
...
from training import preregister_high_volatility_open_interest_refill_reversal_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv


ENV_FILE = "/home/pakchu/rllm/.env"
PREREG_SHA = "4f1d9f48335873129d78fe8c6d210f4f5aae9e4669bd309ec61a8e25a07e11fd"
...
SOURCE_DIR = Path("data/high_volatility_open_interest_refill_reversal_relay_sources_2023_2026")
FEATURES = SOURCE_DIR / "eight_hour_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_open_interest_refill_reversal_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_open_interest_refill_reversal_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_open_interest_refill_reversal_relay_support_2026-08-13.json")
SPLITS = {
...
CONTROLS = ("no_dvol_gate", "no_premium_tail", "no_refill_fraction", "one_block_stale_features", "direction_flip", "forced_long")
PREMIUM_QUERY = """
...
GROUP BY 1 ORDER BY 1
"""
CLOCK_COLUMNS = ("candidate", "control", "split", "decision_time", "feature_available_time", "entry_time", "exit_time", "side", "premium_displacement", "premium_displacement_rank", "prior_oi_change", "current_oi_change", "refill_fraction", "dvol_close", "dvol_level_rank")
...
def build_features(premium: pd.DataFrame, oi: pd.DataFrame, dvol: pd.DataFrame) -> pd.DataFrame:
    ...
    frame["prior_oi_change"] = frame.current_oi_change.shift(1)
    frame["refill_fraction"] = frame.current_oi_change / frame.prior_oi_change.abs()
    frame["source_valid"] = current_valid & oi_valid.shift(1, fill_value=False) & contiguous_prior
...
def conditions(frame: pd.DataFrame, control: str = "primary") -> tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    active &= used.prior_oi_change.lt(0) & used.current_oi_change.gt(0)
    if control != "no_refill_fraction": active &= used.refill_fraction.ge(0.25)
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active]:
        ...
        rows.append({"candidate": "HVOIRR-8", "control": control, "split": split, "decision_time": decision, "feature_available_time": decision, "entry_time": entry, "exit_time": exit_time, "side": int(sides.at[index]), "premium_displacement": float(source.premium_displacement), "premium_displacement_rank": float(source.premium_displacement_rank), "prior_oi_change": float(source.prior_oi_change), "current_oi_change": float(source.current_oi_change), "refill_fraction": float(source.refill_fraction), "dvol_close": float(source.dvol_close), "dvol_level_rank": float(source.dvol_level_rank)})
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA: raise RuntimeError("HVOIRR preregistration drift")
