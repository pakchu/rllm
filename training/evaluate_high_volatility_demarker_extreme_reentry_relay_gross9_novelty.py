"""Evaluate frozen HVDEM-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDEM-24"
PROTOCOL = "hvdem_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_demarker_extreme_reentry_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDEM predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVDEM novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDEM predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDEM primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVDEM-24 unchanged before economic outcomes",
    }
