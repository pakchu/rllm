"""Sequential strict economics for frozen CATLR-12."""
...
PREREG = Path("results/cross_asset_ticket_leadership_relay_preregistration_2026-08-09.json")
PREREG_SHA = "5a578cf1190c2edfc54836b4b6ca7c952d829983cae8d9ce09c46d1b9142ecde"
SUPPORT = Path("results/cross_asset_ticket_leadership_relay_support_2026-08-09.json")
SUPPORT_SHA = "f38e7dd83f940406aae950ced4dee9a08b70a6c4ce6c0f7de1845484b75a2f98"
NOVELTY = Path("results/cross_asset_ticket_leadership_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "6fa2fee9d06ef99a97d43510667fd5727c805fa02ee4d3edc67a0bc75150e963"
CLOCK = Path("data/cross_asset_ticket_leadership_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "fbf1344d191cb9a73fc0612608b6b4649c68d770ec43968ea1cdfc0d796765c6"
CONTROL_DIR = Path("data/cross_asset_ticket_leadership_relay_controls_2023_2026")
FREEZE = Path("results/cross_asset_ticket_leadership_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/cross_asset_ticket_leadership_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("btc_ticket_only", "alt_ticket_suppression_only", "no_return_tail", "no_volatility_gate", "one_block_stale_leadership", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CATLR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CATLR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "catlr_12_sequential_economics_v1", "policy_id": POLICY_ID,
