"""Evaluate frozen QHOIR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "QHOIR-8"
PROTOCOL = "qhoir_8_gross9_novelty_v1"
PREREG = Path("results/quarter_hour_opening_imbalance_relay_preregistration_2026-08-09.json")
PREREG_SHA = "a714b8cc65bef74b603df6428476f38f9aa6e48b5c4c60854fc12d56f73319cb"
SUPPORT = Path("results/quarter_hour_opening_imbalance_relay_support_2026-08-09.json")
SUPPORT_SHA = "15fa18f8a52dd2bfc19f1e15c1503b8aad27c2d351871558ff2e569ea34674ca"
CLOCK = Path("data/quarter_hour_opening_imbalance_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "0e09738e17ce298077aaac5e0e1b59cd26c172007e4a9730fd5e06afe05151b7"
OUTPUT = Path("results/quarter_hour_opening_imbalance_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("QHOIR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("QHOIR novelty limits drift")
    if (
        ...
        raise RuntimeError("QHOIR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="QHOIR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "qhoir_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject QHOIR-8 unchanged before economic outcomes",
    }
