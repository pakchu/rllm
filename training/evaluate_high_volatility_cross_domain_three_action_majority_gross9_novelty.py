"""Evaluate the frozen HVCM3-8 action-vote clock against Gross9."""
...
POLICY = "HVCM3-8"
PROTOCOL = "hvcm3_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_cross_domain_three_action_majority_clocks_2023_2026.csv.gz",
    "sha256": "6399d9a8a3daac0fe50f0bf260af053dd3cd0045fed8bcd5be37e3f8b91168f9",
    "rows": 260,
}
OUTPUT = Path("results/high_volatility_cross_domain_three_action_majority_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCM3-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCM3-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCM3-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCM3-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCM3-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCM3-8 unchanged before economics",
    }
