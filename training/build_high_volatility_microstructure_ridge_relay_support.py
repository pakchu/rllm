"""Open source-only OOS incidence for the frozen HVMRR-6 model."""
...
from training import train_high_volatility_microstructure_ridge_relay as trained
from training.build_binance_aggtrade_microstructure import _write_gzip_csv


PREREG_SHA = "7d0d76391471c6589ab389fc9b7e475adb1f44c7f7c4ebf4405a89f5a9b99380"
SOURCE_MANIFEST_SHA = "af00832a40ad5ad7d0d34a906bccf2df09461d8fb00feb10ceb217503d0a84af"
PANEL_SHA = "408a3944c876da76de82aefdc4093b3180e38fae34c5fa07ac21988613c43137"
MODEL_SHA = "08d9c48a1b0edab509fb18cc17bd84d1befa7e338268ad83365f6c9dd47d0b8c"
...
RESULT = Path("results/high_volatility_microstructure_ridge_relay_support_2026-08-09.json")
SPLITS = {
...
def verify_frozen_inputs() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if source.get("oos_incidence_opened") is not False or source.get("oos_outcomes_opened") is not False:
        raise RuntimeError("HVMRR source was not OOS sealed")
    if model.get("oos_incidence_opened") is not False or model.get("oos_outcomes_opened") is not False or model.get("refit_authorized") is not False:
        raise RuntimeError("HVMRR model was not frozen OOS blind")
    if tuple(model.get("feature_order", ())) != trained.FEATURES:
        raise RuntimeError("HVMRR feature order drift")
...
def clock(frame: pd.DataFrame, model: dict[str, Any], control: str = "primary") -> pd.DataFrame:
    ...
    for index in frame.index[active & frame.decision_time.ge(SPLITS["train"][0])]:
        ...
        if split is None:
            continue
        row = {
            "candidate": "HVMRR-6", "control": control, "split": split, "decision_time": decision,
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvmrr_6_oos_source_support_v1", "policy_id": "HVMRR-6",
