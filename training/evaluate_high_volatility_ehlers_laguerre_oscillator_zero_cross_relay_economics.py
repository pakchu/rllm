"""Sequential strict economics for frozen HVELO-24."""
...
PREREG = Path("results/high_volatility_ehlers_laguerre_oscillator_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "d82d5183fd785522df51c9c17fc9e88880529371f866fe564dd15bc501985520"
SUPPORT = Path("results/high_volatility_ehlers_laguerre_oscillator_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "5251d4cd238129642b89e88d74b15aaf7ea867f9c053be5a685a9df2e9719b30"
NOVELTY = Path("results/high_volatility_ehlers_laguerre_oscillator_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "2028ecc5cadeb316f0e53f9800523c8b039f2e0dfe75b5ab27214e7a5e8a335a"
CLOCK = Path("data/high_volatility_ehlers_laguerre_oscillator_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "6156fca75974ef7dc8e2f4b4f7005c2d041aaac27ffcea3dbe4cffc5d70b3f35"
CONTROL_DIR = Path("data/high_volatility_ehlers_laguerre_oscillator_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_ehlers_laguerre_oscillator_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ehlers_laguerre_oscillator_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "alternate_0_8_20", "oscillator_roc_turn", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVELO frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVELO novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvelo_24_sequential_economics_v1", "policy_id": POLICY_ID,
