"""Sequential strict economics for frozen CFTCMSPS-168."""
...
PREREG = Path("results/cftc_micro_standard_positioning_segmentation_relay_preregistration_2026-08-11.json")
PREREG_SHA = "8a433945bb33e05d282e32778192f75cd5b80886e4fde4c4381bfa1938bae30c"
SUPPORT = Path("results/cftc_micro_standard_positioning_segmentation_relay_support_2026-08-11.json")
SUPPORT_SHA = "ef20ff133ddaf9845472a1c053f61525960235fbef373dd3b76d6f33557f79c2"
NOVELTY = Path("results/cftc_micro_standard_positioning_segmentation_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "e6085d5ad2a376e79fd97e3f24a10172d8add445ca103def34a8ea7a3cd3123e"
CLOCK = Path("data/cftc_micro_standard_positioning_segmentation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d4151b50b14a45545eec880f9a376dcb574b77d37327f7585e7388a0e039d082"
CONTROL_DIR = Path("data/cftc_micro_standard_positioning_segmentation_relay_controls_2023_2026")
FREEZE = Path("results/cftc_micro_standard_positioning_segmentation_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/cftc_micro_standard_positioning_segmentation_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CFTCMSPS frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CFTCMSPS novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "cftcmsps_168_sequential_economics_v1", "policy_id": POLICY_ID,
