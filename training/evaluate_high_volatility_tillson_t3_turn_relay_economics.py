"""Sequential strict economics for frozen HVT3-24."""
...
PREREG = Path("results/high_volatility_tillson_t3_turn_relay_preregistration_2026-08-11.json")
PREREG_SHA = "b8457217ef2abaab3dc814e820cf055a4558b92b4671a5d5b58082c214bdaecb"
SUPPORT = Path("results/high_volatility_tillson_t3_turn_relay_support_2026-08-11.json")
SUPPORT_SHA = "552cd4ffd8216997baf2b11b9cfb2ee4f4db4b8ed1a9f1a471775b8787e612c5"
NOVELTY = Path("results/high_volatility_tillson_t3_turn_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "f1d07a5a5ba070b000e579e13c1c675a45d76beef585a9bf0be25699be1cfbe5"
CLOCK = Path("data/high_volatility_tillson_t3_turn_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "3c89d0b41f02f5b217b7d02f9a120ec795b26d82a13e4dceffce4e217c970ce9"
CONTROL_DIR = Path("data/high_volatility_tillson_t3_turn_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_tillson_t3_turn_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_tillson_t3_turn_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "price_ema_cross", "one_bar_stale_turn", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVT3 frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVT3 novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvt3_24_sequential_economics_v1", "policy_id": POLICY_ID,
