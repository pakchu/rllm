"""Build outcome-blind source support for frozen HVUSDTJ-24.

This module deliberately stops at source incidence. It downloads only the
current Coin Metrics USDT vintage, reads only completed BTC minute
``ts/open/close`` bars, and never opens an entry/exit price, return, PnL,
funding, or comparison set.
"""
...
PREREG_SHA256 = "ccf31a13bd1007b0fdeac03b673dbdace89790ba9febaa4909e08c5eb39b4a78"
...
BTC_QUERY_END = SOURCE_END + pd.Timedelta(days=2)
...
SOURCE_DIR = Path("data/high_volatility_tether_jump_spillover_relay_sources_2022_2026")
DAILY_SOURCE = SOURCE_DIR / "coinmetrics_usdt_daily.csv.gz"
FEATURE_PANEL = SOURCE_DIR / "daily_feature_panel.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_tether_jump_spillover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_tether_jump_spillover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_tether_jump_spillover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_tether_jump_spillover_relay_support_2026-08-13.json")

SOURCE_COLUMNS = ("observation_time", "feature_available_time", "PriceUSD")
PANEL_COLUMNS = (
    "observation_time", "feature_available_time", "decision_time", "source_valid",
    "minute_count", "PriceUSD", "usdt_log_return", "jump_rank", "btc_variation",
    "btc_variation_rank",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "observation_time", "feature_available_time",
    "decision_time", "entry_time", "exit_time", "side", "PriceUSD",
    "usdt_log_return", "jump_rank", "btc_variation", "btc_variation_rank",
)
...
def parse_source_row(raw: Any) -> dict[str, Any]:
    ...
    price = _positive_decimal(raw["PriceUSD"], "PriceUSD")
    return {
...
def prepare_btc_bars(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    if not frame.ts.eq(frame.ts.dt.floor("1min")).all():
        raise RuntimeError("BTC source is not on the exact one-minute grid")
...
def build_panel(source: pd.DataFrame, btc_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    daily["feature_available_time"] = pd.to_datetime(
        daily.feature_available_time, utc=True, errors="coerce"
    )
    daily["PriceUSD"] = pd.to_numeric(daily.PriceUSD, errors="coerce")
    expected = pd.date_range(SOURCE_START, SOURCE_END, freq="1d", inclusive="left")
    if (
    ...
    completion_valid = (
        ...
        & daily.feature_available_time.le(daily.observation_time + pd.Timedelta(days=2))
    )
    value_valid = np.isfinite(daily.PriceUSD) & daily.PriceUSD.gt(0)
    ...
    usdt_return = np.log(daily.PriceUSD / daily.PriceUSD.shift(1)).where(pair_valid)
    usdt_return = usdt_return.where(usdt_return.ne(0))
    ...
    base_valid = pair_valid & usdt_return.notna() & pd.Series(variation_valid, index=daily.index)
    panel = pd.DataFrame({
        ...
        "PriceUSD": daily.PriceUSD.astype(float),
        "usdt_log_return": usdt_return,
        "btc_variation": variation,
    })
    panel["jump_rank"] = strict_prior_midrank(
        panel.usdt_log_return.abs().where(panel.source_valid)
    )
    panel["btc_variation_rank"] = strict_prior_midrank(
        panel.btc_variation.where(panel.source_valid)
    )
...
def active(panel: pd.DataFrame, control: str = "primary") -> tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    eligible = valid & signal.notna() & signal.ne(0) & jump_tail & volatility
    ...
    adjusted = used.copy()
    adjusted["usdt_log_return"] = signal
...
def build_clock(panel: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in panel.index[eligible]:
        ...
        rows.append({
            ...
            "PriceUSD": float(panel.at[index, "PriceUSD"]),
            "usdt_log_return": float(used.at[index, "usdt_log_return"]),
            "jump_rank": float(panel.at[index, "jump_rank"]),
    ...
    try:
        with engine.connect() as connection:
            return pd.read_sql_query(
                ...
                params={
                    "start": BTC_QUERY_START.to_pydatetime(),
                    "end": BTC_QUERY_END.to_pydatetime(),
                },
            )
    finally:
        engine.dispose()
...
def run(*, fetch: Fetch | None = None) -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA256:
        raise RuntimeError("HVUSDTJ frozen preregistration artifact drift")
    registration = json.loads(prereg.DEFAULT_OUTPUT.read_text(encoding="utf-8"))
    if registration != prereg.build() or registration.get("manifest_hash") != PREREG_MANIFEST_HASH:
        raise RuntimeError("HVUSDTJ frozen preregistration payload drift")
    ...
    source_core = {
        "protocol_version": "hvusdtj_24_source_materialization_v1",
        "preregistration": {
            "path": str(prereg.DEFAULT_OUTPUT), "sha256": PREREG_SHA256,
        ...
        "daily_source": {
            "path": str(DAILY_SOURCE), "sha256": sha256_file(DAILY_SOURCE), "rows": len(source),
        },
        "btc_query": BTC_QUERY,
        ...
        "btc_rows": len(btc),
        "feature_panel": {
            "path": str(FEATURE_PANEL), "sha256": sha256_file(FEATURE_PANEL),
            "rows": len(panel), "valid_rows": int(panel.source_valid.sum()),
        },
        "no_imputation": True,
    ...
    core = {
        "protocol_version": "hvusdtj_24_source_support_v1",
        "policy_id": POLICY_ID,
        "preregistration": {
            "path": str(prereg.DEFAULT_OUTPUT), "sha256": PREREG_SHA256,
            "manifest_hash": registration["manifest_hash"],
        ...
        "split_artifacts": {
            name: {
                ...
                "sha256": sha256_file(SPLIT_DIR / f"{name}.csv.gz"), "rows": len(frame),
            }
            for name, frame in splits.items()
        },
        "controls": {
            name: {
                ...
                "sha256": sha256_file(CONTROL_DIR / f"{name}.csv.gz"),
