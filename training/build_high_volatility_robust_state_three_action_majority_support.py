"""Materialize deterministic, outcome-blind source support for HVRM3-8."""
...
from training import preregister_high_volatility_robust_state_three_action_majority as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv

PREREG_SHA = "c98bd816384c92cac4395434d41455a98dc3fa5a52d9509f624d4a35bc6749a0"
CLOCK = Path("data/high_volatility_robust_state_three_action_majority_clocks_2023_2026.csv.gz")
RESULT = Path("results/high_volatility_robust_state_three_action_majority_support_2026-08-16.json")
...
def _registration() -> dict[str, Any]:
    if prereg.sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVRM3-8 preregistration hash drift")
    ...
    for component, artifacts in prereg.COMPONENT_ARTIFACTS.items():
        for kind, artifact in artifacts.items():
            if prereg.sha256_file(artifact["path"]) != artifact["sha256"]:
                raise RuntimeError(f"HVRM3-8 {component} {kind} drift")
...
def build_clock(clocks: Mapping[str, pd.DataFrame]) -> tuple[pd.DataFrame, dict[str, Any]]:
    if set(clocks) != set(prereg.COMPONENT_IDS):
        raise RuntimeError("HVRM3-8 component roster drift")
    ...
    for decision in decisions:
        ...
        if any(row.split != split or row.feature_available_time > entry for row in active):
            raise RuntimeError("HVRM3-8 stage or availability drift")
    ...
    if counts["no_quorum_decisions"] + counts["tie_decisions"] + counts["emitted_decisions"] != counts["union_decisions"]:
        raise RuntimeError("HVRM3-8 incidence accounting drift")
    if len(output) > 1 and not output["entry_time"].iloc[1:].reset_index(drop=True).ge(output["exit_time"].iloc[:-1].reset_index(drop=True)).all():
        raise RuntimeError("HVRM3-8 output overlap")
