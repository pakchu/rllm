"""Sequential strict economics for frozen HVGBR-72."""
...
PREREG = Path("results/high_volatility_gradient_boosting_regression_relay_preregistration_2026-08-09.json")
PREREG_SHA = "651f45d5ab1b4b45a4177e745b60e148f27db060dafba4db098b0c02cd600ee2"
SUPPORT = Path("results/high_volatility_gradient_boosting_regression_relay_support_2026-08-09.json")
SUPPORT_SHA = "73b6101fff48f593597d17bd115f9c700d06ad4f5747b230571d2c06d1f5ef29"
NOVELTY = Path("results/high_volatility_gradient_boosting_regression_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "2ede6808f07a45ae5c0ec2810eaa01ae5ec1dc4b99b7f127dd9af11319ea9002"
CLOCK = Path("data/high_volatility_gradient_boosting_regression_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "933b6987d7ec6a7b4aaa488c5e36f10a5f065dcbae7efb88ee56796d472132d8"
CONTROL_DIR = Path("data/high_volatility_gradient_boosting_regression_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_gradient_boosting_regression_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_gradient_boosting_regression_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVGBR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVGBR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvgbr_72_sequential_economics_v1", "policy_id": POLICY_ID,
