"""Evaluate the frozen HVCASPCVAIV-8 action-vote clock against Gross9."""
...
POLICY = "HVCASPCVAIV-8"
PROTOCOL = "hvcaspcvaiv_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_caspc_variance_acceleration_inventory_opposite_veto_clocks_2023_2026.csv.gz",
    "sha256": "d437e3384c03f2c54b9b916c924b80dd853ba7b31aa9614028ecb753e227295d",
    "rows": 149,
}
OUTPUT = Path("results/high_volatility_caspc_variance_acceleration_inventory_opposite_veto_gross9_novelty_2026-08-17.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCASPCVAIV-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCASPCVAIV-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCASPCVAIV-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCASPCVAIV-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCASPCVAIV-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCASPCVAIV-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCASPCVAIV-8 unchanged before economics",
    }
