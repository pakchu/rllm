"""Evaluate frozen HVRBRAR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVRBRAR-8"
PROTOCOL = "hvrbrar_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_block_range_breakout_retest_acceptance_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVRBRAR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVRBRAR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVRBRAR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVRBRAR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvrbrar_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVRBRAR-8 unchanged before economic outcomes",
    }
