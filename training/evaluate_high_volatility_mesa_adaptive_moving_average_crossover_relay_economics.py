"""Sequential strict economics for frozen HVMAMA-C05-005-24."""
...
POLICY_ID = "HVMAMA-C05-005-24"
BAR = pd.Timedelta(minutes=5)
LEVERAGE = 0.5
BASE_COST = 0.0006
STRESS_COST = 0.0010
ENV_FILE = "/home/pakchu/rllm/.env"
PREREG = Path("results/high_volatility_mesa_adaptive_moving_average_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "fd4e72a1d35ce7b22d5fff12d5252fb9d081f53082a6dd651b72023e6cd9e073"
SUPPORT = Path("results/high_volatility_mesa_adaptive_moving_average_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "c4c095bac5adc1e0bfd1d61c7a3092edca06fcddbf2facf54b18d06eaf84b94e"
NOVELTY = Path("results/high_volatility_mesa_adaptive_moving_average_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "dd7f31838e89c69cdb687e9a77b844b814092302f87f78613e2e68a28d8dc5ad"
CLOCK = Path("data/high_volatility_mesa_adaptive_moving_average_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "efe55b444313f3f5aa7148057c4443a58acd659c34efd009cda13c35434177a8"
CONTROL_DIR = Path("data/high_volatility_mesa_adaptive_moving_average_crossover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_mesa_adaptive_moving_average_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_mesa_adaptive_moving_average_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "raw_median_change", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVMAMA frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVMAMA novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvmama_c05_005_24_sequential_economics_v1", "policy_id": POLICY_ID,
