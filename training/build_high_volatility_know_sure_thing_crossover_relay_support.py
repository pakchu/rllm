"""Build outcome-blind source support for frozen HVKST-24."""
...
from training import preregister_high_volatility_know_sure_thing_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "edac89d16ce5d95622bac6b5c97a3c3d34f8bf73a8c7ad12f0ea9033eb3e441c"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_know_sure_thing_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_know_sure_thing_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_know_sure_thing_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_know_sure_thing_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_know_sure_thing_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "rocma_1", "rocma_2", "rocma_3", "rocma_4",
    "kst", "signal", "difference",
    "entry_side", "zero_line_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "rocma_1", "rocma_2", "rocma_3",
    "rocma_4", "kst", "signal", "difference",
    "entry_side", "zero_line_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVKST source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVKST invalid source key")
...
def kst_values(series: pd.Series, valid: pd.Series) -> pd.DataFrame:
    components=[]
    for roc_period,sma_period in zip(P["roc_periods"],P["sma_periods"]):
        continuous=valid.rolling(roc_period+1,min_periods=roc_period+1).sum().eq(roc_period+1)
        lagged=series.shift(roc_period);roc=(100.0*(series/lagged-1.0)).where(continuous&lagged.gt(0))
        components.append(roc.rolling(sma_period,min_periods=sma_period).mean())
    kst=sum(weight*component for weight,component in zip(P["weights"],components));signal=kst.rolling(P["signal_periods"],min_periods=P["signal_periods"]).mean()
    values={f"rocma_{index+1}":component for index,component in enumerate(components)}
    return pd.DataFrame({**values,"kst":kst,"signal":signal,"difference":kst-signal},index=series.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    values=kst_values(bars.bar_close,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["entry_side"]=cross_side(bars.difference);bars["zero_line_side"]=cross_side(bars.kst)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["rocma_1","rocma_2","rocma_3","rocma_4","kst","signal","difference","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVKST prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvkst_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvkst_24_source_support_v1", "policy_id": prereg.POLICY_ID,
