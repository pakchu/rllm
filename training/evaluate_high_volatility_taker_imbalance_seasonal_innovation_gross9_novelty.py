"""Evaluate frozen HVTISI-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVTISI-8"
PROTOCOL = "hvtisi_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_taker_imbalance_seasonal_innovation_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTISI predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVTISI novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTISI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTISI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVTISI-8 unchanged before economic outcomes",
    }
