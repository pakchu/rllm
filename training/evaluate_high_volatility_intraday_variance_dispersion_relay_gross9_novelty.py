"""Evaluate frozen HVIVDR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVIVDR-8"
PROTOCOL = "hvivdr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_intraday_variance_dispersion_relay_gross9_novelty_2026-08-12.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVIVDR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVIVDR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVIVDR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVIVDR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvivdr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVIVDR-8 unchanged before economic outcomes",
    }
