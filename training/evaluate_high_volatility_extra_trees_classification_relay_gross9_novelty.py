"""Evaluate frozen HVETCR-48 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVETCR-48"
PROTOCOL = "hvetcr_48_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_extra_trees_classification_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVETCR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVETCR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVETCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVETCR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "jvfcr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVETCR-48 unchanged before economic outcomes"
        ),
    }
