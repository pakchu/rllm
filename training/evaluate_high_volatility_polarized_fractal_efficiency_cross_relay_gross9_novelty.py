"""Evaluate frozen HVPFE-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVPFE-24"
PROTOCOL = "hvpfe_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_polarized_fractal_efficiency_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVPFE predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVPFE novelty limits drift")
    if (
        ...
        raise RuntimeError("HVPFE predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVPFE primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvpfe_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVPFE-24 unchanged before economic outcomes",
    }
