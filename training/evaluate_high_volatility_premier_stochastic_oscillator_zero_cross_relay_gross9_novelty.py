"""Evaluate frozen HVPSO-Z14-3-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVPSO-Z14-3-24"
PROTOCOL = "hvpso_z14_3_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_premier_stochastic_oscillator_zero_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVPSO predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVPSO novelty limits drift")
    if (
        ...
        raise RuntimeError("HVPSO predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVPSO primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvpso_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVPSO-Z14-3-24 unchanged before economic outcomes",
    }
