"""Sequential strict economics for frozen HVHHM-C10-N5-8."""
...
POLICY_ID = "HVHHM-C10-N5-8"
BAR = pd.Timedelta(minutes=5)
LEVERAGE = 0.5
BASE_COST = 0.0006
STRESS_COST = 0.0010
ENV_FILE = "/home/pakchu/rllm/.env"
PREREG = Path("results/high_volatility_hammer_hanging_man_context_reversal_relay_preregistration_2026-08-11.json")
PREREG_SHA = "27041f6249a7dc0626a08d85ada1d28485a8cd9194974049d64da66f54f1420a"
SUPPORT = Path("results/high_volatility_hammer_hanging_man_context_reversal_relay_support_2026-08-11.json")
SUPPORT_SHA = "3498d602f503eef002db0853ca3bfe06baeb57fad666faf8b66c455fc75cfe27"
NOVELTY = Path("results/high_volatility_hammer_hanging_man_context_reversal_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "e98e1642110fd3dc8de69c6ca65a0fc7051db298e127a625ebdbf0bb14d49dd5"
CLOCK = Path("data/high_volatility_hammer_hanging_man_context_reversal_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "e14f7cef0010b6627faac97bc1340c5417ec80f3e3a20b00a47a67929f2612fe"
CONTROL_DIR = Path("data/high_volatility_hammer_hanging_man_context_reversal_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_hammer_hanging_man_context_reversal_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_hammer_hanging_man_context_reversal_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = (
    "no_variation_gate",
    "morphology_only_lower_wick_direction",
    "no_near_context",
    "one_hour_stale_pattern",
    "direction_flip",
    "forced_long",
)
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVHHM frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVHHM novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvhhm_c10_n5_8_sequential_economics_v1", "policy_id": POLICY_ID,
