"""Sequential strict economics for frozen HVTCCR-8."""
...
PREREG = Path("results/high_volatility_quote_turnover_concentration_continuation_relay_preregistration_2026-08-10.json")
PREREG_SHA = "b3fd974ae3ba804679e63d30ea02ee6d0ad3246981e80b7308242d09d19e3d26"
SUPPORT = Path("results/high_volatility_quote_turnover_concentration_continuation_relay_support_2026-08-10.json")
SUPPORT_SHA = "d874a611ae2ea65637fbbf7ca607062cc3df9c4d5989019bb23bf1abbb2713b3"
NOVELTY = Path("results/high_volatility_quote_turnover_concentration_continuation_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "910993f4a70ba770a9e66273630e14ca4df175864ba8f7009cb388c31fd8c28f"
CLOCK = Path("data/high_volatility_quote_turnover_concentration_continuation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "2485d9759fb4ebd9f247a87ddaa06c0fb4ec1ced100d4a82e3baac473ba4d294"
CONTROL_DIR = Path("data/high_volatility_quote_turnover_concentration_continuation_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_quote_turnover_concentration_continuation_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_quote_turnover_concentration_continuation_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_concentration_tail", "no_variation_gate", "unweighted_return_direction", "one_boundary_stale_concentration", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTCCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTCCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvtccr_8_sequential_economics_v1", "policy_id": POLICY_ID,
