"""Evaluate frozen HVHRR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVHRR-8"
PROTOCOL = "hvhrr_8_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_higuchi_roughness_reversal_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVHRR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVHRR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVHRR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVHRR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvhrr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVHRR-8 unchanged before economic outcomes"
        ),
    }
