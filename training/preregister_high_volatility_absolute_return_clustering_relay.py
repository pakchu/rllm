"""Outcome-blind preregistration for HVACR-8."""
...
POLICY_ID = "HVACR-8"
DEFAULT_OUTPUT = Path(
    "results/high_volatility_absolute_return_clustering_relay_preregistration_2026-08-10.json"
)
...
def build() -> dict[str, Any]:
    core = {
        "protocol_version": "high_volatility_absolute_return_clustering_relay_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": (
                "A high-variation completed eight-hour BTC block with high lag-one "
                "autocorrelation of absolute one-minute returns exhibits endogenous volatility "
                "clustering rather than an isolated shock. When the full block and final two "
                "hours agree in direction, the active information-arrival episode should "
                "continue in that direction for eight hours."
            ),
            "side": "common strict sign of completed eight-hour and final-two-hour returns",
            "why_distinct": (
                "HVACR measures serial dependence in return magnitudes after removing direction. "
                "The prior serial-autocorrelation candidate correlates signed returns; variance-ratio "
                "compares aggregated signed-return variance; HAR surprise forecasts total variation; "
                "sign entropy and sign runs discard magnitudes. HVACR "
            ...
            "why_suited_to_volatile_regimes": (
                ...
                "volatility, while the clustering tail requires repeated rather than isolated turbulence"
            ),
            "why_low_gross9_overlap_is_plausible": (
                "three fixed UTC magnitude-clustering onsets with an eight-hour reservation are absent "
        ...
        "features": {
            ...
            "absolute_return_autocorrelation": (
                "Pearson correlation between abs(return[t]) and abs(return[t-1]) across the 479 "
                "completed-block close-to-close returns; both vectors must have finite positive standard deviation"
            ),
            "clustering_rank": (
        ...
        "policy": {
            "autocorrelation_lag_returns": 1,
            "history_blocks": 270,
            "minimum_history_blocks": 180,
            "variation_rank_min": 0.65,
            "clustering_rank_min": 0.75,
        ...
        "research_boundary": {
            "prior_signed_serial_dependence_family_outcomes_known": True,
            "repository_absolute_return_autocorrelation_candidate_found": False,
            "prior_event_sets_reused": False,
            "prior_candidate_outcomes_used_to_set_hvacr_formula_ranks_direction_hold_or_clock": False,
            ...
            "selection_basis": "fixed lag-one absolute-return volatility-clustering coefficient",
        },
        "stopping_rule": (
