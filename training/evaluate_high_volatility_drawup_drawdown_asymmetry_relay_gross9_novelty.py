"""Evaluate frozen HVDUDAR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDUDAR-8"
PROTOCOL = "hvdudar_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_drawup_drawdown_asymmetry_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDUDAR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVDUDAR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDUDAR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDUDAR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdudar_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVDUDAR-8 unchanged before economic outcomes",
    }
