"""Build outcome-blind source support for frozen HVPFE-24."""
...
from training import preregister_high_volatility_polarized_fractal_efficiency_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "474eabb0d8dca856e821bbcce4a400925aee1f5944bfa9f89244a959510d7cda"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_polarized_fractal_efficiency_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_polarized_fractal_efficiency_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_polarized_fractal_efficiency_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_polarized_fractal_efficiency_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_polarized_fractal_efficiency_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "net_displacement", "straight_distance", "path_distance",
    "raw_pfe", "pfe", "entry_side", "raw_displacement_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "net_displacement", "straight_distance",
    "path_distance", "raw_pfe", "pfe", "entry_side", "raw_displacement_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVPFE source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVPFE invalid source key")
...
def pfe_values(series: pd.Series, valid: pd.Series, period: int) -> pd.DataFrame:
    segment=(series-series.shift(1)).where(valid&valid.shift(1,fill_value=False));complete=np.isfinite(segment).rolling(period,min_periods=period).sum().eq(period);net=(series-series.shift(period)).where(complete);straight=np.sqrt(np.square(net)+period**2);path=np.sqrt(np.square(segment)+1.0).rolling(period,min_periods=period).sum().where(complete);raw=(100.0*straight/path*np.sign(net)).where(path.gt(0)&net.ne(0));return pd.DataFrame({"net_displacement":net,"straight_distance":straight,"path_distance":path,"raw_pfe":raw},index=series.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    values=pfe_values(bars.bar_close,bars.valid_bar,P["efficiency_periods"])
    for column in values:bars[column]=values[column]
    bars["pfe"]=causal_ema(bars.raw_pfe,np.isfinite(bars.raw_pfe),P["smoothing_periods"]);bars["entry_side"]=cross_side(bars.pfe);bars["raw_displacement_side"]=np.sign(bars.net_displacement).fillna(0).astype(int)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["net_displacement","straight_distance","path_distance","raw_pfe","pfe","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.path_distance.gt(0)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="raw_net_displacement":side=pd.to_numeric(used.raw_displacement_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVPFE prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvpfe_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvpfe_24_source_support_v1", "policy_id": prereg.POLICY_ID,
