"""Build outcome-blind source support for frozen HVHHM-C10-N5-8."""
...
from training import preregister_high_volatility_hammer_hanging_man_context_reversal_relay as prereg


ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2023-04-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "27041f6249a7dc0626a08d85ada1d28485a8cd9194974049d64da66f54f1420a"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_hammer_hanging_man_context_reversal_relay_sources_2023_2026")
PANEL = ROOT / "hourly_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_hammer_hanging_man_context_reversal_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_hammer_hanging_man_context_reversal_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_hammer_hanging_man_context_reversal_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_hammer_hanging_man_context_reversal_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "context_pattern_ready", "context_pattern_output", "entry_side",
    "morphology_side", "no_context_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "context_pattern_ready", "context_pattern_output",
    "entry_side", "morphology_side", "no_context_side", "variation",
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVHHM source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVHHM invalid source key")
...
def context_pattern_outputs(bars: pd.DataFrame, valid: pd.Series) -> pd.DataFrame:
    """Replicate LEAN Hammer/HangingMan defaults and unambiguous context mapping."""
    output = np.zeros(len(bars), dtype=int)
    ready = np.zeros(len(bars), dtype=bool)
    morphology_side = np.zeros(len(bars), dtype=int)
    no_context_side = np.zeros(len(bars), dtype=int)
    ...
    for index, is_valid in enumerate(valid.to_numpy(bool)):
        ...
        history.append((float(current.bar_open), float(current.bar_high), float(current.bar_low), float(current.bar_close)))
        history = history[-P["indicator_period_hours"] :]
        ready[index] = len(history) >= P["indicator_period_hours"]
        if not ready[index]:
            continue
        previous_open, previous_high, previous_low, previous_close = history[-2]
        current_open, current_high, current_low, current_close = history[-1]
        bodies = [abs(close - open_) for open_, _, _, close in history]
        body_average = float(np.mean(bodies[-11:-1])) * P["body_short_factor"]
        high_low_average = float(np.mean([high - low for _, high, low, _ in history[-11:-1]]))
        near_average = float(np.mean([high - low for _, high, low, _ in history[-7:-2]])) * P["near_factor"]
        body = abs(current_close - current_open)
        lower_shadow = min(current_open, current_close) - current_low
        upper_shadow = current_high - max(current_open, current_close)
        morphology = (
            body < body_average
            and lower_shadow > body * P["shadow_long_factor"]
            and upper_shadow < high_low_average * P["shadow_very_short_factor"]
        )
        body_low = min(current_open, current_close)
        hammer = morphology and body_low <= previous_low + near_average
        hanging = morphology and body_low >= previous_high - near_average
        color = 1 if current_close >= current_open else -1
        if morphology:
            morphology_side[index] = 1
            no_context_side[index] = color
        if hammer != hanging:
            output[index] = 1 if hammer else -1
    entry = output.copy()
    return pd.DataFrame(
        {
            "context_pattern_ready": ready,
            "context_pattern_output": output,
            "entry_side": entry,
            "morphology_side": morphology_side,
            "no_context_side": no_context_side,
        },
        index=bars.index,
    )
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control == "morphology_only_lower_wick_direction":
        side = pd.to_numeric(used.morphology_side, errors="coerce").fillna(0).astype(int)
        state = valid & side.ne(0) & variation
    elif control == "no_near_context":
        side = pd.to_numeric(used.no_context_side, errors="coerce").fillna(0).astype(int)
        state = valid & side.ne(0) & variation
    elif control == "one_hour_stale_pattern":
        ...
        rows.append(
            {
                ...
                **{
                    ...
                    if column in ("context_pattern_ready", "eligible")
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVHHM prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvhhm_c10_n5_8_source_v1",
        "query": QUERY,
    ...
    core = {
        "protocol_version": "hvhhm_c10_n5_8_source_support_v1",
