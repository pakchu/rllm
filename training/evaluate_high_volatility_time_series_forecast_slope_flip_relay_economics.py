"""Sequential strict economics for frozen HVTSF3-24."""
...
PREREG = Path("results/high_volatility_time_series_forecast_slope_flip_relay_preregistration_2026-08-11.json")
PREREG_SHA = "41780a8626de04552d17cce889cb7196d2e34e7a204b805ae205bec786b0fd4a"
SUPPORT = Path("results/high_volatility_time_series_forecast_slope_flip_relay_support_2026-08-11.json")
SUPPORT_SHA = "cecf0218e43f0d9a067fc70c2f5e05e224e3ef2fa503e815807dcea851c43599"
NOVELTY = Path("results/high_volatility_time_series_forecast_slope_flip_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "d67816fa81bc89f9d38a92edf52899e6b70fdb7acaddab29f131ece1d7048872"
CLOCK = Path("data/high_volatility_time_series_forecast_slope_flip_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "ff3193831712094ade24be1f22bb7eb429bdba00132a24d68212b345858d5ce5"
CONTROL_DIR = Path("data/high_volatility_time_series_forecast_slope_flip_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_time_series_forecast_slope_flip_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_time_series_forecast_slope_flip_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = (
    "no_variation_gate",
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTSF3 frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTSF3 novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvtsf3_24_sequential_economics_v1", "policy_id": POLICY_ID,
