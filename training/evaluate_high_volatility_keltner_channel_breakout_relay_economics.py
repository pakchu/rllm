"""Sequential strict economics for frozen HVKC-24."""
...
PREREG = Path("results/high_volatility_keltner_channel_breakout_relay_preregistration_2026-08-11.json")
PREREG_SHA = "76a6af3208f29948513d09ee236ef42836023c0ea7f7c0a489c12b499c4ea7c8"
SUPPORT = Path("results/high_volatility_keltner_channel_breakout_relay_support_2026-08-11.json")
SUPPORT_SHA = "6eb96892214f10e9cff4b68f04b5a0a2ac2a0647a33379832dc35edd795b58fa"
NOVELTY = Path("results/high_volatility_keltner_channel_breakout_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "b96b093039665c72bcd5a2fc3122a4a3ead77b29a3e9e2cb796d8365bdf6b7e0"
CLOCK = Path("data/high_volatility_keltner_channel_breakout_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "883e682e7b1b321f0a340300eaccd1f8be64f99e3ba0a4cf13687e210e14e3fb"
CONTROL_DIR = Path("data/high_volatility_keltner_channel_breakout_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_keltner_channel_breakout_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_keltner_channel_breakout_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "simple_moving_center", "one_bar_stale_breakout", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVKC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVKC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvkc_24_sequential_economics_v1", "policy_id": POLICY_ID,
