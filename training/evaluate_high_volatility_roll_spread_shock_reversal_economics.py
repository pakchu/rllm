"""Sequential strict economics for frozen HVRSSR-8."""
...
PREREG = Path("results/high_volatility_roll_spread_shock_reversal_preregistration_2026-08-10.json")
PREREG_SHA = "b712a8b708a1c7fb28a2bef021a6534f04ed241b4b4fd5dd165359d5d1dcdf81"
SUPPORT = Path("results/high_volatility_roll_spread_shock_reversal_support_2026-08-10.json")
SUPPORT_SHA = "2f531003ff623c316d70edc8b4bd9b20d03be3916b6d5b4eddbe4dd5f19f2cd1"
NOVELTY = Path("results/high_volatility_roll_spread_shock_reversal_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "306a59dab9943196d0e3321ca5be50b216ccef4c9c7347b1010cb9897ffe4cff"
CLOCK = Path("data/high_volatility_roll_spread_shock_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d1e5c102e486abd9dd41ce42b822113ced3103d3eb892008a97b201f5ba1574f"
CONTROL_DIR = Path("data/high_volatility_roll_spread_shock_reversal_controls_2023_2026")
FREEZE = Path("results/high_volatility_roll_spread_shock_reversal_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_roll_spread_shock_reversal_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_spread_tail_gate", "no_variation_gate", "all_negative_covariance", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVRSSR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVRSSR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrssr_8_sequential_economics_v1", "policy_id": POLICY_ID,
