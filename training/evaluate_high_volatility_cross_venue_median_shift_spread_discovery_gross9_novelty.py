"""Evaluate the frozen HVCVMSSD-8 action-vote clock against Gross9."""
...
POLICY = "HVCVMSSD-8"
PROTOCOL = "hvcvmssd_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_cross_venue_median_shift_spread_discovery_preregistration_2026-08-18.json")
PREREG_SHA = "c189a434058295aeb46696bdc15f6685b7eb4303976a7f5931da85ec3018a8af"
SUPPORT = Path("results/high_volatility_cross_venue_median_shift_spread_discovery_support_2026-08-18.json")
SUPPORT_SHA = "816a9f7fbf6b07b37203ef839f59c7904af175334a7a6d68f7e2a24d74a6604f"
CLOCK = {
    "path": "data/high_volatility_cross_venue_median_shift_spread_discovery_clocks_2023_2026.csv.gz",
    "sha256": "ac93e3971c51d60dcbf3a466dc607a76678596e4cf5e73cd34b6c9d1db884e0c",
    "rows": 299,
}
OUTPUT = Path("results/high_volatility_cross_venue_median_shift_spread_discovery_gross9_novelty_2026-08-18.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCVMSSD-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVMSSD-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCVMSSD-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVMSSD-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCVMSSD-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCVMSSD-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCVMSSD-8 unchanged before economics",
    }
