"""Strict staged economics for the singleton HVEIV-24 policy."""
...
PREREG = Path("results/high_volatility_ehlers_imi_veto_preregistration_2026-08-16.json")
PREREG_SHA = "09992a113faa45c9ef420ba40f1ff9363e2ee3f2f11548315056b1e1730bb60d"
SUPPORT = Path("results/high_volatility_ehlers_imi_veto_support_2026-08-16.json")
SUPPORT_SHA = "94463c0f3f49cca89c8ef58834cef8e0da035ca379821449454a1409923d9a5b"
GROSS9 = Path("results/high_volatility_ehlers_imi_veto_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "c33dc01156e5b0c0b27f5c85e27d25658695c1fc496d8064c866167fe000adeb"
CLOCK = {
    "path": Path("data/high_volatility_ehlers_imi_veto_clocks_2023_2026.csv.gz"),
    "sha256": "3f16540ef8eb05cff29767cff287dab9dbe00a8537351642e328d39c511e4c13",
    "rows": 183,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ehlers_imi_veto_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVEIV-24 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVEIV-24 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVEIV-24 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVEIV-24 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVEIV-24 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVEIV-24 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVEIV-24 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVEIV-24 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVEIV-24 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVEIV-24 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVEIV-24 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hveiv_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
