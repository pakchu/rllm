"""Evaluate frozen HVCLES-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCLES-8"
PROTOCOL = "hvcles_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_cash_led_excursion_sponsorship_relay_preregistration_2026-08-13.json")
PREREG_SHA = "c93f6298f23c2fa79f7f40e715255ffccd0ea2d8c49af1f9d9679486c82b4285"
SUPPORT = Path("results/high_volatility_cash_led_excursion_sponsorship_relay_support_2026-08-13.json")
SUPPORT_SHA = "eb61aa0cab61beb0a25a5ceeb7c3c1a932eaa87b596b0e8643b1fa9b1335a99a"
CLOCK = Path("data/high_volatility_cash_led_excursion_sponsorship_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "4f15bff6edf89a10c2a7ce9d219510b45df8f792db99baa8cc1553695a6fdfa9"
OUTPUT = Path("results/high_volatility_cash_led_excursion_sponsorship_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCLES predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCLES novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCLES predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCLES primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCLES-8 unchanged before economic outcomes",
    }
