"""Build source-only CLBHAR-6 confirmation-ladder clocks."""
...
from training import preregister_confirmation_ladder_back_half_acceleration_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.download_fetls_block_summaries import OUTPUT as BLOCKS


ENV_FILE = "/home/pakchu/rllm/.env"
PREREG_SHA = "99bb6cfec4a18f1f83076f6d9260c93a4cc41a2c4d9310be9fecb3b54132afa1"
...
CONTROLS = ("direction_agreement_only", "acceleration_only", "one_anchor_stale_ladder", "direction_flip")
...
SOURCE_DIR = Path("data/confirmation_ladder_back_half_acceleration_relay_sources_2023_2026")
FEATURES = SOURCE_DIR / "confirmation_ladders.csv.gz"
MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/confirmation_ladder_back_half_acceleration_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/confirmation_ladder_back_half_acceleration_relay_controls_2023_2026")
RESULT = Path("results/confirmation_ladder_back_half_acceleration_relay_support_2026-08-09.json")
...
FEATURE_COLUMNS = (
    ...
    *RETURN_COLUMNS, "early_return", "late_return", "acceleration_ratio",
    "acceleration_rank", "eligible_state",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "anchor_height", "confirmation_height",
    "feature_available_time", "entry_time", "exit_time", "side", "early_return",
    "late_return", "acceleration_ratio", "acceleration_rank",
)
...
def validate_blocks(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CLBHAR block schema drift")
    ...
    if not np.array_equal(np.diff(blocks.height), np.ones(len(blocks) - 1, dtype=np.int64)):
        raise RuntimeError("CLBHAR block heights are not contiguous")
    if not blocks.previousblockhash.iloc[1:].reset_index(drop=True).equals(blocks.id.iloc[:-1].reset_index(drop=True)):
        raise RuntimeError("CLBHAR canonical previous-hash continuity failed")
    return blocks
...
def prepare_minutes(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CLBHAR minute schema drift")
    ...
    if result.ts.duplicated().any():
        raise RuntimeError("CLBHAR duplicate timestamps")
...
def build_features(blocks: pd.DataFrame, market: pd.DataFrame) -> pd.DataFrame:
    ...
    for index in range(0, len(blocks) - 6):
        ...
        early = float(np.sum(returns[:3])) if valid else np.nan
        late = float(np.sum(returns[3:])) if valid else np.nan
        acceleration = abs(late) / abs(early) if valid and early != 0 else np.nan
        valid = bool(valid and np.isfinite([early, late, acceleration]).all() and early != 0 and late != 0 and acceleration > 0)
        rows.append({
            ...
            **{column: returns[position] if valid else np.nan for position, column in enumerate(RETURN_COLUMNS)},
            "early_return": early if valid else np.nan, "late_return": late if valid else np.nan,
            "acceleration_ratio": acceleration if valid else np.nan,
        })
    output = pd.DataFrame(rows)
    output["acceleration_rank"] = strict_prior_midrank(output.acceleration_ratio.where(output.source_valid))
    output["eligible_state"] = (
        output.source_valid & np.sign(output.early_return).eq(np.sign(output.late_return))
        & output.acceleration_rank.ge(0.80)
    )
...
def active_and_side(features: pd.DataFrame, control: str = "primary"):
    ...
    agreement = np.sign(features.early_return).eq(np.sign(features.late_return))
    primary = features.eligible_state.astype(bool); late = features.late_return.copy()
    if control == "direction_agreement_only":
        state = features.source_valid & agreement
    elif control == "acceleration_only":
        state = features.source_valid & features.acceleration_rank.ge(0.80)
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active & sides.ne(0)]:
        ...
        rows.append({
            ...
            "early_return": float(features.at[index, "early_return"]), "late_return": float(features.at[index, "late_return"]),
            "acceleration_ratio": float(features.at[index, "acceleration_ratio"]), "acceleration_rank": float(features.at[index, "acceleration_rank"]),
        })
...
def run() -> dict[str, Any]:
    ...
    source_core = {
        "protocol_version": "clbhar_6_source_v1", "query_sha256": hashlib.sha256(QUERY.encode()).hexdigest(), "window": [QUERY_START.isoformat(), END.isoformat()],
    ...
    core = {
        "protocol_version": "clbhar_6_source_support_v1", "policy_id": prereg.POLICY_ID,
