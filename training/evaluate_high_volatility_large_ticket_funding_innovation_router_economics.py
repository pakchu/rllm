"""Sequential strict economics for frozen HVLTFIR-8."""
...
PREREG = Path("results/high_volatility_large_ticket_funding_innovation_router_preregistration_2026-08-16.json")
PREREG_SHA = "d0cca3a65193fc1e60e34ddf2458a01b4165ba2a32b708acb158b9b507852fa7"
SUPPORT = Path("results/high_volatility_large_ticket_funding_innovation_router_support_2026-08-16.json")
SUPPORT_SHA = "884824db7b65c4de9f36a4a44837d103f3cdb288890fec8d9a15d51405ea7d72"
NOVELTY = Path("results/high_volatility_large_ticket_funding_innovation_router_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "f67a4cf5fa47be26ffcb828072f45b87aaf32c6660aa5a9b3def792eae3da10c"
CLOCK = Path("data/high_volatility_large_ticket_funding_innovation_router_clocks_2023_2026.csv.gz")
CLOCK_SHA = "5763b0d238d587ae5221863e73f2beeadf282c3488f1d89e5011c6389fdcc3b4"
CONTROL_DIR = Path("data/high_volatility_large_ticket_funding_innovation_router_controls_2023_2026")
FREEZE = Path("results/high_volatility_large_ticket_funding_innovation_router_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_large_ticket_funding_innovation_router_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVLTFIR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVLTFIR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvltfir_8_sequential_economics_v1", "policy_id": POLICY_ID,
