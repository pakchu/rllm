"""Build outcome-blind source support for frozen HVSVF-8."""
...
from training import preregister_high_volatility_signed_variance_feedback_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-01-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="c7193999573aa0dffe77afbd8637d7fcf482598a8124907b73bfd12d967508e8"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_signed_variance_feedback_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_signed_variance_feedback_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_signed_variance_feedback_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_signed_variance_feedback_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_signed_variance_feedback_relay_support_2026-08-10.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","feedback_score","feedback_magnitude","feedback_magnitude_rank","realized_variation","variation_rank","contemporaneous_score","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","feedback_score","feedback_magnitude","feedback_magnitude_rank","realized_variation","variation_rank","contemporaneous_score")
...
def feedback_statistics(closes:np.ndarray)->tuple[float,float,float,float]:
 closes=np.asarray(closes,float)
 if closes.shape!=(96,) or not np.isfinite(closes).all() or np.any(closes<=0):return (math.nan,)*4
 returns=np.diff(np.log(closes));prior=returns[:-1];future_energy=np.square(returns[1:])
 denominator=math.sqrt(float(np.square(prior).sum())*float(np.square(future_energy).sum()))
 contemporaneous_denominator=math.sqrt(float(np.square(returns).sum())*float(np.power(returns,4).sum()))
 if denominator<=0 or contemporaneous_denominator<=0:return (math.nan,)*4
 score=float(np.dot(prior,future_energy)/denominator);contemporaneous=float(np.dot(returns,np.square(returns))/contemporaneous_denominator)
 variation=float(np.square(returns).sum())
 return score,abs(score),variation,contemporaneous
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVSVF source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVSVF invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("8h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   close=block.close.to_numpy(float).reshape(96,5)[:,-1];score,magnitude,var,contemporaneous=feedback_statistics(close);valid=math.isfinite(score) and score!=0 and var>0 and math.isfinite(contemporaneous) and contemporaneous!=0
  else:score=magnitude=var=contemporaneous=math.nan;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"feedback_score":score,"feedback_magnitude":magnitude,"realized_variation":var,"contemporaneous_score":contemporaneous})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["feedback_magnitude_rank"]=prior_rank(panel.feedback_magnitude.where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));panel["eligible"]=valid&panel.feedback_magnitude_rank.ge(P["feedback_magnitude_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"]);panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_block_stale_geometry":
  cols=["source_valid","feedback_magnitude_rank","variation_rank","feedback_score","contemporaneous_score","eligible","onset","feature_available_time"];used[cols]=panel[cols].shift(1)
 valid=used.source_valid.eq(True);feedback=used.feedback_magnitude_rank.ge(P["feedback_magnitude_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&feedback&variation
 if control=="no_feedback_tail_gate":state=valid&variation
 elif control=="no_variation_gate":state=valid&feedback
 onset=previous_valid_onset(state,valid);side=np.sign(pd.to_numeric(used.feedback_score,errors="coerce").fillna(0)).astype(int)
 if control=="contemporaneous_signed_quarticity":side=np.sign(pd.to_numeric(used.contemporaneous_score,errors="coerce").fillna(0)).astype(int)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVSVF prereg drift")
 ...
 source_core={"protocol_version":"hvsvf_8_source_v1","query":QUERY,"query_sha256":hashlib.sha256(QUERY.encode()).hexdigest(),"window":[START.isoformat(),END.isoformat()],"physical_rows":len(raw),"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"panel":{"path":str(PANEL),"sha256":sha(PANEL),"rows":len(panel),"valid_rows":int(panel.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};manifest={**source_core,"manifest_hash":chash(source_core)};immutable(MANIFEST,json_bytes(manifest))
