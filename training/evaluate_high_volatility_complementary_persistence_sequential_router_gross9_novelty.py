"""Evaluate the frozen HVCPSR-8 action-vote clock against Gross9."""
...
POLICY = "HVCPSR-8"
PROTOCOL = "hvcpsr_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_complementary_persistence_sequential_router_preregistration_2026-08-16.json")
PREREG_SHA = "943a075a7209e43b204fe56c947c0d16fdc3495f4e6fbf9cb2c8754cc277b1a8"
SUPPORT = Path("results/high_volatility_complementary_persistence_sequential_router_support_2026-08-16.json")
SUPPORT_SHA = "b45057660a174b8ce0336b907339ed79382b3acfe6c6d89cd664ad25b8735653"
CLOCK = {
    "path": "data/high_volatility_complementary_persistence_sequential_router_clocks_2023_2026.csv.gz",
    "sha256": "55c351e0d55cb19c3d6f02f449779d15230797661ec7855eae5b5737811799ed",
    "rows": 575,
}
OUTPUT = Path("results/high_volatility_complementary_persistence_sequential_router_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCPSR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCPSR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCPSR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCPSR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCPSR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCPSR-8 unchanged before economics",
    }
