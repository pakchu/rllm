"""Evaluate the frozen HVCCLOICR-8 action-vote clock against Gross9."""
...
POLICY = "HVCCLOICR-8"
PROTOCOL = "hvccloicr_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_close_location_open_interest_contraction_reversal_clocks_2020_2026.csv.gz",
    "sha256": "2c533ca13205ed25964fe19e63e09c9841f9aed3217fedb7e540695493742206",
    "rows": 371,
}
OUTPUT = Path("results/high_volatility_causal_close_location_open_interest_contraction_reversal_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCCLOICR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCCLOICR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCCLOICR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCCLOICR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCCLOICR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCCLOICR-8 unchanged before economics",
    }
