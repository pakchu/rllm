"""Evaluate frozen HVIMIOICR-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVIMIOICR-24"
PROTOCOL = "hvimi_oicr_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_intraday_momentum_index_oi_contraction_reentry_relay_gross9_novelty_2026-08-16.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVIMIOICR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVIMIOICR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVIMIOICR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVIMIOICR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvimi_oicr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVIMIOICR-24 unchanged before economic outcomes",
    }
