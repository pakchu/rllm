"""Sequential strict economics for frozen HVDTSA-8."""
...
PREREG = Path("results/high_volatility_directional_ticket_size_asymmetry_relay_preregistration_2026-08-10.json")
PREREG_SHA = "0a05070b5b1fa07907cd6310d81c9b6c1631e051067744f63d4c5edb66896a05"
SUPPORT = Path("results/high_volatility_directional_ticket_size_asymmetry_relay_support_2026-08-10.json")
SUPPORT_SHA = "8e5a956ebf54056f3363afb5b484566b4aa96979a5dcdb8522bf900c2cb917f1"
NOVELTY = Path("results/high_volatility_directional_ticket_size_asymmetry_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "f4e3d5cab4a98371b343266e4cdba4c209fb20163cb677d4d97c9674790b659c"
CLOCK = Path("data/high_volatility_directional_ticket_size_asymmetry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "f4ec92ddd8b3b6ff78554a46098c861986422b3f07e7d24cb7aed7b256657020"
CONTROL_DIR = Path("data/high_volatility_directional_ticket_size_asymmetry_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_directional_ticket_size_asymmetry_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_directional_ticket_size_asymmetry_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_ticket_tail_gate", "no_variation_gate", "mean_ticket_asymmetry", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDTSA frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDTSA novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdtsa_8_sequential_economics_v1", "policy_id": POLICY_ID,
