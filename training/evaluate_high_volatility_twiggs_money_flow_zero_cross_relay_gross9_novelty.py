"""Evaluate frozen HVTMF-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVTMF-24"
PROTOCOL = "hvtmf_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_twiggs_money_flow_zero_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTMF predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVTMF novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTMF predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTMF primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvtmf_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVTMF-24 unchanged before economic outcomes",
    }
