"""Sequential strict economics for frozen HVMRR-6."""
...
PREREG = Path("results/high_volatility_microstructure_ridge_relay_preregistration_2026-08-09.json")
PREREG_SHA = "7d0d76391471c6589ab389fc9b7e475adb1f44c7f7c4ebf4405a89f5a9b99380"
SUPPORT = Path("results/high_volatility_microstructure_ridge_relay_support_2026-08-09.json")
SUPPORT_SHA = "defcc145b186733f918602ceacf186947bbdd8a361fddaee89287c137f8854c7"
NOVELTY = Path("results/high_volatility_microstructure_ridge_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "9af96f6ea4ba5d2a6755f78721994a209250640d3b0aeb4ae35db3ecf94fec42"
CLOCK = Path("data/high_volatility_microstructure_ridge_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "e7440e01a1e54637c4858489f0396e2de26a9c65de57260038d99957baa77e5d"
CONTROL_DIR = Path("data/high_volatility_microstructure_ridge_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_microstructure_ridge_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_microstructure_ridge_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "no_prediction_strength_gate", "one_boundary_stale_features", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVMRR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVMRR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvmrr_6_sequential_economics_v1", "policy_id": POLICY_ID,
