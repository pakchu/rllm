"""Deterministic outcome-blind source support for HVCNAIR-8."""
...
from training import preregister_high_volatility_causal_trade_arrival_acceleration_inventory_relay as prereg
...
BAR_QUERY = """SELECT date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '8 hours' AS decision_time,sum(number_of_trades) FILTER (WHERE ts<date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '4 hours') AS first_trade_count,sum(number_of_trades) FILTER (WHERE ts>=date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '4 hours') AS second_trade_count,sum(ln(close/open)) AS completed_return,sum(power(ln(close/open),2)) AS minute_squared_return,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND high>=greatest(open,close,low) AND low<=least(open,close,high) AND number_of_trades>=0) AS coherent FROM bars_binance WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end GROUP BY 1 ORDER BY 1"""

ROOT = Path("data/high_volatility_causal_trade_arrival_acceleration_inventory_relay_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_trade_arrival_acceleration_inventory_relay_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_trade_arrival_acceleration_inventory_relay_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_causal_trade_arrival_acceleration_inventory_relay_support_2026-08-16.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "sum_open_interest",
    "previous_open_interest", "oi_change", "first_half_trade_count", "second_half_trade_count", "completed_return",
    "trade_arrival_acceleration", "trade_arrival_acceleration_rank", "realized_variation",
    "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "decision_time", "feature_available_time",
    "entry_time", "exit_time", "side", "oi_change", "first_half_trade_count", "second_half_trade_count", "completed_return",
    "trade_arrival_acceleration", "trade_arrival_acceleration_rank", "realized_variation", "variation_rank",
)
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVCNAIR-8 OI schema drift")
    expected_bars = [
        "decision_time", "first_trade_count", "second_trade_count", "completed_return", "minute_squared_return",
        "source_rows", "distinct_rows", "first_ts", "last_ts", "coherent",
    ]
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCNAIR-8 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all():
        raise RuntimeError("HVCNAIR-8 invalid OI rows")

    bars = bars.copy()
    for column in ("decision_time", "first_ts", "last_ts"):
        bars[column] = pd.to_datetime(bars[column], utc=True, errors="raise")
    numeric = ("first_trade_count", "second_trade_count", "completed_return", "minute_squared_return", "source_rows", "distinct_rows")
    for column in numeric:
        bars[column] = pd.to_numeric(bars[column], errors="raise")
    if bars["decision_time"].duplicated().any():
        raise RuntimeError("HVCNAIR-8 duplicate eight-hour bar")
    ...
    bars["trade_arrival_acceleration"] = np.log(bars["second_trade_count"] / bars["first_trade_count"])
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel = panel.join(
        bars[["bar_valid", "first_half_trade_count", "second_half_trade_count", "completed_return", "trade_arrival_acceleration", "realized_variation"]],
        on="decision_time",
    )
    panel["source_valid"] = (
        ...
        & np.isfinite(
            panel[["sum_open_interest", "previous_open_interest", "oi_change", "first_half_trade_count", "second_half_trade_count", "completed_return", "trade_arrival_acceleration", "realized_variation"]]
        ).all(axis=1)
        ...
        & panel["trade_arrival_acceleration"].gt(0)
    )
    valid = panel["source_valid"].eq(True)
    panel["trade_arrival_acceleration_rank"] = causal_midrank(panel["trade_arrival_acceleration"].where(valid))
    panel["variation_rank"] = causal_midrank(panel["realized_variation"].where(valid))
    panel["eligible"] = (
        valid
        & panel["oi_change"].gt(0)
        & panel["trade_arrival_acceleration_rank"].ge(POLICY["trade_arrival_acceleration_rank_min"])
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCNAIR-8 side or availability drift")
        ...
        rows.append({
            ...
            "trade_arrival_acceleration": float(row.trade_arrival_acceleration),
            "trade_arrival_acceleration_rank": float(row.trade_arrival_acceleration_rank),
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCNAIR-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcnair_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvcnair_8_source_support_v1", "policy_id": prereg.POLICY_ID,
