"""Build outcome-blind source support for frozen HVTAMC-8."""
...
PREREG_SHA="1b4e121fed923d600aa27721fce289c960f76049aa839c0b908b5d6c4f4eaae7"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_trade_arrival_memory_continuation_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_trade_arrival_memory_continuation_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_trade_arrival_memory_continuation_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_trade_arrival_memory_continuation_controls_2023_2026")
RESULT=Path("results/high_volatility_trade_arrival_memory_continuation_support_2026-08-10.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","arrival_memory","memory_rank","quote_volume_memory","quote_volume_memory_rank","realized_variation","variation_rank","completed_return","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","arrival_memory","memory_rank","quote_volume_memory","quote_volume_memory_rank","realized_variation","variation_rank","completed_return")
...
def lag_one_correlation(values:np.ndarray)->float:
 ...
 if values.shape!=(480,) or not np.isfinite(values).all():return math.nan
 left=values[:-1]-values[:-1].mean();right=values[1:]-values[1:].mean()
 denominator=math.sqrt(float(np.square(left).sum())*float(np.square(right).sum()))
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close","quote_asset_volume","number_of_trades"]:raise RuntimeError("HVTAMC source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVTAMC invalid source key")
 prices=x[["open","high","low","close"]];counts=x.number_of_trades
 x["row_valid"]=np.isfinite(prices).all(axis=1)&prices.gt(0).all(axis=1)&x.high.ge(prices[["open","close"]].max(axis=1))&x.low.le(prices[["open","close"]].min(axis=1))&x.high.ge(x.low)&np.isfinite(x.quote_asset_volume)&x.quote_asset_volume.ge(0)&np.isfinite(counts)&counts.ge(0)&counts.eq(np.floor(counts))
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("8h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   memory=lag_one_correlation(block.number_of_trades.to_numpy(float));volume_memory=lag_one_correlation(block.quote_asset_volume.to_numpy(float));var=float(np.square(block.minute_return.to_numpy(float)).sum());ret=float(math.log(block.close.iloc[-1]/block.open.iloc[0]));valid=all(math.isfinite(v) for v in (memory,volume_memory,var,ret)) and var>0 and ret!=0 and block.number_of_trades.sum()>0
  else:memory=volume_memory=var=ret=math.nan;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"arrival_memory":memory,"quote_volume_memory":volume_memory,"realized_variation":var,"completed_return":ret})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["memory_rank"]=prior_rank(panel.arrival_memory.where(valid));panel["quote_volume_memory_rank"]=prior_rank(panel.quote_volume_memory.where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));panel["eligible"]=valid&panel.memory_rank.ge(P["memory_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"]);panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_block_stale_geometry":
  cols=["source_valid","memory_rank","quote_volume_memory_rank","variation_rank","completed_return","feature_available_time"];used[cols]=panel[cols].shift(1)
 valid=used.source_valid.eq(True);memory=used.memory_rank.ge(P["memory_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&memory&variation
 if control=="no_memory_tail_gate":state=valid&variation
 elif control=="no_variation_gate":state=valid&memory
 elif control=="quote_volume_memory":state=valid&used.quote_volume_memory_rank.ge(P["memory_rank_min"])&variation
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVTAMC prereg drift")
 ...
 source_core={"protocol_version":"hvtamc_8_source_v1","query":QUERY,"query_sha256":hashlib.sha256(QUERY.encode()).hexdigest(),"window":[START.isoformat(),END.isoformat()],"physical_rows":len(raw),"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"panel":{"path":str(PANEL),"sha256":sha(PANEL),"rows":len(panel),"valid_rows":int(panel.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};manifest={**source_core,"manifest_hash":chash(source_core)};immutable(MANIFEST,json_bytes(manifest))
