"""Evaluate frozen DFSR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "DFSR-12"
PROTOCOL = "dfsr_12_gross9_novelty_v1"
...
OUTPUT = Path("results/dollar_factor_shock_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("DFSR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("DFSR novelty limits drift")
    if (
        ...
        raise RuntimeError("DFSR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="DFSR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "dfsr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject DFSR-12 unchanged before economic outcomes",
    }
