"""Evaluate frozen HVQST-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVQST-24"
PROTOCOL = "hvqst_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_qstick_zero_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVQST predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVQST novelty limits drift")
    if (
        ...
        raise RuntimeError("HVQST predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVQST primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvqst_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVQST-24 unchanged before economic outcomes",
    }
