"""Deterministic outcome-blind source support for HVCATAIR-8."""
...
from training import preregister_high_volatility_causal_average_ticket_acceleration_inventory_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "49e3fa536b7c0ebd83143d2f024fb079af2b9fea7d3980eaf1b670691bc5803d"
...
ROOT = Path("data/high_volatility_causal_average_ticket_acceleration_inventory_relay_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_average_ticket_acceleration_inventory_relay_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_average_ticket_acceleration_inventory_relay_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_causal_average_ticket_acceleration_inventory_relay_support_2026-08-16.json")
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVCATAIR-8 OI schema drift")
    expected_bars = [
    ...
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCATAIR-8 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all():
        raise RuntimeError("HVCATAIR-8 invalid OI rows")
    ...
    if bars["decision_time"].duplicated().any():
        raise RuntimeError("HVCATAIR-8 duplicate eight-hour bar")
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCATAIR-8 side or availability drift")
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCATAIR-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcatair_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvcatair_8_source_support_v1", "policy_id": prereg.POLICY_ID,
