"""Build outcome-blind source support for frozen HVTTF-24."""
...
from training import preregister_high_volatility_trend_trigger_factor_breakout_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "ec354e2f793b24e806de4e3a8faf403c4657b14f97e282be4f9e04e358446a9d"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_trend_trigger_factor_breakout_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_trend_trigger_factor_breakout_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_trend_trigger_factor_breakout_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_trend_trigger_factor_breakout_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_trend_trigger_factor_breakout_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "current_high", "current_low", "prior_high", "prior_low",
    "buy_power", "sell_power", "ttf", "outer_side", "zero_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "current_high", "current_low", "prior_high", "prior_low",
    "buy_power", "sell_power", "ttf", "outer_side", "zero_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVTTF source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVTTF invalid source key")
...
def trend_trigger_factor(high: pd.Series, low: pd.Series, valid: pd.Series) -> pd.DataFrame:
    period = P["trend_trigger_periods"]
    clean_high = high.where(valid)
    clean_low = low.where(valid)
    current_high = clean_high.rolling(period, min_periods=period).max()
    current_low = clean_low.rolling(period, min_periods=period).min()
    prior_high = current_high.shift(period)
    prior_low = current_low.shift(period)
    buy_power = current_high - prior_low
    sell_power = prior_high - current_low
    denominator = buy_power + sell_power
    ttf = (200.0 * (buy_power - sell_power) / denominator).where(denominator.gt(0))
    return pd.DataFrame(
        {"current_high": current_high, "current_low": current_low,
         "prior_high": prior_high, "prior_low": prior_low,
         "buy_power": buy_power, "sell_power": sell_power, "ttf": ttf}, index=high.index,
    )


def trigger_sides(ttf: pd.Series, upper: float, lower: float) -> pd.Series:
    previous = ttf.shift(1)
    long = ttf.gt(upper) & previous.le(upper)
    short = ttf.lt(lower) & previous.ge(lower)
    return pd.Series(np.where(long, 1, np.where(short, -1, 0)), index=ttf.index, dtype=int)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=pd.concat([bars,trend_trigger_factor(bars.bar_high,bars.bar_low,bars.valid_bar)],axis=1)
    bars["outer_side"]=trigger_sides(bars.ttf,P["upper_trigger"],P["lower_trigger"]);bars["zero_side"]=trigger_sides(bars.ttf,0.0,0.0)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum())
    numeric=["current_high","current_low","prior_high","prior_low","buy_power","sell_power","ttf","variation"]
    bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&(bars.buy_power+bars.sell_power).gt(0)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid))
    panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&panel.outer_side.ne(0)&panel.variation_rank.ge(P["variation_rank_min"])
    return panel.loc[:,PANEL_COLUMNS]


def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);variation=used.variation_rank.ge(P["variation_rank_min"]);side=pd.to_numeric(used.outer_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
    if control=="no_variation_gate":state=valid&side.ne(0)
    elif control=="zero_line_crossover":side=pd.to_numeric(used.zero_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
    elif control=="one_bar_stale_trigger":state=state.shift(1,fill_value=False);side=side.shift(1,fill_value=0)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVTTF prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvttf_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvttf_24_source_support_v1", "policy_id": prereg.POLICY_ID,
