"""Sequential strict economics for frozen HVWRBC-24."""
...
PREREG = Path("results/high_volatility_wide_range_body_continuation_relay_preregistration_2026-08-11.json")
PREREG_SHA = "64b28de751f95e66a5b8abeb5d3e8cac86ddb44b3692c5c2a173df96d237e51c"
SUPPORT = Path("results/high_volatility_wide_range_body_continuation_relay_support_2026-08-11.json")
SUPPORT_SHA = "d5d093538bd489c2687ca5097eac67b7de4864137bed9715a0ed162d4e8b2745"
NOVELTY = Path("results/high_volatility_wide_range_body_continuation_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "5a8a794d693409a7e62bb9bf2f3b557a9438b986cd714abdc2c584eb842fa4db"
CLOCK = Path("data/high_volatility_wide_range_body_continuation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "66e69835212a7d9e0a0f330c9b3d2487157a69facc0301fad0e58e34e62986fc"
CONTROL_DIR = Path("data/high_volatility_wide_range_body_continuation_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_wide_range_body_continuation_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_wide_range_body_continuation_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_wide_range_requirement", "no_variation_gate", "body_direction_only", "one_block_stale_prior_ranges", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVWRBC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVWRBC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvwrbc_24_sequential_economics_v1", "policy_id": POLICY_ID,
