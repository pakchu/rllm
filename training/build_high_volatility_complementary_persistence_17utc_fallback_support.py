"""Materialize deterministic, outcome-blind source support for HVCPF17-8."""
...
CLOCK = Path("data/high_volatility_complementary_persistence_17utc_fallback_clocks_2023_2026.csv.gz")
RESULT = Path("results/high_volatility_complementary_persistence_17utc_fallback_support_2026-08-16.json")
    ...
    for component, artifacts in prereg.COMPONENT_ARTIFACTS.items():
        for kind, artifact in artifacts.items():
            if prereg.sha256_file(artifact["path"]) != artifact["sha256"]:
                raise RuntimeError(f"HVCPF17-8 {component} {kind} drift")
...
def build_clock(clocks: Mapping[str, pd.DataFrame]) -> tuple[pd.DataFrame, dict[str, Any]]:
    if set(clocks) != set(prereg.COMPONENT_IDS):
        raise RuntimeError("HVCPF17-8 component roster drift")
    ...
    counts: dict[str, Any] = {
        ...
        "inactive_primary_non17_fallback_skipped": 0,
    ...
    for decision in decisions:
        ...
        side = prereg.fallback_17utc_side(primary_side, fallback_side, decision.hour)
        if side == 0:
            counts["inactive_primary_non17_fallback_skipped"] += 1
            continue
        ...
        if selected is None or selected.split != split or selected.feature_available_time > entry:
            raise RuntimeError("HVCPF17-8 stage or availability drift")
    ...
    if counts["emitted_decisions"] + counts["inactive_primary_non17_fallback_skipped"] != counts["union_decisions"]:
        raise RuntimeError("HVCPF17-8 incidence accounting drift")
    if len(output) > 1 and not output["entry_time"].iloc[1:].reset_index(drop=True).ge(output["exit_time"].iloc[:-1].reset_index(drop=True)).all():
        raise RuntimeError("HVCPF17-8 output overlap")
...
if __name__ == "__main__":
    ...
    print(json.dumps({"events": {k: v["events"] for k, v in report["support"].items()}, "accounting": report["limited_fallback_accounting"], "support_passed": report["support_passed"]}, indent=2))
