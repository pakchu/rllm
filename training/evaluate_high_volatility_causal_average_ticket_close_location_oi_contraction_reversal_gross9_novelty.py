"""Evaluate the frozen HVCATCLOICR-8 action-vote clock against Gross9."""
...
from training import preregister_high_volatility_causal_average_ticket_close_location_oi_contraction_reversal as prereg

POLICY = "HVCATCLOICR-8"
PROTOCOL = "hvcatcloicr_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_average_ticket_close_location_oi_contraction_reversal_clocks_2020_2026.csv.gz",
    "sha256": "fe5b0acaeb868917f458ca8633cfd7448fe2a4728ff73f3334e0252edc0923bd",
    "rows": 74,
}
OUTPUT = Path("results/high_volatility_causal_average_ticket_close_location_oi_contraction_reversal_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCATCLOICR-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATCLOICR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCATCLOICR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATCLOICR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCATCLOICR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCATCLOICR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCATCLOICR-8 unchanged before economics",
    }
