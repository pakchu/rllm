"""Sequential strict economics for frozen HVCCI-24."""
...
PREREG = Path("results/high_volatility_cci_extreme_reentry_relay_preregistration_2026-08-11.json")
PREREG_SHA = "c2f8aa8f42ff1ccb2ec093d84e53e0c48be57a1bdddbecba7c9f25e57cf6e543"
SUPPORT = Path("results/high_volatility_cci_extreme_reentry_relay_support_2026-08-11.json")
SUPPORT_SHA = "a0795c2193b2b2d608a0b3b4fb5cdffde292b562af701ba6c42b2c9d407b37e6"
NOVELTY = Path("results/high_volatility_cci_extreme_reentry_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "f55994cb543fe23b77030714f56cb9495815ed122a3abd757ee11492db5e964f"
CLOCK = Path("data/high_volatility_cci_extreme_reentry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "04536709ae58a19f4df477ef62d0d9621031d742f0d38b3a4774bb699cfcfd2e"
CONTROL_DIR = Path("data/high_volatility_cci_extreme_reentry_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_cci_extreme_reentry_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cci_extreme_reentry_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCCI frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCCI novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcci_24_sequential_economics_v1", "policy_id": POLICY_ID,
