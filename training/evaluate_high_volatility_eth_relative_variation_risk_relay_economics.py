"""Sequential strict economics for frozen HVERVR-8."""
...
PREREG = Path("results/high_volatility_eth_relative_variation_risk_relay_preregistration_2026-08-10.json")
PREREG_SHA = "6964200bdae9f3712a917ff4f867a4a7a505ef06934c007911e48e517a08aafb"
SUPPORT = Path("results/high_volatility_eth_relative_variation_risk_relay_support_2026-08-10.json")
SUPPORT_SHA = "53988f96e13b7c887e1b2c055ca07433598e9d990a5cab2bd0cf56a7265c020a"
NOVELTY = Path("results/high_volatility_eth_relative_variation_risk_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "0e8058616fb206db5be6d191dac8515d760351a2a6da1b39317e185a3acdd73c"
CLOCK = Path("data/high_volatility_eth_relative_variation_risk_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "3e4cad661098e0c82899eabf8d194cf05c3364302d3fd94e350ec533d62aa2db"
CONTROL_DIR = Path("data/high_volatility_eth_relative_variation_risk_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_eth_relative_variation_risk_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_eth_relative_variation_risk_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_relative_variation_gate", "no_btc_variation_gate", "btc_direction", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVERVR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVERVR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvervr_8_sequential_economics_v1", "policy_id": POLICY_ID,
