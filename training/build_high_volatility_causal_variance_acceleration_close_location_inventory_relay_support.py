"""Deterministic outcome-blind source support for HVCVARCLIR-8."""
...
from training import preregister_high_volatility_causal_variance_acceleration_close_location_inventory_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "74e43c0942827bfee4bbabc37eb845aaeb3734137843db3d87a0f9a3afb2f40a"
...
BAR_QUERY = """SELECT date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '8 hours' AS decision_time,sum(power(ln(close/open),2)) FILTER (WHERE ts<date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '4 hours') AS first_variance,sum(power(ln(close/open),2)) FILTER (WHERE ts>=date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '4 hours') AS second_variance,(array_agg(close ORDER BY ts DESC))[1] AS final_close,max(high) AS cycle_high,min(low) AS cycle_low,sum(ln(close/open)) AS completed_return,sum(power(ln(close/open),2)) AS minute_squared_return,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND high>=greatest(open,close,low) AND low<=least(open,close,high)) AS coherent FROM bars_binance WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end GROUP BY 1 ORDER BY 1"""

ROOT = Path("data/high_volatility_causal_variance_acceleration_close_location_inventory_relay_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_variance_acceleration_close_location_inventory_relay_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_variance_acceleration_close_location_inventory_relay_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_causal_variance_acceleration_close_location_inventory_relay_support_2026-08-16.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "sum_open_interest",
    "previous_open_interest", "oi_change", "first_half_variance", "second_half_variance",
    "close_location", "completed_return", "variance_acceleration", "variance_acceleration_rank",
    "realized_variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "decision_time", "feature_available_time",
    "entry_time", "exit_time", "side", "oi_change", "first_half_variance",
    "second_half_variance", "close_location", "completed_return", "variance_acceleration",
    "variance_acceleration_rank", "realized_variation", "variation_rank",
)
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    oi,bars=raw
    if oi.columns.tolist()!=["decision_time","sum_open_interest"]: raise RuntimeError("HVCVARCLIR-8 OI schema drift")
    expected=["decision_time","first_variance","second_variance","final_close","cycle_high","cycle_low","completed_return","minute_squared_return","source_rows","distinct_rows","first_ts","last_ts","coherent"]
    if bars.columns.tolist()!=expected: raise RuntimeError("HVCVARCLIR-8 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all() or not oi["sum_open_interest"].ge(0).all(): raise RuntimeError("HVCVARCLIR-8 invalid OI rows")
    bars=bars.copy()
    for c in ("decision_time","first_ts","last_ts"): bars[c]=pd.to_datetime(bars[c],utc=True,errors="raise")
    numeric=("first_variance","second_variance","final_close","cycle_high","cycle_low","completed_return","minute_squared_return","source_rows","distinct_rows")
    for c in numeric: bars[c]=pd.to_numeric(bars[c],errors="raise")
    start=bars["decision_time"]-pd.Timedelta("8h")
    bars["bar_valid"]=(np.isfinite(bars[list(numeric)]).all(axis=1)&bars["first_variance"].gt(0)&bars["second_variance"].gt(0)&bars["cycle_high"].gt(bars["cycle_low"])&bars["source_rows"].eq(480)&bars["distinct_rows"].eq(480)&bars["first_ts"].eq(start)&bars["last_ts"].eq(bars["decision_time"]-pd.Timedelta("1m"))&bars["coherent"].eq(True))
    bars["first_half_variance"]=bars["first_variance"];bars["second_half_variance"]=bars["second_variance"];bars["close_location"]=(bars["final_close"]-bars["cycle_low"])/(bars["cycle_high"]-bars["cycle_low"]);bars["variance_acceleration"]=np.log(bars["second_variance"]/bars["first_variance"]);bars["realized_variation"]=np.sqrt(bars["minute_squared_return"])
...
def build_panel(raw: tuple[pd.DataFrame,pd.DataFrame])->pd.DataFrame:
    oi,bars=prepare(raw);panel=oi.copy();panel["previous_time"]=panel["decision_time"].shift(1);panel["previous_open_interest"]=panel["sum_open_interest"].shift(1)
    positive=panel["sum_open_interest"].gt(0)&panel["previous_open_interest"].gt(0);panel["oi_change"]=np.nan;panel.loc[positive,"oi_change"]=np.log(panel.loc[positive,"sum_open_interest"]/panel.loc[positive,"previous_open_interest"])
    panel=panel.join(bars[["bar_valid","first_half_variance","second_half_variance","close_location","completed_return","variance_acceleration","realized_variation"]],on="decision_time")
    panel["source_valid"]=(panel["previous_time"].notna()&panel["decision_time"].sub(panel["previous_time"]).eq(pd.Timedelta("8h"))&panel["bar_valid"].eq(True)&np.isfinite(panel[["sum_open_interest","previous_open_interest","oi_change","first_half_variance","second_half_variance","close_location","completed_return","variance_acceleration","realized_variation"]]).all(axis=1)&panel["sum_open_interest"].gt(0)&panel["previous_open_interest"].gt(0)&panel["completed_return"].ne(0)&panel["close_location"].between(0,1)&panel["variance_acceleration"].gt(0)&panel["realized_variation"].gt(0))
    ...
    outer=POLICY["close_location_outer_fraction"];panel["eligible"]=(valid&panel["oi_change"].gt(0)&((panel["completed_return"].gt(0)&panel["close_location"].gt(1-outer))|(panel["completed_return"].lt(0)&panel["close_location"].lt(outer)))&panel["variance_acceleration_rank"].ge(POLICY["variance_acceleration_rank_min"])&panel["variation_rank"].ge(POLICY["variation_rank_min"]));panel["feature_available_time"]=panel["decision_time"];return panel.loc[:,PANEL_COLUMNS]
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCVARCLIR-8 side or availability drift")
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCVARCLIR-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcvarclir_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvcvarclir_8_source_support_v1", "policy_id": prereg.POLICY_ID,
