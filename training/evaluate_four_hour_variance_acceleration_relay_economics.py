"""Sequential strict economics for frozen FHVAR-2."""
...
PREREG = Path("results/four_hour_variance_acceleration_relay_preregistration_2026-08-08.json")
PREREG_SHA = "1d08ac508fb9a27139f3da6d4add2b7edda94f2e616e6897fca704fde021673e"
SUPPORT = Path("results/four_hour_variance_acceleration_relay_support_2026-08-08.json")
SUPPORT_SHA = "6df51705a98de3a0ecc5345046152f976af2f7bfc4d3bd6a5e9476e6c75e8cd3"
NOVELTY = Path("results/four_hour_variance_acceleration_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "811bb91d7ba2b5472f55bdb3e481a88f212da3f94a06254d784cfc8c0f1a22ad"
CLOCK = Path("data/four_hour_variance_acceleration_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "20e00a2b61665a5e2006cf96f6fd3fd940ca79c473cff298346035915e432b11"
CONTROL_DIR = Path("data/four_hour_variance_acceleration_relay_controls_2023_2026")
FREEZE = Path("results/four_hour_variance_acceleration_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/four_hour_variance_acceleration_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "no_acceleration_gate", "one_boundary_stale_geometry", "direction_fade")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("FHVAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("FHVAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "fhvar_2_sequential_economics_v1", "policy_id": POLICY_ID,
