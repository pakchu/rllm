"""Build outcome-blind source support for frozen HVALMA-24."""
...
from training import preregister_high_volatility_arnaud_legoux_moving_average_turn_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "b2a16f4d6e21388d0b77d5605870eb2ceed11c32ffccc2b5d83c288d87280188"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_arnaud_legoux_moving_average_turn_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_arnaud_legoux_moving_average_turn_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_arnaud_legoux_moving_average_turn_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_arnaud_legoux_moving_average_turn_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_arnaud_legoux_moving_average_turn_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "alma", "slope", "sma_9", "sma_9_slope",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "alma", "slope", "sma_9", "sma_9_slope",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVALMA source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVALMA invalid source key")
...
def alma_average(series: pd.Series, valid: pd.Series, periods: int, offset: float, sigma: float) -> pd.Series:
    positions=np.arange(periods,dtype=float);center=offset*(periods-1);width=periods/sigma;weights=np.exp(-np.square(positions-center)/(2*width**2));denominator=float(weights.sum())
    output=series.rolling(periods,min_periods=periods).apply(lambda values: float(np.dot(values,weights)/denominator),raw=True)
    complete=valid.rolling(periods,min_periods=periods).sum().eq(periods)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["alma"]=alma_average(bars.bar_close,bars.valid_bar,P["window_periods"],P["offset"],P["sigma"]);bars["slope"]=bars.alma-bars.alma.shift(1)
    bars["sma_9"]=bars.bar_close.rolling(P["window_periods"],min_periods=P["window_periods"]).mean().where(bars.valid_bar.rolling(P["window_periods"],min_periods=P["window_periods"]).sum().eq(P["window_periods"]));bars["sma_9_slope"]=bars.sma_9-bars.sma_9.shift(1)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum())
    bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["alma","slope","sma_9","sma_9_slope","variation"]]).all(axis=1)&bars.slope.ne(0)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.slope.shift(1)
    panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&previous.ne(0)&np.sign(panel.slope).ne(np.sign(previous))&panel.variation_rank.ge(P["variation_rank_min"])
    return panel.loc[:,PANEL_COLUMNS]

def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);signal=pd.to_numeric(used.slope,errors="coerce");previous=signal.shift(1)
    crossing=signal.ne(0)&previous.ne(0)&np.sign(signal).ne(np.sign(previous));variation=used.variation_rank.ge(P["variation_rank_min"]);side=np.sign(signal).fillna(0).astype(int);state=valid&side.ne(0)&crossing&variation
    if control=="no_variation_gate":state=valid&side.ne(0)&crossing
    elif control=="sma_9_turn":
        signal=pd.to_numeric(used.sma_9_slope,errors="coerce");previous=signal.shift(1);state=valid&signal.ne(0)&previous.ne(0)&np.sign(signal).ne(np.sign(previous))&variation;side=np.sign(signal).fillna(0).astype(int)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVALMA prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvalma_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvalma_24_source_support_v1", "policy_id": prereg.POLICY_ID,
