"""Outcome-blind preregistration for HVCVCOIR-12."""
...
def build() -> dict[str, Any]:
    core = {
        ...
        "mechanism": {
            ...
            "why_distinct": (
                "HVCVCOIR is a causal realized-variance allocation state transition. It uses neither "
        ...
        "clock": {
            ...
            "open_interest_confirmation": "at release decision D, exact positive BTCUSDT period=5m sum_open_interest observations at D-4h and D with strict positive log(OI[D]/OI[D-4h])",
        ...
        "source_plan": {
            "open_interest": "read-only Postgres open_interest_binance BTCUSDT period=5m exact endpoints after preregistration",
        ...
        "research_boundary": {
            "original_HVVCR_source_incidence_and_novelty_failure_known": True,
            "original_HVVCR_economic_outcomes_opened": False,
        ...
        "stopping_rule": "terminal first failure; no threshold, release, OI endpoint or expansion rule, side, hold, or gate repair",
    }
...
def validate(payload: dict[str, Any]) -> None:
    ...
    if payload.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError("HVCVCOIR preregistration hash mismatch")
    for raw, expected in SOURCE_BINDINGS.items():
        if hashlib.sha256(Path(raw).read_bytes()).hexdigest() != expected:
            raise RuntimeError(f"HVCVCOIR source drift: {raw}")
