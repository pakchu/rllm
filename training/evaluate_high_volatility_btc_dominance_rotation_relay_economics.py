"""Sequential strict economics for frozen HVBDRR-8."""
...
PREREG = Path("results/high_volatility_btc_dominance_rotation_relay_preregistration_2026-08-09.json")
PREREG_SHA = "307a9c751cc93709421f1c15f3f6e5e6d42508eaafc60cebaabfff22f93a3440"
SUPPORT = Path("results/high_volatility_btc_dominance_rotation_relay_support_2026-08-09.json")
SUPPORT_SHA = "0061d2c714bd082db5cb21aaa949c1fbfa00b898a07f6f3cfaab67ac208f1128"
NOVELTY = Path("results/high_volatility_btc_dominance_rotation_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "22d4eadbbe5559df5c77cb489b1263683e82a48224ae217d72ffab1aaec42b50"
CLOCK = Path("data/high_volatility_btc_dominance_rotation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "c9831815e8bf8a7af83fc92bbe9cae5d80550273624f65fae66a074b554e5d17"
CONTROL_DIR = Path("data/high_volatility_btc_dominance_rotation_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_btc_dominance_rotation_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_btc_dominance_rotation_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_residual_tail", "no_dispersion_gate", "no_variation_gate", "alt_factor_direction", "one_block_stale_geometry", "direction_flip", "same_clock_forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVBDRR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVBDRR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvbdrr_8_sequential_economics_v1", "policy_id": POLICY_ID,
