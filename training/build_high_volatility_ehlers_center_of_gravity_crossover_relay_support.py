"""Build outcome-blind source support for frozen HVECOG-24."""
...
from training import preregister_high_volatility_ehlers_center_of_gravity_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "1af12ac42688bc10f62fbd459191300781d00e709c8ea09c696d66861df0e3b5"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ehlers_center_of_gravity_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ehlers_center_of_gravity_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ehlers_center_of_gravity_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ehlers_center_of_gravity_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ehlers_center_of_gravity_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "median_price", "cog", "trigger", "difference",
    "entry_side", "zero_line_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "median_price", "cog", "trigger", "difference",
    "entry_side", "zero_line_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVECOG source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVECOG invalid source key")
...
def center_of_gravity(series: pd.Series, valid: pd.Series) -> pd.Series:
    period=P["cog_periods"];output=np.full(len(series),np.nan);window=[];weights=np.arange(1,period+1,dtype=float)
    for i,(value,ok) in enumerate(zip(series.to_numpy(float),valid.to_numpy(bool))):
        ...
        if len(window)==period:
            newest_first=np.asarray(window[::-1]);den=newest_first.sum()
            if den>0:output[i]=-float(np.dot(weights,newest_first))/den+(period+1)/2
    return pd.Series(output,index=series.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    fields=["bar_open","bar_high","bar_low","bar_close"];bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[fields]).all(axis=1)&bars[fields].gt(0).all(axis=1);bars["median_price"]=(bars.bar_high+bars.bar_low)/2;bars["cog"]=center_of_gravity(bars.median_price,bars.valid_bar);bars["trigger"]=bars.cog.shift(1);bars["difference"]=bars.cog-bars.trigger;bars["entry_side"]=cross_side(bars.difference);bars["zero_line_side"]=cross_side(bars.cog)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["median_price","cog","trigger","difference","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="cog_zero_cross":side=pd.to_numeric(used.zero_line_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVECOG prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvecog_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvecog_24_source_support_v1", "policy_id": prereg.POLICY_ID,
