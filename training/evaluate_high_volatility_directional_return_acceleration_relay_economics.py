"""Sequential strict economics for frozen HVDRA-8."""
...
PREREG = Path("results/high_volatility_directional_return_acceleration_relay_preregistration_2026-08-09.json")
PREREG_SHA = "af78e34ab3de62615fdd77b9404bb5423ce3b6f5c741499391c4a7ff54d1efca"
SUPPORT = Path("results/high_volatility_directional_return_acceleration_relay_support_2026-08-09.json")
SUPPORT_SHA = "9768b47b20a8e7162c1b85c3cef309b546921a0c54442e56addb426f1ee8e19b"
NOVELTY = Path("results/high_volatility_directional_return_acceleration_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "17a569ce6dc214bbf47e57bce9c3a02b3040e387d1a5842002086655f38dd8e0"
CLOCK = Path("data/high_volatility_directional_return_acceleration_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "36fab6eb6ee4aedb34ffc300abf69ab3b1dc7c7750f4c4d3e23c1a6215ed8d55"
CONTROL_DIR = Path("data/high_volatility_directional_return_acceleration_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_directional_return_acceleration_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_directional_return_acceleration_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_acceleration_gate", "no_variation_gate", "net_block_return_side", "one_block_stale_features", "direction_flip", "same_clock_forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDRA frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDRA novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdra_8_sequential_economics_v1", "policy_id": POLICY_ID,
