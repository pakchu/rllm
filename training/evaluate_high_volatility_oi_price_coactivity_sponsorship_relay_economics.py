"""Sequential strict economics for frozen HVOIPCSR-8."""
...
PREREG = Path("results/high_volatility_oi_price_coactivity_sponsorship_relay_preregistration_2026-08-10.json")
PREREG_SHA = "7fd82290e7f98c82d59af3df684094b548869ed79047c7fe3c9db1ae16f99e1a"
SUPPORT = Path("results/high_volatility_oi_price_coactivity_sponsorship_relay_support_2026-08-10.json")
SUPPORT_SHA = "771024bb0e286e7db4d7955345b66db5a63d45d65fc95f1f0ddd43642d2724db"
NOVELTY = Path("results/high_volatility_oi_price_coactivity_sponsorship_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "45927fb0328b5ce48de677f9a383da5ce77f25b2438b76d9086bfc7f04c6617f"
CLOCK = Path("data/high_volatility_oi_price_coactivity_sponsorship_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "36b6b7baed774eb8cf1ce17224bc37108f9685265ff6650416a4e6775bcf23f4"
CONTROL_DIR = Path("data/high_volatility_oi_price_coactivity_sponsorship_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_oi_price_coactivity_sponsorship_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_oi_price_coactivity_sponsorship_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_coactivity_gate", "no_gross_oi_activity_gate", "no_variation_gate", "one_block_stale_features", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVOIPCSR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVOIPCSR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvoipcsr_8_sequential_economics_v1", "policy_id": POLICY_ID,
