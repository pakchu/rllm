"""Sequential strict economics for frozen HVWPR-24."""
...
PREREG = Path("results/high_volatility_williams_percent_r_reentry_relay_preregistration_2026-08-11.json")
PREREG_SHA = "a58338902f376c64e0244ed0db888560bdaed01ddfaeaa093a5ee320a6badf58"
SUPPORT = Path("results/high_volatility_williams_percent_r_reentry_relay_support_2026-08-11.json")
SUPPORT_SHA = "399b3789f30a32f1f778e350739a3d7cfb26b91ac3ef161c9b874efd5b0704f5"
NOVELTY = Path("results/high_volatility_williams_percent_r_reentry_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "9a8488910d99923f0ebdf5d51fcf39ba60b96834be0d1bfdf029ae6f1cc474a0"
CLOCK = Path("data/high_volatility_williams_percent_r_reentry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d4dd7487dab7bd4967edbc8e9774e29b1a48a425d0b5f0f28a9600f26cfef7ff"
CONTROL_DIR = Path("data/high_volatility_williams_percent_r_reentry_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_williams_percent_r_reentry_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_williams_percent_r_reentry_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "outward_crossing", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVWPR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVWPR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvwpr_24_sequential_economics_v1", "policy_id": POLICY_ID,
