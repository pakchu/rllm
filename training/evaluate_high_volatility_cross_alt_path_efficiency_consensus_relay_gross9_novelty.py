"""Evaluate frozen HVCAPEC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCAPEC-8"
PROTOCOL = "hvcapec_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_cross_alt_path_efficiency_consensus_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCAPEC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCAPEC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCAPEC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCAPEC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCAPEC-8 unchanged before economic outcomes",
    }
