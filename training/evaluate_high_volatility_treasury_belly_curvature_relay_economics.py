"""Sequential strict economics for frozen HVTBCR-24."""
...
PREREG = Path("results/high_volatility_treasury_belly_curvature_relay_preregistration_2026-08-10.json")
PREREG_SHA = "a7cf756abf1d5ccd096bb8697ac1740abc015bcc1af46e09468a93411994670e"
SUPPORT = Path("results/high_volatility_treasury_belly_curvature_relay_support_2026-08-10.json")
SUPPORT_SHA = "cdbbfff8b9be04e6e11a261f5bdb6cf2eb6d2d7586dbef53858c0c9a3f726b87"
NOVELTY = Path("results/high_volatility_treasury_belly_curvature_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "6b7e9a8ad82e67506f758907dcab618d1203e9480c8b1062e34e0aa9935fb023"
CLOCK = Path("data/high_volatility_treasury_belly_curvature_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "4d458f6ae879d15e3184c057f6c85bd089c7a8117e7c35420644fa39e3c18886"
CONTROL_DIR = Path("data/high_volatility_treasury_belly_curvature_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_treasury_belly_curvature_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_treasury_belly_curvature_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "curvature_level", "one_observation_stale_curvature_change", "direction_flip", "same_clock_forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTBCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTBCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvtbcr_24_sequential_economics_v1", "policy_id": POLICY_ID,
