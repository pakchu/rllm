...
ROOT = Path("data/high_volatility_random_walk_index_cross_relay_sources_2023_2026")
...
PANEL_COLUMNS = (
    ...
    "bar_low", "bar_close", "true_range", "rwi_high", "rwi_low", "rwi_spread",
    "fixed_14_high", "fixed_14_low", "entry_side", "fixed_14_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "true_range", "rwi_high", "rwi_low",
    "rwi_spread", "fixed_14_high", "fixed_14_low", "entry_side", "fixed_14_side",
    "variation", "variation_rank", "eligible",
)
...
def rwi_values(bars: pd.DataFrame, valid: pd.Series, maximum: int) -> pd.DataFrame:
    ...
    for horizon in range(1, maximum+1):
        prior_atr = true_range.shift(1).rolling(horizon, min_periods=horizon).mean()
        denominator = prior_atr * math.sqrt(horizon)
        complete = valid & valid.shift(horizon, fill_value=False) & denominator.gt(0)
        highs.append(((bars.bar_high-bars.bar_low.shift(horizon))/denominator).where(complete))
        lows.append(((bars.bar_high.shift(horizon)-bars.bar_low)/denominator).where(complete))
    high_frame = pd.concat(highs, axis=1)
    low_frame = pd.concat(lows, axis=1)
    full = high_frame.notna().all(axis=1) & low_frame.notna().all(axis=1)
    return pd.DataFrame({
        "true_range": true_range,
        "rwi_high": high_frame.max(axis=1).where(full),
        "rwi_low": low_frame.max(axis=1).where(full),
        "fixed_14_high": highs[-1].where(full),
        "fixed_14_low": lows[-1].where(full),
    }, index=bars.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    values=rwi_values(bars,bars.valid_bar,P["maximum_periods"])
    for column in values:bars[column]=values[column]
    bars["rwi_spread"]=bars.rwi_high-bars.rwi_low;bars["entry_side"]=cross_side(bars.rwi_spread);bars["fixed_14_side"]=cross_side(bars.fixed_14_high-bars.fixed_14_low)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["true_range","rwi_high","rwi_low","rwi_spread","fixed_14_high","fixed_14_low","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.true_range.gt(0)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="fixed_14_only":side=pd.to_numeric(used.fixed_14_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    ...
    source_core = {
        "protocol_version": "hvrwi_24_source_v1", "query": QUERY,
