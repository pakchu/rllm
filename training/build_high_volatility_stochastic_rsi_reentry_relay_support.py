"""Build outcome-blind source support for frozen HVSR-24."""
...
from training import preregister_high_volatility_stochastic_rsi_reentry_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "c079f450d1e5ef0cb408389509d7751ad9f23cf930bc75f645c44e78ed5ef235"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_stochastic_rsi_reentry_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_stochastic_rsi_reentry_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_stochastic_rsi_reentry_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_stochastic_rsi_reentry_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_stochastic_rsi_reentry_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "change", "avg_gain", "avg_loss", "rsi", "lowest_rsi",
    "highest_rsi", "stoch_rsi", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "rsi", "lowest_rsi", "highest_rsi",
    "stoch_rsi", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVSR source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVSR invalid source key")
...
def stochastic_rsi(close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    change=(close-close.shift(1)).where(valid&valid.shift(1,fill_value=False));period=P["rsi_periods"];values=change.to_numpy(float);gain=np.full(len(close),np.nan);loss=np.full(len(close),np.nan);rsi=np.full(len(close),np.nan);run=0;g=l=np.nan
    for i,value in enumerate(values):
        if not math.isfinite(value):run=0;g=l=np.nan;continue
        run+=1
        if run<period:continue
        if run==period:
            window=values[i-period+1:i+1];g=float(np.maximum(window,0).mean());l=float(np.maximum(-window,0).mean())
        else:g=((period-1)*g+max(value,0))/period;l=((period-1)*l+max(-value,0))/period
        gain[i]=g;loss[i]=l
        if g+l>0:rsi[i]=100.0*g/(g+l)
    rsi_series=pd.Series(rsi,index=close.index);stoch_period=P["stoch_periods"];lowest=rsi_series.rolling(stoch_period,min_periods=stoch_period).min();highest=rsi_series.rolling(stoch_period,min_periods=stoch_period).max();span=highest-lowest;stoch=(100.0*(rsi_series-lowest)/span).where(span.gt(0));return pd.DataFrame({"change":change,"avg_gain":gain,"avg_loss":loss,"rsi":rsi_series,"lowest_rsi":lowest,"highest_rsi":highest,"stoch_rsi":stoch},index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[["bar_open","bar_high","bar_low","bar_close"]]).all(axis=1)&bars[["bar_open","bar_high","bar_low","bar_close"]].gt(0).all(axis=1);bars=pd.concat([bars,stochastic_rsi(bars.bar_close,bars.valid_bar)],axis=1);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["change","avg_gain","avg_loss","rsi","lowest_rsi","highest_rsi","stoch_rsi","variation"]]).all(axis=1)&(bars.highest_rsi-bars.lowest_rsi).gt(0)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.stoch_rsi.shift(1);long=previous.le(P["lower_level"])&panel.stoch_rsi.gt(P["lower_level"]);short=previous.ge(P["upper_level"])&panel.stoch_rsi.lt(P["upper_level"]);panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&(long|short)&panel.variation_rank.ge(P["variation_rank_min"]);return panel.loc[:,PANEL_COLUMNS]


def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);value=pd.to_numeric(used.stoch_rsi,errors="coerce");previous=value.shift(1);long=previous.le(P["lower_level"])&value.gt(P["lower_level"]);short=previous.ge(P["upper_level"])&value.lt(P["upper_level"]);variation=used.variation_rank.ge(P["variation_rank_min"]);side=pd.Series(np.where(long,1,np.where(short,-1,0)),index=used.index,dtype=int);state=valid&side.ne(0)&variation
    if control=="no_variation_gate":state=valid&side.ne(0)
    elif control=="outward_crossing":
        long=previous.le(P["upper_level"])&value.gt(P["upper_level"]);short=previous.ge(P["lower_level"])&value.lt(P["lower_level"]);side=pd.Series(np.where(long,1,np.where(short,-1,0)),index=used.index,dtype=int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVSR prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvsr_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvsr_24_source_support_v1", "policy_id": prereg.POLICY_ID,
