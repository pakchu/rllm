"""Evaluate frozen HVIGDR-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVIGDR-6"
PROTOCOL = "hvigdr_6_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_interbar_gap_dominance_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVIGDR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVIGDR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVIGDR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVIGDR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvigdr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVIGDR-6 unchanged before economic outcomes",
    }
