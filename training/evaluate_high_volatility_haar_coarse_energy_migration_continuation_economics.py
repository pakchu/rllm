"""Sequential strict economics for frozen HVHCEM-12."""
...
PREREG = Path("results/high_volatility_haar_coarse_energy_migration_continuation_preregistration_2026-08-10.json")
PREREG_SHA = "524df960b81c1791ea32b97521cbd17db0e3c8c008bc7f58469f309d68ac4900"
SUPPORT = Path("results/high_volatility_haar_coarse_energy_migration_continuation_support_2026-08-10.json")
SUPPORT_SHA = "a881f77c6e6ef03bb472f8fc61770bbe5f9cb7f885a7dbcfc91e46555b489773"
NOVELTY = Path("results/high_volatility_haar_coarse_energy_migration_continuation_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "ab60e79ecc3c1490fd03aa9d10081d126de4ff0421778556a493de1cccfb2241"
CLOCK = Path("data/high_volatility_haar_coarse_energy_migration_continuation_clocks_2023_2026.csv.gz")
CLOCK_SHA = "924837705a4cc6723377209e228acafbbfc669e22242d91a9e81c11b679a35d6"
CONTROL_DIR = Path("data/high_volatility_haar_coarse_energy_migration_continuation_controls_2023_2026")
FREEZE = Path("results/high_volatility_haar_coarse_energy_migration_continuation_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_haar_coarse_energy_migration_continuation_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_migration_gate", "no_volatility_gate", "coarse_share_level", "one_boundary_stale_migration", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVHCEM frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVHCEM novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvhcem_12_sequential_economics_v1", "policy_id": POLICY_ID,
