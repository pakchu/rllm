"""Evaluate the frozen HVCTIIR-8 action-vote clock against Gross9."""
...
POLICY = "HVCTIIR-8"
PROTOCOL = "hvctiir_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_ticket_impact_inventory_relay_clocks_2020_2026.csv.gz",
    "sha256": "152ec08a723345f517c3b099ceb81f03c93f389eec9d9a214437ce01f3a4ff58",
    "rows": 273,
}
OUTPUT = Path("results/high_volatility_causal_ticket_impact_inventory_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCTIIR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCTIIR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCTIIR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCTIIR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCTIIR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCTIIR-8 unchanged before economics",
    }
