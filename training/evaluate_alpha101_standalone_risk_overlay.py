"""Risk overlay sweep for standalone Alpha101 quantile candidates."""
from __future__ import annotations
import json, math
from pathlib import Path
import numpy as np
from training.search_alpha101_derivative_alphas import add_features
from training.search_alpha101_random_quantile_standalone import active
from training.search_vpin_formulaic_alpha import load_market_and_splits, stats

SRC='results/alpha101_random_quantile_standalone_2026-07-09.json'
OUT='results/alpha101_standalone_risk_overlay_2026-07-09.json'
DOC='docs/alpha101-standalone-risk-overlay-2026-07-09.md'
COST=0.0006
TPS=[0.006,0.01,0.015,0.02]
SLS=[None,0.01,0.02]
MAX_HOLDS=[72,96,144]
...
def trade_overlay(m,p,max_hold,side,tp,sl):
    ...
    for j in range(ep,xp):
        if side=='long':
            ...
            # Conservative: stop before target if both touched same bar.
            ...
            if tp is not None and favorable>=tp:
                exit_px=entry*(1-tp); min_rel=min(min_rel,eq*(1+tp)); break
            min_rel=min(min_rel,eq*max(0,1+adverse)); exit_px=op[j+1]
    if side=='long': r=(exit_px-entry)/entry
    else: r=(entry-exit_px)/entry
    fac=max(0,1+r)*(1-COST)
    min_rel=min(min_rel,fac)
...
def eval_terms(m,f,masks,years,terms,side,max_hold,stride,tp,sl):
    ...
    n=len(m); smod=(np.arange(n)%stride)==0; out={}
    for sp,mask in masks.items():
        loc=[]; nxt=0; idx=np.flatnonzero(act & mask & smod); idx=idx[(idx>=143)&(idx<n-max_hold-2)]
        for p in idx:
            p=int(p); xp=p+1+max_hold
            if p<nxt or xp>=n or not mask[min(xp,n-1)]: continue
...
def score(st):
    ...
    pos=all(st[x]['cagr_pct']>0 for x in ['test2024','eval2025','ytd2026'])
    ...
    return (pos and enough, minr, ret, y['cagr_to_strict_mdd'])

def main():
    src=json.load(open(SRC))['top'][:8]
    # unique rules from top plus all-positive stored rules
    seen=set(); rules=[]
    for r in src:
    ...
    rep={'protocol':'Standalone Alpha101 quantile candidates with conservative intrabar TP/SL overlay. Thresholds from train<2024 only; overlay sweep is diagnostic. 6bp/side, strict MDD.','top':[{k:v for k,v in r.items() if k!='score_tuple'} for r in rows[:120]],'all_count':len(rows)}
    ...
    md=['# Alpha101 standalone risk overlay (2026-07-09)','',rep['protocol'],'','| rank | base | side | hold/stride | TP/SL | 2024 | 2025 | 2026 | terms |','|---:|---|---|---:|---:|---:|---:|---:|---|']
    for i,r in enumerate(rep['top'][:50],1):
        st=r['stats']; terms='; '.join(f"{t['feature']} {t['op']} q{t.get('train_q',0):.2f}({t['threshold']:.4g})" for t in r['terms'])
        md.append(f"| {i} | {r['base_name']} | {r['side']} | {r['max_hold']}/{r['stride']} | {r['take_profit']}/{r['stop_loss']} | {fmt(st['test2024'])} | {fmt(st['eval2025'])} | {fmt(st['ytd2026'])} | `{terms}` |")
    Path(DOC).write_text('\n'.join(md)+'\n')
