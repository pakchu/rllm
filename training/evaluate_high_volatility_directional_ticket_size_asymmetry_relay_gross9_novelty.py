"""Evaluate frozen HVDTSA-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDTSA-8"
PROTOCOL = "hvdtsa_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_directional_ticket_size_asymmetry_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDTSA predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVDTSA novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDTSA predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDTSA primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdtsa_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVDTSA-8 unchanged before economic outcomes",
    }
