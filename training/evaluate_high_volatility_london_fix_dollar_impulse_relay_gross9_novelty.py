"""Evaluate frozen HVLFX-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVLFX-12"
PROTOCOL = "hvlfx_12_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_london_fix_dollar_impulse_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVLFX predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVLFX novelty limits drift")
    if (
        ...
        raise RuntimeError("HVLFX predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVLFX primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvlfx_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVLFX-12 unchanged before economic outcomes",
    }
