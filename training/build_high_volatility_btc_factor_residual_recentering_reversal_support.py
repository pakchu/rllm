"""Deterministic source support for HVBFRRCR-8."""
...
from training import preregister_high_volatility_btc_factor_residual_recentering_reversal as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv

PREREG_SHA = "71e3b60c6cbcc78998d183fdf6d852e9d082b99d90d73a4567c7ff4362062971"
FEATURES = Path("data/high_volatility_btc_factor_residual_recentering_reversal_features_2023_2026.csv.gz")
CLOCK = Path("data/high_volatility_btc_factor_residual_recentering_reversal_clocks_2023_2026.csv.gz")
RESULT = Path("results/high_volatility_btc_factor_residual_recentering_reversal_support_2026-08-18.json")
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if any(column not in frame for column in required):
        raise RuntimeError("HVBFRRCR-8 source schema drift")
    ...
    if output["decision_time"].duplicated().any() or not output["decision_time"].is_monotonic_increasing:
        raise RuntimeError("HVBFRRCR-8 decision order drift")
    ...
    same_sign = np.sign(output["residual"]) == np.sign(output["previous_residual"])
    output["eligible"] = (finite & consecutive & output["alt_mad"].gt(0) & output["previous_alt_mad"].gt(0) & output["residual"].ne(0) & output["previous_residual"].ne(0) & same_sign & output["residual"].abs().lt(output["previous_residual"].abs()) & output["prior_standardized_residual"].ge(prereg.build()["policy"]["prior_standardized_residual_min"]) & output["variation_rank"].ge(prereg.build()["policy"]["variation_rank_min"]))
    return output
...
def build_clock(features: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(output)>1 and not output["entry_time"].iloc[1:].reset_index(drop=True).ge(output["exit_time"].iloc[:-1].reset_index(drop=True)).all(): raise RuntimeError("HVBFRRCR-8 overlap")
    return output
...
def run(features_path: Path = FEATURES, clock_path: Path = CLOCK, result_path: Path = RESULT) -> dict[str, Any]:
    if prereg.sha256(prereg.DEFAULT_OUTPUT)!=PREREG_SHA: raise RuntimeError("HVBFRRCR-8 prereg drift")
