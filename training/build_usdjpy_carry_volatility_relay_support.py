"""Materialize source-only UJCVR-12 support clocks."""
...
SOURCE_DIR=Path("data/usdjpy_carry_volatility_relay_sources_2023_2026"); SESSION=SOURCE_DIR/"usdjpy_sessions.csv.gz"; SOURCE_MANIFEST=SOURCE_DIR/"manifest.json"
...
CLOCK=Path("data/usdjpy_carry_volatility_relay_clocks_2023_2026.csv.gz"); CONTROL_DIR=Path("data/usdjpy_carry_volatility_relay_controls_2023_2026"); RESULT=Path("results/usdjpy_carry_volatility_relay_support_2026-08-08.json")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00:00Z"),pd.Timestamp("2024-01-01T00:00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00:00Z"),pd.Timestamp("2025-01-01T00:00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00:00Z"),pd.Timestamp("2026-01-01T00:00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00:00Z"),pd.Timestamp("2026-08-01T00:00:00Z"))}; MINIMUM={"train":8,"test":12,"eval":12,"final":8}; CONTROLS=("no_volatility_gate","no_fx_tail","one_session_stale_fx_shock","direction_flip")
COLUMNS=("candidate","control","split","source_day","decision_time","feature_available_time","entry_time","exit_time","side","fx_return","fx_return_z","btc_realized_variation","btc_realized_variation_rank")
QUERY="""SELECT date_trunc('day',ts) AS source_day,(array_agg(open ORDER BY ts))[1] AS session_open,max(high) AS session_high,min(low) AS session_low,(array_agg(close ORDER BY ts DESC))[1] AS session_close,count(*) AS source_rows,count(DISTINCT ts) AS distinct_timestamps,min(ts) AS first_ts,max(ts) AS last_ts FROM bars_polygon WHERE symbol='USDJPY' AND interval='1m' AND ts>=:start AND ts<:end AND extract(isodow from ts) BETWEEN 1 AND 5 AND extract(hour from ts)>=13 AND extract(hour from ts)<21 GROUP BY 1 ORDER BY 1"""
...
def materialize_sessions()->dict:
 ...
 for c in ("session_open","session_high","session_low","session_close"):frame[c]=pd.to_numeric(frame[c],errors="coerce")
 frame["source_valid"]=frame.distinct_timestamps.ge(450)&frame.first_ts.le(frame.source_day+pd.Timedelta(hours=13,minutes=5))&frame.last_ts.ge(frame.source_day+pd.Timedelta(hours=20,minutes=55))&np.isfinite(frame[["session_open","session_high","session_low","session_close"]]).all(axis=1)&frame[["session_open","session_high","session_low","session_close"]].gt(0).all(axis=1)&frame.session_high.ge(frame[["session_open","session_close"]].max(axis=1))&frame.session_low.le(frame[["session_open","session_close"]].min(axis=1));frame["decision_time"]=frame.source_day+pd.Timedelta(hours=21);frame["fx_return"]=np.log(frame.session_close/frame.session_open).where(frame.source_valid);frame["fx_return_z"]=causal_z(frame.fx_return)
 SOURCE_DIR.mkdir(parents=True,exist_ok=True);_write_gzip_csv(frame,SESSION);core={"protocol_version":"ujcvr_12_usdjpy_source_v1","query":QUERY,"table":"bars_polygon","symbol":"USDJPY","interval":"1m","window":[START.isoformat(),END.isoformat()],"outcomes_opened":False,"candidate_incidence_opened":False,"no_imputation":True,"output":{"path":str(SESSION),"sha256":sha(SESSION),"rows":len(frame),"valid_rows":int(frame.source_valid.sum())}};payload={**core,"manifest_hash":chash(core)};SOURCE_MANIFEST.write_text(json.dumps(payload,indent=2,allow_nan=False)+"\n");return payload
def features()->pd.DataFrame:
 if sha(PRICE)!=PRICE_SHA or sha(PRICE_MANIFEST)!=PRICE_MANIFEST_SHA:raise RuntimeError("UJCVR BTC source drift")
 f=pd.read_csv(SESSION,compression="gzip");f["source_day"]=pd.to_datetime(f.source_day,utc=True);f["decision_time"]=pd.to_datetime(f.decision_time,utc=True);f["source_valid"]=f.source_valid.astype(str).str.lower().eq("true");f["fx_return"]=pd.to_numeric(f.fx_return,errors="coerce");f["fx_return_z"]=pd.to_numeric(f.fx_return_z,errors="coerce")
 ...
 d=p[["decision_time","btc_realized_variation","btc_valid"]];f=f.merge(d,on="decision_time",how="left",validate="one_to_one");f["btc_realized_variation_rank"]=strict_prior_midrank(f.btc_realized_variation.where(f.btc_valid));f["signal_valid"]=f.source_valid&f.btc_valid&np.isfinite(f[["fx_return","fx_return_z","btc_realized_variation","btc_realized_variation_rank"]]).all(axis=1)&f.fx_return.ne(0);return f
def conditions(f:pd.DataFrame,control:str)->tuple[pd.Series,pd.Series]:
 ret=f.fx_return;z=f.fx_return_z
 if control=="one_session_stale_fx_shock":ret=ret.shift(1);z=z.shift(1)
 tail=pd.Series(True,index=f.index) if control=="no_fx_tail" else z.abs().ge(1.);vol=pd.Series(True,index=f.index) if control=="no_volatility_gate" else f.btc_realized_variation_rank.ge(.65);active=f.signal_valid&np.isfinite(ret)&np.isfinite(z)&ret.ne(0)&tail&vol;side=np.sign(ret);side=-side if control=="direction_flip" else side;return active,side
def clock(f:pd.DataFrame,control:str="primary")->pd.DataFrame:
 ...
 for i in f.index[active]:
  ...
  next_allowed=exit_;rows.append({"candidate":"UJCVR-12","control":control,"split":split,"source_day":f.at[i,"source_day"],"decision_time":decision,"feature_available_time":decision,"entry_time":entry,"exit_time":exit_,"side":int(side.at[i]),"fx_return":float(f.at[i,"fx_return"]),"fx_return_z":float(f.at[i,"fx_return_z"]),"btc_realized_variation":float(f.at[i,"btc_realized_variation"]),"btc_realized_variation_rank":float(f.at[i,"btc_realized_variation_rank"])})
