"""Build HVOCPR-8 source features and freeze its pre-2023 ridge model."""
...
SOURCE_DIR = Path("data/high_volatility_ordinal_pattern_classifier_relay_sources_2020_2026")
PANEL = SOURCE_DIR / "eight_hour_feature_panel.csv.gz"
TRAINING = SOURCE_DIR / "pre2023_training_rows.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
MODEL = Path("results/high_volatility_ordinal_pattern_classifier_relay_model_freeze_2026-08-10.json")
PREREG_SHA256 = "14d686b0a657cdf6cc517ecb22ea769f61aafbdebdd182b12bb0e54386b02d4e"
...
def ordinal_pattern_features(window: pd.DataFrame) -> tuple[dict[str, float], float]:
    ...
    if root_variation <= 0 or np.std(absolute[:-1]) <= 0 or np.std(absolute[1:]) <= 0:
        raise ValueError("ordinal-pattern feature denominator invalid")
    pattern_names = ("012", "021", "102", "120", "201", "210")
    counts = {name: 0 for name in pattern_names}
    valid_triples = 0
    for triple in np.lib.stride_tricks.sliding_window_view(log_close, 3):
        if len(np.unique(triple)) != 3:
            continue
        pattern = "".join(str(value) for value in np.argsort(np.argsort(triple)))
        counts[pattern] += 1
        valid_triples += 1
    transitions = {"pp": 0, "pn": 0, "np": 0, "nn": 0}
    valid_transitions = 0
    for left, right in zip(returns[:-1], returns[1:], strict=True):
        if left == 0 or right == 0:
            continue
        transitions[("p" if left > 0 else "n") + ("p" if right > 0 else "n")] += 1
        valid_transitions += 1
    if valid_triples == 0 or valid_transitions == 0:
        raise ValueError("ordinal-pattern population empty")
    values = {
        "normalized_full_return": full_return / root_variation,
        "normalized_late_return": late_return / root_variation,
        **{f"ordinal_{name}": counts[name] / valid_triples for name in pattern_names},
        **{f"transition_{name}": transitions[name] / valid_transitions for name in transitions},
        "semivariance_balance": (float(np.square(positive).sum()) - float(np.square(negative).sum())) / full_variation,
        "absolute_return_autocorrelation": abs(float(np.corrcoef(absolute[:-1], absolute[1:])[0, 1])),
    }
    if not all(math.isfinite(value) for value in values.values()):
        raise ValueError("ordinal-pattern features nonfinite")
...
def feature_panel(bars: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for decision in decisions:
        ...
        if valid:
            try:
                values, full_variation = ordinal_pattern_features(window)
            except ValueError:
                valid = False
...
def fit_model(panel: pd.DataFrame, labels: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if len(training) == 0:
        raise RuntimeError("HVOCPR training population empty")
    mean = matrix.mean(axis=0)
    scale = matrix.std(axis=0, ddof=0)
    if not np.isfinite(mean).all() or not np.isfinite(scale).all() or (scale <= 0).any():
        raise RuntimeError("HVOCPR training standardization invalid")
    ...
    if high_volatility.sum() == 0:
        raise RuntimeError("HVOCPR training high-volatility population empty")
    threshold = float(np.quantile(np.abs(fitted[high_volatility]), 0.50, method="linear"))
    if not math.isfinite(threshold) or threshold <= 0:
        raise RuntimeError("HVOCPR prediction-strength threshold invalid")
...
def run() -> dict[str, Any]:
    ...
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA256:
        raise RuntimeError("HVOCPR preregistration artifact drift")
    registration = json.loads(prereg.DEFAULT_OUTPUT.read_text())
    if registration != prereg.build():
        raise RuntimeError("HVOCPR preregistration payload drift")
    ...
    source_core = {
        "protocol_version": "hvocpr_8_source_and_pretraining_v1",
        "preregistration": {
    ...
    model_core = {
        "protocol_version": "hvocpr_8_model_freeze_v1",
        "policy_id": "HVOCPR-8",
