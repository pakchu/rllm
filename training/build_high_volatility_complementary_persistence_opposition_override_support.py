"""Materialize deterministic, outcome-blind source support for HVCPO-8."""
...
from training import preregister_high_volatility_complementary_persistence_opposition_override as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv

PREREG_SHA = "501b8539fac1c36d52168d1a9e8808344305c7cb8999b73763a8497fe4b3dfa6"
CLOCK = Path("data/high_volatility_complementary_persistence_opposition_override_clocks_2023_2026.csv.gz")
RESULT = Path("results/high_volatility_complementary_persistence_opposition_override_support_2026-08-16.json")
...
def _registration() -> dict[str, Any]:
    if prereg.sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCPO-8 preregistration hash drift")
    ...
    for component, artifacts in prereg.COMPONENT_ARTIFACTS.items():
        for kind, artifact in artifacts.items():
            if prereg.sha256_file(artifact["path"]) != artifact["sha256"]:
                raise RuntimeError(f"HVCPO-8 {component} {kind} drift")
...
def build_clock(clocks: Mapping[str, pd.DataFrame]) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for decision in decisions:
        ...
        override = indexed[override_id].get(decision)
        entry, exit_ = decision + ENTRY_DELAY, decision + ENTRY_DELAY + HOLD
        if primary.split != split or primary.feature_available_time > entry:
            raise RuntimeError("HVCPO-8 primary stage or availability drift")
        if override is not None and (override.split != split or override.feature_available_time > entry):
            raise RuntimeError("HVCPO-8 override stage or availability drift")
        override_side = 0 if override is None else override.side
        side = prereg.override_side(primary.side, override_side)
        if override is None:
            counts["override_absent"] += 1
        elif override.side == primary.side:
            counts["override_same_side"] += 1
        else:
            counts["override_opposite_side"] += 1
        selected = override if override is not None and override.side == -primary.side else primary
        selected_action = override_id if selected is override else primary_id
    ...
    if counts["emitted_decisions"] != counts["primary_decisions"]:
        raise RuntimeError("HVCPO-8 incidence accounting drift")
    if len(output) > 1 and not output["entry_time"].iloc[1:].reset_index(drop=True).ge(output["exit_time"].iloc[:-1].reset_index(drop=True)).all():
        raise RuntimeError("HVCPO-8 output overlap")
...
if __name__ == "__main__":
    ...
    print(json.dumps({"events": {k: v["events"] for k, v in report["support"].items()}, "accounting": report["opposition_override_accounting"], "support_passed": report["support_passed"]}, indent=2))
