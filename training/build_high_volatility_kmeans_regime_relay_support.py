"""Build OOS source-only HVKMR-24 clocks after the model freeze."""
...
MODEL_FREEZE = Path(
    "results/high_volatility_kmeans_regime_relay_model_freeze_2026-08-09.json"
)
MODEL_FREEZE_SHA = "49506393d4bd840ecaa82ed88a9218aef2dea26a186c734de72502ee0ae6ba0d"
MODEL = Path("data/high_volatility_kmeans_regime_relay_model_2026-08-09.joblib")
MODEL_SHA = "f1cac8b03093d775e530a8ca05ee3156eeab1ec2a733f00fcf269aeadf30fc99"
CLOCK = Path("data/high_volatility_kmeans_regime_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_kmeans_regime_relay_controls_2023_2026")
SNAPSHOT = Path(
    "data/high_volatility_kmeans_regime_relay_sources_2023_2026/state_scores.csv.gz"
)
RESULT = Path(
    "results/high_volatility_kmeans_regime_relay_support_2026-08-09.json"
)
...
CONTROLS = ("no_volatility_gate", "one_anchor_stale_features", "direction_flip")
...
def verify_predecessors() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if sha256(MODEL_FREEZE) != MODEL_FREEZE_SHA or sha256(MODEL) != MODEL_SHA:
        raise RuntimeError("HVKMR model predecessor drift")
    ...
    if freeze.get("manifest_hash") != canonical_hash(freeze_core):
        raise RuntimeError("HVKMR model freeze manifest drift")
    if (
        ...
        raise RuntimeError("HVKMR model freeze state drift")
    artifact = joblib.load(MODEL)
    if artifact.get("policy_id") != "HVKMR-24":
        raise RuntimeError("HVKMR model policy drift")
...
def load_combined_market() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if combined.date.duplicated().any() or not combined.date.is_monotonic_increasing:
        raise RuntimeError("HVKMR combined market time drift")
    deltas = combined.date.diff().dropna()
    if not deltas.eq(pd.Timedelta(minutes=5)).all():
        raise RuntimeError("HVKMR combined market is not continuous 5m")
    return combined, {
...
def score_snapshot(market: pd.DataFrame, artifact: dict[str, Any]) -> pd.DataFrame:
    ...
    if missing:
        raise RuntimeError(f"HVKMR OOS feature drift: {missing}")
    ...
    states = artifact["pipeline"].predict(matrix[positions]).astype(int)
    side_map = {int(key): int(value) for key, value in artifact["state_side_map"].items()}
    mapped_side = np.array([side_map.get(int(state), 0) for state in states], dtype=int)
...
def build_clock(scores: pd.DataFrame, artifact: dict[str, Any], control: str = "primary") -> pd.DataFrame:
    ...
    if control == "one_anchor_stale_features":
        frame[["state", "mapped_side", "range_vol"]] = frame[["state", "mapped_side", "range_vol"]].shift(1)
    mapped_side = pd.to_numeric(frame.mapped_side, errors="coerce").to_numpy(float)
    volatility = pd.to_numeric(frame.range_vol, errors="coerce").to_numpy(float)
    long_signal = mapped_side == 1
    short_signal = mapped_side == -1
    ...
    finite = np.isfinite(mapped_side) & np.isfinite(volatility)
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvkmr_24_oos_source_support_v1",
        "policy_id": "HVKMR-24",
