"""Sequential strict economics for frozen HVVTX-24."""
...
PREREG = Path("results/high_volatility_vortex_path_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "92753d47fa9416286efbd0efe7bde6f27ce85ea9869a1658b1a32937ed8d765a"
SUPPORT = Path("results/high_volatility_vortex_path_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "ec8662c8c5e2e66ca119bfb2b35697d545c7976aa87ff9f2db5451b1274358fc"
NOVELTY = Path("results/high_volatility_vortex_path_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "3b6dda99c6d17edc869d23167975f89d88917cc661d0b13700093b91bdfde696"
CLOCK = Path("data/high_volatility_vortex_path_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "5e52ba84d85c5d50ee0390e2e449a2b295efdff7684fa38613b749465542a471"
CONTROL_DIR = Path("data/high_volatility_vortex_path_crossover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_vortex_path_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_vortex_path_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "one_bar_stale_crossover", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVVTX frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVVTX novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvvtx_24_sequential_economics_v1", "policy_id": POLICY_ID,
