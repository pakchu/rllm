"""Build outcome-blind source support for frozen HVCOUNTER-R10-E5-8."""
...
PREREG_SHA = "5b5eca51f8740af88d2d37bcc85a9cbc74bb0ea05541b526cc3109ea1bf0b062"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_counterattack_reversal_relay_sources_2023_2026")
PANEL = ROOT / "hourly_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_counterattack_reversal_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_counterattack_reversal_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_counterattack_reversal_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_counterattack_reversal_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "counterattack_ready", "counterattack_output", "entry_side",
    "no_long_body_side", "opposing_body_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "counterattack_ready", "counterattack_output",
    "entry_side", "no_long_body_side", "opposing_body_side", "variation",
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVCOUNTER source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVCOUNTER invalid source key")
...
def counterattack_outputs(bars: pd.DataFrame, valid: pd.Series) -> pd.DataFrame:
    """Replicate LEAN Counterattack defaults, lag positions, and inequalities."""
    output = np.zeros(len(bars), dtype=int)
    ready = np.zeros(len(bars), dtype=bool)
    no_long = np.zeros(len(bars), dtype=int)
    opposing_only = np.zeros(len(bars), dtype=int)
    ...
    for index, is_valid in enumerate(valid.to_numpy(bool)):
        ...
        if not ready[index]:
            continue
        first_open, _, _, first_close = history[-2]
        second_open, _, _, second_close = history[-1]
        bodies = [abs(close - open_) for open_, _, _, close in history]
        first_long_average = float(np.mean(bodies[-12:-2])) * P["body_long_factor"]
        second_long_average = float(np.mean(bodies[-11:-1])) * P["body_long_factor"]
        equal_average = float(np.mean([high - low for _, high, low, _ in history[-7:-2]])) * P["equal_factor"]
        first_body = abs(first_close - first_open)
        second_body = abs(second_close - second_open)
        first_color = 1 if first_close >= first_open else -1
        second_color = 1 if second_close >= second_open else -1
        opposing = first_color == -second_color
        long_bodies = first_body > first_long_average and second_body > second_long_average
        equal_close = first_close - equal_average <= second_close <= first_close + equal_average
        if opposing and equal_close:
            no_long[index] = second_color
        if opposing and long_bodies:
            opposing_only[index] = second_color
        if opposing and long_bodies and equal_close:
            output[index] = second_color
    entry = output.copy()
    return pd.DataFrame(
        {
            "counterattack_ready": ready,
            "counterattack_output": output,
            "entry_side": entry,
            "no_long_body_side": no_long,
            "opposing_body_side": opposing_only,
        },
        index=bars.index,
    )
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control == "no_long_body_requirements":
        side = pd.to_numeric(used.no_long_body_side, errors="coerce").fillna(0).astype(int)
        state = valid & side.ne(0) & variation
    elif control == "opposing_bodies_without_equal_close":
        side = pd.to_numeric(used.opposing_body_side, errors="coerce").fillna(0).astype(int)
        state = valid & side.ne(0) & variation
        ...
        rows.append(
            {
                ...
                **{
                    ...
                    if column in ("counterattack_ready", "eligible")
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCOUNTER prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvcounter_r10_e5_8_source_v1",
        "query": QUERY,
    ...
    core = {
        "protocol_version": "hvcounter_r10_e5_8_source_support_v1",
