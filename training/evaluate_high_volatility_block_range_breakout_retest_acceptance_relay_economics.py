"""Sequential strict economics for frozen HVRBRAR-8."""
...
PREREG = Path("results/high_volatility_block_range_breakout_retest_acceptance_relay_preregistration_2026-08-10.json")
PREREG_SHA = "49f68b06fa9e5e3334f671fc8dd8862af6c888263773f575a1722ada567d2ce1"
SUPPORT = Path("results/high_volatility_block_range_breakout_retest_acceptance_relay_support_2026-08-10.json")
SUPPORT_SHA = "64e7abc1b7283cbe3b7e4dad010594f028b021e711ff25ba65eba57a8c2fad67"
NOVELTY = Path("results/high_volatility_block_range_breakout_retest_acceptance_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "021614d0e8b1b72e872c18e7c51df3faa5cc70c5805d6d315464656ca5e39a96"
CLOCK = Path("data/high_volatility_block_range_breakout_retest_acceptance_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "77b16b08f928883c67a408304f5fbb694ced3c378534986efe9020f90cc1fc98"
CONTROL_DIR = Path("data/high_volatility_block_range_breakout_retest_acceptance_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_block_range_breakout_retest_acceptance_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_block_range_breakout_retest_acceptance_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_retest_requirement", "no_variation_gate", "terminal_breakout_close", "one_block_stale_prior_range", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVRBRAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVRBRAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrbrar_8_sequential_economics_v1", "policy_id": POLICY_ID,
