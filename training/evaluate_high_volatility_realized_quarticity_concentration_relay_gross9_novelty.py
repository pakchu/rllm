"""Evaluate frozen HVRQC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVRQC-8"
PROTOCOL = "hvrqc_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_realized_quarticity_concentration_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVRQC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVRQC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVRQC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVRQC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvrqc_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVRQC-8 unchanged before economic outcomes",
    }
