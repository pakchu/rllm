"""Evaluate frozen HVHVS-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVHVS-8"
PROTOCOL = "hvhvs_8_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_har_variation_surprise_relay_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVHVS predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVHVS novelty limits drift")
    if (
        ...
        raise RuntimeError("HVHVS predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVHVS primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvhvs_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVHVS-8 unchanged before economic outcomes"
        ),
    }
