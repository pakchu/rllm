"""Build outcome-blind source support for frozen HVCVD-24."""
...
from training import preregister_high_volatility_cumulative_volume_delta_signal_crossover_relay as prereg
...
ROOT = Path("data/high_volatility_cumulative_volume_delta_signal_crossover_relay_sources_2023_2026")
...
SPLIT_DIR = Path("data/high_volatility_cumulative_volume_delta_signal_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_cumulative_volume_delta_signal_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_cumulative_volume_delta_signal_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "bar_quote_volume", "bar_taker_buy_quote", "aggressive_quote_delta", "cvd", "signal", "difference", "delta_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "bar_quote_volume", "bar_taker_buy_quote", "aggressive_quote_delta", "cvd", "signal", "difference", "delta_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close", "quote_asset_volume", "taker_buy_quote"]:
        raise RuntimeError("HVCVD source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVCVD invalid source key")
    ...
    source["row_valid"] = (
        ...
        & source.taker_buy_quote.ge(0)
        & source.taker_buy_quote.le(source.quote_asset_volume)
    )
...
def causal_cvd(delta: pd.Series, valid: pd.Series) -> pd.Series:
    output=np.full(len(delta),np.nan);state=np.nan
    for i,(increment,ok) in enumerate(zip(delta.to_numpy(float),valid.to_numpy(bool))):
        if not ok or not math.isfinite(increment):state=np.nan;continue
        state=increment if not math.isfinite(state) else state+increment;output[i]=state
    return pd.Series(output,index=delta.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=pd.DataFrame({"rows":groups.row_valid.sum(),"bar_open":groups.open.first(),"bar_high":groups.high.max(),"bar_low":groups.low.min(),"bar_close":groups.close.last(),"bar_quote_volume":groups.quote_asset_volume.sum(min_count=240),"bar_taker_buy_quote":groups.taker_buy_quote.sum(min_count=240),"variation_component":groups.minute_sq_return.sum(min_count=240)})
    fields=["bar_open","bar_high","bar_low","bar_close","bar_quote_volume","bar_taker_buy_quote"];bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[fields]).all(axis=1)&bars[["bar_open","bar_high","bar_low","bar_close"]].gt(0).all(axis=1)&bars.bar_quote_volume.gt(0)&bars.bar_taker_buy_quote.ge(0)&bars.bar_taker_buy_quote.le(bars.bar_quote_volume)
    bars["aggressive_quote_delta"]=2.0*bars.bar_taker_buy_quote-bars.bar_quote_volume
    bars["cvd"]=causal_cvd(bars.aggressive_quote_delta,bars.valid_bar)
    bars["signal"]=causal_ema(bars.cvd,np.isfinite(bars.cvd),P["signal_periods"]);bars["difference"]=bars.cvd-bars.signal
    previous_contribution=bars.aggressive_quote_delta.shift(1);bars["delta_side"]=np.where((bars.aggressive_quote_delta>0)&(previous_contribution<=0),1,np.where((bars.aggressive_quote_delta<0)&(previous_contribution>=0),-1,0))
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["bar_quote_volume","bar_taker_buy_quote","aggressive_quote_delta","cvd","signal","difference","variation"]]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="delta_zero_cross":
        side=pd.to_numeric(used.delta_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCVD prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvcvd_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvcvd_24_source_support_v1", "policy_id": prereg.POLICY_ID,
