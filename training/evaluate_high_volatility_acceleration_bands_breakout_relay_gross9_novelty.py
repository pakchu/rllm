"""Evaluate frozen HVABANDS-B10-W4-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVABANDS-B10-W4-24"
PROTOCOL = "hvabands_b10_w4_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_acceleration_bands_breakout_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVABANDS predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVABANDS novelty limits drift")
    if (
        ...
        raise RuntimeError("HVABANDS predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVABANDS primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvabands_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVABANDS-B10-W4-24 unchanged before economic outcomes",
    }
