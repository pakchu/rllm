"""Sequential strict economics for frozen HVEIS-24."""
...
PREREG = Path("results/high_volatility_elder_impulse_color_transition_relay_preregistration_2026-08-11.json")
PREREG_SHA = "f9bd9dfb4bdbfa112dfeb2d435597133bbadfeca15256c192ea19aaae40b0e46"
SUPPORT = Path("results/high_volatility_elder_impulse_color_transition_relay_support_2026-08-11.json")
SUPPORT_SHA = "ae17c512bf5db51634b7a9e9da7b96dbf1fcfbd47e9d0bbc5bca8ed102915c14"
NOVELTY = Path("results/high_volatility_elder_impulse_color_transition_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "5a552e2ba8cb5795c06f6b991543d13ddb86c2bd39602d10205f38f0782d0e3f"
CLOCK = Path("data/high_volatility_elder_impulse_color_transition_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "da302453acb65c00ba36a9755ccf2a74c94c05286470a8e6a3bf2e68998f137d"
CONTROL_DIR = Path("data/high_volatility_elder_impulse_color_transition_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_elder_impulse_color_transition_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_elder_impulse_color_transition_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "histogram_slope_only", "one_bar_stale_transition", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVEIS frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVEIS novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hveis_24_sequential_economics_v1", "policy_id": POLICY_ID,
