"""Sequential strict economics for frozen HVBB-24."""
...
PREREG = Path("results/high_volatility_bollinger_band_reentry_relay_preregistration_2026-08-11.json")
PREREG_SHA = "131004f34c97c51b3c20aa528ac9e92d4b2d07b9133391e422df5deaa2bc3348"
SUPPORT = Path("results/high_volatility_bollinger_band_reentry_relay_support_2026-08-11.json")
SUPPORT_SHA = "a01d0b4ebd5d61ef5a1a95fce74278fc690b0375fb3f76011a0d8746bb27d891"
NOVELTY = Path("results/high_volatility_bollinger_band_reentry_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "95e4ebdb8f3c83c69b14f5a5aa6e8d48cd42c9617681c3d7b77be8e40e0b166b"
CLOCK = Path("data/high_volatility_bollinger_band_reentry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "8cf43452f77ffcb52e8534b3eff687ec727863b0a34916edd694fc2db7bb0c83"
CONTROL_DIR = Path("data/high_volatility_bollinger_band_reentry_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_bollinger_band_reentry_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_bollinger_band_reentry_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "exponential_center", "one_bar_stale_reentry", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVBB frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVBB novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvbb_24_sequential_economics_v1", "policy_id": POLICY_ID,
