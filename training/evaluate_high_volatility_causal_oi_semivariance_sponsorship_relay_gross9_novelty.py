"""Evaluate the frozen HVCOSR-24 action-vote clock against Gross9."""
...
POLICY = "HVCOSR-24"
PROTOCOL = "hvcosr_24_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_oi_semivariance_sponsorship_relay_clocks_2020_2026.csv.gz",
    "sha256": "e500d9a057571e96af96729c59b3b2ce6cb15b93ab2347a30772aa4f974777ee",
    "rows": 90,
}
OUTPUT = Path("results/high_volatility_causal_oi_semivariance_sponsorship_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCOSR-24 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCOSR-24 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCOSR-24 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCOSR-24 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCOSR-24 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCOSR-24 unchanged before economics",
    }
