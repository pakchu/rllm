"""Sequential strict economics for frozen HVBBC-8."""
...
PREREG = Path("results/high_volatility_bilateral_barrier_completion_relay_preregistration_2026-08-13.json")
PREREG_SHA = "aebc345976d7278affdd180446ff1ee75345803aeb7688b566225babaea8043d"
SUPPORT = Path("results/high_volatility_bilateral_barrier_completion_relay_support_2026-08-13.json")
SUPPORT_SHA = "7ad9add351215bd1b18b16a0d7fa1babd12b1f9395aa15944faaaf2d827a5d19"
NOVELTY = Path("results/high_volatility_bilateral_barrier_completion_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "b72c4836a0f56284f39113f14a727c1de682487dd4f86224838227ee24a01bfb"
CLOCK = Path("data/high_volatility_bilateral_barrier_completion_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "3f1873fe8c9c5d181702cac2b9b0836731ab9db35bab0c0eb99496ccf8ac0ccb"
CONTROL_DIR = Path("data/high_volatility_bilateral_barrier_completion_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_bilateral_barrier_completion_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_bilateral_barrier_completion_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "no_terminal_confirmation", "one_decision_stale_barrier", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVBBC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVBBC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvbbc_8_sequential_economics_v1", "policy_id": POLICY_ID,
