"""Build outcome-blind source support for frozen HVUI-12."""
...
from training import preregister_high_volatility_ulcer_index_stress_trend_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "3dd1b892d9810daebb282f0e29035ae47f49319030d45ecf4c535d707b9dcaf4"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ulcer_index_stress_trend_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ulcer_index_stress_trend_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ulcer_index_stress_trend_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ulcer_index_stress_trend_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ulcer_index_stress_trend_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "rolling_high", "percentage_drawdown", "ulcer_index",
    "ulcer_rank", "terminal_return", "direction", "variation", "variation_rank",
    "raw_drawdown_stress", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "rolling_high", "percentage_drawdown",
    "ulcer_index", "ulcer_rank", "terminal_return", "direction", "variation",
    "variation_rank", "raw_drawdown_stress", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVUI source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVUI invalid source key")
...
def ulcer_index(close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    """Canonical nested rolling-high drawdown RMS with gap-safe full warmups."""
    period = P["ulcer_periods"]
    clean = close.where(valid)
    rolling_high = clean.rolling(period, min_periods=period).max()
    drawdown = 100.0 * (clean / rolling_high - 1.0)
    complete = drawdown.notna().rolling(period, min_periods=period).sum().eq(period)
    index = np.sqrt(drawdown.pow(2).rolling(period, min_periods=period).mean()).where(complete)
    return pd.DataFrame(
        {
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars = bars.join(ulcer_index(bars.bar_close, bars.valid_bar))
    bars["terminal_return"] = np.log(bars.bar_close / bars.bar_open)
    bars["direction"] = np.sign(bars.terminal_return)
    bars["variation"] = np.sqrt(bars.variation_component.rolling(P["variation_hours"] // 4, min_periods=P["variation_hours"] // 4).sum())
    bars["source_valid"] = (
        bars.valid_bar
        & np.isfinite(
            bars[["rolling_high", "percentage_drawdown", "ulcer_index", "terminal_return", "direction", "variation"]]
        ).all(axis=1)
        & bars.rolling_high.gt(0)
        & bars.percentage_drawdown.le(0)
        & bars.ulcer_index.gt(0)
        & bars.direction.ne(0)
        & bars.variation.gt(0)
    )
    panel = bars.reset_index(names="source_start");panel["feature_available_time"] = panel.source_start + pd.Timedelta("4h")
    panel["ulcer_rank"] = prior_rank(panel.ulcer_index.where(panel.source_valid))
    panel["variation_rank"] = prior_rank(panel.variation.where(panel.source_valid))
    panel["raw_drawdown_stress"] = panel.percentage_drawdown.lt(0)
    panel["eligible"] = (
        panel.source_valid
        & panel.ulcer_rank.ge(P["ulcer_rank_min"])
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    ulcer = used.ulcer_rank.ge(P["ulcer_rank_min"])
    variation = used.variation_rank.ge(P["variation_rank_min"])
    side = pd.to_numeric(used.direction, errors="coerce").fillna(0).astype(int)
    state = valid & side.ne(0) & ulcer & variation
    if control == "no_ulcer_gate": state = valid & side.ne(0) & variation
    elif control == "no_variation_gate": state = valid & side.ne(0) & ulcer
    elif control == "raw_drawdown_stress":
        state = valid & side.ne(0) & used.raw_drawdown_stress & variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVUI prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvui_12_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvui_12_source_support_v1", "policy_id": prereg.POLICY_ID,
