"""Build outcome-blind source support for frozen HVCHV-12."""
...
from training import preregister_high_volatility_chaikin_volatility_expansion_trend_relay as prereg
...
ROOT = Path("data/high_volatility_chaikin_volatility_expansion_trend_relay_sources_2023_2026")
...
SPLIT_DIR = Path("data/high_volatility_chaikin_volatility_expansion_trend_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_chaikin_volatility_expansion_trend_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_chaikin_volatility_expansion_trend_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "range", "range_ema", "lagged_range_ema",
    "chaikin_volatility", "chaikin_rank", "terminal_return", "direction",
    "variation", "variation_rank", "raw_range_expansion", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "range", "range_ema", "lagged_range_ema",
    "chaikin_volatility", "chaikin_rank", "terminal_return", "direction",
    "variation", "variation_rank", "raw_range_expansion", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVCHV source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVCHV invalid source key")
...
def range_ema(ranges: pd.Series, valid: pd.Series) -> pd.Series:
    """Causal EMA(10), reset on gaps and hidden until the full warmup completes."""
    period = P["range_ema_periods"]
    alpha = 2.0 / (period + 1.0)
    output = np.full(len(ranges), np.nan)
    state = math.nan
    count = 0
    for index, (value, is_valid) in enumerate(zip(ranges.to_numpy(float), valid.to_numpy(bool))):
        if not is_valid or not math.isfinite(value) or value <= 0:
            state = math.nan
            count = 0
            continue
        ...
        if count >= period:
            output[index] = state
    return pd.Series(output, index=ranges.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["range"] = bars.bar_high - bars.bar_low
    bars["range_ema"] = range_ema(bars.range, bars.valid_bar)
    bars["lagged_range_ema"] = bars.range_ema.shift(P["range_roc_periods"])
    bars["chaikin_volatility"] = 100.0 * (
        bars.range_ema / bars.lagged_range_ema - 1.0
    )
    ...
    bars["source_valid"] = (
        bars.valid_bar
        & np.isfinite(
            bars[["range", "range_ema", "lagged_range_ema", "chaikin_volatility", "terminal_return", "direction", "variation"]]
        ).all(axis=1)
        & bars.range.gt(0)
        & bars.range_ema.gt(0)
        & bars.lagged_range_ema.gt(0)
        & bars.direction.ne(0)
        & bars.variation.gt(0)
    )
    panel = bars.reset_index(names="source_start");panel["feature_available_time"] = panel.source_start + pd.Timedelta("4h")
    panel["chaikin_rank"] = prior_rank(panel.chaikin_volatility.where(panel.source_valid))
    panel["variation_rank"] = prior_rank(panel.variation.where(panel.source_valid))
    panel["raw_range_expansion"] = panel.chaikin_volatility.gt(0)
    panel["eligible"] = (
        panel.source_valid
        & panel.chaikin_rank.ge(P["chaikin_rank_min"])
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    chaikin = used.chaikin_rank.ge(P["chaikin_rank_min"])
    variation = used.variation_rank.ge(P["variation_rank_min"])
    side = pd.to_numeric(used.direction, errors="coerce").fillna(0).astype(int)
    state = valid & side.ne(0) & chaikin & variation
    if control == "no_chaikin_gate": state = valid & side.ne(0) & variation
    elif control == "no_variation_gate": state = valid & side.ne(0) & chaikin
    elif control == "raw_range_expansion":
        state = valid & side.ne(0) & used.raw_range_expansion & variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCHV prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvchv_12_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvchv_12_source_support_v1", "policy_id": prereg.POLICY_ID,
