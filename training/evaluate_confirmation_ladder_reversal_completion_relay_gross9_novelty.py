"""Evaluate frozen CLRCR-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "CLRCR-6"
PROTOCOL = "clrcr_6_gross9_novelty_v1"
...
OUTPUT = Path("results/confirmation_ladder_reversal_completion_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CLRCR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("CLRCR novelty limits drift")
    if (
        ...
        raise RuntimeError("CLRCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CLRCR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "clrcr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject CLRCR-6 unchanged before economic outcomes",
    }
