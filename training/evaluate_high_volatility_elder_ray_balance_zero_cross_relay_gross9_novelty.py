"""Evaluate frozen HVERB-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVERB-24"
PROTOCOL = "hverb_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_elder_ray_balance_zero_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVERB predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVERB novelty limits drift")
    if (
        ...
        raise RuntimeError("HVERB predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVERB primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hverb_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVERB-24 unchanged before economic outcomes",
    }
