"""Evaluate frozen HVVCR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVVCR-12"
PROTOCOL = "hvvcr_12_gross9_novelty_v1"
PREREG = Path(
...
SUPPORT = Path("results/high_volatility_variance_concentration_release_support_2026-08-09.json")
SUPPORT_SHA = "9834a325ee6c49e0d77ec892625cda6003cf25f061c17f1cba42b8dcee384fda"
CLOCK = Path("data/high_volatility_variance_concentration_release_clocks_2023_2026.csv.gz")
CLOCK_SHA = "e71588fa4b171a17971b6bcb6841c0a2cb9c67c92b3b7db1ad8cb3a706ff6728"
OUTPUT = Path(
    "results/high_volatility_variance_concentration_release_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVVCR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVVCR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVVCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVVCR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvvcr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVVCR-12 unchanged before economic outcomes"
        ),
    }
