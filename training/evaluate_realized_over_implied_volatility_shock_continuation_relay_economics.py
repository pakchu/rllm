"""Sequential strict economics for frozen RIVSCR-6."""
...
PREREG = Path("results/realized_over_implied_volatility_shock_continuation_relay_preregistration_2026-08-08.json")
PREREG_SHA = "7422e25c72a83117527a8ca99b2b20539b4cc95934934a094b52375fdd8fa034"
SUPPORT = Path("results/realized_over_implied_volatility_shock_continuation_relay_support_2026-08-08.json")
SUPPORT_SHA = "db7a88628a6468d926de1231c955731bdf7fbc39ac82fe3edbe430a105c7634e"
NOVELTY = Path("results/realized_over_implied_volatility_shock_continuation_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "78ec7720c638b323f3ce1e50a07cbcb9d9dd3b7dcdcfd52c261460c5c3ca55c6"
CLOCK = Path("data/realized_over_implied_volatility_shock_continuation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "7cf742c93331044960a7ad5e6bef5d0baedcd75dc3ce83714bb8a8c69653362d"
CONTROL_DIR = Path("data/realized_over_implied_volatility_shock_continuation_relay_controls_2023_2026")
FREEZE = Path("results/realized_over_implied_volatility_shock_continuation_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/realized_over_implied_volatility_shock_continuation_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
CONTROLS = ("absolute_realized_variation", "absolute_implied_level", "no_final_hour_confirmation", "one_block_stale_ratio", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("RIVSCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("RIVSCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "rivscr_6_sequential_economics_v1", "policy_id": POLICY_ID,
