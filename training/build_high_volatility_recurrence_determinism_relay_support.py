"""Source-only support gate for frozen HVRQA-8."""
...
from training import preregister_high_volatility_recurrence_determinism_relay as prereg
...
PREREG_SHA = "fb1ebcf09a0b9fb12861e6ed03ae8706b131773cd0a69905ca647200e7787339"
SPLITS = {
...
ROOT = Path("data/high_volatility_recurrence_determinism_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_recurrence_determinism_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_recurrence_determinism_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_recurrence_determinism_relay_support_2026-08-10.json")
...
def _run_points(mask: np.ndarray, minimum: int = 2) -> int:
    total = current = 0
    for value in np.r_[np.asarray(mask, dtype=bool), False]:
        if value:
            current += 1
        else:
            if current >= minimum:
...
def recurrence_determinism(returns: np.ndarray) -> float:
    returns = np.asarray(returns, dtype=float)
    if len(returns) != 479 or not np.isfinite(returns).all():
        return math.nan
    deviation = float(np.std(returns, ddof=1))
    if deviation <= 0 or not math.isfinite(deviation):
        return math.nan
    z = (returns - np.mean(returns)) / deviation
    recurrence = np.abs(z[:, None] - z[None, :]) <= 0.25
    rows = np.arange(len(z))
    recurrence[rows, rows] = False
    recurrence[rows[:-1], rows[1:]] = False
    recurrence[rows[1:], rows[:-1]] = False
    denominator = int(recurrence.sum())
    if denominator <= 0:
        return math.nan
    diagonal_points = sum(
        _run_points(np.diagonal(recurrence, offset))
        for offset in range(-(len(z) - 1), len(z))
        if abs(offset) > 1
    )
    return diagonal_points / denominator
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVRQA source timestamps")
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        if valid:
            ...
            determinism = recurrence_determinism(returns)
            ...
            valid = variation > 0 and math.isfinite(determinism)
        if not valid:
            variation = determinism = block_return = late_return = math.nan
        rows.append(
            {
    ...
    states["determinism_rank"] = prior_rank(states["recurrence_determinism"].where(states["source_valid"]))
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvrqa_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    determinism_rank = states["determinism_rank"]
    determinism = states["recurrence_determinism"]
    if control == "one_block_stale_geometry":
        block_return, late_return, variation_rank, determinism_rank, determinism = [
            value.shift(1)
            for value in (block_return, late_return, variation_rank, determinism_rank, determinism)
        ]
    variation_gate = (
    ...
    if control == "no_determinism_tail":
        determinism_gate = pd.Series(True, index=states.index)
    elif control == "raw_determinism_at_least_half":
        determinism_gate = determinism.ge(0.5)
    else:
        determinism_gate = determinism_rank.ge(0.75)
    agreement = (
        block_return.ne(0)
        & late_return.ne(0)
        & np.sign(block_return).eq(np.sign(late_return))
    )
    eligible = states["source_valid"] & variation_gate & determinism_gate & agreement
        ...
        rows.append(
            {
                ...
                "recurrence_determinism": float(states.at[index, "recurrence_determinism"]),
                "determinism_rank": float(states.at[index, "determinism_rank"]),
            }
        )
    columns = [
        "candidate", "control", "split", "decision_time", "feature_available_time",
        "entry_time", "exit_time", "side", "realized_variation", "variation_rank",
        "recurrence_determinism", "determinism_rank",
    ]
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVRQA preregistration drift")
    ...
    core = {
        "protocol_version": "hvrqa_8_source_support_v1",
        "policy_id": "HVRQA-8",
        "preregistration": {
