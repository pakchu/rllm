"""Evaluate frozen HVECOG-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVECOG-24"
PROTOCOL = "hvecog_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_ehlers_center_of_gravity_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVECOG predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVECOG novelty limits drift")
    if (
        ...
        raise RuntimeError("HVECOG predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVECOG primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvecog_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVECOG-24 unchanged before economic outcomes",
    }
