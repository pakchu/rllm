"""Sequential strict economics for frozen HVEVP-24."""
...
PREREG = Path("results/high_volatility_ehlers_voss_predictor_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "36492921e1d7a6c4b60749237c178a8272e0b5fa523ed8d6c0298f62d271d4f7"
SUPPORT = Path("results/high_volatility_ehlers_voss_predictor_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "6fdd66fd18dd96cd1a1e2c37e8e768ebe8dd28891b8f16de8820b766ca74ace4"
NOVELTY = Path("results/high_volatility_ehlers_voss_predictor_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "ac6383473fbc5afd056886675b37ded9ce1754a1d2d51291e171a050aa410443"
CLOCK = Path("data/high_volatility_ehlers_voss_predictor_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "ada44fd0ea9b49e55d53f1c00347ce2b52dd067e2e47b78ad5c6f565e07f052e"
CONTROL_DIR = Path("data/high_volatility_ehlers_voss_predictor_crossover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_ehlers_voss_predictor_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ehlers_voss_predictor_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "voss_zero_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVEVP frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVEVP novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvevp_24_sequential_economics_v1", "policy_id": POLICY_ID,
