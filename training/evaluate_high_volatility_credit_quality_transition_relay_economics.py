"""Strict staged economics for the singleton HVCQTR-24 policy."""
...
PREREG = Path("results/high_volatility_credit_quality_transition_relay_preregistration_2026-08-18.json")
PREREG_SHA = "af178c8ef143d693b2d60480c24122ddf0e76d7a1eb4625b634a0de40c03b0a6"
SUPPORT = Path("results/high_volatility_credit_quality_transition_relay_support_2026-08-18.json")
SUPPORT_SHA = "7812ab6b475a1d9fbc1f9d7f90572df05c1409a29f9c1b94e2ab4fec634fa02f"
GROSS9 = Path("results/high_volatility_credit_quality_transition_relay_gross9_novelty_2026-08-18.json")
GROSS9_SHA = "2677e52c18daf07b52f2caf3d2710b99a621f52f1fe84355640c201d76248880"
CLOCK = {
    "path": Path("data/high_volatility_credit_quality_transition_relay_clocks_2023_2026.csv.gz"),
    "sha256": "7026c3e650f9587964b43ff51114c35bf032cf1667b877263ba4842a5aed663e",
    "rows": 199,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_credit_quality_transition_relay_{stage}_economics_2026-08-18.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCQTR-24 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCQTR-24 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCQTR-24 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCQTR-24 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCQTR-24 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCQTR-24 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCQTR-24 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCQTR-24 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCQTR-24 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCQTR-24 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCQTR-24 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcqtr_24_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
