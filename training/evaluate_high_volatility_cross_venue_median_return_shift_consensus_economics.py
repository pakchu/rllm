"""Strict staged economics for the singleton HVCVMRS-8 policy."""
...
PREREG = Path("results/high_volatility_cross_venue_median_return_shift_consensus_preregistration_2026-08-18.json")
PREREG_SHA = "fce6bb10db2727caf4b82b3ecdb6bbbf51549680fb8e2267fa782fb55516bd88"
SUPPORT = Path("results/high_volatility_cross_venue_median_return_shift_consensus_support_2026-08-18.json")
SUPPORT_SHA = "adb44c18195ddf2c19dc1bb446af8a9a49501aaf9f606dcd9c0837191d4b2aab"
GROSS9 = Path("results/high_volatility_cross_venue_median_return_shift_consensus_gross9_novelty_2026-08-18.json")
GROSS9_SHA = "17a52373485b54badb979f0cbc98ea341c0fd13c553f92734af691765daba6fa"
CLOCK = {
    "path": Path("data/high_volatility_cross_venue_median_return_shift_consensus_clocks_2023_2026.csv.gz"),
    "sha256": "46a2240a334431477ff937e589a8a07c61690b518dc0a4fd18960a63aff45f40",
    "rows": 369,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_venue_median_return_shift_consensus_{stage}_economics_2026-08-18.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCVMRS-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCVMRS-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCVMRS-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCVMRS-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVMRS-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVMRS-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCVMRS-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCVMRS-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCVMRS-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCVMRS-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCVMRS-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcvmrs_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
