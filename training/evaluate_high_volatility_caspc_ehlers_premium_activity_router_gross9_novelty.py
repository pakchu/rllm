"""Evaluate the frozen HVCELVPAR-8 action-vote clock against Gross9."""
...
POLICY = "HVCELVPAR-8"
PROTOCOL = "hvcelvpar_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_caspc_ehlers_premium_activity_router_clocks_2023_2026.csv.gz",
    "sha256": "2d91dfd720bbda5e562b11d3800b287aa4bbdc467e3e44ec8268cacdd33c5749",
    "rows": 130,
}
OUTPUT = Path("results/high_volatility_caspc_ehlers_premium_activity_router_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCELVPAR-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCELVPAR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCELVPAR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCELVPAR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCELVPAR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCELVPAR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCELVPAR-8 unchanged before economics",
    }
