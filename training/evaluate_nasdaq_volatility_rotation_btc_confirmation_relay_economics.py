"""Sequential strict economics for frozen NVXCR-6."""
...
NOVELTY = Path("results/nasdaq_volatility_rotation_btc_confirmation_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "36a8c1a7d1c641b03ee3b8d1da32d73078cfb3d5a98d3e0a6805670bfdcc79ca"
...
FREEZE = Path("results/nasdaq_volatility_rotation_btc_confirmation_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/nasdaq_volatility_rotation_btc_confirmation_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("NVXCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("NVXCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "nvxcr_6_sequential_economics_v1", "policy_id": POLICY_ID,
