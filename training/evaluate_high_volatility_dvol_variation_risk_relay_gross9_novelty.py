"""Evaluate frozen HVDVVR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDVVR-12"
PROTOCOL = "hvdvvr_12_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_dvol_variation_risk_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDVVR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVDVVR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDVVR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDVVR primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject HVDVVR-12 unchanged before economic outcomes"
        ),
    }
