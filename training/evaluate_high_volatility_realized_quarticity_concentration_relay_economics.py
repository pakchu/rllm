"""Sequential strict economics for frozen HVRQC-8."""
...
PREREG = Path("results/high_volatility_realized_quarticity_concentration_relay_preregistration_2026-08-11.json")
PREREG_SHA = "63fed92e8cbea41a4cec53633bf41f38f5a34314292a56db2fd77b73e8d67659"
SUPPORT = Path("results/high_volatility_realized_quarticity_concentration_relay_support_2026-08-11.json")
SUPPORT_SHA = "407f6b5241759757a7dc83ebf3b4558dc53ec237d65ecdb1741059e7fcd29975"
NOVELTY = Path("results/high_volatility_realized_quarticity_concentration_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "59e0132dbc38c7b08549c3168ef9ec5bafd8382b658da36e31f790e231c7ad77"
CLOCK = Path("data/high_volatility_realized_quarticity_concentration_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d3851f5aabb18478bc7594912614e583990befe8af947958902d8d836604180b"
CONTROL_DIR = Path("data/high_volatility_realized_quarticity_concentration_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_realized_quarticity_concentration_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_realized_quarticity_concentration_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "diffuse_variation", "one_decision_stale_onset", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVRQC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVRQC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrqc_8_sequential_economics_v1", "policy_id": POLICY_ID,
