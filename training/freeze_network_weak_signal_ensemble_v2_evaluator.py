"""Freeze NWE-8 model, controls, and strict accounting before labels."""
...
def build_manifest(commit: str, *, feature_clock_rows: int) -> dict[str, Any]:
    if len(commit) != 40:
        raise ValueError("evaluator source commit must be a full Git hash")
    core: dict[str, Any] = {
        "protocol": "NWE-8 evaluator pre-outcome freeze v1",
        ...
        "feature_clock_sha256": evaluate.FEATURE_CLOCK_SHA256,
        "feature_clock_rows": int(feature_clock_rows),
        ...
        "outcome_dependent_schedule_hashes": (
            "undefined until the immutable evaluator opens labels; source and "
            "control algorithms, seeds, support clock, and all data hashes are frozen"
        ),
    }
...
def validate_manifest(payload: dict[str, Any]) -> None:
    ...
    if canonical_hash(core) != payload.get("manifest_hash"):
        raise RuntimeError("NWE-8 evaluator freeze hash mismatch")
    if payload.get("outcomes_opened") is not False:
        raise RuntimeError("NWE-8 evaluator freeze cannot open outcomes")
    if payload.get("labels_constructed_during_freeze") is not False:
        raise RuntimeError("NWE-8 evaluator freeze constructed labels")
    if payload.get("market_rows_parsed_during_freeze") != 0:
        raise RuntimeError("NWE-8 evaluator freeze parsed market rows")
    if payload.get("funding_rows_loaded_during_freeze") != 0:
        raise RuntimeError("NWE-8 evaluator freeze loaded funding rows")
    if payload.get("execution_simulation_run_during_freeze") is not False:
        raise RuntimeError("NWE-8 evaluator freeze simulated execution")
    if payload.get("mutable_parameters") != []:
        raise RuntimeError("NWE-8 evaluator freeze permits mutable parameters")
    if payload.get("policy_names") != list(evaluate.POLICY_NAMES):
        raise RuntimeError("NWE-8 evaluator freeze control set changed")
    if payload.get("evaluation_config") != evaluate.asdict(evaluate.EvaluationConfig()):
        raise RuntimeError("NWE-8 evaluator freeze configuration changed")
...
def write_once(path: str | Path, payload: dict[str, Any]) -> str:
    ...
    if output.exists():
        ...
        if existing != payload:
            raise RuntimeError("refusing to overwrite frozen NWE-8 evaluator")
...
def current_clean_commit() -> str:
    ...
    if status:
        raise RuntimeError("NWE-8 evaluator source is not clean at HEAD")
    ...
    if hashlib.sha256(committed).hexdigest() != _sha256(evaluate.EVALUATION_SOURCE):
        raise RuntimeError("NWE-8 evaluator source is not reproducible from HEAD")
    return commit
...
def main() -> None:
    ...
    if args.output != DEFAULT_OUTPUT:
        raise ValueError("NWE-8 evaluator freeze path is immutable")
    if Path(evaluate.DEFAULT_OUTPUT).exists():
        raise RuntimeError("NWE-8 outcome result already exists")
    clock, _, _ = evaluate.verify_support_and_clock()
    payload = build_manifest(current_clean_commit(), feature_clock_rows=len(clock))
        ...
        json.dumps(
            {
                ...
                "feature_clock_rows": payload["feature_clock_rows"],
                "output": args.output,
            },
            indent=2,
        )
