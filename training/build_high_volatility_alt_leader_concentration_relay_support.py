"""Outcome-blind source-support gate for frozen HVALCR-8."""
...
from training import preregister_high_volatility_alt_leader_concentration_relay as prereg


PREREG_SHA = "bd60bbd475461960802ce636718e78a2e5020529cdbb6f01f5ce49202a70f639"
START = common.START
END = common.END
...
SOURCE_DIR = Path("data/high_volatility_alt_leader_concentration_relay_sources_2023_2026")
PANEL = SOURCE_DIR / "eight_hour_states.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_alt_leader_concentration_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_alt_leader_concentration_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_alt_leader_concentration_relay_controls_2023_2026")
...
PANEL_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "leader_index", "leader_return",
    "equal_weight_alt_return", "side", "leader_concentration", "concentration_rank",
    "btc_realized_variation", "variation_rank", "eligible", "onset",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "decision_time", "feature_available_time", "entry_time",
    "exit_time", "side", "leader_index", "leader_return", "equal_weight_alt_return",
    "leader_concentration", "concentration_rank", "btc_realized_variation", "variation_rank",
)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    for decision in pd.date_range(START + pd.Timedelta(hours=26), END, freq="8h", inclusive="left"):
        ...
        leader_index = -1
        leader_return = equal_weight = concentration = variation = math.nan
        side = 0
        if valid:
            alt = block.loc[(slice(None), list(prereg.ALTS)), "hour_return"].unstack("symbol")
            alt_returns = alt.sum(axis=0).reindex(prereg.ALTS).to_numpy(float)
            leader_index, leader_return, concentration = leader_geometry(alt_returns)
            equal_weight = float(alt_returns.mean())
            side = int(np.sign(leader_return)) if leader_index >= 0 else 0
            variation = float(math.sqrt(btc["squared_variation"].to_numpy(float).sum()))
            valid = bool(
                leader_index >= 0 and side != 0 and math.isfinite(equal_weight)
                and math.isfinite(concentration) and math.isfinite(variation) and variation > 0
            )
        rows.append({
            "decision_time": decision, "feature_available_time": decision, "source_valid": valid,
            "leader_index": leader_index, "leader_return": leader_return,
            "equal_weight_alt_return": equal_weight, "side": side,
            "leader_concentration": concentration, "btc_realized_variation": variation,
        })
    panel = pd.DataFrame(rows)
    valid = panel["source_valid"].eq(True)
    panel["concentration_rank"] = strict_prior_midrank(panel["leader_concentration"].where(valid))
    panel["variation_rank"] = strict_prior_midrank(panel["btc_realized_variation"].where(valid))
    panel["eligible"] = (
        valid & panel["concentration_rank"].ge(POLICY["concentration_rank_min"])
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    if control == "one_block_stale_geometry":
        columns = [
            "source_valid", "leader_index", "leader_return", "equal_weight_alt_return", "side",
            "leader_concentration", "concentration_rank", "variation_rank", "feature_available_time",
        ]
        used[columns] = panel[columns].shift(1)
    valid = used["source_valid"].eq(True)
    concentration = used["concentration_rank"].ge(POLICY["concentration_rank_min"])
    variation = used["variation_rank"].ge(POLICY["variation_rank_min"])
    state = valid & concentration & variation
    if control == "no_concentration_tail_gate":
        state = valid & variation
    elif control == "no_variation_gate":
        state = valid & concentration
    onset = common.previous_valid_onset(state, valid)
    side = pd.to_numeric(used["side"], errors="coerce").fillna(0).astype(int)
    if control == "equal_weight_alt_direction":
        side = np.sign(pd.to_numeric(used["equal_weight_alt_return"], errors="coerce").fillna(0)).astype(int)
...
def run() -> dict[str, Any]:
    ...
    source_core = {
        "protocol_version": "hvalcr_8_sources_v1", "query": common.SOURCE_QUERY,
    ...
    core = {
        "protocol_version": "hvalcr_8_source_support_v1", "policy_id": prereg.POLICY_ID,
