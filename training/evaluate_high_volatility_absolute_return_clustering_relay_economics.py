"""Sequential strict economics for frozen HVACR-8."""
...
PREREG = Path("results/high_volatility_absolute_return_clustering_relay_preregistration_2026-08-10.json")
PREREG_SHA = "af3f7afccac1d563a50727d43253c00cc8faa0ec4f13df5d59e448e09bd5f2ee"
SUPPORT = Path("results/high_volatility_absolute_return_clustering_relay_support_2026-08-10.json")
SUPPORT_SHA = "75b28cacd463d6fca3d7cb9262932a143d9c843e2f583437e696d72701033151"
NOVELTY = Path("results/high_volatility_absolute_return_clustering_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "b92867b67d3cef3fd64364a79a39e6aad1a801fc1ae068ff7e53592caba5f0f4"
CLOCK = Path("data/high_volatility_absolute_return_clustering_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "787fbe8263f110c2e0a3d9be40909a2b8779981fe66eb4121140684b9261908f"
CONTROL_DIR = Path("data/high_volatility_absolute_return_clustering_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_absolute_return_clustering_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_absolute_return_clustering_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_clustering_tail", "no_variation_gate", "raw_positive_clustering", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVACR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVACR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvacr_8_sequential_economics_v1", "policy_id": POLICY_ID,
