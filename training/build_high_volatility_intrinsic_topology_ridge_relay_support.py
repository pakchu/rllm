"""Open source-only OOS incidence for the frozen HVITR-8 model."""
...
from training import train_high_volatility_intrinsic_topology_ridge_relay as trained
from training.build_binance_aggtrade_microstructure import _write_gzip_csv


PREREG_SHA = "a5f81522864782a8b88a286abf33039e35995f9776518c9379ae44fdda488c7c"
SOURCE_MANIFEST_SHA = "c1e608a94b825a95245ad22340bda257d3b4a8850c55958054023ecb5eaf88c5"
PANEL_SHA = "5e3d23194c4df46adadcf333e2c88097c9ff8dfb006487497a3135a0923ea818"
MODEL_SHA = "8769db1619eb17afb967c388130f18cdbaeb08bbf69ffeeb36047336582f51d0"
...
RESULT = Path("results/high_volatility_intrinsic_topology_ridge_relay_support_2026-08-10.json")
SPLITS = {
...
def verify_frozen_inputs() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if source.get("oos_incidence_opened") is not False or source.get("oos_outcomes_opened") is not False:
        raise RuntimeError("HVITR source was not OOS sealed")
    if model.get("oos_incidence_opened") is not False or model.get("oos_outcomes_opened") is not False or model.get("refit_authorized") is not False:
        raise RuntimeError("HVITR model was not frozen OOS blind")
    if tuple(model.get("feature_order", ())) != trained.FEATURES:
        raise RuntimeError("HVITR feature order drift")
...
def clock(frame: pd.DataFrame, model: dict[str, Any], control: str = "primary") -> pd.DataFrame:
    ...
    for index in frame.index[active & frame.decision_time.ge(SPLITS["train"][0])]:
        ...
        if split is None:
            continue
        row = {
            "candidate": "HVITR-8", "control": control, "split": split, "decision_time": decision,
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvitr_8_oos_source_support_v1", "policy_id": "HVITR-8",
