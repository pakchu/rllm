"""Sequential strict economics for frozen FETLS-6."""
...
PREREG = Path("results/fast_empty_template_loss_shock_preregistration_2026-08-09.json")
PREREG_SHA = "2d1cdc7898791a821e6de5960b2c7b6333d6b5f526ed6e9e1547349aa7bf577c"
SUPPORT = Path("results/fast_empty_template_loss_shock_support_2026-08-09.json")
SUPPORT_SHA = "414f51dff9e64a467de060ed176d7c2f62dbfc747f614cb53e021fae8e768d83"
NOVELTY = Path("results/fast_empty_template_loss_shock_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "74270b1534390a9b62de5e99a6e51f862fb6447c1d890e30f7c5ab08a16b0571"
CLOCK = Path("data/fast_empty_template_loss_shock_clocks_2023_2026.csv.gz")
CLOCK_SHA = "76a810be097e036816a69e15eaf06f9d1abe9ef4bd3ab09f070fb9dc27e0ef89"
FREEZE = Path("results/fast_empty_template_loss_shock_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/fast_empty_template_loss_shock_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS: tuple[str, ...] = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("FETLS frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("FETLS novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "fetls_6_sequential_economics_v1", "policy_id": POLICY_ID,
