"""Evaluate frozen HVDFRM-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDFRM-12"
PROTOCOL = "hvdfrm_12_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_dollar_factor_response_memory_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDFRM predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVDFRM novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDFRM predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDFRM primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject HVDFRM-12 unchanged before economic outcomes"
        ),
    }
