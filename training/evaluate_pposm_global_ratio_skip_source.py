"""Train-only source-support diagnostic for PPOSM SKIP-vs-TP4.

This diagnostic asks only whether the hash-bound Binance global trader
``count_long_short_ratio`` source contains enough *pre-2024* causal signal to
separate lifecycle residual SKIP switches from the default ALWAYS-TP4 anchors.
It intentionally does not build or read any OOS labels; 2024+ is outside this
script's label boundary.
"""
...
DEFAULT_OUTPUT = Path(
    "results/pposm_global_ratio_skip_source_support_2026-09-05.json"
)
DEFAULT_PREREGISTRATION = Path(
    "results/pposm_global_ratio_skip_source_preregistration_2026-09-05.json"
)
...
class Config:
    ...
    preregistration: Path | None = DEFAULT_PREREGISTRATION
...
def validate_preregistration(cfg: Config) -> dict[str, Any] | None:
    if cfg.preregistration is None:
        return None
    report = json.loads(cfg.preregistration.read_text(encoding="utf-8"))
    if not isinstance(report, dict):
        raise RuntimeError("source-support preregistration must be a JSON object")
    core = {key: value for key, value in report.items() if key != "manifest_hash"}
    if report.get("manifest_hash") != sha256_canonical(core):
        raise RuntimeError("source-support preregistration manifest drift")
    expected = {
    ...
    observed = {
        ...
        "pre2024_prefix_sha256": report.get("source", {}).get(
            "pre2024_prefix_sha256"
        ),
        "features": report.get("design", {}).get("features"),
        "folds": report.get("design", {}).get("folds"),
        "gates": report.get("design", {}).get("gates"),
        "bootstrap_iterations": report.get("design", {}).get(
            "bootstrap_iterations"
        ),
        "random_seed": report.get("design", {}).get("random_seed"),
        "evaluator_sha256": report.get("implementation", {}).get(
            "evaluator_sha256"
        ),
        "manifest_file_sha256": report.get("source", {}).get(
            "pposm_manifest_sha256"
        ),
        "runtime_versions": report.get("implementation", {}).get(
            "runtime_versions"
        ),
    }
    if observed != expected:
        raise RuntimeError("source-support preregistration binding drift")
    ...
    if (
        ...
        raise RuntimeError("source-support preregistration opened outcomes")
    return report


def load_lifecycle_skip_labels(manifest_path: Path) -> pd.DataFrame:
    """Return exactly one pre-2024 SKIP-vs-TP4 label per ALWAYS-TP4 anchor."""
    ...
    if any(row.get("split") != "train" for row in rows):
        raise RuntimeError("source diagnostic received non-train lifecycle rows")
    by_candidate = Counter(row["metadata"]["candidate_action"] for row in rows)
    skip_rows = [row for row in rows if row["metadata"]["candidate_action"] == "SKIP"]
    anchors = len(skip_rows)
    if anchors != EXPECTED_ANCHORS:
        raise RuntimeError(f"expected {EXPECTED_ANCHORS} pre-2024 anchors, got {anchors}")
    out = pd.DataFrame(
        {
            "identity": [row["metadata"]["identity"] for row in skip_rows],
            "base_identity": [row["metadata"]["base_identity"] for row in skip_rows],
            "signal_position": [int(row["metadata"]["signal_position"]) for row in skip_rows],
            "signal_time": [pd.Timestamp(row["metadata"]["signal_time"]).tz_localize(None) for row in skip_rows],
            "target_skip_switch": [1 if row["target"] == "SWITCH" else 0 for row in skip_rows],
        }
    ).sort_values("signal_time", kind="mergesort").reset_index(drop=True)
    if out["signal_time"].max() >= pd.Timestamp("2024-01-01"):
        raise RuntimeError("diagnostic labels must be strictly pre-2024")
    if out["signal_time"].duplicated().any():
        raise RuntimeError("duplicate lifecycle anchor signal_time")
        ...
        [
            {
                ...
                "target_skip_switch": int(row.target_skip_switch),
            }
...
def load_ratio_source(
    ...
    *,
    ...
    prefix_payload = [
        {"create_time": str(row.create_time), RATIO_COLUMN: None if pd.isna(getattr(row, RATIO_COLUMN)) else float(getattr(row, RATIO_COLUMN))}
    ...
    return metrics[["create_time", *FEATURE_COLUMNS[:3]]], summary
...
def valid_model_frame(frame: pd.DataFrame) -> pd.DataFrame:
    return frame.dropna(subset=list(FEATURE_COLUMNS) + ["target_skip_switch"]).copy()
...
def _classifiers(seed: int) -> dict[str, Any]:
    return {
        ...
        "hgb_balanced_weighted": HistGradientBoostingClassifier(
            max_iter=100,
            learning_rate=0.05,
            max_leaf_nodes=7,
...
def expanding_oof(frame: pd.DataFrame, *, seed: int) -> dict[str, dict[str, Any]]:
    ...
    y_all = frame["target_skip_switch"].to_numpy(int)
    ...
    for model_name, estimator in _classifiers(seed).items():
        ...
        for fold_name, train_years, test_year in FOLD_SPECS:
            ...
            if train_mask.sum() == 0 or test_mask.sum() == 0 or len(set(y_train)) < 2 or len(set(y_test)) < 2:
                ...
                continue
            estimator.fit(X_all[train_mask], y_train, **({"sample_weight": _sample_weights(y_train)} if model_name.startswith("hgb") else {}))
...
def joined_feature_hash(frame: pd.DataFrame) -> str:
    payload = []
    for row in frame.sort_values("signal_time", kind="mergesort").itertuples(index=False):
        payload.append(
            {
...
def oof_score_hash(model_results: dict[str, dict[str, Any]], frame: pd.DataFrame) -> str:
    payload = []
    for name in sorted(model_results):
        scores = np.asarray(model_results[name]["oof"], dtype=float)
...
def evaluate(cfg: Config) -> dict[str, Any]:
    ...
    labels = load_lifecycle_skip_labels(cfg.manifest)
    ...
    nulls = feature_null_fractions(joined)
    model_frame = valid_model_frame(joined)
    if model_frame["signal_time"].max() >= pd.Timestamp("2024-01-01"):
        raise RuntimeError("model frame contains OOS labels")
    model_results = expanding_oof(model_frame, seed=cfg.random_seed)
    joined_sha256 = joined_feature_hash(joined)
    y = model_frame["target_skip_switch"].to_numpy(int)
    ci_by_model: dict[str, Any] = {}
    gates: dict[str, Any] = {}
    for idx, (name, payload) in enumerate(model_results.items()):
        ci = bootstrap_auc_ci(
            y,
        ...
        ci_by_model[name] = ci
        gates[name] = gate_for_model(payload, ci, nulls)
    best_name = select_model(model_results, ci_by_model, gates)
    scores_sha256 = oof_score_hash(model_results, model_frame)
    summary = {
        "protocol": "pposm_global_count_long_short_ratio_skip_source_support_v1",
        ...
        "runtime_versions": runtime_versions(),
        "scope_limits": {
            "binary_skip_vs_tp4_only": True,
            "tp12_coverage_satisfied": False,
            "full_professor_critic_rubric_satisfied": False,
            "reason": "This source-support diagnostic tests only lifecycle SKIP-vs-ALWAYS-TP4 labels and cannot by itself prove TP12 coverage or the full text SFT+RLVR OOS economic rubric.",
        },
        "boundary": {
            "labels": "pre-2024 lifecycle residual SKIP-vs-ALWAYS-TP4 only",
        ...
        "anchors": {
            ...
            "actual": int(len(labels)),
            "candidate_counts": labels.attrs.get("candidate_counts", {}),
            "skip_target_counts": dict(sorted(Counter(labels["target_skip_switch"].tolist()).items())),
        ...
        "features": {
            ...
            "null_fraction": nulls,
            "max_null_fraction": float(max(nulls.values())),
            "model_rows_after_dropna": int(len(model_frame)),
        },
        "models": strip_oof(model_results),
        "bootstrap_auc_ci": ci_by_model,
        "gates": gates,
        "selected_model_train_only": best_name,
        "selection_rule": "choose a passing model first; within pass/fail bucket break ties by pooled AUC, lower95 CI, then balanced accuracy",
        "selected_gate": gates[best_name],
    }
