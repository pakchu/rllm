"""Build outcome-blind source support for frozen HVIARR-8."""
...
from training import preregister_high_volatility_inside_auction_resolution_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-04-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="7fd4fb2020ec4e35fa3dd8ad1e07acffb09ef901ee9875c0fb69bd875d75d745"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_inside_auction_resolution_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_inside_auction_resolution_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_inside_auction_resolution_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_inside_auction_resolution_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_inside_auction_resolution_relay_support_2026-08-11.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","prior_high","prior_low","inside_high","inside_low","strict_inside","release_open","release_close","accepted_side","no_inside_side","release_body_side","stale_prior_range_side","realized_variation","variation_rank","eligible")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","prior_high","prior_low","inside_high","inside_low","strict_inside","release_open","release_close","accepted_side","no_inside_side","release_body_side","stale_prior_range_side","realized_variation","variation_rank")
...
def resolution_pattern(inside:pd.DataFrame,release:pd.DataFrame,prior_high:float,prior_low:float)->tuple[float,float,bool,float,float,int,int,int]:
 inside_high=float(inside.high.max());inside_low=float(inside.low.min());strict_inside=inside_high<prior_high and inside_low>prior_low;release_open=float(release.open.iloc[0]);release_close=float(release.close.iloc[-1]);body=1 if release_close>release_open else -1 if release_close<release_open else 0
 outside=1 if release_close>prior_high and body==1 else -1 if release_close<prior_low and body==-1 else 0;accepted=outside if strict_inside else 0
 return inside_high,inside_low,strict_inside,release_open,release_close,accepted,outside,body
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVIARR source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVIARR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("2h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   prior=block.iloc[:240];inside=block.iloc[240:360];release=block.iloc[360:];prior_high=float(prior.high.max());prior_low=float(prior.low.min());inside_high,inside_low,strict_inside,release_open,release_close,side,no_inside,body=resolution_pattern(inside,release,prior_high,prior_low)
   stale_side=0 if previous_prior is None else resolution_pattern(inside,release,*previous_prior)[5]
  ...
  else:prior_high=prior_low=inside_high=inside_low=release_open=release_close=var=math.nan;strict_inside=False;side=no_inside=body=stale_side=0;valid=False;previous_prior=None
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"prior_high":prior_high,"prior_low":prior_low,"inside_high":inside_high,"inside_low":inside_low,"strict_inside":strict_inside,"release_open":release_open,"release_close":release_close,"accepted_side":side,"no_inside_side":no_inside,"release_body_side":body,"stale_prior_range_side":stale_side,"realized_variation":var})
...
def active(panel:pd.DataFrame,control:str="primary"):
 ...
 if control=="no_inside_requirement":side=pd.to_numeric(used.no_inside_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
 elif control=="no_variation_gate":state=valid&side.ne(0)
 elif control=="release_body_only":side=pd.to_numeric(used.release_body_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVIARR prereg drift")
 ...
 source_core={"protocol_version":"hviarr_8_source_v1","query":QUERY,"query_sha256":hashlib.sha256(QUERY.encode()).hexdigest(),"window":[START.isoformat(),END.isoformat()],"physical_rows":len(raw),"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"panel":{"path":str(PANEL),"sha256":sha(PANEL),"rows":len(panel),"valid_rows":int(panel.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};manifest={**source_core,"manifest_hash":chash(source_core)};immutable(MANIFEST,json_bytes(manifest))
