"""Evaluate frozen HVTICC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVTICC-8"
PROTOCOL = "hvticc_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_taker_imbalance_concentration_continuation_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTICC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVTICC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTICC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTICC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVTICC-8 unchanged before economic outcomes",
    }
