"""Build outcome-blind source support for frozen HVLOWR-8."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_leverage_only_wick_rejection_reversal as prereg
ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2023-05-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="fe53b114eef4515f29aa4f34a07bea3cfc48c8127640edda34daa4708aa88470";REG=prereg.build();P=REG["policy"];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG["stages"].items()};GATES=REG["source_support_gates"];CONTROLS=tuple(REG["diagnostic_controls"]["names"]);VENUES=("perpetual","spot")
...
ROOT=Path("data/high_volatility_leverage_only_wick_rejection_reversal_sources_2023_2026");PANEL=ROOT/"five_minute_barrier_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_leverage_only_wick_rejection_reversal_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_leverage_only_wick_rejection_reversal_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_leverage_only_wick_rejection_reversal_controls_2023_2026");RESULT=Path("results/high_volatility_leverage_only_wick_rejection_reversal_support_2026-08-13.json");BUILDER=Path(__file__).relative_to(Path.cwd())
...
def prepare(raw:pd.DataFrame)->pd.DataFrame:
 ...
 if raw.columns.tolist()!=expected:raise RuntimeError("HVLOWR source schema drift")
 ...
 if x.date.isna().any() or x.duplicated(["date","venue"]).any() or not x.venue.isin(("perpetual","spot")).all():raise RuntimeError("HVLOWR invalid source key")
...
def active(panel:pd.DataFrame,control="primary"):
 ...
 elif control in ("rank_0_99","no_close_nonconfirmation"):
  upper=u.upper_excess_rank.ge(.99);lower=u.lower_excess_rank.ge(.99)
  if control=="rank_0_99":upper&=u.perpetual_return.le(u.spot_return);lower&=u.perpetual_return.ge(u.spot_return)
  side=directional_rejection(upper,lower);state=side.ne(0);active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)&u.variation_active.eq(True)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVLOWR prereg drift")
