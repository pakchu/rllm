"""Evaluate frozen HVAMST-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVAMST-8"
PROTOCOL = "hvamst_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_alt_mst_hub_ignition_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVAMST predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVAMST novelty limits drift")
    if (
        ...
        raise RuntimeError("HVAMST predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVAMST primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVAMST-8 unchanged before economic outcomes",
    }
