"""Evaluate frozen HVOID-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVOID-8"
PROTOCOL = "hvoid_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_oi_unwind_directional_dominance_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVOID predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVOID novelty limits drift")
    if (
        ...
        raise RuntimeError("HVOID predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVOID primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvoid_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVOID-8 unchanged before economic outcomes",
    }
