"""Sequential strict economics for frozen HVIABR-8."""
...
PREREG = Path("results/high_volatility_intrabar_acceptance_breadth_relay_preregistration_2026-08-10.json")
PREREG_SHA = "619674be60397f0004ad79b089c8599db59553fbdbd3499cdbc9e3fca502c7ef"
SUPPORT = Path("results/high_volatility_intrabar_acceptance_breadth_relay_support_2026-08-10.json")
SUPPORT_SHA = "624a8f7b9a4fa0e253e91a5c21f4bdc1ac80ba99eb9675836e26dc3dc2e4e151"
NOVELTY = Path("results/high_volatility_intrabar_acceptance_breadth_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "0967f5849c3c86403cc31c42f484a281d59a6f9751b77fe039629ac4c1241da0"
CLOCK = Path("data/high_volatility_intrabar_acceptance_breadth_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "31441774050939b8a2695fa20d2097b0eb246830fb02dc225cb38033216b6711"
CONTROL_DIR = Path("data/high_volatility_intrabar_acceptance_breadth_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_intrabar_acceptance_breadth_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_intrabar_acceptance_breadth_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_acceptance_tail", "no_variation_gate", "raw_absolute_acceptance_above_quarter", "one_block_stale_acceptance", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVIABR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVIABR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hviabr_8_sequential_economics_v1", "policy_id": POLICY_ID,
