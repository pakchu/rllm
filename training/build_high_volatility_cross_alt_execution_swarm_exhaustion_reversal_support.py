"""Build outcome-blind source support for frozen HVCAESR-8."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_cross_alt_execution_swarm_exhaustion_reversal as prereg
ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2023-05-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="ad2eb74800fd3d8b03b0fc391787dc7efcac5c9c9d60d794cc14deb1636e79f4";REG=prereg.build();P=REG["policy"];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG["stages"].items()};GATES=REG["source_support_gates"];CONTROLS=tuple(REG["diagnostic_controls"]["names"]);SYMBOLS=("BTCUSDT",*prereg.ALTS);ALTS=prereg.ALTS
...
ROOT=Path("data/high_volatility_cross_alt_execution_swarm_exhaustion_reversal_sources_2023_2026");PANEL=ROOT/"five_minute_execution_swarm_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_cross_alt_execution_swarm_exhaustion_reversal_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_cross_alt_execution_swarm_exhaustion_reversal_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_cross_alt_execution_swarm_exhaustion_reversal_controls_2023_2026");RESULT=Path("results/high_volatility_cross_alt_execution_swarm_exhaustion_reversal_support_2026-08-13.json");BUILDER=Path(__file__).relative_to(Path.cwd())
...
def prepare(raw:pd.DataFrame)->pd.DataFrame:
 ...
 if raw.columns.tolist()!=expected:raise RuntimeError("HVCAESR source schema drift")
 ...
 if x.date.isna().any() or x.duplicated(["date","symbol"]).any() or not x.symbol.isin(SYMBOLS).all():raise RuntimeError("HVCAESR invalid source key")
 z=x[["open","high","low","close"]];x["row_valid"]=x.source_rows.eq(5)&x.distinct_rows.eq(5)&x.first_ts.eq(x.date)&x.last_ts.eq(x.date+pd.Timedelta("4m"))&x.coherent.eq(True)&np.isfinite(x[["open","high","low","close","quote_turnover","trade_count","minute_squared_return"]]).all(axis=1)&z.gt(0).all(axis=1)&x.quote_turnover.gt(0)&x.trade_count.gt(0)&x.minute_squared_return.ge(0);x["return"]=np.log(x.close/x.open);return x
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 x=prepare(raw);grid=pd.date_range(START,END,freq="5min",inclusive="left");ret=x.pivot(index="date",columns="symbol",values="return").reindex(index=grid,columns=SYMBOLS);counts=x.pivot(index="date",columns="symbol",values="trade_count").reindex(index=grid,columns=SYMBOLS);sq=x.pivot(index="date",columns="symbol",values="minute_squared_return").reindex(index=grid,columns=SYMBOLS);valid=x.pivot(index="date",columns="symbol",values="row_valid").reindex(index=grid,columns=SYMBOLS).eq(True);alt_valid=valid.loc[:,list(ALTS)];alt_counts=counts.loc[:,list(ALTS)].where(alt_valid);threshold=alt_counts.shift(1).rolling(P["count_history_bars"],min_periods=P["minimum_count_history_bars"]).quantile(P["count_quantile"],interpolation="linear");history_valid=alt_valid.shift(1).rolling(P["count_history_bars"],min_periods=P["minimum_count_history_bars"]).sum().ge(P["minimum_count_history_bars"]);shocks=alt_counts.ge(threshold)&threshold.notna()&history_valid&alt_valid;consensus_side,pos,neg=shock_side(ret.loc[:,list(ALTS)],shocks,P["minimum_consensus_breadth"]);side=-consensus_side;market_counts=alt_counts.mean(axis=1);market_threshold=market_counts.shift(1).rolling(P["count_history_bars"],min_periods=P["minimum_count_history_bars"]).quantile(P["count_quantile"],interpolation="linear");all_shocked=pd.DataFrame(True,index=grid,columns=ALTS);market_side,_,_=shock_side(ret.loc[:,list(ALTS)],all_shocked,P["minimum_consensus_breadth"]);market_side=(-market_side).where(market_counts.ge(market_threshold),0);variation=np.sqrt(sq.BTCUSDT.where(valid.BTCUSDT).rolling(P["variation_bars"],min_periods=P["variation_bars"]).sum());vrank=strict_prior_midrank(variation,P["variation_history_decisions"],P["minimum_variation_history_decisions"]);source_valid=valid.all(axis=1)&threshold.notna().all(axis=1)&variation.notna()&vrank.notna();vactive=vrank.ge(P["variation_rank_min"]);broad=side.ne(0);prior_valid=source_valid.shift(1,fill_value=False);eligible=source_valid&prior_valid&broad&~broad.shift(1,fill_value=False)&vactive
...
def active(panel:pd.DataFrame,control="primary"):
 ...
 elif control=="three_of_six_count_shock":
 ...
 elif control=="equal_weight_market_count_shock":
 ...
 elif control=="one_bar_stale_count_shock":
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCAESR prereg drift")
