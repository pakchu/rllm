"""Sequential strict economics for frozen HVFI-24."""
...
PREREG = Path("results/high_volatility_elder_force_index_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "23a20dfa9707a89432346628fdabff0f5ded26486a5d16e57a3a5c6d07834c7f"
SUPPORT = Path("results/high_volatility_elder_force_index_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "ff30c22c6334cc3db01350d107486e2ff366c335042f2791f286641bb7a000bf"
NOVELTY = Path("results/high_volatility_elder_force_index_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "bedcef08ccdfadf160d25b58788f3569ec1f27b8d4c529b5a936db5786cec8a3"
CLOCK = Path("data/high_volatility_elder_force_index_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "1da4d74d21bec6e09df95d0d843a73772a52806a31efa551b2fa9869cc19a10b"
CONTROL_DIR = Path("data/high_volatility_elder_force_index_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_elder_force_index_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_elder_force_index_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "quote_volume_force", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVFI frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVFI novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvfi_24_sequential_economics_v1", "policy_id": POLICY_ID,
