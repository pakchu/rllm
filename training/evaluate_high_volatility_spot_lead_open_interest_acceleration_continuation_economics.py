"""Sequential strict economics for frozen HVSPOIAC-8."""
...
PREREG = Path("results/high_volatility_spot_lead_open_interest_acceleration_continuation_preregistration_2026-08-16.json")
PREREG_SHA = "ecdb8726d00c20e29c0e6f3324e7683e8b57963020fa7162a0c72ec8dd6a937c"
SUPPORT = Path("results/high_volatility_spot_lead_open_interest_acceleration_continuation_support_2026-08-16.json")
SUPPORT_SHA = "b1d0a2a2bd82507742366330b96c3af4276e5b827dc8ca215d0f365d1c853dee"
NOVELTY = Path("results/high_volatility_spot_lead_open_interest_acceleration_continuation_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "85230cfcc866982bcf4ccc4d507ac57131c0a6c8d0451d29feab7010fd1c8d5d"
CLOCK = Path("data/high_volatility_spot_lead_open_interest_acceleration_continuation_clocks_2020_2026.csv.gz")
CLOCK_SHA = "a42d83455ed22ebc681a301b83b03b31cf9bd89b317331b7a8e4dc7f0508110f"
CONTROL_DIR = Path("data/high_volatility_spot_lead_open_interest_acceleration_continuation_controls_2023_2026")
FREEZE = Path("results/high_volatility_spot_lead_open_interest_acceleration_continuation_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_spot_lead_open_interest_acceleration_continuation_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSPOIAC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSPOIAC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvspoiac_8_sequential_economics_v1", "policy_id": POLICY_ID,
