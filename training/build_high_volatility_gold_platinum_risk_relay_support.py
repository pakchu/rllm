"""Materialize outcome-blind source support for frozen HVAUPTR-24."""
...
from training import preregister_high_volatility_gold_platinum_risk_relay as prereg
...
BUILDER = Path("training/build_high_volatility_gold_platinum_risk_relay_support.py")
PREREG_SHA = "5ac24d3b4bcccd67a4436fb272b90d400a3e8260be86b4e3b4ceff925e4c070b"
SOURCE_DIR = Path("data/high_volatility_gold_platinum_risk_relay_sources_2022_2026")
RAW_GLD = SOURCE_DIR / "gld_yahoo_chart.json"
RAW_PPLT = SOURCE_DIR / "pplt_yahoo_chart.json"
EQUITY_PANEL = SOURCE_DIR / "gld_pplt_common_sessions.csv.gz"
FEATURE_PANEL = SOURCE_DIR / "hvauptr_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_gold_platinum_risk_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_gold_platinum_risk_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_gold_platinum_risk_relay_support_2026-08-11.json")
...
CONTROLS = ("no_btc_volatility_gate", "gld_only", "inverse_pplt_only", "one_session_stale_impulse", "direction_flip")
CLOCK_COLUMNS = (
    "candidate", "control", "split", "session_date", "cash_close_time", "feature_available_time",
    "entry_time", "exit_time", "side", "gld_intraday_return", "pplt_intraday_return", "ratio_impulse",
    "ratio_impulse_z", "btc_realized_variation", "btc_variation_rank",
)
QUERY = """
...
def _download_equity(symbol: str, base_url: str) -> tuple[bytes, pd.DataFrame]:
    ...
    if meta["symbol"] != symbol or meta["exchangeTimezoneName"] != "America/New_York":
        raise RuntimeError(f"HVAUPTR {symbol} Yahoo metadata drift")
    ...
    if frame.empty or frame.session_date.duplicated().any():
        raise RuntimeError(f"HVAUPTR {symbol} session dates invalid")
    ...
    if not valid.all():
        ...
        raise RuntimeError(f"HVAUPTR {symbol} invalid source rows: {dates[:5]}")
    return payload, frame


def common_equity_panel(gld: pd.DataFrame, pplt: pd.DataFrame) -> pd.DataFrame:
    left = gld.add_prefix("gld_").rename(columns={"gld_session_date": "session_date"})
    right = pplt.add_prefix("pplt_").rename(columns={"pplt_session_date": "session_date"})
    ...
    if not observed.equals(expected):
        ...
        raise RuntimeError(f"HVAUPTR equity/NYSE calendar mismatch missing={missing[:5]} extra={extra[:5]}")
    ...
    frame["gld_intraday_return"] = np.log(frame.gld_close / frame.gld_open)
    frame["pplt_intraday_return"] = np.log(frame.pplt_close / frame.pplt_open)
    frame["ratio_impulse"] = frame.gld_intraday_return - frame.pplt_intraday_return
    return frame
...
def load_bars() -> pd.DataFrame:
    ...
    if frame.columns.tolist() != ["ts", "open", "close"]:
        raise RuntimeError("HVAUPTR BTC schema drift")
    ...
    if len(frame) != len(expected) or not frame.ts.equals(pd.Series(expected, name="ts")):
        raise RuntimeError("HVAUPTR BTC source is not the exact requested 1m grid")
    for column in ("open", "close"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    if not np.isfinite(frame[["open", "close"]]).all(axis=None) or not frame[["open", "close"]].gt(0).all(axis=None):
        raise RuntimeError("HVAUPTR BTC prices invalid")
...
def build_features(equities: pd.DataFrame, bars: pd.DataFrame) -> pd.DataFrame:
    ...
    frame["ratio_impulse_z"] = strict_prior_zscore(frame.ratio_impulse)
    frame["gld_z"] = strict_prior_zscore(frame.gld_intraday_return)
    frame["pplt_z"] = strict_prior_zscore(frame.pplt_intraday_return)
...
def signal(features: pd.DataFrame, control: str) -> pd.Series:
    ratio_impulse = features.ratio_impulse.copy()
    zscore = features.ratio_impulse_z.copy()
    variation_gate = features.btc_variation_rank.ge(0.65)
    if control == "gld_only":
        ratio_impulse, zscore = features.gld_intraday_return, features.gld_z
    elif control == "inverse_pplt_only":
        ratio_impulse, zscore = -features.pplt_intraday_return, -features.pplt_z
    elif control == "one_session_stale_impulse":
        ratio_impulse, zscore = ratio_impulse.shift(1), zscore.shift(1)
    eligible = ratio_impulse.ne(0) & ratio_impulse.notna() & zscore.abs().ge(0.75) & variation_gate
    if control == "no_btc_volatility_gate":
        eligible = ratio_impulse.ne(0) & ratio_impulse.notna() & zscore.abs().ge(0.75)
    side = np.sign(ratio_impulse).astype("Int64").fillna(0).astype(int).where(eligible, 0)
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[sides.ne(0)]:
        ...
        rows.append(
            {
                "candidate": "HVAUPTR-24", "control": control, "split": split,
                ...
                "gld_intraday_return": float(features.at[index, "gld_intraday_return"]),
                "pplt_intraday_return": float(features.at[index, "pplt_intraday_return"]),
                "ratio_impulse": float(features.at[index, "ratio_impulse"]), "ratio_impulse_z": float(features.at[index, "ratio_impulse_z"]),
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVAUPTR preregistration hash drift")
    registration = json.loads(prereg.DEFAULT_OUTPUT.read_text())
    prereg.validate(registration)
    gld_payload, gld = _download_equity("GLD", prereg.GLD_YAHOO_URL)
    pplt_payload, pplt = _download_equity("PPLT", prereg.PPLT_YAHOO_URL)
    equities = common_equity_panel(gld, pplt)
    bars = load_bars()
    ...
    RAW_GLD.write_bytes(gld_payload)
    RAW_PPLT.write_bytes(pplt_payload)
    ...
    source_core = {
        "protocol_version": "hvauptr_24_sources_v1",
        "equity_urls": {"GLD": prereg.GLD_YAHOO_URL, "PPLT": prereg.PPLT_YAHOO_URL},
        ...
        "outputs": {
            "raw_gld": {"path": str(RAW_GLD), "sha256": sha(RAW_GLD)},
            "raw_pplt": {"path": str(RAW_PPLT), "sha256": sha(RAW_PPLT)},
    ...
    core = {
        "protocol_version": "hvauptr_24_source_support_v1", "policy_id": "HVAUPTR-24",
        "preregistration": {
