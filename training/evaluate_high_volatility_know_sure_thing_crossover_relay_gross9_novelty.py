"""Evaluate frozen HVKST-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVKST-24"
PROTOCOL = "hvkst_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_know_sure_thing_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVKST predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVKST novelty limits drift")
    if (
        ...
        raise RuntimeError("HVKST predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVKST primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvkst_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVKST-24 unchanged before economic outcomes",
    }
