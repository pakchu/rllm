"""Strict staged economics for the singleton HVCATFIR-8 policy."""
...
PREREG = Path("results/high_volatility_causal_average_ticket_flow_inventory_relay_preregistration_2026-08-16.json")
PREREG_SHA = "aee1c5f573850945dd3e2793ac7191004ce08b65bf84647cdd0d1988ead3b26a"
SUPPORT = Path("results/high_volatility_causal_average_ticket_flow_inventory_relay_support_2026-08-16.json")
SUPPORT_SHA = "dbb6a995088ea0aba370310100b220c997f970de61934dfe67f4e61ff1a87787"
GROSS9 = Path("results/high_volatility_causal_average_ticket_flow_inventory_relay_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "ac6aa04cd89d5b69a3953c65e8a59e3551226efb111e73d96944c225c9f0d5d8"
CLOCK = {
    "path": Path("data/high_volatility_causal_average_ticket_flow_inventory_relay_clocks_2020_2026.csv.gz"),
    "sha256": "c0ba875faa89ced9ce00a241e0ac797cf8ea7ad97287c8b2c03dbbc7c20fc730",
    "rows": 114,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_average_ticket_flow_inventory_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCATFIR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCATFIR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCATFIR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCATFIR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATFIR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATFIR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCATFIR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCATFIR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCATFIR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCATFIR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCATFIR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcatfir_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
