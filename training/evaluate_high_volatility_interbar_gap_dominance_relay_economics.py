"""Sequential strict economics for frozen HVIGDR-6."""
...
PREREG = Path("results/high_volatility_interbar_gap_dominance_relay_preregistration_2026-08-10.json")
PREREG_SHA = "aa850bcee14c51a9defecb4e1f828660b226a01e5c84284914085c68e95183d0"
SUPPORT = Path("results/high_volatility_interbar_gap_dominance_relay_support_2026-08-10.json")
SUPPORT_SHA = "29d0dfdfb6cd5e6ddcbd03a3875492672b946a163a1336818cc5d1747fe20593"
NOVELTY = Path("results/high_volatility_interbar_gap_dominance_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "7feaf2682c825f0a1223545a12bee8517086478acbedfd00ff18d471224b01fc"
CLOCK = Path("data/high_volatility_interbar_gap_dominance_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "82d140c589e3f983fb63b17a15b20abea0274d354b47b0beb1e540e39f1d6a9e"
CONTROL_DIR = Path("data/high_volatility_interbar_gap_dominance_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_interbar_gap_dominance_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_interbar_gap_dominance_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_gap_dominance_gate", "no_block_return_tail", "no_volatility_gate", "latest_dominant_gap_tie_break", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVIGDR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVIGDR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvigdr_6_sequential_economics_v1", "policy_id": POLICY_ID,
