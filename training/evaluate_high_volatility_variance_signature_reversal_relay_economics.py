"""Sequential strict economics for frozen HVVSR-24."""
...
PREREG = Path("results/high_volatility_variance_signature_reversal_relay_preregistration_2026-08-11.json")
PREREG_SHA = "af6a8bf5dfb895fb5e586632fcb4e19e1664cb68871c25e1a9e978045a14199d"
SUPPORT = Path("results/high_volatility_variance_signature_reversal_relay_support_2026-08-11.json")
SUPPORT_SHA = "7415af38d4285087e998ce2278c9b4fc03b12d92c3dbae581661615c7f52200d"
NOVELTY = Path("results/high_volatility_variance_signature_reversal_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "32d5d203eaf14635c6488044560afb5dcb97191ce9ef7b1ebac3e88748f8a288"
CLOCK = Path("data/high_volatility_variance_signature_reversal_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "48cba777b4269987dfed0bac83048636a325cfccc6a2290f73b7685a4664b4a1"
CONTROL_DIR = Path("data/high_volatility_variance_signature_reversal_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_variance_signature_reversal_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_variance_signature_reversal_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = (
    "no_signature_gate",
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVVSR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVVSR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvvsr_24_sequential_economics_v1", "policy_id": POLICY_ID,
