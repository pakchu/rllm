"""Evaluate the frozen HVIMR-8 action-vote clock against Gross9."""
...
POLICY = "HVIMR-8"
PROTOCOL = "hvimr_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_intraday_mechanism_router_clocks_2023_2026.csv.gz",
    "sha256": "811c690e0774fe3ce8ef090c56d1724460ca78e614b2b2217b7d85b4746d63c7",
    "rows": 224,
}
OUTPUT = Path("results/high_volatility_intraday_mechanism_router_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVIMR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVIMR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVIMR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVIMR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVIMR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVIMR-8 unchanged before economics",
    }
