"""Evaluate frozen HVECC-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVECC-24"
PROTOCOL = "hvecc_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_ehlers_cyber_cycle_trigger_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVECC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVECC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVECC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVECC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvecc_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVECC-24 unchanged before economic outcomes",
    }
