"""Evaluate the frozen HVDCSATPAPAR-8 action-vote clock against Gross9."""
...
POLICY = "HVDCSATPAPAR-8"
PROTOCOL = "hvdcsatpapar_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_dcs_ticket_premium_activity_router_clocks_2023_2026.csv.gz",
    "sha256": "a87c8e6d58696c79432205245311c562a719ac8a2470ec5ede5e86ed8e70af95",
    "rows": 146,
}
OUTPUT = Path("results/high_volatility_dcs_ticket_premium_activity_router_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVDCSATPAPAR-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVDCSATPAPAR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVDCSATPAPAR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVDCSATPAPAR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVDCSATPAPAR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVDCSATPAPAR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVDCSATPAPAR-8 unchanged before economics",
    }
