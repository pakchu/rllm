"""Deterministic outcome-blind source support for HVCBER-8."""
...
from training import preregister_high_volatility_causal_btc_eth_leverage_consensus_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "33f569507743a8ccb3f67a07ccd2d119df3ca4f23360604f11f2278cb3b46a5d"
...
BAR_QUERY = """SELECT date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '8 hours' AS decision_time,symbol,sum(ln(close/open)) AS completed_return,sum(power(ln(close/open),2)) AS minute_squared_return,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND high>=greatest(open,close,low) AND low<=least(open,close,high)) AS coherent FROM bars_binance WHERE symbol IN ('BTCUSDT','ETHUSDT') AND interval='1m' AND ts>=:start AND ts<:end GROUP BY 1,2 ORDER BY 1,2"""

ROOT = Path("data/high_volatility_causal_btc_eth_leverage_consensus_relay_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_btc_eth_leverage_consensus_relay_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_btc_eth_leverage_consensus_relay_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_causal_btc_eth_leverage_consensus_relay_support_2026-08-16.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "sum_open_interest",
    "previous_open_interest", "oi_change", "btc_return", "eth_return",
    "consensus_displacement", "consensus_rank", "realized_variation",
    "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "decision_time", "feature_available_time",
    "entry_time", "exit_time", "side", "oi_change", "btc_return", "eth_return",
    "consensus_displacement", "consensus_rank", "realized_variation", "variation_rank",
)
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVCBER-8 OI schema drift")
    expected_bars = [
        "decision_time", "symbol", "completed_return", "minute_squared_return",
        "source_rows", "distinct_rows", "first_ts", "last_ts", "coherent",
    ]
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCBER-8 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all():
        raise RuntimeError("HVCBER-8 invalid OI rows")
    ...
    if bars[["decision_time", "symbol"]].duplicated().any() or set(bars["symbol"]) != {"BTCUSDT", "ETHUSDT"}:
        raise RuntimeError("HVCBER-8 duplicate or missing symbol bar")
    ...
    wide = bars.set_index(["decision_time", "symbol"])[fields].unstack("symbol")
    wide.columns = [f"{field}_{symbol}" for field, symbol in wide.columns]
    wide["btc_return"] = wide["completed_return_BTCUSDT"]
    wide["eth_return"] = wide["completed_return_ETHUSDT"]
    wide["realized_variation"] = np.sqrt(wide["minute_squared_return_BTCUSDT"])
    wide["joint_bar_valid"] = wide["bar_valid_BTCUSDT"].eq(True) & wide["bar_valid_ETHUSDT"].eq(True)
    wide["consensus_displacement"] = wide[["btc_return", "eth_return"]].abs().min(axis=1)
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel = panel.join(
        bars[["joint_bar_valid", "btc_return", "eth_return", "consensus_displacement", "realized_variation"]],
        on="decision_time",
    )
    panel["source_valid"] = (
        ...
        & np.isfinite(
            panel[["sum_open_interest", "previous_open_interest", "oi_change", "btc_return", "eth_return", "consensus_displacement", "realized_variation"]]
        ).all(axis=1)
        & panel["sum_open_interest"].gt(0)
        & panel["previous_open_interest"].gt(0)
        & panel["realized_variation"].gt(0)
        & panel["btc_return"].ne(0)
        & panel["eth_return"].ne(0)
        & panel["consensus_displacement"].gt(0)
    )
    valid = panel["source_valid"].eq(True)
    panel["consensus_rank"] = causal_midrank(panel["consensus_displacement"].where(valid))
    panel["variation_rank"] = causal_midrank(panel["realized_variation"].where(valid))
    panel["eligible"] = (
        valid
        & np.sign(panel["btc_return"]).eq(np.sign(panel["eth_return"]))
        & panel["oi_change"].gt(0)
        & panel["consensus_rank"].ge(POLICY["consensus_rank_min"])
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCBER-8 side or availability drift")
        ...
        rows.append({
            ...
            "consensus_displacement": float(row.consensus_displacement),
            "consensus_rank": float(row.consensus_rank),
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCBER-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcber_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvcber_8_source_support_v1", "policy_id": prereg.POLICY_ID,
