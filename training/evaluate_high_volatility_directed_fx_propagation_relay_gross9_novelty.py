"""Evaluate frozen HVDFXPR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDFXPR-12"
PROTOCOL = "hvdfxpr_12_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_directed_fx_propagation_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDFXPR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVDFXPR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDFXPR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDFXPR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdfxpr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVDFXPR-12 unchanged before economic outcomes",
    }
