"""Evaluate frozen HVSENC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVSENC-8"
PROTOCOL = "hvsenc_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_sample_entropy_collapse_continuation_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVSENC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVSENC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVSENC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVSENC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVSENC-8 unchanged before economic outcomes",
    }
