"""Deterministic outcome-blind source support for HVIPRAPR-8."""
...
def rank(s:pd.Series)->pd.Series:
 ...
 for i,v in enumerate(vals):
  prior=np.asarray(hist[-POLICY["history_cycles"]:],float)
  if math.isfinite(v) and len(prior)>=POLICY["minimum_history_cycles"]:out[i]=(np.sum(prior<v)+.5*np.sum(prior==v))/len(prior)
...
def build_panel(raw):
 ...
 if p.columns.tolist()!=ep or b.columns.tolist()!=eb:raise RuntimeError("HVIPRAPR-8 schema drift")
 for f in (p,b):
 ...
 lower=f.relative_activity_rank.le(POLICY["lower_relative_activity_rank_max"]);upper=f.relative_activity_rank.ge(POLICY["upper_relative_activity_rank_min"])
 f["eligible"]=f.source_valid&f.btc_return.ne(0)&f.btc_variation_rank.ge(POLICY["btc_variation_rank_min"])&(lower|upper)
 side=np.sign(f.btc_return).fillna(0).astype(int);f["side"]=side.where(~upper,-side);f["feature_available_time"]=f.decision_time
 return f.loc[:,PANEL_COLUMNS]
def stage(e,x):return next((n for n,(a,b) in STAGES.items() if a<=e and x<=b),None)
def build_clock(p):
 rows=[];reserved=None
 for r in p.loc[p.eligible].itertuples(index=False):
  d=pd.Timestamp(r.decision_time);e=d+pd.Timedelta(minutes=POLICY["entry_delay_minutes"]);x=e+pd.Timedelta(hours=POLICY["hold_hours"])
  if reserved is not None and e<reserved:continue
  s=stage(e,x)
  if s is None:continue
  if r.side not in (-1,1) or r.feature_available_time>e:raise RuntimeError("HVIPRAPR-8 side/availability drift")
  reserved=x;rows.append({"candidate":prereg.POLICY_ID,"control":"primary","split":s,"decision_time":d,"feature_available_time":r.feature_available_time,"entry_time":e,"exit_time":x,"side":int(r.side),**{c:getattr(r,c) for c in PANEL_COLUMNS[3:9]}})
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVIPRAPR-8 preregistration drift")
