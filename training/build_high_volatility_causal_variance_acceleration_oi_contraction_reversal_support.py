"""Deterministic outcome-blind source support for HVCVAROICR-8."""
...
from training import preregister_high_volatility_causal_variance_acceleration_oi_contraction_reversal as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "125f52d7d28a1185b19b088fea590345159a2aaa210874ce9721801d3401e955"
...
ROOT = Path("data/high_volatility_causal_variance_acceleration_oi_contraction_reversal_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_variance_acceleration_oi_contraction_reversal_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_variance_acceleration_oi_contraction_reversal_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_causal_variance_acceleration_oi_contraction_reversal_support_2026-08-16.json")
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    oi,bars=raw
    if oi.columns.tolist()!=["decision_time","sum_open_interest"]: raise RuntimeError("HVCVAROICR-8 OI schema drift")
    ...
    if bars.columns.tolist()!=expected: raise RuntimeError("HVCVAROICR-8 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all() or not oi["sum_open_interest"].ge(0).all(): raise RuntimeError("HVCVAROICR-8 invalid OI rows")
    bars=bars.copy()
...
def build_panel(raw: tuple[pd.DataFrame,pd.DataFrame])->pd.DataFrame:
    ...
    panel["eligible"]=(valid&panel["oi_change"].lt(0)&panel["variance_acceleration_rank"].ge(POLICY["variance_acceleration_rank_min"])&panel["variation_rank"].ge(POLICY["variation_rank_min"]));panel["feature_available_time"]=panel["decision_time"];return panel.loc[:,PANEL_COLUMNS]
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCVAROICR-8 side or availability drift")
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCVAROICR-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcvaroicr_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvcvaroicr_8_source_support_v1", "policy_id": prereg.POLICY_ID,
