"""Build outcome-blind source support for frozen HVCTI-24."""
...
from training import preregister_high_volatility_ehlers_correlation_trend_zero_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "12ee1624c6e44beff404771a27724201a3ab0ec968c4fec346a70ca15260227b"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ehlers_correlation_trend_zero_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ehlers_correlation_trend_zero_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ehlers_correlation_trend_zero_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ehlers_correlation_trend_zero_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ehlers_correlation_trend_zero_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "cti", "slow_cti", "entry_side", "slow_entry_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "cti", "slow_cti", "entry_side",
    "slow_entry_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVCTI source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVCTI invalid source key")
...
def correlation_trend(close: pd.Series, valid: pd.Series, period: int) -> pd.Series:
    output=np.full(len(close),np.nan);window=[];y=np.arange(period,dtype=float);sy=float(y.sum());syy=float(np.square(y).sum())
    for i,(value,is_valid) in enumerate(zip(close.to_numpy(float),valid.to_numpy(bool))):
        ...
        x=np.asarray(window,float);sx=float(x.sum());sxx=float(np.square(x).sum());sxy=float((x*y).sum());den=(period*sxx-sx*sx)*(period*syy-sy*sy)
        if den>0:output[i]=(period*sxy-sx*sy)/math.sqrt(den)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["cti"]=correlation_trend(bars.bar_close,bars.valid_bar,P["cti_periods"]);bars["slow_cti"]=correlation_trend(bars.bar_close,bars.valid_bar,P["slow_control_periods"]);bars["entry_side"]=cross_side(bars.cti);bars["slow_entry_side"]=cross_side(bars.slow_cti)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["cti","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="slow_40_bar_cti":side=pd.to_numeric(used.slow_entry_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCTI prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvcti_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvcti_24_source_support_v1", "policy_id": prereg.POLICY_ID,
