"""Build the outcome-blind QLCD-288 support and novelty decision."""
...
DEFAULT_OUTPUT = Path("results/quantity_lattice_cohort_disagreement_support_2026-07-20.json")
DEFAULT_CLOCK = Path("data/quantity_lattice_cohort_disagreement_clock_2020_2023.csv.gz")
...
def load_preregistration() -> dict[str, Any]:
    if sha256_file(PREREGISTRATION_PATH) != PREREGISTRATION_FILE_SHA256:
        raise ValueError("QLCD preregistration file hash mismatch")
    ...
    if payload.get("manifest_hash") != PREREGISTRATION_MANIFEST_HASH:
        raise ValueError("QLCD preregistration manifest hash mismatch")
    return payload
...
def load_source_access_seal(
    ...
    if (
        ...
        raise RuntimeError("QLCD source access is not hash-sealed yet")
    if sha256_file(SOURCE_ACCESS_SEAL_PATH) != SOURCE_ACCESS_SEAL_FILE_SHA256:
        raise ValueError("QLCD source access seal hash mismatch")
...
def validate_source_frame(frame: pd.DataFrame, payload: dict[str, Any]) -> pd.DataFrame:
    ...
    if len(dates) != len(expected) or not dates.reset_index(drop=True).equals(pd.Series(expected)):
        raise ValueError("QLCD source is not the frozen complete five-minute grid")
    checked["date"] = dates
    ...
    if not np.isfinite(numeric.to_numpy(float)).all():
        raise ValueError("QLCD source contains non-finite numeric values")
    ...
    integer_values = checked.loc[:, list(integer_columns)].to_numpy(float)
    if not np.array_equal(integer_values, np.rint(integer_values)):
        raise ValueError("QLCD integer source fields contain fractional values")
    nonnegative_columns = (
        "agg_trade_count",
        "total_quantity_mbtc",
        "coarse_event_count",
    ...
    if (checked.loc[:, list(nonnegative_columns)].to_numpy(float) < 0.0).any():
        raise ValueError("QLCD count or unsigned quantity is negative")
    if not (
        _series(checked, "coarse_side").astype(int).isin([-1, 0, 1]).all()
        and _series(checked, "coarse_quantity_share").astype(float).between(0.0, 1.0).all()
        and _series(checked, "coarse_coherence").astype(float).between(0.0, 1.0).all()
        and _series(checked, "fine_signed_share").astype(float).between(-1.0, 1.0).all()
        and _series(checked, "cohort_opposition").astype(float).between(0.0, 1.0).all()
        and _series(checked, "qlcd_score").astype(float).between(0.0, 1.0).all()
    ):
        raise ValueError("QLCD normalized source fields are outside their bounds")
    if not (
        _series(checked.loc[observed], "agg_trade_count").astype(int).gt(0).all()
        and _series(checked.loc[observed], "total_quantity_mbtc").astype(int).gt(0).all()
        and _series(checked.loc[~observed], "agg_trade_count").astype(int).eq(0).all()
        and _series(checked.loc[~observed], "total_quantity_mbtc").astype(int).eq(0).all()
    ):
        raise ValueError("QLCD observed/source-empty accounting is inconsistent")
    ...
    if actual_gap_days != expected_gap_days:
        raise ValueError("QLCD source-gap day set differs from preregistration")

    total_quantity = _series(checked, "total_quantity_mbtc").astype(float)
    total_signed = _series(checked, "total_signed_quantity_mbtc").astype(float)
    coarse_quantity = _series(checked, "coarse_quantity_mbtc").astype(float)
    coarse_signed = _series(checked, "coarse_signed_quantity_mbtc").astype(float)
    medium_quantity = _series(checked, "medium_quantity_mbtc").astype(float)
    medium_signed = _series(checked, "medium_signed_quantity_mbtc").astype(float)
    fine_quantity = _series(checked, "fine_quantity_mbtc").astype(float)
    fine_signed = _series(checked, "fine_signed_quantity_mbtc").astype(float)
    ...
    expected_coherence = np.divide(
        np.abs(coarse_signed.to_numpy()),
        coarse_quantity.to_numpy(),
        out=np.zeros(len(checked), dtype=float),
        where=coarse_quantity.to_numpy() != 0.0,
    )
    expected_fine_share = np.divide(
        fine_signed.to_numpy(),
        fine_quantity.to_numpy(),
        out=np.zeros(len(checked), dtype=float),
        where=fine_quantity.to_numpy() != 0.0,
    )
    expected_side = np.sign(coarse_signed.to_numpy()).astype(np.int8)
    expected_opposition = np.clip(
        -expected_side.astype(float) * expected_fine_share, 0.0, 1.0
    )
    expected_score = expected_share * expected_coherence * expected_opposition
    partition = (
        np.array_equal(
            _series(checked, "agg_trade_count").to_numpy(np.int64),
            (
                _series(checked, "coarse_event_count").to_numpy(np.int64)
                + _series(checked, "medium_event_count").to_numpy(np.int64)
                + _series(checked, "fine_event_count").to_numpy(np.int64)
            ),
        )
        and np.allclose(
            total_quantity,
            coarse_quantity + medium_quantity + fine_quantity,
            atol=0.0,
            rtol=0.0,
        )
        and np.allclose(
            total_signed,
            coarse_signed + medium_signed + fine_signed,
            atol=0.0,
            rtol=0.0,
        )
        and bool(
            (coarse_quantity.ge(coarse_signed.abs()))
            .all()
            and (medium_quantity.ge(medium_signed.abs())).all()
            and (fine_quantity.ge(fine_signed.abs())).all()
        )
    )
    identities = (
        partition
        and np.allclose(
            _series(checked, "coarse_quantity_share"),
            expected_share,
            atol=1e-10,
            rtol=1e-10,
        )
        and np.allclose(
            _series(checked, "coarse_coherence"),
            expected_coherence,
            atol=1e-10,
            rtol=1e-10,
        )
        and np.allclose(
            _series(checked, "fine_signed_share"),
            expected_fine_share,
            atol=1e-10,
            rtol=1e-10,
        )
        and np.array_equal(
            _series(checked, "coarse_side").to_numpy(np.int8), expected_side
        )
        and np.allclose(
            _series(checked, "cohort_opposition"),
            expected_opposition,
            atol=1e-10,
            rtol=1e-10,
        )
        and np.allclose(
            _series(checked, "qlcd_score"),
            expected_score,
            atol=1e-10,
            rtol=1e-10,
        )
    ...
    if not identities:
        raise ValueError("QLCD source feature identities do not replay")
    return checked
...
def load_source(
    ...
    if protocol.get("outcomes_opened") is not False:
        raise ValueError("QLCD source manifest opened outcomes")
    if protocol.get("source_archive_manifest_sha256") != payload["source_contract"]["archive_manifest_sha256"]:
        raise ValueError("QLCD source archive contract mismatch")
    if protocol.get("source_audit_sha256") != payload["source_contract"]["source_audit_sha256"]:
        raise ValueError("QLCD source audit contract mismatch")
    source_hash = sha256_file(source)
    if manifest.get("combined_sha256") != source_hash:
        raise ValueError("QLCD combined source hash mismatch")
    if Path(str(manifest.get("combined_output"))) != source:
        raise ValueError("QLCD source path differs from its manifest")
...
def lagged_threshold(
    ...
    *,
    ...
    if not 0.0 < quantile < 1.0:
        raise ValueError("QLCD quantile must be strictly between zero and one")
    return cast(
        pd.Series,
        score.astype(float)
...
def build_clock(frame: pd.DataFrame, policy: dict[str, Any]) -> tuple[pd.DataFrame, pd.Series]:
    score = _series(frame, "qlcd_score").astype(float)
    ...
    eligible = (
        ...
        & _series(frame, "coarse_event_count").astype(float).ge(
            int(policy["minimum_coarse_event_count"])
        )
        & _series(frame, "fine_event_count").astype(float).ge(
            int(policy["minimum_fine_event_count"])
        )
        & _series(frame, "coarse_side").astype(int).ne(0)
        & _series(frame, "cohort_opposition").astype(float).gt(0.0)
        & score.gt(0.0)
        & threshold.notna()
        & score.ge(threshold)
    )
        ...
        rows.append(
            {
                ...
                "exit_time": exit_time,
                "side": int(_series(frame, "coarse_side").iloc[int(position)]),
    ...
    if not clock.empty:
        ...
        if entries.duplicated().any() or not entries.is_monotonic_increasing:
            raise ValueError("QLCD scheduled entries are duplicate or unordered")
        if not (exits > entries).all():
            raise ValueError("QLCD scheduled exits do not follow entries")
        if not set(_series(clock, "side").astype(int).unique()).issubset({-1, 1}):
            raise ValueError("QLCD clock contains an invalid side")
...
def _utc_entries(series: pd.Series, label: str) -> pd.Series:
    ...
    if parsed.isna().any():
        raise ValueError(f"{label} contains missing entries")
    return parsed.drop_duplicates().sort_values().reset_index(drop=True)
...
def one_to_one_matches(
    ...
    *,
    ...
    if tolerance < pd.Timedelta(0):
        raise ValueError("QLCD novelty tolerance cannot be negative")
...
def _member_entries(frame: pd.DataFrame, spec: dict[str, Any]) -> dict[str, pd.Series]:
    ...
    if member_column is None:
        if len(expected_members) != 1:
            raise ValueError("ungrouped QLCD comparator must have one member")
...
def novelty_report(clock: pd.DataFrame, payload: dict[str, Any]) -> dict[str, Any]:
    primary_entries = _utc_entries(_series(clock, "entry_time"), "QLCD primary entries")
    ...
    try:
        ...
        dense_report = {
            "error": f"BAFR: {type(exc).__name__}: {exc}",
            "gated": False,
        }
    return {
        "status": "EVALUATED",
...
def _write_once(path: Path, content: bytes) -> str:
    ...
    if path.exists():
        if path.read_bytes() != content:
            raise RuntimeError(f"refusing to overwrite frozen QLCD artifact: {path}")
...
def run_support(cfg: SupportConfig) -> dict[str, Any]:
    ...
    result = {
        "protocol_version": "quantity_lattice_cohort_disagreement_support_v1",
        "preregistration_hash": payload["manifest_hash"],
