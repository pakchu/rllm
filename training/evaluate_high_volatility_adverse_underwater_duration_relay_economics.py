"""Sequential strict economics for frozen HVAUD-8."""
...
PREREG = Path("results/high_volatility_adverse_underwater_duration_relay_preregistration_2026-08-10.json")
PREREG_SHA = "5a8d279c9b6900482ed9f43ba8054aa368498d4bad7a96af4acffc2bb0ee8286"
SUPPORT = Path("results/high_volatility_adverse_underwater_duration_relay_support_2026-08-10.json")
SUPPORT_SHA = "b803a5ffaf3528c8af9a076987542c787c18c640ffee576321b37a12743b42d2"
NOVELTY = Path("results/high_volatility_adverse_underwater_duration_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "2075bd5cbadb21fcb4fa817076febe7f65edeb8df1343da9c718b36a6b15ceab"
CLOCK = Path("data/high_volatility_adverse_underwater_duration_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "b2bce72a33cea92bbee06345d24fdbb8a1fd103812ad912af4ef4f9332c775ae"
CONTROL_DIR = Path("data/high_volatility_adverse_underwater_duration_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_adverse_underwater_duration_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_adverse_underwater_duration_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_duration_tail", "no_variation_gate", "raw_duration_at_most_half", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVAUD frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVAUD novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvaud_8_sequential_economics_v1", "policy_id": POLICY_ID,
