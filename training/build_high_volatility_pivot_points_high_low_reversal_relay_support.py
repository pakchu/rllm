"""Build outcome-blind source support for frozen HVPPHL-R10-24."""
...
from training import preregister_high_volatility_pivot_points_high_low_reversal_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "c8ca87b0237b332c408e6de1c3983a9ae6e40a35e93da41f471d1b09c2da78f1"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_pivot_points_high_low_reversal_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_pivot_points_high_low_reversal_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_pivot_points_high_low_reversal_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_pivot_points_high_low_reversal_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_pivot_points_high_low_reversal_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "pivot_center_high", "pivot_center_low", "pivot_type",
    "entry_side", "raw_bar_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "pivot_center_high", "pivot_center_low",
    "pivot_type", "entry_side", "raw_bar_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVPPHL source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVPPHL invalid source key")
...
def pivot_points_high_low(
    open_: pd.Series, high: pd.Series, low: pd.Series, close: pd.Series, valid: pd.Series,
) -> pd.DataFrame:
    """Confirm strict LEAN PPHL(10,10) only after ten right-side bars complete."""
    size = 2 * P["surrounding_high_bars"] + 1
    center = P["surrounding_high_bars"]
    values = {name: np.full(len(high), np.nan) for name in ("pivot_center_high", "pivot_center_low", "pivot_type")}
    entry_side = np.zeros(len(high), dtype=int)
    raw_bar_side = np.zeros(len(high), dtype=int)
    window_high: list[float] = []
    window_low: list[float] = []
    for index, is_valid in enumerate(valid.to_numpy(bool)):
        if not is_valid:
            window_high=[];window_low=[];continue
        raw_bar_side[index] = 1 if close.iloc[index] > open_.iloc[index] else -1 if close.iloc[index] < open_.iloc[index] else 0
        window_high.append(float(high.iloc[index]));window_low.append(float(low.iloc[index]))
        if len(window_high) > size: window_high.pop(0);window_low.pop(0)
        if len(window_high) < size: continue
        center_high, center_low = window_high[center], window_low[center]
        is_high = all(value < center_high for offset,value in enumerate(window_high) if offset != center)
        is_low = all(value > center_low for offset,value in enumerate(window_low) if offset != center)
        pivot_type = 2 if is_high and is_low else 1 if is_high else -1 if is_low else 0
        values["pivot_center_high"][index]=center_high;values["pivot_center_low"][index]=center_low;values["pivot_type"][index]=pivot_type
        entry_side[index] = 1 if pivot_type == -1 else -1 if pivot_type == 1 else 0
    return pd.DataFrame({**values,"entry_side":entry_side,"raw_bar_side":raw_bar_side},index=high.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=bars.join(pivot_points_high_low(bars.bar_open,bars.bar_high,bars.bar_low,bars.bar_close,bars.valid_bar));bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["pivot_center_high","pivot_center_low","pivot_type","variation"]]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="one_bar_stale_confirmation":state=state.shift(1,fill_value=False);side=side.shift(1,fill_value=0)
    elif control=="direction_flip":side=-side
    elif control=="raw_bar_direction":side=pd.to_numeric(used.raw_bar_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVPPHL prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvpphl_r10_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvpphl_r10_24_source_support_v1", "policy_id": prereg.POLICY_ID,
