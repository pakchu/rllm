"""Outcome-blind preregistration for HVCVAIR-8."""
...
POLICY_ID = "HVCVAIR-8"
DEFAULT_OUTPUT = Path("results/high_volatility_causal_volume_acceleration_inventory_relay_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_causal_volume_acceleration_inventory_relay_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": "When BTC perpetual quote turnover accelerates unusually from the first to the second four-hour half, realized variation is elevated, open interest expands, and the completed eight-hour price return is nonzero, broad participation and fresh leveraged inventory sponsor the completed price direction for eight hours.",
            "side": "strict sign of the completed eight-hour BTC perpetual return",
            "why_distinct": "HVCVAIR uses within-cycle quote-turnover acceleration plus exact OI expansion and variation, with completed price direction only after participation is established. Existing turnover/backloading candidates omit exact OI sponsorship; HVCFAR used signed aggressive-flow acceleration and failed source. HVCVAIR reuses neither event set nor control and uses no taker-flow sign, spot source, ETH, funding, premium-index, fitted outcome, or promoted control.",
            ...
            "why_low_gross9_overlap_is_plausible": "fixed eight-hour accelerating-turnover plus OI-sponsored states are absent from Gross9 primitives",
        },
        "features": {
            ...
            "completed_cycle": "480 exact coherent bars_binance BTCUSDT one-minute rows with nonnegative quote_asset_volume over [D-8h,D)",
            "half_turnovers": "sum quote_asset_volume separately over first and second 240-minute halves; both strict positive",
            "turnover_acceleration": "log(second_half_turnover/first_half_turnover), finite strict positive",
            "turnover_acceleration_rank": "strict-prior midrank over at most 270 source-valid cycles; current excluded; minimum 180; rank>=0.60",
            ...
            "eligible": "exact coherent completed perpetual cycle, positive turnover acceleration, nonzero completed return, two exact OI endpoints with strict expansion, and both turnover-acceleration and variation ranks pass at D; no taker-flow sign, spot source, funding, range, premium-index, alt, or post-decision condition",
            "no_imputation": True,
        },
        "clock": {
        ...
        "policy": {
            ...
            "path_minutes": 480,
            "half_minutes": 240,
            "turnover_acceleration_rank_min": 0.60,
        ...
        "research_boundary": {
            ...
            "prior_turnover_backloading_candidate_outcomes_known": True,
            "same_mechanism_as_prior_turnover_candidates": False,
            "prior_HVCFAR_source_failure_known": True,
            "same_mechanism_as_HVCFAR": False,
        ...
        "stopping_rule": "terminal first failure; no OI endpoint or expansion rule, turnover acceleration formula, acceleration or variation history, side, clock, hold, subset, threshold, comparator, or control repair",
    }
    return {**core, "manifest_hash": canonical_hash(core)}

def validate(value: Mapping[str, Any]) -> None:
    if value != build():
        raise RuntimeError("HVCVAIR-8 preregistration drift")
    contract = hvcav.build()
    for key in ("stages", "source_support_gates", "gross9_novelty_gates", "economic_gates"):
        if value[key] != contract[key]:
            raise RuntimeError(f"HVCVAIR-8 {key} drift")
