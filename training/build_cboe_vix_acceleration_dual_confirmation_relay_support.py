"""Build outcome-blind source support for preregistered CVVDMR-6."""
...
from training import preregister_cboe_vix_acceleration_dual_confirmation_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.build_cboe_volatility_surface_regime_crossing_relay_support import strict_prior_midrank

CLOCK = Path("data/cboe_vix_acceleration_dual_confirmation_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/cboe_vix_acceleration_dual_confirmation_relay_controls_2023_2026")
RESULT = Path("results/cboe_vix_acceleration_dual_confirmation_relay_support_2026-08-08.json")
SPLITS = {
...
CONTROLS = ("no_vix_level_acceleration", "overnight_direction_only", "opening_direction_only", "one_session_stale_vix_acceleration", "direction_flip")
...
COLUMNS = (
    ...
    "side", "delta_vix_level", "previous_delta_vix_level", "overnight_btc_return", "opening_hour_return",
)
...
def load_features() -> pd.DataFrame:
    for raw, expected in prereg.HASHES.items():
        if sha(Path(raw)) != expected:
            raise RuntimeError(f"CVVDMR frozen source changed: {raw}")
    cboe = pd.read_csv(prereg.CBOE, compression="gzip", usecols=["observation_date", "VIX_close"])
    ...
    if not cboe.observation_date.is_monotonic_increasing or cboe.observation_date.duplicated().any():
        raise RuntimeError("CVVDMR Cboe dates invalid")
    if not np.isfinite(cboe[["VIX_close"]]).all().all() or not cboe[["VIX_close"]].gt(0).all().all():
        raise RuntimeError("CVVDMR VIX-level values invalid")
    cboe["delta_vix_level"] = np.log(cboe.VIX_close).diff()
    cboe["previous_delta_vix_level"] = cboe.delta_vix_level.shift(1)
    ...
    if price.columns.tolist() != ["hour_start", "hour_open", "first_half_close", "second_half_open", "hour_close", "source_rows", "distinct_timestamps", "source_valid", "decision_time"]:
        raise RuntimeError("CVVDMR completed-price schema changed")
    ...
    if price.hour_start.duplicated().any() or not price.hour_start.is_monotonic_increasing:
        raise RuntimeError("CVVDMR completed-price clock invalid")
    ...
    for index in range(1, len(cboe) - 1):
        ...
        rows.append({
            ...
            "delta_vix_level": float(cboe.at[index, "delta_vix_level"]),
            "previous_delta_vix_level": float(cboe.at[index, "previous_delta_vix_level"]),
...
def signal(frame: pd.DataFrame, control: str) -> pd.Series:
    acceleration, overnight, opening = frame.delta_vix_level, frame.overnight_btc_return, frame.opening_hour_return
    ...
    if control == "no_vix_level_acceleration":
    ...
    elif control == "one_session_stale_vix_acceleration":
        eligible = valid & confirmed & frame.previous_delta_vix_level.gt(0)
    else:
...
def build_clock(frame: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in frame.index[sides.ne(0)]:
        ...
        rows.append({
            "candidate": "CVVDMR-6", "control": control, "split": split,
            ...
            "delta_vix_level": float(frame.at[index, "delta_vix_level"]),
            "previous_delta_vix_level": float(frame.at[index, "previous_delta_vix_level"]),
