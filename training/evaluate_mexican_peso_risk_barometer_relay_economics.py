"""Sequential strict economics for frozen MXRBR-12."""
...
PREREG = Path("results/mexican_peso_risk_barometer_relay_preregistration_2026-08-09.json")
PREREG_SHA = "191c1e6448677973c778404ea774bd0bd3cccc343a72183b6270dd7684a42816"
SUPPORT = Path("results/mexican_peso_risk_barometer_relay_support_2026-08-09.json")
SUPPORT_SHA = "3a703844768a2aeba5fa937240d892262437d6c750a52071f6b55631f9bacb8f"
NOVELTY = Path("results/mexican_peso_risk_barometer_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "e1b671a25251fe8b4cf3325edc334cddf0e53f9ebd996e2b645f8fdee84f190d"
CLOCK = Path("data/mexican_peso_risk_barometer_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "9f2c532fb3aab08b0513dec637f1545b04deef95b0a80f45d43fb57e4e032254"
CONTROL_DIR = Path("data/mexican_peso_risk_barometer_relay_controls_2023_2026")
FREEZE = Path("results/mexican_peso_risk_barometer_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/mexican_peso_risk_barometer_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "one_session_stale_fx", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("MXRBR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("MXRBR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "mxrbr_12_sequential_economics_v1", "policy_id": POLICY_ID,
