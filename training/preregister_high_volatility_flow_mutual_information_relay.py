"""Outcome-blind preregistration for HVFMI-8."""
...
POLICY_ID = "HVFMI-8"
DEFAULT_OUTPUT = Path(
    "results/high_volatility_flow_mutual_information_relay_preregistration_2026-08-10.json"
)
...
def build() -> dict[str, Any]:
    core = {
        "protocol_version": "high_volatility_flow_mutual_information_relay_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": (
                "A high-variation completed eight-hour BTC block with strong nonlinear mutual "
                "information between one-minute returns and aggressive-flow imbalance represents "
                "flow-transmitted price discovery rather than volatility unrelated to execution "
                "pressure. When the block and final two hours agree in direction, follow that "
                "direction for eight hours."
            ),
            "side": "common strict sign of completed eight-hour and final-two-hour returns",
            "why_distinct": (
                "HVFMI uses a within-block 3x3 empirical joint distribution and normalized mutual "
                "information. Existing flow candidates use aggregate sign agreement, linear "
                "covariance/correlation, acceleration, HHI, or cross-venue differences. It uses no "
                "funding, OI, cross-assets, prior event rows, outcome-fitted model, or controls."
            ),
            "why_suited_to_volatile_regimes": (
                "the causal realized-variation tail identifies elevated volatility while the "
                "mutual-information tail requires price changes to remain coupled to aggressive flow"
            ),
            "why_low_gross9_overlap_is_plausible": (
                "three fixed UTC nonlinear return-flow dependence onsets are absent from Gross9 primitives"
            ),
        },
        "features": {
            ...
            "normalized_mutual_information": (
                "sum p_ij*log(p_ij/(p_i*p_j)) divided by min(H(return_state),H(flow_state)); "
                "all nine cells use completed-block empirical frequencies and denominator must be positive"
            ),
        ...
        "policy": {
            ...
            "mutual_information_rank_min": 0.75,
        ...
        "research_boundary": {
            "prior_flow_confirmation_and_covariance_family_outcomes_known": True,
            "repository_return_flow_mutual_information_candidate_found": False,
            "prior_event_sets_reused": False,
            "prior_candidate_outcomes_used_to_set_hvfmi_formula_ranks_direction_hold_or_clock": False,
            ...
            "selection_basis": "fixed ternary return-flow normalized mutual information",
        },
        "stopping_rule": (
