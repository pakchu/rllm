"""Freeze outcome-blind CITA-1 support and falsification clocks."""
...
DEFAULT_OUTPUT = "results/cftc_institutional_transfer_absorption_support_2026-07-17.json"
DEFAULT_CLOCKS = "results/cftc_institutional_transfer_absorption_clocks_2026-07-17.csv.gz"
CONTROL_NAMES = (
    "primary",
    "asset_manager_only",
...
def _load_registration() -> dict[str, Any]:
    ...
    if registration.get("policy_id") != "CITA-1":
        raise ValueError("CITA-1 preregistration policy mismatch")
    ...
    if registration.get("policy", {}).get("mutable_parameters") != []:
        raise ValueError("CITA-1 preregistration is mutable")
...
def _random_side(report_date: str, signal_time: str) -> int:
    identity = f"CITA-1|{report_date}|{signal_time}".encode()
...
def build_clock_rows(rows: list[clock.SourceRow]) -> list[dict[str, Any]]:
    ...
    controls: dict[str, list[clock.Event]] = {
        "primary": primary,
        "asset_manager_only": clock.build_events(rows, mode="asset_manager_only"),
        "leveraged_contrarian_only": clock.build_events(
            rows, mode="leveraged_contrarian_only"
        ),
    ...
    ledger.extend(
        _event_row(
            "deterministic_random_side",
            event,
            side=_random_side(event.report_date, event.signal_time),
        )
...
def _window(rows: list[dict[str, Any]], start: str, end: str) -> list[dict[str, Any]]:
    return [
        row
        for row in rows
        if start <= row["entry_time"] and row["exit_time"] <= end
    ]
...
def _summary(rows: list[dict[str, Any]], start: str, end: str) -> dict[str, Any]:
    ...
    return {
        ...
        "special_publication_overrides": sum(
            bool(row["special_publication_override"]) for row in selected
        ),
...
def _registered_summary(item: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "longs": item["longs"],
        "shorts": item["shorts"],
        "special_publication_overrides": item["special_publication_overrides"],
...
def build_report(
    *, clocks_path: str | Path = DEFAULT_CLOCKS, write_clock: bool = True
    ...
    checks = {
        ...
        "source_rows_exactly_299": len(source_rows) == 299,
        ...
        "random_side_is_deterministic": all(
            row["side"] == _random_side(row["report_date"], row["signal_time"])
    ...
    core = {
        "protocol_version": "cftc_institutional_transfer_absorption_support_v1",
        "policy_id": "CITA-1",
