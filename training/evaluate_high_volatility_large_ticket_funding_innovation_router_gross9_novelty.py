"""Evaluate the frozen HVLTFIR-8 action-vote clock against Gross9."""
...
POLICY = "HVLTFIR-8"
PROTOCOL = "hvltfir_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_large_ticket_funding_innovation_router_clocks_2023_2026.csv.gz",
    "sha256": "5763b0d238d587ae5221863e73f2beeadf282c3488f1d89e5011c6389fdcc3b4",
    "rows": 320,
}
OUTPUT = Path("results/high_volatility_large_ticket_funding_innovation_router_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVLTFIR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVLTFIR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVLTFIR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVLTFIR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVLTFIR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVLTFIR-8 unchanged before economics",
    }
