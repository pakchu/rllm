"""Evaluate frozen HVIMI-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVIMI-24"
PROTOCOL = "hvimi_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_intraday_momentum_index_reentry_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVIMI predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVIMI novelty limits drift")
    if (
        ...
        raise RuntimeError("HVIMI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVIMI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvimi_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVIMI-24 unchanged before economic outcomes",
    }
