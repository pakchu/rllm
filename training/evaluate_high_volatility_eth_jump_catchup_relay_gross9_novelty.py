"""Evaluate frozen HVEJCR-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVEJCR-6"
PROTOCOL = "hvejcr_6_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_eth_jump_catchup_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVEJCR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVEJCR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVEJCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVEJCR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvejcr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVEJCR-6 unchanged before economic outcomes"
        ),
    }
