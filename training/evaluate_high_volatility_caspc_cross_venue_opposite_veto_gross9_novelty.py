"""Evaluate the frozen HVCASPCCVDRV-8 action-vote clock against Gross9."""
...
POLICY = "HVCASPCCVDRV-8"
PROTOCOL = "hvcaspccvdrv_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_caspc_cross_venue_opposite_veto_clocks_2023_2026.csv.gz",
    "sha256": "8dac068f55281664fb2d2f2d9c653fd96b90a10988b57661f5883c7ffc25772d",
    "rows": 138,
}
OUTPUT = Path("results/high_volatility_caspc_cross_venue_opposite_veto_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCASPCCVDRV-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCASPCCVDRV-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCASPCCVDRV-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCASPCCVDRV-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCASPCCVDRV-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCASPCCVDRV-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCASPCCVDRV-8 unchanged before economics",
    }
