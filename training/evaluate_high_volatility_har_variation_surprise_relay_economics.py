"""Sequential strict economics for frozen HVHVS-8."""
...
PREREG = Path("results/high_volatility_har_variation_surprise_relay_preregistration_2026-08-10.json")
PREREG_SHA = "a6a6f386218949d9e5d37d3168738970fea38cfd0ed09be5c0a7f0b0fc9ce44f"
SUPPORT = Path("results/high_volatility_har_variation_surprise_relay_support_2026-08-10.json")
SUPPORT_SHA = "c399ce04ba211c9dd32832c781062221a15c57ea08e91720e68c962d0bb1d24b"
NOVELTY = Path("results/high_volatility_har_variation_surprise_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "95d6f34d8e97e16b83b836be9bae6b389cf225435a622814183472a06aa8b753"
CLOCK = Path("data/high_volatility_har_variation_surprise_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "88500ff1ac74df1b87b15f128448a78006040847a5b9db7ebea3c6773b4f38d4"
CONTROL_DIR = Path("data/high_volatility_har_variation_surprise_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_har_variation_surprise_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_har_variation_surprise_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVHVS frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVHVS novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvhvs_8_sequential_economics_v1", "policy_id": POLICY_ID,
