"""Outcome-blind preregistration for CLWMSR-6."""
...
POLICY_ID = "CLWMSR-6"
DEFAULT_OUTPUT = Path(
    "results/confirmation_ladder_witness_migration_sponsorship_relay_preregistration_2026-08-13.json"
)
...
def build() -> dict[str, Any]:
    core = {
        "protocol_version": "confirmation_ladder_witness_migration_sponsorship_relay_v1",
        "policy_id": POLICY_ID,
        ...
        "singleton": True,
        "mechanism": {
            "claim": "When the final three canonical blocks have a higher mean BIP141 witness-byte share than the first three while their completed-minute BTC returns share one strict sign, witness-efficient settlement is broadening with directional price discovery; follow that common direction for six elapsed hours.",
            "side": "common strict sign of confirmation-interval returns 4, 5, and 6",
            "why_distinct": "CLWMSR couples an intraladder BIP141 witness-composition migration to late-three price unanimity. WCTR-288 used seven-day and one-day changes in aggregate 12-hour witness share and took side from the witness change; blockspace fee-witness concordance also used multi-day aggregates. CLWMSR instead compares the first and last three individual finalized blocks and takes side only from the unanimous late price response. CLTDR uses transaction count per weight, while settlement-load and dispersion candidates use weight alone. No prior event set or control is reused.",
            "volatile_market_target": "directional price discovery accompanied by a rapid shift toward witness-efficient finalized settlement identifies volatile blockspace participation; causal RV20 q90 remains only a later audit",
            "why_low_gross9_overlap_is_plausible": "height-modulo anchors, six individual block witness shares, late return unanimity, and six-hour reservation form a sparse asynchronous clock",
        },
        "features": {
            ...
            "block_composition": "S_k and W_k are canonical positive serialized size and block weight for block H+k, k=1..6; require S_k<=W_k<=4*S_k",
            "witness_share": "Q_k=(4*S_k-W_k)/(3*S_k), finite in [0,1], for k=1..6",
            "late_return": "L=sum(R_4,R_5,R_6), finite and strict nonzero",
            "late_unanimity": "R_4,R_5,R_6 have one common strict sign",
            "witness_migration": "mean(Q_4,Q_5,Q_6) > mean(Q_1,Q_2,Q_3), strict and threshold-free",
            "eligible_state": "late unanimity and witness migration are both true",
        ...
        "diagnostic_controls": {
            "definitions": {
                "late_unanimity_only": "late-three return unanimity onset without witness migration",
                "witness_migration_only": "back-half mean witness-share increase onset without late-three return unanimity; side is sign(L)",
                "serialized_size_migration": "replace Q_k with log(S_k) and require the back-half mean to exceed the first-half mean; retain late unanimity",
        ...
        "research_boundary": {
            ...
            "prior_aggregate_witness_source_incidence_known": True,
            "prior_aggregate_witness_outcomes_opened": False,
            "exact_clwmsr_incidence_known": False,
            "exact_clwmsr_outcomes_known": False,
