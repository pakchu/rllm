"""Evaluate frozen AARMR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "AARMR-8"
PROTOCOL = "aarmr_8_gross9_novelty_v1"
PREREG = Path("results/adjacent_auction_range_migration_relay_preregistration_2026-08-09.json")
PREREG_SHA = "0444afeb6c0742f4217016b7712788e72a4806fb71c3df04e6b254ea46b0d4ce"
SUPPORT = Path("results/adjacent_auction_range_migration_relay_support_2026-08-09.json")
SUPPORT_SHA = "4ef412c66b31dd790a901f8b894d0d9c814c4d8b8557d57a2f9ca28fafa3ac11"
CLOCK = Path("data/adjacent_auction_range_migration_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "f57db9aeffa672c0d711a7f7ef3a1771fa222818b4c02743ffc33e194439c714"
OUTPUT = Path("results/adjacent_auction_range_migration_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("AARMR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("AARMR novelty limits drift")
    if (
        ...
        raise RuntimeError("AARMR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="AARMR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "aarmr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject AARMR-8 unchanged before economic outcomes",
    }
