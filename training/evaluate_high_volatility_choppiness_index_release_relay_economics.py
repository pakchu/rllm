"""Sequential strict economics for frozen HVCIR-24."""
...
PREREG = Path("results/high_volatility_choppiness_index_release_relay_preregistration_2026-08-11.json")
PREREG_SHA = "50d593fe85ed6797c3870464c80bdad6655e737cb8d0dfee7397f6e2a32450d6"
SUPPORT = Path("results/high_volatility_choppiness_index_release_relay_support_2026-08-11.json")
SUPPORT_SHA = "7f9ed9baf379d53d8791ec5476354336d76c2b4f8cbd8e24255f21eda1c4eba7"
NOVELTY = Path("results/high_volatility_choppiness_index_release_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "aef2f9f2993fafdd538e9d37d2fcf03af8e2fced48e6f76e356f6c7bcbf5d772"
CLOCK = Path("data/high_volatility_choppiness_index_release_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d7c8ac9ebff3b6f367c59154b320c54a8bf50750bac55eeb363bab0de3684adb"
CONTROL_DIR = Path("data/high_volatility_choppiness_index_release_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_choppiness_index_release_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_choppiness_index_release_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "one_bar_stale_release", "direction_flip", "range_midpoint_direction")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCIR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCIR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcir_24_sequential_economics_v1", "policy_id": POLICY_ID,
