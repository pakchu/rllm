"""Strict staged economics for the singleton HVCCLOICR-8 policy."""
...
PREREG = Path("results/high_volatility_causal_close_location_open_interest_contraction_reversal_preregistration_2026-08-16.json")
PREREG_SHA = "c255fa0997c34f0bcf052db42db7a02c9087d9a5f07b7f38ec636d9886aa8045"
SUPPORT = Path("results/high_volatility_causal_close_location_open_interest_contraction_reversal_support_2026-08-16.json")
SUPPORT_SHA = "81e46c66dcc000d1b6642c09aa64b252aaba1e0e73fa8a098adda18bceb8be8d"
GROSS9 = Path("results/high_volatility_causal_close_location_open_interest_contraction_reversal_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "b54a5ce22be79139f32842522780df60cab1c756681f6f4fe0fbcf1a0442d7f6"
CLOCK = {
    "path": Path("data/high_volatility_causal_close_location_open_interest_contraction_reversal_clocks_2020_2026.csv.gz"),
    "sha256": "2c533ca13205ed25964fe19e63e09c9841f9aed3217fedb7e540695493742206",
    "rows": 371,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_close_location_open_interest_contraction_reversal_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCCLOICR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCCLOICR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCCLOICR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCCLOICR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCCLOICR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCCLOICR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCCLOICR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCCLOICR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCCLOICR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCCLOICR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCCLOICR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvccloicr_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
