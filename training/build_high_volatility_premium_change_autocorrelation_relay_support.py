"""Build outcome-blind source support for frozen HVPACR-12."""
...
PREREG_SHA = "42ac98bfa06e6500ef01f1429e70447f32a3e11781f513e3adb767a65a0e8c7d"
REG = prereg.build()
P = REG["policy"]
...
PREMIUM_QUERY = PERP_QUERY.replace("FROM bars_binance ", "FROM bars_binance_premium ")
ROOT = Path("data/high_volatility_premium_change_autocorrelation_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_premium_change_autocorrelation_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_premium_change_autocorrelation_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_premium_change_autocorrelation_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_premium_change_autocorrelation_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "perp_rows", "premium_rows",
    "premium_change_autocorrelation", "persistence_rank", "premium_displacement",
    "perp_return", "direction_agreement", "perp_variation", "variation_rank",
    "persistence_tail", "variation_tail", "joint_state", "onset", "entry_side", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "premium_change_autocorrelation", "persistence_rank",
    "premium_displacement", "perp_return", "direction_agreement",
    "perp_variation", "variation_rank", "persistence_tail", "variation_tail", "joint_state",
...
def prior_rank(series: pd.Series, continuity_valid: pd.Series) -> pd.Series:
    ...
    for index, value in enumerate(values):
        if not valid[index]:
            continue
        prior = np.asarray(history[-P["persistence_history_decisions"] :], float)
        if math.isfinite(value) and len(prior) >= P["minimum_persistence_history_decisions"]:
...
def premium_change_autocorrelation(closes: np.ndarray) -> float:
    closes = np.asarray(closes, float)
    if closes.shape != (P["five_minute_bars"],) or not np.isfinite(closes).all():
        return math.nan
    changes = np.diff(closes)
    denominator = float(np.sqrt(np.dot(changes[:-1], changes[:-1]) * np.dot(changes[1:], changes[1:])))
    if not math.isfinite(denominator) or denominator <= 0:
        return math.nan
    return float(np.dot(changes[:-1], changes[1:]) / denominator)
...
def prepare(frame: pd.DataFrame, *, positive: bool) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVPACR source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVPACR invalid source key")
    ...
    if positive:
        source["row_valid"] &= prices.gt(0).all(axis=1)
    return source.set_index("ts").sort_index()


def build_panel(perp_raw: pd.DataFrame, premium_raw: pd.DataFrame) -> pd.DataFrame:
    perp, premium = prepare(perp_raw, positive=True), prepare(premium_raw, positive=False)
    rows = []
    for source_start in pd.date_range(START, END, freq="4h", inclusive="left"):
        ...
        perp_block, premium_block = perp.reindex(expected), premium.reindex(expected)
        perp_rows, premium_rows = int(perp_block.row_valid.eq(True).sum()), int(premium_block.row_valid.eq(True).sum())
        valid = perp_rows == 240 and premium_rows == 240 and bool(perp_block.row_valid.eq(True).all()) and bool(premium_block.row_valid.eq(True).all())
        if valid:
            perp_closes = perp_block.close.to_numpy(float).reshape(48, 5)[:, -1]
            premium_closes = premium_block.close.to_numpy(float).reshape(48, 5)[:, -1]
            persistence = premium_change_autocorrelation(premium_closes)
            perp_return = float(np.log(perp_closes[-1] / perp_closes[0]))
            premium_displacement = float(premium_closes[-1] - premium_closes[0])
            variation = math.sqrt(float(np.square(np.diff(np.log(perp_closes))).sum()))
            valid = np.isfinite([persistence, premium_displacement, perp_return, variation]).all() and premium_displacement != 0 and perp_return != 0 and variation > 0
        else:
            persistence = premium_displacement = perp_return = variation = math.nan
        agreement = valid and np.sign(perp_return) == np.sign(premium_displacement)
        rows.append({"source_start": source_start, "source_valid": valid, "perp_rows": perp_rows, "premium_rows": premium_rows, "premium_change_autocorrelation": persistence, "premium_displacement": premium_displacement, "perp_return": perp_return, "direction_agreement": agreement, "perp_variation": variation})
    panel = pd.DataFrame(rows)
    panel["feature_available_time"] = panel.source_start + pd.Timedelta("4h")
    panel["persistence_rank"] = prior_rank(panel.premium_change_autocorrelation.where(panel.source_valid), panel.source_valid)
    panel["variation_rank"] = prior_rank(panel.perp_variation.where(panel.source_valid), panel.source_valid)
    panel["persistence_tail"] = panel.premium_change_autocorrelation.gt(0) & panel.persistence_rank.ge(P["persistence_rank_min"])
    panel["variation_tail"] = panel.variation_rank.ge(P["variation_rank_min"])
    panel["joint_state"] = panel.source_valid & panel.persistence_tail & panel.variation_tail & panel.direction_agreement
    panel["onset"] = fresh_onset(panel.joint_state, panel.source_valid)
    panel["entry_side"] = np.sign(panel.premium_displacement).fillna(0).astype(int)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    if control == "no_persistence_gate":
        ...
        state = fresh_onset(valid & used.persistence_tail & used.direction_agreement, valid) & side.ne(0)
    elif control == "negative_persistence":
        mirror = valid & used.premium_change_autocorrelation.lt(0) & used.persistence_rank.le(1 - P["persistence_rank_min"]) & used.variation_tail & used.direction_agreement
...
def build_clock(panel: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in panel.index[activity]:
        ...
        rows.append({
            ...
            **{column: bool(used.at[index, column]) if column in ("direction_agreement", "persistence_tail", "variation_tail", "joint_state", "onset", "eligible") else float(used.at[index, column]) for column in CLOCK_COLUMNS[8:]},
        })
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVPACR prereg drift")
    perp_raw, premium_raw = load_source()
    panel = build_panel(perp_raw, premium_raw)
    ...
    source_core = {
        "protocol_version": "hvpacr_12_source_v1", "queries": {"perpetual": PERP_QUERY, "premium": PREMIUM_QUERY},
        "query_sha256": {"perpetual": hashlib.sha256(PERP_QUERY.encode()).hexdigest(), "premium": hashlib.sha256(PREMIUM_QUERY.encode()).hexdigest()},
        "window": [START.isoformat(), END.isoformat()], "physical_rows": {"perpetual": len(perp_raw), "premium": len(premium_raw)},
    ...
    core = {
        "protocol_version": "hvpacr_12_source_support_v1", "policy_id": prereg.POLICY_ID,
