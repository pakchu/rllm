"""Evaluate the frozen HVCEDR-24 action-vote clock against Gross9."""
...
from training import preregister_high_volatility_causal_extreme_displacement_reversal as prereg

POLICY = "HVCEDR-24"
PROTOCOL = "hvcedr_24_gross9_novelty_v1"
PREREG = Path("results/high_volatility_causal_extreme_displacement_reversal_preregistration_2026-08-16.json")
PREREG_SHA = "5febd00c5aca66e6740590173d2c0a247b4a41cb5b355bb75aaeb8bc59b81b2e"
SUPPORT = Path("results/high_volatility_causal_extreme_displacement_reversal_support_2026-08-16.json")
SUPPORT_SHA = "b21d63ad36c512501d291e774a71b82525a46476bf3987c67651639f1545640b"
CLOCK = {
    "path": "data/high_volatility_causal_extreme_displacement_reversal_clocks_2020_2026.csv.gz",
    "sha256": "26c96fb2523363929c4f89f9290188d11c133a4affb7939d939ad8ca25b968f9",
    "rows": 340,
}
OUTPUT = Path("results/high_volatility_causal_extreme_displacement_reversal_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCEDR-24 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCEDR-24 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCEDR-24 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCEDR-24 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCEDR-24 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCEDR-24 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCEDR-24 unchanged before economics",
    }
