"""Build outcome-blind source support for frozen HVSPB-24."""
...
from training import preregister_high_volatility_ehlers_super_passband_reentry_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "fa196003a0fb5535b7d91903b8959ff5bc4eb91aff619fab6e5eaa973aa1513f"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ehlers_super_passband_reentry_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ehlers_super_passband_reentry_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ehlers_super_passband_reentry_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ehlers_super_passband_reentry_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ehlers_super_passband_reentry_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "run_length", "passband", "rms",
    "entry_side", "zero_cross_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "run_length", "passband", "rms",
    "entry_side", "zero_cross_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVSPB source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVSPB invalid source key")
...
def super_passband(series: pd.Series, valid: pd.Series) -> pd.DataFrame:
    a1=5.0/P["short_period"];a2=5.0/P["long_period"];values=series.to_numpy(float);ok=valid.to_numpy(bool);pb=np.full(len(series),np.nan);rms=np.full(len(series),np.nan);runs=np.zeros(len(series),dtype=int);run=0;history=[]
    for i,(price,is_valid) in enumerate(zip(values,ok)):
        if not is_valid or not math.isfinite(price):run=0;history=[];continue
        run+=1;runs[i]=run
        if run<=2:pb[i]=0.
        else:pb[i]=(a1-a2)*price+(a2*(1-a1)-a1*(1-a2))*values[i-1]+((1-a1)+(1-a2))*pb[i-1]-(1-a1)*(1-a2)*pb[i-2]
        history.append(float(pb[i]));history=history[-P["rms_period"]:]
        if len(history)==P["rms_period"]:rms[i]=math.sqrt(sum(value*value for value in history)/len(history))
    return pd.DataFrame({"run_length":runs,"passband":pb,"rms":rms},index=series.index)
...
def reentry_side(passband: pd.Series, rms: pd.Series) -> pd.Series:
    prior_pb=passband.shift(1);prior_rms=rms.shift(1);side=np.zeros(len(passband),dtype=int)
    side[(prior_pb<=-prior_rms)&(passband>-rms)]=1;side[(prior_pb>=prior_rms)&(passband<rms)]=-1
    return pd.Series(side,index=passband.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    fields=["bar_open","bar_high","bar_low","bar_close"];bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[fields]).all(axis=1)&bars[fields].gt(0).all(axis=1);values=super_passband(bars.bar_close,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["entry_side"]=reentry_side(bars.passband,bars.rms);bars["zero_cross_side"]=cross_side(bars.passband);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["passband","rms","variation"];bars["source_valid"]=bars.valid_bar&bars.run_length.ge(P["warmup_observations"])&np.isfinite(bars[numeric]).all(axis=1)&bars.rms.gt(0)&bars.variation.gt(0)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVSPB prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvspb_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvspb_24_source_support_v1", "policy_id": prereg.POLICY_ID,
