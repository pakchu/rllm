"""Evaluate frozen HVVWMMR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVVWMMR-8"
PROTOCOL = "hvvwmmr_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_volume_weighted_median_migration_relay_preregistration_2026-08-10.json")
PREREG_SHA = "14ec00d3055ce4d3ec2a78d4493b95d00d91c8d3190638542fb3a051956874c5"
SUPPORT = Path("results/high_volatility_volume_weighted_median_migration_relay_support_2026-08-10.json")
SUPPORT_SHA = "0073277e9799ed151c8207e6bffb8463c0da2d9ae12560af894b5245e19b162a"
CLOCK = Path("data/high_volatility_volume_weighted_median_migration_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "0eb3725b8165cc756aa711b47bb81f83500accdf43c882c3c4a61d5f9c183b7f"
OUTPUT = Path("results/high_volatility_volume_weighted_median_migration_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVVWMMR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVVWMMR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVVWMMR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVVWMMR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvvwmmr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVVWMMR-8 unchanged before economic outcomes",
    }
