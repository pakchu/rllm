"""Build source-only HBDC-6 clocks without opening execution outcomes."""
...
from training import preregister_header_backdating_disorder_correction as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.download_fetls_block_summaries import OUTPUT as BLOCKS


ENV_FILE = "/home/pakchu/rllm/.env"
PREREG_SHA = "ce8754d0a8882a5e1ab2aafc6dc065d69e8798df165375fb627900721c229cd7"
...
CLOCK = Path("data/header_backdating_disorder_correction_clocks_2023_2026.csv.gz")
RESULT = Path("results/header_backdating_disorder_correction_support_2026-08-09.json")
COLUMNS = (
    "candidate", "split", "anchor_height", "confirmation_height", "decision_time",
    "entry_time", "exit_time", "side", "backstep_seconds", "predecessor_mediantime",
    "side_return",
)
...
def validate_blocks(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("HBDC block schema drift")
...
def build_clock(blocks: pd.DataFrame, bars: pd.DataFrame) -> pd.DataFrame:
    ...
    for index in range(1, len(blocks) - 6):
        ...
        delta = int(previous.timestamp - current.timestamp)
        if not (delta >= 300 and int(current.timestamp) > int(previous.mediantime)):
            continue
        ...
        rows.append({
            ...
            "side": int(-np.sign(side_return)), "backstep_seconds": delta,
            "predecessor_mediantime": int(previous.mediantime), "side_return": side_return,
        })
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hbdc_6_source_support_v1", "policy_id": prereg.POLICY_ID,
