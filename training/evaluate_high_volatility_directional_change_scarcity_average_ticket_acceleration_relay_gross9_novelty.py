"""Evaluate frozen HVDCSATA-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDCSATA-8"
PROTOCOL = "hvdcsata_8_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_directional_change_scarcity_average_ticket_acceleration_relay_gross9_novelty_2026-08-16.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDCSATA predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVDCSATA novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDCSATA predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDCSATA primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject HVDCSATA-8 unchanged before economic outcomes"
        ),
    }
