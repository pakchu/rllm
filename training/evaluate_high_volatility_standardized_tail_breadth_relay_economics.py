"""Sequential strict economics for frozen HVSTBR-8."""
...
PREREG = Path("results/high_volatility_standardized_tail_breadth_relay_preregistration_2026-08-10.json")
PREREG_SHA = "6d70486179ac1ac8b35c53fbff02a580e526207216f286c746ef7659c45b69ca"
SUPPORT = Path("results/high_volatility_standardized_tail_breadth_relay_support_2026-08-10.json")
SUPPORT_SHA = "07ae089dfb634bb7e5092def89f5536897d1610b26f9afc79bb54321d62b8fc1"
NOVELTY = Path("results/high_volatility_standardized_tail_breadth_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "168ad6d0120618184aaded4cd9932597e6220c620613c0acd5db4561922f6f40"
CLOCK = Path("data/high_volatility_standardized_tail_breadth_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "05fbef229d0b8c2d0e14e89fb3f935718c7cefb7128de7fc1d240b9849eb10f9"
CONTROL_DIR = Path("data/high_volatility_standardized_tail_breadth_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_standardized_tail_breadth_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_standardized_tail_breadth_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_tail_breadth_share_gate", "no_variation_gate", "raw_absolute_tail_breadth_at_least_one", "one_block_stale_features", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSTBR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSTBR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvstbr_8_sequential_economics_v1", "policy_id": POLICY_ID,
