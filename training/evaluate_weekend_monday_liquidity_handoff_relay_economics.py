"""Sequential strict economics for frozen WMLHR-16."""
...
PREREG = Path("results/weekend_monday_liquidity_handoff_relay_preregistration_2026-08-09.json")
PREREG_SHA = "3f741f7c7750844d214c9b46fe28b123baa52e0cd3ed76e4b4755b7950e06b5c"
SUPPORT = Path("results/weekend_monday_liquidity_handoff_relay_support_2026-08-09.json")
SUPPORT_SHA = "0e71d70affd44afa61df08e7cfaf95ffa9d10276720697d021d30c7d1b538c88"
NOVELTY = Path("results/weekend_monday_liquidity_handoff_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "e52c13b10af68b0d3ea42cc53940125139257a904c1615ef810679e6d11d7b73"
CLOCK = Path("data/weekend_monday_liquidity_handoff_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d3139659669d62691ba1115c089862c79f86d41a4a3b03008f0350c1b15192bf"
CONTROL_DIR = Path("data/weekend_monday_liquidity_handoff_relay_controls_2023_2026")
FREEZE = Path("results/weekend_monday_liquidity_handoff_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/weekend_monday_liquidity_handoff_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_liquidity_reentry", "weekend_continuation", "monday_fade", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("WMLHR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("WMLHR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "wmlhr_16_sequential_economics_v1", "policy_id": POLICY_ID,
