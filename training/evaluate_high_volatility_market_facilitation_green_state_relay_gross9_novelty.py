"""Evaluate frozen HVMFI-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVMFI-24"
PROTOCOL = "hvmfi_24_gross9_novelty_v1"
PREREG = Path("results/high_volatility_market_facilitation_green_state_relay_preregistration_2026-08-11.json")
PREREG_SHA = "2de5ec2a1e596ab81cfdc2175f9d67372b47e0da614d69c5a3426aae7821ccaa"
SUPPORT = Path("results/high_volatility_market_facilitation_green_state_relay_support_2026-08-11.json")
SUPPORT_SHA = "38abe02f0ccd767ac250dec93d72695ce000a18a3ac391ab211651f772fab489"
CLOCK = Path("data/high_volatility_market_facilitation_green_state_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "5c6a4a8d5839894b0396d2ba60a9a5b9733b15973abc64dbe975a48ea642a7f7"
OUTPUT = Path("results/high_volatility_market_facilitation_green_state_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVMFI predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVMFI novelty limits drift")
    if (
        ...
        raise RuntimeError("HVMFI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVMFI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvmfi_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVMFI-24 unchanged before economic outcomes",
    }
