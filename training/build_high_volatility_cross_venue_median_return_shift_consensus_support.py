"""Deterministic source support for HVCVMRS-8."""
...
ENV_FILE="/home/pakchu/rllm/.env";PREREG_SHA="fce6bb10db2727caf4b82b3ecdb6bbbf51549680fb8e2267fa782fb55516bd88"
PRIMARY=Path(prereg.PRIMARY["clock"]["path"]);SPOT=Path("data/high_volatility_cross_venue_median_return_shift_consensus_spot_shifts_2023_2026.csv.gz");CLOCK=Path("data/high_volatility_cross_venue_median_return_shift_consensus_clocks_2023_2026.csv.gz");RESULT=Path("results/high_volatility_cross_venue_median_return_shift_consensus_support_2026-08-18.json")
...
def prepare_spot(frame:pd.DataFrame)->pd.DataFrame:
 ...
 if frame.columns.tolist()!=expected:raise RuntimeError("HVCVMRS-8 spot schema drift")
 x=frame.copy()
...
def confirm(primary:pd.DataFrame,spot:pd.DataFrame)->pd.DataFrame:
 ...
 keep=(merged["source_valid"].eq(True)&np.isfinite(merged["spot_shift"])&merged["spot_shift"].ne(0)&(np.sign(merged["spot_shift"])==merged["side"]))
...
def run(spot_path:Path=SPOT,clock_path:Path=CLOCK,result_path:Path=RESULT)->dict[str,Any]:
 if prereg.sha256(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCVMRS-8 prereg drift")
