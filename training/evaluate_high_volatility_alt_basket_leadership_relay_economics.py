"""Sequential strict economics for frozen HVABLR-6."""
...
PREREG = Path("results/high_volatility_alt_basket_leadership_relay_preregistration_2026-08-10.json")
PREREG_SHA = "285d84214e03743e6120380016fbaf35cee4978fedb3d209359b4e91efd7cc76"
SUPPORT = Path("results/high_volatility_alt_basket_leadership_relay_support_2026-08-10.json")
SUPPORT_SHA = "cb5fa165abd58b1181352a712220739ec287af8390070007868cb17ef0418bb4"
NOVELTY = Path("results/high_volatility_alt_basket_leadership_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "f997a9b3aebcfe7e096fa4c6abc524c14699953555ebd8959556030df8b8c710"
CLOCK = Path("data/high_volatility_alt_basket_leadership_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "08a52e0609a8c40b6243b1b09810b858aef3b92e051a528b6be5a2353da5702f"
CLOCK_ROWS = 402
CONTROL_DIR = Path("data/high_volatility_alt_basket_leadership_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_alt_basket_leadership_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_alt_basket_leadership_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_btc_variation_gate", "no_leadership_gate", "contemporaneous_correlation", "one_hour_stale_features", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVABLR frozen predecessor hash drift")
    support = load_json(SUPPORT)
    if (
        support.get("clock", {}).get("sha256") != CLOCK_SHA
        or support.get("clock", {}).get("rows") != CLOCK_ROWS
    ):
        raise RuntimeError("HVABLR support clock binding drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVABLR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvablr_6_sequential_economics_v1", "policy_id": POLICY_ID,
