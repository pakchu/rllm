"""Sequential strict economics for frozen HVRWI-24."""
...
PREREG = Path("results/high_volatility_random_walk_index_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "d5be749a236881c444d006bcd8ec709071c07a9b94266cfcf6c31ad560f4706c"
SUPPORT = Path("results/high_volatility_random_walk_index_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "429bdecfba73474cd6049c4c0c5ed6b6c75c560d82bcd19b919e1b4f3f5cc88f"
NOVELTY = Path("results/high_volatility_random_walk_index_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "2da18695cd3da8bbfb4f59cc61b7f9f0566149363da6328d78285f5a2deee8a9"
CLOCK = Path("data/high_volatility_random_walk_index_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "80dee714175d59f7695cf13d95d009326dfad301381ace4d049b13e5cde3396f"
CONTROL_DIR = Path("data/high_volatility_random_walk_index_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_random_walk_index_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_random_walk_index_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "fixed_14_only", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVRWI frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVRWI novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrwi_24_sequential_economics_v1", "policy_id": POLICY_ID,
