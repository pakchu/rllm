"""Build HVITR-8 source features and freeze its pre-2023 ridge model."""
...
from training import preregister_high_volatility_intrinsic_topology_ridge_relay as prereg
...
SOURCE_DIR = Path("data/high_volatility_intrinsic_topology_ridge_relay_sources_2020_2026")
PANEL = SOURCE_DIR / "eight_hour_feature_panel.csv.gz"
TRAINING = SOURCE_DIR / "pre2023_training_rows.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
MODEL = Path("results/high_volatility_intrinsic_topology_ridge_relay_model_freeze_2026-08-10.json")
PREREG_SHA256 = "a5f81522864782a8b88a286abf33039e35995f9776518c9379ae44fdda488c7c"
...
def temporal_rank_concordance(close: np.ndarray) -> float:
    ranks = pd.Series(np.asarray(close, dtype=float)).rank(method="average").to_numpy(float)
    if len(ranks) < 2 or np.std(ranks) <= 0:
        return float("nan")
    return float(np.corrcoef(np.arange(len(ranks), dtype=float), ranks)[0, 1])


def directional_change_count(log_close: np.ndarray, threshold: float) -> int:
    prices = np.asarray(log_close, dtype=float)
    if len(prices) < 2 or not math.isfinite(threshold) or threshold <= 0:
        raise ValueError("directional-change inputs invalid")
    state = 0
    extreme = float(prices[0])
    reversals = 0
    for raw_price in prices[1:]:
...
def intrinsic_topology_features(window: pd.DataFrame) -> tuple[dict[str, float], float]:
    log_close = np.log(window.close.to_numpy(float))
    returns = np.diff(log_close)
    full_variation = float(np.square(returns).sum())
    root_variation = math.sqrt(full_variation)
    rms_return = math.sqrt(float(np.square(returns).mean()))
    full_return = math.log(float(window.close.iloc[-1]) / float(window.open.iloc[0]))
    late_return = math.log(float(window.close.iloc[-1]) / float(window.open.iloc[360]))
    positive = returns[returns > 0]
    negative = returns[returns < 0]
    absolute = np.abs(returns)
    autocorrelation = float("nan")
    if np.std(absolute[:-1]) > 0 and np.std(absolute[1:]) > 0:
        autocorrelation = abs(float(np.corrcoef(absolute[:-1], absolute[1:])[0, 1]))
    demeaned = returns - returns.mean()
    energy = np.square(np.abs(np.fft.rfft(demeaned)))
    positive_frequency_energy = float(energy[1:].sum())
    block_low = float(window.low.min())
    block_high = float(window.high.max())
    if root_variation <= 0 or rms_return <= 0 or positive_frequency_energy <= 0 or block_high <= block_low:
        raise ValueError("intrinsic-topology feature denominator invalid")
    values = {
        "normalized_full_return": full_return / root_variation,
        "normalized_late_return": late_return / root_variation,
        "temporal_rank_concordance": temporal_rank_concordance(log_close),
        "directional_change_density_0p5": directional_change_count(log_close, 0.5 * rms_return) / 478.0,
        "directional_change_density_1p0": directional_change_count(log_close, rms_return) / 478.0,
        "directional_change_density_2p0": directional_change_count(log_close, 2.0 * rms_return) / 478.0,
        "semivariance_balance": (float(np.square(positive).sum()) - float(np.square(negative).sum())) / full_variation,
        "absolute_return_autocorrelation": autocorrelation,
        "low_frequency_share": float(energy[1:9].sum()) / positive_frequency_energy,
        "close_location": 2.0 * (float(window.close.iloc[-1]) - block_low) / (block_high - block_low) - 1.0,
    }
    if not all(math.isfinite(value) for value in values.values()):
        raise ValueError("intrinsic-topology features nonfinite")
...
def feature_panel(bars: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for decision in decisions:
        ...
        if valid:
            try:
                values, full_variation = intrinsic_topology_features(window)
            except ValueError:
                valid = False
...
def fit_model(panel: pd.DataFrame, labels: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if len(training) == 0:
        raise RuntimeError("HVITR training population empty")
    mean = matrix.mean(axis=0)
    scale = matrix.std(axis=0, ddof=0)
    if not np.isfinite(mean).all() or not np.isfinite(scale).all() or (scale <= 0).any():
        raise RuntimeError("HVITR training standardization invalid")
    ...
    if high_volatility.sum() == 0:
        raise RuntimeError("HVITR training high-volatility population empty")
    threshold = float(np.quantile(np.abs(fitted[high_volatility]), 0.75, method="linear"))
    if not math.isfinite(threshold) or threshold <= 0:
        raise RuntimeError("HVITR prediction-strength threshold invalid")
...
def run() -> dict[str, Any]:
    ...
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA256:
        raise RuntimeError("HVITR preregistration artifact drift")
    registration = json.loads(prereg.DEFAULT_OUTPUT.read_text())
    if registration != prereg.build():
        raise RuntimeError("HVITR preregistration payload drift")
    ...
    source_core = {
        "protocol_version": "hvitr_8_source_and_pretraining_v1",
        "preregistration": {
    ...
    model_core = {
        "protocol_version": "hvitr_8_model_freeze_v1",
        "policy_id": "HVITR-8",
