"""Build outcome-blind source support for frozen HVRAVI-24."""
...
PREREG_SHA = "0febe5691eb98cd085d9a18d7fef54e11ccebae0a10853645d8b88c1ea328e51"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_range_action_verification_ignition_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_range_action_verification_ignition_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_range_action_verification_ignition_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_range_action_verification_ignition_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_range_action_verification_ignition_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "fast_sma", "slow_sma", "signed_spread_pct", "ravi",
    "entry_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "fast_sma", "slow_sma", "signed_spread_pct",
    "ravi", "entry_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVRAVI source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVRAVI invalid source key")
...
def ravi(close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    values = close.where(valid)
    fast = values.rolling(P["fast_periods"], min_periods=P["fast_periods"]).mean()
    slow = values.rolling(P["slow_periods"], min_periods=P["slow_periods"]).mean()
    signed = (100.0 * (fast - slow) / slow).where(slow.gt(0))
    return pd.DataFrame({"fast_sma": fast, "slow_sma": slow, "signed_spread_pct": signed, "ravi": signed.abs()}, index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[["bar_open","bar_high","bar_low","bar_close"]]).all(axis=1)&bars[["bar_open","bar_high","bar_low","bar_close"]].gt(0).all(axis=1);bars=pd.concat([bars,ravi(bars.bar_close,bars.valid_bar)],axis=1);bars["entry_side"]=np.sign(bars.signed_spread_pct).fillna(0).astype(int);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["fast_sma","slow_sma","signed_spread_pct","ravi","variation"]]).all(axis=1)&bars.slow_sma.gt(0)&bars.entry_side.ne(0)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.ravi.shift(1);ignition=previous.le(P["trend_threshold_pct"])&panel.ravi.gt(P["trend_threshold_pct"]);panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&ignition&panel.variation_rank.ge(P["variation_rank_min"]);return panel.loc[:,PANEL_COLUMNS]


def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);value=pd.to_numeric(used.ravi,errors="coerce");previous=value.shift(1);ignition=previous.le(P["trend_threshold_pct"])&value.gt(P["trend_threshold_pct"]);release=previous.ge(P["trend_threshold_pct"])&value.lt(P["trend_threshold_pct"]);variation=used.variation_rank.ge(P["variation_rank_min"]);side=pd.to_numeric(used.entry_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&ignition&variation
    if control=="no_variation_gate":state=valid&side.ne(0)
    elif control=="trend_release":state=valid&side.ne(0)&release&variation
    elif control=="one_bar_stale_ignition":state=state.shift(1,fill_value=False);side=side.shift(1,fill_value=0)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVRAVI prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvravi_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvravi_24_source_support_v1", "policy_id": prereg.POLICY_ID,
