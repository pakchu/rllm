"""Evaluate frozen HVTBCR-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVTBCR-24"
PROTOCOL = "hvtbcr_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_treasury_belly_curvature_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTBCR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVTBCR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTBCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTBCR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVTBCR-24 unchanged before economic outcomes",
    }
