"""Sequential strict economics for frozen HVHIKKAKE-C3-8."""
...
PREREG = Path("results/high_volatility_hikkake_pattern_relay_preregistration_2026-08-11.json")
PREREG_SHA = "5e8a930d33856288cb670770da33cd85a1a9009377d4ad268cf0a13e49258704"
SUPPORT = Path("results/high_volatility_hikkake_pattern_relay_support_2026-08-11.json")
SUPPORT_SHA = "d348276dd539ee7ce9aab1bfe7198dc0801c9d97b209bf5fb03aec7632af0f9a"
NOVELTY = Path("results/high_volatility_hikkake_pattern_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "02438d6701810b19e9cc0cb2f682d6317c09e07258686fd90671587d786d5720"
CLOCK = Path("data/high_volatility_hikkake_pattern_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "c4370032c2f95062300c08ec5513fb20632d3b7b1b2522a626287ba0fd626160"
CONTROL_DIR = Path("data/high_volatility_hikkake_pattern_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_hikkake_pattern_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_hikkake_pattern_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = (
    "no_variation_gate",
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVHIKKAKE frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVHIKKAKE novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvhikkake_c3_8_sequential_economics_v1", "policy_id": POLICY_ID,
