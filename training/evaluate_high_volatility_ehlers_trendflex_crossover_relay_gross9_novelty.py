"""Evaluate frozen HVETF-24 structural novelty against every Gross9 sleeve."""
...
from training import preregister_high_volatility_ehlers_trendflex_crossover_relay as registration

POLICY = "HVETF-24"
PROTOCOL = "hvetf_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_ehlers_trendflex_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVETF predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVETF novelty limits drift")
    if (
        ...
        raise RuntimeError("HVETF predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVETF primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVETF-24 unchanged before economic outcomes",
    }
