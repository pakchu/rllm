"""Evaluate frozen HVSMI-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVSMI-24"
PROTOCOL = "hvsmi_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_stochastic_momentum_index_zero_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVSMI predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVSMI novelty limits drift")
    if (
        ...
        raise RuntimeError("HVSMI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVSMI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvsmi_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVSMI-24 unchanged before economic outcomes",
    }
