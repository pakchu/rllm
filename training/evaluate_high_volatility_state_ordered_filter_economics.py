"""Strict staged economics and train-only selection for HVSOF-8."""
...
PREREG = Path("results/high_volatility_state_ordered_filter_preregistration_2026-08-14.json")
...
GROSS9 = Path("results/high_volatility_state_ordered_filter_gross9_novelty_2026-08-14.json")
GROSS9_SHA = "b765a3152d1361de2a1effb14b5ca4958fbadb1e67287bdea25874275591f110"
...
CLOCKS = {
    "HVRSSR-8__FILTERED_BY__HVTCCR-8": {
        "path": Path("data/high_volatility_state_ordered_filter_clocks_2023_2026/HVRSSR-8__FILTERED_BY__HVTCCR-8.csv.gz"),
        "sha256": "7bc2d40ee74a82c04e862a523d3e0139b43808d003b0ee00c5d16e0fbb7f3663",
        "rows": 63,
    },
    "HVRSSR-8__FILTERED_BY__HVLZC-8": {
        "path": Path("data/high_volatility_state_ordered_filter_clocks_2023_2026/HVRSSR-8__FILTERED_BY__HVLZC-8.csv.gz"),
        "sha256": "6dd088834f0ca59f09098ea723fbe1e2d3ba6866c9056ed2c09279280d0f54ef",
        "rows": 116,
    },
    "HVSVF-8__FILTERED_BY__HVLZC-8": {
        "path": Path("data/high_volatility_state_ordered_filter_clocks_2023_2026/HVSVF-8__FILTERED_BY__HVLZC-8.csv.gz"),
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_state_ordered_filter_{stage}_economics_2026-08-14.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVSOF-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVSOF-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVSOF-8 {label} hash drift")
    for candidate, binding in CLOCKS.items():
        if sha256(binding["path"]) != binding["sha256"]:
            raise RuntimeError(f"HVSOF-8 eligible clock hash drift: {candidate}")
    ...
    if (
        ...
        raise RuntimeError("HVSOF-8 preregistered economics drift")
    candidates = support.get("candidates", {})
    if (
        support.get("policy_id") != POLICY_ID
        or support.get("eligible_candidates_for_combination_gross9") != list(ELIGIBLE_CANDIDATES)
        or support.get("advance_to_combination_gross9") is not True
        or support.get("advance_to_economic_outcomes") is not False
    ):
        raise RuntimeError("HVSOF-8 source-supported candidate drift")
    for candidate, binding in CLOCKS.items():
        clock = candidates.get(candidate, {}).get("clock", {})
        expected_clock = {
        ...
        if (
            ...
            or clock != expected_clock
        ):
            raise RuntimeError(f"HVSOF-8 source-supported candidate drift: {candidate}")
    gross9_results = gross9.get("candidate_results", {})
    if (
        gross9.get("policy_id") != POLICY_ID
        or gross9.get("eligible_candidates_for_economics") != list(ELIGIBLE_CANDIDATES)
        or gross9.get("all_source_eligible_candidates_passed") is not True
        or gross9.get("advance_to_economic_outcomes") is not True
        or gross9.get("evidence_boundary", {}).get("outcomes_opened") is not False
    ):
        raise RuntimeError("HVSOF-8 Gross9 did not authorize economics")
    for candidate in ELIGIBLE_CANDIDATES:
        ...
        if (
            ...
            raise RuntimeError(f"HVSOF-8 Gross9 candidate drift: {candidate}")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVSOF-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVSOF-8 predecessor did not pass with the frozen winner")
    ...
    if earlier_stage is None:
        if report.get("predecessor") is not None or report.get("selection", {}).get("raw_train_rank_one") != winner:
            raise RuntimeError("HVSOF-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link or earlier.get("frozen_train_winner") != winner:
            raise RuntimeError("HVSOF-8 predecessor chain drift")
    return report
...
def economic_checks(stage: str, primary: Mapping[str, Any]) -> dict[str, bool]:
    ...
    if stage == "train":
        checks["train_bonferroni_raw_weekly_p_max_0_1_over_6"] = weekly_p <= TRAIN_BONFERRONI_RAW_P_MAX
    return checks
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    if stage == "train":
        candidates_opened = ELIGIBLE_CANDIDATES
    else:
        candidates_opened = (predecessor_report["frozen_train_winner"],)
    ...
    core = {
        "protocol_version": "hvsof_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": winner,
        "stage": stage,
