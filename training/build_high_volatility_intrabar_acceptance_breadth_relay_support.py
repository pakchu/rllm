"""Source-only support gate for frozen HVIABR-8."""
...
from training import preregister_high_volatility_intrabar_acceptance_breadth_relay as prereg
...
PREREG_SHA = "619674be60397f0004ad79b089c8599db59553fbdbd3499cdbc9e3fca502c7ef"
SPLITS = {
...
CONTROLS = (
    "no_acceptance_tail",
    "no_variation_gate",
    "raw_absolute_acceptance_above_quarter",
    "one_block_stale_acceptance",
    "direction_flip",
    "forced_long",
)
ROOT = Path("data/high_volatility_intrabar_acceptance_breadth_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_intrabar_acceptance_breadth_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_intrabar_acceptance_breadth_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_intrabar_acceptance_breadth_relay_support_2026-08-10.json")
...
def acceptance_breadth(window: pd.DataFrame) -> float:
    ranges = window["high"].to_numpy(float) - window["low"].to_numpy(float)
    valid = ranges > 0
    if np.count_nonzero(valid) < 450:
        return math.nan
    locations = (window["close"].to_numpy(float)[valid] - window["low"].to_numpy(float)[valid]) / ranges[valid]
    upper = np.count_nonzero(locations > 0.5)
    lower = np.count_nonzero(locations < 0.5)
    value = (upper - lower) / len(locations)
    return float(value) if value != 0 and math.isfinite(value) else math.nan
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVIABR source timestamps")
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        if valid:
            ...
            acceptance = acceptance_breadth(window)
            block_return = float(np.log(window["close"].iloc[-1] / window["open"].iloc[0]))
            valid = variation > 0 and math.isfinite(acceptance)
        if not valid:
            variation = acceptance = block_return = math.nan
        rows.append(
            {
                "decision_time": decision,
                "source_valid": valid,
                "realized_variation": variation,
                "acceptance_breadth": acceptance,
                "block_return": block_return,
            }
        )
    states = pd.DataFrame(rows)
    states["variation_rank"] = prior_rank(states["realized_variation"].where(states["source_valid"]))
    states["acceptance_strength_rank"] = prior_rank(states["acceptance_breadth"].abs().where(states["source_valid"]))
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hviabr_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    acceptance_rank = states["acceptance_strength_rank"]
    acceptance = states["acceptance_breadth"]
    if control == "one_block_stale_acceptance":
        block_return, variation_rank, acceptance_rank, acceptance = [
            value.shift(1)
            for value in (block_return, variation_rank, acceptance_rank, acceptance)
        ]
    variation_gate = (
    ...
    if control == "no_acceptance_tail":
        acceptance_gate = pd.Series(True, index=states.index)
    elif control == "raw_absolute_acceptance_above_quarter":
        acceptance_gate = acceptance.abs().gt(0.25)
    else:
        acceptance_gate = acceptance_rank.ge(0.70)
    agreement = (
        block_return.ne(0)
        & acceptance.ne(0)
        & np.sign(block_return).eq(np.sign(acceptance))
    )
    eligible = states["source_valid"] & variation_gate & acceptance_gate & agreement
        ...
        rows.append(
            {
                ...
                "acceptance_breadth": float(states.at[index, "acceptance_breadth"]),
                "acceptance_strength_rank": float(states.at[index, "acceptance_strength_rank"]),
            }
        )
    columns = [
        "candidate", "control", "split", "decision_time", "feature_available_time",
        "entry_time", "exit_time", "side", "realized_variation", "variation_rank",
        "acceptance_breadth", "acceptance_strength_rank",
    ]
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVIABR preregistration drift")
    ...
    core = {
        "protocol_version": "hviabr_8_source_support_v1",
        "policy_id": "HVIABR-8",
        "preregistration": {
