"""Evaluate frozen HVWADX-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVWADX-24"
PROTOCOL = "hvadx_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_wilder_adx_trend_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVWADX predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVWADX novelty limits drift")
    if (
        ...
        raise RuntimeError("HVWADX predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVWADX primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvadx_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVWADX-24 unchanged before economic outcomes",
    }
