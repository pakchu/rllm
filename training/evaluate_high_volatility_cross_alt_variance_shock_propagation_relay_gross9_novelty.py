"""Evaluate frozen HVCAVS-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCAVS-8"
PROTOCOL = "hvcavs_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_cross_alt_variance_shock_propagation_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCAVS predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCAVS novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCAVS predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCAVS primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCAVS-8 unchanged before economic outcomes",
    }
