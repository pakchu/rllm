"""Evaluate frozen HVCHO-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCHO-24"
PROTOCOL = "hvcho_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_chaikin_ad_oscillator_zero_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCHO predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCHO novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCHO predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCHO primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcho_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCHO-24 unchanged before economic outcomes",
    }
