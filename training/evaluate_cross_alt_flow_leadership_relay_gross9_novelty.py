"""Evaluate frozen CAFLR-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "CAFLR-6"
PROTOCOL = "caflr_6_gross9_novelty_v1"
PREREG = Path("results/cross_alt_flow_leadership_relay_preregistration_2026-08-09.json")
PREREG_SHA = "cac7461ba22bbd5571d315b8186823aca935c26ea67856c9c24dd540317732dd"
SUPPORT = Path("results/cross_alt_flow_leadership_relay_support_2026-08-09.json")
SUPPORT_SHA = "2c50171cd9c853b2994d2b9ea4fe561efb933dcc00ebf9d0079b76f40ba1968a"
CLOCK = Path("data/cross_alt_flow_leadership_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "e0ca97ddf708614186f708bb06dece2dc6adf94272c0227092e6734368e9e703"
OUTPUT = Path("results/cross_alt_flow_leadership_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CAFLR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("CAFLR novelty limits drift")
    if (
        ...
        raise RuntimeError("CAFLR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CAFLR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "caflr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject CAFLR-6 unchanged before economic outcomes",
    }
