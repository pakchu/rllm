"""Sequential strict economics for frozen HVRSH-8."""
...
PREREG = Path("results/high_volatility_rescaled_range_persistence_relay_preregistration_2026-08-10.json")
...
NOVELTY = Path("results/high_volatility_rescaled_range_persistence_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "2ebfd8b152aee58e7ed505f60b379fdcfd36046210ae095fb0031ba0efedbf80"
...
FREEZE = Path("results/high_volatility_rescaled_range_persistence_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_rescaled_range_persistence_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_hurst_tail", "no_variation_gate", "raw_hurst_above_half", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVRSH frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVRSH novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrsh_8_sequential_economics_v1", "policy_id": POLICY_ID,
