"""Evaluate the frozen HVCPOFR-8 action-vote clock against Gross9."""
...
POLICY = "HVCPOFR-8"
PROTOCOL = "hvcpofr_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_perpetual_flow_inventory_relay_clocks_2020_2026.csv.gz",
    "sha256": "da2c8d095217575546d9629186e427597c91ca564b14f466643b2f5f88c2a9fe",
    "rows": 209,
}
OUTPUT = Path("results/high_volatility_causal_perpetual_flow_inventory_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCPOFR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCPOFR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCPOFR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCPOFR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCPOFR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCPOFR-8 unchanged before economics",
    }
