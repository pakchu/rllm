"""Build outcome-blind source support for preregistered CVOBR-6."""
...
from training import preregister_cboe_vix_overnight_btc_confirmation_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.build_cboe_volatility_surface_regime_crossing_relay_support import strict_prior_midrank

CLOCK = Path("data/cboe_vix_overnight_btc_confirmation_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/cboe_vix_overnight_btc_confirmation_relay_controls_2023_2026")
RESULT = Path("results/cboe_vix_overnight_btc_confirmation_relay_support_2026-08-08.json")
SPLITS = {
...
CONTROLS = ("no_extreme_gate", "vix_only_risk_direction", "btc_only_overnight", "confirmation_break", "direction_flip")
...
COLUMNS = (
    ...
    "side", "delta_log_vix", "absolute_vix_change_rank", "overnight_btc_return",
)
...
def load_features() -> pd.DataFrame:
    for raw, expected in prereg.HASHES.items():
        if sha(Path(raw)) != expected:
            raise RuntimeError(f"CVOBR frozen source changed: {raw}")
    ...
    if not cboe.observation_date.is_monotonic_increasing or cboe.observation_date.duplicated().any():
        raise RuntimeError("CVOBR Cboe dates invalid")
    if not np.isfinite(cboe.VIX_close).all() or not cboe.VIX_close.gt(0).all():
        raise RuntimeError("CVOBR VIX values invalid")
    cboe["delta_log_vix"] = np.log(cboe.VIX_close).diff()
    cboe["absolute_vix_change_rank"] = strict_prior_midrank(cboe.delta_log_vix.abs())
    ...
    if price.columns.tolist() != ["hour_start", "hour_open", "first_half_close", "second_half_open", "hour_close", "source_rows", "distinct_timestamps", "source_valid", "decision_time"]:
        raise RuntimeError("CVOBR completed-price schema changed")
    ...
    if price.hour_start.duplicated().any() or not price.hour_start.is_monotonic_increasing:
        raise RuntimeError("CVOBR completed-price clock invalid")
    ...
    for index in range(1, len(cboe) - 1):
        ...
        rows.append({
            ...
            "absolute_vix_change_rank": float(cboe.at[index, "absolute_vix_change_rank"]),
...
def signal(frame: pd.DataFrame, control: str) -> pd.Series:
    dv, btc, rank = frame.delta_log_vix, frame.overnight_btc_return, frame.absolute_vix_change_rank
    ...
    if control == "vix_only_risk_direction":
        eligible = frame.valid & dv.ne(0) & rank.ge(0.75)
        side = -np.sign(dv).astype("Int64").fillna(0).astype(int)
    elif control == "btc_only_overnight":
        eligible = frame.valid & btc.ne(0)
    elif control == "confirmation_break":
        eligible = valid & rank.ge(0.75) & ~confirmed
    else:
        eligible = valid & confirmed
        if control != "no_extreme_gate":
            eligible &= rank.ge(0.75)
...
def build_clock(frame: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in frame.index[sides.ne(0)]:
        ...
        rows.append({
            "candidate": "CVOBR-6", "control": control, "split": split,
            ...
            "absolute_vix_change_rank": float(frame.at[index, "absolute_vix_change_rank"]),
