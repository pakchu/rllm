"""Evaluate the frozen HVOILTJ-8 action-vote clock against Gross9."""
...
POLICY = "HVOILTJ-8"
PROTOCOL = "hvoiltj_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_oi_large_ticket_joint_sponsorship_clocks_2023_2026.csv.gz",
    "sha256": "1f3b8a6bd8127930da5c6123845b6a3f1a68ae86e76dcccdb9d01e0d1380a876",
    "rows": 104,
}
OUTPUT = Path("results/high_volatility_oi_large_ticket_joint_sponsorship_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVOILTJ-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVOILTJ-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVOILTJ-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVOILTJ-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVOILTJ-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVOILTJ-8 unchanged before economics",
    }
