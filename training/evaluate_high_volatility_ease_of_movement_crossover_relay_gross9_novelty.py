"""Evaluate frozen HVEOM-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVEOM-24"
PROTOCOL = "hveom_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_ease_of_movement_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVEOM predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVEOM novelty limits drift")
    if (
        ...
        raise RuntimeError("HVEOM predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVEOM primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hveom_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVEOM-24 unchanged before economic outcomes",
    }
