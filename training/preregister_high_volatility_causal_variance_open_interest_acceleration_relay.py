"""Outcome-blind preregistration for HVCVAROIAR-8."""
...
POLICY_ID = "HVCVAROIAR-8"
DEFAULT_OUTPUT = Path("results/high_volatility_causal_variance_open_interest_acceleration_relay_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_causal_variance_open_interest_acceleration_relay_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            ...
            "why_distinct": "HVCVAROIAR uses within-cycle acceleration in realized variance, not turnover or transaction count alone, plus exact OI-growth acceleration and variation. It reuses neither prior event set nor control and uses no taker-flow sign, spot source, ETH, funding, premium-index, fitted outcome, or promoted control.",
            "why_suited_to_volatile_regimes": "the completed eight-hour BTC realized variation must be in its causal upper 40 percent",
            "why_low_gross9_overlap_is_plausible": "fixed eight-hour variance-acceleration plus accelerating-OI states are absent from Gross9 primitives",
        },
        "features": {
            "decision": "exact 00:00, 08:00, or 16:00 UTC boundary D",
            "open_interest": "finite positive BTCUSDT period=5m sum_open_interest observations at D-16h, D-8h, and D, each exchange-timestamped no later than D",
            "prior_oi_growth": "log(OI[D-8h]/OI[D-16h]), finite",
            "current_oi_growth": "log(OI[D]/OI[D-8h]), finite strict positive",
            "oi_growth_acceleration": "current_oi_growth-prior_oi_growth, finite strict positive",
            ...
            "eligible": "exact coherent completed perpetual cycle, positive variance acceleration, nonzero completed return, three exact OI endpoints with strict positive current growth and growth acceleration, and both variance-acceleration and variation ranks pass at D; no taker-flow sign, spot source, funding, range, premium-index, alt, or post-decision condition",
            "no_imputation": True,
        },
        "clock": {
        ...
        "source_plan": {
            "open_interest": "Postgres open_interest_binance BTCUSDT period=5m exact 8h-grid endpoint observations",
        ...
        "stopping_rule": "terminal first failure; no OI endpoint, growth, or acceleration rule, variance-acceleration formula, acceleration or variation history, side, clock, hold, subset, threshold, comparator, or control repair",
    }
