"""Sequential strict economics for frozen RVSBR-12."""
...
PREREG = Path("results/russell_volatility_spread_break_relay_preregistration_2026-08-09.json")
PREREG_SHA = "20dad547af804c1f2ff662f98ac1f446fa8e859eb22aa8934f458d35f8315368"
SUPPORT = Path("results/russell_volatility_spread_break_relay_support_2026-08-09.json")
SUPPORT_SHA = "a1bd3c026b199f400b2a60792084fd64fc83e3bd3d481a8c959677c6ac556b9d"
NOVELTY = Path("results/russell_volatility_spread_break_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "5f00ab6ad902463e3396dcc4dd2521c778232eff5bbcf1f885de3b68f144965d"
CLOCK = Path("data/russell_volatility_spread_break_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d29b3f56c1f34a1486196896c2f5b4efe735c5f98e067335d92ff345d5504ae3"
CONTROL_DIR = Path("data/russell_volatility_spread_break_relay_controls_2023_2026")
FREEZE = Path("results/russell_volatility_spread_break_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/russell_volatility_spread_break_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("RVSBR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("RVSBR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "rvsbr_12_sequential_economics_v1", "policy_id": POLICY_ID,
