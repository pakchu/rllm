"""Sequential strict economics for frozen HVLTTC-8."""
...
PREREG = Path("results/high_volatility_large_ticket_temporal_clustering_relay_preregistration_2026-08-13.json")
PREREG_SHA = "f5da0987ff2d7f8ec0081c3eff806c0b46c44ebd93fb06ec4497163c4b7565f1"
SUPPORT = Path("results/high_volatility_large_ticket_temporal_clustering_relay_support_2026-08-13.json")
SUPPORT_SHA = "52271ac02bfef0429155947ea5c05a67aff47cdac9ebb3727015203a537300ce"
NOVELTY = Path("results/high_volatility_large_ticket_temporal_clustering_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "d7701615cd82ddeae675a3bec63590ba3ab2402157476d691e8994e0ffae288e"
CLOCK = Path("data/high_volatility_large_ticket_temporal_clustering_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "7529b389ab5a34a361f16b93b68bd865bd5d430532c3fccea2fc344bd79239d5"
CONTROL_DIR = Path("data/high_volatility_large_ticket_temporal_clustering_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_large_ticket_temporal_clustering_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_large_ticket_temporal_clustering_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_clustering_tail", "no_variation_gate", "turnover_hhi_only", "one_hour_stale_clustering", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVLTTC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVLTTC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvlttc_8_sequential_economics_v1", "policy_id": POLICY_ID,
