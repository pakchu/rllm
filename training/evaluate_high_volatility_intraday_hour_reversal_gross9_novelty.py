"""Evaluate frozen HVIHR-1 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVIHR-1"
PROTOCOL = "hvihr_1_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_intraday_hour_reversal_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVIHR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVIHR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVIHR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVIHR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVIHR-1 unchanged before economic outcomes",
    }
