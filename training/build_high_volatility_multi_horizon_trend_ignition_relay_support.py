"""Outcome-blind source-support gate for frozen HVMTI-48."""
...
from training import (
    preregister_high_volatility_multi_horizon_trend_ignition_relay as prereg,
)
from training.build_binance_aggtrade_microstructure import _write_gzip_csv


ENV_FILE = "/home/pakchu/rllm/.env"
PREREG_SHA = "bd37786c69fad9a55fdfcc40a65a1c702a17504e489a17d8370f6f7c0f8f931d"
...
"""
SOURCE_DIR = Path(
    "data/high_volatility_multi_horizon_trend_ignition_relay_sources_2023_2026"
)
FEATURES = SOURCE_DIR / "daily_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path(
    "data/high_volatility_multi_horizon_trend_ignition_relay_clocks_2023_2026.csv.gz"
)
CONTROL_DIR = Path(
    "data/high_volatility_multi_horizon_trend_ignition_relay_controls_2023_2026"
)
RESULT = Path(
    "results/high_volatility_multi_horizon_trend_ignition_relay_support_2026-08-13.json"
)
FEATURE_COLUMNS = (
...
def conditions(frame: pd.DataFrame, control: str="primary") -> tuple[pd.Series,pd.Series,pd.DataFrame]:
    ...
    consensus=pd.Series(True,index=frame.index) if control=="no_trend_consensus" else np.sign(used.return_5d)*np.sign(used.return_20d)>0
    high=valid&used.variation_rank.ge(.65)&consensus
    active=high if control=="no_variation_onset" else high&used.variation_rank.shift(1).lt(.65)&valid.shift(1,fill_value=False)
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVMTI preregistration drift")
    ...
    source_core = {
        "protocol_version": "hvmti_48_sources_v1",
    ...
    core = {
        "protocol_version": "hvmti_48_source_support_v1",
