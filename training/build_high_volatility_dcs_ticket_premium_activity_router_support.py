"""Deterministic outcome-blind source support for HVDCSATPAPAR-8."""
...
from training import preregister_high_volatility_dcs_ticket_premium_activity_router as prereg

START=pd.Timestamp("2023-01-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="53e80657bf35a2ef2bd0ad849336a2cff7a8fe0ee391974ea02d67a65dbc8c80";REG=prereg.build()
...
ROOT=Path("data/high_volatility_dcs_ticket_premium_activity_router_sources_2023_2026");PANEL=ROOT/"cycle_states.csv.gz";MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_dcs_ticket_premium_activity_router_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_dcs_ticket_premium_activity_router_split_clocks_2023_2026")
RESULT=Path("results/high_volatility_dcs_ticket_premium_activity_router_support_2026-08-16.json");BUILDER=Path(__file__).relative_to(Path.cwd())
BASE_CLOCK=Path("data/high_volatility_directional_change_scarcity_ticket_participation_acceleration_relay_clocks_2023_2026.csv.gz")
...
def build_panel(raw):
 ...
 if p.columns.tolist()!=ep or b.columns.tolist()!=eb:raise RuntimeError("HVDCSATPAPAR-8 schema drift")
 for f in (p,b):
...
def build_clock(p):
 ...
 for b in base.itertuples(index=False):
  ...
  side=int(b.side if r.relative_activity_rank<=.50 else -b.side)
  if side not in (-1,1) or r.feature_available_time>b.entry_time:raise RuntimeError("HVDCSATPAPAR-8 side/availability drift")
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVDCSATPAPAR-8 preregistration drift")
