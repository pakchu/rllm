"""Evaluate the frozen HVCATFIR-8 action-vote clock against Gross9."""
...
POLICY = "HVCATFIR-8"
PROTOCOL = "hvcatfir_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_average_ticket_flow_inventory_relay_clocks_2020_2026.csv.gz",
    "sha256": "c0ba875faa89ced9ce00a241e0ac797cf8ea7ad97287c8b2c03dbbc7c20fc730",
    "rows": 114,
}
OUTPUT = Path("results/high_volatility_causal_average_ticket_flow_inventory_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCATFIR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCATFIR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCATFIR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCATFIR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCATFIR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCATFIR-8 unchanged before economics",
    }
