"""Sequential strict economics for frozen HVCE-24."""
...
PREREG = Path("results/high_volatility_chandelier_exit_state_flip_relay_preregistration_2026-08-11.json")
PREREG_SHA = "0e429bb9a97bd213d519b28be9faf8fdac73a7b5f41dd97860a21a5323e3abd4"
SUPPORT = Path("results/high_volatility_chandelier_exit_state_flip_relay_support_2026-08-11.json")
SUPPORT_SHA = "d2ef9d4db1bf38d4a48813352d156d7c36a3b8d4c02258290b78713aacfe6cdc"
NOVELTY = Path("results/high_volatility_chandelier_exit_state_flip_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "d447b2763d0a31218a85d03ce4e6dfab4537e2770e2f8329abc2bdf3b94e7388"
CLOCK = Path("data/high_volatility_chandelier_exit_state_flip_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "2c90230d9819607098ca278fb67bd90a6db780a67a81fed5c9b0cb57934621f4"
CONTROL_DIR = Path("data/high_volatility_chandelier_exit_state_flip_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_chandelier_exit_state_flip_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_chandelier_exit_state_flip_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "untrailed_raw_stop_flip", "one_bar_stale_flip", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCE frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCE novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvce_24_sequential_economics_v1", "policy_id": POLICY_ID,
