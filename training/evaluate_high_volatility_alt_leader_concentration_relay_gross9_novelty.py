"""Evaluate frozen HVALCR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVALCR-8"
PROTOCOL = "hvalcr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_alt_leader_concentration_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVALCR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVALCR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVALCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVALCR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvalcr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVALCR-8 unchanged before economic outcomes",
    }
