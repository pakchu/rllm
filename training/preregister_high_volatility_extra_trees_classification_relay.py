"""Outcome-sequenced preregistration for HVETCR-48."""
...
DEFAULT_OUTPUT = Path(
    "results/high_volatility_extra_trees_classification_relay_preregistration_2026-08-09.json"
)
SOURCE_BINDINGS = {
...
def build() -> dict[str, Any]:
    core = {
        "protocol_version": "high_volatility_extra_trees_classification_relay_v1",
        "policy_id": "HVETCR-48",
        ...
        "mechanism": {
            "claim": (
                "A nonlinear classifier fitted before 2023 can separate positive from "
                "negative 48-hour BTC paths using causal price, derivatives, macro, and "
                "range-state features. Trading only probability tails during high range "
                "volatility targets persistent volatile directional regimes."
            ),
            "side": "long in calibrated upper probability tail; short in lower tail",
            "why_distinct": (
                "HVETCR uses a broad completed-5m feature state, nonlinear ExtraTrees "
                "classification, and a 48-hour horizon. HVMRR used an eight-hour raw "
                "microstructure block, linear ridge regression, and six-hour outcomes. "
                "No HVMRR feature, fitted artifact, threshold, control, or OOS result is reused."
            ),
        ...
        "training_contract": {
            ...
            "label": "1 iff log(open at anchor+1+576/open at anchor+1)>0, else 0",
            ...
            "probability_thresholds": (
                "strict empirical 20th and 80th percentiles of positive-class "
                "probability on calibration anchors only"
            ),
        ...
        "oos_clock": {
            ...
            "eligibility": (
                ...
                ">= frozen q65; probability <= frozen q20 or >= frozen q80"
            ),
            "entry": "anchor completed bar plus one 5m open",
            "side": "probability>=q80 long; probability<=q20 short; conflict impossible",
        ...
        "diagnostic_controls": {
            "names": ["no_volatility_gate", "no_probability_tail_gate", "one_anchor_stale_features", "direction_flip"],
        ...
        "research_boundary": {
            "prior_extra_trees_long_family_outcomes_known": True,
            "exact_classifier_and_bidirectional_high_volatility_rule_outcomes_known": False,
...
def validate(payload: dict[str, Any]) -> None:
    ...
    if payload.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError("HVETCR preregistration hash mismatch")
    for raw, expected in SOURCE_BINDINGS.items():
        if hashlib.sha256(Path(raw).read_bytes()).hexdigest() != expected:
            raise RuntimeError(f"HVETCR source drift: {raw}")
