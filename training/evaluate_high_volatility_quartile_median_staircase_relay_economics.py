"""Sequential strict economics for frozen HVQMSR-8."""
...
PREREG = Path("results/high_volatility_quartile_median_staircase_relay_preregistration_2026-08-10.json")
PREREG_SHA = "d1fac61ae887f9b1c63979aef242488f1eca23dcb7a6777ab94ef7b39781c94c"
SUPPORT = Path("results/high_volatility_quartile_median_staircase_relay_support_2026-08-10.json")
SUPPORT_SHA = "8d120b95b2a41ecd228994a25ff762b5d946c4f0c2e3f7cbcee8862a28fe598c"
NOVELTY = Path("results/high_volatility_quartile_median_staircase_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "51b9edb46eaeb4a31d473149060f791c69147924ed2d16064007a59cfd1df4ed"
CLOCK = Path("data/high_volatility_quartile_median_staircase_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "c868691c01011cee7293e3d8cf3ddeb3aa5a9fa2ba18984a210038a7d0912161"
CONTROL_DIR = Path("data/high_volatility_quartile_median_staircase_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_quartile_median_staircase_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_quartile_median_staircase_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_migration_tail", "no_variation_gate", "endpoint_displacement", "one_block_stale_staircase", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVQMSR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVQMSR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvqmsr_8_sequential_economics_v1", "policy_id": POLICY_ID,
