"""Evaluate frozen HVESO-24 structural novelty against every Gross9 sleeve."""
...
from training import preregister_high_volatility_ehlers_synthetic_oscillator_turn_relay as registration

POLICY = "HVESO-24"
PROTOCOL = "hveso_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_ehlers_synthetic_oscillator_turn_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVESO predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVESO novelty limits drift")
    if (
        ...
        raise RuntimeError("HVESO predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVESO primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hveso_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVESO-24 unchanged before economic outcomes",
    }
