"""Sequential strict economics for frozen HVETF-24."""
...
PREREG = Path("results/high_volatility_ehlers_trendflex_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "e6bb105a92bbc53b96ef52a618c5708a687464c0cb28a1d15541565594ed9077"
SUPPORT = Path("results/high_volatility_ehlers_trendflex_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "f579ce3cefaa4d84f15c48ee7cf2a797c3bb00678f94b651e0b0ba6ec5301303"
NOVELTY = Path("results/high_volatility_ehlers_trendflex_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "8e6afc79b589cc21e143d2144ca0f3d8b5ecb8a693a70581da45997be0825e83"
CLOCK = Path("data/high_volatility_ehlers_trendflex_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "23902b02b69a8867d99cd391115b20190e89c25ad22945a3b14ee115b8af464f"
CONTROL_DIR = Path("data/high_volatility_ehlers_trendflex_crossover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_ehlers_trendflex_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ehlers_trendflex_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "fast_trendflex_zero_cross", "slow_trendflex_zero_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVETF frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVETF novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvetf_24_sequential_economics_v1", "policy_id": POLICY_ID,
