"""Build source-only HVCPFR-72 clocks before Gross9 or economics."""
...
from training import preregister_high_volatility_commercial_paper_funding_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.build_scheduled_trend_concordance_relay_support import load_market

PREREG_SHA = "fd2b4cc7d48bda12489df9e8f6ed2350050129327bfc36e7734dc1335144fda9"
SOURCE_SHA = "a30bbbadf7c78e4e10d5a1acc38244ab27a85dee9492a6bb3533fa15fc96f54a"
SOURCE_MANIFEST = Path("data/commercial_paper_alfred_causal_vintages_2020_2026_manifest.json")
SOURCE_MANIFEST_SHA = "dd106700a30bf003c71be8dabcdea5832ddc2262bcfab8bc27a6ba97378aa773"
...
STATE = Path("data/high_volatility_commercial_paper_funding_relay_sources_2023_2026/weekly_states.csv.gz")
CLOCK = Path("data/high_volatility_commercial_paper_funding_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_commercial_paper_funding_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_commercial_paper_funding_relay_support_2026-08-11.json")
SPLITS = {
...
CONTROLS = (
    "no_variation_gate",
    "one_release_stale_stock",
    "current_first_print_stock",
    "direction_flip",
...
def score_states(source: pd.DataFrame, market: pd.DataFrame) -> pd.DataFrame:
    ...
    states["compout_first"] = pd.to_numeric(states.compout_first, errors="coerce")
    states["compout_revised"] = pd.to_numeric(states.compout_revised, errors="coerce")
    states = states.sort_values("reference_date").reset_index(drop=True)
    if states.reference_date.duplicated().any():
        raise RuntimeError("HVCPFR duplicate reference date")
    indexed = states.set_index("reference_date")
    prior_dates = states.reference_date - pd.Timedelta(days=28)
    states["stock_change"] = states.compout_revised - indexed.compout_revised.reindex(prior_dates).to_numpy()
    states["stock_first_change"] = states.compout_first - indexed.compout_first.reindex(prior_dates).to_numpy()
    states["decision_time"] = (
        states.revised_vintage_date.dt.normalize()
    ...
    squared_returns = np.log(close / open_).pow(2)
    variation = np.sqrt(squared_returns.rolling(2016, min_periods=2016).sum())
    complete = valid.rolling(2016, min_periods=2016).sum().eq(2016) & contiguous.rolling(2016, min_periods=2016).sum().eq(2016)
    lookup = pd.DataFrame(
    ...
    states["state_valid"] = (
        np.isfinite(states[["compout_revised", "stock_change", "btc_variation"]]).all(axis=1)
        & states.stock_change.ne(0)
    )
...
def build_clock(states: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    if control == "one_release_stale_stock":
        ...
        used = states
    change_column = "stock_first_change" if control == "current_first_print_stock" else "stock_change"
        ...
        rows.append(
            {
                ...
                "stock_change": float(source[change_column]),
                "stock_first_change": float(source.stock_first_change),
...
def run() -> dict[str, Any]:
    ...
    for path, expected in bindings.items():
        if sha(path) != expected:
            raise RuntimeError(f"HVCPFR binding drift: {path}")
    ...
    core = {
        "protocol_version": "hvcpfr_72_source_support_v1",
