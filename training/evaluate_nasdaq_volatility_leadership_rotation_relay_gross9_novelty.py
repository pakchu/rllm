"""Evaluate frozen NVLRR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "NVLRR-12"
PROTOCOL = "nvlrr_12_gross9_novelty_v1"
...
CLOCK_ROWS = 99
OUTPUT = Path("results/nasdaq_volatility_leadership_rotation_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def write_immutable(path: Path, content: bytes) -> None:
    ...
    if path.exists():
        if path.read_bytes() != content:
            raise RuntimeError(f"refusing to overwrite immutable NVLRR novelty result: {path}")
        return
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("NVLRR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("NVLRR novelty limits drift")
    if (
        ...
        raise RuntimeError("NVLRR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="NVLRR primary")
    if len(candidate) != CLOCK_ROWS:
        raise RuntimeError("NVLRR clock count drift")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "nvlrr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject NVLRR-12 unchanged before economic outcomes",
    }
