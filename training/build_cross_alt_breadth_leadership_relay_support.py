"""Materialize outcome-blind source support for frozen CABLR-8."""
...
BUILDER = Path("training/build_cross_alt_breadth_leadership_relay_support.py")
PREREG_SHA = "1ba656a8e4833c0424887bc7821497a8251239af480e28d2a0d1715d07b70a27"
SOURCE_DIR = Path("data/cross_alt_breadth_leadership_relay_sources_2023_2026")
FEATURE_PANEL = SOURCE_DIR / "cross_alt_breadth_leadership_relay_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/cross_alt_breadth_leadership_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/cross_alt_breadth_leadership_relay_controls_2023_2026")
RESULT = Path("results/cross_alt_breadth_leadership_relay_support_2026-08-09.json")
...
CONTROLS = (
    "no_volatility_gate", "three_of_six_breadth", "btc_confirmation",
    "one_block_stale_geometry", "direction_flip",
)
...
CLOCK_COLUMNS = (
    ...
    "btc_return", "confirming_alts", "alt_majority_sign",
...
def strict_prior_midrank(values: pd.Series, lookback: int = 810, minimum: int = 540) -> pd.Series:
...
def load_bars(symbol: str) -> pd.DataFrame:
    ...
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError(f"CABLR {symbol} schema drift")
    frame["ts"] = pd.to_datetime(frame.ts, utc=True, errors="raise")
    frame = frame.sort_values("ts").reset_index(drop=True)
    if frame.empty or frame.ts.duplicated().any():
        raise RuntimeError(f"CABLR {symbol} empty or duplicated timestamps")
    ...
    if not np.isfinite(prices).all(axis=None) or not prices.gt(0).all(axis=None) or not coherent.all():
        raise RuntimeError(f"CABLR {symbol} contains invalid or incoherent OHLC")
...
def build_features(bars: dict[str, pd.DataFrame], lookback: int = 810, minimum: int = 540) -> pd.DataFrame:
    ...
    for start in pd.date_range(START, END, freq="8h", inclusive="left"):
        ...
        alt_signs = [int(np.sign(returns[symbol])) for symbol in ALT_SYMBOLS]
        positive_alts = sum(sign == 1 for sign in alt_signs)
        negative_alts = sum(sign == -1 for sign in alt_signs)
        majority_sign = 1 if positive_alts > negative_alts else -1 if negative_alts > positive_alts else 0
        confirming = max(positive_alts, negative_alts)
        rows.append({
            ...
            **{f"{symbol.lower()}_return": returns[symbol] for symbol in ALT_SYMBOLS},
            ...
            "alt_majority_sign": float(majority_sign),
...
def conditions(frame: pd.DataFrame, control: str = "primary") -> tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    btc_relation = np.sign(used.btc_return).eq(majority_sign) if control == "btc_confirmation" else np.sign(used.btc_return).eq(-majority_sign)
    volatility = pd.Series(True, index=frame.index) if control == "no_volatility_gate" else frame.variation_rank.ge(0.65)
    active = breadth & used.btc_return.ne(0) & btc_relation & volatility
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active]:
        ...
        rows.append({
            "candidate": "CABLR-8", "control": control, "split": split,
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("CABLR preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "cablr_8_sources_v1",
    ...
    core = {
        "protocol_version": "cablr_8_source_support_v1", "policy_id": "CABLR-8",
