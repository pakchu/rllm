"""Evaluate frozen HVDOER-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDOER-12"
PROTOCOL = "hvdoer_12_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_daily_online_expert_relay_preregistration_2026-08-09.json"
)
PREREG_SHA = "7ae5faf4a15d91036cb56375aabf775e7c621600468bd5208a348317239ff0a1"
SUPPORT = Path("results/high_volatility_daily_online_expert_relay_support_2026-08-09.json")
SUPPORT_SHA = "1e614c5ffef60b1d2fd523e6efa03ff066db58b68cbc3fb1f53d1c21da958bdd"
CLOCK = Path("data/high_volatility_daily_online_expert_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "3baaba02f9671b47f5d4f8535a465272997da9234bb1266a12d4f57b499b622d"
OUTPUT = Path(
    "results/high_volatility_daily_online_expert_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDOER predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVDOER novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDOER predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDOER primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject HVDOER-12 unchanged before economic outcomes"
        ),
    }
