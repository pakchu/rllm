"""Build outcome-blind source support for frozen HVWTO-C10-21-4-24."""
...
from training import preregister_high_volatility_wave_trend_oscillator_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "03f9d8596f39f97d0779cfbc5ba8aa2e17f7f38e5d81a713e8b5427bcfd5dc61"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_wave_trend_oscillator_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_wave_trend_oscillator_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_wave_trend_oscillator_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_wave_trend_oscillator_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_wave_trend_oscillator_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "typical_price", "channel_average", "channel_deviation",
    "channel_index", "wt1", "wt2", "entry_side", "zero_cross_side", "variation",
    "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "typical_price", "channel_average",
    "channel_deviation", "channel_index", "wt1", "wt2", "entry_side",
    "zero_cross_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVWTO source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVWTO invalid source key")
...
def wave_trend(high: pd.Series, low: pd.Series, close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    """Replicate LEAN WaveTrend(10,21,4), resetting across invalid source bars."""
    highs, lows, closes = (series.to_numpy(float) for series in (high, low, close))
    validity = valid.to_numpy(bool)
    count = len(close)
    names = ("typical_price", "channel_average", "channel_deviation", "channel_index", "wt1", "wt2")
    ...
    esa = LeanEma(P["channel_periods"])
    deviation = LeanEma(P["channel_periods"])
    average = LeanEma(P["average_periods"])
    signal: list[float] = []

    for index in range(count):
        if not validity[index]:
            esa.reset(); deviation.reset(); average.reset(); signal.clear()
            continue
        typical = (highs[index] + lows[index] + closes[index]) / 3
        values["typical_price"][index] = typical
        channel_average = esa.update(typical)
        if not math.isfinite(channel_average):
            continue
        values["channel_average"][index] = channel_average
        channel_deviation = deviation.update(abs(typical - channel_average))
        if not math.isfinite(channel_deviation):
            continue
        values["channel_deviation"][index] = channel_deviation
        denominator = P["normalization_constant"] * channel_deviation
        if not math.isfinite(denominator) or denominator <= 0:
            continue
        channel_index = (typical - channel_average) / denominator
        values["channel_index"][index] = channel_index
        wt1 = average.update(channel_index)
        if not math.isfinite(wt1):
            continue
        values["wt1"][index] = wt1
        signal.append(wt1)
        signal = signal[-P["signal_periods"] :]
        if len(signal) < P["signal_periods"]:
            continue
        wt2 = float(np.mean(signal))
        values["wt2"][index] = wt2
        prior_ready = index > 0 and math.isfinite(values["wt1"][index - 1]) and math.isfinite(values["wt2"][index - 1])
        prior_above = prior_ready and values["wt1"][index - 1] > values["wt2"][index - 1]
        prior_below = prior_ready and values["wt1"][index - 1] < values["wt2"][index - 1]
        entry_side[index] = 1 if wt1 > wt2 and not prior_above else -1 if wt1 < wt2 and not prior_below else 0
        prior_wt1 = index > 0 and math.isfinite(values["wt1"][index - 1])
        prior_positive = prior_wt1 and values["wt1"][index - 1] > 0
        prior_negative = prior_wt1 and values["wt1"][index - 1] < 0
        zero_cross_side[index] = 1 if wt1 > 0 and not prior_positive else -1 if wt1 < 0 and not prior_negative else 0
    return pd.DataFrame({**values, "entry_side": entry_side, "zero_cross_side": zero_cross_side}, index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=bars.join(wave_trend(bars.bar_high,bars.bar_low,bars.bar_close,bars.valid_bar));bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["typical_price","channel_average","channel_deviation","channel_index","wt1","wt2","variation"]]).all(axis=1)&bars.channel_deviation.gt(0)&bars.variation.gt(0)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVWTO prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvwto_c10_21_4_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvwto_c10_21_4_24_source_support_v1", "policy_id": prereg.POLICY_ID,
