"""Evaluate frozen HVVPCI-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVVPCI-24"
PROTOCOL = "hvvpci_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_volume_price_confirmation_zero_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVVPCI predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVVPCI novelty limits drift")
    if (
        ...
        raise RuntimeError("HVVPCI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVVPCI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvvpci_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVVPCI-24 unchanged before economic outcomes",
    }
