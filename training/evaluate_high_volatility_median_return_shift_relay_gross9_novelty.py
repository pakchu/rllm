"""Evaluate frozen HVMRSR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVMRSR-8"
PROTOCOL = "hvmrsr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_median_return_shift_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVMRSR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVMRSR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVMRSR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVMRSR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvmrsr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVMRSR-8 unchanged before economic outcomes",
    }
