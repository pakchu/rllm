"""Build source-only CLSIER-6 confirmation-ladder clocks."""
...
PREREG_SHA = "0e5baf2b0199a26fb680e1c5d21e4fa3d77abd40a6cf1d70e006115d2d6fc1ee"
...
CLOCK = Path("data/confirmation_ladder_settlement_impact_efficiency_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/confirmation_ladder_settlement_impact_efficiency_relay_controls_2023_2026")
RESULT = Path("results/confirmation_ladder_settlement_impact_efficiency_relay_support_2026-08-12.json")
...
FEATURE_COLUMNS = (
    ...
    *RETURN_COLUMNS, *WEIGHT_COLUMNS, "late_return", "late_unanimous", "early_impact", "late_impact",
    "impact_efficiency_escalation", "eligible_state",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "anchor_height", "confirmation_height",
    "feature_available_time", "entry_time", "exit_time", "side", "late_return",
    "late_unanimous", "early_impact", "late_impact", "impact_efficiency_escalation",
)
...
def validate_blocks(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CLSIER block schema drift")
...
def prepare_minutes(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CLSIER minute schema drift")
    ...
    if result.ts.duplicated().any():
        raise RuntimeError("CLSIER duplicate timestamps")
...
def build_features(blocks: pd.DataFrame, market: pd.DataFrame) -> pd.DataFrame:
    ...
    for index in range(0, len(blocks) - 6):
        ...
        impacts = [abs(value) / weight for value, weight in zip(returns, weights)] if valid else [np.nan] * 6
        early_impact = float(np.mean(impacts[:3])) if valid else np.nan
        late_impact = float(np.mean(impacts[3:])) if valid else np.nan
        impact_efficiency_escalation = bool(valid and late_impact > early_impact)
        rows.append({
            ...
            **{column: weights[position] for position, column in enumerate(WEIGHT_COLUMNS)},
            ...
            "early_impact": early_impact, "late_impact": late_impact,
            "impact_efficiency_escalation": impact_efficiency_escalation,
        })
...
def active_and_side(features: pd.DataFrame, control: str = "primary"):
    ...
    elif control == "impact_efficiency_only":
        state = features.source_valid & features.impact_efficiency_escalation
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active & sides.ne(0)]:
        ...
        rows.append({
            ...
            "early_impact": float(features.at[index, "early_impact"]), "late_impact": float(features.at[index, "late_impact"]),
            "impact_efficiency_escalation": bool(features.at[index, "impact_efficiency_escalation"]),
        })
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "clsier_6_source_support_v1", "policy_id": prereg.POLICY_ID,
