"""Source-only support gate for frozen HVFMI-8."""
...
from training import preregister_high_volatility_flow_mutual_information_relay as prereg
...
ROOT = Path("data/high_volatility_flow_mutual_information_sources_2023_2026")
...
def _empirical_ternary(values: np.ndarray) -> np.ndarray:
    values = np.asarray(values, dtype=float)
    order = np.argsort(values, kind="stable")
    ranks = np.empty(len(values), dtype=int)
    ranks[order] = np.arange(len(values), dtype=int)
    return np.minimum(2, (3 * ranks) // len(values))


def normalized_mutual_information(returns: np.ndarray, flow: np.ndarray) -> float:
    returns = np.asarray(returns, dtype=float)
    flow = np.asarray(flow, dtype=float)
    if len(returns) != len(flow) or len(returns) < 3 or not np.isfinite(returns).all() or not np.isfinite(flow).all():
        return math.nan
    left, right = _empirical_ternary(returns), _empirical_ternary(flow)
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        if valid:
            ...
            valid = variation > 0 and np.isfinite(flow).all() and math.isfinite(mutual_information)
        if not valid:
            variation = mutual_information = block_return = late_return = math.nan
        rows.append(
            {
    ...
    states["mutual_information_rank"] = prior_rank(states["normalized_mutual_information"].where(states["source_valid"]))
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvfmi_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    mutual_information_rank = states["mutual_information_rank"]
    mutual_information = states["normalized_mutual_information"]
    if control == "one_block_stale_geometry":
        block_return, late_return, variation_rank, mutual_information_rank, mutual_information = [
            value.shift(1)
            for value in (block_return, late_return, variation_rank, mutual_information_rank, mutual_information)
        ]
    variation_gate = (
    ...
    if control == "no_mutual_information_tail":
        mutual_information_gate = pd.Series(True, index=states.index)
    elif control == "raw_positive_mutual_information":
        mutual_information_gate = mutual_information.gt(0.0)
    else:
        mutual_information_gate = mutual_information_rank.ge(0.75)
    agreement = (
        block_return.ne(0)
        & late_return.ne(0)
        & np.sign(block_return).eq(np.sign(late_return))
    )
    eligible = states["source_valid"] & variation_gate & mutual_information_gate & agreement
        ...
        rows.append(
            {
                ...
                "normalized_mutual_information": float(states.at[index, "normalized_mutual_information"]),
                "mutual_information_rank": float(states.at[index, "mutual_information_rank"]),
            }
        )
    columns = [
        "candidate", "control", "split", "decision_time", "feature_available_time",
        "entry_time", "exit_time", "side", "realized_variation", "variation_rank",
        "normalized_mutual_information", "mutual_information_rank",
    ]
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVFMI preregistration drift")
    ...
    core = {
        "protocol_version": "hvfmi_8_source_support_v1",
        "policy_id": "HVFMI-8",
        "preregistration": {
