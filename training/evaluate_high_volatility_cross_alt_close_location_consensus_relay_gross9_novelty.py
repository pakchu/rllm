"""Evaluate frozen HVCACLR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCACLR-8"
PROTOCOL = "hvcaclr_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_cross_alt_close_location_consensus_relay_preregistration_2026-08-11.json")
PREREG_SHA = "f4739719fd044d14c09d89e4b8532bb660708db98053e563f97f198efacdd06c"
SUPPORT = Path("results/high_volatility_cross_alt_close_location_consensus_relay_support_2026-08-11.json")
SUPPORT_SHA = "a55abb4491fe1f21f4d6f146213adcc2949cb64867bf56387662619bcaafab3f"
CLOCK = Path("data/high_volatility_cross_alt_close_location_consensus_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "c439dcd8b0c96b30c978fc9c2d7caa1599c02e9c4786a94a73c11ba340ea65dc"
OUTPUT = Path("results/high_volatility_cross_alt_close_location_consensus_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCACLR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCACLR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCACLR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCACLR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcaclr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCACLR-8 unchanged before economic outcomes",
    }
