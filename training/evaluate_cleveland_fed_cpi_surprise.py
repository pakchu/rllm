"""Sequential strict evaluator for frozen CFCS-1.
...
SUPPORT_COMMIT = "f403cf8"
STATIC_INPUT_SHA256 = {
    "training/preregister_cleveland_fed_cpi_surprise.py": "6206eaf0c5622dd0eddc260e7d03cf6067a986a33884f6f39e50396e2c935ab3",
    "training/cleveland_fed_cpi_surprise_clock.py": "c697d269de67aebde07342075f39911b76fd9edf1a80bb16130d7b2bdb9ba346",
    "docs/cleveland-fed-cpi-surprise-preregistration-2026-07-18.md": "04c29cfdb2aa47190287a72e9e88adae5ce739b2c13161d6598efada412b7911",
    "results/cleveland_fed_cpi_surprise_preregistration_2026-07-18.json": "9c252a988885c7fa1975b6f7190af4efeab50ee8541a67c0bb8f8882a3fa3e0d",
    "results/cleveland_fed_cpi_surprise_preregistered_clock_2026-07-18.csv.gz": "cff8d0f8d7810400bc78f833cc91996a7b2cd0e9d5903fe0ef154f0e38a71739",
    "training/build_cleveland_fed_cpi_surprise_support.py": "b2221a8bd7a55267c3bd51b99302ee59896f7027d8857ef34f97d0ac503834fb",
    "docs/cleveland-fed-cpi-surprise-support-2026-07-18.md": "e0f292c06666a2d432d2f66e3d84a851abd4a16a172d8da0f8d77103304e88a8",
    "results/cleveland_fed_cpi_surprise_support_2026-07-18.json": "d324aac5292c19314233d35db83d6b69585b929bc4650cf2bfec55a3547c485a",
    "results/cleveland_fed_cpi_surprise_clocks_2026-07-18.csv.gz": "d1223d848a47de64224f9b21429287cbec51052e52c4184b4f10efe79d361511",
...
SUPPORT_RESULT = Path("results/cleveland_fed_cpi_surprise_support_2026-07-18.json")
CLOCKS = Path("results/cleveland_fed_cpi_surprise_clocks_2026-07-18.csv.gz")
...
EVALUATOR_SOURCE = Path("training/evaluate_cleveland_fed_cpi_surprise.py")
EVALUATOR_FREEZE = Path(
    "results/cleveland_fed_cpi_surprise_evaluator_freeze_2026-07-18.json"
)
STAGE1_OUTPUT = Path(
    "results/cleveland_fed_cpi_surprise_stage1_2020_2022_2026-07-18.json"
)
STAGE1_DOC = Path("docs/cleveland-fed-cpi-surprise-stage1-2020-2022-2026-07-18.md")
STAGE2_OUTPUT = Path("results/cleveland_fed_cpi_surprise_stage2_2023_2026-07-18.json")
STAGE2_DOC = Path("docs/cleveland-fed-cpi-surprise-stage2-2023-2026-07-18.md")
...
def _timestamp(value: Any) -> pd.Timestamp:
    ...
    if result is pd.NaT:
        raise ValueError("CFCS-1 timestamp is NaT")
...
def _load_json(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"CFCS-1 expected a JSON object: {path}")
    return payload
...
def _verify_registration(registration: dict[str, Any]) -> None:
    ...
    if registration.get("manifest_hash") != prereg.sha256_bytes(
        ...
        raise ValueError("CFCS-1 preregistration manifest mismatch")
    if registration.get("policy_id") != "CFCS-1":
        raise ValueError("CFCS-1 preregistration identity changed")
...
def _verify_evaluation_contract(registration: dict[str, Any]) -> EvaluationConfig:
    ...
    for key in (
        ...
        if asdict(cfg)[key] != policy[key]:
            raise ValueError(f"CFCS-1 strict engine differs on {key}")
    if cfg.cluster_draws != 20_000 or cfg.cluster_seed != 20_260_717:
        raise ValueError("CFCS-1 sign-flip configuration changed")
    if cfg.mdd_denominator_floor != 1e-9:
        raise ValueError("CFCS-1 MDD denominator floor changed")
    ...
    if stage["stage1_gates"] != expected_stage1:
        raise ValueError("CFCS-1 Stage1 gate contract changed")
    if stage["stage2_gates"] != expected_stage2:
        raise ValueError("CFCS-1 Stage2 gate contract changed")
    return cfg
...
def _verify_static_inputs() -> tuple[dict[str, Any], dict[str, Any]]:
    for path, expected in STATIC_INPUT_SHA256.items():
        if _sha256(path) != expected:
            raise ValueError(f"CFCS-1 frozen input changed: {path}")
    ...
    if support.get("manifest_hash") != _canonical_hash(support_core):
        raise ValueError("CFCS-1 support manifest changed")
    if support.get("policy_id") != "CFCS-1":
        raise ValueError("CFCS-1 support identity changed")
    ...
    if support.get("outcome_sources_opened") != []:
        raise ValueError("CFCS-1 support opened outcome sources")
    ...
    if support.get("support_passed") is not True:
        raise ValueError("CFCS-1 support failed")
    if support.get("advance_to_stage1_outcomes") is not True:
        raise ValueError("CFCS-1 support did not authorize Stage1")
    if support.get("preregistration_manifest_hash") != registration["manifest_hash"]:
        raise ValueError("CFCS-1 support is not bound to preregistration")
    if support.get("clocks", {}).get("sha256") != STATIC_INPUT_SHA256[str(CLOCKS)]:
        raise ValueError("CFCS-1 support is not bound to the clock ledger")
...
def load_schedules() -> dict[str, pd.DataFrame]:
    ...
    if set(ledger["control"]) != set(ALL_CLOCK_NAMES):
        raise ValueError("CFCS-1 support clock family changed")
    schedules = {
    ...
    for name, schedule in schedules.items():
        if len(schedule) != int(expected_counts[name]):
            raise ValueError(f"CFCS-1 {name} source count changed")
        if not bool(schedule["side"].isin((-1, 1)).all()):
            raise ValueError(f"CFCS-1 {name} side changed")
        delay = (
            1 if name == "one_day_delay" else 7 if name == "seven_day_placebo" else 0
        )
        for row in schedule.to_dict(orient="records"):
            ...
            expected_entry, expected_exit = clock._execution_times(
                signal.to_pydatetime(), delay
            )
            if _timestamp(row["entry_time"]) != pd.Timestamp(expected_entry):
                raise ValueError(f"CFCS-1 {name} entry clock changed")
            if _timestamp(row["exit_time"]) != pd.Timestamp(expected_exit):
                raise ValueError(f"CFCS-1 {name} exit clock changed")
        if len(schedule) > 1:
            ...
            if not bool(entries.ge(exits).all()):
                raise ValueError(f"CFCS-1 {name} schedule overlaps")
        if not bool(schedule["exit_time"].lt(STAGE2[1]).all()):
            raise ValueError(f"CFCS-1 {name} clock crosses source horizon")
    return schedules
...
def _schedule_record(frame: pd.DataFrame) -> dict[str, Any]:
    ...
    for row in frame.to_dict(orient="records"):
        expected_entry, expected_exit = clock._execution_times(
            _timestamp(row["signal_day"]).to_pydatetime(), delay
        )
...
def freeze_evaluator(output_path: str | Path = EVALUATOR_FREEZE) -> dict[str, Any]:
    ...
    if not all(
        ...
        raise ValueError("CFCS-1 evaluator schedule invariant failed")
    core = {
        "protocol_version": "cleveland_fed_cpi_surprise_evaluator_v1",
        "policy_id": "CFCS-1",
...
def verify_evaluator_freeze(path: str | Path = EVALUATOR_FREEZE) -> dict[str, Any]:
    ...
    if report.get("manifest_hash") != _canonical_hash(core):
        raise ValueError("CFCS-1 evaluator freeze manifest mismatch")
    if report.get("evaluator_source_sha256") != _sha256(EVALUATOR_SOURCE):
        raise ValueError("CFCS-1 evaluator source changed after freeze")
    if report.get("opened_windows") != []:
        raise ValueError("CFCS-1 evaluator freeze opened a window")
    if report.get("execution_ohlc_rows_parsed_during_freeze") != 0:
        raise ValueError("CFCS-1 evaluator freeze parsed OHLC")
    if report.get("funding_rows_parsed_during_freeze") != 0:
        raise ValueError("CFCS-1 evaluator freeze parsed funding")
    if report.get("simulation_run_during_freeze") is not False:
        raise ValueError("CFCS-1 evaluator freeze simulated outcomes")
    schedules = load_schedules()
    for name, schedule in schedules.items():
        if report["control_schedules"][name] != _schedule_record(schedule):
            raise ValueError(f"CFCS-1 {name} schedule changed after freeze")
    return report
...
def _verify_execution_manifests() -> None:
    ...
    if market_manifest.get("combined_output") != str(MARKET):
        raise ValueError("CFCS-1 market manifest path changed")
    if market_manifest.get("combined_sha256") != MARKET_DATA_SHA256:
        raise ValueError("CFCS-1 market manifest data hash changed")
    if market_manifest.get("rows") != 420_768:
        raise ValueError("CFCS-1 market manifest row count changed")
    if market_manifest.get("protocol", {}).get("outcomes_opened") is not False:
        raise ValueError("CFCS-1 market manifest opened an outcome")
    funding_manifest = _load_json(FUNDING_MANIFEST)
    funding_data = funding_manifest.get("data", {})
    if funding_data.get("path") != str(FUNDING):
        raise ValueError("CFCS-1 funding manifest path changed")
    if funding_data.get("sha256") != FUNDING_DATA_SHA256:
        raise ValueError("CFCS-1 funding manifest data hash changed")
    if funding_data.get("rows") != 4_383:
        raise ValueError("CFCS-1 funding manifest row count changed")
    if funding_manifest.get("outcomes_opened") is not False:
        raise ValueError("CFCS-1 funding manifest opened an outcome")
    ...
    *,
    ...
    if stage == "stage1_2020_2022":
        gates = {
            ...
            <= 0.10,
            "minimum_24_trades": base["trades"] >= 24,
            "minimum_8_long_trades": headline["long_trades"] >= 8,
            "minimum_8_short_trades": headline["short_trades"] >= 8,
            "mean_gross_underlying_at_least_50bp": base["mean_gross_underlying_bp"]
            >= 50.0,
        ...
        gates = {
            ...
            <= 0.10,
            "minimum_8_trades": base["trades"] >= 8,
            "minimum_8_long_trades": headline["long_trades"] >= 8,
            "mean_gross_underlying_at_least_50bp": base["mean_gross_underlying_bp"]
            >= 50.0,
            ...
            "each_half_minimum_4_trades": all(
                value["trades"] >= 4 for value in split.values()
            ),
            "stress_cost_absolute_return_positive": stress["absolute_return_pct"] > 0.0,
        }
    return {
...
def _build_stage_report(
    *,
    ...
    core = {
        "protocol_version": f"cleveland_fed_cpi_surprise_{stage}_v1",
        "policy_id": "CFCS-1",
        "stage": stage,
...
def _verified_passing_stage1(expected_freeze_hash: str) -> dict[str, Any]:
    if not STAGE1_OUTPUT.exists():
        raise ValueError("CFCS-1 Stage1 has not been run")
    ...
    if stored.get("manifest_hash") != _canonical_hash(core):
        raise ValueError("CFCS-1 stored Stage1 manifest changed")
    if stored.get("evaluator_freeze_manifest_hash") != expected_freeze_hash:
        raise ValueError("CFCS-1 stored Stage1 freeze identity changed")
    if stored.get("evaluator_source_sha256") != _sha256(EVALUATOR_SOURCE):
        raise ValueError("CFCS-1 stored Stage1 evaluator changed")
    if stored.get("gate_passed") is not True:
        raise ValueError("CFCS-1 Stage1 failed; 2023 remains sealed")
    market, funding, diagnostics = load_execution_window(STAGE1)
    ...
    if replayed != stored:
        raise ValueError("CFCS-1 Stage1 does not exactly replay")
    return stored
...
def render_stage_doc(report: dict[str, Any]) -> str:
    ...
    return f"""# CFCS-1 {report["stage"]} result — {prereg.AS_OF_DATE}
