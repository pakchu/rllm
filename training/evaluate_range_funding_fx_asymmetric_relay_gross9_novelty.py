"""Evaluate frozen RFFXAR-ASYM structural novelty against every Gross9 sleeve."""
...
POLICY = "RFFXAR-ASYM"
PROTOCOL = "rffxar_asym_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/range_funding_fx_asymmetric_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("RFFXAR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("RFFXAR novelty limits drift")
    if (
        ...
        raise RuntimeError("RFFXAR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="RFFXAR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "rffxar_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject RFFXAR-ASYM unchanged before economic outcomes"
        ),
    }
