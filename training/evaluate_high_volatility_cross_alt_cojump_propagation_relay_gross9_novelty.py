"""Evaluate frozen HVCACJP-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCACJP-6"
PROTOCOL = "hvcacjp_6_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_cross_alt_cojump_propagation_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCACJP predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCACJP novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCACJP predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCACJP primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcacjp_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCACJP-6 unchanged before economic outcomes",
    }
