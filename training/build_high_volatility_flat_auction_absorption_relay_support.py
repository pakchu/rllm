"""Build outcome-blind source support for frozen HVFAAR-8."""
...
from training import preregister_high_volatility_flat_auction_absorption_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-04-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="9b62472beca829eb251500a11424855305cf522ac9a9a89c00d1520e7cb16fb0"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_flat_auction_absorption_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_flat_auction_absorption_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_flat_auction_absorption_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_flat_auction_absorption_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_flat_auction_absorption_relay_support_2026-08-10.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","flat_turnover_share","flat_share_rank","flat_minute_count_share","flat_count_share_rank","realized_variation","variation_rank","completed_return","final_two_hour_return","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","flat_turnover_share","flat_share_rank","flat_minute_count_share","flat_count_share_rank","realized_variation","variation_rank","completed_return","final_two_hour_return")
...
def flat_statistics(block:pd.DataFrame)->tuple[float,float,float,float,float,int]:
 quote=block.quote_asset_volume.to_numpy(float);flat=block.close.to_numpy(float)==block.open.to_numpy(float);flat_count=int(flat.sum());total=float(quote.sum())
 if total<=0 or flat_count<P["minimum_flat_minutes"]:return (math.nan,)*5+(flat_count,)
 turnover_share=float(quote[flat].sum()/total);count_share=float(flat.mean())
 ...
 return turnover_share,count_share,variation,completed,final_two_hour,flat_count
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close","quote_asset_volume"]:raise RuntimeError("HVFAAR source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVFAAR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("3h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   share,count_share,var,ret,final_ret,flat_count=flat_statistics(block);valid=all(math.isfinite(v) for v in (share,count_share,var,ret,final_ret)) and var>0 and share>0 and flat_count>=P["minimum_flat_minutes"] and ret!=0 and final_ret!=0
  else:share=count_share=var=ret=final_ret=math.nan;flat_count=0;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"flat_turnover_share":share,"flat_minute_count_share":count_share,"realized_variation":var,"completed_return":ret,"final_two_hour_return":final_ret})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["flat_share_rank"]=prior_rank(panel.flat_turnover_share.where(valid));panel["flat_count_share_rank"]=prior_rank(panel.flat_minute_count_share.where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));agreement=np.sign(panel.completed_return).eq(np.sign(panel.final_two_hour_return));panel["eligible"]=valid&panel.flat_share_rank.ge(P["flat_share_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"])&agreement;panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_boundary_stale_geometry":
  cols=["source_valid","flat_turnover_share","flat_share_rank","flat_minute_count_share","flat_count_share_rank","variation_rank","completed_return","final_two_hour_return","feature_available_time"];used[cols]=panel[cols].shift(1)
 valid=used.source_valid.eq(True);strong=used.flat_share_rank.ge(P["flat_share_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"]);agreement=np.sign(used.completed_return).eq(np.sign(used.final_two_hour_return));state=valid&strong&variation&agreement
 if control=="no_flat_share_tail":state=valid&variation&agreement
 elif control=="no_variation_gate":state=valid&strong&agreement
 elif control=="flat_minute_count_share":state=valid&used.flat_count_share_rank.ge(P["flat_share_rank_min"])&variation&agreement
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVFAAR prereg drift")
 ...
 source_core={"protocol_version":"hvfaar_8_source_v1","query":QUERY,"query_sha256":hashlib.sha256(QUERY.encode()).hexdigest(),"window":[START.isoformat(),END.isoformat()],"physical_rows":len(raw),"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"panel":{"path":str(PANEL),"sha256":sha(PANEL),"rows":len(panel),"valid_rows":int(panel.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};manifest={**source_core,"manifest_hash":chash(source_core)};immutable(MANIFEST,json_bytes(manifest))
