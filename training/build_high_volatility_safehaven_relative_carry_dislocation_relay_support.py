"""Materialize source-only HVSCRD-8 support clocks."""
...
ENV_FILE="/home/pakchu/rllm/.env"; PREREG_SHA="eff0eb9d1364717c0479e4498180f36f49b4dd66a625cb55452b77eca860e9cd"; START=pd.Timestamp("2023-01-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
SOURCE_DIR=Path("data/high_volatility_safehaven_relative_carry_dislocation_relay_sources_2023_2026"); SESSION=SOURCE_DIR/"safehaven_relative_sessions.csv.gz"; SOURCE_MANIFEST=SOURCE_DIR/"manifest.json"
...
CLOCK=Path("data/high_volatility_safehaven_relative_carry_dislocation_relay_clocks_2023_2026.csv.gz"); CONTROL_DIR=Path("data/high_volatility_safehaven_relative_carry_dislocation_relay_controls_2023_2026"); RESULT=Path("results/high_volatility_safehaven_relative_carry_dislocation_relay_support_2026-08-13.json")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00:00Z"),pd.Timestamp("2024-01-01T00:00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00:00Z"),pd.Timestamp("2025-01-01T00:00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00:00Z"),pd.Timestamp("2026-01-01T00:00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00:00Z"),pd.Timestamp("2026-08-01T00:00:00Z"))}; MINIMUM={"train":8,"test":12,"eval":12,"final":8}; CONTROLS=("no_variation_gate","no_dislocation_tail","raw_return_difference","one_session_stale_dislocation","direction_flip","forced_long")
SYMBOLS=("USDJPY","USDCHF"); RISK_MULTIPLIER={"USDJPY":-1.,"USDCHF":-1.}
COLUMNS=("candidate","control","split","source_day","decision_time","feature_available_time","entry_time","exit_time","side","relative_dislocation","dislocation_rank","btc_realized_variation","btc_realized_variation_rank")
...
def materialize_sessions()->dict:
 ...
 with db.connect() as connection:
  for symbol in SYMBOLS:
   ...
   canonical=(RISK_MULTIPLIER[symbol]*np.log(raw.session_close/raw.session_open)).where(valid);piece=pd.DataFrame({"source_day":raw.source_day,f"{symbol}_valid":valid,f"{symbol}_risk_return":canonical});frames.append(piece)
 db.dispose();frame=frames[0]
 for piece in frames[1:]:frame=frame.merge(piece,on="source_day",how="outer",validate="one_to_one")
 frame=frame.sort_values("source_day").reset_index(drop=True);valid_columns=[f"{symbol}_valid" for symbol in SYMBOLS];frame["source_valid"]=frame[valid_columns].fillna(False).all(axis=1)
 for symbol in SYMBOLS:frame[f"{symbol}_z"]=causal_z(frame[f"{symbol}_risk_return"].where(frame.source_valid))
 frame["relative_dislocation"]=frame.USDJPY_z-frame.USDCHF_z;frame["raw_return_difference"]=frame.USDJPY_risk_return-frame.USDCHF_risk_return;frame["dislocation_rank"]=strict_prior_midrank(frame.relative_dislocation.abs().where(frame.source_valid));frame["decision_time"]=frame.source_day+pd.Timedelta(hours=13)
 SOURCE_DIR.mkdir(parents=True,exist_ok=True);_write_gzip_csv(frame,SESSION);core={"protocol_version":"hvscrd_8_safehaven_relative_source_v1","query":QUERY,"table":"bars_polygon","symbols":list(SYMBOLS),"canonical_risk_multipliers":RISK_MULTIPLIER,"interval":"1m","window":[START.isoformat(),END.isoformat()],"outcomes_opened":False,"candidate_incidence_opened":False,"no_imputation":True,"output":{"path":str(SESSION),"sha256":sha(SESSION),"rows":len(frame),"valid_rows":int(frame.source_valid.sum())}};payload={**core,"manifest_hash":chash(core)};SOURCE_MANIFEST.write_text(json.dumps(payload,indent=2,allow_nan=False)+"\n");return payload
def features()->pd.DataFrame:
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVSCRD prereg drift")
 if sha(PRICE)!=PRICE_SHA or sha(PRICE_MANIFEST)!=PRICE_MANIFEST_SHA:raise RuntimeError("HVSCRD BTC source drift")
 ...
 for column in ("relative_dislocation","raw_return_difference","dislocation_rank"):f[column]=pd.to_numeric(f[column],errors="coerce")
 ...
 d=p[["decision_time","btc_realized_variation","btc_valid"]];f=f.merge(d,on="decision_time",how="left",validate="one_to_one");f["btc_realized_variation_rank"]=strict_prior_midrank(f.btc_realized_variation.where(f.source_valid&f.btc_valid));f["signal_valid"]=f.source_valid&f.btc_valid&np.isfinite(f[["relative_dislocation","dislocation_rank","btc_realized_variation","btc_realized_variation_rank"]]).all(axis=1);return f
def conditions(f:pd.DataFrame,control:str)->tuple[pd.Series,pd.Series]:
 dislocation=pd.to_numeric(f.relative_dislocation,errors="coerce");tail=pd.to_numeric(f.dislocation_rank,errors="coerce")
 if control=="raw_return_difference":dislocation=pd.to_numeric(f.raw_return_difference,errors="coerce")
 if control=="one_session_stale_dislocation":dislocation=dislocation.shift(1);tail=tail.shift(1)
 vol=pd.Series(True,index=f.index) if control=="no_variation_gate" else f.btc_realized_variation_rank.ge(.65)
 tail_gate=pd.Series(True,index=f.index) if control=="no_dislocation_tail" else tail.ge(.75)
 active=f.signal_valid&np.isfinite(dislocation)&dislocation.ne(0)&tail_gate&vol;side=-np.sign(dislocation)
...
def clock(f:pd.DataFrame,control:str="primary")->pd.DataFrame:
 ...
 for i in f.index[active]:
  ...
  next_allowed=exit_;rows.append({"candidate":"HVSCRD-8","control":control,"split":split,"source_day":f.at[i,"source_day"],"decision_time":decision,"feature_available_time":decision,"entry_time":entry,"exit_time":exit_,"side":int(side.at[i]),"relative_dislocation":float(f.at[i,"relative_dislocation"]),"dislocation_rank":float(f.at[i,"dislocation_rank"]),"btc_realized_variation":float(f.at[i,"btc_realized_variation"]),"btc_realized_variation_rank":float(f.at[i,"btc_realized_variation_rank"])})
