"""Sequential strict economics for frozen HVFT-24."""
...
PREREG = Path("results/high_volatility_fisher_transform_turn_relay_preregistration_2026-08-11.json")
PREREG_SHA = "88cb712498240adb136adb369ebbaee2c4ea9ca1d155f88b62851c7fd2610e32"
SUPPORT = Path("results/high_volatility_fisher_transform_turn_relay_support_2026-08-11.json")
SUPPORT_SHA = "5fbabebbc013915e43d74b77bd75535bf74b213239855ad796a3ee88b112d4ee"
NOVELTY = Path("results/high_volatility_fisher_transform_turn_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "1077bb05de033ae78c9ec4c4c023fc17a76c5bcf3c2bcc7edb461e438f8931d6"
CLOCK = Path("data/high_volatility_fisher_transform_turn_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "9353319a5d8329eb3760b8cf093c0c61ff77deb7fe2dece09cde6aab2b1fea00"
CONTROL_DIR = Path("data/high_volatility_fisher_transform_turn_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_fisher_transform_turn_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_fisher_transform_turn_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "unsmoothed_fisher", "one_bar_stale_turn", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVFT frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVFT novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvft_24_sequential_economics_v1", "policy_id": POLICY_ID,
