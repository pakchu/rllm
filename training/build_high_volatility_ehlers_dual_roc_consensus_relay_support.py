"""Build outcome-blind source support for frozen HVEDR-24."""
...
from training import preregister_high_volatility_ehlers_dual_roc_consensus_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "629288a392c64bd708e6f3ff167bceca1d8c77a6dafb1ff07319be2135ea08b3"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ehlers_dual_roc_consensus_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ehlers_dual_roc_consensus_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ehlers_dual_roc_consensus_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ehlers_dual_roc_consensus_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ehlers_dual_roc_consensus_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "run_length", "super_smoother", "fast_high_pass",
    "slow_high_pass", "fast_roc", "slow_roc", "consensus_side", "entry_side",
    "fast_entry_side", "slow_entry_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "run_length", "super_smoother", "fast_high_pass",
    "slow_high_pass", "fast_roc", "slow_roc", "consensus_side", "entry_side",
    "fast_entry_side", "slow_entry_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVEDR source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVEDR invalid source key")
...
def super_smoother(series: pd.Series, valid: pd.Series, period: int) -> pd.DataFrame:
    a1=math.exp(-1.414*math.pi/period);c2=2*a1*math.cos(1.414*math.pi/period);c3=-a1*a1;c1=1-c2-c3
    values=series.to_numpy(float);ok=valid.to_numpy(bool);output=np.full(len(series),np.nan);runs=np.zeros(len(series),dtype=int);run=0
    for i,(price,is_valid) in enumerate(zip(values,ok)):
        if not is_valid or not math.isfinite(price):run=0;continue
        run+=1;runs[i]=run
        output[i]=price if run<=3 else c1*(price+values[i-1])/2+c2*output[i-1]+c3*output[i-2]
    return pd.DataFrame({"run_length":runs,"super_smoother":output},index=series.index)


def high_pass(series: pd.Series, valid: pd.Series, period: int) -> pd.Series:
    ...
    for i,(value,is_valid) in enumerate(zip(values,ok)):
        ...
        run+=1
        output[i]=0. if run<=3 else c1*(value-2*values[i-1]+values[i-2])+c2*output[i-1]+c3*output[i-2]
    return pd.Series(output,index=series.index)


def transition_side(state: pd.Series) -> pd.Series:
    previous=state.shift(1);return state.where(state.ne(0)&state.ne(previous),0).fillna(0).astype(int)


def dual_roc(series: pd.Series, valid: pd.Series) -> pd.DataFrame:
    smooth=super_smoother(series,valid,P["smoother_periods"]);smoothed_valid=np.isfinite(smooth.super_smoother);fast=high_pass(smooth.super_smoother,smoothed_valid,P["fast_high_pass_periods"]);slow=high_pass(smooth.super_smoother,smoothed_valid,P["slow_high_pass_periods"]);within=smooth.run_length.gt(P["roc_lag_periods"]);fast_roc=(fast-fast.shift(P["roc_lag_periods"])).where(within);slow_roc=(slow-slow.shift(P["roc_lag_periods"])).where(within)
    consensus=pd.Series(np.select([(fast_roc>0)&(slow_roc>0),(fast_roc<0)&(slow_roc<0)],[1,-1],default=0),index=series.index,dtype=int)
    fast_state=pd.Series(np.sign(fast_roc.fillna(0)),index=series.index,dtype=int);slow_state=pd.Series(np.sign(slow_roc.fillna(0)),index=series.index,dtype=int)
    return pd.DataFrame({"run_length":smooth.run_length,"super_smoother":smooth.super_smoother,"fast_high_pass":fast,"slow_high_pass":slow,"fast_roc":fast_roc,"slow_roc":slow_roc,"consensus_side":consensus,"entry_side":transition_side(consensus),"fast_entry_side":transition_side(fast_state),"slow_entry_side":transition_side(slow_state)},index=series.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    fields=["bar_open","bar_high","bar_low","bar_close"];bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[fields]).all(axis=1)&bars[fields].gt(0).all(axis=1);values=dual_roc(bars.bar_close,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["super_smoother","fast_high_pass","slow_high_pass","fast_roc","slow_roc","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="fast_roc_only":side=pd.to_numeric(used.fast_entry_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
    elif control=="slow_roc_only":side=pd.to_numeric(used.slow_entry_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVEDR prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvedr_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvedr_24_source_support_v1", "policy_id": prereg.POLICY_ID,
