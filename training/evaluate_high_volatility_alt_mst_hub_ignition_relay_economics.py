"""Sequential strict economics for frozen HVAMST-8."""
...
PREREG = Path("results/high_volatility_alt_mst_hub_ignition_relay_preregistration_2026-08-13.json")
PREREG_SHA = "a41fdf5ce071ed241172a2d2b1247d952d8992a55c7dcf64d3ff2b12127e1654"
SUPPORT = Path("results/high_volatility_alt_mst_hub_ignition_relay_support_2026-08-13.json")
SUPPORT_SHA = "009669d9a9ed549290f3dde201697229f75da0980682752641ee2a3fcd51294d"
NOVELTY = Path("results/high_volatility_alt_mst_hub_ignition_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "ba686efd8f51b37c6bab1f174c4bdc7b71074d76bf63e58142b509361caf5e10"
CLOCK = Path("data/high_volatility_alt_mst_hub_ignition_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "550f65ba1044981a5c4c87129445cadef75a7aa159bde517b3e5a29d8b757205"
CONTROL_DIR = Path("data/high_volatility_alt_mst_hub_ignition_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_alt_mst_hub_ignition_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_alt_mst_hub_ignition_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_centralization_tail", "no_variation_gate", "equal_weight_alt_direction", "one_decision_stale_tree", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVAMST frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVAMST novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvamst_8_sequential_economics_v1", "policy_id": POLICY_ID,
