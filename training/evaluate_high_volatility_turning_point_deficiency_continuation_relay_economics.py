"""Sequential strict economics for frozen HVTPDCR-8."""
...
PREREG = Path("results/high_volatility_turning_point_deficiency_continuation_relay_preregistration_2026-08-12.json")
PREREG_SHA = "57ffeebf061e7adf8e7517e1c9287f94ba8a3360a5e91fc13bb35f12c12d3ae1"
SUPPORT = Path("results/high_volatility_turning_point_deficiency_continuation_relay_support_2026-08-12.json")
SUPPORT_SHA = "9ec8e2050fe3048c9f2b5072ecb98207063d99c1314e0344efdd10a853e79cd7"
NOVELTY = Path("results/high_volatility_turning_point_deficiency_continuation_relay_gross9_novelty_2026-08-12.json")
NOVELTY_SHA = "4eeb61de7007fb7b78149d0e2ee182db18870400dbccc725ece8860f8a46ca69"
CLOCK = Path("data/high_volatility_turning_point_deficiency_continuation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "06b08013bd6e793c771b136f83c757d64dadd7116c9da1b768347be00d5a80ce"
CONTROL_DIR = Path("data/high_volatility_turning_point_deficiency_continuation_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_turning_point_deficiency_continuation_relay_economic_evaluator_freeze_2026-08-12.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_turning_point_deficiency_continuation_relay_{stage}_economics_2026-08-12.json")
    for stage in STAGES
}
CONTROLS = ("no_turning_point_deficiency_tail", "no_variation_gate", "turning_point_share_below_two_thirds", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTPDCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTPDCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvtpdcr_24_sequential_economics_v1", "policy_id": POLICY_ID,
