"""Materialize source-only SQCBI-8 support clocks."""
...
PREREG_SHA="f291d58d4e67d0b812aef848b11ace97c550327d3f72f4a2be2c735449407df3"
...
FEATURE_DIR=Path("data/stablecoin_quote_count_breadth_impulse_sources_2023_2026");FEATURES=FEATURE_DIR/"preentry_features.csv.gz";SOURCE_MANIFEST=FEATURE_DIR/"manifest.json";CLOCK=Path("data/stablecoin_quote_count_breadth_impulse_clocks_2023_2026.csv.gz");CONTROL_DIR=Path("data/stablecoin_quote_count_breadth_impulse_controls_2023_2026");RESULT=Path("results/stablecoin_quote_count_breadth_impulse_support_2026-08-09.json")
SYMBOLS=("BTCUSDT","BTCUSDC","BTCFDUSD");SPLITS={"train":(pd.Timestamp("2023-07-01T00:00:00Z"),pd.Timestamp("2024-01-01T00:00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00:00Z"),pd.Timestamp("2025-01-01T00:00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00:00Z"),pd.Timestamp("2026-01-01T00:00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00:00Z"),pd.Timestamp("2026-08-01T00:00:00Z"))};MINIMUM={"train":8,"test":12,"eval":12,"final":8};CONTROLS=("no_volatility_gate","no_impulse_tail","no_entropy_confirmation","one_block_stale_breadth","direction_flip")
COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","count_share_usdt","count_share_usdc","count_share_fdusd","alternative_count_share","normalized_count_entropy","breadth_impulse","entropy_impulse","impulse_rank","flow_usdc","flow_fdusd","btc_realized_variation","btc_realized_variation_rank")
...
def build_features()->pd.DataFrame:
 if sha(PANEL)!=PANEL_SHA or sha(PANEL_MANIFEST)!=PANEL_MANIFEST_SHA or sha(PRICE)!=PRICE_SHA or sha(PRICE_MANIFEST)!=PRICE_MANIFEST_SHA:raise RuntimeError("SQCBI source drift")
 x=pd.read_csv(PANEL,compression="gzip");x["date"]=pd.to_datetime(x.date,utc=True,errors="raise");x=x[x.symbol.isin(SYMBOLS)].copy()
 if x[["date","symbol"]].duplicated().any():raise RuntimeError("SQCBI duplicate quote hour")
 ...
 w=g.pivot(index="decision_time",columns="symbol",values=["valid","trade_count","signed_flow"]);w.columns=[f"{a}_{b}" for a,b in w.columns];w=w.reset_index().sort_values("decision_time").reset_index(drop=True)
 for s in SYMBOLS:
  for a in ("valid","trade_count","signed_flow"):
   c=f"{a}_{s}"
   if c not in w:w[c]=np.nan
  w[f"trade_count_{s}"]=pd.to_numeric(w[f"trade_count_{s}"],errors="coerce");w[f"signed_flow_{s}"]=pd.to_numeric(w[f"signed_flow_{s}"],errors="coerce")
 w["block_valid"]=w[[f"valid_{s}" for s in SYMBOLS]].eq(True).all(axis=1)&np.isfinite(w[[f"trade_count_{s}" for s in SYMBOLS]+[f"signed_flow_{s}" for s in SYMBOLS]]).all(axis=1)
 total=w[[f"trade_count_{s}" for s in SYMBOLS]].sum(axis=1);shares=pd.DataFrame({s:w[f"trade_count_{s}"]/total for s in SYMBOLS});w["count_share_usdt"]=shares.BTCUSDT;w["count_share_usdc"]=shares.BTCUSDC;w["count_share_fdusd"]=shares.BTCFDUSD;w["alternative_count_share"]=shares.BTCUSDC+shares.BTCFDUSD;w["normalized_count_entropy"]=-(shares*np.log(shares)).sum(axis=1)/np.log(3);consecutive=w.decision_time.diff().eq(pd.Timedelta(hours=8));transition=w.block_valid&w.block_valid.shift(1,fill_value=False)&consecutive;w["breadth_impulse"]=(w.alternative_count_share-w.alternative_count_share.shift(1)).where(transition);w["entropy_impulse"]=(w.normalized_count_entropy-w.normalized_count_entropy.shift(1)).where(transition);w["impulse_rank"]=strict_prior_midrank(w.breadth_impulse.abs().where(transition))
 ...
 w=w.merge(p,on="decision_time",how="left",validate="one_to_one");w["btc_realized_variation_rank"]=strict_prior_midrank(w.btc_realized_variation.where(w.btc_valid));w["signal_valid"]=transition&w.btc_valid.fillna(False)&np.isfinite(w[["breadth_impulse","entropy_impulse","impulse_rank","btc_realized_variation","btc_realized_variation_rank"]]).all(axis=1);return w
def conditions(f:pd.DataFrame,control:str)->tuple[pd.Series,pd.Series]:
 breadth=f.breadth_impulse;entropy=f.entropy_impulse;rank=f.impulse_rank
 if control=="one_block_stale_breadth":breadth=breadth.shift(1);entropy=entropy.shift(1);rank=rank.shift(1)
 usdc=f.signed_flow_BTCUSDC;fdusd=f.signed_flow_BTCFDUSD;consensus=usdc.ne(0)&fdusd.ne(0)&np.sign(usdc).eq(np.sign(fdusd));tail=pd.Series(True,index=f.index) if control=="no_impulse_tail" else rank.ge(.65);entropy_ok=pd.Series(True,index=f.index) if control=="no_entropy_confirmation" else entropy.gt(0);volatile=pd.Series(True,index=f.index) if control=="no_volatility_gate" else f.btc_realized_variation_rank.ge(.65);active=f.signal_valid&np.isfinite(breadth)&breadth.gt(0)&tail&entropy_ok&consensus&volatile;side=np.sign(usdc);side=-side if control=="direction_flip" else side;return active,side
def clock(f:pd.DataFrame,control:str="primary")->pd.DataFrame:
 ...
 for i in f.index[active]:
  ...
  next_allowed=exit_;rows.append({"candidate":"SQCBI-8","control":control,"split":split,"decision_time":decision,"feature_available_time":decision,"entry_time":entry,"exit_time":exit_,"side":int(side.at[i]),"count_share_usdt":float(f.at[i,"count_share_usdt"]),"count_share_usdc":float(f.at[i,"count_share_usdc"]),"count_share_fdusd":float(f.at[i,"count_share_fdusd"]),"alternative_count_share":float(f.at[i,"alternative_count_share"]),"normalized_count_entropy":float(f.at[i,"normalized_count_entropy"]),"breadth_impulse":float(f.at[i,"breadth_impulse"]),"entropy_impulse":float(f.at[i,"entropy_impulse"]),"impulse_rank":float(f.at[i,"impulse_rank"]),"flow_usdc":float(f.at[i,"signed_flow_BTCUSDC"]),"flow_fdusd":float(f.at[i,"signed_flow_BTCFDUSD"]),"btc_realized_variation":float(f.at[i,"btc_realized_variation"]),"btc_realized_variation_rank":float(f.at[i,"btc_realized_variation_rank"])})
...
def run()->dict:
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("SQCBI preregistration hash drift")
 ...
 source_core={"protocol_version":"sqcbi_8_preentry_sources_v1","source_panel":{"path":str(PANEL),"sha256":PANEL_SHA,"manifest_path":str(PANEL_MANIFEST),"manifest_sha256":PANEL_MANIFEST_SHA},"completed_btc":{"path":str(PRICE),"sha256":PRICE_SHA,"manifest_path":str(PRICE_MANIFEST),"manifest_sha256":PRICE_MANIFEST_SHA},"features":{"path":str(FEATURES),"sha256":sha(FEATURES),"rows":len(f)},"candidate_incidence_opened":False,"postentry_outcomes_opened":False,"no_imputation":True};source={**source_core,"manifest_hash":chash(source_core)};SOURCE_MANIFEST.write_text(json.dumps(source,indent=2,allow_nan=False)+"\n")
