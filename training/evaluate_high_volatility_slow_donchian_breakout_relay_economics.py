"""Sequential strict economics for frozen HVSDBR-24."""
...
PREREG = Path("results/high_volatility_slow_donchian_breakout_relay_preregistration_2026-08-10.json")
PREREG_SHA = "a021c20c48efdfd0fd870f1115faaa5cdd7ed7889e709cb4877e430ab99f4b25"
SUPPORT = Path("results/high_volatility_slow_donchian_breakout_relay_support_2026-08-10.json")
SUPPORT_SHA = "f2fcf52885d20524150a6e86358502a062193c52ebdc39aaf9d38185d7250412"
NOVELTY = Path("results/high_volatility_slow_donchian_breakout_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "05be91c9afd23cffb0368f23317f97312654180f1abb49da02820b67273b06d5"
CLOCK = Path("data/high_volatility_slow_donchian_breakout_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "4202a594e478d3e5ebaa03f5a38af502890ffc8e0f965bdcbc463702d8b7e389"
CONTROL_DIR = Path("data/high_volatility_slow_donchian_breakout_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_slow_donchian_breakout_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_slow_donchian_breakout_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "fifteen_block_channel", "intrabar_touch_instead_of_close", "one_boundary_stale_geometry", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSDBR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSDBR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvsdbr_24_sequential_economics_v1", "policy_id": POLICY_ID,
