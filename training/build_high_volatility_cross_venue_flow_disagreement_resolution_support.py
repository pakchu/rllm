"""Build outcome-blind source support for frozen HVFDR-8."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_cross_venue_flow_disagreement_resolution as prereg
ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2023-05-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="803c7b3e88eefa69f732e49c938940d588aaeac4966c1240f6fcb78576eac0bf";REG=prereg.build();P=REG["policy"];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG["stages"].items()};GATES=REG["source_support_gates"];CONTROLS=tuple(REG["diagnostic_controls"]["names"]);VENUES=("perpetual","spot")
QUERY="""SELECT date_bin('5 minutes',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00') AS date,'perpetual' AS venue,(array_agg(open ORDER BY ts))[1] AS open,max(high) AS high,min(low) AS low,(array_agg(close ORDER BY ts DESC))[1] AS close,sum(power(ln(close/open),2)) AS minute_squared_return,sum(quote_asset_volume) AS quote_volume,sum(taker_buy_quote) AS taker_buy_quote,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND high>=greatest(open,close,low) AND low<=least(open,close,high)) AS coherent FROM bars_binance WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end GROUP BY 1 UNION ALL SELECT date_bin('5 minutes',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00') AS date,'spot' AS venue,(array_agg(open ORDER BY ts))[1] AS open,max(high) AS high,min(low) AS low,(array_agg(close ORDER BY ts DESC))[1] AS close,sum(power(ln(close/open),2)) AS minute_squared_return,sum(quote_asset_volume) AS quote_volume,sum(taker_buy_quote) AS taker_buy_quote,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND high>=greatest(open,close,low) AND low<=least(open,close,high)) AS coherent FROM bars_binance_spot WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end GROUP BY 1 ORDER BY 1,2"""
ROOT=Path("data/high_volatility_cross_venue_flow_disagreement_resolution_sources_2023_2026");PANEL=ROOT/"five_minute_barrier_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_cross_venue_flow_disagreement_resolution_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_cross_venue_flow_disagreement_resolution_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_cross_venue_flow_disagreement_resolution_controls_2023_2026");RESULT=Path("results/high_volatility_cross_venue_flow_disagreement_resolution_support_2026-08-13.json");BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLS=("decision_time","feature_available_time","source_valid","flow_side","flow_divergence","divergence_rank","perpetual_imbalance","spot_imbalance","btc_realized_variation","variation_rank","variation_active","eligible")
CLOCK_COLS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","flow_divergence","divergence_rank","btc_realized_variation","variation_rank")
...
def prepare(raw:pd.DataFrame)->pd.DataFrame:
 expected=["date","venue","open","high","low","close","minute_squared_return","quote_volume","taker_buy_quote","source_rows","distinct_rows","first_ts","last_ts","coherent"]
 if raw.columns.tolist()!=expected:raise RuntimeError("HVFDR source schema drift")
 x=raw.copy();x["date"]=pd.to_datetime(x.date,utc=True,errors="coerce");x["venue"]=x.venue.astype(str)
 for c in ("open","high","low","close","minute_squared_return","quote_volume","taker_buy_quote","source_rows","distinct_rows"):x[c]=pd.to_numeric(x[c],errors="coerce")
 for c in ("first_ts","last_ts"):x[c]=pd.to_datetime(x[c],utc=True,errors="coerce")
 if x.date.isna().any() or x.duplicated(["date","venue"]).any() or not x.venue.isin(("perpetual","spot")).all():raise RuntimeError("HVFDR invalid source key")
 z=x[["open","high","low","close"]];x["row_valid"]=x.source_rows.eq(5)&x.distinct_rows.eq(5)&x.first_ts.eq(x.date)&x.last_ts.eq(x.date+pd.Timedelta("4m"))&x.coherent.eq(True)&np.isfinite(x[["open","high","low","close","minute_squared_return","quote_volume","taker_buy_quote"]]).all(axis=1)&z.gt(0).all(axis=1)&x.minute_squared_return.ge(0)&x.quote_volume.gt(0)&x.taker_buy_quote.ge(0)&x.taker_buy_quote.le(x.quote_volume);return x
def flow_side(perpetual:pd.Series,spot:pd.Series,tail:pd.Series,opposite:bool=True)->pd.Series:
 relation=perpetual.mul(spot);eligible=tail&spot.ne(0)&perpetual.ne(0)&(relation.lt(0) if opposite else relation.gt(0));side=pd.Series(0,index=spot.index,dtype=int);side.loc[eligible]=np.sign(spot.loc[eligible]).astype(int);return side
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 x=prepare(raw);grid=pd.date_range(START,END,freq="5min",inclusive="left");q=x.pivot(index="date",columns="venue",values="quote_volume").reindex(index=grid,columns=VENUES);buy=x.pivot(index="date",columns="venue",values="taker_buy_quote").reindex(index=grid,columns=VENUES);sq=x.pivot(index="date",columns="venue",values="minute_squared_return").reindex(index=grid,columns=VENUES);valid=x.pivot(index="date",columns="venue",values="row_valid").reindex(index=grid,columns=VENUES).eq(True);joint=valid.all(axis=1);pimb=2*buy.perpetual/q.perpetual-1;simb=2*buy.spot/q.spot-1;divergence=(simb-pimb).abs();rank=strict_prior_midrank(divergence.where(joint),P["history_decisions"],P["minimum_history_decisions"]);side=flow_side(pimb,simb,rank.ge(P["divergence_rank_min"]));variation=np.sqrt(sq.perpetual.where(valid.perpetual).rolling(P["variation_bars"],min_periods=P["variation_bars"]).sum());vrank=strict_prior_midrank(variation,P["history_decisions"],P["minimum_history_decisions"]);source_valid=joint&rank.notna()&variation.notna()&vrank.notna();vactive=vrank.ge(P["variation_rank_min"]);state=side.ne(0);prior_valid=source_valid.shift(1,fill_value=False);eligible=source_valid&prior_valid&state&~state.shift(1,fill_value=False)&vactive
 return pd.DataFrame({"decision_time":grid+pd.Timedelta("5m"),"feature_available_time":grid+pd.Timedelta("5m"),"source_valid":source_valid.to_numpy(bool),"flow_side":side.to_numpy(int),"flow_divergence":divergence.to_numpy(float),"divergence_rank":rank.to_numpy(float),"perpetual_imbalance":pimb.to_numpy(float),"spot_imbalance":simb.to_numpy(float),"btc_realized_variation":variation.to_numpy(float),"variation_rank":vrank.to_numpy(float),"variation_active":vactive.fillna(False).to_numpy(bool),"eligible":eligible.to_numpy(bool)},columns=PANEL_COLS)
def active(panel:pd.DataFrame,control="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 u=panel.copy();side=pd.to_numeric(u.flow_side,errors="coerce").fillna(0).astype(int);state=side.ne(0);prior_valid=u.source_valid.shift(1,fill_value=False);active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)&u.variation_active.eq(True)
 if control=="no_variation_gate":active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)
 elif control in ("rank_0_99","same_sign_flow"):
  side=flow_side(u.perpetual_imbalance,u.spot_imbalance,u.divergence_rank.ge(.99),control!="same_sign_flow");state=side.ne(0);active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)&u.variation_active.eq(True)
 elif control=="one_bar_stale_disagreement":
  cols=["flow_side","flow_divergence","divergence_rank","feature_available_time"];u[cols]=panel[cols].shift(1);side=pd.to_numeric(u.flow_side,errors="coerce").fillna(0).astype(int);state=side.ne(0);active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)&u.variation_active.eq(True)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVFDR prereg drift")
