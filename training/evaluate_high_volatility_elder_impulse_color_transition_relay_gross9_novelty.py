"""Evaluate frozen HVEIS-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVEIS-24"
PROTOCOL = "hveis_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_elder_impulse_color_transition_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVEIS predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVEIS novelty limits drift")
    if (
        ...
        raise RuntimeError("HVEIS predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVEIS primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVEIS-24 unchanged before economic outcomes",
    }
