"""Sequential strict economics for frozen HVEFPR-24."""
...
PREREG = Path("results/high_volatility_ethereum_flaw_pressure_relay_preregistration_2026-08-12.json")
PREREG_SHA = "225512eb0e165b71637a4abf18c0fc0b749ad3b64d84f0cb35e8d8b8a99706c0"
SUPPORT = Path("results/high_volatility_ethereum_flaw_pressure_relay_support_2026-08-12.json")
SUPPORT_SHA = "fed76db422e146c704bdc5fc912efd01a158fe154f03835185322a28445be5b8"
NOVELTY = Path("results/high_volatility_ethereum_flaw_pressure_relay_gross9_novelty_2026-08-12.json")
NOVELTY_SHA = "836d3a517bebb62783f3735410da4463a35cdd2fdb56020844c996787dd1fd03"
CLOCK = Path("data/high_volatility_ethereum_flaw_pressure_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "7681e52ab8d73a8e1a35215848a1cbbfafb696f0491493d699fe56d5a9cbc34b"
CONTROL_DIR = Path("data/high_volatility_ethereum_flaw_pressure_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_ethereum_flaw_pressure_relay_economic_evaluator_freeze_2026-08-12.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ethereum_flaw_pressure_relay_{stage}_economics_2026-08-12.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVEFPR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVEFPR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvefpr_24_sequential_economics_v1", "policy_id": POLICY_ID,
