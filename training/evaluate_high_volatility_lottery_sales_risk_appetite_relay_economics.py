"""Sequential strict economics for frozen HVLSRA-24."""
...
PREREG = Path("results/high_volatility_lottery_sales_risk_appetite_relay_preregistration_2026-08-12.json")
PREREG_SHA = "80b12477f0d4676da43d83abd0883216a081ce993613c4cd3f76fea8a51379f7"
SUPPORT = Path("results/high_volatility_lottery_sales_risk_appetite_relay_support_2026-08-12.json")
SUPPORT_SHA = "6c30ce499fe99d3442bdfa0c7a1aa9b89f44d5623af9c5bb5e7d2294e02cc371"
NOVELTY = Path("results/high_volatility_lottery_sales_risk_appetite_relay_gross9_novelty_2026-08-12.json")
NOVELTY_SHA = "2d251f67e5b623cc76f4b9ef61e3133f46a57374c8556ba0a36ecf3190b29bb2"
CLOCK = Path("data/high_volatility_lottery_sales_risk_appetite_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "41c7adfcab6d849253c4e0ae7ca50aacb0ba7d91cdac7c78629b26a11f11ef5f"
CONTROL_DIR = Path("data/high_volatility_lottery_sales_risk_appetite_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_lottery_sales_risk_appetite_relay_economic_evaluator_freeze_2026-08-12.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_lottery_sales_risk_appetite_relay_{stage}_economics_2026-08-12.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVLSRA frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVLSRA novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvlsra_24_sequential_economics_v1", "policy_id": POLICY_ID,
