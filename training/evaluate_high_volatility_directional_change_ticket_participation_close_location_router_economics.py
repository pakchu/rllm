"""Sequential strict economics for frozen HVDCSATPCLR-8."""
...
NOVELTY = Path("results/high_volatility_directional_change_ticket_participation_close_location_router_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "0d0077f40c26a017550296bed80945be94020cdc57b20167a2b37d9c0e3c531f"
CLOCK = Path("data/high_volatility_directional_change_ticket_participation_close_location_router_clocks_2023_2026.csv.gz")
CLOCK_SHA = "1b1353d89b7be237a0bff09ab1b8d6f9c390b5760a9a98ca1db155bfc3df4acf"
CONTROL_DIR = Path("data/high_volatility_directional_change_ticket_participation_close_location_router_controls_2023_2026")
FREEZE = Path("results/high_volatility_directional_change_ticket_participation_close_location_router_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_directional_change_ticket_participation_close_location_router_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDCSATPCLR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDCSATPCLR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdcsatpclr_8_sequential_economics_v1", "policy_id": POLICY_ID,
