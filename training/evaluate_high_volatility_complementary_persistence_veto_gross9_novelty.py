"""Evaluate the frozen HVCPV-8 action-vote clock against Gross9."""
...
POLICY = "HVCPV-8"
PROTOCOL = "hvcpv_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_complementary_persistence_veto_clocks_2023_2026.csv.gz",
    "sha256": "2141f5dd21e9f5437117939e646d572793cdcbf922882bd865120d1da3fbc5ba",
    "rows": 334,
}
OUTPUT = Path("results/high_volatility_complementary_persistence_veto_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCPV-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCPV-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCPV-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCPV-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCPV-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCPV-8 unchanged before economics",
    }
