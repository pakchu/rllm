"""Sequential strict economics for frozen HVIARR-8."""
...
PREREG = Path("results/high_volatility_inside_auction_resolution_relay_preregistration_2026-08-11.json")
PREREG_SHA = "7fd4fb2020ec4e35fa3dd8ad1e07acffb09ef901ee9875c0fb69bd875d75d745"
SUPPORT = Path("results/high_volatility_inside_auction_resolution_relay_support_2026-08-11.json")
SUPPORT_SHA = "4ba725bd220d42fee3d68ebb6a7b0dc7d4df7d19ecb88f243a048cf3cfeb1b0f"
NOVELTY = Path("results/high_volatility_inside_auction_resolution_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "99feb762d2302198b10e0a3911bca7028cfbaf2d5bbbec942057c9dbc9428436"
CLOCK = Path("data/high_volatility_inside_auction_resolution_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "2a1f3514825d29666bbdbd28d499ba6b8faa7211ecf154e7198b7a0fe467d4e5"
CONTROL_DIR = Path("data/high_volatility_inside_auction_resolution_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_inside_auction_resolution_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_inside_auction_resolution_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_inside_requirement", "no_variation_gate", "release_body_only", "one_block_stale_prior_range", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVIARR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVIARR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hviarr_8_sequential_economics_v1", "policy_id": POLICY_ID,
