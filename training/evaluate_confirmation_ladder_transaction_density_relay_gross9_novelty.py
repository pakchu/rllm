"""Evaluate frozen CLTDR-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "CLTDR-6"
PROTOCOL = "cltdr_6_gross9_novelty_v1"
...
OUTPUT = Path("results/confirmation_ladder_transaction_density_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CLTDR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("CLTDR novelty limits drift")
    if (
        ...
        raise RuntimeError("CLTDR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CLTDR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "cltdr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject CLTDR-6 unchanged before economic outcomes",
    }
