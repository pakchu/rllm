"""Sequential strict economics for frozen HVOID-8."""
...
PREREG = Path("results/high_volatility_oi_unwind_directional_dominance_relay_preregistration_2026-08-13.json")
PREREG_SHA = "eb52de5d1f1f01a9222242e79e4f3d675c71c22ee07a25c144be587f0cf965ee"
SUPPORT = Path("results/high_volatility_oi_unwind_directional_dominance_relay_support_2026-08-13.json")
SUPPORT_SHA = "154c2cf1ae428904d62e7ed9c4bf5bff910e2d0d982595d5d2853251954b1530"
NOVELTY = Path("results/high_volatility_oi_unwind_directional_dominance_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "56e30e6727f4a10b0cc97fc8617848e0ef4f82ff2b11e262686b31b654964bce"
CLOCK = Path("data/high_volatility_oi_unwind_directional_dominance_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "75640127d2660f6cf4fe96471d0c9962acd4ab63c914058cd8f6cab3749136e0"
CONTROL_DIR = Path("data/high_volatility_oi_unwind_directional_dominance_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_oi_unwind_directional_dominance_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_oi_unwind_directional_dominance_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_oi_activity_gate", "no_variation_gate", "no_dominance_gate", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVOID frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVOID novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvoid_8_sequential_economics_v1", "policy_id": POLICY_ID,
