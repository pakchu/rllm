"""Sequential strict economics for frozen HVDCSATPAPAR-8."""
...
POLICY_ID = "HVDCSATPAPAR-8"
BAR = pd.Timedelta(minutes=5)
LEVERAGE = 0.5
BASE_COST = 0.0006
STRESS_COST = 0.0010
ENV_FILE = "/home/pakchu/rllm/.env"
PREREG = Path("results/high_volatility_dcs_ticket_premium_activity_router_preregistration_2026-08-16.json")
PREREG_SHA = "53e80657bf35a2ef2bd0ad849336a2cff7a8fe0ee391974ea02d67a65dbc8c80"
SUPPORT = Path("results/high_volatility_dcs_ticket_premium_activity_router_support_2026-08-16.json")
SUPPORT_SHA = "49e960677196bd04bcfdabcac33cf5b0a7547337146cd15b703a86c852fe580a"
NOVELTY = Path("results/high_volatility_dcs_ticket_premium_activity_router_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "936aded11b4bbf91c9be64304215ff2f44d46569d937ffd0452304a8ffba4aac"
CLOCK = Path("data/high_volatility_dcs_ticket_premium_activity_router_clocks_2023_2026.csv.gz")
CLOCK_SHA = "a87c8e6d58696c79432205245311c562a719ac8a2470ec5ede5e86ed8e70af95"
CONTROL_DIR = Path("data/high_volatility_dcs_ticket_premium_activity_router_controls_2023_2026")
FREEZE = Path("results/high_volatility_dcs_ticket_premium_activity_router_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_dcs_ticket_premium_activity_router_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDCSATPAPAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDCSATPAPAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdcsatpapar_8_sequential_economics_v1", "policy_id": POLICY_ID,
