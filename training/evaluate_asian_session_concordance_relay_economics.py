"""Sequential strict economics for frozen ASCR-16."""
...
PREREG = Path("results/asian_session_concordance_relay_preregistration_2026-08-09.json")
PREREG_SHA = "c8649fcceb8adbe398e15a41b3bae2dbb6ba1285387954af4bab7e981472cff1"
SUPPORT = Path("results/asian_session_concordance_relay_support_2026-08-09.json")
SUPPORT_SHA = "36eb38df2385bb9f09b3aed3455536ea1e88dc38e2ca3c0b30d31edb1a59b493"
NOVELTY = Path("results/asian_session_concordance_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "8cfdcdf6d8fb63d55a16fb8857d84a16a6cdc4dd8f5eedacf529cee2c8321f02"
CLOCK = Path("data/asian_session_concordance_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "1c43e86cbe48a77be0215eb49d7ba8797d1577e8e8dff599148b036b6ace2d38"
CONTROL_DIR = Path("data/asian_session_concordance_relay_controls_2023_2026")
FREEZE = Path("results/asian_session_concordance_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/asian_session_concordance_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("full_session_return", "first_half_only", "second_half_only", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("ASCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("ASCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "ascr_16_sequential_economics_v1", "policy_id": POLICY_ID,
