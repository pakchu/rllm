"""Sequential strict economics for frozen HVCDBR-8."""
...
PREREG = Path("results/high_volatility_closing_drive_breakout_relay_preregistration_2026-08-11.json")
PREREG_SHA = "fedf22c6f863fccd110dfa4fbe448092af22c04f785920021910a1f311d6412a"
SUPPORT = Path("results/high_volatility_closing_drive_breakout_relay_support_2026-08-11.json")
SUPPORT_SHA = "5d8413ebf4a81c2fd79e05723643b43236091317fbd675deba66b16fab5aea23"
NOVELTY = Path("results/high_volatility_closing_drive_breakout_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "0bea13bc9371bf1d56c552ab6bcc5e9d4e54f1ccdff72344ee864d71bd966284"
CLOCK = Path("data/high_volatility_closing_drive_breakout_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "b9a90a696b55d14ae1ce90fb0c77429a5bc8a95f13aaf08cbcb6b0608752f722"
CONTROL_DIR = Path("data/high_volatility_closing_drive_breakout_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_closing_drive_breakout_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_closing_drive_breakout_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_inside_open", "no_variation_gate", "drive_body_only", "one_block_stale_balance_range", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCDBR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCDBR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcdbr_8_sequential_economics_v1", "policy_id": POLICY_ID,
