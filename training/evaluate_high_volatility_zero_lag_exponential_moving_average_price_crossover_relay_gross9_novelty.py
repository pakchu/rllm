"""Evaluate frozen HVZLEMA-C10-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVZLEMA-C10-24"
PROTOCOL = "hvzlema_c10_24_gross9_novelty_v1"
PREREG = Path("results/high_volatility_zero_lag_exponential_moving_average_price_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "70f541b3d3ccc31bd95f5eb524c03083f2d879c1e95c1ed127754db691401816"
SUPPORT = Path("results/high_volatility_zero_lag_exponential_moving_average_price_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "ee935229e14639700e6b74f7925bb07ea3fe28a52e525c31de053ac226ebb3b3"
CLOCK = Path("data/high_volatility_zero_lag_exponential_moving_average_price_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "665edd34a516ea861707cbcef2d3fc9af604e3b9653a586519affcbcb172d1d8"
OUTPUT = Path("results/high_volatility_zero_lag_exponential_moving_average_price_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVZLEMA predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVZLEMA novelty limits drift")
    if (
        ...
        raise RuntimeError("HVZLEMA predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVZLEMA primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvzlema_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVZLEMA-C10-24 unchanged before economic outcomes",
    }
