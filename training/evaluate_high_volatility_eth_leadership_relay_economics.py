"""Sequential strict economics for frozen HVELR-6."""
...
PREREG = Path("results/high_volatility_eth_leadership_relay_preregistration_2026-08-09.json")
PREREG_SHA = "b91ca28c4edcf38c33d66faa764790ea62b36eca3ba7e1bd12abb1f382c4017a"
SUPPORT = Path("results/high_volatility_eth_leadership_relay_support_2026-08-08.json")
SUPPORT_SHA = "e81ce4a52cd6a201d1e7deb5c2a5d29c5f24244b306badfc4d4fa2d66e7101c2"
NOVELTY = Path("results/high_volatility_eth_leadership_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "5d1ce1ddfdeaaab45c1a575f4852a3f0d3f0c51ca63d7652ccdb618b1c8d796b"
CLOCK = Path("data/high_volatility_eth_leadership_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "e3a2c92be1a6839823fd7b4e37bef16df128d766fa8eb79b0eb6d5520f6e1e44"
CONTROL_DIR = Path("data/high_volatility_eth_leadership_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_eth_leadership_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_eth_leadership_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "no_eth_tail_gate", "no_leadership_ratio_gate", "one_boundary_stale_geometry", "direction_fade")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVELR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVELR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvelr_6_sequential_economics_v1", "policy_id": POLICY_ID,
