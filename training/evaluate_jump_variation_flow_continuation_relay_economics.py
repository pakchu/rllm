"""Sequential strict economics for frozen JVFCR-8."""
...
PREREG = Path("results/jump_variation_flow_continuation_relay_preregistration_2026-08-09.json")
PREREG_SHA = "f33f0204c5410daa159d0603692408d4046890968b41b8415e49a2557485a093"
SUPPORT = Path("results/jump_variation_flow_continuation_relay_support_2026-08-09.json")
SUPPORT_SHA = "8ea7fcfdb1429955c10e694cdd8ea45daf6bbc38a62a742862eb9d13fe22e650"
NOVELTY = Path("results/jump_variation_flow_continuation_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "60e91560c3d933f043748bda0c2315d1844dcd20474e292ef6cf0233634f63ee"
CLOCK = Path("data/jump_variation_flow_continuation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "64a806741a66edc9e9372c013810faefe657417e65c1c8e955072e873c608752"
CONTROL_DIR = Path("data/jump_variation_flow_continuation_relay_controls_2023_2026")
FREEZE = Path("results/jump_variation_flow_continuation_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/jump_variation_flow_continuation_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("JVFCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("JVFCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "jvfcr_8_sequential_economics_v1", "policy_id": POLICY_ID,
