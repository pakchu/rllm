"""Sequential strict economics for frozen HVDTBA-6."""
...
PREREG = Path("results/high_volatility_directional_trade_breadth_asymmetry_relay_preregistration_2026-08-10.json")
PREREG_SHA = "21adc9b086b13be8ed798709f0e49ed2d062fb80722679b60a6f550ef8b0a5db"
SUPPORT = Path("results/high_volatility_directional_trade_breadth_asymmetry_relay_support_2026-08-10.json")
SUPPORT_SHA = "a623eb9facaef1c3c04b00eaf4dd8c8c430aa543187529a20fb4bfd141cdeab3"
NOVELTY = Path("results/high_volatility_directional_trade_breadth_asymmetry_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "b2440637262bcadfc39959071d52be5fe963f82ee7ad5ba0c5459fa984f9957a"
CLOCK = Path("data/high_volatility_directional_trade_breadth_asymmetry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "1c126a15011125161cc3cabb15d0bc2567373d4c501af6ed455e1d5b6d9684d3"
CONTROL_DIR = Path("data/high_volatility_directional_trade_breadth_asymmetry_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_directional_trade_breadth_asymmetry_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_directional_trade_breadth_asymmetry_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "no_breadth_magnitude_gate", "quote_volume_directional_breadth", "one_hour_stale_features", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDTBA frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDTBA novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdtba_6_sequential_economics_v1", "policy_id": POLICY_ID,
