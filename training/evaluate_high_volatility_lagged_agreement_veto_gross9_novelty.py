"""Evaluate the frozen HVLAV-8 action-vote clock against Gross9."""
...
POLICY = "HVLAV-8"
PROTOCOL = "hvlav_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_lagged_agreement_veto_clocks_2023_2026.csv.gz",
    "sha256": "7eedc539d66cd42c3e8fb9038d8788a21704f63fec25fcdfc73d8d36429c0dd6",
    "rows": 146,
}
OUTPUT = Path("results/high_volatility_lagged_agreement_veto_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVLAV-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVLAV-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVLAV-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVLAV-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVLAV-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVLAV-8 unchanged before economics",
    }
