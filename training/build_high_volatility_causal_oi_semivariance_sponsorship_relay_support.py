"""Deterministic outcome-blind source support for HVCOSR-24."""
...
from training import preregister_high_volatility_causal_oi_semivariance_sponsorship_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "5c50eb37492920678720ade1d8dfce1c1b8228e7e4d2a66ce408ecf5e85ea719"
...
OI_QUERY = """SELECT ts AS decision_time,sum_open_interest FROM open_interest_binance WHERE symbol='BTCUSDT' AND period='5m' AND ts>=:start AND ts<:end AND extract(hour FROM ts)=0 AND extract(minute FROM ts)=0 AND extract(second FROM ts)=0 ORDER BY ts"""
BAR_QUERY = """SELECT date_bin('1 day',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '1 day' AS decision_time,coalesce(sum(power(ln(close/open),2)) FILTER (WHERE close>open),0) AS upside_semivariance,coalesce(sum(power(ln(close/open),2)) FILTER (WHERE close<open),0) AS downside_semivariance,sum(power(ln(close/open),2)) AS minute_squared_return,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND high>=greatest(open,close,low) AND low<=least(open,close,high)) AS coherent FROM bars_binance WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end GROUP BY 1 ORDER BY 1"""

ROOT = Path("data/high_volatility_causal_oi_semivariance_sponsorship_relay_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_oi_semivariance_sponsorship_relay_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_oi_semivariance_sponsorship_relay_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_causal_oi_semivariance_sponsorship_relay_support_2026-08-16.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "sum_open_interest",
    "previous_open_interest", "oi_change", "upside_semivariance", "downside_semivariance", "semivariance_imbalance",
    "absolute_imbalance", "absolute_imbalance_rank", "realized_variation",
    "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "decision_time", "feature_available_time",
    "entry_time", "exit_time", "side", "oi_change", "semivariance_imbalance",
    "absolute_imbalance", "absolute_imbalance_rank", "realized_variation", "variation_rank",
)
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVCOSR-24 OI schema drift")
    expected_bars = [
        "decision_time", "upside_semivariance", "downside_semivariance", "minute_squared_return",
        "source_rows", "distinct_rows", "first_ts", "last_ts", "coherent",
    ]
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCOSR-24 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all():
        raise RuntimeError("HVCOSR-24 invalid OI rows")

    bars = bars.copy()
    for column in ("decision_time", "first_ts", "last_ts"):
        bars[column] = pd.to_datetime(bars[column], utc=True, errors="raise")
    numeric = ("upside_semivariance", "downside_semivariance", "minute_squared_return", "source_rows", "distinct_rows")
    for column in numeric:
        bars[column] = pd.to_numeric(bars[column], errors="raise")
    if bars["decision_time"].duplicated().any():
        raise RuntimeError("HVCOSR-24 duplicate daily bar")
    start = bars["decision_time"] - pd.Timedelta("24h")
    bars["bar_valid"] = (
        np.isfinite(bars[list(numeric)]).all(axis=1)
        & bars[["upside_semivariance", "downside_semivariance"]].ge(0).all(axis=1)
        & bars["minute_squared_return"].gt(0)
        & bars["source_rows"].eq(POLICY["day_minutes"])
        & bars["distinct_rows"].eq(POLICY["day_minutes"])
    ...
    bars["semivariance_imbalance"] = (bars["upside_semivariance"] - bars["downside_semivariance"]) / bars["minute_squared_return"]
    bars["absolute_imbalance"] = bars["semivariance_imbalance"].abs()
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel = panel.join(
        bars[["bar_valid", "upside_semivariance", "downside_semivariance", "semivariance_imbalance", "absolute_imbalance", "realized_variation"]],
        on="decision_time",
    )
    panel["source_valid"] = (
        panel["previous_time"].notna()
        & panel["decision_time"].sub(panel["previous_time"]).eq(pd.Timedelta(days=1))
        & panel["bar_valid"].eq(True)
        & np.isfinite(
            panel[["sum_open_interest", "previous_open_interest", "oi_change", "upside_semivariance", "downside_semivariance", "semivariance_imbalance", "absolute_imbalance", "realized_variation"]]
        ).all(axis=1)
        & panel["sum_open_interest"].gt(0)
        & panel["previous_open_interest"].gt(0)
        & panel["realized_variation"].gt(0)
        & panel["semivariance_imbalance"].ne(0)
        & panel["absolute_imbalance"].gt(0)
    )
    valid = panel["source_valid"].eq(True)
    panel["absolute_imbalance_rank"] = causal_midrank(panel["absolute_imbalance"].where(valid))
    panel["variation_rank"] = causal_midrank(panel["realized_variation"].where(valid))
    panel["eligible"] = (
        valid
        & panel["oi_change"].gt(0)
        & panel["absolute_imbalance_rank"].ge(POLICY["absolute_imbalance_rank_min"])
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if split is None:
            continue
        side = int(np.sign(row.semivariance_imbalance))
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCOSR-24 side or availability drift")
        ...
        rows.append({
            ...
            "semivariance_imbalance": float(row.semivariance_imbalance),
            "absolute_imbalance": float(row.absolute_imbalance),
            "absolute_imbalance_rank": float(row.absolute_imbalance_rank),
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCOSR-24 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcosr_24_sources_v1",
    ...
    core = {
        "protocol_version": "hvcosr_24_source_support_v1", "policy_id": prereg.POLICY_ID,
