"""Build outcome-blind source support for frozen HVWSI-Z20-24."""
...
from training import preregister_high_volatility_wilder_swing_index_zero_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "29f66b788fa07e6cc52afb6040b801abe67d418aa371545534272ddcedafebd8"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_wilder_swing_index_zero_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_wilder_swing_index_zero_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_wilder_swing_index_zero_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_wilder_swing_index_zero_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_wilder_swing_index_zero_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "n_value", "r_value", "k_value", "swing_index",
    "entry_side", "raw_close_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "n_value", "r_value", "k_value",
    "swing_index", "entry_side", "raw_close_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVWSI source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVWSI invalid source key")
...
def wilder_swing_index(
    ...
    """Replicate LEAN WilderSwingIndex and reset after an invalid bar."""
    opens, highs, lows, closes = (series.to_numpy(float) for series in (open_, high, low, close))
    validity = valid.to_numpy(bool)
    values = {name: np.full(len(close), np.nan) for name in ("n_value", "r_value", "k_value", "swing_index")}
    entry_side = np.zeros(len(close), dtype=int)
    raw_close_side = np.zeros(len(close), dtype=int)
    previous_index: int | None = None
    previous_raw_sign = 0
    scale = P["limit_move"] if limit_move is None else abs(limit_move)
    ...
    for index in range(len(close)):
        ...
        n_value = closes[index] - closes[prior] + 0.5 * (closes[index] - opens[index]) + 0.25 * (closes[prior] - opens[prior])
        expressions = np.array([
            abs(highs[index] - closes[prior]),
            abs(lows[index] - closes[prior]),
            abs(highs[index] - lows[index]),
        ])
        branch = int(np.argmax(expressions))
        prior_body = abs(0.25 * (closes[prior] - opens[prior]))
        if branch == 0:
            r_value = abs(highs[index] - closes[prior]) - abs(0.5 * (lows[index] - closes[prior])) + prior_body
        elif branch == 1:
            r_value = abs(lows[index] - closes[prior]) - abs(0.5 * (highs[index] - closes[prior])) + prior_body
        else:
            r_value = abs(highs[index] - lows[index]) + prior_body
        k_value = max(abs(highs[index] - closes[prior]), abs(lows[index] - closes[prior]))
        swing = 0.0 if r_value == 0 or scale == 0 else 50.0 * (n_value / r_value) * (k_value / scale)
        values["n_value"][index] = n_value
        values["r_value"][index] = r_value
        values["k_value"][index] = k_value
        values["swing_index"][index] = swing
        prior_ready = index > 0 and math.isfinite(values["swing_index"][index - 1])
        prior_positive = prior_ready and values["swing_index"][index - 1] > 0
        prior_negative = prior_ready and values["swing_index"][index - 1] < 0
        entry_side[index] = 1 if swing > 0 and not prior_positive else -1 if swing < 0 and not prior_negative else 0
        close_change = closes[index] - closes[prior]
        raw_sign = 1 if close_change > 0 else -1 if close_change < 0 else 0
        raw_close_side[index] = raw_sign if raw_sign and raw_sign != previous_raw_sign else 0
        previous_raw_sign = raw_sign
        previous_index = index
    return pd.DataFrame({**values, "entry_side": entry_side, "raw_close_side": raw_close_side}, index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=bars.join(wilder_swing_index(bars.bar_open,bars.bar_high,bars.bar_low,bars.bar_close,bars.valid_bar));bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["n_value","r_value","k_value","swing_index","variation"]]).all(axis=1)&bars.r_value.ne(0)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="raw_close_change":side=pd.to_numeric(used.raw_close_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVWSI prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvwsi_z20_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvwsi_z20_24_source_support_v1", "policy_id": prereg.POLICY_ID,
