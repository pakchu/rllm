"""Build outcome-blind source support for frozen HVSTC-24."""
...
PREREG_SHA = "ffaa990b42548a0eef1389be70376ef63497ab7479096db418df8f5b27d7f415"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_schaff_trend_cycle_reentry_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_schaff_trend_cycle_reentry_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_schaff_trend_cycle_reentry_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_schaff_trend_cycle_reentry_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_schaff_trend_cycle_reentry_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "fast_ema", "slow_ema", "macd", "first_stochastic",
    "first_smooth", "second_stochastic", "stc", "entry_side", "macd_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "fast_ema", "slow_ema", "macd",
    "first_stochastic", "first_smooth", "second_stochastic", "stc", "entry_side",
    "macd_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVSTC source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVSTC invalid source key")
...
def stochastic(series: pd.Series, valid: pd.Series, period: int) -> pd.Series:
    lowest=series.rolling(period,min_periods=period).min();highest=series.rolling(period,min_periods=period).max();complete=valid.rolling(period,min_periods=period).sum().eq(period);spread=highest-lowest
    return (100.0*(series-lowest)/spread).where(complete&spread.gt(0))


def recursive_smooth(series: pd.Series, valid: pd.Series) -> pd.Series:
    factor=P["smoothing_factor"];output=np.full(len(series),np.nan);state=np.nan
    for i,(value,is_valid) in enumerate(zip(series.to_numpy(float),valid.to_numpy(bool))):
        if not is_valid or not math.isfinite(value):state=np.nan;continue
        state=value if not math.isfinite(state) else state+factor*(value-state);output[i]=state
    return pd.Series(output,index=series.index)


def threshold_side(signal: pd.Series) -> pd.Series:
    previous=signal.shift(1);side=np.zeros(len(signal),dtype=int);side[(signal>P["lower_reentry"])&(previous<=P["lower_reentry"])]=1;side[(signal<P["upper_reentry"])&(previous>=P["upper_reentry"])]=-1
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    fields=["bar_open","bar_high","bar_low","bar_close"];bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[fields]).all(axis=1)&bars[fields].gt(0).all(axis=1);bars["fast_ema"]=causal_ema(bars.bar_close,bars.valid_bar,P["fast_ema_periods"]);bars["slow_ema"]=causal_ema(bars.bar_close,bars.valid_bar,P["slow_ema_periods"]);bars["macd"]=bars.fast_ema-bars.slow_ema;macd_valid=np.isfinite(bars.macd)
    bars["first_stochastic"]=stochastic(bars.macd,macd_valid,P["cycle_periods"]);bars["first_smooth"]=recursive_smooth(bars.first_stochastic,np.isfinite(bars.first_stochastic));first_valid=np.isfinite(bars.first_smooth);bars["second_stochastic"]=stochastic(bars.first_smooth,first_valid,P["cycle_periods"]);bars["stc"]=recursive_smooth(bars.second_stochastic,np.isfinite(bars.second_stochastic));bars["entry_side"]=threshold_side(bars.stc)
    previous_macd=bars.macd.shift(1);bars["macd_side"]=np.where((bars.macd>0)&(previous_macd<=0),1,np.where((bars.macd<0)&(previous_macd>=0),-1,0))
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["fast_ema","slow_ema","macd","first_stochastic","first_smooth","second_stochastic","stc","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="raw_macd_zero_cross":side=pd.to_numeric(used.macd_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVSTC prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvstc_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvstc_24_source_support_v1", "policy_id": prereg.POLICY_ID,
