"""Evaluate frozen ESVRR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "ESVRR-12"
PROTOCOL = "esvrr_12_gross9_novelty_v1"
...
CLOCK_ROWS = 106
OUTPUT = Path("results/emerging_safe_haven_volatility_rotation_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def write_immutable(path: Path, content: bytes) -> None:
    ...
    if path.exists():
        if path.read_bytes() != content:
            raise RuntimeError(f"refusing to overwrite immutable ESVRR novelty result: {path}")
        return
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("ESVRR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("ESVRR novelty limits drift")
    if (
        ...
        raise RuntimeError("ESVRR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="ESVRR primary")
    if len(candidate) != CLOCK_ROWS:
        raise RuntimeError("ESVRR clock count drift")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "esvrr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject ESVRR-12 unchanged before economic outcomes",
    }
