"""Evaluate the frozen HVCPO-8 action-vote clock against Gross9."""
...
POLICY = "HVCPO-8"
PROTOCOL = "hvcpo_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_complementary_persistence_opposition_override_clocks_2023_2026.csv.gz",
    "sha256": "4d9a822a556781709ed0cde81425cf12e4bb60cfc2edde3247a6d6ee12c01e71",
    "rows": 373,
}
OUTPUT = Path("results/high_volatility_complementary_persistence_opposition_override_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCPO-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCPO-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCPO-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCPO-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCPO-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCPO-8 unchanged before economics",
    }
