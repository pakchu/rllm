"""Sequential strict economics for frozen HVKRECLV-24."""
...
PREREG = Path("results/high_volatility_regional_bank_close_location_relay_preregistration_2026-08-11.json")
PREREG_SHA = "a86e278fe212d64d58a45af97c5a3907dc77587d93bfaa4158de6bc0d334b4df"
SUPPORT = Path("results/high_volatility_regional_bank_close_location_relay_support_2026-08-11.json")
SUPPORT_SHA = "0b9af97e44b0843cd8950533e41e52175e6a85b0f02905b691885144428514d2"
NOVELTY = Path("results/high_volatility_regional_bank_close_location_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "a9e67efe7dad9f02d0e8de75e1291340c5ef9f678fd52fa2e4b6b60bb922d331"
CLOCK = Path("data/high_volatility_regional_bank_close_location_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "c5486ac3f0d8fbbcef01bcc8839dddb6916c7bacaf0a1ee36b4331bea27c631b"
CONTROL_DIR = Path("data/high_volatility_regional_bank_close_location_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_regional_bank_close_location_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_regional_bank_close_location_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVKRECLV frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVKRECLV novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvkreclv_24_sequential_economics_v1", "policy_id": POLICY_ID,
