#!/usr/bin/env python3
"""Frozen RV20 asymmetric liquidation-reversal candidate evaluator.
...
PREREGISTRATION = Path(
    "results/rv20_asymmetric_liquidation_reversal_preregistration_2026-08-08.json"
)
EXPECTED_PREREGISTRATION_SHA256 = (
    "832464ac5fee0d9585b3361d768b8a2ff40791e6f758501c466cbe6eade67e94"
)
OUTPUT = Path("results/rv20_asymmetric_liquidation_reversal_evaluation_2026-08-08.json")
REPORT = Path("docs/rv20-asymmetric-liquidation-reversal-evaluation-2026-08-08.md")
ATTEMPT = Path(
    "results/rv20_asymmetric_liquidation_reversal_evaluation_attempt_consumed_2026-08-08.json"
)
...
CANDIDATE = "rv20_asymmetric_liquidation_reversal"
...
def load_preregistration(path: str | Path = PREREGISTRATION) -> dict[str, Any]:
    ...
    if payload.get("schema") != "rllm.rv20-asymmetric-liquidation-reversal.preregistration.v1":
    ...
    if payload.get("fixed_signal") != {
        "absolute_hour_return_quantile": ABSOLUTE_HOUR_RETURN_QUANTILE,
        "capitulation_taker_fraction": CAPITULATION_TAKER_FRACTION,
        "feature_threshold_clock": "prior 2160 finite completed hours shifted one hour",
        "hour_range_quantile": HOUR_RANGE_QUANTILE,
        "long": "down shock, taker<=.48, OI change<=Q50",
        "oi_change_upper_quantile": OI_CHANGE_UPPER_QUANTILE,
        "short": "up shock, taker>=.52, OI change<=Q50",
    }:
        raise RuntimeError("fixed signal contract drifted")
    if len(frozen_grid(payload)) != 216:
...
def _execution_config(
    ...
    return ExecutionConfig(
        ...
        output="/tmp/no_write_rv20_asymmetric_liquidation_reversal.json",
        manifest_output="/tmp/no_write_rv20_asymmetric_liquidation_reversal_manifest.json",
...
def _install_candidate_sleeve() -> None:
    ...
    portfolio.FAMILIES[CANDIDATE] = "rv20_asymmetric_liquidation_reversal"
...
def evaluate_frozen_protocol(
    ...
    if top is None:
        return {
            "schema": "rllm.rv20-asymmetric-liquidation-reversal.evaluation.v1",
    ...
    if selected_leverage is None:
        return {
            "schema": "rllm.rv20-asymmetric-liquidation-reversal.evaluation.v1",
            "tested_structural_cells": 216, "structural_rows": structural_rows,
