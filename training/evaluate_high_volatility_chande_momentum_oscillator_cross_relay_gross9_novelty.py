"""Evaluate frozen HVCMO-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCMO-24"
PROTOCOL = "hvcmo_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_chande_momentum_oscillator_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCMO predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCMO novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCMO predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCMO primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcmo_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCMO-24 unchanged before economic outcomes",
    }
