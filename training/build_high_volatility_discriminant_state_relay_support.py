"""Build OOS source-only HVDLDA-24 clocks after the model freeze."""
...
from training import preregister_high_volatility_discriminant_state_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.long_regime_combo_scan import LongComboScanConfig, _load_market
from training.long_regime_interest_gate_validation import build_interest_features


PREREG_SHA = "eaeb58a45878b45d2ab24fdcbf3ca4ba2c5055dc853e51b45811d2890f8c63db"
MODEL_FREEZE = Path(
    "results/high_volatility_discriminant_state_relay_model_freeze_2026-08-09.json"
)
MODEL_FREEZE_SHA = "33154f7e8c65324b8b98199e3ceef5c269c09e318cdc9b5147507a6263598f3a"
MODEL = Path("data/high_volatility_discriminant_state_relay_model_2026-08-09.joblib")
MODEL_SHA = "00ada838da174f7b10fd5d466ea43181e417c003af3bc9ca6f8defed7ee684d9"
CLOCK = Path("data/high_volatility_discriminant_state_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_discriminant_state_relay_controls_2023_2026")
SNAPSHOT = Path(
    "data/high_volatility_discriminant_state_relay_sources_2023_2026/model_scores.csv.gz"
)
RESULT = Path(
    "results/high_volatility_discriminant_state_relay_support_2026-08-09.json"
)
...
CONTROLS = ("no_volatility_gate", "no_probability_tail_gate", "one_anchor_stale_features", "direction_flip")
...
def verify_predecessors() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVDLDA preregistration drift")
    if sha256(MODEL_FREEZE) != MODEL_FREEZE_SHA or sha256(MODEL) != MODEL_SHA:
        raise RuntimeError("HVDLDA model predecessor drift")
    ...
    if freeze.get("manifest_hash") != canonical_hash(freeze_core):
        raise RuntimeError("HVDLDA model freeze manifest drift")
    if (
        ...
        raise RuntimeError("HVDLDA model freeze state drift")
    artifact = joblib.load(MODEL)
    if artifact.get("policy_id") != "HVDLDA-24":
        raise RuntimeError("HVDLDA model policy drift")
...
def load_combined_market() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if combined.date.duplicated().any() or not combined.date.is_monotonic_increasing:
        raise RuntimeError("HVDLDA combined market time drift")
    deltas = combined.date.diff().dropna()
    if not deltas.eq(pd.Timedelta(minutes=5)).all():
        raise RuntimeError("HVDLDA combined market is not continuous 5m")
    return combined, {
...
def score_snapshot(market: pd.DataFrame, artifact: dict[str, Any]) -> pd.DataFrame:
    ...
    if missing:
        raise RuntimeError(f"HVDLDA OOS feature drift: {missing}")
...
def build_clock(scores: pd.DataFrame, artifact: dict[str, Any], control: str = "primary") -> pd.DataFrame:
    ...
    if control == "no_probability_tail_gate":
        long_signal = probability >= 0.5
        short_signal = probability < 0.5
    else:
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvlda_24_oos_source_support_v1",
        "policy_id": "HVDLDA-24",
