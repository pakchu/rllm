"""Sequential strict economics for frozen HVKST-24."""
...
PREREG = Path("results/high_volatility_know_sure_thing_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "edac89d16ce5d95622bac6b5c97a3c3d34f8bf73a8c7ad12f0ea9033eb3e441c"
SUPPORT = Path("results/high_volatility_know_sure_thing_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "a6bed7f25f78b58ea79b65ca075fa931d7618009a64c67618f83d35fb3bbafb2"
NOVELTY = Path("results/high_volatility_know_sure_thing_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "382d5e9ab5d6bdca247f1c54f0dd9ed47b6dfd64ac63d842cc49367b08e3e451"
CLOCK = Path("data/high_volatility_know_sure_thing_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "74a2234d303b06fc08fe1a84e9354dd65a54b8bef42365a06b051f957cd69b2b"
CONTROL_DIR = Path("data/high_volatility_know_sure_thing_crossover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_know_sure_thing_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_know_sure_thing_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVKST frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVKST novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvkst_24_sequential_economics_v1", "policy_id": POLICY_ID,
