"""Strict staged economics for the singleton HVCVAIR-8 policy."""
...
PREREG = Path("results/high_volatility_causal_volume_acceleration_inventory_relay_preregistration_2026-08-16.json")
PREREG_SHA = "4fcba823ba0fd2c28d2e7697443be7d10b9b99d7a716ec0ef008a60263da97f3"
SUPPORT = Path("results/high_volatility_causal_volume_acceleration_inventory_relay_support_2026-08-16.json")
SUPPORT_SHA = "3bf28467e483538569ba986f3420683a3577141fca23345dd74ab5f9d5453c15"
GROSS9 = Path("results/high_volatility_causal_volume_acceleration_inventory_relay_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "12aa784c4d143e1dc9a14fc0238d612c4f055730881c44fb94de22902563fa6b"
CLOCK = {
    "path": Path("data/high_volatility_causal_volume_acceleration_inventory_relay_clocks_2020_2026.csv.gz"),
    "sha256": "8e099848add857b43b178f71ffbc90062303085125c1551f80019510a06a3c14",
    "rows": 187,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_volume_acceleration_inventory_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCVAIR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCVAIR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCVAIR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCVAIR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVAIR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVAIR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCVAIR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCVAIR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCVAIR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCVAIR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCVAIR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcvair_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
