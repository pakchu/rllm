"""Evaluate frozen HVDEC-24 structural novelty against every Gross9 sleeve."""
...
from training import preregister_high_volatility_ehlers_decycler_oscillator_zero_cross_relay as registration

POLICY = "HVDEC-24"
PROTOCOL = "hvdec_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_ehlers_decycler_oscillator_zero_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDEC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVDEC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDEC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDEC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdec_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVDEC-24 unchanged before economic outcomes",
    }
