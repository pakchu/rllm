"""Evaluate frozen CLWMSR-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "CLWMSR-6"
PROTOCOL = "clwmsr_6_gross9_novelty_v1"
...
OUTPUT = Path("results/confirmation_ladder_witness_migration_sponsorship_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CLWMSR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("CLWMSR novelty limits drift")
    if (
        ...
        raise RuntimeError("CLWMSR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CLWMSR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "clwmsr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject CLWMSR-6 unchanged before economic outcomes",
    }
