"""Sequential strict economics for frozen HVTFDAC-8."""
...
PREREG = Path("results/high_volatility_taker_flow_decay_acceptance_continuation_preregistration_2026-08-16.json")
PREREG_SHA = "8b4561990ea6e8e600949c9d833babed3eaf30f651e4a22dd1ef474ee18df27e"
SUPPORT = Path("results/high_volatility_taker_flow_decay_acceptance_continuation_support_2026-08-16.json")
SUPPORT_SHA = "b4b13ab59cc144e5de613982da3594f5e0e9dba8f76019dad401b910b06c9477"
NOVELTY = Path("results/high_volatility_taker_flow_decay_acceptance_continuation_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "9f5d7a4359cf10fb3d55d8112f1f1a6df2f88d13a8c64fb6f6463392b44a48f4"
CLOCK = Path("data/high_volatility_taker_flow_decay_acceptance_continuation_clocks_2023_2026.csv.gz")
CLOCK_SHA = "82ecf1de318b3944481a05f09a8a90c61ab3aa0e76e8188b27073d3086888267"
CONTROL_DIR = Path("data/high_volatility_taker_flow_decay_acceptance_continuation_controls_2023_2026")
FREEZE = Path("results/high_volatility_taker_flow_decay_acceptance_continuation_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_taker_flow_decay_acceptance_continuation_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTFDAC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTFDAC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvtfdac_8_sequential_economics_v1", "policy_id": POLICY_ID,
