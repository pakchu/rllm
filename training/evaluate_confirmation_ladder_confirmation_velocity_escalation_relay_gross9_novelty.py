"""Evaluate frozen CLCVER-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "CLCVER-6"
PROTOCOL = "clcver_6_gross9_novelty_v1"
...
OUTPUT = Path("results/confirmation_ladder_confirmation_velocity_escalation_relay_gross9_novelty_2026-08-12.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("CLCVER predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("CLCVER novelty limits drift")
    if (
        ...
        raise RuntimeError("CLCVER predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="CLCVER primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "clcver_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject CLCVER-6 unchanged before economic outcomes",
    }
