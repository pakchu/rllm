"""Evaluate frozen HVCAPO-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCAPO-8"
PROTOCOL = "hvcapo_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_cross_alt_path_ordered_excursion_dominance_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCAPO predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCAPO novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCAPO predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCAPO primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCAPO-8 unchanged before economic outcomes",
    }
