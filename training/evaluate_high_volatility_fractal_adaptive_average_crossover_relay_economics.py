"""Sequential strict economics for frozen HVFRAMA-24."""
...
PREREG = Path("results/high_volatility_fractal_adaptive_average_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "97e490eab84e5a042ad90ec40b5f44f45c282526363167ed692052bd881d96aa"
SUPPORT = Path("results/high_volatility_fractal_adaptive_average_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "4c18dbb35f3991654ebfbd0a4840053e1f8c3c0715a76dcc8641804c8b663256"
NOVELTY = Path("results/high_volatility_fractal_adaptive_average_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "0ad3ceb39172c5d585671cd0afffdf7d9e9d3b2076c9c59b6b8aa0add640f0d7"
CLOCK = Path("data/high_volatility_fractal_adaptive_average_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "6e4357eec658b1112fe6e60d6e5a730dc5b863b0b1b7607442bf1eae7cfcb425"
CONTROL_DIR = Path("data/high_volatility_fractal_adaptive_average_crossover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_fractal_adaptive_average_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_fractal_adaptive_average_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "fixed_ema_crossover", "one_bar_stale_crossover", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVFRAMA frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVFRAMA novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvframa_24_sequential_economics_v1", "policy_id": POLICY_ID,
