"""Sequential strict economics for frozen HVDFXPR-8."""
...
PREREG = Path("results/high_volatility_directed_fx_propagation_relay_preregistration_2026-08-10.json")
PREREG_SHA = "91075e5ad86381b69dc772a7c4d1e9ca3b02944d8f17eca95e1f0d002b9947f2"
SUPPORT = Path("results/high_volatility_directed_fx_propagation_relay_support_2026-08-10.json")
SUPPORT_SHA = "8e2ba15bc1b1386775c9ef873da51fe90dde7b93a067d02b605f0ccd60ad604e"
NOVELTY = Path("results/high_volatility_directed_fx_propagation_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "744572ab7c69cac41c623bbb60e2975f0e2976c3422d231f097042d7d3559e42"
CLOCK = Path("data/high_volatility_directed_fx_propagation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "3362a02d73681666e7f617a865ab7f9c914155daada6153e74cb0a69309a6fdd"
CONTROL_DIR = Path("data/high_volatility_directed_fx_propagation_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_directed_fx_propagation_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_directed_fx_propagation_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "no_source_strength_tail", "no_breadth_gate", "one_session_stale_network", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDFXPR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDFXPR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdfxpr_12_sequential_economics_v1", "policy_id": POLICY_ID,
