"""Evaluate the frozen HVERDR-24 action-vote clock against Gross9."""
...
POLICY = "HVERDR-24"
PROTOCOL = "hverdr_24_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_equity_duration_rotation_relay_clocks_2023_2026.csv.gz",
    "sha256": "6e6766208faee2465d24becc413ef19542c77179472ac7b0e83f867963d4b178",
    "rows": 132,
}
OUTPUT = Path("results/high_volatility_equity_duration_rotation_relay_gross9_novelty_2026-08-18.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVERDR-24 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVERDR-24 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVERDR-24 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVERDR-24 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVERDR-24 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVERDR-24 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVERDR-24 unchanged before economics",
    }
