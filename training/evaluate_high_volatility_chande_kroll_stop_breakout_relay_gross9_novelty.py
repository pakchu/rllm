"""Evaluate frozen HVCKS-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCKS-24"
PROTOCOL = "hvcks_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_chande_kroll_stop_breakout_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCKS predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCKS novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCKS predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCKS primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcks_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCKS-24 unchanged before economic outcomes",
    }
