"""Build source-only HVDVBR-12 clocks before opening outcomes or Gross9."""
...
from training import preregister_high_volatility_dominant_volume_bar_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv


PREREG_SHA = "8f9d6445fad819a351ff1cac9c7e1ddc7dec0f713f7425970c30bc4497731176"
...
CLOCK = Path("data/high_volatility_dominant_volume_bar_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_dominant_volume_bar_relay_controls_2023_2026")
SNAPSHOT = Path(
    "data/high_volatility_dominant_volume_bar_relay_sources_2023_2026/"
    "dominant_volume_bar_scores.csv.gz"
)
RESULT = Path("results/high_volatility_dominant_volume_bar_relay_support_2026-08-09.json")
SPLITS = {
...
def _five_minute_extension(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    if frame.date.duplicated().any():
        raise RuntimeError("HVDVBR live one-minute source contains duplicate timestamps")
    frame = frame.sort_values("date").set_index("date")
    expected = pd.date_range(EXTENSION_START, END, freq="1min", inclusive="left")
    if not frame.index.equals(expected):
        raise RuntimeError("HVDVBR live one-minute source is not an exact consecutive grid")
    ...
    if not bool((coherent & quote_valid).all()):
        raise RuntimeError("HVDVBR live one-minute source validity drift")
    ...
    if extension.empty or not extension.source_rows.eq(5).all():
        raise RuntimeError("HVDVBR live extension is not a complete one-minute-to-five-minute grid")
...
def load_combined_market() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if combined.empty or combined.date.duplicated().any():
        raise RuntimeError("HVDVBR combined market identity drift")
    if not combined.date.diff().dropna().eq(pd.Timedelta(minutes=5)).all():
        raise RuntimeError("HVDVBR combined market continuity drift")
    return combined, {
...
def _score_anchor(window: pd.DataFrame) -> dict[str, float]:
    invalid = {
        ...
        "dominant_return_last": float("nan"),
        "dominant_return_first": float("nan"),
    ...
    if not np.isfinite(quote).all() or (quote < 0).any() or quote.sum() <= 0:
        return invalid
    maximum = quote.max()
    ties = np.flatnonzero(quote == maximum)
    first = block.iloc[int(ties[0])]
    last = block.iloc[int(ties[-1])]
    range_high = float(window.high.max())
    range_low = float(window.low.min())
    midpoint = 0.5 * (range_high + range_low)
    return {
        "dominant_share": float(maximum / quote.sum()),
        "dominant_return_last": float(np.log(float(last.close) / float(last.open))),
        "dominant_return_first": float(np.log(float(first.close) / float(first.open))),
...
def score_snapshot(market: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, float]]:
    ...
    if len(calibration) < 4_000:
        raise RuntimeError("HVDVBR source-only calibration floor failed")
    thresholds = {
        "dominant_share_q85": float(calibration.dominant_share.quantile(0.85)),
...
def conditions(
    ...
    if control not in ("primary", *CONTROLS):
        raise ValueError(f"unknown HVDVBR control: {control}")
    dominant_feature = (
        "dominant_return_first" if control == "first_dominant_tie_break" else "dominant_return_last"
    )
    share = pd.to_numeric(scores.dominant_share, errors="coerce").to_numpy(float)
    volatility = pd.to_numeric(scores.range_vol, errors="coerce").to_numpy(float)
    dominant_return = pd.to_numeric(scores[dominant_feature], errors="coerce").to_numpy(float)
    block_return = pd.to_numeric(scores.block_return, errors="coerce").to_numpy(float)
    dominant_side = np.sign(dominant_return)
    block_side = np.sign(block_return)
    agreement = (dominant_side != 0) & (dominant_side == block_side)
    if control == "block_direction_only":
        agreement = block_side != 0
    finite = (
        np.isfinite(share)
        & np.isfinite(volatility)
        & np.isfinite(dominant_return)
        & np.isfinite(block_return)
    )
    volatility_gate = volatility >= thresholds["range_vol_q60"]
    if control == "no_volatility_gate":
        volatility_gate[:] = True
    eligible = finite & (share >= thresholds["dominant_share_q85"]) & volatility_gate & agreement
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVDVBR preregistration drift")
    ...
    core = {
        "protocol_version": "hvdvbr_12_source_support_v1",
        "policy_id": "HVDVBR-12",
        "preregistration": {
        ...
        "feature_contract": {
            ...
            **thresholds,
            "dominant_tie_break": "last_maximum_quote_asset_volume_row",
