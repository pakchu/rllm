"""Sequential strict economics for frozen HVOILTJ-8."""
...
PREREG = Path("results/high_volatility_oi_large_ticket_joint_sponsorship_preregistration_2026-08-16.json")
PREREG_SHA = "3496b0b54c186361d37b40d479bba8dac88070bac638393274b1bf7f2d1aa1eb"
SUPPORT = Path("results/high_volatility_oi_large_ticket_joint_sponsorship_support_2026-08-16.json")
SUPPORT_SHA = "d7bdabd59bd7250af8e7acb0977d6f5bf32edbdde26c168704421322db42e1ca"
NOVELTY = Path("results/high_volatility_oi_large_ticket_joint_sponsorship_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "b79c04876e4b969cbffffcc408a00dd8d19716ed7e1479f268c7177fce298614"
CLOCK = Path("data/high_volatility_oi_large_ticket_joint_sponsorship_clocks_2023_2026.csv.gz")
CLOCK_SHA = "1f3b8a6bd8127930da5c6123845b6a3f1a68ae86e76dcccdb9d01e0d1380a876"
CONTROL_DIR = Path("data/high_volatility_oi_large_ticket_joint_sponsorship_controls_2023_2026")
FREEZE = Path("results/high_volatility_oi_large_ticket_joint_sponsorship_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_oi_large_ticket_joint_sponsorship_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVOILTJ frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVOILTJ novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvoiltj_8_sequential_economics_v1", "policy_id": POLICY_ID,
