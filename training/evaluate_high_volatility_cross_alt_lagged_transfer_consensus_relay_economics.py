"""Sequential strict economics for frozen HVCALT-6."""
...
PREREG = Path("results/high_volatility_cross_alt_lagged_transfer_consensus_relay_preregistration_2026-08-13.json")
PREREG_SHA = "0b32ed107a661dbec816ad117e047b6ae75783a6026216c2dfd8b6444bd98943"
SUPPORT = Path("results/high_volatility_cross_alt_lagged_transfer_consensus_relay_support_2026-08-13.json")
SUPPORT_SHA = "6775a9a2df3767f7fbc1962f7c7ee170a3c125165aeaa4d426082ddd405acf3e"
NOVELTY = Path("results/high_volatility_cross_alt_lagged_transfer_consensus_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "d55f949bef33c240ac83a7ab3d126e84f6632687f8382824fc4f860ef5ec5225"
CLOCK = Path("data/high_volatility_cross_alt_lagged_transfer_consensus_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "196532f2c7aa56d820a8a53107aafdb2e7956b6513c865face45811dafe5f8a3"
CONTROL_DIR = Path("data/high_volatility_cross_alt_lagged_transfer_consensus_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_cross_alt_lagged_transfer_consensus_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_alt_lagged_transfer_consensus_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "no_beta_tail", "three_alt_consensus", "contemporaneous_beta", "one_decision_stale_transfer", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCALT frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCALT novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcalt_6_sequential_economics_v1", "policy_id": POLICY_ID,
