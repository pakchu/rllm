"""Evaluate frozen TECC-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "TECC-12"
PROTOCOL = "tecc_12_gross9_novelty_v1"
...
OUTPUT = Path("results/ticket_expansion_concordance_continuation_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("TECC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("TECC novelty limits drift")
    if (
        ...
        raise RuntimeError("TECC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="TECC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "tecc_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject TECC-12 unchanged before economic outcomes",
    }
