"""Evaluate frozen HVPRF-48 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVPRF-48"
PROTOCOL = "hvprf_48_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_path_utility_random_forest_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVPRF predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVPRF novelty limits drift")
    if (
        ...
        raise RuntimeError("HVPRF predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVPRF primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvprf_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVPRF-48 unchanged before economic outcomes"
        ),
    }
