"""Outcome-blind source support for frozen HVCASPC-8."""
...
from training import preregister_high_volatility_cross_alt_serial_persistence_consensus_relay as prereg
ENV_FILE='/home/pakchu/rllm/.env';START=pd.Timestamp('2023-01-01T00:00:00Z');END=pd.Timestamp('2026-08-01T00:00:00Z');PREREG_SHA='be77dfa8d83dfcb54171da4cf3263c57336fc629dd8f7b63ff8e8b33a860784a';REG=prereg.build();P=REG['policy'];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG['stages'].items()};GATES=REG['source_support_gates'];CONTROLS=tuple(REG['diagnostic_controls']['names']);SYMBOLS=('BTCUSDT',*prereg.ALTS)
...
ROOT=Path('data/high_volatility_cross_alt_serial_persistence_consensus_relay_sources_2023_2026');PANEL=ROOT/'block_states.csv.gz';MANIFEST=ROOT/'manifest.json';CLOCK=Path('data/high_volatility_cross_alt_serial_persistence_consensus_relay_clocks_2023_2026.csv.gz');SPLIT_DIR=Path('data/high_volatility_cross_alt_serial_persistence_consensus_relay_split_clocks_2023_2026');CONTROL_DIR=Path('data/high_volatility_cross_alt_serial_persistence_consensus_relay_controls_2023_2026');RESULT=Path('results/high_volatility_cross_alt_serial_persistence_consensus_relay_support_2026-08-13.json');BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=('decision_time','feature_available_time','source_valid','consensus_side','consensus_breadth','consensus_strength','broad_side','broad_breadth','negative_side','negative_breadth','negative_strength','btc_realized_variation','variation_rank','eligible');CLOCK_COLUMNS=('candidate','control','split','decision_time','feature_available_time','entry_time','exit_time','side','consensus_breadth','consensus_strength','btc_realized_variation','variation_rank')
...
def prepare(raw):
 ...
 if raw.columns.tolist()!=required:raise RuntimeError('HVCASPC source schema drift')
 x=raw.copy()
 ...
 if x[['block_start','symbol']].isna().any().any() or x.duplicated(['block_start','symbol']).any():raise RuntimeError('HVCASPC invalid key')
...
def build_panel(raw):
 x=prepare(raw);decisions=pd.date_range(START+pd.Timedelta('3h'),END,freq='8h',inclusive='left');full=x.reindex(pd.MultiIndex.from_product([decisions,SYMBOLS],names=['decision_time','symbol']));valid=full.row_valid.unstack('symbol').reindex(columns=SYMBOLS);disp=full.displacement.unstack('symbol').reindex(columns=SYMBOLS);pers=full.serial_persistence.unstack('symbol').reindex(columns=SYMBOLS);sq=full.minute_squared_return.unstack('symbol').reindex(columns=SYMBOLS);ranks=pd.DataFrame(index=decisions,columns=prereg.ALTS,dtype=float);negative_ranks=pd.DataFrame(index=decisions,columns=prereg.ALTS,dtype=float)
 ...
 for d in decisions:
  ok=bool(valid.loc[d].eq(True).all() and np.isfinite(ranks.loc[d]).all() and math.isfinite(float(btcvar.loc[d])) and btcvar.loc[d]>0);sign=np.sign(disp.loc[d,list(prereg.ALTS)].to_numpy(float));pv=pers.loc[d,list(prereg.ALTS)].to_numpy(float);rr=ranks.loc[d].to_numpy(float);nr=negative_ranks.loc[d].to_numpy(float);side,breadth,st=consensus(sign,rr,pv>0,P['persistence_rank_min']) if ok else (0,0,math.nan);bside,bbreadth,_=consensus(sign,np.ones(6),pv>0,0.) if ok else (0,0,math.nan);nside,nbreadth,nst=consensus(sign,nr,pv<0,P['persistence_rank_min']) if ok else (0,0,math.nan);rows.append({'decision_time':d,'feature_available_time':d,'source_valid':ok,'consensus_side':side,'consensus_breadth':breadth,'consensus_strength':st,'broad_side':bside,'broad_breadth':bbreadth,'negative_side':nside,'negative_breadth':nbreadth,'negative_strength':nst,'btc_realized_variation':float(btcvar.loc[d]) if ok else math.nan})
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError('HVCASPC prereg drift')
