"""Evaluate frozen HVTAB-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVTAB-6"
PROTOCOL = "hvtab_6_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_trade_arrival_backloading_relay_preregistration_2026-08-10.json"
)
...
OUTPUT = Path(
    "results/high_volatility_trade_arrival_backloading_relay_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTAB predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVTAB novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTAB predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTAB primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvtab_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVTAB-6 unchanged before economic outcomes"
        ),
    }
