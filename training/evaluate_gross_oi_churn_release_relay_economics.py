"""Sequential strict economics for frozen GOICR-12."""
...
PREREG = Path("results/gross_oi_churn_release_relay_preregistration_2026-08-09.json")
PREREG_SHA = "3acdda0abf96825f938e96c383649aa56c0cd146474cc9e17cfc947698747b1c"
SUPPORT = Path("results/gross_oi_churn_release_relay_support_2026-08-09.json")
SUPPORT_SHA = "0763d143684dc1db733024c4e8a0413146d852ddce67c9f77af8c18a6379df71"
NOVELTY = Path("results/gross_oi_churn_release_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "e41b729c22c19092e6bf8f2f1768d975717a5f3da9a0378b86f04703ffccc073"
CLOCK = Path("data/gross_oi_churn_release_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "bb3f50c9db5edeb360bfad3dcd0693920b676ac84d8151bb3386b75c364bda08"
CONTROL_DIR = Path("data/gross_oi_churn_release_relay_controls_2023_2026")
FREEZE = Path("results/gross_oi_churn_release_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/gross_oi_churn_release_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("net_oi_tail_only", "gross_churn_without_cancellation", "late_price_escape_only", "one_decision_stale_churn", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("GOICR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("GOICR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "goicr_12_sequential_economics_v1", "policy_id": POLICY_ID,
