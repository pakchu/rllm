"""Sequential strict economics for frozen HVEXDP-24."""
...
NOVELTY = Path("results/high_volatility_exchange_deposit_pressure_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "ab3a05c9e4a5fb6b1c3b2c599f555f3e53b8a307c122e65248d8698322891efc"
...
FREEZE = Path("results/high_volatility_exchange_deposit_pressure_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_exchange_deposit_pressure_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = (
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVEXDP frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVEXDP novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvexdp_24_sequential_economics_v1", "policy_id": POLICY_ID,
