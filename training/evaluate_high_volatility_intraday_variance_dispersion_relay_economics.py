"""Sequential strict economics for frozen HVIVDR-8."""
...
PREREG = Path("results/high_volatility_intraday_variance_dispersion_relay_preregistration_2026-08-12.json")
PREREG_SHA = "7193b15406e0d88266e61fe81131993350e17a2b7a1ee448b20b56f1abd23328"
SUPPORT = Path("results/high_volatility_intraday_variance_dispersion_relay_support_2026-08-12.json")
SUPPORT_SHA = "8475fbf443ee133fa757dae5440b5a7dee82fbbcfd2c20d8315b77ecae5f7649"
NOVELTY = Path("results/high_volatility_intraday_variance_dispersion_relay_gross9_novelty_2026-08-12.json")
NOVELTY_SHA = "ab501c086480be32733f2b43358dacbab33f4650e3f408b4f910070cd5ec88bb"
CLOCK = Path("data/high_volatility_intraday_variance_dispersion_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "05d37053dde59552140cc26ccd841a9a942b32d90c14e05cf1fd179b52486aa6"
CONTROL_DIR = Path("data/high_volatility_intraday_variance_dispersion_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_intraday_variance_dispersion_relay_economic_evaluator_freeze_2026-08-12.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_intraday_variance_dispersion_relay_{stage}_economics_2026-08-12.json")
    for stage in STAGES
}
CONTROLS = ("no_dispersion_tail", "no_variation_gate", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVIVDR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVIVDR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvivdr_8_sequential_economics_v1", "policy_id": POLICY_ID,
