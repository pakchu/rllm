"""Evaluate frozen HVRSVRP-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVRSVRP-12"
PROTOCOL = "hvrsvrp_12_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_relative_signed_variation_risk_premium_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVRSVRP predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVRSVRP novelty limits drift")
    if (
        ...
        raise RuntimeError("HVRSVRP predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVRSVRP primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvrsvrp_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVRSVRP-12 unchanged before economic outcomes",
    }
