"""Sequential strict economics for frozen CVBRPCR-12."""
...
POLICY_ID = "CVBRPCR-12"
BAR = pd.Timedelta(minutes=5)
LEVERAGE = 0.5
BASE_COST = 0.0006
STRESS_COST = 0.0010
ENV_FILE = "/home/pakchu/rllm/.env"
PREREG = Path("results/crypto_volatility_beta_residual_price_confirmation_relay_preregistration_2026-08-08.json")
PREREG_SHA = "9b14278971461b43da9875d39a5cfef3c9e84e55b58d1a77f4ad1e0db2165a69"
SUPPORT = Path("results/crypto_volatility_beta_residual_price_confirmation_relay_support_2026-08-08.json")
SUPPORT_SHA = "06594aff2d5359375000eb930e7e62f238fb085e12e766f2e09c1172a6379a10"
NOVELTY = Path("results/crypto_volatility_beta_residual_price_confirmation_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "cf877c1035e1d447ead435b55cbbed11f352a901b84b7faf66c6d5e03618fa03"
CLOCK = Path("data/crypto_volatility_beta_residual_price_confirmation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "4f050edad500738343edb60912992c1550671ff1dcd77eceedaa19dcde4dc509"
CONTROL_DIR = Path("data/crypto_volatility_beta_residual_price_confirmation_relay_controls_2023_2026")
FREEZE = Path("results/crypto_volatility_beta_residual_price_confirmation_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/crypto_volatility_beta_residual_price_confirmation_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
CONTROLS = ("raw_dvol_change", "fixed_beta_one", "no_residual_gate", "residual_direction", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CVBRPCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CVBRPCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "cvbrpcr_12_sequential_economics_v1", "policy_id": POLICY_ID,
