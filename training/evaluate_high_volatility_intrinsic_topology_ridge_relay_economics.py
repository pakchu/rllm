"""Sequential strict economics for frozen HVITR-8."""
...
PREREG = Path("results/high_volatility_intrinsic_topology_ridge_relay_preregistration_2026-08-10.json")
PREREG_SHA = "a5f81522864782a8b88a286abf33039e35995f9776518c9379ae44fdda488c7c"
SUPPORT = Path("results/high_volatility_intrinsic_topology_ridge_relay_support_2026-08-10.json")
SUPPORT_SHA = "677efab584ed159019e8c0cf2396cb6ae440aaa98d4be0ac62dd86b51eb9ce48"
NOVELTY = Path("results/high_volatility_intrinsic_topology_ridge_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "3001c955f196b158f6417fdeed88c8f1dbba3e5b6f50d095e839807e9ef4473e"
CLOCK = Path("data/high_volatility_intrinsic_topology_ridge_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "f5c166f1b0c7826147fa004e4d36a5fe57d1fa3414e6a69d77dc21d371ba496e"
CONTROL_DIR = Path("data/high_volatility_intrinsic_topology_ridge_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_intrinsic_topology_ridge_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_intrinsic_topology_ridge_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "no_prediction_strength_gate", "one_boundary_stale_features", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVITR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVITR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvitr_8_sequential_economics_v1", "policy_id": POLICY_ID,
