"""Outcome-blind source-support gate for frozen HVLOIPSR-8."""
...
PREREG_SHA = "73b7020c09b116d072c7b237f300ee5787a6556bcdd444354149529e2346f80b"
...
"""
SOURCE_DIR = Path(
    "data/high_volatility_late_oi_participation_sponsorship_relay_sources_2023_2026"
)
FEATURES = SOURCE_DIR / "eight_hour_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path(
    "data/high_volatility_late_oi_participation_sponsorship_relay_clocks_2023_2026.csv.gz"
)
CONTROL_DIR = Path(
    "data/high_volatility_late_oi_participation_sponsorship_relay_controls_2023_2026"
)
RESULT = Path(
    "results/high_volatility_late_oi_participation_sponsorship_relay_support_2026-08-10.json"
)
FEATURE_COLUMNS = (
    "decision_time",
...
def build_features(bars: pd.DataFrame, oi: pd.DataFrame) -> pd.DataFrame:
    ...
    if not required_bars.issubset(bars) or not required_oi.issubset(oi):
        raise ValueError("HVLOIPSR source schema drift")
    prices = bars.copy()
    inventory = oi.copy()
    ...
    decisions = pd.date_range(
        START + pd.Timedelta(hours=8), END, freq="8h", inclusive="left"
    )
    for decision in decisions:
        ...
        completed_return = variation = gross_activity = late_share = math.nan
        if valid:
            bar_returns = np.log(
            ...
            late_share = (
                float(np.abs(oi_changes[-24:]).sum() / gross_activity)
                if gross_activity > 0
                else math.nan
            )
            ...
            valid = bool(
                ...
                and np.isfinite(
                    [completed_return, variation, gross_activity, late_share]
                ).all()
            )
        if not valid:
            completed_return = variation = gross_activity = late_share = math.nan
        rows.append(
            {
                ...
                "late_oi_activity_share": late_share,
            }
        )
    ...
    frame["late_oi_activity_share_rank"] = strict_prior_midrank(
        frame["late_oi_activity_share"].where(frame["source_valid"])
    )
...
def conditions(
    ...
    late_gate = (
        pd.Series(True, index=frame.index)
        if control == "no_late_oi_activity_share_gate"
        else used["late_oi_activity_share_rank"].ge(0.75)
    )
    variation_gate = (
        pd.Series(True, index=frame.index)
    ...
    eligible = valid & gross_gate & late_gate & variation_gate
    onset = (
        eligible
...
def run() -> dict[str, Any]:
    ...
    source_core = {
        "protocol_version": "hvloipsr_8_sources_v1",
        "query_sha256": {
    ...
    core = {
        "protocol_version": "hvloipsr_8_source_support_v1",
