"""Outcome-blind preregistration for HVCATEIR-8."""
...
POLICY_ID = "HVCATEIR-8"
DEFAULT_OUTPUT = Path("results/high_volatility_causal_average_ticket_efficiency_inventory_relay_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_causal_average_ticket_efficiency_inventory_relay_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": "When BTC perpetual average quote ticket size accelerates unusually, the completed eight-hour path has unusually high directional efficiency, realized variation is elevated, and open interest expands, large transactions and fresh leveraged inventory sponsor continuation of the efficient completed direction for eight hours.",
            "side": "strict sign of the completed eight-hour BTC perpetual return",
            "why_distinct": "HVCATEIR combines average-ticket acceleration with an independently ranked endpoint-to-total-variation path-efficiency condition, exact OI expansion, and realized variation. No prior composite event set or control is reused; it uses no taker-flow sign, spot, alt, funding, premium-index, fitted outcome, or promoted control.",
        ...
        "features": {
            ...
            "path_efficiency": "abs(sum minute log returns)/sum abs(minute log returns), finite in (0,1] with strict positive denominator",
            "path_efficiency_rank": "strict-prior midrank over at most 270 source-valid cycles; current excluded; minimum 180; rank>=0.60",
            ...
            "eligible": "exact coherent completed perpetual cycle, positive average-ticket acceleration, nonzero completed return, two exact OI endpoints with strict expansion, and average-ticket-acceleration, path-efficiency, and variation ranks pass at D; no taker-flow sign, spot source, funding, range, premium-index, alt, or post-decision condition",
            "no_imputation": True,
        },
        "clock": {
        ...
        "stopping_rule": "terminal first failure; no OI endpoint or expansion rule, average-ticket acceleration formula, path-efficiency formula, rank histories, side, clock, hold, subset, threshold, comparator, or control repair",
    }
