#!/usr/bin/env python3
"""Fit PSIM S5 residual Ridge FQI and gate its 2021 schedules.
...
from training import psim_direction_residual_fqi as residual  # noqa: E402
from training import psim_semantic_feature_matrix as features  # noqa: E402


PROTOCOL_VERSION = (
    "psim_d8_rllm2_s5_train2020_direction_residual_ridge_fqi_v1"
)
ATTEMPT_PROTOCOL_VERSION = (
    "psim_d8_rllm2_s5_train2020_direction_residual_ridge_fqi_attempt_v1"
)
SCHEDULE_PROTOCOL_VERSION = (
    "psim_d8_rllm2_s5_direction_residual_ridge_fqi_schedule_gate_v1"
)
...
def assert_committed_clean_runner() -> tuple[str, str]:
    ...
    if (
        ...
        raise RuntimeError(
            "PSIM S5 runner requires clean HEAD equal to origin/main"
        )
...
def validate_preregistration() -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("PSIM S5 preregistration file changed")
    ...
    if (
        ...
        raise RuntimeError("PSIM S5 preregistration contract changed")
    return payload
...
def _write_once(path: str | Path, raw: bytes) -> None:
    ...
    if target.exists():
        if (
            ...
            raise RuntimeError(f"PSIM S5 write-once artifact drift: {path}")
        return
...
def _load_frozen_pca(train_indices: np.ndarray) -> features.FrozenPCA:
    ...
    with np.load(target, allow_pickle=False) as payload:
        ...
        if set(payload.files) != required:
            raise RuntimeError("PSIM S5 frozen PCA keys changed")
    ...
    if (
        ...
        raise RuntimeError("PSIM S5 frozen PCA content changed")
...
def _verify_source_artifacts() -> None:
    for path, expected in (
        ...
        (
            ...
            prereg.S4_TRANSITION_LEDGER_SHA256,
        ),
        (prereg.s4.PCA_PATH, prereg.S4_PCA_SHA256),
        (
            prereg.s4.SCHEDULE_PATH,
            prereg.S4_BASE_SCHEDULES_SHA256,
        ),
    ):
        if _sha256_file(path) != expected:
            raise RuntimeError(f"PSIM S5 frozen input changed: {path}")
    ...
    *,
    ...
    core = {
        ...
        "input_bindings": {
            ...
            "pca32_2020_sha256": prereg.S4_PCA_SHA256,
...
def _verify_existing_result(
    ...
    if target.is_symlink() or not target.is_file():
        raise RuntimeError("PSIM S5 result path is unsafe")
    payload = json.loads(target.read_text(encoding="utf-8"))
    core = {
        key: value
        for key, value in payload.items()
    ...
    expected_action = (
        prereg.build_preregistration()["pre2021_schedule_readiness_gate"][
    ...
    if (
        ...
        raise RuntimeError("PSIM S5 existing result changed")
    for name, expected_path in expected_paths.items():
        ...
        if (
            ...
            raise RuntimeError("PSIM S5 existing artifact changed")
    manifest = json.loads(
    ...
    if (
        ...
        raise RuntimeError("PSIM S5 existing schedule gate changed")
    return payload
...
def execute() -> dict[str, Any]:
    ...
    if len(train_indices) != 366 or len(target_indices) != 365:
        raise RuntimeError("PSIM S5 annual source roster changed")
    ...
    for artifact in artifacts.values():
        if _sha256_file(artifact["path"]) != artifact["sha256"]:
            raise RuntimeError("PSIM S5 published artifact hash changed")
    ...
    result_core = {
        ...
        "fit": {
            ...
            "direction_residual_delta_by_current_position": {
                position: float(transformed.deltas[index])
        ...
        "global_2021_outcome_disclosure": {
            ...
            "s5_policy_specific_2021_metrics_exist": False,
            "allowed_future_role": (
