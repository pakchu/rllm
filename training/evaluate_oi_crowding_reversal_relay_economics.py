"""Sequential strict economics for frozen OICRR-18."""
...
PREREG = Path("results/oi_crowding_reversal_relay_preregistration_2026-08-09.json")
PREREG_SHA = "c7f705b2fd72705f2ec5850bf27e201035924c1f0f55127e7de616c88b92cab0"
SUPPORT = Path("results/oi_crowding_reversal_relay_support_2026-08-09.json")
SUPPORT_SHA = "91f1907f9ae7959e04cc0c1174a7b39009512e1a4606d28d61b3358f0ec17a5f"
NOVELTY = Path("results/oi_crowding_reversal_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "99cd19cd813d62c81a490a345691084b2b964378f01475d6c22cafe29d26cad3"
CLOCK = Path("data/oi_crowding_reversal_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "09f2320542320d048570fe23d0ac9d490a0ea37db70b49214769fcbbbbe7a047"
CONTROL_DIR = Path("data/oi_crowding_reversal_relay_controls_2023_2026")
FREEZE = Path("results/oi_crowding_reversal_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/oi_crowding_reversal_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("OICRR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("OICRR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "oicrr_18_sequential_economics_v1", "policy_id": POLICY_ID,
