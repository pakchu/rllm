"""Sequential strict economics for frozen HVMD-24."""
...
PREREG = Path("results/high_volatility_mcginley_dynamic_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "3b03bc28a623ce2ece674557c92f681466f727d8d4a44473bc2e86f2fb29fb85"
SUPPORT = Path("results/high_volatility_mcginley_dynamic_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "ca62c54b781c9c9e3599da78cc5ae6b0fe03a6e98341c91959e9d4d10e225b1f"
NOVELTY = Path("results/high_volatility_mcginley_dynamic_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "c38a0cc1b4f993467e1f6fbfa62109edbb64b31e0ee70205b8daca70dd75f537"
CLOCK = Path("data/high_volatility_mcginley_dynamic_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "efe7e606c4ba240c323589a6b1b850a4aee9e4da344a0f883a297f19a7d27aaa"
CONTROL_DIR = Path("data/high_volatility_mcginley_dynamic_crossover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_mcginley_dynamic_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_mcginley_dynamic_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "simple_average_crossover", "one_bar_stale_crossover", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVMD frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVMD novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvmd_24_sequential_economics_v1", "policy_id": POLICY_ID,
