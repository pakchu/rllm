"""Materialize outcome-blind source support for frozen HVEMBFR-24."""
...
from training import preregister_high_volatility_emerging_bond_forecast_transition_relay as prereg
...
BUILDER = Path("training/build_high_volatility_emerging_bond_forecast_transition_relay_support.py")
PREREG_SHA256 = "be71f09a2e9b4083082804e44a5063bbb9f61f4c3dfa483b32fb78e4bd545a79"
SYMBOLS = ("EMB",)
SOURCE_DIR = Path("data/high_volatility_emerging_bond_forecast_transition_relay_sources_2022_2026")
...
FEATURE_PANEL = SOURCE_DIR / "hvembfr_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_emerging_bond_forecast_transition_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_emerging_bond_forecast_transition_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_emerging_bond_forecast_transition_relay_support_2026-08-12.json")
...
def normalize_yahoo_chart(payload: bytes, symbol: str) -> tuple[bytes, pd.DataFrame, dict[str, Any]]:
    ...
    if chart.get("error") is not None:
        raise RuntimeError(f"HVEMBFR Yahoo chart error for {symbol}")
    results = chart.get("result") or []
    if len(results) != 1:
        raise RuntimeError(f"HVEMBFR Yahoo result count drift for {symbol}")
    result = results[0]
    meta = result.get("meta") or {}
    if meta.get("symbol") != symbol or meta.get("exchangeTimezoneName") != "America/New_York":
        raise RuntimeError(f"HVEMBFR {symbol} Yahoo metadata drift")
    timestamps = result.get("timestamp") or []
    indicators = result.get("indicators") or {}
    quotes = indicators.get("quote") or []
    adjusted = indicators.get("adjclose") or []
    if len(quotes) != 1 or len(adjusted) != 1 or not timestamps:
        raise RuntimeError(f"HVEMBFR {symbol} adjusted chart incomplete")
    quote = quotes[0]
    ...
    if any(values is None or len(values) != len(timestamps) for values in vectors.values()):
        raise RuntimeError(f"HVEMBFR {symbol} Yahoo vector length drift")
    ...
    if frame.empty or frame.session_date.duplicated().any():
        raise RuntimeError(f"HVEMBFR {symbol} session dates invalid")
    ...
    if not valid.all():
        ...
        raise RuntimeError(f"HVEMBFR {symbol} invalid adjusted rows: {dates[:5]}")
    metadata = {
        "symbol": symbol,
...
def build_equity_panel(equities: Mapping[str, pd.DataFrame]) -> pd.DataFrame:
    if set(equities) != set(SYMBOLS):
        raise RuntimeError("HVEMBFR equity symbol set drift")
    ...
    for symbol in SYMBOLS:
        ...
        if not observed.equals(official):
            ...
            raise RuntimeError(
                f"HVEMBFR {symbol}/NYSE calendar mismatch missing={missing[:5]} extra={extra[:5]}"
            )
        if expected is None:
            expected = observed
        elif not observed.equals(expected):
            raise RuntimeError(f"HVEMBFR {symbol} does not share every official common session")
    ...
    if len(panel) != len(expected):
        raise RuntimeError("HVEMBFR common-session intersection dropped an official session")
    ...
    if observed_early != expected_early:
        raise RuntimeError("HVEMBFR frozen early-close schedule drift")
    return panel
...
def normalize_btc_bars(
    ...
    if raw.columns.tolist() != ["ts", "open", "close"]:
        raise RuntimeError("HVEMBFR BTC schema drift")
    frame = raw.copy()
    ...
    if len(frame) != len(expected) or not frame.ts.equals(pd.Series(expected, name="ts")):
        raise RuntimeError("HVEMBFR BTC source is not the exact requested 1m grid")
    for column in ("open", "close"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    values = frame[["open", "close"]]
    if not np.isfinite(values.to_numpy(dtype=float)).all() or not values.gt(0.0).all(axis=None):
        raise RuntimeError("HVEMBFR BTC prices invalid")
    return frame
...
def causal_var_forecasts(frame: pd.DataFrame, trailing: int = 252) -> pd.Series:
    required = frame[["btc_return", "emb_return"]].to_numpy(dtype=float)
...
def build_features(sessions: pd.DataFrame, bars: pd.DataFrame) -> pd.DataFrame:
    ...
    if not isinstance(indexed.index, pd.DatetimeIndex) or indexed.index.tz is None:
        raise RuntimeError("HVEMBFR BTC index must be timezone-aware")
    ...
    frame["emb_return"] = np.log(frame.emb_adjusted_close / frame.emb_adjusted_close.shift(1))
    frame["btc_forecast"] = causal_var_forecasts(frame)
    frame["prior_btc_forecast"] = frame.btc_forecast.shift(1)
...
def _signal(frame: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    prior = frame.prior_btc_forecast.copy()
    ...
    elif control == "emb_sign_transition":
        score = frame.emb_return
        raw_prior = score.shift(1)
        active = score.notna() & raw_prior.notna() & score.ne(0.0) & raw_prior.ne(0.0) & score.mul(raw_prior).lt(0.0)
    elif control == "btc_ar_transition":
        reduced = frame[["btc_return", "emb_return"]].copy()
        reduced["emb_return"] = 0.0
        ...
        rows.append(
            {
                ...
                "prior_btc_forecast": float(frame.at[index, "prior_btc_forecast"]),
                "emb_return": float(frame.at[index, "emb_return"]),
...
def run(env_file: str = ENV_FILE) -> dict[str, Any]:
    ...
    source_core = {
        "protocol_version": "hvembfr_24_sources_v1",
    ...
    core = {
        "protocol_version": "hvembfr_24_source_support_v1",
