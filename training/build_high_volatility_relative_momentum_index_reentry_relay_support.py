"""Build outcome-blind source support for frozen HVRMI-24."""
...
from training import preregister_high_volatility_relative_momentum_index_reentry_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "5a57790f9cdaf87aa57e3b80e74aa490928137897f9b82977f2ce8d079ad04bf"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_relative_momentum_index_reentry_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_relative_momentum_index_reentry_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_relative_momentum_index_reentry_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_relative_momentum_index_reentry_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_relative_momentum_index_reentry_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "momentum", "avg_up", "avg_down", "rmi",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "momentum", "avg_up", "avg_down",
    "rmi", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVRMI source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVRMI invalid source key")
...
def relative_momentum_index(close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    lookback=P["momentum_periods"];period=P["rmi_periods"];continuous=valid.rolling(lookback+1,min_periods=lookback+1).sum().eq(lookback+1);momentum=(close-close.shift(lookback)).where(continuous)
    up=np.maximum(momentum.to_numpy(float),0);down=np.maximum(-momentum.to_numpy(float),0);avg_up=np.full(len(close),np.nan);avg_down=np.full(len(close),np.nan);value=np.full(len(close),np.nan);run=0;gu=gd=np.nan
    for i,m in enumerate(momentum.to_numpy(float)):
        if not math.isfinite(m):run=0;gu=gd=np.nan;continue
        run+=1
        if run<period:continue
        if run==period:gu=float(np.mean(up[i-period+1:i+1]));gd=float(np.mean(down[i-period+1:i+1]))
        else:gu=((period-1)*gu+up[i])/period;gd=((period-1)*gd+down[i])/period
        avg_up[i]=gu;avg_down[i]=gd;den=gu+gd
        if den>0:value[i]=100.0*gu/den
    return pd.DataFrame({"momentum":momentum,"avg_up":avg_up,"avg_down":avg_down,"rmi":value},index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[["bar_open","bar_high","bar_low","bar_close"]]).all(axis=1)&bars[["bar_open","bar_high","bar_low","bar_close"]].gt(0).all(axis=1);bars=pd.concat([bars,relative_momentum_index(bars.bar_close,bars.valid_bar)],axis=1);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["momentum","avg_up","avg_down","rmi","variation"]]).all(axis=1)&(bars.avg_up+bars.avg_down).gt(0)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.rmi.shift(1);long=previous.le(P["lower_level"])&panel.rmi.gt(P["lower_level"]);short=previous.ge(P["upper_level"])&panel.rmi.lt(P["upper_level"]);panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&(long|short)&panel.variation_rank.ge(P["variation_rank_min"]);return panel.loc[:,PANEL_COLUMNS]


def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);value=pd.to_numeric(used.rmi,errors="coerce");previous=value.shift(1);long=previous.le(P["lower_level"])&value.gt(P["lower_level"]);short=previous.ge(P["upper_level"])&value.lt(P["upper_level"]);variation=used.variation_rank.ge(P["variation_rank_min"]);side=pd.Series(np.where(long,1,np.where(short,-1,0)),index=used.index,dtype=int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVRMI prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvrmi_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvrmi_24_source_support_v1", "policy_id": prereg.POLICY_ID,
