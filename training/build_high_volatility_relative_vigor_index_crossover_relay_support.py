...
from training import preregister_high_volatility_relative_vigor_index_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "b12bd7a3e0a7eb3080f3fd8504b6843e594234f41935e2c8bd85d83b4a1f2e9d"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_relative_vigor_index_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_relative_vigor_index_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_relative_vigor_index_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_relative_vigor_index_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_relative_vigor_index_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "body_smooth", "range_smooth", "raw_ratio",
    "rvi", "signal", "difference", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "body_smooth", "range_smooth", "raw_ratio",
    "rvi", "signal", "difference", "variation", "variation_rank", "eligible",
)
...
def relative_vigor(open_:pd.Series,high:pd.Series,low:pd.Series,close:pd.Series,valid:pd.Series)->pd.DataFrame:
    weights=np.asarray(P["symmetric_weights"],float);window=len(weights);consecutive=valid.rolling(window,min_periods=window).sum().eq(window)
    def symmetric(values:pd.Series)->pd.Series:
        return sum(weight*values.shift(offset) for offset,weight in enumerate(weights))/weights.sum()
    body=symmetric(close-open_).where(consecutive);range_=symmetric(high-low).where(consecutive)
    groups=(~consecutive).cumsum();period=P["rvi_periods"]
    body_sum=body.groupby(groups).transform(lambda x:x.rolling(period,min_periods=period).sum());range_sum=range_.groupby(groups).transform(lambda x:x.rolling(period,min_periods=period).sum())
    rvi=(body_sum/range_sum.where(range_sum.gt(0)));rvi_valid=np.isfinite(rvi);signal=symmetric(rvi).where(rvi_valid.rolling(window,min_periods=window).sum().eq(window));raw_ratio=body/range_.where(range_.gt(0))
    return pd.DataFrame({"body_smooth":body,"range_smooth":range_,"raw_ratio":raw_ratio,"rvi":rvi,"signal":signal},index=open_.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[["bar_open","bar_high","bar_low","bar_close"]]).all(axis=1)&bars[["bar_open","bar_high","bar_low","bar_close"]].gt(0).all(axis=1)
    bars=pd.concat([bars,relative_vigor(bars.bar_open,bars.bar_high,bars.bar_low,bars.bar_close,bars.valid_bar)],axis=1);bars["difference"]=bars.rvi-bars.signal
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum())
    bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["body_smooth","range_smooth","raw_ratio","rvi","signal","difference","variation"]]).all(axis=1)&bars.range_smooth.gt(0)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.difference.shift(1)
    panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&panel.difference.ne(0)&previous.ne(0)&np.sign(panel.difference).ne(np.sign(previous))&panel.variation_rank.ge(P["variation_rank_min"])
    return panel.loc[:,PANEL_COLUMNS]


def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);difference=pd.to_numeric(used.raw_ratio if control=="raw_body_range_cross" else used.difference,errors="coerce");previous=difference.shift(1);crossover=difference.ne(0)&previous.ne(0)&np.sign(difference).ne(np.sign(previous));variation=used.variation_rank.ge(P["variation_rank_min"]);side=pd.Series(np.sign(difference),index=used.index).fillna(0).astype(int);state=valid&side.ne(0)&crossover&variation
    if control=="no_variation_gate":state=valid&side.ne(0)&crossover
