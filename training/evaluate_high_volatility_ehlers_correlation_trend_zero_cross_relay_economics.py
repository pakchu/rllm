"""Sequential strict economics for frozen HVCTI-24."""
...
PREREG = Path("results/high_volatility_ehlers_correlation_trend_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "12ee1624c6e44beff404771a27724201a3ab0ec968c4fec346a70ca15260227b"
SUPPORT = Path("results/high_volatility_ehlers_correlation_trend_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "fee06053ada2f74d750236a2b40f4a5edb0062aa8499b3e075b743ea5091139b"
NOVELTY = Path("results/high_volatility_ehlers_correlation_trend_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "be16fa9a0bf7608ccb579f1bd7720569641b9d8854ee5a36d3a1b88007f75ef0"
CLOCK = Path("data/high_volatility_ehlers_correlation_trend_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "5530f124be070cb68eeb8128d01c46ecf804e823dfa5cfc40bd44d564fe4c8ab"
CONTROL_DIR = Path("data/high_volatility_ehlers_correlation_trend_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_ehlers_correlation_trend_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ehlers_correlation_trend_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "slow_40_bar_cti", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCTI frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCTI novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcti_24_sequential_economics_v1", "policy_id": POLICY_ID,
