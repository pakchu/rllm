"""Build outcome-blind source support for frozen HVRSSR-8."""
...
PREREG_SHA="b712a8b708a1c7fb28a2bef021a6534f04ed241b4b4fd5dd165359d5d1dcdf81"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_roll_spread_shock_reversal_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_roll_spread_shock_reversal_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_roll_spread_shock_reversal_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_roll_spread_shock_reversal_controls_2023_2026")
RESULT=Path("results/high_volatility_roll_spread_shock_reversal_support_2026-08-10.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","roll_covariance","roll_spread_proxy","spread_rank","realized_variation","variation_rank","completed_return","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","roll_covariance","roll_spread_proxy","spread_rank","realized_variation","variation_rank","completed_return")
...
def roll_statistics(closes:np.ndarray, first_open:float, minute_returns:np.ndarray)->tuple[float,float,float,float]:
 closes=np.asarray(closes,float); minute_returns=np.asarray(minute_returns,float)
 if closes.shape!=(96,) or minute_returns.shape!=(480,) or not np.isfinite(closes).all() or np.any(closes<=0) or not first_open>0:return (math.nan,)*4
 returns=np.diff(np.log(closes)); left,right=returns[:-1],returns[1:]
 if np.var(left,ddof=0)<=0 or np.var(right,ddof=0)<=0:return (math.nan,)*4
 covariance=float(np.mean((left-left.mean())*(right-right.mean())))
 spread=2*math.sqrt(-covariance) if covariance<0 else math.nan
 variation=float(np.square(returns).sum()); completed=float(math.log(closes[-1]/first_open))
 return covariance,spread,variation,completed
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVRSSR source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVRSSR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("8h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   close=block.close.to_numpy(float).reshape(96,5)[:,-1];cov,spread,var,ret=roll_statistics(close,float(block.open.iloc[0]),block.minute_return.to_numpy(float));valid=math.isfinite(spread) and var>0 and ret!=0
  else:cov=spread=var=ret=math.nan;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"roll_covariance":cov,"roll_spread_proxy":spread,"realized_variation":var,"completed_return":ret})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["spread_rank"]=prior_rank(panel.roll_spread_proxy.where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));panel["eligible"]=valid&panel.spread_rank.ge(P["spread_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"]);panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_block_stale_geometry":
  cols=["source_valid","spread_rank","variation_rank","completed_return","eligible","onset","feature_available_time"];used[cols]=panel[cols].shift(1)
 valid=used.source_valid.eq(True);spread=used.spread_rank.ge(P["spread_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"])
 state=valid&spread&variation
 if control=="no_spread_tail_gate":state=valid&variation
 elif control=="no_variation_gate":state=valid&spread
 elif control=="all_negative_covariance":state=valid
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVRSSR prereg drift")
 ...
 source_core={"protocol_version":"hvrssr_8_source_v1","query":QUERY,"query_sha256":hashlib.sha256(QUERY.encode()).hexdigest(),"window":[START.isoformat(),END.isoformat()],"physical_rows":len(raw),"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"panel":{"path":str(PANEL),"sha256":sha(PANEL),"rows":len(panel),"valid_rows":int(panel.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};manifest={**source_core,"manifest_hash":chash(source_core)};immutable(MANIFEST,json_bytes(manifest))
