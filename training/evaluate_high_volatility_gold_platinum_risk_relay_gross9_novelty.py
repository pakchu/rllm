"""Evaluate frozen HVAUPTR-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVAUPTR-24"
PROTOCOL = "hvauptr_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_gold_platinum_risk_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVAUPTR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVAUPTR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVAUPTR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVAUPTR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvauptr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVAUPTR-24 unchanged before economic outcomes",
    }
