"""Outcome-blind preregistration for HVCATFLIR-8."""
...
POLICY_ID = "HVCATFLIR-8"
DEFAULT_OUTPUT = Path("results/high_volatility_causal_average_ticket_flow_late_inventory_relay_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_causal_average_ticket_flow_late_inventory_relay_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": "When BTC perpetual average quote ticket size accelerates unusually from the first to the second four-hour half, late aggressive quote flow and the final two-hour price return both agree with the completed return, realized variation is elevated, and open interest expands, dual late confirmation and fresh leveraged large transactions sponsor continuation for eight hours.",
            "side": "strict sign of the completed eight-hour BTC perpetual return",
            "why_distinct": "HVCATFLIR requires average-ticket acceleration and independently confirms the completed direction with both second-half aggressive quote flow and the final-two-hour return, plus exact OI expansion and variation. No prior composite event set or control is reused; it uses no spot, ETH, funding, premium-index, fitted outcome, or promoted control.",
            "why_suited_to_volatile_regimes": "the completed eight-hour BTC realized variation must be in its causal upper 40 percent",
            "why_low_gross9_overlap_is_plausible": "fixed eight-hour accelerating-average-ticket plus OI-sponsored states are absent from Gross9 primitives",
        },
        "features": {
            ...
            "second_half_aggressive_flow": "sum(2*taker_buy_quote-quote_asset_volume) over the final 240 completed minutes; finite strict nonzero",
            "flow_confirmation": "strict sign of second-half aggressive flow equals strict sign of completed_return",
            "final_two_hour_return": "sum of the last 120 completed one-minute log(close/open) returns, finite strict nonzero",
            "late_price_confirmation": "strict sign of final_two_hour_return equals strict sign of completed_return",
            ...
            "eligible": "exact coherent completed perpetual cycle, positive average-ticket acceleration, nonzero completed return, same-sign nonzero second-half aggressive flow and final-two-hour return, two exact OI endpoints with strict expansion, and both average-ticket-acceleration and variation ranks pass at D; no spot source, funding, range, premium-index, alt, or post-decision condition",
            "no_imputation": True,
        },
        "clock": {
        ...
        "source_plan": {
            ...
            "bars": "Postgres bars_binance BTCUSDT exact coherent 1m OHLC, quote_asset_volume, number_of_trades, and taker_buy_quote",
            "window": ["2020-01-01T00:00:00Z", "2026-08-01T00:00:00Z"],
            "read_after_preregistration": True,
            "execution_prices": "sealed until source support and Gross9 pass",
        },
        "research_boundary": {
            "prior_average_ticket_candidate_outcomes_known": True,
            "prior_HVCATAIR_source_novelty_and_train_failure_known": True,
            "prior_HVCATFIR_source_novelty_and_train_failure_known": True,
            "prior_HVCATLIR_source_novelty_and_train_failure_known": True,
        ...
        "stopping_rule": "terminal first failure; no OI endpoint or expansion rule, average-ticket acceleration formula, aggressive-flow and final-two-hour confirmations, acceleration or variation history, side, clock, hold, subset, threshold, comparator, or control repair",
    }
