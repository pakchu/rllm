"""Materialize outcome-blind source support for frozen HVLFX-12."""
...
BUILDER = Path("training/build_high_volatility_london_fix_dollar_impulse_relay_support.py")
PREREG_SHA = "2908e8cd471c185625e2b21e6a45289ae89b89a3eaa3bfb841eaa732ad6c6a0a"
SOURCE_DIR = Path("data/high_volatility_london_fix_dollar_impulse_relay_sources_2023_2026")
FEATURES = SOURCE_DIR / "hvlfx_preentry_features.csv.gz"
MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_london_fix_dollar_impulse_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_london_fix_dollar_impulse_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_london_fix_dollar_impulse_relay_support_2026-08-11.json")
...
COLUMNS = (
    ...
    "gbpusd_return", "eurusd_return", "common_impulse",
...
def load_sources() -> tuple[pd.DataFrame, dict[str, pd.DataFrame]]:
    ...
    if btc.columns.tolist() != ["ts", "open", "close"]:
        raise RuntimeError("HVLFX BTC schema drift")
    if fx.columns.tolist() != ["symbol", "ts", "open", "close"]:
        raise RuntimeError("HVLFX FX schema drift")
    ...
    if len(btc_frame) != len(expected) or not btc_frame.index.equals(expected):
        raise RuntimeError("HVLFX BTC source not exact 1m grid")
...
def build_features(btc: pd.DataFrame, fx: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    for day in pd.date_range("2023-01-02", "2026-07-31", freq="B"):
        ...
        start = (local_day + pd.Timedelta(hours=15, minutes=55)).tz_convert("UTC")
        decision = (local_day + pd.Timedelta(hours=16, minutes=5)).tz_convert("UTC")
        ...
        rows.append({
            ...
            "common_impulse": (returns["GBPUSD"] + returns["EURUSD"]) / 2,
...
def signal(features: pd.DataFrame, control: str) -> pd.Series:
    gbp = features.gbpusd_return
    eur = features.eurusd_return
    side = np.sign(features.common_impulse).astype("Int64").fillna(0).astype(int)
    consensus = np.sign(gbp).eq(np.sign(eur)) & gbp.ne(0) & eur.ne(0)
    eligible = consensus & features.btc_variation_rank.ge(0.65)
    if control == "no_volatility_gate":
        eligible = consensus
    elif control == "eurusd_only":
        side = np.sign(eur).astype("Int64").fillna(0).astype(int)
        eligible = eur.ne(0) & features.btc_variation_rank.ge(0.65)
    elif control == "one_session_stale_fix_impulse":
        stale_gbp, stale_eur = gbp.shift(1), eur.shift(1)
        side = np.sign((stale_gbp + stale_eur) / 2).astype("Int64").fillna(0).astype(int)
        eligible = np.sign(stale_gbp).eq(np.sign(stale_eur)) & stale_gbp.ne(0) & stale_eur.ne(0) & features.btc_variation_rank.ge(0.65)
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[sides.ne(0)]:
        ...
        rows.append({
            "candidate": "HVLFX-12", "control": control, "split": split,
            ...
            "common_impulse": float(features.at[index, "common_impulse"]),
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVLFX prereg drift")
    ...
    source_core = {
        "protocol_version": "hvlfx_12_sources_v1",
    ...
    core = {
        "protocol_version": "hvlfx_12_source_support_v1", "policy_id": "HVLFX-12",
