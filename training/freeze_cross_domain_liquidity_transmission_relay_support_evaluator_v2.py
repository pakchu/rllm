"""Freeze corrected CDLTR-72A evaluator v2 after the audited v1 parser failure."""
...
PROTOCOL_VERSION = "cross_domain_liquidity_transmission_relay_evaluator_freeze_v2"
DEFAULT_OUTPUT = Path(
...
PREDECESSOR_FREEZE_SHA256 = (
    "b02091be3dad585c8cea865eaeb2535500ad47d3528af4b418f96ac2310d1bf5"
)
CORRECTION_DOCUMENT = Path(
    "docs/cdltr72a-source-evaluator-v1-failure-v2-correction-2026-07-21.md"
)
CORRECTION_DOCUMENT_SHA256 = (
    "121ed11dc94edbc40e479b2cb6e748daeb5b876b0fe831e33e6178e035e8054f"
)
EXPECTED_EVALUATOR_COMMIT = "aa49346724e8446e04017302d1de408b3a1b743d"
...
def build_manifest(commit: str, source_sha: str) -> dict[str, Any]:
    ...
    _require_file(
        PREDECESSOR_FREEZE,
        PREDECESSOR_FREEZE_SHA256,
        "CDLTR v1 evaluator freeze",
    )
    _require_file(
        CORRECTION_DOCUMENT,
        CORRECTION_DOCUMENT_SHA256,
        "CDLTR evaluator correction document",
    )
    core = {
        ...
        "mutable_parameters": [],
        "source_support_artifact_exists_before_v2_freeze": False,
        "source_support_clock_exists_before_v2_freeze": False,
        "next_action": "rerun the exact corrected source-only evaluator once",
    }
...
def validate_manifest(payload: dict[str, Any]) -> None:
    ...
    if evaluate.canonical_hash(core) != payload.get("manifest_hash"):
        raise RuntimeError("CDLTR v2 evaluator freeze manifest hash mismatch")
    if payload.get("mutable_parameters") != []:
        raise RuntimeError("CDLTR v2 evaluator freeze permits parameter repair")
    boundary = payload.get("v2_freeze_boundary", {})
    if not isinstance(boundary, dict) or any(value != 0 for value in boundary.values()):
        raise RuntimeError("CDLTR v2 freeze opened incidence or outcomes")
    prior = payload.get("prior_failed_run_boundary", {})
    if not isinstance(prior, dict) or prior.get("candidate_event_rows_derived") != 0:
        raise RuntimeError("CDLTR v1 failure boundary drift")
    if prior.get("btc_market_rows_loaded") != 0:
        raise RuntimeError("CDLTR v1 failure opened market outcomes")
...
def freeze(output: str | Path = DEFAULT_OUTPUT) -> tuple[str, dict[str, Any]]:
    if evaluate._repository_path(evaluate.DEFAULT_OUTPUT_REPORT).exists():
        raise RuntimeError("CDLTR source-support artifact predates v2 freeze")
    if evaluate._repository_path(evaluate.DEFAULT_OUTPUT_CLOCK).exists():
        raise RuntimeError("CDLTR source-support clock predates v2 freeze")
    ...
    if target in evaluate._protected_paths():
        raise ValueError("CDLTR v2 freeze aliases a protected input")
    ...
    if target.exists():
        ...
        if existing != payload:
            raise RuntimeError("refusing to overwrite CDLTR v2 evaluator freeze")
