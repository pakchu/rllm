"""Materialize outcome-blind source support for frozen CYMRR-12."""
...
ENV_FILE="/home/pakchu/rllm/.env";BUILDER=Path("training/build_china_morning_yuan_risk_relay_support.py");PREREG_SHA="2617659d80b94015d0cd6a4320517ebbe21c53f5cf1644e67a42ced349a07cc2";SOURCE_DIR=Path("data/china_morning_yuan_risk_relay_sources_2023_2026");FEATURES=SOURCE_DIR/"cymrr_preentry_features.csv.gz";MANIFEST=SOURCE_DIR/"manifest.json";CLOCK=Path("data/china_morning_yuan_risk_relay_clocks_2023_2026.csv.gz");CONTROL_DIR=Path("data/china_morning_yuan_risk_relay_controls_2023_2026");RESULT=Path("results/china_morning_yuan_risk_relay_support_2026-08-10.json")
...
def load_sources()->tuple[pd.DataFrame,pd.DataFrame]:
 ...
 for label,f in (("BTC",btc),("FX",fx)):
  if f.columns.tolist()!=["ts","open","close"]:raise RuntimeError(f"CYMRR {label} schema drift")
  f.ts=pd.to_datetime(f.ts,utc=True,errors="raise");f.sort_values("ts",inplace=True);f.reset_index(drop=True,inplace=True)
  if f.ts.duplicated().any():raise RuntimeError(f"CYMRR {label} duplicate timestamp")
  for c in ("open","close"):f[c]=pd.to_numeric(f[c],errors="coerce")
  if not np.isfinite(f[["open","close"]]).all(axis=None) or not f[["open","close"]].gt(0).all(axis=None):raise RuntimeError(f"CYMRR {label} invalid price")
 expected=pd.date_range(BTC_START,SOURCE_END,freq="1min",inclusive="left")
 if len(btc)!=len(expected) or not btc.ts.equals(pd.Series(expected,name="ts")):raise RuntimeError("CYMRR BTC source not exact 1m grid")
...
def build_features(btc:pd.DataFrame,fx:pd.DataFrame)->pd.DataFrame:
 ...
 for day in pd.date_range("2023-01-02","2026-07-31",freq="B"):
  start=pd.Timestamp(day).tz_localize("UTC")+pd.Timedelta(hours=1,minutes=30);decision=pd.Timestamp(day).tz_localize("UTC")+pd.Timedelta(hours=2)
  ...
  fxret=float(np.log(session.close.iloc[-1]/session.open.iloc[0]));variation=float(np.sqrt(np.square(np.log(bw.close.to_numpy()/bw.open.to_numpy())).sum()));rows.append({"session_date":day,"decision_time":decision,"usdcny_return":fxret,"btc_realized_variation":variation})
...
def signal(f:pd.DataFrame,control:str)->pd.Series:
 ret=f.usdcny_return;side=-np.sign(ret).astype("Int64").fillna(0).astype(int);eligible=ret.ne(0)&f.btc_variation_rank.ge(.65)
...
def build_clock(f:pd.DataFrame,control="primary")->pd.DataFrame:
 ...
 for i in f.index[sides.ne(0)]:
  ...
  next_allowed=exit_time;rows.append({"candidate":"CYMRR-12","control":control,"split":split,"session_date":pd.Timestamp(f.at[i,"session_date"]),"decision_time":d,"feature_available_time":d,"entry_time":entry,"exit_time":exit_time,"side":int(sides.at[i]),"usdcny_return":float(f.at[i,"usdcny_return"]),"btc_realized_variation":float(f.at[i,"btc_realized_variation"]),"btc_variation_rank":float(f.at[i,"btc_variation_rank"])})
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("CYMRR prereg drift")
 ...
 sc={"protocol_version":"cymrr_12_sources_v1","queries":{"btc":BTC_QUERY,"fx":FX_QUERY},"windows":{"btc":[BTC_START.isoformat(),SOURCE_END.isoformat()],"fx":[FX_START.isoformat(),SOURCE_END.isoformat()]},"rows":{"btc":len(btc),"fx":len(fx),"features":len(features)},"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"feature_output":{"path":str(FEATURES),"sha256":sha(FEATURES)},"candidate_outcomes_opened":False,"no_imputation":True};sm={**sc,"manifest_hash":chash(sc)};MANIFEST.write_text(json.dumps(sm,indent=2,ensure_ascii=False,allow_nan=False)+"\n");support={n:stats(primary,n) for n in SPLITS};checks={}
