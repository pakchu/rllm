"""Materialize source-only HVDBR-12 support clocks."""
...
SOURCE_DIR=Path("data/high_volatility_dollar_breadth_relay_sources_2023_2026"); SESSION=SOURCE_DIR/"dollar_breadth_sessions.csv.gz"; SOURCE_MANIFEST=SOURCE_DIR/"manifest.json"
...
CLOCK=Path("data/high_volatility_dollar_breadth_relay_clocks_2023_2026.csv.gz"); CONTROL_DIR=Path("data/high_volatility_dollar_breadth_relay_controls_2023_2026"); RESULT=Path("results/high_volatility_dollar_breadth_relay_support_2026-08-08.json")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00:00Z"),pd.Timestamp("2024-01-01T00:00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00:00Z"),pd.Timestamp("2025-01-01T00:00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00:00Z"),pd.Timestamp("2026-01-01T00:00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00:00Z"),pd.Timestamp("2026-08-01T00:00:00Z"))}; MINIMUM={"train":8,"test":12,"eval":12,"final":8}; CONTROLS=("no_volatility_gate","no_dollar_shock_gate","no_breadth_gate","one_session_stale_dollar_shock","direction_flip")
...
COLUMNS=("candidate","control","split","source_day","decision_time","feature_available_time","entry_time","exit_time","side","common_dollar_direction","agreeing_pairs","median_absolute_pair_z","btc_realized_variation","btc_realized_variation_rank")
...
def features()->pd.DataFrame:
 ...
 for column in ("common_dollar_direction","agreeing_pairs","median_absolute_pair_z"):f[column]=pd.to_numeric(f[column],errors="coerce")
 ...
 d=p[["decision_time","btc_realized_variation","btc_valid"]];f=f.merge(d,on="decision_time",how="left",validate="one_to_one");f["btc_realized_variation_rank"]=strict_prior_midrank(f.btc_realized_variation.where(f.btc_valid));f["signal_valid"]=f.source_valid&f.btc_valid&np.isfinite(f[["common_dollar_direction","agreeing_pairs","median_absolute_pair_z","btc_realized_variation","btc_realized_variation_rank"]]).all(axis=1)&f.common_dollar_direction.abs().eq(1);return f
def conditions(f:pd.DataFrame,control:str)->tuple[pd.Series,pd.Series]:
 ...
 if control=="one_session_stale_dollar_shock":direction=direction.shift(1);breadth=breadth.shift(1);shock=shock.shift(1)
 shock_gate=pd.Series(True,index=f.index) if control=="no_dollar_shock_gate" else shock.ge(1.);breadth_gate=pd.Series(True,index=f.index) if control=="no_breadth_gate" else breadth.ge(5);vol=pd.Series(True,index=f.index) if control=="no_volatility_gate" else f.btc_realized_variation_rank.ge(.65);active=f.signal_valid&np.isfinite(direction)&np.isfinite(breadth)&np.isfinite(shock)&direction.abs().eq(1)&shock_gate&breadth_gate&vol;side=-direction;side=-side if control=="direction_flip" else side;return active,side
