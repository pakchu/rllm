"""Sequential strict economics for frozen HVCAFC-6."""
...
PREREG = Path("results/high_volatility_cross_alt_three_bar_flip_cascade_relay_preregistration_2026-08-13.json")
PREREG_SHA = "8da080cbd9f607564d1e213c9cdf67b728ef46d786ce32280034c16f815c15c4"
SUPPORT = Path("results/high_volatility_cross_alt_three_bar_flip_cascade_relay_support_2026-08-13.json")
SUPPORT_SHA = "8ef99252b5803dae28eb63f8b2a9c7fd24f405b9aca7134abbb8091ab64d60c7"
NOVELTY = Path("results/high_volatility_cross_alt_three_bar_flip_cascade_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "bc119c97a4fe7e7741310ada88911f31d79425783ee19eba968e954d5fd2d8fc"
CLOCK = Path("data/high_volatility_cross_alt_three_bar_flip_cascade_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "34f80b8802db356d2026e931bde3dba54092fe03610eaef6a5ef97bc33c941c0"
CONTROL_DIR = Path("data/high_volatility_cross_alt_three_bar_flip_cascade_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_cross_alt_three_bar_flip_cascade_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_alt_three_bar_flip_cascade_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "three_alt_cascade", "two_bar_flip", "one_decision_stale_cascade", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCAFC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCAFC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcafc_6_sequential_economics_v1", "policy_id": POLICY_ID,
