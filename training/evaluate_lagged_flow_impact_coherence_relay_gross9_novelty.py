"""Evaluate frozen LFIC-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "LFIC-12"
PROTOCOL = "lfic_12_gross9_novelty_v1"
...
OUTPUT = Path("results/lagged_flow_impact_coherence_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("LFIC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("LFIC novelty limits drift")
    if (
        ...
        raise RuntimeError("LFIC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="LFIC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "lfic_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject LFIC-12 unchanged before economic outcomes",
    }
