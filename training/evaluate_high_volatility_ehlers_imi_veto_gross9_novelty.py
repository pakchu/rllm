"""Evaluate the frozen HVEIV-24 action-vote clock against Gross9."""
...
POLICY = "HVEIV-24"
PROTOCOL = "hveiv_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_ehlers_imi_veto_clocks_2023_2026.csv.gz",
    "sha256": "3f16540ef8eb05cff29767cff287dab9dbe00a8537351642e328d39c511e4c13",
    "rows": 183,
}
OUTPUT = Path("results/high_volatility_ehlers_imi_veto_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVEIV-24 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVEIV-24 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVEIV-24 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVEIV-24 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVEIV-24 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVEIV-24 unchanged before economics",
    }
