"""Sequential strict economics for frozen CMMTCR-24."""
...
PREREG = Path("results/cross_market_multi_horizon_trend_concordance_relay_preregistration_2026-08-09.json")
PREREG_SHA = "11f244c7dd4e0791c81eedb6b37eda41a7c82319e2064edec4ccf67674b8e4d3"
SUPPORT = Path("results/cross_market_multi_horizon_trend_concordance_relay_support_2026-08-09.json")
SUPPORT_SHA = "a1eb56b4ac72f1aafc805b35994d8b8fe82bd51fdcbf3f627dd45b7844a1268e"
NOVELTY = Path("results/cross_market_multi_horizon_trend_concordance_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "d8c266f7b795537e25800ada250073b9be265a7d71e0ea98573a74f320ebc08a"
CLOCK = Path("data/cross_market_multi_horizon_trend_concordance_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "e0c5e05fabcc76c8dc63f8026fb364f789dff368249b2e441436bb68c21878c0"
CONTROL_DIR = Path("data/cross_market_multi_horizon_trend_concordance_relay_controls_2023_2026")
FREEZE = Path("results/cross_market_multi_horizon_trend_concordance_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/cross_market_multi_horizon_trend_concordance_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CMMTCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CMMTCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "cmmtcr_24_sequential_economics_v1", "policy_id": POLICY_ID,
