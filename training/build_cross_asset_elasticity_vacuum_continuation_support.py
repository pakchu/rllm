"""Build source-only CAEVC-8 clocks."""
from __future__ import annotations
import argparse,hashlib,json,math
from pathlib import Path
from typing import Any
import numpy as np,pandas as pd
from training import preregister_cross_asset_elasticity_vacuum_continuation as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
ENV_FILE="/home/pakchu/rllm/.env";PREREG_SHA="234d09998bf8f3c56fdb4687a6c563874ebd1e56374ad3acf50a7112f218bc48";START=pd.Timestamp("2023-01-01T00:00Z");END=pd.Timestamp("2026-08-01T00:00Z");SYMBOLS=("BTCUSDT","ETHUSDT","SOLUSDT","BNBUSDT","XRPUSDT","DOGEUSDT","ADAUSDT")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00Z"),pd.Timestamp("2024-01-01T00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00Z"),pd.Timestamp("2025-01-01T00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00Z"),pd.Timestamp("2026-01-01T00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00Z"),END)};MINIMUM={"train":8,"test":12,"eval":12,"final":8};CONTROLS=("btc_only_activity_vacuum","cross_asset_vacuum_without_return_tail","btc_return_tail_only","one_hour_stale_elasticity","direction_flip")
QUERY="""WITH five AS (SELECT symbol,date_bin('5 minutes',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00') AS bar_time,(array_agg(open ORDER BY ts))[1] AS bar_open,(array_agg(close ORDER BY ts DESC))[1] AS bar_close,sum(number_of_trades) AS trades,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND high>=greatest(open,close) AND low<=least(open,close) AND high>=low AND number_of_trades>=0 AND number_of_trades=floor(number_of_trades)) AS coherent FROM bars_binance WHERE symbol=ANY(:symbols) AND interval='1m' AND ts>=:start AND ts<:end GROUP BY symbol,bar_time), hourly AS (SELECT symbol,date_bin('1 hour',bar_time,TIMESTAMPTZ '1970-01-01 00:00:00+00') AS hour_start,(array_agg(bar_open ORDER BY bar_time))[1] AS hour_open,(array_agg(bar_close ORDER BY bar_time DESC))[1] AS hour_close,sqrt(sum(power(ln(bar_close/bar_open),2))) AS path_variation,sum(trades) AS execution_count,sum(source_rows) AS physical_rows,count(*) AS five_rows,min(bar_time) AS first_bar,max(bar_time) AS last_bar,bool_and(source_rows=5 AND distinct_rows=5 AND first_ts=bar_time AND last_ts=bar_time+INTERVAL '4 minutes' AND coherent) AS coherent FROM five GROUP BY symbol,hour_start) SELECT * FROM hourly ORDER BY hour_start,symbol"""
SOURCE_DIR=Path("data/cross_asset_elasticity_vacuum_continuation_sources_2023_2026");FEATURES=SOURCE_DIR/"hourly_elasticity.csv.gz";MANIFEST=SOURCE_DIR/"manifest.json";CLOCK=Path("data/cross_asset_elasticity_vacuum_continuation_clocks_2023_2026.csv.gz");CONTROL_DIR=Path("data/cross_asset_elasticity_vacuum_continuation_controls_2023_2026");RESULT=Path("results/cross_asset_elasticity_vacuum_continuation_support_2026-08-09.json")
FEATURE_COLUMNS=("decision_time","feature_available_time","source_valid","btc_return","btc_abs_return_rank","btc_activity","btc_activity_rank","alt_median_activity","cross_asset_vacuum","vacuum_rank")
...
def build_features(raw:pd.DataFrame)->pd.DataFrame:
 req={"symbol","hour_start","hour_open","hour_close","path_variation","execution_count","physical_rows","five_rows","first_bar","last_bar","coherent"}
 if not req.issubset(raw):raise ValueError("CAEVC schema drift")
 ...
 for c in ("hour_open","hour_close","path_variation","execution_count","physical_rows","five_rows"):f[c]=pd.to_numeric(f[c],errors="coerce")
 grid=pd.date_range(START,END,freq="1h",inclusive="left");acts={};returns={};valids={}
 for symbol in SYMBOLS:
  ...
  valid=np.isfinite(x[["hour_open","hour_close","path_variation","execution_count","physical_rows","five_rows"]]).all(axis=1)&x.hour_open.gt(0)&x.hour_close.gt(0)&x.path_variation.gt(0)&x.execution_count.gt(0)&x.physical_rows.eq(60)&x.five_rows.eq(12)&x.coherent.fillna(False).astype(bool)&x.first_bar.eq(pd.Series(grid,index=grid))&x.last_bar.eq(pd.Series(grid+pd.Timedelta(minutes=55),index=grid))
  count=np.log1p(x.execution_count.where(valid));variation=x.path_variation.where(valid);cm=count.shift(1).rolling(720,min_periods=672).mean();cs=count.shift(1).rolling(720,min_periods=672).std(ddof=1);vm=variation.shift(1).rolling(720,min_periods=672).mean();vs=variation.shift(1).rolling(720,min_periods=672).std(ddof=1);activity=((count-cm)/cs-(variation-vm)/vs).where(valid&(cs>0)&(vs>0));acts[symbol]=activity;returns[symbol]=np.log(x.hour_close/x.hour_open).where(valid);valids[symbol]=valid
 panel=pd.DataFrame(acts,index=grid);source_valid=panel.notna().all(axis=1)&pd.DataFrame(valids,index=grid).all(axis=1);btc_ret=returns["BTCUSDT"].where(source_valid);alt=panel[list(SYMBOLS[1:])].median(axis=1).where(source_valid);q=(panel.BTCUSDT-alt).where(source_valid)
 out=pd.DataFrame({"decision_time":grid+pd.Timedelta(hours=1),"feature_available_time":grid+pd.Timedelta(hours=1),"source_valid":source_valid.to_numpy(),"btc_return":btc_ret.to_numpy(),"btc_activity":panel.BTCUSDT.where(source_valid).to_numpy(),"alt_median_activity":alt.to_numpy(),"cross_asset_vacuum":q.to_numpy()});out["btc_abs_return_rank"]=prior_midrank(out.btc_return.abs().where(out.source_valid));out["btc_activity_rank"]=prior_midrank(out.btc_activity.where(out.source_valid));out["vacuum_rank"]=prior_midrank(out.cross_asset_vacuum.where(out.source_valid));return out[list(FEATURE_COLUMNS)]
...
def active_and_side(f:pd.DataFrame,control:str="primary"):
 ...
 primary=f.source_valid&f.btc_return.ne(0)&f.vacuum_rank.le(.1)&f.btc_abs_return_rank.ge(.8);sidev=f.btc_return
 if control=="btc_only_activity_vacuum":state=f.source_valid&f.btc_return.ne(0)&f.btc_activity_rank.le(.1)&f.btc_abs_return_rank.ge(.8);active=state&f.btc_activity_rank.shift(1).gt(.1)
 elif control=="cross_asset_vacuum_without_return_tail":state=f.source_valid&f.btc_return.ne(0)&f.vacuum_rank.le(.1);active=state&f.vacuum_rank.shift(1).gt(.1)
 ...
 elif control=="one_hour_stale_elasticity":state=primary.shift(1,fill_value=False);sidev=f.btc_return.shift(1);active=onset(state)
 else:active=primary&f.vacuum_rank.shift(1).gt(.1)
 side=np.sign(sidev)
...
def run()->dict[str,Any]:
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("CAEVC prereg drift")
