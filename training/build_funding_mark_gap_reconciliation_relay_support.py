"""Materialize outcome-blind source support for frozen FMGRR-6."""
...
BUILDER = Path("training/build_funding_mark_gap_reconciliation_relay_support.py")
PREREG_SHA = "0f2c65319401437beae734e6d8e2a9d1cf6aba0205fd0082bf0f336cd74cae11"
SOURCE_DIR = Path("data/funding_mark_gap_reconciliation_relay_sources_2023_2026")
FEATURES = SOURCE_DIR / "fmgrr_preentry_features.csv.gz"
MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/funding_mark_gap_reconciliation_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/funding_mark_gap_reconciliation_relay_controls_2023_2026")
RESULT = Path("results/funding_mark_gap_reconciliation_relay_support_2026-08-09.json")
...
CONTROLS = ("no_volatility_gate", "no_gap_tail", "one_day_stale_gap", "direction_flip")
COLUMNS = (
    "candidate", "control", "split", "settlement_date", "decision_time",
    "feature_available_time", "entry_time", "exit_time", "side", "mark_price",
    "last_trade_close", "mark_gap", "absolute_gap_rank", "btc_realized_variation",
    "btc_variation_rank",
)
BTC_QUERY = (
...
SETTLEMENT_QUERY = (
    "SELECT funding_time,mark_price FROM funding_rates_binance WHERE symbol='BTCUSDT' "
...
def load_sources() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if btc.columns.tolist() != ["ts", "open", "close"]:
        raise RuntimeError("FMGRR BTC schema drift")
    if settlements.columns.tolist() != ["funding_time", "mark_price"]:
        raise RuntimeError("FMGRR settlement schema drift")
    btc["ts"] = pd.to_datetime(btc.ts, utc=True, errors="raise")
    btc.sort_values("ts", inplace=True)
    btc.reset_index(drop=True, inplace=True)
    if btc.ts.duplicated().any():
        raise RuntimeError("FMGRR BTC duplicate timestamp")
    ...
    if not np.isfinite(btc[["open", "close"]]).all(axis=None) or not btc[["open", "close"]].gt(0).all(axis=None):
        raise RuntimeError("FMGRR invalid BTC price")
    ...
    if len(btc) != len(expected) or not btc.ts.equals(pd.Series(expected, name="ts")):
        raise RuntimeError("FMGRR BTC source not exact 1m grid")
    settlements["funding_time"] = pd.to_datetime(settlements.funding_time, utc=True, errors="raise")
    settlements["mark_price"] = pd.to_numeric(settlements.mark_price, errors="coerce")
    if settlements.funding_time.duplicated().any():
        raise RuntimeError("FMGRR duplicate settlement timestamp")
    if not np.isfinite(settlements.mark_price).all() or not settlements.mark_price.gt(0).all():
        raise RuntimeError("FMGRR invalid settlement mark")
...
def build_features(btc: pd.DataFrame, settlements: pd.DataFrame) -> pd.DataFrame:
    ...
    selected = settlements[(minute.dt.hour == 8) & (minute.dt.minute == 0)].copy()
    selected["decision_time"] = selected.funding_time.dt.floor("D") + pd.Timedelta(hours=8)
    if selected.decision_time.duplicated().any():
        raise RuntimeError("FMGRR multiple 08:00 settlement records")
    for row in selected.itertuples(index=False):
        decision = pd.Timestamp(row.decision_time)
        if not (decision <= row.funding_time < decision + pd.Timedelta(minutes=1)):
            raise RuntimeError("FMGRR late or early funding timestamp")
        window = btc.loc[decision - pd.Timedelta(hours=24): decision - pd.Timedelta(minutes=1)]
        if len(window) != 1440:
            continue
        last_close = float(window.close.iloc[-1])
        mark_gap = float(np.log(float(row.mark_price) / last_close))
        ...
        rows.append({
            ...
            "feature_available_time": pd.Timestamp(row.funding_time), "mark_price": float(row.mark_price),
            "last_trade_close": last_close, "mark_gap": mark_gap,
            "btc_realized_variation": variation,
        })
    features = pd.DataFrame(rows)
    features["absolute_gap_rank"] = strict_prior_rank(features.mark_gap.abs())
...
def signal(features: pd.DataFrame, control: str) -> pd.Series:
    gap = features.mark_gap
    side = np.sign(gap).astype("Int64").fillna(0).astype(int)
    eligible = gap.ne(0) & features.absolute_gap_rank.ge(0.75) & features.btc_variation_rank.ge(0.65)
    if control == "no_volatility_gate":
        eligible = gap.ne(0) & features.absolute_gap_rank.ge(0.75)
    elif control == "no_gap_tail":
        eligible = gap.ne(0) & features.btc_variation_rank.ge(0.65)
    elif control == "one_day_stale_gap":
        gap = gap.shift(1)
        side = np.sign(gap).astype("Int64").fillna(0).astype(int)
        eligible = gap.ne(0) & features.absolute_gap_rank.shift(1).ge(0.75) & features.btc_variation_rank.ge(0.65)
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[sides.ne(0)]:
        ...
        rows.append({
            "candidate": "FMGRR-6", "control": control, "split": split,
            ...
            "mark_price": float(features.at[index, "mark_price"]),
            "last_trade_close": float(features.at[index, "last_trade_close"]),
            "mark_gap": float(features.at[index, "mark_gap"]),
            "absolute_gap_rank": float(features.at[index, "absolute_gap_rank"]),
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("FMGRR prereg drift")
    ...
    source_core = {
        "protocol_version": "fmgrr_6_sources_v1",
        ...
        "funding_rate_column_opened": False, "candidate_outcomes_opened": False, "no_imputation": True,
    }
    ...
    core = {
        "protocol_version": "fmgrr_6_source_support_v1", "policy_id": "FMGRR-6",
        "preregistration": {"path": str(prereg.DEFAULT_OUTPUT), "sha256": PREREG_SHA, "manifest_hash": registration["manifest_hash"]},
        "source_manifest": {"path": str(MANIFEST), "sha256": sha(MANIFEST), "manifest_hash": source_manifest["manifest_hash"]},
        "completed_preentry_sources_opened": True, "funding_rate_signal_column_opened": False,
