"""Build outcome-blind source support for frozen HVCACLR-8."""
...
from training import preregister_high_volatility_cross_alt_close_location_consensus_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-04-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="f4739719fd044d14c09d89e4b8532bb660708db98053e563f97f198efacdd06c"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_cross_alt_close_location_consensus_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_cross_alt_close_location_consensus_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_cross_alt_close_location_consensus_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_cross_alt_close_location_consensus_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_cross_alt_close_location_consensus_relay_support_2026-08-11.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","consensus_location","strength_rank","equal_weight_location","consensus_breadth","realized_variation","variation_rank","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","consensus_location","strength_rank","equal_weight_location","consensus_breadth","realized_variation","variation_rank")
...
def close_location_consensus_statistics(block:pd.DataFrame)->tuple[float,float,int,float]:
 locations=[]
 for symbol in ALTS:
  x=block.xs(symbol,level="symbol");high=float(x.high.max());low=float(x.low.min());close=float(x.close.iloc[-1]);den=high-low
  if not math.isfinite(den) or den<=0:return math.nan,math.nan,0,math.nan
  value=float((2*close-high-low)/den)
  if not math.isfinite(value) or value==0:return math.nan,math.nan,0,math.nan
  locations.append(value)
 median=float(np.median(locations));side=int(np.sign(median));breadth=sum(int(np.sign(x))==side for x in locations);equal=float(np.sum(locations))
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","symbol","open","high","low","close"]:raise RuntimeError("HVCACLR source schema drift")
 ...
 if x.ts.isna().any() or x.duplicated(["ts","symbol"]).any() or not x.symbol.isin(SYMBOLS).all():raise RuntimeError("HVCACLR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("8h"),END,freq="1h",inclusive="left"):
  ...
  if path_valid:
   pressure,equal,breadth,var=close_location_consensus_statistics(block);valid=all(math.isfinite(v) for v in (pressure,equal,var)) and pressure!=0 and var>0
  else:pressure=equal=var=math.nan;breadth=0;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"consensus_location":pressure,"equal_weight_location":equal,"consensus_breadth":breadth,"realized_variation":var})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["strength_rank"]=prior_rank(panel.consensus_location.abs().where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));panel["eligible"]=valid&panel.strength_rank.ge(P["strength_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"]);panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_hour_stale_geometry":
  cols=["source_valid","consensus_location","strength_rank","equal_weight_location","consensus_breadth","variation_rank","feature_available_time"];used[cols]=panel[cols].shift(1)
 ...
 elif control=="four_of_six_breadth":state=valid&used.consensus_breadth.ge(4)&strong&variation
 onset=previous_valid_onset(state,valid);side=np.sign(pd.to_numeric(used.consensus_location,errors="coerce").fillna(0)).astype(int)
 if control=="equal_weight_close_location_sum":side=np.sign(pd.to_numeric(used.equal_weight_location,errors="coerce").fillna(0)).astype(int)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCACLR prereg drift")
