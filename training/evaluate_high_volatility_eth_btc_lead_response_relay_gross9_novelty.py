"""Evaluate frozen HVEBLR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVEBLR-8"
PROTOCOL = "hveblr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_eth_btc_lead_response_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVEBLR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVEBLR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVEBLR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVEBLR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hveblr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVEBLR-8 unchanged before economic outcomes",
    }
