"""Sequential strict economics for frozen SHRR-12."""
...
PREREG = Path("results/scandinavian_haven_risk_rotation_relay_preregistration_2026-08-09.json")
PREREG_SHA = "edd262b953e0e579ad38cfe4e2a623b176924c9f25afc8c779ce68cb8dae6d10"
SUPPORT = Path("results/scandinavian_haven_risk_rotation_relay_support_2026-08-09.json")
SUPPORT_SHA = "c0372a0945a7a12eaa4531abe6ecc26195e14a23064632b2d01c8d8ad71a89e5"
NOVELTY = Path("results/scandinavian_haven_risk_rotation_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "f41fbb16b0a736642e0d281312f248905db908cfd37aadeb5f3a469278826ecd"
CLOCK = Path("data/scandinavian_haven_risk_rotation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "7d8b1d3edf0961a58002de8c4776fbe847092551dd2f539c1b21cfb97af1584c"
CONTROL_DIR = Path("data/scandinavian_haven_risk_rotation_relay_controls_2023_2026")
FREEZE = Path("results/scandinavian_haven_risk_rotation_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/scandinavian_haven_risk_rotation_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "no_risk_pressure_tail", "no_haven_orientation", "one_session_stale_risk_pressure", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("SHRR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("SHRR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "shrr_12_sequential_economics_v1", "policy_id": POLICY_ID,
