"""Evaluate frozen OVRCR-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "OVRCR-6"
PROTOCOL = "ovrcr_6_gross9_novelty_v1"
...
OUTPUT = Path("results/oil_volatility_rotation_btc_confirmation_relay_gross9_novelty_2026-08-08.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha256(PREREG) != PREREG_SHA or sha256(SUPPORT) != SUPPORT_SHA or sha256(CLOCK) != CLOCK_SHA:
        raise RuntimeError("OVRCR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("OVRCR novelty limits drift")
    if (
        ...
        raise RuntimeError("OVRCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="OVRCR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "ovrcr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject OVRCR-6 unchanged before economic outcomes",
    }
