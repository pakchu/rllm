"""Fresh V3 OVEPR economics with a directly verified package entrypoint."""
...
FAILURE=Path('results/options_led_volatility_expansion_premium_relay_train_economic_attempt_terminal_failure_v2_2026-08-08.json');FAILURE_SHA='fc933789698833ad5f8f7528555512b6cd6e68aa1124d31763567523ee6ad55a'
...
OUTPUTS={s:Path(f'results/options_led_volatility_expansion_premium_relay_{s}_economics_v3_2026-08-08.json') for s in STAGES}
...
def verify(stage:str)->dict[str,Any]:
 ...
 if sha(FAILURE)!=FAILURE_SHA:raise RuntimeError('V2 terminal failure receipt drift')
 failure=load_json(FAILURE)
 if failure.get('economic_metrics_computed') is not False or failure.get('btc_execution_source_opened') is not False or failure.get('attempt_disposition')!='TERMINAL_FAILURE_NO_RETRY_UNDER_V2':raise RuntimeError('V2 failure is not eligible for infrastructure successor')
