"""Build outcome-blind source support for frozen HVDUDAR-8."""
...
from training import preregister_high_volatility_drawup_drawdown_asymmetry_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-04-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="bdb0a9c000fb8091b7f9a3fc99b8f186b7781a7573dc613715dcf1316d9cc017"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_drawup_drawdown_asymmetry_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_drawup_drawdown_asymmetry_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_drawup_drawdown_asymmetry_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_drawup_drawdown_asymmetry_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_drawup_drawdown_asymmetry_relay_support_2026-08-10.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","maximum_drawup","maximum_drawdown","excursion_asymmetry","asymmetry_rank","endpoint_displacement","endpoint_rank","realized_variation","variation_rank","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","maximum_drawup","maximum_drawdown","excursion_asymmetry","asymmetry_rank","endpoint_displacement","endpoint_rank","realized_variation","variation_rank")
...
def excursion_statistics(block:pd.DataFrame)->tuple[float,float,float,float,float]:
 levels=np.log(block.close.to_numpy(float));drawup=float(np.max(levels-np.minimum.accumulate(levels)));drawdown=float(np.max(np.maximum.accumulate(levels)-levels));asymmetry=(drawup-drawdown)/(drawup+drawdown);endpoint=float(np.log(block.close.iloc[-1]/block.open.iloc[0]));minute_returns=np.log(block.close.to_numpy(float)/block.open.to_numpy(float));variation=float(np.sqrt(np.square(minute_returns).sum()));return drawup,drawdown,asymmetry,endpoint,variation
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVDUDAR source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVDUDAR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("5h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   drawup,drawdown,asymmetry,endpoint,var=excursion_statistics(block);valid=all(math.isfinite(v) for v in (drawup,drawdown,asymmetry,endpoint,var)) and drawup+drawdown>0 and var>0 and asymmetry!=0
  else:drawup=drawdown=asymmetry=endpoint=var=math.nan;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"maximum_drawup":drawup,"maximum_drawdown":drawdown,"excursion_asymmetry":asymmetry,"endpoint_displacement":endpoint,"realized_variation":var})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["asymmetry_rank"]=prior_rank(panel.excursion_asymmetry.abs().where(valid));panel["endpoint_rank"]=prior_rank(panel.endpoint_displacement.abs().where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));panel["eligible"]=valid&panel.asymmetry_rank.ge(P["asymmetry_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"]);panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_block_stale_excursions":
  current_valid=panel.source_valid.eq(True);cols=["maximum_drawup","maximum_drawdown","excursion_asymmetry","asymmetry_rank","endpoint_displacement","endpoint_rank","feature_available_time"];used[cols]=panel[cols].shift(1);used["source_valid"]=current_valid&panel.source_valid.shift(1).fillna(False)
 valid=used.source_valid.eq(True);strong=used.asymmetry_rank.ge(P["asymmetry_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&strong&variation
 if control=="no_asymmetry_tail":state=valid&variation
 elif control=="no_variation_gate":state=valid&strong
 elif control=="endpoint_displacement":state=valid&used.endpoint_displacement.ne(0)&used.endpoint_rank.ge(P["asymmetry_rank_min"])&variation
 onset=previous_valid_onset(state,valid);side=np.sign(pd.to_numeric(used.excursion_asymmetry,errors="coerce").fillna(0)).astype(int)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVDUDAR prereg drift")
