"""Build outcome-blind source support for frozen HVELO-24."""
...
from training import preregister_high_volatility_ehlers_laguerre_oscillator_zero_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "d82d5183fd785522df51c9c17fc9e88880529371f866fe564dd15bc501985520"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ehlers_laguerre_oscillator_zero_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ehlers_laguerre_oscillator_zero_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ehlers_laguerre_oscillator_zero_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ehlers_laguerre_oscillator_zero_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ehlers_laguerre_oscillator_zero_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "run_length", "ultimate_smoother", "laguerre_state", "difference",
    "rms", "laguerre_oscillator", "rate", "alternate_laguerre_oscillator", "entry_side",
    "alternate_entry_side", "rate_entry_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "run_length", "ultimate_smoother", "laguerre_state",
    "difference", "rms", "laguerre_oscillator", "rate", "alternate_laguerre_oscillator",
    "entry_side", "alternate_entry_side", "rate_entry_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVELO source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVELO invalid source key")
...
def laguerre_filter(series: pd.Series, valid: pd.Series, gamma: float, period: int) -> pd.DataFrame:
    a1=math.exp(-1.414*math.pi/period);c2=2*a1*math.cos(1.414*math.pi/period);c3=-a1*a1;c1=(1+c2-c3)/4
    values=series.to_numpy(float);ok=valid.to_numpy(bool);smooth=np.full(len(series),np.nan);state=np.full(len(series),np.nan);diff=np.full(len(series),np.nan);rms=np.full(len(series),np.nan);osc=np.full(len(series),np.nan);runs=np.zeros(len(series),dtype=int);run=0;window=[]
    for i,(price,is_valid) in enumerate(zip(values,ok)):
        if not is_valid or not math.isfinite(price):run=0;window=[];continue
        run+=1;runs[i]=run
        if run<=3:smooth[i]=price
        else:smooth[i]=(1-c1)*price+(2*c1-c2)*values[i-1]-(c1+c3)*values[i-2]+c2*smooth[i-1]+c3*smooth[i-2]
        prior_smooth=0. if run==1 else smooth[i-1];prior_state=0. if run==1 else state[i-1]
        state[i]=-gamma*smooth[i]+prior_smooth+gamma*prior_state;diff[i]=smooth[i]-state[i]
        window.append(float(diff[i]));window=window[-P["rms_periods"]:]
        if len(window)==P["rms_periods"]:
            scale=math.sqrt(float(np.mean(np.square(window))))
            if scale>0:rms[i]=scale;osc[i]=diff[i]/scale
    oscillator=pd.Series(osc,index=series.index)
    return pd.DataFrame({"run_length":runs,"ultimate_smoother":smooth,"laguerre_state":state,"difference":diff,"rms":rms,"laguerre_oscillator":oscillator,"rate":oscillator-oscillator.shift(1)},index=series.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    fields=["bar_open","bar_high","bar_low","bar_close"];bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[fields]).all(axis=1)&bars[fields].gt(0).all(axis=1);values=laguerre_filter(bars.bar_close,bars.valid_bar,P["gamma"],P["smoother_periods"]);alternate=laguerre_filter(bars.bar_close,bars.valid_bar,P["alternate_control_gamma"],P["alternate_control_periods"])
    for column in values:bars[column]=values[column]
    bars["alternate_laguerre_oscillator"]=alternate.laguerre_oscillator;bars["entry_side"]=cross_side(bars.laguerre_oscillator);bars["alternate_entry_side"]=cross_side(bars.alternate_laguerre_oscillator);bars["rate_entry_side"]=cross_side(bars.rate);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["ultimate_smoother","laguerre_state","difference","rms","laguerre_oscillator","variation"];bars["source_valid"]=bars.valid_bar&bars.run_length.ge(P["rms_periods"])&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="alternate_0_8_20":side=pd.to_numeric(used.alternate_entry_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
    elif control=="oscillator_roc_turn":side=pd.to_numeric(used.rate_entry_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVELO prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvelo_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvelo_24_source_support_v1", "policy_id": prereg.POLICY_ID,
