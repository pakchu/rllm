"""Open source-only OOS incidence for preregistered HVSCFM-24."""
...
from training import preregister_high_volatility_sha256_chain_fee_migration_relay as prereg
...
PREREG_SHA256 = "09f62e1a7f0f417d23b4731b6e978dff51c7e45cbdafdc9df1154c3df23b0993"
API = "https://community-api.coinmetrics.io/v4/timeseries/asset-metrics"
SOURCE_DIR = Path("data/high_volatility_sha256_chain_fee_migration_relay_sources_2023_2026")
PAIR_PANEL = SOURCE_DIR / "btc_bch_fee_pressure_daily.csv.gz"
FEATURE_PANEL = SOURCE_DIR / "daily_feature_panel.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_sha256_chain_fee_migration_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_sha256_chain_fee_migration_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_sha256_chain_fee_migration_relay_support_2026-08-10.json")
QUERY = """
...
CLOCK_COLUMNS = (
    ...
    "decision_time", "entry_time", "exit_time", "side", "btc_fee_total", "btc_issuance",
    "bch_fee_total", "bch_issuance", "btc_fee_pressure", "bch_fee_pressure",
    "relative_fee_pressure", "fee_migration", "absolute_migration_rank",
    "btc_fee_pressure_change", "realized_variation", "realized_variation_rank",
)
...
def parse_source_row(raw: Any) -> dict[str, Any]:
    if not isinstance(raw, dict) or set(raw) != {
        "asset", "time", "FeeTotNtv", "IssTotNtv", "AssetEODCompletionTime"
    }:
    ...
    fee_total = float(raw["FeeTotNtv"])
    issuance = float(raw["IssTotNtv"])
    if (
        observation.tzinfo is None or observation != observation.floor("1d")
        or not math.isfinite(fee_total) or fee_total < 0
        or not math.isfinite(issuance) or issuance <= 0
...
def download_pair_panel() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    pair = pd.DataFrame({
        ...
        "btc_fee_total": by_asset["btc"].fee_total.to_numpy(float),
        "btc_issuance": by_asset["btc"].issuance.to_numpy(float),
        "bch_fee_total": by_asset["bch"].fee_total.to_numpy(float),
        "bch_issuance": by_asset["bch"].issuance.to_numpy(float),
    })
...
def feature_panel(pair: pd.DataFrame, bars: pd.DataFrame) -> pd.DataFrame:
    ...
    frame = pair.copy()
    frame["btc_fee_pressure"] = frame.btc_fee_total / frame.btc_issuance
    frame["bch_fee_pressure"] = frame.bch_fee_total / frame.bch_issuance
    positive_pressure = frame.btc_fee_pressure.gt(0) & frame.bch_fee_pressure.gt(0)
    frame["relative_fee_pressure"] = np.log(
        frame.btc_fee_pressure.where(positive_pressure)
        / frame.bch_fee_pressure.where(positive_pressure)
    )
    frame["fee_migration"] = frame.relative_fee_pressure.diff(3)
    frame["btc_fee_pressure_change"] = np.log(
        frame.btc_fee_pressure.where(frame.btc_fee_pressure.gt(0))
    ).diff(3)
...
def candidate_clock(panel: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    btc_change = frame.btc_fee_pressure_change
    ...
    side_source = btc_change if control == "btc_fee_pressure_change_only" else migration
    ...
    for index in frame.index[onset]:
        ...
        rows.append({
            ...
            "btc_fee_total": float(frame.at[index, "btc_fee_total"]),
            "btc_issuance": float(frame.at[index, "btc_issuance"]),
            "bch_fee_total": float(frame.at[index, "bch_fee_total"]),
            "bch_issuance": float(frame.at[index, "bch_issuance"]),
            "btc_fee_pressure": float(frame.at[index, "btc_fee_pressure"]),
            "bch_fee_pressure": float(frame.at[index, "bch_fee_pressure"]),
            "relative_fee_pressure": float(frame.at[index, "relative_fee_pressure"]),
            "fee_migration": float(migration.at[index]),
            "absolute_migration_rank": float(migration_rank.at[index]),
            "btc_fee_pressure_change": float(btc_change.at[index]),
...
def run() -> dict[str, Any]:
    ...
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA256:
        raise RuntimeError("HVSCFM preregistration artifact drift")
    registration = json.loads(prereg.DEFAULT_OUTPUT.read_text())
    if registration != prereg.build():
        raise RuntimeError("HVSCFM preregistration payload drift")
    ...
    source_core = {
        "protocol_version": "hvscfm_24_source_materialization_v1",
    ...
    core = {
        "protocol_version": "hvscfm_24_oos_source_support_v1",
        "policy_id": "HVSCFM-24",
        "preregistration": {"path": str(prereg.DEFAULT_OUTPUT), "sha256": PREREG_SHA256, "manifest_hash": registration["manifest_hash"]},
