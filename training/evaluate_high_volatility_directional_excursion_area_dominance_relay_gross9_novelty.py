"""Evaluate frozen HVDEADR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDEADR-8"
PROTOCOL = "hvdeadr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_directional_excursion_area_dominance_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDEADR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVDEADR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDEADR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDEADR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdeadr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVDEADR-8 unchanged before economic outcomes",
    }
