"""Sequential strict economics for frozen HVTICC-8."""
...
PREREG = Path("results/high_volatility_taker_imbalance_concentration_continuation_preregistration_2026-08-13.json")
PREREG_SHA = "85b7b62a4076b39ec5ae274c42d2d9022b75230237bde67ebf21131d03ed2a02"
SUPPORT = Path("results/high_volatility_taker_imbalance_concentration_continuation_support_2026-08-13.json")
SUPPORT_SHA = "7503ee375d978fc11a2c21343553d535d25adedeedcc61bab7fa953295223b20"
NOVELTY = Path("results/high_volatility_taker_imbalance_concentration_continuation_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "7adeb91d03fa1b3a08bac75866ef1d184dd7b0f431e257c03789ce9980bede59"
CLOCK = Path("data/high_volatility_taker_imbalance_concentration_continuation_clocks_2023_2026.csv.gz")
CLOCK_SHA = "55e1cc00aee7f8bdb30726ccd101be668278fb3ba4b63b15f71a384eb20e9719"
CONTROL_DIR = Path("data/high_volatility_taker_imbalance_concentration_continuation_controls_2023_2026")
FREEZE = Path("results/high_volatility_taker_imbalance_concentration_continuation_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_taker_imbalance_concentration_continuation_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_concentration_tail", "no_variation_gate", "aggregate_net_flow_tail", "one_decision_stale_concentration", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTICC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTICC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvticc_8_sequential_economics_v1", "policy_id": POLICY_ID,
