"""Build outcome-blind source support for frozen HVSMIE-24."""
...
from training import preregister_high_volatility_smi_ergodic_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "ea11f758d83e7ee568e2a9343f9652f3f3c6c2c3a7804a613360ff5d5149fad0"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_smi_ergodic_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_smi_ergodic_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_smi_ergodic_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_smi_ergodic_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_smi_ergodic_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "change", "fast_change", "fast_abs_change",
    "smoothed_change", "smoothed_abs_change", "smi", "signal", "difference",
    "entry_side", "zero_line_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "change", "fast_change", "fast_abs_change",
    "smoothed_change", "smoothed_abs_change", "smi", "signal", "difference",
    "entry_side", "zero_line_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVSMIE source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVSMIE invalid source key")
...
def smi_values(series: pd.Series, valid: pd.Series) -> pd.DataFrame:
    change=(series-series.shift(1)).where(valid&valid.shift(1,fill_value=False));ok=np.isfinite(change)
    fast_change=causal_ema(change,ok,P["fast_periods"]);fast_abs=causal_ema(change.abs(),ok,P["fast_periods"])
    smooth_change=causal_ema(fast_change,np.isfinite(fast_change),P["slow_periods"]);smooth_abs=causal_ema(fast_abs,np.isfinite(fast_abs),P["slow_periods"])
    smi=(smooth_change/smooth_abs).where(smooth_abs.gt(0));signal=causal_ema(smi,np.isfinite(smi),P["signal_periods"])
    return pd.DataFrame({"change":change,"fast_change":fast_change,"fast_abs_change":fast_abs,"smoothed_change":smooth_change,"smoothed_abs_change":smooth_abs,"smi":smi,"signal":signal,"difference":smi-signal},index=series.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    values=smi_values(bars.bar_close,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["entry_side"]=cross_side(bars.difference);bars["zero_line_side"]=cross_side(bars.smi)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["change","fast_change","fast_abs_change","smoothed_change","smoothed_abs_change","smi","signal","difference","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.smoothed_abs_change.gt(0)&bars.variation.gt(0)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVSMIE prereg drift")
    raw = load_source()
