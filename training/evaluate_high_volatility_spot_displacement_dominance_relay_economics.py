"""Sequential strict economics for frozen HVSDD-8."""
...
PREREG = Path("results/high_volatility_spot_displacement_dominance_relay_preregistration_2026-08-13.json")
PREREG_SHA = "4ba52d6f20a94293ae20e12b43dd71008e0420523315bbd8a840b5cceaa28d9e"
SUPPORT = Path("results/high_volatility_spot_displacement_dominance_relay_support_2026-08-13.json")
SUPPORT_SHA = "d108d751839850eb20fbd832169b84363673095a0a1a9142e693e234a400856b"
NOVELTY = Path("results/high_volatility_spot_displacement_dominance_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "c9f7d66a4c12fca96242b802b10620229f1fca597754db50847618865dd9fa51"
CLOCK = Path("data/high_volatility_spot_displacement_dominance_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "672385d2eedce4da3fc3a832e777d71b210ed5e8b17fe34ffacf91e177ad48d6"
CONTROL_DIR = Path("data/high_volatility_spot_displacement_dominance_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_spot_displacement_dominance_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_spot_displacement_dominance_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "simple_majority_dominance", "no_aggregate_confirmation", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSDD frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSDD novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvsdd_8_sequential_economics_v1", "policy_id": POLICY_ID,
