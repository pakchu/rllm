"""Build outcome-blind source support for frozen HVCACM-8."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_cross_alt_channel_position_migration_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2023-01-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="8f2a1379e27605e0e2a496f55051edc1571dc567f5db5038f067d9a265be8a0d";REG=prereg.build();P=REG["policy"];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG["stages"].items()};GATES=REG["source_support_gates"];CONTROLS=tuple(REG["diagnostic_controls"]["names"]);SYMBOLS=("BTCUSDT",*prereg.ALTS);ALTS=prereg.ALTS
...
ROOT=Path("data/high_volatility_cross_alt_channel_position_migration_relay_sources_2023_2026");PANEL=ROOT/"scheduled_channel_migration_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_cross_alt_channel_position_migration_relay_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_cross_alt_channel_position_migration_relay_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_cross_alt_channel_position_migration_relay_controls_2023_2026");RESULT=Path("results/high_volatility_cross_alt_channel_position_migration_relay_support_2026-08-13.json");BUILDER=Path(__file__).relative_to(Path.cwd())
BASE_COLS=("decision_time","feature_available_time","source_valid","migration_score","selected_alts","agreeing_alts","side","btc_realized_variation","variation_rank","variation_active","eligible")
CLOCK_COLS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","migration_score","selected_alts","agreeing_alts","btc_realized_variation","variation_rank")
...
def prepare(raw):
 ...
 if raw.columns.tolist()!=expected:raise RuntimeError("HVCACM source schema drift")
...
def migration_frame(ret,high,low,close,valid):
 scheduled=pd.date_range(START,END,freq="12h",inclusive="left")+pd.Timedelta("2h30m");scheduled=scheduled[(scheduled>=START)&(scheduled<END)];rows=[];window_valid=valid.rolling(P["variation_bars"],min_periods=P["variation_bars"]).sum().eq(P["variation_bars"]).all(axis=1);rv=np.sqrt(ret.BTCUSDT.pow(2).where(valid.BTCUSDT).rolling(P["variation_bars"],min_periods=P["variation_bars"]).sum());positions={};fixed={}
 for a in ALTS:
  hi=high[a].where(valid[a]).rolling(P["channel_bars"],min_periods=P["channel_bars"]).max();lo=low[a].where(valid[a]).rolling(P["channel_bars"],min_periods=P["channel_bars"]).min();positions[a]=(close[a]-lo)/(hi-lo).where(hi.gt(lo));fixed[a]=(close[a]-lo.shift(P["migration_lag_bars"]))/(hi.shift(P["migration_lag_bars"])-lo.shift(P["migration_lag_bars"])).where(hi.shift(P["migration_lag_bars"]).gt(lo.shift(P["migration_lag_bars"])))
 for decision in scheduled:
  ...
  for a in ALTS:
   prior=end-pd.Timedelta(minutes=5*P["migration_lag_bars"]);current=positions[a].at[end];previous=positions[a].at[prior] if prior in positions[a].index else np.nan;fixed_current=fixed[a].at[end];row[f"{a}_migration"]=float(current-previous) if math.isfinite(current) and math.isfinite(previous) else np.nan;row[f"{a}_fixed_prior_channel_migration"]=float(fixed_current-previous) if math.isfinite(fixed_current) and math.isfinite(previous) else np.nan
  rows.append(row)
 return pd.DataFrame(rows)
def score_states(f,control="primary"):
 u=f.copy();ranks={}
 for a in ALTS:ranks[a]=midrank(u[f"{a}_migration"].abs().where(u.source_valid),P["migration_prior_decisions"],P["migration_minimum_decisions"]);u[f"{a}_migration_rank"]=ranks[a]
 ...
 for i,row in u.iterrows():
  chosen=[a for a in ALTS if math.isfinite(row[f"{a}_migration"]) and (control=="no_migration_tail" or row[f"{a}_migration_rank"]>=P["migration_rank_min"])]
  value={a:(row[f"{a}_fixed_prior_channel_migration"] if control=="fixed_prior_channel" else row[f"{a}_migration"]) for a in chosen};finite=[value[a] for a in chosen if math.isfinite(value[a])];score=float(np.median(finite)) if finite else np.nan;sgn=int(np.sign(score)) if math.isfinite(score) else 0;scores.append(score);selected.append(len(finite));agree.append(sum(np.sign(v)==sgn for v in finite) if sgn else 0)
 u["migration_score"]=scores;u["selected_alts"]=selected;u["agreeing_alts"]=agree;minimum=3 if control=="three_alt_consensus" else P["minimum_selected_alts"];u["side"]=np.sign(u.migration_score).fillna(0).astype(int);u["variation_active"]=u.variation_rank.ge(P["variation_rank_min"]);u["eligible"]=u.source_valid&u.selected_alts.ge(minimum)&u.migration_score.abs().ge(P["score_absolute_min"])&u.agreeing_alts.ge(P["minimum_agreeing_alts"])&u.variation_active
 if control=="no_variation_gate":u["eligible"]=u.source_valid&u.selected_alts.ge(minimum)&u.migration_score.abs().ge(P["score_absolute_min"])&u.agreeing_alts.ge(P["minimum_agreeing_alts"])
 if control=="one_decision_stale_migration":
  for c in ("migration_score","selected_alts","agreeing_alts","side"):u[c]=u[c].shift(1)
  u["eligible"]=u.source_valid&u.selected_alts.ge(minimum)&u.migration_score.abs().ge(P["score_absolute_min"])&u.agreeing_alts.ge(P["minimum_agreeing_alts"])&u.variation_active
 if control=="direction_flip":u.side=-u.side
 if control=="forced_long":u.side=1
 return u
def build_panel(raw,control="primary"):
 x=prepare(raw);grid=pd.date_range(START,END,freq="5min",inclusive="left");ret=x.pivot(index="date",columns="symbol",values="return").reindex(grid,columns=SYMBOLS);high=x.pivot(index="date",columns="symbol",values="high").reindex(grid,columns=SYMBOLS);low=x.pivot(index="date",columns="symbol",values="low").reindex(grid,columns=SYMBOLS);close=x.pivot(index="date",columns="symbol",values="close").reindex(grid,columns=SYMBOLS);valid=x.pivot(index="date",columns="symbol",values="valid").reindex(grid,columns=SYMBOLS).eq(True);return score_states(migration_frame(ret,high,low,close,valid),control)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCACM prereg drift")
