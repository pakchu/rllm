"""Sequential strict economics for frozen ACUCR-12."""
...
PREREG = Path("results/asian_carry_unwind_concordance_relay_preregistration_2026-08-09.json")
PREREG_SHA = "5dc2e3b6df289a8206a581c38dc91484b4039ed479d75b3e02a82b1a8ccb7eb7"
SUPPORT = Path("results/asian_carry_unwind_concordance_relay_support_2026-08-09.json")
SUPPORT_SHA = "525598b25a691b6f1b61335b0eb1fe4baa2e7fe47d03f9cd77861126438f649b"
NOVELTY = Path("results/asian_carry_unwind_concordance_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "f0b6e6d513ae9e8a397600b1d8e06cb4dcc27cab533ffe2b24069b91509f85fb"
CLOCK = Path("data/asian_carry_unwind_concordance_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "35ae6762bc6494f7a05a95d94c0d44ca9c28e812bdd0db82103ef0be56f144bc"
CONTROL_DIR = Path("data/asian_carry_unwind_concordance_relay_controls_2023_2026")
FREEZE = Path("results/asian_carry_unwind_concordance_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/asian_carry_unwind_concordance_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "aud_leg_only", "jpy_leg_only", "one_session_stale_concordance", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("ACUCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("ACUCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "acucr_12_sequential_economics_v1", "policy_id": POLICY_ID,
