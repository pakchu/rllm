"""Deterministic outcome-blind source support for HVCFMR-8."""
...
ROOT = Path("data/high_volatility_causal_funding_move_reversal_sources_2023_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_funding_move_reversal_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_funding_move_reversal_split_clocks_2023_2026")
RESULT = Path("results/high_volatility_causal_funding_move_reversal_support_2026-08-16.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "funding_rate",
    "absolute_level_rank", "completed_return",
    "realized_variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "decision_time", "feature_available_time",
    "entry_time", "exit_time", "side", "funding_rate", "completed_return",
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if funding.columns.tolist() != ["funding_time", "funding_rate"]:
        raise RuntimeError("HVCFMR-8 funding schema drift")
    expected_bars = [
    ...
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCFMR-8 bar schema drift")
    ...
    if funding["funding_time"].duplicated().any() or not np.isfinite(funding["funding_rate"]).all():
        raise RuntimeError("HVCFMR-8 invalid funding rows")
    ...
    if bars["decision_time"].duplicated().any():
        raise RuntimeError("HVCFMR-8 duplicate eight-hour bar")
    ...
    bars["completed_return"] = np.log(bars["last_close"] / bars["first_open"])
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel = panel.join(
        bars[["bar_valid", "realized_variation", "completed_return"]],
        on="funding_time",
    )
    panel["source_valid"] = (
        ...
        & np.isfinite(
            panel[["funding_rate", "realized_variation", "completed_return"]]
        ).all(axis=1)
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCFMR-8 side or availability drift")
...
def run() -> dict[str, Any]:
    ...
    source_core = {
        "protocol_version": "hvcfmr_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvcfmr_8_source_support_v1", "policy_id": prereg.POLICY_ID,
