"""Sequential strict economics for frozen HVMPS-12."""
...
PREREG = Path("results/high_volatility_month_phase_seasonality_relay_preregistration_2026-08-10.json")
PREREG_SHA = "2e1f9577317c7ec77c49a35b6e0ae781ebc322beb5a8d207296c8ae63f3b734e"
SUPPORT = Path("results/high_volatility_month_phase_seasonality_relay_support_2026-08-10.json")
SUPPORT_SHA = "76519c115f629774121912bbca1be3b2dd90851f886b1501cacc3a4a298913f7"
NOVELTY = Path("results/high_volatility_month_phase_seasonality_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "19d89bd0b5dcd763e43110669048ef41d9e7285713f42288e8e9f11705984a55"
CLOCK = Path("data/high_volatility_month_phase_seasonality_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "2e9e5016a393ed8f5ab5224c335fe2f2bbc97c6973444a2fed74ed7bd23815ba"
CONTROL_DIR = Path("data/high_volatility_month_phase_seasonality_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_month_phase_seasonality_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_month_phase_seasonality_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVMPS frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVMPS novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvmps_12_sequential_economics_v1", "policy_id": POLICY_ID,
