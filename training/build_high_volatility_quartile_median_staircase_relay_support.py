"""Build outcome-blind source support for frozen HVQMSR-8."""
...
from training import preregister_high_volatility_quartile_median_staircase_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2022-10-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="d1fac61ae887f9b1c63979aef242488f1eca23dcb7a6777ab94ef7b39781c94c"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_quartile_median_staircase_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_quartile_median_staircase_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_quartile_median_staircase_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_quartile_median_staircase_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_quartile_median_staircase_relay_support_2026-08-10.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","median_1","median_2","median_3","median_4","staircase_side","migration_span","migration_rank","endpoint_displacement","endpoint_rank","realized_variation","variation_rank","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","median_1","median_2","median_3","median_4","staircase_side","migration_span","migration_rank","endpoint_displacement","endpoint_rank","realized_variation","variation_rank")
...
def staircase_statistics(block:pd.DataFrame)->tuple[float,float,float,float,int,float,float,float]:
 levels=np.log(block.close.to_numpy(float)).reshape(4,120);medians=np.median(levels,axis=1)
 side=1 if bool(np.all(np.diff(medians)>0)) else -1 if bool(np.all(np.diff(medians)<0)) else 0
 migration=float(abs(medians[-1]-medians[0]));endpoint=float(np.log(block.close.iloc[-1]/block.open.iloc[0]));minute_returns=np.log(block.close.to_numpy(float)/block.open.to_numpy(float));variation=float(np.sqrt(np.square(minute_returns).sum()))
 return float(medians[0]),float(medians[1]),float(medians[2]),float(medians[3]),side,migration,endpoint,variation
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVQMSR source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVQMSR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("1h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   m1,m2,m3,m4,side,migration,endpoint,var=staircase_statistics(block);valid=all(math.isfinite(v) for v in (m1,m2,m3,m4,migration,endpoint,var)) and migration>0 and var>0
  else:m1=m2=m3=m4=migration=endpoint=var=math.nan;side=0;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"median_1":m1,"median_2":m2,"median_3":m3,"median_4":m4,"staircase_side":side,"migration_span":migration,"endpoint_displacement":endpoint,"realized_variation":var})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["migration_rank"]=prior_rank(panel.migration_span.where(valid));panel["endpoint_rank"]=prior_rank(panel.endpoint_displacement.abs().where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));panel["eligible"]=valid&panel.staircase_side.ne(0)&panel.migration_rank.ge(P["migration_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"]);panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_block_stale_staircase":
  current_valid=panel.source_valid.eq(True);cols=["median_1","median_2","median_3","median_4","staircase_side","migration_span","migration_rank","endpoint_displacement","endpoint_rank","feature_available_time"];used[cols]=panel[cols].shift(1);used["source_valid"]=current_valid&panel.source_valid.shift(1).fillna(False)
 valid=used.source_valid.eq(True);staircase=used.staircase_side.ne(0);strong=used.migration_rank.ge(P["migration_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&staircase&strong&variation
 if control=="no_migration_tail":state=valid&staircase&variation
 elif control=="no_variation_gate":state=valid&staircase&strong
 elif control=="endpoint_displacement":state=valid&used.endpoint_displacement.ne(0)&used.endpoint_rank.ge(P["migration_rank_min"])&variation
 onset=previous_valid_onset(state,valid);side=pd.to_numeric(used.staircase_side,errors="coerce").fillna(0).astype(int)
 if control=="endpoint_displacement":side=np.sign(pd.to_numeric(used.endpoint_displacement,errors="coerce").fillna(0)).astype(int)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVQMSR prereg drift")
 ...
 source_core={"protocol_version":"hvqmsr_8_source_v1","query":QUERY,"query_sha256":hashlib.sha256(QUERY.encode()).hexdigest(),"window":[START.isoformat(),END.isoformat()],"physical_rows":len(raw),"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"panel":{"path":str(PANEL),"sha256":sha(PANEL),"rows":len(panel),"valid_rows":int(panel.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};manifest={**source_core,"manifest_hash":chash(source_core)};immutable(MANIFEST,json_bytes(manifest))
