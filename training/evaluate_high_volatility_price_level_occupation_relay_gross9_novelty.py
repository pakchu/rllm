"""Evaluate frozen HVPLO-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVPLO-8"
PROTOCOL = "hvplo_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_price_level_occupation_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVPLO predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVPLO novelty limits drift")
    if (
        ...
        raise RuntimeError("HVPLO predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVPLO primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVPLO-8 unchanged before economic outcomes",
    }
