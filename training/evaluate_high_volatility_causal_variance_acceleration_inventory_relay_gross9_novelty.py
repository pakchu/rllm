"""Evaluate the frozen HVCVARIR-8 action-vote clock against Gross9."""
...
POLICY = "HVCVARIR-8"
PROTOCOL = "hvcvarir_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_variance_acceleration_inventory_relay_clocks_2020_2026.csv.gz",
    "sha256": "86f7e4f3b7624396b5a39be132b3c1630087648fbabf2d38a6604863e21b507c",
    "rows": 152,
}
OUTPUT = Path("results/high_volatility_causal_variance_acceleration_inventory_relay_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVCVARIR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCVARIR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVARIR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCVARIR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCVARIR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCVARIR-8 unchanged before economics",
    }
