"""Build outcome-blind source support for frozen HVGATOR-24."""
...
from training import preregister_high_volatility_gator_awakening_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2023-04-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "ed9f9d6146b09519acbf167a67b0541d1ef9026603fe25d27bafd51c907bbfc3"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_gator_awakening_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_gator_awakening_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_gator_awakening_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_gator_awakening_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_gator_awakening_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "raw_jaw", "raw_teeth", "raw_lips", "visible_jaw",
    "visible_teeth", "visible_lips", "upper_gap", "lower_gap", "expanding",
    "entry_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "visible_jaw", "visible_teeth", "visible_lips",
    "upper_gap", "lower_gap", "expanding", "entry_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVGATOR source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVGATOR invalid source key")
...
def smma(values: pd.Series, valid: pd.Series, period: int) -> pd.Series:
    output=np.full(len(values),np.nan);seed=[];state=None;poisoned=False
    for i,(value,is_valid) in enumerate(zip(pd.to_numeric(values,errors="coerce"),valid)):
        if poisoned:continue
        if not bool(is_valid) or not math.isfinite(float(value)):
            poisoned=True;continue
        if state is None:
            seed.append(float(value))
            if len(seed)==period:state=float(np.mean(seed));output[i]=state
        else:
            state=(state*(period-1)+float(value))/period;output[i]=state
    return pd.Series(output,index=values.index)

def gator(high: pd.Series, low: pd.Series, valid: pd.Series) -> pd.DataFrame:
    median=(high+low)/2
    raw_jaw=smma(median,valid,P["jaw_period"]);raw_teeth=smma(median,valid,P["teeth_period"]);raw_lips=smma(median,valid,P["lips_period"])
    visible_jaw=raw_jaw.shift(P["jaw_shift"]);visible_teeth=raw_teeth.shift(P["teeth_shift"]);visible_lips=raw_lips.shift(P["lips_shift"])
    upper=(visible_jaw-visible_teeth).abs();lower=(visible_teeth-visible_lips).abs();expanding=upper.gt(upper.shift(1))&lower.gt(lower.shift(1))
    return pd.DataFrame({"raw_jaw":raw_jaw,"raw_teeth":raw_teeth,"raw_lips":raw_lips,"visible_jaw":visible_jaw,"visible_teeth":visible_teeth,"visible_lips":visible_lips,"upper_gap":upper,"lower_gap":lower,"expanding":expanding},index=high.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[["bar_open","bar_high","bar_low","bar_close"]]).all(axis=1)&bars[["bar_open","bar_high","bar_low","bar_close"]].gt(0).all(axis=1);bars=pd.concat([bars,gator(bars.bar_high,bars.bar_low,bars.valid_bar)],axis=1);long=bars.visible_lips.gt(bars.visible_teeth)&bars.visible_teeth.gt(bars.visible_jaw);short=bars.visible_lips.lt(bars.visible_teeth)&bars.visible_teeth.lt(bars.visible_jaw);bars["entry_side"]=np.where(long,1,np.where(short,-1,0));bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["visible_jaw","visible_teeth","visible_lips","upper_gap","lower_gap","variation"]]).all(axis=1)&bars.upper_gap.gt(0)&bars.lower_gap.gt(0)&bars.entry_side.ne(0)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));awakening=panel.expanding.eq(True)&~panel.expanding.shift(1,fill_value=False);panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&awakening&panel.variation_rank.ge(P["variation_rank_min"]);return panel.loc[:,PANEL_COLUMNS]


def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);upper=pd.to_numeric(used.upper_gap,errors="coerce");lower=pd.to_numeric(used.lower_gap,errors="coerce");both=upper.gt(upper.shift(1))&lower.gt(lower.shift(1));awakening=both&~both.shift(1,fill_value=False);single=upper.gt(upper.shift(1))^lower.gt(lower.shift(1));variation=used.variation_rank.ge(P["variation_rank_min"]);side=pd.to_numeric(used.entry_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&awakening&variation
    if control=="no_variation_gate":state=valid&side.ne(0)
    elif control=="single_gap_expansion":state=valid&side.ne(0)&single&variation
    elif control=="one_bar_stale_awakening":state=state.shift(1,fill_value=False);side=side.shift(1,fill_value=0)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVGATOR prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvgator_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvgator_24_source_support_v1", "policy_id": prereg.POLICY_ID,
