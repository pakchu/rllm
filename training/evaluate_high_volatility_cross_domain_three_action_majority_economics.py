"""Strict staged economics for the singleton HVCM3-8 policy."""
...
PREREG = Path("results/high_volatility_cross_domain_three_action_majority_preregistration_2026-08-16.json")
PREREG_SHA = "99e3ce77b847dbad313a2d62cc3ff8c0dcf7ab9631072bab38f5cf867de1dd0b"
SUPPORT = Path("results/high_volatility_cross_domain_three_action_majority_support_2026-08-16.json")
SUPPORT_SHA = "42d514f9bab766de6e205996fa1cb8287d2ab9d16c2fccfc997f44e681426d51"
GROSS9 = Path("results/high_volatility_cross_domain_three_action_majority_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "e837e5938d5a78669f5fa3bf2dd31eda306046f79aa59f0b670a04b7d3a190a5"
CLOCK = {
    "path": Path("data/high_volatility_cross_domain_three_action_majority_clocks_2023_2026.csv.gz"),
    "sha256": "6399d9a8a3daac0fe50f0bf260af053dd3cd0045fed8bcd5be37e3f8b91168f9",
    "rows": 260,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_domain_three_action_majority_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCM3-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCM3-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCM3-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCM3-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCM3-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCM3-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCM3-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCM3-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCM3-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCM3-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCM3-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcm3_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
