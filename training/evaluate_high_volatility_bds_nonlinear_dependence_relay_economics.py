"""Sequential strict economics for frozen HVBDS-8."""
...
PREREG = Path("results/high_volatility_bds_nonlinear_dependence_relay_preregistration_2026-08-13.json")
PREREG_SHA = "9e10c67e2f1c589e3467abafa53b77f85842e9134dfddcdca6fec831aa2977cc"
SUPPORT = Path("results/high_volatility_bds_nonlinear_dependence_relay_support_2026-08-13.json")
SUPPORT_SHA = "2c4a3b9575176bb45ca71a97df75e4862e4b942da785a151c67d379498fd617e"
NOVELTY = Path("results/high_volatility_bds_nonlinear_dependence_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "d70cd898edb50c571ae45ec361e83ef911ce39abf1acfe5d9696f0a6a6bc4bfc"
CLOCK = Path("data/high_volatility_bds_nonlinear_dependence_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "a2e4f62fa57fe8162acf79733dd5415c1ac983ddd0294b8ec43bcdc2cb3df570"
CONTROL_DIR = Path("data/high_volatility_bds_nonlinear_dependence_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_bds_nonlinear_dependence_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_bds_nonlinear_dependence_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_departure_tail", "no_variation_gate", "linear_lag1_autocorrelation_tail", "one_decision_stale_dependence", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVBDS frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVBDS novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvbds_8_sequential_economics_v1", "policy_id": POLICY_ID,
