"""Evaluate frozen HVTLRR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVTLRR-8"
PROTOCOL = "hvtlrr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_turnover_lead_response_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTLRR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVTLRR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTLRR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTLRR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvtlrr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVTLRR-8 unchanged before economic outcomes",
    }
