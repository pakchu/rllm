"""Sequential strict economics for frozen HVVPCI-24."""
...
PREREG = Path("results/high_volatility_volume_price_confirmation_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "c2532e61901461ddbe5da93c2224216cbf400f8f6cd26cbc3234c3b86f5c057b"
SUPPORT = Path("results/high_volatility_volume_price_confirmation_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "815ad261639e7e1d29a77b7dcf2204f4692d9d6f843fcc27c1372d21f6653ad1"
NOVELTY = Path("results/high_volatility_volume_price_confirmation_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "a2a9caaff1d93f40bdd873c9683ee2ed60e1c70cc41a8265c5f2f82a1caf9a81"
CLOCK = Path("data/high_volatility_volume_price_confirmation_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "3a15eb0845c6893bab1e637f1fafe8c66fd879691d818ff17b20217c5ae1234d"
CONTROL_DIR = Path("data/high_volatility_volume_price_confirmation_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_volume_price_confirmation_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_volume_price_confirmation_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "vpc_only_zero_cross", "one_bar_stale_cross", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVVPCI frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVVPCI novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvvpci_24_sequential_economics_v1", "policy_id": POLICY_ID,
