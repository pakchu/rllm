"""Evaluate frozen HVOBV-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVOBV-24"
PROTOCOL = "hvobv_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_on_balance_volume_signal_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVOBV predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVOBV novelty limits drift")
    if (
        ...
        raise RuntimeError("HVOBV predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVOBV primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvobv_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVOBV-24 unchanged before economic outcomes",
    }
