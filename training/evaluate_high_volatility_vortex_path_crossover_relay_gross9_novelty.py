"""Evaluate frozen HVVTX-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVVTX-24"
PROTOCOL = "hvvtx_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_vortex_path_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVVTX predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVVTX novelty limits drift")
    if (
        ...
        raise RuntimeError("HVVTX predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVVTX primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvvtx_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVVTX-24 unchanged before economic outcomes",
    }
