"""Build outcome-blind source support for frozen HVBB-24."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_bollinger_band_reentry_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2020-01-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="131004f34c97c51b3c20aa528ac9e92d4b2d07b9133391e422df5deaa2bc3348";REG=prereg.build();P=REG["policy"]
...
ROOT=Path("data/high_volatility_bollinger_band_reentry_relay_sources_2023_2026");PANEL=ROOT/"four_hour_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_bollinger_band_reentry_relay_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_bollinger_band_reentry_relay_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_bollinger_band_reentry_relay_controls_2023_2026");RESULT=Path("results/high_volatility_bollinger_band_reentry_relay_support_2026-08-11.json");BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("source_start","feature_available_time","source_valid","bar_open","bar_high","bar_low","bar_close","center","dispersion","upper","lower","ema_center","ema_upper","ema_lower","variation","variation_rank","eligible")
CLOCK_COLUMNS=("candidate","control","split","source_start","feature_available_time","entry_time","exit_time","side","bar_close","center","dispersion","upper","lower","ema_center","ema_upper","ema_lower","variation","variation_rank","eligible")
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVBB source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVBB invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 source=prepare(raw).reindex(pd.date_range(START,END,freq="1min",inclusive="left"));g=source.groupby(source.index.floor("4h"),sort=True);bars=pd.DataFrame({"rows":g.row_valid.sum(),"bar_open":g.open.first(),"bar_high":g.high.max(),"bar_low":g.low.min(),"bar_close":g.close.last(),"variation_component":g.minute_sq_return.sum(min_count=240)});bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[["bar_open","bar_high","bar_low","bar_close"]]).all(axis=1)&bars[["bar_open","bar_high","bar_low","bar_close"]].gt(0).all(axis=1);period=P["window_periods"];consecutive=bars.valid_bar.rolling(period,min_periods=period).sum().eq(period);bars["center"]=bars.bar_close.rolling(period,min_periods=period).mean().where(consecutive);bars["dispersion"]=bars.bar_close.rolling(period,min_periods=period).std(ddof=P["standard_deviation_ddof"]).where(consecutive);bars["upper"]=bars.center+P["band_multiplier"]*bars.dispersion;bars["lower"]=bars.center-P["band_multiplier"]*bars.dispersion;bars["ema_center"]=segmented_ewm(bars.bar_close,bars.valid_bar,2/(period+1),period);bars["ema_upper"]=bars.ema_center+P["band_multiplier"]*bars.dispersion;bars["ema_lower"]=bars.ema_center-P["band_multiplier"]*bars.dispersion;bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["center","dispersion","upper","lower","ema_center","ema_upper","ema_lower","variation"]]).all(axis=1)&bars.dispersion.gt(0)&bars.variation.gt(0);panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));previous=panel.bar_close.shift(1);long=previous.le(panel.lower.shift(1))&panel.bar_close.gt(panel.lower);short=previous.ge(panel.upper.shift(1))&panel.bar_close.lt(panel.upper);panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&(long|short)&panel.variation_rank.ge(P["variation_rank_min"]);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);upper=used.ema_upper if control=="exponential_center" else used.upper;lower=used.ema_lower if control=="exponential_center" else used.lower;previous=used.bar_close.shift(1);long=previous.le(lower.shift(1))&used.bar_close.gt(lower);short=previous.ge(upper.shift(1))&used.bar_close.lt(upper);side=pd.Series(np.where(long,1,np.where(short,-1,0)),index=used.index,dtype=int);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&side.ne(0)&variation
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVBB prereg drift")
