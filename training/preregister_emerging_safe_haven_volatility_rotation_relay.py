"""Outcome-blind preregistration for ESVRR-12."""
...
POLICY_ID = "ESVRR-12"
DEFAULT_OUTPUT = Path(
    "results/emerging_safe_haven_volatility_rotation_relay_preregistration_2026-08-10.json"
)
SOURCES = {
    "vxeem_panel": {
        "path": (
            "data/emerging_market_volatility_confirmation_relay_sources_2022_2026/"
            "cboe_vxeem_vix_panel.csv.gz"
        ),
        "sha256": "7d332f56676bd40c2b7bc816b432ffa2e31c113dbf4f5605fbe8eb4d0dca18ea",
        "date_column": "observation_date",
        "value_column": "VXEEM_close",
    },
    "gvz": {
...
def build() -> dict[str, Any]:
    core = {
        "protocol_version": "emerging_safe_haven_volatility_rotation_relay_v1",
        "policy_id": POLICY_ID,
        "slug": "emerging_safe_haven_volatility_rotation_relay",
        ...
        "mechanism": {
            "claim": (
                "A tail change in log(VXEEM/GVZ) identifies rotation between emerging-market "
                "systemic uncertainty and gold safe-haven uncertainty. In a high-variation BTC "
                "regime, fade the relative uncertainty shock for twelve elapsed hours."
            ),
            "side": "negative strict sign of relative_volatility_change; no BTC direction confirmation",
            "why_distinct": (
                "EMVCR compared VXEEM with VIX and GVRCR compared GVZ with VIX. ESVRR removes "
                "the shared broad-equity denominator and directly measures the relative allocation "
                "of uncertainty between emerging-market and gold safe-haven risk."
            ),
            "volatile_market_target": (
                "BTC prior-24h realized-variation strict-prior decision rank must be at least 0.65"
            ),
            "why_low_gross9_overlap_is_plausible": (
                "Gross9 has no exact VXEEM-versus-GVZ relative-volatility tail clock"
            ),
        },
        "features": {
            "common_source_dates": (
                "exact intersection of source dates having one positive finite close for both "
                "VXEEM and GVZ"
            ),
            "relative_volatility": "log(VXEEM_close/GVZ_close) on exact common source date S",
            "relative_volatility_change": (
                "relative_volatility(S)-relative_volatility(previous exact common source date); "
                "strict nonzero"
            ),
            "absolute_change_rank": {
                "history": (
                    "at most 252 strictly prior common-source-date absolute relative-volatility "
                    "changes; minimum 126; current excluded"
                ),
                "midrank_formula": (
        ...
        "clock": {
            ...
            "side": "-sign(relative_volatility_change); no BTC direction confirmation",
        ...
        "policy": {
            "change_rank_history_observations": 252,
            "change_rank_minimum_history_observations": 126,
        ...
        "research_boundary": {
            "prior_emvcr_outcomes_known": True,
            "prior_gvxcr_outcomes_known": True,
            "prior_equity_commodity_volatility_outcomes_known": True,
            "exact_vxeem_gvz_relative_volatility_change_previously_tested": False,
            ...
            "selection_basis": (
                "independent emerging-versus-safe-haven uncertainty-rotation mechanism frozen without "
        ...
        "stopping_rule": (
            ...
            "audit; no source, ratio, change, rank, side, hold, clock, volatility, subset, "
...
def validate(payload: dict[str, Any]) -> None:
    ...
    if payload.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError("ESVRR preregistration drift")
