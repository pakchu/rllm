"""Evaluate frozen WMLHR-16 structural novelty against every Gross9 sleeve."""
...
POLICY = "WMLHR-16"
PROTOCOL = "wmlhr_16_gross9_novelty_v1"
...
OUTPUT = Path("results/weekend_monday_liquidity_handoff_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("WMLHR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("WMLHR novelty limits drift")
    if (
        ...
        raise RuntimeError("WMLHR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="WMLHR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "wmlhr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject WMLHR-16 unchanged before economic outcomes",
    }
