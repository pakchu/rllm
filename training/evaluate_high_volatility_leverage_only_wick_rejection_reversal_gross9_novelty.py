"""Evaluate frozen HVLOWR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVLOWR-8"
PROTOCOL = "hvlowr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_leverage_only_wick_rejection_reversal_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVLOWR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVLOWR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVLOWR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVLOWR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVLOWR-8 unchanged before economic outcomes",
    }
