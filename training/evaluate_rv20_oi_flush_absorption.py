#!/usr/bin/env python3
"""Frozen RV20/OI flush-absorption candidate evaluator.

The module deliberately keeps discovery, leverage selection, confirmation
weight selection, and future veto as separate operations.  In particular,
``evaluate_frozen_protocol`` never substitutes a lower ranked cell after a
failure.  All rolling statistics are causal and use pandas' linear quantile.
"""
...
PREREGISTRATION = Path(
    "results/rv20_oi_flush_absorption_preregistration_2026-08-08.json"
)
EXPECTED_PREREGISTRATION_SHA256 = (
    "54c40e1e3a5e66cf53d7342c08e5fb7545a3df5db8b17b81705e5b962097ca17"
)
OUTPUT = Path("results/rv20_oi_flush_absorption_evaluation_2026-08-08.json")
REPORT = Path("docs/rv20-oi-flush-absorption-evaluation-2026-08-08.md")
ATTEMPT = Path(
    "results/rv20_oi_flush_absorption_evaluation_attempt_consumed_2026-08-08.json"
)
...
class Cell:
    absolute_hour_return_quantile: float
    capitulation_taker_fraction: float
    hour_range_quantile: float
    max_hold_hours: int
    oi_change_upper_quantile: float
    stop_loss_pct: float
...
def load_preregistration(path: str | Path = PREREGISTRATION) -> dict[str, Any]:
    ...
    if payload.get("schema") != "rllm.rv20-oi-flush-absorption.preregistration.v1":
        raise RuntimeError("unexpected RV20 preregistration schema")
    if payload.get("status") != "FROZEN_BEFORE_CANDIDATE_ECONOMIC_INSPECTION":
    ...
    if len(frozen_grid(payload)) != 432:
        raise RuntimeError("frozen grid cardinality drifted")
    return payload
...
def frozen_grid(preregistration: Mapping[str, Any]) -> tuple[Cell, ...]:
    ...
    if len(cells) != len(set(cells)):
        raise RuntimeError("frozen grid contains duplicate cells")
    return cells
...
def build_hourly_features(market: pd.DataFrame) -> pd.DataFrame:
    ...
    for hour, group in frame.groupby("hour", sort=True):
        ...
        if any(series.isna().any() for series in values.values()):
            continue
    ...
    hourly["oi_change_6h"] = np.log(
        hourly["open_interest"] / hourly["open_interest"].shift(6)
    )
...
def build_rv20_daily_clock(market: pd.DataFrame) -> pd.DataFrame:
    """Causal daily RV20 and shifted Q90 reference clock."""
    ...
    threshold = shifted_rolling_quantile(rv20, 0.90, RV_REFERENCE_WINDOW)
    return pd.DataFrame({"rv20": rv20, "rv20_q90": threshold})
...
def attach_causal_thresholds(
    ...
    output["rv20_q90"] = daily_clock["rv20_q90"].reindex(days).to_numpy(float)
    output["rv20_active"] = output["rv20"] >= output["rv20_q90"]
    specs = {
        ("abs_return", cell.absolute_hour_return_quantile)
        for cell in cells
    } | {
        ("range", cell.hour_range_quantile) for cell in cells
    } | {
        ("oi", cell.oi_change_upper_quantile) for cell in cells
    }
    source = {
        "abs_return": output["hour_return"].abs(),
        "range": output["hour_range"],
        "oi": output["oi_change_6h"],
    }
...
def cell_signal_sides(hourly: pd.DataFrame, cell: Cell) -> np.ndarray:
    absolute = hourly[f"q_abs_return_{cell.absolute_hour_return_quantile:g}"]
    range_threshold = hourly[f"q_range_{cell.hour_range_quantile:g}"]
    oi_threshold = hourly[f"q_oi_{cell.oi_change_upper_quantile:g}"]
    common = (
        hourly["rv20_active"].fillna(False)
        & (hourly["hour_return"].abs() >= absolute)
        & (hourly["hour_range"] >= range_threshold)
        & (hourly["oi_change_6h"] <= oi_threshold)
    )
    long = common & (hourly["hour_return"] < 0.0) & (
        hourly["taker_fraction"] <= cell.capitulation_taker_fraction
    )
    short = common & (hourly["hour_return"] > 0.0) & (
        hourly["taker_fraction"] >= 1.0 - cell.capitulation_taker_fraction
    )
...
def execute_trade(
    ...
    *,
    ...
    """Enter next 5m open and resolve every same-bar ambiguity adverse-first."""
    if side not in (-1, 1):
    ...
    stop = entry_price * (1.0 - side * cell.stop_loss_pct)
    ...
    for position in range(entry, last + 1):
        ...
        if stop_hit:  # deliberately before take_hit
...
def execute_schedule(
    ...
    *, leverage: float = 1.0,
    ...
    for row, side in zip(hourly.itertuples(index=False), sides):
        if not side or int(row.signal_position) + 1 < next_entry:
            continue
        trade = execute_trade(
    ...
    *,
    ...
    for row, side in zip(hourly.itertuples(index=False), sides):
        ...
        if entry_time < start_ts or entry_time >= end_ts:
            continue
        if not side or entry_position < next_entry:
            continue
        trade = execute_trade(
            market, funding, int(row.signal_position), int(side), cell,
            leverage=leverage,
...
def rank_key(row: Mapping[str, Any]) -> tuple[Any, ...]:
    metric = row["train"]
    cell = row["cell"]
    canonical = tuple(cell[field] for field in GRID_FIELDS)
    return (
        -float(metric["cagr_to_strict_mdd"]),
        float(metric["strict_mdd_pct"]),
        -float(metric["absolute_return_pct"]),
        canonical,
    )


def select_structural_top1(rows: Sequence[Mapping[str, Any]]) -> Mapping[str, Any] | None:
    passing = [row for row in rows if row.get("train_pass") is True]
    return min(passing, key=rank_key) if passing else None


def select_leverage(rows: Sequence[Mapping[str, Any]]) -> Mapping[str, Any] | None:
...
def train_cell_passes(normal: Mapping[str, Any], stress: Mapping[str, Any]) -> bool:
    """Exact frozen structural-cell train gates."""
    return bool(
        int(normal["trades"]) >= 40
        and int(normal["active_weeks"]) >= 20
        and float(normal["absolute_return_pct"]) > 0.0
        and float(stress["absolute_return_pct"]) > 0.0
        and float(normal["strict_mdd_pct"]) <= 20.0
        and float(normal["cagr_to_strict_mdd"]) >= 1.5
    )


def leverage_passes(normal: Mapping[str, Any], stress: Mapping[str, Any]) -> bool:
    return bool(
        float(normal["strict_mdd_pct"]) <= 15.0
        and float(normal["cagr_to_strict_mdd"]) >= 3.0
        and float(stress["absolute_return_pct"]) > 0.0
    )
...
def persistent_long_vol_control(
    ...
    *, leverage: float, cost_rate: float,
) -> list[Trade]:
    """Long each active RV20 day from first open through final close."""
    ...
    for day, positions in pd.Series(np.arange(len(market)), index=days).groupby(level=0):
        ...
        if not (np.isfinite(row["rv20_q90"]) and row["rv20"] >= row["rv20_q90"]):
            continue
...
def render_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# RV20 OI Flush Absorption Evaluation\n\n"
    ...
    if isinstance(top, Mapping):
        ...
        lines.append(canonical_json({
            "cell": top.get("cell"),
            "leverage": (
                payload.get("selected_leverage") or {}
            ).get("leverage"),
...
def extend_gross9_context(
    ...
    if len(gross_market) > len(complete_market):
        raise RuntimeError("Gross9 clock exceeds complete G9CB12 clock")
    ...
    for column in ("date", "open", "high", "low", "close"):
        ...
        if not np.array_equal(left, right):
            raise RuntimeError(f"Gross9 is not an exact G9CB12 prefix: {column}")
    ...
    for split, (start, end) in portfolio.SPLIT_BOUNDS.items():
...
def _execution_config(
    ...
    return ExecutionConfig(
        ...
        output="/tmp/no_write_rv20_oi_flush_absorption.json",
        manifest_output="/tmp/no_write_rv20_oi_flush_absorption_manifest.json",
...
def _install_candidate_sleeve() -> None:
    ...
    portfolio.FAMILIES[CANDIDATE] = "rv20_oi_flush_absorption"
...
def candidate_events(
    ...
    *,
    ...
    for split, (start, configured_end) in portfolio.SPLIT_BOUNDS.items():
...
def evaluate_frozen_protocol(
    ...
    """Execute the authenticated one-shot protocol without any future rerank."""
    cells = frozen_grid(preregistration)
    hourly = attach_causal_thresholds(
        build_hourly_features(market), build_rv20_daily_clock(market), cells
    )
    structural_rows = []
    for cell in cells:
        trades = execute_schedule_window(
            market, funding, hourly, cell, start=TRAIN[0], end=TRAIN[1]
        )
        normal = strict_metrics(
            trades, start=TRAIN[0], end=TRAIN[1], cost_rate=ENTRY_COST
        )
        stress = strict_metrics(
            trades, start=TRAIN[0], end=TRAIN[1], cost_rate=STRESS_COST
        )
        structural_rows.append({
            "cell": asdict(cell), "train": normal, "train_stress": stress,
            "train_pass": train_cell_passes(normal, stress),
        })
    top = select_structural_top1(structural_rows)
    if top is None:
        return {
            "schema": "rllm.rv20-oi-flush-absorption.evaluation.v1",
            "tested_structural_cells": len(structural_rows), "structural_rows": structural_rows,
            "frozen_structural_top1": None, "verdict": "REJECT_NO_STRUCTURAL_TOP1",
            "future_opened": False, "repair_attempted": False,
        }
    top_cell = Cell(**top["cell"])
    leverage_rows = []
    for leverage in map(float, preregistration["selection"]["leverage_grid"]):
        trades = execute_schedule_window(
            market, funding, hourly, top_cell, start=TRAIN[0], end=TRAIN[1],
            leverage=leverage,
        )
        normal = strict_metrics(
            trades, start=TRAIN[0], end=TRAIN[1], cost_rate=ENTRY_COST,
        ...
        leverage_rows.append({
            "leverage": leverage, "metrics": normal, "stress": stress,
            "pass": leverage_passes(normal, stress),
        })
    selected_leverage = select_leverage(leverage_rows)
    if selected_leverage is None:
        return {
            "schema": "rllm.rv20-oi-flush-absorption.evaluation.v1",
            "tested_structural_cells": 432, "structural_rows": structural_rows,
            "frozen_structural_top1": top, "leverage_rows": leverage_rows,
            "selected_leverage": None, "verdict": "REJECT_NO_LEVERAGE",
            "future_opened": False, "repair_attempted": False,
        }
    leverage = float(selected_leverage["leverage"])
    evaluation_windows = {
        "train": TRAIN,
        "test2024": CONFIRMATION,
        "eval2025": FUTURE_WINDOWS["2025"],
    ...
    window_schedules = {
        name: execute_schedule_window(
            market, funding, hourly, top_cell, start=bounds[0], end=bounds[1],
            leverage=leverage,
        )
        for name, bounds in evaluation_windows.items()
    }
    trades = sorted(
        [trade for rows in window_schedules.values() for trade in rows],
    ...
    gross_market, masks, baseline_events, source_meta = gross9.build_full_context(cfg)
    gross_market, masks, baseline_events = extend_gross9_context(
        gross_market, masks, baseline_events, market
    )
    ...
    authoritative = gross9.validate_authoritative_gross9(
        cfg, baseline_arrays, tuple(portfolio.SPLIT_BOUNDS)
    )
    normal_events = list(baseline_events) + candidate_events(
        market, funding, trades, preregistration,
    ...
    jaccard = max_per_sleeve_entry_jaccard(
        [trade.entry_time for trade in _split_trades(trades, *CONFIRMATION)],
        {
            sleeve: [market.iloc[int(pos)]["date"] for pos in baseline_arrays["test2024"]["entry_positions"][sleeve]]
    ...
    if selected_weight is None:
        ...
        for name, split in split_names.items():
            row = _same_gross_row(
                weight, normal_arrays, stress_arrays, baseline_arrays, split
            )
            row["checks"] = {
        ...
        combined_weights, comparator_weights = same_gross_weights(weight)
        normal_combined = _concat_arrays([normal_arrays["eval2025"], normal_arrays["ytd2026"]])
        stress_combined = _concat_arrays([stress_arrays["eval2025"], stress_arrays["ytd2026"]])
        base_combined = _concat_arrays([baseline_arrays["eval2025"], baseline_arrays["ytd2026"]])
        ...
        control = persistent_long_vol_control(
            market, funding, build_rv20_daily_clock(market),
            leverage=leverage, cost_rate=ENTRY_COST,
        )
        residuals = {
            "train": _control_report(trades, control, start=TRAIN[0], end=TRAIN[1], leverage=leverage),
            "2024": _control_report(trades, control, start=CONFIRMATION[0], end=CONFIRMATION[1], leverage=leverage),
            **{
                name: _control_report(trades, control, start=bounds[0], end=bounds[1], leverage=leverage)
                for name, bounds in FUTURE_WINDOWS.items()
            },
        }
        residual_checks = {
            name: {
                "full_calendar_positive": (
                    report["full_calendar"]["log_return_residual"] > 0.0
                ),
                "rv20_stress_positive": (
                    report["rv20_stress"]["log_return_residual"] > 0.0
                ),
            }
        ...
        checks = {
            ...
            **{
                f"persistent_long_vol_{name}": residual_report_passes(
                    residuals[name]
                )
                for name in residual_checks
            },
        }
    ...
    return {
        "schema": "rllm.rv20-oi-flush-absorption.evaluation.v1",
        "preregistration_sha256": EXPECTED_PREREGISTRATION_SHA256,
        "tested_structural_cells": len(structural_rows),
        "structural_rows": structural_rows,
        "frozen_structural_top1": top,
        "leverage_rows": leverage_rows,
        "selected_leverage": selected_leverage,
        "gross9": {"authoritative": authoritative, "source_meta": source_meta},
        "entry_jaccard_2024": jaccard,
        "weight_rows_2024": weight_rows,
        "selected_weight": selected_weight,
        "future_veto": future,
        "future_used_for_ranking": False,
        "repair_attempted": False,
        "rerank_attempted": False,
        "verdict": verdict,
    }
