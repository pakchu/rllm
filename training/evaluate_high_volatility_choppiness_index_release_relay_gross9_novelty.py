"""Evaluate frozen HVCIR-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCIR-24"
PROTOCOL = "hvcir_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_choppiness_index_release_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCIR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCIR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCIR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCIR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcir_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCIR-24 unchanged before economic outcomes",
    }
