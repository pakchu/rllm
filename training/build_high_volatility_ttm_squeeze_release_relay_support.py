"""Build outcome-blind source support for frozen HVTTS-24."""
...
from training import preregister_high_volatility_ttm_squeeze_release_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "d8b99307e1490e639f3606e1f737c4ea989604be325a1ba21502f404b76b7b42"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ttm_squeeze_release_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ttm_squeeze_release_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ttm_squeeze_release_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ttm_squeeze_release_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ttm_squeeze_release_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "true_range", "atr", "close_sma", "close_std", "close_ema",
    "bollinger_upper", "bollinger_lower", "keltner_upper", "keltner_lower",
    "squeeze_on", "detrended", "momentum", "side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "true_range", "atr", "close_sma", "close_std", "close_ema",
    "bollinger_upper", "bollinger_lower", "keltner_upper", "keltner_lower",
    "squeeze_on", "detrended", "momentum", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVTTS source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVTTS invalid source key")
...
def wilder_atr(high: pd.Series, low: pd.Series, close: pd.Series, valid: pd.Series) -> tuple[pd.Series,pd.Series]:
    period=P["envelope_periods"];tr=np.full(len(close),np.nan);atr=np.full(len(close),np.nan);history=[];state=np.nan
    highs=high.to_numpy(float);lows=low.to_numpy(float);closes=close.to_numpy(float);validity=valid.to_numpy(bool)
    for i in range(len(close)):
        if i==0 or not validity[i] or not validity[i-1]:history=[];state=np.nan;continue
        ...
        if not math.isfinite(state):
            ...
            state=float(np.mean(history[-period:]))
        else:state=((period-1)*state+value)/period
        atr[i]=state
    return pd.Series(tr,index=close.index),pd.Series(atr,index=close.index)
...
def causal_ema(series: pd.Series, valid: pd.Series, period: int) -> pd.Series:
    ...
    for i,(value,is_valid) in enumerate(zip(series.to_numpy(float),valid.to_numpy(bool))):
        if not is_valid:state=np.nan;run=0;continue
        state=value if not math.isfinite(state) else alpha*value+(1-alpha)*state;run+=1
        if run>=period:output[i]=state
    return pd.Series(output,index=series.index)


def regression_last(series: pd.Series, valid: pd.Series, period: int) -> pd.Series:
    x=np.arange(period,dtype=float);x_mean=float(x.mean());denominator=float(np.sum((x-x_mean)**2))
    def fitted(values):
        slope=float(np.sum((x-x_mean)*(values-values.mean()))/denominator)
        return float(values.mean()+slope*(period-1-x_mean))
    output=series.rolling(period,min_periods=period).apply(fitted,raw=True);complete=valid.rolling(period,min_periods=period).sum().eq(period)
    return output.where(complete)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    fields=["bar_open","bar_high","bar_low","bar_close"];bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[fields]).all(axis=1)&bars[fields].gt(0).all(axis=1);period=P["envelope_periods"]
    bars["true_range"],bars["atr"]=wilder_atr(bars.bar_high,bars.bar_low,bars.bar_close,bars.valid_bar);complete=bars.valid_bar.rolling(period,min_periods=period).sum().eq(period);bars["close_sma"]=bars.bar_close.rolling(period,min_periods=period).mean().where(complete);bars["close_std"]=bars.bar_close.rolling(period,min_periods=period).std(ddof=0).where(complete);bars["close_ema"]=causal_ema(bars.bar_close,bars.valid_bar,period)
    bars["bollinger_upper"]=bars.close_sma+P["bollinger_std_multiplier"]*bars.close_std;bars["bollinger_lower"]=bars.close_sma-P["bollinger_std_multiplier"]*bars.close_std;bars["keltner_upper"]=bars.close_ema+P["keltner_atr_multiplier"]*bars.atr;bars["keltner_lower"]=bars.close_ema-P["keltner_atr_multiplier"]*bars.atr;bars["squeeze_on"]=(bars.bollinger_upper<bars.keltner_upper)&(bars.bollinger_lower>bars.keltner_lower)
    midpoint=(bars.bar_high.rolling(period,min_periods=period).max()+bars.bar_low.rolling(period,min_periods=period).min())/2;bars["detrended"]=bars.bar_close-(midpoint+bars.close_sma)/2;bars["momentum"]=regression_last(bars.detrended,bars.valid_bar,P["momentum_periods"]);bars["side"]=np.sign(bars.momentum)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["true_range","atr","close_sma","close_std","close_ema","bollinger_upper","bollinger_lower","keltner_upper","keltner_lower","detrended","momentum","side","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.atr.gt(0)&bars.close_std.gt(0)&bars.side.ne(0)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));panel["eligible"]=panel.source_valid&panel.source_valid.shift(1,fill_value=False)&panel.squeeze_on.shift(1,fill_value=False)&~panel.squeeze_on&panel.variation_rank.ge(P["variation_rank_min"])
    return panel.loc[:,PANEL_COLUMNS]

def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True)&used.source_valid.shift(1,fill_value=False);release=used.squeeze_on.shift(1,fill_value=False)&~used.squeeze_on;variation=used.variation_rank.ge(P["variation_rank_min"]);side=pd.to_numeric(used.side,errors="coerce").fillna(0).astype(int);state=valid&release&side.ne(0)&variation
    if control=="no_variation_gate":state=valid&release&side.ne(0)
    elif control=="close_sma_direction":side=np.sign(used.bar_close-used.close_sma).fillna(0).astype(int)
    elif control=="one_bar_stale_release":state=state.shift(1,fill_value=False);side=side.shift(1,fill_value=0)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVTTS prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvtts_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvtts_24_source_support_v1", "policy_id": prereg.POLICY_ID,
