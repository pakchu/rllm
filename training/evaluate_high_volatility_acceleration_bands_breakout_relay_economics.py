"""Sequential strict economics for frozen HVABANDS-B10-W4-24."""
...
PREREG = Path("results/high_volatility_acceleration_bands_breakout_relay_preregistration_2026-08-11.json")
PREREG_SHA = "364973b184aead235bf490f5bdb8ab777db5973264feeddc6ebc00bd5708416b"
SUPPORT = Path("results/high_volatility_acceleration_bands_breakout_relay_support_2026-08-11.json")
SUPPORT_SHA = "2ba3e9772d95e9a24d2645795c221645c30aa092648b9027aa0d2bf1ac9b3f13"
NOVELTY = Path("results/high_volatility_acceleration_bands_breakout_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "3cd17616cd2ed74c907f3e845212f25ef39729652c27428ddee3f59546c9aade"
CLOCK = Path("data/high_volatility_acceleration_bands_breakout_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "1e6127d89193f2e92b3f8d75b6719ee2edb0aa16c586de7d76e418640669a2d0"
CONTROL_DIR = Path("data/high_volatility_acceleration_bands_breakout_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_acceleration_bands_breakout_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_acceleration_bands_breakout_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = (
    "no_variation_gate",
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVABANDS frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVABANDS novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvabands_b10_w4_24_sequential_economics_v1", "policy_id": POLICY_ID,
