"""Evaluate the frozen HVCASPCDCV-8 action-vote clock against Gross9."""
...
POLICY = "HVCASPCDCV-8"
PROTOCOL = "hvcaspcdcv_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_caspc_directional_change_opposite_veto_clocks_2023_2026.csv.gz",
    "sha256": "7051dcddc2c5b2473fe86f3e7aec62f87aa5f87fe7f5d64b786653f4c7a26073",
    "rows": 146,
}
OUTPUT = Path("results/high_volatility_caspc_directional_change_opposite_veto_gross9_novelty_2026-08-17.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCASPCDCV-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCASPCDCV-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCASPCDCV-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCASPCDCV-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCASPCDCV-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCASPCDCV-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCASPCDCV-8 unchanged before economics",
    }
