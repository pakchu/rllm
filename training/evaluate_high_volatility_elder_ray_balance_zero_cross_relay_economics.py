"""Sequential strict economics for frozen HVERB-24."""
...
PREREG = Path("results/high_volatility_elder_ray_balance_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "d35ab818b96134125d45d33e50c3d4b8d00d5c6a5b2a4a53713fcec0d57c63d7"
SUPPORT = Path("results/high_volatility_elder_ray_balance_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "5d441822e2e1a5b9c1f7bc4d4b7c5a9fe0df111bf4b9005d8c53d8e82fc5b83f"
NOVELTY = Path("results/high_volatility_elder_ray_balance_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "d468e50f65e470b400a1738db9365bb53724ee85b8f2245ae22843933849ceef"
CLOCK = Path("data/high_volatility_elder_ray_balance_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "7f449365977c7121226eea38bd2e05e44e8ca067026b525bc407d862a5825460"
CONTROL_DIR = Path("data/high_volatility_elder_ray_balance_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_elder_ray_balance_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_elder_ray_balance_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "close_ema_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVERB frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVERB novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hverb_24_sequential_economics_v1", "policy_id": POLICY_ID,
