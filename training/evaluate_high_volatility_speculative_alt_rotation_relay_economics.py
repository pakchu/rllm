"""Sequential strict economics for frozen HVSARR-8."""
...
PREREG = Path("results/high_volatility_speculative_alt_rotation_relay_preregistration_2026-08-12.json")
PREREG_SHA = "9c014fafa912faaa26edd783b9e767396c4ec8eafaf3cca94571ffded8813c0c"
SUPPORT = Path("results/high_volatility_speculative_alt_rotation_relay_support_2026-08-12.json")
SUPPORT_SHA = "ad9830bbc54e7512a593a0b5803f425015aadad26d0aeb88f837961648fea41e"
NOVELTY = Path("results/high_volatility_speculative_alt_rotation_relay_gross9_novelty_2026-08-12.json")
NOVELTY_SHA = "0fd22a847e0fd81a2de294f241571e22abe4864f720b3acd0b6b98410d1ee27a"
CLOCK = Path("data/high_volatility_speculative_alt_rotation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "f69e809595fb55dff5ffe4f04e1f00e3804d85d7909ae9cfc47a574256771a10"
CONTROL_DIR = Path("data/high_volatility_speculative_alt_rotation_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_speculative_alt_rotation_relay_economic_evaluator_freeze_2026-08-12.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_speculative_alt_rotation_relay_{stage}_economics_2026-08-12.json")
    for stage in STAGES
}
CONTROLS = ("no_rotation_tail", "no_variation_gate", "unsigned_dispersion_side", "one_block_stale_features", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSARR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSARR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvsarr_24_sequential_economics_v1", "policy_id": POLICY_ID,
