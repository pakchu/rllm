"""Evaluate frozen HVHILBERT-I7-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVHILBERT-I7-24"
PROTOCOL = "hvhilbert_i7_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_hilbert_in_phase_zero_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVHILBERT predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVHILBERT novelty limits drift")
    if (
        ...
        raise RuntimeError("HVHILBERT predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVHILBERT primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvhilbert_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVHILBERT-I7-24 unchanged before economic outcomes",
    }
