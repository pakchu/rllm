"""Evaluate the frozen HVCFMR-8 action-vote clock against Gross9."""
...
from training import preregister_high_volatility_causal_funding_move_reversal as prereg

POLICY = "HVCFMR-8"
PROTOCOL = "hvcfmr_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_causal_funding_move_reversal_clocks_2023_2026.csv.gz",
    "sha256": "c5f3b6dcc5ad170d37bf7a41a5fb1c8d76d48322da8ec1f5688b6e25790e6bcd",
    "rows": 250,
}
OUTPUT = Path("results/high_volatility_causal_funding_move_reversal_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCFMR-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCFMR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCFMR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCFMR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCFMR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCFMR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCFMR-8 unchanged before economics",
    }
