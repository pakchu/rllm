"""Deterministic source support for HVCQTR-24."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_credit_quality_transition_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
PREREG_SHA="af178c8ef143d693b2d60480c24122ddf0e76d7a1eb4625b634a0de40c03b0a6";CLOCK=Path("data/high_volatility_credit_quality_transition_relay_clocks_2023_2026.csv.gz");RESULT=Path("results/high_volatility_credit_quality_transition_relay_support_2026-08-18.json");STAGES={k:tuple(pd.Timestamp(v) for v in values) for k,values in prereg.build()["stages"].items()};GATES=prereg.build()["source_support_gates"]
def build_clock(states:pd.DataFrame)->pd.DataFrame:
 ...
 previous=x["relative_credit_return"].shift(1);previous_valid=x["source_valid"].shift(1,fill_value=False)
 active=(x["source_valid"].eq(True)&previous_valid&np.isfinite(x[["relative_credit_return","btc_variation","btc_variation_rank"]]).all(axis=1)&np.isfinite(previous)&x["relative_credit_return"].ne(0)&previous.ne(0)&(np.sign(x["relative_credit_return"])==-np.sign(previous))&x["btc_variation_rank"].ge(prereg.build()["policy"]["variation_rank_min"]));rows=[];reserved=None
 for r in x[active].itertuples(index=False):
  ...
  reserved=z;rows.append({"candidate":prereg.POLICY_ID,"control":"primary","split":split,"session_date":r.session_date,"decision_time":d,"feature_available_time":d,"entry_time":e,"exit_time":z,"side":int(np.sign(r.relative_credit_return)),"hyg_return":float(r.hyg_return),"lqd_return":float(r.lqd_return),"relative_credit_return":float(r.relative_credit_return),"btc_variation":float(r.btc_variation),"btc_variation_rank":float(r.btc_variation_rank)})
 return pd.DataFrame(rows,columns=("candidate","control","split","session_date","decision_time","feature_available_time","entry_time","exit_time","side","hyg_return","lqd_return","relative_credit_return","btc_variation","btc_variation_rank"))
...
def run(clock_path:Path=CLOCK,result_path:Path=RESULT)->dict[str,Any]:
 if prereg.sha256(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCQTR prereg drift")
