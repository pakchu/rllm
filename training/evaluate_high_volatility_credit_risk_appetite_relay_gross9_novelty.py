"""Evaluate frozen HVCREDIT-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCREDIT-24"
PROTOCOL = "hvcredit_24_gross9_novelty_v1"
PREREG = Path(
...
SUPPORT = Path("results/high_volatility_credit_risk_appetite_relay_support_2026-08-10.json")
SUPPORT_SHA = "3fb9fb04316ecc23c0c302685ac4bf4f8680b23bf090fbd7998c77b2d78bbdaf"
CLOCK = Path("data/high_volatility_credit_risk_appetite_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "4bc3ac73d2c79d31fc27a3626ac2b7f6cd558f3b3248c41bf86320583c39f3bc"
OUTPUT = Path(
    "results/high_volatility_credit_risk_appetite_relay_gross9_novelty_2026-08-11.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCREDIT predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVCREDIT novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCREDIT predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCREDIT primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcredit_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVCREDIT-24 unchanged before economic outcomes"
        ),
    }
