"""Sequential strict economics for frozen HVSR-24."""
...
PREREG = Path("results/high_volatility_stochastic_rsi_reentry_relay_preregistration_2026-08-11.json")
PREREG_SHA = "c079f450d1e5ef0cb408389509d7751ad9f23cf930bc75f645c44e78ed5ef235"
SUPPORT = Path("results/high_volatility_stochastic_rsi_reentry_relay_support_2026-08-11.json")
SUPPORT_SHA = "f2a577fb77e3a15b68a2f3d1b758a224670c325bdce7acbbe1b54f05607e49ad"
NOVELTY = Path("results/high_volatility_stochastic_rsi_reentry_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "ba895c690ed82b628bbdecb6be4fd573573b8102df75eda5317ec242ebf2fec4"
CLOCK = Path("data/high_volatility_stochastic_rsi_reentry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "80d2ecb3e043ae6d4921831dd1bfda7b5681c09499ea1c0c2e203425961c7db3"
CONTROL_DIR = Path("data/high_volatility_stochastic_rsi_reentry_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_stochastic_rsi_reentry_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_stochastic_rsi_reentry_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvsr_24_sequential_economics_v1", "policy_id": POLICY_ID,
