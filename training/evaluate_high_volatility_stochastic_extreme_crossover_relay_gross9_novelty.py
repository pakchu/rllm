"""Evaluate frozen HVSTOCH-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVSTOCH-24"
PROTOCOL = "hvstoch_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_stochastic_extreme_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVSTOCH predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVSTOCH novelty limits drift")
    if (
        ...
        raise RuntimeError("HVSTOCH predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVSTOCH primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvstoch_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVSTOCH-24 unchanged before economic outcomes",
    }
