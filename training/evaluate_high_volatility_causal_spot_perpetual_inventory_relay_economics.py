"""Strict staged economics for the singleton HVCSPIR-8 policy."""
...
PREREG = Path("results/high_volatility_causal_spot_perpetual_inventory_relay_preregistration_2026-08-16.json")
PREREG_SHA = "7866b1badb3678562feed32d4c1ccc8d9484cffbe2380da8460637caddfee62a"
SUPPORT = Path("results/high_volatility_causal_spot_perpetual_inventory_relay_support_2026-08-16.json")
SUPPORT_SHA = "61ad44ac5858f2b420e4d470452dec1ea667eadaf96e396a421ee3cd9f4d2d27"
GROSS9 = Path("results/high_volatility_causal_spot_perpetual_inventory_relay_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "b89d1bf9ec8462dbbe5968aac562181f760c5d0645a455df44855c27c49735d1"
CLOCK = {
    "path": Path("data/high_volatility_causal_spot_perpetual_inventory_relay_clocks_2020_2026.csv.gz"),
    "sha256": "718a84d4635e7b72d8ca5983c66494dfa43405e00e081b1a78b3c2e190cb0989",
    "rows": 130,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_spot_perpetual_inventory_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCSPIR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCSPIR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCSPIR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCSPIR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCSPIR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCSPIR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCSPIR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCSPIR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCSPIR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCSPIR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCSPIR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcspir_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
