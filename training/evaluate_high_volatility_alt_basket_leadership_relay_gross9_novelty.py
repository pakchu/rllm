"""Evaluate frozen HVABLR-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVABLR-6"
PROTOCOL = "hvablr_6_gross9_novelty_v1"
...
CLOCK_ROWS = 402
OUTPUT = Path("results/high_volatility_alt_basket_leadership_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def write_immutable(path: Path, content: bytes) -> None:
    ...
    if path.exists():
        if path.read_bytes() != content:
            raise RuntimeError(f"refusing to overwrite immutable HVABLR novelty result: {path}")
        return
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVABLR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVABLR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVABLR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVABLR primary")
    if len(candidate) != CLOCK_ROWS:
        raise RuntimeError("HVABLR clock count drift")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvablr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVABLR-6 unchanged before economic outcomes",
    }
    result = {**core, "manifest_hash": canonical_hash(core)}
    content = (json.dumps(result, indent=2, allow_nan=False) + "\n").encode()
    write_immutable(output, content)
    return result
