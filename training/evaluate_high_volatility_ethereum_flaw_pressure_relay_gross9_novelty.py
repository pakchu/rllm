"""Evaluate frozen HVEFPR-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVEFPR-24"
PROTOCOL = "hvefpr_24_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_ethereum_flaw_pressure_relay_preregistration_2026-08-12.json"
)
...
OUTPUT = Path(
    "results/high_volatility_ethereum_flaw_pressure_relay_gross9_novelty_2026-08-12.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVEFPR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVEFPR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVEFPR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVEFPR primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject HVEFPR-24 unchanged before economic outcomes"
        ),
    }
