"""Build source-only CLTDR-6 confirmation-ladder clocks."""
...
from training import preregister_confirmation_ladder_transaction_density_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.download_fetls_block_summaries import OUTPUT as BLOCKS


ENV_FILE = "/home/pakchu/rllm/.env"
PREREG_SHA = "b5066cd48f88dc4f5401379c0e0492c016d1fb92f841ce2f4dba7211a63f9ac9"
...
CONTROLS = ("late_unanimity_only", "transaction_density_only", "one_anchor_stale_ladder", "direction_flip")
...
SOURCE_DIR = Path("data/confirmation_ladder_transaction_density_relay_sources_2023_2026")
FEATURES = SOURCE_DIR / "confirmation_ladders.csv.gz"
MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/confirmation_ladder_transaction_density_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/confirmation_ladder_transaction_density_relay_controls_2023_2026")
RESULT = Path("results/confirmation_ladder_transaction_density_relay_support_2026-08-09.json")
...
TX_COLUMNS = tuple(f"block_tx_count_{number}" for number in range(1, 7))
FEATURE_COLUMNS = (
    "anchor_height", "confirmation_height", "feature_available_time", "source_valid",
    *RETURN_COLUMNS, *WEIGHT_COLUMNS, *TX_COLUMNS, "late_return", "late_unanimous",
    "early_transaction_density", "late_transaction_density",
    "transaction_density_broadening", "eligible_state",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "anchor_height", "confirmation_height",
    "feature_available_time", "entry_time", "exit_time", "side", "late_return",
    "late_unanimous", "early_transaction_density", "late_transaction_density",
    "transaction_density_broadening",
)
...
def validate_blocks(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CLTDR block schema drift")
    blocks = frame.sort_values("height", kind="mergesort").reset_index(drop=True)
    for column in ("height", "timestamp", "mediantime", "weight", "tx_count"):
        blocks[column] = pd.to_numeric(blocks[column], errors="raise").astype("int64")
    if not blocks[["weight", "tx_count"]].gt(0).all().all():
        raise RuntimeError("CLTDR block weight and transaction count must be positive")
    if not np.array_equal(np.diff(blocks.height), np.ones(len(blocks) - 1, dtype=np.int64)):
        raise RuntimeError("CLTDR block heights are not contiguous")
    if not blocks.previousblockhash.iloc[1:].reset_index(drop=True).equals(blocks.id.iloc[:-1].reset_index(drop=True)):
        raise RuntimeError("CLTDR canonical previous-hash continuity failed")
    return blocks
...
def prepare_minutes(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(frame) != required:
        raise ValueError("CLTDR minute schema drift")
    ...
    if result.ts.duplicated().any():
        raise RuntimeError("CLTDR duplicate timestamps")
...
def build_features(blocks: pd.DataFrame, market: pd.DataFrame) -> pd.DataFrame:
    ...
    for index in range(0, len(blocks) - 6):
        ...
        tx_counts = [int(blocks.tx_count.iloc[index + offset]) for offset in range(1, 7)]
        early_density = sum(tx_counts[:3]) / sum(weights[:3])
        late_density = sum(tx_counts[3:]) / sum(weights[3:])
        transaction_density_broadening = bool(valid and late_density > early_density)
        rows.append({
            ...
            **{column: tx_counts[position] for position, column in enumerate(TX_COLUMNS)},
            "late_return": late if valid else np.nan, "late_unanimous": late_unanimous,
            "early_transaction_density": early_density, "late_transaction_density": late_density,
            "transaction_density_broadening": transaction_density_broadening,
        })
    output = pd.DataFrame(rows)
    output["eligible_state"] = output.source_valid & output.late_unanimous & output.transaction_density_broadening
...
def active_and_side(features: pd.DataFrame, control: str = "primary"):
    ...
    elif control == "transaction_density_only":
        state = features.source_valid & features.transaction_density_broadening
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active & sides.ne(0)]:
        ...
        rows.append({
            ...
            "early_transaction_density": float(features.at[index, "early_transaction_density"]),
            "late_transaction_density": float(features.at[index, "late_transaction_density"]),
            "transaction_density_broadening": bool(features.at[index, "transaction_density_broadening"]),
        })
...
def run() -> dict[str, Any]:
    ...
    source_core = {
        "protocol_version": "cltdr_6_source_v1", "query_sha256": hashlib.sha256(QUERY.encode()).hexdigest(), "window": [QUERY_START.isoformat(), END.isoformat()],
    ...
    core = {
        "protocol_version": "cltdr_6_source_support_v1", "policy_id": prereg.POLICY_ID,
