"""Strict staged economics for the singleton HVCORR-24 policy."""
...
PREREG = Path("results/high_volatility_causal_oi_range_regime_relay_preregistration_2026-08-16.json")
PREREG_SHA = "ed1d3e65ef4fefaf82b1bc36beb4deffc158b2d5faf4db7bba70007f270beb28"
SUPPORT = Path("results/high_volatility_causal_oi_range_regime_relay_support_2026-08-16.json")
SUPPORT_SHA = "5d594a2c21e37cff71a1340b10e35584830b1ca6c46aa2080a2278fcb66a74ac"
GROSS9 = Path("results/high_volatility_causal_oi_range_regime_relay_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "bb448172cf84ae9f45ad7fbdd9874936c8b1f8da30fe20eb5eb5f1a9aafd4ecd"
CLOCK = {
    "path": Path("data/high_volatility_causal_oi_range_regime_relay_clocks_2020_2026.csv.gz"),
    "sha256": "20372bf662aa9ab99800b59562f04e8923e6dd7f1fa574e7a2122fcd85dfd38d",
    "rows": 70,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_oi_range_regime_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCORR-24 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCORR-24 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCORR-24 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCORR-24 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCORR-24 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCORR-24 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCORR-24 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCORR-24 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCORR-24 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCORR-24 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCORR-24 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcorr_24_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
