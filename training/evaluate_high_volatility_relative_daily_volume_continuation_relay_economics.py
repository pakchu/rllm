"""Sequential strict economics for frozen HVRDV-8."""
...
PREREG = Path("results/high_volatility_relative_daily_volume_continuation_relay_preregistration_2026-08-11.json")
PREREG_SHA = "7bc629565ae92c6ff5b8e6e00797f816312facaf2aa962bc20d81db718606f81"
SUPPORT = Path("results/high_volatility_relative_daily_volume_continuation_relay_support_2026-08-11.json")
SUPPORT_SHA = "0c0e0036d012017e074a4f1c4886461ae5aec7fcfe61cf533159f85dad4c2ad3"
NOVELTY = Path("results/high_volatility_relative_daily_volume_continuation_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "fe7f3f2ecfa920e8ed31665c84ebd0c2e73cd8c51c0dc1d3f49ed15f9d34978c"
CLOCK = Path("data/high_volatility_relative_daily_volume_continuation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "c00be7e991fe59bbaf0523944588dd610e796471af792cb909346c10323d420e"
CONTROL_DIR = Path("data/high_volatility_relative_daily_volume_continuation_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_relative_daily_volume_continuation_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_relative_daily_volume_continuation_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = (
    "no_variation_gate",
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVRDV frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVRDV novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrdv_8_sequential_economics_v1", "policy_id": POLICY_ID,
