"""Evaluate frozen HVSPPE-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVSPPE-12"
PROTOCOL = "hvsppe_12_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_spot_perpetual_permutation_entropy_leadership_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVSPPE predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVSPPE novelty limits drift")
    if (
        ...
        raise RuntimeError("HVSPPE predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVSPPE primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvsppe_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVSPPE-12 unchanged before economic outcomes",
    }
