"""Sequential strict economics for frozen HVVRT-12."""
...
PREREG = Path("results/high_volatility_variance_ratio_trend_onset_relay_preregistration_2026-08-11.json")
PREREG_SHA = "b9dab67bf11bc7110947f4691fbcb3ddff45fc3b35d8c2c479414361b4380284"
SUPPORT = Path("results/high_volatility_variance_ratio_trend_onset_relay_support_2026-08-11.json")
SUPPORT_SHA = "e2daa7edc8d8f956d8e924809a9a8b7efff1ba17816fb7d3232c6ca5638939f6"
NOVELTY = Path("results/high_volatility_variance_ratio_trend_onset_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "7723c18defa416290907e5f38243d5e48213d6f3bfce078a2dcb0371d66946f1"
CLOCK = Path("data/high_volatility_variance_ratio_trend_onset_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d7b2b9bb6e4819dab854c808f275e5da20f44ee8ddd092587ea03ab9865bf8e0"
CONTROL_DIR = Path("data/high_volatility_variance_ratio_trend_onset_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_variance_ratio_trend_onset_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_variance_ratio_trend_onset_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "below_unity_reversion", "one_bar_stale_onset", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVVRT frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVVRT novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvvrt_12_sequential_economics_v1", "policy_id": POLICY_ID,
