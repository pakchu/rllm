"""Outcome-blind preregistration for HVPLOIEC-8."""
...
POLICY_ID = "HVPLOIEC-8"
DEFAULT_OUTPUT = Path("results/high_volatility_perpetual_lead_open_interest_expansion_continuation_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_perpetual_lead_open_interest_expansion_continuation_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": "When completed BTC spot and perpetual eight-hour returns agree, the perpetual contract overshoots spot in that common direction, realized variation is elevated, and open interest expands, the perpetual displacement has fresh leveraged sponsorship and should continue over eight hours.",
            "side": "same strict sign of the completed eight-hour BTC perpetual and spot returns",
            "why_distinct": "HVPLOIEC requires an exact completed-range close location with exact OI expansion and variation. No prior composite event set or control is reused; it uses no spot, ETH, funding, premium-index, fitted outcome, or promoted control.",
            "why_suited_to_volatile_regimes": "the completed eight-hour BTC realized variation must be in its causal upper 40 percent",
            "why_low_gross9_overlap_is_plausible": "fixed eight-hour spot-perpetual overshoot plus OI-expansion sponsorship states are absent from Gross9 primitives",
        },
        "features": {
            ...
            "directional_perpetual_overshoot": "sign(perpetual_return)*(perpetual_return-spot_return)>0, strict with no fitted magnitude threshold",
            ...
            "eligible": "exact coherent synchronized 480-row spot and perpetual cycles, strict direction agreement, strict directional perpetual overshoot, two exact OI endpoints with strict expansion, and the variation rank passes at D",
            "no_imputation": True,
        },
        "clock": {
        ...
        "research_boundary": {
            ...
            "prior_three_spot_perpetual_OI_state_failures_known": True,
            "prior_OI_contraction_family_failures_known": True,
            "exact_perpetual_lead_OI_expansion_incidence_or_outcomes_known": False,
        ...
        "stopping_rule": "terminal first failure; no OI endpoint or expansion rule, spot-perpetual agreement or overshoot rule, variation history, side, clock, hold, subset, threshold, comparator, or control repair",
    }
