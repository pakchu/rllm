"""Deterministic outcome-blind source support for HVCFIR-8."""
...
from training import preregister_high_volatility_causal_funding_impulse_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2023-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "2365cd0e87cec655d561ad1df354cf8bca85d12ba808252d5427ce6ded70fce3"
...
ROOT = Path("data/high_volatility_causal_funding_impulse_relay_sources_2023_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_funding_impulse_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_funding_impulse_relay_split_clocks_2023_2026")
RESULT = Path("results/high_volatility_causal_funding_impulse_relay_support_2026-08-16.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "funding_rate",
    "previous_funding_rate", "previous_funding_change", "funding_change",
    "impulse_reversal", "absolute_change_rank",
    "realized_variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "decision_time", "feature_available_time",
    "entry_time", "exit_time", "side", "previous_funding_change", "funding_change",
    "absolute_change_rank", "realized_variation", "variation_rank",
)
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if funding.columns.tolist() != ["funding_time", "funding_rate"]:
        raise RuntimeError("HVCFIR-8 funding schema drift")
    expected_bars = [
    ...
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCFIR-8 bar schema drift")
    ...
    if funding["funding_time"].duplicated().any() or not np.isfinite(funding["funding_rate"]).all():
        raise RuntimeError("HVCFIR-8 invalid funding rows")
    ...
    if bars["decision_time"].duplicated().any():
        raise RuntimeError("HVCFIR-8 duplicate eight-hour bar")
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel["prior_time"] = panel["funding_time"].shift(2)
    panel["previous_funding_rate"] = panel["funding_rate"].shift(1)
    panel["funding_change"] = panel["funding_rate"] - panel["previous_funding_rate"]
    panel["previous_funding_change"] = panel["funding_change"].shift(1)
    ...
    panel["source_valid"] = (
        ...
        & panel["previous_time"].sub(panel["prior_time"]).eq(
            pd.Timedelta(hours=POLICY["required_settlement_gap_hours"])
        )
        & panel["bar_valid"].eq(True)
        & np.isfinite(
            panel[["previous_funding_change", "funding_change", "realized_variation"]]
        ).all(axis=1)
        & panel["previous_funding_change"].ne(0)
        & panel["funding_change"].ne(0)
        & panel["realized_variation"].gt(0)
    )
    valid = panel["source_valid"].eq(True)
    panel["absolute_change_rank"] = causal_midrank(panel["funding_change"].abs().where(valid))
    panel["variation_rank"] = causal_midrank(panel["realized_variation"].where(valid))
    panel["impulse_reversal"] = (
        np.sign(panel["funding_change"]).eq(-np.sign(panel["previous_funding_change"]))
    )
    panel["eligible"] = (
        valid
        & panel["impulse_reversal"]
        & panel["absolute_change_rank"].ge(POLICY["absolute_change_rank_min"])
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if split is None:
            continue
        side = int(np.sign(row.funding_change))
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCFIR-8 side or availability drift")
        ...
        rows.append({
            ...
            "previous_funding_change": float(row.previous_funding_change),
            "funding_change": float(row.funding_change),
            "absolute_change_rank": float(row.absolute_change_rank),
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCFIR-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcfir_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvcfir_8_source_support_v1", "policy_id": prereg.POLICY_ID,
