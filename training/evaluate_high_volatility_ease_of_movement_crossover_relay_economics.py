"""Sequential strict economics for frozen HVEOM-24."""
...
PREREG = Path("results/high_volatility_ease_of_movement_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "fc28b0927f339a2c8b2709aa856e6172697dd1a57f38f40b67da5b6cd76612a6"
SUPPORT = Path("results/high_volatility_ease_of_movement_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "04e2e9616b05ce06cc51a86f2f82905286a18607e66d3a86644b8f482cfcf7fc"
NOVELTY = Path("results/high_volatility_ease_of_movement_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "a2bf67d27dc81d81992e1378693ca55a703c12d96bd11dc6d7593e47b96cd99d"
CLOCK = Path("data/high_volatility_ease_of_movement_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "9a942aeccebb49a65091a1159f4fcecef1f3564eddaf30f2c623d3fdde3b0f50"
CONTROL_DIR = Path("data/high_volatility_ease_of_movement_crossover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_ease_of_movement_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ease_of_movement_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "one_bar_stale_crossover", "direction_flip", "midpoint_only_crossover")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVEOM frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVEOM novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hveom_24_sequential_economics_v1", "policy_id": POLICY_ID,
