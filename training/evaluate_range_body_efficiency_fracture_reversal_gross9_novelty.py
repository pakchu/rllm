"""Evaluate frozen RBEFR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "RBEFR-8"
PROTOCOL = "rbefr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/range_body_efficiency_fracture_reversal_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("RBEFR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("RBEFR novelty limits drift")
    if (
        ...
        raise RuntimeError("RBEFR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="RBEFR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "rbefr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject RBEFR-8 unchanged before economic outcomes",
    }
