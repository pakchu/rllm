"""Build outcome-blind source support for frozen HVPGO-24."""
...
from training import preregister_high_volatility_pretty_good_oscillator_breakout_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "0530862a2aa096ebe62c0346cad767b58420d380c774005bb2105d84e602294e"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_pretty_good_oscillator_breakout_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_pretty_good_oscillator_breakout_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_pretty_good_oscillator_breakout_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_pretty_good_oscillator_breakout_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_pretty_good_oscillator_breakout_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "true_range", "close_sma", "range_ema", "pgo",
    "entry_side", "zero_line_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "true_range", "close_sma", "range_ema",
    "pgo", "entry_side", "zero_line_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVPGO source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVPGO invalid source key")
...
def pgo_values(bars: pd.DataFrame, valid: pd.Series) -> pd.DataFrame:
    previous_close = bars.bar_close.shift(1)
    true_range = pd.concat(
        [bars.bar_high-bars.bar_low, (bars.bar_high-previous_close).abs(), (bars.bar_low-previous_close).abs()],
        axis=1,
    ).max(axis=1).where(valid & valid.shift(1, fill_value=False))
    period=P["periods"];complete=valid.rolling(period,min_periods=period).sum().eq(period)
    close_sma=bars.bar_close.rolling(period,min_periods=period).mean().where(complete)
    range_ema=causal_ema(true_range,np.isfinite(true_range),period)
    pgo=((bars.bar_close-close_sma)/range_ema).where(range_ema.gt(0))
    return pd.DataFrame({"true_range":true_range,"close_sma":close_sma,"range_ema":range_ema,"pgo":pgo},index=bars.index)
...
def breakout_side(signal: pd.Series) -> pd.Series:
    previous=signal.shift(1);threshold=P["breakout_threshold"];side=np.zeros(len(signal),dtype=int)
    side[(signal>threshold)&(previous<=threshold)]=1;side[(signal<-threshold)&(previous>=-threshold)]=-1
    return pd.Series(side,index=signal.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    values=pgo_values(bars,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["entry_side"]=breakout_side(bars.pgo);bars["zero_line_side"]=cross_side(bars.pgo)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["true_range","close_sma","range_ema","pgo","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.true_range.gt(0)&bars.range_ema.gt(0)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="zero_line_cross":side=pd.to_numeric(used.zero_line_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVPGO prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvpgo_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvpgo_24_source_support_v1", "policy_id": prereg.POLICY_ID,
