"""Build outcome-blind source support for frozen HVCRSI-24."""
...
PREREG_SHA = "c5d15adea99167743c988d54902dec4307069d4d04846f077759258b03fd8fc6"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_connors_rsi_extreme_reversal_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_connors_rsi_extreme_reversal_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_connors_rsi_extreme_reversal_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_connors_rsi_extreme_reversal_controls_2023_2026")
RESULT = Path("results/high_volatility_connors_rsi_extreme_reversal_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "price_rsi", "streak", "streak_rsi", "one_bar_return",
    "percent_rank", "connors_rsi", "entry_side", "price_rsi_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "price_rsi", "streak", "streak_rsi",
    "one_bar_return", "percent_rank", "connors_rsi", "entry_side", "price_rsi_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVCRSI source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVCRSI invalid source key")
...
def wilder_rsi(series: pd.Series, valid: pd.Series, period: int) -> pd.Series:
    output=np.full(len(series),np.nan);previous=np.nan;gains=[];losses=[];average_gain=average_loss=np.nan
    for index,(value,is_valid) in enumerate(zip(series.to_numpy(float),valid.to_numpy(bool))):
        if not is_valid or not math.isfinite(value):previous=np.nan;gains=[];losses=[];average_gain=average_loss=np.nan;continue
        if not math.isfinite(previous):previous=value;continue
        change=value-previous;previous=value;gain=max(change,0.0);loss=max(-change,0.0)
        if not math.isfinite(average_gain):
            gains.append(gain);losses.append(loss)
            if len(gains)<period:continue
            average_gain=float(np.mean(gains[-period:]));average_loss=float(np.mean(losses[-period:]))
        else:
            average_gain=((period-1)*average_gain+gain)/period;average_loss=((period-1)*average_loss+loss)/period
        if average_gain==0 and average_loss==0:continue
        output[index]=100.0 if average_loss==0 else 0.0 if average_gain==0 else 100.0-100.0/(1.0+average_gain/average_loss)
    return pd.Series(output,index=series.index)


def signed_streak(close: pd.Series, valid: pd.Series) -> pd.Series:
    output=np.full(len(close),np.nan);previous=np.nan;state=0
    for index,(value,is_valid) in enumerate(zip(close.to_numpy(float),valid.to_numpy(bool))):
        if not is_valid or not math.isfinite(value):previous=np.nan;state=0;continue
        if not math.isfinite(previous):state=0
        elif value>previous:state=state+1 if state>0 else 1
        elif value<previous:state=state-1 if state<0 else -1
        else:state=0
        output[index]=state;previous=value
    return pd.Series(output,index=close.index)


def return_percent_rank(close: pd.Series, valid: pd.Series) -> tuple[pd.Series,pd.Series]:
    returns=np.full(len(close),np.nan);ranks=np.full(len(close),np.nan);history=[];previous=np.nan;period=P["return_rank_periods"]
    for index,(value,is_valid) in enumerate(zip(close.to_numpy(float),valid.to_numpy(bool))):
        if not is_valid or not math.isfinite(value):previous=np.nan;history=[];continue
        if not math.isfinite(previous):previous=value;continue
        current=value/previous-1.0;previous=value;returns[index]=current
        if len(history)>=period:ranks[index]=100.0*sum(item<current for item in history[-period:])/period
        history.append(current)
    return pd.Series(returns,index=close.index),pd.Series(ranks,index=close.index)


def entry_side(signal: pd.Series) -> pd.Series:
    previous=signal.shift(1);side=np.zeros(len(signal),dtype=int)
    side[(signal<P["lower_extreme"])&(previous>=P["lower_extreme"])]=1
    side[(signal>P["upper_extreme"])&(previous<=P["upper_extreme"])]=-1
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["price_rsi"]=wilder_rsi(bars.bar_close,bars.valid_bar,P["price_rsi_periods"]);bars["streak"]=signed_streak(bars.bar_close,bars.valid_bar);bars["streak_rsi"]=wilder_rsi(bars.streak,bars.valid_bar,P["streak_rsi_periods"]);bars["one_bar_return"],bars["percent_rank"]=return_percent_rank(bars.bar_close,bars.valid_bar);bars["connors_rsi"]=(bars.price_rsi+bars.streak_rsi+bars.percent_rank)/3;bars["entry_side"]=entry_side(bars.connors_rsi);bars["price_rsi_side"]=entry_side(bars.price_rsi)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["price_rsi","streak","streak_rsi","one_bar_return","percent_rank","connors_rsi","variation"]]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="price_rsi_only_extreme":side=pd.to_numeric(used.price_rsi_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
    elif control=="one_bar_stale_entry":state=state.shift(1,fill_value=False);side=side.shift(1,fill_value=0)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCRSI prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvcrsi_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvcrsi_24_source_support_v1", "policy_id": prereg.POLICY_ID,
