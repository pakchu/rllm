"""Build outcome-blind source support for frozen HVMACD-24."""
...
from training import preregister_high_volatility_macd_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "fb32500402d790a7127a1ac6fdcba08c773adab6166098af8c26e093dc8da67c"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_macd_crossover_relay_sources_2023_2026")
PANEL = ROOT / "daily_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_macd_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_macd_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_macd_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_macd_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = ("source_day","feature_available_time","decision_time","source_valid","daily_close","daily_return","ema_fast","ema_slow","macd","signal","difference","variation","variation_rank","eligible")
CLOCK_COLUMNS = ("candidate","control","split","source_day","feature_available_time","decision_time","entry_time","exit_time","side","daily_close","daily_return","ema_fast","ema_slow","macd","signal","difference","variation","variation_rank","eligible")
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVMACD source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVMACD invalid source key")
...
def seeded_ema(values: pd.Series, period: int) -> pd.Series:
    ...
    if seed_end >= len(raw) or not np.isfinite(raw[first : seed_end + 1]).all():
        return pd.Series(output, index=values.index)
    output[seed_end] = raw[first : seed_end + 1].mean()
    alpha = 2.0 / (period + 1.0)
    for index in range(seed_end + 1, len(raw)):
        if math.isfinite(output[index - 1]) and math.isfinite(raw[index]):
            output[index] = alpha * raw[index] + (1.0 - alpha) * output[index - 1]
    return pd.Series(output, index=values.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    daily = pd.DataFrame({"rows": groups.row_valid.sum(), "daily_close": groups.close.last()})
    daily["valid_day"] = daily.rows.eq(1440) & np.isfinite(daily.daily_close) & daily.daily_close.gt(0)
    daily["daily_return"] = np.log(daily.daily_close / daily.daily_close.shift(1)).where(
        daily.valid_day & daily.valid_day.shift(1, fill_value=False)
    )
    closes = daily.daily_close.where(daily.valid_day)
    daily["ema_fast"] = seeded_ema(closes, P["fast_ema_days"])
    daily["ema_slow"] = seeded_ema(closes, P["slow_ema_days"])
    daily["macd"] = daily.ema_fast - daily.ema_slow
    daily["signal"] = seeded_ema(daily.macd, P["signal_ema_days"])
    daily["difference"] = daily.macd - daily.signal
    daily["variation"] = np.sqrt(
        np.square(daily.daily_return).rolling(P["variation_days"], min_periods=P["variation_days"]).sum()
    )
    daily["source_valid"] = daily.valid_day & np.isfinite(
        daily[["daily_return", "ema_fast", "ema_slow", "macd", "signal", "difference", "variation"]]
    ).all(axis=1) & daily.variation.gt(0)
    ...
    panel["eligible"] = (
        panel.source_valid & panel.source_valid.shift(1, fill_value=False)
        & panel.difference.ne(0) & previous.ne(0)
        & np.sign(panel.difference).ne(np.sign(previous))
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    difference = pd.to_numeric(used.difference, errors="coerce")
    ...
    side = pd.Series(np.sign(difference), index=used.index).fillna(0).astype(int)
    state = valid & side.ne(0) & crossover & variation
    if control == "no_variation_gate":
        state = valid & side.ne(0) & crossover
    elif control == "one_day_stale_crossover":
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVMACD prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvmacd_24_source_v1",
        "query": QUERY,
    ...
    core = {
        "protocol_version": "hvmacd_24_source_support_v1",
