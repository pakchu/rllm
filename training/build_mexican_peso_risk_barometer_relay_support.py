"""Materialize outcome-blind source support for frozen MXRBR-12."""
...
ENV_FILE="/home/pakchu/rllm/.env";BUILDER=Path("training/build_mexican_peso_risk_barometer_relay_support.py");PREREG_SHA="191c1e6448677973c778404ea774bd0bd3cccc343a72183b6270dd7684a42816";SOURCE_DIR=Path("data/mexican_peso_risk_barometer_relay_sources_2023_2026");FEATURES=SOURCE_DIR/"mxrbr_preentry_features.csv.gz";MANIFEST=SOURCE_DIR/"manifest.json";CLOCK=Path("data/mexican_peso_risk_barometer_relay_clocks_2023_2026.csv.gz");CONTROL_DIR=Path("data/mexican_peso_risk_barometer_relay_controls_2023_2026");RESULT=Path("results/mexican_peso_risk_barometer_relay_support_2026-08-09.json")
BTC_START=pd.Timestamp("2022-12-29T00:00:00Z");FX_START=pd.Timestamp("2023-01-01T00:00:00Z");SOURCE_END=pd.Timestamp("2026-08-01T00:00:00Z");NY=ZoneInfo("America/New_York")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00:00Z"),pd.Timestamp("2024-01-01T00:00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00:00Z"),pd.Timestamp("2025-01-01T00:00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00:00Z"),pd.Timestamp("2026-01-01T00:00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00:00Z"),pd.Timestamp("2026-08-01T00:00:00Z"))};MINIMUM_EVENTS={"train":8,"test":12,"eval":12,"final":8};CONTROLS=("no_volatility_gate","one_session_stale_fx","direction_flip");COLUMNS=("candidate","control","split","session_date","decision_time","feature_available_time","entry_time","exit_time","side","usdmxn_return","btc_realized_variation","btc_variation_rank")
BTC_QUERY="SELECT ts,open,close FROM bars_binance WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end ORDER BY ts";FX_QUERY="SELECT ts,open,close FROM bars_polygon WHERE symbol='USDMXN' AND interval='1m' AND ts>=:start AND ts<:end ORDER BY ts"
...
def load_sources()->tuple[pd.DataFrame,pd.DataFrame]:
 ...
 for label,f in (("BTC",btc),("FX",fx)):
  if f.columns.tolist()!=["ts","open","close"]:raise RuntimeError(f"MXRBR {label} schema drift")
  f.ts=pd.to_datetime(f.ts,utc=True,errors="raise");f.sort_values("ts",inplace=True);f.reset_index(drop=True,inplace=True)
  if f.ts.duplicated().any():raise RuntimeError(f"MXRBR {label} duplicate timestamp")
  for c in ("open","close"):f[c]=pd.to_numeric(f[c],errors="coerce")
  if not np.isfinite(f[["open","close"]]).all(axis=None) or not f[["open","close"]].gt(0).all(axis=None):raise RuntimeError(f"MXRBR {label} invalid price")
 expected=pd.date_range(BTC_START,SOURCE_END,freq="1min",inclusive="left")
 if len(btc)!=len(expected) or not btc.ts.equals(pd.Series(expected,name="ts")):raise RuntimeError("MXRBR BTC source not exact 1m grid")
...
def build_features(btc:pd.DataFrame,fx:pd.DataFrame)->pd.DataFrame:
 ...
 for day in pd.date_range("2023-01-02","2026-07-31",freq="B"):
  start=pd.Timestamp(day).tz_localize(NY)+pd.Timedelta(hours=10);decision=pd.Timestamp(day).tz_localize(NY)+pd.Timedelta(hours=16);start,decision=start.tz_convert("UTC"),decision.tz_convert("UTC")
  expected=pd.date_range(start,decision,freq="1min",inclusive="left");session=fx.reindex(expected)
  if session[["open","close"]].isna().any(axis=None):continue
  bw=btc.loc[decision-pd.Timedelta(hours=24):decision-pd.Timedelta(minutes=1)]
  if len(bw)!=1440:continue
  fxret=float(np.log(session.close.iloc[-1]/session.open.iloc[0]));variation=float(np.sqrt(np.square(np.log(bw.close.to_numpy()/bw.open.to_numpy())).sum()));rows.append({"session_date":day,"decision_time":decision,"usdmxn_return":fxret,"btc_realized_variation":variation})
 f=pd.DataFrame(rows);f["btc_variation_rank"]=rank(f.btc_realized_variation);return f
def signal(f:pd.DataFrame,control:str)->pd.Series:
 ret=f.usdmxn_return;side=-np.sign(ret).astype("Int64").fillna(0).astype(int);eligible=ret.ne(0)&f.btc_variation_rank.ge(.65)
 if control=="no_volatility_gate":eligible=ret.ne(0)
 elif control=="one_session_stale_fx":ret=ret.shift(1);side=-np.sign(ret).astype("Int64").fillna(0).astype(int);eligible=ret.ne(0)&f.btc_variation_rank.ge(.65)
...
def build_clock(f:pd.DataFrame,control="primary")->pd.DataFrame:
 ...
 for i in f.index[sides.ne(0)]:
  ...
  next_allowed=exit_time;rows.append({"candidate":"MXRBR-12","control":control,"split":split,"session_date":pd.Timestamp(f.at[i,"session_date"]),"decision_time":d,"feature_available_time":d,"entry_time":entry,"exit_time":exit_time,"side":int(sides.at[i]),"usdmxn_return":float(f.at[i,"usdmxn_return"]),"btc_realized_variation":float(f.at[i,"btc_realized_variation"]),"btc_variation_rank":float(f.at[i,"btc_variation_rank"])})
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("MXRBR prereg drift")
 ...
 sc={"protocol_version":"mxrbr_12_sources_v1","queries":{"btc":BTC_QUERY,"fx":FX_QUERY},"windows":{"btc":[BTC_START.isoformat(),SOURCE_END.isoformat()],"fx":[FX_START.isoformat(),SOURCE_END.isoformat()]},"rows":{"btc":len(btc),"fx":len(fx),"features":len(features)},"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"feature_output":{"path":str(FEATURES),"sha256":sha(FEATURES)},"candidate_outcomes_opened":False,"no_imputation":True};sm={**sc,"manifest_hash":chash(sc)};MANIFEST.write_text(json.dumps(sm,indent=2,ensure_ascii=False,allow_nan=False)+"\n");support={n:stats(primary,n) for n in SPLITS};checks={}
