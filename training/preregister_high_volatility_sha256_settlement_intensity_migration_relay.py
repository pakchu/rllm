"""Outcome-blind preregistration for HVSSIM-24."""
...
DEFAULT_OUTPUT = Path(
    "results/high_volatility_sha256_settlement_intensity_migration_relay_preregistration_2026-08-10.json"
)
...
def build() -> dict[str, Any]:
    core = {
        "protocol_version": "high_volatility_sha256_settlement_intensity_migration_relay_v1",
        "policy_id": "HVSSIM-24",
        ...
        "mechanism": {
            "claim": (
                "BTC and BCH expose related SHA-256 settlement networks. During volatile BTC "
                "trading, a large completed three-day migration of transactions per active address "
                "toward BTC indicates denser BTC-specific settlement usage and is traded long; "
                "migration toward BCH is traded short."
            ),
            "side": (
                "strict sign of the three-observation change in log((BTC TxCnt/AdrActCnt)/"
                "(BCH TxCnt/AdrActCnt))"
            ),
            "why_distinct": (
                "Existing blockspace candidates use BTC-only activity or block state. HVSSIM "
                "compares per-address transaction intensity across BTC and BCH; it does not reuse "
                "the terminal cross-chain hashrate or fee-pressure migration states."
            ),
        ...
        "source_contract": {
            ...
            "official_catalog": (
                ...
                "metrics=TxCnt%2CAdrActCnt%2CAssetEODCompletionTime"
            ),
            "assets": ["btc", "bch"],
            "metrics": ["TxCnt", "AdrActCnt", "AssetEODCompletionTime"],
            "frequency": "1d",
            "observation_window": ["2023-01-01", "2026-07-31"],
            "schema": ["asset", "time", "TxCnt", "AdrActCnt", "AssetEODCompletionTime"],
            "row_validity": (
                "exactly one row per asset and UTC observation day; finite strictly-positive "
                "TxCnt and AdrActCnt, and completion epoch at least observation day plus one "
                "elapsed day"
            ),
            "pair_validity": (
        ...
        "feature_contract": {
            "btc_settlement_intensity": "BTC TxCnt/BTC AdrActCnt; strict positive",
            "bch_settlement_intensity": "BCH TxCnt/BCH AdrActCnt; strict positive",
            "relative_settlement_intensity": (
                "log(btc_settlement_intensity/bch_settlement_intensity)"
            ),
            "intensity_migration": (
                "current relative_settlement_intensity minus the value exactly three paired observation "
                "days earlier; strict nonzero"
            ),
            "absolute_migration_rank": (
                "strict-prior midrank of abs(intensity_migration), current excluded, at most 365 "
            ...
            "eligible_state": (
                ...
                "intensity_migration strict nonzero"
            ),
        ...
        "research_boundary": {
            ...
            "terminal_cross_chain_fee_source_incidence_known": True,
            "prior_bch_or_cross_chain_settlement_intensity_outcomes_known": False,
            ...
            "selection_basis": (
                "new cross-chain settlement-intensity migration mechanism fixed before candidate "
