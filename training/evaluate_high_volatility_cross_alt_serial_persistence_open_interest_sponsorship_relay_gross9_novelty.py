"""Evaluate frozen HVCASPCOIS-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCASPCOIS-8"
PROTOCOL = "hvcaspc_ois_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_cross_alt_serial_persistence_open_interest_sponsorship_relay_gross9_novelty_2026-08-16.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCASPCOIS predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCASPCOIS novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCASPCOIS predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCASPCOIS primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "candidate_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCASPCOIS-8 unchanged before economic outcomes",
    }
