"""Sequential strict economics for frozen HVPVT-24."""
...
PREREG = Path("results/high_volatility_price_volume_trend_signal_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "deaf14da7fc41f42c0a2e2907dee747b9b649864614a9e89390a3ba6a2f46d6b"
SUPPORT = Path("results/high_volatility_price_volume_trend_signal_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "c1ccf90f90d9b64535064cd7c15ffbb157a1a80ab5e5396ab8cee0ed4f4b7b80"
NOVELTY = Path("results/high_volatility_price_volume_trend_signal_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "93f3bfaea6b4504a0259dbb327af36f37a9c781dbe8a441b0c8e06193252a9d3"
CLOCK = Path("data/high_volatility_price_volume_trend_signal_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "1590f76192bc286ed135ec972eb7162613b2949dd92f53c41c7aab6be6f06705"
CONTROL_DIR = Path("data/high_volatility_price_volume_trend_signal_crossover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_price_volume_trend_signal_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_price_volume_trend_signal_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "contribution_zero_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVPVT frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVPVT novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvpvt_24_sequential_economics_v1", "policy_id": POLICY_ID,
