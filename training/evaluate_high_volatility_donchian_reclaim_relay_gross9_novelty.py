"""Evaluate frozen HVDCR-72 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDCR-72"
PROTOCOL = "hvdcr_72_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_donchian_reclaim_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDCR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVDCR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDCR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDCR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdcr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVDCR-72 unchanged before economic outcomes"
        ),
    }
