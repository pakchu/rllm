"""Evaluate frozen HVCARER-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCARER-12"
PROTOCOL = "hvcarer_12_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_cross_alt_range_expansion_confirmation_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCARER predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCARER novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCARER predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCARER primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcarer_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCARER-12 unchanged before economic outcomes",
    }
