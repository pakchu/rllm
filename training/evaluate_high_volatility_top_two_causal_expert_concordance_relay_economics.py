"""Sequential strict economics for frozen HVTCEC-24."""
...
PREREG = Path("results/high_volatility_top_two_causal_expert_concordance_relay_preregistration_2026-08-13.json")
PREREG_SHA = "26f869135ea943147a98449c581a4480373346d3cef5898040a76a0eff6362cc"
SUPPORT = Path("results/high_volatility_top_two_causal_expert_concordance_relay_support_2026-08-13.json")
SUPPORT_SHA = "043e4f749db4821054bd112a0c384a89b5e88ebfa868eea4412c5e8deaabb549"
NOVELTY = Path("results/high_volatility_top_two_causal_expert_concordance_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "591a88d2730ca6864cc29a28ba562ddbf1a6b5e79ff9dccf8b06800cb0a2d1a0"
CLOCK = Path("data/high_volatility_top_two_causal_expert_concordance_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "761371c5e82b232889d88320dc16b074018a8b64540abd30b0f077bf44991070"
CONTROL_DIR = Path("data/high_volatility_top_two_causal_expert_concordance_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_top_two_causal_expert_concordance_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_top_two_causal_expert_concordance_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "no_top_two_concordance", "one_decision_stale_committee", "direction_flip", "same_clock_forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTCEC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTCEC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvtcec_24_sequential_economics_v1", "policy_id": POLICY_ID,
