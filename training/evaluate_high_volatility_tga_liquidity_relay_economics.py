"""Sequential strict economics for frozen HVTGAL-24."""
...
PREREG = Path("results/high_volatility_tga_liquidity_relay_preregistration_2026-08-10.json")
PREREG_SHA = "a076cda574a55ce909f28a26ca0afdad687a281d4657cea16a69c07a3e0705e8"
SUPPORT = Path("results/high_volatility_tga_liquidity_relay_support_2026-08-10.json")
SUPPORT_SHA = "6c8a80aae86db1dd3370195beb5a228e6c6551a31a894807e892ac1ed1c1345d"
NOVELTY = Path("results/high_volatility_tga_liquidity_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "459aa66778b87fe307f9a3a56585ee1e37055072d66b3e6934dd2db6cc950d62"
CLOCK = Path("data/high_volatility_tga_liquidity_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "959dc4465a8e04badcd127d453e7ae0e911bbe88d802487630624a3315a1b42f"
CONTROL_DIR = Path("data/high_volatility_tga_liquidity_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_tga_liquidity_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_tga_liquidity_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTGAL frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTGAL novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvtgal_24_sequential_economics_v1", "policy_id": POLICY_ID,
