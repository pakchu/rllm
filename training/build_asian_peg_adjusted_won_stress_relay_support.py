"""Materialize source-only APWSR-12 support clocks."""
...
PREREG_SHA="69b72d60f72f916da753f4c9ae1780e925a884e529bd98b02fc9e9cf72ec48fa"
SOURCE_DIR=Path("data/asian_peg_adjusted_won_stress_relay_sources_2023_2026"); SESSION=SOURCE_DIR/"asian_peg_adjusted_won_stress_sessions.csv.gz"; SOURCE_MANIFEST=SOURCE_DIR/"manifest.json"
...
CLOCK=Path("data/asian_peg_adjusted_won_stress_relay_clocks_2023_2026.csv.gz"); CONTROL_DIR=Path("data/asian_peg_adjusted_won_stress_relay_controls_2023_2026"); RESULT=Path("results/asian_peg_adjusted_won_stress_relay_support_2026-08-09.json")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00:00Z"),pd.Timestamp("2024-01-01T00:00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00:00Z"),pd.Timestamp("2025-01-01T00:00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00:00Z"),pd.Timestamp("2026-01-01T00:00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00:00Z"),pd.Timestamp("2026-08-01T00:00:00Z"))}; MINIMUM={"train":8,"test":12,"eval":12,"final":8}; CONTROLS=("no_volatility_gate","no_stress_tail","raw_return_spread","one_session_stale_stress","direction_flip","forced_long")
SYMBOLS=("USDKRW","USDHKD"); DOLLAR_MULTIPLIER={"USDKRW":1.,"USDHKD":1.}
COLUMNS=("candidate","control","split","source_day","decision_time","feature_available_time","entry_time","exit_time","side","relative_won_stress","stress_absolute_rank","raw_return_spread","btc_realized_variation","btc_realized_variation_rank")
...
def materialize_sessions()->dict:
 ...
 frame=frame.sort_values("source_day").reset_index(drop=True);valid_columns=[f"{symbol}_valid" for symbol in SYMBOLS];z_columns=[f"{symbol}_z" for symbol in SYMBOLS];return_columns=[f"{symbol}_dollar_return" for symbol in SYMBOLS];frame["source_valid"]=frame[valid_columns].fillna(False).all(axis=1);z=frame[z_columns].astype(float);frame["relative_won_stress"]=(z["USDKRW_z"]-z["USDHKD_z"]).where(frame.source_valid);frame["stress_absolute_rank"]=strict_prior_midrank(frame.relative_won_stress.abs().where(frame.source_valid));returns=frame[return_columns].astype(float);frame["raw_return_spread"]=(returns["USDKRW_dollar_return"]-returns["USDHKD_dollar_return"]).where(frame.source_valid);frame["decision_time"]=frame.source_day+pd.Timedelta(hours=8)
 SOURCE_DIR.mkdir(parents=True,exist_ok=True);_write_gzip_csv(frame,SESSION);core={"protocol_version":"apwsr_12_relative_fx_source_v1","query":QUERY,"table":"bars_polygon","symbols":list(SYMBOLS),"canonical_dollar_multipliers":DOLLAR_MULTIPLIER,"interval":"1m","window":[START.isoformat(),END.isoformat()],"outcomes_opened":False,"candidate_incidence_opened":False,"no_imputation":True,"output":{"path":str(SESSION),"sha256":sha(SESSION),"rows":len(frame),"valid_rows":int(frame.source_valid.sum())}};payload={**core,"manifest_hash":chash(core)};SOURCE_MANIFEST.write_text(json.dumps(payload,indent=2,allow_nan=False)+"\n");return payload
def features()->pd.DataFrame:
 if sha(PRICE)!=PRICE_SHA or sha(PRICE_MANIFEST)!=PRICE_MANIFEST_SHA:raise RuntimeError("APWSR BTC source drift")
 ...
 for column in ("relative_won_stress","stress_absolute_rank","raw_return_spread"):f[column]=pd.to_numeric(f[column],errors="coerce")
 ...
 d=p[["decision_time","btc_realized_variation","btc_valid"]];f=f.merge(d,on="decision_time",how="left",validate="one_to_one");f["btc_realized_variation_rank"]=strict_prior_midrank(f.btc_realized_variation.where(f.btc_valid));f["signal_valid"]=f.source_valid&f.btc_valid&np.isfinite(f[["relative_won_stress","stress_absolute_rank","raw_return_spread","btc_realized_variation","btc_realized_variation_rank"]]).all(axis=1)&f.relative_won_stress.ne(0);return f
def conditions(f:pd.DataFrame,control:str)->tuple[pd.Series,pd.Series]:
 factor=f.raw_return_spread if control=="raw_return_spread" else f.relative_won_stress;rank=f.stress_absolute_rank
 if control=="one_session_stale_stress":factor=factor.shift(1);rank=rank.shift(1)
 tail=pd.Series(True,index=f.index) if control=="no_stress_tail" else rank.ge(.70);vol=pd.Series(True,index=f.index) if control=="no_volatility_gate" else f.btc_realized_variation_rank.ge(.65);active=f.signal_valid&np.isfinite(factor)&factor.ne(0)&tail&vol;side=-np.sign(factor);side=-side if control=="direction_flip" else side;side=pd.Series(1.,index=f.index) if control=="forced_long" else side;return active,side
def clock(f:pd.DataFrame,control:str="primary")->pd.DataFrame:
 ...
 for i in f.index[active]:
  ...
  next_allowed=exit_;rows.append({"candidate":"APWSR-12","control":control,"split":split,"source_day":f.at[i,"source_day"],"decision_time":decision,"feature_available_time":decision,"entry_time":entry,"exit_time":exit_,"side":int(side.at[i]),"relative_won_stress":float(f.at[i,"relative_won_stress"]),"stress_absolute_rank":float(f.at[i,"stress_absolute_rank"]),"raw_return_spread":float(f.at[i,"raw_return_spread"]),"btc_realized_variation":float(f.at[i,"btc_realized_variation"]),"btc_realized_variation_rank":float(f.at[i,"btc_realized_variation_rank"])})
...
def run()->dict:
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("APWSR preregistration hash drift")
