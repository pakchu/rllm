"""Evaluate frozen HVSMIE-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVSMIE-24"
PROTOCOL = "hvsmie_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_smi_ergodic_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVSMIE predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVSMIE novelty limits drift")
    if (
        ...
        raise RuntimeError("HVSMIE predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVSMIE primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvsmie_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVSMIE-24 unchanged before economic outcomes",
    }
