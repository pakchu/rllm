"""Build outcome-blind source support for preregistered CVACR-24."""
...
CLOCK=Path("data/cboe_volatility_acceleration_curve_relay_clocks_2023_2026.csv.gz");CONTROL_DIR=Path("data/cboe_volatility_acceleration_curve_relay_controls_2023_2026");RESULT=Path("results/cboe_volatility_acceleration_curve_relay_support_2026-08-08.json")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00:00Z"),pd.Timestamp("2024-01-01T00:00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00:00Z"),pd.Timestamp("2025-01-01T00:00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00:00Z"),pd.Timestamp("2026-01-01T00:00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00:00Z"),pd.Timestamp("2026-08-01T00:00:00Z"))};MIN={"train":8,"test":12,"eval":12,"final":8};CONTROLS=("no_vix_confirmation","vix_only","front_vix_only","broad_vix_only","direction_flip");NY=ZoneInfo("America/New_York");ECONOMIC_OUTCOMES_AUTHORIZED=False
COLUMNS=("candidate","control","split","observation_date","previous_observation_date","decision_time","feature_available_time","entry_time","exit_time","side","vix","front","broad","delta_vix","delta_front","delta_broad")
...
def features()->pd.DataFrame:
 if sha(prereg.SOURCE)!=prereg.SOURCE_SHA256 or sha(prereg.SOURCE_MANIFEST)!=prereg.SOURCE_MANIFEST_SHA256:raise RuntimeError("CVACR frozen source changed")
 ...
 if f.columns.tolist()!=expected:raise RuntimeError("CVACR source schema changed")
 f.observation_date=pd.to_datetime(f.observation_date,format="%Y-%m-%d")
 if not f.observation_date.is_monotonic_increasing or f.observation_date.duplicated().any():raise RuntimeError("CVACR source dates invalid")
 for c in expected[1:]:f[c]=pd.to_numeric(f[c],errors="coerce")
 if not np.isfinite(f[expected[1:]]).all().all() or not f[expected[1:]].gt(0).all().all():raise RuntimeError("CVACR source values invalid")
 f["vix"]=np.log(f.VIX_close);f["front"]=np.log(f.VIX9D_close/f.VIX_close);f["broad"]=np.log(f.VIX_close/f.VIX3M_close);f["delta_vix"]=f.vix.diff();f["delta_front"]=f.front.diff();f["delta_broad"]=f.broad.diff();return f
def raw_side(f:pd.DataFrame,control:str)->pd.Series:
 names={"no_vix_confirmation":["delta_front","delta_broad"],"vix_only":["delta_vix"],"front_vix_only":["delta_vix","delta_front"],"broad_vix_only":["delta_vix","delta_broad"]}.get(control,["delta_vix","delta_front","delta_broad"]);negative=f[names].lt(0).all(axis=1);positive=f[names].gt(0).all(axis=1);s=pd.Series(0,index=f.index,dtype=int);s.loc[negative]=1;s.loc[positive]=-1;return s
def clock(f:pd.DataFrame,control:str="primary")->pd.DataFrame:
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 s=raw_side(f,control);eligible=s.ne(0)&s.ne(s.shift(1,fill_value=0))
 ...
 for i in f.index[eligible]:
  ...
  rows.append({"candidate":"CVACR-24","control":control,"split":split,"observation_date":od.date().isoformat(),"previous_observation_date":f.at[i-1,"observation_date"].date().isoformat(),"decision_time":pd.Timestamp(od.date()).tz_localize(NY)+pd.Timedelta(hours=16),"feature_available_time":entry,"entry_time":entry,"exit_time":exit_,"side":side,"vix":float(f.at[i,"vix"]),"front":float(f.at[i,"front"]),"broad":float(f.at[i,"broad"]),"delta_vix":float(f.at[i,"delta_vix"]),"delta_front":float(f.at[i,"delta_front"]),"delta_broad":float(f.at[i,"delta_broad"])})
