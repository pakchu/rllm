"""Strict staged economics for the singleton HVIMR-8 policy."""
...
PREREG = Path("results/high_volatility_intraday_mechanism_router_preregistration_2026-08-16.json")
PREREG_SHA = "95ae4f54c1f59d4059687105d975faf964b67f2816c98af1fd2593e1cb329569"
SUPPORT = Path("results/high_volatility_intraday_mechanism_router_support_2026-08-16.json")
SUPPORT_SHA = "bc8f5252466ec1b88282f5116197c8b48fcea32d580b43753a13312c7c1a5aa8"
GROSS9 = Path("results/high_volatility_intraday_mechanism_router_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "aca74c85a840d03373adc3f9071742e37a57c034192780e1c0a407ba23caa74f"
CLOCK = {
    "path": Path("data/high_volatility_intraday_mechanism_router_clocks_2023_2026.csv.gz"),
    "sha256": "811c690e0774fe3ce8ef090c56d1724460ca78e614b2b2217b7d85b4746d63c7",
    "rows": 224,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_intraday_mechanism_router_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVIMR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVIMR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVIMR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVIMR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVIMR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVIMR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVIMR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVIMR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVIMR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVIMR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVIMR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvimr_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
