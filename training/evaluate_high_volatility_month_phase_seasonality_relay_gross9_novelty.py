"""Evaluate frozen HVMPS-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVMPS-12"
PROTOCOL = "hvmps_12_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_month_phase_seasonality_relay_preregistration_2026-08-10.json"
)
...
OUTPUT = Path(
    "results/high_volatility_month_phase_seasonality_relay_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVMPS predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVMPS novelty limits drift")
    if (
        ...
        raise RuntimeError("HVMPS predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVMPS primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvmps_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVMPS-12 unchanged before economic outcomes"
        ),
    }
