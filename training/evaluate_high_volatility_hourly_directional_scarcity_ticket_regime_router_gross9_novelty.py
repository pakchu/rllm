"""Evaluate the frozen HVHDSTR-8 action-vote clock against Gross9."""
...
POLICY = "HVHDSTR-8"
PROTOCOL = "hvhdstr_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_hourly_directional_scarcity_ticket_regime_router_preregistration_2026-08-16.json")
PREREG_SHA = "f1ac9cff4cdbca9f360ae71ac673eb5ca880303336c42e0d97662a9c732c4fcc"
SUPPORT = Path("results/high_volatility_hourly_directional_scarcity_ticket_regime_router_support_2026-08-16.json")
SUPPORT_SHA = "321d448bc46257e6329a830bea8c4915f02dae97bb0c3b436ef2a9f97e80a464"
CLOCK = {
    "path": "data/high_volatility_hourly_directional_scarcity_ticket_regime_router_clocks_2023_2026.csv.gz",
    "sha256": "7d44abe09d3f7df632391e9ee24b4306d348690b095bd07de9c91e9a0ad82b8d",
    "rows": 460,
}
OUTPUT = Path("results/high_volatility_hourly_directional_scarcity_ticket_regime_router_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVHDSTR-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVHDSTR-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVHDSTR-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVHDSTR-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVHDSTR-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVHDSTR-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVHDSTR-8 unchanged before economics",
    }
