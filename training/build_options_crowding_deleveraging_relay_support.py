"""Build outcome-blind OCDR-12A clocks and source-support evidence."""
...
from training import preregister_options_crowding_deleveraging_relay_v2 as prereg


SOURCE_DIR = Path("data/options_crowding_deleveraging_relay_sources_2023_2026")
MANIFEST = SOURCE_DIR / "manifest.json"
OUTPUT_CLOCK = Path("data/options_crowding_deleveraging_relay_clocks_2023_2026.csv.gz")
OUTPUT_CONTROLS = Path("data/options_crowding_deleveraging_relay_controls_2023_2026")
OUTPUT_RESULT = Path("results/options_crowding_deleveraging_relay_support_2026-08-08.json")
...
def joined_features(
    ...
    oi_values = oi.set_index("ts")["sum_open_interest"]
    current_times = joint["decision_time"] - pd.Timedelta(minutes=5)
    prior_times = joint["decision_time"] - pd.Timedelta(minutes=65)
    joint["oi_current"] = oi_values.reindex(current_times).to_numpy()
    joint["oi_prior"] = oi_values.reindex(prior_times).to_numpy()
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "ocdr_12a_source_support_v1",
        "policy_id": "OCDR-12A",
