"""Evaluate frozen HVLZCTR-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVLZCTR-6"
PROTOCOL = "hvlzctr_6_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_last_zero_crossing_trend_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVLZCTR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVLZCTR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVLZCTR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVLZCTR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvlzctr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVLZCTR-6 unchanged before economic outcomes",
    }
