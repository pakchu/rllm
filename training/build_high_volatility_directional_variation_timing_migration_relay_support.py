"""Build outcome-blind source support for frozen HVDVTM-8."""
...
from training import preregister_high_volatility_directional_variation_timing_migration_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env"; START=pd.Timestamp("2023-04-01T00:00:00Z"); END=pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA="7f9e37d31cc07c76c29bd3630b162f948ed2bf246476d9439372b8e428858dfb"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_directional_variation_timing_migration_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_directional_variation_timing_migration_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_directional_variation_timing_migration_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_directional_variation_timing_migration_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_directional_variation_timing_migration_relay_support_2026-08-10.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","positive_variation_centroid","negative_variation_centroid","directional_timing_contrast","timing_rank","directional_variation_mass_asymmetry","mass_asymmetry_rank","realized_variation","variation_rank","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","positive_variation_centroid","negative_variation_centroid","directional_timing_contrast","timing_rank","directional_variation_mass_asymmetry","mass_asymmetry_rank","realized_variation","variation_rank")
...
def timing_statistics(block:pd.DataFrame)->tuple[float,float,float,float,float]:
 returns=np.log(block.close.to_numpy(float)/block.open.to_numpy(float));energy=np.square(returns);time=np.arange(480,dtype=float)/479.;positive=returns>0;negative=returns<0;positive_mass=float(energy[positive].sum());negative_mass=float(energy[negative].sum());positive_centroid=float(np.sum(time[positive]*energy[positive])/positive_mass);negative_centroid=float(np.sum(time[negative]*energy[negative])/negative_mass);contrast=positive_centroid-negative_centroid;mass_asymmetry=(positive_mass-negative_mass)/(positive_mass+negative_mass);variation=float(np.sqrt(positive_mass+negative_mass));return positive_centroid,negative_centroid,contrast,mass_asymmetry,variation
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close"]:raise RuntimeError("HVDVTM source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVDVTM invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("6h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   positive_centroid,negative_centroid,contrast,mass_asymmetry,var=timing_statistics(block);valid=all(math.isfinite(v) for v in (positive_centroid,negative_centroid,contrast,mass_asymmetry,var)) and var>0 and contrast!=0
  else:positive_centroid=negative_centroid=contrast=mass_asymmetry=var=math.nan;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"positive_variation_centroid":positive_centroid,"negative_variation_centroid":negative_centroid,"directional_timing_contrast":contrast,"directional_variation_mass_asymmetry":mass_asymmetry,"realized_variation":var})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["timing_rank"]=prior_rank(panel.directional_timing_contrast.abs().where(valid));panel["mass_asymmetry_rank"]=prior_rank(panel.directional_variation_mass_asymmetry.abs().where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));panel["eligible"]=valid&panel.timing_rank.ge(P["timing_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"]);panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_block_stale_timing":
  current_valid=panel.source_valid.eq(True);cols=["positive_variation_centroid","negative_variation_centroid","directional_timing_contrast","timing_rank","directional_variation_mass_asymmetry","mass_asymmetry_rank","feature_available_time"];used[cols]=panel[cols].shift(1);used["source_valid"]=current_valid&panel.source_valid.shift(1).fillna(False)
 valid=used.source_valid.eq(True);strong=used.timing_rank.ge(P["timing_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"]);state=valid&strong&variation
 if control=="no_timing_tail":state=valid&variation
 elif control=="no_variation_gate":state=valid&strong
 elif control=="directional_variation_mass_asymmetry":state=valid&used.directional_variation_mass_asymmetry.ne(0)&used.mass_asymmetry_rank.ge(P["timing_rank_min"])&variation
 onset=previous_valid_onset(state,valid);side=np.sign(pd.to_numeric(used.directional_timing_contrast,errors="coerce").fillna(0)).astype(int)
 if control=="directional_variation_mass_asymmetry":side=np.sign(pd.to_numeric(used.directional_variation_mass_asymmetry,errors="coerce").fillna(0)).astype(int)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVDVTM prereg drift")
 ...
 source_core={"protocol_version":"hvdvtm_8_source_v1","query":QUERY,"query_sha256":hashlib.sha256(QUERY.encode()).hexdigest(),"window":[START.isoformat(),END.isoformat()],"physical_rows":len(raw),"builder":{"path":str(BUILDER),"sha256":sha(BUILDER)},"panel":{"path":str(PANEL),"sha256":sha(PANEL),"rows":len(panel),"valid_rows":int(panel.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};manifest={**source_core,"manifest_hash":chash(source_core)};immutable(MANIFEST,json_bytes(manifest))
