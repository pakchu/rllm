"""Sequential strict economics for frozen HVTCR-24."""
...
PREREG = Path("results/high_volatility_trough_candle_range_relay_preregistration_2026-08-09.json")
PREREG_SHA = "0da132979e20f8042aca49a19682c22eb111bd6a898c5de5ff0d1dcc3a6162f0"
SUPPORT = Path("results/high_volatility_trough_candle_range_relay_support_2026-08-09.json")
SUPPORT_SHA = "f3721e2c73201308e0bd9e1d17c5ffb8898a27d3e79dea593f32f6e3a253069f"
NOVELTY = Path("results/high_volatility_trough_candle_range_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "5d181d73f7884b21d975e912dee2b1eadf0a86000c1303b6cc96b8376d0e0fab"
CLOCK = Path("data/high_volatility_trough_candle_range_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "446ad50c16a2e22d0317541a59fec9f7e72303c0c9ebd6768a5a62e3399cb0a1"
CONTROL_DIR = Path("data/high_volatility_trough_candle_range_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_trough_candle_range_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_trough_candle_range_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "first_trough_tie_break", "current_candle_range", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvtcr_24_sequential_economics_v1", "policy_id": POLICY_ID,
