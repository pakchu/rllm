"""Deterministic outcome-blind source support for HVCORR-24."""
...
from training import preregister_high_volatility_causal_oi_range_regime_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "ed1d3e65ef4fefaf82b1bc36beb4deffc158b2d5faf4db7bba70007f270beb28"
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVCORR-24 OI schema drift")
    expected_bars = [
        "decision_time", "first_open", "last_close", "cycle_high", "cycle_low", "minute_squared_return",
        "source_rows", "distinct_rows", "first_ts", "last_ts", "coherent",
    ]
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCORR-24 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all():
        raise RuntimeError("HVCORR-24 invalid OI rows")

    bars = bars.copy()
    for column in ("decision_time", "first_ts", "last_ts"):
        bars[column] = pd.to_datetime(bars[column], utc=True, errors="raise")
    numeric = ("first_open", "last_close", "cycle_high", "cycle_low", "minute_squared_return", "source_rows", "distinct_rows")
    for column in numeric:
        bars[column] = pd.to_numeric(bars[column], errors="raise")
    if bars["decision_time"].duplicated().any():
        raise RuntimeError("HVCORR-24 duplicate eight-hour bar")
    ...
    periods = POLICY["prior_range_cycles"]
    bars["prior_range_high"] = bars["cycle_high"].shift(1).rolling(periods, min_periods=periods).max()
    bars["prior_range_low"] = bars["cycle_low"].shift(1).rolling(periods, min_periods=periods).min()
    bars["prior_range_midpoint"] = (bars["prior_range_high"] + bars["prior_range_low"]) / 2
    bars["previous_close"] = bars["last_close"].shift(1)
    bars["prior_range_valid"] = (
        bars["bar_valid"].shift(1).rolling(periods, min_periods=periods).sum().eq(periods)
        & bars["decision_time"].shift(periods).eq(
            bars["decision_time"] - pd.Timedelta(hours=8 * periods)
        )
    )
    bars["regime_cross_side"] = np.select(
        [
            bars["previous_close"].le(bars["prior_range_midpoint"])
            & bars["last_close"].gt(bars["prior_range_midpoint"]),
            bars["previous_close"].ge(bars["prior_range_midpoint"])
            & bars["last_close"].lt(bars["prior_range_midpoint"]),
        ],
        [1, -1],
        default=0,
    )
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel = panel.join(
        bars[["bar_valid", "prior_range_valid", "previous_close", "last_close", "prior_range_midpoint", "regime_cross_side", "realized_variation"]],
        on="decision_time",
    )
    panel["source_valid"] = (
        ...
        & panel["prior_range_valid"].eq(True)
        & np.isfinite(
            panel[["sum_open_interest", "previous_open_interest", "oi_change", "realized_variation", "previous_close", "last_close", "prior_range_midpoint"]]
        ).all(axis=1)
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if split is None:
            continue
        side = int(row.regime_cross_side)
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCORR-24 side or availability drift")
        ...
        rows.append({
            ...
            "prior_range_midpoint": float(row.prior_range_midpoint),
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCORR-24 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcorr_24_sources_v1",
    ...
    core = {
        "protocol_version": "hvcorr_24_source_support_v1", "policy_id": prereg.POLICY_ID,
