"""Strict staged economics for the singleton HVCVCOIR-12 policy."""
...
PREREG = Path("results/high_volatility_variance_concentration_open_interest_release_preregistration_2026-08-16.json")
PREREG_SHA = "867fb4fd8b702477f9a492f2839e0d7c3d1f810aa70dda4b31ee07119e209cff"
SUPPORT = Path("results/high_volatility_variance_concentration_open_interest_release_support_2026-08-16.json")
SUPPORT_SHA = "d68b9e9b3e46ee4084076767196c39e69a61d46a1e2e14bbcd22f3bc36be2a06"
GROSS9 = Path("results/high_volatility_variance_concentration_open_interest_release_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "eb3a89cdf700702c3d8e5b7592fb0928a4ae357fb3c087f1491c6fccdcdc7771"
CLOCK = {
    "path": Path("data/high_volatility_variance_concentration_open_interest_release_clocks_2023_2026.csv.gz"),
    "sha256": "0951df1d4f5af2be52122b1cba429bc1659c93e8a91c1d278b757b64dc9ec1c8",
    "rows": 251,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_variance_concentration_open_interest_release_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCVCOIR-12 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCVCOIR-12 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCVCOIR-12 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCVCOIR-12 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVCOIR-12 preregistered economics drift")
    ...
    if (
        ...
        or support.get("postentry_return_pnl_execution_price_opened") is not False
        or support.get("gross9_rows_opened") is not False
    ):
        raise RuntimeError("HVCVCOIR-12 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCVCOIR-12 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCVCOIR-12 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCVCOIR-12 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCVCOIR-12 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCVCOIR-12 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcvcoir_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
