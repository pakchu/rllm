"""Build source-only HVCQTR-8 clocks before Gross9 or economics."""
...
PREREG_SHA = "e1812f1522342ec502e589dca2666ffc05e07c68f59e400b8f90e7b359a744b4"
...
STATE = Path("data/high_volatility_cross_quote_ticket_rotation_relay_sources_2023_2026/block_states.csv.gz")
CLOCK = Path("data/high_volatility_cross_quote_ticket_rotation_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_cross_quote_ticket_rotation_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_cross_quote_ticket_rotation_relay_support_2026-08-09.json")
...
CONTROLS = ("no_variation_gate", "no_ticket_rotation", "usdt_ticket_expansion", "one_block_stale_ticket_rotation", "direction_flip", "same_clock_forced_long")
COLUMNS = ("candidate", "control", "split", "decision_time", "feature_available_time", "entry_time", "exit_time", "side", "ticket_change_usdt", "ticket_change_usdc", "ticket_change_fdusd", "flow_usdc", "flow_fdusd", "btc_variation", "variation_rank")
...
def block_panel(flow: pd.DataFrame) -> pd.DataFrame:
    ...
    if x[["date", "symbol"]].duplicated().any():
        raise RuntimeError("HVCQTR duplicate quote hour")
    ...
    grouped["valid"] = (
        ...
        & grouped.complete
        & np.isfinite(grouped[["volume", "trade_count", "flow"]]).all(axis=1)
        & grouped.volume.gt(0)
        & grouped.trade_count.ge(80)
        & grouped.trade_count.eq(np.floor(grouped.trade_count))
    )
    grouped["ticket"] = (grouped.volume / grouped.trade_count).where(grouped.valid)
    wide = grouped.pivot(index="block_start", columns="symbol", values=["valid", "ticket", "flow"])
...
def score_states(flow: pd.DataFrame, market: pd.DataFrame) -> pd.DataFrame:
    ...
    states["flow_consensus"] = states.pair_valid & usdc.ne(0) & fdusd.ne(0) & np.sign(usdc).eq(np.sign(fdusd))
    states["ticket_rotation"] = states.ticket_change_BTCUSDC.gt(0) & states.ticket_change_BTCFDUSD.gt(0) & states.ticket_change_BTCUSDT.lt(0)
    states["all_ticket_expand"] = states.ticket_change_BTCUSDC.gt(0) & states.ticket_change_BTCFDUSD.gt(0) & states.ticket_change_BTCUSDT.gt(0)
    return states
...
def build_clock(states: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    rotation = states.ticket_rotation
    if control == "one_block_stale_ticket_rotation":
        rotation = states.ticket_rotation.shift(1).fillna(False) & states.decision_time.diff().eq(pd.Timedelta(hours=8))
    if control == "no_ticket_rotation":
        active = states.flow_consensus & high_variation
    elif control == "usdt_ticket_expansion":
        active = states.flow_consensus & states.all_ticket_expand & high_variation
    else:
        active = states.flow_consensus & rotation & (True if control == "no_variation_gate" else high_variation)
    ...
    for i in states.index[active]:
        ...
        if split is None:
            continue
        side = int(np.sign(float(states.at[i, "flow_BTCUSDC"])))
        ...
        rows.append({
            ...
            "ticket_change_usdt": float(states.at[i, "ticket_change_BTCUSDT"]),
            "ticket_change_usdc": float(states.at[i, "ticket_change_BTCUSDC"]),
            "ticket_change_fdusd": float(states.at[i, "ticket_change_BTCFDUSD"]),
            "flow_usdc": float(states.at[i, "flow_BTCUSDC"]), "flow_fdusd": float(states.at[i, "flow_BTCFDUSD"]),
...
def run() -> dict[str, Any]:
    ...
    for path, expected in bindings.items():
        if sha(path) != expected:
            raise RuntimeError(f"HVCQTR binding drift: {path}")
    ...
    core = {
        "protocol_version": "hvcqtr_8_source_support_v1", "policy_id": prereg.POLICY_ID,
