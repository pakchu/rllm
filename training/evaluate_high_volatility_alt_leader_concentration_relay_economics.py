"""Sequential strict economics for frozen HVALCR-8."""
...
PREREG = Path("results/high_volatility_alt_leader_concentration_relay_preregistration_2026-08-13.json")
PREREG_SHA = "bd60bbd475461960802ce636718e78a2e5020529cdbb6f01f5ce49202a70f639"
SUPPORT = Path("results/high_volatility_alt_leader_concentration_relay_support_2026-08-13.json")
SUPPORT_SHA = "40c7d80e1968644c2e986f615def3252c181b8d98538bc3fcc3081e2bb606c52"
NOVELTY = Path("results/high_volatility_alt_leader_concentration_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "1f5f91568a0d23cea5e9289b5938ecbf789dba54666326f65d9fc97c450a3426"
CLOCK = Path("data/high_volatility_alt_leader_concentration_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "fe48cf0ef06393486005106e5b3dd870f4aa69f66d21a6e6b2cd66cd609d32a5"
CONTROL_DIR = Path("data/high_volatility_alt_leader_concentration_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_alt_leader_concentration_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_alt_leader_concentration_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_concentration_tail_gate", "no_variation_gate", "equal_weight_alt_direction", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVALCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVALCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvalcr_8_sequential_economics_v1", "policy_id": POLICY_ID,
