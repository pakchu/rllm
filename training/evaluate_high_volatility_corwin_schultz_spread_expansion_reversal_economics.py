"""Sequential strict economics for frozen HVCSER-12."""
...
PREREG = Path("results/high_volatility_corwin_schultz_spread_expansion_reversal_preregistration_2026-08-10.json")
PREREG_SHA = "172bb81c4adc31ca9701d646144b803cd54a8ef7fba60e20ca4b81f25c1f6115"
SUPPORT = Path("results/high_volatility_corwin_schultz_spread_expansion_reversal_support_2026-08-10.json")
SUPPORT_SHA = "2fea85d7e18222101465d617ffc49de192ecf548081a9d33ed8ac06e597ea3a6"
NOVELTY = Path("results/high_volatility_corwin_schultz_spread_expansion_reversal_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "fa6044518040512395fb43f691a48ac340593eef35b9d4e2ede5068cd62c00cf"
CLOCK = Path("data/high_volatility_corwin_schultz_spread_expansion_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "b46192cbadbb378c18ad2c91496c2ff0d550988e57140630c299976b26719d7f"
CONTROL_DIR = Path("data/high_volatility_corwin_schultz_spread_expansion_reversal_controls_2023_2026")
FREEZE = Path("results/high_volatility_corwin_schultz_spread_expansion_reversal_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_corwin_schultz_spread_expansion_reversal_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_spread_expansion_gate", "no_volatility_gate", "raw_positive_expansion", "one_day_stale_expansion", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCSER frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCSER novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcser_12_sequential_economics_v1", "policy_id": POLICY_ID,
