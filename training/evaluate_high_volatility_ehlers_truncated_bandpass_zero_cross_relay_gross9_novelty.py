"""Evaluate frozen HVTBP-24 structural novelty against every Gross9 sleeve."""
...
from training import preregister_high_volatility_ehlers_truncated_bandpass_zero_cross_relay as registration

POLICY = "HVTBP-24"
PROTOCOL = "hvtbp_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_ehlers_truncated_bandpass_zero_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTBP predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVTBP novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTBP predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTBP primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvedr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVTBP-24 unchanged before economic outcomes",
    }
