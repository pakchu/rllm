"""Fast evaluator for event-action pairwise A/B ranking rows."""
...
class FastEvalPairwiseRankConfig:
    input_jsonl: str
    output: str
...
def _target(row: dict[str, Any]) -> str:
    val = str(row.get("target", "A")).strip().upper()
    return val if val in LABELS else "A"
        ...
        out.append(
            {
                ...
                "target": _target(row),
                "prediction": str(pred).strip().upper(),
...
def _score_model(rows: list[dict[str, Any]], cfg: FastEvalPairwiseRankConfig) -> tuple[list[str], list[float], list[dict[str, Any]]]:
    ...
    for offset in range(0, len(rows), bs):
        ...
        for row, scores in zip(batch, batch_scores):
            a = float(scores["A"][normalize])
            b = float(scores["B"][normalize])
            pred = "A" if a >= b else "B"
            margin = a - b
            preds.append(pred)
            margins.append(margin)
            score_rows.append({"date": row.get("date"), "signal_pos": row.get("signal_pos"), "target": _target(row), "score": scores, "margin_a_minus_b": margin})
        done = len(preds)
...
def run(cfg: FastEvalPairwiseRankConfig) -> dict[str, Any]:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Evaluate pairwise A/B rank rows")
    p.add_argument("--input-jsonl", required=True)
    p.add_argument("--output", required=True)
    p.add_argument("--predictions-output", default="")
    p.add_argument("--scores-output", default="")
    p.add_argument("--model-name", default=FastEvalPairwiseRankConfig.model_name)
...
def main() -> None:
    print(json.dumps(run(FastEvalPairwiseRankConfig(**vars(parse_args()))), indent=2, ensure_ascii=False))
