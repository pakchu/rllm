"""Evaluate frozen GOICR-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "GOICR-12"
PROTOCOL = "goicr_12_gross9_novelty_v1"
...
OUTPUT = Path("results/gross_oi_churn_release_relay_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("GOICR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("GOICR novelty limits drift")
    if (
        ...
        raise RuntimeError("GOICR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="GOICR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "goicr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject GOICR-12 unchanged before economic outcomes",
    }
