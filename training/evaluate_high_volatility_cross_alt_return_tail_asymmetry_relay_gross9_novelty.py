"""Evaluate frozen HVCARTAR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCARTAR-8"
PROTOCOL = "hvcartar_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_cross_alt_return_tail_asymmetry_relay_preregistration_2026-08-11.json")
PREREG_SHA = "6497a661d784e1caede0f1f81bb70f7531bcab1ea6df0e4c5b9af3117d774268"
SUPPORT = Path("results/high_volatility_cross_alt_return_tail_asymmetry_relay_support_2026-08-11.json")
SUPPORT_SHA = "40b1687b24d65fa76669c8554340b0b8f04434a35ed59dff45578557a0efec97"
CLOCK = Path("data/high_volatility_cross_alt_return_tail_asymmetry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "acdccb1adf6a915cb9750009e37d329f929ba8ff697847bdb6aeb2edcdd1ff97"
OUTPUT = Path("results/high_volatility_cross_alt_return_tail_asymmetry_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCARTAR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCARTAR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCARTAR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCARTAR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcartar_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCARTAR-8 unchanged before economic outcomes",
    }
