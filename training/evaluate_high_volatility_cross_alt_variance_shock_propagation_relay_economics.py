"""Sequential strict economics for frozen HVCAVS-8."""
...
PREREG = Path("results/high_volatility_cross_alt_variance_shock_propagation_relay_preregistration_2026-08-13.json")
PREREG_SHA = "dbc0eadc608cd8008a5ab029cface8474d868631a3ce1f37c3f3f5ce3856f560"
SUPPORT = Path("results/high_volatility_cross_alt_variance_shock_propagation_relay_support_2026-08-13.json")
SUPPORT_SHA = "6bc6b9cf0006d1d7d8495137f91a483749be447cf4d9420ca139695191439a1b"
NOVELTY = Path("results/high_volatility_cross_alt_variance_shock_propagation_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "ec4dc971bee5206c87a60754037fdfc48607d94ea1f5acbe8603e7b357c5a5a3"
CLOCK = Path("data/high_volatility_cross_alt_variance_shock_propagation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "bf6bc89e12904dd1e764ab35feebc2241611b1cf0f9c58489d595940429953de"
CONTROL_DIR = Path("data/high_volatility_cross_alt_variance_shock_propagation_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_cross_alt_variance_shock_propagation_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_alt_variance_shock_propagation_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "no_shock_tail", "three_alt_consensus", "absolute_return_shock", "one_decision_stale_shock", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCAVS frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCAVS novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcavs_8_sequential_economics_v1", "policy_id": POLICY_ID,
