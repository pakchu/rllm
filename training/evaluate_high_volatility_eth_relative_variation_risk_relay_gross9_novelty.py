"""Evaluate frozen HVERVR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVERVR-8"
PROTOCOL = "hvervr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_eth_relative_variation_risk_relay_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVERVR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVERVR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVERVR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVERVR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvervr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVERVR-8 unchanged before economic outcomes",
    }
