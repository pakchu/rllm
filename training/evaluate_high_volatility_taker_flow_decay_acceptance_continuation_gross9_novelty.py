"""Evaluate the frozen HVTFDAC-8 action-vote clock against Gross9."""
...
POLICY = "HVTFDAC-8"
PROTOCOL = "hvtfdac_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_taker_flow_decay_acceptance_continuation_clocks_2023_2026.csv.gz",
    "sha256": "82ecf1de318b3944481a05f09a8a90c61ab3aa0e76e8188b27073d3086888267",
    "rows": 196,
}
OUTPUT = Path("results/high_volatility_taker_flow_decay_acceptance_continuation_gross9_novelty_2026-08-16.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("HVTFDAC-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVTFDAC-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVTFDAC-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVTFDAC-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVTFDAC-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVTFDAC-8 unchanged before economics",
    }
