"""Evaluate frozen HVLZC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVLZC-8"
PROTOCOL = "hvlzc_8_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_lempel_ziv_compressibility_relay_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVLZC predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVLZC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVLZC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVLZC primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvlzc_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVLZC-8 unchanged before economic outcomes"
        ),
    }
