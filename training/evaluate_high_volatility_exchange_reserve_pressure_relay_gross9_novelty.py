"""Evaluate frozen HVEXRP-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVEXRP-24"
PROTOCOL = "hvexrp_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_exchange_reserve_pressure_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVEXRP predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVEXRP novelty limits drift")
    if (
        ...
        raise RuntimeError("HVEXRP predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVEXRP primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvexrp_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVEXRP-24 unchanged before economic outcomes",
    }
