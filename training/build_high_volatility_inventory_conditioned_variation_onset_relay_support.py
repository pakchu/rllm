"""Outcome-blind source-support gate for frozen HVICVO-12."""
...
from training import (
    preregister_high_volatility_inventory_conditioned_variation_onset_relay as prereg,
)
from training.build_binance_aggtrade_microstructure import _write_gzip_csv


ENV_FILE = "/home/pakchu/rllm/.env"
PREREG_SHA = "b3fc8566af0697c62fe32c81df73fe2e98b34a569b1ad21c0206733665d9506b"
...
"""
SOURCE_DIR = Path(
    "data/high_volatility_inventory_conditioned_variation_onset_relay_sources_2023_2026"
)
FEATURES = SOURCE_DIR / "eight_hour_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path(
    "data/high_volatility_inventory_conditioned_variation_onset_relay_clocks_2023_2026.csv.gz"
)
CONTROL_DIR = Path(
    "data/high_volatility_inventory_conditioned_variation_onset_relay_controls_2023_2026"
)
RESULT = Path(
    "results/high_volatility_inventory_conditioned_variation_onset_relay_support_2026-08-13.json"
)
FEATURE_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "completed_return",
    "realized_variation", "oi_change", "variation_rank",
)
CLOCK_COLUMNS = (
...
def build_features(bars: pd.DataFrame, oi: pd.DataFrame) -> pd.DataFrame:
    ...
    for decision in pd.date_range(START + pd.Timedelta(hours=10), END, freq="8h", inclusive="left"):
        ...
        completed_return = variation = oi_change = math.nan
        if valid:
            ...
            valid = bool(completed_return != 0 and variation > 0 and oi_change != 0 and np.isfinite([completed_return,variation,oi_change]).all())
        if not valid: completed_return = variation = oi_change = math.nan
        rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"completed_return":completed_return,"realized_variation":variation,"oi_change":oi_change})
...
def conditions(frame: pd.DataFrame, control: str = "primary") -> tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    high = valid & used.variation_rank.ge(0.65)
    active = high if control == "no_onset_crossing" else high & used.variation_rank.shift(1).lt(0.65) & valid.shift(1, fill_value=False)
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVICVO preregistration drift")
    ...
    source_core = {
        "protocol_version": "hvicvo_12_sources_v1",
        "query_sha256": {
    ...
    core = {
        "protocol_version": "hvicvo_12_source_support_v1",
