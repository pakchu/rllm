"""Build outcome-blind source support for frozen HVKAMA-24."""
...
PREREG_SHA = "af9593c517425d013e51e767d8f144d149b9717be481c88a5f27b66984c6a142"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_kaufman_adaptive_average_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_kaufman_adaptive_average_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_kaufman_adaptive_average_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_kaufman_adaptive_average_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_kaufman_adaptive_average_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "efficiency_ratio", "smoothing_constant", "kama",
    "difference", "slow_ema", "slow_difference", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "efficiency_ratio", "smoothing_constant", "kama",
    "difference", "slow_ema", "slow_difference", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVKAMA source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVKAMA invalid source key")
...
def adaptive_average(close: pd.Series, valid: pd.Series) -> pd.DataFrame:
    period=P["efficiency_periods"]
    fast=2.0/(P["fast_periods"]+1.0);slow=2.0/(P["slow_periods"]+1.0)
    values=close.to_numpy(float);validity=valid.to_numpy(bool)
    er=np.full(len(close),np.nan);sc=np.full(len(close),np.nan);kama=np.full(len(close),np.nan)
    state=np.nan;run=0
    for index,value in enumerate(values):
        ...
        changes=np.abs(np.diff(values[index-period:index+1]))
        denominator=float(np.sum(changes))
        if not np.isfinite(changes).all() or denominator<=0:
            state=np.nan;continue
        efficiency=abs(value-values[index-period])/denominator
        smoothing=(efficiency*(fast-slow)+slow)**2
        er[index]=efficiency;sc[index]=smoothing
        if not math.isfinite(state):state=value
        else:state=state+smoothing*(value-state)
        kama[index]=state
    return pd.DataFrame({"efficiency_ratio":er,"smoothing_constant":sc,"kama":kama},index=close.index)


def slow_ema(close: pd.Series, valid: pd.Series) -> pd.Series:
    alpha=2.0/(P["slow_periods"]+1.0);output=np.full(len(close),np.nan);state=np.nan;run=0
    for index,(value,is_valid) in enumerate(zip(close.to_numpy(float),valid.to_numpy(bool))):
        ...
        if run>=P["slow_periods"]:output[index]=state
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars=bars.join(adaptive_average(bars.bar_close,bars.valid_bar));bars["difference"]=bars.bar_close-bars.kama
    bars["slow_ema"]=slow_ema(bars.bar_close,bars.valid_bar);bars["slow_difference"]=bars.bar_close-bars.slow_ema
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum())
    bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["efficiency_ratio","smoothing_constant","kama","difference","slow_ema","slow_difference","variation"]]).all(axis=1)&bars.difference.ne(0)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="fixed_slow_ema_crossover":
        signal=pd.to_numeric(used.slow_difference,errors="coerce");previous=signal.shift(1);state=valid&signal.ne(0)&previous.ne(0)&np.sign(signal).ne(np.sign(previous))&variation;side=np.sign(signal).fillna(0).astype(int)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVKAMA prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvkama_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvkama_24_source_support_v1", "policy_id": prereg.POLICY_ID,
