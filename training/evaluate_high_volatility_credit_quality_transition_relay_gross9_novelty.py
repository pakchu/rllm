"""Evaluate the frozen HVCQTR-24 action-vote clock against Gross9."""
...
POLICY = "HVCQTR-24"
PROTOCOL = "hvcqtr_24_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_credit_quality_transition_relay_clocks_2023_2026.csv.gz",
    "sha256": "7026c3e650f9587964b43ff51114c35bf032cf1667b877263ba4842a5aed663e",
    "rows": 199,
}
OUTPUT = Path("results/high_volatility_credit_quality_transition_relay_gross9_novelty_2026-08-18.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCQTR-24 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCQTR-24 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCQTR-24 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCQTR-24 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCQTR-24 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCQTR-24 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCQTR-24 unchanged before economics",
    }
