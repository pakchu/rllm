"""Strict staged economics for the singleton HVERDR-24 policy."""
...
PREREG = Path("results/high_volatility_equity_duration_rotation_relay_preregistration_2026-08-18.json")
PREREG_SHA = "71179ab28d5afe0188cde3059ba4a35bb802ad7a786cf7aee09ed59aaa520d33"
SUPPORT = Path("results/high_volatility_equity_duration_rotation_relay_support_2026-08-18.json")
SUPPORT_SHA = "62f5a49fd2769f107674d65f6eeb1c2fd1573dc5cb4f4379707c0e9f0d2cfc2c"
GROSS9 = Path("results/high_volatility_equity_duration_rotation_relay_gross9_novelty_2026-08-18.json")
GROSS9_SHA = "4ed01f241dde88d214281a1c6b1ce426267699666df8539ffb17a598fc9c8642"
CLOCK = {
    "path": Path("data/high_volatility_equity_duration_rotation_relay_clocks_2023_2026.csv.gz"),
    "sha256": "6e6766208faee2465d24becc413ef19542c77179472ac7b0e83f867963d4b178",
    "rows": 132,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_equity_duration_rotation_relay_{stage}_economics_2026-08-18.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVERDR-24 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVERDR-24 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVERDR-24 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVERDR-24 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVERDR-24 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVERDR-24 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVERDR-24 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVERDR-24 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVERDR-24 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVERDR-24 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVERDR-24 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hverdr_24_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
