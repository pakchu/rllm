"""Materialize outcome-blind source support for frozen ECVRS-12."""
...
BUILDER = Path("training/build_equity_commodity_volatility_residual_shock_relay_support.py")
PREREG_SHA = "c4ec96adc9bfe4bb762ddda13b005611c774720171de45126f973b43d24a7290"
SOURCE_DIR = Path("data/equity_commodity_volatility_residual_shock_relay_sources_2021_2026")
FEATURE_PANEL = SOURCE_DIR / "equity_commodity_volatility_residual_shock_relay_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/equity_commodity_volatility_residual_shock_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/equity_commodity_volatility_residual_shock_relay_controls_2023_2026")
RESULT = Path("results/equity_commodity_volatility_residual_shock_relay_support_2026-08-09.json")
...
CLOCK_COLUMNS = (
    ...
    "btc_variation_rank", "equity_commodity_residual", "absolute_residual_rank",
)
BTC_QUERY = """
...
def _validate_daily(frame: pd.DataFrame, value_column: str, label: str) -> pd.DataFrame:
    ...
    if result.empty or result.observation_date.duplicated().any():
        raise RuntimeError(f"ECVRS {label} dates invalid")
    values = result[value_column]
    if not np.isfinite(values).all() or not values.gt(0).all():
        raise RuntimeError(f"ECVRS {label} values invalid")
    return result
...
def combine_cboe_sources(
    ...
    if len(common) < 3:
        raise RuntimeError("ECVRS common Cboe intersection is insufficient")
...
def load_cboe() -> pd.DataFrame:
    ...
    for label, specification in sources.items():
        ...
        if sha(path) != specification["sha256"]:
            raise RuntimeError(f"ECVRS {label} source hash drift")
    vix = pd.read_csv(
...
def load_bars(start: pd.Timestamp, end: pd.Timestamp) -> pd.DataFrame:
    ...
    if frame.columns.tolist() != ["ts", "open", "close"]:
        raise RuntimeError("ECVRS BTC schema must be exactly ts,open,close")
    ...
    if len(frame) != len(expected) or not frame.ts.equals(pd.Series(expected, name="ts")):
        raise RuntimeError("ECVRS BTC source is not the exact requested 1m grid")
    for column in ("open", "close"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    prices = frame[["open", "close"]]
    if not np.isfinite(prices).all(axis=None) or not prices.gt(0).all(axis=None):
        raise RuntimeError("ECVRS BTC source contains invalid prices")
...
def build_features(
    ...
    for current in range(1, len(cboe) - 1):
        ...
        records.append(
            {
                "residual_source_date": pd.Timestamp(cboe.at[current, "observation_date"]),
    ...
    frame["equity_commodity_residual"] = 0.5 * (
        frame.vix_change + frame.vxn_change - frame.gvz_change - frame.ovx_change
    )
    frame["absolute_residual_rank"] = strict_prior_midrank(
        frame.equity_commodity_residual.abs(), lookback=rank_lookback, minimum=rank_minimum
    )
    return frame
...
def conditions(
    ...
    used = frame.shift(1) if control == "one_session_stale_residual" else frame
    residual = used.vix_change - used.gvz_change if control == "vix_minus_gvz" else used.equity_commodity_residual
    ...
    residual_gate = pd.Series(True, index=frame.index) if control == "no_residual_tail" else frame.absolute_residual_rank.ge(0.70)
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active]:
        ...
        rows.append(
            {
                "candidate": "ECVRS-12", "control": control, "split": split,
                ...
                **{column: float(source[column]) for column in CHANGE_COLUMNS},
                ...
                "equity_commodity_residual": float(source.equity_commodity_residual),
                "absolute_residual_rank": float(features.at[index, "absolute_residual_rank"]),
            }
        )
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("ECVRS preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "ecvrs_12_sources_v1",
    ...
    core = {
        "protocol_version": "ecvrs_12_source_support_v1", "policy_id": "ECVRS-12",
