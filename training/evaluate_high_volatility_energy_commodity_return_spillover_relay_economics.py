"""Sequential strict economics for frozen HVECSP-24."""
...
PREREG = Path("results/high_volatility_energy_commodity_return_spillover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "ac7b92ab691254c5efb649979a43584a0046c23d8ba4b0df27b20fa1dd8abed9"
SUPPORT = Path("results/high_volatility_energy_commodity_return_spillover_relay_support_2026-08-11.json")
SUPPORT_SHA = "1b7391ec80b3fe2e67e821af628ed508eafeadde9ebb7c9096bb004ae2534a78"
NOVELTY = Path("results/high_volatility_energy_commodity_return_spillover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "5c580763ccbc0f78b0f89ad7b6f265bd9174dd3c58e62be0bf0300ccde52414a"
CLOCK = Path("data/high_volatility_energy_commodity_return_spillover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "2909ef78c3dfd8f97408c3b50f5ee3c8ea962db6efd3afc9822502942eb68ac5"
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_energy_commodity_return_spillover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVECSP frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVECSP novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvecsp_24_sequential_economics_v1", "policy_id": POLICY_ID,
