"""Sequential strict economics for frozen HVWMR-72."""
...
NOVELTY = Path("results/high_volatility_weekly_momentum_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "0986e6526fc9d4d82ffe4ca35d62884dbc043bef0a3a0d551b502a7c52b2e01e"
...
FREEZE = Path("results/high_volatility_weekly_momentum_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_weekly_momentum_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVWMR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVWMR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvwmr_72_sequential_economics_v1", "policy_id": POLICY_ID,
