"""Evaluate frozen HVRVOL-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVRVOL-24"
PROTOCOL = "hvrvol_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_relative_volatility_index_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVRVOL predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVRVOL novelty limits drift")
    if (
        ...
        raise RuntimeError("HVRVOL predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVRVOL primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvrvol_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVRVOL-24 unchanged before economic outcomes",
    }
