"""Evaluate frozen HVFT-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVFT-24"
PROTOCOL = "hvft_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_fisher_transform_turn_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVFT predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVFT novelty limits drift")
    if (
        ...
        raise RuntimeError("HVFT predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVFT primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVFT-24 unchanged before economic outcomes",
    }
