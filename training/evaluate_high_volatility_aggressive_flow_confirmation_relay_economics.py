"""Sequential strict economics for frozen HVAFC-6."""
...
PREREG = Path("results/high_volatility_aggressive_flow_confirmation_relay_preregistration_2026-08-09.json")
PREREG_SHA = "49a1da1075a28e03a518032335d481112e62cd2eee0e042f169a58615c4a0276"
SUPPORT = Path("results/high_volatility_aggressive_flow_confirmation_relay_support_2026-08-08.json")
SUPPORT_SHA = "6b88ef1337fe626a404ab5fe227d926842ff8f591a135bbb2d5408e64ac64a22"
NOVELTY = Path("results/high_volatility_aggressive_flow_confirmation_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "39ae6ab9da84cd37e42fbb792b3e8b6bb13048c2e9244abb1fec7c60679b128b"
CLOCK = Path("data/high_volatility_aggressive_flow_confirmation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d6f576924e1170ae03799c2ab1652bed3a69f593f60df1dc372111127b09e721"
CONTROL_DIR = Path("data/high_volatility_aggressive_flow_confirmation_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_aggressive_flow_confirmation_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_aggressive_flow_confirmation_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_volatility_gate", "no_flow_magnitude_gate", "one_boundary_stale_geometry", "direction_fade")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVAFC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVAFC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvafc_6_sequential_economics_v1", "policy_id": POLICY_ID,
