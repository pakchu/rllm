"""Materialize outcome-blind source support for frozen CAFAER-12."""
...
BUILDER = Path("training/build_cross_alt_flow_acceleration_exhaustion_reversal_support.py")
PREREG_SHA = "7918b8b251475b035aed2d5113669e9c045a4ed4068179b0232cc72a788ae1b7"
SOURCE_DIR = Path("data/cross_alt_flow_acceleration_exhaustion_reversal_sources_2023_2026")
FEATURE_PANEL = SOURCE_DIR / "cross_alt_flow_acceleration_exhaustion_reversal_preentry_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/cross_alt_flow_acceleration_exhaustion_reversal_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/cross_alt_flow_acceleration_exhaustion_reversal_controls_2023_2026")
RESULT = Path("results/cross_alt_flow_acceleration_exhaustion_reversal_support_2026-08-09.json")
...
CONTROLS = (
    "no_volatility_gate", "three_of_six_breadth", "btc_acceleration_disagreement",
    "one_day_stale_acceleration", "direction_flip",
)
...
def query() -> str:
    return """
SELECT date_trunc('day',ts) AS day_start,
       count(*) AS row_count,
...
def load_daily(symbol: str) -> pd.DataFrame:
    ...
    try:
        ...
        engine.dispose()
    expected_columns = ["day_start", "row_count", "distinct_ts", "min_ts", "max_ts", "minute_aligned", "ohlc_valid", "flow_valid", "flow_imbalance", "realized_variation"]
    if frame.columns.tolist() != expected_columns:
        raise RuntimeError(f"CAFAER {symbol} schema drift")
    for column in ("day_start", "min_ts", "max_ts"):
    ...
    exact = frame.row_count.eq(1440) & frame.distinct_ts.eq(1440) & frame.minute_aligned & frame.ohlc_valid & frame.flow_valid
    exact &= frame.min_ts.eq(frame.day_start) & frame.max_ts.eq(frame.day_start + pd.Timedelta(minutes=1439))
    exact &= np.isfinite(frame.flow_imbalance) & np.isfinite(frame.realized_variation)
    return frame.loc[exact, ["day_start", "flow_imbalance", "realized_variation"]].copy()


def build_features(daily: dict[str, pd.DataFrame], lookback: int = 270, minimum: int = 180) -> pd.DataFrame:
    frame = daily["BTCUSDT"].rename(columns={"flow_imbalance": "btcusdt_flow", "realized_variation": "btc_realized_variation"})
    for symbol in ALT_SYMBOLS:
        part = daily[symbol][["day_start", "flow_imbalance"]].rename(columns={"flow_imbalance": f"{symbol.lower()}_flow"})
        frame = frame.merge(part, on="day_start", how="inner", validate="one_to_one")
    flow_columns = ["btcusdt_flow", *[f"{symbol.lower()}_flow" for symbol in ALT_SYMBOLS]]
    consecutive = frame.day_start.diff().eq(pd.Timedelta(days=1))
    ...
    frame["session_date"] = frame.day_start.dt.date.astype(str)
    frame["decision_time"] = frame.day_start + pd.Timedelta(days=1)
    frame = frame.drop(columns=["day_start", *flow_columns])
...
def conditions(frame: pd.DataFrame, control: str = "primary") -> tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    used = frame.shift(1) if control == "one_day_stale_acceleration" else frame
    ...
    btc_relation = np.sign(used.btc_flow_acceleration).eq(-majority_sign) if control == "btc_acceleration_disagreement" else np.sign(used.btc_flow_acceleration).eq(majority_sign)
    volatility = pd.Series(True, index=frame.index) if control == "no_volatility_gate" else frame.variation_rank.ge(0.65)
    active = breadth & used.btc_flow_acceleration.ne(0) & btc_relation & volatility
...
def build_clock(features: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in features.index[active]:
        ...
        rows.append({
            "candidate": "CAFAER-12", "control": control, "split": split,
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("CAFAER preregistration hash drift")
    registration = json.loads(prereg.DEFAULT_OUTPUT.read_text())
    prereg.validate(registration)
    daily = {symbol: load_daily(symbol) for symbol in SYMBOLS}
    ...
    source_core = {
        "protocol_version": "cafaer_12_sources_v1",
    ...
    core = {
        "protocol_version": "cafaer_12_source_support_v1", "policy_id": "CAFAER-12",
