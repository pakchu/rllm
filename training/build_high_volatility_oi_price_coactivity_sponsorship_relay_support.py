"""Outcome-blind source-support gate for frozen HVOIPCSR-8."""
...
"""
SOURCE_DIR = Path(
    "data/high_volatility_oi_price_coactivity_sponsorship_relay_sources_2023_2026"
)
FEATURES = SOURCE_DIR / "three_hour_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path(
    "data/high_volatility_oi_price_coactivity_sponsorship_relay_clocks_2023_2026.csv.gz"
)
CONTROL_DIR = Path(
    "data/high_volatility_oi_price_coactivity_sponsorship_relay_controls_2023_2026"
)
RESULT = Path(
    "results/high_volatility_oi_price_coactivity_sponsorship_relay_support_2026-08-10.json"
)
FEATURE_COLUMNS = (
    "decision_time",
...
def strict_prior_midrank(
    values: pd.Series, lookback: int = 720, minimum: int = 504
) -> pd.Series:
...
def build_features(bars: pd.DataFrame, oi: pd.DataFrame) -> pd.DataFrame:
    ...
    if not required_bars.issubset(bars) or not required_oi.issubset(oi):
        raise ValueError("HVOIPCSR source schema drift")
    prices = bars.copy()
    inventory = oi.copy()
    ...
    decisions = pd.date_range(
        START + pd.Timedelta(hours=3), END, freq="3h", inclusive="left"
    )
    for decision in decisions:
        price_index = pd.date_range(
            decision - pd.Timedelta(hours=3), decision, freq="5min", inclusive="left"
        )
        oi_index = pd.date_range(
            decision - pd.Timedelta(hours=3), decision, freq="5min", inclusive="both"
        )
        ...
        completed_return = variation = gross_activity = coactivity = math.nan
        if valid:
            bar_returns = np.log(
            ...
            abs_oi = np.abs(oi_changes)
            abs_returns = np.abs(bar_returns)
            coactivity = (
                float(np.corrcoef(abs_oi, abs_returns)[0, 1])
                if float(np.std(abs_oi)) > 0 and float(np.std(abs_returns)) > 0
                else math.nan
            )
            ...
            valid = bool(
                ...
                and np.isfinite(
                    [completed_return, variation, gross_activity, coactivity]
                ).all()
            )
        if not valid:
            completed_return = variation = gross_activity = coactivity = math.nan
    ...
    frame["coactivity_rank"] = strict_prior_midrank(
        frame["coactivity"].where(frame["source_valid"])
    )
...
def conditions(
    ...
    coactivity_gate = (
        pd.Series(True, index=frame.index)
        if control == "no_coactivity_gate"
        else used["coactivity_rank"].ge(0.75)
    )
    variation_gate = (
        pd.Series(True, index=frame.index)
    ...
    eligible = valid & gross_gate & coactivity_gate & variation_gate
    onset = (
        eligible
...
def run() -> dict[str, Any]:
    ...
    source_core = {
        "protocol_version": "hvoipcsr_8_sources_v1",
        "query_sha256": {
    ...
    core = {
        "protocol_version": "hvoipcsr_8_source_support_v1",
