"""Sequential strict economics for frozen HVRVI-24."""
...
PREREG = Path("results/high_volatility_relative_vigor_index_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "b12bd7a3e0a7eb3080f3fd8504b6843e594234f41935e2c8bd85d83b4a1f2e9d"
SUPPORT = Path("results/high_volatility_relative_vigor_index_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "be5d2824c753b9e900db5f8e5a42e6a6f2eb7aecb23598f60c17ca148a2426bc"
NOVELTY = Path("results/high_volatility_relative_vigor_index_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "18352ecb9e21829ae8df45f881f8b4464b42df8298dd08d610e3ea83c0546d96"
CLOCK = Path("data/high_volatility_relative_vigor_index_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "2ad899bad6f0dfc2f003421010935766bcc5f256953bae0d2ad8953de9eca70d"
CONTROL_DIR = Path("data/high_volatility_relative_vigor_index_crossover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_relative_vigor_index_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_relative_vigor_index_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "raw_body_range_cross", "one_bar_stale_crossover", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVRVI frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVRVI novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrvi_24_sequential_economics_v1", "policy_id": POLICY_ID,
