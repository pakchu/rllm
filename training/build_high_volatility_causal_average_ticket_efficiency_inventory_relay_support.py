"""Deterministic outcome-blind source support for HVCATEIR-8."""
...
from training import preregister_high_volatility_causal_average_ticket_efficiency_inventory_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "0b4728920640fd8ef7a7eed1d72b42369e630b7cf2b01479a7fa3d8c9342f08c"
...
PANEL_COLUMNS = (
    ...
    "first_half_trade_count", "second_half_trade_count", "first_half_average_ticket", "second_half_average_ticket", "completed_return", "path_efficiency", "path_efficiency_rank",
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVCATEIR-8 OI schema drift")
    expected_bars = [
        "decision_time", "first_quote_turnover", "first_trade_count", "second_quote_turnover", "second_trade_count", "completed_return", "absolute_return_sum", "minute_squared_return",
        "source_rows", "distinct_rows", "first_ts", "last_ts", "coherent",
    ]
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCATEIR-8 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all():
        raise RuntimeError("HVCATEIR-8 invalid OI rows")

    bars = bars.copy()
    for column in ("decision_time", "first_ts", "last_ts"):
        bars[column] = pd.to_datetime(bars[column], utc=True, errors="raise")
    numeric = ("first_quote_turnover", "first_trade_count", "second_quote_turnover", "second_trade_count", "completed_return", "absolute_return_sum", "minute_squared_return", "source_rows", "distinct_rows")
    for column in numeric:
        bars[column] = pd.to_numeric(bars[column], errors="raise")
    if bars["decision_time"].duplicated().any():
        raise RuntimeError("HVCATEIR-8 duplicate eight-hour bar")
    ...
    bars["path_efficiency"] = np.abs(bars["completed_return"]) / bars["absolute_return_sum"]
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel = panel.join(
        bars[["bar_valid", "first_half_quote_turnover", "second_half_quote_turnover", "first_half_trade_count", "second_half_trade_count", "first_half_average_ticket", "second_half_average_ticket", "completed_return", "path_efficiency", "average_ticket_acceleration", "realized_variation"]],
        on="decision_time",
    )
    panel["source_valid"] = (
        ...
        & np.isfinite(
            panel[["sum_open_interest", "previous_open_interest", "oi_change", "first_half_quote_turnover", "second_half_quote_turnover", "first_half_trade_count", "second_half_trade_count", "first_half_average_ticket", "second_half_average_ticket", "completed_return", "path_efficiency", "average_ticket_acceleration", "realized_variation"]]
        ).all(axis=1)
    ...
    panel["path_efficiency_rank"] = causal_midrank(panel["path_efficiency"].where(valid))
    panel["eligible"] = (
        valid
        ...
        & panel["path_efficiency_rank"].ge(POLICY["path_efficiency_rank_min"])
    )
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCATEIR-8 side or availability drift")
        ...
        rows.append({
            ...
            "path_efficiency_rank": float(row.path_efficiency_rank),
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCATEIR-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvcateir_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvcateir_8_source_support_v1", "policy_id": prereg.POLICY_ID,
