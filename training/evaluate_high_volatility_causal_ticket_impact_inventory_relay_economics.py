"""Strict staged economics for the singleton HVCTIIR-8 policy."""
...
PREREG = Path("results/high_volatility_causal_ticket_impact_inventory_relay_preregistration_2026-08-16.json")
PREREG_SHA = "dd91b8a0b2400bcca4884017c2c566cf4f51f1fde94af09c603037c94432f02a"
SUPPORT = Path("results/high_volatility_causal_ticket_impact_inventory_relay_support_2026-08-16.json")
SUPPORT_SHA = "1723ba921c322a9b73bdd4bc55e8b5a6b6f05ac4b6909f1e2c8ee4550fc5e44d"
GROSS9 = Path("results/high_volatility_causal_ticket_impact_inventory_relay_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "0ec14a2e653bc0459ab1b1e83899f6d9f4de7e28d7ab165caaee6dee5cdd6b1f"
CLOCK = {
    "path": Path("data/high_volatility_causal_ticket_impact_inventory_relay_clocks_2020_2026.csv.gz"),
    "sha256": "152ec08a723345f517c3b099ceb81f03c93f389eec9d9a214437ce01f3a4ff58",
    "rows": 273,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_ticket_impact_inventory_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCTIIR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCTIIR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCTIIR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCTIIR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCTIIR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCTIIR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCTIIR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCTIIR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCTIIR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCTIIR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCTIIR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvctiir_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
