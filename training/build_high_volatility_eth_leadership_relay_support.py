"""Materialize source-only HVELR-6 clocks."""
...
SOURCE_DIR = Path("data/high_volatility_eth_leadership_relay_sources_2023_2026")
PANEL = SOURCE_DIR / "eight_hour_eth_leadership_panel.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path("data/high_volatility_eth_leadership_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_eth_leadership_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_eth_leadership_relay_support_2026-08-08.json")
SPLITS = {
...
COLUMNS = (
    ...
    "exit_time",
    "side",
    "btc_late_return","eth_late_return","eth_to_btc_absolute_return_ratio","absolute_eth_return_rank",
    "full_variation",
    "variation_rank",
)
BTC_QUERY = """
...
def _eth_leadership_panel(btc: pd.DataFrame, eth: pd.DataFrame) -> pd.DataFrame:
    btc = _prepare(btc)
    eth = _prepare(eth)
    ...
    for decision in decisions:
        ...
        if valid:
            ...
            ratio = abs(eth_late_return) / abs(btc_late_return) if btc_late_return != 0 else float("inf")
            full_variation = float(np.log(btc_window.close.astype(float)).diff().dropna().pow(2).sum())
            valid = full_variation > 0 and math.isfinite(ratio)
        else:
            btc_late_return = eth_late_return = ratio = full_variation = float("nan")
        rows.append(
            {
                ...
                "eth_to_btc_absolute_return_ratio": ratio,
                "full_variation": full_variation,
            }
        )
    panel = pd.DataFrame(rows)
    panel["variation_rank"] = strict_prior_midrank(panel.full_variation)
    panel["absolute_eth_return_rank"] = strict_prior_midrank(panel.eth_late_return.abs())
    return panel
...
def materialize() -> dict[str, Any]:
    ...
    database.dispose()
    panel = _eth_leadership_panel(btc, eth)
    SOURCE_DIR.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(panel, PANEL)
    core = {
        "protocol_version": "hvelr_6_binance_cross_asset_source_v1",
...
def conditions(frame: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    ratio = frame.eth_to_btc_absolute_return_ratio
    eth_rank = frame.absolute_eth_return_rank
    ...
    tail_gate = pd.Series(True, index=frame.index) if control == "no_eth_tail_gate" else eth_rank.ge(0.8)
    leadership_gate = pd.Series(True, index=frame.index) if control == "no_leadership_ratio_gate" else ratio.ge(1.5)
    long = btc_return.gt(0) & eth_return.gt(0)
    short = btc_return.lt(0) & eth_return.lt(0)
    active = (
        frame.signal_valid
        & np.isfinite(btc_return)&np.isfinite(eth_return)&np.isfinite(ratio)&np.isfinite(eth_rank)
        & np.isfinite(rank)
        & volatility_gate
        & tail_gate
        & leadership_gate
        ...
        rows.append(
            {
                ...
                "eth_to_btc_absolute_return_ratio": float(frame.at[index, "eth_to_btc_absolute_return_ratio"]),
                "absolute_eth_return_rank": float(frame.at[index, "absolute_eth_return_rank"]),
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvelr_6_source_support_v1",
        "policy_id": "HVELR-6",
        "preregistration": {
