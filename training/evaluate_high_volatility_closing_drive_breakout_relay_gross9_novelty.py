"""Evaluate frozen HVCDBR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCDBR-8"
PROTOCOL = "hvcdbr_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_closing_drive_breakout_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCDBR predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCDBR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCDBR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCDBR primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcdbr_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCDBR-8 unchanged before economic outcomes",
    }
