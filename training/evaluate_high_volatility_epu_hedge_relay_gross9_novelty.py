"""Evaluate frozen HVEPUH-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVEPUH-24"
PROTOCOL = "hvepuh_24_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_epu_hedge_relay_preregistration_2026-08-09.json"
)
...
OUTPUT = Path(
    "results/high_volatility_epu_hedge_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVEPUH predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVEPUH novelty limits drift")
    if (
        ...
        raise RuntimeError("HVEPUH predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVEPUH primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject HVEPUH-24 unchanged before economic outcomes"
        ),
    }
