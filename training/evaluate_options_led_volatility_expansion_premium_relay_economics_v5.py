"""Fresh V5 OVEPR economics with exact offset funding-event posting."""
...
FAILURE=Path('results/options_led_volatility_expansion_premium_relay_train_economic_attempt_terminal_failure_v4_2026-08-08.json');FAILURE_SHA='9c6a5d71212f00974930fcedbec176c60b7f9a70fc7a83fbc1d49be2bd8558d4'
...
OUTPUTS={s:Path(f'results/options_led_volatility_expansion_premium_relay_{s}_economics_v5_2026-08-08.json') for s in STAGES}
...
def verify(stage:str)->dict[str,Any]:
 ...
 if sha(FAILURE)!=FAILURE_SHA:raise RuntimeError('V4 terminal failure receipt drift')
 failure=load_json(FAILURE)
 if failure.get('economic_metrics_computed') is not False or failure.get('candidate_pass_fail_observed') is not False or failure.get('attempt_disposition')!='TERMINAL_FAILURE_NO_RETRY_UNDER_V4':raise RuntimeError('V4 failure is not eligible for infrastructure successor')
 if stage in PREDECESSOR:
  p=OUTPUTS[PREDECESSOR[stage]]
  if not p.is_file():raise RuntimeError(f'missing V5 predecessor: {p}')
  d=load_json(p);core={k:v for k,v in d.items() if k!='manifest_hash'}
  if d.get('manifest_hash')!=chash(core) or d.get('passed') is not True:raise RuntimeError('V5 predecessor did not pass')
 return n
...
def validate_funding(f:pd.DataFrame,start:pd.Timestamp,end:pd.Timestamp)->None:
 ...
 if len(f)>1 and f.date.diff().iloc[1:].max()>pd.Timedelta(hours=8,seconds=1):raise RuntimeError('funding gap exceeds eight hours')
...
def simulate(clock:pd.DataFrame,m:pd.DataFrame,f:pd.DataFrame,start:pd.Timestamp,end:pd.Timestamp,cost:float)->dict[str,Any]:
 ...
 for r in clock.itertuples(index=False):
  ...
  stage_funding=f[(f.date>=r.entry_time)&(f.date<r.exit_time)];fund_by_time={}
  for t,rate,mp in zip(stage_funding.date,stage_funding.funding_rate,stage_funding.mark_price):
   bucket=t.floor('5min');fund_by_time[bucket]=fund_by_time.get(bucket,0.0)+float(-quantity*mp*rate)
  for j in range(a,b):
