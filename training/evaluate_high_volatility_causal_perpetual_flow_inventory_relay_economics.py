"""Strict staged economics for the singleton HVCPOFR-8 policy."""
...
PREREG = Path("results/high_volatility_causal_perpetual_flow_inventory_relay_preregistration_2026-08-16.json")
PREREG_SHA = "7912fcdea95d7fd8568c1cfaa9f9ecc015c8267bb7b232e51f742d833647b87c"
SUPPORT = Path("results/high_volatility_causal_perpetual_flow_inventory_relay_support_2026-08-16.json")
SUPPORT_SHA = "67f1fde3168c5f6c1d743b61fe11f06a1b360c836d2802af8b10ea24e75f6427"
GROSS9 = Path("results/high_volatility_causal_perpetual_flow_inventory_relay_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "efbb3cc275d4281d583e1f5ecd742e6e53aad385fedf3572514cf97c41a86436"
CLOCK = {
    "path": Path("data/high_volatility_causal_perpetual_flow_inventory_relay_clocks_2020_2026.csv.gz"),
    "sha256": "da2c8d095217575546d9629186e427597c91ca564b14f466643b2f5f88c2a9fe",
    "rows": 209,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_perpetual_flow_inventory_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCPOFR-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCPOFR-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCPOFR-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCPOFR-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCPOFR-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCPOFR-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCPOFR-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCPOFR-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCPOFR-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCPOFR-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCPOFR-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcpofr_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
