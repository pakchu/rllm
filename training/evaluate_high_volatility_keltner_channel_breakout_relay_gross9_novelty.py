"""Evaluate frozen HVKC-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVKC-24"
PROTOCOL = "hvkc_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_keltner_channel_breakout_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVKC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVKC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVKC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVKC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVKC-24 unchanged before economic outcomes",
    }
