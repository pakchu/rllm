"""Sequential strict economics for frozen HVCARQ-6."""
...
PREREG = Path("results/high_volatility_cross_alt_realized_quarticity_concentration_spillover_relay_preregistration_2026-08-13.json")
PREREG_SHA = "9a26d30c718e6ad21d92bb4dd5498293b84664ae6a97bbc1d9be11b98571ffdc"
SUPPORT = Path("results/high_volatility_cross_alt_realized_quarticity_concentration_spillover_relay_support_2026-08-13.json")
SUPPORT_SHA = "614ea6da1ebf59e962df07dc8c908f1b30aad189e8f69c2986a66a215d282ea4"
NOVELTY = Path("results/high_volatility_cross_alt_realized_quarticity_concentration_spillover_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "99e6e3cde2368f3abe82e7c4c82dc7fbbf37104f47e29b1cb4ab73ba8f6d6617"
CLOCK = Path("data/high_volatility_cross_alt_realized_quarticity_concentration_spillover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "e8150832db89651cb77dec2e3995be1244b772f4fb20c296cddd70246c6810d5"
CONTROL_DIR = Path("data/high_volatility_cross_alt_realized_quarticity_concentration_spillover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_cross_alt_realized_quarticity_concentration_spillover_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_alt_realized_quarticity_concentration_spillover_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "no_quarticity_tail", "three_alt_consensus", "variance_weighted_score", "one_decision_stale_consensus", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCARQ frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCARQ novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcarq_6_sequential_economics_v1", "policy_id": POLICY_ID,
