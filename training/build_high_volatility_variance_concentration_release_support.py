"""Build source-only HVVCR-12 clocks before opening outcomes or Gross9."""
...
from training import preregister_high_volatility_variance_concentration_release as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv
from training.long_regime_combo_scan import LongComboScanConfig, _load_market


PREREG_SHA = "380ef6369d7d85b728473079591ea2cfa92b0fa1c4a38f0fd120a786b47932b5"
...
CLOCK = Path("data/high_volatility_variance_concentration_release_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_variance_concentration_release_controls_2023_2026")
SNAPSHOT = Path("data/high_volatility_variance_concentration_release_sources_2023_2026/variance_concentration.csv.gz")
RESULT = Path("results/high_volatility_variance_concentration_release_support_2026-08-09.json")
SPLITS = {
...
def load_combined_market() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if combined.date.duplicated().any() or not combined.date.diff().dropna().eq(pd.Timedelta(minutes=5)).all():
        raise RuntimeError("HVVCR combined market continuity drift")
    return combined, {
...
def score_snapshot(market: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, float]]:
    ...
    if len(calibration) < 10_000:
        raise RuntimeError("HVVCR source-only calibration floor failed")
    thresholds = {
    ...
    for index in range(len(frame)):
        ...
        if pending is not None:
            if now > pending["deadline"]:
                ...
                signals.append((now + pd.Timedelta(minutes=5), -pending["shock_side"]))
                pending = None
        if pending is None and bool(onset.iloc[index]):
            ...
            if control == "no_release_requirement":
                signals.append((now + pd.Timedelta(minutes=5), -shock_side))
            else:
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVVCR preregistration drift")
    ...
    if registration["manifest_hash"] != prereg.canonical_hash(core_registration):
        raise RuntimeError("HVVCR preregistration manifest drift")
    ...
    core = {
        "protocol_version": "hvvcr_12_source_support_v1",
        "policy_id": "HVVCR-12",
