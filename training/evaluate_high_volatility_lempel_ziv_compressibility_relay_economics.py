"""Sequential strict economics for frozen HVLZC-8."""
...
PREREG = Path("results/high_volatility_lempel_ziv_compressibility_relay_preregistration_2026-08-10.json")
PREREG_SHA = "1aff82bc58760b8dfc8c14798c20085dc8e9387caaec71f478564855e6afda8b"
SUPPORT = Path("results/high_volatility_lempel_ziv_compressibility_relay_support_2026-08-10.json")
SUPPORT_SHA = "d85fabbd757859f1c9fa38385d992b2a79bf6b9ebfd4ac32673f88e30d4bce3c"
NOVELTY = Path("results/high_volatility_lempel_ziv_compressibility_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "3ca73e95678052c3dc9677fa01b16e3576bc00f53cfef7ffb6dbaad40fed8e59"
CLOCK = Path("data/high_volatility_lempel_ziv_compressibility_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "7d32fd2a4bb3ec02bae29966deb554410f325b2c32b38cb15b459ad2ae8d7ea3"
CONTROL_DIR = Path("data/high_volatility_lempel_ziv_compressibility_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_lempel_ziv_compressibility_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_lempel_ziv_compressibility_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_complexity_tail", "no_variation_gate", "raw_below_unit_complexity", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVLZC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVLZC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvlzc_8_sequential_economics_v1", "policy_id": POLICY_ID,
