"""Sequential strict economics for frozen HVOILVI-12."""
...
PREREG = Path("results/high_volatility_oi_conditioned_late_variation_ignition_relay_preregistration_2026-08-13.json")
PREREG_SHA = "360e048d64b0228ffcedcb1ae7df71c6bd99e3b4adb0e86b0cda93e616aed6c4"
SUPPORT = Path("results/high_volatility_oi_conditioned_late_variation_ignition_relay_support_2026-08-13.json")
SUPPORT_SHA = "48c52efa0582373f88d44c0f9b57f818816e6a2edc8ccfe3270315d9a1702348"
NOVELTY = Path("results/high_volatility_oi_conditioned_late_variation_ignition_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "7775869b4ce76e84b8f01d9c557feeea4261953ed36a474770d7dd036caf7d12"
CLOCK = Path("data/high_volatility_oi_conditioned_late_variation_ignition_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "e4c36e8fffb4805dc397808a246f9cddf54d80cc40153f019e8d773fcdecc719"
CONTROL_DIR = Path("data/high_volatility_oi_conditioned_late_variation_ignition_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_oi_conditioned_late_variation_ignition_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_oi_conditioned_late_variation_ignition_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_overall_variation_gate", "no_late_share_gate", "no_fresh_ignition", "ignore_inventory_sign_follow_return", "one_block_stale_features", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVOILVI frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVOILVI novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvoilvi_12_sequential_economics_v1", "policy_id": POLICY_ID,
