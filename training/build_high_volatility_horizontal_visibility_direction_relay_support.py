"""Source-only support gate for frozen HVHVD-8."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_horizontal_visibility_direction_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2023-01-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="7a19bd2f228f3075b24a319be3dce843723936d3ed3b07a245a346df126f52b7";REG=prereg.build();P=REG["policy"];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG["stages"].items()};GATES=REG["source_support_gates"];CONTROLS=tuple(REG["diagnostic_controls"]["names"])
QUERY="""SELECT date_bin('5 minutes',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00') AS date,(array_agg(open ORDER BY ts))[1] AS open,max(high) AS high,min(low) AS low,(array_agg(close ORDER BY ts DESC))[1] AS close,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND high>=greatest(open,close,low) AND low<=least(open,close,high)) AS coherent FROM bars_binance WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end GROUP BY 1 ORDER BY 1"""
ROOT=Path("data/high_volatility_horizontal_visibility_direction_relay_sources_2023_2026");PANEL=ROOT/"scheduled_visibility_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_horizontal_visibility_direction_relay_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_horizontal_visibility_direction_relay_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_horizontal_visibility_direction_relay_controls_2023_2026");RESULT=Path("results/high_volatility_horizontal_visibility_direction_relay_support_2026-08-13.json");BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLS=("decision_time","feature_available_time","source_valid","visibility_direction","unweighted_direction","nonadjacent_edges","direction_strength_rank","unweighted_rank","realized_variation","variation_rank","block_return","side","eligible")
CLOCK_COLS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","visibility_direction","nonadjacent_edges","direction_strength_rank","realized_variation","variation_rank","block_return")
...
def visibility_direction(log_prices:np.ndarray)->tuple[float,float,int]:
 x=np.asarray(log_prices,float)
 if len(x)!=96 or not np.isfinite(x).all():return math.nan,math.nan,0
 weighted=unweighted=den_weight=0.;edges=0
 for i in range(len(x)-2):
  between=x[i+1]
  for j in range(i+2,len(x)):
   if between<min(x[i],x[j]):
    delta=x[j]-x[i]
    if delta==0:return math.nan,math.nan,0
    sign=float(np.sign(delta));lag=j-i;weighted+=lag*sign;den_weight+=lag;unweighted+=sign;edges+=1
   between=max(between,x[j])
 if edges<P["minimum_nonadjacent_edges"] or den_weight<=0:return math.nan,math.nan,edges
 return weighted/den_weight,unweighted/edges,edges
def postgres_engine():
...
def prepare(raw):
 expected=["date","open","high","low","close","source_rows","distinct_rows","first_ts","last_ts","coherent"]
 if raw.columns.tolist()!=expected:raise RuntimeError("HVHVD source schema drift")
 x=raw.copy()
 for c in ("date","first_ts","last_ts"):x[c]=pd.to_datetime(x[c],utc=True,errors="coerce")
 for c in expected[1:7]:x[c]=pd.to_numeric(x[c],errors="coerce")
 if x.date.isna().any() or x.date.duplicated().any():raise RuntimeError("HVHVD invalid source key")
 z=x[["open","high","low","close"]];x["valid"]=x.source_rows.eq(5)&x.distinct_rows.eq(5)&x.first_ts.eq(x.date)&x.last_ts.eq(x.date+pd.Timedelta("4m"))&x.coherent.eq(True)&np.isfinite(z).all(axis=1)&z.gt(0).all(axis=1)&x.high.ge(z[["open","close"]].max(axis=1))&x.low.le(z[["open","close"]].min(axis=1));x["return"]=np.log(x.close/x.open);return x.set_index("date").sort_index()
def metrics(x:pd.DataFrame,decision:pd.Timestamp)->dict[str,Any]:
 ...
 if not valid:return {"source_valid":False,"visibility_direction":np.nan,"unweighted_direction":np.nan,"nonadjacent_edges":0,"realized_variation":np.nan,"block_return":np.nan,"side":0}
 direction,unweighted,edges=visibility_direction(np.log(b.close.to_numpy(float)));returns=b["return"].to_numpy(float);rv=float(np.sqrt(np.square(returns).sum()));block=float(np.log(b.close.iloc[-1]/b.open.iloc[0]));valid=np.isfinite([direction,unweighted,rv,block]).all() and direction!=0 and rv>0 and block!=0;side=int(np.sign(direction)) if valid and np.sign(direction)==np.sign(block) else 0
 return {"source_valid":bool(valid),"visibility_direction":direction if valid else np.nan,"unweighted_direction":unweighted if valid else np.nan,"nonadjacent_edges":edges,"realized_variation":rv if valid else np.nan,"block_return":block if valid else np.nan,"side":side}
def build_panel(raw):
 x=prepare(raw);first=START.normalize()+pd.Timedelta("6h");rows=[{"decision_time":d,"feature_available_time":d,**metrics(x,d)} for d in pd.date_range(first,END,freq="8h",inclusive="left")];f=pd.DataFrame(rows);valid=f.source_valid.eq(True);f["direction_strength_rank"]=prior_rank(f.visibility_direction.abs().where(valid));f["unweighted_rank"]=prior_rank(f.unweighted_direction.abs().where(valid));f["variation_rank"]=prior_rank(f.realized_variation.where(valid));f["eligible"]=valid&f.direction_strength_rank.ge(P["direction_strength_rank_min"])&f.variation_rank.ge(P["variation_rank_min"])&f.side.ne(0);return f.loc[:,PANEL_COLS]
def active(panel,control="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 u=panel.copy()
 if control=="one_decision_stale_graph":
  for c in ("visibility_direction","direction_strength_rank","side"):u[c]=u[c].shift(1)
 strength=pd.Series(True,index=u.index) if control=="no_direction_tail" else (u.unweighted_rank.ge(P["direction_strength_rank_min"]) if control=="unweighted_visibility_edges" else u.direction_strength_rank.ge(P["direction_strength_rank_min"]))
...
def clock(panel,control="primary"):
 ...
 for i in panel.index[onset]:
  ...
  if split:reserved=exit_;rows.append({"candidate":"HVHVD-8","control":control,"split":split,"decision_time":d,"feature_available_time":d,"entry_time":entry,"exit_time":exit_,"side":int(side.at[i]),**{c:float(panel.at[i,c]) for c in CLOCK_COLS[8:]}})
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVHVD prereg drift")
