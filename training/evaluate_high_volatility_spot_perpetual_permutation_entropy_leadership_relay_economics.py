"""Sequential strict economics for frozen HVSPPE-12."""
...
PREREG = Path("results/high_volatility_spot_perpetual_permutation_entropy_leadership_relay_preregistration_2026-08-11.json")
PREREG_SHA = "034ddae98d3da48c39af1b4a33f834ceafe83331ed813bd2a7da5985f2b84834"
SUPPORT = Path("results/high_volatility_spot_perpetual_permutation_entropy_leadership_relay_support_2026-08-11.json")
SUPPORT_SHA = "c8e1729221ab953f366ee9b75c55293454daf5d78a56610d9f03a1ae6c3f4711"
NOVELTY = Path("results/high_volatility_spot_perpetual_permutation_entropy_leadership_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "c2b4b8af8519e53d8b9c586cc756ffdc8012b4b2e8fb12c37546a7f6406f6a2f"
CLOCK = Path("data/high_volatility_spot_perpetual_permutation_entropy_leadership_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "bf18772bb8fea7ca2a08e11f5bb5014013fe40122a3696d6b32c5a48b92748e0"
CONTROL_DIR = Path("data/high_volatility_spot_perpetual_permutation_entropy_leadership_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_spot_perpetual_permutation_entropy_leadership_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_spot_perpetual_permutation_entropy_leadership_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = (
    "no_entropy_leadership_gate",
    "no_variation_gate",
    "perpetual_more_ordered",
    "one_bar_stale_onset",
    "direction_flip",
    "forced_long",
)
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSPPE frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSPPE novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvsppe_12_sequential_economics_v1", "policy_id": POLICY_ID,
