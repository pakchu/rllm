"""Sequential strict economics for frozen TECC-12."""
...
PREREG = Path("results/ticket_expansion_concordance_continuation_preregistration_2026-08-09.json")
PREREG_SHA = "35b3912969893bf77886ffc687961a242c4234901232c25c991366a3df19d526"
SUPPORT = Path("results/ticket_expansion_concordance_continuation_support_2026-08-09.json")
SUPPORT_SHA = "912355c59cdde6c5e164afbf8e971b6b5ac1bf8d66b73a23f373d457f98b09fb"
NOVELTY = Path("results/ticket_expansion_concordance_continuation_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "3594049b25a742aef0919b2ff62a601e930ecfccd876600f88c95d25bc938c1f"
CLOCK = Path("data/ticket_expansion_concordance_continuation_clocks_2023_2026.csv.gz")
CLOCK_SHA = "c961e742a2d2fc97d0d05f1f3598b6e8efe0f76abb558f0dc4b1cfdbc7b051c0"
CONTROL_DIR = Path("data/ticket_expansion_concordance_continuation_controls_2023_2026")
FREEZE = Path("results/ticket_expansion_concordance_continuation_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/ticket_expansion_concordance_continuation_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_ticket_tail", "no_concordance", "early_ticket_dominance", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("TECC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("TECC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "tecc_12_sequential_economics_v1", "policy_id": POLICY_ID,
