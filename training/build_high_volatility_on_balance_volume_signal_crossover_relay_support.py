"""Build outcome-blind source support for frozen HVOBV-24."""
...
from training import preregister_high_volatility_on_balance_volume_signal_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "5ac86c269566756ded9bc40d7fc3464e0c324f145ba52f7e118d95291b35bf26"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_on_balance_volume_signal_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_on_balance_volume_signal_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_on_balance_volume_signal_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_on_balance_volume_signal_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_on_balance_volume_signal_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "bar_volume", "signed_volume", "obv", "signal", "difference", "signed_volume_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "bar_volume", "signed_volume", "obv", "signal", "difference", "signed_volume_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close", "volume"]:
        raise RuntimeError("HVOBV source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVOBV invalid source key")
...
def causal_obv(close: pd.Series, volume: pd.Series, valid: pd.Series) -> pd.DataFrame:
    signed=np.full(len(close),np.nan);obv=np.full(len(close),np.nan);state=np.nan;prior=np.nan
    for i,(price,amount,ok) in enumerate(zip(close.to_numpy(float),volume.to_numpy(float),valid.to_numpy(bool))):
        ...
        else:increment=amount if price>prior else -amount if price<prior else 0.0;state+=increment
        signed[i]=increment;obv[i]=state;prior=price
    return pd.DataFrame({"signed_volume":signed,"obv":obv},index=close.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    values=causal_obv(bars.bar_close,bars.bar_volume,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["signal"]=causal_ema(bars.obv,np.isfinite(bars.obv),P["signal_periods"]);bars["difference"]=bars.obv-bars.signal
    previous_signed=bars.signed_volume.shift(1);bars["signed_volume_side"]=np.where((bars.signed_volume>0)&(previous_signed<=0),1,np.where((bars.signed_volume<0)&(previous_signed>=0),-1,0))
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["bar_volume","signed_volume","obv","signal","difference","variation"]]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="signed_volume_zero_cross":
        side=pd.to_numeric(used.signed_volume_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVOBV prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvobv_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvobv_24_source_support_v1", "policy_id": prereg.POLICY_ID,
