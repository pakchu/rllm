"""Sequential strict economics for frozen HVPLOIEC-8."""
...
PREREG = Path("results/high_volatility_perpetual_lead_open_interest_expansion_continuation_preregistration_2026-08-16.json")
PREREG_SHA = "5d730fb41ba2cd0d48ba1a08a07e53a1816b0df686cadb631adf42b65a0fc658"
SUPPORT = Path("results/high_volatility_perpetual_lead_open_interest_expansion_continuation_support_2026-08-16.json")
SUPPORT_SHA = "9769754a505c76c830c041240c835548eebde4fce02f83cc91602c1ac242657a"
NOVELTY = Path("results/high_volatility_perpetual_lead_open_interest_expansion_continuation_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "6fbde084ba2bdd81e0cd104c74c6a0b8844eb080ee048ac4a38a76a2f86cf074"
CLOCK = Path("data/high_volatility_perpetual_lead_open_interest_expansion_continuation_clocks_2020_2026.csv.gz")
CLOCK_SHA = "2c905d399320bc3be7cc1fc4649e8be7d077d60668bb8bdc17dd9923455aad0f"
CONTROL_DIR = Path("data/high_volatility_perpetual_lead_open_interest_expansion_continuation_controls_2023_2026")
FREEZE = Path("results/high_volatility_perpetual_lead_open_interest_expansion_continuation_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_perpetual_lead_open_interest_expansion_continuation_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVPLOIEC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVPLOIEC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvploiec_8_sequential_economics_v1", "policy_id": POLICY_ID,
