"""Sequential strict economics for frozen HVSNDR-8."""
...
PREREG = Path("results/high_volatility_self_normalized_displacement_reversal_preregistration_2026-08-10.json")
PREREG_SHA = "5753b620f75a90d0de379b25880050acfd8bf9c945614f9615d04919bfcc9c7f"
SUPPORT = Path("results/high_volatility_self_normalized_displacement_reversal_support_2026-08-10.json")
SUPPORT_SHA = "77fdc7feee648b75b3f6aea58bf2eecfcf3a7933ecef5caea5e0d5e5dafb85aa"
NOVELTY = Path("results/high_volatility_self_normalized_displacement_reversal_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "5ebcef895bb41ebb77e95523089e2c0e3aa08fc8fff8228a231ca3f7f3298204"
CLOCK = Path("data/high_volatility_self_normalized_displacement_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "ba51e11fc949e9c04fa344ea9f882648fa0619735393a3f4e61189d6d4a8720d"
CONTROL_DIR = Path("data/high_volatility_self_normalized_displacement_reversal_controls_2023_2026")
FREEZE = Path("results/high_volatility_self_normalized_displacement_reversal_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_self_normalized_displacement_reversal_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_magnitude_tail_gate", "no_variation_gate", "l1_efficiency", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSNDR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSNDR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvsndr_8_sequential_economics_v1", "policy_id": POLICY_ID,
