"""Evaluate frozen HVDCSPSC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVDCSPSC-8"
PROTOCOL = "hvdcs_psc_8_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_directional_change_scarcity_premium_sponsorship_relay_gross9_novelty_2026-08-16.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVDCSPSC predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVDCSPSC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVDCSPSC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVDCSPSC primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvdcs_psc_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVDCSPSC-8 unchanged before economic outcomes"
        ),
    }
