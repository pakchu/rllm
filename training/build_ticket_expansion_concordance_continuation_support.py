...
def build_features(raw:pd.DataFrame)->pd.DataFrame:
 req=["ts","open","high","low","close","quote_asset_volume","number_of_trades"]
 ...
 f=f.sort_values("ts",kind="mergesort").set_index("ts");rows=[]
 for d in pd.date_range(START+pd.Timedelta(hours=8),END,freq="4h",inclusive="left"):
  w=f.loc[(f.index>=d-pd.Timedelta(hours=6))&(f.index<d)];expected=pd.date_range(d-pd.Timedelta(hours=6),d,freq="1min",inclusive="left");p=w[["open","high","low","close"]]
  ok=bool(len(w)==360 and not w.index.duplicated().any() and w.index.equals(expected) and np.isfinite(w[req[1:]]).all(axis=1).all() and p.gt(0).all(axis=1).all() and w.high.ge(w[["open","close"]].max(axis=1)).all() and w.low.le(w[["open","close"]].min(axis=1)).all() and w.high.ge(w.low).all() and w.quote_asset_volume.ge(0).all() and w.number_of_trades.ge(0).all() and w.number_of_trades.eq(np.floor(w.number_of_trades)).all())
  early=w.iloc[:240];late=w.iloc[240:];eq=float(early.quote_asset_volume.sum()) if ok else np.nan;lq=float(late.quote_asset_volume.sum()) if ok else np.nan;en=float(early.number_of_trades.sum()) if ok else np.nan;ln=float(late.number_of_trades.sum()) if ok else np.nan;ok=bool(ok and eq>0 and lq>0 and en>0 and ln>0)
  er=float(np.log(early.close.iloc[-1]/early.open.iloc[0])) if ok else np.nan;lr=float(np.log(late.close.iloc[-1]/late.open.iloc[0])) if ok else np.nan;et=eq/en if ok else np.nan;lt=lq/ln if ok else np.nan;exp=float(np.log(lt/et)) if ok else np.nan;con=bool(ok and er!=0 and lr!=0 and np.sign(er)==np.sign(lr))
