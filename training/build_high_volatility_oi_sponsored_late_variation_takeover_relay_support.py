"""Outcome-blind source-support gate for frozen HVOILVT-12."""
...
from training import (
    preregister_high_volatility_oi_sponsored_late_variation_takeover_relay as prereg,
)
from training.build_binance_aggtrade_microstructure import _write_gzip_csv


ENV_FILE = "/home/pakchu/rllm/.env"
PREREG_SHA = "8527368651c5fa5dc29b6f61f855258f794b7b3a4b34d4f07f7c1ba983845d72"
...
"""
SOURCE_DIR = Path(
    "data/high_volatility_oi_sponsored_late_variation_takeover_relay_sources_2023_2026"
)
FEATURES = SOURCE_DIR / "eight_hour_features.csv.gz"
SOURCE_MANIFEST = SOURCE_DIR / "manifest.json"
CLOCK = Path(
    "data/high_volatility_oi_sponsored_late_variation_takeover_relay_clocks_2023_2026.csv.gz"
)
CONTROL_DIR = Path(
    "data/high_volatility_oi_sponsored_late_variation_takeover_relay_controls_2023_2026"
)
RESULT = Path(
    "results/high_volatility_oi_sponsored_late_variation_takeover_relay_support_2026-08-13.json"
)
FEATURE_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "early_return", "late_return",
...
def build_features(bars: pd.DataFrame, oi: pd.DataFrame) -> pd.DataFrame:
    ...
    for decision in pd.date_range(START + pd.Timedelta(hours=10), END, freq="8h", inclusive="left"):
        ...
        early_return = late_return = variation = late_share = oi_change = math.nan
        if valid:
            returns = np.log(price.bar_close.to_numpy(float)/price.bar_open.to_numpy(float))
            early_return = float(np.log(price.bar_close.iloc[71]/price.bar_open.iloc[0]))
            late_return = float(np.log(price.bar_close.iloc[-1]/price.bar_open.iloc[72]))
            ...
            valid = bool(early_return != 0 and late_return != 0 and variation > 0 and oi_change != 0 and np.isfinite([early_return,late_return,variation,late_share,oi_change]).all())
        if not valid: early_return = late_return = variation = late_share = oi_change = math.nan
        rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"early_return":early_return,"late_return":late_return,"realized_variation":variation,"late_variance_share":late_share,"oi_change":oi_change})
...
def conditions(frame: pd.DataFrame, control: str = "primary") -> tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    valid = used.source_valid.eq(True) & used.early_return.ne(0) & used.late_return.ne(0) & used.oi_change.ne(0)
    ...
    takeover_gate = pd.Series(True,index=frame.index) if control == "no_directional_takeover" else np.sign(used.early_return)*np.sign(used.late_return)<0
    oi_gate = pd.Series(True,index=frame.index) if control == "no_oi_expansion" else used.oi_change.gt(0)
    eligible = valid & overall_gate & late_gate & takeover_gate & oi_gate
    ...
    side = np.sign(used.late_return).fillna(0).astype(int)
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVOILVT preregistration drift")
    ...
    source_core = {
        "protocol_version": "hvoilvt_12_sources_v1",
        "query_sha256": {
    ...
    core = {
        "protocol_version": "hvoilvt_12_source_support_v1",
