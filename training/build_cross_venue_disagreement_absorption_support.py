"""Build source-support clocks for CVDAR-6 without post-entry outcomes."""
...
CLOCK = Path("data/cross_venue_disagreement_absorption_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/cross_venue_disagreement_absorption_relay_controls_2023_2026")
...
CONTROLS = ("bvol_expanding_only", "dvol_expanding_only", "no_first_half_tail", "no_open_reclaim", "direction_flip")
...
def build_clock(frame: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    bvol, dvol = frame["bvol_body"], frame["dvol_body"]
    if control == "bvol_expanding_only":
        volatility = bvol.gt(0)
    elif control == "dvol_expanding_only":
        volatility = dvol.gt(0)
    else:
        volatility = bvol.ne(0) & dvol.ne(0) & np.sign(bvol).eq(-np.sign(dvol))
    ...
    active = frame["base_valid"] & volatility & shock & absorption
    onset = active & ~active.shift(1, fill_value=False) & frame["base_valid"].shift(1, fill_value=False) & frame["decision_time"].diff().eq(pd.Timedelta(hours=1))
    rows: list[dict[str, Any]] = []
    next_allowed: pd.Timestamp | None = None
    for index in frame.index[onset]:
        decision = pd.Timestamp(frame.at[index, "decision_time"])
        entry = decision + pd.Timedelta(minutes=5); exit_time = entry + pd.Timedelta(hours=6)
        ...
        rows.append({
            "candidate": "CVDAR-6", "control": control, "split": split,
...
def run() -> dict[str, Any]:
    frame = intrahour.features(); primary = build_clock(frame)
    ...
    statistics = {name: split_stats(primary, name) for name in SPLITS}; checks: dict[str, bool] = {}
    ...
    registration = json.loads(prereg.DEFAULT_OUTPUT.read_text()); source_manifest = intrahour.PRICE_DIR / "manifest.json"; passed = all(checks.values())
    core = {
        "protocol_version": "cvdar_6_source_support_v1", "policy_id": "CVDAR-6",
    ...
    report = {**core, "manifest_hash": canonical_hash(core)}; RESULT.write_text(json.dumps(report, indent=2, ensure_ascii=False, allow_nan=False) + "\n"); return report
