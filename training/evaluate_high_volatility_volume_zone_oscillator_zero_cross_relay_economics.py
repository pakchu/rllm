"""Sequential strict economics for frozen HVVZO-24."""
...
PREREG = Path("results/high_volatility_volume_zone_oscillator_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "8225f5c608ad58448507b98b24db7497f29b3fbb3e809063a8725db6716ba734"
SUPPORT = Path("results/high_volatility_volume_zone_oscillator_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "9d8bf72ffd6ef29083a5c6d49339dc4f76f705b5c568c76ed23c370e973089bf"
NOVELTY = Path("results/high_volatility_volume_zone_oscillator_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "0cc5f0f7429baeb606a961ef35911c0466b8d4d7c2adf6f3867a8eaf9a4feb09"
CLOCK = Path("data/high_volatility_volume_zone_oscillator_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d99c7eea11066f0b1e2256437060a97f6b45f73655adea9c29edd64cfb63c25f"
CONTROL_DIR = Path("data/high_volatility_volume_zone_oscillator_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_volume_zone_oscillator_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_volume_zone_oscillator_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "quote_volume_vzo", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVVZO frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVVZO novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvvzo_24_sequential_economics_v1", "policy_id": POLICY_ID,
