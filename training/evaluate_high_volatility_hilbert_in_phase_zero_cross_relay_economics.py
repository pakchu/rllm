"""Sequential strict economics for frozen HVHILBERT-I7-24."""
...
PREREG = Path("results/high_volatility_hilbert_in_phase_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "9df44a09c83785d5b1af2c8aa80d5599b7cd34c9c0054abbeb83dd5797b5c84f"
SUPPORT = Path("results/high_volatility_hilbert_in_phase_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "47d93b0c4fd393f91f94b2424ad9a1bd7af975b9f4f8f456e3cc5c57dbb0e1f5"
NOVELTY = Path("results/high_volatility_hilbert_in_phase_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "5385215a36259f63bdc18fce7601b64907e3fee8b9afc8bfd37bd0c91e993415"
CLOCK = Path("data/high_volatility_hilbert_in_phase_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "fbd91c362ed94b7c2f8595ca5a8eb626a00cf0a001fec097947231204d58fbe6"
CONTROL_DIR = Path("data/high_volatility_hilbert_in_phase_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_hilbert_in_phase_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_hilbert_in_phase_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = (
    "no_variation_gate",
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVHILBERT frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVHILBERT novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvhilbert_i7_24_sequential_economics_v1", "policy_id": POLICY_ID,
