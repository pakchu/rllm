"""Sequential strict economics for frozen TRTSR-24."""
...
PREREG = Path("results/treasury_real_term_spread_relay_preregistration_2026-08-09.json")
PREREG_SHA = "577812844c8de0b52828a6e1a832ef46744e96ce3ee25608aa5b97b383055869"
SUPPORT = Path("results/treasury_real_term_spread_relay_support_2026-08-09.json")
SUPPORT_SHA = "b94d46d1ad2094cc45a501651d5a1fe89307448945acfc59aa5ee6245f2a5451"
NOVELTY = Path("results/treasury_real_term_spread_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "4c4b7824005a283b1bcb344718c2b5a67a1a0d3a96eb15418488e63e0725b98a"
CLOCK = Path("data/treasury_real_term_spread_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "9f22f6e0b33d2fb40b1bac63b0d108e0c0ea1cd853255c1379b95910b28ce382"
CONTROL_DIR = Path("data/treasury_real_term_spread_relay_controls_2023_2026")
FREEZE = Path("results/treasury_real_term_spread_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/treasury_real_term_spread_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = (
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("TRTSR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("TRTSR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "trtsr_24_sequential_economics_v1", "policy_id": POLICY_ID,
