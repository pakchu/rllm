"""Evaluate frozen FETLS-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "FETLS-6"
PROTOCOL = "fetls_6_gross9_novelty_v1"
...
OUTPUT = Path("results/fast_empty_template_loss_shock_gross9_novelty_2026-08-09.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("FETLS predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("FETLS novelty limits drift")
    if (
        ...
        raise RuntimeError("FETLS predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="FETLS primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "fetls_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject FETLS-6 unchanged before economic outcomes",
    }
