"""Sequential strict economics for frozen NVLRR-12."""
...
PREREG = Path("results/nasdaq_volatility_leadership_rotation_relay_preregistration_2026-08-10.json")
PREREG_SHA = "c091adf520e5f9b578922ef79f7a4c523da9b0964884bc523afbbe4a90ed4e6d"
SUPPORT = Path("results/nasdaq_volatility_leadership_rotation_relay_support_2026-08-10.json")
SUPPORT_SHA = "17bf2b112b0685344769b9ff67cb5f70a2960cbe46fd444b9ee13e03b645f3c4"
NOVELTY = Path("results/nasdaq_volatility_leadership_rotation_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "28f8f38682445889c0cab48c0b7c2f0d6a53fe98499a82ed0e735802338cc30d"
CLOCK = Path("data/nasdaq_volatility_leadership_rotation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "420ddb023fe837e189be0517a30eb1884204b15651c8cbc0b502ae748ca87ecd"
CLOCK_ROWS = 99
CONTROL_DIR = Path("data/nasdaq_volatility_leadership_rotation_relay_controls_2023_2026")
FREEZE = Path("results/nasdaq_volatility_leadership_rotation_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/nasdaq_volatility_leadership_rotation_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_btc_variation_gate", "no_leadership_tail", "vxn_minus_vix_raw", "one_session_stale_leadership", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("NVLRR frozen predecessor hash drift")
    support = load_json(SUPPORT)
    if (
        support.get("clock", {}).get("sha256") != CLOCK_SHA
        or support.get("clock", {}).get("rows") != CLOCK_ROWS
    ):
        raise RuntimeError("NVLRR support clock binding drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("NVLRR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "nvlrr_12_sequential_economics_v1", "policy_id": POLICY_ID,
