"""Build source-only support for frozen HVMBC-8."""
...
ENV_FILE="/home/pakchu/rllm/.env";PREREG_SHA="b5c9670de622539af5de133381dc20c2cf0f3ace40281b66c87cf1984a21f1ae";START=pd.Timestamp("2023-01-01T00:00Z");END=pd.Timestamp("2026-08-01T00:00Z")
SPLITS={"train":(pd.Timestamp("2023-07-01T00:00Z"),pd.Timestamp("2024-01-01T00:00Z")),"test":(pd.Timestamp("2024-01-01T00:00Z"),pd.Timestamp("2025-01-01T00:00Z")),"eval":(pd.Timestamp("2025-01-01T00:00Z"),pd.Timestamp("2026-01-01T00:00Z")),"final":(pd.Timestamp("2026-01-01T00:00Z"),END)};MINIMUM={"train":8,"test":12,"eval":12,"final":8};CONTROLS=("no_contradiction_gate","no_variation_gate","block_return_side","mean_bar_return_side","one_block_stale_features","direction_flip","same_clock_forced_long")
...
SOURCE_DIR=Path("data/high_volatility_median_breadth_contradiction_relay_sources_2023_2026");FEATURES=SOURCE_DIR/"eight_hour_median_breadth_contradiction.csv.gz";MANIFEST=SOURCE_DIR/"manifest.json";CLOCK=Path("data/high_volatility_median_breadth_contradiction_relay_clocks_2023_2026.csv.gz");CONTROL_DIR=Path("data/high_volatility_median_breadth_contradiction_relay_controls_2023_2026");RESULT=Path("results/high_volatility_median_breadth_contradiction_relay_support_2026-08-09.json")
FEATURE_COLUMNS=("decision_time","feature_available_time","source_valid","median_bar_return","mean_bar_return","breadth_direction","contradiction","realized_variation","block_return","variation_rank")
...
def prior_midrank(v:pd.Series,lookback:int=270,minimum:int=252)->pd.Series:
 ...
 return out
def breadth_features(opens:np.ndarray,closes:np.ndarray)->tuple[float,float,float,float,bool]:
 returns=np.log(closes/opens);median=float(np.median(returns));mean=float(np.mean(returns));block=float(np.log(closes[-1]/opens[0]));variation=float(np.sqrt(np.square(returns).sum()));contradiction=bool(median!=0 and block!=0 and np.sign(median)!=np.sign(block));return median,mean,block,variation,contradiction
def engine():
...
def build_features(bars:pd.DataFrame)->pd.DataFrame:
 ...
 if not req.issubset(bars):raise ValueError("HVMBC source schema drift")
 b=bars.copy()
 ...
 for d in pd.date_range(START+pd.Timedelta(hours=8),END,freq="8h",inclusive="left"):
  ...
  median_return=mean_return=breadth_direction=variation=block_return=np.nan;contradiction=False
  if valid:
   opens=w.bar_open.to_numpy(float);closes=w.bar_close.to_numpy(float);median_return,mean_return,block_return,variation,contradiction=breadth_features(opens,closes);breadth_direction=float(np.sign(median_return));valid=bool(median_return!=0 and block_return!=0 and variation>0 and np.isfinite([median_return,mean_return,block_return,variation]).all())
  if not valid:median_return=mean_return=breadth_direction=variation=block_return=np.nan;contradiction=False
  rows.append({"decision_time":d,"feature_available_time":d,"source_valid":valid,"median_bar_return":median_return,"mean_bar_return":mean_return,"breadth_direction":breadth_direction,"contradiction":contradiction,"realized_variation":variation,"block_return":block_return})
 out=pd.DataFrame(rows);out["variation_rank"]=prior_midrank(out.realized_variation.where(out.source_valid));return out[list(FEATURE_COLUMNS)]
def conditions(f:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 u=f.shift(1) if control=="one_block_stale_features" else f;valid=u.source_valid.eq(True)&u.breadth_direction.ne(0);cg=pd.Series(True,index=f.index) if control=="no_contradiction_gate" else u.contradiction.eq(True);vg=pd.Series(True,index=f.index) if control=="no_variation_gate" else u.variation_rank.ge(.65);active=valid&cg&vg;basis=u.block_return if control=="block_return_side" else (u.mean_bar_return if control=="mean_bar_return_side" else u.breadth_direction);side=np.sign(basis).fillna(0).astype(int)
...
def run()->dict[str,Any]:
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVMBC prereg drift")
 ...
 sc={"protocol_version":"hvmbc_8_source_v1","query_sha256":hashlib.sha256(BAR_QUERY.encode()).hexdigest(),"table":"bars_binance","window":[START.isoformat(),END.isoformat()],"physical_rows_1m":int(pd.to_numeric(bars.source_rows).sum()),"features":{"path":str(FEATURES),"sha256":sha(FEATURES),"rows":len(features),"valid_rows":int(features.source_valid.sum())},"outcomes_opened":False,"gross9_rows_opened":False,"no_imputation":True};sm={**sc,"manifest_hash":chash(sc)};MANIFEST.write_text(json.dumps(sm,indent=2,allow_nan=False)+"\n");support={n:stats(primary,n) for n in SPLITS};checks={}
