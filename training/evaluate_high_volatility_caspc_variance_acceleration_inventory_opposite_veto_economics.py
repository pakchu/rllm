"""Strict staged economics for the singleton HVCASPCVAIV-8 policy."""
...
PREREG = Path("results/high_volatility_caspc_variance_acceleration_inventory_opposite_veto_preregistration_2026-08-17.json")
PREREG_SHA = "eb4b40c5c79ff6ba5173b7ce927b5fa8f29a40187dec8dff3715a718f47523bb"
SUPPORT = Path("results/high_volatility_caspc_variance_acceleration_inventory_opposite_veto_support_2026-08-17.json")
SUPPORT_SHA = "f9d4c580bbea2cf1988deb9f8d52ba2519fadd314443e222f043c1b0917b8e60"
GROSS9 = Path("results/high_volatility_caspc_variance_acceleration_inventory_opposite_veto_gross9_novelty_2026-08-17.json")
GROSS9_SHA = "bb47a1544c86a23ff37f2322cd3d07e5aa116e67fe89d2a0fb83ea41daa1a93d"
CLOCK = {
    "path": Path("data/high_volatility_caspc_variance_acceleration_inventory_opposite_veto_clocks_2023_2026.csv.gz"),
    "sha256": "d437e3384c03f2c54b9b916c924b80dd853ba7b31aa9614028ecb753e227295d",
    "rows": 149,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_caspc_variance_acceleration_inventory_opposite_veto_{stage}_economics_2026-08-17.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCASPCVAIV-8 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCASPCVAIV-8 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCASPCVAIV-8 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCASPCVAIV-8 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCASPCVAIV-8 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCASPCVAIV-8 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCASPCVAIV-8 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCASPCVAIV-8 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCASPCVAIV-8 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCASPCVAIV-8 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCASPCVAIV-8 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcaspcvaiv_8_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
