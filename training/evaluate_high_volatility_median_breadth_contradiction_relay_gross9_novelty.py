"""Evaluate frozen HVMBC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVMBC-8"
PROTOCOL = "hvmbc_8_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_median_breadth_contradiction_relay_preregistration_2026-08-09.json"
)
...
OUTPUT = Path(
    "results/high_volatility_median_breadth_contradiction_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVMBC predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVMBC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVMBC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVMBC primary")
    ...
    core = {
        ...
        "failure_action": (
            None if advance else "reject HVMBC-8 unchanged before economic outcomes"
        ),
    }
