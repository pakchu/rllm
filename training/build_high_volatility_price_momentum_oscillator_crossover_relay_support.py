"""Build outcome-blind source support for frozen HVPMO-24."""
...
from training import preregister_high_volatility_price_momentum_oscillator_crossover_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "ca64fa1b68f71bcdeda1b196c47392c132896b00d1c1ef823efeea32e4331721"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_price_momentum_oscillator_crossover_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_price_momentum_oscillator_crossover_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_price_momentum_oscillator_crossover_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_price_momentum_oscillator_crossover_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_price_momentum_oscillator_crossover_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "roc", "first_smoothing", "pmo", "signal", "difference",
    "entry_side", "zero_line_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "roc", "first_smoothing", "pmo", "signal", "difference",
    "entry_side", "zero_line_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVPMO source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVPMO invalid source key")
...
def custom_ema(series: pd.Series, valid: pd.Series, period: int) -> pd.Series:
    alpha=2.0/period;output=np.full(len(series),np.nan);state=np.nan;run=0
    for i,(value,is_valid) in enumerate(zip(series.to_numpy(float),valid.to_numpy(bool))):
        if not is_valid or not math.isfinite(value):state=np.nan;run=0;continue
        state=value if not math.isfinite(state) else alpha*value+(1-alpha)*state;run+=1
        if run>=period:output[i]=state
...
def pmo_values(series: pd.Series, valid: pd.Series) -> pd.DataFrame:
    previous=series.shift(1);roc=(100.0*(series/previous-1.0)).where(valid&valid.shift(1,fill_value=False)&previous.gt(0));ok=np.isfinite(roc)
    first=P["first_scale"]*custom_ema(roc,ok,P["first_smoothing_periods"])
    pmo=custom_ema(first,np.isfinite(first),P["second_smoothing_periods"])
    signal=causal_ema(pmo,np.isfinite(pmo),P["signal_periods"])
    return pd.DataFrame({"roc":roc,"first_smoothing":first,"pmo":pmo,"signal":signal,"difference":pmo-signal},index=series.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    values=pmo_values(bars.bar_close,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["entry_side"]=cross_side(bars.difference);bars["zero_line_side"]=cross_side(bars.pmo)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["roc","first_smoothing","pmo","signal","difference","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVPMO prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvpmo_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvpmo_24_source_support_v1", "policy_id": prereg.POLICY_ID,
