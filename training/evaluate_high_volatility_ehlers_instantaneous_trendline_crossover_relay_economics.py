"""Sequential strict economics for frozen HVEIT-24."""
...
PREREG = Path("results/high_volatility_ehlers_instantaneous_trendline_crossover_relay_preregistration_2026-08-11.json")
PREREG_SHA = "fb2f567d129079ed56bd083ca877511b4a0d54269ce9d51ca3ac322716db8365"
SUPPORT = Path("results/high_volatility_ehlers_instantaneous_trendline_crossover_relay_support_2026-08-11.json")
SUPPORT_SHA = "461c4b68198f2236c1a13bbefece5ed5a0b1271b6c413f9b717022c2ac407982"
NOVELTY = Path("results/high_volatility_ehlers_instantaneous_trendline_crossover_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "8b7ad9eab8f31db49b62ca11cf98cbb00b39f3448c33039773fd654cde2a98e3"
CLOCK = Path("data/high_volatility_ehlers_instantaneous_trendline_crossover_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "b85a43802f011bfa7fbc093007c6a68c1e1e0fbeec6ca9005edd0beacabbfab5"
CONTROL_DIR = Path("data/high_volatility_ehlers_instantaneous_trendline_crossover_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_ehlers_instantaneous_trendline_crossover_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_ehlers_instantaneous_trendline_crossover_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "price_itrend_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVEIT frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVEIT novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hveit_24_sequential_economics_v1", "policy_id": POLICY_ID,
