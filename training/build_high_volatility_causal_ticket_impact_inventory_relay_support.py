"""Deterministic outcome-blind source support for HVCTIIR-8."""
...
from training import preregister_high_volatility_causal_ticket_impact_inventory_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "dd91b8a0b2400bcca4884017c2c566cf4f51f1fde94af09c603037c94432f02a"
...
BAR_QUERY = """SELECT date_bin('8 hours',ts,TIMESTAMPTZ '1970-01-01 00:00:00+00')+INTERVAL '8 hours' AS decision_time,corr(ln(1+quote_asset_volume/nullif(number_of_trades,0)),abs(ln(close/open))) AS ticket_impact_alignment,stddev_pop(ln(1+quote_asset_volume/nullif(number_of_trades,0))) AS ticket_log_std,stddev_pop(abs(ln(close/open))) AS absolute_return_std,sum(ln(close/open)) AS completed_return,sum(power(ln(close/open),2)) AS minute_squared_return,count(*) AS source_rows,count(DISTINCT ts) AS distinct_rows,min(ts) AS first_ts,max(ts) AS last_ts,bool_and(open>0 AND high>0 AND low>0 AND close>0 AND high>=greatest(open,close,low) AND low<=least(open,close,high) AND quote_asset_volume>=0 AND number_of_trades>0 AND number_of_trades=floor(number_of_trades)) AS coherent FROM bars_binance WHERE symbol='BTCUSDT' AND interval='1m' AND ts>=:start AND ts<:end GROUP BY 1 ORDER BY 1"""

ROOT = Path("data/high_volatility_causal_ticket_impact_inventory_relay_sources_2020_2026")
PANEL = ROOT / "settlement_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_causal_ticket_impact_inventory_relay_clocks_2020_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_causal_ticket_impact_inventory_relay_split_clocks_2020_2026")
RESULT = Path("results/high_volatility_causal_ticket_impact_inventory_relay_support_2026-08-16.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "decision_time", "feature_available_time", "source_valid", "sum_open_interest",
    "previous_open_interest", "oi_change", "ticket_impact_alignment", "ticket_impact_rank", "completed_return", "realized_variation",
    "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "decision_time", "feature_available_time",
    "entry_time", "exit_time", "side", "oi_change", "ticket_impact_alignment", "ticket_impact_rank", "completed_return", "realized_variation", "variation_rank",
)
...
def prepare(raw: tuple[pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if oi.columns.tolist() != ["decision_time", "sum_open_interest"]:
        raise RuntimeError("HVCTIIR-8 OI schema drift")
    expected_bars = [
        "decision_time", "ticket_impact_alignment", "ticket_log_std", "absolute_return_std", "completed_return", "minute_squared_return",
        "source_rows", "distinct_rows", "first_ts", "last_ts", "coherent",
    ]
    if bars.columns.tolist() != expected_bars:
        raise RuntimeError("HVCTIIR-8 bar schema drift")
    ...
    if oi["decision_time"].duplicated().any() or not np.isfinite(oi["sum_open_interest"]).all():
        raise RuntimeError("HVCTIIR-8 invalid OI rows")

    bars = bars.copy()
    for column in ("decision_time", "first_ts", "last_ts"):
        bars[column] = pd.to_datetime(bars[column], utc=True, errors="raise")
    numeric = ("ticket_impact_alignment", "ticket_log_std", "absolute_return_std", "completed_return", "minute_squared_return", "source_rows", "distinct_rows")
    for column in numeric:
        bars[column] = pd.to_numeric(bars[column], errors="raise")
    if bars["decision_time"].duplicated().any():
        raise RuntimeError("HVCTIIR-8 duplicate eight-hour bar")
    start = bars["decision_time"] - pd.Timedelta("8h")
    bars["bar_valid"] = (
        np.isfinite(bars[list(numeric)]).all(axis=1)
        & bars["ticket_log_std"].gt(0)
        & bars["absolute_return_std"].gt(0)
...
def build_panel(raw: tuple[pd.DataFrame, pd.DataFrame]) -> pd.DataFrame:
    ...
    panel = panel.join(
        bars[["bar_valid", "ticket_impact_alignment", "completed_return", "realized_variation"]],
        on="decision_time",
    )
    panel["source_valid"] = (
        ...
        & np.isfinite(
            panel[["sum_open_interest", "previous_open_interest", "oi_change", "ticket_impact_alignment", "completed_return", "realized_variation"]]
        ).all(axis=1)
        & panel["sum_open_interest"].gt(0)
        & panel["previous_open_interest"].gt(0)
        & panel["realized_variation"].gt(0)
        & panel["completed_return"].ne(0)
        & panel["ticket_impact_alignment"].gt(0)
    )
    valid = panel["source_valid"].eq(True)
    panel["ticket_impact_rank"] = causal_midrank(panel["ticket_impact_alignment"].where(valid))
    panel["variation_rank"] = causal_midrank(panel["realized_variation"].where(valid))
    panel["eligible"] = (
        valid
        & panel["oi_change"].gt(0)
        & panel["ticket_impact_rank"].ge(POLICY["ticket_impact_rank_min"])
...
def build_clock(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in panel.loc[panel["eligible"]].itertuples(index=False):
        ...
        if side not in (-1, 1) or row.feature_available_time > entry:
            raise RuntimeError("HVCTIIR-8 side or availability drift")
        ...
        rows.append({
            ...
            "ticket_impact_alignment": float(row.ticket_impact_alignment),
            "ticket_impact_rank": float(row.ticket_impact_rank),
...
def run() -> dict[str, Any]:
    if sha256_file(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCTIIR-8 preregistration hash drift")
    ...
    source_core = {
        "protocol_version": "hvctiir_8_sources_v1",
    ...
    core = {
        "protocol_version": "hvctiir_8_source_support_v1", "policy_id": prereg.POLICY_ID,
