"""Build outcome-blind source support for frozen HVEFD-24."""
...
from training import preregister_high_volatility_ehlers_deviation_scaled_fisher_reentry_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "13478df628b5c04602b180d2c70803f59b91ed1b8890ab277c51f628790b44f2"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ehlers_deviation_scaled_fisher_reentry_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ehlers_deviation_scaled_fisher_reentry_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ehlers_deviation_scaled_fisher_reentry_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ehlers_deviation_scaled_fisher_reentry_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ehlers_deviation_scaled_fisher_reentry_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "run_length", "zeros", "filtered", "rms", "scaled",
    "fisher", "entry_side", "raw_entry_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "run_length", "zeros", "filtered", "rms",
    "scaled", "fisher", "entry_side", "raw_entry_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVEFD source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVEFD invalid source key")
...
def deviation_scaled_fisher(series: pd.Series, valid: pd.Series) -> pd.DataFrame:
    period=P["smoother_periods"];a1=math.exp(-1.414*math.pi/period);c2=2*a1*math.cos(1.414*math.pi/period);c3=-a1*a1;c1=1-c2-c3
    values=series.to_numpy(float);ok=valid.to_numpy(bool);zeros=np.full(len(series),np.nan);filt=np.full(len(series),np.nan);rms=np.full(len(series),np.nan);scaled=np.full(len(series),np.nan);fisher=np.full(len(series),np.nan);runs=np.zeros(len(series),dtype=int);run=0;window=[]
    for i,(price,is_valid) in enumerate(zip(values,ok)):
        if not is_valid or not math.isfinite(price):run=0;window=[];continue
        run+=1;runs[i]=run
        if run<=2:continue
        zeros[i]=price-values[i-2]
        filt[i]=zeros[i] if run<=5 else c1*(zeros[i]+zeros[i-1])/2+c2*filt[i-1]+c3*filt[i-2]
        window.append(float(filt[i]));window=window[-P["periods"]:]
        if len(window)==P["periods"]:
            scale=math.sqrt(float(np.mean(np.square(window))))
            if scale>0:
                rms[i]=scale;scaled[i]=filt[i]/scale
                if abs(scaled[i])<2:fisher[i]=.5*math.log((1+scaled[i]/2)/(1-scaled[i]/2))
    return pd.DataFrame({"run_length":runs,"zeros":zeros,"filtered":filt,"rms":rms,"scaled":scaled,"fisher":fisher},index=series.index)


def reentry_side(signal: pd.Series, upper: float, lower: float) -> pd.Series:
    previous=signal.shift(1);side=np.zeros(len(signal),dtype=int);side[(signal>lower)&(previous<=lower)]=1;side[(signal<upper)&(previous>=upper)]=-1;return pd.Series(side,index=signal.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    fields=["bar_open","bar_high","bar_low","bar_close"];bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[fields]).all(axis=1)&bars[fields].gt(0).all(axis=1);values=deviation_scaled_fisher(bars.bar_close,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["entry_side"]=reentry_side(bars.fisher,P["overbought"],P["oversold"]);raw=P["raw_scaled_equivalent"];bars["raw_entry_side"]=reentry_side(bars.scaled,raw,-raw);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["zeros","filtered","rms","scaled","fisher","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="unfisherized_equivalent_reentry":side=pd.to_numeric(used.raw_entry_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVEFD prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvefd_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvefd_24_source_support_v1", "policy_id": prereg.POLICY_ID,
