"""Evaluate frozen HVVRT-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVVRT-12"
PROTOCOL = "hvvrt_12_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_variance_ratio_trend_onset_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVVRT predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVVRT novelty limits drift")
    if (
        ...
        raise RuntimeError("HVVRT predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVVRT primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvvrt_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVVRT-12 unchanged before economic outcomes",
    }
