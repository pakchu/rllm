"""Evaluate frozen HVCAQF-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCAQF-6"
PROTOCOL = "hvcaqf_6_gross9_novelty_v1"
PREREG = Path("results/high_volatility_cross_alt_quarter_hour_flow_consensus_relay_preregistration_2026-08-13.json")
PREREG_SHA = "ed8a0ec652cd96b9a59c89337f7ae44ba8999e0254e1908aa2fad737a0b2452e"
SUPPORT = Path("results/high_volatility_cross_alt_quarter_hour_flow_consensus_relay_support_2026-08-13.json")
SUPPORT_SHA = "e93dfe0af4cc93d8a1e9116c4593de56c0de9b2159c4a2da2123fb58aadd1bc4"
CLOCK = Path("data/high_volatility_cross_alt_quarter_hour_flow_consensus_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "85fb8ceb1f04d95d2be32bef400a7d1b6febfd6b3efb9d73896243b1933b3122"
OUTPUT = Path("results/high_volatility_cross_alt_quarter_hour_flow_consensus_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCAQF predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCAQF novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCAQF predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCAQF primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvcaqf_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVCAQF-6 unchanged before economic outcomes",
    }
