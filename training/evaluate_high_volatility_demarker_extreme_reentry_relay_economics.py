"""Sequential strict economics for frozen HVDEM-24."""
...
PREREG = Path("results/high_volatility_demarker_extreme_reentry_relay_preregistration_2026-08-11.json")
PREREG_SHA = "a66976c7a542de9865b417d3eef82f79c2e19eebb11e5c66339036db19e84dae"
SUPPORT = Path("results/high_volatility_demarker_extreme_reentry_relay_support_2026-08-11.json")
SUPPORT_SHA = "bf74e61e59073c2f5e03012523652eff86191abbb99c66946185678b41c33e70"
NOVELTY = Path("results/high_volatility_demarker_extreme_reentry_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "1f173f9d3caa56813b9a98020a2092a0f869718ff15feafe73103f56808583b7"
CLOCK = Path("data/high_volatility_demarker_extreme_reentry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "c701ebfeaee9dda74dbf9f8eed3d0c1fd29dd0ab3473c65d3bfa1409da1c5368"
CONTROL_DIR = Path("data/high_volatility_demarker_extreme_reentry_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_demarker_extreme_reentry_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_demarker_extreme_reentry_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDEM frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDEM novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdem_24_sequential_economics_v1", "policy_id": POLICY_ID,
