"""Evaluate frozen HVCASPC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCASPC-8"
PROTOCOL = "hvcaspc_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_cross_alt_serial_persistence_consensus_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCASPC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCASPC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCASPC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCASPC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCASPC-8 unchanged before economic outcomes",
    }
