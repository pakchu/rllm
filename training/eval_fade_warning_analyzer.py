"""Evaluate fade-warning analyzer JSON predictions."""
...
DEFAULTS = {
    ...
    "skip_reason": "NO_EDGE",
    "trend_continuation_quality": "NO_CONTINUATION",
}
PRIMARY_KEYS = ("fade_warning",)
AUX_KEYS = ("skip_reason", "trend_continuation_quality")
...
def parse_fade_warning_json(text: str) -> dict[str, str]:
    obj = _extract_json_object(text)
    out: dict[str, str] = {}
    for key, valid in VALID.items():
        val = str(obj.get(key, DEFAULTS[key])).upper()
...
def _metrics(rows: list[dict[str, Any]], preds: list[dict[str, str]]) -> dict[str, Any]:
    ...
    for key in VALID:
        ...
        for row, pred in zip(rows, preds):
            target = parse_fade_warning_json(str(row["target"]))[key]
            pval = pred[key]
    ...
    for row, pred in zip(rows, preds):
        target = parse_fade_warning_json(str(row["target"]))
...
def _generate_predictions(rows: list[dict[str, Any]], *, model_name: str, adapter_dir: str, max_new_tokens: int, batch_size: int, progress_every: int, prediction_output: str = "") -> list[dict[str, str]]:
    ...
    try:
        for start in range(0, len(rows), max(1, int(batch_size))):
            ...
            for idx, generated_ids in enumerate(out[:, prompt_width:]):
                ...
                pred = parse_fade_warning_json(generated)
                preds.append(pred)
            ...
            if progress_every > 0 and (done == len(rows) or done % progress_every == 0):
                print(f"generated {done}/{len(rows)} fade-warning predictions", flush=True)
    finally:
        if stream is not None:
            stream.close()
    return preds
...
def evaluate_fade_warning_analyzer(
    *,
    eval_jsonl: str,
    output: str,
    ...
    if prediction_mode == "target_echo":
        preds = [parse_fade_warning_json(str(r["target"])) for r in rows]
    elif prediction_mode == "model":
        if not adapter_dir:
            raise ValueError("adapter_dir is required for prediction_mode=model")
        preds = _generate_predictions(rows, model_name=model_name, adapter_dir=adapter_dir, max_new_tokens=max_new_tokens, batch_size=batch_size, progress_every=progress_every, prediction_output=prediction_output)
    else:
    ...
    report = {
        ...
        "leakage_guard": {
            ...
            "fade_warning_targets_are_decomposed_from_future_path_shape_labels": True,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Evaluate fade-warning analyzer predictions")
...
def main() -> None:
    print(json.dumps(evaluate_fade_warning_analyzer(**vars(parse_args())), indent=2, ensure_ascii=False))
