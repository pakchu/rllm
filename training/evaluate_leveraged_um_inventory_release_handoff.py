"""Frozen pre-2024 return evaluator for LURI-48.

The evaluator must be committed and hash-recorded before it can load USD-M
execution prices or realized funding. It reserves the primary and every
control clock from causal features first, then opens only 2020-2023 outcomes.
"""
...
CLOCK_COMMIT = "510fdaee4d98fc23327cc5e334b56444677ae767"
FUNDING_COMMIT = "868bd7a4f2b792d3570bae93c4d4b7cc04f3c6b9"
PREREGISTRATION_SOURCE = Path(
    "training/preregister_leveraged_um_inventory_release_handoff.py"
)
...
PREREGISTRATION_DOCUMENT = Path(
    "docs/leveraged-um-inventory-release-handoff-preregistration-2026-07-14.md"
)
PREREGISTRATION_DOCUMENT_SHA256 = (
    "9273912c1993470def6e9c1ee06c1a935232722521d6d26b297dbddb9dc6b691"
)
SUPPORT_DOCUMENT = Path(
    "docs/leveraged-um-inventory-release-handoff-support-freeze-2026-07-14.md"
)
SUPPORT_DOCUMENT_SHA256 = (
    "303c9a182ed6f0a9d0edd54efda60c2d8a52d49060ccb327dd0c33ee3395c4b6"
)
CLOCK_DOCUMENT = Path(
    "docs/leveraged-um-inventory-release-handoff-clock-freeze-2026-07-14.md"
)
CLOCK_DOCUMENT_SHA256 = (
    "7d33afe43b2b5d7155f066852b4b1e5ef3ac642478ff682259389b72411dd634"
)
EVALUATOR_DOCUMENT = Path(
    "docs/leveraged-um-inventory-release-handoff-evaluator-freeze-2026-07-14.md"
)
EVALUATOR_DOCUMENT_SHA256 = (
    "83436673dbad7a3c8ece0b8ccbaedb00eeeb4b148f7df22db3d4ab9162f9adb3"
)
SUPPORT_RESULT = Path(
...
CLOCK_FREEZE_SOURCE = Path(
    "training/freeze_leveraged_um_inventory_release_handoff_clock.py"
)
CLOCK_FREEZE_SOURCE_SHA256 = (
    "d84479f365d8431ca1a3b207ccfe94f7091e9b0f4052cd7f451aa568eda439cb"
)
...
FUNDING_BUILDER_SOURCE = Path(
    "training/freeze_binance_um_btcusdt_funding_2020_2023.py"
)
FUNDING_BUILDER_SOURCE_SHA256 = (
    "9771e6d1135ac75b47cffdaf3a999da9b22c91624401c4d1c7513854406509fb"
)
FUNDING_DOCUMENT = Path(
    "docs/binance-um-btcusdt-realized-funding-freeze-2026-07-14.md"
)
FUNDING_DOCUMENT_SHA256 = (
    "d6dfa4c1123a00a406950c82be20b54316c9fe63190aed6080645ca347d0d613"
)
...
EVALUATION_SOURCE = Path(
    "training/evaluate_leveraged_um_inventory_release_handoff.py"
)
EVALUATION_FREEZE = Path(
    "results/leveraged_um_inventory_release_handoff_evaluator_freeze_2026-07-14.json"
)
...
POLICY_NAMES = tuple(luri.CONTROL_SIDE_RULES)
SCORE_BEARING_CONTROLS = luri.SCORE_BEARING_CONTROLS
FALSIFICATION_CONTROLS = ("direction_flip", "signal_delay_1bar")
CLOCK_COLUMNS = luri.SCHEDULE_COLUMNS
...
class EvaluationConfig:
    ...
    minimum_2023_half_trades: int = 45
...
def _validate_evaluation_config(cfg: EvaluationConfig) -> None:
    ...
    if changed:
        raise ValueError(f"LURI evaluation config is frozen: {changed}")


def verify_evaluation_freeze() -> dict[str, Any]:
    if not EVALUATION_FREEZE.is_file():
        raise ValueError("LURI evaluator freeze manifest is missing")
    freeze = _read_json(EVALUATION_FREEZE)
    if freeze.get("outcomes_opened_for_luri48") is not False:
        raise ValueError("LURI evaluator was not frozen before outcomes")
    if freeze.get("evaluation_source") != str(EVALUATION_SOURCE):
        raise ValueError("LURI evaluator freeze path changed")
    if freeze.get("evaluation_source_sha256") != _sha256(EVALUATION_SOURCE):
        raise ValueError("LURI evaluator differs from pre-outcome freeze")
    if len(str(freeze.get("evaluation_source_commit", ""))) != 40:
        raise ValueError("LURI evaluator source commit is not full length")
    expected_values = {
    ...
    for key, expected in expected_values.items():
        if freeze.get(key) != expected:
            raise ValueError(f"LURI evaluator freeze {key} changed")
    if freeze.get("opened_windows") != []:
        raise ValueError("LURI evaluator freeze already opened a window")
    if freeze.get("returns_prices_or_funding_loaded_during_freeze") is not False:
        raise ValueError("LURI evaluator freeze loaded outcomes")
    if freeze.get("mutable_parameters") != []:
        raise ValueError("LURI evaluator freeze permits mutable parameters")
    expected_sealed = [*WINDOWS, "test2024", "eval2025", "ytd2026"]
    if freeze.get("sealed_windows") != expected_sealed:
        raise ValueError("LURI evaluator freeze sealed windows changed")
    return freeze
...
def verify_preregistration() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    frozen_files = (
        ...
        (FUNDING_BUILDER_SOURCE, FUNDING_BUILDER_SOURCE_SHA256),
        (FUNDING_DOCUMENT, FUNDING_DOCUMENT_SHA256),
        (FUNDING_MANIFEST, FUNDING_MANIFEST_SHA256),
    )
    for path, expected in frozen_files:
        if _sha256(path) != expected:
            raise ValueError(f"frozen LURI dependency changed: {path}")

    result = _read_json(SUPPORT_RESULT)
    protocol = result.get("protocol", {})
    if protocol.get("luri_outcomes_opened") is not False:
        raise ValueError("LURI support artifact opened outcomes")
    if protocol.get("support_only") is not True:
        raise ValueError("LURI support artifact is not support-only")
    if protocol.get("control_side_rules") != luri.CONTROL_SIDE_RULES:
        raise ValueError("LURI control side rules changed")
    if protocol.get("score_bearing_controls") != list(SCORE_BEARING_CONTROLS):
        raise ValueError("LURI score-bearing controls changed")
    if result.get("support_decision") != "pass":
        raise ValueError("LURI support gate did not pass")
    if result.get("selected_basis_quantile") != SELECTED_BASIS_QUANTILE:
        raise ValueError("LURI selected basis quantile changed")
    if result.get("config") != asdict(luri.Config()):
        raise ValueError("LURI signal config differs from frozen support")
    selected = result.get("selected_support", {})
    if selected.get("passes_support") is not True:
        raise ValueError("LURI selected support row is not passing")
    if selected.get("support", {}).get("nonoverlap_total") != 432:
        raise ValueError("LURI selected event count changed")
    ...
    if frozen_protocol.get("outcomes_opened") is not False:
        raise ValueError("LURI event clock opened outcomes")
    if frozen_protocol.get("preregistration_commit") != PREREGISTRATION_COMMIT:
        raise ValueError("LURI event clock preregistration changed")
    if frozen_protocol.get("support_commit") != SUPPORT_COMMIT:
        raise ValueError("LURI event clock support commit changed")
    if (
        frozen_protocol.get("selected_basis_quantile")
        != SELECTED_BASIS_QUANTILE
    ):
        raise ValueError("LURI event clock quantile changed")
    if clock.get("sha256") != EVENT_CLOCK_SHA256 or clock.get("rows") != 432:
        raise ValueError("LURI event clock record changed")
    if clock_manifest.get("support", {}).get("sha256") != SUPPORT_RESULT_SHA256:
        raise ValueError("LURI event clock support hash changed")
    ...
    if market_protocol.get("archive_checksums_verified") is not True:
        raise ValueError("LURI execution archives were not checksum-verified")
    if market_protocol.get("end_is_exclusive") is not True:
        raise ValueError("LURI execution manifest end contract changed")
    if market_manifest.get("combined_output") != str(MARKET_DATA):
        raise ValueError("LURI execution market path changed")
    if market_manifest.get("combined_sha256") != MARKET_DATA_SHA256:
        raise ValueError("LURI execution market hash changed")
    if market_manifest.get("rows") != 420_768:
        raise ValueError("LURI execution market row count changed")
    ...
    if funding_protocol.get("luri_outcomes_opened") is not False:
        raise ValueError("LURI funding source opened outcomes")
    if funding_protocol.get("selection_end_exclusive") != (
        "2024-01-01T00:00:00.000Z"
    ):
        raise ValueError("LURI funding source sealed boundary changed")
    if funding_data.get("path") != str(FUNDING_DATA):
        raise ValueError("LURI funding data path changed")
    if funding_data.get("sha256") != FUNDING_DATA_SHA256:
        raise ValueError("LURI funding data hash changed")
    if funding_data.get("rows") != 4_383:
        raise ValueError("LURI funding row count changed")
...
def _canonical_clock_records(schedule: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    if missing:
        raise ValueError(f"LURI clock is missing columns: {sorted(missing)}")
...
def verify_signal_replay(
    ...
    signal, controls, control_sides, _ = luri.classify_events(
        frame,
        cfg,
        basis_quantile=SELECTED_BASIS_QUANTILE,
    )
    replayed = luri.nonoverlapping_schedule(signal, frame)
    frozen = pd.read_csv(EVENT_CLOCK)
    if _canonical_clock_records(replayed) != _canonical_clock_records(frozen):
        raise ValueError("LURI primary event-clock replay differs from freeze")
    if not replayed["entry_position"].eq(replayed["signal_position"] + 1).all():
        raise ValueError("LURI replay is not next-open entry")
    if not replayed["hold_bars"].eq(cfg.hold_bars).all():
        raise ValueError("LURI replay hold changed")
    if not replayed["exit_position"].eq(
        replayed["entry_position"] + cfg.hold_bars
    ).all():
        raise ValueError("LURI replay exit changed")

    selected = support_result["selected_support"]
    if luri._support(replayed, cfg) != selected["support"]:
        raise ValueError("LURI selected support replay differs from freeze")
    raw_counts = {
        name: int(mask.sum()) for name, mask in controls.items() if name != "primary"
    }
    if raw_counts != selected["control_raw_counts"]:
        raise ValueError("LURI control raw clocks differ from freeze")
    if int(controls["primary"].sum()) != selected["raw_primary"]:
        raise ValueError("LURI raw primary clock differs from freeze")
    clock_manifest = _read_json(EVENT_CLOCK_MANIFEST)
    if source != clock_manifest.get("source"):
        raise ValueError("LURI source replay differs from clock freeze")
...
def _schedule_from_control(
    ...
    *,
    ...
    if not numeric_side.loc[active].isin((-1.0, 1.0)).all():
        raise ValueError(f"LURI control {branch} has an invalid active side")
    signal = pd.DataFrame(
        {
    ...
    return (
        luri.nonoverlapping_schedule(signal, frame)
...
def build_control_schedules(
    ...
    if tuple(schedules) != POLICY_NAMES:
        raise ValueError("LURI evaluator control set differs from preregistration")
    ...
    if support_result is not None:
        ...
        if observed != expected:
            raise ValueError("LURI control scheduled clocks differ from freeze")
    return schedules
...
def slice_schedule(
    ...
    *,
    ...
    if start_timestamp >= end_timestamp:
        raise ValueError("LURI split start must precede end")
    if schedule.empty:
...
def load_execution_market(
    ...
    if _sha256(MARKET_DATA) != MARKET_DATA_SHA256:
        raise ValueError("LURI execution market differs from frozen SHA-256")
    market = pd.read_csv(
        MARKET_DATA,
        compression="gzip",
    ...
    if len(market) != market_manifest["rows"]:
        raise ValueError("LURI execution market row count differs from manifest")
    if market["date"].duplicated().any() or not market["date"].is_monotonic_increasing:
        raise ValueError("LURI execution timestamps are duplicate or unordered")
    if not market["date"].equals(signal_frame["date"]):
        raise ValueError("LURI execution market does not align with signal clock")
    prices = market[["open", "high", "low", "close"]].to_numpy(float)
    if not np.isfinite(prices).all() or (prices <= 0.0).any():
        raise ValueError("LURI execution market contains invalid prices")
    ...
    if (
        ...
        raise ValueError("LURI execution market violates OHLC invariants")
...
def load_realized_funding(
    ...
    if _sha256(FUNDING_DATA) != FUNDING_DATA_SHA256:
        raise ValueError("LURI realized funding differs from frozen SHA-256")
    funding = pd.read_csv(
        FUNDING_DATA,
    ...
    if list(funding.columns) != expected_columns:
        raise ValueError("LURI funding columns differ from freeze")
    if len(funding) != funding_manifest["data"]["rows"]:
        raise ValueError("LURI funding row count differs from manifest")
    ...
    if not funding_time.equals(funding_from_ms):
        raise ValueError("LURI funding UTC strings differ from epoch milliseconds")
    if funding["funding_time_ms"].duplicated().any() or not funding[
        "funding_time_ms"
    ].is_monotonic_increasing:
        raise ValueError("LURI funding timestamps are duplicate or unordered")
    if not funding["symbol"].eq("BTCUSDT").all():
        raise ValueError("LURI funding source contains a non-BTCUSDT symbol")
    rates = pd.to_numeric(funding["funding_rate"], errors="raise").to_numpy(float)
    if not np.isfinite(rates).all():
        raise ValueError("LURI funding source contains an invalid rate")
    if funding_time.max() >= luri.SELECTION_END:
        raise ValueError("LURI funding source opens the sealed interval")
    ...
    *,
    ...
    for row in schedule.itertuples(index=False):
        ...
        funding_debit_factor = float(
            np.prod(np.minimum(factors, 1.0), dtype=float)
        )
    ...
    return {
        ...
        "mean_funding_factor": (
            float(np.mean(funding_factors)) if funding_factors else 1.0
        ),
...
def run_evaluation(cfg: EvaluationConfig) -> dict[str, Any]:
    ...
    signal_cfg = luri.Config()
    signal_frame, signal_source = luri.load_causal_frame()
    ...
    return {
        "protocol": {
            "name": "LURI-48 frozen pre-2024 selection evaluation",
            "preregistration_commit": PREREGISTRATION_COMMIT,
            "support_commit": SUPPORT_COMMIT,
            "clock_commit": CLOCK_COMMIT,
            "funding_commit": FUNDING_COMMIT,
            ...
            "outcomes_opened_for_luri48": True,
            ...
            "control_side_rules": luri.CONTROL_SIDE_RULES,
            ...
            "exit": "scheduled USD-M open 48 completed 5m bars after entry",
            "funding_window": "entry_time <= funding_time <= exit_time",
            "trade_multiplier": (
                "(1-0.0003)*(1+0.5*r)*"
                "product(1-0.5*side*funding_rate)*(1-0.0003)"
            ),
            "strict_mdd": (
        ...
        "selection": {
            "selected_alpha": "luri48" if qualification["qualifies"] else None,
            "rejected": not qualification["qualifies"],
            "reason": (
                "passed every frozen pre-2024 gate"
                if qualification["qualifies"]
                else "LURI-48 failed at least one frozen pre-2024 gate"
            ),
        },
    }
