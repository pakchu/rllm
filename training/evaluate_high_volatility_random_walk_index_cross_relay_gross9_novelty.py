"""Evaluate frozen HVRWI-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVRWI-24"
PROTOCOL = "hvrwi_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_random_walk_index_cross_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVRWI predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVRWI novelty limits drift")
    if (
        ...
        raise RuntimeError("HVRWI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVRWI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvrwi_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVRWI-24 unchanged before economic outcomes",
    }
