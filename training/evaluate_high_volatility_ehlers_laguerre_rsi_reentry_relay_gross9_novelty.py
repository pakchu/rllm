"""Evaluate frozen HVELRSI-24 structural novelty against every Gross9 sleeve."""
...
OUTPUT = Path("results/high_volatility_ehlers_laguerre_rsi_reentry_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVELRSI novelty limits drift")
    if (
        ...
        raise RuntimeError("HVELRSI predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVELRSI primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVELRSI-24 unchanged before economic outcomes",
    }
