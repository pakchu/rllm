"""Evaluate frozen HVPZO-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVPZO-6"
PROTOCOL = "hvpzo_6_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_price_zone_oscillator_zero_cross_relay_preregistration_2026-08-11.json"
)
PREREG_SHA = "b093498375e36a2b91b375a1cccefff4c4d9fedd47cb3df65f155ca2b76daac4"
SUPPORT = Path("results/high_volatility_price_zone_oscillator_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "750d97b20711fd296a2aa414a24e6103bfcf91a354b40ad8ccfd8cc38a1cffca"
CLOCK = Path("data/high_volatility_price_zone_oscillator_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "4284b499f62b0815898d299fcd2d09731db6263e2658c56c7874b6c33d414e96"
OUTPUT = Path(
    "results/high_volatility_price_zone_oscillator_zero_cross_relay_gross9_novelty_2026-08-11.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVPZO predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVPZO novelty limits drift")
    if (
        ...
        raise RuntimeError("HVPZO predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVPZO primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvpzo_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVPZO-6 unchanged before economic outcomes"
        ),
    }
