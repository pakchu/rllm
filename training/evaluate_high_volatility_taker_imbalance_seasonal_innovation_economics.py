"""Sequential strict economics for frozen HVTISI-8."""
...
PREREG = Path("results/high_volatility_taker_imbalance_seasonal_innovation_preregistration_2026-08-13.json")
PREREG_SHA = "f134927a7dc62e3b1981cfe8dbeaf57d8cdb4937ee08d13a1fdd4fddd05c6b8f"
SUPPORT = Path("results/high_volatility_taker_imbalance_seasonal_innovation_support_2026-08-13.json")
SUPPORT_SHA = "e88b526a95360c59682962c5024c10853c2ce736d038fc5f191c9c234ec1bce9"
NOVELTY = Path("results/high_volatility_taker_imbalance_seasonal_innovation_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "a6c18816810617151913826628d36d51c4f8056ce5169b2496bd585a66619063"
CLOCK = Path("data/high_volatility_taker_imbalance_seasonal_innovation_clocks_2023_2026.csv.gz")
CLOCK_SHA = "14411e102623f0ff489e6d38359fe289fc47f11b48333857353c3d4eb68ac68e"
CONTROL_DIR = Path("data/high_volatility_taker_imbalance_seasonal_innovation_controls_2023_2026")
FREEZE = Path("results/high_volatility_taker_imbalance_seasonal_innovation_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_taker_imbalance_seasonal_innovation_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_innovation_tail", "no_variation_gate", "raw_net_flow_tail", "one_decision_stale_innovation", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTISI frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTISI novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvtisi_8_sequential_economics_v1", "policy_id": POLICY_ID,
