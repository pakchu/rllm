"""Build source-only HVRSOIR-12 clocks before opening outcomes or Gross9."""
...
from training import preregister_high_volatility_realized_skew_open_interest_sponsorship_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv

PREREG_SHA = "869f99368aabd274756bbee5d920ff60bee41274533f0cf58441527de0ff1300"
...
SNAPSHOT = Path("data/high_volatility_realized_skew_open_interest_sponsorship_relay_sources_2023_2026/realized_skew_daily.csv.gz")
CLOCK = Path("data/high_volatility_realized_skew_open_interest_sponsorship_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_realized_skew_open_interest_sponsorship_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_realized_skew_open_interest_sponsorship_relay_support_2026-08-16.json")
SPLITS = {
...
CLOCK_COLUMNS = ("candidate", "control", "split", "decision_time", "feature_available_time", "entry_time", "exit_time", "side", "oi_start", "oi_end", "oi_change", "realized_variation", "realized_skew", "variation_rank", "absolute_skew_rank", "raw_third_moment")
...
def load_combined_market() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    historical = historical[historical.date < pd.Timestamp("2026-06-02T00:00:00Z")].copy()
    ...
    if combined.date.duplicated().any() or not combined.date.diff().dropna().eq(pd.Timedelta(minutes=5)).all():
        raise RuntimeError("HVRSOIR combined market continuity drift")
...
def load_open_interest() -> pd.Series:
    from sqlalchemy import text
    query = text("SELECT ts,sum_open_interest FROM open_interest_binance WHERE symbol='BTCUSDT' AND period='5m' AND ts>=:start AND ts<:end ORDER BY ts")
    engine = postgres_engine()
    try:
        with engine.connect() as connection:
            frame = pd.read_sql_query(query, connection, params={"start": pd.Timestamp("2020-01-01T00:00:00Z"), "end": pd.Timestamp("2026-08-01T00:05:00Z")})
    finally:
        engine.dispose()
    frame["ts"] = pd.to_datetime(frame["ts"], utc=True, errors="raise"); frame["sum_open_interest"] = pd.to_numeric(frame["sum_open_interest"], errors="raise")
    if frame.ts.duplicated().any() or not np.isfinite(frame.sum_open_interest).all() or not frame.sum_open_interest.ge(0).all(): raise RuntimeError("HVRSOIR invalid OI source")
    return frame.set_index("ts")["sum_open_interest"].sort_index()


def score_snapshot(market: pd.DataFrame, oi: pd.Series) -> pd.DataFrame:
    ...
    decisions = pd.to_datetime(frame.decision_time, utc=True); frame["oi_start"] = (decisions-pd.Timedelta("24h")).map(oi); frame["oi_end"] = decisions.map(oi)
    positive = frame.oi_start.gt(0) & frame.oi_end.gt(0); frame["oi_change"] = np.nan; frame.loc[positive,"oi_change"] = np.log(frame.loc[positive,"oi_end"]/frame.loc[positive,"oi_start"])
    frame["source_valid"] = frame.source_valid & positive & np.isfinite(frame.oi_change)
...
def conditions(frame: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    if control == "one_day_stale_features":
        columns = ["oi_start", "oi_end", "oi_change", "realized_variation", "realized_skew", "raw_third_moment", "variation_rank", "absolute_skew_rank", "source_valid"]
    ...
    active = work.source_valid.eq(True) & np.isfinite(factor) & factor.ne(0) & work.oi_change.gt(0) & volatility & skew_tail
    side = np.sign(factor)
...
def build_clock(frame: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in frame.index[active]:
        ...
        if split is None:
            continue
        rows.append({"candidate": "HVRSOIR-12", "control": control, "split": split, "decision_time": decision, "feature_available_time": decision, "entry_time": entry, "exit_time": exit_time, "side": int(side.at[index]), "oi_start": float(frame.at[index,"oi_start"]), "oi_end": float(frame.at[index,"oi_end"]), "oi_change": float(frame.at[index,"oi_change"]), "realized_variation": float(frame.at[index, "realized_variation"]), "realized_skew": float(frame.at[index, "realized_skew"]), "variation_rank": float(frame.at[index, "variation_rank"]), "absolute_skew_rank": float(frame.at[index, "absolute_skew_rank"]), "raw_third_moment": float(frame.at[index, "raw_third_moment"])})
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVRSOIR preregistration drift")
    registration = json.loads(prereg.DEFAULT_OUTPUT.read_text())
    market, source = load_combined_market()
    oi = load_open_interest()
    scores = score_snapshot(market, oi)
