"""Sequential strict economics for frozen CBSTR-6."""
...
PREREG = Path("results/cboe_broad_slope_crypto_volatility_transmission_relay_preregistration_2026-08-08.json")
PREREG_SHA = "4482deac73948d227d177a2154c02d6e1a03fd25cf8b9a6ae35cccd15558ee38"
SUPPORT = Path("results/cboe_broad_slope_crypto_volatility_transmission_relay_support_2026-08-08.json")
SUPPORT_SHA = "b3d4d23539456a224a7ea4d798253d4ebf3da95958ad6e44c58b7fd580847102"
NOVELTY = Path("results/cboe_broad_slope_crypto_volatility_transmission_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "fe2e367e5230e3ced74f9b6dde8e18b981141d047c32d5c0f09d8f32c5acf489"
CLOCK = Path("data/cboe_broad_slope_crypto_volatility_transmission_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "4f7bba89ba4b3de5187037b7fe708f2d994356b4a0d90564026dde84c6054636"
CONTROL_DIR = Path("data/cboe_broad_slope_crypto_volatility_transmission_relay_controls_2023_2026")
FREEZE = Path("results/cboe_broad_slope_crypto_volatility_transmission_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/cboe_broad_slope_crypto_volatility_transmission_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
CONTROLS = ("no_crypto_confirmation", "bvol_only_confirmation", "dvol_only_confirmation", "one_session_stale_broad_slope_change", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("CBSTR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("CBSTR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "cbstr_6_sequential_economics_v1", "policy_id": POLICY_ID,
