"""Sequential strict economics for frozen HVCKS-24."""
...
PREREG = Path("results/high_volatility_chande_kroll_stop_breakout_relay_preregistration_2026-08-11.json")
PREREG_SHA = "a850b9a0145d4bde0bca8c16202de35fe5ac09af3a53646e0ad42e12a2963485"
SUPPORT = Path("results/high_volatility_chande_kroll_stop_breakout_relay_support_2026-08-11.json")
SUPPORT_SHA = "fd7423ebd6c6e5a5378f14c344cf2a39b8d2173bf72221afa698232bc8b33455"
NOVELTY = Path("results/high_volatility_chande_kroll_stop_breakout_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "ef27ebb21e4a791b9ddac8bbd67f83286b92bf000ee466f070e9a5429fe90a8c"
CLOCK = Path("data/high_volatility_chande_kroll_stop_breakout_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "24cfce2a9776cbd9831a7b1599b603d1ac30335a26075f4d23bc58625bff6a1b"
CONTROL_DIR = Path("data/high_volatility_chande_kroll_stop_breakout_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_chande_kroll_stop_breakout_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_chande_kroll_stop_breakout_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "preliminary_stop_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCKS frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCKS novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcks_24_sequential_economics_v1", "policy_id": POLICY_ID,
