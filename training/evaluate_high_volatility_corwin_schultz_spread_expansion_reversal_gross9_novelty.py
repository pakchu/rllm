"""Evaluate frozen HVCSER-12 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCSER-12"
PROTOCOL = "hvcser_12_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_corwin_schultz_spread_expansion_reversal_gross9_novelty_2026-08-10.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCSER predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCSER novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCSER predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCSER primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCSER-12 unchanged before economic outcomes",
    }
