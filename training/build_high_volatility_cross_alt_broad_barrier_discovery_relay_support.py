"""Build outcome-blind source support for frozen HVCABBD-8."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_cross_alt_broad_barrier_discovery_relay as prereg
ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2023-05-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="1535319ea5dd66239e814649f82901b0d87b84cb923f8a189e4550c02bd3902c";REG=prereg.build();P=REG["policy"];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG["stages"].items()};GATES=REG["source_support_gates"];CONTROLS=tuple(REG["diagnostic_controls"]["names"]);SYMBOLS=("BTCUSDT",*prereg.ALTS);ALTS=prereg.ALTS
...
ROOT=Path("data/high_volatility_cross_alt_broad_barrier_discovery_relay_sources_2023_2026");PANEL=ROOT/"five_minute_barrier_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_cross_alt_broad_barrier_discovery_relay_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_cross_alt_broad_barrier_discovery_relay_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_cross_alt_broad_barrier_discovery_relay_controls_2023_2026");RESULT=Path("results/high_volatility_cross_alt_broad_barrier_discovery_relay_support_2026-08-13.json");BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLS=("decision_time","feature_available_time","source_valid","discovery_side","up_discovery_count","down_discovery_count","high_low_side","high_low_up_count","high_low_down_count","btc_realized_variation","variation_rank","variation_active","eligible")
CLOCK_COLS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","up_discovery_count","down_discovery_count","btc_realized_variation","variation_rank")
...
def prepare(raw:pd.DataFrame)->pd.DataFrame:
 ...
 if raw.columns.tolist()!=expected:raise RuntimeError("HVCABBD source schema drift")
 ...
 if x.date.isna().any() or x.duplicated(["date","symbol"]).any() or not x.symbol.isin(SYMBOLS).all():raise RuntimeError("HVCABBD invalid source key")
...
def discovery_side(up:pd.DataFrame,down:pd.DataFrame,breadth:int):
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 x=prepare(raw);grid=pd.date_range(START,END,freq="5min",inclusive="left");close=x.pivot(index="date",columns="symbol",values="close").reindex(index=grid,columns=SYMBOLS);high=x.pivot(index="date",columns="symbol",values="high").reindex(index=grid,columns=SYMBOLS);low=x.pivot(index="date",columns="symbol",values="low").reindex(index=grid,columns=SYMBOLS);sq=x.pivot(index="date",columns="symbol",values="minute_squared_return").reindex(index=grid,columns=SYMBOLS);valid=x.pivot(index="date",columns="symbol",values="row_valid").reindex(index=grid,columns=SYMBOLS).eq(True);alts=close.loc[:,list(ALTS)];prior_high=alts.shift(1).rolling(P["barrier_bars"],min_periods=P["barrier_bars"]).max();prior_low=alts.shift(1).rolling(P["barrier_bars"],min_periods=P["barrier_bars"]).min();history_valid=valid.loc[:,list(ALTS)].shift(1).rolling(P["barrier_bars"],min_periods=P["barrier_bars"]).sum().eq(P["barrier_bars"]);up=alts.gt(prior_high)&history_valid&valid.loc[:,list(ALTS)];down=alts.lt(prior_low)&history_valid&valid.loc[:,list(ALTS)];side,pos,neg=discovery_side(up,down,P["minimum_discovery_breadth"]);hl_up=high.loc[:,list(ALTS)].gt(prior_high)&history_valid&valid.loc[:,list(ALTS)];hl_down=low.loc[:,list(ALTS)].lt(prior_low)&history_valid&valid.loc[:,list(ALTS)];hl_side,hl_pos,hl_neg=discovery_side(hl_up,hl_down,P["minimum_discovery_breadth"]);variation=np.sqrt(sq.BTCUSDT.where(valid.BTCUSDT).rolling(P["barrier_bars"],min_periods=P["barrier_bars"]).sum());vrank=strict_prior_midrank(variation,P["variation_history_decisions"],P["minimum_variation_history_decisions"]);source_valid=valid.all(axis=1)&history_valid.all(axis=1)&variation.notna()&vrank.notna();vactive=vrank.ge(P["variation_rank_min"]);broad=side.ne(0);prior_valid=source_valid.shift(1,fill_value=False);prior_broad=broad.shift(1,fill_value=False);eligible=source_valid&prior_valid&broad&~prior_broad&vactive
 return pd.DataFrame({"decision_time":grid+pd.Timedelta("5m"),"feature_available_time":grid+pd.Timedelta("5m"),"source_valid":source_valid.to_numpy(bool),"discovery_side":side.to_numpy(int),"up_discovery_count":pos.to_numpy(int),"down_discovery_count":neg.to_numpy(int),"high_low_side":hl_side.to_numpy(int),"high_low_up_count":hl_pos.to_numpy(int),"high_low_down_count":hl_neg.to_numpy(int),"btc_realized_variation":variation.to_numpy(float),"variation_rank":vrank.to_numpy(float),"variation_active":vactive.fillna(False).to_numpy(bool),"eligible":eligible.to_numpy(bool)},columns=PANEL_COLS)
def active(panel:pd.DataFrame,control="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 u=panel.copy();side=pd.to_numeric(u.discovery_side,errors="coerce").fillna(0).astype(int);broad=side.ne(0);prior_valid=u.source_valid.shift(1,fill_value=False);state=u.source_valid.eq(True)&prior_valid&broad&~broad.shift(1,fill_value=False)&u.variation_active.eq(True)
 if control=="no_variation_gate":state=u.source_valid.eq(True)&prior_valid&broad&~broad.shift(1,fill_value=False)
 elif control=="three_of_six_discovery":
  pos=u.up_discovery_count;neg=u.down_discovery_count;side=pd.Series(0,index=u.index,dtype=int);side.loc[pos.ge(3)&neg.eq(0)]=1;side.loc[neg.ge(3)&pos.eq(0)]=-1;broad=side.ne(0);state=u.source_valid.eq(True)&prior_valid&broad&~broad.shift(1,fill_value=False)&u.variation_active.eq(True)
 elif control=="current_bar_high_low_break":
  side=pd.to_numeric(u.high_low_side,errors="coerce").fillna(0).astype(int);broad=side.ne(0);state=u.source_valid.eq(True)&prior_valid&broad&~broad.shift(1,fill_value=False)&u.variation_active.eq(True)
 elif control=="one_bar_stale_discovery":
  cols=["discovery_side","up_discovery_count","down_discovery_count","feature_available_time"];u[cols]=panel[cols].shift(1);side=pd.to_numeric(u.discovery_side,errors="coerce").fillna(0).astype(int);broad=side.ne(0);state=u.source_valid.eq(True)&prior_valid&broad&~broad.shift(1,fill_value=False)&u.variation_active.eq(True)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCABBD prereg drift")
