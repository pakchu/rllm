"""Sequential strict economics for frozen HVQHLF-4."""
...
PREREG = Path("results/high_volatility_quarter_hour_lagged_flow_relay_preregistration_2026-08-13.json")
PREREG_SHA = "130da7294898237b9033ab6fee41bc57f63a9a4507a987f5c24990436be0aa06"
SUPPORT = Path("results/high_volatility_quarter_hour_lagged_flow_relay_support_2026-08-13.json")
SUPPORT_SHA = "09b000b4f1a678956bdd605e9e00fa35337d63e92ad37731b43c55f560e29a86"
NOVELTY = Path("results/high_volatility_quarter_hour_lagged_flow_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "cc369a980f8c4d5fdc4f713bc235b93791ff8977996f8039d336119038099e2f"
CLOCK = Path("data/high_volatility_quarter_hour_lagged_flow_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "22432585e6ce24ebfcde66438035a61fbd23238824bc2e0fff17d83ac2ac9292"
CONTROL_DIR = Path("data/high_volatility_quarter_hour_lagged_flow_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_quarter_hour_lagged_flow_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_quarter_hour_lagged_flow_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("raw_current_opening_imbalance", "include_ols_intercept", "no_flow_strength_tail", "no_variation_gate", "shifted_phase_plus_2m", "one_quarter_stale_prediction", "direction_flip", "same_clock_forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVQHLF frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVQHLF novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvqhlf_4_sequential_economics_v1", "policy_id": POLICY_ID,
