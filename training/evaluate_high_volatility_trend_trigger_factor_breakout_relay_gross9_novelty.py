"""Evaluate frozen HVTTF-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVTTF-24"
PROTOCOL = "hvttf_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_trend_trigger_factor_breakout_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVTTF predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVTTF novelty limits drift")
    if (
        ...
        raise RuntimeError("HVTTF predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVTTF primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvttf_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVTTF-24 unchanged before economic outcomes",
    }
