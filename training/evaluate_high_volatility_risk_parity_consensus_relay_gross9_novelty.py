"""Evaluate frozen HVRPC-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVRPC-24"
PROTOCOL = "hvrpc_24_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_risk_parity_consensus_relay_preregistration_2026-08-10.json"
)
...
OUTPUT = Path(
    "results/high_volatility_risk_parity_consensus_relay_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVRPC predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVRPC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVRPC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVRPC primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvrpc_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVRPC-24 unchanged before economic outcomes"
        ),
    }
