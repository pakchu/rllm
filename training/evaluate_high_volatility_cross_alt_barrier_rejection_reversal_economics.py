"""Sequential strict economics for frozen HVCABRR-8."""
...
PREREG = Path("results/high_volatility_cross_alt_barrier_rejection_reversal_preregistration_2026-08-13.json")
PREREG_SHA = "3da9c8a42036ba3dadc9553ed350018ab71f560dcbc411ce056901433bc2e0ac"
SUPPORT = Path("results/high_volatility_cross_alt_barrier_rejection_reversal_support_2026-08-13.json")
SUPPORT_SHA = "a378e06ce2deaf6fe4be03d98a1dd860e03a13122dfe49872a7b0cb712fa08c1"
NOVELTY = Path("results/high_volatility_cross_alt_barrier_rejection_reversal_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "bea85ae1bbd2fb8e511236112f7fa090504e4d251039949da58366224d92d5f9"
CLOCK = Path("data/high_volatility_cross_alt_barrier_rejection_reversal_clocks_2023_2026.csv.gz")
CLOCK_SHA = "f75a5b693d27a6db133bf73708fd2c72364de5583bef858e892e678c25941fa0"
CONTROL_DIR = Path("data/high_volatility_cross_alt_barrier_rejection_reversal_controls_2023_2026")
FREEZE = Path("results/high_volatility_cross_alt_barrier_rejection_reversal_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_alt_barrier_rejection_reversal_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "three_of_six_rejection", "close_outside_barrier", "one_bar_stale_rejection", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCABRR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCABRR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcabrr_8_sequential_economics_v1", "policy_id": POLICY_ID,
