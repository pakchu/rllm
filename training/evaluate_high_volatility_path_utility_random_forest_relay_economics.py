"""Sequential strict economics for frozen HVPRF-48."""
...
PREREG = Path("results/high_volatility_path_utility_random_forest_relay_preregistration_2026-08-09.json")
PREREG_SHA = "fa99c46a6811985cacd516c45e7f5b4c9d1f3f19a132a2ce7962a89851096b5a"
SUPPORT = Path("results/high_volatility_path_utility_random_forest_relay_support_2026-08-09.json")
SUPPORT_SHA = "583f3a71b41b31cbd096c5f278e354cb8ccea65500fa77f12a0a4d2a802982b8"
NOVELTY = Path("results/high_volatility_path_utility_random_forest_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "a8e79694b4cef8c3a0841c794be06bf5ae7ffa99094d51467bb3a6ea73014500"
CLOCK = Path("data/high_volatility_path_utility_random_forest_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "f9e339c89a4b106ae1fdd1e23470aeb2f9c660fa3aaf9b8b1e80fd39550a896d"
CONTROL_DIR = Path("data/high_volatility_path_utility_random_forest_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_path_utility_random_forest_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_path_utility_random_forest_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVPRF frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVPRF novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvprf_48_sequential_economics_v1", "policy_id": POLICY_ID,
