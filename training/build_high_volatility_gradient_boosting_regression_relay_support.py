"""Build OOS source-only HVGBR-72 clocks after the model freeze."""
...
PREREG_SHA = "651f45d5ab1b4b45a4177e745b60e148f27db060dafba4db098b0c02cd600ee2"
MODEL_FREEZE = Path(
    "results/high_volatility_gradient_boosting_regression_relay_model_freeze_2026-08-09.json"
)
MODEL_FREEZE_SHA = "d8ead12b66c3589cf8af2cdb10b01a8583f3b11958faffac5d331ca4de496000"
MODEL = Path("data/high_volatility_gradient_boosting_regression_relay_model_2026-08-09.joblib")
MODEL_SHA = "0840d6754c01844242283fa3ceacd245f01436676bbb5cb098e8d6809fddd674"
CLOCK = Path("data/high_volatility_gradient_boosting_regression_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_gradient_boosting_regression_relay_controls_2023_2026")
SNAPSHOT = Path(
    "data/high_volatility_gradient_boosting_regression_relay_sources_2023_2026/predictions.csv.gz"
)
RESULT = Path(
    "results/high_volatility_gradient_boosting_regression_relay_support_2026-08-09.json"
)
...
CONTROLS = ("no_volatility_gate", "no_prediction_tail_gate", "one_anchor_stale_features", "direction_flip")
...
def verify_predecessors() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVGBR preregistration drift")
    if sha256(MODEL_FREEZE) != MODEL_FREEZE_SHA or sha256(MODEL) != MODEL_SHA:
        raise RuntimeError("HVGBR model predecessor drift")
    ...
    if freeze.get("manifest_hash") != canonical_hash(freeze_core):
        raise RuntimeError("HVGBR model freeze manifest drift")
    if (
        ...
        raise RuntimeError("HVGBR model freeze state drift")
    artifact = joblib.load(MODEL)
    if artifact.get("policy_id") != "HVGBR-72":
        raise RuntimeError("HVGBR model policy drift")
...
def load_combined_market() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if combined.date.duplicated().any() or not combined.date.is_monotonic_increasing:
        raise RuntimeError("HVGBR combined market time drift")
    deltas = combined.date.diff().dropna()
    if not deltas.eq(pd.Timedelta(minutes=5)).all():
        raise RuntimeError("HVGBR combined market is not continuous 5m")
    return combined, {
...
def score_snapshot(market: pd.DataFrame, artifact: dict[str, Any]) -> pd.DataFrame:
    ...
    if missing:
        raise RuntimeError(f"HVGBR OOS feature drift: {missing}")
    ...
    predictions = artifact["pipeline"].predict(matrix[positions])
    ...
    return pd.DataFrame(
        {
            ...
            "predicted_return_72h": predictions,
            "range_vol": range_vol,
        }
    )
...
def build_clock(scores: pd.DataFrame, artifact: dict[str, Any], control: str = "primary") -> pd.DataFrame:
    ...
    if control == "one_anchor_stale_features":
        frame[["predicted_return_72h", "range_vol"]] = frame[["predicted_return_72h", "range_vol"]].shift(1)
    prediction = pd.to_numeric(frame.predicted_return_72h, errors="coerce").to_numpy(float)
    volatility = pd.to_numeric(frame.range_vol, errors="coerce").to_numpy(float)
    if control == "no_prediction_tail_gate":
        long_signal = prediction >= 0.0
        short_signal = prediction < 0.0
    else:
        long_signal = prediction >= float(artifact["prediction_high"])
        short_signal = prediction <= float(artifact["prediction_low"])
    ...
    finite = np.isfinite(prediction) & np.isfinite(volatility)
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvgbr_72_oos_source_support_v1",
        "policy_id": "HVGBR-72",
