"""Sequential strict economics for frozen HVRSAR-8."""
...
PREREG = Path("results/high_volatility_rogers_satchell_excursion_asymmetry_reversal_relay_preregistration_2026-08-11.json")
PREREG_SHA = "4146c2d7e83ea4fd05a82ef1ab61ccd03a87e39c8c55ce989d90334e2b6dd3a4"
SUPPORT = Path("results/high_volatility_rogers_satchell_excursion_asymmetry_reversal_relay_support_2026-08-11.json")
SUPPORT_SHA = "79c1b9d092087e49aadc85dc9b156006a286fee98431484daf24b82773a9b046"
NOVELTY = Path("results/high_volatility_rogers_satchell_excursion_asymmetry_reversal_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "45b443409b08c3631aab67bd792acf2c75703572decd00b55a1626c135754e1f"
CLOCK = Path("data/high_volatility_rogers_satchell_excursion_asymmetry_reversal_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "49373be1051a4398889f2b8314affba51b53daa4eca2493b5869c4d446979537"
CONTROL_DIR = Path("data/high_volatility_rogers_satchell_excursion_asymmetry_reversal_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_rogers_satchell_excursion_asymmetry_reversal_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_rogers_satchell_excursion_asymmetry_reversal_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = (
    "no_asymmetry_tail",
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVRSAR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVRSAR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvrsar_8_sequential_economics_v1", "policy_id": POLICY_ID,
