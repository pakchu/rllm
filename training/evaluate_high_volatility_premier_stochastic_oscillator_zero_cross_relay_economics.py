"""Sequential strict economics for frozen HVPSO-Z14-3-24."""
...
PREREG = Path("results/high_volatility_premier_stochastic_oscillator_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "3230f687b66df53075ef44de3bd93d2d09c156e42a77c035584b759ecff5cb66"
SUPPORT = Path("results/high_volatility_premier_stochastic_oscillator_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "e566343895abc9cdaf5351c555973a97cba59c67c600857d5ad57d2a23798a6f"
NOVELTY = Path("results/high_volatility_premier_stochastic_oscillator_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "f68d16704a12748469d420574c1980f614b78632a801efd7a52b743686293ee9"
CLOCK = Path("data/high_volatility_premier_stochastic_oscillator_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "2e3ee9f3e4a1ed77d35814fa33cf4298faee6081e603267220cf1e7ac480b4f7"
CONTROL_DIR = Path("data/high_volatility_premier_stochastic_oscillator_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_premier_stochastic_oscillator_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_premier_stochastic_oscillator_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "raw_fast_k_midline", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVPSO frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVPSO novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvpso_z14_3_24_sequential_economics_v1", "policy_id": POLICY_ID,
