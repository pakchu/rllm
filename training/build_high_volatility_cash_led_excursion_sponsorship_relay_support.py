"""Build outcome-blind source support for frozen HVCLES-8."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_cash_led_excursion_sponsorship_relay as prereg
ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2023-05-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="c93f6298f23c2fa79f7f40e715255ffccd0ea2d8c49af1f9d9679486c82b4285";REG=prereg.build();P=REG["policy"];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG["stages"].items()};GATES=REG["source_support_gates"];CONTROLS=tuple(REG["diagnostic_controls"]["names"]);VENUES=("perpetual","spot")
...
ROOT=Path("data/high_volatility_cash_led_excursion_sponsorship_relay_sources_2023_2026");PANEL=ROOT/"five_minute_barrier_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_cash_led_excursion_sponsorship_relay_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_cash_led_excursion_sponsorship_relay_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_cash_led_excursion_sponsorship_relay_controls_2023_2026");RESULT=Path("results/high_volatility_cash_led_excursion_sponsorship_relay_support_2026-08-13.json");BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLS=("decision_time","feature_available_time","source_valid","rejection_side","upper_excess","lower_excess","upper_excess_rank","lower_excess_rank","perpetual_return","spot_return","btc_realized_variation","variation_rank","variation_active","eligible")
CLOCK_COLS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","upper_excess","lower_excess","upper_excess_rank","lower_excess_rank","btc_realized_variation","variation_rank")
...
def prepare(raw:pd.DataFrame)->pd.DataFrame:
 ...
 if raw.columns.tolist()!=expected:raise RuntimeError("HVCLES source schema drift")
 ...
 if x.date.isna().any() or x.duplicated(["date","venue"]).any() or not x.venue.isin(("perpetual","spot")).all():raise RuntimeError("HVCLES invalid source key")
...
def directional_sponsorship(upper:pd.Series,lower:pd.Series)->pd.Series:
 side=pd.Series(0,index=upper.index,dtype=int);side.loc[upper&~lower]=1;side.loc[lower&~upper]=-1;return side
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 x=prepare(raw);grid=pd.date_range(START,END,freq="5min",inclusive="left");op=x.pivot(index="date",columns="venue",values="open").reindex(index=grid,columns=["perpetual","spot"]);high=x.pivot(index="date",columns="venue",values="high").reindex(index=grid,columns=["perpetual","spot"]);low=x.pivot(index="date",columns="venue",values="low").reindex(index=grid,columns=["perpetual","spot"]);close=x.pivot(index="date",columns="venue",values="close").reindex(index=grid,columns=["perpetual","spot"]);sq=x.pivot(index="date",columns="venue",values="minute_squared_return").reindex(index=grid,columns=["perpetual","spot"]);valid=x.pivot(index="date",columns="venue",values="row_valid").reindex(index=grid,columns=["perpetual","spot"]).eq(True);joint=valid.all(axis=1);pret=np.log(close.perpetual/op.perpetual);sret=np.log(close.spot/op.spot);upper=np.log(high.spot/op.spot)-np.log(high.perpetual/op.perpetual);lower=np.log(op.spot/low.spot)-np.log(op.perpetual/low.perpetual);urank=strict_prior_midrank(upper.where(joint),P["history_decisions"],P["minimum_history_decisions"]);lrank=strict_prior_midrank(lower.where(joint),P["history_decisions"],P["minimum_history_decisions"]);up=urank.ge(P["excess_rank_min"])&pret.gt(0)&sret.gt(0);down=lrank.ge(P["excess_rank_min"])&pret.lt(0)&sret.lt(0);side=directional_sponsorship(up,down);variation=np.sqrt(sq.perpetual.where(valid.perpetual).rolling(P["variation_bars"],min_periods=P["variation_bars"]).sum());vrank=strict_prior_midrank(variation,P["history_decisions"],P["minimum_history_decisions"]);source_valid=joint&urank.notna()&lrank.notna()&variation.notna()&vrank.notna();vactive=vrank.ge(P["variation_rank_min"]);state=side.ne(0);prior_valid=source_valid.shift(1,fill_value=False);eligible=source_valid&prior_valid&state&~state.shift(1,fill_value=False)&vactive
 return pd.DataFrame({"decision_time":grid+pd.Timedelta("5m"),"feature_available_time":grid+pd.Timedelta("5m"),"source_valid":source_valid.to_numpy(bool),"rejection_side":side.to_numpy(int),"upper_excess":upper.to_numpy(float),"lower_excess":lower.to_numpy(float),"upper_excess_rank":urank.to_numpy(float),"lower_excess_rank":lrank.to_numpy(float),"perpetual_return":pret.to_numpy(float),"spot_return":sret.to_numpy(float),"btc_realized_variation":variation.to_numpy(float),"variation_rank":vrank.to_numpy(float),"variation_active":vactive.fillna(False).to_numpy(bool),"eligible":eligible.to_numpy(bool)},columns=PANEL_COLS)
def active(panel:pd.DataFrame,control="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 u=panel.copy();side=pd.to_numeric(u.rejection_side,errors="coerce").fillna(0).astype(int);state=side.ne(0);prior_valid=u.source_valid.shift(1,fill_value=False);active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)&u.variation_active.eq(True)
 if control=="no_variation_gate":active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)
 elif control in ("rank_0_99","no_close_confirmation"):
  upper=u.upper_excess_rank.ge(.99);lower=u.lower_excess_rank.ge(.99)
  if control=="rank_0_99":upper&=u.perpetual_return.gt(0)&u.spot_return.gt(0);lower&=u.perpetual_return.lt(0)&u.spot_return.lt(0)
  side=directional_sponsorship(upper,lower);state=side.ne(0);active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)&u.variation_active.eq(True)
 elif control=="one_bar_stale_sponsorship":
  cols=["rejection_side","upper_excess","lower_excess","upper_excess_rank","lower_excess_rank","feature_available_time"];u[cols]=panel[cols].shift(1);side=pd.to_numeric(u.rejection_side,errors="coerce").fillna(0).astype(int);state=side.ne(0);active=u.source_valid.eq(True)&prior_valid&state&~state.shift(1,fill_value=False)&u.variation_active.eq(True)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCLES prereg drift")
