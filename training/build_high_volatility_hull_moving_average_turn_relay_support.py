"""Build outcome-blind source support for frozen HVHMA-24."""
...
PREREG_SHA = "41d4ed869f5bb25484eedcd3127c09bd6711ac47bda4255cb971a5bab578d635"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_hull_moving_average_turn_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_hull_moving_average_turn_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_hull_moving_average_turn_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_hull_moving_average_turn_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_hull_moving_average_turn_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "half_wma", "full_wma", "raw_hull", "hma",
    "slope", "full_wma_slope", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "half_wma", "full_wma", "raw_hull", "hma",
    "slope", "full_wma_slope", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVHMA source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVHMA invalid source key")
...
def weighted_average(series: pd.Series, valid: pd.Series, periods: int) -> pd.Series:
    weights=np.arange(1,periods+1,dtype=float);denominator=float(weights.sum())
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["half_wma"]=weighted_average(bars.bar_close,bars.valid_bar,P["half_periods"]);bars["full_wma"]=weighted_average(bars.bar_close,bars.valid_bar,P["hma_periods"])
    bars["raw_hull"]=2*bars.half_wma-bars.full_wma;raw_valid=np.isfinite(bars.raw_hull)
    bars["hma"]=weighted_average(bars.raw_hull,raw_valid,P["sqrt_periods"]);bars["slope"]=bars.hma-bars.hma.shift(1);bars["full_wma_slope"]=bars.full_wma-bars.full_wma.shift(1)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum())
    bars["source_valid"]=bars.valid_bar&np.isfinite(bars[["half_wma","full_wma","raw_hull","hma","slope","full_wma_slope","variation"]]).all(axis=1)&bars.slope.ne(0)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="full_length_wma_turn":
        signal=pd.to_numeric(used.full_wma_slope,errors="coerce");previous=signal.shift(1);state=valid&signal.ne(0)&previous.ne(0)&np.sign(signal).ne(np.sign(previous))&variation;side=np.sign(signal).fillna(0).astype(int)
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVHMA prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvhma_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvhma_24_source_support_v1", "policy_id": prereg.POLICY_ID,
