"""Sequential strict economics for frozen HVTFR-8."""
...
PREREG = Path("results/high_volatility_time_price_trend_fit_relay_preregistration_2026-08-10.json")
...
NOVELTY = Path("results/high_volatility_time_price_trend_fit_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "890196f90cc3e9ffd58c4fe0d71dcee61681246b509e858c3f7591277a93e984"
...
FREEZE = Path("results/high_volatility_time_price_trend_fit_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_time_price_trend_fit_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_trend_fit_tail", "no_variation_gate", "raw_r_squared_at_least_half", "one_block_stale_geometry", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVTFR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVTFR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvtfr_8_sequential_economics_v1", "policy_id": POLICY_ID,
