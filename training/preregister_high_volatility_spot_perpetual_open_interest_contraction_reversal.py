"""Outcome-blind preregistration for HVSPOICR-8."""
...
POLICY_ID = "HVSPOICR-8"
DEFAULT_OUTPUT = Path("results/high_volatility_spot_perpetual_open_interest_contraction_reversal_preregistration_2026-08-16.json")
...
def build() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "high_volatility_spot_perpetual_open_interest_contraction_reversal_v1",
        "policy_id": POLICY_ID,
        ...
        "mechanism": {
            "claim": "When completed BTC spot and perpetual eight-hour returns agree, the perpetual contract overshoots spot in that common direction, realized variation is elevated, and open interest contracts, the excess perpetual displacement is liquidation-driven without fresh leveraged sponsorship and should reverse over eight hours.",
            "side": "opposite strict sign of the completed eight-hour BTC perpetual return",
            "why_distinct": "HVSPOICR requires an exact completed-range close location with exact OI contraction and variation. No prior composite event set or control is reused; it uses no spot, ETH, funding, premium-index, fitted outcome, or promoted control.",
            "why_suited_to_volatile_regimes": "the completed eight-hour BTC realized variation must be in its causal upper 40 percent",
            "why_low_gross9_overlap_is_plausible": "fixed eight-hour spot-perpetual overshoot plus OI-contraction exhaustion states are absent from Gross9 primitives",
        },
        "features": {
            ...
            "eligible": "exact coherent synchronized 480-row spot and perpetual cycles, strict direction agreement, strict directional perpetual overshoot, two exact OI endpoints with strict contraction, and the variation rank passes at D",
            "no_imputation": True,
        },
        "clock": {
        ...
        "research_boundary": {
            ...
            "exact_spot_perpetual_overshoot_OI_contraction_incidence_or_outcomes_known": False,
        ...
        "stopping_rule": "terminal first failure; no OI endpoint or contraction rule, spot-perpetual agreement or overshoot rule, variation history, side, clock, hold, subset, threshold, comparator, or control repair",
    }
