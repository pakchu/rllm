"""Source-only support gate for frozen HVLZC-8."""
...
from training import preregister_high_volatility_lempel_ziv_compressibility_relay as prereg
...
PREREG_SHA = "1aff82bc58760b8dfc8c14798c20085dc8e9387caaec71f478564855e6afda8b"
SPLITS = {
...
ROOT = Path("data/high_volatility_lempel_ziv_compressibility_sources_2023_2026")
PANEL = ROOT / "states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_lempel_ziv_compressibility_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_lempel_ziv_compressibility_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_lempel_ziv_compressibility_relay_support_2026-08-10.json")
...
def lz76_phrase_count(symbols: np.ndarray) -> int:
    encoded = bytes((np.asarray(symbols, dtype=np.int8) + 1).tolist())
    phrases = 0
    index = 0
    while index < len(encoded):
        length = 1
        prefix = encoded[:index]
        while index + length <= len(encoded) and encoded[index:index + length] in prefix:
            length += 1
        phrases += 1
...
def normalized_complexity(returns: np.ndarray) -> float:
    returns = np.asarray(returns, dtype=float)
    if len(returns) < 2 or not np.isfinite(returns).all():
        return math.nan
    symbols = np.sign(returns).astype(np.int8)
    value = lz76_phrase_count(symbols) * math.log(len(symbols)) / len(symbols)
    return float(value) if math.isfinite(value) and value > 0 else math.nan
...
def materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError("duplicate HVLZC source timestamps")
    ...
    for decision in pd.date_range(START.ceil("8h"), END, freq="8h", inclusive="left"):
        ...
        if valid:
            ...
            complexity = normalized_complexity(returns)
            ...
            valid = variation > 0 and math.isfinite(complexity)
        if not valid:
            variation = complexity = block_return = late_return = math.nan
        rows.append(
            {
    ...
    states["complexity_rank"] = prior_rank(states["normalized_complexity"].where(states["source_valid"]))
    ROOT.mkdir(parents=True, exist_ok=True)
    _write_gzip_csv(states, PANEL)
    core = {
        "protocol_version": "hvlzc_source_v1",
        "query": QUERY,
...
def active(states: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    complexity_rank = states["complexity_rank"]
    complexity = states["normalized_complexity"]
    if control == "one_block_stale_geometry":
        block_return, late_return, variation_rank, complexity_rank, complexity = [
            value.shift(1)
            for value in (block_return, late_return, variation_rank, complexity_rank, complexity)
        ]
    variation_gate = (
    ...
    if control == "no_complexity_tail":
        complexity_gate = pd.Series(True, index=states.index)
    elif control == "raw_below_unit_complexity":
        complexity_gate = complexity.lt(1.0)
    else:
        complexity_gate = complexity_rank.le(0.25)
    agreement = (
        block_return.ne(0)
        & late_return.ne(0)
        & np.sign(block_return).eq(np.sign(late_return))
    )
    eligible = states["source_valid"] & variation_gate & complexity_gate & agreement
        ...
        rows.append(
            {
                ...
                "normalized_complexity": float(states.at[index, "normalized_complexity"]),
                "complexity_rank": float(states.at[index, "complexity_rank"]),
            }
        )
    columns = [
        "candidate", "control", "split", "decision_time", "feature_available_time",
        "entry_time", "exit_time", "side", "realized_variation", "variation_rank",
        "normalized_complexity", "complexity_rank",
    ]
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVLZC preregistration drift")
    ...
    core = {
        "protocol_version": "hvlzc_8_source_support_v1",
        "policy_id": "HVLZC-8",
        "preregistration": {
