"""Build outcome-blind source support for frozen HVCAPO-8."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_cross_alt_path_ordered_excursion_dominance_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2023-01-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="c8359f6523d93e5ccb957223501dd909ad16fd0c42aabc915532b5cf20d3ba64";REG=prereg.build();P=REG["policy"];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG["stages"].items()};GATES=REG["source_support_gates"];CONTROLS=tuple(REG["diagnostic_controls"]["names"]);SYMBOLS=("BTCUSDT",*prereg.ALTS);ALTS=prereg.ALTS
...
ROOT=Path("data/high_volatility_cross_alt_path_ordered_excursion_dominance_relay_sources_2023_2026");PANEL=ROOT/"scheduled_path_excursion_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_cross_alt_path_ordered_excursion_dominance_relay_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_cross_alt_path_ordered_excursion_dominance_relay_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_cross_alt_path_ordered_excursion_dominance_relay_controls_2023_2026");RESULT=Path("results/high_volatility_cross_alt_path_ordered_excursion_dominance_relay_support_2026-08-13.json");BUILDER=Path(__file__).relative_to(Path.cwd())
BASE_COLS=("decision_time","feature_available_time","source_valid","asymmetry_score","selected_alts","agreeing_alts","side","btc_realized_variation","variation_rank","variation_active","eligible")
CLOCK_COLS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","asymmetry_score","selected_alts","agreeing_alts","btc_realized_variation","variation_rank")
...
def prepare(raw):
 ...
 if raw.columns.tolist()!=expected:raise RuntimeError("HVCAPO source schema drift")
...
def asymmetry_frame(ret,close,valid):
 ...
 for decision in scheduled:
  ...
  for a in ALTS:
   path=np.log(close[a].loc[:end].tail(P["path_bars"]).to_numpy(float))
   if len(path)!=P["path_bars"] or not np.isfinite(path).all():row[f"{a}_asymmetry"]=np.nan;continue
   drawup=float(np.max(path-np.minimum.accumulate(path)));drawdown=float(np.max(np.maximum.accumulate(path)-path));den=drawup+drawdown
   row[f"{a}_asymmetry"]=(drawup-drawdown)/den if den>0 and drawup!=drawdown else np.nan
  rows.append(row)
 return pd.DataFrame(rows)
def score_states(f,control="primary"):
 u=f.copy()
 for a in ALTS:
  u[f"{a}_asymmetry_rank"]=midrank(u[f"{a}_asymmetry"].abs().where(u.source_valid),P["asymmetry_prior_decisions"],P["asymmetry_minimum_decisions"])
 ...
 for i,row in u.iterrows():
  chosen=[a for a in ALTS if math.isfinite(row[f"{a}_asymmetry"]) and (control=="no_asymmetry_tail" or row[f"{a}_asymmetry_rank"]>=P["asymmetry_rank_min"])]
  signs={a:int(np.sign(row[f"{a}_asymmetry"])) for a in chosen};weights={a:1. if control=="equal_weighted_score" else abs(row[f"{a}_asymmetry"]) for a in chosen};den=sum(weights.values());score=sum(weights[a]*signs[a] for a in chosen)/den if den>0 else np.nan;sgn=int(np.sign(score)) if math.isfinite(score) else 0;scores.append(score);selected.append(len(chosen));agree.append(sum(v==sgn for v in signs.values()) if sgn else 0)
 u["asymmetry_score"]=scores;u["selected_alts"]=selected;u["agreeing_alts"]=agree;minimum=3 if control=="three_alt_consensus" else P["minimum_selected_alts"];u["side"]=np.sign(u.asymmetry_score).fillna(0).astype(int);u["variation_active"]=u.variation_rank.ge(P["variation_rank_min"]);u["eligible"]=u.source_valid&u.selected_alts.ge(minimum)&u.asymmetry_score.abs().ge(P["score_absolute_min"])&u.agreeing_alts.ge(P["minimum_agreeing_alts"])&u.variation_active
 if control=="no_variation_gate":u["eligible"]=u.source_valid&u.selected_alts.ge(minimum)&u.asymmetry_score.abs().ge(P["score_absolute_min"])&u.agreeing_alts.ge(P["minimum_agreeing_alts"])
 if control=="one_decision_stale_consensus":
  for c in ("asymmetry_score","selected_alts","agreeing_alts","side"):u[c]=u[c].shift(1)
  u["eligible"]=u.source_valid&u.selected_alts.ge(minimum)&u.asymmetry_score.abs().ge(P["score_absolute_min"])&u.agreeing_alts.ge(P["minimum_agreeing_alts"])&u.variation_active
 if control=="direction_flip":u.side=-u.side
 if control=="forced_long":u.side=1
 return u
def build_panel(raw,control="primary"):
 x=prepare(raw);grid=pd.date_range(START,END,freq="5min",inclusive="left");ret=x.pivot(index="date",columns="symbol",values="return").reindex(grid,columns=SYMBOLS);close=x.pivot(index="date",columns="symbol",values="close").reindex(grid,columns=SYMBOLS);valid=x.pivot(index="date",columns="symbol",values="valid").reindex(grid,columns=SYMBOLS).eq(True);return score_states(asymmetry_frame(ret,close,valid),control)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCAPO prereg drift")
