"""Build outcome-blind source support for frozen HVTCCR-8."""
...
PREREG_SHA="b3fd974ae3ba804679e63d30ea02ee6d0ad3246981e80b7308242d09d19e3d26"; REG=prereg.build(); P=REG["policy"]
...
ROOT=Path("data/high_volatility_quote_turnover_concentration_continuation_relay_sources_2023_2026"); PANEL=ROOT/"block_states.csv.gz"; MANIFEST=ROOT/"manifest.json"
CLOCK=Path("data/high_volatility_quote_turnover_concentration_continuation_relay_clocks_2023_2026.csv.gz"); SPLIT_DIR=Path("data/high_volatility_quote_turnover_concentration_continuation_relay_split_clocks_2023_2026"); CONTROL_DIR=Path("data/high_volatility_quote_turnover_concentration_continuation_relay_controls_2023_2026")
RESULT=Path("results/high_volatility_quote_turnover_concentration_continuation_relay_support_2026-08-10.json"); BUILDER=Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS=("decision_time","feature_available_time","source_valid","minute_count","turnover_hhi","concentration_rank","realized_variation","variation_rank","quote_weighted_return","completed_return","eligible","onset")
CLOCK_COLUMNS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","turnover_hhi","concentration_rank","realized_variation","variation_rank","quote_weighted_return","completed_return")
...
def turnover_statistics(block:pd.DataFrame)->tuple[float,float,float,float]:
 ...
 total=float(five.quote.sum())
 if total<=0:return (math.nan,)*4
 returns=np.log(five.close.to_numpy(float)/five.open.to_numpy(float));shares=five.quote.to_numpy(float)/total
 hhi=float(np.square(shares).sum());variation=float(np.sqrt(np.square(returns).sum()));weighted=float(np.dot(shares,returns));completed=float(math.log(block.close.iloc[-1]/block.open.iloc[0]))
 return hhi,variation,weighted,completed
...
def prepare(frame:pd.DataFrame)->pd.DataFrame:
 if frame.columns.tolist()!=["ts","open","high","low","close","quote_asset_volume"]:raise RuntimeError("HVTCCR source schema drift")
 ...
 if x.ts.isna().any() or x.ts.duplicated().any():raise RuntimeError("HVTCCR invalid source key")
...
def build_panel(raw:pd.DataFrame)->pd.DataFrame:
 ...
 for decision in pd.date_range(START+pd.Timedelta("8h"),END,freq="8h",inclusive="left"):
  ...
  if path_valid:
   hhi,var,weighted,ret=turnover_statistics(block);valid=all(math.isfinite(v) for v in (hhi,var,weighted,ret)) and var>0 and weighted!=0 and ret!=0
  else:hhi=var=weighted=ret=math.nan;valid=False
  rows.append({"decision_time":decision,"feature_available_time":decision,"source_valid":valid,"minute_count":count,"turnover_hhi":hhi,"realized_variation":var,"quote_weighted_return":weighted,"completed_return":ret})
 panel=pd.DataFrame(rows);valid=panel.source_valid.eq(True);panel["concentration_rank"]=prior_rank(panel.turnover_hhi.where(valid));panel["variation_rank"]=prior_rank(panel.realized_variation.where(valid));agreement=np.sign(panel.quote_weighted_return)==np.sign(panel.completed_return);panel["eligible"]=valid&panel.concentration_rank.ge(P["concentration_rank_min"])&panel.variation_rank.ge(P["variation_rank_min"])&agreement;panel["onset"]=previous_valid_onset(panel.eligible,valid);return panel.loc[:,PANEL_COLUMNS]

def active(panel:pd.DataFrame,control:str="primary"):
 if control not in ("primary",*CONTROLS):raise ValueError(control)
 used=panel.copy()
 if control=="one_boundary_stale_concentration":
  cols=["source_valid","concentration_rank","variation_rank","quote_weighted_return","completed_return","feature_available_time"];used[cols]=panel[cols].shift(1)
 valid=used.source_valid.eq(True);concentrated=used.concentration_rank.ge(P["concentration_rank_min"]);variation=used.variation_rank.ge(P["variation_rank_min"]);agreement=np.sign(used.quote_weighted_return)==np.sign(used.completed_return);state=valid&concentrated&variation&agreement
 if control=="no_concentration_tail":state=valid&variation&agreement
 elif control=="no_variation_gate":state=valid&concentrated&agreement
 onset=previous_valid_onset(state,valid);side=np.sign(pd.to_numeric(used.quote_weighted_return,errors="coerce").fillna(0)).astype(int)
 if control=="unweighted_return_direction":side=np.sign(pd.to_numeric(used.completed_return,errors="coerce").fillna(0)).astype(int)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVTCCR prereg drift")
