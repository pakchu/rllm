"""Sequential strict economics for frozen OVRCR-6."""
...
PREREG = Path("results/oil_volatility_rotation_btc_confirmation_relay_preregistration_2026-08-08.json")
PREREG_SHA = "73729f8fe9acd6277d203226e2b179eb0591c7b717da12b72fc421d9b45fd69f"
SUPPORT = Path("results/oil_volatility_rotation_btc_confirmation_relay_support_2026-08-08.json")
SUPPORT_SHA = "e8ca7c0136f5126f29a1412c485c0750031d36392e1912e663dc7949945fb68e"
NOVELTY = Path("results/oil_volatility_rotation_btc_confirmation_relay_gross9_novelty_2026-08-08.json")
NOVELTY_SHA = "52f32511295adcbf2963218999a10130ff96127dab4d613846b4ef9b27c006d7"
CLOCK = Path("data/oil_volatility_rotation_btc_confirmation_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d34eb69431601daa57481a2310cd809106ea58888b2aff2d52cdac1fbf289305"
CONTROL_DIR = Path("data/oil_volatility_rotation_btc_confirmation_relay_controls_2023_2026")
FREEZE = Path("results/oil_volatility_rotation_btc_confirmation_relay_economic_evaluator_freeze_2026-08-08.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/oil_volatility_rotation_btc_confirmation_relay_{stage}_economics_2026-08-08.json")
    for stage in STAGES
}
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("OVRCR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("OVRCR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "ovrcr_6_sequential_economics_v1", "policy_id": POLICY_ID,
