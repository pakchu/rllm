"""Build source-only RFFXAR-ASYM clocks; never compute post-entry BTC outcomes."""
...
from training import preregister_range_funding_fx_asymmetric_relay as prereg
from training.build_binance_aggtrade_microstructure import _write_gzip_csv


CLOCK = Path("data/range_funding_fx_asymmetric_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/range_funding_fx_asymmetric_relay_controls_2023_2026")
SNAPSHOT = Path(
    "data/range_funding_fx_asymmetric_relay_sources_2023_2026/signal_features.csv.gz"
)
RESULT = Path("results/range_funding_fx_asymmetric_relay_support_2026-08-09.json")
SPLITS = {
    "train": (
...
def state_signals(
    snapshot: pd.DataFrame, control: str = "primary"
) -> tuple[np.ndarray, np.ndarray]:
    states = prereg.build()["frozen_states"]
    components = dict(states["long_range_funding_premium"]["components"])
    removed = {
        "no_range_component": "range_bb90",
        "no_funding_component": "funding10_trend70",
        "no_premium_component": "premium20_mom90",
    }.get(control)
    if removed is not None:
        components.pop(removed)
    long_signal = np.zeros(len(snapshot), dtype=bool)
    for gates in components.values():
        long_signal |= gate_mask(snapshot, list(gates))
    short_signal = gate_mask(snapshot, list(states["short_fx_stress"]["gates"]))
    if control == "long_state_only":
        short_signal[:] = False
    if control == "short_state_only":
        long_signal[:] = False
...
def build_clock(snapshot: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for position in np.flatnonzero(long_signal | short_signal):
        ...
        hold = pd.Timedelta(hours=48 if side == 1 else 24)
        ...
        state = "long_range_funding_premium" if side == 1 else "short_fx_stress"
        rows.append(
            {
                "candidate": "RFFXAR-ASYM",
                "control": control,
                "split": split,
...
def query_snapshot() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if missing:
        raise RuntimeError(f"RFFXAR feature columns missing: {missing}")
    dates = pd.to_datetime(market["date"], utc=True)
    snapshot = pd.DataFrame(
        {
            "date": dates,
            **{
                name: pd.to_numeric(feature_frame[name], errors="coerce")
    ...
    if snapshot.date.duplicated().any() or not snapshot.date.is_monotonic_increasing:
        raise RuntimeError("RFFXAR snapshot time drift")
    finite = np.isfinite(snapshot.loc[:, FEATURES].to_numpy(float)).all(axis=1)
    return snapshot, {
        "mode": "postgres_live_feature_builder_completed_bar",
        "rows": len(snapshot),
        "rows_all_features_finite": int(finite.sum()),
        "first": str(snapshot.date.iloc[0]),
        "last": str(snapshot.date.iloc[-1]),
        "signal_dependent_tables": [
            "bars_binance",
            "funding_rates_binance",
            "bars_binance_premium",
        ],
        "external_backward_asof_feature": "usdkrw_zscore",
    }
...
def run() -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "rffxar_asym_source_support_v1",
