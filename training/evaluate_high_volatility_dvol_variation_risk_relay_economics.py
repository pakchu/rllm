"""Sequential strict economics for frozen HVDVVR-12."""
...
PREREG = Path("results/high_volatility_dvol_variation_risk_relay_preregistration_2026-08-09.json")
PREREG_SHA = "3bd6bafad4aebcb0b97dee8dc40844999d4f55a9bd37bdc0cde0c0fd223da47d"
SUPPORT = Path("results/high_volatility_dvol_variation_risk_relay_support_2026-08-09.json")
SUPPORT_SHA = "53ca7c9739b29f058d0a309a72a679c2679ad7455065e6a126895dd42a43cc69"
NOVELTY = Path("results/high_volatility_dvol_variation_risk_relay_gross9_novelty_2026-08-09.json")
NOVELTY_SHA = "e30913537533cf35fdbf7131eb3d14d4d24f1537593204ee87070c9ca78c30fe"
CLOCK = Path("data/high_volatility_dvol_variation_risk_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "8c6824919cb0f7827f99a6c820a064b1f0f21e0fd9728b22b9aa735592cd2707"
CONTROL_DIR = Path("data/high_volatility_dvol_variation_risk_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_dvol_variation_risk_relay_economic_evaluator_freeze_2026-08-09.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_dvol_variation_risk_relay_{stage}_economics_2026-08-09.json")
    for stage in STAGES
}
CONTROLS = ("no_dvol_variation_gate", "no_btc_variation_gate", "dvol_direction", "one_day_stale_dvol", "direction_flip", "same_clock_forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDVVR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDVVR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdvvr_12_sequential_economics_v1", "policy_id": POLICY_ID,
