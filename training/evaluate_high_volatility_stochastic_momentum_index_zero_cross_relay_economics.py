"""Sequential strict economics for frozen HVSMI-24."""
...
PREREG = Path("results/high_volatility_stochastic_momentum_index_zero_cross_relay_preregistration_2026-08-11.json")
PREREG_SHA = "d8fc102b5d888f88cbd2ffda30e3853e65f98caeb8676054ebd12c9e046cb718"
SUPPORT = Path("results/high_volatility_stochastic_momentum_index_zero_cross_relay_support_2026-08-11.json")
SUPPORT_SHA = "952374fbabea84f60112683d9311f4c35e7c092fff0b26986e1a9f7085cf2004"
NOVELTY = Path("results/high_volatility_stochastic_momentum_index_zero_cross_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "a81300e4957440533b015b2b3cfb3159bf8c0e45415b5531bb6d5eabff17f0d4"
CLOCK = Path("data/high_volatility_stochastic_momentum_index_zero_cross_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "f3af11a44a2247c947fe3d5c72500dfdfdc198b60090759d28999bfdf73a74a3"
CONTROL_DIR = Path("data/high_volatility_stochastic_momentum_index_zero_cross_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_stochastic_momentum_index_zero_cross_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_stochastic_momentum_index_zero_cross_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "unsmoothed_midpoint_cross", "one_bar_stale_cross", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSMI frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSMI novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvsmi_24_sequential_economics_v1", "policy_id": POLICY_ID,
