"""Evaluate frozen HVEIT-24 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVEIT-24"
PROTOCOL = "hveit_24_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_ehlers_instantaneous_trendline_crossover_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVEIT predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVEIT novelty limits drift")
    if (
        ...
        raise RuntimeError("HVEIT predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVEIT primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hveit_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVEIT-24 unchanged before economic outcomes",
    }
