"""Evaluate frozen HVCAVSC-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVCAVSC-8"
PROTOCOL = "hvcavsc_8_gross9_novelty_v1"
PREREG = Path("results/high_volatility_cross_alt_volume_shock_consensus_relay_preregistration_2026-08-13.json")
PREREG_SHA = "7996662fa06ad47e5aa188a73ec7e1be0c014ed0b695c31d3d493b78a41bffed"
SUPPORT = Path("results/high_volatility_cross_alt_volume_shock_consensus_relay_support_2026-08-13.json")
SUPPORT_SHA = "8bfb139ff1d6431e399a9ae944a458437bc28f8d591868410db612b6e519c5b6"
CLOCK = Path("data/high_volatility_cross_alt_volume_shock_consensus_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "0b4a7843f10a7964d34c55d3bcbefa2af4f3cee82edd842c81c12d40792718e0"
OUTPUT = Path("results/high_volatility_cross_alt_volume_shock_consensus_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVCAVSC predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVCAVSC novelty limits drift")
    if (
        ...
        raise RuntimeError("HVCAVSC predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVCAVSC primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVCAVSC-8 unchanged before economic outcomes",
    }
