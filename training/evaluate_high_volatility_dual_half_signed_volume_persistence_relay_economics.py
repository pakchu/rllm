"""Sequential strict economics for frozen HVDHSVPR-8."""
...
PREREG = Path("results/high_volatility_dual_half_signed_volume_persistence_relay_preregistration_2026-08-10.json")
PREREG_SHA = "ba0915e2efc25765aaa45a1279376d7dfa43aa93a232a4949e9f5cd6f0a1655f"
SUPPORT = Path("results/high_volatility_dual_half_signed_volume_persistence_relay_support_2026-08-10.json")
SUPPORT_SHA = "46ea4650e20355a080571c1847db2da6e586aa09e2615608e6c426a3cbfa9a8d"
NOVELTY = Path("results/high_volatility_dual_half_signed_volume_persistence_relay_gross9_novelty_2026-08-10.json")
NOVELTY_SHA = "0da25d140075206fb86dcb5b372f6345b2af3d0d16d5fd6fdd2c9ff6e0a818a3"
CLOCK = Path("data/high_volatility_dual_half_signed_volume_persistence_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "6069932b4980243fb8c23a70a1a1386aa682610bb822367be8740985210cdb14"
CONTROL_DIR = Path("data/high_volatility_dual_half_signed_volume_persistence_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_dual_half_signed_volume_persistence_relay_economic_evaluator_freeze_2026-08-10.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_dual_half_signed_volume_persistence_relay_{stage}_economics_2026-08-10.json")
    for stage in STAGES
}
CONTROLS = ("no_weak_half_rank", "no_variation_gate", "full_block_signed_volume", "equal_weight_half_sign_breadth", "one_block_stale_participation", "direction_flip", "same_clock_forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDHSVPR frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDHSVPR novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdhsvpr_8_sequential_economics_v1", "policy_id": POLICY_ID,
