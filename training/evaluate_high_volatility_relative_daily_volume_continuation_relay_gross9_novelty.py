"""Evaluate frozen HVRDV-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVRDV-8"
PROTOCOL = "hvrdv_8_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_relative_daily_volume_continuation_relay_gross9_novelty_2026-08-11.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVRDV predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVRDV novelty limits drift")
    if (
        ...
        raise RuntimeError("HVRDV predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVRDV primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvrdv_clock_rows_opened": len(candidate),
        ...
        "failure_action": None if advance else "reject HVRDV-8 unchanged before economic outcomes",
    }
