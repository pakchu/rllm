"""Evaluate frozen HVRDBR-6 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVRDBR-6"
PROTOCOL = "hvrdbr_6_gross9_novelty_v1"
PREREG = Path(
    "results/high_volatility_record_discovery_breadth_relay_preregistration_2026-08-09.json"
)
PREREG_SHA = "4f7e4f9d00ea0951325faf48ae73c738a578db4fc92a527104ca55b589ce070a"
SUPPORT = Path("results/high_volatility_record_discovery_breadth_relay_support_2026-08-09.json")
SUPPORT_SHA = "f0c01513273e970e1251cefaba286f4b163eeb8cc32b24aa1d4c5f93fd70ca84"
CLOCK = Path("data/high_volatility_record_discovery_breadth_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "fd4ee1d8769f21b787a6cbbb0b1a3f7077f71df9d30c2a21c2229ec6cc884683"
OUTPUT = Path(
    "results/high_volatility_record_discovery_breadth_relay_gross9_novelty_2026-08-09.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVRDBR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVRDBR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVRDBR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVRDBR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hvrdbr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVRDBR-6 unchanged before economic outcomes"
        ),
    }
