"""Build outcome-blind source support for frozen HVCALT-6."""
...
import numpy as np
import pandas as pd
from training import preregister_high_volatility_cross_alt_lagged_transfer_consensus_relay as prereg

ENV_FILE="/home/pakchu/rllm/.env";START=pd.Timestamp("2023-01-01T00:00:00Z");END=pd.Timestamp("2026-08-01T00:00:00Z");PREREG_SHA="0b32ed107a661dbec816ad117e047b6ae75783a6026216c2dfd8b6444bd98943";REG=prereg.build();P=REG["policy"];SPLITS={k:tuple(map(pd.Timestamp,v)) for k,v in REG["stages"].items()};GATES=REG["source_support_gates"];CONTROLS=tuple(REG["diagnostic_controls"]["names"]);SYMBOLS=("BTCUSDT",*prereg.ALTS);ALTS=prereg.ALTS
...
ROOT=Path("data/high_volatility_cross_alt_lagged_transfer_consensus_relay_sources_2023_2026");PANEL=ROOT/"scheduled_transfer_states.csv.gz";MANIFEST=ROOT/"manifest.json";CLOCK=Path("data/high_volatility_cross_alt_lagged_transfer_consensus_relay_clocks_2023_2026.csv.gz");SPLIT_DIR=Path("data/high_volatility_cross_alt_lagged_transfer_consensus_relay_split_clocks_2023_2026");CONTROL_DIR=Path("data/high_volatility_cross_alt_lagged_transfer_consensus_relay_controls_2023_2026");RESULT=Path("results/high_volatility_cross_alt_lagged_transfer_consensus_relay_support_2026-08-13.json");BUILDER=Path(__file__).relative_to(Path.cwd())
BASE_COLS=("decision_time","feature_available_time","source_valid","transfer_score","selected_alts","agreeing_contributions","side","btc_realized_variation","variation_rank","variation_active","eligible")
CLOCK_COLS=("candidate","control","split","decision_time","feature_available_time","entry_time","exit_time","side","transfer_score","selected_alts","agreeing_contributions","btc_realized_variation","variation_rank")
...
def prepare(raw):
 ...
 if raw.columns.tolist()!=expected:raise RuntimeError("HVCALT source schema drift")
...
def transfer_frame(ret,valid,contemporaneous=False):
 scheduled=pd.date_range(START,END,freq="8h",inclusive="left")+pd.Timedelta("2h30m");scheduled=scheduled[(scheduled>=START)&(scheduled<END)];rows=[]
 product={};denom={}
 for alt in ALTS:
  predictor=ret[alt] if contemporaneous else ret[alt].shift(1);ok=valid[alt] if contemporaneous else valid[alt].shift(1).fillna(False);product[alt]=(predictor*ret.BTCUSDT).where(ok&valid.BTCUSDT);denom[alt]=predictor.pow(2).where(ok)
  product[alt]=product[alt].rolling(P["beta_bars"],min_periods=P["beta_bars"]).sum();denom[alt]=denom[alt].rolling(P["beta_bars"],min_periods=P["beta_bars"]).sum()
 beta=pd.DataFrame({a:product[a]/denom[a].where(denom[a].gt(0)) for a in ALTS});window_valid=valid.rolling(P["beta_bars"]+1,min_periods=P["beta_bars"]+1).sum().eq(P["beta_bars"]+1).all(axis=1);rv=np.sqrt(ret.BTCUSDT.pow(2).where(valid.BTCUSDT).rolling(P["beta_bars"],min_periods=P["beta_bars"]).sum())
 for decision in scheduled:
  ...
  for a in ALTS:row[f"{a}_beta"]=float(beta.at[end,a]) if math.isfinite(beta.at[end,a]) else np.nan;row[f"{a}_latest_return"]=float(ret.at[end,a]) if math.isfinite(ret.at[end,a]) else np.nan
  rows.append(row)
 return pd.DataFrame(rows)
def score_states(f,control="primary"):
 u=f.copy();ranks={}
 for a in ALTS:ranks[a]=midrank(u[f"{a}_beta"].abs().where(u.source_valid),P["beta_strength_prior_decisions"],P["beta_strength_minimum_decisions"]);u[f"{a}_beta_rank"]=ranks[a]
 ...
 for i,row in u.iterrows():
  chosen=[a for a in ALTS if math.isfinite(row[f"{a}_beta"]) and (control=="no_beta_tail" or row[f"{a}_beta_rank"]>=P["beta_strength_rank_min"])]
  contributions={a:int(np.sign(row[f"{a}_beta"])*np.sign(row[f"{a}_latest_return"])) for a in chosen};weights={a:abs(row[f"{a}_beta"]) for a in chosen};den=sum(weights.values());score=sum(weights[a]*contributions[a] for a in chosen)/den if den>0 else np.nan;sgn=int(np.sign(score)) if math.isfinite(score) else 0;scores.append(score);selected.append(len(chosen));agree.append(sum(v==sgn for v in contributions.values()) if sgn else 0)
 u["transfer_score"]=scores;u["selected_alts"]=selected;u["agreeing_contributions"]=agree;minimum=3 if control=="three_alt_consensus" else P["minimum_selected_alts"];u["side"]=np.sign(u.transfer_score).fillna(0).astype(int);u["variation_active"]=u.variation_rank.ge(P["variation_rank_min"]);u["eligible"]=u.source_valid&u.selected_alts.ge(minimum)&u.transfer_score.abs().ge(P["score_absolute_min"])&u.agreeing_contributions.ge(P["minimum_agreeing_contributions"])&u.variation_active
 if control=="no_variation_gate":u["eligible"]=u.source_valid&u.selected_alts.ge(minimum)&u.transfer_score.abs().ge(P["score_absolute_min"])&u.agreeing_contributions.ge(P["minimum_agreeing_contributions"])
 if control=="one_decision_stale_transfer":
  for c in ("transfer_score","selected_alts","agreeing_contributions","side"):u[c]=u[c].shift(1)
  u["eligible"]=u.source_valid&u.selected_alts.ge(minimum)&u.transfer_score.abs().ge(P["score_absolute_min"])&u.agreeing_contributions.ge(P["minimum_agreeing_contributions"])&u.variation_active
 if control=="direction_flip":u.side=-u.side
 if control=="forced_long":u.side=1
 return u
def build_panel(raw,control="primary"):
 x=prepare(raw);grid=pd.date_range(START,END,freq="5min",inclusive="left");ret=x.pivot(index="date",columns="symbol",values="return").reindex(grid,columns=SYMBOLS);valid=x.pivot(index="date",columns="symbol",values="valid").reindex(grid,columns=SYMBOLS).eq(True);f=transfer_frame(ret,valid,contemporaneous=control=="contemporaneous_beta");return score_states(f,control)
...
def run():
 if sha(prereg.DEFAULT_OUTPUT)!=PREREG_SHA:raise RuntimeError("HVCALT prereg drift")
