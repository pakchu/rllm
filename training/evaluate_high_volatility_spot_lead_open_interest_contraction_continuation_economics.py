"""Sequential strict economics for frozen HVSPOICC-8."""
...
PREREG = Path("results/high_volatility_spot_lead_open_interest_contraction_continuation_preregistration_2026-08-16.json")
PREREG_SHA = "e2584ee04e1a3c868db3817c710da929ca3addd3c058931eb9ea315cad78ad3a"
SUPPORT = Path("results/high_volatility_spot_lead_open_interest_contraction_continuation_support_2026-08-16.json")
SUPPORT_SHA = "c3b77a3289c566f5d7039284112e8e3b520880cc4301d18498a93afcc6743261"
NOVELTY = Path("results/high_volatility_spot_lead_open_interest_contraction_continuation_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "11302ea77e908542d213ea4c16eeedcb9bae78030318ac105fd993d2b0eb4f1c"
CLOCK = Path("data/high_volatility_spot_lead_open_interest_contraction_continuation_clocks_2020_2026.csv.gz")
CLOCK_SHA = "4212bc52538591e9a1baf51b2115c2396d4d47912774b1047d399786d89a049b"
CONTROL_DIR = Path("data/high_volatility_spot_lead_open_interest_contraction_continuation_controls_2023_2026")
FREEZE = Path("results/high_volatility_spot_lead_open_interest_contraction_continuation_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_spot_lead_open_interest_contraction_continuation_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ()
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVSPOICC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVSPOICC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvspoicc_8_sequential_economics_v1", "policy_id": POLICY_ID,
