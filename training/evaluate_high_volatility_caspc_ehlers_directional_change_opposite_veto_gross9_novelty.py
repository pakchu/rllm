"""Evaluate the frozen HVCELVDCV-8 action-vote clock against Gross9."""
...
POLICY = "HVCELVDCV-8"
PROTOCOL = "hvcelvdcv_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_caspc_ehlers_directional_change_opposite_veto_clocks_2023_2026.csv.gz",
    "sha256": "dd408a9d4a569519069f8a971d76b2702c6001dead58e046e8b8381486d8898d",
    "rows": 125,
}
OUTPUT = Path("results/high_volatility_caspc_ehlers_directional_change_opposite_veto_gross9_novelty_2026-08-17.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCELVDCV-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCELVDCV-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCELVDCV-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCELVDCV-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCELVDCV-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCELVDCV-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCELVDCV-8 unchanged before economics",
    }
