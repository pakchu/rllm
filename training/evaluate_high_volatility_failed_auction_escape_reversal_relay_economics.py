"""Sequential strict economics for frozen HVFAER-8."""
...
PREREG = Path("results/high_volatility_failed_auction_escape_reversal_relay_preregistration_2026-08-11.json")
PREREG_SHA = "476bc4f8b33e04e04cd8f8004c40f83e65d721c64bb63b31cabb66a54d880870"
SUPPORT = Path("results/high_volatility_failed_auction_escape_reversal_relay_support_2026-08-11.json")
SUPPORT_SHA = "e82e5278ff9d830f948d3d579ad80bbf8c2bdab9cc4dfe0445b2861a199bbc61"
NOVELTY = Path("results/high_volatility_failed_auction_escape_reversal_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "80aaff0361a9bce86f1dae8ea78389e2419f4c33629fa8be908c04d3e4c2945f"
CLOCK = Path("data/high_volatility_failed_auction_escape_reversal_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "5b7ba90030c340218ca10f70709eed65b1e19fe51d9451acc45b179c7a4cd394"
CONTROL_DIR = Path("data/high_volatility_failed_auction_escape_reversal_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_failed_auction_escape_reversal_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_failed_auction_escape_reversal_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_failed_escape_requirement", "no_variation_gate", "reclaim_close_only", "one_block_stale_prior_range", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVFAER frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVFAER novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvfaer_8_sequential_economics_v1", "policy_id": POLICY_ID,
