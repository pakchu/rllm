"""Sequential strict economics for frozen HVDYMI-24."""
...
PREREG = Path("results/high_volatility_dynamic_momentum_index_reentry_relay_preregistration_2026-08-11.json")
PREREG_SHA = "443e0ff0f08c1b8240bd895f68ecb3ea8c9a7e399a1618b4f2a244ef91e2bfe8"
SUPPORT = Path("results/high_volatility_dynamic_momentum_index_reentry_relay_support_2026-08-11.json")
SUPPORT_SHA = "6c18d2ebdba4e1b86a54b26f4d399dbc7e90f8f2a53e4db3821e374f161960e5"
NOVELTY = Path("results/high_volatility_dynamic_momentum_index_reentry_relay_gross9_novelty_2026-08-11.json")
NOVELTY_SHA = "e548ea368648b9568eeed00eefd8c2682b701a435ba7cbad29ae429a4157125e"
CLOCK = Path("data/high_volatility_dynamic_momentum_index_reentry_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "d0ef362bb5419bc9457c2dfa0dc8a0f23f511ed3664ba3d68c25617cf381935d"
CONTROL_DIR = Path("data/high_volatility_dynamic_momentum_index_reentry_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_dynamic_momentum_index_reentry_relay_economic_evaluator_freeze_2026-08-11.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_dynamic_momentum_index_reentry_relay_{stage}_economics_2026-08-11.json")
    for stage in STAGES
}
CONTROLS = ("no_variation_gate", "fixed_fourteen_rsi", "one_bar_stale_reentry", "direction_flip")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVDYMI frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVDYMI novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvdymi_24_sequential_economics_v1", "policy_id": POLICY_ID,
