"""Build outcome-blind source support for frozen HVDHSM-12."""
...
PREREG_SHA = "0d197d9dbc601220b399a1ac3942cde0568ab4fa14bc9969d8fd910aa7e6d07a"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_daily_half_session_momentum_relay_sources_2023_2026")
PANEL = ROOT / "daily_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_daily_half_session_momentum_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_daily_half_session_momentum_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_daily_half_session_momentum_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_daily_half_session_momentum_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_day", "feature_available_time", "source_valid", "first_half_return",
    "late_six_hour_return", "realized_variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_day", "feature_available_time",
    "decision_time", "entry_time", "exit_time", "side", "first_half_return",
    "late_six_hour_return", "realized_variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVDHSM source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVDHSM invalid source key")
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    for day in pd.date_range(START.normalize() + pd.Timedelta("1d"), END.normalize(), freq="1d", inclusive="left"):
        feature_time = day + pd.Timedelta("12h")
        predictor = source.loc[day : feature_time - pd.Timedelta("1min")]
        late_six = source.loc[day + pd.Timedelta("6h") : feature_time - pd.Timedelta("1min")]
        variation = source.loc[feature_time - pd.Timedelta("1d") : feature_time - pd.Timedelta("1min")]
        valid = (
            len(predictor) == 720
            and len(late_six) == 360
            and len(variation) == 1440
            and predictor.row_valid.eq(True).all()
            and late_six.row_valid.eq(True).all()
            and variation.row_valid.eq(True).all()
        )
        first_half_return = (
            float(np.log(predictor.close.iloc[-1] / predictor.open.iloc[0])) if valid else math.nan
        )
        late_six_hour_return = (
            float(np.log(late_six.close.iloc[-1] / late_six.open.iloc[0])) if valid else math.nan
        )
        realized_variation = (
        ...
        valid = bool(
            valid
            and math.isfinite(first_half_return)
            and first_half_return != 0
            and math.isfinite(late_six_hour_return)
        ...
        rows.append(
            {
                ...
                "late_six_hour_return": late_six_hour_return,
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    side = np.sign(pd.to_numeric(used.first_half_return, errors="coerce")).fillna(0).astype(int)
    state = valid & side.ne(0) & variation
    if control == "no_variation_gate":
        state = valid & side.ne(0)
    elif control == "late_six_hour_direction":
        side = np.sign(pd.to_numeric(used.late_six_hour_return, errors="coerce")).fillna(0).astype(int)
        state = valid & side.ne(0) & variation
    elif control == "one_day_stale_first_half":
        stale_valid = valid.shift(1, fill_value=False)
        side = side.shift(1, fill_value=0)
        state = stale_valid & side.ne(0) & variation
...
def build_clock(panel: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in panel.index[activity]:
        ...
        decision = pd.Timestamp(used.at[index, "feature_available_time"])
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVDHSM prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvdhsm_12_source_v1",
        "query": QUERY,
    ...
    core = {
        "protocol_version": "hvdhsm_12_source_support_v1",
