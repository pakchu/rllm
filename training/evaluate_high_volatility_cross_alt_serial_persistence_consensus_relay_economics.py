"""Sequential strict economics for frozen HVCASPC-8."""
...
PREREG = Path("results/high_volatility_cross_alt_serial_persistence_consensus_relay_preregistration_2026-08-13.json")
PREREG_SHA = "be77dfa8d83dfcb54171da4cf3263c57336fc629dd8f7b63ff8e8b33a860784a"
SUPPORT = Path("results/high_volatility_cross_alt_serial_persistence_consensus_relay_support_2026-08-13.json")
SUPPORT_SHA = "53f401d5ab9a499838c128b7023b4e420925455ca7e858e5a4db5c6e4f83d52e"
NOVELTY = Path("results/high_volatility_cross_alt_serial_persistence_consensus_relay_gross9_novelty_2026-08-13.json")
NOVELTY_SHA = "61c7fe10f3b331528315e8d7625aff7c8fb8ff1c82b6c553aa34f19ad89832f3"
CLOCK = Path("data/high_volatility_cross_alt_serial_persistence_consensus_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "bffadb70bea5d96fc779c641dbe3a1f50ba94fdf76a4e950d778a32a2b64b085"
CONTROL_DIR = Path("data/high_volatility_cross_alt_serial_persistence_consensus_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_cross_alt_serial_persistence_consensus_relay_economic_evaluator_freeze_2026-08-13.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_alt_serial_persistence_consensus_relay_{stage}_economics_2026-08-13.json")
    for stage in STAGES
}
CONTROLS = ("no_persistence_tail", "no_variation_gate", "negative_autocorrelation_consensus", "one_block_stale_persistence", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCASPC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCASPC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcaspc_8_sequential_economics_v1", "policy_id": POLICY_ID,
