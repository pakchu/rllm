"""Strict staged economics for the singleton HVCOSR-24 policy."""
...
PREREG = Path("results/high_volatility_causal_oi_semivariance_sponsorship_relay_preregistration_2026-08-16.json")
PREREG_SHA = "5c50eb37492920678720ade1d8dfce1c1b8228e7e4d2a66ce408ecf5e85ea719"
SUPPORT = Path("results/high_volatility_causal_oi_semivariance_sponsorship_relay_support_2026-08-16.json")
SUPPORT_SHA = "a1e858228e4370007f7942ea490b4db670f957b89fbe41728fbd8598e41a195d"
GROSS9 = Path("results/high_volatility_causal_oi_semivariance_sponsorship_relay_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "e5e921c4232d739ff22e98b588be7f97a0dcefaf19469b6aa816c56ffbc52c35"
CLOCK = {
    "path": Path("data/high_volatility_causal_oi_semivariance_sponsorship_relay_clocks_2020_2026.csv.gz"),
    "sha256": "e500d9a057571e96af96729c59b3b2ce6cb15b93ab2347a30772aa4f974777ee",
    "rows": 90,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_oi_semivariance_sponsorship_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCOSR-24 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCOSR-24 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCOSR-24 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCOSR-24 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCOSR-24 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCOSR-24 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCOSR-24 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCOSR-24 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCOSR-24 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCOSR-24 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCOSR-24 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcosr_24_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
