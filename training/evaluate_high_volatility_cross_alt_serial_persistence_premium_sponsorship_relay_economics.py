"""Sequential strict economics for frozen HVCASPCPSC-8."""
...
POLICY_ID = "HVCASPCPSC-8"
BAR = pd.Timedelta(minutes=5)
LEVERAGE = 0.5
BASE_COST = 0.0006
STRESS_COST = 0.0010
ENV_FILE = "/home/pakchu/rllm/.env"
PREREG = Path("results/high_volatility_cross_alt_serial_persistence_premium_sponsorship_relay_preregistration_2026-08-16.json")
PREREG_SHA = "98d0b3ae230334542aadb217cf53411c0291f0f78149d83616d1af74646ecfe4"
SUPPORT = Path("results/high_volatility_cross_alt_serial_persistence_premium_sponsorship_relay_support_2026-08-16.json")
SUPPORT_SHA = "2e56513ba3fef1509641cccdb9d16fe14e81953f75646ccf2e77c81ec08c1b5d"
NOVELTY = Path("results/high_volatility_cross_alt_serial_persistence_premium_sponsorship_relay_gross9_novelty_2026-08-16.json")
NOVELTY_SHA = "8c0dcaf7d74252570f90834fe3e13d235c0346343f9feaa9f2c73669dbe0e565"
CLOCK = Path("data/high_volatility_cross_alt_serial_persistence_premium_sponsorship_relay_clocks_2023_2026.csv.gz")
CLOCK_SHA = "2d52a14644f695887533137d5fd68ed3f4f239ee0c0de193fc57713a185647c0"
CONTROL_DIR = Path("data/high_volatility_cross_alt_serial_persistence_premium_sponsorship_relay_controls_2023_2026")
FREEZE = Path("results/high_volatility_cross_alt_serial_persistence_premium_sponsorship_relay_economic_evaluator_freeze_2026-08-16.json")
STAGES = {
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_cross_alt_serial_persistence_premium_sponsorship_relay_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
CONTROLS = ("no_premium_sponsorship_gate", "no_persistence_tail", "no_variation_gate", "negative_autocorrelation_consensus", "one_block_stale_persistence", "direction_flip", "forced_long")
...
def verify(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(sha256(path) != digest for path, digest in expected):
        raise RuntimeError("HVCASPCPSC frozen predecessor hash drift")
    ...
    if novelty.get("advance_to_economic_outcomes") is not True or novelty.get("evidence_boundary", {}).get("outcomes_opened") is not False:
        raise RuntimeError("HVCASPCPSC novelty did not authorize economics")
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcaspc_psc_8_sequential_economics_v1", "policy_id": POLICY_ID,
