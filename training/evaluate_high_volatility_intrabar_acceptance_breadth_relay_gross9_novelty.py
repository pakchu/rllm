"""Evaluate frozen HVIABR-8 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVIABR-8"
PROTOCOL = "hviabr_8_gross9_novelty_v1"
PREREG = Path(
...
OUTPUT = Path(
    "results/high_volatility_intrabar_acceptance_breadth_relay_gross9_novelty_2026-08-10.json"
)
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict[str, Any]:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVIABR predecessor hash drift")
    ...
    if registration.get("novelty_gates") != expected:
        raise RuntimeError("HVIABR novelty limits drift")
    if (
        ...
        raise RuntimeError("HVIABR predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVIABR primary")
    ...
    core = {
        ...
        "evidence_boundary": {
            "hviabr_clock_rows_opened": len(candidate),
        ...
        "failure_action": (
            None if advance else "reject HVIABR-8 unchanged before economic outcomes"
        ),
    }
