"""Evaluate the frozen HVCVMRS-8 action-vote clock against Gross9."""
...
from training import preregister_high_volatility_cross_venue_median_return_shift_consensus as prereg

POLICY = "HVCVMRS-8"
PROTOCOL = "hvcvmrs_8_gross9_novelty_v1"
...
CLOCK = {
    "path": "data/high_volatility_cross_venue_median_return_shift_consensus_clocks_2023_2026.csv.gz",
    "sha256": "46a2240a334431477ff937e589a8a07c61690b518dc0a4fd18960a63aff45f40",
    "rows": 369,
}
OUTPUT = Path("results/high_volatility_cross_venue_median_return_shift_consensus_gross9_novelty_2026-08-18.json")
...
def load_frozen_controls() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    if sha(PREREG) != PREREG_SHA:
        raise RuntimeError("HVCVMRS-8 preregistration hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVMRS-8 preregistration state drift")
    if sha(SUPPORT) != SUPPORT_SHA:
        raise RuntimeError("HVCVMRS-8 source-support artifact hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCVMRS-8 source eligibility drift")
    sealed = (
    ...
    if any(support.get(field) is not False for field in sealed):
        raise RuntimeError("HVCVMRS-8 economic evidence boundary drift")
...
def run(output: str | Path = OUTPUT) -> dict[str, Any]:
    ...
    if len(candidate) != CLOCK["rows"]:
        raise RuntimeError("HVCVMRS-8 clock row-count drift")
    comparators = {}
    ...
    core = {
        ...
        "failure_action": None if passed else "reject HVCVMRS-8 unchanged before economics",
    }
