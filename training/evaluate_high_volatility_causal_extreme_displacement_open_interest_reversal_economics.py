"""Strict staged economics for the singleton HVCEDOIR-24 policy."""
...
PREREG = Path("results/high_volatility_causal_extreme_displacement_open_interest_reversal_preregistration_2026-08-16.json")
PREREG_SHA = "ddf463605f12e6188e849d9094e4282449784f799a5c4e981b8fce295ae72c0d"
SUPPORT = Path("results/high_volatility_causal_extreme_displacement_open_interest_reversal_support_2026-08-16.json")
SUPPORT_SHA = "3d1d6bc27d8b6a3586644c8ca17261d9a9aa0802caaf2921f05ce152d06dd37f"
GROSS9 = Path("results/high_volatility_causal_extreme_displacement_open_interest_reversal_gross9_novelty_2026-08-16.json")
GROSS9_SHA = "f58cc4eba5d67774b522739f558d590288574b9be08713f65149e7b75d8acfe6"
CLOCK = {
    "path": Path("data/high_volatility_causal_extreme_displacement_open_interest_reversal_clocks_2020_2026.csv.gz"),
    "sha256": "215ec68ce56b87193e95b25e267d93b31cfafd1e8a0a0ef472bcaf0b101a88c7",
    "rows": 253,
}
...
OUTPUTS = {
    stage: Path(f"results/high_volatility_causal_extreme_displacement_open_interest_reversal_{stage}_economics_2026-08-16.json")
    for stage in STAGES
}
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"HVCEDOIR-24 artifact is not a JSON object: {path}")
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    if value.get("manifest_hash") != canonical_hash(core):
        raise RuntimeError(f"HVCEDOIR-24 manifest drift: {path}")
    return value
...
def _verify_frozen_authorization() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        if sha256(path) != expected:
            raise RuntimeError(f"HVCEDOIR-24 {label} hash drift")
    if sha256(CLOCK["path"]) != CLOCK["sha256"]:
        raise RuntimeError("HVCEDOIR-24 clock hash drift")
    ...
    if (
        ...
        raise RuntimeError("HVCEDOIR-24 preregistered economics drift")
    ...
    if (
        ...
        raise RuntimeError("HVCEDOIR-24 source-supported candidate drift")
    if (
        ...
        raise RuntimeError("HVCEDOIR-24 Gross9 did not authorize economics")
...
def _load_passing_stage_report(stage: str) -> dict[str, Any]:
    path = OUTPUTS[stage]
    if not path.is_file():
        raise RuntimeError(f"missing HVCEDOIR-24 predecessor: {path}")
    ...
    if (
        ...
        raise RuntimeError("HVCEDOIR-24 predecessor did not pass for the singleton candidate")
    earlier_stage = PREDECESSOR.get(stage)
    if earlier_stage is None:
        if report.get("predecessor") is not None:
            raise RuntimeError("HVCEDOIR-24 train predecessor chain drift")
    else:
        ...
        if report.get("predecessor") != expected_link:
            raise RuntimeError("HVCEDOIR-24 predecessor chain drift")
    return report
...
def run(stage: str, output: Path | None = None) -> dict[str, Any]:
    ...
    core = {
        "protocol_version": "hvcedoir_24_strict_staged_economics_v1",
        "policy_id": POLICY_ID,
        "candidate": POLICY_ID,
        "stage": stage,
