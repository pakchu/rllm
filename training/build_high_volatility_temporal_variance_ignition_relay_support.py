"""Build source-only HVTVIR-12 clocks before opening outcomes or Gross9."""
...
PREREG_SHA = "fe4c216d46fa593e1b3edac8a5a87f6921ba3a26522588951d26e2cac1fb4f95"
...
SNAPSHOT = Path("data/high_volatility_temporal_variance_ignition_relay_sources_2023_2026/temporal_variance_ignition_daily.csv.gz")
CLOCK = Path("data/high_volatility_temporal_variance_ignition_relay_clocks_2023_2026.csv.gz")
CONTROL_DIR = Path("data/high_volatility_temporal_variance_ignition_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_temporal_variance_ignition_relay_support_2026-08-09.json")
SPLITS = {
...
CONTROLS = ("no_volatility_gate", "no_ignition_tail", "no_direction_agreement", "equal_half_variance_share", "one_day_stale_features", "direction_flip", "forced_long")
CLOCK_COLUMNS = ("candidate", "control", "split", "decision_time", "feature_available_time", "entry_time", "exit_time", "side", "early_return", "late_return", "realized_variation", "late_variance_share", "variation_rank", "ignition_rank")
...
def load_combined_market() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if combined.date.duplicated().any() or not combined.date.diff().dropna().eq(pd.Timedelta(minutes=5)).all():
        raise RuntimeError("HVTVIR combined market continuity drift")
...
def score_snapshot(market: pd.DataFrame) -> pd.DataFrame:
    ...
    for position in positions:
        ...
        early_return = float(np.sum(window[:252])) if valid else math.nan
        late_return = float(np.sum(window[252:])) if valid else math.nan
        late_share = float(np.sum(window[252:] ** 2) / sum2) if valid and sum2 > 0 else math.nan
        direction = np.sign(early_return) == np.sign(late_return) and np.sign(early_return) != 0 if valid else False
        rows.append({"decision_time": dates.iloc[position], "source_valid": bool(valid and sum2 > 0), "early_return": early_return, "late_return": late_return, "direction_agreement": bool(direction), "realized_variation": math.sqrt(sum2) if sum2 > 0 else math.nan, "late_variance_share": late_share, "valid_return_count": 288 if valid else 0})
    ...
    frame["ignition_rank"] = strict_prior_midrank(frame.late_variance_share.where(valid))
...
def conditions(frame: pd.DataFrame, control: str) -> tuple[pd.Series, pd.Series]:
    ...
    if control == "one_day_stale_features":
        columns = ["early_return", "late_return", "direction_agreement", "realized_variation", "late_variance_share", "variation_rank", "ignition_rank", "source_valid"]
    ...
    ignition = pd.Series(True, index=work.index) if control == "no_ignition_tail" else work.ignition_rank.ge(0.80)
    agreement = pd.Series(True, index=work.index) if control == "no_direction_agreement" else work.direction_agreement.fillna(False).astype(bool)
    if control == "equal_half_variance_share":
        ignition = work.late_variance_share.ge(0.125)
    active = work.source_valid.fillna(False).astype(bool) & np.isfinite(work.early_return) & np.isfinite(work.late_return) & work.late_return.ne(0) & volatility & ignition & agreement
    side = np.sign(work.late_return)
...
def build_clock(frame: pd.DataFrame, control: str = "primary") -> pd.DataFrame:
    ...
    for index in frame.index[active]:
        ...
        if split is None:
            continue
        rows.append({"candidate": "HVTVIR-12", "control": control, "split": split, "decision_time": decision, "feature_available_time": decision, "entry_time": entry, "exit_time": exit_time, "side": int(side.at[index]), "early_return": float(frame.at[index, "early_return"]), "late_return": float(frame.at[index, "late_return"]), "realized_variation": float(frame.at[index, "realized_variation"]), "late_variance_share": float(frame.at[index, "late_variance_share"]), "variation_rank": float(frame.at[index, "variation_rank"]), "ignition_rank": float(frame.at[index, "ignition_rank"])})
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVTVIR preregistration drift")
