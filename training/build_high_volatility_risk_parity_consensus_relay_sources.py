...
SOURCE_DIR = Path("data/high_volatility_risk_parity_consensus_relay_sources_2020_2026")
OUTPUT = SOURCE_DIR / "spy_tlt_sessions.csv.gz"
MANIFEST = SOURCE_DIR / "build_manifest.json"
BUILDER = Path("training/build_high_volatility_risk_parity_consensus_relay_sources.py")
PREREG_SHA256 = "fbf24f786677c02ad63e91329c96487cebe545e61cb4d31500114203d54ca8da"
...
def parse_payload(raw: bytes, symbol: str) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if chart.get("error") is not None:
        raise RuntimeError(f"HVRPC Yahoo error for {symbol}: {chart['error']}")
    results = chart.get("result") or []
    if len(results) != 1:
        raise RuntimeError(f"HVRPC Yahoo result count for {symbol}: {len(results)}")
    result = results[0]
    timestamps = result.get("timestamp") or []
    quotes = ((result.get("indicators") or {}).get("quote") or [])
    if not timestamps or len(quotes) != 1:
        raise RuntimeError(f"HVRPC incomplete Yahoo payload for {symbol}")
    quote = quotes[0]
    ...
    if opens is None or closes is None or len(opens) != len(timestamps) or len(closes) != len(timestamps):
        raise RuntimeError(f"HVRPC Yahoo vector mismatch for {symbol}")
    timezone = str((result.get("meta") or {}).get("exchangeTimezoneName") or "")
    if timezone != "America/New_York":
        raise RuntimeError(f"HVRPC timezone drift for {symbol}: {timezone}")
    ...
    if not valid.all() or frame["session_date"].duplicated().any() or not frame["session_date"].is_monotonic_increasing:
        raise RuntimeError(f"HVRPC invalid daily rows for {symbol}")
    metadata = {
        "symbol": symbol,
...
def run() -> dict[str, Any]:
    if sha256(prereg.DEFAULT_OUTPUT) != PREREG_SHA256:
        raise RuntimeError("HVRPC preregistration hash drift")
    ...
    panel = frames["SPY"].merge(frames["TLT"], on="session_date", how="inner", validate="one_to_one")
    if panel.empty or panel["session_date"].iloc[-1] != pd.Timestamp("2026-07-31"):
        raise RuntimeError("HVRPC shared-session coverage drift")
    deterministic_gzip_csv(panel, OUTPUT)
    core = {
        "protocol_version": "hvrpc_24_sources_v1",
