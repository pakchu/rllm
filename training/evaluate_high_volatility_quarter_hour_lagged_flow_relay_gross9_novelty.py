"""Evaluate frozen HVQHLF-4 structural novelty against every Gross9 sleeve."""
...
POLICY = "HVQHLF-4"
PROTOCOL = "hvqhlf_4_gross9_novelty_v1"
...
OUTPUT = Path("results/high_volatility_quarter_hour_lagged_flow_relay_gross9_novelty_2026-08-13.json")
LIMITS = {
...
def run(output: Path = OUTPUT) -> dict:
    if sha(PREREG) != PREREG_SHA or sha(SUPPORT) != SUPPORT_SHA or sha(CLOCK) != CLOCK_SHA:
        raise RuntimeError("HVQHLF predecessor hash drift")
    ...
    if preregistration.get("novelty_gates") != expected_limits:
        raise RuntimeError("HVQHLF novelty limits drift")
    if (
        ...
        raise RuntimeError("HVQHLF predecessor state drift")
    ...
    candidate = metric.load_clock(CLOCK, label="HVQHLF primary")
    sleeve_results = {}
    ...
    core = {
        ...
        "failure_action": None if advance else "reject HVQHLF-4 unchanged before economic outcomes",
    }
