"""Build outcome-blind source support for frozen HVPVO-12."""
...
from training import preregister_high_volatility_percentage_volume_oscillator_ignition_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "fbfd95ca5e8d67072ee61d3dfc15613d12f95bcf03aa6684f5dc143c0715ada2"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_percentage_volume_oscillator_ignition_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_percentage_volume_oscillator_ignition_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_percentage_volume_oscillator_ignition_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_percentage_volume_oscillator_ignition_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_percentage_volume_oscillator_ignition_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "bar_volume", "terminal_return", "fast_ema", "slow_ema", "pvo", "signal", "difference",
    "signal_ignition", "zero_line_ignition", "entry_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "bar_volume", "terminal_return", "fast_ema", "slow_ema", "pvo", "signal", "difference",
    "signal_ignition", "zero_line_ignition", "entry_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close", "volume"]:
        raise RuntimeError("HVPVO source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVPVO invalid source key")
...
def pvo_values(series: pd.Series, valid: pd.Series) -> pd.DataFrame:
    fast=causal_ema(series,valid,P["fast_periods"])
    slow=causal_ema(series,valid,P["slow_periods"])
    pvo=(100.0*(fast-slow)/slow).where(slow.gt(0))
    signal=causal_ema(pvo,np.isfinite(pvo),P["signal_periods"])
    return pd.DataFrame({"fast_ema":fast,"slow_ema":slow,"pvo":pvo,"signal":signal,"difference":pvo-signal},index=series.index)


def cross_up(signal: pd.Series) -> pd.Series:
    previous=signal.shift(1)
    return (signal>0)&(previous<=0)&np.isfinite(signal)&np.isfinite(previous)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    bars["terminal_return"]=np.log(bars.bar_close/bars.bar_open)
    values=pvo_values(bars.bar_volume,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["signal_ignition"]=cross_up(bars.difference);bars["zero_line_ignition"]=cross_up(bars.pvo);bars["entry_side"]=np.sign(bars.terminal_return).fillna(0).astype(int)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["bar_volume","terminal_return","fast_ema","slow_ema","pvo","signal","difference","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.entry_side.ne(0)&bars.variation.gt(0)
    panel=bars.reset_index(names="source_start");panel["feature_available_time"]=panel.source_start+pd.Timedelta("4h");panel["variation_rank"]=prior_rank(panel.variation.where(panel.source_valid));panel["eligible"]=panel.source_valid&panel.signal_ignition&panel.variation_rank.ge(P["variation_rank_min"])
    return panel.loc[:,PANEL_COLUMNS]

def active(panel: pd.DataFrame, control: str = "primary"):
    if control not in ("primary",*CONTROLS):raise ValueError(control)
    used=panel.copy();valid=used.source_valid.eq(True);variation=used.variation_rank.ge(P["variation_rank_min"]);side=pd.to_numeric(used.entry_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&used.signal_ignition&variation
    if control=="no_variation_gate":state=valid&side.ne(0)&used.signal_ignition
    elif control=="zero_line_ignition":state=valid&side.ne(0)&used.zero_line_ignition&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVPVO prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvpvo_12_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvpvo_12_source_support_v1", "policy_id": prereg.POLICY_ID,
