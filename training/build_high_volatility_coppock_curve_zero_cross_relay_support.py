"""Build outcome-blind source support for frozen HVCC-24."""
...
from training import preregister_high_volatility_coppock_curve_zero_cross_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "4b67f769a3c2bae4ecdcfdf5937a73c75a2dfe548d7c7b3ab2517936dc8c207c"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_coppock_curve_zero_cross_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_coppock_curve_zero_cross_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_coppock_curve_zero_cross_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_coppock_curve_zero_cross_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_coppock_curve_zero_cross_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "roc_11", "roc_14", "roc_sum", "coppock",
    "entry_side", "raw_roc_sum_side",
    "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "roc_11", "roc_14", "roc_sum",
    "coppock", "entry_side", "raw_roc_sum_side",
    "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVCC source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVCC invalid source key")
...
def coppock_values(series: pd.Series, valid: pd.Series) -> pd.DataFrame:
    rocs=[]
    for period in (P["short_roc_periods"],P["long_roc_periods"]):
        continuous=valid.rolling(period+1,min_periods=period+1).sum().eq(period+1);lagged=series.shift(period)
        rocs.append((100.0*(series/lagged-1.0)).where(continuous&lagged.gt(0)))
    roc_sum=rocs[0]+rocs[1];period=P["wma_periods"];weights=np.arange(1,period+1,dtype=float);denominator=float(weights.sum())
    coppock=roc_sum.rolling(period,min_periods=period).apply(lambda values: float(np.dot(values,weights)/denominator),raw=True)
    return pd.DataFrame({"roc_11":rocs[0],"roc_14":rocs[1],"roc_sum":roc_sum,"coppock":coppock},index=series.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    values=coppock_values(bars.bar_close,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["entry_side"]=cross_side(bars.coppock);bars["raw_roc_sum_side"]=cross_side(bars.roc_sum)
    bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["roc_11","roc_14","roc_sum","coppock","variation"];bars["source_valid"]=bars.valid_bar&np.isfinite(bars[numeric]).all(axis=1)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="raw_roc_sum":side=pd.to_numeric(used.raw_roc_sum_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVCC prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hvcc_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hvcc_24_source_support_v1", "policy_id": prereg.POLICY_ID,
