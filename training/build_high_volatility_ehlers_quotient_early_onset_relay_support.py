"""Build outcome-blind source support for frozen HVEQT-24."""
...
from training import preregister_high_volatility_ehlers_quotient_early_onset_relay as prereg

ENV_FILE = "/home/pakchu/rllm/.env"
START = pd.Timestamp("2020-01-01T00:00:00Z")
END = pd.Timestamp("2026-08-01T00:00:00Z")
PREREG_SHA = "34f0633dbffbcec43077283ce63252f17c6d0468305aa57d16d2e8a694f95c1f"
REG = prereg.build()
P = REG["policy"]
...
ROOT = Path("data/high_volatility_ehlers_quotient_early_onset_relay_sources_2023_2026")
PANEL = ROOT / "four_hour_states.csv.gz"
MANIFEST = ROOT / "manifest.json"
CLOCK = Path("data/high_volatility_ehlers_quotient_early_onset_relay_clocks_2023_2026.csv.gz")
SPLIT_DIR = Path("data/high_volatility_ehlers_quotient_early_onset_relay_split_clocks_2023_2026")
CONTROL_DIR = Path("data/high_volatility_ehlers_quotient_early_onset_relay_controls_2023_2026")
RESULT = Path("results/high_volatility_ehlers_quotient_early_onset_relay_support_2026-08-11.json")
BUILDER = Path(__file__).relative_to(Path.cwd())
PANEL_COLUMNS = (
    "source_start", "feature_available_time", "source_valid", "bar_open", "bar_high",
    "bar_low", "bar_close", "run_length", "high_pass", "filtered", "peak", "normalized_roofing",
    "long_quotient", "short_quotient", "entry_side", "roofing_cross_side", "variation", "variation_rank", "eligible",
)
CLOCK_COLUMNS = (
    "candidate", "control", "split", "source_start", "feature_available_time",
    "entry_time", "exit_time", "side", "run_length", "high_pass", "filtered", "peak", "normalized_roofing",
    "long_quotient", "short_quotient", "entry_side", "roofing_cross_side", "variation", "variation_rank", "eligible",
)
...
def prepare(frame: pd.DataFrame) -> pd.DataFrame:
    if frame.columns.tolist() != ["ts", "open", "high", "low", "close"]:
        raise RuntimeError("HVEQT source schema drift")
    source = frame.copy()
    ...
    if source.ts.isna().any() or source.ts.duplicated().any():
        raise RuntimeError("HVEQT invalid source key")
...
def quotient_transform(series: pd.Series, valid: pd.Series) -> pd.DataFrame:
    angle=.707*2*math.pi/P["high_pass_period"];alpha=(math.cos(angle)+math.sin(angle)-1)/math.cos(angle);a1=math.exp(-1.414*math.pi/P["low_pass_period"]);c2=2*a1*math.cos(1.414*math.pi/P["low_pass_period"]);c3=-a1*a1;c1=1-c2-c3;k=P["linearity_control"]
    values=series.to_numpy(float);ok=valid.to_numpy(bool);hp=np.full(len(series),np.nan);filt=np.full(len(series),np.nan);peak=np.full(len(series),np.nan);x=np.full(len(series),np.nan);q_long=np.full(len(series),np.nan);q_short=np.full(len(series),np.nan);runs=np.zeros(len(series),dtype=int);run=0
    for i,(price,is_valid) in enumerate(zip(values,ok)):
        if not is_valid or not math.isfinite(price):run=0;continue
        run+=1;runs[i]=run
        if run<=2:hp[i]=0.;filt[i]=0.;peak[i]=0.;continue
        hp[i]=(1-alpha/2)**2*(price-2*values[i-1]+values[i-2])+2*(1-alpha)*hp[i-1]-(1-alpha)**2*hp[i-2]
        filt[i]=c1*(hp[i]+hp[i-1])/2+c2*filt[i-1]+c3*filt[i-2];peak[i]=max(abs(filt[i]),P["peak_decay"]*peak[i-1])
        if peak[i]>0:x[i]=filt[i]/peak[i];q_long[i]=(x[i]+k)/(k*x[i]+1);q_short[i]=(x[i]-k)/(1-k*x[i])
    return pd.DataFrame({"run_length":runs,"high_pass":hp,"filtered":filt,"peak":peak,"normalized_roofing":x,"long_quotient":q_long,"short_quotient":q_short},index=series.index)


def onset_side(long_quotient: pd.Series, short_quotient: pd.Series) -> pd.Series:
    prior_long=long_quotient.shift(1);prior_short=short_quotient.shift(1);side=np.zeros(len(long_quotient),dtype=int)
    side[(long_quotient>0)&(prior_long<=0)]=1;side[(short_quotient<0)&(prior_short>=0)]=-1
    return pd.Series(side,index=long_quotient.index)
...
def build_panel(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    fields=["bar_open","bar_high","bar_low","bar_close"];bars["valid_bar"]=bars.rows.eq(240)&np.isfinite(bars[fields]).all(axis=1)&bars[fields].gt(0).all(axis=1);values=quotient_transform(bars.bar_close,bars.valid_bar)
    for column in values:bars[column]=values[column]
    bars["entry_side"]=onset_side(bars.long_quotient,bars.short_quotient);bars["roofing_cross_side"]=cross_side(bars.normalized_roofing);bars["variation"]=np.sqrt(bars.variation_component.rolling(P["variation_hours"]//4,min_periods=P["variation_hours"]//4).sum());numeric=["high_pass","filtered","peak","normalized_roofing","long_quotient","short_quotient","variation"];bars["source_valid"]=bars.valid_bar&bars.run_length.ge(P["warmup_observations"])&np.isfinite(bars[numeric]).all(axis=1)&bars.peak.gt(0)&bars.variation.gt(0)
...
def active(panel: pd.DataFrame, control: str = "primary"):
    ...
    elif control=="normalized_roofing_zero_cross":side=pd.to_numeric(used.roofing_cross_side,errors="coerce").fillna(0).astype(int);state=valid&side.ne(0)&variation
...
def run() -> dict[str, Any]:
    if sha(prereg.DEFAULT_OUTPUT) != PREREG_SHA:
        raise RuntimeError("HVEQT prereg drift")
    raw = load_source()
    ...
    source_core = {
        "protocol_version": "hveqt_24_source_v1", "query": QUERY,
    ...
    core = {
        "protocol_version": "hveqt_24_source_support_v1", "policy_id": prereg.POLICY_ID,
