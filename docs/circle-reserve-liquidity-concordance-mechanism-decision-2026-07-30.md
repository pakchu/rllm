# CRLC-336 mechanism decision — 2026-07-30

## Decision

Freeze exactly one source-unseen, outcome-unseen candidate:

**CRLC-336 — Circle Reserve Liquidity Concordance, 336-hour hold.**

CRLC-336 combines four equal, discrete votes from one monthly Circle Reserve
Fund N-MFP filing:
...
hypothesis. CRLC does not use net assets, shareholder flows, USDC supply,
...
- source-axis commit `0d99f67cef61a3ad9b33cd5b5783440322f93f93`;
- source identity `CRF-NMFP`;
...
```text
CRLC-336|primary|<accession>|<source_available_at_utc>|<vote_sum>
```
...
```text
CRLC-336|control|<control_name>|<accession>|<source_available_at_utc>|<control_vote_sum>
```
...
- `deterministic_random_side`: `LONG` iff the first byte of
  `SHA256("CRLC-336|<primary_signal_id>|RANDOM_SIDE")` is below 128;
- `constant_long`;
- `constant_short`; and
...
```text
CRLC-336|control|<control_name>|<primary_signal_id>|<entry_time_utc>|<exit_time_utc>|<side>
```
...
Failure of any ordered source gate retires CRLC-336 before external novelty.
...
One failed novelty comparison permanently retires CRLC-336 before economics.
...
Only these CRLC candidate weights are tested:

```text
0.25, 0.50, 0.75, 1.00
```

For candidate weight `c`, scale every Gross9 sleeve by `(9-c)/9` and add CRLC
...
The common global leverage is `0.5`, so CRLC contributes actual entry notional
...
flag is true. If raw rank one fails, CRLC-336 is retired even when a lower rank
passes; rank two can never substitute.

The exact top one is frozen before future arrays are opened. If no weight
passes, CRLC-336 is retired. Future cannot rerank, open rank two, change a
...
- standalone CRLC must pass the ordinary return, ratio, MDD, move, trade-count,
...
future, integrity, or reproduction gate permanently retires `CRLC-336`.
...
No `CRLC-336B`, direction flip, threshold relaxation, source-field addition,
