...
import copy
import pytest
from training import preregister_high_volatility_causal_funding_impulse_relay as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    ...
    assert value["policy_id"]=="HVCFIR-8"
    assert value["candidate_family"]==["HVCFIR-8"]
    assert value["candidate_family_size"]==1
    assert value["policy"]=={"required_settlement_gap_hours":8,"history_cycles":270,"minimum_history_cycles":180,"absolute_change_rank_min":.60,"variation_rank_min":.60,"entry_delay_minutes":5,"hold_hours":8,"leverage":.5,"base_cost_per_notional_side":.0006,"stress_cost_per_notional_side":.0010}
...
def test_causal_boundary_and_no_price_direction_condition() -> None:
    ...
    assert "exact opposite signs" in value["features"]["eligible"]
    assert value["features"]["impulse_reversal"].startswith("strict sign")
    assert value["mechanism"]["side"]=="strict sign of funding_rate[S]-funding_rate[P]"
