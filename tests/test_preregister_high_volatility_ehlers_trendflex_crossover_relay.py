from training import preregister_high_volatility_ehlers_trendflex_crossover_relay as p
def test_frozen():
 x=p.build();p.validate(x)
 assert x["policy_id"]=="HVETF-24" and x["policy"]["fast_length"]==20 and x["policy"]["slow_length"]==50
