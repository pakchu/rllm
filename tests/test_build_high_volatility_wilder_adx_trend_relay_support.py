...
from training import build_high_volatility_wilder_adx_trend_relay_support as support
...
def frame() -> pd.DataFrame:
    return pd.DataFrame(
        {"source_valid": [True] * 5, "plus_di": [30, 10, 20, 40, 10],
         "minus_di": [10, 30, 20, 10, 40], "adx": [30, 30, 30, 20, 30],
...
def test_primary_and_controls() -> None:
    ...
    assert support.active(frame(), "no_adx_gate")[0].iloc[3]
    assert support.active(frame(), "no_variation_gate")[0].iloc[4]
    flipped = support.active(frame(), "direction_flip")[1]
    assert flipped[active].tolist() == [-1, 1]
