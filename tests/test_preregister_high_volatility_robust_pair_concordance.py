...
from training import preregister_high_volatility_robust_pair_concordance as p
...
def test_manifest_fixes_independent_singleton_and_exact_components() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVRC-8"
    assert value["action_ids"] == ["HVAUD-8", "HVACR-8"]
    assert value["component_ids"] == ["HVAUD-8", "HVACR-8"]
    assert value["candidate_family"] == ["HVRC-8"]
