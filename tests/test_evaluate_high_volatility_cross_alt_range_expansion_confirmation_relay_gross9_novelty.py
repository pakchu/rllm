...
from training import evaluate_high_volatility_cross_alt_range_expansion_confirmation_relay_gross9_novelty as novelty


def test_hvcarer_novelty_evaluator_is_outcome_blind_and_frozen():
    source = Path(novelty.__file__).read_text()
    assert novelty.POLICY == "HVCARER-12"
...
def test_hvcarer_novelty_binds_predecessors():
