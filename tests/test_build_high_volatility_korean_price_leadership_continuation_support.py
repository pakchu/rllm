...
from training import build_high_volatility_korean_price_leadership_continuation_support as s
...
def test_clock_follows_common_direction(monkeypatch):
 monkeypatch.setattr(s,'stage_for',lambda *_:'train');d=pd.to_datetime(['2023-07-01T00:00:00Z','2023-07-01T16:00:00Z']);p=pd.DataFrame({'decision_time':d,'feature_available_time':d,'onset':[True,True],'binance_return':[.01,-.01],'upbit_krw_return':[.02,-.02],'usdkrw_return':[.001,-.001],'upbit_implied_usd_return':[.019,-.019],'realized_variation':[.1,.2],'variation_rank':[.8,.9]});assert s.build_clock(p).side.tolist()==[1,-1]
