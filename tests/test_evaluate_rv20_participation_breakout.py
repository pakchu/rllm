...
import training.evaluate_rv20_participation_breakout as evaluator


def _cell(**changes):
    base = evaluator.Cell(0.8, 0.75, 6, 0.4, 0.8, 0.02, 0.04, 0.52)
...
def test_daily_rv20_regime_quantiles_are_shifted_and_cell_selectable():
    ...
    first = clock["rv20_q80"].first_valid_index()
    assert first == pd.Timestamp("2022-02-16")
    preceding = clock.loc[: first - pd.Timedelta(days=1), "rv20"].dropna().iloc[-756:]
    assert clock.loc[first, "rv20_q80"] == pytest.approx(
        preceding.quantile(0.8, interpolation="linear")
    )
    assert clock.loc[first, "rv20_q90"] == pytest.approx(
        preceding.quantile(0.9, interpolation="linear")
    )


def test_continuation_signals_require_selected_regime_oi_and_taker_alignment():
    hourly = pd.DataFrame({
        "rv20": [0.25] * 6,
        "rv20_q80": [0.20] * 6,
        "rv20_q90": [0.30] * 6,
        ...
        "hour_range": [0.04] * 6,
        "oi_change_1h": [0.02, 0.02, -0.01, 0.02, 0.02, 0.02],
        "taker_fraction": [0.60, 0.40, 0.60, 0.60, 0.50, 0.60],
    ...
    assert evaluator.cell_signal_sides(
        hourly, _cell(rv20_regime_quantile=0.9)
    ).tolist() == [0] * 6


def test_train_gate_uses_preregistered_30_trade_15_week_minimums():
    normal = {
        "trades": 30, "active_weeks": 15, "absolute_return_pct": 1.0,
    ...
    assert not evaluator.train_cell_passes({**normal, "trades": 29}, stress)
    assert not evaluator.train_cell_passes({**normal, "active_weeks": 14}, stress)
...
def test_persistent_long_vol_control_uses_selected_cell_regime_quantile():
    ...
    daily = pd.DataFrame(
        {"rv20": [0.25], "rv20_q80": [0.20], "rv20_q90": [0.30]},
        index=[pd.Timestamp("2024-01-01")],
    )
    funding = pd.DataFrame(columns=["date", "funding_rate"])
    q80 = evaluator.persistent_long_vol_control(
        market, funding, daily, regime_quantile=0.8, leverage=1.0,
        cost_rate=evaluator.ENTRY_COST,
    )
    q90 = evaluator.persistent_long_vol_control(
        market, funding, daily, regime_quantile=0.9, leverage=1.0,
        cost_rate=evaluator.ENTRY_COST,
    )
    assert len(q80) == 1
    assert q90 == []
