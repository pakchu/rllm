...
import numpy as np
import pandas as pd
from training import build_high_volatility_causal_spot_participation_inventory_relay_support as s
...
def test_clock_side_follows_joint_return_direction(monkeypatch) -> None:
    monkeypatch.setattr(s,"stage_for",lambda *_:"train")
    decisions=pd.to_datetime(["2023-07-01T00:00:00Z","2023-07-01T16:00:00Z"])
    panel=pd.DataFrame({"decision_time":decisions,"feature_available_time":decisions,"eligible":[True,True],"oi_change":[.01,.02],"perpetual_return":[.04,-.05],"spot_return":[.06,-.08],"spot_participation":[.4,.5],"spot_participation_rank":[.8,.9],"realized_variation":[.1,.1],"variation_rank":[.7,.8]})
...
def test_panel_requires_exact_joint_source_and_causal_participation_ranks(monkeypatch) -> None:
    ...
    rows=[];perp_returns=[.01,.02,.04,-.06,.08];spot_returns=[.015,.03,.06,.08,.12];spot_turnover=[50.,60.,80.,100.,140.]
    for market,returns,turnovers in (("perpetual",perp_returns,[100.]*5),("spot",spot_returns,spot_turnover)):
        for t,ret,turnover in zip(times,returns,turnovers):rows.append({"decision_time":t,"market":market,"completed_return":ret,"quote_turnover":turnover,"minute_squared_return":ret*ret+.001,"source_rows":480,"distinct_rows":480,"first_ts":t-pd.Timedelta("8h"),"last_ts":t-pd.Timedelta("1m"),"coherent":True})
    panel=s.build_panel((oi,pd.DataFrame(rows).sort_values(["decision_time","market"]).reset_index(drop=True)))
