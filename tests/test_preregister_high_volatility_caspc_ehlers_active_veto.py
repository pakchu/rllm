...
from training import preregister_high_volatility_caspc_ehlers_active_veto as p
...
def test_manifest_fixes_independent_singleton_and_exact_components() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVCELV-8"
    assert value["action_ids"] == ["HVCASPC-8", "HVELRSI-24"]
    assert value["component_ids"] == ["HVCASPC-8", "HVELRSI-24"]
    assert value["candidate_family"] == ["HVCELV-8"]
...
def test_exact_clock_and_no_tuning_or_hvcav_repair() -> None:
    ...
    assert construction["priority"] == (
        "none; HVCASPC-8 is the fixed primary and HVELRSI-24 is only an opposite-side veto"
    )
