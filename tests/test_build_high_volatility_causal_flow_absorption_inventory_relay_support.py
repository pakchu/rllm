...
import numpy as np
import pandas as pd
from training import build_high_volatility_causal_flow_absorption_inventory_relay_support as s
...
def test_clock_side_follows_resisted_price_direction(monkeypatch) -> None:
    monkeypatch.setattr(s,"stage_for",lambda *_:"train")
    decisions=pd.to_datetime(["2023-07-01T00:00:00Z","2023-07-01T16:00:00Z"])
    panel=pd.DataFrame({"decision_time":decisions,"feature_available_time":decisions,"eligible":[True,True],"oi_change":[.01,.02],"signed_flow_share":[-.04,.05],"absolute_flow":[.04,.05],"completed_return":[.02,-.03],"absolute_flow_rank":[.8,.9],"realized_variation":[.1,.1],"variation_rank":[.7,.8]})
...
def test_panel_requires_opposite_flow_price_signs_and_causal_ranks(monkeypatch) -> None:
    monkeypatch.setitem(s.POLICY,"minimum_history_cycles",1)
    times=pd.date_range("2023-07-01",periods=5,freq="8h",tz="UTC")
    shares=[.01,.02,-.04,.06,-.08];returns=[-.01,-.02,.03,.04,.05]
    oi=pd.DataFrame({"decision_time":times,"sum_open_interest":[100.,110.,120.,130.,140.]})
    bars=pd.DataFrame({"decision_time":times,"signed_quote_flow":np.array(shares)*100.,"quote_turnover":[100.]*5,"completed_return":returns,"minute_squared_return":[.001,.002,.003,.004,.005],"source_rows":[480]*5,"distinct_rows":[480]*5,"first_ts":times-pd.Timedelta("8h"),"last_ts":times-pd.Timedelta("1m"),"coherent":[True]*5})
