...
from training import preregister_high_volatility_ehlers_caspc_active_veto as p
...
def test_manifest_fixes_independent_singleton_and_exact_components() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVELCAV-24"
    assert value["action_ids"] == ["HVELRSI-24", "HVCASPC-8"]
    assert value["component_ids"] == ["HVELRSI-24", "HVCASPC-8"]
    assert value["candidate_family"] == ["HVELCAV-24"]
...
def test_exact_clock_and_no_tuning_or_hvcav_repair() -> None:
    ...
    assert construction["priority"] == (
        "none; HVELRSI-24 is the fixed primary and HVCASPC-8 is only an opposite-side veto"
    )
...
def test_manifest_hash_and_component_pass_scalars_detect_drift(
    ...
    def drifted_read(path: str) -> dict[str, object]:
        ...
        if path.endswith("_support_2026-08-13.json"):
