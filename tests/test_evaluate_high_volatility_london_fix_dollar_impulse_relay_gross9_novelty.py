...
from training import evaluate_high_volatility_london_fix_dollar_impulse_relay_gross9_novelty as novelty


def test_hvlfx_novelty_evaluator_is_outcome_blind_and_frozen():
    source = Path(novelty.__file__).read_text()
    assert novelty.POLICY == "HVLFX-12"
...
def test_hvlfx_novelty_binds_predecessors():
