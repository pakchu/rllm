...
from training import preregister_high_volatility_complementary_persistence_veto as p
...
def test_opposite_side_veto_truth_table_is_exact() -> None:
    assert p.veto_side(0, 1) == 0
    assert p.veto_side(1, 0) == 1
    assert p.veto_side(1, 1) == 1
    assert p.veto_side(-1, -1) == -1
    assert p.veto_side(1, -1) == 0
    assert p.veto_side(-1, 1) == 0
...
def test_exact_clock_and_no_tuning_or_hvcav_repair() -> None:
    ...
    assert construction["priority"] == (
        "none; HVMRSR-8 is the fixed primary and HVSARR-8 is only an opposite-side veto"
    )
