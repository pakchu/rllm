...
from training import evaluate_high_volatility_causal_spot_perpetual_inventory_relay_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVCSPIR-8"
    ...
    assert evaluator.CLOCK["rows"] == 130
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVCSPIR-8 predecessor"):
