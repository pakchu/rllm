...
from training import evaluate_high_volatility_daily_half_session_momentum_relay_gross9_novelty as novelty


def test_hvdhsm_novelty_evaluator_is_outcome_blind_and_frozen():
    source = Path(novelty.__file__).read_text()
    assert novelty.POLICY == "HVDHSM-12"
...
def test_hvdhsm_novelty_binds_predecessors():
