...
from training import build_high_volatility_sha256_settlement_intensity_migration_relay_support as support
...
def test_parse_source_row_enforces_completion_lag() -> None:
    ...
    assert row["tx_count"] == 1.5
    assert row["active_addresses"] == 900.0
    assert row["available_at"] == pd.Timestamp("2023-01-02T00:00:00Z")


def test_parse_source_row_rejects_zero_activity() -> None:
    with pytest.raises(ValueError, match="row value invalid"):
...
def test_reservation_keeps_equal_exit_entry_and_uses_migration_side() -> None:
    ...
    panel = pd.DataFrame({
        ...
        "btc_tx_count": 10.0,
        "btc_active_addresses": 900.0,
        "bch_tx_count": 1.0,
        "bch_active_addresses": 900.0,
        "btc_settlement_intensity": 0.01,
        "bch_settlement_intensity": 0.001,
        "relative_settlement_intensity": 1.0,
        "intensity_migration": [1.0, 0.0, -1.0, 0.0, 1.0, 0.0],
        "btc_settlement_intensity_change": [1.0, 0.0, -1.0, 0.0, 1.0, 0.0],
        "source_valid": True,
