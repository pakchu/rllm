...
import numpy as np
import pandas as pd
from training import build_high_volatility_causal_ticket_impact_inventory_relay_support as s
...
def test_clock_side_follows_completed_return(monkeypatch) -> None:
    ...
    panel=pd.DataFrame({"decision_time":decisions,"feature_available_time":decisions,"eligible":[True,True],"oi_change":[.01,.02],"ticket_impact_alignment":[.4,.5],"ticket_impact_rank":[.8,.9],"completed_return":[.04,-.05],"realized_variation":[.1,.1],"variation_rank":[.7,.8]})
...
def test_panel_requires_positive_ticket_impact_and_causal_ranks(monkeypatch) -> None:
    ...
    bars=pd.DataFrame({"decision_time":times,"ticket_impact_alignment":[.1,.2,.4,.5,.8],"ticket_log_std":[1.]*5,"absolute_return_std":[1.]*5,"completed_return":[.01,.02,-.04,0.,.08],"minute_squared_return":[.001,.002,.003,.004,.005],"source_rows":[480]*5,"distinct_rows":[480]*5,"first_ts":times-pd.Timedelta("8h"),"last_ts":times-pd.Timedelta("1m"),"coherent":[True]*5})
