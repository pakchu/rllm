...
def test_common_intersection_and_next_common_session_feature_clock():
    ...
    assert first.breadth_source_date == pd.Timestamp("2024-06-28")
...
def test_breadth_sides_zeros_and_exact_controls():
    frame = pd.DataFrame(
        {
    ...
    assert support.CONTROLS == (
        "no_volatility_gate", "vix_only", "equity_pair_only",
        "one_session_stale_breadth", "direction_flip",
    )
    ...
    stale_active, stale_side, _ = support.conditions(frame, "one_session_stale_breadth")
...
def test_clock_uses_0935_et_holds_12h_and_reserves_half_open():
    ...
    features = pd.DataFrame(
        {
            "breadth_source_date": pd.to_datetime(["2024-06-28", "2024-07-01"]),
            ...
            "positive_count": [3, 1], "negative_count": [1, 3],
...
def test_builder_is_structurally_bound_and_outcomes_remain_sealed():
    ...
    assert support.PREREG_SHA == "7c5764bab97c530467a8b930e2d2e38a083096c98abfcf7d83a6d902b93d1775"
