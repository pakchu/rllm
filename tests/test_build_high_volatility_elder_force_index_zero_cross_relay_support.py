import numpy as np
import pandas as pd
from training import build_high_volatility_elder_force_index_zero_cross_relay_support as s
...
def panel():return pd.DataFrame({"source_valid":[True]*7,"force_index":[0.,1.,2.,-1.,1.,2.,-1.],"quote_force_index":[1.,1.,0.,-1.,1.,1.,-1.],"variation_rank":[.8,.8,.8,.8,.4,.8,.8]})
def test_controls():
 ...
 quote,side,_=s.active(panel(),"quote_volume_force");assert quote.iloc[3] and side.iloc[3]==-1 and quote.iloc[6] and side.iloc[6]==-1
