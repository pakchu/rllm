...
def test_exact_offset_funding_posts_to_containing_bar():
    ...
    market = pd.DataFrame({"date": dates, "open": 100., "high": 100., "low": 100., "close": 100.})
    funding = pd.DataFrame({"date": [start + pd.Timedelta(milliseconds=5), end], "funding_rate": [.01, .5], "mark_price": [100., 100.]})
    clock = pd.DataFrame({"entry_time": [start], "exit_time": [end], "side": [1]})
    report = economics.engine.simulate(clock, market, funding, start, end, cost=0.)
    assert math.isclose(report["final_equity"], .995, abs_tol=1e-12)


def test_frozen_accounting_and_stage_contract():
    assert economics.POLICY_ID == "HVDFRM-12"
