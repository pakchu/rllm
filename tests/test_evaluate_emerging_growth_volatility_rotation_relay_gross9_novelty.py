...
from training import evaluate_emerging_growth_volatility_rotation_relay_gross9_novelty as novelty
...
def test_frozen_predecessor_hashes_and_limits():
    ...
    assert novelty.CLOCK_ROWS == 100
    assert novelty.POLICY == "EGVRR-12"
...
def test_result_write_is_deterministic_and_immutable(tmp_path):
    ...
    with pytest.raises(RuntimeError, match="immutable EGVRR novelty result"):
