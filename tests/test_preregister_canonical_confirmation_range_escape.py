...
def test_frozen_range_escape():
    payload = prereg.build()
    features = payload["features"]
    assert "divisible by 36" in features["anchor"]
    assert "288 exact five-minute bars" in features["reference_window"]
    assert "+1 iff C>U" in features["eligible_state"]
    assert "inequalities are strict" in features["eligible_state"]
