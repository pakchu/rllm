...
from training import evaluate_high_volatility_causal_variance_open_interest_acceleration_relay_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVCVAROIAR-8"
    ...
    assert evaluator.CLOCK["rows"] == 109
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVCVAROIAR-8 predecessor"):
