...
from training import evaluate_high_volatility_realized_leverage_cross_moment_relay_gross9_novelty as novelty


def test_novelty_evaluator_is_outcome_blind_and_frozen():
    source = Path(novelty.__file__).read_text()
    assert novelty.POLICY == "HVRLXC-12"
    assert novelty.LIMITS == {"exact_entry_jaccard": 0.10, "one_to_one_6h_max_matched_share": 0.35, "occupied_5m_bar_jaccard": 0.25, "absolute_signed_exposure_pearson": 0.35}
...
def test_pair_enforces_all_registered_limits(monkeypatch):
    monkeypatch.setattr(novelty.metric, "evaluate_pair", lambda _a, _b: {"metrics": {"exact_entry_jaccard": 0.1, "one_to_one_6h_max_matched_share": 0.35, "occupied_5m_bar_jaccard": 0.25, "absolute_signed_exposure_pearson": 0.35}})
