...
from training import evaluate_high_volatility_causal_extreme_displacement_open_interest_reversal_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVCEDOIR-24"
    ...
    assert evaluator.CLOCK["rows"] == 253
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVCEDOIR-24 predecessor"):
