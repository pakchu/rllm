...
from training import preregister_high_volatility_causal_close_location_open_interest_acceptance_relay as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVCCLOIAR-8"
    assert value["candidate_family"] == ["HVCCLOIAR-8"]
    assert value["policy"] == {
        "history_cycles": 270,
...
def test_prior_failures_disclosed_without_repair() -> None:
    ...
    assert boundary["prior_HVCATCLIR_source_novelty_and_train_failure_known"] is True
    assert boundary["same_mechanism_as_HVCATCLIR"] is False
