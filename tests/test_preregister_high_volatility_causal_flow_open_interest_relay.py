...
import copy
import pytest
from training import preregister_high_volatility_causal_flow_open_interest_relay as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    ...
    assert value["policy_id"]=="HVCFOIR-8" and value["candidate_family"]==["HVCFOIR-8"]
    assert value["policy"]=={"history_cycles":270,"minimum_history_cycles":180,"path_minutes":480,"spot_flow_lead_rank_min":.60,"variation_rank_min":.60,"entry_delay_minutes":5,"hold_hours":8,"leverage":.5,"base_cost_per_notional_side":.0006,"stress_cost_per_notional_side":.0010}
...
def test_causal_flow_boundary() -> None:
    value=p.build();f=value["features"]
    assert f["oi_expansion"].startswith("strict positive")
    assert f["spot_flow_lead_rank"].startswith("strict-prior") and f["variation_rank"].startswith("strict-prior")
    assert "taker_buy_quote" in f["completed_cycles"] and "no return direction" in f["eligible"]
...
def test_prior_failures_disclosed_without_repair() -> None:
    ...
    assert b["prior_HVSFL_outcomes_known"] is True and b["same_mechanism_as_HVSFL"] is False
