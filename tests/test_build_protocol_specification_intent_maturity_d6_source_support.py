...
def _synthetic_origin(tmp_path: Path) -> tuple[Path, str, str]:
    ...
    _run(git, "config", "user.name", "PSIM D6 Test", cwd=work)
    _run(
        git,
        "config",
        "user.email",
        "psim-d6@example.test",
        cwd=work,
    )
...
def _group(
    *,
    ...
    old_path = (
        None if old_raw is None else f"EIPS/eip-{proposal_number}.md"
    )
    new_path = (
        None if new_raw is None else f"EIPS/eip-{proposal_number}.md"
    )
...
def test_self_check_is_synthetic_only_and_opens_no_source(
    ...
    assert payload["policy_id"] == "PSIM-D6"
...
def test_runner_namespace_authority_and_import_surface_are_frozen() -> None:
    ...
    assert runner.DEFAULT_SOURCE_ROOT == Path("/tmp/psim-d6-source")
    assert runner.DEFAULT_SOURCE_ROOT != d5_runner.DEFAULT_SOURCE_ROOT
    assert runner.SEALED_REF == "refs/psim-d6/sealed-tip"
    assert runner.PREREGISTRATION_COMMIT == (
        "a2ff036d03f01750da3527666e3be3d44737cbe2"
    )
    assert runner.D5_IMPLEMENTATION_COMMIT == (
        "90e7740edcd68a3b4c3acf8e9fe9a14f9e4eb8e1"
    )
    assert runner._load_preregistration() == prereg.build_preregistration()
    assert runner._load_mechanism_probe() == mechanism.build_probe()
    assert (
...
def test_public_migration_authorizer_is_exact_frozen_and_noninjectable() -> None:
    ...
    assert manifest_hash == prereg.D5_EPISODE_RECEIPT_MANIFEST_HASH
...
def test_real_local_clone_uses_d5_hydration_with_fresh_d6_namespace(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    assert (tmp_path / "source" / ".psim-d6-traces").is_dir()
    assert not (tmp_path / "source" / ".psim-d5-traces").exists()
