...
from training import build_high_volatility_caspc_quiet_premium_activity_support as s
def test_rank_excludes_current():
 r=s.rank(pd.Series([1.,2.,3.,2.]),3,2);assert np.isnan(r.iloc[:2]).all() and r.iloc[2]==1 and r.iloc[3]==.5
def test_clock_keeps_only_quiet_base_side(tmp_path,monkeypatch):
 ...
 p=pd.DataFrame({'decision_time':d,'feature_available_time':d,'source_valid':[True,True],'premium_total_variation':[1.,2.],'btc_return':[.1,-.1],'btc_realized_variation':[.2,.3],'relative_premium_activity':[.1,.9],'relative_activity_rank':[.2,.8],'btc_variation_rank':[.8,.8],'eligible':[True,True],'side':[0,0]});assert s.build_clock(p).side.tolist()==[1]
