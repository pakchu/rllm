...
from training import evaluate_high_volatility_dcs_imi_active_override_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVDIMIO-8"
    ...
    assert evaluator.CLOCK["rows"] == 235
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVDIMIO-8 predecessor"):
