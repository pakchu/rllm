...
from training import build_high_volatility_directional_change_scarcity_premium_sponsorship_relay_support as support
...
def test_contract_is_frozen():
    assert support.PREREG_SHA == "74c490e6d4e22336ba004f0dceacf31792205796a606c88ab4ae70a05babaaad"
    assert "FROM bars_binance" in support.QUERY
    assert "FROM bars_binance_premium" in support.PREMIUM_QUERY
    ...
    states = pd.DataFrame(
        {
            ...
            "premium_displacement": [-0.1, 0.1, -0.1],
        }
    )
    primary, side = support.active(states, "primary")
    assert primary.tolist() == [False, True, False]
    assert side[primary].tolist() == [1.0]
    no_premium, _ = support.active(states, "no_premium_sponsorship_gate")
    assert no_premium.tolist() == [False, False, False]
