...
def test_primary_and_frozen_controls():
    frame = pd.DataFrame({
        ...
        "btc_flow": [0.02, -0.02, 0.02], "confirming_alts": [5, 6, 3],
    ...
    assert support.conditions(frame, "btc_flow_confirmation")[0].tolist() == [False, False, False]
    assert support.conditions(frame, "direction_flip")[1].tolist() == [1, -1, 1]


def test_clock_uses_five_minute_delay_holds_6h_and_reserves_half_open():
    frame = pd.DataFrame({
        "session_date": ["2024-01-01", "2024-01-01"],
        "decision_time": [pd.Timestamp("2024-01-01T08:00:00Z"), pd.Timestamp("2024-01-01T10:00:00Z")],
        "btc_flow": [0.02, -0.02], "confirming_alts": [5, 6],
...
def test_builder_is_bound_and_outcomes_are_sealed():
    ...
    assert support.PREREG_SHA == "cac7461ba22bbd5571d315b8186823aca935c26ea67856c9c24dd540317732dd"
