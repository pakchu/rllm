...
from training import evaluate_high_volatility_variance_acceleration_oi_regime_router_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVCVAROIR-8"
    ...
    assert evaluator.CLOCK["rows"] == 284
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVCVAROIR-8 predecessor"):
