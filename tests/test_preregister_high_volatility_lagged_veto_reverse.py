...
from training import preregister_high_volatility_lagged_veto_reverse as p
...
def test_manifest_fixes_independent_singleton_and_exact_components() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVLVR-8"
    assert value["action_ids"] == ["HVDCS-8", "HVCASPC-8"]
    assert value["component_ids"] == ["HVDCS-8", "HVCASPC-8"]
    assert value["candidate_family"] == ["HVLVR-8"]
...
def test_lagged_veto_reverse_truth_table_is_exact() -> None:
...
def test_exact_clock_and_no_tuning_or_hvcav_repair() -> None:
    ...
    assert construction["priority"] == (
        "none; HVDCS-8 defines incidence and exact D-5h HVCASPC-8 is only a causal veto"
    )
