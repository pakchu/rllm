...
from training import build_high_volatility_robust_state_three_action_majority_support as support
...
def test_quorum_tie_and_strict_majority(monkeypatch) -> None:
    ...
    assert list(clock["decision_time"]) == [pd.Timestamp(d2), pd.Timestamp(d3)]
    assert list(clock["side"]) == [-1, 1]
    assert counts["no_quorum_decisions"] == 1
    assert counts["tie_decisions"] == 1
