...
import copy
import pytest
from training import preregister_high_volatility_causal_oi_range_regime_relay as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    ...
    assert value["policy_id"]=="HVCORR-24"
    assert value["candidate_family"]==["HVCORR-24"]
    assert value["candidate_family_size"]==1
    assert value["policy"]=={"history_cycles":270,"minimum_history_cycles":180,"prior_range_cycles":21,"variation_rank_min":.60,"entry_delay_minutes":5,"hold_hours":24,"leverage":.5,"base_cost_per_notional_side":.0006,"stress_cost_per_notional_side":.0010}
...
def test_causal_boundary_and_no_price_direction_condition() -> None:
    ...
    assert value["features"]["regime_cross"].startswith("last close at D-8h")
