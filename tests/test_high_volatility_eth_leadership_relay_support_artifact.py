...
def test_hvelr_source_support_pass_is_outcome_sealed():
    ...
    assert [result["support"][stage]["events"] for stage in ("train", "test", "eval", "final")] == [31, 45, 91, 36]


def test_hvelr_support_hashes_bind_frozen_files():
    result = json.loads(support.RESULT.read_text())
