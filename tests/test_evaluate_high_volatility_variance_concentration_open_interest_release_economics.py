...
from training import evaluate_high_volatility_variance_concentration_open_interest_release_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVCVCOIR-12"
    ...
    assert evaluator.CLOCK["rows"] == 251
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVCVCOIR-12 predecessor"):
