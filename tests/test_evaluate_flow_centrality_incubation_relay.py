...
def test_control_derivation_is_source_only_deterministic_and_stage_contained() -> None:
    ...
    assert len(frame) == 247 * len(evaluator.ALL_CONTROLS)
    assert bool(cast(pd.Series, frame.groupby("control").size()).eq(247).all())
    schedules = {
...
def test_freeze_opens_no_market_funding_or_simulation(
    ...
    assert report["support_commit"] == "a57fc55cf1507bc5d5cab33f3e5c936e7ac39e05"
...
def test_full_calendar_cagr_counts_idle_time() -> None:
    market = _market()
    market.loc[144, ["open", "high", "low", "close"]] = [
        110.0,
        110.0,
        110.0,
        110.0,
