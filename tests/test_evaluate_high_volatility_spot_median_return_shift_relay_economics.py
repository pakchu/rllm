...
from training import evaluate_high_volatility_spot_median_return_shift_relay_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVSMRSR-8"
    ...
    assert evaluator.CLOCK["rows"] == 342
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVSMRSR-8 predecessor"):
