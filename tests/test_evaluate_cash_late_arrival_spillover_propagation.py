...
from training import evaluate_cash_late_arrival_spillover_propagation as ev
...
def test_slice_schedule_requires_signal_entry_and_exit_inside_split() -> None:
    schedule = _schedule()
    sliced = ev.slice_schedule(schedule, start="2020-01-01", end="2020-01-01 03:00:00")
    assert len(sliced) == 1
...
def test_simulation_uses_24_bar_hold_and_excludes_exit_bar_extreme() -> None:
    dates = pd.date_range("2020-01-01", periods=30, freq="5min")
    frame = pd.DataFrame(
        {
            "date": dates,
            "open": 100.0,
    ...
    broken.loc[0, "exit_position"] = 24
    with pytest.raises(ValueError, match="24-bar hold"):
