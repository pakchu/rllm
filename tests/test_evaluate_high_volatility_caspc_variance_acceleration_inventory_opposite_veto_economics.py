...
from training import evaluate_high_volatility_caspc_variance_acceleration_inventory_opposite_veto_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVCASPCVAIV-8"
    ...
    assert evaluator.CLOCK["rows"] == 149
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVCASPCVAIV-8 predecessor"):
