...
ARTIFACT = Path("results/high_volatility_lottery_sales_risk_appetite_relay_train_economics_2026-08-12.json")


def test_hvlsra_train_economics_is_terminal_and_reproducible():
    ...
    assert payload["physical_rows_opened"]["primary_clock"] == 17
    core = {key: value for key, value in payload.items() if key != "manifest_hash"}
    assert payload["manifest_hash"] == evaluator.canonical_hash(core)


def test_hvlsra_later_stage_artifacts_do_not_exist():
