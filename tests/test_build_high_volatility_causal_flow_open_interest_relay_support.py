...
import numpy as np
import pandas as pd
from training import build_high_volatility_causal_flow_open_interest_relay_support as s
...
def test_clock_side_follows_common_cash_led_flow(monkeypatch) -> None:
    monkeypatch.setattr(s,"stage_for",lambda *_:"train")
    decisions=pd.to_datetime(["2023-07-01T00:00:00Z","2023-07-01T16:00:00Z"])
    panel=pd.DataFrame({"decision_time":decisions,"feature_available_time":decisions,"eligible":[True,True],"oi_change":[.01,.02],"perpetual_flow_share":[.04,-.05],"spot_flow_share":[.06,-.08],"spot_flow_lead":[.02,.03],"spot_flow_lead_rank":[.8,.9],"realized_variation":[.1,.1],"variation_rank":[.7,.8]})
...
def test_panel_requires_exact_joint_flow_source_and_causal_ranks(monkeypatch) -> None:
    ...
    rows=[];perp=[.01,.02,.04,-.06,.08];spot=[.015,.03,.06,.08,.12]
    for market,shares in (("perpetual",perp),("spot",spot)):
        for t,share in zip(times,shares):rows.append({"decision_time":t,"market":market,"signed_quote_flow":share*100.,"quote_turnover":100.,"minute_squared_return":share*share+.001,"source_rows":480,"distinct_rows":480,"first_ts":t-pd.Timedelta("8h"),"last_ts":t-pd.Timedelta("1m"),"coherent":True})
