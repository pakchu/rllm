...
def test_primary_and_frozen_controls():
    frame = pd.DataFrame({
        ...
        "btc_return": [0.02, -0.02, 0.02], "confirming_alts": [5, 6, 4],
        "median_alt_abs_return": [0.03, 0.03, 0.03],
    ...
    assert support.conditions(frame, "no_exhaustion_gate")[0].tolist() == [True, False, False]
    assert support.conditions(frame, "four_of_six_breadth")[0].tolist() == [True, False, True]
...
def test_clock_uses_0805_holds_12h_and_reserves_half_open():
    frame = pd.DataFrame({
        ...
        "btc_return": [0.02, -0.02], "confirming_alts": [5, 6],
        "median_alt_abs_return": [0.03, 0.03],
...
def test_builder_is_bound_and_outcomes_are_sealed():
    ...
    assert support.PREREG_SHA == "8244a77c65914e13dac2006e52dcb88a398b7c1ab66bbf45b8cfd70f5da802d6"
