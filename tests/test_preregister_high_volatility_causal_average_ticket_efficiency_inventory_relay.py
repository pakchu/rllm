...
from training import preregister_high_volatility_causal_average_ticket_efficiency_inventory_relay as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVCATEIR-8"
    assert value["candidate_family"] == ["HVCATEIR-8"]
    assert value["policy"] == {
        "history_cycles": 270,
...
def test_causal_average_ticket_boundary() -> None:
    ...
    assert features["path_efficiency_rank"].startswith("strict-prior")
    assert "sum abs" in features["path_efficiency"]
