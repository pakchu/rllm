...
from training import preregister_high_volatility_ehlers_imi_veto as p
...
def test_manifest_fixes_independent_singleton_and_exact_components() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVEIV-24"
    assert value["action_ids"] == ["HVELRSI-24", "HVIMI-24"]
    assert value["component_ids"] == ["HVELRSI-24", "HVIMI-24"]
    assert value["candidate_family"] == ["HVEIV-24"]
...
def test_opposite_side_veto_truth_table_is_exact() -> None:
    assert p.indicator_veto_side(0, 1) == 0
    assert p.indicator_veto_side(1, 0) == 1
    assert p.indicator_veto_side(1, 1) == 1
    assert p.indicator_veto_side(-1, -1) == -1
    assert p.indicator_veto_side(1, -1) == 0
    assert p.indicator_veto_side(-1, 1) == 0
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.indicator_veto_side(1, True)
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.indicator_veto_side(2, 1)
...
def test_exact_clock_and_no_tuning_or_hvcav_repair() -> None:
    ...
    assert construction["priority"] == (
        "none; HVELRSI-24 is the fixed primary and HVIMI-24 is only an opposite-side veto"
    )
...
def test_manifest_hash_and_component_pass_scalars_detect_drift(
    ...
    def drifted_read(path: str) -> dict[str, object]:
        ...
        if path.endswith("_support_2026-08-11.json"):
