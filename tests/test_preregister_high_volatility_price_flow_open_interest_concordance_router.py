...
from training import preregister_high_volatility_price_flow_open_interest_concordance_router as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVPFOICR-8"
    assert value["candidate_family"] == ["HVPFOICR-8"]
    assert value["policy"] == {
        "history_cycles": 270,
...
def test_causal_spot_perpetual_oi_boundary() -> None:
    ...
    assert features["oi_change"].startswith("log(OI")
    assert features["variation_rank"].startswith("strict-prior")
    assert "state_identity true" in features["eligible"]
    assert "sign(price_return)*sign(flow_share)" in features["state_identity"]
    assert features["side"].startswith("sign(flow_share)")
...
def test_prior_failures_disclosed_without_repair() -> None:
    ...
    assert boundary["prior_flow_OI_source_and_router_failures_known"] is True
    assert boundary["exact_symmetric_price_flow_OI_identity_incidence_or_outcomes_known"] is False
