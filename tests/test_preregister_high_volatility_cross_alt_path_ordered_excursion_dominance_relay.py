from training import preregister_high_volatility_cross_alt_path_ordered_excursion_dominance_relay as s
...
def test_fixed_asymmetry_spillover_policy():
    payload = s.build()
    assert len(s.ALTS) == 6
    assert payload["policy"]["path_bars"] == 96
    assert payload["policy"]["asymmetry_rank_min"] == 0.70
    assert payload["policy"]["minimum_selected_alts"] == 4
    assert payload["policy"]["decision_hours_utc"] == [5, 13, 21]
