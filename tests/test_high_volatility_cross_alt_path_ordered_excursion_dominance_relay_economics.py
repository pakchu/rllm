...
def test_frozen_contract():
 assert e.CONTROLS==('no_variation_gate','no_asymmetry_tail','three_alt_consensus','equal_weighted_score','one_decision_stale_consensus','direction_flip','forced_long');assert e.LEVERAGE==.5 and e.BASE_COST==.0006 and e.STRESS_COST==.001;assert 'load_clock_allow_empty' in open(e.__file__).read()
