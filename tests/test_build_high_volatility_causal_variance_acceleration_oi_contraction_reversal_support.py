...
import numpy as np
import pandas as pd
from training import build_high_volatility_causal_variance_acceleration_oi_contraction_reversal_support as s
...
def test_panel_requires_variance_acceleration_oi_and_ranks(monkeypatch):
    ...
    oi=pd.DataFrame({"decision_time":t,"sum_open_interest":[130.,120.,110.,115.,100.]})
...
def test_clock_follows_completed_return(monkeypatch):
    ...
    p=pd.DataFrame({"decision_time":d,"feature_available_time":d,"eligible":[True,True],"oi_change":[.1,.2],"first_half_variance":[1.,1.],"second_half_variance":[2.,3.],"completed_return":[.02,-.03],"variance_acceleration":[.69,1.1],"variance_acceleration_rank":[.8,.9],"realized_variation":[.1,.2],"variation_rank":[.8,.9]})
