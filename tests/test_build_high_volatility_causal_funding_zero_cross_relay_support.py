...
import numpy as np
import pandas as pd
from training import build_high_volatility_causal_funding_zero_cross_relay_support as s
...
def test_panel_requires_exact_zero_cross_and_causal_ranks(monkeypatch) -> None:
    monkeypatch.setitem(s.POLICY,"minimum_history_cycles",1)
    times=pd.date_range("2023-07-01",periods=4,freq="8h",tz="UTC")
    funding=pd.DataFrame({"funding_time":times,"funding_rate":[-.001,.001,-.002,.003]})
    bars=pd.DataFrame({
        "decision_time":times,"first_open":[100.]*4,"last_close":[101.]*4,
        "minute_squared_return":[.000625,.0025,.01,.04],"source_rows":[480]*4,
    ...
    assert panel["zero_cross"].tolist()==[False,True,True,True]
    assert panel["eligible"].tolist()==[False,False,True,True]
