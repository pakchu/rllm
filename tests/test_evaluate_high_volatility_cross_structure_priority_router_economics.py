...
from training import evaluate_high_volatility_cross_structure_priority_router_economics as evaluator
...
def test_frozen_bindings_and_two_candidate_authorization_with_four_member_family() -> None:
    assert evaluator.POLICY_ID == "HVCSPR-8"
    assert evaluator.LEVERAGE == 0.5
    assert evaluator.BASE_COST == 0.0006
    assert evaluator.STRESS_COST == 0.0010
    assert len(evaluator.FAMILY_ORDER) == 4
    assert evaluator.ELIGIBLE_CANDIDATES == (
        evaluator.FAMILY_ORDER[0], evaluator.FAMILY_ORDER[2]
    )
...
def test_predecessor_is_enforced_before_any_source_opens(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="missing HVCSPR-8 predecessor"):
...
def test_train_evaluates_both_eligible_and_never_substitutes_raw_rank_one(
