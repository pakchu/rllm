...
def frame():
    return pd.DataFrame(
        {
            "signal_valid": [True] * 4,
            "late_return":[.015,-.015,.03,.015],"late_taker_imbalance":[.2,-.2,-.2,.2],
...
def test_controls_remain_diagnostic_only():
    ...
    candidate.loc[1,"late_taker_imbalance"]=-.05
    assert support.conditions(candidate,"no_flow_magnitude_gate")[0].tolist()==[True,True,False,False]
