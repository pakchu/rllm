...
import numpy as np
import pandas as pd
from training import build_high_volatility_causal_oi_range_regime_relay_support as s
...
def test_clock_side_follows_range_midpoint_transition(monkeypatch) -> None:
    ...
    panel=pd.DataFrame({"decision_time":decisions,"feature_available_time":decisions,"eligible":[True,True],"oi_change":[.01,.02],"previous_close":[99.,101.],"last_close":[101.,99.],"prior_range_midpoint":[100.,100.],"regime_cross_side":[1,-1],"realized_variation":[.1,.1],"variation_rank":[.7,.8]})
