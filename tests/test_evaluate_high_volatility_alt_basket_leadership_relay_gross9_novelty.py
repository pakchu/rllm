...
from training import evaluate_high_volatility_alt_basket_leadership_relay_gross9_novelty as novelty
...
def test_frozen_predecessor_hashes_and_limits():
    ...
    assert novelty.CLOCK_ROWS == 402
    assert novelty.POLICY == "HVABLR-6"
...
def test_result_write_is_deterministic_and_immutable(tmp_path):
    ...
    with pytest.raises(RuntimeError, match="immutable HVABLR novelty result"):
