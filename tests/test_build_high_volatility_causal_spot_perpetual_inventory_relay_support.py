...
import numpy as np
import pandas as pd
from training import build_high_volatility_causal_spot_perpetual_inventory_relay_support as s
...
def test_clock_side_follows_common_cash_led_direction(monkeypatch) -> None:
    monkeypatch.setattr(s,"stage_for",lambda *_:"train")
    decisions=pd.to_datetime(["2023-07-01T00:00:00Z","2023-07-01T16:00:00Z"])
    panel=pd.DataFrame({"decision_time":decisions,"feature_available_time":decisions,"eligible":[True,True],"oi_change":[.01,.02],"perpetual_return":[.04,-.05],"spot_return":[.06,-.08],"spot_lead_displacement":[.02,.03],"spot_lead_rank":[.8,.9],"realized_variation":[.1,.1],"variation_rank":[.7,.8]})
...
def test_panel_requires_exact_joint_source_spot_lead_and_causal_ranks(monkeypatch) -> None:
    ...
    for market,returns in (("perpetual",perp),("spot",spot)):
        for t,ret in zip(times,returns):rows.append({"decision_time":t,"market":market,"completed_return":ret,"minute_squared_return":ret*ret+.001,"source_rows":480,"distinct_rows":480,"first_ts":t-pd.Timedelta("8h"),"last_ts":t-pd.Timedelta("1m"),"coherent":True})
