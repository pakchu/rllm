...
from training import evaluate_high_volatility_spot_underwater_causal_sign_adapter_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVSAUDCA-8"
    ...
    assert evaluator.CLOCK["rows"] == 175
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVSAUDCA-8 predecessor"):
