...
from training import evaluate_high_volatility_energy_commodity_return_spillover_relay_gross9_novelty as novelty


def test_hvecsp_novelty_evaluator_is_outcome_blind_and_frozen():
    source = Path(novelty.__file__).read_text()
    assert novelty.POLICY == "HVECSP-24"
...
def test_hvecsp_novelty_binds_predecessors():
