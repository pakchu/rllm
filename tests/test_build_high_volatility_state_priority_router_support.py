...
from training import build_high_volatility_state_priority_router_support as support
...
def _actions() -> dict[str, pd.DataFrame]:
    return {
        "HVTFR-8": _action("HVTFR-8", ["2023-07-01T00:00:00Z"], [1]),
        "HVSVF-8": _action(
            "HVSVF-8",
            ["2023-07-01T00:00:00Z", "2023-07-01T08:00:00Z"],
            [-1, 1],
        ),
        "HVRSSR-8": _action("HVRSSR-8", ["2023-07-01T08:00:00Z"], [-1]),
    }
...
def test_router_chooses_first_active_and_cash_without_materializing_position() -> None:
    ...
    assert clock["action_id"].tolist() == ["HVTFR-8", "HVSVF-8"]
    assert clock["side"].tolist() == [1, 1]
    assert routing == {
        "eligible_state_decisions": 3,
        "routed_action_decisions": 2,
        "cash_decisions": 1,
        "selected_action_counts": {"HVTFR-8": 1, "HVSVF-8": 1, "HVRSSR-8": 0},
    }


def test_reversed_priority_changes_only_simultaneous_choice_and_preserves_action_fields() -> None:
    actions = _actions()
    clock, _ = support.route_priority_clock(
        prereg.PRIORITY_ORDERS[1], "HVLZC-8", actions, _state("HVLZC-8")
    )
    assert clock["action_id"].tolist() == ["HVSVF-8", "HVRSSR-8"]
    chosen = actions["HVRSSR-8"].iloc[0]
    row = clock.iloc[1]
    for field in ("side", "feature_available_time", "entry_time", "exit_time"):
...
def test_exact_state_grid_and_routed_nonoverlap_fail_closed() -> None:
    ...
    actions = _actions()
    actions["HVTFR-8"].loc[0, "exit_time"] += pd.Timedelta("1ns")
...
def test_support_stats_and_all_stage_gates_match_preregistration() -> None:
    ...
    stats = {stage: support.support_stats(frame, stage) for stage in prereg.build()["stages"]}
...
def test_frozen_inputs_and_double_materialization_are_deterministic(tmp_path) -> None:
    assert support.PREREG_SHA == "80b8865a9b3b44af04e682f5179fac4482c5f839a9a96e6c10daa9f396304c3b"
    ...
    first_clocks = {candidate: row["clock"]["sha256"] for candidate, row in first["candidates"].items()}
    second = support.run(clock_dir, result_path)
    assert result_path.read_bytes() == first_bytes
    assert {candidate: row["clock"]["sha256"] for candidate, row in second["candidates"].items()} == first_clocks
    ...
    assert first["eligible_router_count"] == len(first["eligible_routers_for_combination_gross9"])
