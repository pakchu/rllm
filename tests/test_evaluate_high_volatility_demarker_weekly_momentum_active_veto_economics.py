...
from training import evaluate_high_volatility_demarker_weekly_momentum_active_veto_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVDEMWMV-24"
    ...
    assert evaluator.CLOCK["rows"] == 110
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVDEMWMV-24 predecessor"):
