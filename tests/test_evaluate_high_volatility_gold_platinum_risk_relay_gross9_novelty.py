...
from training import evaluate_high_volatility_gold_platinum_risk_relay_gross9_novelty as novelty


def test_hvauptr_novelty_evaluator_is_outcome_blind_and_frozen():
    source = Path(novelty.__file__).read_text()
    assert novelty.POLICY == "HVAUPTR-24"
...
def test_hvauptr_novelty_binds_predecessors():
