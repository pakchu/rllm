...
def test_macro_feature_parity_against_frozen_research_helper():
    ...
    expected = pd.DataFrame(index=hourly.index)
    for field, flag in (
        ("dxy", "dxy_available"),
        ("usdkrw", "usdkrw_available"),
        ("kimchi_premium", "kimchi_available"),
    ):
        valid = hourly[flag] > 0.5
        values = hourly[field].where(valid)
        if field != "kimchi_premium":
            values = np.log(values.where(values > 0))
        expected[f"{field}_valid"] = valid.astype(float)
        for hours in (6, 24):
            expected[f"{field}_change{hours}"] = values.diff(hours)
        std = values.rolling(168, min_periods=72).std(ddof=0).replace(0, np.nan)
        expected[f"{field}_z"] = (
            values - values.rolling(168, min_periods=72).mean()
        ) / std
    expected = expected.reindex(idx).replace([np.inf, -np.inf], np.nan)
