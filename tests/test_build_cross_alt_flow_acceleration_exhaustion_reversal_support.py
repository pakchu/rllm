...
def test_primary_and_frozen_controls():
    ...
    assert support.conditions(frame, "btc_acceleration_disagreement")[0].tolist() == [False, False, False]
    assert support.conditions(frame, "direction_flip")[1].tolist() == [-1, 1, -1]


def test_clock_uses_five_minute_delay_holds_12h_and_reserves_half_open():
    frame = pd.DataFrame({
...
def test_builder_is_bound_and_outcomes_are_sealed():
    ...
    assert support.PREREG_SHA == "7918b8b251475b035aed2d5113669e9c045a4ed4068179b0232cc72a788ae1b7"
