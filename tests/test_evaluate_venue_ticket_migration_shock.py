...
def _market(rows: int = 310, price: float = 100.0) -> pd.DataFrame:
    return pd.DataFrame(
        {
...
def _schedule(*, side: int = 1, entry: int = 4, exit_: int = 292) -> pd.DataFrame:
    dates = pd.date_range("2020-01-01", periods=320, freq="5min")
    signal = entry - 2
    return pd.DataFrame(
        [
            {
...
def test_strict_mdd_applies_favorable_then_adverse_and_liquidation_cost() -> None:
    ...
    market.loc[291, "low"] = 90.0
    metrics = evaluate.simulate_schedule(
        market,
        _funding(),
        _schedule(),
        period_start=pd.Timestamp("2020-01-01"),
        period_end=pd.Timestamp("2020-01-03"),
        cost_rate=0.001,
        cfg=_cfg(),
    )
...
def test_evaluation_config_matches_frozen_execution_policy() -> None:
    evaluate.verify_evaluation_config_contract()


def test_branch_diagnostics_reject_unknown_primary_branch() -> None:
    changed = _schedule()
    changed.loc[0, "branch"] = "unknown"
    with pytest.raises(ValueError, match="unknown branch"):
        evaluate.branch_diagnostics(
            _market(),
            _funding(),
            changed,
            period_start=pd.Timestamp("2020-01-01"),
...
def test_hash_valid_but_stale_stage1_cannot_unlock_stage2(
    ...
    monkeypatch.setattr(
        ...
        lambda: {"evaluation_source_commit": frozen_commit},
    )
    payload = {
        "candidate_id": "VTMS-288",
        "stage": "stage1_2020_2022",
        ...
        "gate": {"all": True},
        "next_action": "open_2023",
        "evaluation_freeze_sha256": evaluate._sha256(freeze_path),
        "evaluation_source_commit": frozen_commit,
        "source": {
            "cutoff": evaluate.STAGE1_END.isoformat(),
            "last_market_time": "2022-12-31 23:55:00",
            "last_funding_time": "2022-12-31 16:00:00",
        },
    }
    payload["manifest_hash"] = evaluate._canonical_hash(payload)
    result_path = tmp_path / "stage1.json"
    result_path.write_text(json.dumps(payload))
    monkeypatch.setattr(evaluate, "STAGE1_OUTPUT", result_path)
    monkeypatch.setattr(evaluate, "evaluate_stage1", lambda: {"different": True})
...
def test_all_preregistered_component_removals_are_gate_enforced() -> None:
    assert evaluate.COMPONENT_CONTROLS == (
        "no_ticket_level",
        "no_ticket_shock",
        "no_coherence",
        "no_price_acceptance",
...
def test_stage2_placebo_full_gate_requires_both_halves() -> None:
    ...
    halves = {
        "2023_h1": {"absolute_return_pct": 1.0, "trade_count": 35},
        "2023_h2": {"absolute_return_pct": -0.1, "trade_count": 35},
    }
