...
def test_boundary_is_outcome_blind_singleton():
    value = prereg.build()
    assert value["policy_id"] == "HVTTS-24"
    assert value["outcomes_opened"] is False
    assert value["source_incidence_opened"] is False
    assert value["gross9_rows_opened"] is False
    assert value["singleton"] is True
    assert value["policy"]["envelope_periods"] == 20
    assert value["policy"]["bollinger_std_multiplier"] == 2.0
    assert value["policy"]["keltner_atr_multiplier"] == 1.5
