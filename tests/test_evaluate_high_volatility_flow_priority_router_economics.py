...
from training import (
    evaluate_high_volatility_flow_priority_router_economics as evaluator,
)
...
def test_frozen_bindings_and_two_router_authorization() -> None:
    assert evaluator.POLICY_ID == "HVFPR-6"
    assert evaluator.LEVERAGE == 0.5
    assert evaluator.BASE_COST == 0.0006
    assert evaluator.STRESS_COST == 0.0010
    assert len(evaluator.FAMILY_ORDER) == 2
    assert evaluator.ELIGIBLE_CANDIDATES == evaluator.FAMILY_ORDER
    assert evaluator.TRAIN_BONFERRONI_RAW_P_MAX == pytest.approx(0.05)
...
def test_train_bonferroni_is_additional_to_every_normal_gate() -> None:
    assert all(evaluator.economic_checks("test", _primary(pvalue=0.06)).values())
    train = evaluator.economic_checks("train", _primary(pvalue=0.06))
    assert train["cluster_signflip_p_max_0_1"] is True
    assert train["train_bonferroni_raw_weekly_p_max_0_05"] is False
    assert all(evaluator.economic_checks("train", _primary(pvalue=0.05)).values())


def test_raw_ranking_uses_ratio_return_then_frozen_family_order() -> None:
    first, second = evaluator.ELIGIBLE_CANDIDATES
    results = {
        first: {"primary": _primary(ratio=4.0, absolute_return=5.0)},
        second: {"primary": _primary(ratio=4.0, absolute_return=6.0)},
    }
    assert evaluator.rank_train(results) == [second, first]
...
def test_predecessor_is_enforced_before_any_source_opens(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="missing HVFPR-6 predecessor"):
...
@pytest.mark.parametrize("winner_p,passed,frozen,decision", [
    ...
    (0.06, False, None, "terminal_reject_no_substitution"),
])
def test_train_evaluates_both_routers_and_never_substitutes_raw_rank_one(
    ...
    assert report["selection"]["full_bonferroni_family"] == list(evaluator.FAMILY_ORDER)
    assert report["selection"]["raw_train_ranking"] == [winner, evaluator.ELIGIBLE_CANDIDATES[0]]
