...
def test_primary_signal_id_hashes_exact_canonical_identity_bytes() -> None:
    ...
    raw = f"CRLC-336|primary|{accession}|{timestamp}|-2".encode("utf-8")
...
def test_source_control_signal_id_hashes_exact_identity(
    ...
    timestamp = "2025-02-03T04:05:06Z"
    raw = (
        f"CRLC-336|control|{control}|{accession}|{timestamp}|{vote_sum}"
    ).encode("utf-8")
...
def test_deterministic_random_side_uses_first_raw_digest_byte() -> None:
    primary_ids = ("0" * 64, "f" * 64)
    observed = set()
    for primary_id in primary_ids:
        digest = hashlib.sha256(
            f"CRLC-336|{primary_id}|RANDOM_SIDE".encode("utf-8")
        ).digest()
...
def test_same_parent_control_signal_id_hashes_exact_interval_and_side() -> None:
    ...
    raw = (
        f"CRLC-336|control|constant_short|{primary_id}|{entry}|{exit_}|SHORT"
    ).encode("utf-8")
...
def test_same_parent_control_signal_id_rejects_invalid_interval_or_side() -> None:
    primary_id = "a" * 64
    for exit_, side in (
        ("2025-02-03T04:10:06Z", 1),
        ("2025-02-03T04:09:06Z", 1),
        ("2025-02-17T04:10:06Z", 0),
        ("2025-02-17T04:10:06Z", True),
    ):
...
def test_same_parent_constant_and_random_controls_enforce_frozen_side() -> None:
    primary_id = "a" * 64
    entry = "2025-02-03T04:10:06Z"
    exit_ = "2025-02-17T04:10:06Z"
    for control, side, message in (
        ("constant_long", -1, "constant-long"),
        ("constant_short", 1, "constant-short"),
        (
...
def test_same_gross_weights_preserve_nine_configured_gross(
    ...
    assert treatment["crlc"] == candidate_weight
