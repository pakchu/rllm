...
from training import evaluate_high_volatility_credit_transition_variation_phase_router_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVCQVAR-24"
    ...
    assert evaluator.CLOCK["rows"] == 199
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVCQVAR-24 predecessor"):
