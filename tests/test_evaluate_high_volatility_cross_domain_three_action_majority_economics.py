...
from training import evaluate_high_volatility_cross_domain_three_action_majority_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVCM3-8"
    ...
    assert evaluator.CLOCK["rows"] == 260
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVCM3-8 predecessor"):
