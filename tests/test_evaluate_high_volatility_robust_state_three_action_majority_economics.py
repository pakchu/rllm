...
from training import evaluate_high_volatility_robust_state_three_action_majority_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVRM3-8"
    ...
    assert evaluator.CLOCK["rows"] == 206
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVRM3-8 predecessor"):
