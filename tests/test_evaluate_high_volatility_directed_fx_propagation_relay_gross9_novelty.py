...
from training import evaluate_high_volatility_directed_fx_propagation_relay_gross9_novelty as novelty


def test_hvdfxpr_novelty_evaluator_is_outcome_blind_and_frozen():
    source = Path(novelty.__file__).read_text()
    assert novelty.POLICY == "HVDFXPR-12"
...
def test_hvdfxpr_novelty_binds_predecessors():
