import numpy as np
import pandas as pd
from training import build_high_volatility_percentage_price_oscillator_crossover_relay_support as s

def test_cross_side():
 assert s.cross_side(pd.Series([-1.,1.,2.,-1.])).tolist()==[0,1,0,-1]
 old=dict(s.P);s.P.update(fast_periods=2,slow_periods=3,signal_periods=2)
 try: values=s.ppo_values(pd.Series([1.,2.,3.,4.,5.,6.,7.]),pd.Series([True]*7));assert np.isnan(values.ppo.iloc[1]) and np.isnan(values.difference.iloc[2]) and values.ppo.iloc[-1]>0 and np.isfinite(values.difference.iloc[-1])
