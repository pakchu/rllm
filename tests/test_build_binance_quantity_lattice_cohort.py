...
from training import build_binance_quantity_lattice_cohort as builder
...
def _contract(
    ...
    *,
    ...
    for day, payload in payloads.items():
        ...
        observed = builder.aggregate_quantity_lattice_five_minute(raw)
        facts[day.isoformat()] = {
...
def test_build_is_outcome_blind_deterministic_and_rechecks_checksum(tmp_path: Path) -> None:
    ...
    assert manifest["protocol"]["quantity_lattice"].startswith("exact integer milli-BTC")
...
def test_resume_metadata_cannot_redirect_verified_output(tmp_path: Path) -> None:
    ...
    redirected.loc[0, "total_quantity_mbtc"] = 999
    ...
    assert output.loc[0, "total_quantity_mbtc"] != 999
