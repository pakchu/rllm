...
import numpy as np
import pandas as pd
from training import build_high_volatility_causal_trade_arrival_acceleration_inventory_relay_support as s
...
def test_clock_side_follows_completed_return(monkeypatch) -> None:
    ...
    panel=pd.DataFrame({"decision_time":decisions,"feature_available_time":decisions,"eligible":[True,True],"oi_change":[.01,.02],"first_half_trade_count":[100.,100.],"second_half_trade_count":[150.,180.],"completed_return":[.04,-.05],"trade_arrival_acceleration":[.4,.5],"trade_arrival_acceleration_rank":[.8,.9],"realized_variation":[.1,.1],"variation_rank":[.7,.8]})
...
def test_panel_requires_positive_trade_arrival_acceleration_and_causal_ranks(monkeypatch) -> None:
    ...
    bars=pd.DataFrame({"decision_time":times,"first_trade_count":[100.]*5,"second_trade_count":[110.,120.,140.,160.,200.],"completed_return":[.01,.02,-.04,0.,.08],"minute_squared_return":[.001,.002,.003,.004,.005],"source_rows":[480]*5,"distinct_rows":[480]*5,"first_ts":times-pd.Timedelta("8h"),"last_ts":times-pd.Timedelta("1m"),"coherent":[True]*5})
