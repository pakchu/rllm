...
from training import preregister_high_volatility_causal_variance_acceleration_oi_contraction_reversal as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVCVAROICR-8"
    assert value["candidate_family"] == ["HVCVAROICR-8"]
    assert value["policy"] == {
        "history_cycles": 270,
...
def test_causal_average_ticket_boundary() -> None:
    ...
    assert features["variance_acceleration_rank"].startswith("strict-prior")
    assert features["variation_rank"].startswith("strict-prior")
    assert "OHLC" in features["completed_cycle"]
    assert "OHLC" in features["completed_cycle"]
    assert "no taker-flow" in features["eligible"]
