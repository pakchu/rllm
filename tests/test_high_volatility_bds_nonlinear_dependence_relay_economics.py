...
def test_frozen_contract():
 assert e.CONTROLS==('no_departure_tail','no_variation_gate','linear_lag1_autocorrelation_tail','one_decision_stale_dependence','direction_flip','forced_long');assert e.LEVERAGE==.5 and e.BASE_COST==.0006 and e.STRESS_COST==.001;assert 'load_clock_allow_empty' in open(e.__file__).read()
