import json
from training import preregister_high_volatility_btc_factor_residual_recentering_reversal as p

def test_preregistration_freezes_singleton_before_incidence():
    value=p.build();p.validate(value)
    assert value["policy_id"]=="HVBFRRCR-8"
    assert value["candidate_family"]==["HVBFRRCR-8"]
