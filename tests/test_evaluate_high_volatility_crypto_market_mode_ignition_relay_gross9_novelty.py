...
from training import evaluate_high_volatility_crypto_market_mode_ignition_relay_gross9_novelty as novelty


def test_hvcmmi_novelty_evaluator_is_outcome_blind_and_frozen():
    source = Path(novelty.__file__).read_text()
    assert novelty.POLICY == "HVCMMI-8"
...
def test_hvcmmi_novelty_binds_predecessors():
