import pandas as pd
from training import build_daily_aggressive_flow_contrarian_absorption_relay_support as support
def frame():
...
def test_dafcar_follows_price_against_absorbed_flow():
...
def test_dafcar_rejects_agreement_small_flow_or_low_vol():
...
def test_dafcar_price_reversal_is_diagnostic_only():
 a=support.clock(frame());b=support.clock(frame(),"price_reversal");assert len(a)==len(b)==1;assert a.iloc[0].side==-b.iloc[0].side
