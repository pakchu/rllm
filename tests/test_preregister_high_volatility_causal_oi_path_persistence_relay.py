...
import copy
import pytest
from training import preregister_high_volatility_causal_oi_path_persistence_relay as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    ...
    assert value["policy_id"]=="HVCOPR-24"
    assert value["candidate_family"]==["HVCOPR-24"]
    assert value["candidate_family_size"]==1
    assert value["policy"]=={"history_cycles":270,"minimum_history_cycles":180,"path_minutes":480,"half_minutes":240,"efficiency_rank_min":.60,"variation_rank_min":.60,"entry_delay_minutes":5,"hold_hours":24,"leverage":.5,"base_cost_per_notional_side":.0006,"stress_cost_per_notional_side":.0010}
...
def test_causal_boundary_and_no_price_direction_condition() -> None:
    ...
    assert value["features"]["efficiency_rank"].startswith("strict-prior")
    assert "no funding" in value["features"]["eligible"]
    assert value["features"]["half_returns"].startswith("sum of the first")
