import numpy as np
import pandas as pd
from training import build_confirmation_ladder_transaction_density_relay_support as support
...
def test_primary_and_flip():
 f=pd.DataFrame({"source_valid":[True,True],"eligible_state":[False,True],"late_return":[-.1,-.2],"transaction_density_broadening":[False,True],"late_unanimous":[False,True]});a,s=support.active_and_side(f);assert a.tolist()==[False,True] and s.tolist()==[-1,-1];_,flip=support.active_and_side(f,"direction_flip");assert flip.tolist()==[1,1]

def test_controls_are_distinct():
 f=pd.DataFrame({"source_valid":[True,True],"eligible_state":[False,True],"late_return":[-.2,-.2],"transaction_density_broadening":[True,True],"late_unanimous":[False,True]})
 late,_=support.active_and_side(f,"late_unanimity_only"); density,_=support.active_and_side(f,"transaction_density_only")
 assert late.tolist()==[False,True] and density.tolist()==[True,False]
