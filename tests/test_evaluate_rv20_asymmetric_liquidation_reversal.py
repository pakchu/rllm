...
import training.evaluate_rv20_asymmetric_liquidation_reversal as evaluator


def _cell(**changes):
    base = evaluator.Cell(6, 0.6, "both", 0.03, 0.02)
...
def test_adverse_barrier_wins_when_take_and_stop_touch_same_bar():
    ...
    assert trade.price_factor == pytest.approx(0.97)
...
def test_frozen_grid_cardinality_and_canonical_uniqueness():
    ...
    assert len(grid) == 216
    assert len(set(grid)) == 216
    assert {cell.rv20_regime_quantile for cell in grid} == {0.6, 0.75}
    assert {cell.side_mode for cell in grid} == {
        "both", "long_only", "short_only",
    }
    assert {cell.max_hold_hours for cell in grid} == {6, 9, 12}
    assert {cell.stop_loss_pct for cell in grid} == {0.03, 0.04, 0.05}
    assert {cell.take_profit_pct for cell in grid} == {0.02, 0.03, 0.04, 0.05}
...
def test_side_mode_is_applied_in_the_causal_signal():
    ...
    assert evaluator.cell_signal_sides(
        hourly, _cell(side_mode="both")
    ).tolist() == [1, -1]
    assert evaluator.cell_signal_sides(
        hourly, _cell(side_mode="long_only")
    ).tolist() == [1, 0]
    assert evaluator.cell_signal_sides(
        hourly, _cell(side_mode="short_only")
    ).tolist() == [0, -1]
    with pytest.raises(ValueError, match="invalid side_mode"):
        evaluator.cell_signal_sides(hourly, _cell(side_mode="invalid"))
