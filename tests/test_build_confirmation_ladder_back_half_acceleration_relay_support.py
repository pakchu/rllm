import numpy as np
import pandas as pd
from training import build_confirmation_ladder_back_half_acceleration_relay_support as support
...
def test_primary_and_flip():
 f=pd.DataFrame({"source_valid":[True,True],"eligible_state":[False,True],"early_return":[.1,.1],"late_return":[.1,.2],"acceleration_rank":[.7,.9]});a,s=support.active_and_side(f);assert a.tolist()==[False,True] and s.tolist()==[1,1];_,flip=support.active_and_side(f,"direction_flip");assert flip.tolist()==[-1,-1]
