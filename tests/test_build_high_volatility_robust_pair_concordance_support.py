...
from training import build_high_volatility_robust_pair_concordance_support as support
...
def test_exact_pair_concordance_and_disagreement(monkeypatch) -> None:
    ...
    assert list(clock["decision_time"]) == [pd.Timestamp(d3)]
    assert list(clock["side"]) == [1]
    assert counts["no_quorum_decisions"] == 2
    assert counts["disagreement_decisions"] == 1
    assert counts["emitted_decisions"] == 1
