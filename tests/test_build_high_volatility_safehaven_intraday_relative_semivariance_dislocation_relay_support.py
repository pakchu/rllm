...
def frame():
 return pd.DataFrame({'signal_valid':[True]*5,'relative_semivariance_dislocation':[.4,-.3,0.,.2,-.5],'raw_semivariance_difference':[-.1,.2,.3,-.4,.1],'imbalance_rank':[.8,.9,.95,.6,.8],'btc_realized_variation_rank':[.7,.8,.9,.9,.4]})
def test_primary_follows_relative_safehaven_pressure():
...
def test_controls_are_diagnostic():
 ...
 active,side=s.conditions(frame(),'raw_semivariance_difference');assert side[active].tolist()==[1.,-1.,-1.]
...
def test_registration_is_frozen():assert s.PREREG_SHA=='a265aa69b72255c0ecdb64fcb2f73020dee6e85afcd4663bbabf52fe20cc6069'
