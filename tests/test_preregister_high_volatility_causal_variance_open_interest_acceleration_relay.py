...
from training import preregister_high_volatility_causal_variance_open_interest_acceleration_relay as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVCVAROIAR-8"
    assert value["candidate_family"] == ["HVCVAROIAR-8"]
    assert value["policy"] == {
        "history_cycles": 270,
...
def test_causal_average_ticket_boundary() -> None:
    ...
    assert features["current_oi_growth"].startswith("log(")
