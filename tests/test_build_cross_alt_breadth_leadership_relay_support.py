...
def test_primary_and_frozen_controls():
    frame = pd.DataFrame({
        ...
        "btc_return": [0.02, -0.02, 0.02], "confirming_alts": [5, 6, 3],
    ...
    assert support.conditions(frame, "btc_confirmation")[0].tolist() == [False, False, False]
    assert support.conditions(frame, "direction_flip")[1].tolist() == [1, -1, 1]


def test_clock_uses_five_minute_delay_holds_8h_and_reserves_half_open():
    frame = pd.DataFrame({
    ...
    assert clock.iloc[0].exit_time == pd.Timestamp("2024-01-01T16:05:00Z")


def test_builder_is_bound_and_outcomes_are_sealed():
    source = support.BUILDER.read_text()
    assert support.PREREG_SHA == "1ba656a8e4833c0424887bc7821497a8251239af480e28d2a0d1715d07b70a27"
