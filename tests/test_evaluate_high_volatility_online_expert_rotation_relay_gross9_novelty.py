...
from training import evaluate_high_volatility_online_expert_rotation_relay_gross9_novelty as novelty


def test_hvoer_novelty_evaluator_is_outcome_blind_and_frozen():
    source = Path(novelty.__file__).read_text()
    assert novelty.POLICY == "HVOER-8"
...
def test_hvoer_novelty_binds_predecessors():
