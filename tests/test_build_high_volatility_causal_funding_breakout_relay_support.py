...
import numpy as np
import pandas as pd
from training import build_high_volatility_causal_funding_breakout_relay_support as s
...
def test_clock_side_follows_external_range_breakout(monkeypatch) -> None:
    monkeypatch.setattr(s,"stage_for",lambda *_:"train")
    decisions=pd.to_datetime(["2023-07-01T00:00:00Z","2023-07-02T00:00:00Z"])
    panel=pd.DataFrame({"decision_time":decisions,"feature_available_time":decisions,"eligible":[True,True],"funding_rate":[.001,.002],"last_close":[110.,90.],"prior_range_high":[105.,105.],"prior_range_low":[95.,95.],"breakout_side":[1,-1],"absolute_level_rank":[.8,.9],"realized_variation":[.1,.1],"variation_rank":[.7,.8]})
...
def test_panel_requires_exact_gap_and_causal_ranks(monkeypatch) -> None:
    ...
    assert panel["breakout_side"].tolist()==[0,0,1,-1,0]
    assert panel["eligible"].tolist()==[False,False,False,True,False]
