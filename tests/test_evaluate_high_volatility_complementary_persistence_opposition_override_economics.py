...
from training import evaluate_high_volatility_complementary_persistence_opposition_override_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVCPO-8"
    ...
    assert evaluator.CLOCK["rows"] == 373
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVCPO-8 predecessor"):
