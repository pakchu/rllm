...
from training import preregister_high_volatility_cross_domain_temporal_handoff as p
...
def test_manifest_fixes_independent_singleton_and_exact_components() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVTCH-8"
    assert value["memory_id"] == "HVTAMC-8"
    assert value["topology_id"] == "HVITR-8"
    assert value["veto_id"] == "HVRBRAR-8"
    assert value["component_ids"] == ["HVTAMC-8", "HVITR-8", "HVRBRAR-8"]
    assert value["candidate_family"] == ["HVTCH-8"]
...
def test_temporal_handoff_truth_table_is_exact() -> None:
    assert p.temporal_handoff_side(0, 1, 0) == 0
    assert p.temporal_handoff_side(1, 0, 0) == 0
    assert p.temporal_handoff_side(1, 1, 0) == 1
    assert p.temporal_handoff_side(-1, -1, 0) == -1
    assert p.temporal_handoff_side(1, 1, 1) == 1
    assert p.temporal_handoff_side(-1, -1, -1) == -1
    assert p.temporal_handoff_side(1, -1, 0) == 0
    assert p.temporal_handoff_side(-1, 1, 0) == 0
    assert p.temporal_handoff_side(1, 1, -1) == 0
    assert p.temporal_handoff_side(-1, -1, 1) == 0
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.temporal_handoff_side(1, True, 0)
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.temporal_handoff_side(2, 1, 0)
...
def test_manifest_hash_and_component_pass_scalars_detect_drift(
    ...
    drifted["construction"]["current_veto"] = "ignore opposite barrier"
