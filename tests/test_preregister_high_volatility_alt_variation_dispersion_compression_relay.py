...
def test_frozen_cross_alt_dispersion_contract() -> None:
    payload = prereg.build()
    assert payload["policy_id"] == "HVAVDCR-6"
    assert payload["source_incidence_opened"] is False
    assert payload["features"]["universe"] == prereg.SYMBOLS
    assert payload["policy"]["alt_majority_min"] == 4
    assert payload["policy"]["dispersion_rank_max"] == 0.15
