...
from training import evaluate_high_volatility_causal_close_location_open_interest_contraction_reversal_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVCCLOICR-8"
    ...
    assert evaluator.CLOCK["rows"] == 371
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVCCLOICR-8 predecessor"):
