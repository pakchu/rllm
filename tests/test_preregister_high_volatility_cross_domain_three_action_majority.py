...
from training import preregister_high_volatility_cross_domain_three_action_majority as p
...
def test_manifest_fixes_independent_singleton_and_exact_components() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVCM3-8"
    assert value["action_ids"] == ["HVECE-8", "HVIVDR-8", "HVSTBR-8"]
    assert value["component_ids"] == ["HVECE-8", "HVIVDR-8", "HVSTBR-8"]
    assert value["candidate_family"] == ["HVCM3-8"]
...
def test_three_action_majority_truth_table_is_exact() -> None:
    assert p.three_action_majority_side(0, 1, 0) == 0
    assert p.three_action_majority_side(1, 0, 0) == 0
    assert p.three_action_majority_side(1, 1, 0) == 1
    assert p.three_action_majority_side(-1, -1, 0) == -1
    assert p.three_action_majority_side(1, 1, 1) == 1
    assert p.three_action_majority_side(-1, -1, -1) == -1
    assert p.three_action_majority_side(1, -1, 0) == 0
    assert p.three_action_majority_side(-1, 1, 0) == 0
    assert p.three_action_majority_side(1, 1, -1) == 1
    assert p.three_action_majority_side(-1, -1, 1) == -1
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.three_action_majority_side(1, True, 0)
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.three_action_majority_side(2, 1, 0)
...
def test_manifest_hash_and_component_pass_scalars_detect_drift(
    ...
    drifted["construction"]["vote_rule"] = "plurality without quorum"
