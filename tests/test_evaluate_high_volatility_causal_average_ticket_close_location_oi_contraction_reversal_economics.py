...
from training import evaluate_high_volatility_causal_average_ticket_close_location_oi_contraction_reversal_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVCATCLOICR-8"
    ...
    assert evaluator.CLOCK["rows"] == 74
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVCATCLOICR-8 predecessor"):
