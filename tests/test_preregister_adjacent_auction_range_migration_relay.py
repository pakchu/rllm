...
def test_frozen_range_clock_and_side():
    payload = prereg.build()
    features = payload["features"]
    assert "twelve coherent" in features["preceding_auction"]
    assert "min(range_prev,range_cur)" in features["overlap_ratio"]
    assert ">=0.80" in features["eligible_state"]
    assert "<=0.20" in features["eligible_state"]
