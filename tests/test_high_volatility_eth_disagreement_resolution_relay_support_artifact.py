...
def test_hvedr_source_support_pass_is_outcome_sealed():
    ...
    assert [result["support"][stage]["events"] for stage in ("train", "test", "eval", "final")] == [23, 38, 20, 9]


def test_hvedr_support_hashes_bind_frozen_files():
    result = json.loads(support.RESULT.read_text())
