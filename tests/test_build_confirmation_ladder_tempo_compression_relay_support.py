import numpy as np
import pandas as pd
from training import build_confirmation_ladder_tempo_compression_relay_support as support
...
def test_primary_and_flip():
 f=pd.DataFrame({"source_valid":[True,True],"eligible_state":[False,True],"late_return":[-.1,-.2],"tempo_compression":[False,True],"late_unanimous":[False,True]});a,s=support.active_and_side(f);assert a.tolist()==[False,True] and s.tolist()==[-1,-1];_,flip=support.active_and_side(f,"direction_flip");assert flip.tolist()==[1,1]

def test_controls_are_distinct():
 f=pd.DataFrame({"source_valid":[True,True],"eligible_state":[False,True],"late_return":[-.2,-.2],"tempo_compression":[True,True],"late_unanimous":[False,True]})
 late,_=support.active_and_side(f,"late_unanimity_only"); tempo,_=support.active_and_side(f,"tempo_compression_only")
 assert late.tolist()==[False,True] and tempo.tolist()==[True,False]
