...
from training import preregister_high_volatility_cross_alt_magnitude_leadership_continuation as p


def test_outcome_blind_singleton_and_fixed_policy():
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVCAMLC-8"
    assert value["candidate_family"] == ["HVCAMLC-8"]
...
def test_cross_alt_ordering_is_strict_and_causal():
    features = p.build()["features"]
    assert "at least five of six" in features["breadth"]
    assert "strictly greater" in features["magnitude_leadership"]
