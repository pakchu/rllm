...
def test_outcome_blind_evaluator_freeze_is_bound():
 import hashlib,json
 assert hashlib.sha256(economics.FREEZE.read_bytes()).hexdigest()=="c08d0a96ab51026f9f7896ad0765d4289055ecc4fcee8ac1d20b16fd5951b006";p=json.loads(economics.FREEZE.read_text());core={k:v for k,v in p.items() if k!="manifest_hash"};assert p["manifest_hash"]==economics.canonical_hash(core) and p["outcomes_opened"] is False;assert p["evaluator"]["sha256"]==economics.sha256(Path(economics.__file__));novelty,freeze=economics.verify("train");assert novelty["advance_to_economic_outcomes"] is True and freeze["outcomes_opened"] is False
