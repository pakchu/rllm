...
from training import build_korean_cash_volume_sponsorship_relay_support as support
...
def test_primary_and_frozen_controls():
    frame = pd.DataFrame({
        ...
        "upbit_base_volume": [100.0, 100.0, 100.0], "upbit_volume_rank": [0.8, 0.8, 0.7],
        "perp_return": [0.02, -0.02, 0.02], "perp_realized_variation": [0.1, 0.1, 0.1],
    ...
    assert support.conditions(frame, "no_variation_gate")[0].tolist() == [True, True, False]
    assert support.conditions(frame, "no_volume_gate")[0].tolist() == [True, False, True]
...
def test_clock_uses_0805_holds_12h_and_reserves_half_open():
    frame = pd.DataFrame({
        ...
        "upbit_base_volume": [100.0, 100.0], "upbit_volume_rank": [0.8, 0.8],
        "perp_return": [0.02, -0.02], "perp_realized_variation": [0.1, 0.1],
        "variation_rank": [0.7, 0.7],
    })
...
def test_builder_is_bound_and_outcomes_are_sealed():
    ...
    assert support.PREREG_SHA == "7f01e72a3aa092f51617aa5aaff97417c3773e8841d2d7058560eba0e52cd969"
