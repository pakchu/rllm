...
from training import build_high_volatility_cross_alt_passive_absorption_consensus_relay_support as s
...
def test_schema_is_outcome_blind_and_controls_frozen():
 assert not {'pnl','funding','execution_price','gross9','future_return'}.intersection(c.lower() for c in (*s.PANEL_COLUMNS,*s.CLOCK_COLUMNS));assert s.CONTROLS==('no_absorption_tail','no_variation_gate','flow_confirmation_consensus','one_block_stale_absorption','direction_flip','forced_long')
