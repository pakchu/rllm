...
import numpy as np
import pandas as pd
from training import build_high_volatility_spot_lead_open_interest_contraction_continuation_support as s
...
def test_panel_requires_oi_contraction_agreement_spot_lead_and_variation(monkeypatch):
    monkeypatch.setitem(s.POLICY,'minimum_history_cycles',1)
    t=pd.date_range('2023-07-01',periods=5,freq='8h',tz='UTC')
    oi=pd.DataFrame({'decision_time':t,'sum_open_interest':[140.,120.,125.,110.,100.]})
