...
from training import preregister_high_volatility_relative_premium_quiescence_continuation as p


def test_singleton_fixed_quiescence_policy():
    v=p.build();p.validate(v);assert v["policy_id"]=="HVPRQC-12";assert v["candidate_family"]==["HVPRQC-12"]
    assert v["policy"]["relative_premium_activity_rank_max"]==.25;assert v["policy"]["btc_variation_rank_min"]==.65;assert v["policy"]["hold_hours"]==12
...
def test_causal_boundary_and_prior_disclosure():
    ...
    b=v["research_boundary"];assert b["prior_high_premium_activity_HVPASR_train_failure_known"] is True;assert b["relative_ratio_control_existed"] is False;assert b["repair_of_prior_candidate"] is False


def test_manifest_drift_rejected():
    v=copy.deepcopy(p.build());v["policy"]["relative_premium_activity_rank_max"]=.30
