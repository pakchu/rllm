...
from training import build_high_volatility_multi_domain_pairwise_concordance_support as support
        ...
        values.append(
            {
                ...
                "entry_time": entry,
                "exit_time": entry + pd.Timedelta("6h"),
                "side": side,
            }
        )
...
def test_exact_intersection_requires_same_entry_and_strict_same_side() -> None:
    ...
    assert result.loc[0, "exit_time"] == result.loc[0, "entry_time"] + pd.Timedelta("6h")
...
def test_clock_loader_reads_only_sealed_fields_and_validates_hold_and_availability(
    monkeypatch,
) -> None:
    component = prereg.COMPONENT_ORDER[0]
    requested_columns: list[str] = []

    def read_valid(_path, *, usecols):
        requested_columns.extend(usecols)
        return _clock(
            component,
            [("2023-07-01T00:05:00Z", 1, "2023-07-01T00:00:00Z", "2023-07-01T00:00:00Z")],
        )

    monkeypatch.setattr(support.pd, "read_csv", read_valid)
    loaded = support.load_component_clock(component)
    assert tuple(requested_columns) == support.INPUT_COLUMNS
    assert tuple(loaded.columns) == support.INPUT_COLUMNS

    invalid_exit = _clock(
        component,
        [("2023-07-01T00:05:00Z", 1, "2023-07-01T00:00:00Z", "2023-07-01T00:00:00Z")],
    )
    invalid_exit.loc[0, "exit_time"] += pd.Timedelta("5m")
    monkeypatch.setattr(support.pd, "read_csv", lambda _path, *, usecols: invalid_exit)
    with pytest.raises(RuntimeError, match="hold drift"):
        support.load_component_clock(component)

    unavailable = _clock(
        component,
        [("2023-07-01T00:05:00Z", 1, "2023-07-01T00:00:00Z", "2023-07-01T00:10:00Z")],
    )
    monkeypatch.setattr(support.pd, "read_csv", lambda _path, *, usecols: unavailable)
    with pytest.raises(RuntimeError, match="availability after entry"):
        support.load_component_clock(component)
...
def test_support_stats_and_gates_cover_each_stage() -> None:
    ...
    with pytest.raises(RuntimeError, match="every frozen stage"):
        support._support_checks({"train": all_stats["train"]})


def test_frozen_contract_and_artifact_hashes() -> None:
    assert support.PREREG_SHA == "ba21fd9cb7d89de2391497d7db8642d15eafe0a5973606be9c4528774938957b"
    verified = support.verify_frozen_inputs()
    assert tuple(verified) == prereg.COMPONENT_ORDER
    assert all(value["source_support_passed"] is True for value in verified.values())
    ...
    assert all(
        ...
        if name not in {"source_support_passed", "gross9_passed"}
    )
