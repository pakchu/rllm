...
from training import evaluate_high_volatility_btc_factor_residual_flip_continuation_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVBFRRFC-8"
    ...
    assert evaluator.CLOCK["rows"] == 202
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVBFRRFC-8 predecessor"):
