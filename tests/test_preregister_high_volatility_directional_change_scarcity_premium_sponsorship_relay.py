...
from training import preregister_high_volatility_directional_change_scarcity_premium_sponsorship_relay as p


def test_hvdcs_psc_is_outcome_blind_singleton():
    result = p.build()
    assert result["policy_id"] == "HVDCSPSC-8"
    ...
    assert result["research_boundary"]["exact_hvdcs_premium_sponsorship_intersection_incidence_or_outcomes_known"] is False
