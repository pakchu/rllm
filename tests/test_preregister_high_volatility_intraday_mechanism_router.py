...
from training import preregister_high_volatility_intraday_mechanism_router as p
...
def test_manifest_fixes_independent_singleton_and_exact_components() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVIMR-8"
    assert value["action_ids"] == ["HVAUD-8", "HVACR-8", "HVRQA-8"]
    assert value["component_ids"] == ["HVAUD-8", "HVACR-8", "HVRQA-8"]
    assert value["candidate_family"] == ["HVIMR-8"]
...
def test_intraday_router_truth_table_is_exact() -> None:
    assert p.intraday_router_side(0, 1, -1, -1) == 1
    assert p.intraday_router_side(8, 1, -1, 1) == -1
    assert p.intraday_router_side(16, -1, -1, 1) == 1
    assert p.intraday_router_side(0, 0, 1, 1) == 0
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.intraday_router_side(0, True, 0, 0)
    with pytest.raises(ValueError, match="decision hour"):
        p.intraday_router_side(4, 1, 1, 1)
...
def test_known_prior_evidence_disclosed_while_combination_is_sealed() -> None:
    ...
    assert boundary["prior_HVRM3_and_HVRC_outcomes_known"] is True
...
def test_manifest_hash_and_component_pass_scalars_detect_drift(
    ...
    drifted["construction"]["routing_rule"] = "use any active action"
