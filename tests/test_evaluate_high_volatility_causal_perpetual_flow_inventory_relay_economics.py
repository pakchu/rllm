...
from training import evaluate_high_volatility_causal_perpetual_flow_inventory_relay_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVCPOFR-8"
    ...
    assert evaluator.CLOCK["rows"] == 209
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVCPOFR-8 predecessor"):
