...
from training import preregister_high_volatility_causal_average_ticket_trade_arrival_inventory_relay as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVCATTAIR-8"
    assert value["candidate_family"] == ["HVCATTAIR-8"]
    assert value["policy"] == {
        "history_cycles": 270,
...
def test_causal_average_ticket_boundary() -> None:
    ...
    assert "strict positive trade-arrival acceleration" in features["eligible"]
    assert features["trade_arrival_acceleration"].startswith("log(second-half")
