...
from training import preregister_high_volatility_mining_equity_leadership_relay as p


def test_build_is_singleton_outcome_blind_and_canonical():
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVMELR-24"
...
def test_frozen_mechanism_clock_gates_and_controls():
    value = p.build()
    assert value["policy"]["miner_symbols"] == ["RIOT", "MARA", "HUT"]
