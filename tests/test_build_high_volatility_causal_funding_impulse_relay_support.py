...
import numpy as np
import pandas as pd
from training import build_high_volatility_causal_funding_impulse_relay_support as s
...
def test_clock_side_follows_new_funding_impulse(monkeypatch) -> None:
    monkeypatch.setattr(s,"stage_for",lambda *_:"train")
    panel=pd.DataFrame({"decision_time":pd.to_datetime(["2023-07-01T00:00:00Z","2023-07-01T08:00:00Z"]),"feature_available_time":pd.to_datetime(["2023-07-01T00:00:00Z","2023-07-01T08:00:00Z"]),"eligible":[True,True],"previous_funding_change":[-.001,.001],"funding_change":[.001,-.002],"absolute_change_rank":[.8,.9],"realized_variation":[.1,.1],"variation_rank":[.7,.8]})
...
def test_panel_requires_two_exact_gaps_and_opposite_impulses(monkeypatch) -> None:
    monkeypatch.setitem(s.POLICY,"minimum_history_cycles",1)
    times=pd.date_range("2023-07-01",periods=4,freq="8h",tz="UTC")
    funding=pd.DataFrame({"funding_time":times,"funding_rate":[0.,.001,0.,.002]})
    bars=pd.DataFrame({
        "decision_time":times,"first_open":[100.]*4,"last_close":[101.]*4,
        "minute_squared_return":[.0025,.0025,.01,.04],"source_rows":[480]*4,
        "distinct_rows":[480]*4,"first_ts":times-pd.Timedelta("8h"),
        "last_ts":times-pd.Timedelta("1m"),"coherent":[True]*4,
    })
    panel=s.build_panel((funding,bars))
    assert panel["source_valid"].tolist()==[False,False,True,True]
    assert panel["impulse_reversal"].tolist()==[False,False,True,True]
    assert panel["eligible"].tolist()==[False,False,False,True]
    assert panel.loc[3,"previous_funding_change"]==-.001
