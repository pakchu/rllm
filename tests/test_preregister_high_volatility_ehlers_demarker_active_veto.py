...
from training import preregister_high_volatility_ehlers_demarker_active_veto as p
...
def test_manifest_fixes_independent_singleton_and_exact_components() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVEDAV-24"
    assert value["action_ids"] == ["HVELRSI-24", "HVDEM-24"]
    assert value["component_ids"] == ["HVELRSI-24", "HVDEM-24"]
    assert value["candidate_family"] == ["HVEDAV-24"]
...
def test_exact_clock_and_no_tuning_or_hvcav_repair() -> None:
    ...
    assert construction["priority"] == (
        "none; HVELRSI-24 is the fixed primary and HVDEM-24 is only an opposite-side veto"
    )
