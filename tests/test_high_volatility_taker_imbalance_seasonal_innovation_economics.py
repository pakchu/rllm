...
def test_contract():
 assert economics.CONTROLS==("no_innovation_tail","no_variation_gate","raw_net_flow_tail","one_decision_stale_innovation","direction_flip","forced_long");assert economics.LEVERAGE==.5 and economics.BASE_COST==.0006 and economics.STRESS_COST==.001;assert 'load_clock_allow_empty' in open(economics.__file__).read()
