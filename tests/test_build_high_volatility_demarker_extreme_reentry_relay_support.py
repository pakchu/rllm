import numpy as np
import pandas as pd
from training import build_high_volatility_demarker_extreme_reentry_relay_support as s
...
def panel():return pd.DataFrame({"source_valid":[True]*7,"demarker":[.2,.25,.4,.5,.8,.65,.5],"variation_rank":[.8,.8,.8,.8,.8,.4,.8]})
def test_controls():
