...
from training import preregister_high_volatility_cross_domain_primary_confirmation_veto as p
...
def test_manifest_fixes_independent_singleton_and_exact_components() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVCPCV-8"
    assert value["primary_id"] == "HVOIPCSR-8"
    assert value["confirmer_ids"] == ["HVRBRAR-8", "HVLTTC-8"]
    assert value["component_ids"] == ["HVOIPCSR-8", "HVRBRAR-8", "HVLTTC-8"]
    assert value["candidate_family"] == ["HVCPCV-8"]
...
def test_primary_confirmation_veto_truth_table_is_exact() -> None:
    assert p.primary_confirmation_veto_side(0, [0, 0]) == 0
    assert p.primary_confirmation_veto_side(0, [1, 1]) == 0
    assert p.primary_confirmation_veto_side(1, [0, 0]) == 0
    assert p.primary_confirmation_veto_side(-1, [0, 0]) == 0
    assert p.primary_confirmation_veto_side(1, [1, 0]) == 1
    assert p.primary_confirmation_veto_side(1, [0, 1]) == 1
    assert p.primary_confirmation_veto_side(-1, [-1, 0]) == -1
    assert p.primary_confirmation_veto_side(-1, [0, -1]) == -1
    assert p.primary_confirmation_veto_side(1, [1, 1]) == 1
    assert p.primary_confirmation_veto_side(-1, [-1, -1]) == -1
    assert p.primary_confirmation_veto_side(1, [-1, 0]) == 0
    assert p.primary_confirmation_veto_side(-1, [1, 0]) == 0
    assert p.primary_confirmation_veto_side(1, [1, -1]) == 0
    assert p.primary_confirmation_veto_side(-1, [-1, 1]) == 0
    with pytest.raises(ValueError, match="exactly two confirmer sides"):
        p.primary_confirmation_veto_side(1, [1])
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.primary_confirmation_veto_side(1, [True, 0])
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.primary_confirmation_veto_side(2, [1, 0])
...
def test_known_prior_evidence_disclosed_while_combination_is_sealed() -> None:
    ...
    assert boundary["all_three_component_prior_outcomes_known"] is True
    assert boundary["prior_HVCAV_incidence_known"] is True
    assert boundary["prior_HVCAV_outcomes_known"] is True
    assert boundary["known_component_or_HVCAV_evidence_used_to_alter_components"] is False
...
def test_manifest_hash_and_component_pass_scalars_detect_drift(
    ...
    drifted["construction"]["veto_rule"] = "ignore opposite confirmer"
