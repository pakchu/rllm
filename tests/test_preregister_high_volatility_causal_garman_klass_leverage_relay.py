...
import copy
import pytest
from training import preregister_high_volatility_causal_garman_klass_leverage_relay as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    ...
    assert value["policy_id"]=="HVCGKR-24"
    assert value["candidate_family"]==["HVCGKR-24"]
    assert value["candidate_family_size"]==1
    assert value["policy"]=={"history_cycles":270,"minimum_history_cycles":180,"day_minutes":1440,"efficiency_rank_min":.60,"variation_rank_min":.60,"entry_delay_minutes":5,"hold_hours":24,"leverage":.5,"base_cost_per_notional_side":.0006,"stress_cost_per_notional_side":.0010}
...
def test_causal_boundary_and_no_price_direction_condition() -> None:
    ...
    assert value["features"]["garman_klass_variance"].startswith("0.5*")
