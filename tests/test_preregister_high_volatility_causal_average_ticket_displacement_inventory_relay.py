...
from training import preregister_high_volatility_causal_average_ticket_displacement_inventory_relay as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVCATDIR-8"
    assert value["candidate_family"] == ["HVCATDIR-8"]
    assert value["policy"] == {
        "history_cycles": 270,
...
def test_causal_average_ticket_boundary() -> None:
    ...
    assert features["absolute_displacement_rank"].startswith("strict-prior")
...
def test_prior_failures_disclosed_without_repair() -> None:
    ...
    assert boundary["prior_composite_attempt_outcomes_known"] is True
