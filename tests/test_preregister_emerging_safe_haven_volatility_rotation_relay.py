...
def test_esvrr_formula_clock_and_side_are_frozen():
    result = prereg.build()
    features = result["features"]
    assert "exact intersection" in features["common_source_dates"]
    assert features["relative_volatility"] == (
        "log(VXEEM_close/GVZ_close) on exact common source date S"
    )
...
def test_esvrr_hashes_gates_controls_and_boundary_are_frozen():
    result = prereg.build()
    prereg.validate(result)
    assert result["policy_id"] == "ESVRR-12"
    assert result["slug"] == "emerging_safe_haven_volatility_rotation_relay"
    ...
    assert prereg.SOURCES["gvz"]["sha256"] == (
        "eaf949af798669fb6a0d5eb0ee5a3d148a9f2abf4679450e7a9d85c6a5e1bcbb"
    )
    ...
    assert boundary["prior_gvxcr_outcomes_known"] is True
    assert boundary["prior_equity_commodity_volatility_outcomes_known"] is True
    assert boundary["exact_vxeem_gvz_relative_volatility_change_previously_tested"] is False
    assert boundary["candidate_count"] == 1
    assert boundary["grid"] is False
    assert boundary["repair_of_prior_candidate"] is False


def test_esvrr_manifest_round_trip_and_deterministic_cli(tmp_path):
    result = prereg.build()
    ...
    module = "training.preregister_emerging_safe_haven_volatility_rotation_relay"
