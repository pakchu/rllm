...
from training import freeze_cash_auction_transfer_catchup_handoff_evaluator as freeze
...
def test_build_freeze_manifest_is_outcome_blind(
    ...
    assert manifest["outcomes_opened_for_catch12"] is False
    assert manifest["returns_or_prices_loaded_during_freeze"] is False
    ...
    assert manifest["evaluator_document_sha256"] == evaluator.EVALUATOR_DOCUMENT_SHA256
...
def test_build_freeze_manifest_rejects_uncommitted_evaluator(
    ...
    monkeypatch.setattr(
        freeze, "_committed_blob_sha256", lambda _commit, _path: "0" * 64
    )
