...
from training import preregister_high_volatility_causal_average_ticket_flow_late_inventory_relay as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVCATFLIR-8"
    assert value["candidate_family"] == ["HVCATFLIR-8"]
    assert value["policy"] == {
        "history_cycles": 270,
...
def test_causal_average_ticket_boundary() -> None:
    ...
    assert "same-sign nonzero second-half aggressive flow" in features["eligible"]
    assert features["flow_confirmation"].startswith("strict sign")
    assert "last 120" in features["final_two_hour_return"]
    assert features["late_price_confirmation"].startswith("strict sign")
...
def test_prior_failures_disclosed_without_repair() -> None:
    ...
    assert boundary["prior_HVCATFIR_source_novelty_and_train_failure_known"] is True
    assert boundary["prior_HVCATLIR_source_novelty_and_train_failure_known"] is True
