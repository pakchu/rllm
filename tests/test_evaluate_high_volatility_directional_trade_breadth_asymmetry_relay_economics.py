...
from training import evaluate_high_volatility_directional_trade_breadth_asymmetry_relay_economics as economics
...
def test_frozen_accounting_and_stage_contract():
    assert economics.POLICY_ID == "HVDTBA-6"
    ...
    assert economics.CONTROLS == (
        "no_variation_gate",
        "no_breadth_magnitude_gate",
        "quote_volume_directional_breadth",
...
def test_frozen_predecessor_artifact_hashes_are_exact():
    ...
    assert economics.SUPPORT_SHA == (
        "a623eb9facaef1c3c04b00eaf4dd8c8c430aa543187529a20fb4bfd141cdeab3"
    )
    assert economics.CLOCK_SHA == (
        "1c126a15011125161cc3cabb15d0bc2567373d4c501af6ed455e1d5b6d9684d3"
    )
    assert economics.NOVELTY_SHA == (
        "b2440637262bcadfc39959071d52be5fe963f82ee7ad5ba0c5459fa984f9957a"
    )
...
def _bind_verify_inputs(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict:
    ...
    _write_json(prereg, {"policy_id": "HVDTBA-6"})
    _write_json(support, {"policy_id": "HVDTBA-6"})
    _write_json(
        novelty,
        {
