from training import preregister_high_volatility_cross_alt_barrier_rejection_reversal as s
...
def test_fixed_policy_and_reversal_side():
 x=s.build();assert x["policy"]["barrier_bars"]==288 and x["policy"]["minimum_rejection_breadth"]==4;assert x["clock"]["side"].startswith("opposite");assert not x["research_boundary"]["repair_of_prior_candidate"]
