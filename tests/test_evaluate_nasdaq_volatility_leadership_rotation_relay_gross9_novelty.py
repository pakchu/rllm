...
from training import evaluate_nasdaq_volatility_leadership_rotation_relay_gross9_novelty as novelty
...
def test_frozen_predecessor_hashes_and_limits():
    ...
    assert novelty.CLOCK_ROWS == 99
    assert novelty.POLICY == "NVLRR-12"
...
def test_result_write_is_deterministic_and_immutable(tmp_path):
    ...
    with pytest.raises(RuntimeError, match="immutable NVLRR novelty result"):
