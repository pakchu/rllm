...
from training import (
    evaluate_high_volatility_state_priority_router_gross9_novelty as novelty,
)
...
def test_frozen_controls_verify_prereg_support_all_four_router_and_gross9_hashes() -> (
    None
):
    ...
    assert novelty.ELIGIBLE_ROUTERS == prereg.CANDIDATE_FAMILY
    assert tuple(support["eligible_routers_for_combination_gross9"]) == (
        novelty.ELIGIBLE_ROUTERS
    )
    assert len(novelty.ELIGIBLE_ROUTERS) == 4
