from training import preregister_high_volatility_commercial_paper_funding_relay as prereg


def test_boundary_and_causal_embargo():
    value = prereg.build()
    assert value["policy_id"] == "HVCPFR-72"
    assert value["outcomes_opened"] is False
    assert value["source_incidence_opened"] is False
    assert value["gross9_rows_opened"] is False
    assert value["singleton"] is True
    assert value["policy"]["first_revision_delay_days"] == 8
    assert value["policy"]["decision_utc_hour"] == 5
    assert value["research_boundary"]["grid"] is False
    assert value["research_boundary"]["repair_of_prior_candidate"] is False


def test_canonical_hash():
    value = prereg.build()
    core = {key: item for key, item in value.items() if key != "manifest_hash"}
    assert value["manifest_hash"] == prereg.canonical_hash(core)
