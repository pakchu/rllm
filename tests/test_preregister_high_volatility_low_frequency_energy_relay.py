...
from training import preregister_high_volatility_low_frequency_energy_relay as p


def test_hvlfe_is_outcome_blind_singleton():
    result = p.build()
    assert result["policy_id"] == "HVLFE-8"
    ...
    assert result["policy"]["low_frequency_rank_min"] == 0.75
