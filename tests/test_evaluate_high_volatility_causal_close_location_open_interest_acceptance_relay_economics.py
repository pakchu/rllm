...
from training import evaluate_high_volatility_causal_close_location_open_interest_acceptance_relay_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVCCLOIAR-8"
    ...
    assert evaluator.CLOCK["rows"] == 378
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVCCLOIAR-8 predecessor"):
