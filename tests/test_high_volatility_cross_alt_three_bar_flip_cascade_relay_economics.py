...
def test_frozen_contract():
 assert e.CONTROLS==('no_variation_gate','three_alt_cascade','two_bar_flip','one_decision_stale_cascade','direction_flip','forced_long');assert e.LEVERAGE==.5 and e.BASE_COST==.0006 and e.STRESS_COST==.001;assert 'load_clock_allow_empty' in open(e.__file__).read()
