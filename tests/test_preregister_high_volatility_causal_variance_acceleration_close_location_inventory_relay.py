...
from training import preregister_high_volatility_causal_variance_acceleration_close_location_inventory_relay as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVCVARCLIR-8"
    assert value["candidate_family"] == ["HVCVARCLIR-8"]
    assert value["policy"] == {
        "history_cycles": 270,
