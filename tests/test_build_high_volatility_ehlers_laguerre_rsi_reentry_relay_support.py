import numpy as np
import pandas as pd
from training import build_high_volatility_ehlers_laguerre_rsi_reentry_relay_support as s
...
def panel():return pd.DataFrame({"source_valid":[True]*7,"laguerre_rsi":[.1,.15,.3,.5,.9,.75,.5],"variation_rank":[.8,.8,.8,.8,.8,.4,.8]})
def test_controls():
