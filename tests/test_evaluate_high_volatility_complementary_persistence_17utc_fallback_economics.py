...
from training import evaluate_high_volatility_complementary_persistence_17utc_fallback_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVCPF17-8"
    ...
    assert evaluator.CLOCK["rows"] == 435
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVCPF17-8 predecessor"):
