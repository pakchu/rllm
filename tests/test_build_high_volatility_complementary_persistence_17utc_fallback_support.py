...
from training import build_high_volatility_complementary_persistence_17utc_fallback_support as support
...
def test_exact_priority_router_and_fallback(monkeypatch) -> None:
    d0, d1, d2, d3, d4 = "2023-07-01T01:00:00Z", "2023-07-01T09:00:00Z", "2023-07-01T17:00:00Z", "2023-07-02T01:00:00Z", "2023-07-02T09:00:00Z"
    first, second = prereg.ACTION_IDS
    clocks = {
        first: _frame({d0: 1, d1: -1, d3: 1}),
        second: _frame({d1: 1, d2: -1, d3: 1, d4: 1}),
    }
    ...
    assert list(clock["selected_action"]) == [first, first, second, first]
    assert counts["selected_by_action"] == {first: 3, second: 1}
    assert counts["coactive_opposite_side"] == 1
    assert counts["coactive_same_side"] == 1
    assert counts["emitted_decisions"] == 4
    assert counts["inactive_primary_non17_fallback_skipped"] == 1
