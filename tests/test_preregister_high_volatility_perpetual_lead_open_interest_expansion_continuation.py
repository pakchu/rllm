...
from training import preregister_high_volatility_perpetual_lead_open_interest_expansion_continuation as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVPLOIEC-8"
    assert value["candidate_family"] == ["HVPLOIEC-8"]
    assert value["policy"] == {
        "history_cycles": 270,
...
def test_prior_failures_disclosed_without_repair() -> None:
    ...
    assert boundary["exact_perpetual_lead_OI_expansion_incidence_or_outcomes_known"] is False
