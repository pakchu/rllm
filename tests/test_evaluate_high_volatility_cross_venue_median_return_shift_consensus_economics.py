...
from training import evaluate_high_volatility_cross_venue_median_return_shift_consensus_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVCVMRS-8"
    ...
    assert evaluator.CLOCK["rows"] == 369
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVCVMRS-8 predecessor"):
