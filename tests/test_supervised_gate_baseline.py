...
class TestSupervisedGateBaseline(unittest.TestCase):
    def test_run_baseline_produces_leakage_guarded_report(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertTrue(report["leakage_guard"]["side_is_oracle_future_label"])
            self.assertFalse(report["leakage_guard"]["deployable_policy"])
            self.assertIn("eval", report["splits"])

    def test_supervised_side_mode_removes_oracle_side_leakage(self):
        with tempfile.TemporaryDirectory() as td:
            market = Path(td) / "market.csv"
            out = Path(td) / "report.json"
            _market_csv(market)
            report = run_baseline(
                ...
                min_trades=1,
                stride_bars=1,
                side_mode="supervised",
            )
            self.assertFalse(report["leakage_guard"]["side_is_oracle_future_label"])
            self.assertTrue(report["leakage_guard"]["deployable_policy"])
            self.assertEqual(report["model"]["side_mode"], "supervised")
            self.assertIn("eval", report["splits"])
