...
def test_rvsbr_source_support_pass_is_outcome_sealed():
    ...
    assert [result["support"][stage]["events"] for stage in ("train", "test", "eval", "final")] == [18, 48, 20, 17]


def test_rvsbr_support_hashes_bind_frozen_files():
    result = json.loads(support.RESULT.read_text())
