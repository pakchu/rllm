...
from training import evaluate_high_volatility_robust_pair_concordance_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVRC-8"
    ...
    assert evaluator.CLOCK["rows"] == 71
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVRC-8 predecessor"):
