...
import copy
import pytest
from training import preregister_high_volatility_causal_extreme_displacement_open_interest_reversal as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    ...
    assert value["policy_id"]=="HVCEDOIR-24"
    assert value["candidate_family"]==["HVCEDOIR-24"]
    assert value["candidate_family_size"]==1
    assert value["policy"]=={"history_decisions":540,"minimum_history_decisions":360,"prior_close_periods":22,"current_auction_minutes":240,"variation_minutes":1440,"displacement_rank_min":.70,"variation_rank_min":.60,"entry_delay_minutes":5,"hold_hours":24,"leverage":.5,"base_cost_per_notional_side":.0006,"stress_cost_per_notional_side":.0010}
...
def test_causal_boundary_and_no_price_direction_condition() -> None:
    ...
    assert "exact OI expansion" in value["features"]["eligible"]
    assert "D-4h and D" in value["features"]["open_interest"]
    assert value["features"]["dominant_displacement"].startswith("strictly larger")
    assert value["mechanism"]["side"].startswith("long when")
