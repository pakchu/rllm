import pandas as pd
from training import build_high_volatility_btc_factor_residual_recentering_reversal_support as s

def sample() -> pd.DataFrame:
    rows=[]
    for i,(btc,alts) in enumerate([(0.06,[.01,.011,.009,.012,.008,.01]),(.04,[.01,.011,.009,.012,.008,.01])]):
...
def test_same_sign_contracting_residual_fades_current_side():
...
def test_expanding_residual_is_not_eligible():
    frame=sample();frame.loc[1,"btc_return"]=.08;assert not s.prepare(frame).iloc[1]["eligible"]
