...
from training import build_high_volatility_exchange_deposit_pressure_relay_support as support


def raw_row(day: str, completion: str, deposit: str = "2500000.5") -> dict[str, str]:
    return {
        "asset": "btc",
        "time": f"{day}T00:00:00.000000000Z",
        "FlowInExNtv": deposit,
        "AssetEODCompletionTime": completion,
        "FlowInExNtv-status": "reviewed",
        "FlowInExNtv-status-time": f"{day}T00:00:00.000000000Z",
    }
...
def test_source_row_strict_schema_exact_value_and_completion_timestamp() -> None:
    ...
    assert parsed["FlowInExNtv"] == "2500000.5"
    ...
    with pytest.raises(ValueError, match="non-empty string"):
        support.parse_source_row({**raw_row("2022-01-01", "1641081601"), "FlowInExNtv-status": ""})
    with pytest.raises(ValueError, match="exact ISO-8601"):
        support.parse_source_row({
            **raw_row("2022-01-01", "1641081601"),
            "FlowInExNtv-status-time": "not-a-time",
        })
...
def test_pagination_contract_and_response_chain_are_frozen(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert frame.FlowInExNtv.tolist() == ["2500000.5", "2500000.5"]
...
def test_panel_uses_variable_completion_clock_deposit_change_and_decision_window(
    ...
    assert panel.deposit_log_change.iloc[1] == pytest.approx(np.log(1.1))
    assert panel.deposit_log_change.iloc[2] == pytest.approx(np.log(0.9))
...
def test_panel_rejects_boundary_late_and_invalid_prior_rows_without_imputation(
    ...
    source = pd.DataFrame({
        ...
        "FlowInExNtv": [100.0, 101.0, 102.0, 103.0],
    })
    ...
    assert panel.deposit_log_change.isna().all()
...
def clock_panel() -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        "source_valid": True,
        "minute_count": 1440,
        "FlowInExNtv": [100.0, 90.0, 120.0, 80.0],
        "deposit_log_change": [0.1, -0.1, 0.2, -0.2],
        "deposit_level_rank": [0.8, 0.2, 0.9, 0.1],
        "btc_variation": 1.0,
...
def test_clock_side_variable_entry_half_open_reservation_and_controls() -> None:
    ...
    assert support.CONTROLS == (
        "no_btc_variation_gate", "exchange_deposit_direction_flip",
        "one_day_stale_deposit_change", "exchange_deposit_level_rank",
        "same_clock_forced_long",
    )
    primary_side = support.active(panel)[1]
    assert support.active(panel, "exchange_deposit_direction_flip")[1].equals(-primary_side)
    assert support.active(panel, "same_clock_forced_long")[1].eq(1).all()
    assert support.active(panel, "exchange_deposit_level_rank")[1].tolist() == [-1, 1, -1, 1]
