...
from training import evaluate_high_volatility_causal_oi_semivariance_sponsorship_relay_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVCOSR-24"
    ...
    assert evaluator.CLOCK["rows"] == 90
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVCOSR-24 predecessor"):
