...
from training import preregister_high_volatility_korean_price_leadership_continuation as p


def test_outcome_blind_singleton_and_fixed_policy():
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVKPLC-8"
    assert value["candidate_family"] == ["HVKPLC-8"]
...
def test_korean_leadership_is_strict_and_causal():
    features = p.build()["features"]
    assert features["usdkrw_return"].endswith("no fill")
    assert "strict with no fitted magnitude threshold" in features["korean_price_leadership"]
