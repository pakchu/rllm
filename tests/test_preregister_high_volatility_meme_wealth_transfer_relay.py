...
from training import preregister_high_volatility_meme_wealth_transfer_relay as p


def test_build_is_singleton_outcome_blind_and_canonical():
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVMWTR-24"
...
def test_frozen_mechanism_clock_gates_and_controls():
    value = p.build()
    assert value["policy"]["meme_symbols"] == ["GME", "AMC"]
    assert value["policy"]["benchmark_symbol"] == "SPY"
    assert value["policy"]["beta_prior_sessions"] == 60
    assert value["policy"]["beta_prior_minimum"] == 40
    assert value["policy"]["magnitude_midrank_min"] == 0.70
