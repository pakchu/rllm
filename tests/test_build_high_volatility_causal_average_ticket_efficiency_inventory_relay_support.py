...
import numpy as np
import pandas as pd
from training import build_high_volatility_causal_average_ticket_efficiency_inventory_relay_support as s
...
def test_clock_side_follows_completed_return(monkeypatch) -> None:
    ...
    panel=pd.DataFrame({"decision_time":decisions,"feature_available_time":decisions,"eligible":[True,True],"oi_change":[.01,.02],"first_half_quote_turnover":[1000.,1200.],"second_half_quote_turnover":[1800.,2200.],"first_half_trade_count":[100.,100.],"second_half_trade_count":[150.,180.],"first_half_average_ticket":[10.,12.],"second_half_average_ticket":[12.,12.222222],"completed_return":[.04,-.05],"path_efficiency":[.7,.8],"path_efficiency_rank":[.8,.9],"average_ticket_acceleration":[.1823,.01835],"average_ticket_acceleration_rank":[.8,.9],"realized_variation":[.1,.1],"variation_rank":[.7,.8]})
...
def test_panel_requires_ticket_acceleration_efficiency_and_causal_ranks(monkeypatch) -> None:
    ...
    bars=pd.DataFrame({"decision_time":times,"first_quote_turnover":[1000.]*5,"first_trade_count":[100.]*5,"second_quote_turnover":[1100.,1320.,1680.,1600.,3000.],"second_trade_count":[110.,120.,140.,160.,200.],"completed_return":[.01,.02,-.04,0.,.08],"absolute_return_sum":[.02,.03,.05,.06,.1],"minute_squared_return":[.001,.002,.003,.004,.005],"source_rows":[480]*5,"distinct_rows":[480]*5,"first_ts":times-pd.Timedelta("8h"),"last_ts":times-pd.Timedelta("1m"),"coherent":[True]*5})
