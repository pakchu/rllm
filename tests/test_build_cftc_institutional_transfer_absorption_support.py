...
from training import build_cftc_institutional_transfer_absorption_support as support
...
def test_real_support_passes_without_opening_outcomes(tmp_path: Path) -> None:
    ...
    assert report["source_rows"] == 299
    assert report["source_complete_rows"] == 298
    assert report["source_quarantined_rows"] == 1
    assert all(report["support_checks"].values())
    assert report["clocks"]["sha256"] == hashlib.sha256(
        clocks.read_bytes()
    ).hexdigest()
...
def test_clock_summaries_and_controls_are_frozen(tmp_path: Path) -> None:
    ...
    assert primary["stage1_2020_2022"] == {
        ...
        "max_single_month_share": 5 / 98,
    }
    assert primary["stage2_2023"] == {
        "events": 23,
        "longs": 16,
        "shorts": 7,
        "special_publication_overrides": 2,
        "max_single_month_count": 3,
        "max_single_month_share": 3 / 23,
    }
