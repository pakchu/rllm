...
from training import evaluate_high_volatility_spot_underwater_autocorrelation_router_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVSAUDAR-8"
    ...
    assert evaluator.CLOCK["rows"] == 183
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVSAUDAR-8 predecessor"):
