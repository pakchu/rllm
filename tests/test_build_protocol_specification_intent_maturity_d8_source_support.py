...
def _synthetic_origin(tmp_path: Path) -> tuple[Path, str, str]:
    ...
    _run(git, "config", "user.name", "PSIM D8 Test", cwd=work)
    _run(
        git,
        "config",
        "user.email",
...
def test_self_check_is_synthetic_only_and_opens_no_source(
    ...
    assert payload["policy_id"] == "PSIM-D8"
    ...
    assert payload["checks"][
        "synthetic_all_cards_bind_relation_subcards"
    ] is True
    assert payload["mechanism_probe"]["result_hash"] == (
        runner.RELATION_SUBCARD_MECHANISM_PROBE_RESULT_HASH
    )
    assert payload["d7_grammar_mechanism_probe"]["result_hash"] == (
...
def test_runner_namespace_authority_and_import_surface_are_frozen() -> None:
    ...
    assert runner.DEFAULT_SOURCE_ROOT == Path("/tmp/psim-d8-source")
    assert runner.DEFAULT_SOURCE_ROOT != d6_runner.DEFAULT_SOURCE_ROOT
    assert runner.DEFAULT_SOURCE_ROOT != d7_runner.DEFAULT_SOURCE_ROOT
    assert runner.SEALED_REF == "refs/psim-d8/sealed-tip"
    assert runner.PREREGISTRATION_COMMIT == (
        "17cae3081abd76319f3dc42c4dbf8e135dd64bfb"
    )
    ...
    assert runner._load_d7_grammar_mechanism_probe() == (
        d7_mechanism.build_probe()
    )
    assert runner._load_relation_subcard_mechanism_probe() == (
        d8_mechanism.build_probe()
    )
...
def _same_day_relation_events(
    ethereum_count: int,
    bitcoin_count: int,
) -> list[runner.ProposalEvent]:
    template = runner.synthetic_events()[0]
    event_day = date(2021, 6, 1)
    available_at = {
        ...
        for row in runner.core.prereg.ARCHIVE_SCHEDULES
    }
    return [
        dataclasses.replace(
            template,
            ...
            event_id=runner.canonical_hash(
                {
                    "protocol": protocol,
                    "same_day_relation_index": index,
                }
            ),
            first_parent_index=index,
            effective_day=event_day,
            available_at=available_at,
        )
        for protocol, count, offset in (
            ("ethereum", ethereum_count, 0),
            ("bitcoin", bitcoin_count, ethereum_count),
        )
        for index in range(offset, offset + count)
    ]


def _relation_day_card(
    cards: Sequence[runner.DailyCard],
) -> runner.DailyCard:
    return next(
        card
        for card in cards
        if card.schedule == runner.core.prereg.ARCHIVE_SCHEDULES[0].name
        and card.decision_at.date() == date(2021, 6, 1)
    )


def test_d8_cards_are_d7_equal_except_subcard_manifest_and_hashes() -> None:
    d7_cards = d7_runner.build_daily_cards(d7_runner.synthetic_events())
    d8_cards = runner.build_daily_cards(runner.synthetic_events())

    assert len(d8_cards) == len(d7_cards)
    for inherited, successor in zip(d7_cards, d8_cards, strict=True):
        successor_payload = copy.deepcopy(successor.local_payload)
        manifest = successor_payload.pop("relation_subcard_manifest")
        assert successor_payload == inherited.local_payload
        assert successor.schedule == inherited.schedule
        assert successor.decision_at == inherited.decision_at
        assert successor.split == inherited.split
        assert successor.event_ids == inherited.event_ids
        assert successor.eligible == inherited.eligible
        assert manifest["schedule"] == successor.schedule
        assert manifest["decision_at"] == runner.format_time(
            successor.decision_at
        )
        assert manifest["relation_unit_count"] == len(
            successor.local_payload["relation_units"]
        )
        d8_mechanism.validate_logical_daily_card_envelope(
            runner._logical_card_envelope(successor)
        )


@pytest.mark.parametrize(
    ("ethereum_count", "bitcoin_count", "expected_units", "expected_slices"),
    [
        (8, 8, 64, [64]),
        (13, 5, 65, [64, 1]),
        (9, 8, 72, [64, 8]),
...
def test_runner_partitions_relation_overflow_without_new_logical_cards(
    ethereum_count: int,
    bitcoin_count: int,
    expected_units: int,
    expected_slices: list[int],
) -> None:
    events = _same_day_relation_events(ethereum_count, bitcoin_count)
    cards = runner.build_daily_cards(events)
    card = _relation_day_card(cards)
    manifest = card.local_payload["relation_subcard_manifest"]

    assert len(cards) == (
        len(runner._decision_times())
        * len(runner.core.prereg.ARCHIVE_SCHEDULES)
    )
    assert len(card.local_payload["relation_units"]) == expected_units
    assert manifest["relation_unit_count"] == expected_units
    assert [
        row["relation_unit_count"] for row in manifest["subcards"]
    ] == expected_slices
    assert all(
        row["relation_unit_count"]
        <= d8_mechanism.MAX_RELATION_UNITS_PER_SUBCARD
...
def test_daily_gate_rejects_relation_subcard_manifest_tamper(
    mutation: str,
) -> None:
    cards = runner.build_daily_cards(_same_day_relation_events(13, 5))
    card = _relation_day_card(cards)
    card_index = cards.index(card)
    payload = copy.deepcopy(card.local_payload)
    manifest = payload["relation_subcard_manifest"]

    if mutation == "missing_manifest":
        payload.pop("relation_subcard_manifest")
    elif mutation == "roster_hash":
        manifest["complete_relation_roster_sha256"] = "0" * 64
    elif mutation == "payload_hash":
        manifest["subcards"][0]["subcard_payload_sha256"] = "0" * 64
    elif mutation == "prior_hash":
        manifest["subcards"][0]["prior_subcard_hash"] = "0" * 64
    elif mutation == "range":
        manifest["subcards"][0]["end_exclusive"] = 65
        manifest["subcards"][0]["relation_unit_count"] = 65
    elif mutation == "cap":
        manifest["maximum_relation_units_per_subcard"] = 65
    elif mutation == "subcard_order":
        manifest["subcards"].reverse()
    elif mutation == "duplicate_subcard":
        manifest["subcards"][1] = copy.deepcopy(
            manifest["subcards"][0]
        )
    elif mutation == "missing_subcard":
        manifest["subcards"].pop()
    elif mutation == "subcard_count":
        manifest["subcard_count"] += 1
    else:
        raise AssertionError("unknown mutation")

    local_sha = runner.canonical_hash(payload)
    card_hash = runner.canonical_hash(
        {
            "schedule": card.schedule,
            "decision_at": runner.format_time(card.decision_at),
    ...
    tampered_cards = list(cards)
    tampered_cards[card_index] = tampered
    gate = runner.gate_daily_cards(tampered_cards)

    assert gate.passed is False
    assert "relation_subcard_manifest" in gate.failure
    assert gate.metrics["relation_subcards"]["validation_errors"]


def test_controls_rebuild_and_validate_relation_subcard_manifests() -> None:
    events = runner.synthetic_events()
    baseline_cards = runner.build_daily_cards(events)
    baseline = runner._card_map(baseline_cards)
    changed_manifest_pairs = 0

    for control in runner.core.prereg.RELATION_CONTROLS:
        controlled_cards = runner.build_daily_cards(
            runner.transform_events(events, control)
        )
        controlled = runner._card_map(controlled_cards)
        assert set(controlled) == set(baseline)
        assert runner.gate_daily_cards(controlled_cards).passed
        for key, card in controlled.items():
            d8_mechanism.validate_logical_daily_card_envelope(
                runner._logical_card_envelope(card)
            )
            if (
                card.local_payload["relation_units"]
                != baseline[key].local_payload["relation_units"]
            ):
                changed_manifest_pairs += 1
                assert card.local_payload[
                    "relation_subcard_manifest"
                ] != baseline[key].local_payload[
                    "relation_subcard_manifest"
                ]

    assert changed_manifest_pairs > 0
    assert all(
        row["passed"]
        for row in runner.build_control_metrics(
            events,
            baseline_cards,
        ).values()
    )
...
def test_public_migration_authorizer_is_exact_frozen_and_noninjectable() -> None:
    ...
    assert manifest_hash == prereg.d7.d6.D5_EPISODE_RECEIPT_MANIFEST_HASH
...
def test_real_local_clone_uses_d6_hydration_with_fresh_d8_namespace(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    assert (tmp_path / "source" / ".psim-d8-traces").is_dir()
    assert not (tmp_path / "source" / ".psim-d7-traces").exists()
...
def test_artifacts_and_controls_preserve_chunk_only_model_boundary() -> None:
    ...
    assert b'"relation_subcard_manifest"' in card_bytes
    card_rows = [
        json.loads(line)
        for line in card_bytes.decode("utf-8").splitlines()
    ]
    assert len(card_rows) == len(cards)
    assert all(
        row["local_payload"]["relation_subcard_manifest"][
    ...
    assert all(
        len(row["local_payload"]["relation_units"])
        == row["local_payload"]["relation_subcard_manifest"][
            "relation_unit_count"
        ]
        for row in card_rows
...
def test_official_run_refuses_without_seal_before_source_access(
    ...
    monkeypatch.setattr(
        ...
        lambda: (_ for _ in ()).throw(
            RuntimeError("synthetic missing D8 seal")
        ),
    )
    with pytest.raises(RuntimeError, match="missing D8 seal"):
...
def test_dual_epoch_verification_combines_archive_and_current_counts(
    ...
    assert receipt["current_d8"] == current
