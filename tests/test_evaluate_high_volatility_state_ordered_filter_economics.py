...
from training import evaluate_high_volatility_state_ordered_filter_economics as evaluator
...
def test_frozen_bindings_and_three_candidate_authorization() -> None:
    assert evaluator.POLICY_ID == "HVSOF-8"
    ...
    assert gross9["eligible_candidates_for_economics"] == list(evaluator.ELIGIBLE_CANDIDATES)
...
def test_train_bonferroni_is_additional_to_every_normal_gate() -> None:
    assert all(evaluator.economic_checks("test", _primary(pvalue=0.02)).values())
    train = evaluator.economic_checks("train", _primary(pvalue=0.02))
    assert train["cluster_signflip_p_max_0_1"] is True
    assert train["train_bonferroni_raw_weekly_p_max_0_1_over_6"] is False
    assert all(evaluator.economic_checks("train", _primary(pvalue=0.1 / 6)).values())


def test_raw_ranking_uses_ratio_return_then_frozen_family_order() -> None:
    first, second, third = evaluator.ELIGIBLE_CANDIDATES
    results = {
    ...
    assert evaluator.rank_train(results) == [third, second, first]
...
def test_predecessor_is_enforced_before_any_source_opens(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="missing HVSOF-8 predecessor"):
...
def test_train_evaluates_all_three_and_never_substitutes_raw_rank_one(
    ...
    assert report["selection"]["raw_train_ranking"] == [winner, evaluator.ELIGIBLE_CANDIDATES[2], evaluator.ELIGIBLE_CANDIDATES[0]]
