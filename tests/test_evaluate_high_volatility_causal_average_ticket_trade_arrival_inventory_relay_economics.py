...
from training import evaluate_high_volatility_causal_average_ticket_trade_arrival_inventory_relay_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVCATTAIR-8"
    ...
    assert evaluator.CLOCK["rows"] == 99
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVCATTAIR-8 predecessor"):
