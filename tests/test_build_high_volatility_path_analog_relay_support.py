...
def test_prediction_is_reconstructed_from_frozen_analog_references():
    ...
    training = pd.read_csv(support.trained.TRAINING, compression="gzip")
    references = training[training.reference_row.astype(str).str.lower().eq("true")]
    matrix = references.loc[:, support.trained.FEATURES].to_numpy(float)
    matrix = (matrix - np.asarray(frozen["mean"])) / np.asarray(frozen["scale"])
    labels = references.label.to_numpy(float)
    order = pd.to_datetime(references.decision_time, utc=True).astype("int64").to_numpy()
    query = (vector - np.asarray(frozen["mean"])) / np.asarray(frozen["scale"])
    expected = support.trained.analog_prediction(query, matrix, labels, order)
