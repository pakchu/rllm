...
from training import evaluate_high_volatility_exchange_deposit_pressure_relay_economics as evaluator


def test_frozen_bindings_controls_and_loader_order() -> None:
    assert evaluator.POLICY_ID == "HVEXDP-24"
    ...
    assert evaluator.FREEZE == Path(
        "results/high_volatility_exchange_deposit_pressure_relay_economic_evaluator_freeze_2026-08-13.json"
    )
