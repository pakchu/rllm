...
from training import evaluate_high_volatility_top_two_causal_expert_concordance_relay_gross9_novelty as novelty


def test_hvtcec_novelty_evaluator_is_outcome_blind_and_frozen():
    source = Path(novelty.__file__).read_text()
    assert novelty.POLICY == "HVTCEC-24"
...
def test_hvtcec_novelty_binds_predecessors():
