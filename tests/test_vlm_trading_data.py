...
class TestVlmTradingData(unittest.TestCase):
    ...
    def test_build_samples_edge_state_prompt_mode(self):
        samples = build_vlm_training_samples(
            market_df=_rich_market_df(),
            timeframe="5m",
            window_size=32,
            ...
            sample_mode="uniform",
            prompt_style="hybrid",
            prompt_feature_mode="edge_state_v2",
        )
        self.assertEqual(len(samples), 4)
        prompt = samples[0].prompt
        self.assertIn("Past Return 1h:", prompt)
        self.assertIn("Past Path Drawdown 6h:", prompt)
        self.assertIn("Realized Vol 8h:", prompt)
        self.assertIn("Playbook:", prompt)
        self.assertIn("Cross Market Pressure:", prompt)
        self.assertIn("Tags:", prompt)
    ...
    def test_build_samples_edge_state_v4_prompt_mode(self):
        samples = build_vlm_training_samples(
            market_df=_rich_market_df(),
            timeframe="5m",
            window_size=96,
            ...
            sample_mode="uniform",
            prompt_style="hybrid",
            prompt_feature_mode="edge_state_v4",
        )
        self.assertEqual(len(samples), 4)
        prompt = samples[0].prompt
        self.assertIn("Regime Memory:", prompt)
        self.assertIn("Regime Trap Risk:", prompt)
        self.assertIn("Similar Regime Count:", prompt)
        self.assertIn("Similar Regime Forward 36:", prompt)
        self.assertNotIn("Target", prompt)
        self.assertNotIn("Future", prompt)
