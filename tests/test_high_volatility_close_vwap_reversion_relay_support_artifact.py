...
def test_hvcvr_source_support_pass_is_outcome_sealed():
 r=json.loads(support.RESULT.read_text());assert r["policy_id"]=="HVCVR-2";assert r["support_passed"] is True;assert r["decision"]=="pass_to_novelty";assert r["advance_to_gross9_novelty"] is True;assert r["advance_to_economic_outcomes"] is False;assert r["postentry_return_pnl_execution_price_opened"] is False;assert r["gross9_rows_opened"] is False;assert [r["support"][s]["events"] for s in ("train","test","eval","final")]==[176,424,426,210]
def test_hvcvr_support_hashes_bind_frozen_files():
