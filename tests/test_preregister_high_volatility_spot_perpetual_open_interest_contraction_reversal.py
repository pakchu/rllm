...
from training import preregister_high_volatility_spot_perpetual_open_interest_contraction_reversal as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVSPOICR-8"
    assert value["candidate_family"] == ["HVSPOICR-8"]
    assert value["policy"] == {
        "history_cycles": 270,
...
def test_causal_spot_perpetual_oi_boundary() -> None:
    ...
    assert features["oi_contraction"].startswith("strict negative")
    assert features["variation_rank"].startswith("strict-prior")
    assert "strict directional perpetual overshoot" in features["eligible"]
    assert features["directional_perpetual_overshoot"].startswith("sign(perpetual_return)")
    assert features["direction_agreement"].startswith("perpetual_return and spot_return")
    assert value["mechanism"]["side"].startswith("opposite strict sign")
...
def test_prior_failures_disclosed_without_repair() -> None:
    ...
    assert boundary["prior_OI_contraction_family_failures_known"] is True
    assert boundary["exact_spot_perpetual_overshoot_OI_contraction_incidence_or_outcomes_known"] is False
