...
from training import preregister_high_volatility_causal_average_ticket_close_location_oi_contraction_reversal as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVCATCLOICR-8"
    assert value["candidate_family"] == ["HVCATCLOICR-8"]
    assert value["policy"] == {
        "history_cycles": 270,
