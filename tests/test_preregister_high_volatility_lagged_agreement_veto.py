...
from training import preregister_high_volatility_lagged_agreement_veto as p
...
def test_manifest_fixes_independent_singleton_and_exact_components() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVLAV-8"
    assert value["action_ids"] == ["HVCASPC-8", "HVDCS-8"]
    assert value["component_ids"] == ["HVCASPC-8", "HVDCS-8"]
    assert value["candidate_family"] == ["HVLAV-8"]
...
def test_lagged_agreement_veto_truth_table_is_exact() -> None:
    assert p.lagged_veto_side(0, 1) == 0
    assert p.lagged_veto_side(1, 0) == 1
    assert p.lagged_veto_side(1, 1) == 1
    assert p.lagged_veto_side(-1, -1) == -1
    assert p.lagged_veto_side(1, -1) == 0
    assert p.lagged_veto_side(-1, 1) == 0
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.lagged_veto_side(1, True)
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.lagged_veto_side(2, 1)
...
def test_exact_clock_and_no_tuning_or_hvcav_repair() -> None:
    ...
    assert construction["priority"] == (
        "none; HVCASPC-8 defines incidence and exact D-3h HVDCS-8 is only a causal veto"
    )
