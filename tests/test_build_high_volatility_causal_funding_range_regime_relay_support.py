...
import numpy as np
import pandas as pd
from training import build_high_volatility_causal_funding_range_regime_relay_support as s
...
def test_clock_side_follows_range_midpoint_transition(monkeypatch) -> None:
    monkeypatch.setattr(s,"stage_for",lambda *_:"train")
    decisions=pd.to_datetime(["2023-07-01T00:00:00Z","2023-07-02T00:00:00Z"])
    panel=pd.DataFrame({"decision_time":decisions,"feature_available_time":decisions,"eligible":[True,True],"funding_rate":[.001,.002],"previous_close":[99.,101.],"last_close":[101.,99.],"prior_range_midpoint":[100.,100.],"regime_cross_side":[1,-1],"absolute_level_rank":[.8,.9],"realized_variation":[.1,.1],"variation_rank":[.7,.8]})
...
def test_panel_requires_exact_gap_and_causal_ranks(monkeypatch) -> None:
    ...
    monkeypatch.setitem(s.POLICY,"prior_range_cycles",2)
    times=pd.date_range("2023-07-01",periods=5,freq="8h",tz="UTC")
    funding=pd.DataFrame({"funding_time":times,"funding_rate":[.001,.001,.002,.003,.004]})
    bars=pd.DataFrame({
        "decision_time":times,"first_open":[100.]*5,
        "last_close":[100.,101.,103.,95.,100.],"cycle_high":[101.,102.,104.,101.,102.],
        "cycle_low":[99.,98.,97.,94.,98.],
    ...
    assert panel["regime_cross_side"].tolist()==[0,0,0,-1,1]
    assert panel["eligible"].tolist()==[False,False,False,True,True]
