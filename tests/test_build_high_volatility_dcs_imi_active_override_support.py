...
from training import build_high_volatility_dcs_imi_active_override_support as support
...
def test_exact_active_opposite_side_override(monkeypatch) -> None:
    d0, d1, d2, d3 = "2023-07-01T01:00:00Z", "2023-07-01T09:00:00Z", "2023-07-01T17:00:00Z", "2023-07-02T01:00:00Z"
    ...
    assert list(clock["decision_time"]) == [pd.Timestamp(d0), pd.Timestamp(d1), pd.Timestamp(d3)]
    assert list(clock["side"]) == [1, 1, 1]
    assert list(clock["selected_action"]) == [first, second, first]
    assert counts["primary_decisions"] == 3
    assert counts["override_absent"] == 1
    assert counts["override_opposite_side"] == 1
    assert counts["override_same_side"] == 1
    assert counts["emitted_decisions"] == 3
