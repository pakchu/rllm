...
RESULT=Path('results/high_volatility_cross_alt_channel_position_migration_relay_gross9_novelty_2026-08-13.json');EXPECTED='75c14f838c82d87b562c0f264c5c28bf6528543d91cc81f8624d2a451856ce98'
def test_novelty_pass_is_immutable_and_economics_remained_sealed():
 ...
 assert x['policy_id']=='HVCACM-8' and x['every_gross9_sleeve_passed'] and x['advance_to_economic_outcomes']
 ...
 assert max(v['metrics']['one_to_one_6h_max_matched_share'] for v in x['gross9_sleeves'].values())<.177
 assert max(v['metrics']['occupied_5m_bar_jaccard'] for v in x['gross9_sleeves'].values())<.057
 assert max(v['metrics']['absolute_signed_exposure_pearson'] for v in x['gross9_sleeves'].values())<.045
