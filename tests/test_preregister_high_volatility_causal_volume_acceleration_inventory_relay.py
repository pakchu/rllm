...
import copy
import pytest
from training import preregister_high_volatility_causal_volume_acceleration_inventory_relay as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    ...
    assert value["policy_id"]=="HVCVAIR-8" and value["candidate_family"]==["HVCVAIR-8"]
    assert value["policy"]=={"history_cycles":270,"minimum_history_cycles":180,"path_minutes":480,"half_minutes":240,"turnover_acceleration_rank_min":.60,"variation_rank_min":.60,"entry_delay_minutes":5,"hold_hours":8,"leverage":.5,"base_cost_per_notional_side":.0006,"stress_cost_per_notional_side":.0010}
...
def test_causal_turnover_boundary() -> None:
    value=p.build();f=value["features"]
    assert f["oi_expansion"].startswith("strict positive")
    assert f["turnover_acceleration_rank"].startswith("strict-prior") and f["variation_rank"].startswith("strict-prior")
    assert "quote_asset_volume" in f["completed_cycle"] and "no taker-flow sign" in f["eligible"]
...
def test_prior_failures_disclosed_without_repair() -> None:
    ...
    assert b["prior_turnover_backloading_candidate_outcomes_known"] is True and b["same_mechanism_as_prior_turnover_candidates"] is False
    assert b["prior_HVCFAR_source_failure_known"] is True and b["same_mechanism_as_HVCFAR"] is False
