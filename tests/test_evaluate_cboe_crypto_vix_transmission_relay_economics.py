...
def test_strict_accounting_stage_and_controls_are_frozen():
 assert economics.LEVERAGE==.5 and economics.BASE_COST==.0006 and economics.STRESS_COST==.001 and economics.STAGES["final"][2]=="2026-08-01T00:00:00Z";source=Path(economics.__file__).read_text();assert "full calendar including idle time" in source and "global peak, every held favorable then adverse" in source
 assert economics.CONTROLS==("no_crypto_confirmation","bvol_only_confirmation","dvol_only_confirmation","one_session_stale_vix_change","direction_flip")
