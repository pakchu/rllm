...
import copy
import pytest
from training import preregister_high_volatility_causal_perpetual_flow_inventory_relay as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    ...
    assert value["policy_id"]=="HVCPOFR-8" and value["candidate_family"]==["HVCPOFR-8"]
...
def test_causal_perpetual_flow_boundary() -> None:
    ...
    assert "taker_buy_quote" in f["completed_cycle"] and "no spot source" in f["eligible"]
...
def test_prior_failures_disclosed_without_repair() -> None:
    ...
    assert b["prior_perpetual_taker_candidate_outcomes_known"] is True and b["same_mechanism_as_prior_taker_candidates"] is False
    assert b["prior_HVCFOIR_source_failure_known"] is True and b["same_mechanism_as_HVCFOIR"] is False
