...
import copy
import pytest
from training import preregister_high_volatility_causal_funding_breakout_relay as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    ...
    assert value["policy_id"]=="HVCFBR-24"
    assert value["candidate_family"]==["HVCFBR-24"]
...
def test_causal_boundary_and_no_price_direction_condition() -> None:
    ...
    assert value["features"]["breakout"].startswith("last close")
    assert value["mechanism"]["side"].startswith("long above")
