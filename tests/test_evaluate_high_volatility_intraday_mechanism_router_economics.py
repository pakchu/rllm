...
from training import evaluate_high_volatility_intraday_mechanism_router_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVIMR-8"
    ...
    assert evaluator.CLOCK["rows"] == 224
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVIMR-8 predecessor"):
