...
import training.evaluate_rv20_low_participation_shock_reversal_breakeven1_fixed1x as evaluator
...
def test_breakeven_arms_at_bar_close_and_applies_next_bar_only(side):
    market = _market(count=20, high=100.5, low=99.5)
    activation = 101.0 if side > 0 else 99.0
    market.loc[1, "high" if side > 0 else "low"] = activation
    # Crossing entry on the activation bar must not trigger the newly armed stop.
    market.loc[1, "low" if side > 0 else "high"] = 99.0 if side > 0 else 101.0
    # The same entry-price stop is fixed before the following bar and does apply.
    market.loc[2, "open"] = 100.1 if side > 0 else 99.9
    market.loc[2, "low" if side > 0 else "high"] = 99.0 if side > 0 else 101.0
    trade = evaluator.execute_trade(
        market, pd.DataFrame(columns=["date", "funding_rate"]), 0, side,
        _cell(max_hold_hours=1),
    )
    ...
    assert trade.exit_kind == "stop"
    assert trade.exit_position == 2
    assert trade.price_factor == pytest.approx(1.0)


def test_armed_breakeven_stop_is_adverse_first_when_take_touches_same_bar():
    market = _market(count=20, high=100.5, low=99.5)
    market.loc[1, "high"] = 101.0  # arms breakeven for bar 2
    market.loc[2, "open"] = 100.1
    market.loc[2, ["high", "low"]] = [104.0, 99.0]
    trade = evaluator.execute_trade(
        market, pd.DataFrame(columns=["date", "funding_rate"]), 0, 1,
        _cell(max_hold_hours=1),
    )
...
@pytest.mark.parametrize(("side", "gap_open", "expected"), [(1, 99.0, 0.99), (-1, 101.0, 0.99)])
def test_armed_breakeven_gap_stop_exits_at_worse_next_bar_open(
    side, gap_open, expected
):
    market = _market(count=20, high=100.5, low=99.5)
    market.loc[1, "high" if side > 0 else "low"] = 101.0 if side > 0 else 99.0
    market.loc[2, "open"] = gap_open
    trade = evaluator.execute_trade(
        market, pd.DataFrame(columns=["date", "funding_rate"]), 0, side,
        _cell(max_hold_hours=1),
...
def test_frozen_grid_contains_exactly_the_preregistered_fixed_cell():
    ...
    assert grid == (evaluator.Cell(12, 0.6, "both", 0.04, 0.04),)
...
def test_pre2024_selection_is_an_authenticated_input():
    ...
    assert prereg["inputs"]["pre2024_selection"].endswith(
        "rv20_low_participation_shock_reversal_evaluation_2026-08-08.json"
    )
    assert evaluator.INPUT_SHA256["pre2024_selection"] == (
        "8cb0cdd20232885575c70de1e150a706254489289d310052f3372c70ce61d934"
    )
...
def test_selection_gross9_context_is_cropped_to_exact_pre2025_prefix():
    ...
    complete.loc[complete.index[0], "open"] += 5e-11
    dates = pd.DatetimeIndex(gross["date"])
    masks = {
        split: np.asarray(
            (dates >= pd.Timestamp(start)) & (dates < pd.Timestamp(end)),
...
@pytest.mark.parametrize("bad_side", ["gross", "complete"])
def test_gross9_prefix_rejects_nonfinite_ohlc(bad_value, bad_side):
    complete = pd.DataFrame({
        "date": pd.date_range("2024-12-31 23:50", periods=2, freq="5min"),
        "open": [1.0, 1.0], "high": [2.0, 2.0], "low": [0.5, 0.5],
        "close": [1.0, 1.0],
    })
    gross = complete.copy()
    (gross if bad_side == "gross" else complete).loc[0, "open"] = bad_value
    dates = pd.DatetimeIndex(gross["date"])
    masks = {
        split: np.asarray(
            (dates >= pd.Timestamp(start)) & (dates < pd.Timestamp(end)),
    ...
    event = {
        "split": "test2024", "sleeve": "x", "ret": np.zeros(2),
        "adv": np.zeros(2), "fav": np.zeros(2), "low": np.zeros(2),
        "high": np.zeros(2),
    }
    with pytest.raises(RuntimeError, match="authenticated G9CB12 prefix: open"):
        evaluator.extend_gross9_context(gross, masks, [event], complete)
...
@pytest.mark.parametrize("existing_name", ["result.json", "report.md"])
def test_preexisting_output_consumes_no_new_one_shot_marker(tmp_path, existing_name):
    marker = tmp_path / "attempt.json"
    output = tmp_path / "result.json"
    report = tmp_path / "report.md"
    (tmp_path / existing_name).write_text("already exists", encoding="utf-8")
    with pytest.raises(RuntimeError, match="already consumed"):
        evaluator.consume_one_shot(marker, output, report)
