...
import numpy as np
import pandas as pd
from training import build_high_volatility_causal_funding_move_reversal_support as s
...
def test_clock_side_fades_completed_price_move(monkeypatch) -> None:
    monkeypatch.setattr(s,"stage_for",lambda *_:"train")
    panel=pd.DataFrame({"decision_time":pd.to_datetime(["2023-07-01T00:00:00Z","2023-07-01T08:00:00Z"]),"feature_available_time":pd.to_datetime(["2023-07-01T00:00:00Z","2023-07-01T08:00:00Z"]),"eligible":[True,True],"funding_rate":[.001,.002],"completed_return":[.01,-.02],"absolute_level_rank":[.8,.9],"realized_variation":[.1,.1],"variation_rank":[.7,.8]})
