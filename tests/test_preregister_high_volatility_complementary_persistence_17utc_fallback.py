...
from training import preregister_high_volatility_complementary_persistence_17utc_fallback as p
...
def test_priority_router_truth_table_is_exact() -> None:
    assert p.fallback_17utc_side(0, 1, 17) == 1
    assert p.fallback_17utc_side(0, 1, 9) == 0
    assert p.fallback_17utc_side(1, 0, 1) == 1
    assert p.fallback_17utc_side(1, 1, 9) == 1
    assert p.fallback_17utc_side(-1, -1, 17) == -1
    assert p.fallback_17utc_side(1, -1, 17) == 1
    assert p.fallback_17utc_side(-1, 1, 17) == -1
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.fallback_17utc_side(1, True, 17)
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.fallback_17utc_side(2, 1, 17)
...
def test_exact_clock_and_no_tuning_or_hvcav_repair() -> None:
    ...
    assert construction["priority"] == "frozen HVMRSR-8; HVSARR-8 fallback restricted to exact 17 UTC"
