...
from training import preregister_high_volatility_complementary_persistence_concordance as p
...
def test_pair_concordance_truth_table_is_exact() -> None:
    assert p.pair_concordance_side(0, 1) == 0
    assert p.pair_concordance_side(1, 0) == 0
    assert p.pair_concordance_side(1, 1) == 1
    assert p.pair_concordance_side(-1, -1) == -1
    assert p.pair_concordance_side(1, -1) == 0
    assert p.pair_concordance_side(-1, 1) == 0
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.pair_concordance_side(1, True)
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.pair_concordance_side(2, 1)
...
def test_manifest_hash_and_component_pass_scalars_detect_drift(
    ...
    drifted["construction"]["concordance_rule"] = "allow disagreement"
