...
from training import evaluate_high_volatility_causal_oi_range_regime_relay_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVCORR-24"
    ...
    assert evaluator.CLOCK["rows"] == 70
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVCORR-24 predecessor"):
