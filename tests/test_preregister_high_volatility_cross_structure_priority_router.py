...
from training import preregister_high_volatility_cross_structure_priority_router as p
...
def test_manifest_fixes_exact_four_candidate_family() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVCSPR-8"
    assert value["action_order"] == ["CARSC-8", "HVCMMI-8", "HVIABR-8"]
...
def test_all_action_and_hvsof_eligibility_artifacts_are_hash_pinned() -> None:
    value = p.build()
    assert p.ELIGIBILITY_ARTIFACTS is hvsof.ELIGIBILITY_ARTIFACTS
    assert set(value["action_artifacts"]) == set(p.ACTION_ORDER)
    assert set(value["eligibility_artifacts"]) == set(p.ELIGIBILITY_ORDER)
    for artifacts in (
...
def test_prior_outcomes_are_disclosed_but_router_remains_unopened() -> None:
    value = p.build()
    ...
    assert boundary["prior_CARSC_outcomes_known"] is True
    assert boundary["prior_HVCMMI_outcomes_known"] is True
    assert boundary["prior_HVIABR_outcomes_known"] is True
