...
from training import preregister_high_volatility_complementary_persistence_sequential_router as p
...
def test_priority_router_truth_table_is_exact() -> None:
    assert p.priority_router_side(0, 1) == 1
    assert p.priority_router_side(1, 0) == 1
    assert p.priority_router_side(1, 1) == 1
    assert p.priority_router_side(-1, -1) == -1
    assert p.priority_router_side(1, -1) == 1
    assert p.priority_router_side(-1, 1) == -1
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.priority_router_side(1, True)
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.priority_router_side(2, 1)
...
def test_exact_clock_and_no_tuning_or_hvcav_repair() -> None:
    ...
    assert construction["priority"] == "frozen HVMRSR-8 then HVSARR-8"
