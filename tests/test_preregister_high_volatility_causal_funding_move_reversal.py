...
import copy
import pytest
from training import preregister_high_volatility_causal_funding_move_reversal as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    ...
    assert value["policy_id"]=="HVCFMR-8"
    assert value["candidate_family"]==["HVCFMR-8"]
...
def test_causal_boundary_and_no_price_direction_condition() -> None:
    ...
    assert value["mechanism"]["side"].startswith("negative strict sign of log")
