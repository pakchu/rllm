import numpy as np
import pandas as pd
from training import build_confirmation_ladder_settlement_load_relay_support as support
...
def test_primary_and_flip():
 f=pd.DataFrame({"source_valid":[True,True],"eligible_state":[False,True],"late_return":[-.1,-.2],"settlement_load_expansion":[False,True],"late_unanimous":[False,True]});a,s=support.active_and_side(f);assert a.tolist()==[False,True] and s.tolist()==[-1,-1];_,flip=support.active_and_side(f,"direction_flip");assert flip.tolist()==[1,1]

def test_controls_are_distinct():
 f=pd.DataFrame({"source_valid":[True,True],"eligible_state":[False,True],"late_return":[-.2,-.2],"settlement_load_expansion":[True,True],"late_unanimous":[False,True]})
 late,_=support.active_and_side(f,"late_unanimity_only"); load,_=support.active_and_side(f,"settlement_load_only")
 assert late.tolist()==[False,True] and load.tolist()==[True,False]
