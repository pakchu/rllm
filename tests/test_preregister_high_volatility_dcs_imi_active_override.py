...
from training import preregister_high_volatility_dcs_imi_active_override as p
...
def test_manifest_fixes_independent_singleton_and_exact_components() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVDIMIO-8"
    assert value["action_ids"] == ["HVDCS-8", "HVIMI-24"]
    assert value["component_ids"] == ["HVDCS-8", "HVIMI-24"]
    assert value["candidate_family"] == ["HVDIMIO-8"]
...
def test_opposite_side_veto_truth_table_is_exact() -> None:
    assert p.active_state_override_side(0, 1) == 0
    assert p.active_state_override_side(1, 0) == 1
    assert p.active_state_override_side(1, 1) == 1
    assert p.active_state_override_side(-1, -1) == -1
    assert p.active_state_override_side(1, -1) == -1
    assert p.active_state_override_side(-1, 1) == 1
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.active_state_override_side(1, True)
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.active_state_override_side(2, 1)
...
def test_exact_clock_and_no_tuning_or_hvcav_repair() -> None:
    ...
    assert construction["priority"] == (
        "none; HVDCS-8 is the fixed primary and HVIMI-24 is only an opposite-side override"
    )
