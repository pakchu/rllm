...
from training import evaluate_deribit_led_shock_deceleration_economics as economics


def test_exact_offset_funding_posts_to_containing_bar():
    start = pd.Timestamp("2023-07-01T00:00:00Z"); end = start + pd.Timedelta(minutes=10)
    ...
    funding = pd.DataFrame({"date": [start + pd.Timedelta(milliseconds=5), end], "funding_rate": [0.01, 0.50], "mark_price": [100.0, 100.0]})
