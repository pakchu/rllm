...
from training import preregister_high_volatility_complementary_persistence_opposition_override as p
...
def test_manifest_fixes_independent_singleton_and_exact_components() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVCPO-8"
    assert value["action_ids"] == ["HVMRSR-8", "HVSARR-8"]
    assert value["component_ids"] == ["HVMRSR-8", "HVSARR-8"]
    assert value["candidate_family"] == ["HVCPO-8"]
...
def test_opposition_override_truth_table_is_exact() -> None:
    assert p.override_side(0, 1) == 0
    assert p.override_side(1, 0) == 1
    assert p.override_side(1, 1) == 1
    assert p.override_side(-1, -1) == -1
    assert p.override_side(1, -1) == -1
    assert p.override_side(-1, 1) == 1
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.override_side(1, True)
    with pytest.raises(ValueError, match=r"exact -1, 0, or \+1"):
        p.override_side(2, 1)
...
def test_exact_clock_and_no_tuning_or_hvcav_repair() -> None:
    ...
    assert construction["priority"] == (
        "none; HVMRSR-8 defines incidence and HVSARR-8 only overrides exact opposite conflicts"
    )
...
def test_known_prior_evidence_disclosed_while_combination_is_sealed() -> None:
    ...
    assert boundary["prior_HVCPR_pair_incidence_known"] is True
    assert boundary["prior_HVCPR_outcomes_opened"] is False
    assert boundary["prior_HVCPSR_source_incidence_known"] is True
    assert boundary["prior_HVCPSR_gross9_novelty_known"] is True
    assert boundary["prior_HVCPSR_outcomes_opened"] is False
    assert boundary["prior_HVCPV_source_and_gross9_known"] is True
    assert boundary["prior_HVCPV_train_outcomes_known"] is True
...
def test_manifest_hash_and_component_pass_scalars_detect_drift(
    ...
    def drifted_read(path: str) -> dict[str, object]:
        ...
        if path.endswith("_support_2026-08-12.json"):
