...
from training import build_high_volatility_lagged_agreement_veto_support as support
...
def test_exact_opposite_side_veto(monkeypatch) -> None:
    d0, d1, d2 = "2023-07-01T03:00:00Z", "2023-07-01T11:00:00Z", "2023-07-01T19:00:00Z"
    v1, v2 = "2023-07-01T08:00:00Z", "2023-07-01T16:00:00Z"
    first, second = prereg.ACTION_IDS
    clocks = {
        first: _frame({d0: 1, d1: -1, d2: 1}),
        second: _frame({v1: 1, v2: 1}),
    }
    monkeypatch.setattr(support, "_stage", lambda _: "train")
    clock, counts = support.build_clock(clocks)
    assert list(clock["decision_time"]) == [pd.Timestamp(d0), pd.Timestamp(d2)]
