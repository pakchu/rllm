...
import numpy as np
import pandas as pd
from training import build_high_volatility_spot_perpetual_open_interest_contraction_reversal_support as s
...
def test_panel_requires_oi_contraction_agreement_overshoot_and_variation(monkeypatch):
    monkeypatch.setitem(s.POLICY,'minimum_history_cycles',1)
    t=pd.date_range('2023-07-01',periods=5,freq='8h',tz='UTC')
    oi=pd.DataFrame({'decision_time':t,'sum_open_interest':[130.,120.,125.,110.,100.]})
    bars=pd.DataFrame({'decision_time':t,'perpetual_return':[.01,.02,-.03,.04,-.05],'spot_return':[.005,.01,-.02,-.02,-.03],'minute_squared_return':[.001,.002,.003,.004,.005],'source_rows':[480]*5,'distinct_rows':[480]*5,'first_ts':t-pd.Timedelta('8h'),'last_ts':t-pd.Timedelta('1m'),'coherent':[True]*5})
...
def test_clock_reverses_perpetual_direction(monkeypatch):
    monkeypatch.setattr(s,'stage_for',lambda *_:'train');d=pd.to_datetime(['2023-07-01T00:00:00Z','2023-07-01T16:00:00Z'])
    p=pd.DataFrame({'decision_time':d,'feature_available_time':d,'eligible':[True,True],'oi_change':[-.1,-.2],'perpetual_return':[.02,-.03],'spot_return':[.01,-.02],'directional_overshoot':[.01,.01],'realized_variation':[.1,.2],'variation_rank':[.8,.9]})
