...
from training import evaluate_high_volatility_causal_oi_path_persistence_relay_economics as evaluator
...
def test_frozen_singleton_authorization_and_bindings() -> None:
    assert evaluator.POLICY_ID == "HVCOPR-24"
    ...
    assert evaluator.CLOCK["rows"] == 174
...
def test_predecessor_is_enforced_before_any_price_or_funding_source_opens(
    ...
    with pytest.raises(RuntimeError, match="missing HVCOPR-24 predecessor"):
