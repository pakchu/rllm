...
from training import preregister_high_volatility_demarker_weekly_momentum_active_veto as p
...
def test_manifest_fixes_independent_singleton_and_exact_components() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVDEMWMV-24"
    assert value["action_ids"] == ["HVDEM-24", "HVWMR-72"]
    assert value["component_ids"] == ["HVDEM-24", "HVWMR-72"]
    assert value["candidate_family"] == ["HVDEMWMV-24"]
...
def test_exact_clock_and_no_tuning_or_hvcav_repair() -> None:
    ...
    assert construction["priority"] == (
        "none; HVDEM-24 is the fixed primary and HVWMR-72 is only an opposite-side veto"
    )
...
def test_known_prior_evidence_disclosed_while_combination_is_sealed() -> None:
    ...
    assert boundary["component_test_eval_final_outcomes_used"] is True
    assert boundary["archived_component_later_stage_artifacts_exist"] is True
    assert boundary["archived_component_later_stage_outcomes_used_for_selection"] is True
