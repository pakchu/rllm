...
from training import preregister_high_volatility_causal_close_location_open_interest_contraction_reversal as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVCCLOICR-8"
    assert value["candidate_family"] == ["HVCCLOICR-8"]
    assert value["policy"] == {
        "history_cycles": 270,
...
def test_causal_average_ticket_boundary() -> None:
    ...
    assert "directional outer-35-percent completed-range close location" in features["eligible"]
    assert features["close_location_confirmation"].startswith("close_location>0.65")
...
def test_prior_failures_disclosed_without_repair() -> None:
    ...
    assert boundary["prior_HVCCLOIAR_source_novelty_and_train_failure_known"] is True
    assert boundary["same_mechanism_as_HVCCLOIAR"] is False
