...
from training import evaluate_high_volatility_treasury_belly_curvature_relay_gross9_novelty as novelty


def test_hvtbcr_novelty_evaluator_is_outcome_blind_and_frozen():
    source = Path(novelty.__file__).read_text()
    assert novelty.POLICY == "HVTBCR-24"
...
def test_hvtbcr_novelty_binds_predecessors():
