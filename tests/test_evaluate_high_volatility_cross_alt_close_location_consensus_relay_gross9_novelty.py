...
from training import evaluate_high_volatility_cross_alt_close_location_consensus_relay_gross9_novelty as novelty


def test_hvcaclr_novelty_evaluator_is_outcome_blind_and_frozen():
    source = Path(novelty.__file__).read_text()
    assert novelty.POLICY == "HVCACLR-8"
...
def test_hvcaclr_novelty_binds_predecessors():
