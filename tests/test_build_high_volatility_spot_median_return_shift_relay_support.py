import pandas as pd
from training import build_high_volatility_spot_median_return_shift_relay_support as s
def panels(n=183):
...
def test_causal_rank_excludes_current_and_onset_is_strict():
 p,q=panels();x=s.prepare(p,q);assert x["shift_rank"].iloc[:180].isna().all();assert x["eligible"].sum()>=1
