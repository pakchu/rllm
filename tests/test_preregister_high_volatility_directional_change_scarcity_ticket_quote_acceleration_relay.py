...
from training import preregister_high_volatility_directional_change_scarcity_ticket_quote_acceleration_relay as p


def test_hvdcs_atqa_is_outcome_blind_singleton():
    result = p.build()
    assert result["policy_id"] == "HVDCSATQA-8"
    ...
    assert result["research_boundary"]["exact_hvdcs_joint_ticket_quote_acceleration_incidence_or_outcomes_known"] is False
