...
from training import preregister_high_volatility_spot_lead_open_interest_contraction_continuation as p
...
def test_singleton_mechanism_and_fixed_policy() -> None:
    value = p.build()
    p.validate(value)
    assert value["policy_id"] == "HVSPOICC-8"
    assert value["candidate_family"] == ["HVSPOICC-8"]
    assert value["policy"] == {
        "history_cycles": 270,
...
def test_prior_failures_disclosed_without_repair() -> None:
    ...
    assert boundary["exact_spot_lead_OI_contraction_incidence_or_outcomes_known"] is False
